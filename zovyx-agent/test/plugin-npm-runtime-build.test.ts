...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.zuvix).toBe(
        plan.packageJson.zuvix.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.zuvix.optional).toBe(true);
    }
  });
