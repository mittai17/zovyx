...
import { bundledPluginFile, bundledPluginRoot } from "zuvix/plugin-sdk/test-fixtures";
...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @zuvix/zalo, @zuvix/feishu  @zuvix/zalo "),
    ).toEqual(["@zuvix/feishu", "@zuvix/zalo"]);
  });
});
...
describe("collectPublishablePluginPackageErrors", () => {
  ...
  it("flags invalid publishable plugin metadata", () => {
    expect(
      ...
      'package name must start with "@zuvix/"; found "broken".',
      "package.json private must not be true.",
      'package.json type must be "module" so built .js runtime entries load as ESM.',
      `package.json repository.url must be "${ZUVIX_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "<missing>".`,
      'package.json version must match YYYY.M.PATCH, YYYY.M.PATCH-N, YYYY.M.PATCH-alpha.N, or YYYY.M.PATCH-beta.N; found "latest".',
      "zuvix.extensions must contain only non-empty strings.",
      "zuvix.install.npmSpec must be a non-empty string for publishable plugins.",
    ]);
  });
  ...
  it("requires npm install metadata for publishable plugins", () => {
    ...
    ).toEqual(["zuvix.install.npmSpec must be a non-empty string for publishable plugins."]);
  });
  ...
  it("requires the external plugin package compatibility contract for npm publish", () => {
    expect(
      ...
      "zuvix.build.zuvixVersion is required for external code plugin packages.",
    ]);
  });
...
describe("collectPluginReleaseVersionFloorErrors", () => {
  it("blocks selected plugin stable and beta releases below the June 2026 floor", () => {
    expect(
      ...
      '@zuvix/demo@2026.6.4-beta.1: June 2026 stable and beta release trains must use patch 5 or higher because 2026.6.5-beta.1 is already published; found "2026.6.4-beta.1".',
    ]);
  });
...
describe("collectPublishablePluginPackages", () => {
  ...
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "zuvix-plugin-npm-release-");
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@zuvix/demo-plugin",
      },
    ]);
  });
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@zuvix/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @zuvix/missing.");
  });
});
