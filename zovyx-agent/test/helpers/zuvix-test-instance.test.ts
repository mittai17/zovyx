// Zuvix test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it } from "vitest";
import { createZuvixTestInstance, testing } from "./zuvix-test-instance.js";
...
describe("zuvix test instance", () => {
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createZuvixTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".zuvix"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "zuvix.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.ZUVIX_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.ZUVIX_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.ZUVIX_SKIP_CRON).toBe("0");
