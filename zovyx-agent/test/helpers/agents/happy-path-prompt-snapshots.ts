...
import type { Model } from "zuvix/plugin-sdk/llm";
...
import type { ZuvixConfig } from "../../../src/config/types.zuvix.js";
import type {
  AnyAgentTool,
...
import { createZuvixCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/zuvix-happy-path/workspace";
const AGENT_DIR = "/tmp/zuvix-happy-path/agent";
const SESSION_FILE = "/tmp/zuvix-happy-path/session.jsonl";
...
const CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT = [
  "Zuvix loaded these user-editable workspace files for the current turn. Codex loads AGENTS.md natively. TOOLS.md is provided as inherited Codex developer instructions. SOUL.md, IDENTITY.md, and USER.md are provided as turn-scoped collaboration instructions so native Codex subagents do not inherit them. HEARTBEAT.md is handled by heartbeat collaboration-mode guidance. Those files are not repeated here.",
  "",
  "# Project Context",
  "",
...
const CODEX_WORKSPACE_THREAD_DEVELOPER_INSTRUCTIONS = [
  "## Zuvix Workspace Instructions",
  "",
  "Zuvix loaded these workspace instruction files from the active agent workspace. Internalize and follow them accordingly.",
  "",
...
const CODEX_WORKSPACE_TURN_SCOPED_DEVELOPER_INSTRUCTIONS = [
  "## Zuvix Agent Soul",
  "",
  "Zuvix loaded these workspace instruction files from the active agent workspace. They are the canonical definitions of who you are, how you think and work, and the human you work alongside. Internalize and follow them accordingly.",
  "",
...
const dynamicToolsConfig: ZuvixConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
function createScenarios(codexApi: CodexPromptSnapshotApi): PromptScenario[] {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "Zuvix maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "Zuvix",
    ConversationLabel: "Zuvix/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@Zuvix please verify the Codex happy path too.",
      },
    ],
    Body: "@Zuvix can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to Zuvix/Codex.",
      ],
      trigger: "user",
      ctx: telegramDirectCtx,
      prompt: createPrompt(
...
function renderModelBoundPromptLayers(params: {
  ...
  const codexConfigInstructions =
    ...
    typeof params.codexSnapshot.threadStartParams.config.instructions === "string"
      ...
      : "";
  const zuvixDeveloperInstructions = params.codexSnapshot.developerInstructions;
  ...
  return [
    ...
    "",
    "This is the deterministic model-bound layer stack Zuvix can snapshot for the Codex happy path. It uses a pinned Codex `gpt-5.5` prompt fixture generated from Codex's model catalog/cache shape, then adds the Codex permission developer text, Codex thread config instructions when present, Zuvix developer instructions, turn-scoped collaboration-mode instructions when Zuvix provides them, turn input with Zuvix runtime context, and the Zuvix dynamic tool catalog. Codex can still add runtime-owned context such as native workspace `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions inside the Codex runtime.",
    "",
    "### Layer Metadata",
    "",
    markdownFence(
      ...
      stableJson({
        ...
        zuvixRuntime: {
          ...
          workspaceBootstrapContextFrom:
            "extensions/codex app-server turn/start input Zuvix runtime context",
    ...
    markdownFence(
      ...
      stableJson({
        ...
        zuvixDeveloperInstructions: textStats(zuvixDeveloperInstructions),
    ...
    "",
    "### Developer: Zuvix Runtime Instructions",
    "",
    markdownFence("text", zuvixDeveloperInstructions),
    "",
...
function buildCodexZuvixRuntimeContext(): string {
  return [
    "Zuvix runtime context for this turn:",
    "Treat this Zuvix-provided context as supporting project/user reference for the current request.",
    "",
    "## Zuvix Workspace Context",
    "",
    CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT,
  ].join("\n");
}

function prependCodexZuvixRuntimeContext(prompt: string): string {
  return [buildCodexZuvixRuntimeContext(), "", "Current user request:", prompt].join("\n");
}
...
function renderScenarioSnapshot(
  ...
  const codexTurnPromptText = prependCodexZuvixRuntimeContext(scenario.prompt);
  ...
  return [
    ...
    "- This captures the Zuvix-owned Codex app-server inputs and reconstructs the stable Codex model/permission layers from committed Codex prompt fixtures.",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, Zuvix developer instructions, turn input with simulated Zuvix workspace bootstrap runtime context, heartbeat collaboration-mode guidance when applicable, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how Zuvix routes stable profile files into Codex developer instructions, keeps `MEMORY.md` in turn input, and points heartbeat turns at `HEARTBEAT.md` without inlining it. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path Zuvix tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after Zuvix sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
