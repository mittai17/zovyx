...
import { createZuvixTestInstance, type ZuvixTestInstance } from "./zuvix-test-instance.js";
...
export type GatewayInstance = ZuvixTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createZuvixTestInstance({ name });
  try {
