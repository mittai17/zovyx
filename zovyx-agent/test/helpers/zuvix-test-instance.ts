// Zuvix test instance helper spawns isolated Zuvix processes.
...
import {
  ...
  type ZuvixTestState,
  type ZuvixTestStateOptions,
} from "../../src/test-utils/zuvix-test-state.js";
import { sleep } from "../../src/utils.js";

export type ZuvixTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<ZuvixTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type ZuvixTestInstanceCommandResult = {
  code: number | null;
...
export type ZuvixTestInstance = {
  ...
  ) => Promise<ZuvixTestInstanceCommandResult>;
...
export async function createZuvixTestInstance(
  options: ZuvixTestInstanceOptions,
): Promise<ZuvixTestInstance> {
  ...
  const instance: ZuvixTestInstance = {
    name: options.name,
    port,
