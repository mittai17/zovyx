...
const embeddedAgentMocks = getSharedMocks("zuvix.trigger-handling.embedded-agent-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("zuvix.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("zuvix.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("zuvix.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    zuvixHome: process.env.ZUVIX_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("ZUVIX_HOME", snapshot.zuvixHome);
  restoreKey("ZUVIX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.ZUVIX_STATE_DIR = join(home, ".zuvix");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "zuvix-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".zuvix", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): ZuvixConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-7" },
        workspace: join(home, "zuvix"),
