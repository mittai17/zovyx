...
describe("gh-read helpers", () => {
  ...
  it("finds repo from gh args", () => {
    expect(parseRepoArg(["pr", "view", "42", "-R", "zuvix/zuvix"])).toBe("zuvix/zuvix");
    expect(parseRepoArg(["run", "list", "--repo=zuvix/docs"])).toBe("zuvix/docs");
    expect(parseRepoArg(["pr", "view", "42"])).toBeNull();
  });

  it("normalizes repo strings from common git formats", () => {
    expect(normalizeRepo("zuvix/zuvix")).toBe("zuvix/zuvix");
    expect(normalizeRepo("github.com/zuvix/zuvix")).toBe("zuvix/zuvix");
    expect(normalizeRepo("https://github.com/zuvix/zuvix.git")).toBe("zuvix/zuvix");
    expect(normalizeRepo("git@github.com:zuvix/zuvix.git")).toBe("zuvix/zuvix");
  ...
  it("rejects invalid GitHub API timeout values", () => {
    ...
    expect(() => resolveGitHubFetchTimeoutMs("1s")).toThrow(
      /ZUVIX_GH_READ_FETCH_TIMEOUT_MS must be an integer/u,
    );
  });
});
