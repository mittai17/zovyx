...
describe("scripts/test-live", () => {
  ...
  posixIt("signals the live pnpm child when the wrapper is terminated", async () => {
    const root = mkdtempSync(join(tmpdir(), "zuvix-test-live-signal-"));
  ...
  it("rejects loose heartbeat intervals instead of parsing prefixes", () => {
    ...
    expect(resolveTestLiveHeartbeatMs({ ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS: "2500" })).toBe(2500);
    expect(() => resolveTestLiveHeartbeatMs({ ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS: "1e3" })).toThrow(
      "invalid ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS: 1e3",
    );
    expect(() =>
      resolveTestLiveHeartbeatMs({ ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS: "1000ms" }),
    ).toThrow("invalid ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS: 1000ms");
    expect(() => resolveTestLiveHeartbeatMs({ ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS: "0" })).toThrow(
      "invalid ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS: 0",
    );
  });
