...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("zuvix-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.ZUVIX_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.ZUVIX_STATE_DIR = "state";
delete process.env["ZUVIX_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("ZUVIX_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, ZUVIX_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "ZUVIX_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.ZUVIX_HOME = "not code"';
process.env.NOT_ZUVIX = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.ZUVIX_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.ZUVIX_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/test-utils/zuvix-test-state.ts",
    `
...
describe("collectTestEnvMutationReport", () => {
  it("reports active Zuvix env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
      ...
      { file: "src/example.test.ts", key: "ZUVIX_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "ZUVIX_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "ZUVIX_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "ZUVIX_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("Zuvix test env mutation report");
    ...
    expect(rendered).toContain("L2 ZUVIX_STATE_DIR assign process.env");
