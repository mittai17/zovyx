...
function makeTempDir(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-secret-provider-proof-"));
  tempDirs.push(root);
  return root;
}

function writeStallingZuvix(
  root: string,
  options: {
  ...
  const descendantScript = options.gatewayDescendantMarkerPath
    ...
    : "";
  const scriptPath = path.join(root, "fake-zuvix.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
      ...
      "  process.exit(0);",
      "}",
      "console.error(`unexpected fake zuvix args: ${args.join(' ')}`);",
      "process.exit(2);",
      "",
    ].join("\n"),
    { mode: 0o755 },
...
function writeLeakingStartupZuvix(root: string): string {
  const scriptPath = path.join(root, "fake-leaking-zuvix.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeSignaledStartupZuvix(root: string): string {
  const scriptPath = path.join(root, "fake-signaled-zuvix.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeNoisySecretsConfigureZuvix(root: string): string {
  const scriptPath = path.join(root, "fake-noisy-secrets-configure-zuvix.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function runProofHarness(
  ...
  return spawnSync(process.execPath, [harnessPath, proofScriptPath, root, mode], {
    ...
    env: {
      ...process.env,
      ZUVIX_ENTRY: fakeZuvix,
      ZUVIX_SECRET_PROOF_READY_MS: "60",
      ZUVIX_SECRET_PROOF_RPC_MS: "1000",
      ...envOverrides,
    },
    timeout: 5_000,
  });
...
describe("secret provider integration proof harness", () => {
  it("runs pnpm-backed Zuvix commands through the repo pnpm runner", async () => {
    ...
    const command = await proof.resolveZuvixCommand(
      ["gateway", "status"],
      { ...process.env, ZUVIX_SECRET_PROOF_SENTINEL: "1" },
      {
        nodeExecPath: "/opt/node/bin/node",
        npmExecPath: fakePnpm,
        runner: { pnpm: true, baseArgs: ["zuvix"], label: "pnpm zuvix" },
      },
    );

    expect(command.command).toBe("/opt/node/bin/node");
    expect(command.args).toEqual([fakePnpm, "zuvix", "gateway", "status"]);
    expect(command.options.env.ZUVIX_SECRET_PROOF_SENTINEL).toBe("1");
    expect(command.options.shell).toBe(false);
  });

  it("keeps stalled startup health probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeZuvix = writeStallingZuvix(root);
    const result = runProofHarness(root, fakeZuvix, "start");
  ...
  it("fails fast when startup exits by signal", () => {
    const root = makeTempDir();
    const fakeZuvix = writeSignaledStartupZuvix(root);
    const result = runProofHarness(root, fakeZuvix, "start", {
      ZUVIX_SECRET_PROOF_READY_MS: "2000",
    });
  ...
  it("kills a stalled startup gateway before returning a readiness failure", async () => {
    ...
    const result = runProofHarness(root, fakeZuvix, "start", {
      ZUVIX_SECRET_PROOF_TEARDOWN_GRACE_MS: "100",
    });
  ...
  it("bounds captured command output", async () => {
    const previousLimit = process.env.ZUVIX_SECRET_PROOF_OUTPUT_BYTES;
    process.env.ZUVIX_SECRET_PROOF_OUTPUT_BYTES = "1024";
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.ZUVIX_SECRET_PROOF_OUTPUT_BYTES;
      } else {
        process.env.ZUVIX_SECRET_PROOF_OUTPUT_BYTES = previousLimit;
      }
    }
  });
  ...
  it("blocks skipped secret proofs unless local rehearsals explicitly allow skips", async () => {
    const previousAllowSkips = process.env.ZUVIX_SECRET_PROOF_ALLOW_SKIPS;
    const proof = await import(
    ...
    try {
      delete process.env.ZUVIX_SECRET_PROOF_ALLOW_SKIPS;
      expect(proof.collectBlockingProofResults(entries)).toEqual(entries);

      process.env.ZUVIX_SECRET_PROOF_ALLOW_SKIPS = "1";
      expect(proof.collectBlockingProofResults(entries)).toEqual([]);
    } finally {
      if (previousAllowSkips === undefined) {
        delete process.env.ZUVIX_SECRET_PROOF_ALLOW_SKIPS;
      } else {
        process.env.ZUVIX_SECRET_PROOF_ALLOW_SKIPS = previousAllowSkips;
      }
    }
  });
  ...
  it.runIf(process.platform !== "win32")("bounds captured PTY configure output", async () => {
    const root = makeTempDir();
    const fakeZuvix = writeNoisySecretsConfigureZuvix(root);
    const previousLimit = process.env.ZUVIX_SECRET_PROOF_OUTPUT_BYTES;
    const previousEntry = process.env.ZUVIX_ENTRY;
    process.env.ZUVIX_SECRET_PROOF_OUTPUT_BYTES = "128";
    process.env.ZUVIX_ENTRY = fakeZuvix;
    try {
      const proof = await import(
      ...
      if (previousEntry === undefined) {
        delete process.env.ZUVIX_ENTRY;
      } else {
        process.env.ZUVIX_ENTRY = previousEntry;
      }
    }
  });
  ...
  it.each([
    ["ZUVIX_SECRET_PROOF_COMMAND_MS", "150ms"],
    ["ZUVIX_SECRET_PROOF_READY_MS", "0"],
    ["ZUVIX_SECRET_PROOF_OUTPUT_BYTES", "4mb"],
    ["ZUVIX_SECRET_PROOF_RESOLVER_STDIN_BYTES", "4mb"],
  ...
  it("bounds generated resolver stdin before reading the secret store", async () => {
    ...
    const previousLimit = process.env.ZUVIX_SECRET_PROOF_RESOLVER_STDIN_BYTES;
    process.env.ZUVIX_SECRET_PROOF_RESOLVER_STDIN_BYTES = "64";
    ...
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.ZUVIX_SECRET_PROOF_RESOLVER_STDIN_BYTES;
      } else {
        process.env.ZUVIX_SECRET_PROOF_RESOLVER_STDIN_BYTES = previousLimit;
      }
    }
  });
  ...
  it("fails when proof temp cleanup cannot remove the root", async () => {
    ...
    try {
      await expect(
        proof.cleanupEnv("/tmp/zuvix-secret-provider-proof-stuck", {
          attempts: 3,
          retryDelayMs: 1,
        }),
  ...
  it("detects startup secret leaks after the retained output cap", () => {
    const root = makeTempDir();
    const fakeZuvix = writeLeakingStartupZuvix(root);
    const result = runProofHarness(root, fakeZuvix, "startup-fails", {
      ZUVIX_SECRET_PROOF_OUTPUT_BYTES: "128",
    });
  ...
  it("keeps stalled managed status probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeZuvix = writeStallingZuvix(root);
    const result = runProofHarness(root, fakeZuvix, "status");
