...
async function withPluginAssetFixture(run: (rootDir: string) => Promise<void>) {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-assets-"));
  try {
    ...
    fs.writeFileSync(
      path.join(rootDir, "extensions", "canvas", "zuvix.plugin.json"),
  ...
  it("discovers plugin-owned asset scripts by manifest id", async () => {
    await withPluginAssetFixture(async (rootDir) => {
      ...
      expect(hooks).toEqual([
        {
          aliases: ["@zuvix/canvas-plugin", "canvas", "canvas-plugin"],
          command: "node scripts/bundle-a2ui.mjs",
          packageName: "@zuvix/canvas-plugin",
          phase: "build",
