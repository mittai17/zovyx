...
function writeConfig(home: string, channels: Record<string, unknown>): void {
  ...
  fs.writeFileSync(path.join(configDir, "zuvix.json"), JSON.stringify({ channels }));
}
...
function writeAuthProfileStoreSqlite(agentDir: string, store: unknown): void {
  ...
  const db = new DatabaseSync(path.join(agentDir, "zuvix-agent.sqlite"));
  try {
    db.exec(`
...
function runStatusAssert(channel: string, channelsStatus: unknown, statusText: string) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-status-assertions-"));
  try {
...
describe("npm onboard channel agent assertions", () => {
  it("validates OpenAI env refs from the SQLite auth profile store", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-onboard-assertions-"));
    const agentDir = path.join(tempDir, ".zuvix", "agents", "main", "agent");
  ...
  it("validates channel tokens in their canonical config fields", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-channel-assertions-"));
    try {
      writeConfig(tempDir, {
