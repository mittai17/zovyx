...
describe("bench-cli-startup", () => {
  it("fails reports with no measured samples", () => {
    ...
    ).toEqual(["zuvix.mjs version: no measured samples"]);
  });
  ...
  it("fails reports with samples that did not report RSS", () => {
    ...
    ).toEqual(["zuvix.mjs version sample 1: did not report max RSS"]);
  });
  ...
  it("rejects allowed nonzero exits without their expected clean-state output", () => {
    ...
    expect(
      ...
      "zuvix.mjs health sample 1: exited with expected code 1 but output did not match expected clean-state markers (Gateway target:)",
    ]);
  });
  ...
  it("parses config fixture gateway ports strictly from env", () => {
    ...
    expect(
      withEnv({ ZUVIX_GATEWAY_PORT: "45678" }, () =>
    ...
    for (const invalid of ["45678abc", "127.0.0.1:45678abc"]) {
      expect(() =>
        withEnv({ ZUVIX_GATEWAY_PORT: invalid }, () =>
          testing.buildConfigFixture({
            id: "gatewayHealthJson",
            name: "gateway health --json",
            args: ["gateway", "health", "--json"],
      ...
      ).toThrow("ZUVIX_GATEWAY_PORT must be an integer >= 1");
    }
  });
});
