...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "zuvix-codex-media-path-"));
  tempRoots.push(root);
  return root;
}
...
describe("codex media path limits", () => {
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    expect(() =>
      readPositiveIntEnv("ZUVIX_CODEX_MEDIA_PATH_TIMEOUT_SECONDS", 180, {
        ZUVIX_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: "1e3",
      }),
    ).toThrow("invalid ZUVIX_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: 1e3");
    expect(() =>
      readPositiveIntEnv("ZUVIX_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES", 2 * 1024 * 1024, {
        ZUVIX_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES: "64bytes",
      }),
    ).toThrow("invalid ZUVIX_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES: 64bytes");
  });

  it("writes strict positive timeout and port values into generated config", () => {
    const root = makeTempRoot();
    const result = runWriteConfig(root, {
      ZUVIX_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: "240",
      PORT: "19002",
    });
  ...
  it("rejects loose write-config timeout env values", () => {
    ...
    expect(result.stderr).toContain("invalid ZUVIX_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: 1e3");
  });
});
...
describe("codex media path fake app-server", () => {
  it("returns a structured error when request logging fails", async () => {
    ...
    const child: ChildProcessWithoutNullStreams = spawn(process.execPath, [fakeAppServerPath], {
      env: {
        ...process.env,
        ZUVIX_CODEX_MEDIA_PATH_APP_SERVER_LOG: requestLogDirectory,
      },
