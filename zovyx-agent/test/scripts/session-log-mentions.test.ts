...
function makeTempRoot() {
  const root = mkdtempSync(path.join(tmpdir(), "zuvix-session-log-mentions-"));
  tempRoots.push(root);
  return root;
}
...
describe("session log mention scanner", () => {
  ...
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readSessionLogMentionLimits({
        ZUVIX_SESSION_LOG_MENTION_FILE_MAX_BYTES: "1e3",
      }),
    ).toThrow("invalid ZUVIX_SESSION_LOG_MENTION_FILE_MAX_BYTES: 1e3");
    expect(() =>
      readSessionLogMentionLimits({
        ZUVIX_SESSION_LOG_MENTION_TOTAL_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid ZUVIX_SESSION_LOG_MENTION_TOTAL_MAX_BYTES: 1000ms");
  });
});
