  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "zuvix/plugin-sdk/foo";',
          'export type * from "zuvix/plugin-sdk/bar";',
          'export { named } from "zuvix/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "zuvix/plugin-sdk/foo";' },
      { line: 2, text: 'export type * from "zuvix/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "zuvix/plugin-sdk/foo";',
          'export type { Named } from "zuvix/plugin-sdk/foo";',
