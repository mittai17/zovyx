...
const scrubbedEnvKeys = [
  "MOCK_PORT",
  "MOCK_REQUEST_LOG",
  "ZUVIX_CONFIG_RELOAD_LOG_MAX_READ_BYTES",
  "ZUVIX_CONFIG_RELOAD_LOG_PATH",
  "ZUVIX_CONFIG_RELOAD_LOG_TIMEOUT_MS",
  "ZUVIX_MOCK_OPENAI_PORT",
  "RAW_SCHEMA_ERROR",
  "SUCCESS_MARKER",
];
...
describe("e2e mock and config helper numeric limits", () => {
  it("rejects loose mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      ZUVIX_MOCK_OPENAI_PORT: "44080http",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid ZUVIX_MOCK_OPENAI_PORT: 44080http");
  });
  ...
  it("rejects loose config-reload log timeout env values", () => {
    const result = runScript(configReloadAssertPath, {
      ZUVIX_CONFIG_RELOAD_LOG_TIMEOUT_MS: "30000ms",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid ZUVIX_CONFIG_RELOAD_LOG_TIMEOUT_MS: 30000ms");
  });

  it("rejects loose config-reload log read caps", () => {
    const result = runScript(configReloadAssertPath, {
      ZUVIX_CONFIG_RELOAD_LOG_MAX_READ_BYTES: "256kb",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid ZUVIX_CONFIG_RELOAD_LOG_MAX_READ_BYTES: 256kb");
  });

  it("returns a clear error when mock OpenAI cannot append request logs", async () => {
    const requestLogDirectory = await mkdtemp(join(tmpdir(), "zuvix-mock-request-log-"));
    try {
      await withMockServer(
        ...
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            ...
            body: JSON.stringify({ input: "ZUVIX_E2E_OK" }),
          });
  ...
  it("returns a clear error when web-search mock cannot append request logs", async () => {
    const requestLogDirectory = await mkdtemp(join(tmpdir(), "zuvix-web-search-log-"));
    try {
      await withMockServer(
        webSearchMockPath,
        {
          ...
          SUCCESS_MARKER: "ZUVIX_SCHEMA_E2E_OK",
        },
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            method: "POST",
