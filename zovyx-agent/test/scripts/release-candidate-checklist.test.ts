...
describe("release candidate checklist", () => {
  ...
  it("runs Parallels against the exact prepared candidate tarball", () => {
    expect(candidateParallelsArgs(".artifacts/preflight/zuvix.tgz")).toEqual([
      "test:parallels:npm-update",
      "--",
      "--target-tarball",
      ".artifacts/preflight/zuvix.tgz",
      "--json",
    ]);
    expect(
      candidateParallelsShellCommand(
        ".artifacts/preflight/zuvix candidate.tgz",
    ...
    ).toContain("'--target-tarball' '.artifacts/preflight/zuvix candidate.tgz'");
  });
  ...
  it("requires and carries an exact Windows Node tag for stable release candidates", () => {
    ...
    const options = {
      ...
      windowsNodeInstallerDigests: JSON.stringify({
        "ZuvixCompanion-Setup-x64.exe": `sha256:${"a".repeat(64)}`,
        "ZuvixCompanion-Setup-arm64.exe": `sha256:${"b".repeat(64)}`,
      }),
    };

    expect(buildPublishCommand(options)).toContain("'windows_node_tag=v0.6.3'");
    expect(buildPublishCommand(options)).toContain(
      `'windows_node_installer_digests={"ZuvixCompanion-Setup-x64.exe":"sha256:${"a".repeat(64)}","ZuvixCompanion-Setup-arm64.exe":"sha256:${"b".repeat(64)}"}'`,
    );
  });
  ...
  it("validates the stable Windows source release and immutable installer digests", async () => {
    const assets = [
      {
        name: "ZuvixCompanion-Setup-x64.exe",
        digest: `sha256:${"a".repeat(64)}`,
      },
      {
        name: "ZuvixCompanion-Setup-arm64.exe",
    ...
    const fetchImpl = vi.fn(async () => ({
      ...
      json: async () => ({
        ...
        draft: false,
        prerelease: false,
        html_url: "https://github.com/zuvix/zuvix-windows-node/releases/tag/v0.6.3",
        assets,
      }),
    }));
    ...
    await expect(
      ...
      tag: "v0.6.3",
      url: "https://github.com/zuvix/zuvix-windows-node/releases/tag/v0.6.3",
      assets,
    });
  });
  ...
  it.each([
    ...
    [
      { assets: [] },
      "must contain exactly one required asset ZuvixCompanion-Setup-x64.exe; found 0",
    ],
    [
      {
        assets: [
      ...
      "must contain exactly one required asset ZuvixCompanion-Setup-x64.exe; found 2",
    ],
    [
      {
        assets: [
          { name: "ZuvixCompanion-Setup-x64.exe", digest: "" },
          { name: "ZuvixCompanion-Setup-arm64.exe", digest: `sha256:${"b".repeat(64)}` },
        ],
      },
      "asset ZuvixCompanion-Setup-x64.exe is missing its SHA-256 digest",
    ],
  ...
  it("rejects selected plugin publish scope for release candidates", () => {
    ...
    ).toThrow("release candidates publish Zuvix with --plugin-publish-scope all-publishable");
  });

  it("extracts a workflow run id from gh dispatch output", () => {
    expect(
      parseRunIdFromDispatchOutput(
        "https://github.com/zuvix/zuvix/actions/runs/25922042055\n",
      ),
    ).toBe("25922042055");
  });
  ...
  it("falls back to a single compatible artifact from the same run", () => {
    expect(
      resolveArtifactName(
        [{ name: "zuvix-npm-preflight-dba00", expired: false }],
        "zuvix-npm-preflight-v2026.5.16-beta.2",
        "zuvix-npm-preflight-",
      ),
    ).toBe("zuvix-npm-preflight-dba00");
  });
  ...
  it("bounds GitHub API requests with a timeout signal", async () => {
    ...
    await expect(
      githubApi("repos/zuvix/zuvix/actions/runs", {
        fetchImpl,
        timeoutMs: 1234,
        token: "test-token",
      }),
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "https://api.github.com/repos/zuvix/zuvix/actions/runs",
  ...
  it("includes the GitHub API path when a request times out", async () => {
    ...
    await expect(
      githubApi("repos/zuvix/zuvix/actions/runs/123/jobs", {
        fetchImpl,
        timeoutMs: 5,
        token: "test-token",
      }),
    ).rejects.toThrow(
      "GitHub API repos/zuvix/zuvix/actions/runs/123/jobs timed out after 5ms",
    );
  });
});
