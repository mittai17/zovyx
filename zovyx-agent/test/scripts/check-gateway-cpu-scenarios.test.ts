...
describe("gateway CPU scenario guard", () => {
  ...
  it("prebuilds private QA dist before running QA scenarios when it is missing", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        ...
        if (args.includes("zuvix") && args.includes("qa")) {
    ...
    expect(calls[0]?.env).toMatchObject({
      ...
      ZUVIX_CONFIG_PATH: path.join(outputDir, "qa-state-root", "state", "zuvix.json"),
      ZUVIX_ENABLE_PRIVATE_QA_CLI: "1",
      ZUVIX_HOME: path.join(outputDir, "qa-state-root", "home"),
      ZUVIX_RUN_NODE_SKIP_DTS_BUILD: "1",
      ZUVIX_STATE_DIR: path.join(outputDir, "qa-state-root", "state"),
      ZUVIX_TEST_DISABLE_UPDATE_CHECK: "1",
      USERPROFILE: path.join(outputDir, "qa-state-root", "home"),
    });
    expect(calls[0]?.env?.ZUVIX_BUNDLED_PLUGIN_BUILD_IDS).toBeUndefined();
  });
  ...
  it("does not prebuild private QA dist when the required entries already exist", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      env: {
        HOME: "/real/user/home",
        ZUVIX_CONFIG_PATH: "/real/user/.zuvix/zuvix.json",
        ZUVIX_HOME: "/real/user/home",
        ZUVIX_STATE_DIR: "/real/user/.zuvix",
      },
      silent: true,
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        calls.push({ args, env: opts?.env });
