...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and ClawHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual({
      command: "ZUVIX_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@zuvix/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@zuvix/kitchen-sink@beta");
    expect(script).toContain("clawhub:@zuvix/kitchen-sink@latest");
    expect(script).toContain("clawhub:@zuvix/kitchen-sink@beta");
    expect(script).toContain("ZUVIX_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB");
    expect(script).toContain(
      "npm-to-clawhub|clawhub:@zuvix/kitchen-sink@latest|zuvix-kitchen-sink-fixture|clawhub|success|basic||${KITCHEN_SINK_NPM_SPEC}",
    );
    ...
    expect(sweepScript).toContain("ZUVIX_KITCHEN_SINK_PERSONALITY");
  ...
  it("keeps kitchen-sink RPC coverage package-backed and resource-guarded", () => {
    ...
    expect(script).toContain("ZUVIX_ENTRY=/app/zuvix.mjs");
    expect(script).toContain("ZUVIX_KITCHEN_SINK_COMMAND_MAX_RSS_MIB");
    expect(script).toContain("docker_e2e_sample_stats_until_exit");
    expect(script).toContain("scripts/e2e/lib/docker-stats/assert-resource-ceiling.mjs");
    ...
    expect(walkScript).toContain("usesBuiltZuvixEntry(runner)");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    expect(lane).toEqual({
      command: "ZUVIX_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
      e2eImageKind: "functional",
      live: false,
      name: "plugins",
    ...
    expect(assertionsScript).toContain("assertClawHubExternalInstallContract");
    expect(assertionsScript).toContain('node_modules", "zuvix');
    expect(fixtureServer).toContain('"is-number": "7.0.0"');
    expect(fixtureServer).toContain('zuvix: ">=2026.4.11"');
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(manifestEnv).toEqual({
      ZUVIX_CI_CHECKOUT_REVISION: "${{ steps.checkout_ref.outputs.sha }}",
      ZUVIX_CI_DOCS_CHANGED:
      ...
      ZUVIX_CI_EVENT_NAME: "${{ github.event_name }}",
      ZUVIX_CI_REPOSITORY: "${{ github.repository }}",
      ZUVIX_CI_RUN_ANDROID:
        "${{ github.event_name == 'workflow_dispatch' && inputs.include_android && 'true' || steps.changed_scope.outputs.run_android || 'false' }}",
      ZUVIX_CI_RUN_CONTROL_UI_I18N:
    ...
    expect(inspectorRun.env).toEqual({
      ZUVIX_PLUGIN_INSPECTOR_ROOT: ".artifacts/plugin-inspector",
      ZUVIX_PLUGIN_INSPECTOR_VERSION: "0.3.10",
    });
    expect(inspectorRun.run).toContain("extensions/");
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@zuvix/plugin-inspector@${ZUVIX_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    const releaseChecksWorkflow = parse(
      readFileSync(".github/workflows/zuvix-release-checks.yml", "utf8"),
    );
    const fullReleaseWorkflow = readFullReleaseValidationWorkflow();

    expect(releaseChecksWorkflow.concurrency).toEqual({
      group:
        "zuvix-release-checks-${{ inputs.expected_sha || inputs.ref }}-${{ inputs.rerun_group }}",
    ...
    expect(dockerPreflightStep?.run).toContain(
      '--build-arg ZUVIX_EXTENSIONS="diagnostics-otel,codex"',
    );
    expect(
    ...
    expect(runtimeToolCoverage.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          run: expect.stringContaining("pnpm zuvix qa coverage"),
        }),
      ]),
    );
    expect(runtimeToolCoverage.steps).toEqual(
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, ZUVIX_DISCORD_TOKEN)");
