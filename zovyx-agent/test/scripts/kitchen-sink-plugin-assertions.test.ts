...
function runAssertInstalled({
  ...
  const pluginId = "zuvix-kitchen-sink-fixture";
  const home = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-home-"));
  const installPath = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-install-"));
  ...
  const installsPath = path.join(home, ".zuvix", "plugins", "installs.json");
  ...
  try {
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          installPath,
          resolvedSpec: "@zuvix/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          source: "npm",
          spec: "@zuvix/kitchen-sink@latest",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        ...env,
        HOME: home,
        ZUVIX_STATE_DIR: path.join(home, ".zuvix"),
        KITCHEN_SINK_ID: pluginId,
        KITCHEN_SINK_LABEL: label,
        KITCHEN_SINK_SOURCE: "npm",
        KITCHEN_SINK_SPEC: "npm:@zuvix/kitchen-sink@latest",
  ...
  try {
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          ...
          clawhubPackage: "@zuvix/kitchen-sink",
          integrity: "sha256-test",
          installPath,
          resolvedSpec: "clawhub:@zuvix/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          resolvedAt: 1,
          source: "clawhub",
          spec: "clawhub:@zuvix/kitchen-sink@latest",
          version: "1.0.0",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        KITCHEN_SINK_SPEC: "clawhub:@zuvix/kitchen-sink@latest",
...
describe("kitchen-sink plugin assertions", () => {
  it("bounds expected-failure output before matching failure diagnostics", () => {
    const scratchRoot = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-failure-cap-"));
      ...
      const result = spawnSync(
        ...
        [ASSERTIONS_SCRIPT, "expect-failure", outputPath],
        {
          ...
          env: {
            ...
            KITCHEN_SINK_SOURCE: "npm",
            KITCHEN_SINK_SPEC: "npm:@zuvix/kitchen-sink@0.0.0",
          },
        },
      );
  ...
  it("fails kitchen-sink inspect-all diagnostics for the installed plugin", () => {
    const inspectPayload = fullSurfaceInspectPayload("zuvix-kitchen-sink-fixture");
    const result = runAssertInstalled({
      allInspectPayload: [
        {
          ...inspectPayload,
  ...
  it("accepts ClawHub kitchen-sink fixtures with a context engine", () => {
    const result = runAssertClawhubInstalled({
      contextEngineIds: ["zuvix-kitchen-sink-fixture"],
    });
  ...
  it("scans only the configured kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-scan-"));
  ...
  it("bounds irrelevant Zuvix home traversal during log scans", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-scan-"));
    const home = path.join(parent, "home");
    const scratchRoot = path.join(parent, "scratch");
    try {
      mkdirSync(path.join(home, ".zuvix"), { recursive: true });
      mkdirSync(scratchRoot, { recursive: true });
      writeFileSync(path.join(scratchRoot, "scenario.log"), "0 errors\n");
      for (let index = 0; index < 20; index += 1) {
        const dir = path.join(home, ".zuvix", `cache-${index}`);
  ...
  it("allocates an isolated scratch root by default", () => {
    ...
    expect(sweep).toContain('mktemp -d "/tmp/zuvix-kitchen-sink.XXXXXX"');
    ...
    expect(sweep).not.toContain('mktemp -d "/tmp/zuvix-kitchen-sink-clawhub.XXXXXX"');
  });

  it("cleans the default kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-cleanup-"));
...
`,
      ...
      expect(scratchRoot).toContain("/tmp/zuvix-kitchen-sink.");
  ...
  it("preserves successful kitchen-sink CLI command logs for the final scan", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-log-"));
    const scratchRoot = path.join(parent, "scratch");
...
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_zuvix_logged "install/log" plugins install demo
...
`,
  ...
  it("bounds printed kitchen-sink CLI command logs without truncating saved logs", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-log-print-"));
    const scratchRoot = path.join(parent, "scratch");
    const entry = path.join(parent, "entry.mjs");
    try {
      mkdirSync(scratchRoot, { recursive: true });
...
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_zuvix_logged "install/noisy" plugins install demo
...
`,
  ...
  it("includes expected-failure transcripts in the final kitchen-sink log scan", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-failure-log-"));
    const home = path.join(parent, "home");
    const scratchRoot = path.join(parent, "scratch");
    try {
      mkdirSync(home, { recursive: true });
...
export KITCHEN_SINK_SPEC=npm:@zuvix/kitchen-sink@0.0.0
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_expect_failure "install/failure" bash -c 'printf "%s\\n" "npm ERR! No matching version @zuvix/kitchen-sink@0.0.0"; exit 1'
...
`,
  ...
  it("cleans a ClawHub fixture server that times out before readiness", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-clawhub-"));
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export ZUVIX_CLAWHUB_FIXTURE_WAIT_ATTEMPTS=1
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
...
`,
  ...
  it("bounds ClawHub fixture server logs on startup timeout", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-sink-clawhub-log-"));
    const fakeBin = path.join(parent, "bin");
    const scratchRoot = path.join(parent, "scratch");
    const fixtureDir = path.join(scratchRoot, "clawhub-fixture");
    const nodeShim = path.join(fakeBin, "node");
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export ZUVIX_CLAWHUB_FIXTURE_WAIT_ATTEMPTS=5
export ZUVIX_DOCKER_E2E_LOG_PRINT_BYTES=64
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
start_kitchen_sink_clawhub_fixture_server "$FIXTURE_DIR"
