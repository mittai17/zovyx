...
describe("Mantis Telegram Desktop proof workflow", () => {
  ...
  it("releases Telegram Desktop proof leases left by interrupted agents", () => {
    ...
    expect(codexStep.env?.ZUVIX_QA_CREDENTIAL_OWNER_ID).toContain(
      "mantis-telegram-desktop-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    expect(workflowStep("Prepare Codex user").run).toContain("ZUVIX_QA_CREDENTIAL_OWNER_ID");
    ...
    expect(cleanupStep.env?.ZUVIX_QA_CONVEX_SECRET_CI).toContain(
      "secrets.ZUVIX_QA_CONVEX_SECRET_CI",
    );
    expect(cleanupStep.env?.ZUVIX_QA_CONVEX_SITE_URL).toContain(
      "secrets.ZUVIX_QA_CONVEX_SITE_URL",
    );
  ...
  it("uses the Zuvix Mantis mention as the comment trigger", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");
    const liveWorkflow = readFileSync(LIVE_WORKFLOW, "utf8");
    expect(workflow).toContain("@zuvix-mantis");
    expect(workflow).toContain("/zuvix-mantis");
  ...
  it("installs local proof tools before the Codex agent runs", () => {
    ...
    expect(install.run).toContain("/usr/local/bin/zuvix-telegram-user-crabbox-proof");
    ...
    expect(agent.env?.ZUVIX_TELEGRAM_USER_DRIVER_SCRIPT).toBe(
      "${{ github.workspace }}/scripts/e2e/telegram-user-driver.py",
    );
    expect(agent.env?.ZUVIX_TELEGRAM_USER_PROOF_CMD).toBe(
      "/usr/local/bin/zuvix-telegram-user-crabbox-proof",
    );
    expect(agent.env?.ZUVIX_TELEGRAM_USER_CRABBOX_BIN).toBe("/usr/local/bin/crabbox");
    expect(agent.env?.CRABBOX_COORDINATOR).toContain(
      "secrets.CRABBOX_COORDINATOR || secrets.ZUVIX_QA_MANTIS_CRABBOX_COORDINATOR",
    );
    expect(agent.env?.CRABBOX_COORDINATOR_TOKEN).toContain(
      "secrets.CRABBOX_COORDINATOR_TOKEN || secrets.ZUVIX_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN",
    );

    const prepare = workflowStep("Prepare Codex user");
    expect(prepare.run).toContain(
      "ZUVIX_TELEGRAM_USER_CRABBOX_BIN ZUVIX_TELEGRAM_USER_CRABBOX_PROVIDER ZUVIX_TELEGRAM_USER_DRIVER_SCRIPT ZUVIX_TELEGRAM_USER_PROOF_CMD",
    );
    expect(prepare.run).toContain("MANTIS_CANDIDATE_TRUST");

    const prompt = readFileSync(PROMPT, "utf8");
    expect(prompt).toContain("$ZUVIX_TELEGRAM_USER_PROOF_CMD");
