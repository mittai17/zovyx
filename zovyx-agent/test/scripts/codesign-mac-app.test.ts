...
function entitlementTemps(dir: string): string[] {
  return readdirSync(dir).filter((name) => name.startsWith("zuvix-entitlements"));
}
...
describe("codesign-mac-app temp file hygiene", () => {
  ...
  it("does not allocate entitlement temp files for help output", () => {
    const tempRoot = makeTempDir("zuvix-codesign-help-");
  ...
  it("does not allocate entitlement temp files before app validation", () => {
    const tempRoot = makeTempDir("zuvix-codesign-missing-");
  ...
  it("cleans entitlement temp files when signing fails", () => {
    const tempRoot = makeTempDir("zuvix-codesign-fail-");
  ...
  it("passes generated app entitlements to signing commands and cleans them", () => {
    const tempRoot = makeTempDir("zuvix-codesign-success-");
    const app = path.join(tempRoot, "Fake.app");
    ...
    writeFileSync(path.join(app, "Contents", "MacOS", "Zuvix"), "#!/bin/sh\n");
    ...
    expect(signLines[0]).toContain(`${path.join(app, "Contents", "MacOS", "Zuvix")}\t`);
    ...
    for (const line of signLines) {
      ...
      expect(entitlementPath).toContain("zuvix-entitlements");
