...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "Zuvix installation failed with exit code/);
  ...
  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-Zuvix");
  ...
  it("runs Windows command shims from a Windows-local cwd", () => {
    ...
    const zuvixPathBody = extractFunctionBody(source, "Ensure-ZuvixOnPath");
    ...
    expect(zuvixPathBody).toContain('Invoke-NpmCommand -Arguments @("config", "get", "prefix")');
    ...
    expect(mainBody).toContain('Invoke-NpmCommand -Arguments @("uninstall", "-g", "zuvix")');
  ...
  it("rejects Zuvix GitHub source targets for npm installs", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-Zuvix");
    const sourceTargetBody = extractFunctionBody(source, "Test-ZuvixSourcePackageInstallSpec");
    expect(sourceTargetBody).toContain('$normalizedTag -eq "main"');
    expect(sourceTargetBody).toContain("^github:zuvix/zuvix");
    expect(npmInstallBody).toContain("Test-ZuvixSourcePackageInstallSpec -RequestedTag $Tag");
    expect(npmInstallBody).toContain("npm installs do not support Zuvix GitHub source targets");
  ...
  it("preserves caller-relative local tarball install specs before safe-cwd npm calls", () => {
    const resolveSpecBody = extractFunctionBody(source, "Resolve-NpmZuvixInstallSpec");
  ...
  it("falls back to a user-local portable Node.js bootstrap when package managers are absent", () => {
    ...
    const depsRootBody = extractFunctionBody(source, "Get-ZuvixDepsRoot");
    ...
    expect(depsRootBody).toContain("Zuvix\\deps");
  ...
  it("persists user-local portable Git for future git-backed updates", () => {
    ...
    expect(portableGitRootBody).toContain("Get-ZuvixDepsRoot");
  ...
  it("activates the repo-pinned pnpm version for git installs", () => {
    ...
    const ensurePnpmBody = extractFunctionBody(source, "Ensure-Pnpm");
    const gitInstallBody = extractFunctionBody(source, "Install-ZuvixFromGit");
    ...
    expect(mainBody).toContain("$gitInstallResults = @(Install-ZuvixFromGit");
    expect(mainBody).toContain("Test-BooleanSuccessResult -Results $gitInstallResults");
    expect(mainBody).toContain("$npmInstallResults = @(Install-Zuvix)");
    ...
    expect(gitInstallBody).toContain('Write-Host "[!] Zuvix build did not produce $entryPath"');
  ...
  it("launches interactive onboarding outside Main's captured output", () => {
    const interactiveCommandBody = extractFunctionBody(source, "Invoke-InteractiveZuvixCommand");
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(mainBody).toContain("Invoke-InteractiveZuvixCommand onboard");
  });

  runIfPowerShell("fails install when interactive onboarding exits non-zero", () => {
    const tempDir = harness.createTempDir("zuvix-install-ps1-");
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingZuvix { return $false }",
        "function Get-NpmCommandPath { return 'npm.cmd' }",
        "function Install-Zuvix { return $true }",
        "function Ensure-ZuvixOnPath { return $true }",
        "function Add-ToUserPath { param([string]$Path) }",
        "function Get-ZuvixCommandPath { return 'cmd.exe' }",
    ...
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "zuvix onboard failed with exit code 17",
    );
  });
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.stdout).toContain("caught=Zuvix installation failed with exit code 1.");
  ...
  runIfPowerShell("treats noisy Git install false as failure", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "  return $false",
        "}",
        "function Ensure-ZuvixOnPath { throw 'should not continue after failed git install' }",
        "$InstallMethod = 'git'",
        "$GitDir = 'C:\\\\zuvix-test'",
        "$NoOnboard = $true",
        "$result = Main",
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Install-Zuvix { Write-Output 'npm stdout'; return $true }",
        "function Ensure-ZuvixOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-ZuvixCommand { return 'Zuvix test-version' }",
        "$NoOnboard = $true",
        "$result = Main",
