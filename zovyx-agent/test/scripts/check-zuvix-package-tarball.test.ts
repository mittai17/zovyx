// Check Zuvix Package Tarball tests cover check zuvix package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-zuvix-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "zuvix-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "zuvix", version }));
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><zuvix-app></zuvix-app>",
    ...
    const tarball = join(root, "zuvix.tgz");
...
describe("check-zuvix-package-tarball", () => {
  it.runIf(process.platform !== "win32")(
    "removes the extract dir when tar extraction fails",
    () => {
      const root = mkdtempSync(join(tmpdir(), "zuvix-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.ZUVIX_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            ZUVIX_TEST_EXTRACT_DIR_FILE: extractDirFile,
  ...
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("Zuvix package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
