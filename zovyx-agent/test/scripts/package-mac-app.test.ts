...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "zuvix-plistbuddy-"));
  tempDirs.push(dir);
...
function runStopPackagedAppHarness(killZeroStatus: 0 | 1) {
  const root = mkdtempSync(path.join(tmpdir(), "zuvix-package-stop-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "zuvix-package-stop-tools-"));
  tempDirs.push(root, toolsDir);

  const appRoot = path.join(root, "dist", "Zuvix.app");
  const appBinary = path.join(appRoot, "Contents", "MacOS", "Zuvix");
...
function runSwiftCompatibilityHarness(buildConfig: "debug" | "release") {
  const root = mkdtempSync(path.join(tmpdir(), "zuvix-package-swift-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "zuvix-package-swift-tools-"));
  const developerDir = path.join(root, "Xcode.app", "Contents", "Developer");
  const appRoot = path.join(root, "Zuvix.app");
...
describe("package-mac-app plist stamping", () => {
  ...
  it("falls back to corepack pnpm when the pnpm shim is absent", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "zuvix-package-pnpm-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "zuvix-package-pnpm-tools-"));
    ...
    const result = runHelper(`
      ...
      ZUVIX_TEST_LOG=${JSON.stringify(logPath)}
      export ZUVIX_TEST_LOG
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      ${helperBlock}
  ...
  it("runs Sparkle build metadata derivation from the repository root", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "zuvix-package-sparkle-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "zuvix-package-sparkle-tools-"));
    tempDirs.push(tempRoot, toolsDir);
    ...
    const result = runHelper(`
      set -euo pipefail
      ROOT_DIR=${JSON.stringify(tempRoot)}
      ZUVIX_ROOT=${JSON.stringify(tempRoot)}
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      export ZUVIX_ROOT PATH
      cd /tmp
      ${helperBlock}
  ...
  it("does not kill unrelated Zuvix processes during packaging", () => {
    const script = readFileSync(scriptPath, "utf8");
    ...
    expect(script).not.toContain("killall -q Zuvix");
    expect(stopBlock).toContain('local app_binary="$APP_ROOT/Contents/MacOS/Zuvix"');
  ...
  it("fails when the packaged app survives forced shutdown", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("ERROR: Packaged Zuvix bundle did not exit: 123");
  });
  ...
  it("fails closed when required Swift resources are missing", () => {
    const script = readFileSync(scriptPath, "utf8");
    const zuvixKitBlock = script.slice(
      script.indexOf(
        'ZUVIXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ZuvixKit_ZuvixKit.bundle"',
      ),
      script.indexOf('echo "📦 Copying Textual resources"'),
    );

    expect(zuvixKitBlock).toContain("ERROR: ZuvixKit resource bundle not found");
    expect(zuvixKitBlock).toContain("exit 1");
    expect(zuvixKitBlock).not.toContain("WARN:");
    expect(zuvixKitBlock).not.toContain("continuing");
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      ...
      const result = runHelper(`
        ...
        plist_set_string_required ${JSON.stringify(plist)} CFBundleIdentifier 'ai.zuvix.test'
      ...
      expect(result.status).toBe(1);
      expect(result.stdout).toContain("ai.zuvix.test");
