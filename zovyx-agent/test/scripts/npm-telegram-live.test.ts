...
function mkTempRoot() {
  const root = mkdtempSync(path.join(tmpdir(), "zuvix-npm-telegram-live-"));
  tempRoots.push(root);
  return root;
}
...
describe("package Telegram live Docker E2E", () => {
  it("supports npm-specific Convex credential aliases", () => {
    ...
    expect(script).toContain("ZUVIX_NPM_TELEGRAM_CREDENTIAL_SOURCE");
    expect(script).toContain("ZUVIX_NPM_TELEGRAM_CREDENTIAL_ROLE");
    expect(script).toContain('docker_env+=(-e ZUVIX_QA_CREDENTIAL_SOURCE="$credential_source")');
    expect(script).toContain('docker_env+=(-e ZUVIX_QA_CREDENTIAL_ROLE="$credential_role")');
  });
  ...
  it("defaults CI runs to Convex when broker credentials are present", () => {
    ...
    expect(script).toContain("ZUVIX_QA_CONVEX_SECRET_CI");
    expect(script).toContain("ZUVIX_QA_CONVEX_SECRET_MAINTAINER");
  ...
  it("installs the package candidate before forwarding runtime secrets", () => {
    ...
    expect(installRun).not.toContain(
      "running package install without ZUVIX_E2E_NPM_INSTALL_TIMEOUT",
    );
  ...
  it("bounds installed-package hot path Zuvix commands", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");
    ...
    expect(runtimeRun).toContain("source scripts/lib/zuvix-e2e-instance.sh");
    expect(runtimeRun).toContain("zuvix_e2e_run_command zuvix --version");
    expect(runtimeRun).toContain("zuvix_e2e_run_command zuvix onboard");
    ...
    expect(runtimeRun).toContain("zuvix_e2e_run_command zuvix channels add");
    expect(runtimeRun).toContain("zuvix_e2e_run_command zuvix doctor --fix");
    expect(runtimeRun).toContain("zuvix_e2e_run_command zuvix doctor --non-interactive");
    expect(runtimeRun).toContain('zuvix_e2e_print_log "$file"');
    expect(runtimeRun).not.toContain("sed -n '1,220p'");
    expect(runtimeRun).not.toMatch(/^\s*zuvix (onboard|channels add|doctor )/mu);
  });

  it("can install a resolved package tarball instead of a registry spec", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");

    expect(script).toContain("ZUVIX_NPM_TELEGRAM_PACKAGE_TGZ");
    expect(script).toContain("ZUVIX_CURRENT_PACKAGE_TGZ");
    expect(script).toContain('-e ZUVIX_QA_PACKAGE_SOURCE="$package_install_source"');
    expect(script).toContain('-e ZUVIX_QA_PACKAGE_SOURCE_KIND="$package_source_kind"');
    expect(script).toContain("ZUVIX_QA_PACKAGE_SOURCE_SHA");
    expect(script).toContain(
      'package_mount_args=(-v "$resolved_package_tgz:$package_install_source:ro")',
    );
    expect(script).toContain('validate_zuvix_package_spec "$PACKAGE_SPEC"');
    expect(script.indexOf('if [ -n "$resolved_package_tgz" ]; then')).toBeLessThan(
      script.indexOf('validate_zuvix_package_spec "$PACKAGE_SPEC"'),
    );
  });
  ...
  it("keeps live Docker artifacts isolated by default", () => {
    ...
    expect(script).toContain('-e ZUVIX_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR"');
    expect(script).not.toContain(
      'OUTPUT_DIR="${ZUVIX_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-live}"',
    );
  });
  ...
  it("forwards repeated RTT controls to the package Telegram live lane", () => {
    ...
    expect(script).toContain("ZUVIX_NPM_TELEGRAM_RTT_SAMPLES");
    expect(script).toContain("ZUVIX_NPM_TELEGRAM_RTT_TIMEOUT_MS");
    expect(script).toContain("ZUVIX_NPM_TELEGRAM_RTT_MAX_FAILURES");
    expect(script).toContain("ZUVIX_NPM_TELEGRAM_RTT_CHECKS");
  });
  ...
  it("keeps private QA harness imports local while using the installed package dist", () => {
    ...
    expect(script).toContain('ln -sfnT "$zuvix_package_dir/dist" /app/dist');
    expect(script).toContain('cp "$zuvix_package_dir/package.json" /app/package.json');
    expect(script).toContain('-v "$ROOT_DIR/extensions/qa-lab:/app/extensions/qa-lab:ro"');
    expect(script).not.toContain('ln -sfnT /app/extensions "$zuvix_package_dir/extensions"');
    expect(script).toContain("node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs");
    expect(script).toContain("/app/node_modules/zuvix/package.json");
    ...
    expect(gatewayRpcClient).toContain('from "zuvix/plugin-sdk/gateway-runtime"');
    expect(qaRuntimeApi).toContain('from "zuvix/plugin-sdk/gateway-runtime"');
  });
  ...
  it("lets npm-specific credential aliases override shared QA env", () => {
    expect(
      testing.resolveCredentialSource({
        ZUVIX_NPM_TELEGRAM_CREDENTIAL_SOURCE: "convex",
        ZUVIX_QA_CREDENTIAL_SOURCE: "env",
      }),
    ).toBe("convex");
    expect(
      testing.resolveCredentialRole({
        ZUVIX_NPM_TELEGRAM_CREDENTIAL_ROLE: "ci",
        ZUVIX_QA_CREDENTIAL_ROLE: "maintainer",
      }),
    ).toBe("ci");
  });
  ...
  it("maps repeated RTT env onto package Telegram live options", () => {
    expect(
      testing.resolveRttOptions({
        ZUVIX_NPM_TELEGRAM_RTT_SAMPLES: "7",
        ZUVIX_NPM_TELEGRAM_RTT_TIMEOUT_MS: "45000",
        ZUVIX_NPM_TELEGRAM_RTT_MAX_FAILURES: "2",
        ZUVIX_NPM_TELEGRAM_RTT_CHECKS: "telegram-mentioned-message-reply",
      }),
    ).toEqual({
      rttCount: 7,
      rttTimeoutMs: 45_000,
  ...
  it("rejects invalid repeated RTT env", () => {
    expect(() =>
      testing.resolveRttOptions({
        ZUVIX_NPM_TELEGRAM_RTT_SAMPLES: "7samples",
      }),
    ).toThrow("invalid ZUVIX_NPM_TELEGRAM_RTT_SAMPLES: 7samples");
  });
  ...
  it("gates package Telegram status on the summary artifact", async () => {
    ...
    writeFileSync(
      summaryPath,
      JSON.stringify({
        kind: "zuvix.qa.evidence-summary",
        schemaVersion: 2,
    ...
    await expect(
      testing.shouldFailPackageTelegramRun(
        { summaryPath },
        { ZUVIX_NPM_TELEGRAM_ALLOW_FAILURES: "" },
      ),
    ).resolves.toBe(true);
  });
  ...
  it("does not read package Telegram summaries when failures are allowed", async () => {
    await expect(
      testing.shouldFailPackageTelegramRun(
        ...
        { ZUVIX_NPM_TELEGRAM_ALLOW_FAILURES: "1" },
      ),
    ).resolves.toBe(false);
  });
});
