...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/zuvix-pr-maintainer/scripts/github-activity.sh",
);
...
describe("zuvix-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("zuvix/zuvix last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/zuvix\/zuvix\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/zuvix\/zuvix\/commits/g)).toHaveLength(1);
