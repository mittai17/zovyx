...
describe("scripts/run-vitest", () => {
  ...
  it("restores the workspace node_modules link from a hydrated pnpm modules directory", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { PNPM_CONFIG_MODULES_DIR: "/runner/zuvix-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/zuvix-pnpm-node-modules/vitest/package.json",
    ...
    expect(symlinks).toEqual([
      {
        target: "/runner/zuvix-pnpm-node-modules",
        path: "/runner/zuvix-pnpm-node-modules/node_modules",
        type: "junction",
      },
      {
        target: "/runner/zuvix-pnpm-node-modules",
  ...
  it("self-links hydrated pnpm modules when pnpm lowercases the env key", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { npm_config_modules_dir: "/runner/zuvix-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/zuvix-pnpm-node-modules/vitest/package.json",
        ...
        platform: "win32",
        requireResolve: () => "/runner/zuvix-pnpm-node-modules/vitest/package.json",
      }),
    ).toBe("/repo/node_modules/vitest/vitest.mjs");
    expect(symlinks).toEqual([
      {
  ...
  it("keeps tooling-excluded explicit tests on existing routing", () => {
    const argv = ["run", "test/scripts/zuvix-e2e-instance.test.ts"];
    expect(resolveImplicitVitestArgs(argv)).toBe(argv);
  });
  ...
  it("parses the optional no-output timeout env", () => {
    ...
    expect(resolveVitestNoOutputTimeoutMs({ ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500" })).toBe(
      2500,
    );
    expect(
      resolveVitestNoOutputTimeoutMs({ ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS: "0" }),
    ).toBeNull();
  });
  ...
  it("defaults direct non-watch runs to the stall watchdog", () => {
    expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run"])).toEqual({
      PATH: "/usr/bin",
      ZUVIX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
      ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS: "120000",
    });
    ...
    expect(
      resolveRunVitestSpawnEnv({ ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS: "0", PATH: "/usr/bin" }, [
        "run",
      ]),
    ).toEqual({
  ...
  it("uses a longer default stall watchdog for broad e2e and project shard configs", () => {
    ...
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        ZUVIX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS: timeout,
      });
    }
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        ZUVIX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS: extraLongTimeout,
      });
    }
    expect(
      resolveDefaultVitestNoOutputTimeoutMs([
  ...
  it("does not force the stall watchdog into delegated runner environments", () => {
    ...
    expect(
      resolveTestProjectsRunnerEnv({
        ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
        PATH: "/usr/bin",
      }),
    ).toEqual({
      ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
  ...
  posixIt("cleans delegated test-project children when the wrapper is signaled", async () => {
    ...
    const childPidPath = nodePath.join(
      os.tmpdir(),
      `zuvix-run-vitest-delegated-child-${process.pid}-${Date.now()}.pid`,
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  fs.writeFileSync(process.env.ZUVIX_DELEGATED_SIGNAL_CHILD_PID!, String(process.pid));",
        "  await new Promise(() => {});",
        "});",
        "",
      ].join("\n"),
    ...
    const runner = spawn(
      ...
      ["scripts/run-vitest.mjs", fixturePath, "--reporter=verbose"],
      {
        env: {
          ...process.env,
          ZUVIX_DELEGATED_SIGNAL_CHILD_PID: childPidPath,
        },
        stdio: "ignore",
      },
    );
  ...
  it("parses the optional watchdog heartbeat interval", () => {
    expect(
      resolveVitestNoOutputHeartbeatMs({ ZUVIX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "120000" }),
    ).toBe(120000);
    expect(
      resolveVitestNoOutputHeartbeatMs({ ZUVIX_VITEST_NO_OUTPUT_HEARTBEAT_MS: "0" }),
    ).toBeNull();
  });
});
