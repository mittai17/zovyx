...
describe("plugins Docker assertions", () => {
  it("rejects loose ClawHub preflight limits instead of parsing prefixes", () => {
    const timeoutResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@zuvix/kitchen-sink",
        ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
      },
    });
    expect(timeoutResult.status).not.toBe(0);
    expect(timeoutResult.stderr).toContain(
      "invalid ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: 1e3",
    );
    ...
    const bodyLimitResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@zuvix/kitchen-sink",
        ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "1000bytes",
      },
    });
    expect(bodyLimitResult.status).not.toBe(0);
    expect(bodyLimitResult.stderr).toContain(
      "invalid ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: 1000bytes",
    );
  });
  ...
  it("keeps sweep artifact paths aligned with the assertion scratch root", () => {
    ...
    for (const scriptPath of scripts) {
      ...
      const scriptWithoutDefaultScratch = script.replace(
        'mktemp -d "/tmp/zuvix-plugins.XXXXXX"',
        "",
      );
      expect(script).toContain("ZUVIX_PLUGINS_TMP_DIR");
      expect(scriptWithoutDefaultScratch).not.toMatch(
        /\/tmp\/(?:plugins|marketplace|demo-plugin|is-number|zuvix-plugin|zuvix-clawhub)/,
      );
    }
  });

  it("cleans the default plugin sweep scratch root", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-plugin-sweep-cleanup-"));
    const marker = path.join(root, "scratch-path.txt");
    try {
      const result = runPluginsSweepShell(
        `
set -euo pipefail
export ZUVIX_PLUGINS_SWEEP_SOURCE_ONLY=1
source scripts/e2e/lib/plugins/sweep.sh
printf '%s\\n' "$ZUVIX_PLUGINS_TMP_DIR" > "$MARKER"
test -d "$ZUVIX_PLUGINS_TMP_DIR"
cleanup_zuvix_plugins_sweep
test ! -e "$ZUVIX_PLUGINS_TMP_DIR"
`,
        { MARKER: marker },
      );
      ...
      expect(scratchRoot).toContain("/tmp/zuvix-plugins.");
  ...
  it("preserves caller-provided plugin sweep scratch roots", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-plugin-sweep-caller-"));
...
set -euo pipefail
export ZUVIX_PLUGINS_SWEEP_SOURCE_ONLY=1
export ZUVIX_PLUGINS_TMP_DIR="$SCRATCH_ROOT"
source scripts/e2e/lib/plugins/sweep.sh
test -d "$ZUVIX_PLUGINS_TMP_DIR"
cleanup_zuvix_plugins_sweep
test -d "$ZUVIX_PLUGINS_TMP_DIR"
`,
  ...
  it("scans plugin assertion logs without echoing whole files on failure", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-plugin-update-log-"));
    try {
      ...
      const pass = await runAssertionAsync(["plugin-dir-update-skipped"], {
        ZUVIX_PLUGINS_TMP_DIR: passRoot,
      });
      ...
      const fail = await runAssertionAsync(["plugin-dir-update-skipped"], {
        ZUVIX_PLUGINS_TMP_DIR: failRoot,
      });
      ...
      writeFileSync(
        path.join(invalidRoot, "plugins-invalid-zuvix-extensions.log"),
        `zuvix.extensions[1]\n${"x".repeat(256 * 1024)}\nmissing validation tail`,
        "utf8",
      );
      writeJson(path.join(invalidRoot, "plugins-invalid-zuvix-extensions-list.json"), {
        plugins: [],
      });
      const invalid = await runAssertionAsync(["invalid-zuvix-extensions"], {
        HOME: invalidHome,
        ZUVIX_PLUGINS_TMP_DIR: invalidRoot,
      });
  ...
  it("cleans npm fixture registry children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-plugin-npm-fixture-cleanup-"));
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            ZUVIX_TEST_FIXTURE_SERVER_PID: pidPath,
  ...
  it("bounds npm fixture registry logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-plugin-npm-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      mkdirSync(fixtureDir);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            ZUVIX_DOCKER_E2E_LOG_PRINT_BYTES: "80",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
  ...
  it("cleans ClawHub fixture children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-plugin-clawhub-fixture-cleanup-"));
    try {
      const binDir = path.join(root, "bin");
      const cleanupPath = path.join(root, "caller-cleanup");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            ZUVIX_PLUGINS_E2E_LIVE_CLAWHUB: "0",
            ZUVIX_PLUGINS_TMP_DIR: tmpDir,
            ZUVIX_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
  ...
  it("bounds ClawHub fixture server logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-plugin-clawhub-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const tmpDir = path.join(root, "scratch");
      mkdirSync(tmpDir);
  ...
  it("uses the configured scratch root and resolves Windows home-relative install paths", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-plugins-assertions-"));
    ...
    try {
      ...
      writeJson(path.join(home, ".zuvix", "plugins", "installs.json"), {
        installRecords: {
          "demo-plugin-tgz": {
            source: "archive",
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "plugin-tgz"], {
        ...
        env: {
          ...
          HOME: home,
          ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
          ZUVIX_PLUGINS_TMP_DIR: scratchRoot,
        },
      });
  ...
  it("compares local plugin source paths by canonical path", () => {
    ...
    const installPath = path.join(home, ".zuvix", "extensions", "demo-plugin-dir");
  ...
  it("still requires archive managed install directories to be removed", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const installPath = path.join(home, ".zuvix", "extensions", "demo-plugin-tgz");
    mkdirSync(installPath, { recursive: true });
  ...
  it("rejects unreadable config during plugin uninstall proof", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const removedInstallPath = path.join(home, ".zuvix", "extensions", "demo-plugin-tgz");
    ...
    try {
      ...
      writeFileSync(path.join(home, ".zuvix", "zuvix.json"), "{ malformed\n", "utf8");
      ...
      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("failed to read Zuvix config");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });
  ...
  it("times out stalled ClawHub package metadata requests", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        CLAWHUB_PLUGIN_ID: "zuvix-kitchen-sink-fixture",
        CLAWHUB_PLUGIN_SPEC: "clawhub:@zuvix/kitchen-sink",
        ZUVIX_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "25",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight for @zuvix/kitchen-sink timed out after 25ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
  ...
  it("times out stalled ClawHub package metadata bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@zuvix/kitchen-sink",
        ZUVIX_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "75",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight response for @zuvix/kitchen-sink timed out after 75ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
        server.close(() => resolve());
  ...
  it("bounds ClawHub package metadata response bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@zuvix/kitchen-sink",
        ZUVIX_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "16",
        ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1000",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight response for @zuvix/kitchen-sink response body exceeded 16 bytes",
      );
