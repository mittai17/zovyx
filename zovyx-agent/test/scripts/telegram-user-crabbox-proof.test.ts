...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-telegram-proof-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("telegram user Crabbox proof log polling", () => {
  it("starts the local gateway through the repo pnpm runner", () => {
    ...
    const spec = createZuvixGatewaySpawnSpec({
      env: { ...process.env, ZUVIX_TELEGRAM_PROOF_SENTINEL: "1" },
      gatewayPort: 19042,
    ...
    expect(spec.args).toEqual([fakePnpm, "zuvix", "gateway", "--port", "19042"]);
    expect(spec.options.cwd).toBe(root);
    expect(spec.options.env?.ZUVIX_TELEGRAM_PROOF_SENTINEL).toBe("1");
  ...
  it("rejects loose numeric log tail limits instead of parsing prefixes", () => {
    expect(() =>
      readTelegramUserProofLogTailBytes({
        ZUVIX_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1e3",
      }),
    ).toThrow("invalid ZUVIX_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1e3");
    expect(() =>
      readTelegramUserProofLogTailBytes({
        ZUVIX_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1000bytes",
      }),
    ).toThrow("invalid ZUVIX_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1000bytes");
    expect(
      readTelegramUserProofLogTailBytes({
        ZUVIX_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "4096",
      }),
    ).toBe(4096);
  });
