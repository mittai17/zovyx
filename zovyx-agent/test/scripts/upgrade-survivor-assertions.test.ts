...
function writeMigratedSessionState(stateDir: string): void {
  ...
  const db = new DatabaseSync(join(agentDbDir, "zuvix-agent.sqlite"));
  try {
    db.exec(`
...
function assertConfiguredPluginState(params: { installPath?: string } = {}): void {
  const root = mkdtempSync(join(tmpdir(), "zuvix-upgrade-survivor-"));
  try {
    ...
    writeJson(join(stateDir, "plugins", "installs.json"), {
      installRecords: {
        matrix: {
          source: "clawhub",
          spec: "clawhub:@zuvix/matrix",
          installPath: matrixInstallDir,
          clawhubPackage: "@zuvix/matrix",
    ...
    execFileSync(process.execPath, [ASSERTIONS_PATH, "assert-state"], {
      env: {
        ...
        ZUVIX_TEST_WORKSPACE_DIR: workspace,
        ZUVIX_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON: coveragePath,
        ZUVIX_UPGRADE_SURVIVOR_SCENARIO: "configured-plugin-installs",
      },
      stdio: "pipe",
    });
  } finally {
...
describe("upgrade survivor assertions", () => {
  ...
  it("rejects ClawHub npm-pack installs outside the managed extensions root", () => {
    const root = mkdtempSync(join(tmpdir(), "zuvix-upgrade-survivor-outside-"));
    try {
      expect(() =>
