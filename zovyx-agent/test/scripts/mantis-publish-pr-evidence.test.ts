...
describe("scripts/mantis/publish-pr-evidence", () => {
  it("renders a manifest-driven PR comment with inline screenshots and video links", () => {
    ...
    const body = renderEvidenceComment({
      artifactUrl: "https://github.com/zuvix/zuvix/actions/runs/1/artifacts/2",
      manifest,
      marker: "<!-- mantis-discord-status-reactions -->",
      rawBase: "https://qa.zuvix.ai/mantis/discord/pr-1/run-1",
      requestSource: "workflow_dispatch",
      runUrl: "https://github.com/zuvix/zuvix/actions/runs/1",
      treeUrl: "https://qa.zuvix.ai/mantis/discord/pr-1/run-1",
    });
    ...
    expect(body).toContain(
      '<td width="50%" align="center"><img src="https://qa.zuvix.ai/mantis/discord/pr-1/run-1/baseline.png" width="100%"',
    );
    expect(body).toContain(
      "[Baseline change MP4](https://qa.zuvix.ai/mantis/discord/pr-1/run-1/baseline-change.mp4)",
    );
  ...
  it("uploads manifest artifacts to R2-compatible object storage", async () => {
    ...
    const published = await publishArtifactFiles({
      ...
      storageConfig: {
        ...
        publicBaseUrl: "https://qa.zuvix.ai",
        region: "auto",
    ...
    expect(published).toEqual({
      artifactRoot: "mantis/discord/pr-1/run-1",
      rawBase: "https://qa.zuvix.ai/mantis/discord/pr-1/run-1",
      treeUrl: "https://qa.zuvix.ai/mantis/discord/pr-1/run-1/index.json",
    });
    ...
    expect(String(requests[4]?.body)).toContain(
      '"url": "https://qa.zuvix.ai/mantis/discord/pr-1/run-1/baseline.png"',
    );
  });
  ...
  it("allows failure manifests to omit optional visual artifacts", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        scenario: "slack-zuvix-desktop-smoke",
        comparison: {
          candidate: {
    ...
    const body = renderEvidenceComment({
      ...
      rawBase: "https://qa.zuvix.ai/mantis/slack/pr-1/run-1",
      requestSource: "workflow_dispatch",
      runUrl: "https://github.com/zuvix/zuvix/actions/runs/1",
      treeUrl: "https://qa.zuvix.ai/mantis/slack/pr-1/run-1",
    });
  ...
  it("renders a successful no-visual-proof manifest without media tables", () => {
    ...
    const body = renderEvidenceComment({
      ...
      rawBase:
        "https://raw.githubusercontent.com/zuvix/zuvix/qa-artifacts/mantis/telegram-desktop/pr-1/run-1",
      requestSource: "issue_comment",
      runUrl: "https://github.com/zuvix/zuvix/actions/runs/1",
      treeUrl:
        "https://github.com/zuvix/zuvix/tree/qa-artifacts/mantis/telegram-desktop/pr-1/run-1",
    });
  ...
  it("does not publish PR comments for Telegram capture infrastructure failures", () => {
    ...
    const body = renderEvidenceComment({
      manifest,
      marker: "<!-- mantis-telegram-desktop-proof -->",
      rawBase: "https://artifacts.zuvix.ai/mantis/telegram-desktop/pr-1/run-1",
      requestSource: "pull_request_target",
      runUrl: "https://github.com/zuvix/zuvix/actions/runs/1",
      treeUrl: "https://artifacts.zuvix.ai/mantis/telegram-desktop/pr-1/run-1/index.json",
    });
