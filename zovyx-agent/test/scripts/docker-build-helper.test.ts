...
describe("docker build helper", () => {
  it("forces BuildKit for centralized Docker builds", () => {
    ...
    expect(helper).toContain("ZUVIX_DOCKER_BUILD_RETRIES");
    expect(helper).toContain("ZUVIX_DOCKER_BUILD_TIMEOUT");
    expect(helper).toContain('docker_build_run_logged "$label" "$timeout_value" "$log_file"');
    expect(helper).toContain("ZUVIX_DOCKER_BUILD_REQUIRE_TIMEOUT");
  ...
  it("treats Docker registry auth 5xx failures as transient build failures", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-transient-"));
...
`;
  ...
  it("routes standalone Docker smoke runs through the timeout-aware helper", () => {
    ...
    expect(cleanupSmoke).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${ZUVIX_CLEANUP_SMOKE_DOCKER_TIMEOUT:-600s}}"',
    );
    expect(cleanupSmoke).toContain(
  ...
  it("bounds cleanup-smoke failure log output", () => {
    ...
    expect(cleanupRun).toContain("ZUVIX_CLEANUP_SMOKE_LOG_PRINT_BYTES");
    expect(cleanupRun.match(/print_log_tail \/tmp\/zuvix-cleanup-/g)).toHaveLength(3);
    expect(cleanupRun).not.toContain("cat /tmp/zuvix-cleanup-");
  });
  ...
  it("prints in-container Docker client logs through bounded helpers", () => {
    for (const scriptPath of [CODEX_MEDIA_PATH_SCENARIO_PATH, OPENAI_CHAT_TOOLS_SCENARIO_PATH]) {
      ...
      expect(script, scriptPath).toContain("source scripts/lib/zuvix-e2e-instance.sh");
      expect(script, scriptPath).toContain('zuvix_e2e_print_log "$CLIENT_LOG"');
      expect(script, scriptPath).not.toContain('cat "$CLIENT_LOG"');
    }
  });
  ...
  it("runs cleanup smoke on the native ARM platform instead of pulling an amd64 tag", () => {
    ...
    expect(
      runCleanupDefaultPlatform({ ZUVIX_CLEANUP_SMOKE_PLATFORM: "linux/s390x" }, "x86_64"),
    ).toBe("linux/s390x");
  });
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(helper).toContain("ZUVIX_DOCKER_BUILD_ON_MISSING");
    expect(helper).toContain("ZUVIX_TESTBOX");
    ...
    expect(liveBuild).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${ZUVIX_LIVE_DOCKER_PULL_TIMEOUT:-600s}}"',
    );
    expect(liveBuild).toContain(
      'LIVE_IMAGE_PULL_ATTEMPTS="${ZUVIX_LIVE_DOCKER_PULL_ATTEMPTS:-3}"',
    );
    ...
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (ZUVIX_SKIP_DOCKER_BUILD=1)"',
    );
  });

  it("wraps centralized Docker builds with the timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-timeout-"));
...
export ZUVIX_DOCKER_BUILD_TIMEOUT=17s
...
`;
  ...
  it("prints heartbeat progress for long successful centralized Docker builds", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-heartbeat-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export ZUVIX_DOCKER_BUILD_HEARTBEAT_SECONDS=1
...
`;
  ...
  it("stops the tracked build command without retrying when interrupted", async () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-signal-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export ZUVIX_DOCKER_BUILD_RETRIES=3
source "$ROOT_DIR/scripts/lib/docker-build.sh"
docker_build_run e2e-build -t demo-image .
`,
      );
  ...
  it("does not delay fast successful centralized Docker builds until the next heartbeat", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-fast-heartbeat-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export ZUVIX_DOCKER_BUILD_HEARTBEAT_SECONDS=30
...
ROOT_DIR=${shellQuote(rootDir)}
export ROOT_DIR
export ZUVIX_DOCKER_BUILD_HEARTBEAT_SECONDS=08
...
`;
  ...
  it("fails centralized Docker builds fast when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-timeout-required-"));
...
export ZUVIX_DOCKER_BUILD_TIMEOUT=19s
...
`;
  ...
  it("keeps setup-style Docker builds compatible when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-timeout-optional-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export ZUVIX_DOCKER_BUILD_TIMEOUT=23s
...
`;
  ...
  it("keeps reused Docker image probes behind the timeout-aware helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-image-reuse-timeout-"));
...
export ZUVIX_SKIP_DOCKER_BUILD=1
...
grep -q '^image inspect zuvix-reuse-image$' "$TMPDIR/docker-seen"
grep -q '^pull zuvix-reuse-image$' "$TMPDIR/docker-seen"
`;
  ...
  it("fails Docker commands fast when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-timeout-required-"));
...
`;
  ...
  it("uses a Node watchdog for Docker commands when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-node-timeout-"));
...
export PATH="$TMPDIR/bin"
export DOCKER_COMMAND_TIMEOUT=7s
unset ZUVIX_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset ZUVIX_DOCKER_E2E_MEMORY ZUVIX_DOCKER_E2E_CPUS ZUVIX_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("adds default Docker run resource limits without overriding explicit limits", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-resource-limits-"));
...
unset ZUVIX_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset ZUVIX_DOCKER_E2E_MEMORY ZUVIX_DOCKER_E2E_CPUS ZUVIX_DOCKER_E2E_PIDS_LIMIT
export ZUVIX_DOCKER_E2E_AVAILABLE_CPUS=32
...
ZUVIX_DOCKER_E2E_MEMORY=12g ZUVIX_DOCKER_E2E_CPUS=4 ZUVIX_DOCKER_E2E_PIDS_LIMIT=512 docker_e2e_docker_cmd run demo
ZUVIX_DOCKER_E2E_AVAILABLE_CPUS=8 ZUVIX_DOCKER_E2E_MEMORY=12g ZUVIX_DOCKER_E2E_CPUS=16 ZUVIX_DOCKER_E2E_PIDS_LIMIT=512 docker_e2e_docker_cmd run demo
docker_e2e_docker_cmd run --memory 2g --cpus 3 --pids-limit 99 demo
ZUVIX_DOCKER_E2E_DISABLE_RESOURCE_LIMITS=1 docker_e2e_docker_cmd run demo
...
`;
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Docker watchdog children that ignore parent SIG${shellSignal}`, () => {
      const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-node-signal-"));
...
export ZUVIX_DOCKER_TIMEOUT_KILL_GRACE_MS=100
...
`;
  ...
  it("uses plain timeout when kill-after is unsupported", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-plain-timeout-"));
...
`;
  ...
  it("uses gtimeout when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-gtimeout-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export ZUVIX_DOCKER_E2E_RUN_TIMEOUT=13s
export ZUVIX_DOCKER_E2E_AVAILABLE_CPUS=8
unset ZUVIX_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset ZUVIX_DOCKER_E2E_MEMORY ZUVIX_DOCKER_E2E_CPUS ZUVIX_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("keeps package-backed Docker runs bounded without the shared timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-package-timeout-required-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export ZUVIX_DOCKER_E2E_RUN_TIMEOUT=11s
...
`;
  ...
  it("uses gtimeout for package-backed Docker runs without the shared timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-package-gtimeout-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export ZUVIX_DOCKER_E2E_RUN_TIMEOUT=15s
export ZUVIX_DOCKER_E2E_AVAILABLE_CPUS=8
unset ZUVIX_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset ZUVIX_DOCKER_E2E_MEMORY ZUVIX_DOCKER_E2E_CPUS ZUVIX_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("removes functional Docker build package inputs after the build", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-cleanup-"));
...
node() {
  local script="$1"
  shift
  if [[ "$script" != "$ROOT_DIR/scripts/package-zuvix-for-docker.mjs" ]]; then
    command node "$script" "$@"
    return
  fi
...
docker_build_run() {
  ...
  for arg in "$@"; do
    case "$arg" in
      zuvix_package=*)
        build_context="\${arg#zuvix_package=}"
        ;;
    esac
  done

  test -n "$build_context"
  test -f "$build_context/zuvix-current.tgz"
...
leftovers="$(find "$TMPDIR" -maxdepth 1 \\( \\
  -name 'zuvix-docker-e2e-pack.*' \\
  -o -name 'zuvix-docker-e2e-package-context.*' \\
\\) -print)"
if [[ -n "$leftovers" ]]; then
...
`;
  ...
  it("keeps caller-provided functional Docker build packages", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-build-external-package-"));
...
printf fixture >"$external_dir/zuvix-current.tgz"
ZUVIX_CURRENT_PACKAGE_TGZ="$external_dir/zuvix-current.tgz"
export ZUVIX_CURRENT_PACKAGE_TGZ
...
test -f "$TMPDIR/build-context-seen"
test -f "$ZUVIX_CURRENT_PACKAGE_TGZ"
leftovers="$(find "$TMPDIR" -maxdepth 1 -name 'zuvix-docker-e2e-package-context.*' -print)"
if [[ -n "$leftovers" ]]; then
...
`;
  ...
  it("cleans generated package mounts after harness Docker runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-package-mount-cleanup-"));
...
test ! -e "$pack_dir"
test -z "$(find "$TMPDIR" -maxdepth 1 -name 'zuvix-docker-e2e-container.*' -print)"

external_dir="$TMPDIR/external-package"
mkdir -p "$external_dir"
printf fixture >"$external_dir/zuvix-current.tgz"
docker_e2e_package_mount_args "$external_dir/zuvix-current.tgz"
...
test -f "$external_dir/zuvix-current.tgz"
`;
  ...
  it("propagates shared E2E command timeouts into package-backed containers", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-package-timeout-env-"));
...
package="$TMPDIR/zuvix-current.tgz"
printf fixture >"$package"
export ZUVIX_E2E_NPM_INSTALL_TIMEOUT=42s
export ZUVIX_E2E_COMMAND_TIMEOUT=23s
docker_e2e_package_mount_args "$package"
printf "%s\\n" "\${DOCKER_E2E_PACKAGE_ARGS[@]}" >"$TMPDIR/package-args"

grep -qx -- "-e" "$TMPDIR/package-args"
grep -qx -- "ZUVIX_CURRENT_PACKAGE_TGZ=/tmp/zuvix-current.tgz" "$TMPDIR/package-args"
grep -qx -- "ZUVIX_E2E_NPM_INSTALL_TIMEOUT=42s" "$TMPDIR/package-args"
grep -qx -- "ZUVIX_E2E_COMMAND_TIMEOUT=23s" "$TMPDIR/package-args"
`;
  ...
  it("passes plugin lifecycle sampler timeout overrides into Docker", () => {
    ...
    expect(runner).toContain('if [ -n "${ZUVIX_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS:-}" ]; then');
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "ZUVIX_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS=$ZUVIX_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS")',
    );
    expect(runner).toContain(
      'if [ -n "${ZUVIX_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS:-}" ]; then',
    );
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "ZUVIX_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS=$ZUVIX_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS")',
    );
    expect(runner).toContain('if [ -n "${ZUVIX_PLUGIN_LIFECYCLE_MAX_RSS_KB:-}" ]; then');
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "ZUVIX_PLUGIN_LIFECYCLE_MAX_RSS_KB=$ZUVIX_PLUGIN_LIFECYCLE_MAX_RSS_KB")',
    );
    expect(runner).toContain('if [ -n "${ZUVIX_PLUGIN_LIFECYCLE_MAX_WALL_MS:-}" ]; then');
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "ZUVIX_PLUGIN_LIFECYCLE_MAX_WALL_MS=$ZUVIX_PLUGIN_LIFECYCLE_MAX_WALL_MS")',
    );
    expect(runner).toContain('if [ -n "${ZUVIX_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO:-}" ]; then');
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "ZUVIX_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO=$ZUVIX_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO")',
    );
  ...
  it("cleans plugin lifecycle matrix temp roots on exit", () => {
    ...
    expect(sweep).toContain("zuvix_plugins_cleanup_fixture_servers");
    expect(sweep).toContain(
      'resource_dir="$(mktemp -d "/tmp/zuvix-plugin-lifecycle-matrix.XXXXXX")"',
    );
    ...
    expect(sweep).not.toContain('resource_dir="/tmp/zuvix-plugin-lifecycle-matrix"');
  ...
  it("wraps direct Docker E2E npm installs with the shared timeout helper", () => {
    ...
    expect(multiNode).toContain(
      'zuvix_e2e_install_package "$ARTIFACTS/install-a.log" "Zuvix package under node-A prefix" "$NPM_PREFIX_A"',
    );
    expect(updateChannel).toContain(
      'zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(updateChannel).toContain(
      'zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path"',
    );
    expect(updateChannel).toContain("zuvix_e2e_print_log /tmp/zuvix-git-install.log");
    expect(updateChannel).toContain('zuvix_e2e_print_log "$package_install_log"');
    expect(updateChannel).not.toContain("cat /tmp/zuvix-git-install.log");
    expect(updateChannel).not.toContain('cat "$package_install_log"');
    expect(doctorSwitch).toContain(
      'zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(doctorSwitch).toContain(
      'zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz"',
    );
    for (const script of [releaseUpgrade, upgradeSurvivor, pluginCorrupt]) {
      expect(script).toContain(
        'zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g',
      );
    }
  });
  ...
  it("keeps upgrade survivor mutable state off the host-mounted artifact tree", () => {
    ...
    for (const script of [runner, publishedRunner]) {
      expect(script).toContain("zuvix-upgrade-survivor-runtime");
      expect(script).toContain("ZUVIX_UPGRADE_SURVIVOR_TMPDIR");
      expect(script).toContain("ZUVIX_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR");
      expect(script).toContain(
      ...
      expect(script).not.toContain('export TMPDIR="$ZUVIX_UPGRADE_SURVIVOR_ARTIFACT_ROOT/tmp"');
      expect(script).not.toContain('export npm_config_cache="$ARTIFACT_ROOT/npm-cache"');
      expect(script).not.toContain(
        'export npm_config_cache="$ZUVIX_UPGRADE_SURVIVOR_ARTIFACT_ROOT/npm-cache"',
      );
    }
  });

  it("wraps package-backed scenario Zuvix CLI calls with the shared timeout helper", () => {
    const paths = [
    ...
    for (const path of paths) {
      ...
      expect(script, path).toContain("zuvix_e2e_enable_zuvix_cli_timeout");
    }
    expect(readFileSync(RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH, "utf8")).toContain(
      'zuvix_e2e_run_command node "$baseline_entry" onboard',
    );
  });
  ...
  it("keeps append-only mock E2E state under per-run scratch roots", () => {
    const scripts = [
      {
        ...
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/zuvix-release-typed-onboarding.XXXXXX")"',
        ...
        removed: [
          "/tmp/zuvix-release-typed-onboarding-openai.jsonl",
          "/tmp/zuvix-release-typed-onboarding-install.log",
          "/tmp/zuvix-release-typed-onboarding.log",
          "/tmp/zuvix-release-typed-onboarding-openai.log",
          "/tmp/zuvix-release-typed-onboarding-agent.log",
          'mktemp -d "/tmp/zuvix-release-typed-onboarding.XXXXXX"',
        ],
      },
      {
        path: RELEASE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/zuvix-release-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        ...
        removed: [
          "/tmp/zuvix-release-user-journey-openai.jsonl",
          "/tmp/zuvix-release-user-journey-clickclack.json",
          "/tmp/zuvix-release-user-journey-install.log",
          "/tmp/zuvix-release-user-journey-onboard.log",
          "/tmp/zuvix-release-user-journey-agent.log",
          "/tmp/zuvix-release-user-journey-plugin-a-install-path.txt",
          "/tmp/zuvix-release-user-journey-plugin-a-source-path.txt",
          'mktemp -d "/tmp/zuvix-release-journey-plugin-a.XXXXXX"',
          'mktemp -d "/tmp/zuvix-release-journey-plugin-b.XXXXXX"',
        ],
      },
      {
        path: RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/zuvix-release-upgrade-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        extraState: 'CLICKCLACK_STATE="$scenario_tmp/clickclack.json"',
        expectedPaths: [
        ...
        removed: [
          "/tmp/zuvix-release-upgrade-user-journey-openai.jsonl",
          "/tmp/zuvix-release-upgrade-user-journey-clickclack.json",
          "/tmp/zuvix-release-upgrade-baseline-install.log",
          "/tmp/zuvix-release-upgrade-candidate-install.log",
          "/tmp/zuvix-release-upgrade-onboard.log",
          "/tmp/zuvix-release-upgrade-agent.log",
          'mktemp -d "/tmp/zuvix-release-upgrade-plugin.XXXXXX"',
        ],
      },
      {
        ...
        removed: ["/tmp/zuvix-mock-openai-requests.jsonl"],
      },
    ];
    ...
    for (const {
      ...
      expect(script, path).not.toMatch(/\/tmp\/zuvix-release-[\w-]+\.(?:log|json|err|txt)/u);
    }
  });
  ...
  it("keeps multi-node update Docker artifacts isolated by default", () => {
    ...
    expect(multiNode).toContain(
      'RUN_ID="${ZUVIX_MULTI_NODE_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"',
    );
    expect(multiNode).toContain(
      'ARTIFACT_DIR="${ZUVIX_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update/$RUN_ID}"',
    );
    expect(multiNode).toContain('-v "$ARTIFACT_DIR:/tmp/artifacts"');
    expect(multiNode).not.toContain(
      'ARTIFACT_DIR="${ZUVIX_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update}"',
    );
  });

  it("bounds upgrade survivor foreground Zuvix CLI calls", () => {
    const runner = readFileSync(UPGRADE_SURVIVOR_DOCKER_E2E_PATH, "utf8");
    const publishedRunner = readFileSync(UPGRADE_SURVIVOR_RUN_SCRIPT, "utf8");
    ...
    expect(runner).toContain('-e ZUVIX_UPGRADE_SURVIVOR_COMMAND_TIMEOUT="$COMMAND_TIMEOUT"');
    expect(runner).toContain(
      'command_timeout="${ZUVIX_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"',
    );
    expect(runner).toContain(
      'zuvix_e2e_maybe_timeout "$command_timeout" env -u ZUVIX_GATEWAY_TOKEN',
    );
    expect(runner).toContain(
      'zuvix_e2e_maybe_timeout "$command_timeout" zuvix doctor --fix --non-interactive',
    );
    expect(runner).toContain(
      'zuvix_e2e_maybe_timeout "$command_timeout" zuvix config validate',
    );
    expect(runner).toContain(
      'zuvix_e2e_maybe_timeout "$command_timeout" zuvix gateway status',
    );
    expect(runner).toContain(
      'zuvix gateway --port "$PORT" --bind loopback --allow-unconfigured',
    );
    ...
    expect(publishedRunner).toContain(
      'zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" zuvix --version',
    );
    expect(publishedRunner).toContain(
      'zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" zuvix config validate >"$BASELINE_CONFIG_VALIDATE_LOG"',
    );
    expect(publishedRunner).toContain(
      'zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" zuvix',
    );
    expect(publishedRunner).toContain(
      'zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" zuvix',
    );
    expect(publishedRunner).toContain(
      'zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" zuvix doctor --fix --non-interactive',
    );
    ...
    expect(publishedRunner).toContain('zuvix gateway --port "$port" --bind loopback');
    expect(publishedRunner).toContain("start_gateway legacy-ready-log-ok");
    expect(publishedRunner).toContain(
      'zuvix_e2e_wait_gateway_ready "$gateway_pid" "$GATEWAY_LOG" 360 "$port" "${1:-strict}"',
    );
    ...
    expect(updateRestartAuth).toContain('zuvix gateway --port "$port" --bind loopback');
    expect(updateRestartAuth).toContain(
      'zuvix_e2e_wait_gateway_ready "$gateway_pid" "$log_file" 360 "$port"',
    );
  });
  ...
  it("bounds upgrade survivor failure log diagnostics", () => {
    ...
    expect(runner).toContain("zuvix_e2e_print_log /tmp/zuvix-upgrade-survivor-update.err");
    expect(runner).toContain("zuvix_e2e_print_log /tmp/zuvix-upgrade-survivor-update.json");
    expect(runner).toContain("zuvix_e2e_print_log /tmp/zuvix-upgrade-survivor-doctor.log");
    expect(runner).toContain("zuvix_e2e_print_log /tmp/zuvix-upgrade-survivor-status.err");
    expect(runner).toContain("zuvix_e2e_print_log /tmp/zuvix-upgrade-survivor-status.json");
    expect(runner).toContain('zuvix_e2e_print_log "$GATEWAY_LOG"');
    expect(runner).toContain('zuvix_e2e_print_log "$SYSTEMCTL_SHIM_DAEMON_LOG"');
    expect(runner).toContain('zuvix_e2e_print_log "$log_file"');
    expect(runner).not.toContain("cat /tmp/zuvix-upgrade-survivor-update.err");
    expect(runner).not.toContain("cat /tmp/zuvix-upgrade-survivor-update.json");
    expect(runner).not.toContain("cat /tmp/zuvix-upgrade-survivor-doctor.log");
    expect(runner).not.toContain("cat /tmp/zuvix-upgrade-survivor-status.err");
    expect(runner).not.toContain("cat /tmp/zuvix-upgrade-survivor-status.json");
    ...
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$BASELINE_INSTALL_LOG"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$UPDATE_ERR"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$UPDATE_JSON"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$DOCTOR_LOG"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$GATEWAY_LOG"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$STATUS_ERR"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$STATUS_JSON"');
    expect(publishedRunner).toContain('zuvix_e2e_print_log "$log_file"');
  ...
  it("keeps the harness run wrapper available with pre-sourced Docker command helpers", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-package-helper-guard-"));
...
`;
  ...
  it("forwards harness stdin to backgrounded Docker runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-harness-stdin-"));
...
`;
  ...
  it("preserves caller-owned file descriptors around harness runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-harness-fd-"));
    try {
      const rootDir = process.cwd();
...
`;
  ...
  it("bounds Codex npm plugin live assertion file and transcript reads", () => {
    ...
    expect(assertions).toContain("ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES");
    expect(assertions).toContain("ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES");
    expect(assertions).toContain("ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES");
    expect(assertions).toContain("ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES");
    expect(assertions).toContain("ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES");
    ...
    expect(runner).toContain("docker_e2e_print_log /tmp/zuvix-codex-plugin-pack.log");
    expect(runner).not.toContain("cat /tmp/zuvix-codex-plugin-pack.log");
    expect(runner).toContain("tail -n 120 /tmp/zuvix-codex-agent-after-uninstall.err");
    expect(runner).not.toContain("cat /tmp/zuvix-codex-agent-after-uninstall.err");
  });
  ...
  it("threads the live plugin tool output cap into the Docker harness", () => {
    ...
    expect(runner).toContain(
      'AGENT_OUTPUT_MAX_BYTES="${ZUVIX_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES:-1048576}"',
    );
    expect(runner).toContain(
      '-e "ZUVIX_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES=$AGENT_OUTPUT_MAX_BYTES"',
    );
    expect(runner).toContain("ZUVIX_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES");
    expect(runner).toContain('tail -c "$agent_output_dump_bytes" /tmp/zuvix-agent.json');
    const dumpLogsStart = runner.indexOf("zuvix_e2e_dump_logs \\");
    const dumpLogsEnd = runner.indexOf("\n}", dumpLogsStart);
    expect(runner.slice(dumpLogsStart, dumpLogsEnd)).not.toContain("/tmp/zuvix-agent.json");
  });
  ...
  it("prints plugins Docker E2E logs on successful runs", () => {
    ...
    expect(helper).toContain("ZUVIX_DOCKER_E2E_LOG_HEARTBEAT_SECONDS");
    expect(readFileSync("scripts/lib/docker-e2e-logs.sh", "utf8")).toContain(
      "ZUVIX_DOCKER_E2E_LOG_PRINT_BYTES",
    );
  ...
  it("bounds printed Docker E2E logs to the configured tail", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-e2e-log-print-tail-"));
...
TMPDIR=${shellQuote(workDir)}
export ROOT_DIR TMPDIR
export ZUVIX_DOCKER_E2E_LOG_PRINT_BYTES=64
...
`;
  ...
  it("prints heartbeat progress for long successful Docker E2E log captures", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-e2e-log-heartbeat-"));
...
`;
  ...
  it("cleans the heartbeat command when the wrapper is terminated", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-e2e-log-term-cleanup-"));
...
`;
  ...
  it("cleans harness containers when heartbeat-wrapped Docker runs are terminated", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-e2e-harness-term-cleanup-"));
...
`;
  ...
  it("does not delay fast successful Docker E2E log captures until the next heartbeat", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-e2e-log-fast-heartbeat-"));
...
`;
  ...
  it("normalizes zero-padded Docker E2E log heartbeat intervals", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-e2e-log-zero-heartbeat-"));
...
`;
  ...
  it("normalizes zero-padded Docker E2E stats heartbeat intervals", () => {
    const workDir = mkdtempSync(join(tmpdir(), "zuvix-docker-e2e-stats-zero-heartbeat-"));
...
`;
  ...
  it("keeps onboarding Docker E2E resource-guarded", () => {
    ...
    expect(runner).toContain("ZUVIX_ONBOARD_MAX_MEMORY_MIB");
    expect(runner).toContain("ZUVIX_ONBOARD_MAX_CPU_PERCENT");
    expect(runner).toContain(
      'COMMAND_TIMEOUT="${ZUVIX_ONBOARD_COMMAND_TIMEOUT:-${ZUVIX_E2E_COMMAND_TIMEOUT:-300s}}"',
    );
    expect(runner).toContain('-e "ZUVIX_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"');
  ...
  it("cleans resource-sampled Docker E2E temp logs on every exit path", () => {
    for (const { path, label } of [
      ...
      expect(runner, path).toContain('DOCKER_RUN_TIMEOUT="${ZUVIX_');
  ...
  it("keeps Open WebUI Docker E2E resource-guarded", () => {
    ...
    expect(runner).toContain("ZUVIX_OPENWEBUI_MAX_MEMORY_MIB");
    expect(runner).toContain("ZUVIX_OPENWEBUI_MAX_CPU_PERCENT");
  ...
  it("forwards every kitchen-sink RPC runtime env knob into Docker", () => {
    ...
    const consumed = new Set(
      [...walk.matchAll(/\b(?:env|process\.env)\.(ZUVIX_KITCHEN_SINK_[A-Z0-9_]+)/gu)].map(
        (match) => match[1],
      ),
    );
    const forwarded = new Set(
      [...runner.matchAll(/\b(ZUVIX_KITCHEN_SINK_[A-Z0-9_]+)\b/gu)].map((match) => match[1]),
    );
  ...
  it("bounds kitchen-sink plugin CLI commands inside the Docker sweep", () => {
    ...
    expect(runner).toContain(
      'KITCHEN_SINK_CLI_TIMEOUT="${ZUVIX_KITCHEN_SINK_PLUGIN_CLI_TIMEOUT:-${KITCHEN_SINK_CLI_TIMEOUT:-180s}}"',
    );
    ...
    expect(sweep).toContain("run_kitchen_sink_zuvix_logged()");
    expect(sweep).toContain("run_kitchen_sink_zuvix_capture()");
    expect(sweep).toContain(
      'zuvix_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$ZUVIX_ENTRY" "$@" >"$log_file" 2>&1',
    );
    expect(sweep).toContain('local log_file="${KITCHEN_SINK_TMP_DIR}/${safe_label}.log"');
    for (const line of sweep.split("\n")) {
      if (!line.includes('node "$ZUVIX_ENTRY" plugins')) {
        continue;
      }

      expect(line).toContain("zuvix_e2e_maybe_timeout");
    }
  });
  ...
  it("requires TCP readiness for the gateway network runner", () => {
    ...
    expect(runner).toContain("zuvix_e2e_probe_tcp 127.0.0.1 $PORT");
    expect(runner).not.toMatch(/zuvix_e2e_probe_tcp[^\n]*\|\|[^\n]*gateway-net-e2e\.log/u);
  });
  ...
  it("preserves pnpm lookup paths for scheduled Docker child lanes", () => {
    ...
    expect(scheduler).toContain("ZUVIX_DOCKER_ALL_PNPM_COMMAND");
  });
  ...
  it("runs release installer E2E against the npm beta tag", () => {
    ...
    expect(openWebUiRunner).toContain(
      'SMOKE_MODE="${OPENWEBUI_SMOKE_MODE:-${ZUVIX_OPENWEBUI_SMOKE_MODE:-chat}}"',
    );
  ...
  it("times and parallelizes release installer E2E agent turns after gateway startup", () => {
    ...
    expect(runner).toContain(
      'AGENT_TURNS_PARALLEL="${ZUVIX_INSTALL_E2E_AGENT_TURNS_PARALLEL:-1}"',
    );
    ...
    expect(wrapper).toContain("ZUVIX_INSTALL_E2E_AGENT_TURNS_PARALLEL");
    expect(wrapper).toContain("ZUVIX_INSTALL_E2E_AGENT_TOOL_SMOKE");
    expect(wrapper).toContain("ZUVIX_INSTALL_E2E_OPENAI_MODEL");
    expect(wrapper).toContain("ZUVIX_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(wrapper).toContain("ZUVIX_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-300");
    expect(wrapper).toContain("ZUVIX_INSTALL_E2E_PROFILE_FILE");
    expect(wrapper).toContain("ZUVIX_PROFILE_FILE");
    expect(wrapper).toContain("ZUVIX_TESTBOX_PROFILE_FILE");
    ...
    expect(runner).toContain("ZUVIX_INSTALL_E2E_OPENAI_MODEL");
    expect(runner).toContain("ZUVIX_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(runner).toContain(
      'AGENT_TURN_TIMEOUT_SECONDS="${ZUVIX_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-300}"',
    );
  });
  ...
  it("keeps package acceptance plugin coverage offline-capable", () => {
    ...
    expect(scenarios).toContain("ZUVIX_PLUGINS_E2E_CLAWHUB=0");
  });
  ...
  it("fails the multi-node update probe on update or restart regressions", () => {
    ...
    expect(runner).toContain("ZUVIX_NO_RESPAWN=1");
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    expect(scripts.join("\n")).toContain("ZUVIX_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("routes doctor install switch commands through the E2E timeout helper", () => {
    ...
    expect(scenario).toContain(
      'command_timeout="${ZUVIX_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"',
    );
    expect(scenario).toContain(
      'zuvix_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd"',
    );
    expect(scenario).toContain(
      'zuvix_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd"',
    );
    expect(scenario).toContain(
      'zuvix_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force',
    );
    expect(scenario).toContain(
      'zuvix_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes',
    );
  ...
  it("bounds doctor install switch command log diagnostics", () => {
    ...
    expect(scenario).toContain('zuvix_e2e_print_log "$npm_log"');
    expect(scenario).toContain('zuvix_e2e_print_log "$install_log"');
    expect(scenario).toContain('zuvix_e2e_print_log "$doctor_log"');
    expect(scenario).toContain('zuvix_e2e_print_log "$reinstall_log"');
    expect(scenario).toContain('zuvix_e2e_print_log "$env_repair_log"');
    expect(scenario).toContain('zuvix_e2e_print_log "$clear_log"');
  ...
  it("prepares pnpm workspace package fixtures without package dependencies", () => {
    const root = mkdtempSync(join(tmpdir(), "zuvix-update-channel-fixture-"));
    try {
      mkdirSync(join(root, "patches"));
      writeFileSync(
        join(root, "package.json"),
        `${JSON.stringify({ name: "zuvix", version: "2026.5.6", scripts: {} }, null, 2)}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    expect(runner).toContain("ZUVIX_BUNDLED_PLUGIN_SWEEP_TOTAL");
    expect(runner).toContain("ZUVIX_BUNDLED_PLUGIN_SWEEP_INDEX");
    expect(runner).toContain("ZUVIX_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT");
    expect(runner).toContain("ZUVIX_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runner).toContain("ZUVIX_PLUGIN_LIFECYCLE_TRACE");
    ...
    expect(probe).toContain('"zuvix.plugin.json"');
    expect(runtimeSmoke).toContain(
      'readPositiveIntEnv("ZUVIX_BUNDLED_PLUGIN_RUNTIME_READY_MS", 900000)',
    );
    expect(sweep).toContain("read -r plugin_id plugin_dir requires_config");
    expect(sweep).toContain('node "$ZUVIX_ENTRY" plugins install "$plugin_id"');
    expect(sweep).toContain('node "$ZUVIX_ENTRY" plugins uninstall "$plugin_id" --force');
  ...
  it("passes installer tag env to bash, not curl", () => {
    ...
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | ZUVIX_BETA=1 bash');
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | ZUVIX_VERSION="$INSTALL_TAG" bash');
    expect(runner).not.toContain('ZUVIX_BETA=1 curl -fsSL "$INSTALL_URL" | bash');
    expect(runner).not.toContain(
      'ZUVIX_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash',
    );
  });
  ...
  it("bounds installer E2E session transcript tool scans", () => {
    ...
    expect(helper).toContain("ZUVIX_INSTALL_E2E_SESSION_SCAN_BYTES");
    expect(helper).toContain("ZUVIX_INSTALL_E2E_SESSION_LINE_BYTES");
    expect(helper).toContain("ZUVIX_INSTALL_E2E_SESSION_SCAN_DEPTH");
    expect(helper).toContain("ZUVIX_INSTALL_E2E_SESSION_SCAN_NODES");
  ...
  it("cleans OpenAI web search smoke processes through the E2E helpers", () => {
    ...
    expect(scenario).toContain('zuvix_e2e_terminate_gateways "${gateway_pid:-}"');
    expect(scenario).toContain('zuvix_e2e_stop_process "${mock_pid:-}"');
    expect(scenario).toContain(
      'gateway_pid="$(zuvix_e2e_start_gateway "$entry" "$PORT" "$GATEWAY_LOG")"',
    );
    expect(scenario).toContain('zuvix_e2e_wait_mock_openai "$MOCK_PORT"');
    expect(scenario).toContain(
  ...
  it("keeps OpenAI web search smoke logs isolated per run", () => {
    ...
    expect(scenario).toContain(
      'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/zuvix-openai-web-search-minimal.XXXXXX")"',
    );
    ...
    expect(scenario).toContain('zuvix_e2e_print_log "$file"');
    expect(scenario).toContain('rm -rf "$scenario_tmp"');
    expect(scenario).not.toContain("sed -n '1,260p'");
    expect(scenario).not.toContain("/tmp/zuvix-openai-web-search-minimal-requests.jsonl");
    expect(scenario).not.toContain("/tmp/zuvix-openai-web-search-minimal-client-success.log");
    expect(scenario).not.toContain("/tmp/zuvix-openai-web-search-minimal-client-reject.log");
  });
  ...
  it("keeps ClawHub plugin Docker smoke hermetic by default", () => {
    ...
    expect(runner).toContain("ZUVIX_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(sweep).toContain("scripts/e2e/lib/plugins/clawhub.sh");
    expect(clawhub).toContain("start_clawhub_fixture_server()");
    expect(clawhub).toContain('ZUVIX_CLAWHUB_URL="http://127.0.0.1:');
    expect(clawhub).toContain("ZUVIX_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(clawhub).toContain("ZUVIX_PLUGINS_E2E_LIVE_NPM_REGISTRY");
    expect(clawhub).toContain("live ClawHub can rate-limit CI");
    expect(clawhub).toContain('[[ -n "${ZUVIX_CLAWHUB_URL:-}" || -n "${CLAWHUB_URL:-}" ]]');
    expect(clawhub).toContain("Ignoring ambient ClawHub URL for fixture-mode plugin E2E");
    expect(clawhub).toContain("unset ZUVIX_CLAWHUB_URL CLAWHUB_URL");
  });
  ...
  it("keeps the plugin binding command escape Docker smoke focused", () => {
    ...
    expect(runner).toContain(
      'DOCKER_RUN_TIMEOUT="${ZUVIX_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"',
    );
    expect(runner).toContain(
    ...
    expect(dockerfile).toContain("ZUVIX_DISABLE_BUNDLED_PLUGIN_POSTINSTALL=1");
    expect(dockerfile).toContain(
      "pnpm install --frozen-lockfile --ignore-scripts --filter zuvix",
    );
  });
  ...
  it("covers plugin CLI sources in the Docker plugin sweep", () => {
    ...
    expect(sweep).toContain('ZUVIX_PLUGINS_CLI_TIMEOUT="${ZUVIX_PLUGINS_CLI_TIMEOUT:-180s}"');
    expect(sweep).toContain(
      'run_logged "$label" zuvix_e2e_maybe_timeout "$ZUVIX_PLUGINS_CLI_TIMEOUT" node "$ZUVIX_ENTRY" "$@"',
    );
    expect(sweep).toContain("run_plugins_zuvix_capture()");
    expect(sweep).toContain(
      'zuvix_e2e_maybe_timeout "$ZUVIX_PLUGINS_CLI_TIMEOUT" node "$ZUVIX_ENTRY" "$@" >"$output_file"',
    );
    expect(sweep).not.toContain('run_logged install-npm node "$ZUVIX_ENTRY"');
    for (const [path, script] of [
      ...
      const unboundedPluginCliLines = script
        .split("\n")
        .filter((line) => line.includes('node "$ZUVIX_ENTRY" plugins'))
        .filter((line) => !line.includes("zuvix_e2e_maybe_timeout"));
    ...
    expect(sweep).toContain('plugins install "npm:@zuvix/demo-plugin-npm@0.0.1"');
    ...
    expect(clawhub).toContain("run_plugins_zuvix_logged install-clawhub");
    expect(clawhub).toContain('zuvix_e2e_maybe_timeout "$ZUVIX_PLUGINS_CLI_TIMEOUT"');
    expect(clawhub).toContain("clawhub:@zuvix/kitchen-sink");
