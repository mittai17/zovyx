...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "zuvix-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "zuvix-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$ZUVIX_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$ZUVIX_TEST_CONTRIB"',
    ],
    {
      ZUVIX_PR_CHANGELOG_SH: changelogScriptPath,
      ZUVIX_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$ZUVIX_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      ZUVIX_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @zuvix.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @Zuvix.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "zuvix", text: "- Org-owned fix. Thanks @zuvix." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      { line: 4, handle: "zuvix", text: "- Mixed credit. Thanks @contributor and @Zuvix." },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @zuvix and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "zuvix",
        text: "- Mixed credit (#123). Thanks @zuvix and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("zuvix")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("zuvix-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("zuvix-clawsweeper[bot]")).toBe(true);
    ...
    try {
      const output = run(
        ...
        [
          ...
          `
set -euo pipefail
source "$ZUVIX_PR_COMMON_SH"
source "$ZUVIX_PR_CHANGELOG_SH"
source "$ZUVIX_PR_GATES_SH"
...
`,
        ...
        {
          ZUVIX_PR_COMMON_SH: commonScriptPath,
          ZUVIX_PR_CHANGELOG_SH: changelogScriptPath,
          ZUVIX_PR_GATES_SH: gatesScriptPath,
          ZUVIX_TEST_CALLS: callsPath,
        },
      );
