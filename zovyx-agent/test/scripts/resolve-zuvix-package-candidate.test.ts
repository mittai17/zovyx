// Resolve Zuvix Package Candidate tests cover resolve zuvix package candidate script behavior.
...
import {
  ...
  runCommandForTest,
  validateZuvixPackageSpec,
} from "../../scripts/resolve-zuvix-package-candidate.mjs";
...
describe("resolve-zuvix-package-candidate", () => {
  it("accepts only Zuvix release package specs for npm candidates", () => {
    for (const spec of [
      "zuvix@beta",
      "zuvix@alpha",
      "zuvix@latest",
      "zuvix@2026.4.27",
      "zuvix@2026.4.27-1",
      "zuvix@2026.4.27-beta.2",
      "zuvix@2026.4.27-alpha.2",
    ]) {
      expect(validateZuvixPackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateZuvixPackageSpec("@evil/zuvix@1.0.0")).toThrow(
      "package_spec must be zuvix@alpha",
    );
    expect(() => validateZuvixPackageSpec("zuvix@canary")).toThrow(
      "package_spec must be zuvix@alpha",
    );
    expect(() => validateZuvixPackageSpec("zuvix@2026.04.27")).toThrow(
      "package_spec must be zuvix@alpha",
    );
    expect(() => validateZuvixPackageSpec("zuvix@npm:other-package")).toThrow(
      "package_spec must be zuvix@alpha",
    );
    expect(() => validateZuvixPackageSpec("zuvix@file:../other-package.tgz")).toThrow(
      "package_spec must be zuvix@alpha",
    );
  });
  ...
  it("parses optional empty workflow inputs without rejecting the command line", () => {
    expect(
      ...
      outputName: "zuvix-current.tgz",
      packageSha256: "",
      packageRef: "release/2026.4.27",
      packageSpec: "zuvix@beta",
      packageUrl: "",
      source: "npm",
      trustedSourceId: "",
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "zuvix@2026.5.26-beta.1",
      "C:\\zuvix\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack zuvix@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\zuvix\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "zuvix-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "zuvix-package-runner-signal-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-zuvix-package-candidate.mjs"),
    ).href;
    let childPid: number | undefined;
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/zuvix-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/zuvix-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "zuvix-trusted-package-source-"));
    tempDirs.push(dir);
      ...
      JSON.stringify({
        ...
        sources: {
          "enterprise-artifactory": {
            ...
            pathPrefixes: ["/artifactory/zuvix/"],
            ports: [443, 8443],
  ...
  it("rejects unsafe package_url downloads before fetching private targets", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "zuvix-package-download-"));
    tempDirs.push(dir);
    const target = path.join(dir, "zuvix.tgz");

    await expect(
      downloadUrl("http://packages.example/zuvix.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must use https");
    await expect(
      downloadUrl("https://user@packages.example/zuvix.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must not include credentials");
    await expect(
      downloadUrl("https://localhost/zuvix.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "127.0.0.1", family: 4 }]),
      }),
    ).rejects.toThrow(/private\/internal\/special-use/iu);
    await expect(
      downloadUrl("https://packages.example/zuvix.tgz", target, {
        fetchImpl: unexpectedFetch,
  ...
  it("allows private package_url downloads only through an explicit trusted source policy", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/zuvix/zuvix.tgz", target, {
    ...
    expect(requestedUrls).toEqual([
      "https://packages.internal:8443/artifactory/zuvix/zuvix.tgz",
    ]);
    await expect(readFile(target)).resolves.toEqual(Buffer.from([4, 5, 6]));

    await expect(
      downloadUrl("https://evil.internal:8443/artifactory/zuvix/zuvix.tgz", target, {
        fetchImpl: unexpectedFetch,
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/other/zuvix.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("keeps trusted package_url redirects inside the named source policy", async () => {
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/zuvix/zuvix.tgz", target, {
        fetchImpl: async () =>
          new Response(null, {
            headers: { location: "https://metadata.internal:8443/artifactory/zuvix/pwn.tgz" },
            status: 302,
          }),
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/zuvix.tgz"]);
  });
  ...
  it("bounds package_url downloads and writes completed files atomically", async () => {
    ...
    await downloadUrl("https://packages.example/zuvix.tgz", target, {
      fetchImpl: async () =>
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "zuvix-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "zuvix-package-build-info-"));
    tempDirs.push(dir);
    const root = path.join(dir, "package");
    await mkdir(path.join(root, "dist"), { recursive: true });
    await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "zuvix" }));
    await writeFile(
      path.join(root, "dist", "build-info.json"),
      JSON.stringify({ commit: "66CE632B9B7C5C7FDD3E66C739687D51638AD6E2" }),
    );
    const tarball = path.join(dir, "zuvix.tgz");
