...
describe("scripts/changed-lanes", () => {
  it("keeps a non-executed changed-gate warning fixture", () => {
    // zuvix-temp-dir: allow test fixture for the temp warning report
    const warningFixture = 'fs.mkdtemp("zuvix-warning-fixture-", () => {})';
  ...
  it("prints changed check help without running the changed gate", () => {
    const result = spawnSync(process.execPath, ["scripts/check-changed.mjs", "--help"], {
      cwd: repoRoot,
      encoding: "utf8",
      env: { ...createNestedGitEnv(), ZUVIX_TESTBOX: "1" },
    });
  ...
  it("includes untracked worktree files in the default local diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-changed-lanes-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
  ...
  it("falls back to a two-dot diff when a delegated checkout has no merge base", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-changed-lanes-no-merge-base-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(path.join(dir, "README.md"), "initial\n", "utf8");
    git(dir, ["add", "README.md"]);
    git(dir, [
  ...
  it("prefers raw sync worktree paths over an implausibly broad no-merge-base diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-changed-lanes-raw-sync-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    ...
    const previousRawSync = process.env.ZUVIX_CHANGED_LANES_RAW_SYNC;
    delete process.env.ZUVIX_CHANGED_LANES_RAW_SYNC;
    try {
      ...
      process.env.ZUVIX_CHANGED_LANES_RAW_SYNC = "1";
      expect(listChangedPathsFromGit({ base: "origin/main", cwd: dir })).toEqual([
        "src/feature.ts",
      ]);
    } finally {
      if (previousRawSync === undefined) {
        ...
        process.env.ZUVIX_CHANGED_LANES_RAW_SYNC = previousRawSync;
      }
    }
  });

  it("uses the merge commit first parent instead of a stale PR payload base", () => {
    const { dir, staleBase } = createSyntheticMergeRepo("zuvix-changed-lanes-merge-");
  ...
  it("ignores local Crabbox metadata in the default local diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-changed-lanes-crabbox-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
  ...
  it("includes deleted worktree files in the default local diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-changed-lanes-deleted-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
  ...
  it("includes deleted staged files in the staged diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-changed-lanes-staged-deleted-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    mkdirSync(path.join(dir, "src", "shared"), { recursive: true });
    writeFileSync(
      path.join(dir, "src", "shared", "obsolete.ts"),
  ...
  it("routes core production changes to core prod and core test lanes", () => {
    ...
    expect(plan.commands.find((command) => command.args[0] === "tsgo:core")?.env).toEqual({
      PATH: "/usr/bin",
      ZUVIX_OXLINT_SKIP_LOCK: "1",
      ZUVIX_TEST_HEAVY_CHECK_LOCK_HELD: "1",
      ZUVIX_TSGO_HEAVY_CHECK_LOCK_HELD: "1",
      ZUVIX_TSGO_SPARSE_SKIP: "1",
    });
  ...
  it("reenables local-check policy for changed typecheck commands", () => {
    ...
    const plan = createChangedCheckPlan(result, {
      env: { ZUVIX_LOCAL_CHECK: "0", PATH: "/usr/bin" },
    });
  ...
  it("cleans CI Corepack pnpm shim temp dirs", () => {
    ...
    expect(path.basename(shimDir)).toMatch(/^zuvix-corepack-pnpm-/u);
  ...
  it("delegates staged changed gates as explicit remote paths", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-check-changed-staged-delegate-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(path.join(dir, "README.md"), "initial\n", "utf8");
    git(dir, ["add", "README.md"]);
    git(dir, [
  ...
  it("delegates empty staged changed gates without rediscovering unstaged paths", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-check-changed-empty-staged-delegate-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(path.join(dir, "README.md"), "initial\n", "utf8");
    git(dir, ["add", "README.md"]);
    git(dir, [
  ...
  it("does not delegate dry-run, CI, or remote-child changed gates", () => {
    ...
    expect(
      shouldDelegateChangedCheckToCrabbox([], { ZUVIX_CHECK_CHANGED_REMOTE_CHILD: "1" }),
    ).toBe(false);
  });
  ...
  it("routes live Docker ACP tooling changes through a focused gate", () => {
    ...
    expect(schedulerDryRun?.env?.ZUVIX_DOCKER_ALL_DRY_RUN).toBe("1");
    expect(schedulerDryRun?.env?.ZUVIX_DOCKER_ALL_LIVE_MODE).toBe("only");
  });
    ...
    const after = `${JSON.stringify(
      {
        ...
        scripts: {
          ...
          "test:docker:live-acp-bind:droid":
            "ZUVIX_LIVE_ACP_BIND_AGENT=droid bash scripts/test-live-acp-bind-docker.sh",
        },
        dependencies: {
          leftpad: "1.0.0",
        },
  ...
  it("classifies live Docker package script changes from the git diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-live-docker-package-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(
  ...
  it("classifies normal package script changes from the git diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-package-scripts-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(
      path.join(dir, "package.json"),
      `${JSON.stringify(
  ...
  it("keeps release metadata commits off the full changed gate", () => {
    const result = detectChangedLanes([
      ...
      "apps/macos/Sources/Zuvix/Resources/Info.plist",
  ...
  it("guards release metadata package changes to the top-level version field", () => {
    const dir = makeTempRepoRoot(tempDirs, "zuvix-release-metadata-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(
      path.join(dir, "package.json"),
  ...
  it("keeps app lint explicit when non-macOS hosts lack SwiftLint", () => {
    const result = detectChangedLanes([
      "apps/shared/ZuvixKit/Sources/ZuvixProtocol/GatewayModels.swift",
    ]);
    const plan = createChangedCheckPlan(result, {
      env: { PATH: "/usr/bin" },
      platform: "linux",
