...
describe("kitchen-sink RPC isolated state", () => {
  it("prints help without creating temp state or installing the plugin", async () => {
    ...
    expect(result.stdout).toContain("ZUVIX_KITCHEN_SINK_NPM_SPEC");
  ...
  it("prints help before parsing malformed runtime guardrails", async () => {
    const result = await runCommand(
      ...
      ["scripts/e2e/kitchen-sink-rpc-walk.mjs", "--help"],
      {
        env: {
          ...process.env,
          ZUVIX_KITCHEN_SINK_MAX_RSS_MIB: "1e3",
        },
      },
    );
  ...
  it("rejects loose numeric env values before they bypass runtime guardrails", () => {
    ...
    expect(readPositiveInt(" 1000 ", 60_000)).toBe(1000);
    expect(() => readPositiveInt("1e3", 60_000, "ZUVIX_KITCHEN_SINK_MAX_RSS_MIB")).toThrow(
      'ZUVIX_KITCHEN_SINK_MAX_RSS_MIB must be a positive integer. Got: "1e3"',
    );
    expect(() => readPositiveInt("1000ms", 60_000, "ZUVIX_KITCHEN_SINK_RPC_READY_MS")).toThrow(
      'ZUVIX_KITCHEN_SINK_RPC_READY_MS must be a positive integer. Got: "1000ms"',
    );
    expect(() => readPositiveInt("0", 60_000, "ZUVIX_KITCHEN_SINK_RPC_PORT")).toThrow(
      'ZUVIX_KITCHEN_SINK_RPC_PORT must be a positive integer. Got: "0"',
    );
  });
  ...
  it("cleans up the generated temporary home tree", async () => {
    ...
    expect(root).toContain("zuvix-kitchen-sink-rpc-");
    expect(env.HOME).toBe(path.join(root, "home"));
    expect(env.USERPROFILE).toBe(env.HOME);
    expect(env.ZUVIX_HOME).toBe(env.HOME);
    expect(env.ZUVIX_STATE_DIR).toBe(path.join(env.HOME, ".zuvix"));
    expect(env.ZUVIX_CONFIG_PATH).toBe(path.join(env.ZUVIX_STATE_DIR, "zuvix.json"));
    expect(existsSync(env.ZUVIX_STATE_DIR)).toBe(true);
  ...
  it("can fail the walk when generated temp cleanup cannot remove the root", async () => {
    ...
    try {
      await expect(
        cleanupKitchenSinkEnv("/tmp/zuvix-kitchen-sink-rpc-stuck", {
          attempts: 3,
          delayMs: 1,
          throwOnFailure: true,
          warn: false,
        ...
        "failed to remove Kitchen Sink RPC temp root: /tmp/zuvix-kitchen-sink-rpc-stuck",
      );
...
describe("kitchen-sink RPC gateway teardown", () => {
  ...
  it("fails readiness waits before polling after signaled gateway exits", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-signal-ready-"));
    try {
  ...
  it("aborts stalled readiness probes when the gateway exits mid-probe", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-exit-during-ready-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("keeps stalled readiness probes inside the caller deadline", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-stalled-ready-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("requires /readyz body.ready before accepting gateway readiness", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-ready-body-"));
    try {
      const logPath = path.join(root, "gateway.log");
...
describe("kitchen-sink RPC gateway readiness logs", () => {
  it("scans gateway readiness logs incrementally across appended chunks", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-log-scan-"));
    try {
      const logPath = path.join(root, "gateway.log");
      writeFileSync(logPath, "booting\n".repeat(1000));
  ...
  it("resets the readiness scanner after log rotation", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-log-rotate-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("tails large gateway logs without returning older content", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-log-tail-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("scans gateway error logs incrementally and keeps the latest findings", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-log-errors-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("does not allowlist dirty error lines that mention zero errors", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-log-zero-error-smuggle-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("bounds scanner memory for very long log lines", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-log-long-line-"));
    try {
      const logPath = path.join(root, "gateway.log");
...
describe("kitchen-sink RPC command output capture", () => {
  ...
  posixIt("kills timed command process groups", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-timeout-"));
...
`,
  ...
  posixIt("rejects timed commands that exit cleanly after SIGTERM", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-kitchen-rpc-timeout-zero-"));
...
`,
  ...
  it("rejects command spawn failures as Error objects", async () => {
    await expect(runCommand("zuvix-definitely-missing-command", [])).rejects.toMatchObject({
      code: "ENOENT",
    });
  });
});
...
describe("kitchen-sink RPC caller loading", () => {
  ...
  it("uses built callGateway chunks for dist and packaged entries", () => {
    expect(usesBuiltZuvixEntry({ command: "node", baseArgs: ["dist/index.js"] })).toBe(true);
    expect(
      usesBuiltZuvixEntry({ command: "node", baseArgs: ["/app/zuvix.mjs"] }, "/repo", {
        ZUVIX_ENTRY: "/app/zuvix.mjs",
      }),
    ).toBe(true);
  });

  it("does not deep-import gateway TypeScript for source pnpm runners", () => {
    expect(usesBuiltZuvixEntry({ pnpm: true, baseArgs: ["zuvix"] })).toBe(false);
    expect(usesBuiltZuvixEntry({ command: "node", baseArgs: ["scripts/dev.mjs"] })).toBe(false);
  });

  it("finds only built callGateway chunks", () => {
    const root = mkdtempSync(path.join(tmpdir(), "zuvix-rpc-call-chunks-"));
    try {
...
describe("kitchen-sink RPC command catalog assertions", () => {
  ...
  it("requires every expected Kitchen Sink plugin tool", () => {
    expect(() =>
      assertExpectedKitchenSinkToolEntries(
        [{ id: "kitchen_sink_text", source: "plugin", pluginId: "zuvix-kitchen-sink-fixture" }],
  ...
  it("requires plugin provenance for expected catalog tools", () => {
    expect(() =>
      assertExpectedKitchenSinkToolEntries(
        [
          { id: "kitchen_sink_text", source: "plugin", pluginId: "zuvix-kitchen-sink-fixture" },
          { id: "kitchen_sink_search", source: "core", pluginId: "zuvix-kitchen-sink-fixture" },
      ...
      assertExpectedKitchenSinkToolEntries(
        [
          { id: "kitchen_sink_text", source: "plugin", pluginId: "zuvix-kitchen-sink-fixture" },
          {
            ...
            source: "plugin",
            pluginId: "zuvix-kitchen-sink-fixture",
          },
          {
  ...
  it("requires Kitchen Sink UI descriptor coverage", () => {
    expect(() =>
      assertKitchenSinkUiDescriptors({
        ok: true,
        descriptors: [{ pluginId: "zuvix-kitchen-sink-fixture", id: "kitchen-sink-panel" }],
      }),
    ).not.toThrow();
  ...
  it("accepts minimally valid gateway health and status RPC payloads", () => {
    expect(() =>
      assertGatewayHealthPayload({
        ...
        agents: [],
        sessions: {
          path: "/tmp/zuvix-sessions.sqlite",
          count: 0,
          recent: [],
        },
      }),
...
describe("kitchen-sink RPC process sampling", () => {
  ...
  it("samples the POSIX gateway child instead of the pnpm launcher", async () => {
    const sample = await sampleProcess(4321, {
      ...
      runCommand: async (command: string, args: string[]) => {
        ...
        return {
          stdout: [
            " 4321     1   16384   0.0 node /usr/local/bin/corepack pnpm zuvix gateway --port 19080",
  ...
  it("falls back to the POSIX gateway process title when the port arg is rewritten", async () => {
    const sample = await sampleProcess(4321, {
      ...
      runCommand: async () => ({
        stdout: [
          " 4321     1 1048576   0.0 node /usr/local/bin/corepack pnpm zuvix gateway --port 19080",
          " 4322  4321  262144  12.5 zuvix-gateway",
          " 4323  4322   32768   1.5 node helper.js",
        ].join("\n"),
        stderr: "",
      }),
  ...
  it("does not accept a POSIX launcher sample when the gateway child is missing", async () => {
    const sample = await sampleProcess(4321, {
      ...
      posixCommandLineNeedles: ["gateway", "--port", "19080"],
      runCommand: async () => ({
        stdout: " 4321     1   16384   0.0 node /usr/local/bin/corepack pnpm zuvix status\n",
        stderr: "",
      }),
    });
