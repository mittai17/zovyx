// Zuvix E2E Instance tests cover zuvix e2e instance script behavior.
...
const helperPath = path.resolve("scripts/lib/zuvix-e2e-instance.sh");
const hostPath = [
  ...
  return spawnSync(
    ...
    [
      ...
      [
        ...
        `zuvix_e2e_eval_test_state_from_b64 ${shellQuote(payload)}`,
        'printf "value=%s" "${ZUVIX_E2E_INSTANCE_TEST:-unset}"',
      ].join("; "),
    ],
    { encoding: "utf8" },
  );
...
function writePackageFixture(packagePath: string): void {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-package-"));
  try {
    const packageDir = path.join(root, "package");
    fs.mkdirSync(packageDir);
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "zuvix-e2e-fixture", version: "0.0.0" }),
    );
...
function expectNpmInstallObserved(argsPath: string, expectedArgs: string, prefix: string): void {
  ...
  expect(
    fs.existsSync(path.join(prefix, "lib/node_modules/zuvix-e2e-fixture/package.json")),
  ).toBe(true);
}

describe("scripts/lib/zuvix-e2e-instance.sh", () => {
  it("sources decoded test-state scripts", () => {
    const result = runHelper(base64('export ZUVIX_E2E_INSTANCE_TEST="ok"\n'));
  ...
  it("fails when the test-state payload is not valid base64", () => {
    ...
    expect(result.stderr).toContain("Invalid Zuvix test-state base64 payload");
  });
  ...
  it("requires /readyz after the gateway ready log", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-readyz-required-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            "zuvix_e2e_probe_http() { return 1; }",
            "sleep 30 &",
            'gateway_pid="$!"',
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
            `printf '[gateway] ready ws://127.0.0.1:23456\\n' >${shellQuote(logPath)}`,
            `if zuvix_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2; then`,
  ...
  it("probes /readyz on the explicit gateway port", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-readyz-port-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `zuvix_e2e_probe_http() { printf "%s\\n" "$1" >${shellQuote(probePath)}; return 0; }`,
            "sleep 30 &",
            'gateway_pid="$!"',
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
            `printf '[gateway] ready\\n' >${shellQuote(logPath)}`,
            `zuvix_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("allows explicit legacy ready-log mode without /readyz", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-readyz-legacy-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      const result = spawnSync(
        "/bin/bash",
        [
          ...
          [
            ...
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
            `printf '[gateway] ready\\n' >${shellQuote(logPath)}`,
            `zuvix_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456 legacy-ready-log-ok`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("wraps package installs with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-"));
    try {
      ...
      const packagePath = path.join(tempDir, "zuvix.tgz");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `zuvix_e2e_install_package ${shellQuote(logPath)} ${shellQuote("fixture package")} ${shellQuote(prefixPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            ...
            ZUVIX_CURRENT_PACKAGE_TGZ: packagePath,
            ZUVIX_E2E_NPM_INSTALL_TIMEOUT: "42s",
            ZUVIX_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            ZUVIX_TEST_NPM_ARGS: npmArgsPath,
            ZUVIX_TEST_NPM_BIN: path.join(tempDir, "npm"),
          }),
        },
      );
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-plain-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses gtimeout when GNU timeout is not on PATH", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-gtimeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-no-timeout-"));
    try {
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
      const packagePath = path.join(tempDir, "zuvix.tgz");
  ...
  it("bounds npm install failure logs to the configured tail", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-install-log-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const logPath = path.join(tempDir, "install.log");
      const packagePath = path.join(tempDir, "zuvix.tgz");
  ...
  it("bounds commands with the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-node-watchdog-"));
    try {
      writeNodeShim(tempDir);
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `zuvix_e2e_maybe_timeout 200ms ${shellQuote(process.execPath)} -e ${shellQuote("setInterval(() => {}, 1000)")}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
      ...
      expect(result.stderr).toContain("Zuvix E2E command timed out after 200ms");
    } finally {
      fs.rmSync(tempDir, { force: true, recursive: true });
    }
  });
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Node watchdog children that ignore parent SIG${shellSignal}`, () => {
      const tempDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "zuvix-e2e-instance-node-watchdog-signal-"),
      );
      try {
        writeNodeShim(tempDir);
...
set -euo pipefail
source ${shellQuote(helperPath)}
export ZUVIX_E2E_TIMEOUT_KILL_GRACE_MS=100
zuvix_e2e_maybe_timeout 30s node ${shellQuote(childPath)} ${shellQuote(pidPath)} ${shellQuote(watchdogPidPath)} &
wrapper_pid="$!"
...
`;
  ...
  it("terminates only the tracked gateway process", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-gateway-terminate-"));
    try {
      ...
      writeBashExecutable(path.join(tempDir, "pkill"), [
        'printf "pkill %s\\n" "$*" >>"$ZUVIX_TEST_FORBIDDEN_PROCESS_TOOL_LOG"',
        "exit 42",
      ]);
...
set -euo pipefail
source ${shellQuote(helperPath)}
zuvix_e2e_terminate_gateways ""
/bin/sleep 30 &
tracked_pid="$!"
zuvix_e2e_terminate_gateways "$tracked_pid"
if kill -0 "$tracked_pid" 2>/dev/null; then
  echo "tracked gateway process still alive" >&2
  exit 1
fi
[ ! -s "$ZUVIX_TEST_FORBIDDEN_PROCESS_TOOL_LOG" ]
`;
      ...
      const result = spawnSync("/bin/bash", ["-c", script], {
        ...
        env: shellTestEnv({
          ...
          ZUVIX_TEST_FORBIDDEN_PROCESS_TOOL_LOG: forbiddenToolLog,
        }),
        timeout: 5_000,
      });
  ...
  it("terminates descendants in the tracked process group", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-process-group-"));
...
set -euo pipefail
source ${shellQuote(helperPath)}
tracked_pid="$(zuvix_e2e_start_tracked_process ${shellQuote(logPath)} ${shellQuote(process.execPath)} ${shellQuote(parentPath)} ${shellQuote(childPath)} ${shellQuote(parentPidPath)} ${shellQuote(childPidPath)} ${shellQuote(childTermPath)})"
for ((i = 0; i < 100; i += 1)); do
...
zuvix_e2e_stop_process "$tracked_pid"
for ((i = 0; i < 100; i += 1)); do
...
`;
  ...
  it("bounds HTTP readiness probes when a server accepts connections but never responds", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-http-probe-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `source ${shellQuote(helperPath)}`,
            'zuvix_e2e_probe_http_status "http://127.0.0.1:${port}/health" 200 100',
          ].join("; "),
        ],
  ...
  it("does not repeatedly grep the full gateway log while waiting for readiness", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-readyz-incremental-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `printf 'old log line\\n' >${shellQuote(logPath)}`,
            `printf '[gateway] ready ws://127.0.0.1:23456\\n' >>${shellQuote(logPath)}`,
            `zuvix_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("detects gateway ready markers split across incremental log reads", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-readyz-split-marker-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      const result = spawnSync(
        "/bin/bash",
        [
          ...
          [
            ...
            `zuvix_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 8`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `zuvix_e2e_probe_http() { printf '%s' "$1" >${shellQuote(probePath)}; [[ "$1" = "http://127.0.0.1:23456/readyz" ]]; }`,
            "sleep 30 &",
            'gateway_pid="$!"',
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
  ...
  it("wraps logged Zuvix E2E commands with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-run-logged-"));
    ...
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `zuvix_e2e_run_logged ${shellQuote(logLabel)} fixture-command one two`,
            `printf "%s" "$ZUVIX_E2E_LAST_LOG_PATH" > ${shellQuote(logPathFile)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            ZUVIX_E2E_LOG_DIR: logDir,
            ZUVIX_E2E_COMMAND_TIMEOUT: "17s",
            ZUVIX_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            ZUVIX_TEST_COMMAND_ARGS: commandArgsPath,
            ZUVIX_TEST_COMMAND_BIN: path.join(tempDir, "fixture-command"),
          }),
        },
      );
      ...
      expect(path.basename(logPath)).toMatch(new RegExp(`^zuvix-${logLabel}\\..+\\.log$`, "u"));
  ...
  it("bounds logged command failure output to the configured tail", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-run-log-tail-"));
    const logLabel = path.basename(tempDir);
    const logDir = path.join(tempDir, "logs");
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `zuvix_e2e_run_logged ${shellQuote(logLabel)} fixture-command`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
  ...
  it("installs the trash shim under isolated test state", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-trash-shim-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `printf "%s" "$ZUVIX_E2E_BIN_DIR" > ${shellQuote(binDirFile)}`,
      ...
      expect(binDir).not.toBe("/tmp/zuvix-bin");
  ...
  it("wraps package-installed Zuvix CLI calls with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-zuvix-cli-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const commandArgsPath = path.join(tempDir, "zuvix-args.txt");
      fs.writeFileSync(
        path.join(tempDir, "timeout"),
        [
          "#!/usr/bin/env bash",
          ...
          'if [ "${1:-}" = "--kill-after=1s" ]; then exit 0; fi',
          'printf "%s\\n" "$*" >"$ZUVIX_TEST_TIMEOUT_ARGS"',
          `while [ "$#" -gt 0 ] && [ "$1" != ${shellQuote(path.join(tempDir, "zuvix"))} ]; do shift; done`,
          '[ "$#" -gt 0 ] || exit 127',
          "shift",
          'exec "$ZUVIX_TEST_ZUVIX_BIN" "$@"',
          "",
        ].join("\n"),
      );
      fs.writeFileSync(
        path.join(tempDir, "zuvix"),
        [
          "#!/usr/bin/env bash",
          "set -euo pipefail",
          'printf "%s\\n" "$*" >"$ZUVIX_TEST_COMMAND_ARGS"',
      ...
      fs.chmodSync(path.join(tempDir, "timeout"), 0o755);
      fs.chmodSync(path.join(tempDir, "zuvix"), 0o755);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            ZUVIX_E2E_COMMAND_TIMEOUT: "23s",
            ZUVIX_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            ZUVIX_TEST_COMMAND_ARGS: commandArgsPath,
            ZUVIX_TEST_ZUVIX_BIN: path.join(tempDir, "zuvix"),
          }),
        },
      );
      ...
      expect(result.status).toBe(0);
      expect(fs.readFileSync(timeoutArgsPath, "utf8").trim()).toBe(
        `--kill-after=30s 23s ${path.join(tempDir, "zuvix")} plugins list --json`,
      );
  ...
  it("wraps interactive PTY scripts with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-e2e-instance-pty-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `zuvix_e2e_run_script_with_pty ${shellQuote("node /tmp/entry onboard")} ${shellQuote(logPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            ZUVIX_E2E_COMMAND_TIMEOUT: "31s",
            ZUVIX_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            ZUVIX_TEST_SCRIPT_ARGS: scriptArgsPath,
            ZUVIX_TEST_SCRIPT_BIN: path.join(tempDir, "script"),
          }),
        },
      );
