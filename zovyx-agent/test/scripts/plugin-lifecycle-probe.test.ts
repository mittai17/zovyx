...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "zuvix-plugin-lifecycle-probe-"));
  tempDirs.push(dir);
  return dir;
}
...
function runProbe(args: string[], home = makeTempDir()) {
  return spawnSync(process.execPath, [probePath, ...args], {
    ...
    env: {
      ...process.env,
      HOME: home,
      ZUVIX_CONFIG_PATH: path.join(home, ".zuvix", "zuvix.json"),
      USERPROFILE: home,
    },
  });
}
...
describe("plugin lifecycle matrix probe", () => {
  ...
  it("rejects unreadable config during uninstall proof", () => {
    const dir = makeTempDir();
    const configPath = path.join(dir, ".zuvix", "zuvix.json");
