...
function writeStartupMetadataSourceSignatureFixture(rootDir: string): void {
  const fixtures = new Map<string, string>([
    ...
    ["src/cli/banner.ts", "export const banner = 'zuvix';\n"],
    [
...
describe("write-cli-startup-metadata", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = createTempDir("zuvix-startup-metadata-timeout-");
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("zuvix-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: zuvix\n",
      renderSourceBrowserHelpText: () => "Usage: zuvix browser\n",
      renderSourceSecretsHelpText: () => "Usage: zuvix secrets\n",
      renderSourceNodesHelpText: () => "Usage: zuvix nodes\n",
      renderSourceSubcommandHelpTextRecord: () => ({
        doctor: "Usage: zuvix doctor\n",
        gateway: "Usage: zuvix gateway\n",
        models: "Usage: zuvix models\n",
        plugins: "Usage: zuvix plugins\n",
      }),
    });
    ...
    expect(written.browserHelpText).toContain("zuvix browser");
    expect(written.secretsHelpText).toContain("Usage:");
    expect(written.secretsHelpText).toContain("zuvix secrets");
    expect(written.nodesHelpText).toContain("Usage:");
    expect(written.nodesHelpText).toContain("zuvix nodes");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("zuvix");
    expect(written.subcommandHelpText.doctor).toContain("zuvix doctor");
    expect(written.subcommandHelpText.gateway).toContain("zuvix gateway");
    expect(written.subcommandHelpText.models).toContain("zuvix models");
    expect(written.subcommandHelpText.plugins).toContain("zuvix plugins");
  });

  it("renders independent startup help snapshots concurrently", async () => {
    const tempRoot = createTempDir("zuvix-startup-metadata-concurrency-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writePromise = writeCliStartupMetadata({
      ...
      renderBundledRootHelpText: async () => "Usage: zuvix\n",
      renderSourceBrowserHelpText: renderAfterUnblock("browser", "Usage: zuvix browser\n"),
      renderSourceSecretsHelpText: renderAfterUnblock("secrets", "Usage: zuvix secrets\n"),
      renderSourceNodesHelpText: renderAfterUnblock("nodes", "Usage: zuvix nodes\n"),
  ...
  it("regenerates nodes help when bundled canvas CLI help sources change", async () => {
    const tempRoot = createTempDir("zuvix-startup-metadata-signature-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writeMetadata = async (): Promise<void> => {
      await writeCliStartupMetadata({
        ...
        renderSourceNodesHelpText: () => {
          nodesRenderCount += 1;
          return `Usage: zuvix nodes ${nodesRenderCount}\n`;
        },
        renderSourceSubcommandHelpTextRecord: () => ({
          doctor: "Usage: zuvix doctor\n",
          gateway: "Usage: zuvix gateway\n",
    ...
    expect(written.nodesHelpText).toContain("zuvix nodes 3");
  });
});
