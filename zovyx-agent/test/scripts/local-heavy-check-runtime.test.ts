...
function makeEnv(overrides: Record<string, string | undefined> = {}) {
  ...
  if (!Object.hasOwn(overrides, "ZUVIX_LOCAL_CHECK_MODE")) {
    delete env.ZUVIX_LOCAL_CHECK_MODE;
  }
  return env;
}
...
describe("local-heavy-check-runtime", () => {
  it("reenables local heavy-check policy for local wrapper entrypoints", () => {
    expect(resolveLocalHeavyCheckEnv({ ZUVIX_LOCAL_CHECK: "0", PATH: "/usr/bin" })).toEqual({
      ZUVIX_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
    expect(resolveLocalHeavyCheckEnv({ ZUVIX_LOCAL_CHECK: "false", PATH: "/usr/bin" })).toEqual({
      ZUVIX_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
  });
  ...
  it("skips declaration transforms for no-emit tsgo checks", () => {
    const { args } = applyLocalTsgoPolicy([], makeEnv({ ZUVIX_LOCAL_CHECK: "0" }), ROOMY_HOST);
  ...
  it("keeps explicit tsgo flags and Go env overrides intact when throttled", () => {
    const { args, env } = applyLocalTsgoPolicy(
      ...
      makeEnv({
        GOGC: "80",
        GOMEMLIMIT: "5GiB",
        ZUVIX_TSGO_PPROF_DIR: "/tmp/profile",
      }),
      CONSTRAINED_HOST,
    );
  ...
  it("keeps explicit tsgo declaration flags intact", () => {
    const env = makeEnv({ ZUVIX_LOCAL_CHECK_MODE: "full" });
  ...
  it("uses the configured local tsgo build info file", () => {
    const { args } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        ZUVIX_LOCAL_CHECK_MODE: "full",
        ZUVIX_TSGO_BUILD_INFO_FILE: ".artifacts/custom/tsgo.tsbuildinfo",
      }),
      ROOMY_HOST,
    );
  ...
  it("avoids incremental cache reuse for ad hoc tsgo runs", () => {
    const { args } = applyLocalTsgoPolicy(
      ["--extendedDiagnostics"],
      makeEnv({ ZUVIX_LOCAL_CHECK_MODE: "full" }),
      ROOMY_HOST,
    );
  ...
  it("allows forcing the tsgo lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLockForTsgo(
        ["--help"],
        makeEnv({ ZUVIX_TSGO_FORCE_LOCK: "1" }),
      ),
    ).toBe(true);
  });
  ...
  it("skips the heavy-check lock for explicit oxlint file targets", () => {
    const cwd = createTempDir("zuvix-oxlint-lock-skip-");
  ...
  it("keeps the heavy-check lock for directory targets and broad oxlint runs", () => {
    const cwd = createTempDir("zuvix-oxlint-lock-keep-");
  ...
  it("allows forcing the oxlint lock back on", () => {
    const cwd = createTempDir("zuvix-oxlint-lock-force-");
    ...
    expect(
      shouldAcquireLocalHeavyCheckLockForOxlint(["--type-aware", "--", "sample.ts"], {
        cwd,
        env: makeEnv({ ZUVIX_OXLINT_FORCE_LOCK: "1" }),
      }),
    ).toBe(true);
  });

  it("reclaims stale local heavy-check locks from dead pids", () => {
    const cwd = createTempDir("zuvix-local-heavy-check-");
    const commonDir = path.join(cwd, ".git");
    const lockDir = path.join(commonDir, "zuvix-local-checks", "heavy-check.lock");
  ...
  it("uses a worktree-local heavy-check lock when explicitly requested", () => {
    const repoRoot = createTempDir("zuvix-local-heavy-check-worktree-");
    execFileSync("git", ["init"], { cwd: repoRoot, stdio: "ignore" });
    const cwd = path.join(repoRoot, "nested", "tooling");
    fs.mkdirSync(cwd, { recursive: true });
    const commonLockDir = path.join(repoRoot, ".git", "zuvix-local-checks", "heavy-check.lock");
    ...
    const nestedLockDir = path.join(cwd, ".artifacts", "zuvix-local-checks", "heavy-check.lock");

    const release = acquireLocalHeavyCheckLockSync({
      cwd,
      env: makeEnv({ ZUVIX_HEAVY_CHECK_LOCK_SCOPE: "worktree" }),
  ...
  it("rejects malformed heavy-check lock timing env values", () => {
    const cwd = createTempDir("zuvix-local-heavy-check-malformed-env-");

    expect(() =>
      acquireLocalHeavyCheckLockSync({
        cwd,
        env: makeEnv({ ZUVIX_HEAVY_CHECK_LOCK_TIMEOUT_MS: "10ms" }),
        toolName: "oxlint",
      }),
    ).toThrow("ZUVIX_HEAVY_CHECK_LOCK_TIMEOUT_MS must be a positive integer; got: 10ms");
    expect(() =>
      acquireLocalHeavyCheckLockSync({
        cwd,
        env: makeEnv({ ZUVIX_HEAVY_CHECK_LOCK_POLL_MS: "0" }),
        toolName: "oxlint",
      }),
    ).toThrow("ZUVIX_HEAVY_CHECK_LOCK_POLL_MS must be a positive integer; got: 0");
  });

  it("cleans up stale legacy test locks when acquiring the shared heavy-check lock", () => {
    const cwd = createTempDir("zuvix-local-heavy-check-legacy-");
    const commonDir = path.join(cwd, ".git");
    const locksDir = path.join(commonDir, "zuvix-local-checks");
