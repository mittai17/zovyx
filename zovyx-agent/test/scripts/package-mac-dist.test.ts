...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "zuvix-dist-plist-"));
  tempDirs.push(dir);
...
describe("package-mac-dist plist validation", () => {
  it("fails closed for required Info.plist reads", () => {
    ...
    const readBlock = script.slice(
      ...
      script.indexOf('ZIP="$ROOT_DIR/dist/Zuvix-$VERSION.zip"'),
    );
  ...
  it("keeps dependency bootstrap output out of captured Sparkle build values", () => {
    ...
    const dir = mkdtempSync(path.join(tmpdir(), "zuvix-dist-sparkle-"));
    tempDirs.push(dir);
    ...
    const result = runHelper(`
      ...
      ZUVIX_ROOT=${JSON.stringify(process.cwd())}
      ZUVIX_MARKER=${JSON.stringify(marker)}
      PATH=${JSON.stringify(tools)}:/usr/bin:/bin
      export ZUVIX_MARKER ZUVIX_ROOT PATH
      ${helpers}
