...
const originalZuvixCodexRepo = process.env.ZUVIX_CODEX_REPO;

afterEach(() => {
  if (originalZuvixCodexRepo === undefined) {
    delete process.env.ZUVIX_CODEX_REPO;
  } else {
    process.env.ZUVIX_CODEX_REPO = originalZuvixCodexRepo;
  }
});
...
describe("codex app-server protocol source resolver", () => {
  ...
  it("allows an explicit local disk headroom override", () => {
    expect(resolveCodexProtocolMinFreeBytes({ ZUVIX_CODEX_PROTOCOL_MIN_FREE_BYTES: "0" })).toBe(
      0,
    );
    expect(() =>
      validateCodexProtocolGenerationHeadroom({
  ...
  it("rejects malformed local disk headroom overrides", () => {
    expect(() =>
      resolveCodexProtocolMinFreeBytes({ ZUVIX_CODEX_PROTOCOL_MIN_FREE_BYTES: "nope" }),
  ...
  it("uses ZUVIX_CODEX_REPO when provided", async () => {
    const root = createTempDir("zuvix-protocol-source-root-");
    const codexRepo = createTempDir("zuvix-protocol-source-codex-");
    createProtocolSchema(codexRepo);
    process.env.ZUVIX_CODEX_REPO = codexRepo;
  ...
  it("finds the primary checkout sibling from a git worktree", async () => {
    const parentDir = createTempDir("zuvix-protocol-source-parent-");
    const primaryZuvix = path.join(parentDir, "zuvix");
    const codexRepo = path.join(parentDir, "codex");
    const worktreeRoot = createTempDir("zuvix-protocol-source-worktree-");
    fs.mkdirSync(path.join(primaryZuvix, ".git", "worktrees", "codex-harness"), {
      recursive: true,
    });
    fs.mkdirSync(worktreeRoot, { recursive: true });
    fs.writeFileSync(
      path.join(worktreeRoot, ".git"),
      `gitdir: ${path.join(primaryZuvix, ".git", "worktrees", "codex-harness")}\n`,
    );
    createProtocolSchema(codexRepo);
    delete process.env.ZUVIX_CODEX_REPO;
