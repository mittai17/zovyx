...
describe("e2e shell tempfile hygiene", () => {
  ...
  it("preserves wizard exit status when reporting failures", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "zuvix-onboard-status-test-"));
...
export ZUVIX_ONBOARD_SCENARIO_SOURCE_ONLY=1
export ZUVIX_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
ZUVIX_ENTRY=node
zuvix_test_state_create() { :; }
...
`,
  ...
  it("checks local onboarding logs for systemd noise", async () => {
    ...
    expect(contents).toContain('ZUVIX_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"');
    ...
    expect(contents).toContain('validate_local_basic_log "$ZUVIX_E2E_LAST_LOG_PATH"');
    expect(contents).not.toContain(
      "validate_local_basic_log /tmp/zuvix-onboard-local-basic.log",
    );
    expect(contents).toContain(
  ...
  it("probes onboarding gateway readiness through TCP", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "zuvix-onboard-gateway-log-"));
...
`,
  ...
  it("rejects onboarding gateway readiness when the TCP probe fails", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "zuvix-onboard-gateway-tcp-"));
...
export ZUVIX_ONBOARD_SCENARIO_SOURCE_ONLY=1
export ZUVIX_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
export ZUVIX_ONBOARD_GATEWAY_WAIT_ATTEMPTS=2
export ZUVIX_ONBOARD_GATEWAY_WAIT_INTERVAL_S=0.1
ZUVIX_ENTRY=node
source scripts/e2e/lib/onboard/scenario.sh

zuvix_e2e_probe_tcp() { return 1; }
sleep 30 &
GATEWAY_PID="$!"
printf 'listening on ws://127.0.0.1:18789\\n' >"$GATEWAY_LOG_PATH"
...
`,
  ...
  it("removes fallback ClawHub skill install HOME on failure", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "zuvix-clawhub-home-test-"));
...
`,
    ...
    try {
      ...
      expect(
        scratchEntries.filter((entry) => entry.startsWith("zuvix-skill-install-home.")),
      ).toEqual([]);
    } finally {
      await rm(tempRoot, { force: true, recursive: true });
    }
