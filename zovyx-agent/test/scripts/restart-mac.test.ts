...
function runGatewayPortCheck(fakeLsof: string) {
  const root = mkdtempSync(join(tmpdir(), "zuvix-restart-mac-test-"));
  tempRoots.push(root);
...
function runCleanupFunction(fakePs: string) {
  ...
  writeFileSync(
    ...
    [
      ...
      'DEBUG_PROCESS_PATTERN="/worktree/apps/macos/.build/debug/Zuvix"',
      'LOCAL_PROCESS_PATTERN="/worktree/apps/macos/.build-local/debug/Zuvix"',
      'RELEASE_PROCESS_PATTERN="/worktree/apps/macos/.build/release/Zuvix"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$ZUVIX_TEST_KILL_CALLS"',
      "  return 0",
      "}",
      "kill_all_zuvix",
    ].join("\n"),
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      ZUVIX_TEST_KILL_CALLS: killCallsPath,
      PATH: `${binDir}:${process.env.PATH ?? ""}`,
    },
  });
...
describe("scripts/restart-mac.sh", () => {
  ...
  it("keeps the default restart log scoped to the current worktree lock", () => {
    ...
    expect(script).not.toContain('LOG_PATH="${ZUVIX_RESTART_LOG:-/tmp/zuvix-restart.log}"');
  });
  ...
  it("prefers the freshly packaged app unless an explicit app bundle is set", () => {
    ...
    expect(script).toContain('fail "ZUVIX_APP_BUNDLE does not exist: ${APP_BUNDLE}"');
    expect(chooseBlock).toContain("canonicalize_app_bundle");
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Zuvix.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("/Applications/Zuvix.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Zuvix.app")).toBeLessThan(
      chooseBlock.indexOf("/Applications/Zuvix.app"),
    );
  });

  it("keeps restart cleanup scoped to known Zuvix app and build paths", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    ...
    expect(cleanupBlock).toContain('"/Applications/Zuvix.app/${APP_EXECUTABLE_RELATIVE_PATH}"');
  ...
  it("normalizes custom app bundle paths before process matching", () => {
    const root = mkdtempSync(join(tmpdir(), "zuvix-restart-mac-test-"));
    tempRoots.push(root);
    const appBundle = join(root, "dist", "Zuvix.app");
    mkdirSync(appBundle, { recursive: true });

    const { result } = runCanonicalizeAppBundle(`${appBundle}/../Zuvix.app/`);
  ...
  it("fails restart cleanup when scoped processes survive every kill attempt", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  321 /worktree/dist/Zuvix.app/Contents/MacOS/Zuvix --attach-only'",
      ].join("\n"),
    );
  ...
  it("passes restart cleanup when the final kill attempt clears the process", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        'kill_count="$(wc -l < "$ZUVIX_TEST_KILL_CALLS" 2>/dev/null || echo 0)"',
  ...
  it("does not kill unrelated Zuvix app bundles", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  654 /tmp/Other/Zuvix.app/Contents/MacOS/Zuvix'",
      ].join("\n"),
    );
