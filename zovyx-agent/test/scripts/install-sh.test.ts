...
function runInstallShell(script: string, env: NodeJS.ProcessEnv = {}) {
  const home = mkdtempSync(join(tmpdir(), "zuvix-install-home-"));
  try {
...
describe("install.sh", () => {
  ...
  it("runs installer snippets without inherited shell startup files", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-shell-env-"));
    const bashEnvPath = join(tmp, "bash_env");
    writeFileSync(bashEnvPath, "export ZUVIX_BASH_ENV_LEAKED=1\n");

    try {
      const result = runInstallShell('printf "leaked=%s\\n" "${ZUVIX_BASH_ENV_LEAKED:-0}"', {
  ...
  it("installs Git with apk on Alpine", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-git-apk-"));
  ...
  it("does not select apk Git on non-Alpine hosts", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-git-native-"));
    const bin = join(tmp, "bin");
    const apkLog = join(tmp, "apk-args.txt");
    mkdirSync(bin, { recursive: true });
    const fakeApk = join(bin, "apk");
  ...
  it("does not emit --before when raw user npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-npmrc-"));
    const bin = join(tmp, "bin");
    ...
    try {
      const result = runInstallShell(
        [
          ...
          `run_npm_global_install zuvix@latest ${JSON.stringify(join(tmp, "install.log"))}`,
  ...
  it("does not emit --before when default global npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-global-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("does not emit --before when builtin npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-builtin-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("uses ZUVIX_HOME for git and onboarding defaults", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-home-"));
    const osHome = join(tmp, "os-home");
    const zuvixHome = join(tmp, "zuvix-home");
    mkdirSync(osHome, { recursive: true });
    mkdirSync(zuvixHome, { recursive: true });
    ...
    expect(output).toContain(`git=${join(zuvixHome, "zuvix")}`);
    expect(output).toContain(`workspace=${join(zuvixHome, ".zuvix", "workspace")}`);
    expect(output).toContain(
      `workspaceProfile=${join(zuvixHome, ".zuvix", "workspace-work")}`,
    );
  ...
  it("skips bootstrap onboarding when legacy HOME config exists with ZUVIX_HOME", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-legacy-config-"));
    const osHome = join(tmp, "os-home");
    const zuvixHome = join(tmp, "zuvix-home");
    const legacyConfigDir = join(osHome, ".zuvix");
    const bootstrapDir = join(zuvixHome, ".zuvix", "workspace");
    mkdirSync(legacyConfigDir, { recursive: true });
    mkdirSync(bootstrapDir, { recursive: true });
    writeFileSync(join(legacyConfigDir, "zuvix.json"), "{}\n");
  ...
  it("rejects Zuvix GitHub source targets for npm installs", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      set +e
    ...
    expect(result.stdout).toContain("npm installs do not support Zuvix GitHub source targets");
  ...
  it("does not emit before args when npmrc min-release-age computes a before cutoff", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-npm-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("ignores project npmrc when choosing global install freshness args", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-global-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("bounds installer npm prefix probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-npm-probe-"));
    ...
    try {
      const result = runInstallShell(
        ...
        {
          ZUVIX_INSTALL_PROBE_TIMEOUT_SECONDS: "1",
          PATH: `${tmp}:${process.env.PATH ?? ""}`,
        },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("/tmp/zuvix-npm/bin");
  ...
  it("bounds daemon status probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-probe-"));
    const claw = join(tmp, "zuvix");
    writeFileSync(
      claw,
      [
        "#!/usr/bin/env bash",
    ...
    try {
      const result = runInstallShell(
        ...
        { ZUVIX_INSTALL_PROBE_TIMEOUT_SECONDS: "0.1" },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("not-loaded");
      expect(result.stderr).toContain(
        "timed out during installer finalization probe: zuvix daemon status --json",
      );
    } finally {
      rmSync(tmp, { force: true, recursive: true });
    }
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-nvm-"));
    const home = join(tmp, "home");
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-node-promote-"));
  ...
  it("uses the package engine floor when accepting existing Node runtimes", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-node-floor-"));
    const bin = join(tmp, "bin");
    mkdirSync(bin, { recursive: true });
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g zuvix@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute zuvix path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "zuvix-install-command-"));
    const npmBin = join(tmp, "npm bin");
    const visibleBin = join(tmp, "visible-bin");
    mkdirSync(npmBin, { recursive: true });
    mkdirSync(visibleBin, { recursive: true });
    const zuvixBin = join(npmBin, "zuvix");
    writeFileSync(zuvixBin, "#!/bin/sh\nexit 0\n");
    chmodSync(zuvixBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(zuvix_command_for_user "${zuvixBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${zuvixBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=zuvix");
  });
  ...
  it("resolves requested git install versions to checkout refs", () => {
    const result = runInstallShell(`
      ...
      source "${SCRIPT_PATH}"
      npm() {
        if [[ "$1" == "view" && "$2" == "zuvix" && "$3" == "dist-tags.beta" ]]; then
          printf '2026.5.12-beta.3\\n'
          return 0
        fi
        return 1
      }
      ZUVIX_VERSION=v2026.5.12-beta.3
      printf 'tag=%s\\n' "$(resolve_git_zuvix_ref)"
      ZUVIX_VERSION=2026.5.12-beta.3
      printf 'semver=%s\\n' "$(resolve_git_zuvix_ref)"
      ZUVIX_VERSION=beta
      printf 'beta=%s\\n' "$(resolve_git_zuvix_ref)"
      ZUVIX_VERSION=main
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "zuvix-install-sh-gum-"));
    try {
...
describe("install.sh duplicate Zuvix install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/zuvix" "$root/fnm/zuvix"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/zuvix/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/zuvix/package.json"
      collect_zuvix_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      ZUVIX_BIN="$root/fnm/.bin/zuvix"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_zuvix_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple Zuvix global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/zuvix");
    expect(result.stdout).toContain("/fnm/zuvix");
    expect(result.stdout).toContain("Active zuvix:");
    expect(result.stdout).toContain("npm uninstall -g zuvix");
  });

  it("stays quiet when only one Zuvix npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/zuvix"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/zuvix/package.json"
      collect_zuvix_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_zuvix_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple Zuvix global installs detected");
  });
});
