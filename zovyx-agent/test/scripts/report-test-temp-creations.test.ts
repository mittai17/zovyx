...
describe("report-test-temp-creations", () => {
  it("keeps a non-executed warning fixture for changed-gate proof", () => {
    ...
    const warningFixture = 'fs.mkdtempSync("zuvix-warning-fixture-")';
  ...
  it("honors explicit allow comments with reasons", () => {
    ...
    const inlineAllowedSource = `const inlineAllowed = ${tmpDirCall}; // zuvix-temp-dir: allow verifies tmp API behavior`;
    const blockedSource = `const blocked = ${mkdtempCall};`;
    const stringMarkerSource = `const stringMarker = ${mkdtempCall}; const note = "zuvix-temp-dir: allow quoted text";`;
  ...
  it("formats GitHub warning annotations for CI report mode", () => {
    expect(
      formatGithubWarning({
        ...
        line: 12,
        reason: "new mkdtemp temp directory creation",
        // zuvix-temp-dir: allow test fixture for GitHub warning formatting
  ...
  it("exits non-zero for staged findings when requested", () => {
    const root = tempDirs.make("zuvix-temp-report-");
