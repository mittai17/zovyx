// Package Zuvix For Docker tests cover package zuvix for docker script behavior.
...
import {
  ...
  packZuvixPackageForDocker,
  parseArgs,
  runCommandForTest,
} from "../../scripts/package-zuvix-for-docker.mjs";
...
describe("package-zuvix-for-docker", () => {
  ...
  it("uses build-all as the single bounded package artifact build step", async () => {
    ...
    const previousTimeout = process.env.ZUVIX_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    process.env.ZUVIX_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.ZUVIX_BUILD_ALL_NO_PNPM,
            skipDts: options.env?.ZUVIX_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.ZUVIX_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.ZUVIX_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
    }
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packZuvixPackageForDocker("/repo", "/out", {
      ...
      runCaptureImpl: async (
        ...
        return "zuvix-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "zuvix-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare:/repo",
  ...
  it("restores the changelog when ignore-scripts packaging fails", async () => {
    ...
    await expect(
      packZuvixPackageForDocker("/repo", "/out", {
        prepareChangelog: async (cwd: string) => {
          calls.push(`prepare:${cwd}`);
        },
        restoreChangelog: async (cwd: string) => {
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-package-timeout-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-zuvix-for-docker.mjs")).href;
    let childPid = 0;
    let runnerPid;
    try {
