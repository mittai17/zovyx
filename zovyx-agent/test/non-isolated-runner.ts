...
const SHARED_TEST_SETUP = Symbol.for("zuvix.sharedTestSetup");
const EMBEDDED_RUN_STATE = Symbol.for("zuvix.embeddedRunState");
const REPLY_RUN_REGISTRY = Symbol.for("zuvix.replyRunRegistry");
...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.ZUVIX_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.ZUVIX_TEST_HOME = testHome;
...
function resetZuvixGlobalRunState(): void {
...
export default class ZuvixNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    super.onCollectStart(file);
    restoreRealTimers();
    restoreNativeTimerGlobals();
    restoreSharedTestHomeAfterEnvUnstub(getSharedTestHome());
    const orderLogPath = process.env.ZUVIX_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
