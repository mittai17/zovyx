...
function createTempHome(): string {
  return makeTempDir(tempDirs, "zuvix-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-zuvix.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.zuvix/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.zuvix/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".zuvix", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".zuvix", "external-plugins", "glueclaw", "zuvix.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".zuvix", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.ZUVIX_LIVE_TEST_QUIET = "1";
    process.env.ZUVIX_CONFIG_PATH = "~/custom-zuvix.json5";
    process.env.ZUVIX_TEST_HOME = priorIsolatedHome;
    process.env.ZUVIX_STATE_DIR = path.join(priorIsolatedHome, ".zuvix");
    ...
    expect(process.env.ZUVIX_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".zuvix", "zuvix.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".zuvix", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".zuvix", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.ZUVIX_LIVE_TEST = "1";
    process.env.ZUVIX_LIVE_USE_REAL_HOME = "1";
    process.env.ZUVIX_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.ZUVIX_LIVE_TEST;
    delete process.env.ZUVIX_LIVE_GATEWAY;
    delete process.env.ZUVIX_LIVE_USE_REAL_HOME;
    delete process.env.ZUVIX_LIVE_TEST_QUIET;
