...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.ZUVIX_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "ZUVIX_TEST_FAST", value: process.env.ZUVIX_TEST_FAST },
    {
      key: "ZUVIX_STRICT_FAST_REPLY_CONFIG",
      value: process.env.ZUVIX_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "ZUVIX_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.ZUVIX_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "ZUVIX_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.ZUVIX_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "ZUVIX_STATE_DIR", value: process.env.ZUVIX_STATE_DIR },
    { key: "ZUVIX_CONFIG_PATH", value: process.env.ZUVIX_CONFIG_PATH },
    { key: "ZUVIX_GATEWAY_PORT", value: process.env.ZUVIX_GATEWAY_PORT },
    { key: "ZUVIX_BRIDGE_ENABLED", value: process.env.ZUVIX_BRIDGE_ENABLED },
    { key: "ZUVIX_BRIDGE_HOST", value: process.env.ZUVIX_BRIDGE_HOST },
    { key: "ZUVIX_BRIDGE_PORT", value: process.env.ZUVIX_BRIDGE_PORT },
    { key: "ZUVIX_CANVAS_HOST_PORT", value: process.env.ZUVIX_CANVAS_HOST_PORT },
    { key: "ZUVIX_TEST_HOME", value: process.env.ZUVIX_TEST_HOME },
    { key: "ZUVIX_AGENT_DIR", value: process.env.ZUVIX_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ZUVIX_TEST_HOME = tempHome;
  process.env.ZUVIX_TEST_FAST = "1";
  process.env.ZUVIX_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.ZUVIX_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.ZUVIX_BRIDGE_ENABLED;
  delete process.env.ZUVIX_BRIDGE_HOST;
  delete process.env.ZUVIX_BRIDGE_PORT;
  delete process.env.ZUVIX_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.ZUVIX_STATE_DIR = path.join(tempHome, ".zuvix");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.ZUVIX_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.ZUVIX_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".zuvix");
  const priorIsolatedHome = params.env.ZUVIX_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".zuvix")
  ) {
    realStateDir = path.join(params.realHome, ".zuvix");
  }
  const tempStateDir = path.join(params.tempHome, ".zuvix");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.ZUVIX_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.ZUVIX_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "zuvix.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "zuvix.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.ZUVIX_LIVE_TEST === "1" ||
    process.env.ZUVIX_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.ZUVIX_LIVE_USE_REAL_HOME);
