...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.ZUVIX_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through zuvix infer", () => {
    const outputBase = path.join(os.tmpdir(), `zuvix-infer-image-${process.pid}.png`);
