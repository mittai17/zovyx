...
vi.mock("zuvix/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: zuvix", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "zuvix" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: zuvix", async () => {
