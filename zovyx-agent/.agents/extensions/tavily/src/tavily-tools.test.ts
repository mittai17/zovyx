...
function fakeApi(): ZuvixPluginApi {
  return {
    config: {},
  } as ZuvixPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.zuvix.ai", "", "zuvix.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.zuvix.ai", "zuvix.ai"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<ZuvixPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<ZuvixPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "zuvix" });
    ...
    expect(searchParams.query).toBe("zuvix");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as ZuvixConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
