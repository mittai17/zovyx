...
import { resolvePositiveTimeoutSeconds } from "zuvix/plugin-sdk/provider-web-search";
import {
...
export function resolveTavilySearchConfig(cfg?: ZuvixConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: ZuvixConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: ZuvixConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
