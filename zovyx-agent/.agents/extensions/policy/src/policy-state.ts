...
import { normalizeProviderId } from "zuvix/plugin-sdk/provider-model-shared";
import { normalizeAgentId } from "zuvix/plugin-sdk/routing";
import { coerceSecretRef } from "zuvix/plugin-sdk/secret-input";
import {
  asBoolean as readBoolean,
  isRecord,
  normalizeOptionalString as readString,
} from "zuvix/plugin-sdk/string-coerce-runtime";
...
const DEFAULT_POLICY_SANDBOX_BROWSER_NETWORK = "zuvix-sandbox-browser";
  ...
  return Object.entries(configuredChannels(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        provider: id,
        source: `oc://zuvix.config/channels/${id}`,
      };
  ...
  return Object.entries(configuredMcpServers(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        source: `oc://zuvix.config/mcp/servers/${ocPathSegment(id)}`,
      };
      if (isRecord(value)) {
...
export function scanPolicyModelProviders(
  ...
  return Object.keys(configuredModelProviders(cfg))
    ...
    .map((id) => ({
      ...
      source: `oc://zuvix.config/models/providers/${id}`,
    }));
}
...
export function scanPolicyModelRefs(
  ...
  if (isRecord(cfg.agents)) {
    collectModelRefsFromRecord(refs, cfg.agents, "oc://zuvix.config/agents");
...
export function scanPolicyNetwork(cfg: Record<string, unknown>): readonly PolicyNetworkEvidence[] {
  return [
    networkBooleanEvidence(
      ...
      "oc://zuvix.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "browser-private-network-legacy",
      ["browser", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://zuvix.config/browser/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network",
      ["tools", "web", "fetch", "ssrfPolicy", "dangerouslyAllowPrivateNetwork"],
      "oc://zuvix.config/tools/web/fetch/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network-legacy",
      ["tools", "web", "fetch", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://zuvix.config/tools/web/fetch/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-rfc2544-benchmark-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowRfc2544BenchmarkRange"],
      "oc://zuvix.config/tools/web/fetch/ssrfPolicy/allowRfc2544BenchmarkRange",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-ipv6-unique-local-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowIpv6UniqueLocalRange"],
      "oc://zuvix.config/tools/web/fetch/ssrfPolicy/allowIpv6UniqueLocalRange",
    ),
...
export function scanPolicyIngress(cfg: Record<string, unknown>): readonly PolicyIngressEvidence[] {
  ...
  const channelDefaultsSource = "oc://zuvix.config/channels/defaults";
  ...
  entries.push({
    id: "session-dm-scope",
    kind: "sessionDmScope",
    source: "oc://zuvix.config/session/dmScope",
    value: dmScope ?? "main",
  ...
  for (const [channel, value] of Object.entries(channels)) {
    ...
    const channelSource = `oc://zuvix.config/channels/${ocPathSegment(channel)}`;
...
export function scanPolicyGatewayExposure(
  ...
  entries.push({
    ...
    kind: "bind",
    source: "oc://zuvix.config/gateway/bind",
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://zuvix.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://zuvix.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://zuvix.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://zuvix.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://zuvix.config/gateway/controlUi/allowInsecureAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://zuvix.config/gateway/controlUi/dangerouslyDisableDeviceAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://zuvix.config/gateway/controlUi/dangerouslyAllowHostHeaderOriginFallback",
  );
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://zuvix.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://zuvix.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://zuvix.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://zuvix.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
export function scanPolicyAgentWorkspace(
  ...
  pushAgentWorkspaceEvidence(entries, {
    ...
    tools: defaultTools,
    inheritedTools: {},
    workspaceSourceBase: "oc://zuvix.config/agents/defaults",
    inheritedWorkspaceSourceBase: "oc://zuvix.config/agents/defaults",
    toolsSourceBase: "oc://zuvix.config/tools",
    inheritedToolsSourceBase: "oc://zuvix.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushAgentWorkspaceEvidence(entries, {
      ...
      workspaceSourceBase: `oc://zuvix.config/agents/list/#${index}`,
      inheritedWorkspaceSourceBase: "oc://zuvix.config/agents/defaults",
      toolsSourceBase: `oc://zuvix.config/agents/list/#${index}/tools`,
      inheritedToolsSourceBase: "oc://zuvix.config/tools",
    });
  });
  return entries.toSorted((a, b) => a.source.localeCompare(b.source) || a.id.localeCompare(b.id));
...
export function scanPolicySandboxPosture(
  ...
  pushSandboxPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://zuvix.config/agents/defaults/sandbox",
    inheritedSourceBase: "oc://zuvix.config/agents/defaults/sandbox",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushSandboxPostureEvidence(entries, {
      ...
      sourceBase: `oc://zuvix.config/agents/list/#${index}/sandbox`,
      inheritedSourceBase: "oc://zuvix.config/agents/defaults/sandbox",
    });
  });
...
export function scanPolicyToolPosture(
  ...
  pushToolPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://zuvix.config/tools",
    inheritedSourceBase: "oc://zuvix.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushToolPostureEvidence(entries, {
      ...
      inheritedSandbox: defaultSandbox,
      sourceBase: `oc://zuvix.config/agents/list/#${index}/tools`,
      inheritedSourceBase: "oc://zuvix.config/tools",
    });
  });
  ...
  return Object.entries(profiles)
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        source: `oc://zuvix.config/auth/profiles/${ocPathSegment(id)}`,
...
function pushMemorySessionTranscriptIndexing(
  ...
  if (defaultSessionMemory !== undefined) {
    entries.push({
      ...
      kind: "memorySessionTranscriptIndexing",
      source: "oc://zuvix.config/agents/defaults/memorySearch/experimental/sessionMemory",
      scope: "global",
  ...
  agents.list.forEach((rawAgent, index) => {
    ...
    entries.push({
      ...
      kind: "memorySessionTranscriptIndexing",
      source: explicit
        ? `oc://zuvix.config/agents/list/#${index}/memorySearch/experimental/sessionMemory`
        : "oc://zuvix.config/agents/defaults/memorySearch/experimental/sessionMemory",
      scope: "agent",
...
function scanPolicySecretProviders(cfg: Record<string, unknown>): readonly PolicySecretEvidence[] {
  ...
  return Object.entries(providers).map(([id, value]) => {
    ...
    const entry: {
      ...
      id,
      kind: "provider",
      source: `oc://zuvix.config/secrets/providers/${ocPathSegment(id)}`,
    };
...
function configPathSource(path: readonly string[]): string {
  return `oc://zuvix.config/${path.map(ocPathSegment).join("/")}`;
}
  ...
  const sandboxModeSource =
    explicitSandboxMode !== undefined
      ...
      : inheritedSandboxMode !== undefined
        ...
        : "oc://zuvix.config/agents/defaults/sandbox/mode";
    ...
    source:
      explicitWorkspaceAccess !== undefined
        ...
        : inheritedWorkspaceAccess !== undefined
          ...
          : "oc://zuvix.config/agents/defaults/sandbox/workspaceAccess",
    scope: params.scope,
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://zuvix.config/gateway/http/endpoints/${endpoint}`;
...
function collectModelRefsFromAgentAllowlist(
  ...
  if (isRecord(defaults) && isRecord(defaults.models)) {
    collectModelRefsFromModelMap(
      refs,
      defaults.models,
      "oc://zuvix.config/agents/defaults/models",
    );
  }
  ...
  for (const [index, agent] of list.entries()) {
    ...
    collectModelRefsFromModelMap(
      refs,
      agent.models,
      `oc://zuvix.config/agents/list/#${index}/models`,
    );
  }
}
