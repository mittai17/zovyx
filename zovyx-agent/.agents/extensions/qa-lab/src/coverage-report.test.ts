...
describe("qa coverage report", () => {
  ...
  it("renders Playwright matches as qa suite targets", () => {
    ...
    expect(report).toContain(
      "- Suite command: `pnpm zuvix qa suite --scenario control-ui-chat-flow-playwright`",
    );
  ...
  it("splits qa suite targets when matches mix execution kinds", () => {
    ...
    expect(report).toContain("  - flow: `pnpm zuvix qa suite --scenario flow-proof`");
    expect(report).toContain(
      "  - playwright: `pnpm zuvix qa suite --scenario playwright-proof`",
    );
  });
    ...
    const report = buildQaScorecardTaxonomyReport({
      ...
      scenarios: [
        scenarioWithCoverage({
          primary: [TEST_EXECUTABLE_COVERAGE_ID],
          executionKind: "playwright",
          executionPath: "../outside-zuvix.test.ts",
        }),
      ],
    });
