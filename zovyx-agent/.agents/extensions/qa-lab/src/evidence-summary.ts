...
export const QA_EVIDENCE_SUMMARY_KIND = "zuvix.qa.evidence-summary";
...
function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["ZUVIX_E2E_PROFILE", params.env?.ZUVIX_E2E_PROFILE],
    ["ZUVIX_QA_PROFILE", params.env?.ZUVIX_QA_PROFILE],
  ] as const;
...
function resolveQaEvidenceRunner(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  return params.env?.ZUVIX_QA_RUNNER?.trim() || params.fallback || "host";
}

function resolveQaEvidenceChannelDriver(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  const id =
    params.fallback?.trim() ||
    params.env?.ZUVIX_QA_CHANNEL_DRIVER?.trim() ||
    params.env?.ZUVIX_E2E_CHANNEL_DRIVER?.trim();
  return id ? { id } : undefined;
}

function resolveQaEvidenceEnvironment(env: NodeJS.ProcessEnv | undefined) {
  return {
    ref: env?.ZUVIX_QA_REF?.trim() || env?.GITHUB_SHA?.trim() || null,
    os: process.platform,
...
function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.ZUVIX_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.ZUVIX_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
  const explicitKind = env?.ZUVIX_QA_PACKAGE_SOURCE_KIND?.trim();
  const kind =
    explicitKind ||
