...
const QA_LAB_UI_OVERLAY_DIR = "/opt/zuvix-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        ZUVIX_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${params.bindUiDist ? `      - ${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - ZUVIX_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/zuvix-scaffold/zuvix.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://zuvix-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
...
${imageBlock}    pull_policy: never
    ...
    environment:
      ZUVIX_CONFIG_PATH: /tmp/zuvix/zuvix.json
      ZUVIX_STATE_DIR: /tmp/zuvix/state
      ZUVIX_NO_RESPAWN: "1"
      ZUVIX_SKIP_GMAIL_WATCHER: "1"
      ZUVIX_SKIP_BROWSER_CONTROL_SERVER: "1"
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/zuvix/workspace /tmp/zuvix/state && cp /opt/zuvix-scaffold/zuvix.json /tmp/zuvix/zuvix.json && cp -R /opt/zuvix-scaffold/seed-workspace/. /tmp/zuvix/workspace/ && ln -snf /opt/zuvix-repo /tmp/zuvix/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t zuvix:qa-local-prebaked --build-arg ZUVIX_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "zuvix:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "zuvix.json"),
  ];
