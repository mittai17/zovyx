...
function parseQaRuntimePair(value: string | undefined): [RuntimeId, RuntimeId] | undefined {
  ...
  if (runtimes.length !== 2) {
    throw new Error('--runtime-pair must use exactly two runtimes, e.g. "zuvix,codex".');
  }
  const [left, right] = runtimes;
  if (!left || !right) {
    throw new Error('--runtime-pair only supports "zuvix" and "codex".');
  }
  if (left === right) {
...
async function withTemporaryQaProfileEnv<T>(profile: string, run: () => Promise<T>): Promise<T> {
  ...
  process.env.ZUVIX_QA_PROFILE = profile;
  try {
    return await run();
  } finally {
...
export async function runQaJsonlReplayCommand(opts: {
  ...
  const repoRoot = path.resolve(opts.repoRoot ?? process.cwd());
  const runtimePair = parseQaRuntimePair(opts.runtimePair) ?? ["zuvix", "codex"];
  if (runtimePair[0] !== "zuvix" || runtimePair[1] !== "codex") {
    throw new Error('--runtime-pair for jsonl-replay must be "zuvix,codex".');
  }
...
export async function runQaLabUiCommand(opts: {
  ...
  const server = await startQaLabServer({
    ...
    controlUiProxyToken: process.env.ZUVIX_QA_CONTROL_UI_PROXY_TOKEN,
