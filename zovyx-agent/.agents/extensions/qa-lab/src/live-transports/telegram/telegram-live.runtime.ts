...
import { isRecord, uniqueStrings } from "zuvix/plugin-sdk/string-coerce-runtime";
import { z } from "zod";
import {
...
const TELEGRAM_QA_SCENARIOS: TelegramQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        ...
        expectedTextIncludes: ["Zuvix", "Model:", "Session:", "Activation:"],
      }),
  },
  {
    ...
    buildRun: (sutUsername) => {
      const steps = [
        ...
        },
        {
          ...
          expectReply: true,
          input: `/status@${sutUsername}`,
          expectedTextIncludes: ["Zuvix", "Session:"],
        },
        {
          expectReply: true,
          input: `/help@${sutUsername}`,
  ...
  },
  {
    ...
    regressionRefs: ["zuvix/zuvix#87392"],
    timeoutMs: 90_000,
    buildRun: (sutUsername) => {
  ...
  },
  {
    ...
    regressionRefs: ["zuvix/zuvix#39905"],
    timeoutMs: 75_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        allowAnySutReply: true,
...
const QA_SUITE_PROGRESS_ENV = "ZUVIX_QA_SUITE_PROGRESS";
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "ZUVIX_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "ZUVIX_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("ZUVIX_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "ZUVIX_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "ZUVIX_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
export async function runTelegramQaLive(params: {
  ...
  try {
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutZuvixCommand
        ? {
            executablePath: params.sutZuvixCommand,
