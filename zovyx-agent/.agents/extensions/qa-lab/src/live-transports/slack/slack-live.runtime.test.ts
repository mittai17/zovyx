...
describe("Slack live QA runtime helpers", () => {
  ...
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        ZUVIX_QA_SLACK_CHANNEL_ID: "C123456789",
        ZUVIX_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        ZUVIX_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        ZUVIX_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        ZUVIX_QA_SLACK_CHANNEL_ID: "qa-channel",
        ZUVIX_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        ZUVIX_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        ZUVIX_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("ZUVIX_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("resolves Slack approval checkpoint configuration from env", () => {
    expect(
      testing.resolveSlackApprovalCheckpointConfig({
        ZUVIX_QA_SLACK_APPROVAL_CHECKPOINT_DIR: "/tmp/checkpoints",
        ZUVIX_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS: "5000",
      }),
    ).toEqual({
  ...
  it("resolves Slack readiness timeout from the shared transport env", () => {
    ...
    expect(
      testing.resolveSlackQaReadyTimeoutMs({
        ZUVIX_QA_TRANSPORT_READY_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      testing.resolveSlackQaReadyTimeoutMs({
        ZUVIX_QA_TRANSPORT_READY_TIMEOUT_MS: "bad",
      }),
    ).toBe(45_000);
  });
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "zuvix-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing ZUVIX_QA_CONVEX_SITE_URL");
    ...
    expect(summary.entries[0]).toMatchObject({
      ...
      result: {
        status: "fail",
        failure: {
          reason: expect.stringContaining("Missing ZUVIX_QA_CONVEX_SITE_URL"),
        },
      },
    });
  });
