...
} from "@zuvix/whatsapp/api.js";
import { normalizeE164 } from "zuvix/plugin-sdk/account-resolution";
import type { ZuvixConfig } from "zuvix/plugin-sdk/config-contracts";
import { formatErrorMessage } from "zuvix/plugin-sdk/error-runtime";
import { normalizeStringEntries, uniqueStrings } from "zuvix/plugin-sdk/string-coerce-runtime";
...
const WHATSAPP_QA_CAPTURE_CONTENT_ENV = "ZUVIX_QA_WHATSAPP_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "ZUVIX_QA_REDACT_PUBLIC_METADATA";
...
const WHATSAPP_QA_SCENARIOS: WhatsAppQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      matchText: /Zuvix: access not configured|Pairing code:/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `zuvixqa reply with only this exact marker: ${replyToken}`,
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      expectReply: true,
      input: "/help",
      matchText: /Zuvix|commands|status|\/new/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => ({
      ...
      expectReply: true,
      input: "/status",
      matchText: /Zuvix|status|session|agent/iu,
      target: "dm",
    }),
  },
  {
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `zuvixqa reply with only this exact marker under groupPolicy open: ${token}`,
        matchText: token,
        target: "group",
      };
    },
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `zuvixqa groupPolicy disabled must not reply with ${token}`,
        matchText: token,
        target: "group",
      };
    },
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `zuvixqa blocked group should not reply with ${quietToken}`,
...
function inferWhatsAppCredentialSource(
  ...
  const normalized =
    value?.trim().toLowerCase() || env.ZUVIX_QA_CREDENTIAL_SOURCE?.trim().toLowerCase();
...
function resolveWhatsAppQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): WhatsAppQaRuntimeEnv {
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "ZUVIX_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "ZUVIX_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "ZUVIX_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "ZUVIX_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.ZUVIX_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "ZUVIX_QA_WHATSAPP",
  );
}
...
async function writeWhatsAppQaWorkspaceFixture(
  ...
  const fixtureDir = path.join(context.gatewayWorkspaceDir, ".zuvix", "qa-whatsapp-media");
...
export async function runWhatsAppQaLive(params: {
  ...
  try {
    ...
    tempAuthRoot = await fs.mkdtemp(
      path.join(resolvePreferredZuvixTmpDir(), "zuvix-whatsapp-qa-"),
    );
