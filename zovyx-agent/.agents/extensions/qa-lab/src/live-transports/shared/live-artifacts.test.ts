...
describe("live transport artifacts", () => {
  ...
  it("preserves cleanup phase labels while redacting details", () => {
    expect(
      ...
      "credential lease release: details redacted (ZUVIX_QA_REDACT_PUBLIC_METADATA=1)",
      "live gateway cleanup: details redacted (ZUVIX_QA_REDACT_PUBLIC_METADATA=1)",
    ]);
  });
    ...
    expect(
      redactQaLiveLaneIssues([
        [
          ...
          "Artifacts:",
          "- gatewayDebug: /tmp/zuvix-whatsapp-qa/gateway-debug",
        ].join("\n"),
      ]),
    ).toEqual(["details redacted (ZUVIX_QA_REDACT_PUBLIC_METADATA=1)"]);
  });
});
