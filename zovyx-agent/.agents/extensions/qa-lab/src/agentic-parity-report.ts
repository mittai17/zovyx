...
function scenarioHasRuntimeToolCallEvidence(scenario: QaParityReportScenario): boolean {
  ...
  return (
    ...
    parity.cells.zuvix.toolCalls.length > 0 &&
...
function describeLiveUsageFailure(scenarioName: string, scenario: QaRuntimeParityScenarioReport) {
  const missing = [
    scenario.zuvixTokens > 0
      ? undefined
      : `${scenario.zuvixStatus === "pass" ? "zuvix" : "zuvix failed"}=0`,
...
export function renderQaAgenticParityMarkdownReport(comparison: QaAgenticParityComparison): string {
  ...
  const lines = [
    `# Zuvix Agentic Parity Report — ${comparison.candidateLabel} vs ${comparison.baselineLabel}`,
    "",
...
export function buildQaRuntimeParityReport(params: {
  ...
  const scenarios: QaRuntimeParityScenarioReport[] = params.summary.scenarios.map((scenario) => {
    ...
    const zuvixCell = parity.cells.zuvix;
    const codexCell = parity.cells.codex;
    const zuvixStatus = runtimeParityCellStatus(zuvixCell);
    ...
    const reportScenario = {
      ...
      zuvixStatus,
      codexStatus,
      zuvixTokens: zuvixCell.usage.totalTokens,
      codexTokens: codexCell.usage.totalTokens,
      zuvixToolCalls: zuvixCell.toolCalls.length,
...
export function renderQaRuntimeParityMarkdownReport(report: QaRuntimeParityReport): string {
  const lines = [
    `# Zuvix Runtime Parity Report — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  for (const scenario of report.scenarios) {
    ...
    lines.push(
      `- zuvix: ${scenario.zuvixStatus} (${scenario.zuvixToolCalls} tool calls, ${scenario.zuvixTokens} tokens)`,
    );
    lines.push(
