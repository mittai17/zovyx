...
import { sleep } from "zuvix/plugin-sdk/runtime-env";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/zuvix-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/zuvix";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/zuvix-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `zuvix-qa-${createVmSuffix()}`;
...
export function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          ...
          key !== "ZUVIX_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `ZUVIX_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `ZUVIX_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm zuvix qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredZuvixTmpDir(), `${plan.vmName}-qa-suite-`),
  );
