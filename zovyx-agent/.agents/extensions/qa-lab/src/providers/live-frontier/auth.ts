...
} from "zuvix/plugin-sdk/provider-auth";
...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "ZUVIX_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "ZUVIX_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "ZUVIX_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["ZUVIX_LIVE_ANTHROPIC_KEY"],
  gemini: ["ZUVIX_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "ZUVIX_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: ZuvixConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: ZuvixConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: ZuvixConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.ZUVIX_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "zuvix") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or ZUVIX_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host Zuvix OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
