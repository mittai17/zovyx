    ...
    try {
      const response = await fetch(`${server.baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({
          model: "aimock/gpt-5.5",
          stream: false,
          input: [makeResponsesInput("@zuvix explain the QA lab")],
        }),
      });
      expect(response.status).toBe(200);
      ...
      const expectedBody = {
        model: "aimock/gpt-5.5",
        messages: [{ role: "user", content: "@zuvix explain the QA lab" }],
        stream: false,
        _endpointType: "chat",
      };
      expect(await debug.json()).toEqual([
        {
          raw: JSON.stringify(expectedBody),
          body: expectedBody,
          prompt: "@zuvix explain the QA lab",
          allInputText: "@zuvix explain the QA lab",
          toolOutput: "",
          model: "aimock/gpt-5.5",
          providerVariant: "openai",
          imageInputCount: 0,
