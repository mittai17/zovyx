...
import { escapeRegExp } from "zuvix/plugin-sdk/text-utility-runtime";
import { readRequestBodyWithLimit } from "zuvix/plugin-sdk/webhook-ingress";
...
const QA_GROUP_AUDIO_TRANSCRIPTION_TEXT =
  "zuvixqa reply with only this exact marker after group audio preflight: WHATSAPP_QA_GROUP_AUDIO_TRANSCRIPT_OK";
...
function buildQaToolSearchArgs(targetTool: string, failureMode: boolean): Record<string, unknown> {
  ...
  if (targetTool === "web_search") {
    return { query: "Zuvix runtime parity fixed query", count: 1 };
  }
...
function buildAssistantText(
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this Zuvix repo from source + docs before acting.`;
  }
  ...
  if (
    ...
    if (targetTool && hasDeclaredTool(body, "tool_search_code")) {
      return buildToolCallEventsWithArgs("tool_search_code", {
        code: [
          `const hits = await zuvix.tools.search(${JSON.stringify(targetTool)}, { limit: 1 });`,
          "const match = hits.find((tool) => tool.name === " + JSON.stringify(targetTool) + ");",
          "if (!match) throw new Error('target tool not found');",
          `return await zuvix.tools.call(match.id, ${JSON.stringify(plannedArgs)});`,
        ].join("\n"),
      });
    }
    ...
    if (
      ...
      return buildToolCallEventsWithArgs("write", {
        ...
        content: [
          ...
          "Trial prompt: Prepare a release readiness reply for a local Zuvix QA change.",
  ...
  if (/personal task followthrough check/i.test(allInputText)) {
    ...
    if (
      ...
      (!taskEvidenceText.includes("# Personal task ledger") &&
        !taskEvidenceText.includes("Task: prepare a local Zuvix PR readiness note."))
    ) {
