...
import { disposeRegisteredAgentHarnesses } from "zuvix/plugin-sdk/agent-harness";
...
} from "zuvix/plugin-sdk/qa-runtime";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.ZUVIX_QA_SUITE_PROGRESS);
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.ZUVIX_QA_DEBUG === "1") {
...
function buildRuntimeParityScenarioResult(params: {
  ...
  const zuvixCell = params.result.cells.zuvix;
  return {
    ...
    steps: [
      {
        name: zuvixCell.runtime,
        status:
          zuvixCell.runtimeErrorClass || zuvixCell.transportErrorClass ? "fail" : "pass",
        details: formatRuntimeParityCellDetails(zuvixCell),
      },
      {
...
function buildQaRuntimeEnvPatch(params: {
  ...
  if (params.forcedRuntime) {
    patch.ZUVIX_BUILD_PRIVATE_QA = "1";
    patch.ZUVIX_QA_FORCE_RUNTIME = params.forcedRuntime;
  }
  if (params.forcedRuntime !== "codex" || params.providerMode !== "mock-openai") {
  ...
  patch.ZUVIX_CODEX_APP_SERVER_ARGS = `app-server -c openai_base_url=${mockBaseUrl}/v1 --listen stdio://`;
...
function shouldCaptureGatewayHeapCheckpoints(env: NodeJS.ProcessEnv = process.env) {
  return parseQaSuiteBooleanEnv(env.ZUVIX_QA_GATEWAY_HEAP_CHECKPOINTS) === true;
}
...
export async function runQaFlowSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const enabledPluginIds = [
    ...new Set([
      ...
      ...(params?.forcedRuntime && params.forcedRuntime !== "zuvix"
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as ZuvixConfig
      : undefined,
  ...
  try {
    ...
    const keepTemp = process.env.ZUVIX_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
