...
describe("qa cli runtime", () => {
  ...
  beforeEach(async () => {
    ...
    listTelegramQaScenarioCatalog.mockReturnValue([
      {
        ...
        regressionRefs: ["zuvix/zuvix#74698"],
      },
    ]);
    ...
    writeQaDockerHarnessFiles.mockResolvedValue({
      outputDir: "/tmp/zuvix-repo/.artifacts/qa-docker",
    });
  ...
  it("dispatches a taxonomy-backed profile category through the suite runner", async () => {
    const previousProfile = process.env.ZUVIX_QA_PROFILE;
    process.env.ZUVIX_QA_PROFILE = "release";
    try {
      runQaSuite.mockImplementationOnce(async () => {
        expect(process.env.ZUVIX_QA_PROFILE).toBe("smoke-ci");
        await fs.writeFile(
          suiteEvidencePath,
          JSON.stringify(
            makeQaEvidence([
      ...
      expectFields(suiteArgs, {
        repoRoot: path.resolve("/tmp/zuvix-repo"),
        outputDir: path.resolve("/tmp/zuvix-repo", ".artifacts/qa-e2e/smoke-ci"),
        transportId: "qa-channel",
      ...
      expect(process.env.ZUVIX_QA_PROFILE).toBe("release");
      ...
      if (previousProfile === undefined) {
        delete process.env.ZUVIX_QA_PROFILE;
      } else {
        process.env.ZUVIX_QA_PROFILE = previousProfile;
      }
    }
  });
  ...
  it("resolves suite repo-root-relative paths before dispatching", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/zuvix-repo"),
      outputDir: path.resolve("/tmp/zuvix-repo", ".artifacts/qa/frontier"),
      transportId: "qa-channel",
      providerMode: "live-frontier",
      primaryModel: "openai/gpt-5.5",
      alternateModel: "anthropic/claude-sonnet-4-6",
  ...
  it("rejects unknown runtime-pair ids at the CLI boundary", async () => {
    ...
    ).rejects.toThrow('--runtime-pair only supports "zuvix" and "codex".');
    expect(runQaSuite).not.toHaveBeenCalled();
  });
  ...
  it("resolves telegram qa repo-root-relative paths before dispatching", async () => {
    ...
    expect(runTelegramQaLive).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/zuvix-repo"),
      outputDir: path.resolve("/tmp/zuvix-repo", ".artifacts/qa/telegram"),
      providerMode: "live-frontier",
      primaryModel: "openai/gpt-5.5",
      alternateModel: "openai/gpt-5.5",
      fastMode: true,
  ...
  it("rejects output dirs that escape the repo root", () => {
    expect(() => resolveRepoRelativeOutputDir("/tmp/zuvix-repo", "../outside")).toThrow(
      "--output-dir must stay within the repo root.",
    );
    expect(() => resolveRepoRelativeOutputDir("/tmp/zuvix-repo", "/tmp/outside")).toThrow(
  ...
  it("prints telegram scenario catalog without starting the live lane", async () => {
    ...
    expectWriteContains(
      stdoutWrite,
      "telegram-status-command\tdefault\tTelegram status command reply\tstatus rationale refs=zuvix/zuvix#74698",
    );
  });
  ...
  it("runs a host-only parity preflight against the sentinel scenario", async () => {
    const repoRoot = path.resolve("/tmp/zuvix-repo");
    await runQaSuiteCommand({
      repoRoot: "/tmp/zuvix-repo",
      providerMode: "mock-openai",
      primaryModel: "openai/gpt-5.5",
  ...
  it("prints a markdown tool coverage report from runtime tool fixtures", async () => {
    ...
    expectWriteContains(stdoutWrite, "# Zuvix Runtime Tool Coverage");
  ...
  it("writes a curated mock JSONL replay report and summary", async () => {
    ...
    try {
      ...
      expect(report).toContain("# Zuvix JSONL Replay Report - zuvix vs codex");
  ...
  it("resolves character eval paths and passes model refs through", async () => {
    ...
    expectFields(characterEvalArgs, {
      repoRoot: path.resolve("/tmp/zuvix-repo"),
      outputDir: path.resolve("/tmp/zuvix-repo", ".artifacts/qa/character"),
  ...
  it("routes suite runs through multipass when the runner is selected", async () => {
    ...
    expect(runQaMultipass).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/zuvix-repo"),
      outputDir: path.resolve("/tmp/zuvix-repo", ".artifacts/qa-multipass"),
      transportId: "qa-channel",
      providerMode: "mock-openai",
      primaryModel: undefined,
      alternateModel: undefined,
  ...
  it("resolves self-check repo-root-relative paths before starting the lab server", async () => {
    ...
    expect(startQaLabServer).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/zuvix-repo"),
      outputPath: path.resolve("/tmp/zuvix-repo", ".artifacts/qa/self-check.md"),
    });
  });
  ...
  it("resolves docker scaffold paths relative to the explicit repo root", async () => {
    ...
    expect(writeQaDockerHarnessFiles).toHaveBeenCalledWith({
      outputDir: path.resolve("/tmp/zuvix-repo", ".artifacts/qa-docker"),
      repoRoot: path.resolve("/tmp/zuvix-repo"),
  ...
  it("resolves docker up paths relative to the explicit repo root", async () => {
    ...
    expect(runQaDockerUp).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/zuvix-repo"),
      outputDir: path.resolve("/tmp/zuvix-repo", ".artifacts/qa-up"),
      gatewayPort: undefined,
      qaLabPort: undefined,
