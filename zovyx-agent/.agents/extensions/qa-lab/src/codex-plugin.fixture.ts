...
export const CODEX_PLUGIN_LIFECYCLE_MESSAGES = Object.freeze({
  missingPlugin:
    'Codex plugin is required for Codex runtime. Run "zuvix doctor --fix" to install @zuvix/codex, then retry.',
});
...
function formatPinnedOldRemediation(pluginVersion: string, hostVersion: string) {
  return `Codex plugin version ${pluginVersion} is older than Zuvix ${hostVersion}. Run "zuvix plugins update codex" or unpin codex, then rerun "zuvix doctor --fix".`;
}

function formatPinnedNewRemediation(pluginVersion: string, hostVersion: string) {
  return `Codex plugin version ${pluginVersion} requires a newer Zuvix host than ${hostVersion}. Upgrade Zuvix or install a codex plugin version pinned to ${hostVersion}.`;
}
...
export async function seedCodexPluginAt(
  ...
  await fs.writeFile(
    path.join(targetDir, "zuvix.plugin.json"),
