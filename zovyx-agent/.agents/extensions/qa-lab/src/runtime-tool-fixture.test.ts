...
describe("runtime tool fixture", () => {
  ...
  it("does not fail Codex-native fixtures solely because Zuvix dynamic exposure is absent", async () => {
    const env = await makeEnv({
      ...
      gateway: {
        ...
        tempRoot: "",
        workspaceDir: "",
        runtimeEnv: { ZUVIX_QA_FORCE_RUNTIME: "codex" },
        call: vi.fn(),
      },
    });
    ...
    expect(details).toContain("Zuvix dynamic exposure is intentionally omitted");
  ...
  it("still fails required Zuvix dynamic fixtures when the tool is absent", async () => {
    const env = await makeEnv();
