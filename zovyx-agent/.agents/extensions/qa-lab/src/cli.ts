...
export function registerQaLabCli(program: Command) {
  ...
  qa.command("suite")
    ...
    .option("--runtime-pair <pair>", "Run each scenario under both runtimes, e.g. zuvix,codex")
    .option(
  ...
  qa.command("jsonl-replay")
    ...
    .option("--runtime-pair <pair>", "Runtime pair label, e.g. zuvix,codex", "zuvix,codex")
    .option(
      "--provider-mode <mode>",
  ...
  credentials
    ...
    .option("--site-url <url>", "Override ZUVIX_QA_CONVEX_SITE_URL")
    .option("--endpoint-prefix <path>", "Override ZUVIX_QA_CONVEX_ENDPOINT_PREFIX")
  ...
  qa.command("docker-scaffold")
    ...
    .option("--image <name>", "Prebaked image name", "zuvix:qa-local-prebaked")
  ...
  qa.command("docker-build-image")
    ...
    .option("--repo-root <path>", "Repository root to target when running from a neutral cwd")
    .option("--image <name>", "Image tag", "zuvix:qa-local-prebaked")
