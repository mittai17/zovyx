...
import { fetchWithSsrFGuard } from "zuvix/plugin-sdk/ssrf-runtime";
import {
...
export type RuntimeId = "zuvix" | "codex";
...
export function isRuntimeParityResultPass(result: RuntimeParityResult) {
  return (
    ...
    isRuntimeParityCellPassable(result.cells.zuvix) &&
...
const HEARTBEAT_TRANSCRIPT_PROMPT = "[Zuvix heartbeat poll]";
...
function classifyRuntimeParityCells(params: {
  zuvix: RuntimeParityCell;
  codex: RuntimeParityCell;
  zuvixScenarioStatus: "pass" | "fail";
  codexScenarioStatus: "pass" | "fail";
}): Pick<RuntimeParityResult, "drift" | "driftDetails"> {
  if (
    isHardFailureRuntimeError(params.zuvix.runtimeErrorClass) ||
    isHardFailureRuntimeError(params.codex.runtimeErrorClass) ||
    params.zuvix.transportErrorClass ||
    params.codex.transportErrorClass
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.zuvix.transportErrorClass || params.codex.transportErrorClass
  ...
  if (
    hasMissingToolResult(params.zuvix.toolCalls) ||
  ...
  const zuvixTranscriptLines = params.zuvix.transcriptBytes.trim().length
    ? params.zuvix.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  const codexTranscriptLines = params.codex.transcriptBytes.trim().length
    ? params.codex.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  if (
    zuvixTranscriptLines !== codexTranscriptLines ||
    (!params.zuvix.finalText && Boolean(params.codex.finalText)) ||
    (Boolean(params.zuvix.finalText) && !params.codex.finalText)
  ) {
    return {
      drift: "structural",
      driftDetails: `transcript/final-text structure differs (${zuvixTranscriptLines} lines vs ${codexTranscriptLines})`,
    };
  }

  if (
    params.zuvixScenarioStatus === "fail" ||
    params.codexScenarioStatus === "fail" ||
    !isRuntimeParityCellPassable(params.zuvix) ||
    !isRuntimeParityCellPassable(params.codex)
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.zuvixScenarioStatus === params.codexScenarioStatus
          ? "at least one runtime failed"
          : `scenario status differs (${params.zuvixScenarioStatus} vs ${params.codexScenarioStatus})`,
    };
  }

  if (
    normalizeTextForParity(params.zuvix.finalText) ===
...
export async function runRuntimeParityScenario(params: {
  ...
  const zuvix = await params.runCell("zuvix");
  const codex = await params.runCell("codex");
  const drift = classifyRuntimeParityCells({
    zuvix: zuvix.cell,
    codex: codex.cell,
    zuvixScenarioStatus: zuvix.scenarioStatus,
