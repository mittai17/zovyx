...
import { resolveTimerTimeoutMs } from "zuvix/plugin-sdk/number-runtime";
...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    ZUVIX_HOME: params.homeDir,
    ZUVIX_CONFIG_PATH: params.configPath,
    ZUVIX_STATE_DIR: params.stateDir,
    ZUVIX_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    ZUVIX_NO_RESPAWN: "1",
    ZUVIX_TEST_FAST: "1",
    ZUVIX_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    ZUVIX_QA_PARENT_PID: String(process.pid),
    ZUVIX_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { ZUVIX_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    ZUVIX_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    ZUVIX_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { ZUVIX_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { ZUVIX_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: ZuvixConfig) => ZuvixConfig;
  runtimeEnvPatch?: NodeJS.ProcessEnv;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredZuvixTmpDir(), "zuvix-qa-suite-"),
  );
  ...
  const keepTemp = process.env.ZUVIX_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
