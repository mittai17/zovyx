...
export type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { zuvix: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { zuvix: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      zuvix: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.zuvix.push(parity.cells.zuvix);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# Zuvix JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
