...
describe("codex plugin lifecycle: pinned-old codex plugin with new Zuvix", () => {
  it("blocks with a precise update remediation when the plugin is older than the host", async () => {
    ...
    expect(result.remediation).toBe(
      'Codex plugin version 2026.5.19 is older than Zuvix 2026.5.21. Run "zuvix plugins update codex" or unpin codex, then rerun "zuvix doctor --fix".',
    );
  });
});

describe("codex plugin lifecycle: pinned-new codex plugin with old Zuvix", () => {
  it("blocks with a precise host-upgrade remediation when the plugin is newer than the host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toBe(
      "Codex plugin version 2026.5.22 requires a newer Zuvix host than 2026.5.21. Upgrade Zuvix or install a codex plugin version pinned to 2026.5.21.",
    );
  });
});
...
describe("codex plugin lifecycle: doctor migration safety matrix", () => {
  it.each([
    ...
    {
      name: "mixed profile with defaults Zuvix pin",
      profileShape: "mixed" as const,
      config: { agents: { defaults: { agentRuntime: { id: "zuvix" } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=zuvix"],
    },
    {
      name: "mixed profile with main-agent Zuvix pin",
      profileShape: "mixed" as const,
      config: { agents: { list: { main: { agentRuntime: { id: "zuvix" } } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=zuvix"],
    },
  ])(
    "keeps codex auth and strips stale Zuvix runtime pins for $name",
