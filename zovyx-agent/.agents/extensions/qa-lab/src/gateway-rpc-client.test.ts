...
vi.mock("zuvix/plugin-sdk/gateway-runtime", () => ({
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.ZUVIX_HOME = originalHome;
      }
    }

    expect(process.env.ZUVIX_HOME).toBe(originalHome);
  });
