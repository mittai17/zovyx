...
import { getExecApprovalReplyMetadata } from "zuvix/plugin-sdk/approval-runtime";
import {
...
import { sanitizeAssistantVisibleText } from "zuvix/plugin-sdk/text-chunking";
...
import { toGatewayAccount, writeZuvixConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped zuvix.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // zuvix.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeZuvixConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe zuvix.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeZuvixConfigThroughRuntime(getQQBotRuntime(), nextCfg as ZuvixConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as ZuvixConfig, accountId);
