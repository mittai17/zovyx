...
import { coerceSecretRef, normalizeSecretInputString } from "zuvix/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: ZuvixConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: ZuvixConfig): string {
...
/** Apply account config updates back into the Zuvix config object. */
