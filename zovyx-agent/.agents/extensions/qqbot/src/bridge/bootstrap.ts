...
let mediaRuntimeModulePromise: Promise<typeof import("zuvix/plugin-sdk/media-runtime")> | null =
  null;

const loadMediaRuntimeModule = async () => {
  mediaRuntimeModulePromise ??= import("zuvix/plugin-sdk/media-runtime");
...
function createBuiltinAdapter(): PlatformAdapter {
  return {
    ...
    async resolveApproval(approvalId: string, decision: string): Promise<boolean> {
      try {
        const { getRuntimeConfig } = await import("zuvix/plugin-sdk/runtime-config-snapshot");
        const { resolveApprovalOverGateway } =
          await import("zuvix/plugin-sdk/approval-gateway-runtime");
