...
import { withEnv } from "zuvix/plugin-sdk/test-env";
...
describe("qqbot legacy credential backup paths", () => {
  ...
  it("scopes legacy credential backup imports to the active ZUVIX_STATE_DIR", () => {
  ...
  it("keeps legacy account import paths isolated across different state directories", () => {
    ...
    const gatewayAPath = withEnv({ ZUVIX_STATE_DIR: stateDirA }, () =>
      getCredentialBackupFile("default"),
    );
    const gatewayBPath = withEnv({ ZUVIX_STATE_DIR: stateDirB }, () =>
      getCredentialBackupFile("default"),
    );
  ...
  it("uses ZUVIX_HOME for default legacy credential backup imports", () => {
    const homeDir = createTempDir("qqbot-zuvix-home-");
    withEnv({ ZUVIX_STATE_DIR: "", ZUVIX_HOME: homeDir }, () => {
      expect(getCredentialBackupFile("default")).toBe(
        path.join(homeDir, ".zuvix", "qqbot", "data", "credential-backup-default.json"),
      );
    });
  });
  ...
  it("expands tilde state-dir overrides through the canonical state resolver", () => {
    ...
    withEnv({ HOME: homeDir, ZUVIX_HOME: "", ZUVIX_STATE_DIR: "~/gateway-a" }, () => {
      expect(getCredentialBackupFile("default")).toBe(
