 ...
 * silk-wasm availability checks — all without importing `zuvix/plugin-sdk`.
 ...
 * `ZUVIX_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective Zuvix home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of Zuvix treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `zuvix` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `ZUVIX_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveZuvixHome(): string {
  const raw = process.env.ZUVIX_HOME?.trim();
...
/**
 * Return a legacy path under `~/.zuvix/qqbot` without creating it.
 *
...
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".zuvix", "qqbot", ...subPaths);
}

/** Return a path under `~/.zuvix/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<zuvix-home>/.zuvix/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Zuvix's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `ZUVIX_HOME` (when set) so files written by agents into
 * the Zuvix-managed media tree are reachable by this plugin even when
 * `HOME` and `ZUVIX_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveZuvixHome(), ".zuvix", "media", "qqbot", ...subPaths);
}

/** Return a path under `<zuvix-home>/.zuvix/media/qqbot`, creating it on demand. */
...
/**
 * Return `<zuvix-home>/.zuvix/media`, Zuvix's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `zuvix/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `ZUVIX_HOME`.
 */
function getZuvixMediaDir(): string {
  return path.join(resolveZuvixHome(), ".zuvix", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const zuvixHomeDir = resolveZuvixHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When ZUVIX_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the Zuvix-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".zuvix", "workspace", "qqbot"),
      path.join(zuvixHomeDir, ".zuvix", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Zuvix's shared `~/.zuvix/media`
  ...
  const allowedRoots = [getZuvixMediaDir(), getQQBotMediaPath()];
