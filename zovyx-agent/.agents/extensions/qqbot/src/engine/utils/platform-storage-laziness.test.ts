...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.zuvix/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".zuvix", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".zuvix", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "zuvix.sqlite");
