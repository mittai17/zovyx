...
let zuvixVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${pluginVersion} (Node/${process.versions.node}; ${os.platform()}; Zuvix/${zuvixVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; zuvixVersion?: string }): void {
  if (options.pluginVersion) {
    pluginVersion = options.pluginVersion;
  }
  if (options.zuvixVersion) {
    zuvixVersion = options.zuvixVersion;
  }
}

/** Update the Zuvix framework version in the User-Agent (called after runtime injection). */
export function setZuvixVersion(version: string): void {
  if (version) {
