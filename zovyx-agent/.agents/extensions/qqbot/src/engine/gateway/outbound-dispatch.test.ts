...
function makeRuntime(params: {
  ...
  return {
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/zuvix/qqbot-sessions.json"),
    ...
    tts: {
      textToSpeech: vi.fn(async () => ({
        success: true,
        audioPath: "/tmp/zuvix-qqbot/tts.wav",
        provider: "test-tts",
        outputFormat: "wav",
      })),
    },
...
describe("dispatchOutbound", () => {
  ...
  it("synthesizes plain audioAsVoice text as a QQ voice reply", async () => {
    ...
    expect(audioFileToSilkBase64Mock).toHaveBeenCalledWith("/tmp/zuvix-qqbot/tts.wav");
