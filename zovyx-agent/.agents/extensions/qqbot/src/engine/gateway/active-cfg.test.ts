...
const getRuntimeConfigMock = vi.hoisted(() => vi.fn<() => ZuvixConfig>());

vi.mock("zuvix/plugin-sdk/runtime-config-snapshot", () => ({
  getRuntimeConfig: getRuntimeConfigMock,
}));

function asCfg(shape: { bindings: Array<{ id: string }> }): ZuvixConfig {
  return shape as unknown as ZuvixConfig;
}
