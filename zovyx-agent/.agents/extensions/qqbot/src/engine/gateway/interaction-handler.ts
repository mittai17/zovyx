...
import { isImplicitSameChatApprovalAuthorization } from "zuvix/plugin-sdk/approval-auth-runtime";
...
async function handleApprovalButtonInteraction(params: {
  ...
  event: InteractionEvent;
  getActiveCfg?: () => ZuvixConfig | Record<string, unknown>;
  log?: EngineLogger;
  ...
  let cfg: ZuvixConfig;
  try {
    cfg = params.getActiveCfg() as ZuvixConfig;
  } catch (err) {
    await acknowledgeApprovalInteraction(params.creds, params.event, params.log, {
      content: "Approval is unavailable.",
    });
