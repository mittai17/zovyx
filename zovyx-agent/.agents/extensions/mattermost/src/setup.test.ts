...
import type { ZuvixConfig, ZuvixPluginApi } from "../runtime-api.js";
...
vi.mock("./setup.accounts.runtime.js", () => ({
  listMattermostAccountIds: vi.fn((cfg: ZuvixConfig) => {
  ...
  it("offers env shortcut only for the default account when env is present and config is empty", () => {
    ...
    expect(
      mattermostSetupWizard.envShortcut?.isAvailable?.({
        cfg: { channels: { mattermost: {} } } as ZuvixConfig,
        accountId: "default",
      } as never),
    ).toBe(true);
  ...
  it("keeps env shortcut as a no-op patch for the selected account", () => {
    expect(
      mattermostSetupWizard.envShortcut?.apply?.({
        cfg: { channels: { mattermost: { enabled: false } } } as ZuvixConfig,
        accountId: "default",
      } as never),
    ).toEqual({
      channels: {
