...
import { createInboundDebouncer } from "zuvix/plugin-sdk/channel-inbound-debounce";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { monitorMattermostProvider } from "./monitor.js";
import type { ZuvixConfig, RuntimeEnv } from "./runtime-api.js";
...
function createRuntimeCore(
  ...
  return {
    ...
    channel: {
      ...
      session: {
        resolveStorePath: () => "/tmp/zuvix-test-sessions.json",
...
describe("mattermost inbound user posts", () => {
  ...
  it("merges Mattermost progress preview updates by line identity", async () => {
    ...
    const progressConfig: ZuvixConfig = {
      channels: {
        mattermost: {
          enabled: true,
          baseUrl: "https://mattermost.example.com",
  ...
  it("does not drop inline command-looking group text from non-command-authorized senders", async () => {
    ...
    const abortController = new AbortController();
    mockState.abortController = abortController;
    const inlineCommandConfig: ZuvixConfig = {
  ...
  it("tags authorized typed text-slash control commands with CommandSource: text", async () => {
    ...
    const abortController = new AbortController();
    mockState.abortController = abortController;
    const directConfig: ZuvixConfig = {
      channels: {
        mattermost: {
          enabled: true,
          baseUrl: "https://mattermost.example.com",
  ...
  it("drops posts when neither REST nor websocket channel type can be resolved", async () => {
    ...
    const abortController = new AbortController();
    mockState.abortController = abortController;
    const channelTypeConfig: ZuvixConfig = {
      channels: {
        mattermost: {
          enabled: true,
          baseUrl: "https://mattermost.example.com",
  ...
  it("flushes pending group text before authorizing a bare abort without a mention", async () => {
    ...
    const abortController = new AbortController();
    mockState.abortController = abortController;
    const mentionConfig: ZuvixConfig = {
  ...
  it("pins direct-message main route updates to the configured owner", async () => {
    ...
    expect(recordCall?.storePath).toBe("/tmp/zuvix-test-sessions.json");
