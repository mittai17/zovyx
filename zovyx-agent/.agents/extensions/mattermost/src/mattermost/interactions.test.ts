  ...
  it("uses interactions.callbackBaseUrl when configured", () => {
    const url = resolveInteractionCallbackUrl("default", {
      channels: {
        mattermost: {
          interactions: {
            callbackBaseUrl: "https://gateway.example.com/zuvix",
          },
        },
      },
    });
    expect(url).toBe("https://gateway.example.com/zuvix/mattermost/interactions/default");
  });
...
describe("createMattermostInteractionHandler", () => {
  ...
  function createActionContext(actionId = "approve", channelId = "chan-1") {
    const context = { action_id: actionId, __zuvix_channel_id: channelId };
