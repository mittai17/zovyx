...
/**
 * Built-in Zuvix commands to register as native slash commands.
...
/**
 * Register all Zuvix slash commands for a given team.
...
export async function registerSlashCommands(params: {
  ...
  for (const spec of commands) {
    ...
    if (ownedCommands.length === 0 && foreignCommands.length > 0) {
      log?.(
        `mattermost: trigger /${spec.trigger} already used by non-Zuvix command(s); skipping to avoid mutating external integrations`,
      );
      continue;
    }
...
/**
 * Map the trigger word back to the original Zuvix command name.
