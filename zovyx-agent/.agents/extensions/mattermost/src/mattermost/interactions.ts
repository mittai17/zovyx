...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__zuvix_channel_id";
...
type InteractionCallbackConfig = Pick<ZuvixConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "zuvix-mattermost-interactions").update(botToken).digest("hex");
}
