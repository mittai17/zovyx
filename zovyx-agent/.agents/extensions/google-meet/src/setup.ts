...
import { isBlockedHostnameOrIp } from "zuvix/plugin-sdk/ssrf-runtime";
import { asRecord, normalizeOptionalString } from "zuvix/plugin-sdk/string-coerce-runtime";
...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the Zuvix browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the Zuvix browser profile; configure browser.defaultProfile to choose another profile",
  });
