...
import type { PluginRuntime, RuntimeLogger } from "zuvix/plugin-sdk/plugin-runtime";
import { sleep } from "zuvix/plugin-sdk/runtime-env";
import { normalizeOptionalString, uniqueStrings } from "zuvix/plugin-sdk/string-coerce-runtime";
import type {
  GoogleMeetConfig,
  GoogleMeetMode,
  GoogleMeetModeInput,
  ...
  if (health?.manualActionRequired) {
    return {
      ...
      message:
        ...
        "Resolve the Google Meet browser prompt before asking Zuvix to speak.",
    };
  }
  if (health?.inCall === true) {
    if (health.micMuted === true) {
      return {
        ...
        message: "Turn on the Zuvix Google Meet microphone before asking Zuvix to speak.",
      };
    }
    if (session.chrome.audioBridge) {
      return { ready: true };
