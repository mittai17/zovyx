...
import {
  ...
  formatHelpExamples,
  theme,
} from "zuvix/plugin-sdk/memory-core-host-runtime-cli";
import {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix memory status", "Show index and provider status."],
          [
            "zuvix memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["zuvix memory status --deep", "Probe embedding provider readiness."],
          ["zuvix memory index --force", "Force a full reindex."],
          ['zuvix memory search "meeting notes"', "Quick search using positional query."],
          [
            'zuvix memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `zuvix memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "zuvix memory rem-backfill --path ./memory",
          ...
          ["zuvix memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.zuvix.ai/cli/memory")}\n`,
    );
