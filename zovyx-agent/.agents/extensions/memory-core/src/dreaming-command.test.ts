...
function resolveStoredDreaming(config: ZuvixConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: ZuvixConfig = {}) {
  const registered: { command?: ZuvixPluginCommandDefinition } = {};
  let runtimeConfig: ZuvixConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: ZuvixConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: ZuvixConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: ZuvixPluginCommandDefinition) => {
