...
import { peekSystemEventEntries } from "zuvix/plugin-sdk/system-event-runtime";
...
export async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: ZuvixPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: ZuvixPluginApi): void {
  ...
  const resolveCurrentConfig = (): ZuvixConfig =>
    (api.runtime.config?.current?.() ?? api.config) as ZuvixConfig;
