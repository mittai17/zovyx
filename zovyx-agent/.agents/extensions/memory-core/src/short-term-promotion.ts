...
import type { MemorySearchResult } from "zuvix/plugin-sdk/memory-core-host-runtime-files";
import {
  DEFAULT_MEMORY_DEEP_DREAMING_MAX_PROMOTED_SNIPPET_TOKENS,
  formatMemoryDreamingDay,
  isSameMemoryDreamingDay,
} from "zuvix/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "zuvix/plugin-sdk/memory-host-events";
import {
...
const PROMOTION_MARKER_PREFIX = "zuvix-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*zuvix-memory-promotion:/i.test(snippet) ||
...
const DREAMING_FENCE_START_RE = /<!--\s*zuvix:dreaming:[a-z][a-z0-9-]*:start\s*-->/i;
const DREAMING_FENCE_END_RE = /<!--\s*zuvix:dreaming:[a-z][a-z0-9-]*:end\s*-->/i;
...
async function rehydratePromotionCandidate(
  ...
  for (const sourcePath of sourcePaths) {
    ...
    // content. If rehydration lands inside an zuvix:dreaming fence (for example
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*zuvix-memory-promotion:([^\n]*?)\s*-->/gi);
