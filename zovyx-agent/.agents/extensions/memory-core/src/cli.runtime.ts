...
import type { MemoryEmbeddingProbeResult } from "zuvix/plugin-sdk/memory-core-host-engine-storage";
import {
  resolveMemoryDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "zuvix/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "zuvix/plugin-sdk/routing";
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: zuvix memory status --index --agent ${agentId}`,
  };
}
...
function resolveMemoryPluginConfig(cfg: ZuvixConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredZuvixTmpDir(), "zuvix-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: ZuvixConfig): string {
...
function resolveAgent(cfg: ZuvixConfig, agent?: string) {
...
function resolveAgentIds(cfg: ZuvixConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: zuvix memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredZuvixTmpDir(), "zuvix-rem-backfill-"),
      );
      try {
