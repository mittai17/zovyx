...
import { resolveMemoryDreamingConfig } from "zuvix/plugin-sdk/memory-core-host-status";
import type { ZuvixPluginApi, PluginCommandContext } from "zuvix/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "zuvix/plugin-sdk/string-coerce-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: ZuvixConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: ZuvixConfig, enabled: boolean): ZuvixConfig {
...
function formatStatus(cfg: ZuvixConfig): string {
...
export async function handleDreamingCommand(api: ZuvixPluginApi, ctx: PluginCommandContext) {
...
export function registerDreamingCommand(api: ZuvixPluginApi): void {
  api.registerCommand({
    name: "dreaming",
