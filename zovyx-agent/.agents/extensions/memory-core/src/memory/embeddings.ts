...
} from "zuvix/plugin-sdk/embedding-providers";
import {
...
function createMissingLlamaCppProviderError(): Error {
  return new Error(
    [
      ...
      "Install it with: zuvix plugins install @zuvix/llama-cpp-provider",
      "Then restart Zuvix and retry: zuvix memory status --deep",
    ].join("\n"),
  );
}
