...
import type { EmbeddingProviderAdapter } from "zuvix/plugin-sdk/embedding-providers";
import type { MemoryEmbeddingProviderAdapter } from "zuvix/plugin-sdk/memory-core-host-engine-embeddings";
...
const mockEmbeddingRegistry = vi.hoisted(() => ({
  ...
  genericLookupConfigs: [] as Array<ZuvixConfig | undefined>,
}));

vi.mock("zuvix/plugin-sdk/embedding-providers", () => ({
  getEmbeddingProvider: (id: string, config?: ZuvixConfig) => {
...
vi.mock("zuvix/plugin-sdk/memory-core-host-engine-embeddings", () => ({
