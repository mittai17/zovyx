...
import { withFileLock } from "zuvix/plugin-sdk/file-lock";
import {
...
const QMD_UPDATE_QUEUE_KEY = Symbol.for("zuvix.qmdUpdateQueueState");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so Zuvix search semantics match direct qmd search.
  return trimmed;
}
...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private warnIfWatchPressure(count: number): void {
    warnIfMemoryWatchPressureHigh(
      ...
      count,
      "paths",
      "Large QMD collections can make Zuvix run out of file watchers or open files.",
