...
import type {
  MemorySearchConfig,
  ZuvixConfig,
} from "zuvix/plugin-sdk/memory-core-host-engine-foundation";
...
const {
  ...
  const chokidarKey = Symbol.for("zuvix.test.memoryWatchFactory");
  const nativeKey = Symbol.for("zuvix.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("zuvix.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("zuvix.test.memoryNativeWatchFactory");

vi.mock("zuvix/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("zuvix/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
} from "zuvix/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): ZuvixConfig {
    const defaults: NonNullable<NonNullable<ZuvixConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: ZuvixConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. Zuvix watches directories only so
