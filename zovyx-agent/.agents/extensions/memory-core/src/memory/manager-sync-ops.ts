...
import { classifyMemoryMultimodalPath } from "zuvix/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
import {
  ...
  sessionPathForFile,
} from "zuvix/plugin-sdk/memory-core-host-engine-qmd";
import {
  buildFileEntry,
...
const TEST_MEMORY_WATCH_FACTORY_KEY = Symbol.for("zuvix.test.memoryWatchFactory");
const TEST_MEMORY_NATIVE_WATCH_FACTORY_KEY = Symbol.for("zuvix.test.memoryNativeWatchFactory");
...
export abstract class MemoryManagerSyncOps {
  protected abstract readonly cfg: ZuvixConfig;
  ...
  private warnIfMemoryWatchPressure(count: number, unit: MemoryWatchPressureUnit): void {
    warnIfMemoryWatchPressureHigh(
      ...
      count,
      unit,
      "Large memory folders or extraPaths can make Zuvix run out of file watchers or open files.",
    ...
    try {
      if (needsFullReindex) {
        if (
          ...
          process.env.ZUVIX_TEST_MEMORY_UNSAFE_REINDEX === "1"
        ) {
