...
import { withMockedWindowsPlatform } from "zuvix/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("zuvix.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("zuvix.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("zuvix.qmdEmbedQueueTail");
...
vi.mock("zuvix/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("zuvix/plugin-sdk/memory-core-host-engine-foundation")
  >("zuvix/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/file-lock")>(
    "zuvix/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { ZuvixConfig } from "zuvix/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
...
describe("QmdMemoryManager", () => {
  ...
  it("rebinds a stale in-container collection root to the host workspace (sandbox-mode transition)", async () => {
    ...
    // /home/node/.zuvix/... stored that in-container path as the collection root. Resolved
    ...
    const containerRoot = "/home/node/.zuvix/teams/x/workspace";
  ...
  it("parseShownCollection extracts path and pattern from qmd collection show output", async () => {
    ...
    const sampleOutput = [
      ...
      "  Path:     /home/node/.zuvix/teams/example-team/workspace-example/memory",
      "  Pattern:  **/*.md",
      "  Include:  yes (default)",
    ].join("\n");

    const result = parser(sampleOutput);
    expect(result.path).toBe("/home/node/.zuvix/teams/example-team/workspace-example/memory");
