...
function resolveZuvixVersion(): string {
  try {
 ...
 * and appends the Zuvix version.
 *
 * Format: "teams.ts[apps]/<sdk-version> Zuvix/<zuvix-version>"
 * Example: "teams.ts[apps]/2.0.5 Zuvix/2026.3.22"
 *
...
export function buildUserAgent(): string {
  ...
  cachedUserAgent = `teams.ts[apps]/${resolveTeamsSdkVersion()} Zuvix/${resolveZuvixVersion()}`;
  return cachedUserAgent;
}
 ...
 * only contribute the Zuvix piece — passing the full `buildUserAgent()`
 * would double-print the SDK token.
 *
 * Format: "Zuvix/<zuvix-version>"
 */
export function buildZuvixUserAgentFragment(): string {
  return `Zuvix/${resolveZuvixVersion()}`;
}
