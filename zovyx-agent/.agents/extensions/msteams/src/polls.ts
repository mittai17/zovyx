...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["zuvixPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["zuvix", "pollId"]) ??
    readNestedString(value, ["zuvix", "poll", "id"]) ??
    readNestedString(value, ["data", "zuvixPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "zuvix", "pollId"]) ??
    // Action.Execute (Universal Action Model) payload shape: value.action.data
    readNestedString(value, ["action", "data", "zuvixPollId"]) ??
