...
/**
 * Bridges zuvix's reply pipeline callbacks to the SDK's `ctx.stream`.
...
export function createTeamsReplyStreamController(params: {
  ...
  // zuvix's reply pipeline calls onPartialReply with the cumulative text on
  ...
  return {
    ...
    onPartialReply(payload: { text?: string }): void {
      ...
      // mode, zuvix's pipeline doesn't deliver tokens — the model output
