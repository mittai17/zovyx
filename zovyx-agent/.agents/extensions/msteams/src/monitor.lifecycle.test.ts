...
function createConfig(port: number): ZuvixConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  cfg: ZuvixConfig,
  patch: NonNullable<NonNullable<ZuvixConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): ZuvixConfig {
  ...
  it("gates poll card votes before recording them", async () => {
    ...
    const response = await cardActionHandler({
      activity: {
        ...
        value: { action: { data: { zuvixPollId: "poll-1", choices: "0" } } },
      },
    });
