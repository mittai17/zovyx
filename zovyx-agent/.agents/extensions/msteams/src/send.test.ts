...
// pulls from `zuvix/plugin-sdk/outbound-media` (post-migration). Mock the
// canonical source so the re-export carries our stub through.
vi.mock("zuvix/plugin-sdk/outbound-media", () => ({
  loadOutboundMediaFromUrl: mockState.loadOutboundMediaFromUrl,
}));

vi.mock("zuvix/plugin-sdk/markdown-table-runtime", () => ({
  resolveMarkdownTableMode: mockState.resolveMarkdownTableMode,
}));

vi.mock("zuvix/plugin-sdk/text-chunking", async (importOriginal) => {
  const actual = await importOriginal<typeof import("zuvix/plugin-sdk/text-chunking")>();
  return {
    ...actual,
