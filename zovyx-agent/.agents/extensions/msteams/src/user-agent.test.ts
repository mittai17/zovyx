...
describe("buildUserAgent", () => {
  ...
  it("returns teams.ts[apps]/<sdk> Zuvix/<version> format", () => {
    const ua = buildUserAgent();
    expect(ua).toMatch(/^teams\.ts\[apps\]\/.+ Zuvix\/2026\.3\.19$/);
  });

  it("reflects the runtime version", () => {
    vi.mocked(getMSTeamsRuntime).mockReturnValue({ version: "1.2.3" } as never);
    const ua = buildUserAgent();
    expect(ua).toMatch(/Zuvix\/1\.2\.3$/);
  });

  it("returns Zuvix/unknown when runtime is not initialized", () => {
    vi.mocked(getMSTeamsRuntime).mockImplementation(() => {
      throw new Error("MSTeams runtime not initialized");
    });
    const ua = buildUserAgent();
    expect(ua).toMatch(/Zuvix\/unknown$/);
  ...
  it("sends the generated User-Agent in Graph requests by default", async () => {
    ...
    expect(init.headers["User-Agent"]).toMatch(/^teams\.ts\[apps\]\/.+ Zuvix\/2026\.3\.19$/);
  ...
  it("adds the generated User-Agent to Headers instances without overwriting callers", () => {
    ...
    expect(generated.get("User-Agent")).toMatch(/^teams\.ts\[apps\]\/.+ Zuvix\/2026\.3\.19$/);
