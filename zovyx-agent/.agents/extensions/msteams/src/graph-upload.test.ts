...
import { withFetchPreconnect } from "zuvix/plugin-sdk/test-env";
...
function expectGraphUploadFetch(fetchFn: ReturnType<typeof vi.fn>, expectedUrl: string): void {
  ...
  expect(init?.headers?.["User-Agent"]).toMatch(/^teams\.ts\[apps\]\/.+ Zuvix\/.+$/);
}
...
describe("graph upload helpers", () => {
  ...
  it("uploads to OneDrive with the personal drive path", async () => {
    ...
    expectGraphUploadFetch(
      fetchFn,
      "https://graph.microsoft.com/v1.0/me/drive/root:/ZuvixShared/a.txt:/content",
    );
  ...
  it("uploads to SharePoint with the site drive path", async () => {
    ...
    expectGraphUploadFetch(
      fetchFn,
      "https://graph.microsoft.com/v1.0/sites/site-123/drive/root:/ZuvixShared/b.txt:/content",
    );
    expect(result).toEqual({
      id: "item-2",
