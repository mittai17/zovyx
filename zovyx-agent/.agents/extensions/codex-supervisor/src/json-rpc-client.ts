...
export function resolveSafeApprovalResult(method: string): Record<string, unknown> | undefined {
  ...
  if (method.endsWith("/requestApproval")) {
    return {
      decision: "decline",
      reason: "Zuvix Codex supervisor does not grant native approvals.",
    };
  }
...
abstract class BaseCodexJsonRpcConnection implements CodexJsonRpcConnection {
  ...
  async initialize(): Promise<void> {
    await this.request("initialize", {
      clientInfo: {
        name: "zuvix-codex-supervisor",
        title: "Zuvix Codex Supervisor",
        version: "0.1.0",
      },
      capabilities: {
        ...
        JSON.stringify(
          result === undefined
            ? {
                ...
                error: {
                  code: -32601,
                  message: `Zuvix Codex supervisor cannot handle app-server request: ${method}`,
                },
              }
            : { id, result },
        ),
