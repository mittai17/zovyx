...
import type { PluginStateSyncKeyedStore } from "zuvix/plugin-sdk/plugin-state-runtime";
...
export function createCopilotAgentHarness(
  ...
  // Maps Zuvix session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  return {
    ...
    async runAttempt(params: AgentHarnessAttemptParams): Promise<AgentHarnessAttemptResult> {
      const attemptPromise = (async () => {
        ...
        const zuvixSessionId =
          typeof params.sessionId === "string" ? params.sessionId : undefined;

        // Dogfood finding #4: reuse the SDK session across turns within
        // the same Zuvix session so that the GitHub Copilot agent runtime's prompt
        ...
        const tracked = zuvixSessionId ? trackedSessions.get(zuvixSessionId) : undefined;
        const stored = zuvixSessionId
          ? resetBlockedStoredSessions.has(zuvixSessionId)
            ? undefined
            : lookupStoredBinding(options?.sessionStore, zuvixSessionId)
          : undefined;
        ...
        return runCopilotAttempt(effectiveParams, {
          pool,
          onSessionEstablished: zuvixSessionId
            ? ({
                sdkSessionId,
                pooledClient,
                sessionConfig,
                ...
                trackedSessions.set(zuvixSessionId, {
                  sdkSessionId,
                ...
                const persisted = registerStoredBinding(options?.sessionStore, zuvixSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                if (persisted) {
                  resetBlockedStoredSessions.delete(zuvixSessionId);
                }
              }
            : undefined,
        });
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      const zuvixSessionId = typeof params.sessionId === "string" ? params.sessionId : undefined;
      if (!zuvixSessionId) {
        return;
      }
      const tracked = trackedSessions.get(zuvixSessionId);
      if (deleteStoredBinding(options?.sessionStore, zuvixSessionId)) {
        resetBlockedStoredSessions.delete(zuvixSessionId);
      } else {
        resetBlockedStoredSessions.add(zuvixSessionId);
      }
      if (!tracked) {
        // Session was created by a different harness, or already reset.
        return;
      }
      trackedSessions.delete(zuvixSessionId);
      try {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. Zuvix only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // Zuvix session/transcript state stays in SQLite, with no marker
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with Zuvix hooks/tools, so preserve the
      ...
      try {
        ...
        if (isStaleSdkSessionError(err)) {
          trackedSessions.delete(zuvixSessionId);
          deleteStoredBinding(options?.sessionStore, zuvixSessionId);
          return {
            ok: false,
            compacted: false,
