 ...
 *     use cases.
 *   - Falls back to ZUVIX_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<zuvixHome>/.zuvix/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `ZUVIX_HOME` for the home directory root.
 ...
 *   4. ZUVIX_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with Zuvix's own files
  ...
  const zuvixHome = readString(args.env.ZUVIX_HOME);
  const rootHome = zuvixHome ? resolve(zuvixHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <zuvixHome>/.zuvix/agents/<agentId>/copilot
  return resolve(join(rootHome, ".zuvix", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // ZUVIX_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "ZUVIX_GITHUB_TOKEN", value: readString(env.ZUVIX_GITHUB_TOKEN) },
