 ...
 * Zuvix audit transcript that sits next to (but is distinct from) the
 * SDK's own session storage.
 *
 * The Zuvix shell (src/agents/command/attempt-execution.ts) already
 ...
 * assistant + toolResult) into the Zuvix transcript via the same
...
export function attachCopilotMirrorIdentity<T extends AgentMessage>(
  ...
  return {
    ...record,
    __zuvix: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}
 ...
 * the Zuvix audit trail just misses the intermediate messages for
 * this turn.
 */
