...
import type { AgentHarnessAttemptParams } from "zuvix/plugin-sdk/agent-harness-runtime";
...
vi.mock("zuvix/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("zuvix/plugin-sdk/agent-harness")>();

  return {
    ...actual,
    createZuvixCodingTools: vi.fn(() => [
      {
...
const LIVE = isLiveTestEnabled(["ZUVIX_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.ZUVIX_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "zuvix-copilot-live-"));
