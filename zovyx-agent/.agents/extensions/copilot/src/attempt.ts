...
// zuvix/plugin-sdk/agent-harness-runtime declares auth, messages,
// onAssistantDelta, and initialReplayState.sdkSessionId fields. Tracked by
// project zuvix-copilot-harness; reviewer-attempt-bridge note.
...
type AttemptParamsLike = AgentHarnessAttemptParams & {
  ...
  // current-turn user message for the Zuvix audit transcript so
...
export interface CopilotAttemptDeps {
   ...
   * `zuvix/plugin-sdk/agent-harness-runtime#resolveSandboxContext`, which is
   ...
   * harness uses this to track the zuvixSessionId -> sdkSessionId
...
export async function runCopilotAttempt(
  ...
  try {
    ...
    try {
      const toolBridge = await createToolBridge({
        ...
        // tool-bridge.ts buildZuvixCodingToolsOptions().
        attemptParams: input,
        sessionRef,
        onYieldDetected: () => {
          yieldDetected = true;
    ...
    // Load Zuvix workspace bootstrap files (SOUL.md, IDENTITY.md,
    ...
    // Zuvix bootstrap block (SDK still loads AGENTS.md natively).
    ...
    if (abortRequested || params.abortSignal?.aborted) {
      ...
      if (!bridge.recordSendResult(result) && !aborted) {
        ...
        // capability inventory. Do not call session.abort() here: Zuvix may
  ...
  //
  // Without this synthesis the Zuvix audit transcript never sees
  ...
  // (user/assistant/toolResult) into the Zuvix audit transcript at
  // params.sessionFile, alongside the SDK's own session storage. The
  // Zuvix shell (attempt-execution.ts) writes only the user prompt
  ...
  if (sessionFileForMirror && messagesSnapshot.length > 0) {
    ...
    await dualWriteCopilotTranscriptBestEffort({
      ...
      // authoritative; only the Zuvix audit transcript would be
...
function createSessionConfig(
  ...
  return {
    ...
    // tool-bridge.ts) so 100% of tool calls go through Zuvix's
    ...
    // requires routing the request to the Zuvix channel/TUI prompt
    ...
    // bypass Zuvix's wrapped-tool enforcement under any permissive
    ...
    ...(resolvedAuth.authMode === "gitHubToken" && resolvedAuth.gitHubToken
      ...
      : {}),
    // Zuvix workspace bootstrap plus per-turn runtime guidance
    // injected via the SDK's `systemMessage` field in append mode:
    // SDK foundation + Zuvix context. Append keeps every SDK
    ...
    // L1036). Omitted when there is no Zuvix-owned context so the
...
function hasMirrorIdentity(message: AgentMessage): boolean {
  const record = message as unknown as { __zuvix?: unknown };
