...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { zuvixDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { zuvixDefaultImporterSentinel?: boolean };

      expect(sdk.zuvixDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("zuvix plugins install @zuvix/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("zuvix plugins install @zuvix/copilot");
  });
  ...
  it("default fallback dir points at ~/.zuvix/npm-runtime/copilot", () => {
    expect(COPILOT_SDK_FALLBACK_DIR).toMatch(/\.zuvix[\\/]+npm-runtime[\\/]+copilot$/);
  });

  it("resolves the fallback dir from ZUVIX_STATE_DIR for relocated profiles", () => {
    expect(
      resolveCopilotSdkFallbackDir({
        ...process.env,
        ZUVIX_STATE_DIR: "/tmp/zuvix-state",
      }),
    ).toBe(path.join("/tmp/zuvix-state", "npm-runtime", "copilot"));
  });
