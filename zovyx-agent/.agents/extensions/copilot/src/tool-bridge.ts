...
type CreateZuvixCodingTools =
  (typeof import("zuvix/plugin-sdk/agent-harness"))["createZuvixCodingTools"];
type ZuvixCodingToolsOptions = NonNullable<Parameters<CreateZuvixCodingTools>[0]>;
 ...
 * authoritative `createZuvixCodingTools({...})` call shape).
 *
 * Declared as `Partial<EmbeddedRunAttemptParams>` (imported from the
 * `zuvix/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
export interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createZuvixCodingTools` so the
  ...
  createZuvixCodingTools?: (opts: unknown) => AnyAgentTool[] | Promise<AnyAgentTool[]>;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createZuvixCodingTools =
    input.createZuvixCodingTools ??
    (await import("zuvix/plugin-sdk/agent-harness")).createZuvixCodingTools;

  const toolOptions = buildZuvixCodingToolsOptions(input, effectiveToolPlan);

  let sourceTools: unknown;
  try {
    sourceTools = await createZuvixCodingTools(toolOptions);
  } catch (error: unknown) {
    throw createError(
      `[copilot-tool-bridge] createZuvixCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[copilot-tool-bridge] createZuvixCodingTools must return an array of tools",
    );
  }
  ...
  return {
    sdkTools: filteredTools.map((sourceTool) =>
      convertZuvixToolToSdkTool(sourceTool, {
...
/**
 * Builds the full `createZuvixCodingTools` options bag mirroring the
 * PI in-tree call at `src/agents/pi-embedded-runner/run/attempt.ts:1029-1117`.
 *
 * Why PI parity matters: bridged Zuvix tools register with the SDK
 * as `overridesBuiltInTool: true, skipPermission: true` (see
 * `convertZuvixToolToSdkTool` below). That means the wrapped-tool
 * enforcement layer
...
function buildZuvixCodingToolsOptions(
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as ZuvixCodingToolsOptions["modelCompat"])
      : undefined;
...
export function convertZuvixToolToSdkTool(
  sourceTool: AnyAgentTool,
  ctx: {
    abortSignal?: AbortSignal;
  ...
  return {
    ...
    // Zuvix owns its bridged tools by design (the harness docs:
    // "Zuvix still owns ... Zuvix dynamic tools (bridged)"). The bundled
    // Copilot CLI ships built-in tools whose names (edit, read, write, bash,
    // ...) collide with Zuvix's coding-tool set. Mark every bridged tool as
    ...
    // same name." Zuvix's tool layer is the source of truth for these
    // names within a copilot attempt.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Bridged Zuvix tools enforce their own permission/policy decisions
    ...
    // `kind: "custom-tool"` would either short-circuit Zuvix's richer
    // enforcement (if we allow-all) or block every call (if we
    // reject-all) — neither matches PI parity. The in-tree codex harness
    // takes the same approach: bridged Zuvix tools are wrapped with
