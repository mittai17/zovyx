...
import type { AnyAgentTool, SandboxContext } from "zuvix/plugin-sdk/agent-harness-runtime";
import { afterEach, describe, expect, it, vi } from "vitest";
import {
  createCopilotToolBridge,
  convertZuvixToolToSdkTool,
  supportsModelTools,
} from "./tool-bridge.js";
...
describe("supportsModelTools", () => {
  it("returns true for github-copilot and false otherwise", () => {
    ...
    expect(supportsModelTools("zuvix")).toBe(false);
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createZuvixCodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createZuvixCodingTools).toHaveBeenCalledTimes(0);
  });

  it("forwards supported fields to injected createZuvixCodingTools", async () => {
    ...
    expect(createZuvixCodingTools).toHaveBeenCalledTimes(1);
    ...
    expect(createZuvixCodingTools).toHaveBeenCalledWith(
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createZuvixCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("throws when createZuvixCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createZuvixCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createZuvixCodingTools must return an array");
  });

  it("throws when createZuvixCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createZuvixCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged Zuvix tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createZuvixCodingTools,
        getOpts: () =>
          (createZuvixCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createZuvixCodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building Zuvix tools", async () => {
      const { createZuvixCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createZuvixCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createZuvixCodingTools", async () => {
  ...
  // Zuvix attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createZuvixCodingTools", async () => {
      const createZuvixCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("filters constructed tools to exactly the allowlist when toolsAllow is narrow", async () => {
      const createZuvixCodingTools = vi.fn(async () => [
        makeTool({ name: "read" }),
        makeTool({ name: "edit" }),
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createZuvixCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createZuvixCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeZuvixTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeZuvixTools).toBe(true);
    });
...
describe("convertZuvixToolToSdkTool", () => {
  it("throws on empty and non-string names", () => {
    expect(() => convertZuvixToolToSdkTool(makeTool({ name: "" as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
    expect(() => convertZuvixToolToSdkTool(makeTool({ name: 42 as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
  });

  it("throws on non-function execute", () => {
    expect(() => convertZuvixToolToSdkTool(makeTool({ execute: "nope" as never }), {})).toThrow(
  ...
  it("preserves name, description, and parameters exactly", () => {
    ...
    const result = convertZuvixToolToSdkTool(sourceTool, {});
  ...
  it("sets skipPermission: true so Zuvix's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", () => {
    // Per the harness docs: every bridged Zuvix tool comes from
    // `createZuvixCodingTools`, which already wraps each tool with
  ...
  it("marks every bridged tool as overridesBuiltInTool so Zuvix owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", () => {
    // Real-world dogfood found that zuvix's createZuvixCodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = convertZuvixToolToSdkTool(makeTool({ name }), {}) as SdkTool & {
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = convertZuvixToolToSdkTool(sourceTool, { abortSignal: controller.signal });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertZuvixToolToSdkTool(sourceTool, { beforeExecute });
  ...
  it("returns a failure result when beforeExecute throws", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertZuvixToolToSdkTool(sourceTool, {
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = convertZuvixToolToSdkTool(makeTool({}, sourceResult), { onAgentToolResult });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = convertZuvixToolToSdkTool(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("reports returned Zuvix error results as observer failures", async () => {
    const onAgentToolResult = vi.fn();
    const sourceResult = {
      content: [{ text: '{"status":"error","error":"backend unavailable"}', type: "text" }],
      details: { status: "error", error: "backend unavailable" },
    };
    const sdkTool = convertZuvixToolToSdkTool(makeTool({}, sourceResult), {
      onAgentToolResult,
    });
