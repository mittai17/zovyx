...
import type { AgentMessage } from "zuvix/plugin-sdk/agent-harness-runtime";
import {
...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-copilot-mirror-"));
  tempDirs.push(dir);
...
describe("mirrorCopilotTranscript", () => {
  it("mirrors user, assistant, and tool result messages into the Zuvix transcript", async () => {
  ...
  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("zuvix-copilot-mirror-missing-dir-");
  ...
  it("uses attached identity instead of content fingerprint when provided", async () => {
    ...
    await mirrorCopilotTranscript({
      sessionFile,
      messages: [tagged],
      idempotencyScope: "copilot:zuvix-session-1",
    });

    const raw = await fs.readFile(sessionFile, "utf8");
    expect(raw).toContain(
      '"idempotencyKey":"copilot:zuvix-session-1:sdk-session-1:assistant:0"',
    );
