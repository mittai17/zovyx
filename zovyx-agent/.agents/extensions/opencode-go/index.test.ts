...
import type { ProviderRuntimeModel } from "zuvix/plugin-sdk/plugin-entry";
import {
  registerProviderPlugin,
  registerSingleProviderPlugin,
} from "zuvix/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "zuvix/plugin-sdk/provider-auth-runtime";
import { clearLiveCatalogCacheForTests } from "zuvix/plugin-sdk/provider-catalog-live-runtime";
...
import zuvixGoProviderDiscovery from "./provider-discovery.js";
...
describe("zuvix-go provider plugin", () => {
  beforeEach(() => {
    clearLiveCatalogCacheForTests();
  });

  it("registers image media understanding through the Zuvix Go plugin", async () => {
    ...
    const mediaProvider = mediaProviders.find((provider) => provider.id === "zuvix-go");
    if (!mediaProvider) {
      throw new Error("Expected zuvix-go media provider");
    }
  ...
  it("keeps Zuvix Go catalog coverage aligned with upstream", async () => {
    ...
    for (const modelId of expectedModelIds) {
      ...
      if (!model) {
        throw new Error(`expected Zuvix Go model ${modelId}`);
      }
    ...
    expect(kimi.baseUrl).toBe("https://zuvix.ai/zen/go/v1");
    ...
    expect(minimax.baseUrl).toBe("https://zuvix.ai/zen/go");
    ...
    expect(minimaxM3.baseUrl).toBe("https://zuvix.ai/zen/go");
    ...
    expect(mimoPro.baseUrl).toBe("https://zuvix.ai/zen/go/v1");
    ...
    expect(qwenMax.baseUrl).toBe("https://zuvix.ai/zen/go");
    ...
    expect(qwenPlus.baseUrl).toBe("https://zuvix.ai/zen/go");

    const qwen37Plus = requireMapEntry(models, "qwen3.7-plus");
    expect(qwen37Plus.api).toBe("anthropic-messages");
    expect(qwen37Plus.baseUrl).toBe("https://zuvix.ai/zen/go");
    ...
    expect(dynamicModel.provider).toBe("zuvix-go");
    expect(dynamicModel.baseUrl).toBe("https://zuvix.ai/zen/go/v1");
  ...
  it("loads Zuvix Go model discovery through the provider runtime", () => {
    expect(manifest.providerCatalogEntry).toBe("./provider-discovery.ts");
    expect(manifest.modelCatalog.discovery["zuvix-go"]).toBe("runtime");
  });

  it("exposes the complete offline catalog through provider discovery", async () => {
    const result = await zuvixGoProviderDiscovery.staticCatalog?.run({} as never);
    if (!result || !("provider" in result)) {
      throw new Error("expected Zuvix Go static provider");
    }
  ...
  it("skips live Zuvix Go catalog discovery when no shared key is configured", async () => {
    const provider = await registerSingleProviderPlugin(plugin);
      ...
      const result = await provider.catalog?.run({
        ...
        resolveProviderApiKey: (providerId: string) =>
          providerId === "zuvix-go"
            ...
            : {
                apiKey: "shared-zuvix-key",
                discoveryApiKey: "shared-zuvix-key",
              },
        resolveProviderAuth: () => ({ apiKey: undefined, mode: "none", source: "none" }),
      } as never);

      if (!result || !("provider" in result)) {
        throw new Error("expected Zuvix Go provider result");
      }
  ...
  it("uses cached live Zuvix Go discovery and falls back to static rows on failure", async () => {
    const fetchGuard = vi.fn(async () => ({
      ...
      finalUrl: "https://zuvix.ai/zen/go/v1/models",
      release: vi.fn(async () => undefined),
    }));

    const first = await buildOpencodeGoLiveProviderConfig({
      apiKey: "OPENCODE_API_KEY",
      discoveryApiKey: "resolved-zuvix-key",
      fetchGuard,
    });
  ...
  it("disables invalid DeepSeek V4 reasoning_effort off payloads on Zuvix Go", async () => {
    const provider = await registerSingleProviderPlugin(plugin);
    ...
    await streamFn?.(
      { provider: "zuvix-go", id: "deepseek-v4-flash" } as never,
      {} as never,
      {},
    );
  ...
  it("strips unsupported Kimi reasoning payloads on Zuvix Go", async () => {
    const provider = await registerSingleProviderPlugin(plugin);
    const capturedPayloads: Record<string, unknown>[] = [];
    const baseStreamFn = (_model: unknown, _context: unknown, options: unknown) => {
      const payload = {
    ...
    expect(streamFn).toBeTypeOf("function");
    await streamFn?.(
      { provider: "zuvix-go", id: "kimi-k2.6", api: "openai-completions" } as never,
      {} as never,
      {},
    );
  ...
  it("canonicalizes stale Zuvix Go base URLs", async () => {
    const provider = await registerSingleProviderPlugin(plugin);
    ...
    const normalizedConfig = requireRecord(
      provider.normalizeConfig?.({
        ...
        providerConfig: {
          ...
          baseUrl: "https://zuvix.ai/go/v1/",
          models: [],
        },
      } as never),
      "normalized config",
    );
    expect(normalizedConfig.baseUrl).toBe("https://zuvix.ai/zen/go/v1");
    ...
    expect(normalizedModel.baseUrl).toBe("https://zuvix.ai/zen/go/v1");
    ...
    const normalizedKimi = requireRecord(
      provider.normalizeResolvedModel?.({
        ...
        model: {
          ...
          name: "Kimi K2.6",
          api: "openai-completions",
          baseUrl: "https://zuvix.ai/zen/go/v1",
          reasoning: true,
          input: ["text", "image"],
          cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
          contextWindow: 262_144,
    ...
    expect(
      provider.normalizeTransport?.({
        ...
        baseUrl: "https://zuvix.ai/go",
      } as never),
    ).toEqual({
      api: "anthropic-messages",
      baseUrl: "https://zuvix.ai/zen/go",
    });
  });
});
