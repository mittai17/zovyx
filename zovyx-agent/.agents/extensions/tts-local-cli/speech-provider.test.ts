...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "zuvix/plugin-sdk/speech-core";
...
function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.ZUVIX_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("zuvix/plugin-sdk/media-runtime")>(
