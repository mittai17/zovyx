...
import type { AgentToolResult } from "zuvix/plugin-sdk/agent-core";
import {
  optionalFiniteNumberSchema,
  optionalPositiveIntegerSchema,
} from "zuvix/plugin-sdk/channel-actions";
import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "zuvix/plugin-sdk/chat-channel-ids";
import type { ZuvixConfig } from "zuvix/plugin-sdk/config-contracts";
import type { MemoryEmbeddingProvider } from "zuvix/plugin-sdk/memory-core-host-engine-embeddings";
import { MESSAGE_TOOL_DELIVERY_HINTS } from "zuvix/plugin-sdk/message-tool-delivery-hints";
import {
  parseStrictPositiveInteger,
  resolveTimerTimeoutMs,
} from "zuvix/plugin-sdk/number-runtime";
import { readFiniteNumberParam, readPositiveIntegerParam } from "zuvix/plugin-sdk/param-readers";
import { resolveLivePluginConfigObject } from "zuvix/plugin-sdk/plugin-config-runtime";
import { ensureGlobalUndiciEnvProxyDispatcher } from "zuvix/plugin-sdk/runtime-env";
import {
  asOptionalRecord as asRecord,
  normalizeLowercaseStringOrEmpty,
} from "zuvix/plugin-sdk/string-coerce-runtime";
import { truncateUtf16Safe } from "zuvix/plugin-sdk/text-utility-runtime";
import { Type } from "typebox";
import { definePluginEntry, type ZuvixPluginApi } from "./api.js";
import {
...
let memoryEmbeddingProviderModulePromise:
  | Promise<typeof import("zuvix/plugin-sdk/memory-core-host-engine-embeddings")>
  | undefined;
function loadMemoryEmbeddingProviderModule(): Promise<
  typeof import("zuvix/plugin-sdk/memory-core-host-engine-embeddings")
> {
  memoryEmbeddingProviderModulePromise ??=
    import("zuvix/plugin-sdk/memory-core-host-engine-embeddings");
  return memoryEmbeddingProviderModulePromise;
}

let memoryHostCoreModulePromise:
  | Promise<typeof import("zuvix/plugin-sdk/memory-host-core")>
  | undefined;
function loadMemoryHostCoreModule(): Promise<
  typeof import("zuvix/plugin-sdk/memory-host-core")
> {
  memoryHostCoreModulePromise ??= import("zuvix/plugin-sdk/memory-host-core");
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as ZuvixConfig;
...
function createEmbeddings(api: ZuvixPluginApi, cfg: MemoryConfig): Embeddings {
...
/**
 * Matches JSON blobs that look like Zuvix transport envelope metadata.
...
/**
 * Returns true if `text` looks like it contains Zuvix-injected envelope or
...
/**
 * Strips Zuvix-injected envelope metadata from a user message so that only
    ...
    const resolveCurrentHookConfig = () => {
      const runtimePluginConfig = resolveLivePluginConfigObject(
        api.runtime.config?.current
          ? () => api.runtime.config.current() as ZuvixConfig
          : undefined,
        "memory-lancedb",
