...
import { runPluginCommandWithTimeout } from "zuvix/plugin-sdk/run-command";
import type { RuntimeEnv } from "zuvix/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "zuvix/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredZuvixTmpDir(), "zuvix-signal-"));
