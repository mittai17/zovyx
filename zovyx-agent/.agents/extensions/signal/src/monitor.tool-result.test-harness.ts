...
import type { MockFn } from "zuvix/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/zuvix-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("zuvix/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/security-runtime")>(
    "zuvix/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/system-event-runtime")>(
    "zuvix/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("zuvix/plugin-sdk/reply-runtime"),
      import("zuvix/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
