...
export function createCodexCliSessionNodeHostCommands(): ZuvixPluginNodeHostCommand[] {
  return [
    {
...
export function createCodexCliSessionNodeInvokePolicies(): ZuvixPluginNodeInvokePolicy[] {
  return [
    {
...
async function runCodexExecResume(params: {
  ...
  const outputPath = path.join(
    await fs.mkdtemp(path.join(resolvePreferredZuvixTmpDir(), "zuvix-codex-cli-")),
    "last-message.txt",
  );
  try {
    const args = [
