...
import { loadAuthProfileStoreWithoutExternalProfiles } from "zuvix/plugin-sdk/agent-runtime";
import {
  createMigrationItem,
  markMigrationItemConflict,
  markMigrationItemError,
  markMigrationItemSkipped,
} from "zuvix/plugin-sdk/migration";
...
type AgentDefaultModelConfigs = NonNullable<
  NonNullable<NonNullable<ZuvixConfig["agents"]>["defaults"]>["models"]
>;
...
function replaceConfigDraft(draft: ZuvixConfig, next: ZuvixConfig): void {
  for (const key of Object.keys(draft) as Array<keyof ZuvixConfig>) {
    delete draft[key];
  }
...
function existingAuthProfileConfigIsCompatible(
  existing: NonNullable<NonNullable<ZuvixConfig["auth"]>["profiles"]>[string],
...
function hasCurrentAuthProfileConfigConflict(
  ...
  try {
    config = (ctx.runtime?.config?.current?.() as ZuvixConfig | undefined) ?? config;
  } catch {
...
function applyDefaultModelIfMissing(cfg: ZuvixConfig): ZuvixConfig {
...
async function applyCodexAuthProfileConfig(
  ctx: MigrationProviderContext,
  profile: CodexAuthProfileConfig,
  applyConfig: (config: ZuvixConfig) => ZuvixConfig,
