...
/**
 * Lightweight read/write surface over the Zuvix config file. Plugged in by
...
// Plugin lifecycle changes (enable/disable) write to zuvix.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in zuvix.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
export function buildPluginsHelp(): string {
  return [
    "Codex sub-plugin management (writes only to ~/.zuvix/zuvix.json, never to ~/.codex/config.toml):",
...
export function formatPluginList(
  ...
  return [
    "Codex sub-plugins in Zuvix config (~/.zuvix/zuvix.json):",
    "",
    ...rows.map(
      (r) =>
