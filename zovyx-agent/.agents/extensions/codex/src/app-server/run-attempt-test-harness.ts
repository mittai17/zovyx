...
import { resetDiagnosticEventsForTest } from "zuvix/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "zuvix/plugin-sdk/hook-runtime";
import { clearMemoryPluginState } from "zuvix/plugin-sdk/memory-core-host-runtime-core";
import { clearPluginCommands } from "zuvix/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("ZUVIX_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(resolvePreferredZuvixTmpDir(), "zuvix-codex-run-"));
  });
  ...
  afterEach(async () => {
    ...
    testing.resetZuvixCodingToolsFactoryForTests();
