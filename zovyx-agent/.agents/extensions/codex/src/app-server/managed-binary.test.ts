...
describe("managed Codex app-server binary", () => {
  ...
  it("resolves the plugin-local bundled Codex binary", async () => {
    const pluginRoot = path.join("/tmp", "zuvix", "extensions", "codex");
  ...
  it("resolves Windows Codex command shims", () => {
    const pluginRoot = path.win32.join("C:\\", "Zuvix", "dist", "extensions", "codex");
  ...
  it("uses the package root when the resolver is bundled into a dist chunk", () => {
    expect(testing.resolveDefaultCodexPluginRoot("/repo/zuvix/dist")).toBe("/repo/zuvix");
    expect(testing.resolveDefaultCodexPluginRoot("/repo/zuvix/dist-runtime")).toBe(
      "/repo/zuvix",
    );
    expect(
      testing.resolveDefaultCodexPluginRoot("/repo/zuvix/extensions/codex/src/app-server"),
    ).toBe("/repo/zuvix/extensions/codex");
  });

  it("finds Codex in the package install root used by packaged plugins", async () => {
    const installRoot = path.join("/tmp", "zuvix-plugin-package", "codex");
  ...
  it("finds Codex bins hoisted into an isolated npm project root", async () => {
    const projectRoot = path.join("/tmp", "state", "npm", "projects", "zuvix-codex-hash");
    const pluginRoot = path.join(projectRoot, "node_modules", "@zuvix", "codex");
  ...
  it("finds Windows Codex shims hoisted into an isolated npm project root", async () => {
    ...
    const pluginRoot = path.win32.join(projectRoot, "node_modules", "@zuvix", "codex");
  ...
  it("falls back to the resolved Codex package bin when no command shim exists", async () => {
    const installRoot = await mkdtemp(path.join(os.tmpdir(), "zuvix-codex-package-"));
    const pluginRoot = path.join(installRoot, "dist", "extensions", "codex");
  ...
  it("fails clearly when the managed Codex binary is missing", async () => {
    await expect(
      resolveManagedCodexAppServerStartOptions(startOptions("managed"), {
        platform: "darwin",
        pluginRoot: path.join("/tmp", "zuvix", "extensions", "codex"),
