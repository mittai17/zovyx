...
import { parseStrictNonNegativeInteger } from "zuvix/plugin-sdk/number-runtime";
...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `Zuvix dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `Zuvix dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
export async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    const message = "Zuvix dynamic tool call aborted before execution.";
  ...
  const abortFromRun = () => {
    const message = "Zuvix dynamic tool call aborted.";
    params.onFallbackSelected?.();
  ...
  try {
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("Zuvix dynamic tool call finished."));
    }
  }
}
...
function readDynamicToolResponseText(response: CodexDynamicToolCallResponse): string {
  const text = response.contentItems
    ...
    .join("\n")
    .trim();
  return text || "Zuvix dynamic tool call failed.";
}
...
/** Strips Zuvix-only metadata before sending a dynamic tool response to Codex. */
...
type TerminalDynamicToolReleaseState = {
  ...
  pendingZuvixDynamicToolCompletionIdsCount: number;
};
...
export function shouldReleaseTurnAfterTerminalDynamicTool(
  ...
  return (
    ...
    state.pendingZuvixDynamicToolCompletionIdsCount === 0
  );
}
