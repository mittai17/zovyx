...
import { SessionManager } from "zuvix/plugin-sdk/agent-sessions";
import {
  onInternalDiagnosticEvent,
  waitForDiagnosticEventsDrained,
  type DiagnosticEventPayload,
} from "zuvix/plugin-sdk/diagnostic-runtime";
import { initializeGlobalHookRunner, registerInternalHook } from "zuvix/plugin-sdk/hook-runtime";
import { registerMemoryCapability } from "zuvix/plugin-sdk/memory-core-host-runtime-core";
import { MESSAGE_TOOL_DELIVERY_HINTS } from "zuvix/plugin-sdk/message-tool-delivery-hints";
import { registerPluginCommand } from "zuvix/plugin-sdk/plugin-runtime";
import { createMockPluginRegistry } from "zuvix/plugin-sdk/plugin-test-runtime";
import { describe, expect, it, vi } from "vitest";
import WebSocket from "ws";
import { CODEX_GPT5_BEHAVIOR_CONTRACT } from "../../prompt-overlay.js";
import { defaultCodexAppInventoryCache } from "./app-inventory-cache.js";
import {
  buildCodexZuvixPromptContext,
  buildCodexSystemPromptReport,
  buildCodexWorkspaceBootstrapContext,
  getCodexWorkspaceMemoryToolNames,
  prependCodexZuvixPromptContext,
} from "./attempt-context.js";
import { resolveCodexAppServerEnvApiKeyCacheKey } from "./auth-bridge.js";
import { CodexAppServerRpcError } from "./client.js";
import { readCodexPluginConfig, resolveCodexAppServerRuntimeOptions } from "./config.js";
import {
  CODEX_ZUVIX_DYNAMIC_TOOL_NAMESPACE,
...
async function buildCodexTurnContextForTest(
  ...
  const threadDeveloperInstructions = [
    ...
    .join("\n\n");
  const zuvixPromptContext = buildCodexZuvixPromptContext({
    params,
    workspacePromptContext: workspaceBootstrapContext.promptContext,
  });
  const codexTurnPromptText = prependCodexZuvixPromptContext(
    params.prompt,
    zuvixPromptContext,
  );
...
describe("runCodexAppServerAttempt", () => {
  ...
  it("starts active Zuvix sandbox threads with Codex native execution disabled", async () => {
    testing.setZuvixCodingToolsFactoryForTests(() => [
  ...
  it("routes native Codex execution through an Zuvix sandbox exec-server when opted in", async () => {
    const appServer = {
    ...
    try {
      ...
      expect(environmentAddParams?.environmentId).toMatch(/^zuvix-sandbox-/);
  ...
  it("starts Codex threads without duplicate Zuvix workspace tools by default", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("includes Codex app-server scoped plugin command guidance in developer instructions", () => {
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "Zuvix main command guidance.",
          surfaces: ["zuvix_main"],
        },
      ],
    ...
    expect(instructions).not.toContain("Zuvix main command guidance.");
  });

  it("passes Zuvix skills as turn collaboration developer instructions", async () => {
    const llmInput = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "llm_input", handler: llmInput }]),
    );
    vi.stubEnv("ZUVIX_TRAJECTORY", "1");
    vi.stubEnv("ZUVIX_TRAJECTORY_DIR", path.join(tempDir, "trajectory"));
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).toContain("## Zuvix Skills");
    expect(collaborationInstructions).toContain("<available_skills>");
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).not.toContain("## Zuvix Skills");
    ...
    expect(compiledContext?.data?.systemPrompt).toContain("## Zuvix Skills");
  ...
  it("emits TUI-compatible tool events for Codex dynamic tool calls", async () => {
    ...
    await expect(
      ...
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown Zuvix tool: python" }],
    });
    await harness.completeTurn({ threadId: "thread-1", turnId: "turn-1" });
    await run;
    ...
    expect(onRunAgentEvent).toHaveBeenCalledWith({
      ...
      data: {
        ...
        isError: true,
        result: {
          content: [{ type: "text", text: "Unknown Zuvix tool: python" }],
        },
      },
    });
  ...
  it("keeps leading delivery hints out of the Codex current user request", async () => {
    for (const [index, deliveryHint] of MESSAGE_TOOL_DELIVERY_HINTS.entries()) {
      ...
      const inputText = turnStartParams.input?.[0]?.text ?? "";
      expect(inputText).toContain("Zuvix delivery metadata:");
  ...
  it("keeps searchable Zuvix dynamic tools when code-mode-only is enabled", () => {
    const tools = [
      createRuntimeDynamicTool("message"),
    ...
    expect(webSearch?.namespace).toBe(CODEX_ZUVIX_DYNAMIC_TOOL_NAMESPACE);
    expect(webSearch?.deferLoading).toBe(true);
    expect(heartbeat?.namespace).toBe(CODEX_ZUVIX_DYNAMIC_TOOL_NAMESPACE);
    expect(heartbeat?.deferLoading).toBe(true);
    expect(sessionsSpawn?.namespace).toBe(CODEX_ZUVIX_DYNAMIC_TOOL_NAMESPACE);
  ...
  it("registers heartbeat response durably without advertising it on normal turns", async () => {
    testing.setZuvixCodingToolsFactoryForTests((options) => [
      createRuntimeDynamicTool("message"),
    ...
    expect(normalInstructions).toContain(
      "Deferred searchable Zuvix dynamic tools available: message.",
    );
    expect(normalInstructions).not.toContain(
      "Deferred searchable Zuvix dynamic tools available: heartbeat_respond",
    );
  ...
  it("fails closed for Codex app defaults when restricted native tools have no plugin config", async () => {
    testing.setZuvixCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
    const params = createParams(
      path.join(tempDir, "session.jsonl"),
      path.join(tempDir, "workspace"),
    );
  ...
  it("applies before_prompt_build to Codex developer instructions and turn input", async () => {
    ...
    const wrappedPluginSystemContext = (text: string) =>
      `---\n\nZuvix plugin-injected system context. This block is not workspace file content.\n\n${text}\n\n---`;
  ...
  it("projects bounded continuity when starting Codex without a native thread binding", async () => {
    ...
    const harness = createStartedThreadHarness();
    const params = createParams(sessionFile, workspaceDir);
    params.prompt = "make the default webpage zuvix";
    ...
    expect(inputText).toContain("Zuvix assembled context for this turn:");
    expect(inputText).toContain("we are fixing the Opik default project");
    expect(inputText).toContain("Opik default project context");
    expect(inputText).toContain("Current user request:");
    expect(inputText).toContain("make the default webpage zuvix");
  });
  ...
  it("does not replay mirrored history already covered by an existing Codex binding", async () => {
    ...
    expect(inputText).not.toContain("Zuvix assembled context for this turn:");
  ...
  it("projects only newer visible history when a resumed Codex binding is stale", async () => {
    ...
    const copilotMirrorMessage = {
      ...
      __zuvix: { mirrorIdentity: "copilot:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __zuvix: { mirrorIdentity: string } };
  ...
  it("does not project Codex mirrored transcript echoes as stale binding continuity", async () => {
    ...
    const codexMirrorAssistantMessage = {
      ...
      __zuvix: { mirrorIdentity: "codex-app-server:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __zuvix: { mirrorIdentity: string } };
  ...
  it("does not project mirrored messages on consecutive resumes", async () => {
    ...
    const firstInputText =
      (firstTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(firstInputText).toContain("Zuvix assembled context for this turn:");
    ...
    const secondInputText =
      (secondTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(secondInputText).not.toContain("Zuvix assembled context for this turn:");
  ...
  it("passes stable workspace files as Codex developer instructions and routes MEMORY.md through tools", async () => {
    ...
    expect(threadDeveloperInstructions).toContain("Zuvix Workspace Instructions");
    ...
    expect(collaborationInstructions).toContain("Zuvix Agent Soul");
    ...
    expect(collaborationInstructions).toContain("Zuvix Workspace Memory");
    ...
    expect(inputText).not.toContain("Zuvix runtime context for this turn:");
    ...
    expect(inputText).not.toContain("Zuvix Workspace Memory");
  ...
  it("adds memory recall guidance when dated memory notes exist without root MEMORY.md", async () => {
    ...
    expect(collaborationInstructions).toContain("memory_search");
    expect(collaborationInstructions).toContain("memory_get");
    expect(collaborationInstructions).not.toContain("Zuvix Workspace Memory");
  ...
  it("sends workspace bootstrap instructions through Codex app-server payloads", async () => {
    ...
    expect(threadStartParams.developerInstructions).toContain("Zuvix Workspace Instructions");
  ...
  it("routes MEMORY.md through memory_get when search is unavailable", async () => {
    ...
    await fs.writeFile(path.join(workspaceDir, "MEMORY.md"), memorySummary);
    registerMemoryPromptForTest();
    testing.setZuvixCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("memory_get")]);
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    setAgentWorkspaceForTest(params, workspaceDir);
  ...
  it("omits heartbeat Codex workspace pointers for empty HEARTBEAT.md files", async () => {
    ...
    expect(collaborationInstructions).toContain("This is an Zuvix heartbeat turn");
    expect(collaborationInstructions).not.toContain("HEARTBEAT.md exists");
  });

  it("keeps lightweight cron Codex turns out of Zuvix bootstrap context", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const exactCommand =
      "cd /Users/phaedrus/Projects/zuvix && /Users/phaedrus/clawd/scripts/clawsweeper-related-scan.py";
    await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("keeps lightweight cron delivery hints byte-for-byte without Zuvix prompt context", async () => {
  ...
  it("promotes implicit Codex yolo approval policy when Zuvix tool policy exists", async () => {
    initializeGlobalHookRunner(
    ...
    expect(info).toHaveBeenCalledWith(
      "codex app-server approval policy promoted for Zuvix tool policy",
      {
        from: "never",
        to: "untrusted",
  ...
  it("keeps explicit Codex yolo mode unpromoted when Zuvix tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("keeps normalized full exec mode unpromoted when Zuvix tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("ignores invalid Codex app-server env overrides when promoting tool policy approval", async () => {
    ...
    vi.stubEnv("ZUVIX_CODEX_APP_SERVER_MODE", " ");
    vi.stubEnv("ZUVIX_CODEX_APP_SERVER_APPROVAL_POLICY", "always");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const harness = createStartedThreadHarness();
