...
async function createCodexSideToolBridge(input: {
  ...
  if (supportsModelTools(runtimeModel)) {
    const createZuvixCodingTools = (await import("zuvix/plugin-sdk/agent-harness"))
...
async function handleSideDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedSideDynamicToolResponse("Zuvix dynamic tool call aborted before execution.");
  }
  ...
  const timeoutPromise = new Promise<CodexDynamicToolCallResponse>((resolve) => {
    ...
    timeout = setTimeout(() => {
      controller.abort(new Error(`Zuvix dynamic tool call timed out after ${timeoutMs}ms.`));
      resolve(
        failedSideDynamicToolResponse(`Zuvix dynamic tool call timed out after ${timeoutMs}ms.`),
      );
    }, timeoutMs);
    timeout.unref?.();
  });
