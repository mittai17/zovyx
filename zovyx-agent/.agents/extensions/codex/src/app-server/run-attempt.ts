...
import { loadExecApprovals } from "zuvix/plugin-sdk/exec-approvals-runtime";
import { pathExists } from "zuvix/plugin-sdk/security-runtime";
import {
  resolveCodexAppServerForModelProvider,
  resolveCodexAppServerForZuvixToolPolicy,
...
import {
  ...
  resolveZuvixCodingToolsSessionKeys,
  resetZuvixCodingToolsFactoryForTests,
  setZuvixCodingToolsFactoryForTests,
...
export async function runCodexAppServerAttempt(
  ...
  let configuredAppServer = resolveCodexAppServerRuntimeOptions({
    ...
    config: params.config,
    agentDir,
    zuvixSandboxActive: sandbox?.enabled === true,
  });
  ...
  let policyAppServer = resolveCodexAppServerForZuvixToolPolicy({
  ...
  if (configuredAppServer.approvalPolicy === "never" && appServer.approvalPolicy === "untrusted") {
    embeddedAgentLog.info("codex app-server approval policy promoted for Zuvix tool policy", {
      from: "never",
      to: "untrusted",
  ...
  policyAppServer = resolveCodexAppServerForZuvixToolPolicy({
    appServer: configuredAppServer,
    pluginConfig,
    env: process.env,
    shouldPromote:
  ...
  // Zuvix transcript is persistence/search state. Context-engine output is
  ...
  const decorateCodexTurnPromptText = (prompt: string) =>
    prependCodexZuvixPromptContext(prompt, zuvixPromptContext, {
  ...
  const pendingZuvixDynamicToolCompletionIds = new Set<string>();
  ...
  const releaseTurnAfterTerminalDynamicTool = (paramsValue: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingZuvixDynamicToolCompletionIdsCount: pendingZuvixDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
  ...
  const handleNotification = async (notification: CodexServerNotification) => {
    ...
    const notificationState = applyCodexTurnNotificationState({
      ...
      activeTurnItemIds,
      activeAppServerTurnRequests,
      pendingZuvixDynamicToolCompletionIds,
  ...
  const requestCleanup = client.addRequestHandler(async (request) => {
    ...
    try {
      ...
      pendingZuvixDynamicToolCompletionIds.add(call.callId);
      ...
      try {
        ...
        pendingZuvixDynamicToolCompletionIds.delete(call.callId);
  ...
  if (activeNativeTurnIds.length > 0) {
    ...
    // Starting an Zuvix turn before that native turn completes can wedge the
  ...
  try {
    ...
    if (isCodexActiveCompactTurnError(turnStartError)) {
      ...
      // the next Zuvix turn collides with that compact turn, wait for the
    ...
    if (
      ...
      // Do not try to pre-compact or summarize through Zuvix here. Codex owns
      // automatic compaction; Zuvix may only discard a stale projection thread
  ...
  try {
    ...
    const canUseCompletedAssistantTextAfterClientClose =
      ...
      pendingZuvixDynamicToolCompletionIds.size === 0;
...
function handleApprovalRequest(params: {
  ...
  execPolicy?: Pick<ZuvixExecPolicyForCodexAppServer, "mode">;
