...
import type { ManagedProcess, ZuvixExecServer, ProcessChunk } from "./types.js";
...
async function runProcess(
  ...
  if (!backend) {
    throw new Error("Zuvix sandbox backend is unavailable.");
  }
  ...
  if (!command) {
    throw new Error("Zuvix sandbox exec spec did not provide a command.");
  }
