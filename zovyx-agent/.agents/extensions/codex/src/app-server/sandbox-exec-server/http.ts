 ...
 * access through the active Zuvix sandbox backend.
 */
...
import { SsrFBlockedError, isBlockedHostnameOrIp } from "zuvix/plugin-sdk/ssrf-runtime";
...
import type { HttpHeader, ZuvixExecServer } from "./types.js";
...
async function runStreamingSandboxHttpRequest(
  ...
  if (!command) {
    throw new Error("Zuvix sandbox HTTP exec spec did not provide a command.");
  }
...
tmp=$(mktemp "$TMPDIR/zuvix-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/zuvix-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
