 ...
 * Zuvix-owned tools, Codex native-tool fallback rules, sandbox shell shims,
...
import { resolveAgentDir } from "zuvix/plugin-sdk/agent-runtime";
import { isToolAllowed } from "zuvix/plugin-sdk/sandbox";
...
type ZuvixCodingToolsOptions = NonNullable<
  Parameters<(typeof import("zuvix/plugin-sdk/agent-harness"))["createZuvixCodingTools"]>[0]
>;

/** Factory seam for constructing Zuvix runtime tools without eagerly loading agent-harness. */
export type ZuvixCodingToolsFactory =
  (typeof import("zuvix/plugin-sdk/agent-harness"))["createZuvixCodingTools"];
type ZuvixDynamicTool = ReturnType<ZuvixCodingToolsFactory>[number];
type ZuvixSandboxContext = Awaited<ReturnType<typeof resolveSandboxContext>>;
...
let zuvixCodingToolsFactoryForTests: ZuvixCodingToolsFactory | undefined;

/** Overrides the runtime tool factory for tests that need deterministic tool catalogs. */
export function setZuvixCodingToolsFactoryForTests(factory: ZuvixCodingToolsFactory): void {
  zuvixCodingToolsFactoryForTests = factory;
}

/** Clears the test-only runtime tool factory override. */
export function resetZuvixCodingToolsFactoryForTests(): void {
  zuvixCodingToolsFactoryForTests = undefined;
}

/** Splits sandbox and run session keys so tool calls can bind to both scopes when needed. */
export function resolveZuvixCodingToolsSessionKeys(
  params: EmbeddedRunAttemptParams,
  sandboxSessionKey: string,
): Pick<ZuvixCodingToolsOptions, "sessionKey" | "runSessionKey"> {
  return {
...
export async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const createZuvixCodingTools =
    zuvixCodingToolsFactoryForTests ??
    (await import("zuvix/plugin-sdk/agent-harness")).createZuvixCodingTools;
  toolBuildStages.mark("load-agent-harness-tools");
  const sessionKeys = resolveZuvixCodingToolsSessionKeys(params, input.sandboxSessionKey);
  const allTools = createZuvixCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as ZuvixCodingToolsOptions["modelCompat"])
        : undefined,
  ...
  toolBuildStages.mark("create-zuvix-coding-tools");
...
export function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow Zuvix allowlists must fail closed rather than
...
/** Returns true when Zuvix policy requires the Node-owned exec/process tools instead. */
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  sandbox: ZuvixSandboxContext | undefined,
  ...
  // active Zuvix sandboxing must disable the native surface and route shell
...
/** Requires a Codex sandbox environment only when native tools must run inside Zuvix sandboxing. */
...
/** Converts Zuvix sandbox networking into Codex's external-sandbox policy shape. */
export function resolveCodexExternalSandboxPolicyForZuvixSandbox(
  sandbox: ZuvixSandboxContext | undefined,
): CodexSandboxPolicy {
  return {
    type: "externalSandbox",
    networkAccess: codexNetworkAccessForZuvixSandbox(sandbox) ? "enabled" : "restricted",
  };
}

function codexNetworkAccessForZuvixSandbox(
  sandbox: ZuvixSandboxContext | undefined,
): boolean {
...
export function addSandboxShellDynamicToolsIfAvailable(
  filteredTools: ZuvixDynamicTool[],
  allTools: ZuvixDynamicTool[],
  ...
  const sandboxExecTool: ZuvixDynamicTool = {
    ...execTool,
    name: "sandbox_exec",
    description:
      "Run a shell command through Zuvix's configured sandbox backend for this session. Use when Zuvix sandboxing is active or when a command must execute in the sandbox backend, such as an SSH-backed sandbox or Docker container-path bind layout. Use Codex's native shell only when no Zuvix sandbox is active and native Code Mode is available.",
  ...
  const sandboxProcessTool: ZuvixDynamicTool = {
    ...processTool,
    name: "sandbox_process",
    description:
      "Manage sandbox_exec sessions that were started through Zuvix's configured sandbox backend for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for sandbox_exec follow-up; use Codex's native shell session handling only when no Zuvix sandbox is active and native Code Mode is available.",
  };
