...
} from "zuvix/plugin-sdk/agent-harness-exec-review-runtime";
/**
 * Bridges Codex app-server approval requests into Zuvix policy hooks and
 * plugin approval UX.
 */
import {
...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  try {
    const policyOutcome = await runZuvixToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
    ...
    // Native hook/model policy did not decide; fall back to the Zuvix
...
/** Converts an Zuvix approval outcome into the app-server method response. */
...
async function runInternalExecAutoReviewForApprovalRequest(params: {
  ...
  return {
    ...
    reason: `Codex app-server command approval granted by Zuvix exec auto-reviewer: ${formatCodexDisplayText(
      decision.rationale,
    )}`,
  };
}
...
async function runZuvixToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  context: ApprovalContext;
  ...
  const policyRequest = buildZuvixToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `Zuvix native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        formatErrorMessage(error),
      )}`,
    };
  }
...
function readNativeRelayPreToolUseDecision(
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "Zuvix native hook relay failed for Codex app-server approval.",
    };
  }
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "Zuvix native hook policy denied Codex app-server approval.",
    };
  }
  ...
  return {
    blocked: true,
    reason: output
      ? "Zuvix native hook relay returned a non-deny Codex app-server approval decision."
...
function buildZuvixToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,
