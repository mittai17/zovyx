...
} from "zuvix/plugin-sdk/exec-approvals-runtime";
import { resolvePositiveTimerTimeoutMs } from "zuvix/plugin-sdk/number-runtime";
import { normalizeAgentId } from "zuvix/plugin-sdk/routing";
import { normalizeTrimmedStringList } from "zuvix/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "zuvix/plugin-sdk/windows-spawn";
import { z } from "zod";
import type { CodexSandboxPolicy, CodexServiceTier } from "./protocol.js";

const START_OPTIONS_KEY_SECRET_SYMBOL = Symbol.for("zuvix.codexAppServerStartOptionsKeySecret");
...
type ZuvixExecMode = "deny" | "allowlist" | "ask" | "auto" | "full";
type ZuvixExecSecurity = "deny" | "allowlist" | "full";
type ZuvixExecAsk = "off" | "on-miss" | "always";
type ZuvixExecApprovalFloorsForCodexAppServer = {
  security?: ZuvixExecSecurity;
  ask?: ZuvixExecAsk;
};
export type ZuvixExecPolicyForCodexAppServer = {
  mode?: ZuvixExecMode;
  security: ZuvixExecSecurity;
  ask: ZuvixExecAsk;
  touched: boolean;
};
type ZuvixExecPolicy = ZuvixExecPolicyForCodexAppServer;
...
export function resolveCodexAppServerRuntimeOptions(
  ...
  const envCommand = readNonEmptyString(env.ZUVIX_CODEX_APP_SERVER_BIN);
  ...
  const args = resolveArgs(config.args, env.ZUVIX_CODEX_APP_SERVER_ARGS);
  const headers = normalizeHeaders(config.headers);
  const clearEnv = normalizeStringList(config.clearEnv);
  const authToken = readNonEmptyString(config.authToken);
  const url = readNonEmptyString(config.url);
  const execMode = resolveEffectiveZuvixExecModeForCodexAppServer({
    execMode: params.execMode,
    execPolicy: params.execPolicy,
  });
  assertCodexAppServerAllowedForZuvixExecMode(execMode);
  const explicitPolicyMode =
    resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.ZUVIX_CODEX_APP_SERVER_MODE);
  const configuredSandbox =
    resolveSandbox(config.sandbox) ?? resolveSandbox(env.ZUVIX_CODEX_APP_SERVER_SANDBOX);
  const explicitApprovalsReviewer = resolveApprovalsReviewer(config.approvalsReviewer);
  const normalizedPolicyMode = resolveCodexPolicyModeForZuvixExecMode(execMode);
  ...
  const execModeRequiringPromptingApprovals: Extract<ZuvixExecMode, "auto" | "ask"> | undefined =
        ...
        sandbox: preserveExplicitAutoSandbox
          ...
          : forceDangerFullAccessSandbox
            ? selectForcedDangerFullAccessSandbox({
                configuredSandbox,
                defaultPolicy,
                zuvixSandboxActive: Boolean(params.zuvixSandboxActive),
              })
  ...
  const envApprovalPolicy = resolveApprovalPolicy(env.ZUVIX_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.ZUVIX_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.ZUVIX_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.ZUVIX_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.ZUVIX_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.ZUVIX_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.ZUVIX_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.ZUVIX_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.ZUVIX_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
...
function resolveDefaultCodexAppServerPolicy(params: {
  ...
  execModeRequiringPromptingApprovals?: Extract<ZuvixExecMode, "auto" | "ask">;
  execModeRequiringUserReviewer?: ZuvixExecMode;
  env?: NodeJS.ProcessEnv;
  requirementsToml?: string | null;
  requirementsPath?: string;
  readRequirementsFile?: (path: string) => string | undefined;
...
function selectGuardianApprovalsReviewer(
  ...
  execModeRequiringAutoReviewer?: Extract<ZuvixExecMode, "auto">,
...
export function resolveZuvixExecModeFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): ZuvixExecMode | undefined {
  const policy = resolveZuvixExecPolicyFromConfig(params);
  return policy.touched ? policy.mode : undefined;
}

function resolveZuvixExecPolicyFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): ZuvixExecPolicy {
  const root = readRecord(params.config);
  const globalExec = readRecord(readRecord(root?.tools)?.exec);
  const globalPolicy = applyZuvixExecPolicyLayer(createDefaultZuvixExecPolicy(), globalExec);
  ...
  return applyZuvixExecPolicyLayer(globalPolicy, agentExec);
}

export function resolveZuvixExecModeForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const policy = resolveZuvixExecPolicyForCodexAppServer(params);
  return policy.touched ? policy.mode : undefined;
}

export function resolveZuvixExecPolicyForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  config?: unknown;
  agentId?: string;
}): ZuvixExecPolicyForCodexAppServer {
  const basePolicy = resolveZuvixExecPolicyFromConfig({
    config: params.config,
    agentId: params.agentId,
  });
  const overridePolicy = applyZuvixExecPolicyLayer(basePolicy, params.execOverrides);
  const approvalFloors = resolveZuvixExecApprovalFloorsForCodexAppServer({
    approvals: params.approvals,
    agentId: params.agentId,
    policy: overridePolicy,
  });
  return applyZuvixExecApprovalFloors(overridePolicy, approvalFloors);
}

function resolveEffectiveZuvixExecModeForCodexAppServer(params: {
  execMode?: ZuvixExecMode;
  execPolicy?: ZuvixExecPolicyForCodexAppServer;
}): ZuvixExecMode | undefined {
...
function resolveCodexPolicyModeForZuvixExecMode(
  mode: ZuvixExecMode | undefined,
...
function assertCodexAppServerAllowedForZuvixExecMode(mode: ZuvixExecMode | undefined): void {
...
function createDefaultZuvixExecPolicy(): ZuvixExecPolicy {
  return {
    security: "full",
    ask: "off",
    touched: false,
...
function applyZuvixExecPolicyLayer(
  base: ZuvixExecPolicy,
  ...
  if (mode !== undefined) {
    return {
      ...resolveZuvixExecPolicyForMode(mode),
      touched: true,
    };
  }
  ...
  return {
    mode: resolveZuvixExecModeFromPolicy({ security: nextSecurity, ask: nextAsk }),
...
function resolveZuvixExecApprovalFloorsForCodexAppServer(params: {
  approvals?: ExecApprovalsFile;
  agentId?: string;
  policy: ZuvixExecPolicy;
}): ZuvixExecApprovalFloorsForCodexAppServer | undefined {
  if (!params.approvals) {
    return undefined;
  }
...
function applyZuvixExecApprovalFloors(
  base: ZuvixExecPolicy,
  approvalFloors?: ZuvixExecApprovalFloorsForCodexAppServer,
): ZuvixExecPolicy {
  if (!approvalFloors) {
    return base;
  }
  const nextSecurity = approvalFloors.security
    ? minZuvixExecSecurity(base.security, approvalFloors.security)
    : base.security;
  const nextAsk = approvalFloors.ask ? maxZuvixExecAsk(base.ask, approvalFloors.ask) : base.ask;
...
function resolveZuvixExecPolicyForMode(
  mode: ZuvixExecMode,
): Omit<ZuvixExecPolicy, "touched"> {
  switch (mode) {
    case "deny":
...
function resolveZuvixExecModeFromPolicy(params: {
  security: ZuvixExecSecurity;
  ask: ZuvixExecAsk;
}): ZuvixExecMode {
...
function minZuvixExecSecurity(
  left: ZuvixExecSecurity,
  right: ZuvixExecSecurity,
): ZuvixExecSecurity {
  const order: Record<ZuvixExecSecurity, number> = { deny: 0, allowlist: 1, full: 2 };
  return order[left] <= order[right] ? left : right;
}

function maxZuvixExecAsk(left: ZuvixExecAsk, right: ZuvixExecAsk): ZuvixExecAsk {
  const order: Record<ZuvixExecAsk, number> = { off: 0, "on-miss": 1, always: 2 };
  return order[left] >= order[right] ? left : right;
}

function readExecMode(value: unknown): ZuvixExecMode | undefined {
  return value === "deny" ||
    value === "allowlist" ||
    value === "ask" ||
    value === "auto" ||
...
function readExecSecurity(value: unknown): ZuvixExecSecurity | undefined {
  return value === "deny" || value === "allowlist" || value === "full" ? value : undefined;
}

function readExecAsk(value: unknown): ZuvixExecAsk | undefined {
