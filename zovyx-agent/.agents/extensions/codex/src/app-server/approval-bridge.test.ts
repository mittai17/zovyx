...
import { reviewExecRequestWithConfiguredModel } from "zuvix/plugin-sdk/agent-harness-exec-review-runtime";
import {
  callGatewayTool,
...
vi.mock("zuvix/plugin-sdk/agent-harness-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("zuvix/plugin-sdk/agent-harness-runtime")>()),
...
vi.mock("zuvix/plugin-sdk/agent-harness-exec-review-runtime", async (importOriginal) => ({
  ...(await importOriginal<
    typeof import("zuvix/plugin-sdk/agent-harness-exec-review-runtime")
  >()),
...
describe("Codex app-server approval bridge", () => {
  ...
  it("routes command approvals through plugin approvals and accepts allowed commands", async () => {
    ...
    expect(requestPayload.pluginId).toBe("zuvix-codex-app-server");
  ...
  it("uses the configured Zuvix exec auto-review model before plugin approvals", async () => {
    const params = createParams();
    ...
    findApprovalEvent(params, {
      status: "approved",
      message:
        "Codex app-server command approval granted by Zuvix exec auto-reviewer: read-only version check",
    });
  });
  ...
  it("falls back to plugin approval when Codex native OpenAI config uses a local base URL", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-codex-approval-"));
    try {
  ...
  it.each([
    ...
    "sudo -EH bash -lc 'zuvix channels login --channel whatsapp'",
  ...
  it("keeps security audit suppression edits on the plugin approval route", async () => {
    ...
    const result = await handleCodexAppServerApprovalRequest({
      ...
      requestParams: {
        ...
        command: "zuvix config set security.audit.suppressions '[]'",
      },
      paramsForRun: params,
      threadId: "thread-1",
      turnId: "turn-1",
  ...
  it("normalizes prefixed channel targets for Zuvix tool policy context", async () => {
    const params = createParams();
  ...
  it("denies command approvals before prompting when Zuvix tool policy blocks", async () => {
    const params = createParams();
  ...
  it("fails closed when the native hook relay returns unreadable approval output", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "Zuvix native hook relay returned an unreadable Codex app-server approval result.",
    });
  });
  ...
  it("fails closed when the native hook relay returns a non-deny decision", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message: "Zuvix native hook relay returned a non-deny Codex app-server approval decision.",
    });
  });
  ...
  it("fails closed when the expected native hook relay cannot be invoked", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "Zuvix native hook relay unavailable for Codex app-server approval: native hook relay not found",
    });
  });
  ...
  it("denies command approvals when Zuvix tool policy rewrites params", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: false,
      params: {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "Zuvix tool policy rewrote Codex app-server approval params; refusing original request.",
    });
  });

  it("keeps Zuvix plugin allow-always approvals scoped to one Codex request", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: false,
      params: {
  ...
  it("denies command approvals when Zuvix tool policy requires approval", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: true,
      kind: "failure",
  ...
  it("fails closed for unsupported native approval methods without requesting plugin approval", async () => {
    ...
    expect(result).toEqual({
      decision: "decline",
      reason: "Zuvix codex app-server bridge does not grant native approvals yet.",
    });
    expect(mockCallGatewayTool).not.toHaveBeenCalled();
