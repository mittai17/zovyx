...
import { resolveAgentWorkspaceDir } from "zuvix/plugin-sdk/agent-runtime";
...
export async function buildCodexWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    const developerInstructionFiles = shouldInjectCodexZuvixPromptContext(params.params)
      ? selectCodexWorkspaceInheritedDeveloperInstructionFiles(contextFiles)
      : [];
    const turnScopedDeveloperInstructionFiles = shouldInjectCodexZuvixPromptContext(
      params.params,
    )
    ...
    return {
      ...
      memoryCollaborationInstructions: shouldInjectCodexZuvixPromptContext(params.params)
...
/**
 * Builds Zuvix-provided workspace prompt context for the current Codex turn.
 */
export function buildCodexZuvixPromptContext(params: {
  params: EmbeddedRunAttemptParams;
  workspacePromptContext?: string;
}): string | undefined {
  if (!shouldInjectCodexZuvixPromptContext(params.params)) {
    return undefined;
  }
  const sections = [
    params.workspacePromptContext?.trim()
      ? ["## Zuvix Workspace Context", "", params.workspacePromptContext.trim()].join("\n")
      : undefined,
  ].filter(isNonEmptyString);
...
function shouldInjectCodexZuvixPromptContext(params: EmbeddedRunAttemptParams): boolean {
...
/** Renders loaded Zuvix skill prompts as Codex collaboration instructions. */
export function renderCodexSkillsCollaborationInstructions(params: {
  attempt: EmbeddedRunAttemptParams;
  skillsPrompt?: string;
}): string | undefined {
  if (!shouldInjectCodexZuvixPromptContext(params.attempt)) {
    return undefined;
  }
  return params.skillsPrompt?.trim()
    ? ["## Zuvix Skills", "", params.skillsPrompt.trim()].join("\n")
    : undefined;
}

/**
 * Prepends Zuvix context while preserving leading delivery metadata as
 * routing guidance instead of user request text.
 */
export function prependCodexZuvixPromptContext(
  prompt: string,
...
function renderCodexWorkspaceThreadDeveloperInstructions(
  ...
  return renderCodexWorkspaceDeveloperInstructions({
    files,
    header: "## Zuvix Workspace Instructions",
    preamble:
...
export function renderCodexWorkspaceMemoryReference(params: {
  ...
  const lines = [
    "## Zuvix Workspace Memory",
    "",
    `MEMORY.md exists in the active agent workspace as a memory file, not an instruction file. Zuvix does not paste its contents into native Codex turns; use ${toolNames.join(" or ")} when durable memory is relevant and the tools are available.`,
    "",
  ];
