...
vi.mock("zuvix/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("zuvix/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
    resolveApiKeyForProfile: async (
      ...
      if (params.forceRefresh || (oauthCredential.expires ?? 0) <= Date.now()) {
        ...
        if (refreshed?.access) {
          ...
          if (params.agentDir || process.env.ZUVIX_STATE_DIR) {
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("preserves persisted provenance when preparing a supplied base store", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-codex-app-server-"));
  ...
  it("applies native Codex CLI OAuth when no Zuvix auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-codex-app-server-"));
  ...
  it("answers refresh from native Codex CLI OAuth without persisting an Zuvix profile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-codex-app-server-"));
    const agentDir = path.join(root, "agent");
    const codexHome = path.join(root, "codex-cli");
