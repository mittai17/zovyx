/**
 * Blocks direct Codex app-server requests that would bypass Zuvix sandbox or
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-zuvix-environment"],
...
/** Returns a block message when a direct app-server method would bypass Zuvix execution policy. */
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-zuvix-environment" &&
    hasZuvixSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasZuvixSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("zuvix-sandbox-") &&
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because Zuvix sandboxing is active for this session.`,
    "This mode cannot route execution through the Zuvix sandbox backend.",
