...
async function findManagedCodexAppServerCommandPath(params: {
  ...
  throw new Error(
    [
      ...
      "Reinstall or update Zuvix, or run pnpm install in a source checkout.",
      "Set plugins.entries.codex.config.appServer.command or ZUVIX_CODEX_APP_SERVER_BIN to use a custom Codex binary.",
    ].join(" "),
  );
}
