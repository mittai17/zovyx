/**
 * Bridges Zuvix runtime tools into Codex app-server dynamic tool specs and
 * tool-call responses.
 */
...
import { emitTrustedDiagnosticEvent } from "zuvix/plugin-sdk/diagnostic-runtime";
import type { ImageContent, TextContent } from "zuvix/plugin-sdk/llm";
...
/** Namespace attached to Zuvix-owned dynamic tools exposed to Codex. */
export const CODEX_ZUVIX_DYNAMIC_TOOL_NAMESPACE = "zuvix";

// Keep Zuvix session spawning searchable in Codex mode so Codex's native
...
/**
 * Creates dynamic tool specs and a call handler that executes Zuvix tools,
    ...
    handleToolCall: async (call, options) => {
      ...
      if (!toolEntry) {
        const message = registeredToolNames.has(call.tool)
          ? `Zuvix tool is not available for this turn: ${call.tool}`
          : `Unknown Zuvix tool: ${call.tool}`;
        finalizeToolTerminalPresentation({
...
function convertToolContents(
  ...
  const noticeText = `...(Zuvix truncated dynamic tool result: original ${totalTextChars} chars, showing ${maxChars}; rerun with narrower args.)`;
  const notice = `\n${noticeText}`;
