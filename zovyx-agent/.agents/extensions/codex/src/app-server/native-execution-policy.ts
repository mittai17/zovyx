 ...
 * or whether Zuvix must keep exec/process on a configured node host.
 */
import type { ZuvixConfig } from "zuvix/plugin-sdk/config-contracts";
import { resolveSandboxRuntimeStatus } from "zuvix/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "zuvix/plugin-sdk/session-store-runtime";
...
type AgentEntry = NonNullable<NonNullable<ZuvixConfig["agents"]>["list"]>[number];
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "Zuvix exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected Zuvix node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because Zuvix exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected Zuvix node.",
    "Use a normal Codex harness turn so Zuvix exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: ZuvixConfig): AgentEntry[] {
