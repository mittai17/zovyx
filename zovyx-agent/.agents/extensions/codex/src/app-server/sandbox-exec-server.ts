/**
 * Hosts the local Zuvix sandbox exec-server that Codex app-server native
...
import { embeddedAgentLog } from "zuvix/plugin-sdk/agent-harness-runtime";
import type { SandboxContext } from "zuvix/plugin-sdk/sandbox";
...
const SANDBOX_EXEC_SERVERS = new Map<string, Promise<ZuvixExecServer>>();
  ...
  await Promise.all(
    servers.map(async (entry) => {
      if (entry.status === "fulfilled") {
        ...
        await closeZuvixExecServer(entry.value);
      }
    }),
  );
}
...
export async function ensureCodexSandboxExecServerEnvironment(params: {
  ...
  if (!canExposeLocalExecServerToAppServer(params.appServerStartOptions)) {
    throw new Error(
      "Zuvix Codex exec-server uses a local loopback URL and cannot be registered with a remote Codex app-server.",
    );
  }
  assertCodexSandboxExecServerSupported(params.client);
  const execServer = await acquireZuvixExecServer(params.sandbox);
  try {
    ...
    await releaseZuvixExecServer(execServer);
...
export async function releaseCodexSandboxExecServerEnvironment(
  ...
  if (server) {
    await releaseZuvixExecServer(server);
  }
}
...
function assertCodexSandboxExecServerSupported(client: CodexAppServerClient): void {
  ...
  if (
    ...
    throw new Error(
      `Codex app-server ${MIN_CODEX_SANDBOX_EXEC_SERVER_APP_SERVER_VERSION} or newer is required for Zuvix sandbox exec-server environments, but detected ${
...
async function acquireZuvixExecServer(sandbox: SandboxContext): Promise<ZuvixExecServer> {
  const key = sandbox.runtimeId;
  while (true) {
    const existing = SANDBOX_EXEC_SERVERS.get(key);
    const promise = existing ?? startAndRememberZuvixExecServer(sandbox);
...
function startAndRememberZuvixExecServer(sandbox: SandboxContext): Promise<ZuvixExecServer> {
  const created = startZuvixExecServer(sandbox);
  const key = sandbox.runtimeId;
...
async function startZuvixExecServer(sandbox: SandboxContext): Promise<ZuvixExecServer> {
  ...
  if (!address || typeof address === "string") {
    throw new Error("Zuvix Codex exec-server did not bind to a TCP port.");
  }
  const environmentId = buildEnvironmentId(sandbox);
  const authPath = `/zuvix-${randomUUID()}`;
  const url = `ws://127.0.0.1:${(address as AddressInfo).port}${authPath}`;
  const execServer: ZuvixExecServer = {
    authPath,
    closed: false,
    environmentId,
    refCount: 0,
...
async function releaseZuvixExecServer(execServer: ZuvixExecServer): Promise<void> {
  ...
  await closeZuvixExecServer(execServer);
}

async function closeZuvixExecServer(execServer: ZuvixExecServer): Promise<void> {
  if (execServer.closed) {
    return;
  }
...
function buildEnvironmentId(sandbox: SandboxContext): string {
  ...
  return `zuvix-sandbox-${hash}`;
}

function isAuthorizedExecServerRequest(
  execServer: ZuvixExecServer,
...
function handleConnection(execServer: ZuvixExecServer, socket: WebSocket): void {
...
async function dispatchRequest(
  ...
  switch (request.method) {
    ...
    default:
      throw new Error(`Unsupported Zuvix sandbox exec-server method: ${request.method}`);
  }
}
