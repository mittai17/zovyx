 ...
 */
import { asBoolean } from "zuvix/plugin-sdk/string-coerce-runtime";
import {
...
/** Detects completion for an Zuvix dynamic tool result still awaited by Codex. */
export function isPendingZuvixDynamicToolCompletionNotification(
  notification: CodexServerNotification,
  pendingZuvixDynamicToolCompletionIds: ReadonlySet<string>,
): boolean {
  if (notification.method !== "item/completed" || !isJsonObject(notification.params)) {
    return false;
  }
  const itemId = readNotificationItemId(notification);
  if (!itemId || !pendingZuvixDynamicToolCompletionIds.has(itemId)) {
    return false;
  }
  const item = isJsonObject(notification.params.item) ? notification.params.item : undefined;
