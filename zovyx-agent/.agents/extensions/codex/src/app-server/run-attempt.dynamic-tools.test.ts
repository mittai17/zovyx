...
import { initializeGlobalHookRunner } from "zuvix/plugin-sdk/hook-runtime";
...
describe("runCodexAppServerAttempt dynamic tools", () => {
    ...
    const rawWebSearch = harness.notify({
      ...
      params: {
        ...
        item: {
          ...
          status: "completed",
          action: { type: "search", query: "Zuvix" },
        },
      },
    });
    await harness.notify({
  ...
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(
      testing.resolveZuvixCodingToolsSessionKeys(
        params,
    ...
    expect(testing.resolveZuvixCodingToolsSessionKeys(params, "agent:main:main")).toEqual({
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown Zuvix tool: lookup$/u);
    ...
    expect(resultEvent?.data?.result?.content?.[0]?.text).toBe("Unknown Zuvix tool: lookup");
      ...
      emitDynamicToolTerminalDiagnostic({
        ...
        response: {
          ...
          contentItems: [
            {
              type: "inputText",
              text: "Zuvix dynamic tool call timed out after 1ms while running tool echo.",
            },
          ],
        },
      });
