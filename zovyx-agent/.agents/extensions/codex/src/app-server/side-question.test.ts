...
import { nativeHookRelayTesting } from "zuvix/plugin-sdk/agent-harness-runtime";
import {
...
const createZuvixCodingToolsMock = vi.fn();
...
vi.mock("zuvix/plugin-sdk/agent-harness", () => ({
  createZuvixCodingTools: (...args: unknown[]) => createZuvixCodingToolsMock(...args),
}));
...
describe("runCodexAppServerSideQuestion", () => {
  beforeEach(() => {
    ...
    createZuvixCodingToolsMock.mockReset();
    ...
    createZuvixCodingToolsMock.mockReturnValue([
      {
        name: "wiki_status",
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createZuvixCodingToolsMock);
  ...
  it("rejects /btw before forking when the current Zuvix session is sandboxed", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because Zuvix sandboxing is active for this session.",
    );
  ...
  it("rejects /btw before forking when exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because Zuvix exec host=node is active for this session.",
    );
  ...
  it("bridges side-thread dynamic tool requests to Zuvix tools", async () => {
    const client = createFakeClient();
  ...
  it("normalizes hook channel ids for side-thread dynamic tool requests", async () => {
    ...
    expect(createZuvixCodingToolsMock).toHaveBeenCalledWith(
  ...
  it("cleans up notification handlers when side tool setup fails", async () => {
    const client = createFakeClient();
    createZuvixCodingToolsMock.mockImplementation(() => {
