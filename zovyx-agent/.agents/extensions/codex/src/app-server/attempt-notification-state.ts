...
import {
  ...
  isPendingZuvixDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingZuvixDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingZuvixDynamicToolCompletionNotification(
    notification,
    params.pendingZuvixDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after Zuvix hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served Zuvix
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingZuvixDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }
