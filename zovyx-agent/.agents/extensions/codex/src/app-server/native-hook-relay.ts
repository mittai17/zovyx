/**
 * Bridges Codex native hook callbacks into Zuvix's native hook relay so
 * app-server tool events can still run Zuvix policy and diagnostics.
 */
...
/** Codex hook events that can be registered through Zuvix's native relay. */
...
/** Registers an Zuvix native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian Zuvix plugin approval prompt.
...
export function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("zuvix:codex:native-hook-relay:v1");
  hash.update("\0");
