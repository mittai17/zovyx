...
import {
  ...
  type UsageWindow,
} from "zuvix/plugin-sdk/provider-usage";
import { asFiniteNumber, parseStrictFiniteNumber } from "zuvix/plugin-sdk/string-coerce-runtime";
...
export function formatCodexUsageLimitErrorMessage(params: {
  ...
  if (nextReset) {
    ...
    if (codexRetryHint) {
      ...
      parts.push("Zuvix could not determine a reset time from Codex.");
    }
  }
  parts.push(
