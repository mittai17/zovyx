...
/** Tool names owned by Codex app-server and normally excluded from Zuvix dynamic tools. */
...
type CodexDynamicToolProfileEnv = {
  ZUVIX_BUILD_PRIVATE_QA?: string;
  ZUVIX_QA_FORCE_RUNTIME?: string;
};

/** Normalizes Zuvix/Codex tool names before filtering and allowlist checks. */
...
export function isForcedPrivateQaCodexRuntime(
  ...
  return (
    ...
    env.ZUVIX_QA_FORCE_RUNTIME?.trim().toLowerCase() === "codex"
  );
}
...
/** Filters Zuvix tools that Codex owns natively or config explicitly excludes. */
