...
describe("Codex app-server policy", () => {
  it("keeps implicit Codex yolo approval policy when untrusted approvals are disallowed", () => {
    ...
    const resolved = resolveCodexAppServerForZuvixToolPolicy({
      appServer,
  ...
  it("promotes implicit yolo approval policy when Zuvix tool policy requires review", () => {
    const appServer = resolveCodexAppServerRuntimeOptions({ env: {}, requirementsToml: null });
  ...
  it("preserves explicit operator app-server policy", () => {
    ...
    const explicitConfig = resolveCodexAppServerForZuvixToolPolicy({
      appServer,
    ...
    const explicitEnv = resolveCodexAppServerForZuvixToolPolicy({
      appServer,
      pluginConfig: readCodexPluginConfig({}),
      env: { ZUVIX_CODEX_APP_SERVER_APPROVAL_POLICY: "never" },
      shouldPromote: true,
      canUseUntrustedApprovalPolicy: true,
    });
    const explicitRequirements = resolveCodexAppServerForZuvixToolPolicy({
