...
import type { DoctorSessionRouteStateOwner } from "zuvix/plugin-sdk/runtime-doctor";
...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.codex.config.codexDynamicToolsProfile is retired; Codex app-server always keeps Codex-native workspace tools native. Run "zuvix doctor --fix".',
    match: hasRetiredDynamicToolsProfile,
  },
  {
    path: ["plugins", "entries", "codex", "config", "codexPlugins"],
    message:
      'plugins.entries.codex.config.codexPlugins.allow_destructive_actions="on-request" was renamed to "auto". Run "zuvix doctor --fix".',
 ...
 */
export function normalizeCompatibilityConfig({ cfg }: { cfg: ZuvixConfig }): {
  config: ZuvixConfig;
  changes: string[];
} {
  ...
  const nextConfig = structuredClone(cfg) as ZuvixConfig & {
