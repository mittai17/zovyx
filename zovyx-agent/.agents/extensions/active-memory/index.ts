...
import { closeActiveMemorySearchManager } from "zuvix/plugin-sdk/memory-host-search";
import {
  asDateTimestampMs,
...
} from "zuvix/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type ZuvixPluginApi } from "zuvix/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "zuvix/plugin-sdk/routing";
import { isPathInside } from "zuvix/plugin-sdk/security-runtime";
import {
...
import { tempWorkspace, resolvePreferredZuvixTmpDir } from "zuvix/plugin-sdk/temp-path";
...
function resolveDefaultToolsAllow(cfg: ZuvixConfig | undefined): string[] {
  return cfg?.plugins?.slots?.memory === "memory-lancedb"
    ? [...LANCEDB_ACTIVE_MEMORY_TOOLS_ALLOW]
    : [...DEFAULT_ACTIVE_MEMORY_TOOLS_ALLOW];
}

function resolveToolsAllow(params: { pluginToolsAllow: unknown; cfg?: ZuvixConfig }): string[] {
  return (
...
function resolvePersistentTranscriptBaseDir(api: ZuvixPluginApi, agentId: string): string {
  return path.join(
...
function openActiveMemoryToggleStore(api: ZuvixPluginApi) {
...
function isActiveMemoryGloballyEnabled(cfg: ZuvixConfig): boolean {
...
function resolveActiveMemoryCleanupConfig(api: ZuvixPluginApi): ZuvixConfig | undefined {
  try {
    return (
      (api.runtime.config?.current?.() as ZuvixConfig | undefined) ??
      (api.config as ZuvixConfig | undefined)
    );
  } catch {
    return api.config as ZuvixConfig | undefined;
  }
}
...
export default definePluginEntry({
  ...
  register(api: ZuvixPluginApi) {
    const readCurrentConfig = (): ZuvixConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as ZuvixConfig | undefined) ??
          (api.config as ZuvixConfig | undefined)
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as ZuvixConfig;
          if (action === "status") {
            return {
