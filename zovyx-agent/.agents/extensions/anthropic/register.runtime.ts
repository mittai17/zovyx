 ...
 */
import { formatCliCommand, parseDurationMs } from "zuvix/plugin-sdk/cli-runtime";
import { resolveExpiresAtMsFromDurationMs } from "zuvix/plugin-sdk/number-runtime";
import type {
  ZuvixPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type ZuvixConfig as ProviderAuthConfig,
...
import { fetchClaudeUsage } from "zuvix/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("zuvix models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("zuvix models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("zuvix doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: ZuvixPluginApi): void {
