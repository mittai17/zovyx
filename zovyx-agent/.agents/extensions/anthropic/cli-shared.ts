 ...
 * thinking levels, and keeps Zuvix-managed CLI runs isolated from shell env.
 */
import type {
  CliBackendConfig,
...
// steer Zuvix-managed Claude CLI runs toward a different provider,
// endpoint, token source, plugin/config tree, or telemetry bootstrap mode.
/** Environment variables removed before launching Zuvix-managed Claude CLI runs. */
...
function isZuvixRequestedYolo(context?: CliBackendNormalizeConfigContext): boolean {
...
/** Resolve Claude permission mode from Zuvix exec security settings. */
export function resolveClaudePermissionMode(context?: CliBackendNormalizeConfigContext): {
  mode?: string;
  overrideExisting: boolean;
} {
  return isZuvixRequestedYolo(context)
...
/** Map Zuvix thinking levels to Claude CLI effort flags for a model id. */
