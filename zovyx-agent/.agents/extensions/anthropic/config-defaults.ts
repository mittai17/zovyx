 ...
 */
import type { ZuvixConfig } from "zuvix/plugin-sdk/plugin-entry";
import {
  isRecord,
...
function usesClaudeCliModelSelection(config: ZuvixConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: ZuvixConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: ZuvixConfig): string[] {
  const refs = new Set<string>(
...
/** Apply Anthropic and Claude CLI defaults to an Zuvix config object. */
