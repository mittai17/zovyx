...
describe("resolveClaudeCliExecutionArgs", () => {
  ...
  it("maps Zuvix thinking levels to Claude effort args", () => {
    expect(
      resolveClaudeCliExecutionArgs({
        workspaceDir: "/tmp",
        provider: "claude-cli",
...
describe("normalizeClaudeBackendConfig", () => {
  ...
  it("derives Claude bypass from Zuvix YOLO policy and disables it for safer policy", () => {
  ...
  it("derives Claude bypass from per-agent Zuvix exec policy", () => {
    expect(
      resolveClaudePermissionMode({
        backendId: "claude-cli",
        agentId: "safe-agent",
  ...
  it("passes system prompt on every turn (issue #80374 — systemPromptWhen must be 'always')", () => {
    ...
    // Zuvix system prompt context on resumed / compacted claude-cli sessions.
    const backend = buildAnthropicCliBackend();
