...
import { streamSimple } from "zuvix/plugin-sdk/llm";
import type { ProviderWrapStreamFnContext } from "zuvix/plugin-sdk/plugin-entry";
import {
...
} from "zuvix/plugin-sdk/provider-stream-shared";
...
const ZUVIX_DEFAULT_ANTHROPIC_BETAS = [
  "fine-grained-tool-streaming-2025-05-14",
  "interleaved-thinking-2025-05-14",
] as const;
const ZUVIX_OAUTH_ANTHROPIC_BETAS = [
  "claude-code-20250219",
  "oauth-2025-04-20",
  ...ZUVIX_DEFAULT_ANTHROPIC_BETAS,
] as const;
...
/** Wrap a stream function to merge Zuvix and configured Anthropic beta headers. */
export function createAnthropicBetaHeadersWrapper(
  ...
  return (model, context, options) => {
    ...
    const zuvixBetas = isOauth
      ? (ZUVIX_OAUTH_ANTHROPIC_BETAS as readonly string[])
      : (ZUVIX_DEFAULT_ANTHROPIC_BETAS as readonly string[]);
    const allBetas = [...new Set([...zuvixBetas, ...effectiveBetas])];
