...
import { createMockIncomingRequest } from "zuvix/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "zuvix/plugin-sdk/webhook-request-guards";
...
vi.mock("zuvix/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/webhook-ingress")>(
    "zuvix/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("zuvix/plugin-sdk/reply-runtime");
    vi.doUnmock("zuvix/plugin-sdk/runtime-env");
    vi.doUnmock("zuvix/plugin-sdk/webhook-ingress");
