...
import { resolveAccountEntry } from "zuvix/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: ZuvixConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: ZuvixConfig): string {
