...
import { getChatChannelMeta, type ChannelPlugin } from "zuvix/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: ZuvixConfig, requestedAccountId?: string): string {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.ZUVIX_TWITCH_ACCESS_TOKEN?.trim();
