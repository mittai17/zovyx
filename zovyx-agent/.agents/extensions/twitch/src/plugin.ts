/**
 * Twitch channel plugin for Zuvix.
 *
...
} from "zuvix/plugin-sdk/channel-pairing";
...
} from "zuvix/plugin-sdk/extension-shared";
import {
 ...
 * for Zuvix. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: ZuvixConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: ZuvixConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: ZuvixConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: ZuvixConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("zuvix/plugin-sdk/runtime-env").RuntimeEnv;
