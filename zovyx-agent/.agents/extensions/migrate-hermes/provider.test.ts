...
import { createCapturedPluginRegistration } from "zuvix/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when ZUVIX_HOME is set", () => {
    const previous = process.env.ZUVIX_HOME;
    process.env.ZUVIX_HOME = path.join(path.sep, "tmp", "zuvix-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.ZUVIX_HOME = previous;
      }
    }
  });
