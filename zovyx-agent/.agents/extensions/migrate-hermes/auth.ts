...
async function readZuvixOpenAICandidates(
...
async function readCodexAuthProfilesFromSource(
  ...
  const candidates = (await readZuvixOpenAICandidates(source.zuvixAuthPath)).toSorted(
...
async function readCodexAuthProfilesFromPath(params: {
  ...
  if (params.sourceKind === "zuvix-auth-json") {
    return await readCodexAuthProfilesFromSource({
      root: "",
      archivePaths: [],
      ...(params.sourcePath ? { zuvixAuthPath: params.sourcePath } : {}),
    });
  }
  return [];
}
  ...
  if (await hasLegacyHermesAuthJson(params.source.authPath)) {
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes auth.json contains legacy OAuth credentials. Zuvix no longer imports those into live auth during Hermes migration.",
        recommendation:
          "Run zuvix models auth login --provider openai after migration, or run zuvix doctor --fix for existing Zuvix legacy auth state.",
      }),
    );
  }
      ...
      return createMigrationItem({
        ...
        reason: skipped
          ...
          : conflict
            ...
            : undefined,
        message: skipped
          ? "OpenAI OAuth credentials detected in Zuvix."
