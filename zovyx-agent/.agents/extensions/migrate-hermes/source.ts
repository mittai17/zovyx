...
const OPENCODE_AUTH_RELATIVE_PATH = path.join(".local", "share", "zuvix", "auth.json");
...
function resolveZuvixXdgAuthPath(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const xdgDataHome = env.XDG_DATA_HOME?.trim();
  return xdgDataHome ? path.join(resolveHomePath(xdgDataHome), "zuvix", "auth.json") : undefined;
}

async function discoverZuvixAuthPath(params: {
  root: string;
  includeGlobalFallback: boolean;
  includeHomeFallback: boolean;
}): Promise<string | undefined> {
  const rootParent = path.dirname(params.root);
  const xdgAuthPath = resolveZuvixXdgAuthPath();
...
export async function discoverHermesSource(input?: string): Promise<HermesSource> {
  ...
  const zuvixAuthPath = await discoverZuvixAuthPath({
    root,
