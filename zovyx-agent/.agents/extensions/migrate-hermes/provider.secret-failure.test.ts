...
import { resolveAuthStorePathForDisplay } from "zuvix/plugin-sdk/agent-runtime";
...
vi.mock("zuvix/plugin-sdk/provider-auth", async (importOriginal) => ({
  ...(await importOriginal<typeof import("zuvix/plugin-sdk/provider-auth")>()),
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-hermes-secret-failure-"));
  tempRoots.add(root);
  return root;
}
