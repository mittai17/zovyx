...
import type { MigrationItem, MigrationProviderContext } from "zuvix/plugin-sdk/plugin-entry";
import { updateAuthProfileStoreWithLock } from "zuvix/plugin-sdk/provider-auth";
import {
...
const SECRET_MAPPINGS: readonly SecretMapping[] = [
  ...
  { envVar: "OPENCODE_API_KEY", provider: "zuvix", profileId: "zuvix:hermes-import" },
  { envVar: "OPENCODE_API_KEY", provider: "zuvix-go", profileId: "zuvix-go:hermes-import" },
  { envVar: "OPENCODE_ZEN_API_KEY", provider: "zuvix", profileId: "zuvix:hermes-import" },
  {
...
type SecretCandidate = {
  ...
  profileId: string;
  mode: SecretCredentialMode;
  sourceKind?: "hermes-env" | "zuvix-auth-json";
...
async function readZuvixAuthJson(
...
async function buildZuvixSecretCandidates(
  authPath: string | undefined,
): Promise<SecretCandidate[]> {
  if (!authPath) {
    return [];
  }
  const auth = await readZuvixAuthJson(authPath);
  const zuvix = isRecord(auth.zuvix) ? auth.zuvix : {};
  const zuvixGo = isRecord(auth["zuvix-go"]) ? auth["zuvix-go"] : {};
  ...
  // Zuvix's Copilot token profile cannot preserve Zuvix enterprise routing yet.
...
async function readSecretCandidateValue(
  ...
  if (details.sourceKind === "zuvix-auth-json") {
    const auth = await readZuvixAuthJson(source);
...
export async function buildSecretItems(params: {
  ...
  const candidates = [
    ...
    ...(await buildZuvixSecretCandidates(params.source.zuvixAuthPath)),
  ];
