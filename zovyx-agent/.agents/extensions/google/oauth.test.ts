...
describe("loginGeminiCliOAuth", () => {
  ...
  function userInfoResponse(): Response {
    return responseJson({ email: "lobster@zuvix.ai" });
  }
  ...
  beforeEach(() => {
    envSnapshot = Object.fromEntries(ENV_KEYS.map((key) => [key, process.env[key]]));
    process.env.ZUVIX_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.ZUVIX_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
