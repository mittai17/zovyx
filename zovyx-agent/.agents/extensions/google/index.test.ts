...
import { createCapturedThinkingConfigStream } from "zuvix/plugin-sdk/provider-test-contracts";
...
describe("google provider plugin hooks", () => {
  ...
  it("resolves Google Vertex ADC auth evidence to the config marker", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "zuvix-google-vertex-config-key-"));
