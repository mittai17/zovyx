...
import { transcodeAudioBufferToOpus } from "zuvix/plugin-sdk/media-runtime";
import {
...
import { asObject, trimToUndefined } from "zuvix/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: ZuvixConfig): string | undefined {
...
function isZuvixGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isZuvixGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
