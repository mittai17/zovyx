...
import { readJsonFileWithFallback } from "zuvix/plugin-sdk/json-store";
import {
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--zuvix-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @zuvix/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Zuvix's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const zuvixWrapperArgs = new Set([
  ${quoteCommandPart(ZUVIX_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(ZUVIX_GATEWAY_INSTANCE_ID_ARG)},
]);

function readZuvixWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(ZUVIX_ACPX_LEASE_ID_ENV)}] ||
    readZuvixWrapperArg(args, ${quoteCommandPart(ZUVIX_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripZuvixWrapperArgs(args) {
  const stripped = [];
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index];
    if (zuvixWrapperArgs.has(value)) {
      index += 1;
      continue;
    }
    stripped.push(value);
...
const configuredArgs = stripZuvixWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[zuvix] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[zuvix] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Zuvix; fallback must not float to an unpatched newer release.
