...
import { finiteSecondsToTimerSafeMilliseconds } from "zuvix/plugin-sdk/number-runtime";
import type {
...
const ENABLE_STARTUP_PROBE_ENV = "ZUVIX_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "ZUVIX_SKIP_ACPX_RUNTIME_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        zuvixGatewayInstanceId: params.gatewayInstanceId,
        zuvixProcessLeaseStore: params.processLeaseStore,
        zuvixWrapperRoot: params.wrapperRoot,
...
function resolveAllowedAgentsProbeAgent(ctx: ZuvixPluginServiceContext): string | undefined {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleZuvixOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: ZuvixPluginServiceContext): Promise<void> {
      if (process.env.ZUVIX_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (ZUVIX_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale Zuvix-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
    ...
    async stop(_ctx: ZuvixPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
