/**
 * Zuvix ACPX runtime adapter. It wraps the upstream acpx runtime with
 * Zuvix session metadata, lease tracking, model scoping, and cleanup policy.
 */
...
import { parseStrictPositiveInteger } from "zuvix/plugin-sdk/number-runtime";
import { redactSensitiveText } from "zuvix/plugin-sdk/security-runtime";
import { normalizeStringEntries } from "zuvix/plugin-sdk/string-coerce-runtime";
...
import {
  cleanupZuvixOwnedAcpxProcessTree,
  isZuvixLeaseAwareAcpxProcessCommand,
...
type ZuvixAcpxRuntimeOptions = AcpRuntimeOptions & {
  zuvixWrapperRoot?: string;
  zuvixGatewayInstanceId?: string;
  zuvixProcessLeaseStore?: AcpxProcessLeaseStore;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  zuvixProcessCleanup?: AcpxProcessCleanupDeps;
};
type ZuvixRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type ZuvixRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
...
type ZuvixLeaseSessionMetadata = {
  zuvixLeaseId: string;
  zuvixGatewayInstanceId: string;
};

function withZuvixManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // Zuvix owns ACP turn deadlines. acpx treats timeout after partial agent
...
function withZuvixLeaseSessionMetadata<T extends object>(
  record: T,
  metadata: ZuvixLeaseSessionMetadata,
): T & ZuvixLeaseSessionMetadata {
  return {
    ...record,
    zuvixLeaseId: metadata.zuvixLeaseId,
    zuvixGatewayInstanceId: metadata.zuvixGatewayInstanceId,
  };
}
...
function readZuvixLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { zuvixLeaseId } = record as { zuvixLeaseId?: unknown };
  return typeof zuvixLeaseId === "string" ? zuvixLeaseId.trim() || undefined : undefined;
}

function readZuvixGatewayInstanceIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { zuvixGatewayInstanceId } = record as { zuvixGatewayInstanceId?: unknown };
  return typeof zuvixGatewayInstanceId === "string"
    ? zuvixGatewayInstanceId.trim() || undefined
    : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withZuvixLeaseSessionMetadata(record, {
        zuvixLeaseId: lease.leaseId,
        zuvixGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withZuvixLeaseSessionMetadata(
          {
            ...record,
          ...
          {
            zuvixLeaseId: launch.leaseId,
            zuvixGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const ZUVIX_BRIDGE_EXECUTABLE = "zuvix";
const ZUVIX_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_ZUVIX_PREFIX = "openai/";
const CLAUDE_ACP_ZUVIX_PREFIX = "anthropic/";
...
function isZuvixBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === ZUVIX_BRIDGE_EXECUTABLE) {
    return parts[1] === ZUVIX_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^zuvix(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === ZUVIX_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_ZUVIX_PREFIX)) {
    value = value.slice(CODEX_ACP_ZUVIX_PREFIX.length);
  }
...
function normalizeClaudeAcpModelOverride(rawModel: string | undefined): string | undefined {
  ...
  if (!raw.toLowerCase().startsWith(CLAUDE_ACP_ZUVIX_PREFIX)) {
    return raw;
  }
  return raw.slice(CLAUDE_ACP_ZUVIX_PREFIX.length).trim() || undefined;
}

function withAcpxSessionOptions(input: ZuvixRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isZuvixBridgeCommand(command);
}
...
/** Zuvix-managed ACP runtime implementation backed by the upstream acpx runtime. */
export class AcpxRuntime implements AcpRuntime {
  ...
  constructor(options: ZuvixAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { zuvixProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = zuvixProcessCleanup;
    this.wrapperRoot = options.zuvixWrapperRoot;
    this.gatewayInstanceId = options.zuvixGatewayInstanceId;
    this.processLeaseStore = options.zuvixProcessLeaseStore;
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readZuvixLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readZuvixLeaseIdFromRecord(record);
    ...
    if (lease && lease.gatewayInstanceId === this.gatewayInstanceId && lease.rootPid > 0) {
      ...
      const result = await cleanupZuvixOwnedAcpxProcessTree({
    ...
    const expectedGatewayInstanceId = readZuvixGatewayInstanceIdFromRecord(record);
    await cleanupZuvixOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withZuvixManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: ZuvixRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withZuvixManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;
