 ...
 * Zuvix-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const ZUVIX_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
export type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-zuvix-owned" | "unverified-root";
};
...
function resolveZuvixInstallRoot(pluginRoot: string): string {
  if (
...
function resolveOwnedAcpPackageRootCandidates(packageName: string): string[] {
  ...
  const zuvixRoot = resolveZuvixInstallRoot(pluginRoot);
  return [
    resolvePackageRoot(packageName),
    path.join(pluginRoot, "node_modules", packageName),
    path.join(zuvixRoot, "node_modules", packageName),
...
/** Check whether a command references an Zuvix-generated ACPX wrapper path. */
export function isZuvixLeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, ZUVIX_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, ZUVIX_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}

/** Check whether a command is owned by Zuvix ACPX runtime packages or wrappers. */
export function isZuvixOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(ZUVIX_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
/** Terminate one validated Zuvix-owned ACPX wrapper process tree. */
export async function cleanupZuvixOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an Zuvix-owned wrapper, fail closed to
...
/** Reap orphaned Zuvix-owned ACPX wrapper trees during runtime startup. */
export async function reapStaleZuvixOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
