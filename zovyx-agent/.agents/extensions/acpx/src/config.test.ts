...
describe("embedded acpx plugin config", () => {
  it("resolves workspace stateDir and cwd by default", () => {
    const workspaceDir = path.resolve("/tmp/zuvix-acpx");
  ...
  it("carries an explicit probeAgent through to the resolved plugin config, trimmed and lowercased", () => {
    ...
    expect(resolved.probeAgent).toBe("zuvix");
  });
  ...
  it("injects the built-in plugin-tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["zuvix-plugin-tools"];
  ...
  it("injects the built-in Zuvix tools MCP server only when explicitly enabled", () => {
    const resolved = resolveAcpxPluginConfig({
      rawConfig: {
    ...
    const server = resolved.mcpServers["zuvix-tools"];
    expect(server).toEqual({
      command: process.execPath,
      args: expectedMcpServerArgs({
        sourceEntry: "src/mcp/zuvix-tools-serve.ts",
        distEntry: "dist/mcp/zuvix-tools-serve.js",
      }),
    });
  });
  ...
  it("keeps the runtime json schema in sync with the manifest config schema", () => {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(pluginRoot, "zuvix.plugin.json"), "utf8"),
