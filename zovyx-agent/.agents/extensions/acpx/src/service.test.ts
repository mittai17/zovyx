...
import type { OpenKeyedStoreOptions } from "zuvix/plugin-sdk/plugin-state-runtime";
import {
...
const { cleanupZuvixOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupZuvixOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleZuvixOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleZuvixOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupZuvixOwnedAcpxProcessTree: cleanupZuvixOwnedAcpxProcessTreeMock,
  reapStaleZuvixOwnedAcpxOrphans: reapStaleZuvixOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { ZuvixPluginServiceContext } from "../runtime-api.js";
...
const previousEnv = {
  ZUVIX_ACPX_RUNTIME_STARTUP_PROBE: process.env.ZUVIX_ACPX_RUNTIME_STARTUP_PROBE,
  ZUVIX_SKIP_ACPX_RUNTIME: process.env.ZUVIX_SKIP_ACPX_RUNTIME,
  ZUVIX_SKIP_ACPX_RUNTIME_PROBE: process.env.ZUVIX_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupZuvixOwnedAcpxProcessTreeMock.mockClear();
  reapStaleZuvixOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("ZUVIX_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("ZUVIX_SKIP_ACPX_RUNTIME");
  restoreEnv("ZUVIX_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): ZuvixPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".zuvix-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: ZuvixPluginServiceContext) {
  const env = { ...process.env, ZUVIX_STATE_DIR: ctx.stateDir };
...
function openGatewayInstanceStore(ctx: ZuvixPluginServiceContext) {
...
function openProcessLeaseStore(ctx: ZuvixPluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.ZUVIX_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.ZUVIX_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.ZUVIX_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupZuvixOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupZuvixOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Zuvix-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    reapStaleZuvixOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupZuvixOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleZuvixOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Zuvix-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.ZUVIX_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("forwards a configured probeAgent to the runtime factory so the probe does not hardcode the default", async () => {
    ...
    const runtimeFactory = vi.fn(() => runtime as never);
    const service = createAcpxRuntimeService(ctx, {
      pluginConfig: { probeAgent: "zuvix" },
      runtimeFactory,
    });

    await service.start(ctx);

    expect(readFirstRuntimeFactoryInput(runtimeFactory).pluginConfig.probeAgent).toBe("zuvix");
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.ZUVIX_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.ZUVIX_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.ZUVIX_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (ZUVIX_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
