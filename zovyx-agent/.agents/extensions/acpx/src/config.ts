...
import { formatPluginConfigIssue } from "zuvix/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "zuvix-plugin-tools";
const ACPX_ZUVIX_TOOLS_MCP_SERVER_NAME = "zuvix-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "zuvix.plugin.json")) &&
...
function resolveAcpxPluginRootFromZuvixLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Zuvix layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromZuvixLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveZuvixRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const zuvixRoot = resolveZuvixRoot(pluginRoot);
  const distEntry = path.join(zuvixRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(zuvixRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveZuvixToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const zuvixRoot = resolveZuvixRoot(pluginRoot);
  const distEntry = path.join(zuvixRoot, "dist", "mcp", "zuvix-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(zuvixRoot, "src", "mcp", "zuvix-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.zuvixToolsMcpBridge && resolved[ACPX_ZUVIX_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_ZUVIX_TOOLS_MCP_SERVER_NAME} is reserved when zuvixToolsMcpBridge=true`,
    );
  }
  ...
  if (params.zuvixToolsMcpBridge) {
    resolved[ACPX_ZUVIX_TOOLS_MCP_SERVER_NAME] = resolveZuvixToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
/** Convert Zuvix MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const zuvixToolsMcpBridge = normalized.zuvixToolsMcpBridge === true;
  ...
  // writes `probeAgent: "Zuvix"` would silently miss the stored "zuvix"
  // key.
