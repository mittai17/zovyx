...
The ACPX extension is a thin Zuvix wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `zuvix/acpx`, not in this extension.
...
Use this flow when Zuvix needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `zuvix/acpx` repo first.
2. In Zuvix, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Zuvix and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Zuvix ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Zuvix-specific glue, move it to `zuvix/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
