...
vi.mock("zuvix/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.ZUVIX_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.ZUVIX_SKIP_ACPX_RUNTIME;
  } else {
    process.env.ZUVIX_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/zuvix-acpx-register-test",
    stateDir: "/tmp/zuvix-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
