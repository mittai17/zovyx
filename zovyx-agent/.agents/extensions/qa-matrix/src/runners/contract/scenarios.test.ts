...
const {
  ...
  resolveMatrixQaZuvixCliEntryPath,
  runMatrixQaZuvixCli,
  startMatrixQaZuvixCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `zuvix ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaZuvixCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaZuvixCli: vi.fn(),
  startMatrixQaZuvixCli: vi.fn(),
}));
...
async function writeMatrixInboundDedupePluginStateEntry(params: {
  ...
  const databasePath = path.join(params.stateRoot, "state", "zuvix.sqlite");
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaZuvixCli.mockReset();
    startMatrixQaZuvixCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "Zuvix Matrix QA run",
        scenarios: [
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          ZUVIX_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaZuvixCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaZuvixCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.ZUVIX_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.ZUVIX_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          ZUVIX_CONFIG_PATH: "/tmp/gateway-config.json",
          ZUVIX_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaZuvixCli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaZuvixCli, "startMatrixQaZuvixCli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(mockObjectArg(startMatrixQaZuvixCli, "startMatrixQaZuvixCli").timeoutMs).toBe(
        16_000,
      );
      ...
      expect(registrationRequest?.deviceName).toBe(
        "Zuvix Matrix QA CLI Self Verification Owner",
      );
      if (
      ...
      expect(loginWithPassword).toHaveBeenCalledWith({
        deviceName: "Zuvix Matrix QA CLI Self Verification Device",
        password: "cli-owner-password",
        userId: "@cli-owner:matrix-qa.test",
      });
      const e2eeClientOptions = mockObjectArg(
      ...
      expect(runMatrixQaZuvixCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaZuvixCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaZuvixCli, "runMatrixQaZuvixCli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaZuvixCli, "startMatrixQaZuvixCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.ZUVIX_STATE_DIR).toContain("zuvix-matrix-cli-qa-");
      expect(cliEnv?.ZUVIX_CONFIG_PATH).toContain("zuvix-matrix-cli-qa-");
      const configPath = String(cliEnv?.ZUVIX_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.ZUVIX_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaZuvixCli.mockImplementation(async ({ args, env }) => {
        if (env.ZUVIX_CONFIG_PATH) {
      ...
      expect(runMatrixQaZuvixCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "Zuvix Matrix QA CLI Account Add E2EE",
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Zuvix Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaZuvixCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.ZUVIX_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.ZUVIX_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Zuvix Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Zuvix Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("zuvix matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaZuvixCli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaZuvixCli, "startMatrixQaZuvixCli").env as Record<
            string,
            unknown
          >
        ).ZUVIX_CONFIG_PATH,
      ).toContain("zuvix-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Zuvix Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Zuvix Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Zuvix Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaZuvixCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.ZUVIX_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Zuvix Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Zuvix Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Zuvix Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
