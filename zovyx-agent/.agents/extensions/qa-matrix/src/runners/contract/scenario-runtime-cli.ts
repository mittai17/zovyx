...
import { spawn as startZuvixCliProcess } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `zuvix ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaZuvixCliEntryPath(cwd: string): string {
...
function killMatrixQaCliChild(
  child: ReturnType<typeof startZuvixCliProcess>,
...
export function startMatrixQaZuvixCli(params: {
  ...
  const distEntryPath = resolveMatrixQaZuvixCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startZuvixCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaZuvixCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaZuvixCli(params).wait();
}
...
export async function createMatrixQaZuvixCliRuntime(params: {
  accountId: string;
  accessToken: string;
