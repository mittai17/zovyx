...
import {
  ...
  type ZuvixConfig,
} from "zuvix/plugin-sdk/account-resolution";
import { resolveAccountEntry } from "zuvix/plugin-sdk/routing";
...
function mergeIMessageAccountConfig(cfg: ZuvixConfig, accountId: string): IMessageAccountConfig {
...
// caused duplicate inbound handling in zuvix/zuvix#65141.
...
function resolveIMessageAccountSourceOwner(params: {
  ...
  // bindings tied to the named account keep working (zuvix/zuvix#65141).
 ...
 * watchers (zuvix/zuvix#65141) without otherwise marking the duplicate
...
export function listEnabledIMessageAccounts(cfg: ZuvixConfig): ResolvedIMessageAccount[] {
