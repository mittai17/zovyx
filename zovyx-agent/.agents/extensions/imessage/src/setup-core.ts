...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: ZuvixConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: ZuvixConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: ZuvixConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run Zuvix on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `zuvix channels status --probe` to verify private API actions.",
    "Ensure Zuvix has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: ZuvixConfig; accountId?: string }) =>
