...
import type { BaseProbeResult } from "zuvix/plugin-sdk/channel-contract";
import {
  asDateTimestampMs,
  resolveExpiresAtMsFromDurationMs,
} from "zuvix/plugin-sdk/number-runtime";
import { runCommandWithTimeout } from "zuvix/plugin-sdk/process-runtime";
...
import { detectBinary } from "zuvix/plugin-sdk/setup";
import {
...
export function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run Zuvix on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by zuvix/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // zuvix/imsg#114. We do this even when the bridge is unavailable
