...
import { readPluginPackageVersion } from "zuvix/plugin-sdk/extension-shared";
import {
...
const USER_AGENT = `zuvix-parallel/${PLUGIN_VERSION} (${process.platform})`;
...
export async function executeParallelWebSearchProviderTool(
  ...
  // Generic `query` arg fallback: zuvix's operator-facing CLI
  // (`zuvix capability web.search ...`) always passes the shared
  ...
  // Always pass max_results so Parallel matches the zuvix web_search default
