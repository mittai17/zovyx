...
import { withTrustedWebSearchEndpoint } from "zuvix/plugin-sdk/provider-web-search";
...
// without this, undici sends a generic `node` UA and Zuvix usage is only
  ...
  const init = await postMcp({
    ...
    body: {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "zuvix-parallel", version: PLUGIN_VERSION },
      },
    },
  });
  if (!init.ok) {
