...
import type { PluginLogger } from "zuvix/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "zuvix/plugin-sdk/runtime-env";
...
/** Input data used to publish Zuvix gateway Bonjour records. */
...
// See https://github.com/zuvix/zuvix/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.ZUVIX_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Zuvix"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Zuvix";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Zuvix\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.ZUVIX_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "zuvix";
    ...
    const recreateAdvertiser = async (reason: string, optsLocal?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or ZUVIX_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
