// Tlon plugin entrypoint registers its Zuvix integration.
import type { ReplyPayload } from "zuvix/plugin-sdk/reply-runtime";
import type { RuntimeEnv } from "zuvix/plugin-sdk/runtime";
import { asFiniteNumber } from "zuvix/plugin-sdk/string-coerce-runtime";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.current() as ZuvixConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in Zuvix config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your Zuvix config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.zuvix.ai/concepts/session#secure-dm-mode`;
