...
import { createScopedDmSecurityResolver } from "zuvix/plugin-sdk/channel-config-helpers";
...
function resolveZalouserOutboundChunkMode(cfg: ZuvixConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: ZuvixConfig, accountId?: string) {
...
export const zalouserPairingTextAdapter = {
  ...
  notify: async ({ cfg, id, message }: { cfg: ZuvixConfig; id: string; message: string }) => {
    const { sendMessageZalouser } = await loadZalouserChannelRuntime();
