...
function hasTrustedZuvixRootIndicator(packageRoot, packageJson) {
  ...
  const hasZuvixBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("zuvix")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.zuvix === "string");
  const hasZuvixEntrypoint = fs.existsSync(path.join(packageRoot, "zuvix.mjs"));
  return hasCliEntryExport || hasZuvixBin || hasZuvixEntrypoint;
}

function findZuvixPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "zuvix" && hasTrustedZuvixRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findZuvixPackageRoot(moduleDir);
  if (location) {
