...
import { writeJsonFileAtomically } from "zuvix/plugin-sdk/json-store";
import { resolveRequiredHomeDir, resolveStateDir } from "zuvix/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "zuvix-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("zuvix/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("zuvix/plugin-sdk/runtime")).createBackupArchive;
