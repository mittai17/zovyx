...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'zuvix matrix verify status --verbose --account ${accountId}' and 'zuvix matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Zuvix runtime is using the same account. " +
    `Check 'zuvix matrix verify status --verbose --account ${accountId}' and 'zuvix matrix devices list --account ${accountId}'.`
  );
}
