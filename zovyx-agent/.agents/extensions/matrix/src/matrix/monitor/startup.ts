...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleZuvixDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Zuvix devices detected for ${params.auth.userId}: ${deviceHealth.staleZuvixDevices.map((device) => device.deviceId).join(", ")}. Run 'zuvix matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
