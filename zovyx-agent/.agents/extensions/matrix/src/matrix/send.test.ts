...
vi.mock("zuvix/plugin-sdk/plugin-config-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/plugin-config-runtime")>(
    "zuvix/plugin-sdk/plugin-config-runtime",
  );
  return {
    ...actual,
...
describe("sendMessageMatrix media", () => {
  ...
  it("passes caller mediaLocalRoots to media loading", async () => {
    ...
    await sendMessageMatrix("room:!room:example", "caption", {
      ...
      cfg: {} as never,
      mediaUrl: "file:///tmp/photo.png",
      mediaLocalRoots: ["/tmp/zuvix-matrix-test"],
    });
    ...
    expect(mediaOptions.localRoots).toEqual(["/tmp/zuvix-matrix-test"]);
  });
});
...
describe("sendMessageMatrix threads", () => {
  ...
  it("merges extra content into only the first chunked text event", async () => {
    ...
    await sendMessageMatrix("room:!room:example", "ignored", {
      client,
      cfg: {} as never,
      extraContent: { "com.zuvix.approval": { id: "req-1" } },
    });

    expect(sendMessage).toHaveBeenCalledTimes(3);
    expect(sentContent(sendMessage, 0).body).toBe("first");
    expect(sentContent(sendMessage, 0)["com.zuvix.approval"]).toEqual({ id: "req-1" });
    expect(sentContent(sendMessage, 1).body).toBe("second");
    expect(sentContent(sendMessage, 1)).not.toHaveProperty("com.zuvix.approval");
    expect(sentContent(sendMessage, 2).body).toBe("third");
    expect(sentContent(sendMessage, 2)).not.toHaveProperty("com.zuvix.approval");
  });
});
...
describe("sendSingleTextMessageMatrix", () => {
  ...
  it("merges extra content fields into single-event sends", async () => {
    ...
    const result = await sendSingleTextMessageMatrix("room:!room:example", "done", {
      client,
      cfg: {} as never,
      extraContent: { [MATRIX_ZUVIX_FINALIZED_PREVIEW_KEY]: true },
    });

    expect(sentContent(sendMessage).body).toBe("done");
    expect(sentContent(sendMessage)[MATRIX_ZUVIX_FINALIZED_PREVIEW_KEY]).toBe(true);
...
describe("editMessageMatrix mentions", () => {
  ...
  it("merges extra content fields into edit payloads and m.new_content", async () => {
    ...
    const content = sentContent(sendMessage);
    expect(content[MATRIX_ZUVIX_FINALIZED_PREVIEW_KEY]).toBe(true);
    expect(newContent(content)[MATRIX_ZUVIX_FINALIZED_PREVIEW_KEY]).toBe(true);
  });
