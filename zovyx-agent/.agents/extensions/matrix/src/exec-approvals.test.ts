...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<ZuvixConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<ZuvixConfig["channels"]>["matrix"]>>,
): ZuvixConfig {
  return {
    channels: {
      matrix: {
