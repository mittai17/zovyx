...
import type { ChannelDoctorAdapter } from "zuvix/plugin-sdk/channel-contract";
...
function hasConfiguredMatrixChannel(cfg: ZuvixConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: ZuvixConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: ZuvixConfig, env: NodeJS.ProcessEnv): boolean {
  return (
...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "zuvix doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "zuvix doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: ZuvixConfig): Promise<string[]> {
  ...
  return formatPluginInstallPathIssue({
    issue,
    pluginLabel: "Matrix",
    defaultInstallCommand: "zuvix plugins install @zuvix/matrix",
  }).map((entry) => `- ${entry}`);
}

export async function cleanStaleMatrixPluginConfig(cfg: ZuvixConfig) {
  const issue = await detectPluginInstallPathIssue({
    pluginId: "matrix",
    install: cfg.plugins?.installs?.matrix,
  });
