...
import type { DmPolicy } from "zuvix/plugin-sdk/config-contracts";
import {
...
import { isPrivateNetworkOptInEnabled } from "zuvix/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "Zuvix agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "Zuvix Gateway",
      }),
    ) ?? "";
