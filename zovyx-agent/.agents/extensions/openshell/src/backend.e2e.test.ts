...
import { createSandboxTestContext } from "zuvix/plugin-sdk/test-fixtures";
import {
...
const ZUVIX_OPENSHELL_E2E = process.env.ZUVIX_E2E_OPENSHELL === "1";
const ZUVIX_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const ZUVIX_OPENSHELL_COMMAND =
  process.env.ZUVIX_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "zuvix-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && ZUVIX_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: ZUVIX_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(ZUVIX_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(ZUVIX_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-openshell-e2e-"));
      ...
      const gatewayName = `zuvix-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `zuvix-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: ZUVIX_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("zuvix-openshell-e2e");
