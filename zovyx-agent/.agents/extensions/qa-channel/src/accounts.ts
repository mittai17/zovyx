...
import { resolveMergedAccountConfig } from "zuvix/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "zuvix";
  const botDisplayName = merged.botDisplayName?.trim() || "Zuvix QA";
  return {
    accountId,
    enabled,
