...
vi.mock("zuvix/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("zuvix/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("zuvix/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("zuvix/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/zuvix/synology-chat-sessions.json"),
