/**
 * Synology Chat Channel Plugin for Zuvix.
 *
...
import { waitUntilAbort } from "zuvix/plugin-sdk/channel-outbound";
import {
...
import { createEmptyChannelDirectoryAdapter } from "zuvix/plugin-sdk/directory-runtime";
import {
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "zuvix pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: ZuvixConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: ZuvixConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to Zuvix",
        order: 90,
      },
      capabilities: {
