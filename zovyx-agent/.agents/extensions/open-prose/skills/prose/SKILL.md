...
metadata: { "zuvix": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Zuvix `sessions_spawn`
- **File I/O** == Zuvix `read`/`write`
- **Remote fetch** == Zuvix `web_fetch` (or `exec` with curl when POST is required)
