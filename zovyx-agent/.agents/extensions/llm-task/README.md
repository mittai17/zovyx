...
`zuvix.invoke --each`) without adding new Zuvix code per workflow.
...
This extension depends on Zuvix internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Zuvix extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.zuvix/extensions` as a standalone plugin directory.
