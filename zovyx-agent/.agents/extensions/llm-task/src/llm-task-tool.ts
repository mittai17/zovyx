...
import { buildModelAliasIndex, resolveModelRefFromString } from "zuvix/plugin-sdk/agent-runtime";
import {
...
import { resolvePreferredZuvixTmpDir, withTempWorkspace } from "../api.js";
...
type ThinkingPolicy = ReturnType<ZuvixPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;

export const llmTaskToolDefinition = {
  name: "llm-task",
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via zuvix.invoke.",
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<ZuvixPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return Boolean(level) && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: ZuvixPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<ZuvixPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredZuvixTmpDir(), prefix: "zuvix-llm-task-" },
        async ({ dir: tmpDir }) => {
