...
import { createProviderUsageFetch, makeResponse } from "zuvix/plugin-sdk/test-env";
...
vi.mock("zuvix/plugin-sdk/provider-model-shared", async (importOriginal) => ({
  ...(await importOriginal<typeof import("zuvix/plugin-sdk/provider-model-shared")>()),
...
vi.mock("zuvix/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("zuvix/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/zuvix-state",
}));

import type { ProviderResolveDynamicModelContext } from "zuvix/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/zuvix-state/credentials/github-copilot.token.json";
