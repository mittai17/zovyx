...
import { MAX_DATE_TIMESTAMP_MS, MAX_TIMER_TIMEOUT_MS } from "zuvix/plugin-sdk/number-runtime";
import type {
  ZuvixConfig,
  ZuvixPluginApi,
  ProviderAuthResult,
...
type RegisteredMemoryEmbeddingProvider = Parameters<
  ZuvixPluginApi["registerMemoryEmbeddingProvider"]
>[0];
type RegisteredProvider = Parameters<ZuvixPluginApi["registerProvider"]>[0];
type GithubCopilotTestProvider = RegisteredProvider & {
  auth: Array<{
    run: (ctx: unknown) => Promise<ProviderAuthResult | null>;
    runNonInteractive: (ctx: unknown) => Promise<ZuvixConfig | null>;
  }>;
  catalog: {
...
async function createAgentDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-github-copilot-test-"));
  tempDirs.push(dir);
  return dir;
}
...
function registerProviderAndCatalogWithPluginConfig(pluginConfig: Record<string, unknown>) {
  const registerProviderMock = vi.fn<ZuvixPluginApi["registerProvider"]>();
  const registerModelCatalogProviderMock =
    vi.fn<ZuvixPluginApi["registerModelCatalogProvider"]>();
...
describe("github-copilot plugin", () => {
  ...
  it("registers embedding provider", () => {
    const registerMemoryEmbeddingProviderMock =
      vi.fn<ZuvixPluginApi["registerMemoryEmbeddingProvider"]>();
