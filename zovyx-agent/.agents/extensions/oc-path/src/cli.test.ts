/**
 * Smoke tests for the `zuvix path` CLI handlers.
 *
...
describe("zuvix path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "zuvix.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://zuvix.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://zuvix.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );
