...
import type { FixedWindowRateLimiter } from "zuvix/plugin-sdk/webhook-ingress";
...
vi.mock("zuvix/plugin-sdk/webhook-request-guards", () => ({
  readJsonWebhookBodyOrReject,
}));

vi.mock("zuvix/plugin-sdk/webhook-targets", () => ({
...
describe("googlechat monitor webhook", () => {
  ...
  afterAll(() => {
    vi.doUnmock("zuvix/plugin-sdk/webhook-request-guards");
    vi.doUnmock("zuvix/plugin-sdk/webhook-targets");
    vi.doUnmock("./auth.js");
    vi.resetModules();
  });
    ...
    expect(processEvent).toHaveBeenCalledWith(
      {
        ...
        action: {
          parameters: [
            { key: "zuvix_action", value: "approval" },
