...
const EAGER_BROWSER_CONTROL_SERVICE_ENV = "ZUVIX_EAGER_BROWSER_CONTROL_SERVER";
...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage Zuvix's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via Zuvix's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated Zuvix-managed browser (`zuvix`).",
...
function createBrowserToolOptions(ctx: ZuvixPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
export const browserPluginNodeHostCommands: ZuvixPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: ZuvixPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): ZuvixPluginService {
  let service: ZuvixPluginService | null = null;
...
export function registerBrowserPlugin(api: ZuvixPluginApi) {
  api.registerTool(((ctx: ZuvixPluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as ZuvixPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
