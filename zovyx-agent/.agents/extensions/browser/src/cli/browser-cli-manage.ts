...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "zuvix";
...
function formatBrowserDoctorGatewayError(error: unknown): string {
  ...
  return "Gateway auth SecretRef is unavailable in this command path; browser doctor cannot reach the admin-scoped browser.request endpoint. Set ZUVIX_GATEWAY_TOKEN or ZUVIX_GATEWAY_PASSWORD, then retry.";
}
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "zuvix"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `zuvix browser start`",
  });
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "zuvix" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (zuvix|existing-session). Default: zuvix")
    .action(
      async (
        opts: {
          name: string;
