...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<ZuvixConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: ZuvixConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): ZuvixConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: ZuvixConfig) {
