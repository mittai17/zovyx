...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        zuvix: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ZUVIX_GATEWAY_PORT;
    process.env.ZUVIX_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ZUVIX_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ZUVIX_GATEWAY_PASSWORD;
