...
type StopChromeTarget = Parameters<typeof stopZuvixChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "zuvix-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateZuvixProfile(userDataDir, { color: DEFAULT_ZUVIX_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0xff4500) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_ZUVIX_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_ZUVIX_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".zuvix-profile-decorated"),
      "utf-8",
    );
  ...
  it("writes managed download prefs when a download dir is provided", async () => {
    ...
    decorateZuvixProfile(userDataDir, {
      color: DEFAULT_ZUVIX_BROWSER_COLOR,
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateZuvixProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_ZUVIX_BROWSER_PROFILE_NAME);
  });
...
describe("browser chrome helpers", () => {
  ...
  it("finds Playwright-managed Linux Chromium", () => {
    const browserPath = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-ms-playwright-"));
  ...
  it("stopZuvixChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopZuvixChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopZuvixChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
  ...
  it("stopZuvixChrome releases the managed-proxy CDP bypass exactly once on a double stop", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("down")));
    const proc = makeChromeTestProc();
    const release = vi.fn();
    const running = {
    ...
    await stopZuvixChrome(running, 10);
    await stopZuvixChrome(running, 10);
    expect(release).toHaveBeenCalledOnce();
  });

  it("stopZuvixChrome still releases the bypass when the SIGKILL fallback fires", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
    ...
    await stopZuvixChrome(running, 1);
    expect(proc.kill).toHaveBeenNthCalledWith(1, "SIGTERM");
    expect(proc.kill).toHaveBeenNthCalledWith(2, "SIGKILL");
    expect(release).toHaveBeenCalledOnce();
  });

  it("stopZuvixChrome swallows a throw from the bypass release callback", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("down")));
    const proc = makeChromeTestProc();
    const release = vi.fn(() => {
    ...
    await expect(stopZuvixChrome(running, 10)).resolves.toBeUndefined();
    expect(release).toHaveBeenCalledOnce();
  });
});
...
describe("browser chrome launch args", () => {
  it("does not force an about:blank tab at startup", () => {
    const args = buildZuvixChromeLaunchArgs({
      ...
      userDataDir: "/tmp/zuvix-test-user-data",
    });

    expect(args).not.toContain("about:blank");
    expect(args).toContain("--remote-debugging-port=18800");
    expect(args).toContain("--user-data-dir=/tmp/zuvix-test-user-data");
  });
});
