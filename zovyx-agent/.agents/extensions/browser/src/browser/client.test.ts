...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("zuvix");
    ...
    expect(deepDoctorResult.profile).toBe("zuvix");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=zuvix&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=zuvix&deep=true"));
