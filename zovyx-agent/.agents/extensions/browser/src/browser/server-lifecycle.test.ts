...
const { stopZuvixChromeMock } = vi.hoisted(() => ({
  stopZuvixChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopZuvixChrome: stopZuvixChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["zuvix", "user"]);
    ...
    expect(stopMap.zuvix).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopZuvixChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("zuvix browser stop failed: Error: oops");
  });
});
