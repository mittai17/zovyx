...
const resolvePreferredZuvixTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/zuvix"));
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    testRoot = realRealpathSyncNative(realMkdtempSync(path.join(os.tmpdir(), "zuvix-browser-")));
    ...
    resolvePreferredZuvixTmpDirMock.mockReturnValue(tmpDir);
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    const outsidePath = path.join(outsideDir, "zuvix-demo");
