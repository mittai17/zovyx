...
import type { BrowserConfig, BrowserProfileConfig, ZuvixConfig } from "../config/config.js";
...
export const ZUVIX_BROWSER_HEADLESS_ENV = "ZUVIX_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_ZUVIX_BROWSER_COLOR;
  }
...
function isLocalManagedProfile(profile: ResolvedBrowserProfile): boolean {
  return profile.driver === "zuvix" && profile.cdpIsLoopback && !profile.attachOnly;
}
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ZUVIX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ZUVIX_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_ZUVIX_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_ZUVIX_BROWSER_PROFILE_NAME]
        ? DEFAULT_ZUVIX_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl;
  const driver = profile.driver === "existing-session" ? "existing-session" : "zuvix";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[ZUVIX_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  return (
    ...
    `Set ${ZUVIX_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
