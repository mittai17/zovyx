...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.ZUVIX_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "zuvix",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/zuvix" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    zuvix: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/zuvix.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/zuvix.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/zuvix.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("./chrome.js", () => ({
  ...
  launchZuvixChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveZuvixUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopZuvixChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.ZUVIX_GATEWAY_PORT;
  process.env.ZUVIX_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.ZUVIX_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.ZUVIX_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.ZUVIX_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.ZUVIX_GATEWAY_PASSWORD;
  } else {
    process.env.ZUVIX_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
