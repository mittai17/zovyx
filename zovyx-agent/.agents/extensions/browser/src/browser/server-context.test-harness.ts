...
/** Mocks a launched Zuvix Chrome process with the supplied pid. */
export function mockLaunchedChrome(
  launchZuvixChrome: { mockResolvedValue: (value: RunningChrome) => unknown },
  pid: number,
) {
  const proc = new EventEmitter() as unknown as ChildProcessWithoutNullStreams;
  launchZuvixChrome.mockResolvedValue({
    pid,
