/**
 * Zuvix-managed Chrome profile decoration.
 *
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".zuvix-profile-decorated");
}
 ...
 */
export function decorateZuvixProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ZUVIX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ZUVIX_BROWSER_COLOR).toUpperCase();
