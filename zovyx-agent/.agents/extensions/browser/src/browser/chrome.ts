/**
 * Zuvix-managed Chrome lifecycle and CDP helpers.
 *
...
import { prepareOomScoreAdjustedSpawn } from "zuvix/plugin-sdk/process-runtime";
...
import {
  DEFAULT_ZUVIX_BROWSER_COLOR,
  DEFAULT_ZUVIX_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function chromeLaunchHints(params: {
  ...
  if (CHROME_MISSING_DISPLAY_PATTERN.test(params.stderrOutput) && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set ZUVIX_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  if (CHROME_SINGLETON_IN_USE_PATTERN.test(params.stderrOutput)) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.zuvix/browser/${params.profile.name}/user-data.`,
    );
  }
...
export type RunningChrome = {
   ...
   * through stopZuvixChrome without type churn.
   */
...
/** Resolve the user-data-dir path for a managed Zuvix Chrome profile. */
export function resolveZuvixUserDataDir(profileName = DEFAULT_ZUVIX_BROWSER_PROFILE_NAME) {
...
/** Build Chrome launch arguments for the managed Zuvix browser. */
export function buildZuvixChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
...
/** Launch or attach to the managed Zuvix Chrome profile. */
export async function launchZuvixChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  launchOptions: ManagedBrowserHeadlessOptions = {},
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ZUVIX_BROWSER_COLOR).toUpperCase(),
  ...
  const spawnOnce = () => {
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredZuvixTmpDir(), ".chromium");
  ...
  if (needsDecorate) {
    try {
      decorateZuvixProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
        downloadDir: DEFAULT_DOWNLOAD_DIR,
      });
      log.info(`🦞 zuvix browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`zuvix browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`zuvix browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `🦞 zuvix browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
...
export async function stopZuvixChrome(
