...
describe("normalizeBrowserEvaluateFunctionSource", () => {
  ...
  it("wraps expressions as page functions", () => {
    expect(normalizeBrowserEvaluateFunctionSource("document.title")).toBe(
      [
        "() => {",
        "const __zuvixEvaluateExpressionResult = (document.title);",
        'return typeof __zuvixEvaluateExpressionResult === "function" ? __zuvixEvaluateExpressionResult() : __zuvixEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
  });
  ...
  it("preserves function-valued expression invocation", () => {
    expect(normalizeBrowserEvaluateFunctionSource("extractTitle")).toBe(
      [
        "() => {",
        "const __zuvixEvaluateExpressionResult = (extractTitle);",
        'return typeof __zuvixEvaluateExpressionResult === "function" ? __zuvixEvaluateExpressionResult() : __zuvixEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
    expect(normalizeBrowserEvaluateFunctionSource("extractText", { argumentName: "el" })).toBe(
      [
        "(el) => {",
        "const __zuvixEvaluateExpressionResult = (extractText);",
        'return typeof __zuvixEvaluateExpressionResult === "function" ? __zuvixEvaluateExpressionResult(el) : __zuvixEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
  });
