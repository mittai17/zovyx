...
vi.mock("zuvix/plugin-sdk/ssrf-runtime-internal", () => ({
...
vi.mock("../infra/tmp-zuvix-dir.js", () => ({
  resolvePreferredZuvixTmpDir: () => "/tmp/zuvix-browser-test",
}));
...
import {
  buildZuvixChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchZuvixChrome,
  resolveZuvixUserDataDir,
  stopZuvixChrome,
} from "./chrome.js";
 ...
 * not exercise: launchZuvixChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopZuvixChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildZuvixChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveZuvixUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveZuvixUserDataDir();
      expect(dir.endsWith(path.join("zuvix", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveZuvixUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildZuvixChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildZuvixChromeLaunchArgs({
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchZuvixChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchZuvixChrome(resolved, profile);
  ...
  describe("launchZuvixChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zuvix-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchZuvixChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchZuvixChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("accepts a ready CDP diagnostic after the launch HTTP probe expires", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const port = new URL(baseUrl).port;
          const profile = makeProfile(Number(port));
          const running = await launchZuvixChrome(
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      const configPath = path.join(tmpDir, "zuvix.json");
      await fsp.writeFile(
        configPath,
        JSON.stringify({
          logging: {
      ...
      vi.stubEnv("ZUVIX_CONFIG_PATH", configPath);
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const userDataDir = resolveZuvixUserDataDir(profile.name);
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchZuvixChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopZuvixChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopZuvixChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopZuvixChrome>[0],
        1,
      );
  ...
  describe("launchZuvixChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zuvix-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveZuvixUserDataDir(profileName));
        ...
        const staged = resolveZuvixUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_ZUVIX_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const configDir = await fsp.mkdtemp(path.join(os.tmpdir(), "zuvix-redact-off-"));
      ...
      let message = "";
      try {
        await launchZuvixChrome(resolved, profile);
      } catch (err) {
    ...
    it("logs a warning when decorateZuvixProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateZuvixProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateZuvixProfile }, "decorateZuvixProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".zuvix-profile-decorated") || s.endsWith("Preferences")) {
  ...
  describe("launchZuvixChrome managed-proxy CDP bypass", () => {
    ...
    it("surfaces loopbackMode=block as BrowserProfileUnavailableError without spawning Chrome", async () => {
      ...
      await expect(launchZuvixChrome(makeResolved(), profile)).rejects.toBeInstanceOf(
    ...
    it("does not register a bypass for a remote attachOnly CDP URL (loopback gate)", async () => {
      stubExecutableAndPrefsExist();
      // For this test we want launchZuvixChrome to reject before any
      ...
      await expect(launchZuvixChrome(makeResolved(), remoteProfile)).rejects.toThrow(
        /is remote; cannot launch local Chrome/,
      );
