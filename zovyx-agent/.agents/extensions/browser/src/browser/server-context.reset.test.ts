...
function localZuvixProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "zuvix",
...
function createLocalZuvixResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localZuvixProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveZuvixUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-reset-"));
    const profileDir = path.join(tempRoot, "zuvix");
    ...
    const ops = createLocalZuvixResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveZuvixUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-reset-no-own-"));
    const profileDir = path.join(tempRoot, "zuvix");
    fs.mkdirSync(profileDir, { recursive: true });
