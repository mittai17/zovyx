...
import { clearRuntimeConfigSnapshot } from "zuvix/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("zuvix-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".zuvix", "zuvix.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to Zuvix");
      expect(message).toContain("Restarting the Zuvix gateway will not launch it");
      expect(message).not.toContain("Restart the Zuvix gateway");
