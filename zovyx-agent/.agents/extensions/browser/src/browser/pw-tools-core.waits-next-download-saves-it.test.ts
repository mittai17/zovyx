...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredZuvixTmpDir: vi.fn(() => "/tmp/zuvix"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-zuvix-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-zuvix-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredZuvixTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredZuvixTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredZuvixTmpDir.mockReturnValue("/tmp/zuvix");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredZuvixTmpDir.mockReturnValue("/tmp/zuvix-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "zuvix-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "zuvix-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredZuvixTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "zuvix-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "zuvix-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredZuvixTmpDir.mockReturnValue(tempDir);
