 ...
 */
import type { ZuvixPluginSecurityAuditContext } from "zuvix/plugin-sdk/plugin-entry";
...
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "zuvix/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: ZuvixPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("zuvix security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.ZUVIX_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.ZUVIX_GATEWAY_PASSWORD) ||
