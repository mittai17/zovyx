...
import type { ZuvixPluginApi } from "./runtime-api.js";
import setupPlugin from "./setup-api.js";

type BrowserAutoEnableProbe = Parameters<ZuvixPluginApi["registerAutoEnableProbe"]>[0];
...
describe("browser plugin", () => {
  ...
  it("bundles the browser automation skill with the plugin", () => {
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, "zuvix.plugin.json"), "utf8"),
    ) as { skills?: string[] };
  ...
  it("registers a lazy browser control service", async () => {
    ...
    await service.start({ config: {}, stateDir: "/tmp/zuvix", logger: { warn: vi.fn() } });
    expect(runtimeApiMocks.createBrowserPluginService).not.toHaveBeenCalled();

    await service.stop({ config: {}, stateDir: "/tmp/zuvix", logger: { warn: vi.fn() } });
  ...
  it("eager-loads the browser control service when explicitly requested", async () => {
    vi.stubEnv("ZUVIX_EAGER_BROWSER_CONTROL_SERVER", "1");
    const { api, registerService } = createApi();
    registerBrowserPlugin(api);
  ...
  for (const value of ["false", "", "disabled"]) {
    it(`keeps browser control service env value ${JSON.stringify(value)} lazy`, async () => {
      vi.stubEnv("ZUVIX_EAGER_BROWSER_CONTROL_SERVER", value);
      const { api, registerService } = createApi();
      registerBrowserPlugin(api);
  ...
  it("declares setup auto-enable reasons for browser config surfaces", () => {
    ...
    expect(probe({ config: { browser: { defaultProfile: "zuvix" } }, env: {} })).toBe(
      "browser configured",
    );
    ...
    expect(
      probe({ config: { browser: { defaultProfile: "zuvix", enabled: false } }, env: {} }),
    ).toBeNull();
  });
});
