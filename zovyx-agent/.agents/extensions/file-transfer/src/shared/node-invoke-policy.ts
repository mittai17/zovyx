...
import { readPositiveIntegerParam } from "zuvix/plugin-sdk/param-readers";
import type {
  ZuvixPluginNodeInvokePolicy,
  ZuvixPluginNodeInvokePolicyContext,
  ZuvixPluginNodeInvokePolicyResult,
...
}): Promise<ZuvixPluginNodeInvokePolicyResult | null> {
  const nodeDisplayName = input.ctx.node?.displayName;
  const missingCode =
...
type PreflightResult =
  ...
  | {
      ok: false;
      result: ZuvixPluginNodeInvokePolicyResult;
    };
...
async function handleFileTransferInvoke(
  ctx: ZuvixPluginNodeInvokePolicyContext,
): Promise<ZuvixPluginNodeInvokePolicyResult> {
...
export function createFileTransferNodeInvokePolicy(): ZuvixPluginNodeInvokePolicy {
  return {
