...
import type {
  ...
  ZuvixPluginNodeInvokePolicyContext,
...
function createPolicyContext(
  overrides: Partial<ZuvixPluginNodeInvokePolicyContext> = {},
): ZuvixPluginNodeInvokePolicyContext {
  return {
    nodeId: "node-1",
    command: "file.fetch",
    ...
    client: null,
    invokeNode: vi.fn<ZuvixPluginNodeInvokePolicyContext["invokeNode"]>(async () => ({
      ok: true,
      payload: { ok: true },
      payloadJSON: null,
    })),
...
describe("lazy file-transfer node invoke policy", () => {
  it("exposes command metadata without loading the delegate", () => {
    const loadPolicy = vi.fn<() => Promise<ZuvixPluginNodeInvokePolicy>>();
  ...
  it("loads and caches the delegate on first handle", async () => {
    const invokeNode = vi.fn<ZuvixPluginNodeInvokePolicyContext["invokeNode"]>(async () => ({
      ok: true,
      payload: { ok: true },
      payloadJSON: null,
    }));
    const delegateHandle = vi.fn<ZuvixPluginNodeInvokePolicy["handle"]>(async (ctx) => {
      await ctx.invokeNode();
      return { ok: true, payload: { delegated: true } };
    });
    const loadPolicy = vi.fn<() => Promise<ZuvixPluginNodeInvokePolicy>>(async () => ({
      commands: ["file.fetch"],
