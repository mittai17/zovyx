...
import type { ZuvixPluginNodeInvokePolicyContext } from "zuvix/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: ZuvixPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<ZuvixPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<ZuvixPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<ZuvixPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
