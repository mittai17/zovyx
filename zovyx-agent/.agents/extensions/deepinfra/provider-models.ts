...
import { isProviderApiKeyConfigured } from "zuvix/plugin-sdk/provider-auth";
import {
...
import { asPositiveSafeInteger } from "zuvix/plugin-sdk/string-coerce-runtime";
...
export const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=zuvix&filter=with_meta`;
...
// Static fallback. Chat rows live in zuvix.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=zuvix&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `ZUVIX_LIVE_*`
// indirection too).
