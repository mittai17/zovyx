...
describe("buildMinimaxSpeechProvider", () => {
  ...
  describe("isConfigured", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "zuvix-minimax-tts-auth-"));
  ...
  describe("synthesize", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "zuvix-minimax-tts-synth-"));
      tempAgentDir = path.join(tempStateDir, "agents", "main", "agent");
      await mkdir(tempAgentDir, { recursive: true });
      process.env = {
        ...savedEnv,
        ZUVIX_AGENT_DIR: tempAgentDir,
        ZUVIX_STATE_DIR: tempStateDir,
      };
      clearMinimaxAuthEnv();
      clearRuntimeAuthProfileStoreSnapshots();
