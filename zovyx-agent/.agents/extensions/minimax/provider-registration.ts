...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "zuvix/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "zuvix/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: ZuvixConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: ZuvixConfig): string | undefined {
...
export function registerMinimaxProviders(api: ZuvixPluginApi) {
