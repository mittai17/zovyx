...
import { logInfo } from "zuvix/plugin-sdk/logging-core";
...
import { defaultRuntime, type RuntimeEnv } from "zuvix/plugin-sdk/runtime-env";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      runtime.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("zuvix channels login")} and scan the QR again.`,
        ),
      );
