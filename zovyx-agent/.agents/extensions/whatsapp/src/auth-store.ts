...
import { info, success } from "zuvix/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("zuvix channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
