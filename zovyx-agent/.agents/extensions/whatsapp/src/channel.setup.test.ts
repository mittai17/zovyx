...
import { createQueuedWizardPrompter } from "zuvix/plugin-sdk/plugin-test-runtime";
...
vi.mock("zuvix/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: ZuvixConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<ZuvixConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
