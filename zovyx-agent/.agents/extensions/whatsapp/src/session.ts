...
import { VERSION } from "zuvix/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "zuvix/plugin-sdk/fetch-runtime";
import { danger, success } from "zuvix/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "zuvix/plugin-sdk/runtime-env";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("zuvix channels login")}`,
              ),
            );
          }
        }
