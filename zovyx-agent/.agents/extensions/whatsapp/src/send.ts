...
import { generateSecureUuid } from "zuvix/plugin-sdk/core";
import { redactIdentifier } from "zuvix/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "zuvix/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "zuvix/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "zuvix/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "zuvix/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: ZuvixConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`zuvix channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: ZuvixConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
