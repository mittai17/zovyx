...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/zuvix-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/zuvix-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
vi.mock("zuvix/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("zuvix/plugin-sdk/channel-activity-runtime")
  >("zuvix/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("Zuvix: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "zuvix-auth-"));
    } else {
      authDir = undefined;
    }
  });
