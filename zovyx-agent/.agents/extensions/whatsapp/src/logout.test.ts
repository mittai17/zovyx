...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "zuvix-test-web-logout-"));
    previousOAuthDir = process.env.ZUVIX_OAUTH_DIR;
    process.env.ZUVIX_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the Zuvix auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Zuvix auth root", async () => {
