...
import { resolveStateDir } from "zuvix/plugin-sdk/state-paths";
...
import { withMockedWindowsPlatform, withRestoredMocks } from "zuvix/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "zuvix/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredZuvixTmpDir(), "zuvix-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub ZUVIX_STATE_DIR.
    // Also keep it outside the Zuvix temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["ZUVIX_STATE_DIR"]);
    process.env.ZUVIX_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "zuvix-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredZuvixTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/zuvix` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredZuvixTmpDir();
  ...
  it("allows default Zuvix state workspace and sandbox roots", async () => {
  ...
  it("rejects default Zuvix state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
