...
import { formatEnvelopeTimestamp } from "zuvix/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("zuvix:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("zuvix:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[zuvix]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/zuvix-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("zuvix:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("zuvix:lastSocket")];
  if (typeof getter === "function") {
