...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "zuvix/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("zuvix/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("zuvix/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: ZuvixConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("zuvix/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `zuvix-outbound-${crypto.randomUUID()}.log`);
