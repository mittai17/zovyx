...
vi.mock("zuvix/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/logging-core")>(
    "zuvix/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("Zuvix WhatsApp listener close");
