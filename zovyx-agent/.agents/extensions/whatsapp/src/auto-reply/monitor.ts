...
import { resolveAccountEntry } from "zuvix/plugin-sdk/account-core";
import { CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "zuvix/plugin-sdk/approval-handler-runtime";
import { resolveInboundDebounceMs } from "zuvix/plugin-sdk/channel-inbound-debounce";
import { registerChannelRuntimeContext } from "zuvix/plugin-sdk/channel-runtime-context";
import { formatCliCommand } from "zuvix/plugin-sdk/cli-runtime";
import { isControlCommandMessage } from "zuvix/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "zuvix/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "zuvix/plugin-sdk/reply-history";
import { resolveAgentRoute } from "zuvix/plugin-sdk/routing";
import { logVerbose } from "zuvix/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "zuvix/plugin-sdk/runtime-env";
import { getChildLogger } from "zuvix/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  warn,
  type RuntimeEnv,
} from "zuvix/plugin-sdk/runtime-env";
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("zuvix channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("zuvix channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
        ...
        if (setupDecision) {
          ...
          if (setupDecision.action === "stop") {
            ...
            if (setupDecision.healthState === "logged-out") {
              ...
              runtime.error(
                `WhatsApp session logged out during setup. Run \`${formatCliCommand("zuvix channels login --channel whatsapp")}\` to relink.`,
              );
              ...
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("zuvix channels login --channel whatsapp")}\`. Stopping web monitoring.`,
              );
            } else {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}) after ${setupDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("zuvix channels login --channel whatsapp")}\` if the issue persists.`,
              );
            }
            await controller.shutdown();
            break;
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("zuvix channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("zuvix channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
