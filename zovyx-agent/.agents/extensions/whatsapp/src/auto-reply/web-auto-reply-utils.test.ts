...
import { upsertSessionEntry } from "zuvix/plugin-sdk/session-store-runtime";
import { withTempDir } from "zuvix/plugin-sdk/test-env";
...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\bzuvix\b/i] };
  ...
  it("ignores JID mentions in a true 1:1 self-chat (not a group)", () => {
    const cfg = { mentionRegexes: [/\bzuvix\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
  ...
  it("detects an explicit group @mention even when self is in allowFrom (#49317)", () => {
    ...
    const cfg = { mentionRegexes: [/\bzuvix\b/i], allowFrom: ["+15551234567"] };
    const msg = makeMsg({
...
describe("resolveMentionTargets with @lid mapping", () => {
  it("uses @lid reverse mapping for mentions and self identity", async () => {
    await withTempDir("zuvix-lid-mapping-", async (authDir) => {
      await fs.writeFile(
...
describe("getSessionSnapshot", () => {
  it("uses channel reset overrides when configured", async () => {
    ...
    try {
      await withTempDir("zuvix-snapshot-", async (root) => {
...
describe("web auto-reply util", () => {
  describe("mentions diagnostics", () => {
    it("returns normalized debug fields and mention outcome", () => {
      ...
      const result = debugMention(msg, { mentionRegexes: [/\bzuvix\b/i] });
      expect(result.wasMentioned).toBe(true);
      expect(result.details.bodyClean).toBe("zuvix ping");
