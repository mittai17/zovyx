...
import type { DmPolicy, GroupPolicy, ReplyToMode } from "zuvix/plugin-sdk/config-contracts";
import { resolveOAuthDir } from "zuvix/plugin-sdk/state-paths";
...
export function listWhatsAppAuthDirs(cfg: ZuvixConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: ZuvixConfig): boolean {
...
export function resolveWhatsAppAuthDir(params: { cfg: ZuvixConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: ZuvixConfig): ResolvedWhatsAppAccount[] {
