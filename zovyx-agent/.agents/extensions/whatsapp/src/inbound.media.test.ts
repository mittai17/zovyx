...
vi.mock("zuvix/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/channel-pairing")>(
...
vi.mock("zuvix/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/media-store")>(
    "zuvix/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("zuvix/plugin-sdk/plugin-state-test-runtime")>(
      "zuvix/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/zuvix-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `zuvix-inbound-media-${crypto.randomUUID()}`);
