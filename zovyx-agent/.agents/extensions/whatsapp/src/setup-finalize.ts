...
import type { ChannelSetupWizard } from "zuvix/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "zuvix/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<ZuvixConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: ZuvixConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: ZuvixConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: ZuvixConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: ZuvixConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("zuvix channels login"),
      }),
      "WhatsApp",
    );
  }
