...
import type { ZuvixPluginService } from "zuvix/plugin-sdk/core";
import { listDevicePairing } from "zuvix/plugin-sdk/device-bootstrap";
...
import type { PluginStateKeyedStore } from "zuvix/plugin-sdk/plugin-state-runtime";
...
async function readNotifyState(api: ZuvixPluginApi): Promise<NotifyStateFile> {
...
async function writeNotifyState(api: ZuvixPluginApi, state: NotifyStateFile): Promise<void> {
  const subscriberStore = openNotifySubscriberStore(api);
...
async function notifyPendingPairingRequests(params: { api: ZuvixPluginApi }): Promise<void> {
...
export function createPairingNotifierService(api: ZuvixPluginApi): ZuvixPluginService {
...
export function registerPairingNotifierService(api: ZuvixPluginApi): void {
