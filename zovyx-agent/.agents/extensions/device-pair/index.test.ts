...
import type { ZuvixPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredZuvixTmpDir: vi.fn(() => path.join(os.tmpdir(), "zuvix-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredZuvixTmpDir: pluginApiMocks.resolvePreferredZuvixTmpDir,
...
function createApi(params?: {
  ...
  runtime?: ZuvixPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as ZuvixPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: ZuvixPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as ZuvixPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredZuvixTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredZuvixTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Zuvix iOS app:");
    ...
    expect(text).not.toContain("![Zuvix pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Zuvix iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://zuvix.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://zuvix.local:18789");
  });
