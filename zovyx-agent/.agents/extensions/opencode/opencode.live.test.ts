// Opencode tests cover zuvix plugin behavior.
import {
  completeSimple,
...
import { extractNonEmptyAssistantText, isLiveTestEnabled } from "zuvix/plugin-sdk/test-env";
...
const LIVE_MODEL_ID =
  process.env.ZUVIX_LIVE_OPENCODE_DEEPSEEK_MODEL?.trim() || "deepseek-v4-flash-free";
...
function requireToolCall(message: AssistantMessage) {
  ...
  if (toolCall?.type !== "toolCall") {
    throw new Error(`Zuvix DeepSeek live model did not call a tool: ${message.stopReason}`);
  }
  return toolCall;
}
...
describeLive("zuvix plugin live", () => {
  it("accepts DeepSeek V4 tier-suffixed thinking replay after a tool call", async () => {
    ...
    if (first.stopReason === "error") {
      throw new Error(first.errorMessage || "Zuvix DeepSeek first turn returned an error");
    }
    ...
    if (second.stopReason === "error") {
      throw new Error(second.errorMessage || "Zuvix DeepSeek replay returned an error");
    }
