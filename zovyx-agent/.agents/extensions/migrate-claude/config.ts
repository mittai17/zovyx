    ...
    if (settingsPath && settings.hooks !== undefined) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation: "Review hook commands before recreating equivalent Zuvix automation.",
        }),
      );
    }
    ...
    if (settingsPath && settings.env !== undefined) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Move non-secret values manually and store credentials through Zuvix credential flows.",
        }),
      );
    }
  }
