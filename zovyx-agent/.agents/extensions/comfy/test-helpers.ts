...
export function buildComfyConfig(config: Record<string, unknown>): ZuvixConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): ZuvixConfig {
  return {
    models: {
      providers: {
        comfy: config,
