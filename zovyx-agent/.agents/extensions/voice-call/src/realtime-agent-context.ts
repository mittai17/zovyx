...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "zuvix/plugin-sdk/realtime-voice";
import { root } from "zuvix/plugin-sdk/security-runtime";
import { normalizeOptionalString as normalizeString } from "zuvix/plugin-sdk/string-coerce-runtime";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "Zuvix agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the Zuvix agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call zuvix_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];

  if (contextConfig.includeIdentity) {
    const identity = params.agentRuntime.resolveAgentIdentity(
      params.coreConfig as ZuvixConfig,
      agentId,
