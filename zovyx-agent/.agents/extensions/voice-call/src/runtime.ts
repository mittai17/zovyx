...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured Zuvix agent receiving delegated requests from a live phone voice bridge.",
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as ZuvixConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as ZuvixConfig),
    agentRuntime,
    log,
  );
  if (realtimeProvider) {
