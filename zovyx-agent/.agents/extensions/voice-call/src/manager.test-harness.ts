...
import { createPluginStateSyncKeyedStoreForTests } from "zuvix/plugin-sdk/plugin-state-test-runtime";
...
export function createTestStorePath(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-voice-call-test-"));
}
