...
import { describeAccountSnapshot } from "zuvix/plugin-sdk/account-helpers";
import { formatAllowFromLowercase } from "zuvix/plugin-sdk/allow-from";
import {
...
import { createChatChannelPlugin } from "zuvix/plugin-sdk/channel-core";
import {
...
import { getSessionBindingService } from "zuvix/plugin-sdk/conversation-runtime";
import {
  createChannelDirectoryAdapter,
  createRuntimeDirectoryLiveAdapter,
} from "zuvix/plugin-sdk/directory-runtime";
import { normalizeMessagePresentation } from "zuvix/plugin-sdk/interactive-runtime";
import { createLazyRuntimeNamedExport } from "zuvix/plugin-sdk/lazy-runtime";
import { parseStrictPositiveInteger } from "zuvix/plugin-sdk/number-runtime";
import { createComputedAccountStatusAdapter } from "zuvix/plugin-sdk/status-helpers";
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("zuvix/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("zuvix/plugin-sdk/config-mutation");
