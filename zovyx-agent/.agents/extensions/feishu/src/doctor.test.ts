...
function feishuConfig(): ZuvixConfig {
  return {
    channels: {
      feishu: {
        appId: "cli_xxx",
...
function stateDir(): string {
  const dir = process.env.ZUVIX_STATE_DIR;
  if (!dir) {
...
describe("Feishu doctor state repair", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-feishu-doctor-"));
