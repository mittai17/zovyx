...
import { resolveChannelConfigWrites } from "zuvix/plugin-sdk/channel-config-writes";
import { normalizeAccountId, resolveAgentRoute } from "zuvix/plugin-sdk/routing";
import type { ZuvixConfig, PluginRuntime } from "../runtime-api.js";
import { resolveFeishuAccount } from "./accounts.js";
import type { DynamicAgentCreationConfig } from "./types.js";

type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: ZuvixConfig;
  agentId?: string;
};
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: ZuvixConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: ZuvixConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as ZuvixConfig;
  ...
  let skippedCfg: ZuvixConfig | undefined;
      ...
      mutate: async (draft) => {
        ...
        if (!agentExists) {
          const workspaceTemplate =
            dynamicCfg.workspaceTemplate ?? "~/.zuvix/workspace-{agentId}";
          const agentDirTemplate =
            dynamicCfg.agentDirTemplate ?? "~/.zuvix/agents/{agentId}/agent";
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as ZuvixConfig,
