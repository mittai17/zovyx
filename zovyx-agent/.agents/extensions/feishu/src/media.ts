...
import type { MessageReceipt } from "zuvix/plugin-sdk/channel-outbound";
import { mediaKindFromMime } from "zuvix/plugin-sdk/media-mime";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS, runFfmpeg } from "zuvix/plugin-sdk/media-runtime";
import { saveMediaBuffer, saveMediaStream, type SavedMedia } from "zuvix/plugin-sdk/media-store";
import { readByteStreamWithLimit } from "zuvix/plugin-sdk/response-limit-runtime";
import { readRegularFile, writeExternalFileWithinRoot } from "zuvix/plugin-sdk/security-runtime";
...
async function downloadMessageResourceWithType(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "zuvix-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredZuvixTmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
