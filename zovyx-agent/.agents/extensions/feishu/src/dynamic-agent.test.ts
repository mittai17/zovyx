...
beforeEach(async () => {
  tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "zuvix-feishu-agent-"));
});
...
function createRuntime(
  currentCfg?: ZuvixConfig,
  persistedCfg?: ZuvixConfig,
  mutationCfg?: ZuvixConfig,
) {
  let runtimeCfg = structuredClone(currentCfg ?? ({} as ZuvixConfig));
  const commitConfig = vi.fn();
  const mutateConfigFile = vi.fn(
    async (params: {
      mutate: (draft: ZuvixConfig, context: { snapshot: never; previousHash: null }) => unknown;
    }) => {
...
describe("maybeCreateDynamicAgent", () => {
  ...
  it("rechecks current ingress inside the config mutation lock", async () => {
    ...
    const canCreateForConfig = vi
      .fn<(cfg: ZuvixConfig) => Promise<boolean>>()
  ...
  it("preserves concurrent runtime config when creating from a stale request snapshot", async () => {
    ...
    const result = await maybeCreateDynamicAgent({
      cfg: { agents: { list: [] }, bindings: [] } as ZuvixConfig,
      runtime,
      accountId: "default",
      senderOpenId: "ou_sender",
      canCreateForConfig: async () => true,
