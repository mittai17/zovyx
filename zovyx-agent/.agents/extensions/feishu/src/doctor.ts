...
function collectConfiguredAgentIds(cfg: ZuvixConfig): string[] {
...
function resolveConfiguredDefaultAgentId(cfg: ZuvixConfig): string {
...
function formatPreviewWarning(inspection: FeishuDoctorInspection): string {
  ...
  return [
    ...
    '- Run "zuvix doctor --fix" to rebuild Feishu local state.',
  ].join("\n");
}
...
function hasConfiguredFeishuChannel(cfg: ZuvixConfig): boolean {
