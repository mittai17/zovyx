...
import { createPluginStateKeyedStoreForTests as createPluginStateKeyedStore } from "zuvix/plugin-sdk/plugin-state-test-runtime";
import type {
...
describe("workboard doctor contract", () => {
  it("migrates shipped .28 plugin-state workboard data into sqlite", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-workboard-doctor-"));
    const env = { ...process.env, ZUVIX_STATE_DIR: stateDir };
    try {
