...
vi.mock("zuvix/plugin-sdk/runtime-env", () => ({
...
import type { ZuvixPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/zuvix-diagnostics-otel-test";
...
const ORIGINAL_ZUVIX_OTEL_PRELOADED = process.env.ZUVIX_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  captureContent?: NonNullable<
    NonNullable<ZuvixPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["captureContent"];
};
...
function createTraceOnlyContext(endpoint: string): ZuvixPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.ZUVIX_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_ZUVIX_OTEL_PRELOADED === undefined) {
      delete process.env.ZUVIX_OTEL_PRELOADED;
    } else {
      process.env.ZUVIX_OTEL_PRELOADED = ORIGINAL_ZUVIX_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["zuvix.provider"]).toBe("openai");
    for (const key of [
      "zuvix.callId",
      "zuvix.call_id",
      "zuvix.chatId",
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("zuvix.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "zuvix.channel": "telegram",
      "zuvix.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("zuvix.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "zuvix.channel": "telegram",
      "zuvix.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("zuvix.message.queued")?.add).toHaveBeenCalledWith(1, {
      "zuvix.channel": "telegram",
      "zuvix.source": "telegram",
    });
    expect(telemetryState.histograms.get("zuvix.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("zuvix.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "zuvix.channel": "telegram",
      "zuvix.source": "telegram",
    });
    expect(telemetryState.histograms.get("zuvix.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "zuvix.lane": "main",
    });
    expect(telemetryState.counters.get("zuvix.message.processed")?.add).toHaveBeenCalledWith(1, {
      "zuvix.channel": "telegram",
      "zuvix.outcome": "completed",
    });
    expect(telemetryState.counters.get("zuvix.message.received")?.add).toHaveBeenCalledWith(1, {
      "zuvix.channel": "telegram",
      "zuvix.source": "webhook",
    });
    expect(telemetryState.counters.get("zuvix.message.received")?.add).toHaveBeenCalledWith(1, {
      "zuvix.channel": "unknown",
      "zuvix.source": "unknown",
    });
    expect(
      telemetryState.counters.get("zuvix.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("zuvix.message.dispatch.completed")?.add,
    ...
    expect(
      telemetryState.histograms.get("zuvix.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "zuvix.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("zuvix.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "zuvix.channel": "telegram",
      "zuvix.outcome": "completed",
    });
    expect(telemetryState.histograms.get("zuvix.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "zuvix.lane": "main",
      },
    );
    expect(telemetryState.counters.get("zuvix.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("zuvix.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "zuvix.state": "processing",
    });
    expect(
      telemetryState.histograms.get("zuvix.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "zuvix.state": "processing",
    });
    expect(telemetryState.counters.get("zuvix.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "zuvix.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("zuvix.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "zuvix.agent": "agent.default",
        "zuvix.channel": "telegram",
        "zuvix.trigger": "user",
      },
    );
    ...
    expect(spanNames).toContain("zuvix.webhook.processed");
    expect(spanNames).toContain("zuvix.message.processed");
    expect(spanNames).toContain("zuvix.session.stuck");
    const webhookSpanOptions = startedSpanOptions("zuvix.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("zuvix.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("zuvix.message.processed");
    expect(messageSpanOptions?.attributes?.["zuvix.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["zuvix.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["zuvix.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("zuvix.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("zuvix.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.ZUVIX_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("zuvix.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["zuvix.provider"]).toBe("openai");
    expect(runDurationAttributes?.["zuvix.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("zuvix.run");
    expect(runSpanOptions?.attributes?.["zuvix.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("zuvix.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "zuvix.exporter": "diagnostics-otel",
      "zuvix.signal": "logs",
      "zuvix.status": "started",
      "zuvix.reason": "configured",
    });
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("zuvix.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "zuvix.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("zuvix.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "zuvix.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("zuvix.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "zuvix.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("zuvix.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["zuvix.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["zuvix.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["zuvix.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "zuvix.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("zuvix.payload.large")?.add).toHaveBeenCalledWith(1, {
      "zuvix.payload.action": "rejected",
      "zuvix.payload.surface": "gateway.frame",
      "zuvix.channel": "web",
      "zuvix.plugin": "none",
      "zuvix.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("zuvix.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "zuvix.exporter": "diagnostics-otel",
      "zuvix.signal": "logs",
      "zuvix.status": "failure",
      "zuvix.reason": "emit_failed",
      "zuvix.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("zuvix.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("zuvix.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["zuvix.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["zuvix.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("zuvix.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["zuvix.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const modelCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "zuvix.model.call",
    );
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["zuvix.content.input_messages"]).toBe("user prompt");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["zuvix.token"];
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "zuvix.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "zuvix.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "zuvix.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/zuvix/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/zuvix/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["zuvix.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["zuvix.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `zuvix.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "zuvix.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "zuvix.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "zuvix.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "zuvix.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "zuvix.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("zuvix.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "zuvix.channel": "webchat",
      "zuvix.agent": "ops",
      "zuvix.provider": "openai",
      "zuvix.model": "gpt-5.4",
      "zuvix.token": "input",
    });
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("zuvix.tokens")?.add).toHaveBeenCalledWith(2, {
      "zuvix.channel": "unknown",
      "zuvix.agent": "unknown",
      "zuvix.provider": "openai",
      "zuvix.model": "gpt-5.4",
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("zuvix.tokens")?.add.mock.calls),
  ...
  test("drops session-shaped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("zuvix.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "zuvix.lane": "session",
      },
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("zuvix.queue.lane.enqueue")?.add.mock.calls),
    ).not.toContain("Agent:qa:otel-trace-smoke");
    await service.stop?.(ctx);
  });
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("zuvix.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "zuvix.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "zuvix.sessionId")).toBe(false);
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "zuvix.agent": "main",
      "zuvix.skill.activation": "read",
      "zuvix.skill.name": "tiny-llm-brainstorm",
      "zuvix.skill.source": "workspace",
      "zuvix.toolName": "read",
    };
    expect(telemetryState.counters.get("zuvix.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "zuvix.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("zuvix.run");
    expect(spanNames).toContain("zuvix.model.call");
    expect(spanNames).toContain("zuvix.harness.run");
    expect(spanNames).toContain("zuvix.tool.execution");

    const runOptions = startedSpanOptions("zuvix.run");
    expect(runOptions?.attributes?.["zuvix.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["zuvix.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["zuvix.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["zuvix.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "zuvix.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "zuvix.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "zuvix.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("zuvix.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "zuvix.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "zuvix.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "zuvix.sessionKey")).toBe(false);
    ...
    const harnessCall = startedSpanCall("zuvix.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["zuvix.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["zuvix.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["zuvix.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["zuvix.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["zuvix.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["zuvix.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["zuvix.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["zuvix.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["zuvix.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["zuvix.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["zuvix.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "zuvix.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "zuvix.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "zuvix.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "zuvix.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("zuvix.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["zuvix.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["zuvix.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["zuvix.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["zuvix.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["zuvix.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "zuvix.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "zuvix.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "zuvix.sessionKey")).toBe(false);
    expect(toolOptions?.startTime).toBeTypeOf("number");
    expect(toolCall?.[2]).toBeUndefined();

    const modelCallDuration = lastHistogramRecord("zuvix.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["zuvix.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["zuvix.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("zuvix.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["zuvix.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["zuvix.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("zuvix.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["zuvix.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["zuvix.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("zuvix.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["zuvix.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["zuvix.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("zuvix.model.call");
    expect(modelSpanAttributes["zuvix.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["zuvix.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["zuvix.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("zuvix.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "zuvix.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("zuvix.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["zuvix.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["zuvix.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["zuvix.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "zuvix.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "zuvix.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("zuvix.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["zuvix.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "zuvix.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "zuvix.runId")).toBe(false);

    const toolSpan = spanByName("zuvix.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("zuvix.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("zuvix.model.failover");
    expect(failoverOptions?.attributes?.["zuvix.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["zuvix.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["zuvix.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["zuvix.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["zuvix.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["zuvix.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["zuvix.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["zuvix.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "zuvix.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "zuvix.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("zuvix.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("zuvix.model.failover")).toStrictEqual([
      1,
      {
        "zuvix.failover.reason": "overloaded",
        "zuvix.failover.suspended": "true",
        "zuvix.lane": "main",
        "zuvix.model": "claude-opus-4-6",
        "zuvix.provider": "anthropic",
        "zuvix.failover.to_model": "gpt-5.4",
        "zuvix.failover.to_provider": "openai",
      },
    ]);
    await service.stop?.(ctx);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("zuvix.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "zuvix.toolName": "browser",
        "zuvix.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "zuvix.tool.owner": "browser-tools",
        "zuvix.tool.params.kind": "object",
        "zuvix.deniedReason": "tools.deny",
      },
    ]);
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("zuvix.model.failover");
    expect(failoverOptions?.attributes?.["zuvix.lane"]).toBe("session");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "zuvix.model.call")
  ...
  test("uses latest GenAI inference span shape only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("zuvix.model.call")).toBeUndefined();
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("zuvix.model.call");
    expect(modelCallOptions?.attributes?.["zuvix.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "zuvix.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "zuvix.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("zuvix.provider.request", {
      "zuvix.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("zuvix.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["zuvix.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "zuvix.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("zuvix.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "zuvix.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["zuvix.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["zuvix.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["zuvix.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["zuvix.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["zuvix.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["zuvix.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["zuvix.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["zuvix.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["zuvix.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["zuvix.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["zuvix.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["zuvix.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["zuvix.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("zuvix.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "zuvix.toolName": "process",
      "zuvix.loop.level": "critical",
      "zuvix.loop.action": "block",
      "zuvix.loop.detector": "known_poll_no_progress",
      "zuvix.loop.count": 20,
      "zuvix.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("zuvix.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["zuvix.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["zuvix.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["zuvix.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["zuvix.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["zuvix.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["zuvix.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "zuvix.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("zuvix.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("zuvix.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "zuvix.memory.level": "critical",
        "zuvix.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("zuvix.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "zuvix.memory.level": "critical",
      "zuvix.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("zuvix.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["zuvix.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["zuvix.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["zuvix.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["zuvix.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["zuvix.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["zuvix.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["zuvix.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["zuvix.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["zuvix.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["zuvix.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "zuvix.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("zuvix.diagnostic.async_queue.dropped");
    expect(counter?.add).toHaveBeenCalledWith(4, {
      "zuvix.diagnostic.async_queue.drop_class": "total",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "zuvix.diagnostic.async_queue.drop_class": "trusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(2, {
      "zuvix.diagnostic.async_queue.drop_class": "untrusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "zuvix.diagnostic.async_queue.drop_class": "priority",
    });
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "zuvix.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "zuvix.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "zuvix.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["zuvix.run"]).toBeUndefined();
    expect(parentBySpanName["zuvix.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["zuvix.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("zuvix.message.processed");
    const harnessSpan = spanByName("zuvix.harness.run");
    const runSpan = spanByName("zuvix.run");
    const usageSpan = spanByName("zuvix.model.usage");
    const modelCallSpan = spanByName("zuvix.model.call");
    ...
    expect(parentBySpanName["zuvix.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["zuvix.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["zuvix.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["zuvix.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["zuvix.model.call"]?.spanId).toBe(runSpanContext.spanId);
    await service.stop?.(ctx);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("zuvix.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("zuvix.message.processed")[0]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("zuvix.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("zuvix.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("zuvix.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("zuvix.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("zuvix.message.processed")["zuvix.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
    await service.stop?.(ctx);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("zuvix.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("zuvix.harness.run")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("zuvix.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not parent sibling active runs through shared upstream aliases", async () => {
    ...
    const runContexts = startedSpanParentContextsByName("zuvix.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("zuvix.run").spanContext();
    const modelParentContext = startedSpanParentContexts("zuvix.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("zuvix.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("zuvix.model.usage")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["zuvix.run"]).toBeUndefined();
    expect(parentBySpanName["zuvix.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["zuvix.run"]).toBeUndefined();
    expect(parentBySpanName["zuvix.model.call"]).toBeUndefined();
    expect(parentBySpanName["zuvix.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "zuvix.run"),
    ).toHaveLength(1);
    expect(
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "zuvix.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "zuvix.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("zuvix.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["zuvix.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["zuvix.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["zuvix.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["zuvix.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("zuvix.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["zuvix.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["zuvix.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["zuvix.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["zuvix.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["zuvix.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["zuvix.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["zuvix.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "zuvix.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "zuvix.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "zuvix.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("zuvix.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("zuvix.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("zuvix.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "zuvix.channel": "matrix",
      "zuvix.delivery.kind": "text",
    });
    const deliveryDurationRecords = telemetryState.histograms.get(
      "zuvix.message.delivery.duration_ms",
    )?.record.mock.calls as Array<[unknown, Record<string, unknown>]>;
    expect(deliveryDurationRecords[0]?.[0]).toBe(25);
    expect(deliveryDurationRecords[0]?.[1]["zuvix.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["zuvix.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["zuvix.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["zuvix.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["zuvix.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["zuvix.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["zuvix.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "zuvix.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["zuvix.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["zuvix.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["zuvix.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["zuvix.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["zuvix.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["zuvix.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["zuvix.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["zuvix.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "zuvix.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "zuvix.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "zuvix.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "zuvix.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "zuvix.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "zuvix.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "zuvix.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("zuvix.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["zuvix.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["zuvix.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["zuvix.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("zuvix.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["zuvix.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["zuvix.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["zuvix.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["zuvix.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("zuvix.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["zuvix.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["zuvix.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["zuvix.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("zuvix.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["zuvix.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["zuvix.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["zuvix.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("zuvix.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["zuvix.status"]).toBe("released");
    expect(telemetryState.counters.get("zuvix.talk.event")?.add).toHaveBeenCalledWith(1, {
      "zuvix.talk.brain": "agent-consult",
      "zuvix.talk.event_type": "input.audio.delta",
      "zuvix.talk.mode": "realtime",
      "zuvix.talk.provider": "openai",
      "zuvix.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("zuvix.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "zuvix.talk.brain": "agent-consult",
        "zuvix.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("zuvix.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "zuvix.talk.brain": "agent-consult",
      "zuvix.talk.event_type": "latency.metrics",
      "zuvix.talk.mode": "realtime",
      "zuvix.talk.provider": "openai",
      "zuvix.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("zuvix.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("zuvix.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "zuvix.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "zuvix.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "zuvix.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("zuvix.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "zuvix.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "zuvix.content.tool_output")).toBe(
      false,
    );
    expect(toolOptions?.startTime).toBeTypeOf("number");
    await service.stop?.(ctx);
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["zuvix.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["zuvix.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["zuvix.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["zuvix.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["zuvix.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["zuvix.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("omits absent model content fields when capture fields are opted in", async () => {
    ...
    expect(attrs["zuvix.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "zuvix.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "zuvix.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "zuvix.content.tool_definitions")).toBe(false);
  ...
  test("exports tool definitions without requiring input message capture", async () => {
    ...
    expect(Object.hasOwn(attrs ?? {}, "zuvix.content.input_messages")).toBe(false);
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.tool.definitions"))).toEqual([
      {
        type: "function",
        name: "lookup",
    ...
    expect(JSON.parse(String(attrs?.["zuvix.content.tool_definitions"]))).toEqual([
      {
        name: "lookup",
        description: "Lookup data",
        parameters: { type: "object" },
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("zuvix.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["zuvix.reason"])).toContain("…");
    expect(typeof attrs?.["zuvix.reason"]).toBe("string");
    expect(String(attrs?.["zuvix.reason"])).not.toContain(
