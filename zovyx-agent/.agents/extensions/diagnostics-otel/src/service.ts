...
import { waitForDiagnosticEventsDrained } from "zuvix/plugin-sdk/diagnostic-runtime";
import { registerUnhandledRejectionHandler } from "zuvix/plugin-sdk/runtime-env";
import type {
...
const DEFAULT_SERVICE_NAME = "zuvix";
...
const PRELOADED_OTEL_SDK_ENV = "ZUVIX_OTEL_PRELOADED";
...
function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "zuvix.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "zuvix.model_call.response_bytes", evt.responseStreamBytes);
  assignPositiveNumberAttr(
    attrs,
    "zuvix.model_call.time_to_first_byte_ms",
...
function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("zuvix.provider.request", {
    "zuvix.upstreamRequestIdHash": boundedHash,
  });
}
...
function assignOtelModelContentAttributes(
  ...
  if (policy.toolDefinitions) {
    assignOtelContentAttribute(
      attributes,
      "zuvix.content.tool_definitions",
  ...
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "zuvix.content.output_messages",
      content?.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "zuvix.content.system_prompt", content?.systemPrompt);
  }
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    assignOtelContentAttribute(attributes, "zuvix.content.tool_input", content?.toolInput);
  }
  if (policy.toolOutputs) {
    assignOtelContentAttribute(attributes, "zuvix.content.tool_output", content?.toolOutput);
  }
}
...
function assignOtelLogEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(attributes, `zuvix.${key}`, eventAttributes[rawKey]);
  }
}
...
function addTraceAttributes(
  ...
  attributes["zuvix.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["zuvix.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["zuvix.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["zuvix.traceFlags"] = normalized.traceFlags;
  }
}

export function createDiagnosticsOtelService(): ZuvixPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("zuvix");
      const tracer = trace.getTracer("zuvix");
      ...
      const tokensCounter = meter.createCounter("zuvix.tokens", {
        unit: "1",
      ...
      const costCounter = meter.createCounter("zuvix.cost.usd", {
        unit: "1",
        description: "Estimated model cost (USD)",
      });
      const durationHistogram = meter.createHistogram("zuvix.run.duration_ms", {
        unit: "ms",
        description: "Agent run duration",
      });
      const harnessDurationHistogram = meter.createHistogram("zuvix.harness.duration_ms", {
        unit: "ms",
        description: "Agent harness lifecycle duration",
      });
      const contextHistogram = meter.createHistogram("zuvix.context.tokens", {
        unit: "1",
        description: "Context window size and usage",
      });
      const webhookReceivedCounter = meter.createCounter("zuvix.webhook.received", {
        unit: "1",
        description: "Webhook requests received",
      });
      const webhookErrorCounter = meter.createCounter("zuvix.webhook.error", {
        unit: "1",
        description: "Webhook processing errors",
      });
      const webhookDurationHistogram = meter.createHistogram("zuvix.webhook.duration_ms", {
        unit: "ms",
        description: "Webhook processing duration",
      });
      const messageQueuedCounter = meter.createCounter("zuvix.message.queued", {
        unit: "1",
        description: "Messages queued for processing",
      });
      const messageReceivedCounter = meter.createCounter("zuvix.message.received", {
        unit: "1",
        description: "Inbound messages received",
      });
      const messageDispatchStartedCounter = meter.createCounter(
        "zuvix.message.dispatch.started",
        {
          unit: "1",
      ...
      const messageDispatchCompletedCounter = meter.createCounter(
        "zuvix.message.dispatch.completed",
        {
          unit: "1",
      ...
      const messageDispatchDurationHistogram = meter.createHistogram(
        "zuvix.message.dispatch.duration_ms",
        {
          unit: "ms",
          description: "Inbound message dispatch duration",
        },
      );
      const messageProcessedCounter = meter.createCounter("zuvix.message.processed", {
        unit: "1",
        description: "Messages processed by outcome",
      });
      const messageDurationHistogram = meter.createHistogram("zuvix.message.duration_ms", {
        unit: "ms",
        description: "Message processing duration",
      });
      const messageDeliveryStartedCounter = meter.createCounter(
        "zuvix.message.delivery.started",
        {
          unit: "1",
      ...
      const queueDepthHistogram = meter.createHistogram("zuvix.queue.depth", {
        unit: "1",
        description: "Queue depth on enqueue/dequeue",
      });
      const queueWaitHistogram = meter.createHistogram("zuvix.queue.wait_ms", {
        unit: "ms",
        description: "Queue wait time before execution",
      });
      const laneEnqueueCounter = meter.createCounter("zuvix.queue.lane.enqueue", {
        unit: "1",
        description: "Command queue lane enqueue events",
      });
      const laneDequeueCounter = meter.createCounter("zuvix.queue.lane.dequeue", {
        unit: "1",
        description: "Command queue lane dequeue events",
      });
      const sessionStateCounter = meter.createCounter("zuvix.session.state", {
        unit: "1",
        description: "Session state transitions",
      });
      const sessionTurnCreatedCounter = meter.createCounter("zuvix.session.turn.created", {
        unit: "1",
        description: "Agent session turns created",
      });
      const sessionStuckCounter = meter.createCounter("zuvix.session.stuck", {
        unit: "1",
        description: "Sessions stuck in processing",
      });
      const sessionStuckAgeHistogram = meter.createHistogram("zuvix.session.stuck_age_ms", {
        unit: "ms",
        description: "Age of stuck sessions",
      });
      const sessionRecoveryRequestedCounter = meter.createCounter(
        "zuvix.session.recovery.requested",
        {
          unit: "1",
      ...
      const sessionRecoveryCompletedCounter = meter.createCounter(
        "zuvix.session.recovery.completed",
        {
          unit: "1",
      ...
      const sessionRecoveryAgeHistogram = meter.createHistogram(
        "zuvix.session.recovery.age_ms",
        {
          unit: "ms",
          description: "Age of sessions selected for recovery",
        },
      );
      const talkEventCounter = meter.createCounter("zuvix.talk.event", {
        unit: "1",
        description: "Talk events emitted by type",
      });
      const talkEventDurationHistogram = meter.createHistogram("zuvix.talk.event.duration_ms", {
        unit: "ms",
        description: "Talk event duration when reported",
      });
      const talkAudioBytesHistogram = meter.createHistogram("zuvix.talk.audio.bytes", {
        unit: "By",
        description: "Talk audio frame byte lengths",
      });
      const runAttemptCounter = meter.createCounter("zuvix.run.attempt", {
        unit: "1",
        description: "Run attempts",
      });
      const toolLoopCounter = meter.createCounter("zuvix.tool.loop", {
        unit: "1",
        description: "Detected repetitive tool-call loop events",
      });
      const skillUsedCounter = meter.createCounter("zuvix.skill.used", {
        unit: "1",
        description: "Skills used by agent runs",
      });
      const modelCallDurationHistogram = meter.createHistogram("zuvix.model_call.duration_ms", {
        unit: "ms",
        description: "Model call duration",
      });
      const modelCallRequestBytesHistogram = meter.createHistogram(
        "zuvix.model_call.request_bytes",
        {
          unit: "By",
      ...
      const modelCallResponseBytesHistogram = meter.createHistogram(
        "zuvix.model_call.response_bytes",
        {
          unit: "By",
      ...
      const modelFailoverCounter = meter.createCounter("zuvix.model.failover", {
        unit: "1",
      ...
      const toolExecutionDurationHistogram = meter.createHistogram(
        "zuvix.tool.execution.duration_ms",
        {
          unit: "ms",
          description: "Tool execution duration",
        },
      );
      const toolExecutionBlockedCounter = meter.createCounter("zuvix.tool.execution.blocked", {
        unit: "1",
        description: "Tool executions blocked by policy or sandbox diagnostics",
      });
      const execProcessDurationHistogram = meter.createHistogram("zuvix.exec.duration_ms", {
        unit: "ms",
        description: "Exec process duration",
      });
      const memoryRssHistogram = meter.createHistogram("zuvix.memory.rss_bytes", {
        unit: "By",
        description: "Resident set size reported by diagnostic memory samples",
      });
      const memoryHeapUsedHistogram = meter.createHistogram("zuvix.memory.heap_used_bytes", {
        unit: "By",
        description: "Heap used bytes reported by diagnostic memory samples",
      });
      const memoryHeapTotalHistogram = meter.createHistogram("zuvix.memory.heap_total_bytes", {
        unit: "By",
        description: "Heap total bytes reported by diagnostic memory samples",
      });
      const memoryExternalHistogram = meter.createHistogram("zuvix.memory.external_bytes", {
        unit: "By",
      ...
      const livenessEventLoopDelayMaxHistogram = meter.createHistogram(
        "zuvix.liveness.event_loop_delay_max_ms",
        {
          unit: "ms",
      ...
      const livenessEventLoopUtilizationHistogram = meter.createHistogram(
        "zuvix.liveness.event_loop_utilization",
        {
          unit: "1",
      ...
      const livenessCpuCoreRatioHistogram = meter.createHistogram(
        "zuvix.liveness.cpu_core_ratio",
        {
          unit: "1",
          description: "CPU core ratio reported by diagnostic liveness warnings",
        },
      );
      const telemetryExporterCounter = meter.createCounter("zuvix.telemetry.exporter.events", {
        unit: "1",
      ...
      if (logsEnabled) {
        ...
        const otelLogger = logProvider.getLogger("zuvix");
        recordLogRecord = (evt, metadata) => {
          try {
            ...
            assignOtelLogAttribute(attributes, "zuvix.log.level", logLevelName);
            if (evt.loggerName) {
              assignOtelLogAttribute(attributes, "zuvix.logger", evt.loggerName);
            }
            if (evt.loggerParents?.length) {
              assignOtelLogAttribute(
                attributes,
                "zuvix.logger.parents",
      ...
      const addRunAttrs = (
        ...
        if (evt.provider) {
          spanAttrs["zuvix.provider"] = evt.provider;
        }
        if (evt.model) {
          spanAttrs["zuvix.model"] = evt.model;
        }
        if (evt.channel) {
          spanAttrs["zuvix.channel"] = evt.channel;
        }
        if (evt.trigger) {
          spanAttrs["zuvix.trigger"] = evt.trigger;
        }
      };
      ...
      const paramsSummaryAttrs = (
        ...
        return {
          "zuvix.tool.params.kind": summary.kind,
          ...("length" in summary ? { "zuvix.tool.params.length": summary.length } : {}),
        };
      };
      ...
      const recordModelUsage = (
        ...
        const attrs = {
          "zuvix.channel": evt.channel ?? "unknown",
          "zuvix.agent": lowCardinalityAttr(evt.agentId),
          "zuvix.provider": evt.provider ?? "unknown",
          "zuvix.model": evt.model ?? "unknown",
        };
        ...
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "zuvix.token": "input" });
        ...
        if (usage.output) {
          tokensCounter.add(usage.output, { ...attrs, "zuvix.token": "output" });
        ...
        if (usage.cacheRead) {
          tokensCounter.add(usage.cacheRead, { ...attrs, "zuvix.token": "cache_read" });
        }
        if (usage.cacheWrite) {
          tokensCounter.add(usage.cacheWrite, { ...attrs, "zuvix.token": "cache_write" });
        }
        if (usage.promptTokens) {
          tokensCounter.add(usage.promptTokens, { ...attrs, "zuvix.token": "prompt" });
        }
        if (usage.total) {
          tokensCounter.add(usage.total, { ...attrs, "zuvix.token": "total" });
        }
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "zuvix.tokens.input": usage.input ?? 0,
          "zuvix.tokens.output": usage.output ?? 0,
          "zuvix.tokens.cache_read": usage.cacheRead ?? 0,
          "zuvix.tokens.cache_write": usage.cacheWrite ?? 0,
          "zuvix.tokens.total": usage.total ?? 0,
        };
        ...
        const span = spanWithDuration("zuvix.model.usage", spanAttrs, evt.durationMs, {
      ...
      const recordWebhookReceived = (
        ...
        const attrs = {
          "zuvix.channel": evt.channel ?? "unknown",
          "zuvix.webhook": evt.updateType ?? "unknown",
        };
      ...
      const recordWebhookProcessed = (
        ...
        const attrs = {
          "zuvix.channel": lowCardinalityAttr(evt.channel),
          "zuvix.webhook": lowCardinalityAttr(evt.updateType),
        };
        ...
        const span = spanWithDuration("zuvix.webhook.processed", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookError = (
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "zuvix.error": redactedError,
        };
        const span = tracer.startSpan("zuvix.webhook.error", {
      ...
      const recordMessageQueued = (
        ...
        const attrs = {
          "zuvix.channel": lowCardinalityAttr(evt.channel),
          "zuvix.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageDispatchStarted = (
        ...
        trackInternalOrTrustedSpan(
          evt,
          metadata,
          spanWithDuration("zuvix.message.processed", attrs, undefined, {
      ...
      const recordMessageDispatchCompleted = (
        ...
        const attrs = {
          "zuvix.channel": lowCardinalityAttr(evt.channel),
          "zuvix.outcome": evt.outcome,
          "zuvix.reason": lowCardinalityAttr(evt.reason, "none"),
          "zuvix.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageProcessed = (
        ...
        const attrs = {
          "zuvix.channel": lowCardinalityAttr(evt.channel),
          "zuvix.outcome": evt.outcome ?? "unknown",
        };
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        if (evt.reason) {
          spanAttrs["zuvix.reason"] = lowCardinalityAttr(evt.reason, "unknown");
        }
        const trackedSpan = getTrackedInternalOrTrustedSpan(evt, metadata);
        const span =
          trackedSpan ??
          spanWithDuration("zuvix.message.processed", spanAttrs, evt.durationMs, {
            parentContext: internalOrTrustedExplicitParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
      ...
      const messageDeliveryAttrs = (
        ...
        "zuvix.channel": lowCardinalityAttr(evt.channel),
        "zuvix.delivery.kind": lowCardinalityAttr(evt.deliveryKind, "other"),
      });
      ...
      const recordMessageDeliveryCompleted = (
        ...
        const span = spanWithDuration(
          ...
          {
            ...attrs,
            "zuvix.delivery.result_count": evt.resultCount,
          },
          evt.durationMs,
      ...
      const recordMessageDeliveryError = (
        ...
        const attrs = {
          ...messageDeliveryAttrs(evt),
          "zuvix.outcome": "error",
          "zuvix.errorCategory": lowCardinalityAttr(evt.errorCategory, "other"),
        };
        messageDeliveryDurationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        }
        const span = spanWithDuration("zuvix.message.delivery", attrs, evt.durationMs, {
      ...
      const recordRunStarted = (
        ...
        const span = trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("zuvix.run", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordLaneEnqueue = (
        ...
        const attrs = { "zuvix.lane": lowCardinalityQueueLaneAttr(evt.lane) };
      ...
      const recordSessionState = (
        ...
        const attrs: Record<string, string> = { "zuvix.state": evt.state };
        if (evt.reason) {
          attrs["zuvix.reason"] = redactSensitiveText(evt.reason);
        }
      ...
      const recordSessionTurnCreated = (
        ...
        sessionTurnCreatedCounter.add(1, {
          "zuvix.agent": lowCardinalityAttr(evt.agentId, "unknown"),
          "zuvix.channel": lowCardinalityAttr(evt.channel, "unknown"),
          "zuvix.trigger": evt.trigger,
        });
      };
      ...
      const recordSessionStuck = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        spanAttrs["zuvix.queueDepth"] = evt.queueDepth ?? 0;
        spanAttrs["zuvix.ageMs"] = evt.ageMs;
        const span = tracer.startSpan("zuvix.session.stuck", { attributes: spanAttrs });
      ...
      const sessionRecoveryAttrs = (evt: SessionRecoveryDiagnosticEvent) => {
        ...
        if (evt.activeWorkKind) {
          attrs["zuvix.active_work_kind"] = evt.activeWorkKind;
        }
        return attrs;
      };
      ...
      const recordSessionRecoveryRequested = (
        ...
        attrs["zuvix.action"] = evt.allowActiveAbort ? "abort" : "recover";
      ...
      const recordSessionRecoveryCompleted = (
        ...
        const attrs = sessionRecoveryAttrs(evt);
        attrs["zuvix.status"] = evt.status;
        attrs["zuvix.action"] = lowCardinalityAttr(evt.action, "unknown");
        if (evt.outcomeReason) {
          attrs["zuvix.reason"] = redactSensitiveText(evt.outcomeReason);
        }
      ...
      const talkEventAttrs = (evt: TalkDiagnosticEvent): Record<string, string> => ({
        "zuvix.talk.brain": lowCardinalityAttr(evt.brain),
        "zuvix.talk.event_type": lowCardinalityAttr(evt.talkEventType),
        "zuvix.talk.mode": lowCardinalityAttr(evt.mode),
        "zuvix.talk.provider": lowCardinalityAttr(evt.provider),
        "zuvix.talk.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordRunAttempt = (evt: Extract<DiagnosticEventPayload, { type: "run.attempt" }>) => {
        runAttemptCounter.add(1, { "zuvix.attempt": evt.attempt });
      };

      const toolLoopAttrs = (
        evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>,
      ): Record<string, string | number> => ({
        "zuvix.toolName": lowCardinalityAttr(evt.toolName, "tool"),
        "zuvix.loop.level": evt.level,
        "zuvix.loop.action": evt.action,
        "zuvix.loop.detector": evt.detector,
        "zuvix.loop.count": evt.count,
        ...(evt.pairedToolName
          ? { "zuvix.loop.paired_tool": lowCardinalityAttr(evt.pairedToolName, "tool") }
          : {}),
      });
      ...
      const recordToolLoop = (evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>) => {
        ...
        const span = spanWithDuration("zuvix.tool.loop", attrs, 0, { endTimeMs: evt.ts });
      ...
      const recordMemoryPressure = (
        ...
        const attrs = {
          "zuvix.memory.level": evt.level,
          "zuvix.memory.reason": evt.reason,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "zuvix.memory.rss_bytes": evt.memory.rssBytes,
          "zuvix.memory.heap_used_bytes": evt.memory.heapUsedBytes,
          "zuvix.memory.heap_total_bytes": evt.memory.heapTotalBytes,
          "zuvix.memory.external_bytes": evt.memory.externalBytes,
          "zuvix.memory.array_buffers_bytes": evt.memory.arrayBuffersBytes,
          ...(evt.thresholdBytes !== undefined
            ? { "zuvix.memory.threshold_bytes": evt.thresholdBytes }
            : {}),
          ...(evt.rssGrowthBytes !== undefined
            ? { "zuvix.memory.rss_growth_bytes": evt.rssGrowthBytes }
            : {}),
          ...(evt.windowMs !== undefined ? { "zuvix.memory.window_ms": evt.windowMs } : {}),
        };
        const span = spanWithDuration("zuvix.memory.pressure", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
      ...
      const recordRunCompleted = (
        ...
        if (evt.channel) {
          attrs["zuvix.channel"] = evt.channel;
        }
        if (evt.blockedBy) {
          attrs["zuvix.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        durationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.blockedBy) {
          spanAttrs["zuvix.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        if (evt.errorCategory) {
          spanAttrs["zuvix.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
        }
        ...
        const span =
          trackedSpan ??
          spanWithDuration("zuvix.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const harnessRunMetricAttrs = (evt: HarnessRunDiagnosticEvent) => ({
        "zuvix.harness.id": lowCardinalityAttr(evt.harnessId, "unknown"),
        "zuvix.harness.plugin": lowCardinalityAttr(evt.pluginId),
        ...(evt.type === "harness.run.started"
          ? {}
          : {
              "zuvix.outcome": evt.type === "harness.run.error" ? "error" : evt.outcome,
            }),
        "zuvix.provider": lowCardinalityAttr(evt.provider, "unknown"),
        "zuvix.model": lowCardinalityAttr(evt.model, "unknown"),
        ...(evt.channel ? { "zuvix.channel": lowCardinalityAttr(evt.channel) } : {}),
      });
      ...
      const recordHarnessRunStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("zuvix.harness.run", harnessRunMetricAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordHarnessRunCompleted = (
        ...
        if (evt.resultClassification) {
          spanAttrs["zuvix.harness.result_classification"] = lowCardinalityAttr(
            evt.resultClassification,
          );
        }
        if (typeof evt.yieldDetected === "boolean") {
          spanAttrs["zuvix.harness.yield_detected"] = evt.yieldDetected;
        }
        if (evt.itemLifecycle) {
          spanAttrs["zuvix.harness.items.started"] = evt.itemLifecycle.startedCount;
          spanAttrs["zuvix.harness.items.completed"] = evt.itemLifecycle.completedCount;
          spanAttrs["zuvix.harness.items.active"] = evt.itemLifecycle.activeCount;
        }
        const trustedTrace = trustedTraceContext(evt, metadata);
        const trackedSpan = trustedTrace?.spanId
          ? activeTrustedSpans.get(trustedTrace.spanId)
        ...
        const span =
          trackedSpan ??
          spanWithDuration("zuvix.harness.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordHarnessRunError = (
        ...
        const attrs = {
          ...harnessRunMetricAttrs(evt),
          "zuvix.harness.phase": evt.phase,
          "zuvix.errorCategory": errorType,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...
          ...(evt.cleanupFailed ? { "zuvix.harness.cleanup_failed": true } : {}),
        };
        const span =
      ...
      const recordContextAssembled = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "zuvix.context.message_count": evt.messageCount,
          "zuvix.context.history_text_chars": evt.historyTextChars,
          "zuvix.context.history_image_blocks": evt.historyImageBlocks,
          "zuvix.context.max_message_text_chars": evt.maxMessageTextChars,
          "zuvix.context.system_prompt_chars": evt.systemPromptChars,
          "zuvix.context.prompt_chars": evt.promptChars,
          "zuvix.context.prompt_images": evt.promptImages,
        };
        addRunAttrs(spanAttrs, evt);
        if (evt.contextTokenBudget !== undefined) {
          spanAttrs["zuvix.context.token_budget"] = evt.contextTokenBudget;
        }
        if (evt.reserveTokens !== undefined) {
          spanAttrs["zuvix.context.reserve_tokens"] = evt.reserveTokens;
        }
        const span = spanWithDuration("zuvix.context.assembled", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const recordModelFailover = (
        ...
        const metricAttrs: Record<string, string> = {
          "zuvix.failover.reason": lowCardinalityAttr(evt.reason, "unknown"),
          "zuvix.failover.suspended":
            evt.suspended === undefined ? "unknown" : String(evt.suspended),
          "zuvix.lane": lowCardinalityQueueLaneAttr(evt.lane, "unknown"),
          "zuvix.model": lowCardinalityAttr(evt.fromModel),
          "zuvix.provider": lowCardinalityAttr(evt.fromProvider),
          "zuvix.failover.to_model": lowCardinalityAttr(evt.toModel),
          "zuvix.failover.to_provider": lowCardinalityAttr(evt.toProvider),
        };
        ...
        if (evt.fromProvider) {
          spanAttrs["zuvix.provider"] = evt.fromProvider;
        }
        if (evt.fromModel) {
          spanAttrs["zuvix.model"] = evt.fromModel;
        }
        if (evt.toProvider) {
          spanAttrs["zuvix.failover.to_provider"] = evt.toProvider;
        }
        if (evt.toModel) {
          spanAttrs["zuvix.failover.to_model"] = evt.toModel;
        }
        if (evt.lane) {
          spanAttrs["zuvix.lane"] = lowCardinalityQueueLaneAttr(evt.lane, "unknown");
        }
        if (evt.suspended !== undefined) {
          spanAttrs["zuvix.failover.suspended"] = evt.suspended;
        }
        if (evt.cascadeDepth !== undefined) {
          spanAttrs["zuvix.failover.cascade_depth"] = evt.cascadeDepth;
        }
        const span = spanWithDuration("zuvix.model.failover", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const modelCallMetricAttrs = (evt: ModelCallLifecycleDiagnosticEvent) => ({
        "zuvix.provider": evt.provider,
        "zuvix.model": evt.model,
        "zuvix.api": lowCardinalityAttr(evt.api),
        "zuvix.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordModelCallStarted = (
        ...
        if (evt.api) {
          spanAttrs["zuvix.api"] = evt.api;
        }
        if (evt.transport) {
          spanAttrs["zuvix.transport"] = evt.transport;
        }
        trackTrustedSpan(
          evt,
          metadata,
      ...
      const recordModelCallError = (
        ...
        const metricAttrs = {
          ...
          "zuvix.errorCategory": errorType,
          ...(evt.failureKind
            ? { "zuvix.failureKind": lowCardinalityAttr(evt.failureKind, "other") }
            : {}),
        };
        modelCallDurationHistogram.record(evt.durationMs, metricAttrs);
        recordModelCallSizeTimingMetrics(evt, metricAttrs);
        ...
        if (evt.failureKind) {
          spanAttrs["zuvix.failureKind"] = lowCardinalityAttr(evt.failureKind, "other");
        }
        assignGenAiModelCallAttrs(spanAttrs, evt);
        if (evt.api) {
          spanAttrs["zuvix.api"] = evt.api;
      ...
      const toolExecutionBaseAttrs = (
        ...
        "zuvix.toolName": evt.toolName,
        "zuvix.tool.source": lowCardinalityAttr(evt.toolSource, "core"),
        "gen_ai.tool.name": evt.toolName,
        ...(evt.toolOwner ? { "zuvix.tool.owner": lowCardinalityAttr(evt.toolOwner) } : {}),
      ...
      const skillUsedAttrs = (
        ...
        "zuvix.skill.name": lowCardinalityAttr(evt.skillName, "skill"),
        "zuvix.skill.source": lowCardinalityAttr(evt.skillSource),
        "zuvix.skill.activation": lowCardinalityAttr(evt.activation),
        ...(evt.agentId ? { "zuvix.agent": lowCardinalityAttr(evt.agentId) } : {}),
        ...(evt.toolName ? { "zuvix.toolName": lowCardinalityAttr(evt.toolName, "tool") } : {}),
      });
      ...
      const recordSkillUsed = (
        ...
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("zuvix.skill.used", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("zuvix.tool.execution", toolExecutionBaseAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordToolExecutionCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("zuvix.tool.execution", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionError = (
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.errorCode) {
          spanAttrs["zuvix.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
        }
        assignOtelToolContentAttributes(spanAttrs, toolContent, contentCapturePolicy);
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
      ...
      const recordToolExecutionBlocked = (
        ...
        toolExecutionBlockedCounter.add(1, {
          ...toolExecutionBaseAttrs(evt),
          "zuvix.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        });
        if (!tracesEnabled) {
          return;
        }
        const spanAttrs: Record<string, string | number | boolean> = {
          ...toolExecutionBaseAttrs(evt),
          "zuvix.outcome": "blocked",
          "zuvix.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        };
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("zuvix.tool.execution", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordPayloadLarge = (
        ...
        const attrs = {
          "zuvix.payload.action": evt.action,
          "zuvix.payload.surface": lowCardinalityAttr(evt.surface, "unknown"),
          "zuvix.channel": lowCardinalityAttr(evt.channel, "none"),
          "zuvix.plugin": lowCardinalityAttr(evt.pluginId, "none"),
          "zuvix.reason": lowCardinalityAttr(evt.reason, "none"),
        };
      ...
      const recordExecProcessCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "zuvix.exec.target": evt.target,
          "zuvix.exec.mode": evt.mode,
          "zuvix.outcome": evt.outcome,
        };
        if (evt.failureKind) {
          attrs["zuvix.failureKind"] = evt.failureKind;
        }
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "zuvix.exec.command_length": evt.commandLength,
        };
        if (typeof evt.exitCode === "number") {
          spanAttrs["zuvix.exec.exit_code"] = evt.exitCode;
        }
        if (evt.exitSignal) {
          spanAttrs["zuvix.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
        }
        if (evt.timedOut !== undefined) {
          spanAttrs["zuvix.exec.timed_out"] = evt.timedOut;
        }

        const span = spanWithDuration("zuvix.exec", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
      ...
      const recordHeartbeat = (
        ...
        queueDepthHistogram.record(evt.queued, { "zuvix.channel": "heartbeat" });
      };
      ...
      const recordLivenessWarning = (
        ...
        const attrs = {
          "zuvix.liveness.reason": lowCardinalityAttr(reason, "unknown"),
        };
        livenessWarningCounter.add(1, attrs);
        queueDepthHistogram.record(evt.queued, { "zuvix.channel": "liveness" });
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "zuvix.liveness.active": evt.active,
          "zuvix.liveness.waiting": evt.waiting,
          "zuvix.liveness.queued": evt.queued,
          "zuvix.liveness.interval_ms": evt.intervalMs,
          ...(evt.eventLoopDelayP99Ms !== undefined
            ? { "zuvix.liveness.event_loop_delay_p99_ms": evt.eventLoopDelayP99Ms }
            : {}),
          ...(evt.eventLoopDelayMaxMs !== undefined
            ? { "zuvix.liveness.event_loop_delay_max_ms": evt.eventLoopDelayMaxMs }
            : {}),
          ...(evt.eventLoopUtilization !== undefined
            ? { "zuvix.liveness.event_loop_utilization": evt.eventLoopUtilization }
            : {}),
          ...(evt.cpuUserMs !== undefined
            ? { "zuvix.liveness.cpu_user_ms": evt.cpuUserMs }
            : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "zuvix.liveness.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "zuvix.liveness.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "zuvix.liveness.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        const span = spanWithDuration("zuvix.liveness.warning", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
          code: SpanStatusCode.ERROR,
      ...
      const recordDiagnosticPhaseCompleted = (
        ...
        const spanAttrs: Record<string, string | number> = {
          "zuvix.phase": lowCardinalityAttr(evt.name, "unknown"),
          ...(evt.cpuUserMs !== undefined ? { "zuvix.phase.cpu_user_ms": evt.cpuUserMs } : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "zuvix.phase.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "zuvix.phase.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "zuvix.phase.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        for (const [key, value] of Object.entries(evt.details ?? {})) {
          spanAttrs[`zuvix.phase.detail.${key}`] =
            typeof value === "boolean" ? String(value) : value;
        }
        const span = spanWithDuration("zuvix.diagnostic.phase", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      };
      ...
      const recordTelemetryExporter = (
        ...
        telemetryExporterCounter.add(1, {
          "zuvix.exporter": lowCardinalityAttr(evt.exporter, "unknown"),
          "zuvix.signal": evt.signal,
          "zuvix.status": evt.status,
          ...(evt.reason ? { "zuvix.reason": evt.reason } : {}),
          ...(evt.errorCategory
            ? { "zuvix.errorCategory": lowCardinalityAttr(evt.errorCategory, "other") }
            : {}),
        });
