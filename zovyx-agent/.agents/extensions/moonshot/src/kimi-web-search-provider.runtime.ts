...
function resolveKimiBaseUrl(kimi?: KimiConfig, zuvixConfig?: ZuvixConfig): string {
  ...
  const moonshotBaseUrl = zuvixConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: ZuvixConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.zuvix.ai/tools/kimi-search",
