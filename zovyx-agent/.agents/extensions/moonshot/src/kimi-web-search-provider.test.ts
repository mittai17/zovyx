...
describe("kimi web search provider", () => {
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "Zuvix is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "Zuvix is available on GitHub.");
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "Zuvix", url: "https://github.com/zuvix/zuvix" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "Zuvix is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "Zuvix is on GitHub.");
      expect(result.citations).toEqual(["https://github.com/zuvix/zuvix"]);
      expect(result).not.toHaveProperty("error");
    });
  });
