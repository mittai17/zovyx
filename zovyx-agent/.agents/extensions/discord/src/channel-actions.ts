...
import { createUnionActionGate } from "zuvix/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "zuvix/plugin-sdk/channel-contract";
import type { DiscordActionConfig, ZuvixConfig } from "zuvix/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: ZuvixConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: ZuvixConfig) {
