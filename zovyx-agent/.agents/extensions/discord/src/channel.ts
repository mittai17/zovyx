...
import { sleepWithAbort } from "zuvix/plugin-sdk/runtime-env";
import {
...
import { resolveTargetsWithOptionalToken } from "zuvix/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupDelayMs(cfg: ZuvixConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Zuvix adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
