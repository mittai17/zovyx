...
import type { RealtimeVoiceAgentControlResult } from "zuvix/plugin-sdk/realtime-voice";
  ...
  return {
    ...
    controlRealtimeVoiceAgentRunMock: vi.fn<() => Promise<RealtimeVoiceAgentControlResult>>(
      async () => ({
        ...
        message: "There is no active Zuvix run to steer.",
        speak: true,
        show: true,
        suppress: false,
      }),
...
vi.mock("zuvix/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/routing")>(
    "zuvix/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/agent-runtime")>(
    "zuvix/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
    agentCommandFromIngress: agentCommandMock,
    getTtsProvider: vi.fn(() => "openai"),
    resolveAgentDir: vi.fn(() => "/tmp/zuvix-agent"),
    resolveTtsConfig: vi.fn(() => ({
      modelOverrides: {},
      providerConfigs: {},
    })),
    resolveTtsPrefsPath: vi.fn(() => "/tmp/zuvix-tts.json"),
  };
});

vi.mock("zuvix/plugin-sdk/realtime-bootstrap-context", async () => {
  const actual = await vi.importActual<
    typeof import("zuvix/plugin-sdk/realtime-bootstrap-context")
  >("zuvix/plugin-sdk/realtime-bootstrap-context");
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/runtime-env")>(
    "zuvix/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/realtime-voice")>(
    "zuvix/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same Zuvix agent");
    expect(bridgeParams?.instructions).toContain("short natural backchannel");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("zuvix_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("zuvix_agent_control");
    const player = getLastAudioPlayer();
    bridgeParams?.audioSink?.sendAudio(Buffer.alloc(24_000));
    ...
    bridgeParams?.onToolCall?.(
      {
        ...
        args: {
          question: [
            "Speak this exact Zuvix answer to the Discord voice channel, without adding, removing, or rephrasing words.",
  ...
  it("routes active-run realtime transcripts to voice control before forced consults", async () => {
    controlRealtimeVoiceAgentRunMock.mockResolvedValueOnce({
      ...
      active: true,
      aborted: true,
      message: "Cancelled the active Zuvix run.",
      speak: true,
      show: true,
      suppress: false,
    });
    ...
    await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active Zuvix run."));
    expect(textToSpeechMock).not.toHaveBeenCalledWith(
      expect.objectContaining({ text: "Cancelled the active Zuvix run." }),
    );

    const stopCallsAfterControl = player.stop.mock.calls.length;
    bridgeParams?.onTranscript?.("assistant", "Cancelled the active Zuvix run.", true);
  ...
  it("accepts Zuvix as a default wake name before realtime agent-proxy consults", async () => {
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "zuvix wake answer" }] });
    const manager = createManager(
      {
        groupPolicy: "open",
        voice: {
    ...
    ownerTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "Zuvix, how is it going");
    ...
    expect(lastAgentCommandArgs().message).toContain("how is it going");
    expect(lastAgentCommandArgs().message).not.toContain("Zuvix");
    expectUserMessageIncludes("zuvix wake answer");
  });
  ...
  it("ignores default agent wake names longer than two words", async () => {
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "Zuvix, fallback still wakes");
  ...
  it("accepts leading fuzzy wake names before realtime agent-proxy consults", async () => {
    ...
    const zuvixTurn = entry.realtime?.beginSpeakerTurn(
      { extraSystemPrompt: undefined, senderIsOwner: true, speakerLabel: "Owner" },
      "u-owner",
    );
    zuvixTurn?.sendInputAudio(Buffer.alloc(8));
  ...
  it("does not accept configured realtime wake names longer than two words", async () => {
    ...
    fallbackTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "Zuvix, ship it");
  ...
  it("keeps forced agent-proxy fallback diagnostics out of agent prompts", async () => {
    ...
    expect(lastAgentCommandArgs().message).not.toContain("zuvix_agent_consult");
  ...
  it("reuses forced agent-proxy answers for late matching consult tool calls", async () => {
    ...
    expect(realtimeSessionMock.submitToolResult).toHaveBeenCalledWith(
      "call-late",
      {
        ...
        message: "Zuvix already delivered this answer to Discord voice.",
      },
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.autoRespondToAudio).toBe(true);
    expect(bridgeParams?.interruptResponseOnInputAudio).toBe(false);
    expect(bridgeParams?.instructions).toContain("Call zuvix_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("zuvix_agent_consult");
  ...
  it("adds default bootstrap profile context to realtime voice instructions", async () => {
    ...
    resolveRealtimeBootstrapContextInstructionsMock.mockResolvedValue(
      "Zuvix realtime voice profile context:\n\n### IDENTITY.md\nName: Wilfred",
    );
    const manager = createManager({
      groupPolicy: "open",
      voice: {
    ...
    const bridgeParams = lastRealtimeBridgeParams() as
      ...
      | undefined;
    expect(bridgeParams?.instructions).toContain("Zuvix realtime voice profile context");
