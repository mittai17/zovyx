...
import type { RuntimeEnv } from "zuvix/plugin-sdk/runtime-env";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: ZuvixConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): ZuvixConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("zuvix/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/acp-runtime")>(
    "zuvix/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/reply-runtime")>(
...
vi.mock("zuvix/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("zuvix/plugin-sdk/native-command-config-runtime")
  >("zuvix/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("zuvix/plugin-sdk/runtime-config-snapshot")
  >("zuvix/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
    getRuntimeConfig: () => ({}),
  };
...
vi.mock("zuvix/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/error-runtime")>(
    "zuvix/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
