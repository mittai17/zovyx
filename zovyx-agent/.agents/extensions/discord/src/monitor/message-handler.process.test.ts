...
import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "zuvix/plugin-sdk/channel-feedback";
import {
  recordChannelBotPairLoopAndCheckSuppression,
  type ChannelBotLoopProtectionFacts,
} from "zuvix/plugin-sdk/channel-inbound";
import type { ReplyPayload } from "zuvix/plugin-sdk/reply-dispatch-runtime";
import { setReplyPayloadMetadata } from "zuvix/plugin-sdk/reply-payload-testing";
import * as runtimeEnvModule from "zuvix/plugin-sdk/runtime-env";
...
function createNonTerminalToolWarningPayload(): ReplyPayload {
  return setReplyPayloadMetadata(
    {
      text: "⚠️ 🛠️ `run zuvix definitely-not-a-real-subcommand (agent)` failed",
      isError: true,
    },
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveAndPersistSessionFile: vi.fn<(params?: unknown) => Promise<{ sessionFile: string }>>(
    async () => ({ sessionFile: "/tmp/zuvix-discord-process-test-session.jsonl" }),
  ),
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/zuvix-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("zuvix/plugin-sdk/reply-runtime", () => ({
...
vi.mock("zuvix/plugin-sdk/conversation-runtime", () => ({
...
vi.mock("zuvix/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveAndPersistSessionFile.mockResolvedValue({
    sessionFile: "/tmp/zuvix-discord-process-test-session.jsonl",
  });
  resolveSessionStoreEntry.mockImplementation((params) => ({
    existing: params.sessionKey ? params.store[params.sessionKey] : undefined,
  }));
  resolveStorePath.mockReturnValue("/tmp/zuvix-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/zuvix-discord-process-test-sessions.json" },
      },
    });
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/zuvix-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
