...
let listNativeCommandSpecs: typeof import("zuvix/plugin-sdk/command-auth-native").listNativeCommandSpecs;
...
function createNativeCommand(
  ...
  opts?: {
    cfg?: ZuvixConfig;
    discordConfig?: NonNullable<ZuvixConfig["channels"]>["discord"];
  },
  ...
  const baseCfg: ZuvixConfig = opts?.cfg ?? {};
  const discordConfig: NonNullable<ZuvixConfig["channels"]>["discord"] =
...
function createAllowedGuildAutocompleteConfig(
  commands: NonNullable<ZuvixConfig["commands"]>,
): ZuvixConfig {
  return {
    commands,
    channels: {
...
describe("createDiscordNativeCommand option wiring", () => {
  beforeAll(async () => {
    ({ listNativeCommandSpecs } = await import("zuvix/plugin-sdk/command-auth-native"));
  ...
  it("returns no autocomplete choices for group DMs outside dm.groupChannels", async () => {
    ...
    } satisfies NonNullable<ZuvixConfig["channels"]>["discord"];
    const command = createNativeCommand("think", {
      cfg: {
        commands: {
          allowFrom: {
  ...
  it("truncates Discord command and option descriptions to Discord's limit", () => {
    ...
    const discordConfig = {} as NonNullable<ZuvixConfig["channels"]>["discord"];
    const command = createDiscordNativeCommand({
      command: {
        name: "longdesc",
