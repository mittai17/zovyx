...
vi.mock("zuvix/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("zuvix/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<ZuvixConfig["channels"]>["discord"]>["execApprovals"],
): ZuvixConfig {
  return {
    channels: {
      discord: {
