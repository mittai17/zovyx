...
import { createPluginRuntimeMock } from "zuvix/plugin-sdk/channel-test-helpers";
...
let runtimeEnvModule: typeof import("zuvix/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): ZuvixConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("zuvix/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("zuvix/plugin-sdk/runtime-env");
  ...
  it("passes Zuvix event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
