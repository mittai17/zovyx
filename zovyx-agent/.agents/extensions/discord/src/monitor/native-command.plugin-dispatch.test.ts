...
import type { NativeCommandSpec } from "zuvix/plugin-sdk/command-auth-native";
import { resolveDirectStatusReplyForSession } from "zuvix/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "zuvix/plugin-sdk/reply-dispatch-runtime";
import {
  clearRuntimeConfigSnapshot,
  setRuntimeConfigSnapshot,
} from "zuvix/plugin-sdk/runtime-config-snapshot";
import { getSessionEntry } from "zuvix/plugin-sdk/session-store-runtime";
...
function createConfig(): ZuvixConfig {
  return {
    channels: {
      discord: {
...
async function createNativeCommand(cfg: ZuvixConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: ZuvixConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
async function createStatusCommand(cfg: ZuvixConfig) {
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("zuvix/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("zuvix/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    ...
    discordNativeCommandTesting.setGetSessionEntry(
      runtimeModuleMocks.getSessionEntry as typeof import("zuvix/plugin-sdk/session-store-runtime").getSessionEntry,
    );
  });
  ...
  it("refreshes native command routing config between invocations", async () => {
    ...
    const resolveRouteState = vi.fn(async (params: { cfg: ZuvixConfig }) =>
      createUnboundRouteState({
        sessionKey:
  ...
  it("passes the configured binding agent to plugin-owned Discord command sessions", async () => {
    const cfg = createConfig();
    const interaction = createInteraction();
    const pluginSessionKey = "plugin-binding:zuvix-codex-app-server:dm";
