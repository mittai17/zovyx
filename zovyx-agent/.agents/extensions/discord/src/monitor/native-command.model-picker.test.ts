...
import * as commandRegistryModule from "zuvix/plugin-sdk/command-auth-native";
import type {
  ChatCommandDefinition,
  CommandArgsParsing,
} from "zuvix/plugin-sdk/command-auth-native";
import type { ModelsProviderData } from "zuvix/plugin-sdk/command-auth-native";
import type { ZuvixConfig } from "zuvix/plugin-sdk/config-contracts";
import * as globalsModule from "zuvix/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  upsertSessionEntry,
} from "zuvix/plugin-sdk/session-store-runtime";
import * as commandTextModule from "zuvix/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "zuvix-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "zuvix", label: "Zuvix Default", description: "Use Zuvix." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();
