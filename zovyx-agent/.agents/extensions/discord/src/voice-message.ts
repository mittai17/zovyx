...
import { unlinkIfExists } from "zuvix/plugin-sdk/media-runtime";
import { parseStrictFiniteNumber } from "zuvix/plugin-sdk/number-runtime";
import type { RetryRunner } from "zuvix/plugin-sdk/retry-runtime";
...
async function generateWaveformFromPcm(filePath: string): Promise<string> {
  const tempDir = resolvePreferredZuvixTmpDir();
