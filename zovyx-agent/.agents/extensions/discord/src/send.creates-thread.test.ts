...
vi.mock("zuvix/plugin-sdk/web-media", async () => {
...
afterAll(() => {
  vi.doUnmock("zuvix/plugin-sdk/web-media");
});
...
describe("uploadStickerDiscord", () => {
  ...
  it("uploads sticker assets", async () => {
    ...
    expect(stickerBody.name).toBe("zuvix_wave");
    expect(stickerBody.description).toBe("Zuvix waving");
