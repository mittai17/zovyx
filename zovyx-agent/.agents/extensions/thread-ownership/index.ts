// Thread Ownership plugin entrypoint registers its Zuvix integration.
...
function resolveOwnershipAgent(config: ZuvixConfig): { id: string; name: string } {
...
export default definePluginEntry({
  ...
  register(api: ZuvixPluginApi) {
    const resolveCurrentState = () => {
      const currentConfig = (api.runtime.config?.current?.() ?? api.config) as ZuvixConfig;
