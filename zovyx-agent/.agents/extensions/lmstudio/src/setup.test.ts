...
import { CUSTOM_LOCAL_AUTH_MARKER } from "zuvix/plugin-sdk/provider-auth";
import type { ZuvixConfig } from "zuvix/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "zuvix/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "zuvix/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "zuvix/plugin-sdk/provider-setup";
import type { WizardPrompter } from "zuvix/plugin-sdk/setup";
...
vi.mock("zuvix/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("zuvix/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("zuvix/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("zuvix/plugin-sdk/provider-auth");
  vi.doUnmock("zuvix/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): ZuvixConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.ZUVIX_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("ZUVIX_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
