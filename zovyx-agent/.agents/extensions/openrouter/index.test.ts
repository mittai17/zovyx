...
vi.mock("zuvix/plugin-sdk/provider-stream-family", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("zuvix/plugin-sdk/provider-stream-family")>();
  return {
    ...actual,
...
function readManifest(): OpenRouterManifest {
  return JSON.parse(readFileSync(new URL("./zuvix.plugin.json", import.meta.url), "utf8"));
}
...
describe("openrouter provider hooks", () => {
  ...
  it("injects provider routing into compat before applying stream wrappers", async () => {
    ...
    const baseStreamFn = vi.fn(
      (
        ...args: Parameters<import("zuvix/plugin-sdk/agent-core").StreamFn>
      ): ReturnType<import("zuvix/plugin-sdk/agent-core").StreamFn> => {
