...
import { clearLiveCatalogCacheForTests } from "zuvix/plugin-sdk/provider-catalog-shared";
import {
...
vi.mock("zuvix/plugin-sdk/provider-http", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/provider-http")>(
    "zuvix/plugin-sdk/provider-http",
  );
  return {
    ...actual,
