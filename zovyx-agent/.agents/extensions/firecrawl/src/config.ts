...
import { canResolveEnvSecretRefInReadOnlyPath } from "zuvix/plugin-sdk/extension-shared";
import { resolvePositiveTimeoutSeconds } from "zuvix/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "zuvix/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<ZuvixConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<ZuvixConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ZuvixConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: ZuvixConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: ZuvixConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: ZuvixConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: ZuvixConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: ZuvixConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: ZuvixConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: ZuvixConfig, override?: number): number {
