...
import type { ResolvedTtsConfig } from "zuvix/plugin-sdk/agent-runtime";
import { AuthStorage, ModelRegistry } from "zuvix/plugin-sdk/agent-sessions";
import type { ZuvixConfig } from "zuvix/plugin-sdk/config-contracts";
import { encodePngRgba, fillPixel } from "zuvix/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "zuvix/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "zuvix/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.ZUVIX_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.ZUVIX_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.ZUVIX_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.ZUVIX_LIVE_TEST === "1";
...
function createLiveConfig(): ZuvixConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "Zuvix integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing Zuvix OpenAI realtime transcription integration test OK.";
