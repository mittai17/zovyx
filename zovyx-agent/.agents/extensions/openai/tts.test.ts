...
} from "zuvix/plugin-sdk/proxy-capture";
...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds Zuvix attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.originator).toBe("zuvix");
      expect(headers?.version).toBe("2026.3.22");
      expect(headers?.["User-Agent"]).toBe("zuvix/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.ZUVIX_DEBUG_PROXY_SESSION_ID = "tts-session";
      ...
      store.upsertSession({
        ...
        sourceScope: "zuvix",
        sourceProcess: "zuvix",
        dbPath: process.env.ZUVIX_DEBUG_PROXY_DB_PATH,
        blobDir: process.env.ZUVIX_DEBUG_PROXY_BLOB_DIR,
      });
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.ZUVIX_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
      process.env.ZUVIX_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
      process.env.ZUVIX_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
