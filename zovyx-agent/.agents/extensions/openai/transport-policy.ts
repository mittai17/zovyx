...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-zuvix-turn-id": turnId,
      "x-zuvix-turn-attempt": attempt,
    },
    metadata: {
      zuvix_session_id: sessionHeaders["x-zuvix-session-id"] ?? "",
      zuvix_turn_id: turnId,
      zuvix_turn_attempt: attempt,
      zuvix_transport: ctx.transport,
    },
  };
}
