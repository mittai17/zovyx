...
import { CODEX_CLI_PROFILE_ID, type OAuthCredential } from "zuvix/plugin-sdk/provider-auth";
import { buildOauthProviderAuthResult } from "zuvix/plugin-sdk/provider-auth";
import {
...
import { fetchCodexUsage } from "zuvix/plugin-sdk/provider-usage";
import {
 ...
 * entirely when they were written by older Zuvix versions. When that row wins
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.zuvix.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `zuvix models auth login --provider openai` or `zuvix configure`.";
}
