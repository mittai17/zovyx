...
} from "zuvix/plugin-sdk/image-generation";
import {
  parseOpenAiCompatibleImageResponse,
  toImageDataUrl,
} from "zuvix/plugin-sdk/image-generation";
import { createSubsystemLogger } from "zuvix/plugin-sdk/logging-core";
import { resolveClosestSize } from "zuvix/plugin-sdk/media-generation-runtime";
import { extensionForMime } from "zuvix/plugin-sdk/media-mime";
import {
...
import { resolveApiKeyForProvider } from "zuvix/plugin-sdk/provider-auth-runtime";
import {
...
function shouldAllowPrivateImageEndpoint(req: {
  ...
  return process.env.ZUVIX_QA_ALLOW_LOCAL_IMAGE_PROVIDER === "1";
}
...
function hasExplicitOpenAIImageApiKeyConfig(cfg: ZuvixConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return providerConfig?.apiKey !== undefined || providerConfig?.auth === "api-key";
}

function hasExplicitDirectOpenAIImageConfig(cfg: ZuvixConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  if (!providerConfig) {
    return false;
  }
...
function hasChatGPTImageRouteConfig(cfg: ZuvixConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return (
...
function forceOpenAIImageApiKeyAuth(cfg: ZuvixConfig | undefined): ZuvixConfig | undefined {
