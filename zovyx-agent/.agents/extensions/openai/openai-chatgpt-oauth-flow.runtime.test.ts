...
describe("OpenAI Codex OAuth flow", () => {
  ...
  it("waits for Node OAuth runtime before creating an authorization flow", async () => {
    const flow = await testing.createAuthorizationFlow("zuvix-test");
    const url = new URL(flow.url);

    expect(flow.state).toMatch(/^[a-f0-9]{32}$/u);
    expect(url.searchParams.get("state")).toBe(flow.state);
    expect(url.searchParams.get("originator")).toBe("zuvix-test");
  ...
  it("rejects non-loopback callback bind hosts", () => {
    expect(() => testing.resolveCallbackHost({ ZUVIX_OAUTH_CALLBACK_HOST: "0.0.0.0" })).toThrow(
