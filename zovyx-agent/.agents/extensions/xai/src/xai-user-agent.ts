// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatZuvixUserAgent`.

import { ZUVIX_VERSION as PACKAGE_VERSION } from "zuvix/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "zuvix";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.ZUVIX_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.ZUVIX_SERVICE_VERSION) ??
...
// an empty record so the zuvix identity is not forwarded to the proxy.
