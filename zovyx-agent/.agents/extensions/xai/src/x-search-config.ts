...
function resolveLegacyXSearchConfig(config?: ZuvixConfig): JsonRecord | undefined {
...
function resolvePluginXSearchConfig(config?: ZuvixConfig): JsonRecord | undefined {
...
function resolveLegacyGrokWebSearchConfig(config?: ZuvixConfig): JsonRecord | undefined {
  const web = config?.tools?.web as Record<string, unknown> | undefined;
...
function resolvePluginWebSearchConfig(config?: ZuvixConfig): JsonRecord | undefined {
  const pluginConfig = config?.plugins?.entries?.xai?.config;
...
export function resolveEffectiveXSearchConfig(config?: ZuvixConfig): JsonRecord | undefined {
