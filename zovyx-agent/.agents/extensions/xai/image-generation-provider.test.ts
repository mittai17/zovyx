...
const {
  ...
  resolveProviderHttpRequestConfigMock: vi.fn((params: Record<string, unknown>) => {
    ...
    if (params.provider === "xai") {
      const version = process.env.ZUVIX_VERSION?.trim() || "unknown";
      headers.set("User-Agent", `zuvix/${version}`);
      headers.set("originator", "zuvix");
...
vi.mock("zuvix/plugin-sdk/provider-auth-runtime", () => ({
...
vi.mock("zuvix/plugin-sdk/provider-http", () => ({
...
describe("xai image generation provider", () => {
  ...
  it("builds provider with correct models, default, and capabilities", () => {
    ...
    expect(isConfigured({ agentDir: "/tmp/zuvix-xai-test" })).toBe(true);
    expect(isProviderApiKeyConfiguredMock).toHaveBeenCalledWith({
      provider: "xai",
      agentDir: "/tmp/zuvix-xai-test",
    });
  });
  ...
  it("forwards xAI attribution User-Agent through the SDK image request", async () => {
    ...
    const request = requirePostJsonCall();
    expect(request.headers?.get("user-agent")).toBe("zuvix/2026.3.22");
    expect(request.headers?.get("originator")).toBe("zuvix");
