// Xai plugin entrypoint registers its Zuvix integration.
import { defineSingleProviderPluginEntry } from "zuvix/plugin-sdk/provider-entry";
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "zuvix/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "zuvix/plugin-sdk/provider-stream-shared";
import { jsonResult } from "zuvix/plugin-sdk/provider-web-search";
import {
      ...
      run: async (ctx) => {
        ...
        try {
          const { resolveApiKeyForProvider } =
            await import("zuvix/plugin-sdk/provider-auth-runtime");
