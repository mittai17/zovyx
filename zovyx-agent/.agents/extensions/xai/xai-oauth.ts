...
import type { ProviderAuthContext, ProviderAuthMethod } from "zuvix/plugin-sdk/plugin-entry";
import {
...
import { waitForLocalOAuthCallback } from "zuvix/plugin-sdk/provider-auth-runtime";
...
export function buildXaiOAuthAuthorizeUrl(params: {
  ...
  url.searchParams.set("referrer", "zuvix");
  return url.toString();
}
  ...
  try {
    ...
    return buildOauthProviderAuthResult({
      ...
      notes: [
        ...
        "xAI may label the consent app as Grok Build because Zuvix uses xAI's shared OAuth client.",
      ],
    });
  } catch (err) {
