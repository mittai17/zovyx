// Phone Control plugin entrypoint registers its Zuvix integration.
import {
  asDateTimestampMs,
...
function openArmStateStore(api: ZuvixPluginApi) {
...
async function readArmState(api: ZuvixPluginApi): Promise<ArmStateFile | null> {
  return (await openArmStateStore(api).lookup(ARM_STATE_KEY)) ?? null;
}

async function writeArmState(api: ZuvixPluginApi, state: ArmStateFile | null): Promise<void> {
...
export default definePluginEntry({
  ...
  register(api: ZuvixPluginApi) {
    ...
    const timerService: ZuvixPluginService = {
