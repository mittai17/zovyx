...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("zuvix/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("zuvix/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "zuvix-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as ZuvixConfig) as TelegramBotDeps["getRuntimeConfig"];
