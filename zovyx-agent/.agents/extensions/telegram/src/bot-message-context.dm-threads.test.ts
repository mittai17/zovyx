...
const { recordInboundSessionMock, resolveStorePathMock } = vi.hoisted(() => ({
  ...
  resolveStorePathMock: vi.fn<ResolveStorePathFn>(() => "/tmp/zuvix-session-store.json"),
}));
...
const { clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
  await import("zuvix/plugin-sdk/runtime-config-snapshot");
...
afterEach(() => {
  ...
  resolveStorePathMock.mockReturnValue("/tmp/zuvix-session-store.json");
});
...
describe("buildTelegramMessageContext group sessions without forum", () => {
  ...
  it("does not add a topic-cache store lookup for non-forum group reply threads", async () => {
    const resolveStorePath = vi.fn(() => "/tmp/zuvix/session-store.json");
  ...
  it("reloads topic name from disk after cache reset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-telegram-topic-name-"));
