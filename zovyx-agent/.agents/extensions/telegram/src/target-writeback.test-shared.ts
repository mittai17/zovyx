...
vi.mock("zuvix/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/config-mutation")>(
    "zuvix/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/cron-store-runtime")>(
    "zuvix/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/zuvix.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
