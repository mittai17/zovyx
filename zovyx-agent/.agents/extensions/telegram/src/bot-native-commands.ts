...
import { resolveChannelStreamingBlockEnabled } from "zuvix/plugin-sdk/channel-outbound";
import { resolveNativeCommandSessionTargets } from "zuvix/plugin-sdk/command-auth-native";
import {
...
import type { ChannelGroupPolicy } from "zuvix/plugin-sdk/config-contracts";
import type {
  ReplyToMode,
...
import { resolveMarkdownTableMode } from "zuvix/plugin-sdk/markdown-table-runtime";
import { resolveSendableOutboundReplyParts } from "zuvix/plugin-sdk/reply-payload";
import { resolveAgentRoute } from "zuvix/plugin-sdk/routing";
import { getRuntimeConfigSnapshot } from "zuvix/plugin-sdk/runtime-config-snapshot";
import { danger, logVerbose } from "zuvix/plugin-sdk/runtime-env";
...
type TelegramChunkMode = ReturnType<
  typeof import("zuvix/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("zuvix/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): ZuvixConfig => telegramDeps.getRuntimeConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: ZuvixConfig): TelegramAccountConfig => {
    try {
