...
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("zuvix.telegramSentMessagesState");
const TELEGRAM_SENT_MESSAGES_STORE_FOR_TEST_KEY = Symbol.for(
  "zuvix.telegramSentMessagesStoreForTest",
);
...
function resolveSentMessageStorePath(cfg?: Pick<ZuvixConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<ZuvixConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<ZuvixConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<ZuvixConfig, "session">): SentMessageStore {
