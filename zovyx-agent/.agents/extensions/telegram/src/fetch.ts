...
} from "zuvix/plugin-sdk/fetch-runtime";
import {
...
import { resolveRequestUrl } from "zuvix/plugin-sdk/request-url";
...
// fix for the leak observed in zuvix#68128 is the transport lifecycle that
...
function resolveZuvixProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.ZUVIX_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveZuvixProxyUrlForTelegram() : undefined;
