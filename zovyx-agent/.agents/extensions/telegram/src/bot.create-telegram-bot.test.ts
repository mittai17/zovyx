...
import { escapeRegExp, formatEnvelopeTimestamp } from "zuvix/plugin-sdk/channel-test-helpers";
import type { TelegramGroupConfig } from "zuvix/plugin-sdk/config-contracts";
...
import { sanitizeTerminalText } from "zuvix/plugin-sdk/test-fixtures";
...
const pluginStateTestRuntime = await import("zuvix/plugin-sdk/plugin-state-test-runtime");
const conversationRuntime = await import("zuvix/plugin-sdk/conversation-runtime");
const configMutation = await import("zuvix/plugin-sdk/config-mutation");
const sessionStoreRuntime = await import("zuvix/plugin-sdk/session-store-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "zuvix-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.ZUVIX_STATE_DIR;
    process.env.ZUVIX_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
  ...
  it.each(["stop", "/stop@zuvix_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "Zuvix Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
            from: { id: 42, first_name: "Ada" },
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("zuvix pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Zuvix", username: "zuvix_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "Zuvix", username: "zuvix_bot" },
          text: "Binding: Review pull request 54118 (zuvix)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "Zuvix", username: "zuvix_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "Zuvix Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Zuvix", username: "zuvix_bot" },
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "zuvix_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "zuvix_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/zuvix-telegram-group-activation.json";
