...
type ResolveConfiguredBindingRouteFn =
  typeof import("zuvix/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("zuvix/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("zuvix/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("zuvix/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("zuvix/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/conversation-runtime")>(
    "zuvix/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/session-store-runtime")>(
    "zuvix/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("zuvix/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/command-auth-native")>(
    "zuvix/plugin-sdk/command-auth-native",
  );
...
vi.mock("zuvix/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/plugin-runtime")>(
    "zuvix/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/zuvix-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/zuvix-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/zuvix-sessions.json");
    expectSendMessageCall({
      sendMessage,
      chatId: 100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/zuvix-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as ZuvixConfig,
    ...
    expectRecordFields(
      firstMockArg(sessionMocks.resolveAndPersistSessionFile, "resolveAndPersistSessionFile"),
      {
        ...
        storePath: "/tmp/zuvix-sessions/sessions.json",
        sessionsDir: "/tmp/zuvix-sessions",
        fallbackSessionFile: path.resolve("/tmp/zuvix-sessions", "sess-topic-topic-42.jsonl"),
      },
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/zuvix-sessions", "sess-topic-topic-42.jsonl"),
