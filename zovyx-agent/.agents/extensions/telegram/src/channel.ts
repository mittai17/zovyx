...
import type { ChannelMessageActionAdapter } from "zuvix/plugin-sdk/channel-contract";
import {
...
import { createAccountStatusSink } from "zuvix/plugin-sdk/channel-outbound";
...
import type { RoutePeer } from "zuvix/plugin-sdk/routing";
import {
...
const telegramMessageAdapter = createChannelMessageAdapterFromOutbound<ZuvixConfig>({
  id: "telegram",
  live: {
    capabilities: {
      draftPreview: true,
...
function resolveTelegramNativeTopicThreadId(
  ...
  // Keep the chat-scoped canonical id inside Zuvix state; translate it back
  ...
  base: {
    ...
    gateway: {
      ...
      logoutAccount: async ({ accountId, cfg }) => {
        ...
        const nextCfg = { ...cfg } as ZuvixConfig;
