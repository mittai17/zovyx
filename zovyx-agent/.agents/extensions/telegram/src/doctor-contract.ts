...
} from "zuvix/plugin-sdk/runtime-doctor";
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  ...
  },
  {
    ...
    message:
      'channels.telegram.dm and direct.<chatId>.threadReplies were removed; DM topic sessions now follow Telegram getMe.has_topics_enabled, so topics-enabled bots may use thread-scoped DM sessions. Run "zuvix doctor --fix".',
    match: hasRetiredTelegramDmConfig,
  },
  {
    path: ["channels", "telegram", "accounts"],
    message:
      'channels.telegram.accounts.<id>.dm and direct.<chatId>.threadReplies were removed; DM topic sessions now follow Telegram getMe.has_topics_enabled, so topics-enabled bots may use thread-scoped DM sessions. Run "zuvix doctor --fix".',
    match: hasRetiredTelegramAccountDmConfig,
  },
  {
    path: ["channels", "telegram"],
    message:
      'channels.telegram.streaming.preview.nativeToolProgress and nativeToolProgressAllowFrom were removed; Telegram previews now use rich send/edit messages. Run "zuvix doctor --fix".',
    match: hasRetiredTelegramNativeDraftConfig,
  },
  {
    path: ["channels", "telegram", "accounts"],
    message:
      'channels.telegram.accounts.<id>.streaming.preview.nativeToolProgress and nativeToolProgressAllowFrom were removed; Telegram previews now use rich send/edit messages. Run "zuvix doctor --fix".',
...
export function normalizeCompatibilityConfig({
  ...
  return {
    config: {
      ...
      channels: {
        ...cfg.channels,
        telegram: updated as unknown as NonNullable<ZuvixConfig["channels"]>["telegram"],
