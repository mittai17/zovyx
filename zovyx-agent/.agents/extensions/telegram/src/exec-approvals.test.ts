...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<ZuvixConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<ZuvixConfig["channels"]>["telegram"]>>,
): ZuvixConfig {
  return {
    channels: {
      telegram: {
