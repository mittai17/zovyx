...
import { resolveStorePath } from "zuvix/plugin-sdk/session-store-runtime";
...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): ZuvixConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as ZuvixConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): ZuvixConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  function topicCacheScopeFor(cfg: ZuvixConfig, accountId: string): string {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ZUVIX_STATE_DIR", "TELEGRAM_BOT_TOKEN"]);
    resetTopicNameCacheForTest();
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as ZuvixConfig;
    await expect(
  ...
  it("persists sendMessage action deliveries before Telegram platform send", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-telegram-action-durable-"));
    const {
      createOutboundTestPlugin,
      createTestRegistry,
      readQueuedDeliveryEntriesForTest,
      setActivePluginRegistry,
    } = await import("zuvix/plugin-sdk/plugin-test-runtime");
  ...
  it("stores created forum topic names in the account-scoped cache", async () => {
    ...
    const cfg = {
      ...
      session: { store: path.join(os.tmpdir(), "zuvix-telegram-action-sessions.json") },
    } as ZuvixConfig;
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: ZuvixConfig, accountId = "media") {
    await handleTelegramAction(
