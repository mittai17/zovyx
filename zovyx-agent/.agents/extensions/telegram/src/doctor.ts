...
function formatTelegramAccountConfigPath(cfg: ZuvixConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: ZuvixConfig): {
  config: ZuvixConfig;
  changes: string[];
} {
...
async function repairTelegramConfig(params: { cfg: ZuvixConfig }): Promise<{
  config: ZuvixConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: ZuvixConfig): Promise<{
  config: ZuvixConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("zuvix/plugin-sdk/runtime");
