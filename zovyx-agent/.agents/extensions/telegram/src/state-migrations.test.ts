...
import { resolvePersistentDedupePluginStateNamespace } from "zuvix/plugin-sdk/persistent-dedupe";
import {
...
describe("telegram state migrations", () => {
  it("detects legacy bot-info cache import", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "zuvix-telegram-state-migration-"));
    const env = { ...process.env, ZUVIX_STATE_DIR: dir };
