...
import { parseExecApprovalCommandText } from "zuvix/plugin-sdk/approval-reply-runtime";
import { resolveChannelConfigWrites } from "zuvix/plugin-sdk/channel-config-helpers";
import {
  buildMentionRegexes,
...
} from "zuvix/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "zuvix/plugin-sdk/command-auth-native";
import { hasControlCommand } from "zuvix/plugin-sdk/command-detection";
import { isAbortRequestText } from "zuvix/plugin-sdk/command-primitives-runtime";
import { buildCommandsMessagePaginated } from "zuvix/plugin-sdk/command-status";
import type { DmPolicy, ZuvixConfig } from "zuvix/plugin-sdk/config-contracts";
import type {
  TelegramGroupConfig,
  TelegramTopicConfig,
} from "zuvix/plugin-sdk/config-contracts";
import { mutateConfigFile } from "zuvix/plugin-sdk/config-mutation";
import {
...
import { applyModelOverrideToSessionEntry } from "zuvix/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "zuvix/plugin-sdk/models-provider-runtime";
...
import { resolveThreadSessionKeys } from "zuvix/plugin-sdk/routing";
import { danger, logVerbose, warn } from "zuvix/plugin-sdk/runtime-env";
import {
  getSessionEntry,
  listSessionEntries,
  patchSessionEntry,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in zuvix.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,
