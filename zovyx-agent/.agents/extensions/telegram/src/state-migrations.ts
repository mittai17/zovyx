...
import type { ChannelLegacyStateMigrationPlan } from "zuvix/plugin-sdk/channel-contract";
import { resolveChannelAllowFromPath } from "zuvix/plugin-sdk/channel-pairing";
...
} from "zuvix/plugin-sdk/persistent-dedupe";
import { createPluginStateSyncKeyedStore } from "zuvix/plugin-sdk/runtime-doctor";
import { statRegularFileSync } from "zuvix/plugin-sdk/security-runtime";
...
function detectTelegramMessageDispatchLegacyStateMigration(params: {
  ...
  const storePath = resolveStorePath(params.cfg.session?.store, { env: params.env });
  const legacyStorePath = resolveLegacySessionStorePath(params);
  const env = params.stateDir ? { ...params.env, ZUVIX_STATE_DIR: params.stateDir } : params.env;
