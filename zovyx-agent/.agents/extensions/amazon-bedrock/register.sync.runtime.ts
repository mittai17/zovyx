...
import { registerApiProvider, streamSimple } from "zuvix/plugin-sdk/llm";
import { resolvePluginConfigObject } from "zuvix/plugin-sdk/plugin-config-runtime";
import type {
  ZuvixPluginApi,
...
import { streamWithPayloadPatch } from "zuvix/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When Zuvix's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if Zuvix identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher Zuvix uses for direct model IDs.
 */
 ...
 * the Zuvix config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: ZuvixPluginApi): void {
