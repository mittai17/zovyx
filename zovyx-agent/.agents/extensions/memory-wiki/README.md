...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Zuvix**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.zuvix-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
Unmanaged raw Markdown can live under `sources/` without Zuvix page frontmatter. Add `<!-- zuvix:wiki:raw-source -->` near the top of the page body to opt it out of wiki page metadata and freshness lint; generated or source-sync tracked imports still require their structured metadata.
...
- The compiled agent digests in `.zuvix-wiki/cache/agent-digest.json` and `.zuvix-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
