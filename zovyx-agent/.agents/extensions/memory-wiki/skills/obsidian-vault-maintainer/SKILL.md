...
- Start from `zuvix wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `zuvix wiki obsidian status` before shelling out, then prefer the dedicated helpers like `zuvix wiki obsidian search`, `zuvix wiki obsidian open`, `zuvix wiki obsidian command`, and `zuvix wiki obsidian daily`.
