...
import { FsSafeError, pathExists, root as fsRoot } from "zuvix/plugin-sdk/security-runtime";
...
- Use \`.zuvix-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the Zuvix memory-wiki plugin.
...
- \`.zuvix-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".zuvix-wiki/log.jsonl", "", createdFiles);
