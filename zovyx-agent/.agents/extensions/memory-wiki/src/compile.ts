...
import { runTasksWithConcurrency } from "zuvix/plugin-sdk/concurrency-runtime";
import { retryTransientMemoryRead } from "zuvix/plugin-sdk/memory-core-host-engine-storage";
import {
  replaceManagedMarkdownBlock,
  withTrailingNewline,
} from "zuvix/plugin-sdk/memory-host-markdown";
...
const AGENT_DIGEST_PATH = ".zuvix-wiki/cache/agent-digest.json";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- zuvix:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- zuvix:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- zuvix:wiki:index:start -->",
      endMarker: "<!-- zuvix:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- zuvix:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- zuvix:wiki:${group.dir}:index:end -->`,
