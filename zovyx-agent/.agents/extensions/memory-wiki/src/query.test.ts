...
vi.mock("zuvix/plugin-sdk/memory-host-search", () => ({
  getActiveMemorySearchManager: getActiveMemorySearchManagerMock,
}));

vi.mock("zuvix/plugin-sdk/memory-host-core", () => ({
  resolveDefaultAgentId: resolveDefaultAgentIdMock,
  resolveSessionAgentId: resolveSessionAgentIdMock,
}));

vi.mock("zuvix/plugin-sdk/session-transcript-hit", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("zuvix/plugin-sdk/session-transcript-hit")>();
  return {
    ...actual,
...
function createAppConfig(): ZuvixConfig {
  return {
    agents: {
...
function createSessionVisibilityAppConfig(): ZuvixConfig {
  return {
    agents: {
  ...
  loadCombinedSessionStoreForGatewayMock.mockReturnValue({
    ...
    store: {
      "agent:main:child-session": {
        ...
        updatedAt: 1,
        sessionFile: "/tmp/zuvix/child-session.jsonl",
      },
      "agent:main:sibling-session": {
        sessionId: "sibling-session",
        updatedAt: 2,
        sessionFile: "/tmp/zuvix/sibling-session.jsonl",
      },
    },
  });
}
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Related",
          "<!-- zuvix:wiki:related:start -->",
          "### Related Pages",
          "- [Needle Person](entities/needle-person.md)",
          "<!-- zuvix:wiki:related:end -->",
          "",
        ].join("\n"),
      }),
      "utf8",
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Agent Card",
          "- Maintainer lane: CEO; Microsoft-facing Zuvix maintainer",
          "",
          "## AI Notes",
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:main:abc-uuid": {
          sessionId: "abc-uuid",
          updatedAt: 1,
          sessionFile: "/tmp/zuvix/abc-uuid.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:visible-session": {
          ...
          updatedAt: 1,
          sessionFile: "/tmp/zuvix/visible-session.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:main": {
          sessionId: "main",
          updatedAt: 1,
          sessionFile: "/tmp/zuvix/main.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
