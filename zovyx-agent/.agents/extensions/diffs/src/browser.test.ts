...
import { createMockServerResponse } from "zuvix/plugin-sdk/test-env";
import { afterAll, afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import type { ZuvixConfig } from "../api.js";
import type { ZuvixPluginApi, ZuvixPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("zuvix-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("uses live runtime tool config through the registered tool factory", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<ZuvixPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: ZuvixPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    let registeredHttpRouteHandler: HttpRouteHandler | undefined;
    let configFile: ZuvixConfig = {
      gateway: {
        port: 18789,
        bind: "loopback",
      },
      plugins: {
        entries: {
          diffs: {
            config: {
              viewerBaseUrl: "https://startup.example.com/zuvix",
              defaults: {
                mode: "view",
                theme: "light",
                background: false,
    ...
    const api = createTestPluginApi({
      ...
      registerTool(tool: Parameters<ZuvixPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as ZuvixPluginApi);
