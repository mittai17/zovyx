...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-zuvix-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-zuvix-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.zuvixDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.zuvixDiffsError = "true";
    console.error("Failed to hydrate diff viewer", error);
  }
}

export const disableAutoStartKey = Symbol.for("zuvix.diffs.disableAutoStart");
