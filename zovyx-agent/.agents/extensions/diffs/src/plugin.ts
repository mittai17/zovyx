...
export function registerDiffsPlugin(api: ZuvixPluginApi): void {
  const store = new DiffArtifactStore({
    rootDir: path.join(resolvePreferredZuvixTmpDir(), "zuvix-diffs"),
    logger: api.logger,
  });
...
export function resolveDiffsLanguagePackAvailability(api: ZuvixPluginApi): boolean {
  const currentConfig = (api.runtime.config?.current?.() ?? api.config) as ZuvixConfig;
...
function hasSiblingLanguagePackRuntime(rootDir: string | undefined): boolean {
  ...
  return (
    fs.existsSync(path.join(languagePackRoot, "zuvix.plugin.json")) &&
