...
const disableAutoStartKey = Symbol.for("zuvix.diffs.disableAutoStart");
...
function renderCard(): void {
  document.body.insertAdjacentHTML(
    ...
    `<section class="oc-diff-card">
      <div data-zuvix-diff-host></div>
      <script type="application/json" data-zuvix-diff-payload>${viewerPayload}</script>
    </section>`,
  );
}
...
describe("hydrateViewer", () => {
  beforeEach(() => {
    ...
    delete document.documentElement.dataset.zuvixDiffsError;
    delete document.documentElement.dataset.zuvixDiffsReady;
    vi.clearAllMocks();
  });
  ...
  it("continues hydrating later cards when one card throws", async () => {
    ...
    expect(document.documentElement.dataset.zuvixDiffsError).toBeUndefined();
    warn.mockRestore();
  });
