...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/zuvix/",
      }),
    ).toBe("https://example.com/zuvix");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/zuvix?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/zuvix#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/zuvix/" })).toBe(true);
  });
  ...
  it("preserves defaults and security for direct safeParse callers", () => {
    ...
    expect(data.viewerBaseUrl).toBe("https://example.com/zuvix");
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/zuvix",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/zuvix/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("zuvixDiffsReady");
...
describe("resolveDiffsLanguagePackAvailability", () => {
  it.each(["assets", "dist/assets"])(
    ...
    (assetDir) => {
      const root = fs.mkdtempSync(join(os.tmpdir(), "zuvix-diffs-language-pack-"));
      try {
        ...
        fs.writeFileSync(
          join(languagePackRoot, "zuvix.plugin.json"),
