...
Read-only diff viewer plugin for **Zuvix** agents.

## Install

```bash
zuvix plugins install @zuvix/diffs
```
...
Before:
Zuvix supports plugins.

After:
Zuvix supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/zuvix-diffs`).
