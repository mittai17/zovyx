...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.ZUVIX_SPARSE_PROFILE || process.env.ZUVIX_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because ZUVIX_A2UI_SKIP_MISSING=1 or ZUVIX_SPARSE_PROFILE is set.",
      );
      return;
    }
