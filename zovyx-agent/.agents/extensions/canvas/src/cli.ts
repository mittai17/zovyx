...
import { runCommandWithRuntime, theme } from "zuvix/plugin-sdk/cli-runtime";
...
import { defaultRuntime } from "zuvix/plugin-sdk/runtime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalString,
} from "zuvix/plugin-sdk/string-coerce-runtime";
import { shortenHomePath } from "zuvix/plugin-sdk/text-utility-runtime";
...
/** Creates the default Canvas CLI dependency bundle backed by the Zuvix gateway CLI. */
    ...
    a2ui
      ...
      .action(async (opts: CanvasNodesRpcOpts) => {
        await deps.runNodesCommand("canvas a2ui push", async () => {
          ...
          if (version === "v0.9") {
            throw new Error(
              "Detected A2UI v0.9 JSONL (createSurface). Zuvix currently supports v0.8 only.",
            );
          }
