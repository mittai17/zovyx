...
import { detectMime } from "zuvix/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "zuvix/plugin-sdk/runtime-env";
import { resolveStateDir } from "zuvix/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "zuvix/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "zuvix/plugin-sdk/text-utility-runtime";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.zuvixCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.zuvixCanvasA2UIAction &&
        typeof window.zuvixCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.zuvixSendUserAction === "function";
  ...
  window.addEventListener("zuvix:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Zuvix node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.zuvixSendUserAction === "function"
        ? window.zuvixSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ZUVIX_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Zuvix Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
