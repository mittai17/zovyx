...
import { themeContext } from "@zuvix/a2ui-theme-context";
...
class ZuvixA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.zuvixA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["zuvix:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.zuvixCanvasA2UIAction ??
      globalThis.zuvixCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.zuvixCanvasA2UIAction) {
...
if (!customElements.get("zuvix-a2ui-host")) {
  customElements.define("zuvix-a2ui-host", ZuvixA2UIHost);
}
