...
import { asOptionalRecord as readRecord } from "zuvix/plugin-sdk/string-coerce-runtime";
...
export function migrateLegacyCanvasHostConfig(config: ZuvixConfig): {
  config: ZuvixConfig;
  changes: string[];
} | null {
  ...
  const next = { ...config, plugins } as ZuvixConfig & { canvasHost?: unknown };
