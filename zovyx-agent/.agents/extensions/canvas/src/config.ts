...
export function isCanvasPluginEnabled(config?: ZuvixConfig): boolean {
  if (!config) {
    return true;
  }
...
export function isCanvasHostEnabled(config?: ZuvixConfig): boolean {
...
export const canvasConfigSchema: CanvasPluginConfigSchema = {
  ...
  uiHints: {
    ...
    "host.root": {
      ...
      help: "Directory to serve. Defaults to the Zuvix state canvas directory.",
      advanced: true,
    },
    "host.port": {
