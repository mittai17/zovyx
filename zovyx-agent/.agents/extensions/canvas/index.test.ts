...
import type { AnyAgentTool, ZuvixPluginApi } from "zuvix/plugin-sdk/plugin-entry";
...
function registerCanvas() {
  const routes: Array<Parameters<ZuvixPluginApi["registerHttpRoute"]>[0]> = [];
  const services: Array<Parameters<ZuvixPluginApi["registerService"]>[0]> = [];
  const resolvers: Array<Parameters<ZuvixPluginApi["registerHostedMediaResolver"]>[0]> = [];
  const tools: Array<Parameters<ZuvixPluginApi["registerTool"]>[0]> = [];
  const cliFeatures: Array<{
    registrar: Parameters<ZuvixPluginApi["registerNodeCliFeature"]>[0];
    opts: Parameters<ZuvixPluginApi["registerNodeCliFeature"]>[1];
  }> = [];
...
describe("Canvas plugin entry", () => {
  ...
  it("defers Canvas host implementation until a registered route is used", async () => {
    ...
    await routes[0]?.handler({ url: "/__zuvix__/canvas" } as never, {} as never);
  ...
  it("defers Canvas resolver, CLI, and tool implementations until use", async () => {
    ...
    await expect(resolvers[0]?.("/__zuvix__/canvas/documents/id/index.html")).resolves.toBe(
      "/tmp/canvas-asset",
    );
