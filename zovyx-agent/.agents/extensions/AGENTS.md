...
- Extension production code should import from `zuvix/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `zuvix.plugin.json` and the package
  `zuvix` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `zuvix/plugin-sdk/*`.
