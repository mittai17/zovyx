...
import type { ZuvixPluginApi, ZuvixPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<ZuvixPluginApi> = {}): ZuvixPluginApi {
...
function fakeCtx(overrides: Partial<ZuvixPluginToolContext> = {}): ZuvixPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: ZuvixPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
