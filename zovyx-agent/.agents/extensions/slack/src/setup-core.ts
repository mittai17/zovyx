...
import { hasConfiguredSecretInput } from "zuvix/plugin-sdk/secret-input";
import {
  ...
  type ZuvixConfig,
} from "zuvix/plugin-sdk/setup-runtime";
import { formatDocsLink } from "zuvix/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: ZuvixConfig, accountId: string): ZuvixConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: ZuvixConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: ZuvixConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: ZuvixConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: ZuvixConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: ZuvixConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: ZuvixConfig; accountId: string }) =>
    ...
    disable: (cfg: ZuvixConfig) => setSetupChannelEnabled(cfg, channel, false),
