...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): ZuvixConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as ZuvixConfig;
  ...
  function expectLastSlackSend(content: string, cfg: ZuvixConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: ZuvixConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: ZuvixConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/zuvix-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/zuvix-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/zuvix-media/report.pdf",
      outbound: false,
      contentType: "application/pdf",
    });
  });
