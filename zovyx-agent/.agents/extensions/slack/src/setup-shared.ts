...
import { patchChannelConfigForAccount } from "zuvix/plugin-sdk/setup-runtime";
import { formatDocsLink } from "zuvix/plugin-sdk/setup-tools";
import type { ResolvedSlackAccount } from "./accounts.js";
import type { ZuvixConfig } from "./channel-api.js";

export const SLACK_CHANNEL = "slack" as const;

export function buildSlackManifest(botName = "Zuvix") {
  const safeName = botName.trim() || "Zuvix";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Zuvix`,
    },
    features: {
      bot_user: {
      ...
      assistant_view: {
        assistant_description: `${safeName} connects Slack assistant threads to Zuvix agents.`,
        suggested_prompts: [
          {
