...
vi.mock("zuvix/plugin-sdk/channel-ingress-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("zuvix/plugin-sdk/channel-ingress-runtime")
  >("zuvix/plugin-sdk/channel-ingress-runtime");
  return {
    ...actual,
...
describe("authorizeSlackSystemEventSender", () => {
  ...
  beforeEach(() => {
    clearSlackAllowFromCacheForTest();
    delete process.env.ZUVIX_SLACK_CHANNEL_MEMBERS_CACHE_TTL_MS;
  });
  ...
  it("ignores non-decimal channel member cache ttl env values", async () => {
    process.env.ZUVIX_SLACK_CHANNEL_MEMBERS_CACHE_TTL_MS = "0x0";
