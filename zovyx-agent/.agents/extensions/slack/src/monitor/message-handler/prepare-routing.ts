...
type SlackRouteBinding = NonNullable<ZuvixConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  ZuvixConfig,
  { bindingsRef: ZuvixConfig["bindings"]; normalizedCfg: ZuvixConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: ZuvixConfig): ZuvixConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as ZuvixConfig)
    : cfg;
