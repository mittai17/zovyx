...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): ZuvixConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): ZuvixConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
    ...
    const result = await tool.execute({ query: "zuvix docs", count: 3 });
    ...
    expect(result.query).toBe("zuvix docs");
    expect(result.provider).toBe("ollama");
    expect(result.count).toBe(1);
    expectSingleSearchResultUrl(result.results, "https://zuvix.ai/docs");
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest zuvix release" })).rejects.toThrow(
      "ollama signin",
    );
  });
