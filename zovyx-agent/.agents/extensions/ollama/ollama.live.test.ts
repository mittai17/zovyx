...
const LIVE = process.env.ZUVIX_LIVE_TEST === "1" && process.env.ZUVIX_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.ZUVIX_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.ZUVIX_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.ZUVIX_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.ZUVIX_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.ZUVIX_LIVE_OLLAMA_WEB_SEARCH !== "0";
const RUN_EMBEDDINGS =
  process.env.ZUVIX_LIVE_OLLAMA_EMBEDDINGS === "1" ||
  (process.env.ZUVIX_LIVE_OLLAMA_EMBEDDINGS !== "0" && !isOllamaCloudBaseUrl(OLLAMA_BASE_URL));
...
function requireOllamaRuntimeApiKey(): string | undefined {
  ...
  if (!apiKey) {
    throw new Error(
      "ZUVIX_LIVE_OLLAMA_BASE_URL points at a remote Ollama host; set OLLAMA_API_KEY.",
    );
  }
  return apiKey;
}
...
async function withTempZuvixState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-ollama-cli-live-"));
  try {
    await fs.writeFile(
      path.join(root, "zuvix.json"),
      JSON.stringify(
        {
          models: {
            providers: {
...
async function runZuvixCli(args: string[], env: NodeJS.ProcessEnv) {
  ...
  const commandArgs = sourceRunnerAvailable
    ...
    : ["zuvix.mjs", ...args];
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "zuvix-ollama-cli-output-"));
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    ZUVIX_LIVE_TEST: "1",
    ZUVIX_LIVE_OLLAMA: "1",
    ZUVIX_LIVE_OLLAMA_WEB_SEARCH: "0",
    ZUVIX_STATE_DIR: path.join(root, "state"),
    ZUVIX_CONFIG_PATH: path.join(root, "zuvix.json"),
    ZUVIX_NO_RESPAWN: "1",
    ZUVIX_TEST_FAST: "1",
...
describe.skipIf(!LIVE)("ollama live", () => {
  it("runs infer model run through the local CLI path without static model discovery", async () => {
    await withTempZuvixState(async ({ root }) => {
      const result = await runZuvixCli(
        [
          "infer",
          "model",
          "run",
