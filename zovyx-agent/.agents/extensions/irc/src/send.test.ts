...
import { verifyChannelMessageAdapterCapabilityProofs } from "zuvix/plugin-sdk/channel-outbound";
import { createSendCfgThreadingRuntime } from "zuvix/plugin-sdk/channel-test-helpers";
...
vi.mock("zuvix/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("zuvix/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("zuvix/plugin-sdk/text-chunking", async () => {
  const original = (await vi.importActual("zuvix/plugin-sdk/text-chunking")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("zuvix/plugin-sdk/plugin-config-runtime");
  vi.doUnmock("zuvix/plugin-sdk/text-chunking");
  vi.resetModules();
});
