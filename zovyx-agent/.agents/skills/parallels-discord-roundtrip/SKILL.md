...
```bash
export ZUVIX_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.zuvix/zuvix.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env ZUVIX_PARALLELS_DISCORD_TOKEN \
...
- Use the `zuvix` wrapper for guest `message send/read`; `node zuvix.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/zuvix-parallels-smoke.*`
...
- host inbound nonce appears in `zuvix message read` output
