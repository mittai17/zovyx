---
name: zuvix-landable-bug-sweep
description: "Find or repair small high-confidence non-SDK-boundary Zuvix bugfix PRs until five are landable."
---

# Zuvix Landable Bug Sweep

Autonomous maintainer workflow for producing five landable Zuvix bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$zuvix-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when contributor trust matters, `$zuvix-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
