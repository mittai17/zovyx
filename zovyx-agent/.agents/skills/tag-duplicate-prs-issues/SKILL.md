...
description: Use gitcrawl to search duplicate Zuvix PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local Zuvix history and clustering source.
...
gitcrawl neighbors zuvix/zuvix --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search zuvix/zuvix --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search zuvix/zuvix --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```

Inspect likely clusters:

```bash
gitcrawl cluster-detail zuvix/zuvix --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
```bash
gh search prs --repo zuvix/zuvix --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo zuvix/zuvix --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo zuvix/zuvix --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R zuvix/zuvix "<problem phrase>" --types group --limit 10
prtags search similar -R zuvix/zuvix "<problem summary>" --types group --limit 10
prtags group list -R zuvix/zuvix
```
...
```bash
prtags group create -R zuvix/zuvix \
  --kind mixed \
...
```bash
prtags field ensure -R zuvix/zuvix --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R zuvix/zuvix --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R zuvix/zuvix --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R zuvix/zuvix --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R zuvix/zuvix --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R zuvix/zuvix --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R zuvix/zuvix --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R zuvix/zuvix --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R zuvix/zuvix --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R zuvix/zuvix <pr-number> \
...
```bash
prtags annotation issue set -R zuvix/zuvix <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R zuvix/zuvix
```
