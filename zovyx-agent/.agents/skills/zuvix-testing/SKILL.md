---
name: zuvix-testing
description: Choose, run, rerun, or debug Zuvix tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
--provider blacksmith-testbox ... -- env ZUVIX_CHECK_CHANGED_REMOTE_CHILD=1 ZUVIX_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed`. For direct AWS
...
ZUVIX_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
ZUVIX_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `ZUVIX_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `ZUVIX_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `Zuvix Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the public `zuvix/releases` evidence ledger.
Use the manual `Zuvix Release Evidence`
(`zuvix-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation zuvix/zuvix <run-id> blocking
package-acceptance zuvix/zuvix <run-id> blocking
release-checks zuvix/zuvix <run-id> blocking
```
...
When `Full Release Validation` completes and `ZUVIX_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`Zuvix Release Evidence From Full Validation` workflow. That workflow reads
...
`Zuvix Release Checks` (`zuvix-release-checks.yml`) is the release child
...
```bash
gh workflow run zuvix-release-checks.yml \
  --repo zuvix/zuvix \
  --ref main \
  -f ref=<branch-or-sha> \
  -f provider=openai \
...
`Zuvix Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/zuvix/zuvix-live-media-runner:ubuntu-24.04` container so
...
`pnpm zuvix qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm zuvix qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `ZUVIX_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
Matrix selection. `Zuvix Release Checks` uses explicit fast Matrix; run the
all-lanes workflow when release investigation needs full Matrix media/E2EE
inventory.

### Reusable Live/E2E Checks

`Zuvix Live And E2E Checks (Reusable)`
(`zuvix-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run zuvix-live-and-e2e-checks-reusable.yml \
  --repo zuvix/zuvix \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `ZUVIX_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`ZUVIX_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
ZUVIX_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
ZUVIX_DOCKER_ALL_DRY_RUN=1 ZUVIX_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
ZUVIX_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
ZUVIX_DOCKER_ALL_LANES=<lane> \
ZUVIX_DOCKER_ALL_BUILD=0 \
ZUVIX_DOCKER_ALL_PREFLIGHT=0 \
ZUVIX_SKIP_DOCKER_BUILD=1 \
ZUVIX_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
ZUVIX_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  `npm view zuvix dist-tags --json --prefer-online --cache /tmp/zuvix-npm-cache-verify-$$`
  and `npm view zuvix@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/zuvix-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=zuvix@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=zuvix@YYYY.M.D-beta.N` or
  `package_spec=zuvix@YYYY.M.D`.
- For stable package proof, use `package_spec=zuvix@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `zuvix@beta`,
  `zuvix@latest`, or exact Zuvix release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `zuvix update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `zuvix@beta`, `zuvix@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an Zuvix branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-zuvix-for-docker.mjs` is the
...
from `zuvix skills search`, installs it, and verifies `.clawhub` origin/lock
