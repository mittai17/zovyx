---
name: crabbox
description: Use the Crabbox wrapper for Zuvix remote validation across Linux, macOS, Windows, and WSL2, including delegated Blacksmith Testbox proof. Report the actual provider and id.
---

# Crabbox

Use the Crabbox wrapper when Zuvix needs remote Linux proof for broad tests,
...
For Zuvix maintainer broad `pnpm` gates, Blacksmith Testbox through the
...
comparison. Use `--provider blacksmith-testbox` when the task needs Zuvix
...
- Zuvix scripts prefer `../crabbox/bin/crabbox` when present. The user PATH
  shim can be stale.
...
- The brokered AWS default is a Linux developer image in `eu-west-1`; the repo
  ...
  region/AZ placement, and FSR state before blaming Zuvix.
- For broad Zuvix maintainer `pnpm` gates, prefer the repo wrapper with
...
- Do not treat inherited shell env as operator intent. In particular,
  `ZUVIX_LOCAL_CHECK_MODE=throttled` from the local shell is not permission
  to move broad `pnpm check:changed`, `pnpm test:changed`, full `pnpm test`, or
  lint/typecheck fan-out onto the laptop.
- Only use `ZUVIX_LOCAL_CHECK_MODE=throttled|full` when the user explicitly
...
Use these only when the task needs an existing non-Linux host. Zuvix broad
...
Do not silently substitute AWS macOS for normal Zuvix Linux proof. Report
...
Use this for Zuvix maintainer broad/heavy `pnpm` gates when the prepared CI
...
node scripts/crabbox-wrapper.mjs run \
  --provider blacksmith-testbox \
  --cache-volume pnpm-store=zuvix-node24-pnpm-lock:/tmp/zuvix-pnpm-store \
  --timing-json \
  -- \
  corepack pnpm check:changed
```
...
install Zuvix as the user would, run the same setup/update/CLI/Gateway/API
...
- Prefer `ZUVIX_CURRENT_PACKAGE_TGZ` with Docker/package lanes when testing a
...
- Prefer deterministic arrow navigation over search typing for Clack-style
  ...
  sequences.
- Isolate mutable state with `ZUVIX_STATE_DIR=$(mktemp -d)`. Plugin npm
  installs live under that state dir (`npm/node_modules/...`), not under
  `ZUVIX_CONFIG_DIR`. Verify downloads by checking the state dir, package
  lock, and installed package metadata.
- To test automatic setup installs against local package artifacts, use
  `ZUVIX_ALLOW_PLUGIN_INSTALL_OVERRIDES=1` plus
  `ZUVIX_PLUGIN_INSTALL_OVERRIDES='{"plugin-id":"npm-pack:/tmp/plugin.tgz"}'`.
  Pack with `npm pack`, set an isolated `ZUVIX_STATE_DIR`, and verify the
...
- For OpenAI/Codex onboarding proof, the useful markers are the UI line
  `Installed Codex plugin`, `npm/node_modules/@zuvix/codex`, and the
...
- Bad local config: inspect `.crabbox.yaml`, `crabbox config show`, and
  `crabbox whoami`; normal Zuvix proof should use brokered AWS without
...
```sh
blacksmith auth login --non-interactive --organization zuvix
```
...
Use AWS for normal Zuvix remote proof. The repo `.crabbox.yaml` already
...
```sh
brew install zuvix/tap/crabbox
...
- If broker auth is missing, run `crabbox login --url https://crabbox.zuvix.ai --provider aws`.
- If the CLI asks for `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or AWS
  profile setup during normal Zuvix validation, assume the agent selected
...
- Ask for AWS keys only for explicit direct-provider/account administration,
  not for normal brokered Zuvix proof.
- Trusted automation may still use
  `printf '%s' "$CRABBOX_COORDINATOR_TOKEN" | crabbox login --url https://crabbox.zuvix.ai --provider aws --token-stdin`.
...
for Zuvix lanes. Let that config drive normal validation.
...
Do not add Zuvix-specific setup to Crabbox itself. Put repo setup in the
