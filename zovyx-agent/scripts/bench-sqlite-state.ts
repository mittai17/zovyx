// SQLite state benchmark seeds Zuvix DBs and reports hot-query proof lines.
...
} from "../src/state/zuvix-agent-db.js";
import {
  closeZuvixStateDatabaseForTest,
  openZuvixStateDatabase,
} from "../src/state/zuvix-state-db.js";
...
function printUsage(): void {
  console.log(`Zuvix SQLite state benchmark
...
function main(): void {
  ...
  const stateDir =
    options.stateDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-sqlite-perf-"));
  const env = { ZUVIX_STATE_DIR: stateDir };
  const started = nowMs();
  try {
    const stateDatabase = openZuvixStateDatabase({ env });
    const agentDatabases = Array.from({ length: config.agentCount }, (_, index) =>
      openZuvixAgentDatabase({ agentId: `perf-agent-${index}`, env }),
    );
