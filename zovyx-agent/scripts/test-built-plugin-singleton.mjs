...
const { clearPluginCommands, getPluginCommandSpecs, loadZuvixPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadZuvixPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "zuvix.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'zuvix/plugin-sdk';",
...
const registry = loadZuvixPlugins({
  ...
  env: {
    ...process.env,
    ZUVIX_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
