#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports Zuvix repository automation.
...
type DriverMode = "token" | "webhook" | "zuvix";
...
const DEFAULT_ZUVIX_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  const override = process.env.ZUVIX_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.ZUVIX_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".zuvix");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "zuvix") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or zuvix.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver zuvix] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Zuvix spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|zuvix> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override Zuvix state dir for plugin-state polling\n" +
    "  --zuvix-bin <path>        Zuvix CLI binary for driver=zuvix (default: zuvix)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  ZUVIX_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ZUVIX_DISCORD_SMOKE_DRIVER\n" +
    "  ZUVIX_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ZUVIX_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ZUVIX_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ZUVIX_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ZUVIX_DISCORD_SMOKE_AGENT\n" +
    "  ZUVIX_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ZUVIX_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ZUVIX_DISCORD_SMOKE_POLL_MS\n" +
    "  ZUVIX_STATE_DIR\n" +
    "  ZUVIX_DISCORD_SMOKE_ZUVIX_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.ZUVIX_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.ZUVIX_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token") || process.env.ZUVIX_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ZUVIX_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ZUVIX_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.ZUVIX_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.ZUVIX_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.ZUVIX_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.ZUVIX_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ZUVIX_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ZUVIX_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir") || resolveStateDir());
  const zuvixBin =
    resolveArg("--zuvix-bin") || process.env.ZUVIX_DISCORD_SMOKE_ZUVIX_BIN || "zuvix";
...
async function zuvixCliJson<T>(params: {
  zuvixBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.zuvixBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_ZUVIX_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`zuvix ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithZuvix(params: {
  zuvixBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await zuvixCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    zuvixBin: params.zuvixBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "zuvix") {
    return await readMessagesWithZuvix({
      zuvixBin: params.args.zuvixBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `zuvix-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("zuvix message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "zuvix"
            ? await readMessagesWithZuvix({
                zuvixBin: args.zuvixBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Zuvix.`,
        diagnostics: {
          bindingCandidates: [
            {
