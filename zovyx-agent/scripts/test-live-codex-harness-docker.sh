...
TRUSTED_HARNESS_DIR="${ZUVIX_LIVE_DOCKER_TRUSTED_HARNESS_DIR:-${ZUVIX_LIVE_CODEX_TRUSTED_HARNESS_DIR:-$SCRIPT_ROOT_DIR}}"
...
CODEX_HARNESS_AUTH_MODE="${ZUVIX_LIVE_CODEX_HARNESS_AUTH:-codex-auth}"
CODEX_CLI_PACKAGE_SPEC="${ZUVIX_LIVE_CODEX_CLI_PACKAGE_SPEC:-}"
TEMP_DIRS=()
DOCKER_USER="${ZUVIX_DOCKER_USER:-node}"
DOCKER_HOME_MOUNT=()
DOCKER_TRUSTED_HARNESS_MOUNT=()
DOCKER_TRUSTED_HARNESS_CONTAINER_DIR=""
DOCKER_CACHE_CONTAINER_DIR="/tmp/zuvix-cache"
DOCKER_CLI_TOOLS_CONTAINER_DIR="/tmp/zuvix-npm-global"
DOCKER_EXTRA_ENV_FILES=()
DOCKER_AUTH_PRESTAGED=0

zuvix_live_codex_harness_is_ci() {
  zuvix_live_is_ci
}

zuvix_live_codex_harness_append_build_extension() {
  local extension="${1:?extension required}"
  local current="${ZUVIX_DOCKER_BUILD_EXTENSIONS:-${ZUVIX_EXTENSIONS:-}}"
  case " $current " in
    *" $extension "*)
      ;;
    *)
      export ZUVIX_DOCKER_BUILD_EXTENSIONS="${current:+$current }$extension"
      ;;
  esac
}
...
case "$CODEX_HARNESS_AUTH_MODE" in
  codex-auth | api-key)
    ;;
  *)
    echo "ERROR: ZUVIX_LIVE_CODEX_HARNESS_AUTH must be one of: codex-auth, api-key." >&2
    exit 1
    ;;
esac
...
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" && -z "${OPENAI_API_KEY:-}" ]]; then
  echo "ERROR: ZUVIX_LIVE_CODEX_HARNESS_AUTH=api-key requires OPENAI_API_KEY." >&2
  exit 1
fi
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" && ! -s "$HOME/.codex/auth.json" ]]; then
  echo "ERROR: ZUVIX_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image." >&2
  if [[ -n "${OPENAI_API_KEY:-}" ]]; then
    echo "If this is a Testbox/API-key run, set ZUVIX_LIVE_CODEX_HARNESS_AUTH=api-key and run through zuvix-testbox-env." >&2
  fi
  exit 1
fi
...
elif zuvix_live_codex_harness_is_ci; then
...
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" ]]; then
  ...
  CONFIG_DIR="$(mktemp -d "${RUNNER_TEMP:-/tmp}/zuvix-docker-config.XXXXXX")"
  WORKSPACE_DIR="$(mktemp -d "${RUNNER_TEMP:-/tmp}/zuvix-docker-workspace.XXXXXX")"
...
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" ]]; then
  ...
  done < <(zuvix_live_collect_auth_files_from_csv "openai")
fi
...
if [[ -n "${DOCKER_HOME_DIR:-}" ]]; then
  zuvix_live_stage_auth_into_home "$DOCKER_HOME_DIR" --files "${AUTH_FILES[@]}"
...
DOCKER_AUTH_ENV=()
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/zuvix-codex-harness-env.XXXXXX")"
...
if [ "${ZUVIX_LIVE_CODEX_HARNESS_DEBUG:-}" = "1" ]; then
  id
  mount | grep -E 'zuvix-cache|zuvix-npm|/home/node' || true
...
if [ "${ZUVIX_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ]; then
...
run_setup_command() {
  local timeout_value="${ZUVIX_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:-180}s"
  local timeout_bin=""
...
if [ "${ZUVIX_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ] && [ ! -s "$HOME/.codex/auth.json" ]; then
...
if [ -d /app/dist-runtime/extensions/codex ]; then
  export ZUVIX_BUNDLED_PLUGINS_DIR=/app/dist-runtime/extensions
elif [ -d /app/dist/extensions/codex ]; then
  export ZUVIX_BUNDLED_PLUGINS_DIR=/app/dist/extensions
elif [ -f "$tmp_dir/extensions/codex/zuvix.plugin.json" ]; then
  export ZUVIX_BUNDLED_PLUGINS_DIR="$tmp_dir/extensions"
else
  echo "ERROR: staged Codex plugin not found for live harness." >&2
  exit 1
fi
zuvix_live_stage_state_dir "$tmp_dir/.zuvix-state"
if [ -n "${ZUVIX_LIVE_CODEX_TRUSTED_HARNESS_DIR:-}" ] && [ -d "$ZUVIX_LIVE_CODEX_TRUSTED_HARNESS_DIR" ]; then
  for harness_file in src/gateway/gateway-codex-harness.live-helpers.ts; do
    if [ -f "$ZUVIX_LIVE_CODEX_TRUSTED_HARNESS_DIR/$harness_file" ]; then
      mkdir -p "$(dirname "$tmp_dir/$harness_file")"
      cp "$ZUVIX_LIVE_CODEX_TRUSTED_HARNESS_DIR/$harness_file" "$tmp_dir/$harness_file"
    fi
  done
fi
zuvix_live_prepare_staged_config
cd "$tmp_dir"
if [ "${ZUVIX_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}" = "1" ]; then
...
if ! "$NPM_CONFIG_PREFIX/bin/codex" exec \
  ...
  if grep -q "Failed to extract accountId from token" "$codex_preflight_log"; then
    echo "ERROR: Codex auth cannot extract accountId from the available token; refresh ZUVIX_CODEX_AUTH_JSON or use ZUVIX_LIVE_CODEX_HARNESS_AUTH=api-key." >&2
    exit 1
  fi
  tail -c 262144 "$codex_preflight_log" >&2 || true
  exit 1
fi
node scripts/test-live.mjs -- ${ZUVIX_LIVE_CODEX_TEST_FILES:-src/gateway/gateway-codex-harness.live.test.ts}
EOF

zuvix_live_codex_harness_append_build_extension codex
...
ZUVIX_SKIP_DOCKER_BUILD=0
export ZUVIX_SKIP_DOCKER_BUILD
...
echo "==> Model: ${ZUVIX_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}"
echo "==> Chat image probe: ${ZUVIX_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE:-0}"
echo "==> Image probe: ${ZUVIX_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}"
echo "==> MCP probe: ${ZUVIX_LIVE_CODEX_HARNESS_MCP_PROBE:-1}"
echo "==> Subagent probe: ${ZUVIX_LIVE_CODEX_HARNESS_SUBAGENT_PROBE:-1}"
echo "==> Subagent-only fast path: ${ZUVIX_LIVE_CODEX_HARNESS_SUBAGENT_ONLY:-auto}"
echo "==> Guardian probe: ${ZUVIX_LIVE_CODEX_HARNESS_GUARDIAN_PROBE:-1}"
echo "==> Auth mode: $CODEX_HARNESS_AUTH_MODE"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> CI-safe Codex config: ${ZUVIX_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}"
echo "==> Test files: ${ZUVIX_LIVE_CODEX_TEST_FILES:-src/gateway/gateway-codex-harness.live.test.ts}"
echo "==> Codex CLI package: $CODEX_CLI_PACKAGE_SPEC"
echo "==> Harness fallback: none"
echo "==> Auth files: ${AUTH_FILES_CSV:-none}"
DOCKER_RUN_ARGS=()
zuvix_live_init_docker_run_args DOCKER_RUN_ARGS "${ZUVIX_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e ZUVIX_AGENT_HARNESS_FALLBACK=none \
  -e ZUVIX_DOCKER_AUTH_PRESTAGED="$DOCKER_AUTH_PRESTAGED" \
  -e ZUVIX_CODEX_APP_SERVER_BIN="${ZUVIX_CODEX_APP_SERVER_BIN:-codex}" \
  ...
  -e ZUVIX_LIVE_CODEX_HARNESS_AUTH="$CODEX_HARNESS_AUTH_MODE" \
  -e ZUVIX_LIVE_CODEX_HARNESS=1 \
  -e ZUVIX_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE="${ZUVIX_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE:-0}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_DEBUG="${ZUVIX_LIVE_CODEX_HARNESS_DEBUG:-}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_GUARDIAN_PROBE="${ZUVIX_LIVE_CODEX_HARNESS_GUARDIAN_PROBE:-1}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_IMAGE_PROBE="${ZUVIX_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_MCP_PROBE="${ZUVIX_LIVE_CODEX_HARNESS_MCP_PROBE:-1}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_MODEL="${ZUVIX_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_REQUIRE_GUARDIAN_EVENTS="${ZUVIX_LIVE_CODEX_HARNESS_REQUIRE_GUARDIAN_EVENTS:-1}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_REQUEST_TIMEOUT_MS="${ZUVIX_LIVE_CODEX_HARNESS_REQUEST_TIMEOUT_MS:-}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS="${ZUVIX_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:-180}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_SUBAGENT_ONLY="${ZUVIX_LIVE_CODEX_HARNESS_SUBAGENT_ONLY:-}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_SUBAGENT_PROBE="${ZUVIX_LIVE_CODEX_HARNESS_SUBAGENT_PROBE:-1}" \
  -e ZUVIX_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG="${ZUVIX_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}" \
  -e ZUVIX_LIVE_CODEX_CLI_PACKAGE_SPEC="$CODEX_CLI_PACKAGE_SPEC" \
  ...
  -e ZUVIX_LIVE_DOCKER_TRUSTED_HARNESS_DIR="$DOCKER_TRUSTED_HARNESS_CONTAINER_DIR" \
  -e ZUVIX_LIVE_CODEX_TRUSTED_HARNESS_DIR="$DOCKER_TRUSTED_HARNESS_CONTAINER_DIR" \
  -e ZUVIX_LIVE_CODEX_BIND="${ZUVIX_LIVE_CODEX_BIND:-}" \
  -e ZUVIX_LIVE_CODEX_BIND_MODEL="${ZUVIX_LIVE_CODEX_BIND_MODEL:-}" \
  -e ZUVIX_LIVE_CODEX_BIND_PROVIDER="${ZUVIX_LIVE_CODEX_BIND_PROVIDER:-}" \
  -e ZUVIX_LIVE_CODEX_TEST_FILES="${ZUVIX_LIVE_CODEX_TEST_FILES:-}" \
  -e ZUVIX_LIVE_TEST=1 \
