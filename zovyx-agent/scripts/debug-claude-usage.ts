// Debug Claude Usage script supports Zuvix repository automation.
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.ZUVIX_STATE_DIR?.trim() || path.join(os.homedir(), ".zuvix");
...
const resolveFetchTimeoutMs = (raw = process.env.ZUVIX_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "ZUVIX_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
