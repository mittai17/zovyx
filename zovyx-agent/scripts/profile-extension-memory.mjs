...
const RSS_MARKER = "__ZUVIX_MAX_RSS_KB__=";
...
async function main() {
  ...
  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "zuvix-extension-memory-"));
  const hookPath = path.join(tmpHome, "measure-rss.mjs");
  const jsonPath = options.jsonPath ?? path.join(os.tmpdir(), "zuvix-extension-memory.json");
