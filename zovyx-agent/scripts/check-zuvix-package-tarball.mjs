...
function usage() {
  return "Usage: node scripts/check-zuvix-package-tarball.mjs <zuvix.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`Zuvix package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.ZUVIX_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-zuvix-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "zuvix") {
      errors.push("npm-shrinkwrap.json root name must be zuvix");
    }
    ...
    if (!rootPackage || rootPackage.name !== "zuvix") {
      errors.push("npm-shrinkwrap.json packages root must name zuvix");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`Zuvix package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Zuvix package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("Zuvix package tarball integrity passed.");
