// Verifies installed packages can bootstrap the default Zuvix workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "zuvix-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "zuvix.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".zuvix", "workspace");
