// Zuvix release ClawHub plan script supports release workflow routing.
...
export type ZuvixReleaseClawHubPlanArgs = {
  releaseTag: string;
...
export type ZuvixReleaseClawHubPlan = {
...
export type ZuvixReleaseClawHubRuntimeStateArgs = {
  repository: string;
...
export type ZuvixReleaseClawHubRuntimeState = {
  verifierArgs: string[];
  proofLines: {
    normal: string;
    bootstrap: string;
...
export function buildZuvixReleaseClawHubRuntimeState(
  args: ZuvixReleaseClawHubRuntimeStateArgs,
): ZuvixReleaseClawHubRuntimeState {
...
export function parseZuvixReleaseClawHubPlanArgs(
  argv: string[],
): ZuvixReleaseClawHubPlanArgs {
  const values = [...argv];
  if (values[0] === "--") {
    values.shift();
  }
...
export async function buildZuvixReleaseClawHubPlan(
  args: ZuvixReleaseClawHubPlanArgs,
  options: {
    rootDir?: string;
    fetchImpl?: typeof fetch;
    registryBaseUrl?: string;
  } = {},
): Promise<ZuvixReleaseClawHubPlan> {
