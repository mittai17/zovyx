...
export const DEFAULT_E2E_BARE_IMAGE = "zuvix-docker-e2e-bare:local";
export const DEFAULT_E2E_FUNCTIONAL_IMAGE = "zuvix-docker-e2e-functional:local";
...
export function normalizeUpgradeSurvivorBaselineSpec(raw) {
  ...
  const spec = value.startsWith("zuvix@") ? value : `zuvix@${value}`;
  if (
    !/^zuvix@(?:alpha|beta|latest|[0-9]{4}\.[0-9]+\.[0-9]+(?:-(?:[0-9]+|alpha\.[0-9]+|beta\.[0-9]+))?)$/u.test(
      spec,
    )
  ) {
    throw new Error(
      `invalid published upgrade survivor baseline: ${JSON.stringify(
        value,
      )}. Expected zuvix@latest, zuvix@beta, zuvix@alpha, or zuvix@YYYY.M.PATCH.`,
    );
  }
  return spec;
}
...
function parsePublishedReleaseVersion(spec) {
  const match = /^zuvix@([0-9]{4})\.([0-9]+)\.([0-9]+)/u.exec(String(spec ?? ""));
  if (!match) {
    return null;
  }
  return {
    ...
    return matrixBaselines.flatMap((baselineSpec) =>
      matrixScenarios
        ...
        .map((scenario) => {
          ...
          const commandPrefix = [
            `ZUVIX_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}"`,
            baselineSpec
              ? `ZUVIX_UPGRADE_SURVIVOR_BASELINE_SPEC=${shellQuote(baselineSpec)}`
              : "",
            scenario ? `ZUVIX_UPGRADE_SURVIVOR_SCENARIO=${shellQuote(scenario)}` : "",
          ]
            .filter(Boolean)
            .join(" ");
...
export function parseLiveMode(raw) {
  ...
  throw new Error(
    `ZUVIX_DOCKER_ALL_LIVE_MODE must be one of: all, skip, only. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function parseProfile(raw) {
  ...
  throw new Error(
    `ZUVIX_DOCKER_ALL_PROFILE must be one of: ${DEFAULT_PROFILE}, ${RELEASE_PATH_PROFILE}. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function lanesNeedZuvixPackage(poolLanes) {
...
export function findLaneByName(name) {
  return dedupeLanes(
    expandUpgradeSurvivorBaselineLanes(
      ...
      process.env.ZUVIX_UPGRADE_SURVIVOR_BASELINE_SPECS,
      process.env.ZUVIX_UPGRADE_SURVIVOR_SCENARIOS,
    ),
...
function laneCredentialRequirements(poolLane) {
  ...
  if (resources.includes("live:zuvix")) {
    credentials.push("zuvix");
  }
  return credentials;
}
...
function buildPlanJson(params) {
  ...
  return {
    ...
    needs: {
      ...
      package: lanesNeedZuvixPackage(scheduledLanes),
    },
  ...
  const selectedLanes =
    options.selectedLaneNames.length > 0
      ? options.selectedLaneNames.flatMap((selectedName) => {
          ...
          selectNamedLanes(selectableLanes, [selectedName], "ZUVIX_DOCKER_ALL_LANES");
          return [];
        })
      : undefined;
