// Plugin Npm Release script supports Zuvix repository automation.
...
import { parseReleaseVersion } from "../zuvix-npm-release-check.ts";
...
export const ZUVIX_PLUGIN_NPM_REPOSITORY_URL = "https://github.com/zuvix/zuvix";
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.zuvix?.install?.npmSpec);
  const repositoryUrl =
    typeof packageJson.repository === "string"
      ? packageJson.repository.trim()
      : (packageJson.repository?.url?.trim() ?? "");
  const extensions = packageJson.zuvix?.extensions ?? [];

  if (!packageName.startsWith("@zuvix/")) {
    errors.push(
      `package name must start with "@zuvix/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (repositoryUrl !== ZUVIX_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${ZUVIX_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("zuvix.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("zuvix.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("zuvix.install.npmSpec must be a non-empty string for publishable plugins.");
  }
  errors.push(
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.zuvix?.install?.npmSpec),
    });
  }
...
function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "zuvix-plugin-npm-view-"));
