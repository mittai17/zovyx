...
export function isPublishablePluginPackage(packageJson) {
  return (
    packageJson.zuvix?.release?.publishToNpm === true ||
    packageJson.zuvix?.release?.publishToClawHub === true
  );
}
...
function createNeverBundleDependencyMatcher(packageJson) {
  ...
  return (id) => {
    if (id === "zuvix" || id.startsWith("zuvix/")) {
      return true;
    }
...
export function resolvePluginNpmRuntimePackageFiles(plan) {
  ...
  merged.add("dist/**");
  if (packageRelativePathExists(plan.packageDir, "zuvix.plugin.json")) {
    merged.add("zuvix.plugin.json");
  }
...
function normalizeZuvixPeerRange(value) {
...
function resolveZuvixPeerRange(packageJson, rootPackageJson) {
  return (
    normalizeZuvixPeerRange(packageJson.zuvix?.compat?.pluginApi) ||
    normalizeZuvixPeerRange(packageJson.peerDependencies?.zuvix) ||
    normalizeZuvixPeerRange(packageJson.zuvix?.build?.zuvixVersion) ||
    normalizeZuvixPeerRange(rootPackageJson?.version) ||
    normalizeZuvixPeerRange(packageJson.version)
  );
}

/** Resolve package peer dependency metadata for the Zuvix plugin API. */
export function resolvePluginNpmRuntimePackagePeerMetadata(plan) {
  const zuvixPeerRange = resolveZuvixPeerRange(plan.packageJson, plan.rootPackageJson);
  if (!zuvixPeerRange) {
    throw new Error(
      `cannot infer zuvix peerDependency range for ${plan.pluginDir}; set zuvix.compat.pluginApi or package version`,
    );
  }
  ...
  const existingZuvixMeta = getRecord(existingPeerDependenciesMeta.zuvix);
  return {
    peerDependencies: {
      ...existingPeerDependencies,
      zuvix: zuvixPeerRange,
    },
...
export function resolvePluginNpmRuntimeBuildPlan(params) {
  ...
  const plan = {
    ...
    runtimeExtensions: (Array.isArray(packageJson.zuvix?.extensions)
      ? packageJson.zuvix.extensions
      : []
    )
      .map(normalizePackageEntry)
      .filter(Boolean)
      .map(toPackageRuntimeEntry),
    runtimeSetupEntry: normalizePackageEntry(packageJson.zuvix?.setupEntry)
      ? toPackageRuntimeEntry(packageJson.zuvix.setupEntry)
      : undefined,
  };
  return {
    ...plan,
