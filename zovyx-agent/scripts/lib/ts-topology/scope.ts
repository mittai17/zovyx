// Scope script supports Zuvix repository automation.
...
export function createPluginSdkScope(_repoRoot: string): TopologyScope {
  const entrypoints = publicPluginSdkEntrypoints.map((entrypoint) => ({
    ...
    importSpecifier:
      entrypoint === "index" ? "zuvix/plugin-sdk" : `zuvix/plugin-sdk/${entrypoint}`,
  }));
  return buildScopeFromEntrypoints("plugin-sdk", "Zuvix plugin-sdk public surface", entrypoints);
}
