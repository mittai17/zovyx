// Release Beta Verifier script supports Zuvix repository automation.
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--clawhub-workflow-ref REF] [--full-release-validation-run ID] [--zuvix-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID] [--plugin-clawhub-bootstrap-run ID] [--npm-telegram-run ID] [--skip-github-release] [--skip-clawhub]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--zuvix-npm-run":
        parsed.workflowRuns.zuvixNpm = next();
        break;
      case "--plugin-npm-run":
...
export async function verifyBetaRelease(
  ...
  const zuvixNpm = verifyNpmPackage("zuvix", args.version, args.distTag);
  lines.push(`zuvix npm OK: ${args.version} (${args.distTag})`);
  ...
  if (!args.skipPostpublish) {
    ...
    lines.push("zuvix postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.zuvixNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.zuvixNpm,
        label: "Zuvix NPM Release",
        repo: args.repo,
        expectedWorkflowName: "Zuvix NPM Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: false,
      }),
    );
      ...
      `${JSON.stringify(
        {
          ...
          zuvixNpmIntegrity: zuvixNpm.integrity,
