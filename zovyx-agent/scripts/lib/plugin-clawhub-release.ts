// Plugin Clawhub Release script supports Zuvix repository automation.
...
const ZUVIX_PLUGIN_CLAWHUB_REPOSITORY = "zuvix/zuvix";
const ZUVIX_PLUGIN_CLAWHUB_WORKFLOW_FILENAME = "plugin-clawhub-release.yml";
...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/zuvix-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.zuvix?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.zuvix?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
async function hasClawHubTrustedPublisher(
  ...
  return isZuvixPluginTrustedPublisher(trustedPublisherDetail.trustedPublisher);
}

function isZuvixPluginTrustedPublisher(value: unknown): boolean {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  const trustedPublisher = value as ClawHubTrustedPublisherConfig;
  return (
    trustedPublisher.repository === ZUVIX_PLUGIN_CLAWHUB_REPOSITORY &&
    trustedPublisher.workflowFilename === ZUVIX_PLUGIN_CLAWHUB_WORKFLOW_FILENAME &&
