...
zuvix_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${ZUVIX_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "ZUVIX_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
zuvix_live_scrub_staged_plugin_index() {
  local dest_dir="${1:?destination directory required}"
  local db_path="$dest_dir/state/zuvix.sqlite"
...
zuvix_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.zuvix"
  ...
  if [ -d "$source_dir" ]; then
    ...
    zuvix_live_scrub_staged_plugin_index "$dest_dir"
    if [ -d "$source_dir/workspace" ] && [ ! -e "$dest_dir/workspace" ]; then
      ln -s "$source_dir/workspace" "$dest_dir/workspace"
    fi
  fi

  export ZUVIX_STATE_DIR="$dest_dir"
  export ZUVIX_CONFIG_PATH="$dest_dir/zuvix.json"
}
