...
const logLevel = process.env.ZUVIX_BUILD_VERBOSE ? "info" : "warn";
const INEFFECTIVE_DYNAMIC_IMPORT_MARKER = "[INEFFECTIVE_DYNAMIC_IMPORT]";
const UNRESOLVED_IMPORT_RE = /\[UNRESOLVED_IMPORT\]/;
const ANSI_ESCAPE_RE = new RegExp(String.raw`\u001B\[[0-9;]*m`, "g");
const DEPENDENCY_PATH_MARKERS = ["node_modules/", "zuvix-pnpm-node-modules/"];
...
const PRESERVE_CLI_STARTUP_METADATA_ENV = "ZUVIX_PRESERVE_CLI_STARTUP_METADATA";
...
export function tsdownBuildUsage() {
  return [
    ...
    "",
    "Builds Zuvix with tsdown and validates emitted import diagnostics.",
    "",
    "Options:",
...
export function resolveTsdownBuildInvocation(params = {}) {
  ...
  if (env.ZUVIX_BUILD_ALL_NO_PNPM === "1") {
    return {
...
export async function runTsdownBuildInvocation(invocation, params = {}) {
  ...
  const timeoutMs = parsePositiveIntegerEnv(
    env.ZUVIX_TSDOWN_TIMEOUT_MS,
    "ZUVIX_TSDOWN_TIMEOUT_MS",
  );
  const heartbeatMs =
    parseNonNegativeIntegerEnv(env.ZUVIX_TSDOWN_HEARTBEAT_MS, "ZUVIX_TSDOWN_HEARTBEAT_MS") ??
