...
const DEFAULT_CLAWHUB_SOURCE_REPO = "zuvix/clawhub";
const CLAWHUB_DOCS_TARGET_DIR = "clawhub";
const CLAWHUB_REPO_ENV = "ZUVIX_DOCS_SYNC_CLAWHUB_REPO";
...
const SYNC_SUPPORT_FILES = [
  {
    ...
    target: path.join(".zuvix-sync", "check-docs-mdx.mjs"),
  },
  {
    source: path.join(ROOT, "scripts", "lib", "mintlify-accordion.mjs"),
    target: path.join(".zuvix-sync", "lib", "mintlify-accordion.mjs"),
  },
  {
    source: path.join(ROOT, ".github", "codex", "prompts", "docs-mdx-repair.md"),
    target: path.join(".zuvix-sync", "docs-mdx-repair.md"),
  },
];
...
export function parseArgs(argv) {
  const args = {
    ...
    clawhubSourceRepo:
      process.env.ZUVIX_DOCS_SYNC_CLAWHUB_SOURCE_REPO || DEFAULT_CLAWHUB_SOURCE_REPO,
    clawhubSourceSha: process.env.ZUVIX_DOCS_SYNC_CLAWHUB_SOURCE_SHA || "",
  };
...
function writeSyncMetadata(targetRoot, args, sources) {
  ...
  writeJson(path.join(targetRoot, ".zuvix-sync", "source.json"), metadata);
}
