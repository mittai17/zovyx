...
export function testLiveUsage() {
  return [
    ...
    "",
    "Runs live Vitest suites with ZUVIX_LIVE_TEST=1.",
    "",
    "Options:",
...
export function buildTestLiveEnv(args, baseEnv = process.env) {
  return {
    ...
    pnpm_config_verify_deps_before_run: baseEnv.pnpm_config_verify_deps_before_run || "false",
    ZUVIX_LIVE_TEST: baseEnv.ZUVIX_LIVE_TEST || "1",
    ZUVIX_LIVE_TEST_QUIET: args.quietOverride ?? baseEnv.ZUVIX_LIVE_TEST_QUIET ?? "1",
    ...(args.forceCodexHarness ? { ZUVIX_LIVE_CODEX_HARNESS: "1" } : {}),
  };
}
...
export function resolveTestLiveHeartbeatMs(baseEnv = process.env) {
  const value = baseEnv.ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS;
  if (value === undefined || value === "") {
    return 20_000;
  }
  const text = value.trim();
  if (!/^\d+$/u.test(text)) {
    throw new Error(`invalid ZUVIX_LIVE_WRAPPER_HEARTBEAT_MS: ${text}`);
  }
