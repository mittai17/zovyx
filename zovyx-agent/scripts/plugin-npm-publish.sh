...
if [[ "${ZUVIX_NPM_PUBLISH_PROVENANCE:-1}" != "0" && "${ZUVIX_NPM_PUBLISH_PROVENANCE:-1}" != "false" ]]; then
...
if [[ "${ZUVIX_NPM_PUBLISH_AUTH_MODE:-}" == "trusted-publisher" ]]; then
  publish_auth_token=""
...
if [[ "${mode}" == "--pack-dry-run" ]]; then
  ZUVIX_PLUGIN_NPM_BUNDLE_DEPENDENCIES=1 \
