...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("ZUVIX_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("ZUVIX_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.ZUVIX_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.ZUVIX_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.ZUVIX_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
function zuvixCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function sourceZuvixCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    ZUVIX_LOG_DIR: path.join(runRoot, "logs"),
    ZUVIX_QA_SUITE_PROGRESS: process.env.ZUVIX_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...zuvixCommand(params.repoRoot, [command, "--help"]),
    ...
    for (const { phase, args, alias } of commands) {
      ...
      params.rows.push(
        await runMeasuredCommand({
          ...
          ...zuvixCommand(params.repoRoot, ["plugins", ...args]),
          label: `${plugin.id}-${phase}`,
...
async function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceZuvixCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-gauntlet-"));
