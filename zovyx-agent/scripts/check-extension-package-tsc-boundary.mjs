...
export function resolveCompileConcurrency(
  ...
  const raw = env.ZUVIX_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "ZUVIX_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.ZUVIX_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
