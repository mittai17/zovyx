...
const DEFAULT_REPO = "zuvix/zuvix";
...
const WINDOWS_NODE_REPO = "zuvix/zuvix-windows-node";
const WINDOWS_NODE_REQUIRED_ASSETS = [
  "ZuvixCompanion-Setup-x64.exe",
  "ZuvixCompanion-Setup-arm64.exe",
];
...
Zuvix Release Publish command only after everything is green.
...
Options:
  ...
  --npm-preflight-run <id>            Reuse successful Zuvix NPM Release preflight run.
...
export function parseArgs(argv) {
  ...
  if (options.pluginPublishScope === "selected") {
    throw new Error(
      "--plugin-publish-scope selected is only for plugin-only repair publishes; release candidates publish Zuvix with --plugin-publish-scope all-publishable",
    );
  }
...
function githubApiTimeoutMs() {
  const raw = process.env.ZUVIX_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_GITHUB_API_TIMEOUT_MS;
  }
  const value = Number(raw);
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error("ZUVIX_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive number");
  }
...
async function runTelegramIfNeeded(options, artifactName) {
  ...
  const dispatchedRunId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
    package_spec: `zuvix@${options.tag.replace(/^v/u, "")}`,
...
async function main() {
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    const workflowFile = "zuvix-npm-release.yml";
    const before = await beforeRunIds(options.repo, workflowFile);
    const dispatchedRunId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
      tag: options.tag,
      preflight_only: "true",
    ...
    options.npmPreflightRunId =
      dispatchedRunId ||
      (await findNewRunId(options.repo, workflowFile, "Zuvix NPM Release", before));
  }
  ...
  const npmRun = await waitForSuccessfulRun(options.repo, options.npmPreflightRunId, {
    workflowName: "Zuvix NPM Release",
    workflowRef: options.workflowRef,
  });
  ...
  const npmArtifactName = await downloadResolvedArtifact(
    options.repo,
    options.npmPreflightRunId,
    `zuvix-npm-preflight-${options.tag}`,
    "zuvix-npm-preflight-",
    npmDir,
  );
