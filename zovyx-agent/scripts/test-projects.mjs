...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.ZUVIX_VITEST_MAX_WORKERS || env.ZUVIX_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      ZUVIX_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
function printNoChangedTestTargets(args, cwd, baseEnv) {
  ...
  console.error("[test] run `ZUVIX_TEST_CHANGED_BROAD=1 pnpm test:changed` for broad coverage.");
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.ZUVIX_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.ZUVIX_TEST_PROJECTS_PARALLEL &&
        !baseEnv.ZUVIX_VITEST_MAX_WORKERS &&
        !baseEnv.ZUVIX_TEST_WORKERS &&
