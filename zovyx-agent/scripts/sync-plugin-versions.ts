// Sync Plugin Versions script supports Zuvix repository automation.
...
const ZUVIX_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncZuvixDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.zuvix;
  if (!current || current === "workspace:*" || !ZUVIX_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.zuvix?.compat;
  const current = compat?.pluginApi;
  if (!current || !ZUVIX_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildZuvixVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.zuvix?.build;
  const current = build?.zuvixVersion;
  if (!current) {
    return false;
  }
  ...
  build.zuvixVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncZuvixDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncZuvixDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildZuvixVersionChanged = syncBuildZuvixVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildZuvixVersionChanged;
    if (!packageChanged) {
