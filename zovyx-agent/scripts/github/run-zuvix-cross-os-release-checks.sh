...
tsx_version="${ZUVIX_RELEASE_TSX_VERSION:-${TSX_VERSION:-4.21.0}}"
script_path="${ZUVIX_RELEASE_CHECKS_SCRIPT:-workflow/scripts/zuvix-cross-os-release-checks.ts}"
...
temp_root="${ZUVIX_RELEASE_TSX_TOOL_ROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}}"
if command -v cygpath >/dev/null 2>&1; then
  temp_root="$(cygpath -u "${temp_root}")"
fi

tool_dir="${ZUVIX_RELEASE_TSX_TOOL_DIR:-${temp_root}/zuvix-release-tsx-${tsx_version}}"
