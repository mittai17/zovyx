...
REMOTE_URL="${ZUVIX_REF_REMOTE:-https://github.com/zuvix/zuvix.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-zuvix-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Zuvix branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve Zuvix ref: ${REF}" >&2
exit 1
