// Tool Search Gateway E2E script supports Zuvix repository automation.
...
export function readToolSearchGatewayFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "ZUVIX_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv(
      "ZUVIX_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS",
      180_000,
      env,
    ),
  };
...
export function snapshotToolSearchGatewayEnv(
  ...
  return {
    configPath: env.ZUVIX_CONFIG_PATH,
    stateDir: env.ZUVIX_STATE_DIR,
    testFast: env.ZUVIX_TEST_FAST,
  };
}
...
function restoreEnvValue(
  ...
  key: keyof Pick<
    NodeJS.ProcessEnv,
    "ZUVIX_CONFIG_PATH" | "ZUVIX_STATE_DIR" | "ZUVIX_TEST_FAST"
  >,
...
export function restoreToolSearchGatewayEnv(
  ...
  restoreEnvValue(env, "ZUVIX_CONFIG_PATH", snapshot.configPath);
  restoreEnvValue(env, "ZUVIX_STATE_DIR", snapshot.stateDir);
  restoreEnvValue(env, "ZUVIX_TEST_FAST", snapshot.testFast);
}
...
async function writeFakePlugin(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        name: "@zuvix/tool-search-e2e-fixture",
        version: "0.0.0",
        type: "module",
        zuvix: {
          extensions: ["./index.js"],
...
export async function main() {
  ...
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-tool-search-"));
