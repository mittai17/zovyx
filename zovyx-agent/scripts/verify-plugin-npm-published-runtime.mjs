...
export function collectPluginNpmPublishedRuntimeErrors(params) {
  ...
  const runtimeExtensionsResult = readPackageStringList(
    packageLabel,
    "zuvix.runtimeExtensions",
    packageJson.zuvix?.runtimeExtensions,
  );
  const setupEntryResult = readOptionalPackageString(
    packageLabel,
    "zuvix.setupEntry",
    packageJson.zuvix?.setupEntry,
  );
  const runtimeSetupEntryResult = readOptionalPackageString(
    packageLabel,
    "zuvix.runtimeSetupEntry",
    packageJson.zuvix?.runtimeSetupEntry,
  );
  errors.push(
  ...
  if (runtimeExtensions.length > 0 && runtimeExtensions.length !== extensions.length) {
    errors.push(
      `${packageLabel} package.json zuvix.runtimeExtensions length (${runtimeExtensions.length}) must match zuvix.extensions length (${extensions.length})`,
    );
    return errors;
  }
  ...
  if (runtimeSetupEntry && !setupEntry) {
    errors.push(
      `${packageLabel} package.json zuvix.runtimeSetupEntry requires zuvix.setupEntry`,
    );
    return errors;
  }
...
export function readPluginNpmCommandOptions(env = process.env) {
  return {
    ...
    maxBuffer: readPositiveIntEnv(
      "ZUVIX_PLUGIN_NPM_COMMAND_MAX_BUFFER_BYTES",
      DEFAULT_NPM_COMMAND_MAX_BUFFER_BYTES,
      env,
    ),
    stdio: ["ignore", "pipe", "pipe"],
    timeout: readPositiveIntEnv(
      "ZUVIX_PLUGIN_NPM_COMMAND_TIMEOUT_MS",
...
async function packPublishedPackage(spec, destinationDir) {
  const attempts = readPositiveIntEnv("ZUVIX_PLUGIN_NPM_VERIFY_ATTEMPTS", 90);
  const delayMs = readPositiveIntEnv("ZUVIX_PLUGIN_NPM_VERIFY_DELAY_MS", 10000);
  let lastError;
...
async function verifyPublishedPackageReadme(spec) {
  const attempts = readPositiveIntEnv("ZUVIX_PLUGIN_NPM_README_VERIFY_ATTEMPTS", 6);
  const delayMs = readPositiveIntEnv("ZUVIX_PLUGIN_NPM_README_VERIFY_DELAY_MS", 10000);
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
...
export async function verifyPublishedPluginRuntime(spec) {
  const workingDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-npm-runtime."));
  try {
