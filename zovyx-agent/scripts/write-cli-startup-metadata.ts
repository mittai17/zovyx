// Write Cli Startup Metadata script supports Zuvix repository automation.
...
import type { ZuvixConfig } from "../src/config/config.js";
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.zuvix?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.zuvix?.channel?.order;
      const labelRaw = parsed.zuvix?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".zuvix-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "zuvix-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "zuvix-build",
...
async function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "zuvix", "browser", "--help"]);`,
...
async function renderSourceCommandHelpText(
  ...
  return await spawnText(["zuvix.mjs", command, "--help"], {
    cwd: rootDir,
    env: {
      ...renderContext.env,
      ZUVIX_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
