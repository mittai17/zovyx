...
#
# Run the Zuvix precise OpenGrep rulepack against the local working tree
...
resolve_changed_diff_ref() {
  local diff_ref="${ZUVIX_OPENGREP_BASE_REF:-origin/main...HEAD}"
  local base_ref
  local head_ref
  local resolved_base
  ...
  if [[ "${ZUVIX_OPENGREP_MERGE_HEAD_FIRST_PARENT:-0}" != "1" ]]; then
    printf '%s\n' "$diff_ref"
    return 0
  fi
