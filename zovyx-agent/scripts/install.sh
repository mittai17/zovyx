...
# Zuvix Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Zuvix."
...
resolve_zuvix_effective_home() {
    local zuvix_home="${ZUVIX_HOME:-}"
    if [[ -z "$zuvix_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$zuvix_home" == "~" ]]; then
...
GUM_VERSION="${ZUVIX_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Zuvix Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://zuvix.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$ZUVIX_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.zuvix.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_zuvix_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.zuvix-* "$npm_root"/zuvix 2>/dev/null || true
}

extract_zuvix_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_zuvix_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/zuvix" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/zuvix"|"/usr/local/bin/zuvix")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/zuvix/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale zuvix symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing zuvix binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Zuvix package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing Zuvix package"
    "${cmd[@]}" >"$log" 2>&1
}
...
install_zuvix_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Zuvix npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*zuvix" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_zuvix_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_zuvix_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Zuvix npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an zuvix binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${ZUVIX_TAGLINE_INDEX:-}" ]]; then
        if [[ "${ZUVIX_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((ZUVIX_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${ZUVIX_NO_ONBOARD:-0}
NO_PROMPT=${ZUVIX_NO_PROMPT:-0}
DRY_RUN=${ZUVIX_DRY_RUN:-0}
INSTALL_METHOD=${ZUVIX_INSTALL_METHOD:-}
ZUVIX_VERSION=${ZUVIX_VERSION:-latest}
USE_BETA=${ZUVIX_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_zuvix_effective_home)/zuvix"
GIT_DIR=${ZUVIX_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${ZUVIX_GIT_UPDATE:-1}
NPM_LOGLEVEL="${ZUVIX_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${ZUVIX_VERBOSE:-0}"
VERIFY_INSTALL="${ZUVIX_VERIFY_INSTALL:-0}"
ZUVIX_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Zuvix installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/zuvix)
...
Environment variables:
  ZUVIX_INSTALL_METHOD=git|npm
  ZUVIX_VERSION=latest|next|<semver>|<spec>
  ZUVIX_BETA=0|1
  ZUVIX_GIT_DIR=...
  ZUVIX_GIT_UPDATE=0|1
  ZUVIX_NO_PROMPT=1
  ZUVIX_VERIFY_INSTALL=1
  ZUVIX_DRY_RUN=1
  ZUVIX_NO_ONBOARD=1
  ZUVIX_VERBOSE=1
  ZUVIX_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Zuvix checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Zuvix source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_zuvix_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"zuvix"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://zuvix.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Zuvix updates; use npm i -g zuvix@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_zuvix_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/zuvix" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/zuvix" ]]; then
        ln -sf "$npm_root/zuvix/dist/entry.js" "${npm_bin}/zuvix"
        ui_info "Created zuvix bin link at ${npm_bin}/zuvix"
    fi
    return 0
}

# Check for existing Zuvix installation
check_existing_zuvix() {
    if [[ -n "$(type -P zuvix 2>/dev/null || true)" ]]; then
        ui_info "Existing Zuvix installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_zuvix_ref() {
    local requested="${ZUVIX_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "zuvix" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_zuvix_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${ZUVIX_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_zuvix_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/zuvix"
        ...
        local version=""
        version="$(zuvix_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_zuvix_npm_root_candidates)
}

warn_duplicate_zuvix_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_zuvix_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple Zuvix global installs detected"
    echo "  Different Node/npm environments can run different Zuvix versions."

    local active_node active_npm active_zuvix
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_zuvix="${ZUVIX_BIN:-}"
    if [[ -z "$active_zuvix" ]]; then
        active_zuvix="$(type -P zuvix 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active zuvix: ${INFO}${active_zuvix:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make zuvix show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

zuvix_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "zuvix"
        return 0
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${ZUVIX_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_zuvix_not_found() {
    ui_warn "Installed, but zuvix is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t zuvix 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named zuvix; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_zuvix_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P zuvix 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/zuvix" ]]; then
        echo "${npm_bin}/zuvix"
        return 0
    fi
...
install_zuvix_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/zuvix/zuvix.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Zuvix from git checkout: ${repo_dir}"
    else
        ui_info "Installing Zuvix from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        ...
        run_quiet_step "Cloning Zuvix" git clone "$repo_url" "$repo_dir"
    fi

    local git_ref
    git_ref="$(resolve_git_zuvix_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_zuvix_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building Zuvix" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/zuvix" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/zuvix"
    ui_success "Zuvix wrapper installed to \$HOME/.local/bin/zuvix"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view zuvix dist-tags.beta 2>/dev/null || true)"
...
is_zuvix_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#zuvix@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:zuvix/zuvix($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/zuvix/zuvix(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]zuvix/zuvix(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/zuvix/zuvix(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:zuvix/zuvix(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
install_zuvix() {
    local package_name="zuvix"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            ZUVIX_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="zuvix"
        else
            ZUVIX_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${ZUVIX_VERSION}" ]]; then
        ZUVIX_VERSION="latest"
    fi

    if is_zuvix_source_package_install_spec "${ZUVIX_VERSION}"; then
        ui_error "npm installs do not support Zuvix GitHub source targets like '${ZUVIX_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${ZUVIX_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${ZUVIX_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Zuvix v${resolved_version}"
    else
        ui_info "Installing Zuvix (${ZUVIX_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${ZUVIX_VERSION}")"

    if ! install_zuvix_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_zuvix_paths
        install_zuvix_npm "${install_spec}"
    fi

    if [[ "${ZUVIX_VERSION}" == "latest" && "${package_name}" == "zuvix" ]]; then
        if ! resolve_zuvix_bin &> /dev/null; then
            ui_warn "npm install zuvix@latest failed; retrying zuvix@next"
            cleanup_npm_zuvix_paths
            install_zuvix_npm "zuvix@next"
        fi
    fi

    ensure_zuvix_bin_link || true

    ui_success "Zuvix installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${ZUVIX_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_zuvix_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (zuvix not on PATH yet)"
        warn_zuvix_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${ZUVIX_PROFILE:-default}"
    local effective_home
    effective_home="$(resolve_zuvix_effective_home)"
    if [[ "${profile}" != "default" ]]; then
        echo "${effective_home}/.zuvix/workspace-${profile}"
    else
        echo "${effective_home}/.zuvix/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local effective_home
    effective_home="$(resolve_zuvix_effective_home)"
    local config_path="${ZUVIX_CONFIG_PATH:-$effective_home/.zuvix/zuvix.json}"
    local legacy_config_path="${HOME}/.zuvix/zuvix.json"
    local legacy_clawdbot_path="${HOME}/.clawdbot/clawdbot.json"
    if [[ -f "${config_path}" || -f "$effective_home/.clawdbot/clawdbot.json" ]]; then
        return
    fi
    if [[ -z "${ZUVIX_CONFIG_PATH:-}" && "${effective_home}" != "${HOME}" ]]; then
    ...
    if ! is_promptable; then
        local user_claw
        user_claw="$(zuvix_command_for_user "${ZUVIX_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but zuvix not on PATH; skipping onboarding"
        warn_zuvix_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(zuvix_command_for_user "$claw")"
...
resolve_zuvix_version() {
    local version=""
    local raw_version_output=""
    local claw="${ZUVIX_BIN:-}"
    if [[ -z "$claw" ]] && command -v zuvix &> /dev/null; then
        claw="$(command -v zuvix)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_zuvix_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/zuvix/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/zuvix/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "zuvix daemon status --json" "$claw" daemon status --json || true)"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: zuvix not on PATH yet"
        warn_zuvix_not_found
        return 1
    fi

    run_quiet_step "Checking Zuvix version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: zuvix gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_zuvix_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Zuvix checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set ZUVIX_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_zuvix_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/zuvix" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/zuvix"
    ...
    ZUVIX_BIN="$(resolve_zuvix_bin || true)"
    warn_duplicate_zuvix_global_installs || true
    ...
    installed_version=$(resolve_zuvix_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Zuvix installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Zuvix installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/zuvix"
        ui_kv "Update command" "zuvix update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash -s -- --install-method npm"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running zuvix doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                ZUVIX_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                ZUVIX_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                ZUVIX_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if is_promptable; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (zuvix not on PATH yet)"
                    warn_zuvix_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v zuvix &> /dev/null; then
        local claw="${ZUVIX_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_zuvix_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (zuvix daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if ZUVIX_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: zuvix daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${ZUVIX_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main
