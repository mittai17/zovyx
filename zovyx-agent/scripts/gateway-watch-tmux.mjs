...
const TMUX_CWD_ENV_KEY = "ZUVIX_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@zuvix.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "ZUVIX_CONFIG_PATH",
  "ZUVIX_DIAGNOSTICS",
  "ZUVIX_DIAGNOSTICS_EVENT_LOOP",
  "ZUVIX_DIAGNOSTICS_TIMELINE_PATH",
  "ZUVIX_GATEWAY_PORT",
  "ZUVIX_GATEWAY_RESTART_TRACE",
  "ZUVIX_GATEWAY_STARTUP_TRACE",
  "ZUVIX_HOME",
  "ZUVIX_PROFILE",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.ZUVIX_TRACE_SYNC_IO ??= "0";
    if (nextEnv.ZUVIX_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  ...
  const port = env.ZUVIX_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "zuvix",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `ZUVIX_GATEWAY_WATCH_TMUX_CHILD=1`,
    `ZUVIX_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.ZUVIX_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.ZUVIX_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.ZUVIX_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
