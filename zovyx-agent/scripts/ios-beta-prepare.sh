...
Optional custom relay:
  ZUVIX_PUSH_RELAY_BASE_URL=https://relay.example.com \
    scripts/ios-beta-prepare.sh --build-number 7 [--team-id TEAMID]
...
- regenerates apps/ios/Zuvix.xcodeproj via xcodegen
EOF
}
...
DEFAULT_IOS_PUSH_RELAY_BASE_URL="https://ios-push-relay.zuvix.ai"
PUSH_RELAY_BASE_URL="${ZUVIX_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-${DEFAULT_IOS_PUSH_RELAY_BASE_URL}}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ZUVIX_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ZUVIX_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ZUVIX_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ZUVIX_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ZUVIX_URL_SLASH)$(ZUVIX_URL_SLASH)#g'
)"
...
ZUVIX_CODE_SIGN_STYLE = Automatic
ZUVIX_DEVELOPMENT_TEAM = ${TEAM_ID}
ZUVIX_IOS_SELECTED_TEAM = ${TEAM_ID}
ZUVIX_APP_BUNDLE_ID = ai.zuvix.client
ZUVIX_SHARE_BUNDLE_ID = ai.zuvix.client.share
ZUVIX_ACTIVITY_WIDGET_BUNDLE_ID = ai.zuvix.client.activitywidget
ZUVIX_WATCH_APP_BUNDLE_ID = ai.zuvix.client.watchkitapp
ZUVIX_WATCH_EXTENSION_BUNDLE_ID = ai.zuvix.client.watchkitapp.extension
ZUVIX_APP_PROFILE =
ZUVIX_SHARE_PROFILE =
ZUVIX_PUSH_TRANSPORT = relay
ZUVIX_PUSH_DISTRIBUTION = official
ZUVIX_URL_SLASH = /
ZUVIX_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ZUVIX_PUSH_APNS_ENVIRONMENT = production
EOF
