...
function resolveCliShardSpec(args, env) {
  ...
  return env.ZUVIX_ADDITIONAL_BOUNDARY_SHARD ?? "";
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const concurrencyRaw =
    process.env.ZUVIX_ADDITIONAL_BOUNDARY_CONCURRENCY ??
    process.env.ZUVIX_EXTENSION_BOUNDARY_CONCURRENCY;
  const concurrencyLabel =
    process.env.ZUVIX_ADDITIONAL_BOUNDARY_CONCURRENCY === undefined
      ? "ZUVIX_EXTENSION_BOUNDARY_CONCURRENCY"
      : "ZUVIX_ADDITIONAL_BOUNDARY_CONCURRENCY";
  const concurrency = resolveConcurrency(concurrencyRaw, 4, concurrencyLabel);
  const checkTimeoutMs = resolvePositiveInteger(
    process.env.ZUVIX_ADDITIONAL_BOUNDARY_TIMEOUT_MS,
    DEFAULT_CHECK_TIMEOUT_MS,
    "ZUVIX_ADDITIONAL_BOUNDARY_TIMEOUT_MS",
  );
  const outputMaxBytes = resolvePositiveInteger(
    process.env.ZUVIX_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES,
    DEFAULT_OUTPUT_MAX_BYTES,
    "ZUVIX_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES",
  );
