...
const ignoreRepoBinary = process.env.ZUVIX_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.ZUVIX_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function enforceBrokeredAws(commandArgs, providerName) {
  ...
  console.error(
    [
      "[crabbox] provider=aws requires a configured Crabbox broker for Zuvix proof.",
      "[crabbox] run `crabbox login --url https://crabbox.zuvix.ai --provider aws`, then retry.",
      "[crabbox] for intentional direct AWS provider debugging, set ZUVIX_CRABBOX_ALLOW_DIRECT_AWS=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function remoteGitBootstrapForChangedGate(changedGateBase) {
  ...
  return [
    ...
    "rm -rf .git;",
    "git init -q;",
    "git remote add origin https://github.com/zuvix/zuvix.git 2>/dev/null || git remote set-url origin https://github.com/zuvix/zuvix.git;",
    `git fetch -q --depth=1 origin ${quotedBase}:refs/remotes/origin/main;`,
    "git reset --mixed --quiet refs/remotes/origin/main;",
    "git add -A;",
    "if ! git diff --cached --quiet; then git -c user.name=Zuvix -c user.email=ci@zuvix.local commit -q --no-gpg-sign -m remote-changed-gate-tree; fi;",
    "fi",
  ].join(" ");
}
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$zuvixModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($zuvixModulesDir) {",
    'if (-not (Test-Path $zuvixModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $zuvixModulesDir" }',
    '$zuvixWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$zuvixSelfModules = Join-Path $zuvixModulesDir "node_modules"',
    'if (-not (Test-Path $zuvixSelfModules)) { cmd /c mklink /J "$zuvixSelfModules" "$zuvixModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $zuvixWorkspaceModules)) { cmd /c mklink /J "$zuvixWorkspaceModules" "$zuvixModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remoteAwsMacosJsBootstrap({ packageManager = false } = {}) {
  const nodeVersion = process.env.ZUVIX_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "zuvix_crabbox_bootstrap_macos_js() {",
    'tool_root="${ZUVIX_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.zuvix-crabbox-toolchain}";',
    ...
    'macos_locale="${ZUVIX_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.zuvix-crabbox-node-ready";',
    ...
    "zuvix_env_args=();",
    "zuvix_env_ignore=0;",
    "zuvix_env_path_seen=0;",
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) zuvix_env_ignore=1; zuvix_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${zuvix_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) zuvix_env_ignore=1; zuvix_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) zuvix_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then zuvix_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) zuvix_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$zuvix_env_ignore" = "1" ]; then zuvix_env_args+=("PATH=$PATH:${1#PATH=}"); else zuvix_env_args+=("$1"); fi; zuvix_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) zuvix_env_args+=("$1"); shift ;;',
    '--) zuvix_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$zuvix_env_ignore" = "1" ] && [ "$zuvix_env_path_seen" = "0" ]; then zuvix_env_args+=("PATH=$PATH"); fi;',
    'command env "${zuvix_env_args[@]}" "$@";',
    "};",
  ];
  if (packageManager) {
    bootstrap.push(
  ...
  bootstrap.push("};", "zuvix_crabbox_bootstrap_macos_js");
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    shellCommand: `zuvix_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "zuvix-crabbox-macos-script-"));
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/zuvix-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_zuvix_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_zuvix_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `ZUVIX_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "zuvix", "crabbox-sync");
  }
  return resolve(tmpdir(), "zuvix-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.ZUVIX_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "ZUVIX_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set ZUVIX_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower ZUVIX_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "zuvix-crabbox-sync-"));
  let active = false;
...
function fullCheckoutKeepaliveIntervalMs() {
  return parseNonNegativeIntegerEnv(
    "ZUVIX_CRABBOX_SYNC_KEEPALIVE_MS",
    5000,
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        ...
        "[crabbox] if using ../crabbox, rebuild it: version=$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../crabbox -trimpath -ldflags \"-s -w -X github.com/zuvix/crabbox/internal/cli.version=${version}\" -o bin/crabbox ./cmd/crabbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    ...
    console.error(
      `[crabbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for Zuvix CI-like proof; not switching providers automatically`,
    );
  }
}
...
if (
  ...
  console.error(
    "[crabbox] provider=docker enabling host Docker socket pass-through for Zuvix Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/zuvix-crabbox-docker-work";
  console.error(
