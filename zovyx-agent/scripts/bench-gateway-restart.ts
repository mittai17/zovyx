// Bench Gateway Restart script supports Zuvix repository automation.
...
function printUsage(): void {
  console.log(`Zuvix Gateway restart benchmark
...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "zuvix.json");
...
function sanitizedEnv(
  ...
  return {
    ...
    LOGNAME: process.env.LOGNAME ?? "zuvix-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "zuvix-bench",
    npm_config_update_notifier: "false",
    ZUVIX_CONFIG: configPath,
    ...
    ZUVIX_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "zuvix-gateway-restart-bench-"));
