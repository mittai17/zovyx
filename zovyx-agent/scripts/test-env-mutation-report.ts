#!/usr/bin/env node
// Test Env Mutation Report script supports Zuvix repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/zuvix-test-state.ts", "canonical Zuvix test state helper"],
  ...
  ["test/setup-zuvix-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "Zuvix test env mutation report",
