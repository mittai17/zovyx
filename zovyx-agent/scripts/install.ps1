# Zuvix Installer for Windows
# Usage: powershell -c "irm https://zuvix.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://zuvix.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
function Complete-Install {
    ...
    throw "Zuvix installation failed with exit code $($script:InstallExitCode)."
}

Write-Host ""
Write-Host "  Zuvix Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:ZUVIX_INSTALL_METHOD)) {
        $InstallMethod = $env:ZUVIX_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:ZUVIX_GIT_DIR)) {
        $GitDir = $env:ZUVIX_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:ZUVIX_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:ZUVIX_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:ZUVIX_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "zuvix")
}
...
function Get-ZuvixDepsRoot {
    ...
    return (Join-Path $localAppData "Zuvix\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-ZuvixDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or zuvix is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingZuvix {
    if (Get-ZuvixCommandPath) {
        Write-Host "[*] Existing Zuvix installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-ZuvixDepsRoot) "portable-git")
}
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("zuvix-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install Zuvix." -ForegroundColor Red
...
function Get-ZuvixCommandPath {
    $zuvixCmd = Get-Command zuvix.cmd -ErrorAction SilentlyContinue
    if ($zuvixCmd -and $zuvixCmd.Source) {
        return $zuvixCmd.Source
    }

    $zuvix = Get-Command zuvix -ErrorAction SilentlyContinue
    if ($zuvix -and $zuvix.Source) {
        return $zuvix.Source
    }
...
function Invoke-ZuvixCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-ZuvixCommandPath
    if (-not $commandPath) {
        throw "zuvix command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveZuvixCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "zuvix $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-ZuvixOnPath {
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "zuvix.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] zuvix is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmZuvixInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-ZuvixSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("zuvix@")) {
        $normalizedTag = $normalizedTag.Substring("zuvix@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:zuvix/zuvix($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/zuvix/zuvix(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]zuvix/zuvix(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/zuvix/zuvix(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:zuvix/zuvix(\.git)?($|[?#])'
    )
}
...
function Install-Zuvix {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    if (Test-ZuvixSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support Zuvix GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use zuvix package for beta, zuvix for stable
    $packageName = "zuvix"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "zuvix"
    }
    $installSpec = Resolve-NpmZuvixInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing Zuvix ($installSpec)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://zuvix.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] Zuvix installed" -ForegroundColor Green
    return $true
}

# Install Zuvix from GitHub
function Install-ZuvixFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $repoUrl = "https://github.com/zuvix/zuvix.git"
    Write-Host "[*] Installing Zuvix from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] Zuvix build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "zuvix.cmd"
    ...
    Write-Host "[OK] Zuvix wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-ZuvixCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-ZuvixCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    ...
    try {
        Invoke-ZuvixCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-ZuvixCommand gateway restart | Out-Null
        Invoke-ZuvixCommand gateway status --json | Out-Null
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:ZUVIX_GIT_DIR)) {
        return $env:ZUVIX_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "zuvix")
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "zuvix") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-ZuvixFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "zuvix.cmd"
        ...
        $npmInstallResults = @(Install-Zuvix)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-ZuvixOnPath)) {
        Write-Host "Install completed, but Zuvix is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: zuvix doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-ZuvixCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.zuvix -and $npmList.dependencies.zuvix.version) {
                $installedVersion = $npmList.dependencies.zuvix.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "Zuvix installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "Zuvix installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\zuvix.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "zuvix doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "zuvix onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveZuvixCommand onboard
        }
    }
