#!/usr/bin/env node
// Builds the Zuvix package artifact used by Docker E2E.
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building Zuvix package artifacts",
    command: "node",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      env: {
        ...
        ZUVIX_RUN_NODE_SKIP_DTS_BUILD: "1",
      },
      timeoutMs: resolveTimeoutMs(
        "ZUVIX_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestZuvixTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Zuvix tarball in ${outputDir}`);
  }
  return path.join(outputDir, packed);
}

export async function packZuvixPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing Zuvix package");
  ...
  try {
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "ZUVIX_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  return await newestZuvixTarball(outputDir, packOutput);
}
...
async function main() {
  ...
  console.error("==> Writing Zuvix package inventory");
  await run(
    "node",
    [
      "--import",
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "ZUVIX_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  let tarball = await packZuvixPackageForDocker(sourceDir, outputDir);
  ...
  console.error("==> Checking Zuvix package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-zuvix-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "ZUVIX_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
