...
// Release Check script supports Zuvix repository automation.
...
} from "./zuvix-npm-postpublish-verify.ts";
...
export const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "zuvix/plugin-sdk/core",
  "zuvix/plugin-sdk/provider-entry",
  "zuvix/plugin-sdk/runtime",
] as const;
export const CRITICAL_PLUGIN_SDK_IMPORT_SMOKE_SPECIFIERS = ["zuvix/plugin-sdk/core"] as const;
...
export function runReleaseCheckCommand(
  ...
  const output = execFileSync(invocation.command, invocation.args, {
    ...
    maxBuffer:
      options.maxBuffer ??
      positiveEnvInt(
        "ZUVIX_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES",
    ...
    timeout:
      options.timeoutMs ??
      positiveEnvInt(
        "ZUVIX_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
...
export function collectPackedInstalledPackageVerificationErrors(params: {
  ...
  if (
    ...
    errors.push(
      `installed zuvix binary version mismatch: expected ${params.expectedVersion}, found ${params.installedBinaryVersion || "<missing>"}.`,
    );
  }
...
export function createPackedPluginSdkTypescriptSmokeProject(params: {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: "zuvix-plugin-sdk-type-smoke",
        private: true,
        type: "module",
        dependencies: {
          zuvix: params.packageSpec,
        },
      },
      null,
      2,
...
function runPackedPluginSdkTypescriptSmoke(tarballPath: string, tmpRoot: string): void {
  ...
  const installedZuvixRoot = join(consumerDir, "node_modules", "zuvix");
  const tscPath = [
    join(consumerDir, "node_modules", "typescript", "bin", "tsc"),
    join(installedZuvixRoot, "node_modules", "typescript", "bin", "tsc"),
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".zuvix", "zuvix.json");
  mkdirSync(join(homeDir, ".zuvix"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-zuvix-release-check",
  });
  ...
  runReleaseCheckCommand(
    {
      command: process.execPath,
      args: [join(packageRoot, "zuvix.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    },
    {
      cwd: packageRoot,
      stdio: "inherit",
  ...
  runReleaseCheckCommand(
    { command: process.execPath, args: [join(packageRoot, "zuvix.mjs"), "plugins", "doctor"] },
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "zuvix-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "zuvix");
    ...
    runReleaseCheckCommand(
      {
        command: process.execPath,
        args: [join(packageRoot, "zuvix.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      },
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.zuvix-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.zuvix-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from zuvix/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("zuvix/plugin-sdk/".length);
