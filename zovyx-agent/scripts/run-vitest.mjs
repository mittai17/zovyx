...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.ZUVIX_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestCliEntry({
  ...
  try {
    ...
    if (isMissingVitestResolveError(error)) {
      ...
      wrappedError.code = "ZUVIX_MISSING_VITEST";
      throw wrappedError;
    }
    throw error;
  }
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.ZUVIX_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.ZUVIX_VITEST_MAX_WORKERS ?? env.ZUVIX_TEST_WORKERS);
}
...
function main(argv = process.argv.slice(2), env = process.env) {
  ...
  try {
    ...
    if (error instanceof Error && error.code === "ZUVIX_MISSING_VITEST") {
