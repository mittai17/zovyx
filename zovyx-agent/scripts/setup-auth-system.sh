#!/bin/bash
# Setup Zuvix Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'zuvix-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Zuvix message: Send warning via Zuvix itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/zuvix-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/zuvix-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/zuvix-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now zuvix-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status zuvix-auth-monitor.timer"
echo ""
echo "Quick commands:"
