...
DOCKER_BUILD_EXTENSIONS="${ZUVIX_DOCKER_BUILD_EXTENSIONS:-${ZUVIX_EXTENSIONS:-}}"
LIVE_IMAGE_PULL_ATTEMPTS="${ZUVIX_LIVE_DOCKER_PULL_ATTEMPTS:-3}"
LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS="${ZUVIX_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"

if ! [[ "$LIVE_IMAGE_PULL_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
  echo "ZUVIX_LIVE_DOCKER_PULL_ATTEMPTS must be a positive integer, got: $LIVE_IMAGE_PULL_ATTEMPTS" >&2
  exit 2
fi
if ! [[ "$LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" =~ ^[0-9]+$ ]]; then
  echo "ZUVIX_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS must be a non-negative integer, got: $LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" >&2
  exit 2
fi
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "ZUVIX_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi
...
if [[ "${ZUVIX_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  if ! docker_build_on_missing_enabled; then
    ...
    echo "Build it first or unset ZUVIX_SKIP_DOCKER_BUILD." >&2
    exit 1
  fi
  echo "==> Live-test image not available; building because ZUVIX_DOCKER_BUILD_ON_MISSING/ZUVIX_TESTBOX allows fallback."
fi
