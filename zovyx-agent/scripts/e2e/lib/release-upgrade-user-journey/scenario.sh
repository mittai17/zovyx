...
export OPENAI_API_KEY="sk-zuvix-release-upgrade-user-journey"
export CLICKCLACK_BOT_TOKEN="clickclack-release-upgrade-token"

PORT="18789"
MOCK_PORT="44210"
CLICKCLACK_PORT="44211"
SUCCESS_MARKER="ZUVIX_E2E_OK_RELEASE_UPGRADE"
...
BASELINE_SPEC="${ZUVIX_RELEASE_UPGRADE_BASELINE_SPEC:-zuvix@latest}"
export SUCCESS_MARKER MOCK_REQUEST_LOG CLICKCLACK_STATE

candidate_version="$(
  tar -xOf "${ZUVIX_CURRENT_PACKAGE_TGZ:?missing ZUVIX_CURRENT_PACKAGE_TGZ}" package/package.json |
...
cleanup() {
  ...
  zuvix_e2e_stop_process "${clickclack_pid:-}"
...
start_gateway() {
  local log_path="$1"
  gateway_pid="$(zuvix_e2e_start_gateway "$entry" "$PORT" "$log_path")"
  zuvix_e2e_wait_gateway_ready "$gateway_pid" "$log_path" 300 "$PORT"
}

echo "Installing published baseline $BASELINE_SPEC..."
if ! zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v zuvix >/dev/null
baseline_root="$(zuvix_e2e_package_root)"
baseline_entry="$(zuvix_e2e_package_entrypoint "$baseline_root")"
zuvix_e2e_enable_zuvix_cli_timeout

mock_pid="$(zuvix_e2e_start_mock_openai "$MOCK_PORT" "$OPENAI_LOG")"
...
for _ in $(seq 1 100); do
  if zuvix_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200 >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
zuvix_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200
...
zuvix plugins install "$plugin_dir" >"$PLUGIN_INSTALL_LOG" 2>&1
zuvix release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
zuvix_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate Zuvix package"
...
zuvix release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1
...
zuvix channels status --json >"$STATUS_JSON" 2>"$STATUS_ERR"
