    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/zuvix-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.ZUVIX_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
export function resolveUpgradeSurvivorZuvixCommand(argv, params = {}) {
  const platform = params.platform ?? process.platform;
  if (platform === "win32") {
    const comSpec = params.comSpec ?? process.env.ComSpec ?? "cmd.exe";
    return {
      command: comSpec,
      args: ["/d", "/s", "/c", buildCmdExeCommandLine("zuvix.cmd", argv)],
      commandLabel: ["zuvix", ...argv].join(" "),
      shell: false,
...
export function runUpgradeSurvivorZuvixStep(step, params = {}) {
  const invocation = resolveUpgradeSurvivorZuvixCommand(step.argv);
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runUpgradeSurvivorZuvixStep(adaptedStep);
