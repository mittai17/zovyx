        ...
        resolvedSkills: [
          {
            ...
            filePath: "/tmp/zuvix-old-package/skills/legacy-heavy-skill-cache/SKILL.md",
          },
        ],
      },
    },
...
function getScenario() {
  const scenario = process.env.ZUVIX_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("ZUVIX_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.ZUVIX_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  const stateDir = requireEnv("ZUVIX_STATE_DIR");
  const workspace = requireEnv("ZUVIX_TEST_WORKSPACE_DIR");
  ...
  writeJson(path.join(workspace, ".zuvix", "workspace-state.json"), {
    version: 1,
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".zuvix-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
  ...
  if (scenario === "versioned-runtime-deps") {
    const version = process.env.ZUVIX_UPGRADE_SURVIVOR_BASELINE_VERSION || "2026.4.24";
    for (const plugin of ["discord", "feishu", "telegram", "whatsapp"]) {
      writeJson(
        path.join(
          runtimeRoot,
          `zuvix-${version}-${plugin}`,
          ".zuvix-runtime-deps-stamp.json",
        ),
        {
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/zuvix-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const workspace = requireEnv("ZUVIX_TEST_WORKSPACE_DIR");
  const scenario = getScenario();
  const stage = process.env.ZUVIX_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-zuvix");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`zuvix-${version}-`))
      : [];
    assert(
...
function readMigratedSessionStore(stateDir, targetStorePath) {
  ...
  const dbPath = path.join(stateDir, "agents", "main", "agent", "zuvix-agent.sqlite");
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  const extensionsRoot = path.join(requireEnv("ZUVIX_STATE_DIR"), "extensions");
  assert(
...
function assertConfiguredPluginInstalls() {
  ...
  assertOptionalConfiguredPluginIndex(records, index.plugins ?? [], {
    packageName: "@zuvix/brave-plugin",
    pluginId: "brave",
  });
