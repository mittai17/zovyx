...
ARTIFACT_ROOT="$(dirname "${ZUVIX_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/zuvix-upgrade-survivor-artifacts/summary.json}")"
export ZUVIX_UPGRADE_SURVIVOR_RUNTIME_ROOT="${ZUVIX_UPGRADE_SURVIVOR_RUNTIME_ROOT:-/tmp/zuvix-upgrade-survivor-runtime}"
RUNTIME_ROOT="$ZUVIX_UPGRADE_SURVIVOR_RUNTIME_ROOT"
STATE_HOME_ROOT="${ZUVIX_UPGRADE_SURVIVOR_STATE_HOME_ROOT:-$RUNTIME_ROOT/state-home}"
mkdir -p "$ARTIFACT_ROOT"
mkdir -p "$RUNTIME_ROOT"
export TMPDIR="${ZUVIX_UPGRADE_SURVIVOR_TMPDIR:-$RUNTIME_ROOT/tmp}"
export ZUVIX_TEST_STATE_TMPDIR="${ZUVIX_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR:-$RUNTIME_ROOT/state-tmp}"
mkdir -p "$TMPDIR" "$ZUVIX_TEST_STATE_TMPDIR"
...
SUMMARY_JSON="${ZUVIX_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${ZUVIX_UPGRADE_SURVIVOR_BASELINE:?missing ZUVIX_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${ZUVIX_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${ZUVIX_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${ZUVIX_CURRENT_PACKAGE_TGZ:-}}"
...
export ZUVIX_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
...
validate_baseline_package_spec() {
  ...
  echo "ZUVIX_UPGRADE_SURVIVOR_BASELINE must be zuvix@latest, zuvix@beta, zuvix@alpha, an exact Zuvix release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "ZUVIX_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    zuvix@*)
      baseline_spec="$raw"
      baseline_version="${raw#zuvix@}"
      ;;
    *@*)
      echo "ZUVIX_UPGRADE_SURVIVOR_BASELINE must be zuvix@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="zuvix@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "ZUVIX_UPGRADE_SURVIVOR_BASELINE must be zuvix@latest, zuvix@beta, zuvix@alpha, zuvix@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "ZUVIX_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.ZUVIX_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.ZUVIX_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.ZUVIX_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      zuvix_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/zuvix\n' "$npm_config_prefix"
}

legacy_runtime_deps_symlink_plugin() {
  local plugin="${ZUVIX_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "ZUVIX_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@zuvix-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${ZUVIX_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$ZUVIX_STATE_DIR/extensions/opik-zuvix"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-zuvix",
...
JSON
  cat >"$shadow_root/zuvix.plugin.json" <<'JSON'
{
  "id": "opik-zuvix",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.zuvix-runtime-deps.json" \
      "$plugin_dir/.zuvix-runtime-deps-stamp.json" \
      "$plugin_dir/.zuvix-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.zuvix-install-stage-upgrade-survivor" \
      "$plugin_dir/.zuvix-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$ZUVIX_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$ZUVIX_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/zuvix-upgrade-survivor-dep" \
      "$plugin_dir/.zuvix-runtime-deps-copy-upgrade-survivor/node_modules/zuvix-upgrade-survivor-dep" \
      "$plugin_dir/.zuvix-install-stage-upgrade-survivor" \
      "$plugin_dir/.zuvix-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/zuvix-upgrade-survivor-dep" \
      "$ZUVIX_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/zuvix-upgrade-survivor-dep" \
      "$ZUVIX_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/zuvix-upgrade-survivor-dep"
    printf '{"name":"zuvix-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/zuvix-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.zuvix-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.zuvix-runtime-deps-stamp.json"
    printf '{"name":"zuvix-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.zuvix-runtime-deps-copy-upgrade-survivor/node_modules/zuvix-upgrade-survivor-dep/package.json"
    printf '{"name":"zuvix-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/zuvix-upgrade-survivor-dep/package.json"
    printf '{"name":"zuvix-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$ZUVIX_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/zuvix-upgrade-survivor-dep/package.json"
    printf '{"name":"zuvix-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$ZUVIX_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/zuvix-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"zuvix-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
reset_run_state() {
  rm_rf_retry "$npm_config_prefix" "$TMPDIR" "$ZUVIX_TEST_STATE_TMPDIR" "$STATE_HOME_ROOT"
  rm -f "$SYSTEMCTL_SHIM_PID_FILE" "$SYSTEMCTL_SHIM_DAEMON_LOG"
  mkdir -p "$npm_config_prefix" "$npm_config_cache" "$TMPDIR" "$ZUVIX_TEST_STATE_TMPDIR"
}

install_baseline() {
  normalize_baseline
  echo "Installing baseline package: $baseline_spec"
  if ! zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix "$npm_config_prefix" "$baseline_spec" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
    echo "baseline npm install failed" >&2
    zuvix_e2e_print_log "$BASELINE_INSTALL_LOG" >&2
    return 1
  fi
  if ! command -v zuvix >/dev/null; then
    echo "baseline install did not expose zuvix on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" zuvix --version 2>&1)"; then
    echo "baseline zuvix --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline zuvix --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  zuvix_e2e_eval_test_state_from_b64 "${ZUVIX_TEST_STATE_FUNCTION_B64:?missing ZUVIX_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.zuvix /root/workspace
    zuvix_test_state_create /root minimal
  else
    zuvix_test_state_create "$STATE_HOME_ROOT" minimal
  fi
  export ZUVIX_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" zuvix config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
    echo "generated baseline config failed baseline validation" >&2
    zuvix_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG" >&2
    return 1
  fi
}
...
log_file="${ZUVIX_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/zuvix-systemctl-shim.log}"
pid_file="${ZUVIX_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/zuvix-systemctl-shim.pid}"
daemon_log="${ZUVIX_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/zuvix-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/zuvix-gateway.service\n' "${HOME:?missing HOME}"
}
...
install_update_restart_service_unit() {
  if ! zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u ZUVIX_GATEWAY_TOKEN -u ZUVIX_GATEWAY_PASSWORD zuvix gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
    echo "baseline gateway service install failed" >&2
    zuvix_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR" >&2
    zuvix_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON" >&2
    return 1
  fi
}
...
const stateDir = process.env.ZUVIX_STATE_DIR;
if (!stateDir) {
  throw new Error("missing ZUVIX_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  ...
  local dotenv_path="$ZUVIX_STATE_DIR/.env"
...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >>"$tmp_path"
  mv "$tmp_path" "$dotenv_path"
  local systemd_env_path="$ZUVIX_STATE_DIR/gateway.systemd.env"
...
assert_baseline_state() {
  ZUVIX_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing ZUVIX_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  local update_env=(
    env
    -u ZUVIX_GATEWAY_TOKEN
    -u ZUVIX_GATEWAY_PASSWORD
    -u ZUVIX_ALLOW_ROOT
  )
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(ZUVIX_ALLOW_ROOT=1)
  fi
  if ! zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" zuvix "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
    echo "zuvix update failed" >&2
    zuvix_e2e_print_log "$UPDATE_ERR" >&2
    zuvix_e2e_print_log "$UPDATE_JSON" >&2
    return 1
  fi
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
assert_root_managed_vps_cli_usable() {
  ...
  zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" zuvix config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" zuvix plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" zuvix doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
    echo "zuvix doctor failed" >&2
    zuvix_e2e_print_log "$DOCTOR_LOG" >&2
    return 1
  fi
}

validate_post_doctor_config() {
  if ! zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" zuvix config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${ZUVIX_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$ZUVIX_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
  if [ "${ZUVIX_UPGRADE_SURVIVOR_READYZ_ALLOW_DEGRADED:-}" = "1" ]; then
...
start_gateway() {
  local port=18789
  local budget="${ZUVIX_UPGRADE_SURVIVOR_START_BUDGET_SECONDS:-90}"
  local start_epoch
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u ZUVIX_GATEWAY_TOKEN -u ZUVIX_GATEWAY_PASSWORD zuvix gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_status() {
  local port=18789
  local budget="${ZUVIX_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS:-30}"
  local status_start
  local status_end
  status_start="$(node -e "process.stdout.write(String(Date.now()))")"
  if ! zuvix_e2e_maybe_timeout "$COMMAND_TIMEOUT" zuvix gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
    echo "gateway status failed" >&2
    zuvix_e2e_print_log "$STATUS_ERR" >&2
    zuvix_e2e_print_log "$GATEWAY_LOG" >&2
    return 1
  fi
  ...
  if [ "$status_seconds" -gt "$budget" ]; then
    ...
    zuvix_e2e_print_log "$STATUS_JSON" >&2
    return 1
  fi
