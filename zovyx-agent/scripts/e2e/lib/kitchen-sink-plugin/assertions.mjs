...
function shouldScanLogFile(entry) {
  if (!(/\.(?:log|jsonl)$/u.test(entry) || /zuvix-kitchen-sink-/u.test(path.basename(entry)))) {
    return false;
  }
...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".zuvix")];
  const deny = [
  ...
  if (scannedFiles === 0) {
    throw new Error(
      "kitchen-sink log scan found no files under the isolated scratch root or Zuvix home",
    );
  }
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(zuvixPeerPath)) {
    throw new Error(`missing kitchen-sink zuvix peer symlink: ${zuvixPeerPath}`);
  }
  if (!fs.lstatSync(zuvixPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink zuvix peer is not a symlink: ${zuvixPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(zuvixPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink zuvix peer ${linkedHostRoot} to target ${hostRoot}`);
  }
