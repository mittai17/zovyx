...
const packageName = "@zuvix/kitchen-sink";
...
async function buildNpmPackArtifact(fixture) {
  const packRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "zuvix-clawhub-fixture-"));
  try {
...
`,
    ...
    packageDetail(artifact) {
      ...
      const packageDetail = {
        package: {
          name: packageName,
          displayName: "Zuvix Kitchen Sink",
          family: "code-plugin",
          runtimeId: pluginId,
          channel: "official",
          isOfficial: true,
          ...
          verification: {
            tier: "source-linked",
            sourceRepo: "https://github.com/zuvix/kitchen-sink",
            hasProvenance: false,
            scanStatus: "passed",
          },
...
async function main() {
  ...
  zip.file("package/zuvix.plugin.json", manifestJson, { date: new Date(0) });
  ...
  const artifactResolverDetail = {
    package: versionDetail.package ?? {
      name: packageName,
      displayName: packageDetail.package?.displayName ?? "Zuvix Kitchen Sink",
