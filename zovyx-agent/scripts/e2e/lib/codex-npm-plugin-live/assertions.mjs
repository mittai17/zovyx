...
const allowBetaCompatDiagnostics =
  process.env.ZUVIX_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS === "1";
const MAX_TEXT_FILE_BYTES = readPositiveIntEnv(
  "ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES",
  1024 * 1024,
);
const MAX_ERROR_TAIL_BYTES = readPositiveIntEnv(
  "ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES",
  64 * 1024,
);
const MAX_TRANSCRIPT_FILES = readPositiveIntEnv(
  "ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES",
  64,
);
const MAX_TRANSCRIPT_WALK_ENTRIES = readPositiveIntEnv(
  "ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES",
  4096,
);
const MAX_TRANSCRIPT_SCAN_BYTES = readPositiveIntEnv(
  "ZUVIX_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES",
  2 * 1024 * 1024,
);
...
function assertPlugin() {
  const spec = process.argv[3] || "npm:@zuvix/codex";
  const list = readJson("/tmp/zuvix-codex-plugins-list.json");
  const inspect = readJson("/tmp/zuvix-codex-plugin-inspect.json");
...
function codexNpmProjectRoot() {
  return npmProjectRootForInstalledPackage(codexInstallPath(), "@zuvix/codex");
}
...
function assertNpmDeps() {
  ...
  if (!fs.existsSync(pluginPackageJson)) {
    throw new Error(`missing npm-installed @zuvix/codex package.json: ${pluginPackageJson}`);
  }
  ...
  if (pluginPackage.name !== "@zuvix/codex") {
  ...
  if (!openAiCodexPackageJson) {
    throw new Error("missing @openai/codex dependency under .zuvix/npm");
  }
...
function assertPreflight() {
  ...
  const output = readTextFileBounded("/tmp/zuvix-codex-preflight.log", "Codex preflight log");
...
function assertAgentTurn() {
  ...
  const stdout = readTextFileBounded("/tmp/zuvix-codex-agent.json", "Zuvix agent JSON");
  const stderr = readTextFileTail("/tmp/zuvix-codex-agent.err", "Zuvix agent stderr");
  const response = JSON.parse(stdout);
  const text = extractAgentReplyTexts(JSON.stringify(response)).join("\n");
  if (!text.includes(marker)) {
    throw new Error(
      `Zuvix agent reply did not contain ${marker}:\nstdout=${stdout}\nstderr=${stderr}`,
    );
  }
  ...
  if (typeof entry.sessionFile !== "string" || !fs.existsSync(entry.sessionFile)) {
    throw new Error(`missing Zuvix session file: ${entry.sessionFile}`);
  }
...
function assertUninstalled() {
  ...
  const list = readJson("/tmp/zuvix-codex-plugins-list-after-uninstall.json");
  const plugin = (list.plugins || []).find((entry) => entry.id === "codex");
  if (plugin?.status === "loaded" || plugin?.enabled === true) {
...
function assertAgentError() {
  ...
  if (!Number.isInteger(status) || status === 0) {
    throw new Error(
      `expected Zuvix agent to fail after Codex uninstall, got status ${process.argv[3]}`,
    );
  }
  const stdout = fs.existsSync("/tmp/zuvix-codex-agent-after-uninstall.json")
    ? readTextFileTail(
        "/tmp/zuvix-codex-agent-after-uninstall.json",
        "post-uninstall agent stdout",
      )
    : "";
  const stderr = fs.existsSync("/tmp/zuvix-codex-agent-after-uninstall.err")
    ? readTextFileTail(
