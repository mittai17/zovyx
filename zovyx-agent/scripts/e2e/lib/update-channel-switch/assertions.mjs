...
function assertConfigChannel(channel) {
  const config = readJson(path.join(process.env.HOME, ".zuvix", "zuvix.json"));
  if (config.update?.channel === channel) {
    return;
  }
  if (process.env.ZUVIX_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1") {
    console.log(
...
switch (command) {
  case "prepare-git-fixture":
    prepareGitFixture(args[0] ?? "/tmp/zuvix-git");
    break;
  case "write-control-ui":
    writeControlUi(args[0] ?? "/tmp/zuvix-git");
    break;
  case "assert-update":
    assertUpdate(args[0]);
    break;
