ZUVIX_PLUGINS_FIXTURE_PID_FILES=()
ZUVIX_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=0
ZUVIX_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=""

zuvix_plugins_cleanup_fixture_servers() {
  local pid_file
  local pid
  for pid_file in "${ZUVIX_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    [[ -f "$pid_file" ]] || continue
...
zuvix_plugins_print_fixture_log() {
  local log_file="$1"
...
zuvix_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  ZUVIX_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  zuvix_plugins_install_fixture_cleanup_trap
}

zuvix_plugins_install_fixture_cleanup_trap() {
  if [[ "${ZUVIX_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED:-0}" = "1" ]]; then
    return
  fi

  local existing_trap
  existing_trap="$(trap -p EXIT || true)"
  if [[ -n "$existing_trap" && "$existing_trap" != *zuvix_plugins_fixture_exit_trap* ]]; then
    local existing_action="${existing_trap#trap -- }"
    existing_action="${existing_action% EXIT}"
    eval "ZUVIX_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=$existing_action"
  fi

  ZUVIX_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=1
  trap zuvix_plugins_fixture_exit_trap EXIT
}

zuvix_plugins_fixture_exit_trap() {
  local status="$?"
  zuvix_plugins_cleanup_fixture_servers
  if [[ -n "${ZUVIX_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION:-}" ]]; then
    eval "$ZUVIX_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION"
  fi
  exit "$status"
}
...
packageJson.zuvix.extensions = ["./index.js", " "];
