...
const scratchRoot = process.env.ZUVIX_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("ZUVIX_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  const configPath = zuvixConfigPath();
  const config = readZuvixConfig();
  const allowLegacyCompat = process.env.ZUVIX_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function zuvixConfigPath() {
  return path.join(process.env.HOME, ".zuvix", "zuvix.json");
}

function readZuvixConfig() {
  const configPath = zuvixConfigPath();
  return fs.existsSync(configPath) ? readRequiredZuvixConfig() : {};
}

function readRequiredZuvixConfig() {
  const configPath = zuvixConfigPath();
  try {
    return readJson(configPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`failed to read Zuvix config ${configPath}: ${message}`, { cause: error });
  }
}
...
function recordFixturePluginTrust() {
  ...
  const configPath = path.join(process.env.HOME, ".zuvix", "zuvix.json");
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".zuvix", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  const zuvixPeerPath = path.join(installPath, "node_modules", "zuvix");
  if (!fs.existsSync(zuvixPeerPath)) {
    throw new Error(`missing ClawHub zuvix peer symlink: ${zuvixPeerPath}`);
  }
  if (!fs.lstatSync(zuvixPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub zuvix peer is not a symlink: ${zuvixPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(zuvixPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub zuvix peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@zuvix/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@zuvix/demo-plugin-npm") {
...
function assertInvalidZuvixExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  for (const expected of ["zuvix.extensions[1]", "non-empty string"]) {
    assertTextFileIncludes(
      scratchFile("plugins-invalid-zuvix-extensions.log"),
      expected,
      "malformed metadata install output",
    );
  }

  const list = readJson(scratchFile("plugins-invalid-zuvix-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".zuvix", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.ZUVIX_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.ZUVIX_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = resolveHomePath(record.installPath);
  const extensionsRoot = path.join(process.env.HOME, ".zuvix", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".zuvix", "zuvix.json");
...
const commands = {
  ...
  "invalid-zuvix-extensions": assertInvalidZuvixExtensionsRejected,
