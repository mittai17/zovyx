...
function zuvixPath(...parts) {
  return path.join(home, ".zuvix", ...parts);
}
...
function config() {
  return readJson(process.env.ZUVIX_CONFIG_PATH ?? zuvixPath("zuvix.json"));
}

function requiredConfig() {
  return readRequiredJson(process.env.ZUVIX_CONFIG_PATH ?? zuvixPath("zuvix.json"));
}
...
function assertNpmProjectRoot(pluginId, packageName) {
  const record = recordFor(pluginId);
  assert(record?.installPath, `install path missing for ${pluginId}`);
  const relative = path.relative(zuvixPath("npm", "projects"), record.installPath);
  assert(
  ...
  assert(
    !fs.existsSync(zuvixPath("npm", "node_modules", ...packageSegments)),
