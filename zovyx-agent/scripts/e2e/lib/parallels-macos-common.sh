...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/zuvix-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/zuvix-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${ZUVIX_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${ZUVIX_SECRET_ENV_NAME:-}" ] && [ -n "${ZUVIX_SECRET_FILE:-}" ] && [ -f "$ZUVIX_SECRET_FILE" ]; then
  secret_value="$(cat "$ZUVIX_SECRET_FILE")"
  rm -f "$ZUVIX_SECRET_FILE"
  export "${ZUVIX_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "ZUVIX_SECRET_ENV_NAME=$api_key_env" \
      "ZUVIX_SECRET_FILE=$secret_path" \
      "ZUVIX_WRAPPER_FILE=$wrapper_path" \
