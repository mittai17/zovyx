...
const inspect = readJson("/tmp/zuvix-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@zuvix/codex")) {
  throw new Error(`expected @zuvix/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @zuvix/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@zuvix/codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@zuvix/codex");
...
function readAuthProfileStoreText(agentDir) {
  const dbPath = path.join(agentDir, "zuvix-agent.sqlite");
...
assertOpenAiEnvAuthProfileStore(authRaw, {
  ...
  rawKeyNeedle: "sk-zuvix-codex-on-demand-e2e",
});
