...
const agentTurnTimeoutSeconds = readPositiveIntEnv(
  "ZUVIX_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS",
  300,
);
...
const AGENT_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "ZUVIX_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
const SESSION_FILE_LIST_LIMIT = 20;
const SESSION_SCAN_MAX_ENTRIES = readPositiveIntEnv(
  "ZUVIX_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES",
  50_000,
);
...
function stateDir() {
  return process.env.ZUVIX_STATE_DIR || path.join(process.env.HOME, ".zuvix");
}

function configPath() {
  return process.env.ZUVIX_CONFIG_PATH || path.join(stateDir(), "zuvix.json");
}

function agentOutputPath() {
  return process.env.ZUVIX_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH || "/tmp/zuvix-agent.json";
}

function agentErrorPath() {
  return process.env.ZUVIX_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH || "/tmp/zuvix-agent.err";
}
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/zuvix-plugin-inspect.json")
    ? readJson("/tmp/zuvix-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "zuvix.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const list = readJson("/tmp/zuvix-plugins-list.json");
  ...
  const inspect = readJson("/tmp/zuvix-plugin-inspect.json");
