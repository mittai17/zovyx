...
function resolveStateDir() {
  if (process.env.ZUVIX_STATE_DIR) {
    return process.env.ZUVIX_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".zuvix");
}
...
function resolveZuvixEntry() {
  if (process.env.ZUVIX_ENTRY) {
    return process.env.ZUVIX_ENTRY;
  }
  ...
  throw new Error("Missing ZUVIX_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveZuvixEntry();
  const timeoutMs = readPositiveIntEnv(
    "ZUVIX_BUNDLED_PLUGIN_LIST_TIMEOUT_MS",
  ...
  const result = spawnSync(process.execPath, [entry, "plugins", "list", "--json"], {
    ...
    maxBuffer: readPositiveIntEnv(
      "ZUVIX_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES",
...
async function loadManifestEntries() {
  const explicit = (process.env.ZUVIX_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `ZUVIX_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("ZUVIX_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("ZUVIX_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `ZUVIX_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.ZUVIX_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
