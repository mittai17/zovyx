...
if [ -f dist/index.mjs ]; then
  ZUVIX_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  ZUVIX_ENTRY="dist/index.js"
else
...
sweep_command_timeout="${ZUVIX_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT:-300s}"
...
run_logged_sweep_command() {
  ...
  local log_file="$2"
  shift 2
  if zuvix_e2e_maybe_timeout "$sweep_command_timeout" "$@" >"$log_file" 2>&1; then
    return 0
  else
    local status=$?
...
lifecycle_trace_enabled() {
  case "${ZUVIX_PLUGIN_LIFECYCLE_TRACE:-}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
    *)
...
echo "Selected ${#plugin_entries[@]} bundled plugins for shard ${ZUVIX_BUNDLED_PLUGIN_SWEEP_INDEX:-0}/${ZUVIX_BUNDLED_PLUGIN_SWEEP_TOTAL:-1}: ${selected_labels[*]}"

plugin_index=0
for plugin_entry in "${plugin_entries[@]}"; do
  IFS=$'\t' read -r plugin_id plugin_dir requires_config plugin_root <<<"$plugin_entry"
  install_log="/tmp/zuvix-install-${plugin_index}.log"
  uninstall_log="/tmp/zuvix-uninstall-${plugin_index}.log"
  ...
  run_logged_sweep_command "install $plugin_id" "$install_log" \
    node "$ZUVIX_ENTRY" plugins install "$plugin_id"
  ...
  if [[ "${ZUVIX_BUNDLED_PLUGIN_RUNTIME_SMOKE:-1}" != "0" ]]; then
    ...
    if [[ "$plugin_id" == "${ZUVIX_BUNDLED_PLUGIN_TTS_LIVE_PROVIDER:-openai}" ]]; then
  ...
  run_logged_sweep_command "uninstall $plugin_id" "$uninstall_log" \
    node "$ZUVIX_ENTRY" plugins uninstall "$plugin_id" --force
  if lifecycle_trace_enabled; then
