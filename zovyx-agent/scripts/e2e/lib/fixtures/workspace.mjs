...
const AGENTS_DELETE_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "ZUVIX_FIXTURE_AGENTS_DELETE_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.ZUVIX_WORKSPACE_DIR || path.join(process.env.HOME, ".zuvix", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.ZUVIX_STATE_DIR, "ZUVIX_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.ZUVIX_GATEWAY_TOKEN?.trim();
  fs.mkdirSync(sharedWorkspace, { recursive: true });
  writeJson(path.join(stateDir, "zuvix.json"), {
    agents: {
      list: [
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.ZUVIX_STATE_DIR, "zuvix.json"))?.agents?.list ?? [];
