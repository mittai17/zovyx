...
export OPENAI_API_KEY="sk-zuvix-release-media-memory"
export ZUVIX_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="ZUVIX_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/zuvix-release-media-memory.XXXXXX)"
...
entry="$(zuvix_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'HOME=%s\n' "$HOME"
  printf 'ZUVIX_HOME=%s\n' "$ZUVIX_HOME"
  printf 'ZUVIX_STATE_DIR=%s\n' "$ZUVIX_STATE_DIR"
  printf 'ZUVIX_CONFIG_PATH=%s\n' "$ZUVIX_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(zuvix_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$ZUVIX_CONFIG_PATH" "$CONFIG_JSON"
zuvix plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$ZUVIX_STATE_DIR/workspace/memory"
...
cat >"$ZUVIX_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

zuvix memory index --force >"$INDEX_LOG" 2>&1
zuvix memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
zuvix memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
