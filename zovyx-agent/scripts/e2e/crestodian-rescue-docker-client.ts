...
import type { ZuvixConfig } from "../../dist/config/types.zuvix.js";
...
function makeParams(commandBody: string, cfg: ZuvixConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: ZuvixConfig, isGroup = false): Promise<string> {
...
async function main() {
  const tempState = await createE2eStateDir("zuvix-crestodian-");
  tempState.registerExitCleanup();
  const stateDir = tempState.stateDir;
  const configPath = process.env.ZUVIX_CONFIG_PATH ?? path.join(stateDir, "zuvix.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env ZUVIX_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/zuvix-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/zuvix-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as ZuvixConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "ZUVIX_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/zuvix-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/zuvix-work",
    ),
