...
IMAGE_NAME="$(docker_e2e_resolve_image "zuvix-onboard-e2e" ZUVIX_ONBOARD_E2E_IMAGE)"
ZUVIX_TEST_STATE_FUNCTION_B64="$(docker_e2e_test_state_function_b64)"
MAX_MEMORY_MIB="${ZUVIX_ONBOARD_MAX_MEMORY_MIB:-2048}"
MAX_CPU_PERCENT="${ZUVIX_ONBOARD_MAX_CPU_PERCENT:-1200}"
DOCKER_RUN_TIMEOUT="${ZUVIX_ONBOARD_DOCKER_RUN_TIMEOUT:-1200s}"
COMMAND_TIMEOUT="${ZUVIX_ONBOARD_COMMAND_TIMEOUT:-${ZUVIX_E2E_COMMAND_TIMEOUT:-300s}}"
CONTAINER_NAME="zuvix-onboard-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/zuvix-onboard.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/zuvix-onboard-stats.XXXXXX")"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --name "$CONTAINER_NAME" "${DOCKER_E2E_HARNESS_ARGS[@]}" -t \
  ...
  -e "ZUVIX_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT" \
