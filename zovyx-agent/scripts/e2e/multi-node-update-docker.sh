...
# Zuvix under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `zuvix update`. Verifies that:
#
...
#
# Requires: Docker, a built zuvix-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="zuvix-multi-node-update-e2e"
DOCKER_RUN_TIMEOUT="${ZUVIX_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
docker_e2e_build_or_reuse "$IMAGE_NAME" multi-node-update "$ROOT_DIR/scripts/e2e/Dockerfile" "$ROOT_DIR" "bare" "${ZUVIX_SKIP_DOCKER_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${ZUVIX_CURRENT_PACKAGE_TGZ:-}")"
...
echo ""
echo "── Step 2: Install Zuvix under node-A ──"

# Use node-A to install zuvix with npm prefix A.
...
echo "Installing Zuvix package under node-A prefix: $NPM_PREFIX_A"
zuvix_e2e_install_package "$ARTIFACTS/install-a.log" "Zuvix package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking zuvix location..."

ZUVIX_A="$(command -v zuvix)"
echo "zuvix binary: $ZUVIX_A"
echo "zuvix version: $(zuvix --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/zuvix"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/zuvix-gateway.service"
...
if ! zuvix gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
# but zuvix is NOT installed there.
...
echo "which zuvix: $(command -v zuvix)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run zuvix update (this is the bug) ──"
...
echo "Running zuvix update --yes --json..."
UPDATE_EXIT=0
zuvix update --yes --json \
  --tag /tmp/zuvix-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: zuvix update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but Zuvix package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the Zuvix package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/zuvix/package.json" ]; then
  echo "WARNING: Zuvix was ALSO installed under node-B prefix (split install)"
else
  echo "OK: Zuvix is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell Zuvix root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
