#!/usr/bin/env bash
# Installs Zuvix from a prepared package tarball, installs @zuvix/codex
...
CANDIDATE_ROOT="${ZUVIX_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "zuvix-codex-npm-plugin-live-e2e" ZUVIX_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${ZUVIX_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${ZUVIX_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${ZUVIX_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${ZUVIX_CODEX_NPM_PLUGIN_PROFILE_FILE:-${ZUVIX_TESTBOX_PROFILE_FILE:-$HOME/.zuvix-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${ZUVIX_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${ZUVIX_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "ZUVIX_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires ZUVIX_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/zuvix-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/zuvix-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      docker_e2e_print_log /tmp/zuvix-codex-plugin-pack.log >&2
      exit 1
    fi
...
ZUVIX_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e ZUVIX_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${ZUVIX_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e ZUVIX_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${ZUVIX_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e ZUVIX_CODEX_NPM_PLUGIN_MODEL="${ZUVIX_CODEX_NPM_PLUGIN_MODEL:-codex/gpt-5.4}" \
  -e ZUVIX_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${ZUVIX_CODEX_NPM_PLUGIN_SPEC:?missing ZUVIX_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${ZUVIX_CODEX_NPM_PLUGIN_MODEL:?missing ZUVIX_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="codex/${MODEL_REF#*/}"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="ZUVIX-CODEX-NPM-PLUGIN-LIVE-OK"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${ZUVIX_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  zuvix_e2e_dump_logs \
    ...
    /tmp/zuvix-codex-plugin-install.log \
    /tmp/zuvix-codex-plugin-enable.log \
    /tmp/zuvix-codex-plugins-list.json \
    /tmp/zuvix-codex-plugin-inspect.json \
    /tmp/zuvix-codex-preflight.log \
    /tmp/zuvix-codex-agent.json \
    /tmp/zuvix-codex-agent.err \
    /tmp/zuvix-codex-agent-turn1.json \
    /tmp/zuvix-codex-agent-turn1.err \
    /tmp/zuvix-codex-agent-turn2.json \
    /tmp/zuvix-codex-agent-turn2.err \
    /tmp/zuvix-codex-plugin-uninstall.log \
    /tmp/zuvix-codex-plugins-list-after-uninstall.json \
    /tmp/zuvix-codex-agent-after-uninstall.json \
    /tmp/zuvix-codex-agent-after-uninstall.err
}
...
zuvix plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/zuvix-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
zuvix plugins enable codex >/tmp/zuvix-codex-plugin-enable.log 2>&1

zuvix plugins list --json >/tmp/zuvix-codex-plugins-list.json
zuvix plugins inspect codex --runtime --json >/tmp/zuvix-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/zuvix-codex-preflight.log 2>&1 </dev/null
...
echo "Running Zuvix local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the Zuvix Codex plugin." \
  /tmp/zuvix-codex-agent-turn1.json \
  /tmp/zuvix-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
zuvix plugins uninstall codex --force >/tmp/zuvix-codex-plugin-uninstall.log 2>&1
zuvix plugins list --json >/tmp/zuvix-codex-plugins-list-after-uninstall.json
node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs assert-uninstalled

if zuvix agent --local \
  --agent main \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/zuvix-codex-agent-after-uninstall.json 2>/tmp/zuvix-codex-agent-after-uninstall.err; then
  echo "Expected Zuvix agent to fail after Codex uninstall, got status 0" >&2
  exit 1
fi
if ! grep -Fq 'Requested agent harness "codex" is not registered' /tmp/zuvix-codex-agent-after-uninstall.err &&
  ! grep -Fq 'Unknown model: codex/' /tmp/zuvix-codex-agent-after-uninstall.err; then
  echo "Unexpected post-uninstall agent error:" >&2
  tail -n 120 /tmp/zuvix-codex-agent-after-uninstall.err >&2 || true
  exit 1
fi
