// Cron Mcp Cleanup Docker Client script supports Zuvix repository automation.
...
export function readCronMcpCleanupProbePidWaitMs(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveIntEnv("ZUVIX_CRON_MCP_CLEANUP_PID_WAIT_MS", 120_000, env);
}
...
async function waitForProbeExit(params: {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (!args || !args.includes("zuvix-cron-mcp-cleanup-probe")) {
      return;
    }
    await delay(100);
  }
  ...
  while (Date.now() - startedAt < params.timeoutMs) {
    ...
    if (observed.length > 0) {
      ...
      for (const pid of observed) {
        const args = await describeProbePid(pid);
        if (args?.includes("zuvix-cron-mcp-cleanup-probe")) {
          allExited = false;
          break;
        }
      }
...
async function runCronCleanupScenario(params: {
  ...
  assert(
    initialArgs === undefined || initialArgs.includes("zuvix-cron-mcp-cleanup-probe"),
...
async function main() {
  ...
  const stateDir = process.env.ZUVIX_STATE_DIR?.trim() || path.join(os.homedir(), ".zuvix");
