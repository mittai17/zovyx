...
IMAGE_NAME="$(docker_e2e_resolve_image "zuvix-update-channel-switch-e2e" ZUVIX_UPDATE_CHANNEL_SWITCH_E2E_IMAGE)"
SKIP_BUILD="${ZUVIX_UPDATE_CHANNEL_SWITCH_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz update-channel-switch "${ZUVIX_CURRENT_PACKAGE_TGZ:-}")"
# Bare lanes mount the package artifact instead of baking app sources into the image.
docker_e2e_package_mount_args "$PACKAGE_TGZ"
ZUVIX_TEST_STATE_SCRIPT_B64="$(
  node "$ROOT_DIR/scripts/lib/zuvix-test-state.mjs" shell \
...
(
  cd "$git_root"
  if ! zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit >/tmp/zuvix-git-install.log 2>&1; then
    zuvix_e2e_print_log /tmp/zuvix-git-install.log >&2
    exit 1
  fi
)
node scripts/e2e/lib/update-channel-switch/assertions.mjs write-control-ui "$git_root"

git config --global user.email "docker-e2e@zuvix.local"
git config --global user.name "Zuvix Docker E2E"
...
package_install_log="/tmp/zuvix-update-channel-switch-package-install.log"
if ! zuvix_e2e_maybe_timeout "${ZUVIX_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path" >"$package_install_log" 2>&1; then
  zuvix_e2e_print_log "$package_install_log" >&2
  exit 1
fi
package_version="$(node -p "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/npm-prefix/lib/node_modules/zuvix/package.json\", \"utf8\")).version")"
...
export ZUVIX_GIT_DIR="$git_root"
export ZUVIX_UPDATE_DEV_TARGET_REF="$fixture_sha"

echo "==> package -> git dev channel"
set +e
dev_json="$(zuvix update --channel dev --yes --json --no-restart)"
dev_status=$?
set -e
printf "%s\n" "$dev_json"
...
status_json="$(zuvix update status --json)"
printf "%s\n" "$status_json"
...
set +e
stable_json="$(zuvix update --channel stable --tag "$pkg_tgz_path" --yes --json --no-restart)"
stable_status=$?
set -e
printf "%s\n" "$stable_json"
