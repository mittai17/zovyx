...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${ZUVIX_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${ZUVIX_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${ZUVIX_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
COMMAND_TIMEOUT_MS="${ZUVIX_BUN_GLOBAL_SMOKE_TIMEOUT_MS:-180000}"
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "ZUVIX_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (ZUVIX_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set ZUVIX_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/zuvix-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack Zuvix tarball"
...
main() {
  ...
  echo "==> Bun global install packed Zuvix"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  zuvix_bin="$BUN_INSTALL/bin/zuvix"
  if [ ! -x "$zuvix_bin" ]; then
    zuvix_bin="$(command -v zuvix || true)"
  fi
  if [ -z "$zuvix_bin" ] || [ ! -x "$zuvix_bin" ]; then
    echo "Bun global install did not create an executable zuvix binary" >&2
    exit 1
  fi

  echo "==> Zuvix version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$zuvix_bin" --version

  echo "==> Zuvix image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$zuvix_bin" infer image providers --json)"
  ZUVIX_IMAGE_PROVIDERS_JSON="$providers_json" node scripts/e2e/lib/bun-global-install/assertions.mjs assert-image-providers
}
