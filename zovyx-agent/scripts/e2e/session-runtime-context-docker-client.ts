...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".zuvix", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "zuvix.runtime-context",
  ...
  assert(
    !userText.includes("ZUVIX_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-session-runtime-context-"));
  process.env.HOME = root;
  process.env.ZUVIX_STATE_DIR = path.join(root, ".zuvix");
  process.env.ZUVIX_CONFIG_PATH = path.join(process.env.ZUVIX_STATE_DIR, "zuvix.json");
  try {
    ...
    if (process.env.ZUVIX_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
