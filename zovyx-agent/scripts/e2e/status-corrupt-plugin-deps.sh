...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/zuvix-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.ZUVIX_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".zuvix-rename-tmp");
...
run_zuvix() {
  HOME="$HOME_DIR" \
  ZUVIX_HOME="$STATE_DIR" \
  ZUVIX_STATE_DIR="$STATE_DIR" \
  ZUVIX_CONFIG_PATH="$CONFIG_PATH" \
  ZUVIX_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_zuvix status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run zuvix doctor --fix" "$BEFORE" >/dev/null

run_zuvix doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_zuvix status --all --timeout 1 > "$AFTER"
