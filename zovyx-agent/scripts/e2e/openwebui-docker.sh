#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized Zuvix Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "zuvix-openwebui-e2e" ZUVIX_OPENWEBUI_E2E_IMAGE)"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
MAX_MEMORY_MIB="${ZUVIX_OPENWEBUI_MAX_MEMORY_MIB:-8192}"
MAX_CPU_PERCENT="${ZUVIX_OPENWEBUI_MAX_CPU_PERCENT:-1600}"
...
MODEL="${ZUVIX_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${ZUVIX_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${ZUVIX_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${ZUVIX_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${ZUVIX_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${ZUVIX_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="zuvix-openwebui-e2e-$$"
GW_NAME="zuvix-openwebui-gateway-$$"
OW_NAME="zuvix-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${ZUVIX_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
PROBE_FETCH_TIMEOUT_MS="${ZUVIX_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS * 1000 + 60000))}"
DOCKER_COMMAND_TIMEOUT="${ZUVIX_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS + 90))s}"
DOCKER_PULL_TIMEOUT="${ZUVIX_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROFILE_FILE="${ZUVIX_TESTBOX_PROFILE_FILE:-$HOME/.zuvix-testbox-live.profile}"
...
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/zuvix-openwebui-stats.XXXXXX")"
PROBE_LOG="$(mktemp "${TMPDIR:-/tmp}/zuvix-openwebui-probe.XXXXXX")"
STATS_STOP_FILE="$(mktemp "${TMPDIR:-/tmp}/zuvix-openwebui-stats-stop.XXXXXX")"
STATS_PIDS=()
...
docker_e2e_docker_cmd run -d \
  ...
  -e "ZUVIX_OPENWEBUI_MODEL=$MODEL" \
  -e "ZUVIX_SKIP_CHANNELS=1" \
  -e "ZUVIX_SKIP_GMAIL_WATCHER=1" \
  -e "ZUVIX_SKIP_CRON=1" \
  -e "ZUVIX_SKIP_CANVAS_HOST=1" \
  -e "ZUVIX_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=$PROVIDER_TIMEOUT_SECONDS" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/zuvix-openwebui-config.XXXXXX.json)"
    ZUVIX_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    zuvix_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null
start_openwebui_stats_sampler

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "ZUVIX_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Zuvix E2E" \
  -e WEBUI_SECRET_KEY="zuvix-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Zuvix E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="zuvix/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "ZUVIX_HTTP_PROBE_BEARER='$TOKEN' ZUVIX_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> Zuvix smoke..."
set +e
