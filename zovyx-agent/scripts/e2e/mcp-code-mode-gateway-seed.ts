// Mcp Code Mode Gateway Seed script supports Zuvix repository automation.
import fs from "node:fs/promises";
import { createRequire } from "node:module";
import os from "node:os";
import path from "node:path";
import { applyDockerOpenAiProviderConfig, type ZuvixConfig } from "./docker-openai-seed.ts";
...
async function main() {
  ...
  const configPath =
    process.env.ZUVIX_CONFIG_PATH?.trim() || path.join(stateDir, "zuvix.json");
  const workspaceDir = path.join(stateDir, "workspace");
  const serverPath = path.join(stateDir, "mcp-code-mode-fixture", "fixture-server.mjs");
  const apiKey =
    process.env.OPENAI_API_KEY?.trim() ||
    process.env.ZUVIX_MCP_CODE_MODE_OPENAI_API_KEY?.trim() ||
