...
const PLUGIN_SPEC =
  process.env.ZUVIX_KITCHEN_SINK_NPM_SPEC || "npm:@zuvix/kitchen-sink@latest";
const PLUGIN_ID = process.env.ZUVIX_KITCHEN_SINK_PLUGIN_ID || "zuvix-kitchen-sink-fixture";
...
Runs the external Kitchen Sink plugin RPC walk against a built Zuvix entry.

Environment:
  ZUVIX_ENTRY                         Built Zuvix entrypoint. Defaults to dist/index.mjs or dist/index.js.
  ZUVIX_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@zuvix/kitchen-sink@latest.
  ZUVIX_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: zuvix-kitchen-sink-fixture.
  ZUVIX_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  ZUVIX_KITCHEN_SINK_RPC_COMMAND_MS   Zuvix command timeout.
  ZUVIX_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  ZUVIX_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  ZUVIX_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  ZUVIX_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  ZUVIX_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      ZUVIX_KITCHEN_SINK_PERSONALITY:
        process.env.ZUVIX_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runZuvix(runner, args, env, options = {}) {
  const config = resolveKitchenSinkRpcConfig(env);
  const command = await resolveZuvixCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveZuvixCommand(runner, args, env, options = {}) {
  if (runner.pnpm) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.ZUVIX_CONFIG_PATH),
        configPath: options.env.ZUVIX_CONFIG_PATH,
...
export function usesBuiltZuvixEntry(runner, cwd = process.cwd(), env = process.env) {
  if (runner?.pnpm || !runner?.baseArgs?.[0]) {
    return false;
  }
  const entry = runner.baseArgs[0];
  if (env.ZUVIX_ENTRY && entry === env.ZUVIX_ENTRY) {
    return true;
  }
...
function configureKitchenSink(env, port) {
  const configPath = env.ZUVIX_CONFIG_PATH;
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.ZUVIX_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function samplePosixProcessTree(pid, run, commandLineNeedles) {
  ...
  try {
    ...
    const gatewayTitleMatches = descendants.filter((row) =>
      row.command.toLowerCase().includes("zuvix-gateway"),
    );
...
export async function main() {
  const config = resolveKitchenSinkRpcConfig();
  let runner = resolveZuvixRunner();
  const port = readPositiveInt(
    process.env.ZUVIX_KITCHEN_SINK_RPC_PORT,
    DEFAULT_PORT,
    "ZUVIX_KITCHEN_SINK_RPC_PORT",
  );
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.ZUVIX_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  let child;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runZuvix(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,
    ...
    assertKitchenSinkUiDescriptors(uiDescriptors, {
      expectDescriptor: env.ZUVIX_KITCHEN_SINK_PERSONALITY !== "conformance",
    });
