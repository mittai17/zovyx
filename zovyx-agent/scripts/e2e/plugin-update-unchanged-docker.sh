#!/usr/bin/env bash
# Verifies `zuvix plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "zuvix-plugin-update-e2e" ZUVIX_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${ZUVIX_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${ZUVIX_CURRENT_PACKAGE_TGZ:-}")"
...
ZUVIX_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
