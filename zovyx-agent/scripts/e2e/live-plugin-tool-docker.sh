...
IMAGE_NAME="$(docker_e2e_resolve_image "zuvix-live-plugin-tool-e2e" ZUVIX_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${ZUVIX_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${ZUVIX_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${ZUVIX_CURRENT_PACKAGE_TGZ:-}"
AGENT_TURN_TIMEOUT_SECONDS="${ZUVIX_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}"
AGENT_OUTPUT_MAX_BYTES="${ZUVIX_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES:-1048576}"
PROFILE_FILE="${ZUVIX_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${ZUVIX_TESTBOX_PROFILE_FILE:-$HOME/.zuvix-testbox-live.profile}}"
run_log=""
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${ZUVIX_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "ZUVIX_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires ZUVIX_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
ZUVIX_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e ZUVIX_LIVE_PLUGIN_TOOL_MODEL="${ZUVIX_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  -e "ZUVIX_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES=$AGENT_OUTPUT_MAX_BYTES" \
  -e "ZUVIX_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${ZUVIX_LIVE_PLUGIN_TOOL_MODEL:?missing ZUVIX_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@zuvix/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="Zuvix E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  local status="$1"
  local agent_output_dump_bytes="${ZUVIX_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES:-16384}"
  echo "Live plugin tool scenario failed with exit code $status" >&2
  if [ -f /tmp/zuvix-agent.json ]; then
    echo "--- /tmp/zuvix-agent.json (last ${agent_output_dump_bytes} bytes) ---" >&2
    tail -c "$agent_output_dump_bytes" /tmp/zuvix-agent.json >&2 || true
    echo >&2
  fi
  zuvix_e2e_dump_logs \
    /tmp/zuvix-install.log \
    /tmp/zuvix-plugin-install.log \
    /tmp/zuvix-plugin-enable.log \
    /tmp/zuvix-plugins-list.json \
    /tmp/zuvix-plugin-inspect.json \
    /tmp/zuvix-agent.err
}
...
fixture_dir="$(mktemp -d /tmp/zuvix-live-plugin-tool.XXXXXX)"
...
zuvix plugins install "npm-pack:$plugin_tgz" --force >/tmp/zuvix-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
zuvix plugins enable "$PLUGIN_ID" >/tmp/zuvix-plugin-enable.log 2>&1
zuvix plugins list --json >/tmp/zuvix-plugins-list.json
zuvix plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/zuvix-plugin-inspect.json
...
zuvix agent --local \
  ...
  --thinking off \
  --timeout "${ZUVIX_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/zuvix-agent.json 2>/tmp/zuvix-agent.err
