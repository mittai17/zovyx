// Mcp Code Mode Gateway Client script supports Zuvix repository automation.
...
export function readMcpCodeModeClientFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "ZUVIX_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv("ZUVIX_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", 300_000, env),
  };
}
...
async function main() {
  ...
  const model = process.env.ZUVIX_MCP_CODE_MODE_MODEL?.trim() || "zuvix/main";
  assert(gatewayUrl, "missing GW_URL");
  assert(gatewayToken, "missing GW_TOKEN");
  assert(stateDir, "missing ZUVIX_STATE_DIR");
