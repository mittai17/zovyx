// Smoke Common helper supports Zuvix script workflows.
import { readFile, rm } from "node:fs/promises";
import path from "node:path";
import { extractLastZuvixVersionFromLog } from "./filesystem.ts";
...
export async function packAndServeSmokeArtifact(
  ...
  const artifact = await packZuvix({
    destination: tgzDir,
    packageSpec,
    requireControlUi,
  });
...
export async function extractLastZuvixVersion(
  runDir: string,
  phaseName: string,
  pattern: RegExp,
): Promise<string> {
  return await extractLastZuvixVersionFromLog(path.join(runDir, `${phaseName}.log`), pattern);
}
