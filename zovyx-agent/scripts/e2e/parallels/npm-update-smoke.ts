...
// Npm Update Smoke script supports Zuvix repository automation.
...
const updateTimeoutSeconds = readPositiveIntEnv("ZUVIX_PARALLELS_NPM_UPDATE_TIMEOUT_S", 1200);
const updateCleanupBackstopMs = 60_000;
const freshLaneTimeoutKillGraceMs = readPositiveIntEnv(
  "ZUVIX_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_KILL_GRACE_MS",
  2_000,
);
...
export function freshLaneTimeoutMs(platform: Platform): number {
  ...
  return readPositiveIntEnv("ZUVIX_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S", defaultSeconds) * 1000;
}
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: zuvix@latest
  --update-target <target>    Target passed to guest 'zuvix update --tag'.
...
function zuvixVersionFamily(version: string): string {
  return /^(\d{4}\.\d{1,2}\.\d{1,2})(?:[-.]|$)/u.exec(version.trim())?.[1] ?? "";
}

function parseZuvixPackageSpecVersion(spec: string): string {
  const value = spec.trim();
  if (!value) {
    return "";
  }
  return resolveZuvixRegistryVersion(value) || "";
}
...
export class NpmUpdateSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await this.makeRunTempDir("zuvix-parallels-npm-update.");
    this.tgzDir = await this.makeRunTempDir("zuvix-parallels-npm-update-tgz.");
    try {
      await this.runSteps();
    } finally {
  ...
  protected async runSteps(): Promise<void> {
    ...
    this.packageSpec = this.options.packageSpec || `zuvix@${this.latestVersion}`;
    ...
    say(`Run same-guest zuvix update to ${this.updateTargetEffective}`);
    ...
    if (this.options.platforms.has("linux")) {
      jobs.push(
        this.spawnFresh("Linux", "linux", ["--vm", this.linuxVm], {
          ZUVIX_PARALLELS_LINUX_DISABLE_BONJOUR: "1",
        }),
      );
    }
  ...
  private async prepareUpdateTarget(): Promise<void> {
    ...
    this.updateExpectedNeedle = this.isExplicitPackageTarget(this.updateTargetEffective)
      ? ""
      : resolveZuvixRegistryVersion(this.updateTargetEffective) || this.updateTargetEffective;
  ...
  private resolveRegistryPackageMetadata(target: string): {
    ...
    const spec = target.startsWith("zuvix@") ? target : `zuvix@${target}`;
  ...
  private async guestMacos(
    ...
    const scriptPath = this.writeGuestScript(
      this.macosVm,
      script,
      "zuvix-parallels-npm-update-macos",
    );
  ...
  private async guestLinux(
    ...
    const scriptPath = this.writeGuestScript(
      this.linuxVm,
      script,
      "zuvix-parallels-npm-update-linux",
    );
    try {
      const status = await this.runStreamingToJobLog(
        "prlctl",
  ...
  private preflightRegistryUpdateTarget(): void {
    ...
    const baseline = resolveZuvixRegistryVersion(this.packageSpec);
    const target = resolveZuvixRegistryVersion(this.options.updateTarget);
    if (baseline && target && baseline === target) {
      die(
        `--update-target ${this.options.updateTarget} resolves to zuvix@${target}, same as baseline ${this.packageSpec}; publish or choose a newer --update-target before running VM update coverage`,
      );
    }
  }
  ...
  private async extractLastVersion(logPath: string): Promise<string> {
    return await extractLastZuvixVersionFromLog(logPath);
  }
  ...
  private async configureTargets(): Promise<void> {
    ...
    if (this.options.betaValidation) {
      const version = resolveZuvixRegistryVersion(this.options.betaValidation);
      if (!version) {
        die(`could not resolve beta validation target: ${this.options.betaValidation}`);
      }
      this.options.updateTarget = version;
      this.options.freshTargetSpec = `zuvix@${version}`;
      say(`Beta validation target: zuvix@${version}`);
    } else if (
      this.options.updateTarget &&
      this.options.updateTarget !== "local-main" &&
      !this.isExplicitPackageTarget(this.options.updateTarget)
    ) {
      const version = resolveZuvixRegistryVersion(this.options.updateTarget);
      if (version) {
        this.options.updateTarget = version;
      }
    }
    ...
    if (this.options.freshTargetSpec) {
      const version = resolveZuvixRegistryVersion(this.options.freshTargetSpec);
      this.freshTargetSpec = version ? `zuvix@${version}` : this.options.freshTargetSpec;
    }
  }

  private assertPublishedTargetMatchesHarnessCheckout(): void {
    if (process.env.ZUVIX_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH === "1") {
      return;
    }
    const candidateVersion =
      this.targetTarballVersion ||
      (this.freshTargetSpec
        ? parseZuvixPackageSpecVersion(this.freshTargetSpec)
        : parseZuvixPackageSpecVersion(this.options.updateTarget));
    const targetFamily = zuvixVersionFamily(candidateVersion);
    if (!targetFamily) {
      return;
    }
    this.harnessTargetFamily = targetFamily;
    const checkoutFamily = zuvixVersionFamily(this.harnessCheckoutVersion);
    if (checkoutFamily === targetFamily) {
      return;
    }
    die(
      `refusing to run Parallels ${candidateVersion} target with harness checkout ${this.harnessCheckoutVersion || "unknown"}; checkout the matching release branch or set ZUVIX_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH=1 for an intentional cross-version harness run`,
    );
  }
