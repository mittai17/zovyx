// Filesystem script supports Zuvix repository automation.
...
const ZUVIX_VERSION_PATTERN = /Zuvix\s+([0-9][^\s]*)/gi;
...
export async function extractLastZuvixVersionFromLog(
  logPath: string,
  pattern = ZUVIX_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.ZUVIX_PARALLELS_ARTIFACT_ROOT || path.join(repoRoot, ".artifacts", "parallels");
