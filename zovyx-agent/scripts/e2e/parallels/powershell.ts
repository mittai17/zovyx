// Powershell script supports Zuvix repository automation.
import {
  configPathMapKey,
...
export function windowsModelProviderTimeoutScript(modelId: string): string {
  ...
  return `$providerTimeoutBatchPath = Join-Path ([System.IO.Path]::GetTempPath()) 'zuvix-provider-timeout.batch.json'
@'
${batchJson}
'@ | Set-Content -Path $providerTimeoutBatchPath -Encoding UTF8
Invoke-Zuvix config set --batch-file $providerTimeoutBatchPath --strict-json
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:ZUVIX_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.zuvix\\zuvix.json' }
$agentTurnVersionText = Invoke-Zuvix --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'Zuvix\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:ZUVIX_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:ZUVIX_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:ZUVIX_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'zuvix-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.ZUVIX_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.ZUVIX_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.ZUVIX_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@zuvix", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "zuvix" };
  } else {
...
Remove-Item Env:ZUVIX_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:ZUVIX_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:ZUVIX_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsZuvixResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-ZuvixCommand {
  if ($script:ZuvixResolvedCommand) { return $script:ZuvixResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\zuvix.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\zuvix.ps1'
  }
  foreach ($name in @('zuvix.cmd', 'zuvix.ps1', 'zuvix')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'zuvix.cmd'
    $shimCandidates += Join-Path $npmPrefix 'zuvix.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:ZuvixResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:ZuvixResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\zuvix\zuvix.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:ZuvixResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:ZuvixResolvedCommand
    }
  }
  throw 'zuvix command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-Zuvix {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $ZuvixArgs)
  $command = Resolve-ZuvixCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @ZuvixArgs
    } else {
      & $command.Path @ZuvixArgs
    }
  } finally {
