...
// Windows Smoke script supports Zuvix repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://zuvix.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-zuvix-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Zuvix\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-zuvix-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://zuvix.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run zuvix update --channel dev.
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  private gatewayRecoveryAfterMs =
    readPositiveIntEnv("ZUVIX_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S", 180) * 1000;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("zuvix-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("zuvix-parallels-windows-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("zuvix-main-fresh.tgz"));
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsZuvixResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanZuvix: boolean): void {
    const cleanScript = cleanZuvix
      ? "npm.cmd uninstall -g zuvix --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
if ($LASTEXITCODE -ne 0) { throw "zuvix --version failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-Zuvix --version");
...
Invoke-Zuvix onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "zuvix onboard failed with exit code $LASTEXITCODE" }
...
${windowsScopedEnvFunction}
$script:ZuvixUpdateExit = 0
Invoke-WithScopedEnv @{ ZUVIX_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; ZUVIX_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-Zuvix update --channel dev --yes --json
  $script:ZuvixUpdateExit = $LASTEXITCODE
}
if ($script:ZuvixUpdateExit -ne 0) { throw "zuvix update failed with exit code $script:ZuvixUpdateExit" }
Invoke-Zuvix --version
Invoke-Zuvix update status --json`,
...
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-Zuvix gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-Zuvix gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-Zuvix gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-Zuvix gateway status ${suffix}`);
  }
...
if (-not $agentOk) { throw 'zuvix agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastZuvixVersion(this.runDir, phaseName, /Zuvix\s+([0-9][^\s]*)/gi);
  }
