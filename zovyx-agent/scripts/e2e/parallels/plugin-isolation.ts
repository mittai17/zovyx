// Plugin Isolation script supports Zuvix repository automation.
...
export function posixProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  const homeEnv = options.homeFallback
    ? `ZUVIX_PARALLELS_HOME=${shellQuote(options.homeFallback)} `
    : "";
...
export function windowsProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  return `$env:ZUVIX_PARALLELS_PLUGIN_ISOLATION = @'
${payloadJson}
'@
$isolationScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'zuvix-parallels-plugin-isolation.cjs'
@'
...
Remove-Item Env:ZUVIX_PARALLELS_PLUGIN_ISOLATION -Force -ErrorAction SilentlyContinue`;
}
...
function providerOnlyPluginIsolationNodeScript(options: PluginIsolationOptions): string {
  ...
  return `process.env.ZUVIX_PARALLELS_PLUGIN_ISOLATION = ${JSON.stringify(payloadJson)};
${providerOnlyPluginIsolationNodeSource()}`;
}
...
const payload = JSON.parse(process.env.ZUVIX_PARALLELS_PLUGIN_ISOLATION || "{}");
const home =
  process.env.ZUVIX_PARALLELS_HOME ||
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@zuvix", "codex"), {
  recursive: true,
  force: true,
});
