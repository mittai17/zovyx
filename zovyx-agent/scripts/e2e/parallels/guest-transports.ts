// Guest Transports script supports Zuvix repository automation.
...
export async function runWindowsBackgroundPowerShell(
  ...
  const fileBase = `zuvix-parallels-${nonce}`;
...
${pathsScript}
function Add-ZuvixBackgroundLog {
...
try {
  & {
${options.script}
  } *>&1 | Add-ZuvixBackgroundLog
  Set-Content -Path $exitPath -Value '0' -Encoding UTF8
} catch {
  $_ | Add-ZuvixBackgroundLog
  ...
  try {
    ...
    if ($length -gt $offset) {
      ...
      if ($read -gt 0) {
        ...
        "__ZUVIX_LOG_OFFSET__:$nextOffset"
...
if (Test-Path $donePath) {
  ...
  "__ZUVIX_BACKGROUND_EXIT__:$backgroundExit"
  ...
  exit 0
      ...
      const offsetMatch = poll.stdout.match(/__ZUVIX_LOG_OFFSET__:(\d+)/);
      if (offsetMatch) {
        lastLogOffset = Number(offsetMatch[1]);
      }
      const lengthMatch = poll.stdout.match(/__ZUVIX_LOG_LENGTH__:(\d+)/);
      const logLength = lengthMatch ? Number(lengthMatch[1]) : lastLogOffset;
      if (poll.stdout.includes("__ZUVIX_BACKGROUND_DONE__")) {
        doneSeen = true;
        ...
        const exitMatch = poll.stdout.match(/__ZUVIX_BACKGROUND_EXIT__:(\S+)/);
...
if (Test-Path $pidPath) {
  ...
  if ($backgroundPid) {
    Stop-ZuvixBackgroundProcessTree ([int]$backgroundPid)
  }
}
`
    : "";
...
export class LinuxGuest {
  ...
  exec(args: string[], options: GuestExecOptions = {}): string {
    const result = run(
      "prlctl",
      ["exec", this.vmName, "/usr/bin/env", "HOME=/root", "ZUVIX_ALLOW_ROOT=1", ...args],
      {
        check: false,
        input: options.input,
        quiet: true,
  ...
  bash(script: string): string {
    const scriptPath = `/tmp/zuvix-parallels-${process.pid}-${Date.now()}.sh`;
    const write = run(
      "prlctl",
      [
        "exec",
...
export class WindowsGuest {
  ...
  powershell(script: string, options: GuestExecOptions = {}): string {
    const scriptName = `zuvix-parallels-${process.pid}-${Date.now()}.ps1`;
