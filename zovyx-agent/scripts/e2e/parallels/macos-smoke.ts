...
// Macos Smoke script supports Zuvix repository automation.
...
const guestZuvix = "zuvix";
const guestZuvixEntry = '"$(npm root -g)/zuvix/zuvix.mjs"';
const guestZuvixEntryRunner = `node ${guestZuvixEntry}`;
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://zuvix.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://zuvix.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("zuvix-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("zuvix-parallels-macos-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("zuvix-main-fresh.tgz"),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("zuvix-main-upgrade.tgz"),
      );
  ...
  private guestZuvixEntryExec(
    ...
    return this.guestSh(
      `set -e
entry="$(npm root -g)/zuvix/zuvix.mjs"
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'zuvix.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'zuvix-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'zuvix.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g zuvix >/dev/null 2>&1 || true
rm -rf "$HOME/.zuvix"
...
curl -fsSL ${shellQuote(this.options.installUrl)} -o /tmp/zuvix-install.sh
bash /tmp/zuvix-install.sh --version ${shellQuote(this.installVersion)}
${guestZuvix} --version`,
    );
  }
...
${guestZuvix} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestZuvix, "--version"]);
...
check_path "$root/zuvix"
check_path "$root/zuvix/extensions"
if [ -d "$root/zuvix/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/zuvix/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/zuvix-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/zuvix`)}
export PATH=${shellQuote(`/tmp/zuvix-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".zuvix", "zuvix.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 ZUVIX_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ZUVIX_DISABLE_BUNDLED_PLUGINS=1 ${guestZuvixEntryRunner} update --channel dev --yes --json
${guestZuvixEntryRunner} --version
${guestZuvixEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestZuvixEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'zuvix.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'zuvix-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'zuvix.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} ZUVIX_HOME=${shellQuote(home)} ZUVIX_STATE_DIR=${shellQuote(`${home}/.zuvix`)} ZUVIX_CONFIG_PATH=${shellQuote(
        `${home}/.zuvix/zuvix.json`,
      )} ${guestZuvixEntryRunner} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/zuvix-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestZuvix, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestZuvix(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestZuvix(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`zuvix ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/zuvix-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Zuvix Control</title>' /tmp/zuvix-dashboard-smoke.html >/dev/null &&
      grep -F '<zuvix-app></zuvix-app>' /tmp/zuvix-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/zuvix-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestZuvixEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestZuvixEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestZuvixEntryExec([
      "config",
      "set",
    ...
    this.guestZuvixEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestZuvixEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestZuvixEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastZuvixVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }
