// Package Artifact script supports Zuvix repository automation.
...
export function resolveZuvixRegistryVersion(specOrAlias: string): string {
  const rawValue = specOrAlias.trim();
  const value = rawValue.startsWith("zuvix@") ? rawValue.slice("zuvix@".length) : rawValue;
  if (!value) {
    return "";
  }
  if (value === "latest" || value === "beta" || /^\d/.test(value)) {
    return npmViewVersion(`zuvix@${value}`);
  }
  const betaMatch = /^beta(\d+)$/u.exec(value);
  if (betaMatch) {
    const betaSuffix = `-beta.${betaMatch[1]}`;
    const versions = JSON.parse(
      run("npm", ["view", "zuvix", "versions", "--json"], { quiet: true }).stdout,
    ) as string[];
    const match = versions
      ...
      .at(-1);
    if (!match) {
      die(`no zuvix registry version found for alias ${value}`);
    }
    return match;
  }
  return "";
...
export async function packZuvix(input: {
  destination: string;
  packageSpec?: string;
  requireControlUi?: boolean;
  ...
  return await withPackageLock(path.join(tmpdir(), "zuvix-parallels-build.lock"), async () => {
    ...
    const tgzPath = path.join(input.destination, `zuvix-main-${shortHead}.tgz`);
...
async function acquirePackageLock(lockDir: string, ownerToken: string): Promise<void> {
  const timeoutMs = readPositiveIntEnv("ZUVIX_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000);
  const staleMs = readPositiveIntEnv("ZUVIX_PARALLELS_PACKAGE_LOCK_STALE_MS", 2 * 60 * 60_000);
