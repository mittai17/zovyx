...
async function main() {
  ...
  const tempState = await createE2eStateDir("zuvix-crestodian-first-run-");
  ...
  assert(
    await shouldStartOnboardingForFreshInstall(["node", "zuvix"]),
    "fresh bare Zuvix invocation did not route to onboarding",
  );
  assert(
    shouldStartCrestodianForModernOnboard(["node", "zuvix", "onboard", "--modern"]),
    "modern onboard invocation did not route to Crestodian",
  );
  process.exitCode = undefined;
  await runCli(["node", "zuvix", "onboard", "--modern", "--non-interactive", "--json"]);
  assert(
