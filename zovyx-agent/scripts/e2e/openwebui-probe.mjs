...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.ZUVIX_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "zuvix/default") ?? modelIds.find((id) => id === "zuvix");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing zuvix model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `zuvix model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
