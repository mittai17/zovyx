...
IMAGE_NAME="$(docker_e2e_resolve_image "zuvix-mcp-code-mode-gateway-e2e" ZUVIX_IMAGE)"
PORT="${ZUVIX_MCP_CODE_MODE_GATEWAY_PORT:-18789}"
MOCK_PORT="${ZUVIX_MCP_CODE_MODE_MOCK_PORT:-44082}"
TOKEN="mcp-code-mode-e2e-$(date +%s)-$$"
CONTAINER_NAME="zuvix-mcp-code-mode-e2e-$$"
CLIENT_LOG="$(mktemp -t zuvix-mcp-code-mode-client-log.XXXXXX)"
...
ZUVIX_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    mock_pid=\"\$(zuvix_e2e_start_mock_openai \"$MOCK_PORT\" /tmp/mcp-code-mode-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(zuvix_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-gateway.log)\"
    zuvix_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-gateway.log 480 $PORT
