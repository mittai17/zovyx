...
#   mode against the installed Zuvix package.
#
# Parameters:
#   --container: run the in-container scenario. Host mode is the default.
#   ZUVIX_CHANNEL_PLUGIN_TRUST_E2E_IMAGE: override the Docker image name.
#   ZUVIX_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD=1: reuse/pull the image.
#
# Outputs:
...
Description:
  Proves the packaged Zuvix CLI enforces local channel plugin trust for
...
Environment:
  ZUVIX_CHANNEL_PLUGIN_TRUST_E2E_IMAGE       Override Docker image name.
  ZUVIX_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD  Reuse/pull image instead of building.
  ZUVIX_TEST_STATE_SCRIPT_B64                Required in --container mode.
...
Examples:
  ...
  ZUVIX_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD=1 bash scripts/e2e/channel-plugin-trust-docker.sh
EOF
}
...
EOF
  ...
  cat >"$plugin_dir/zuvix.plugin.json" <<EOF
{
  "id": "$plugin_id",
...
run_case() {
  ...
  local trusted="${2:?missing trusted flag}"
  local scratch
  scratch="$(mktemp -d "/tmp/zuvix-channel-plugin-trust-$case_id.XXXXXX")"
  ...
  PLUGINTRUST_IMPORT_MARKER="$marker_dir/import.marker" \
    ...
    run_zuvix channels add --channel e2e-load-paths --token "$canary" \
...
run_container() {
  ...
  export ZUVIX_WORKSPACE_DIR="$HOME/.zuvix/workspace"
...
run_host() {
  ...
  image_name="$(
    docker_e2e_resolve_image \
      "zuvix-channel-plugin-trust-e2e:local" \
      ZUVIX_CHANNEL_PLUGIN_TRUST_E2E_IMAGE
  )"
  local skip_build="${ZUVIX_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD:-0}"
  ...
  docker_e2e_run_logged_print_with_harness \
    ...
    -e "ZUVIX_TEST_STATE_SCRIPT_B64=$state_script_b64" \
    "$image_name" \
    bash scripts/e2e/channel-plugin-trust-docker.sh --container
}
