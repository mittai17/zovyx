// Zuvix bundle MCP tools Docker harness.
...
server.tool("docker_probe", "Docker Zuvix MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("zuvix-agent-bundle-mcp-");
