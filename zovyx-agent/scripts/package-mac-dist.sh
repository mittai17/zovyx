...
# Output:
# - dist/Zuvix.app
# - dist/Zuvix-<version>.zip
# - dist/Zuvix-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.zuvix.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/zuvix-sparkle-build.XXXXXX")" || {
...
ZIP="$ROOT_DIR/dist/Zuvix-$VERSION.zip"
DMG="$ROOT_DIR/dist/Zuvix-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/Zuvix-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/Zuvix-$VERSION.dSYM.zip"
