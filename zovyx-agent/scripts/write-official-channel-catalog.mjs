...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.zuvix) ? packageJson.zuvix : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.zuvix?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.zuvix?.channel?.id) || left.name;
    const rightId = trimString(right.zuvix?.channel?.id) || right.name;
