...
SERVER="${ZUVIX_SERVER:-l36}"

# Check auth status
termux-toast "Checking Zuvix auth..."

STATUS=$(ssh "$SERVER" '$HOME/zuvix/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/zuvix/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ZUVIX_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/zuvix/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Zuvix won't work until re-auth"
                ;;
        esac
        ;;
