...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.ZUVIX_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[zuvix] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[zuvix] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[zuvix] This usually means a file in node_modules is empty or truncated.");
  console.error("[zuvix] Recommended recovery:");
  console.error("[zuvix]   rm -rf node_modules");
  console.error("[zuvix]   pnpm store prune");
  console.error("[zuvix]   pnpm install");
  console.error("");
  console.error("[zuvix] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.ZUVIX_WATCH_MODE = "1";
  childEnv.ZUVIX_WATCH_SESSION = watchSession;
  ...
  childEnv.ZUVIX_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.ZUVIX_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `zuvix doctor --fix --non-interactive` once.",
        deps,
      );
