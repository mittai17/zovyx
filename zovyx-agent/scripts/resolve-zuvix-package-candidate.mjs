...
const DEFAULT_OUTPUT_NAME = "zuvix-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "ZUVIX_TRUSTED_PACKAGE_TOKEN";
...
export const ZUVIX_PACKAGE_SPEC_RE =
  /^zuvix@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-zuvix-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
export function validateZuvixPackageSpec(spec) {
  if (!ZUVIX_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be zuvix@alpha, zuvix@beta, zuvix@latest, or an exact Zuvix release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateZuvixPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Zuvix branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `zuvix-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^zuvix-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no Zuvix tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking Zuvix package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-zuvix-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `Zuvix package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "zuvix") {
    throw new Error(`package candidate must be named "zuvix"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
