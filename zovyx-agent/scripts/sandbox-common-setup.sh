...
BASE_IMAGE="${BASE_IMAGE:-zuvix-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-zuvix-sandbox-common:bookworm-slim}"
...
ZUVIX_DOCKER_BUILD_USE_BUILDX="${ZUVIX_DOCKER_BUILD_USE_BUILDX:-0}"
ZUVIX_DOCKER_BUILD_CACHE_FROM="${ZUVIX_DOCKER_BUILD_CACHE_FROM:-}"
ZUVIX_DOCKER_BUILD_CACHE_TO="${ZUVIX_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=zuvix.sandbox=1)
NOTE
