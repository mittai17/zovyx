 ...
 * `zuvix update`. Use it to verify the completion installation flow
...
\`zuvix update\`, \`zuvix doctor\`, and \`zuvix onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Zuvix Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`zuvix completion --install\` later to enable.`));
    return;
  }
