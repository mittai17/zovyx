...
if zuvix_live_truthy "${ZUVIX_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  ZUVIX_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${ZUVIX_LIVE_PROVIDERS:-}" || -n "${ZUVIX_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      zuvix_live_collect_auth_dirs_from_csv "${ZUVIX_LIVE_PROVIDERS:-}"
      zuvix_live_collect_auth_dirs_from_csv "${ZUVIX_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      zuvix_live_collect_auth_files_from_csv "${ZUVIX_LIVE_PROVIDERS:-}"
      zuvix_live_collect_auth_files_from_csv "${ZUVIX_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${ZUVIX_DOCKER_PROFILE_ENV_ONLY:-0}"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> External auth dirs: ${AUTH_DIRS_CSV:-none}"
echo "==> External auth files: ${AUTH_FILES_CSV:-none}"
DOCKER_RUN_ARGS=()
zuvix_live_init_docker_run_args DOCKER_RUN_ARGS "${ZUVIX_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e ZUVIX_LIVE_TEST=1 \
  -e ZUVIX_LIVE_MODELS="${ZUVIX_LIVE_MODELS:-modern}" \
  -e ZUVIX_LIVE_PROVIDERS="${ZUVIX_LIVE_PROVIDERS:-}" \
  -e ZUVIX_LIVE_MAX_MODELS="${ZUVIX_LIVE_MAX_MODELS:-}" \
  -e ZUVIX_LIVE_MODEL_TIMEOUT_MS="${ZUVIX_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e ZUVIX_LIVE_REQUIRE_PROFILE_KEYS="${ZUVIX_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e ZUVIX_LIVE_GATEWAY_MODELS="${ZUVIX_LIVE_GATEWAY_MODELS:-}" \
  -e ZUVIX_LIVE_GATEWAY_PROVIDERS="${ZUVIX_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ZUVIX_LIVE_GATEWAY_MAX_MODELS="${ZUVIX_LIVE_GATEWAY_MAX_MODELS:-}" \
