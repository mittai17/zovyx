...
const MANUAL_SECTION_START = "<!-- zuvix-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- zuvix-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving Zuvix messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to Zuvix.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an Zuvix plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.zuvix?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    return "included in Zuvix";
  }
  const install = packageJson.zuvix?.install;
  const release = packageJson.zuvix?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.zuvix?.install?.clawhubSpec === "string" ||
    typeof packageJson.zuvix?.install?.npmSpec === "string";
...
function renderReferenceIndex(records) {
  ...
  return `---
summary: "Generated index of Zuvix plugin reference pages"
read_when:
  - You need a reference page for a specific Zuvix plugin
...
\`zuvix.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "zuvix.plugin.json");
