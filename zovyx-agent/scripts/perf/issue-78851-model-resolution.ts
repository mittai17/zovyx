// Issue 78851 Model Resolution script supports Zuvix repository automation.
...
function printUsage(): void {
  process.stdout.write(`Zuvix issue #78851 model-resolution profiler
...
function buildConfig(options: Options, workspaceDir: string): ZuvixConfig {
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureZuvixModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "zuvix-issue-78851-"));
