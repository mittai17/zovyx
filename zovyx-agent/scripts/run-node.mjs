#!/usr/bin/env node
// Development runner that rebuilds Zuvix, runs runtime postbuild steps, and
...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "zuvix.plugin.json"));
    }
...
const listRequiredZuvixExtensionAliasOutputs = (deps) => {
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  ...
  const aliasDir = path.join(distRoot, "extensions", "node_modules", "zuvix");
  return [
...
const listRequiredStaticExtensionAssetOutputs = (deps) => {
  if (deps.env.ZUVIX_RUNTIME_POSTBUILD_STATIC_ASSETS === "0") {
    return [];
  }
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  const runtimeRoot = resolveRuntimePostBuildRuntimeRoot(deps);
...
export const listRequiredRuntimePostBuildOutputs = (deps) => {
  ...
  return [
    ...
    ...listRequiredZuvixExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.ZUVIX_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.ZUVIX_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.ZUVIX_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by ZUVIX_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by ZUVIX_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "ZUVIX_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "ZUVIX_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "ZUVIX_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "ZUVIX_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "ZUVIX_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "ZUVIX_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "ZUVIX_RUN_NODE_BUILD_LOCK_STALE_MS";
const RUN_NODE_SKIP_DTS_BUILD_ENV = "ZUVIX_RUN_NODE_SKIP_DTS_BUILD";
...
const logRunner = (message, deps) => {
  if (deps.env.ZUVIX_RUNNER_LOG === "0") {
    return;
  }
  const line = `[zuvix] ${message}\n`;
...
const shouldUseRunNodeProgress = (deps) =>
  ...
  deps.env.ZUVIX_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[zuvix] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const listRunNodeCpuProfiles = (deps, absoluteProfileDir, commandName) => {
  ...
  const prefix = `zuvix-${commandName}-`;
  return entries
    .filter(
      (entry) =>
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `zuvix-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.ZUVIX_TRACE_SYNC_IO === "1") {
...
const runZuvix = async (deps) => {
  const diagnosticArgs = resolveRunNodeDiagnosticArgs(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "zuvix.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[zuvix] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const canUseStampedGatewayClientDist = (deps) => {
  ...
  if (
    ...
    deps.env.ZUVIX_FORCE_RUNTIME_POSTBUILD === "1"
  ) {
    return false;
  }
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.ZUVIX_BUILD_PRIVATE_QA = "1";
    deps.env.ZUVIX_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.ZUVIX_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
