...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/zuvix/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Zuvix Re-Auth" -c "After getting key, run: ssh $SERVER '~/zuvix/scripts/mobile-reauth.sh'" --id zuvix-auth
        ;;
    *)
