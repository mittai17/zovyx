...
Options:
  ...
  --keep                         Keep the synthetic ZUVIX_HOME and workspace after the run.
...
export function parseArgs(argv) {
  ...
  const options = {
    fileCount: undefined,
    mode: process.env.ZUVIX_MEMORY_FD_REPRO_MODE || "fixed",
    ...
    keep: process.env.ZUVIX_MEMORY_FD_REPRO_KEEP === "1",
    allowNonDarwin: process.env.ZUVIX_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1",
  };
  ...
  options.fileCount ??= readPositiveNumberEnv("ZUVIX_MEMORY_FD_REPRO_FILES", DEFAULT_FILE_COUNT);
  options.maxWorkspaceRegFds ??= readNumberEnv(
    "ZUVIX_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS",
    DEFAULT_MAX_WORKSPACE_REG_FDS,
  );
  options.invokeTimeoutMs ??= readPositiveNumberEnv("ZUVIX_MEMORY_FD_REPRO_TIMEOUT_MS", 30_000);
  options.sampleDelayMs ??= readNumberEnv("ZUVIX_MEMORY_FD_REPRO_SAMPLE_DELAY_MS", 1_000);
  options.settleDelayMs ??= readNumberEnv("ZUVIX_MEMORY_FD_REPRO_SETTLE_DELAY_MS", 5_000);
...
/**
 * Writes isolated Zuvix config for the synthetic memory workspace.
 */
export function writeConfig({ homeDir, workspaceDir, port, token }) {
  const configDir = path.join(homeDir, ".zuvix");
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    ZUVIX_STATE_DIR: path.join(homeDir, ".zuvix"),
