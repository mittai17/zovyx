...
# Build and bundle Zuvix into a minimal .app we can open.
# Outputs to dist/Zuvix.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT_DIR/scripts/lib/plistbuddy.sh"
APP_ROOT="$ROOT_DIR/dist/Zuvix.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Zuvix"
MLX_TTS_HELPER_PRODUCT="zuvix-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.zuvix.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/zuvix/zuvix/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Zuvix/Resources/Info.plist"
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" ZuvixBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" ZuvixGitCommit "$GIT_COMMIT"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Zuvix"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Zuvix"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Zuvix"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Zuvix" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Zuvix/Resources/Zuvix.icns" "$APP_ROOT/Contents/Resources/Zuvix.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Zuvix/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ZuvixKit resources"
ZUVIXKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ZuvixKit_ZuvixKit.bundle"
if [ -d "$ZUVIXKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ZuvixKit_ZuvixKit.bundle"
  cp -R "$ZUVIXKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ZuvixKit_ZuvixKit.bundle"
else
  echo "ERROR: ZuvixKit resource bundle not found at $ZUVIXKIT_BUNDLE" >&2
  exit 1
fi
...
running_packaged_app_pids() {
  ...
  local app_binary="$APP_ROOT/Contents/MacOS/Zuvix"
  local pid
...
stop_packaged_app_if_running() {
  ...
  echo "⏹  Stopping packaged Zuvix bundle (${pids[*]})"
  ...
  echo "ERROR: Packaged Zuvix bundle did not exit: ${pids[*]}" >&2
  return 1
}
