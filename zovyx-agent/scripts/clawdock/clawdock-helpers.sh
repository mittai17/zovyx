#!/usr/bin/env bash
# ClawDock - Docker helpers for Zuvix
# Inspired by Simon Willison's "Running Zuvix in Docker"
# https://til.simonwillison.net/llms/zuvix-docker
#
# Installation:
...
# Common paths to check for Zuvix
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/zuvix"
  "${HOME}/workspace/zuvix"
  "${HOME}/projects/zuvix"
  "${HOME}/dev/zuvix"
  "${HOME}/code/zuvix"
  "${HOME}/src/zuvix"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Zuvix at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/zuvix"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Zuvix not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/zuvix/zuvix.git ~/zuvix"
    echo "  cd ~/zuvix && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ZUVIX_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d zuvix-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart zuvix-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f zuvix-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.zuvix"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show zuvix.json
  if [[ -f "${config_dir}/zuvix.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/zuvix.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/zuvix.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No zuvix.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec zuvix-gateway \
    bash -c 'echo "alias zuvix=\"./zuvix.mjs\"" > /tmp/.bashrc_zuvix && bash --rcfile /tmp/.bashrc_zuvix'
}
...
clawdock-cli() {
  _clawdock_compose run --rm zuvix-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build zuvix-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d zuvix-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build zuvix-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ZUVIX_GATEWAY_TOKEN=$token" zuvix-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" zuvix-gateway \
    bash -c './zuvix.mjs config set gateway.remote.token "$TOKEN" && ./zuvix.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec zuvix-gateway \
    bash -c "./zuvix.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart zuvix-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm zuvix-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec zuvix-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'zuvix config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Zuvix${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (zuvix alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to zuvix project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.zuvix)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'zuvix channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'zuvix status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.zuvix.ai${_CLR_RESET}"
  echo ""
}
