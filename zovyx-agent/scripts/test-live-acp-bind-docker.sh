...
ACP_AGENT_LIST_RAW="${ZUVIX_LIVE_ACP_BIND_AGENTS:-${ZUVIX_LIVE_ACP_BIND_AGENT:-claude,codex,gemini}}"
TEMP_DIRS=()
...
zuvix_live_acp_bind_append_build_extension() {
...
zuvix_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    claude) printf '%s\n' "claude-cli" ;;
    codex) printf '%s\n' "codex-cli" ;;
    droid) printf '%s\n' "droid" ;;
    ...
    opencode) printf '%s\n' "opencode" ;;
    *)
      echo "Unsupported ZUVIX_LIVE_ACP_BIND agent: ${1:-} (expected claude, codex, droid, gemini, or opencode)" >&2
      return 1
      ;;
  esac
}

zuvix_live_acp_bind_resolve_agent_command() {
  case "${1:-}" in
    claude) printf '%s' "${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND_CLAUDE:-${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    codex) printf '%s' "${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND_CODEX:-${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    droid) printf '%s' "${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND_DROID:-${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    gemini) printf '%s' "${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND_GEMINI:-${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    opencode) printf '%s' "${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND_OPENCODE:-${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    *) return 1 ;;
  esac
}
...
zuvix_live_acp_bind_load_factory_api_key_from_profile() {
...
run_setup_command() {
  local timeout_value="${ZUVIX_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:-180}s"
  local timeout_bin=""
...
agent="${ZUVIX_LIVE_ACP_BIND_AGENT:-claude}"
case "$agent" in
  claude)
...
if [ -n "\${ZUVIX_LIVE_ACP_BIND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${ZUVIX_LIVE_ACP_BIND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${ZUVIX_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${ZUVIX_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD}"
fi
...
NODE
  ...
  opencode)
    ...
    export OPENCODE_CONFIG_CONTENT="$(
      node -e 'process.stdout.write(JSON.stringify({model: process.env.ZUVIX_LIVE_ACP_BIND_OPENCODE_MODEL || "opencode/kimi-k2.6"}))'
    )"
    ;;
  *)
    echo "Unsupported ZUVIX_LIVE_ACP_BIND_AGENT: $agent" >&2
    exit 1
    ;;
esac
tmp_dir="$(mktemp -d)"
...
cd "$tmp_dir"
export ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND="${ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND:-}"
node scripts/test-live.mjs -- ${ZUVIX_LIVE_ACP_BIND_TEST_FILES:-src/gateway/gateway-acp-bind.live.test.ts}
EOF

zuvix_live_acp_bind_append_build_extension acpx
...
for token in "${ACP_AGENT_TOKENS[@]}"; do
  agent="$(zuvix_live_trim "$token")"
  [[ -n "$agent" ]] || continue
  zuvix_live_acp_bind_resolve_auth_provider "$agent" >/dev/null
  ACP_AGENTS+=("$agent")
done

if ((${#ACP_AGENTS[@]} == 0)); then
  echo "No ACP bind agents selected. Use ZUVIX_LIVE_ACP_BIND_AGENTS=claude,codex,droid,gemini,opencode." >&2
  exit 1
fi

for ACP_AGENT in "${ACP_AGENTS[@]}"; do
  AUTH_PROVIDER="$(zuvix_live_acp_bind_resolve_auth_provider "$ACP_AGENT")"
  AGENT_COMMAND="$(zuvix_live_acp_bind_resolve_agent_command "$ACP_AGENT")"
  ...
  if [[ -n "${ZUVIX_DOCKER_AUTH_DIRS:-}" ]]; then
    ...
    while IFS= read -r auth_dir; do
      [[ -n "$auth_dir" ]] || continue
      AUTH_DIRS+=("$auth_dir")
    done < <(zuvix_live_collect_auth_dirs_from_csv "$AUTH_PROVIDER")
    while IFS= read -r auth_file; do
      [[ -n "$auth_file" ]] || continue
      AUTH_FILES+=("$auth_file")
    done < <(zuvix_live_collect_auth_files_from_csv "$AUTH_PROVIDER")
  fi
  ...
  echo "==> Run ACP bind live test in Docker"
  echo "==> Agent: $ACP_AGENT"
  echo "==> Test files: ${ZUVIX_LIVE_ACP_BIND_TEST_FILES:-src/gateway/gateway-acp-bind.live.test.ts}"
  echo "==> Profile file: $PROFILE_STATUS"
  echo "==> Auth dirs: ${AUTH_DIRS_CSV:-none}"
  echo "==> Auth files: ${AUTH_FILES_CSV:-none}"
  if zuvix_live_uses_managed_bind_dirs; then
  ...
  DOCKER_RUN_ARGS=()
  zuvix_live_init_docker_run_args DOCKER_RUN_ARGS "${ZUVIX_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s}"
  DOCKER_RUN_ARGS+=(--rm -t \
    -u "$DOCKER_USER" \
    --entrypoint bash \
    -e ANTHROPIC_API_KEY \
    -e ANTHROPIC_API_KEY_OLD \
    -e ZUVIX_LIVE_ACP_BIND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" \
    -e ZUVIX_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}" \
    -e GEMINI_API_KEY \
    -e GOOGLE_API_KEY \
    -e FACTORY_API_KEY \
    -e OPENAI_API_KEY \
    ...
    -e ZUVIX_LIVE_TEST=1 \
    -e ZUVIX_LIVE_ACP_BIND=1 \
    -e ZUVIX_LIVE_ACP_BIND_AGENT="$ACP_AGENT" \
    -e ZUVIX_LIVE_ACP_BIND_TEST_FILES="${ZUVIX_LIVE_ACP_BIND_TEST_FILES:-}" \
    -e ZUVIX_LIVE_ACP_BIND_CODEX_MODEL="${ZUVIX_LIVE_ACP_BIND_CODEX_MODEL:-}" \
    -e ZUVIX_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS="${ZUVIX_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:-180}" \
    -e ZUVIX_LIVE_ACP_BIND_OPENCODE_MODEL="${ZUVIX_LIVE_ACP_BIND_OPENCODE_MODEL:-opencode/kimi-k2.6}" \
    -e ZUVIX_LIVE_ACP_SPAWN_DEFAULTS="${ZUVIX_LIVE_ACP_SPAWN_DEFAULTS:-}" \
    -e ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_AGENT="${ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_AGENT:-}" \
    -e ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_CONNECT_TIMEOUT_MS="${ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_CONNECT_TIMEOUT_MS:-}" \
    -e ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_MODEL="${ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_MODEL:-}" \
    -e ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_THINKING="${ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_THINKING:-}" \
    -e ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_TIMEOUT_MS="${ZUVIX_LIVE_ACP_SPAWN_DEFAULTS_TIMEOUT_MS:-}" \
    -e ZUVIX_LIVE_ACP_BIND_AGENT_COMMAND="$AGENT_COMMAND")
