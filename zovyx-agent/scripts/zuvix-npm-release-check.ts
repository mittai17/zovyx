...
// Zuvix Npm Release Check script supports Zuvix repository automation.
...
const EXPECTED_REPOSITORY_URL = "https://github.com/zuvix/zuvix";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@zuvix/fs-safe";
...
const skipPackValidationEnv = "ZUVIX_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "ZUVIX_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "zuvix") {
    errors.push(`package.json name must be "zuvix"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.zuvix !== "zuvix.mjs") {
    errors.push(
      `package.json bin.zuvix must be "zuvix.mjs"; found "${pkg.bin?.zuvix ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`zuvix-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `zuvix-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
