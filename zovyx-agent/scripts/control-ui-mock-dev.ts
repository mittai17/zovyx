// Control Ui Mock Dev script supports Zuvix repository automation.
...
function createChatPickerScenario(): ControlUiMockGatewayScenario {
  ...
  const workspaceListCases = ["main", "alpha", "zuvix-mock"].map((agentId) => ({
    match: { agentId },
    response: {
      agentId,
  ...
  const workspaceFileCases = ["main", "alpha", "zuvix-mock"].flatMap((agentId) =>
  ...
  <text x="320" y="326" text-anchor="middle" font-family="ui-sans-serif, system-ui" font-size="24" fill="#f6f7f9">zuvix session artifact</text>
</svg>`;
  const lobsterArtifact = {
    id: "artifact-zuvix-lobster",
    type: "image",
    title: "zuvix-lobster-preview.svg",
  ...
  return {
    assistantAgentId: "zuvix-mock",
    assistantName: "Zuvix mock",
    defaultAgentId: "zuvix-mock",
...
function createMockGatewayPlugin(scenario: ControlUiMockGatewayScenario): Plugin {
  ...
  return {
    ...
    name: "zuvix-control-ui-mock-gateway",
    transformIndexHtml(html) {
      return html.replace(
        "</head>",
        `    <script data-zuvix-control-ui-mock-gateway>\n${initScript}\n    </script>\n  </head>`,
      );
    },
  };
}
...
const server = await createServer({
  ...
  define: {
    ZUVIX_CONTROL_UI_BUILD_ID: JSON.stringify("mock"),
  },
  logLevel: "error",
  optimizeDeps: {
