#!/usr/bin/env node
// Generate Bundled Channel Config Metadata script supports Zuvix repository automation.
...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const zuvixMeta =
    ...
    "zuvix" in source.packageJson
      ? (source.packageJson.zuvix as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    zuvixMeta &&
    typeof zuvixMeta.channel === "object" &&
    zuvixMeta.channel &&
    !Array.isArray(zuvixMeta.channel)
      ? (zuvixMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
