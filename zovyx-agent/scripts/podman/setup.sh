#!/usr/bin/env bash
# One-time host setup for rootless Zuvix in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.zuvix by default, and uses the repo-local
# launch script at ./scripts/run-zuvix-podman.sh.
#
...
#   Or set ZUVIX_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-zuvix-podman.sh launch
#   ./scripts/run-zuvix-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start zuvix.service
set -euo pipefail

REPO_PATH="${ZUVIX_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-zuvix-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/zuvix.container.in"
ZUVIX_USER="$(id -un)"
ZUVIX_HOME="${HOME:-}"
ZUVIX_CONFIG_DIR="${ZUVIX_CONFIG_DIR:-}"
ZUVIX_WORKSPACE_DIR="${ZUVIX_WORKSPACE_DIR:-}"
ZUVIX_IMAGE="${ZUVIX_PODMAN_IMAGE:-${ZUVIX_IMAGE:-zuvix:local}}"
ZUVIX_CONTAINER_NAME="${ZUVIX_PODMAN_CONTAINER:-zuvix}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${ZUVIX_PODMAN_GATEWAY_HOST_PORT:-${ZUVIX_GATEWAY_PORT:-18789}}"
QUADLET_GATEWAY_PORT="18789"
PODMAN_PULL_TIMEOUT="${ZUVIX_PODMAN_SETUP_PULL_TIMEOUT:-600s}"
PODMAN_BUILD_TIMEOUT="${ZUVIX_PODMAN_SETUP_BUILD_TIMEOUT:-1800s}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate ZUVIX_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${ZUVIX_PODMAN_QUADLET:-}" ]]; then
  case "${ZUVIX_PODMAN_QUADLET,,}" in
...
if [[ "$ZUVIX_IMAGE" == "zuvix:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set ZUVIX_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$ZUVIX_HOME" ]]; then
  ZUVIX_HOME="$(resolve_user_home "$ZUVIX_USER")"
fi
if [[ -z "$ZUVIX_HOME" ]]; then
  echo "Unable to resolve HOME for user $ZUVIX_USER." >&2
  exit 1
fi
if [[ -z "$ZUVIX_CONFIG_DIR" ]]; then
  ZUVIX_CONFIG_DIR="$ZUVIX_HOME/.zuvix"
fi
if [[ -z "$ZUVIX_WORKSPACE_DIR" ]]; then
  ZUVIX_WORKSPACE_DIR="$ZUVIX_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$ZUVIX_HOME"
validate_mount_source_path "config directory" "$ZUVIX_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$ZUVIX_WORKSPACE_DIR"
validate_container_name "$ZUVIX_CONTAINER_NAME"
validate_image_name "$ZUVIX_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$ZUVIX_CONFIG_DIR" "$ZUVIX_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$ZUVIX_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$ZUVIX_WORKSPACE_DIR"

ZUVIX_IMAGE_APT_PACKAGES="${ZUVIX_IMAGE_APT_PACKAGES-${ZUVIX_DOCKER_APT_PACKAGES:-}}"
ZUVIX_IMAGE_PIP_PACKAGES="${ZUVIX_IMAGE_PIP_PACKAGES:-}"
BUILD_ARGS=()
if [[ -n "$ZUVIX_IMAGE_APT_PACKAGES" ]]; then
  BUILD_ARGS+=(--build-arg "ZUVIX_IMAGE_APT_PACKAGES=${ZUVIX_IMAGE_APT_PACKAGES}")
fi
if [[ -n "$ZUVIX_IMAGE_PIP_PACKAGES" ]]; then
  BUILD_ARGS+=(--build-arg "ZUVIX_IMAGE_PIP_PACKAGES=${ZUVIX_IMAGE_PIP_PACKAGES}")
fi
if [[ -n "${ZUVIX_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "ZUVIX_EXTENSIONS=${ZUVIX_EXTENSIONS}")
fi
if [[ -n "${ZUVIX_INSTALL_BROWSER:-}" ]]; then
  BUILD_ARGS+=(--build-arg "ZUVIX_INSTALL_BROWSER=${ZUVIX_INSTALL_BROWSER}")
fi

if [[ "$ZUVIX_IMAGE" == "zuvix:local" ]]; then
  echo "Building image $ZUVIX_IMAGE ..."
  run_podman_build -t "$ZUVIX_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$ZUVIX_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $ZUVIX_IMAGE"
  else
    echo "Pulling image $ZUVIX_IMAGE ..."
    run_podman_pull "$ZUVIX_IMAGE"
  fi
fi

ENV_FILE="$ZUVIX_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
  TOKEN="$(generate_token_hex_32)"
  (
    umask 077
    write_file_atomically "$ENV_FILE" 600 <<EOF
ZUVIX_GATEWAY_TOKEN=$TOKEN
EOF
  )
  echo "Generated ZUVIX_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "ZUVIX_PODMAN_CONTAINER" "$ZUVIX_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "ZUVIX_PODMAN_IMAGE" "$ZUVIX_IMAGE"

CONFIG_JSON="$ZUVIX_CONFIG_DIR/zuvix.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$ZUVIX_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/zuvix.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  ZUVIX_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$ZUVIX_HOME")"
  ZUVIX_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$ZUVIX_CONFIG_DIR")"
  ZUVIX_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$ZUVIX_WORKSPACE_DIR")"
  ZUVIX_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$ZUVIX_IMAGE")"
  ZUVIX_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$ZUVIX_CONTAINER_NAME")"
  sed \
    -e "s|{{ZUVIX_HOME}}|$ZUVIX_HOME_ESCAPED|g" \
    -e "s|{{ZUVIX_CONFIG_DIR}}|$ZUVIX_CONFIG_ESCAPED|g" \
    -e "s|{{ZUVIX_WORKSPACE_DIR}}|$ZUVIX_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$ZUVIX_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$ZUVIX_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start zuvix.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start zuvix.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-zuvix-podman.sh launch"
echo "  ./scripts/run-zuvix-podman.sh launch setup"
echo "  zuvix --container $ZUVIX_CONTAINER_NAME dashboard --no-open"
