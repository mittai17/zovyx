...
# Zuvix CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
ZUVIX_EFFECTIVE_HOME="$(resolve_zuvix_effective_home)"
PREFIX="${ZUVIX_PREFIX:-${HOME}/.zuvix}"
ZUVIX_VERSION="${ZUVIX_VERSION:-latest}"
NODE_VERSION="${ZUVIX_NODE_VERSION:-22.22.0}"
NODE_VERSION_REQUESTED=0
if [[ -n "${ZUVIX_NODE_VERSION:-}" ]]; then
  NODE_VERSION_REQUESTED=1
fi
MIN_NODE_VERSION="22.19.0"
APK_NODE_BIN_DIR="/usr/bin"
NPM_LOGLEVEL="${ZUVIX_NPM_LOGLEVEL:-error}"
INSTALL_METHOD="${ZUVIX_INSTALL_METHOD:-npm}"
GIT_DIR="${ZUVIX_GIT_DIR:-${ZUVIX_EFFECTIVE_HOME}/zuvix}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.zuvix; use \$ZUVIX_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/zuvix, or \$ZUVIX_HOME/zuvix)
  --version <ver>                     Zuvix version (default: latest)
...
Environment variables:
  ...
  ZUVIX_HOME=...
  ZUVIX_PREFIX=...
  ZUVIX_VERSION=latest|next|<semver>
  ZUVIX_GIT_DIR=...
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${ZUVIX_GIT_DIR:-${ZUVIX_EFFECTIVE_HOME}/zuvix}}"
...
resolve_git_zuvix_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "zuvix" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_zuvix() {
  local requested="${ZUVIX_VERSION:-latest}"
  if is_zuvix_source_package_install_spec "$requested"; then
    fail "npm installs do not support Zuvix GitHub source targets like '${requested}'. Use --install-method git --version main, latest, beta, an exact version, or a built .tgz package."
  fi
  ...
  emit_json "{\"event\":\"step\",\"name\":\"zuvix\",\"status\":\"start\",\"version\":\"${requested}\"}"
  log "Installing Zuvix (${requested})..."
  if [[ "$SET_NPM_PREFIX" -eq 1 ]]; then
    fix_npm_prefix_if_needed
  fi

  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "zuvix@latest"; then
      log "npm install zuvix@latest failed; retrying zuvix@next"
      emit_json "{\"event\":\"step\",\"name\":\"zuvix\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "zuvix@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "zuvix@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/zuvix"
  cat > "${PREFIX}/bin/zuvix" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/zuvix/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/zuvix"
  emit_json "{\"event\":\"step\",\"name\":\"zuvix\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
install_zuvix_from_git() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"zuvix\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
  if [[ -d "$repo_dir/.git" ]]; then
    log "Installing Zuvix from git checkout: ${repo_dir}"
  else
    log "Installing Zuvix from GitHub (${repo_url})..."
  fi
...
EOF
  chmod +x "${PREFIX}/bin/zuvix"
  emit_json "{\"event\":\"step\",\"name\":\"zuvix\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_zuvix_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/zuvix" ]]; then
    version="$("${PREFIX}/bin/zuvix" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
main() {
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_zuvix_from_git "$GIT_DIR"
  ...
  if [[ -n "$installed_version" ]]; then
    ...
    log "Zuvix installed (${installed_version})."
  else
  ...
  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/zuvix" onboard
  fi
}

if [[ "${ZUVIX_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
