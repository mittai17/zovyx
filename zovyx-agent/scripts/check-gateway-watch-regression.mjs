...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  ZUVIX_RUNTIME_POSTBUILD_STATIC_ASSETS: "0",
...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[zuvix] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const isolatedStateDir = path.join(isolatedHomeDir, ".zuvix");
  const isolatedConfigPath = path.join(isolatedStateDir, "zuvix.json");
  const shellSource = [
    'echo "$$" > "$ZUVIX_WATCH_PID_FILE"',
    'mkdir -p "$ZUVIX_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$ZUVIX_CONFIG_PATH"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    ZUVIX_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    ZUVIX_HOME: isolatedHomeDir,
    ZUVIX_CONFIG_PATH: isolatedConfigPath,
    ZUVIX_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-gateway-watch-"));
