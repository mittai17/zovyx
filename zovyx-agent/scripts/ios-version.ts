// Ios Version script supports Zuvix repository automation.
...
if (options.format === "shell") {
  process.stdout.write(
    [
      `ZUVIX_IOS_VERSION=${version.canonicalVersion}`,
      `ZUVIX_MARKETING_VERSION=${version.marketingVersion}`,
      `ZUVIX_BUILD_VERSION=${version.buildVersion}`,
    ].join("\n") + "\n",
  );
} else {
