// Mcp Code Mode Gateway E2E script supports Zuvix repository automation.
...
export async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-mcp-code-mode-"));
  const keep = process.env.ZUVIX_MCP_CODE_MODE_GATEWAY_E2E_KEEP === "1";
  const previousEnv = {
  ...
  try {
    ...
    await provider?.stop();
    resetConfigRuntimeState();
    restoreEnvValue("ZUVIX_STATE_DIR", previousEnv.stateDir);
    restoreEnvValue("ZUVIX_CONFIG_PATH", previousEnv.configPath);
    restoreEnvValue("ZUVIX_TEST_FAST", previousEnv.testFast);
    if (!keep) {
