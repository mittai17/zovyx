...
export async function startGateway({
  ...
  let child;
  const launcherArgs = resolveZuvixLaunchArgs(repoRoot, sourceEntryExists);
  try {
    child = spawnImpl(
      process.execPath,
      [
      ...
      ],
      {
        ...
        env: {
          ...
          ZUVIX_STATE_DIR: path.join(tempRoot, "state"),
...
async function main() {
  ...
  const repoRoot = path.resolve(args.repoRoot ?? process.env.ZUVIX_REPO_ROOT ?? process.cwd());
  ...
  try {
    ...
    const requireFromZuvix = createRequire(path.join(repoRoot, "package.json"));
    const WebSocket = requireFromZuvix("ws");
    ...
    const connect = await client.request(
      "connect",
      {
        ...
        client: {
          id: "gateway-client",
          displayName: "zuvix-rtt rpc probe",
          version: "rtt",
          platform: process.platform,
          mode: "backend",
          instanceId: `zuvix-rtt-rpc-${randomUUID()}`,
        },
        locale: "en-US",
        userAgent: "zuvix-rtt-rpc",
        role: "operator",
