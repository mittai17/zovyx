...
#
# Suggested cron: */30 * * * * /home/admin/zuvix/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Zuvix notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., zuvix-alerts)
...
STATE_FILE="$HOME/.zuvix/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Zuvix if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use zuvix
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Zuvix to $NOTIFY_PHONE..."
            zuvix send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Zuvix is down. Run: ssh l36 '~/zuvix/scripts/mobile-reauth.sh'" "urgent"
    exit 1
