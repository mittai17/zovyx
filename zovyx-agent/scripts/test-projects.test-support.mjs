...
const TOOLING_ISOLATED_TEST_TARGET = "test/scripts/zuvix-e2e-instance.test.ts";
...
const INCLUDE_FILE_ENV_KEY = "ZUVIX_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "ZUVIX_VITEST_FS_MODULE_CACHE_PATH";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-zuvix-runtime)?\.ts$/u,
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  ["scripts/lib/zuvix-test-state.mjs", ["test/scripts/zuvix-test-state.test.ts"]],
  ...
  ["scripts/zuvix-npm-postpublish-verify.ts", ["test/zuvix-npm-postpublish-verify.test.ts"]],
  ...
  ["scripts/package-zuvix-for-docker.mjs", ["test/scripts/package-zuvix-for-docker.test.ts"]],
  ...
  [
    ...
    ["test/scripts/docker-build-helper.test.ts", "test/scripts/zuvix-test-state.test.ts"],
  ],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/zuvix-test-state.test.ts", ["test/scripts/zuvix-test-state.test.ts"]],
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/zuvix-test-state.ts", ["src/test-utils/zuvix-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "ZUVIX_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "ZUVIX_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_HEARTBEAT_ENV_KEY = "ZUVIX_VITEST_NO_OUTPUT_HEARTBEAT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "ZUVIX_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = parsePositiveInt(
    process.env.ZUVIX_TEST_PROJECTS_PARALLEL,
    "ZUVIX_TEST_PROJECTS_PARALLEL",
  );
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.ZUVIX_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.ZUVIX_VITEST_MAX_WORKERS ?? env.ZUVIX_TEST_WORKERS,
    env.ZUVIX_VITEST_MAX_WORKERS === undefined
      ? "ZUVIX_TEST_WORKERS"
...
export function resolveParallelFullSuiteConcurrency(specCount, envInput, hostInfo) {
  ...
  const override = parsePositiveInt(
    env.ZUVIX_TEST_PROJECTS_PARALLEL,
    "ZUVIX_TEST_PROJECTS_PARALLEL",
  );
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.ZUVIX_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.ZUVIX_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `zuvix-vitest-include-${process.pid}-${Date.now()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.ZUVIX_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.ZUVIX_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
