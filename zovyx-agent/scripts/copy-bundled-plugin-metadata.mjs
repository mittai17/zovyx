...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.ZUVIX_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "zuvix.plugin.json");
    ...
    if (packageJson.zuvix && "extensions" in packageJson.zuvix) {
      packageJson.zuvix = {
        ...packageJson.zuvix,
        extensions: rewritePackageExtensions(packageJson.zuvix.extensions),
        ...(typeof packageJson.zuvix.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.zuvix.setupEntry) }
          : {}),
      };
    }
