...
// Test Force script supports Zuvix repository automation.
...
function usage(): string {
  return [
    ...
    "",
    "Clears the configured Zuvix gateway port, then runs the local test suite.",
    "",
    "Options:",
...
function runTests() {
  const isolatedLock =
    process.env.ZUVIX_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `zuvix-gateway.lock.test.${Date.now()}`);
  const result = spawnSync(process.execPath, ["scripts/test-projects.mjs"], {
    stdio: "inherit",
    env: {
      ...process.env,
      ZUVIX_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
