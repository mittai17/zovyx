// Bench Cli Startup script supports Zuvix repository automation.
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "ZUVIX_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "ZUVIX_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.ZUVIX_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".zuvix");
...
function printUsage(): void {
  console.log(`Zuvix CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: zuvix.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-bench-"));
