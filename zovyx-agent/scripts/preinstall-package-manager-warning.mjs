...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[zuvix] warning: detected ${packageManager} for install lifecycle.`,
    "[zuvix] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[zuvix] prefer: corepack pnpm install",
  ].join("\n");
}
