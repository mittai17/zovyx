...
export function resolveWindowsExtensionChunkSize(env = process.env) {
  return resolvePositiveEnvIntWithFallback(
    env,
    "ZUVIX_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE",
...
export function shouldRunOxlintShardsSerial({
  ...
  const explicitMode = env.ZUVIX_OXLINT_SHARDS_SERIAL?.trim();
  if (explicitMode === "1") {
    return true;
  }
  if (platform === "win32") {
  ...
  const localCheckMode = env.ZUVIX_LOCAL_CHECK_MODE?.trim().toLowerCase();
...
function isRemoteChangedGateEnv(env) {
  return (
    env.ZUVIX_CHECK_CHANGED_REMOTE_CHILD === "1" || env.ZUVIX_CHANGED_LANES_RAW_SYNC === "1"
  );
}
...
export function resolveOxlintShardConcurrency({
  ...
  const explicitConcurrency = resolvePositiveEnvInt(env, "ZUVIX_OXLINT_SHARD_CONCURRENCY");
...
export function resolveShardHeartbeatMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "ZUVIX_OXLINT_SHARD_HEARTBEAT_MS",
...
export function resolveShardKillGraceMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "ZUVIX_OXLINT_SHARD_KILL_GRACE_MS",
