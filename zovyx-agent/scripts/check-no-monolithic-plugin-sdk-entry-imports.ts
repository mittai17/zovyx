// Check No Monolithic Plugin Sdk Entry Imports script supports Zuvix repository automation.
import fs from "node:fs";
import path from "node:path";
import { discoverZuvixPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']zuvix\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']zuvix\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']zuvix\/plugin-sdk\/channel-runtime["']/,
    label: "zuvix/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']zuvix\/plugin-sdk\/config-runtime["']/,
    label: "zuvix/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']zuvix\/plugin-sdk\/infra-runtime["']/,
    label: "zuvix/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic zuvix/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy zuvix/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused zuvix/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
