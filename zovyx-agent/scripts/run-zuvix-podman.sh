#!/usr/bin/env bash
# Rootless Zuvix in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-zuvix-podman.sh launch        # Start gateway
#   ./scripts/run-zuvix-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   zuvix --container zuvix dashboard --no-open
#   zuvix --container zuvix channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      ZUVIX_GATEWAY_TOKEN|ZUVIX_PODMAN_CONTAINER|ZUVIX_PODMAN_IMAGE|ZUVIX_IMAGE|ZUVIX_PODMAN_PULL|ZUVIX_PODMAN_RUN_TIMEOUT|ZUVIX_PODMAN_GATEWAY_HOST_PORT|ZUVIX_GATEWAY_PORT|ZUVIX_PODMAN_BRIDGE_HOST_PORT|ZUVIX_BRIDGE_PORT|ZUVIX_GATEWAY_BIND|ZUVIX_PODMAN_USERNS|ZUVIX_BIND_MOUNT_OPTIONS|ZUVIX_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-zuvix-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${ZUVIX_CONFIG_DIR:-$EFFECTIVE_HOME/.zuvix}"
ENV_FILE="${ZUVIX_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${ZUVIX_CONFIG_DIR:-$EFFECTIVE_HOME/.zuvix}"
ENV_FILE="${ZUVIX_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${ZUVIX_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${ZUVIX_PODMAN_CONTAINER:-zuvix}"
ZUVIX_IMAGE="${ZUVIX_PODMAN_IMAGE:-${ZUVIX_IMAGE:-zuvix:local}}"
PODMAN_PULL="${ZUVIX_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${ZUVIX_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${ZUVIX_PODMAN_GATEWAY_HOST_PORT:-${ZUVIX_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${ZUVIX_PODMAN_BRIDGE_HOST_PORT:-${ZUVIX_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${ZUVIX_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$ZUVIX_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  ZUVIX_CONTAINER="" ZUVIX_CONFIG_DIR="$config_dir" \
    zuvix config get gateway.bind 2>/dev/null || true
}
...
# ZUVIX_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${ZUVIX_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v zuvix >/dev/null 2>&1; then
    echo "Warning: zuvix not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    ZUVIX_CONTAINER="" ZUVIX_CONFIG_DIR="$config_dir" \
      zuvix config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    ZUVIX_CONTAINER="" ZUVIX_CONFIG_DIR="$config_dir" \
      zuvix config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  ZUVIX_CONTAINER="" ZUVIX_CONFIG_DIR="$config_dir" \
    zuvix config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${ZUVIX_GATEWAY_TOKEN:-}" ]]; then
  export ZUVIX_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "ZUVIX_GATEWAY_TOKEN" "$ZUVIX_GATEWAY_TOKEN"
  echo "Generated ZUVIX_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/zuvix.json"
...
PODMAN_USERNS="${ZUVIX_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported ZUVIX_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (ZUVIX_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${ZUVIX_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${ZUVIX_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$ZUVIX_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.zuvix/.npm \
    -e ZUVIX_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.zuvix:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.zuvix/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$ZUVIX_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME zuvix dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME zuvix devices approve --latest  # if pairing required"
