...
read_when:
  - You are moving Zuvix to a new laptop or server
  - You are coming from another agent system and want to keep state
  - You are upgrading an in-place plugin
title: "Migration guide"
---

Zuvix supports three migration paths: importing from another agent system, moving an existing install to a new machine, and upgrading a plugin in place.

## Import from another agent system

Use the bundled migration providers to bring instructions, MCP servers, skills, model config, and (opt-in) API keys into Zuvix. Plans are previewed before any change, secrets are redacted in reports, and apply is backed by a verified backup.
...
The CLI entry point is [`zuvix migrate`](/cli/migrate). Onboarding can also offer migration when it detects a known source (`zuvix onboard --flow import`).

## Move Zuvix to a new machine

Copy the **state directory** (`~/.zuvix/` by default) and your **workspace** to preserve:

- **Config** — `zuvix.json` and all gateway settings.
...
<Tip>
Run `zuvix status` on the old machine to confirm your state directory path. Custom profiles use `~/.zuvix-<profile>/` or a path set via `ZUVIX_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    zuvix gateway stop
    cd ~
    tar -czf zuvix-state.tgz .zuvix
    ```

    If you use multiple profiles (for example `~/.zuvix-work`), archive each separately.

  </Step>

  <Step title="Install Zuvix on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine. It is fine if onboarding creates a fresh `~/.zuvix/`. You will overwrite it next.
  </Step>
...
```bash
awk -F= '/^(TELEGRAM_BOT_TOKEN|DISCORD_BOT_TOKEN)=/ { print $1 "=present" }' ~/.zuvix/.env
```

`zuvix doctor` also warns when an enabled default Telegram or Discord account has no configured token and the matching env variable is unavailable to the doctor process.

### Common pitfalls

<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `ZUVIX_STATE_DIR` and the new one does not, channels will appear logged out and sessions will be empty. Launch the gateway with the **same** profile or state-dir you migrated, then rerun `zuvix doctor`.
  </Accordion>

  <Accordion title="Copying only zuvix.json">
...
- [ ] `zuvix status` shows the gateway running.
...
- [`zuvix migrate`](/cli/migrate): CLI reference for cross-system imports.
- [Install overview](/install): all installation methods.
- [Doctor](/gateway/doctor): post-migration health check.
- [Uninstall](/install/uninstall): removing Zuvix cleanly.
