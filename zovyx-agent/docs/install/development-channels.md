...
Zuvix ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/zuvix`, or
  `$ZUVIX_HOME/zuvix` when `ZUVIX_HOME` is set; override with
  `ZUVIX_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
zuvix update --tag 2026.4.1-beta.1
...
zuvix update --tag zuvix@2026.4.1-beta.1
...
- The tag is not persisted. Your next `zuvix update` uses your configured
  channel as usual.
- For package installs, Zuvix pre-packs GitHub/git source specs into a
...
- Downgrade protection: if the target version is older than your current version,
  Zuvix prompts for confirmation (skip with `--yes`).
...
Preview what `zuvix update` would do without making changes:

```bash
zuvix update --dry-run
zuvix update --channel beta --dry-run
zuvix update --tag 2026.4.1-beta.1 --dry-run
zuvix update --dry-run --json
```
...
When you switch channels with `zuvix update`, Zuvix also syncs plugin
sources:
