---
summary: "ClawDock shell helpers for Docker-based Zuvix installs"
read_when:
  - You run Zuvix with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based Zuvix installs.
...
| `clawdock-cli <command>`  | Run Zuvix CLI commands in Docker           |
...
- `~/.zuvix/.env` for env-backed provider keys and bot tokens
- `~/.zuvix/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.zuvix/zuvix.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `zuvix.json` quickly. It redacts `.env` values in its printed output.

## Related

<CardGroup cols={2}>
  <Card title="Docker" href="/install/docker" icon="docker">
    Canonical Docker install for Zuvix.
  </Card>
  ...
  <Card title="Updating" href="/install/updating" icon="arrow-up-right-from-square">
    Updating the Zuvix package and managed services.
  </Card>
</CardGroup>
