---
summary: "Host Zuvix on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying Zuvix to Upstash Box
  - You want a managed Linux environment for Zuvix with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent Zuvix Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current Zuvix Box walkthrough at
[Zuvix Setup](https://upstash.com/docs/box/guides/zuvix-setup).

## Connect with an SSH tunnel

Forward the Zuvix dashboard port to your local machine. Use your Box API key
...
```bash
zuvix config set gateway.bind lan
nohup zuvix gateway > gateway.log 2>&1 &
```
...
- [Updating Zuvix](/install/updating)
