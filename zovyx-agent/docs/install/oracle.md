---
summary: "Host Zuvix on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Zuvix on Oracle Cloud
  - Looking for free VPS hosting for Zuvix
  - Want 24/7 Zuvix on a small server
title: "Oracle Cloud"
---

Run a persistent Zuvix Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname zuvix
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=zuvix
    ```

    From now on, connect via Tailscale: `ssh ubuntu@zuvix`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    zuvix config set gateway.bind loopback
    zuvix config set gateway.auth.mode token
    zuvix doctor --generate-gateway-token
    zuvix config set gateway.tailscale.mode serve
    zuvix config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://zuvix.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.zuvix` to restrict credential file permissions.
- `zuvix security audit` for an Zuvix-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Zuvix features work fine; a small number of native binaries need ARM builds:
...
- `~/.zuvix/` — `zuvix.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.zuvix/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@zuvix
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=zuvix --reset` to re-authenticate.

**Gateway will not start** -- Run `zuvix doctor --non-interactive` and check logs with `journalctl --user -u zuvix-gateway.service -n 50`.
