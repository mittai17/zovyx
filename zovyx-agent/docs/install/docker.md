---
summary: "Optional Docker-based setup and onboarding for Zuvix"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Zuvix on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export ZUVIX_IMAGE="ghcr.io/zuvix/zuvix:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/zuvix/zuvix/pkgs/container/zuvix).
  ...
  <Step title="Complete onboarding">
    ...
    `zuvix-gateway` directly. `zuvix-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm zuvix-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm zuvix-cli channels login

    # Telegram
    docker compose run --rm zuvix-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm zuvix-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t zuvix:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node zuvix-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node zuvix-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d zuvix-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `ZUVIX_EXTRA_MOUNTS`
or `ZUVIX_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
<Note>
Because `zuvix-cli` shares `zuvix-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d zuvix-gateway`, run onboarding
and setup-time config writes through `zuvix-gateway` with
...
| Variable                                   | Purpose                                                               |
| ------------------------------------------ | --------------------------------------------------------------------- |
| `ZUVIX_IMAGE`                           | Use a remote image instead of building locally                        |
| `ZUVIX_IMAGE_APT_PACKAGES`              | Install extra apt packages during build (space-separated)             |
| `ZUVIX_IMAGE_PIP_PACKAGES`              | Install extra Python packages during build (space-separated)          |
| `ZUVIX_EXTENSIONS`                      | Pre-install plugin dependencies at build time (space-separated names) |
| `ZUVIX_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`)       |
| `ZUVIX_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                         |
| `ZUVIX_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                |
| `ZUVIX_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)       |
| `ZUVIX_DOCKER_SOCKET`                   | Override Docker socket path                                           |
| `ZUVIX_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)         |
| `ZUVIX_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays                     |
...
| `ZUVIX_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded        |

The official Docker image does not ship Homebrew. During onboarding, Zuvix
...
`ZUVIX_IMAGE_APT_PACKAGES` during image build. The legacy
`ZUVIX_DOCKER_APT_PACKAGES` name is still accepted.
For Python dependencies, use `ZUVIX_IMAGE_PIP_PACKAGES`. This runs
...
`ZUVIX_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export ZUVIX_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="zuvix-gateway"
./scripts/docker/setup.sh
```

Install the official `@zuvix/diagnostics-otel` plugin from ClawHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`ZUVIX_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`clawhub:@zuvix/diagnostics-prometheus`, enable the
...
```bash
docker compose exec zuvix-gateway node dist/index.js health --token "$ZUVIX_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `ZUVIX_GATEWAY_BIND=lan` so host access to
...
When Zuvix runs in Docker, `127.0.0.1` inside the container is the container
...
`ZUVIX_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `ZUVIX_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `ZUVIX_CONFIG_DIR` to `/home/node/.zuvix`,
`ZUVIX_WORKSPACE_DIR` to `/home/node/.zuvix/workspace`, and
`ZUVIX_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/zuvix`, so those
...
That mounted config directory is where Zuvix keeps:

- `zuvix.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `ZUVIX_GATEWAY_TOKEN`
...
but separate from `ZUVIX_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
Zuvix home, so plugin install records and package roots survive container
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/zuvix/zuvix/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export ZUVIX_SANDBOX=1
    export ZUVIX_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the Zuvix sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm zuvix-cli gateway probe
    docker compose run -T --rm zuvix-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `zuvix-cli` uses `network_mode: "service:zuvix-gateway"` so CLI
    ...
    `no-new-privileges` on both `zuvix-gateway` and `zuvix-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in zuvix-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `zuvix-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `zuvix plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm zuvix-cli plugins install <package>
    ```

    If you already created a long-running `zuvix-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.zuvix`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/zuvix-config /path/to/zuvix-workspace
    ```
    ...
    `/path/to/zuvix-config/npm` to `root:root` if you intentionally run
    Zuvix as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export ZUVIX_HOME_VOLUME="zuvix_home"`
    2. **Bake system deps**: `export ZUVIX_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export ZUVIX_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export ZUVIX_INSTALL_BROWSER=1`
    5. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm zuvix-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **Persist browser downloads**: use `ZUVIX_HOME_VOLUME` or
       `ZUVIX_EXTRA_MOUNTS`. Zuvix auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/zuvix/zuvix/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Zuvix runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm zuvix-cli dashboard --no-open
    docker compose run --rm zuvix-cli devices list
    docker compose run --rm zuvix-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm zuvix-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm zuvix-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Zuvix up to date
