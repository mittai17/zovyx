---
summary: "Host Zuvix on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up Zuvix on a Raspberry Pi
  - Running Zuvix on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on Zuvix Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well — typical hardware cost is **$35–80 one-time**, no monthly fees.
...
<Steps>
  ...
  <Step title="Verify">
    ```bash
    zuvix status
    systemctl --user status zuvix-gateway.service
    journalctl --user -u zuvix-gateway.service -f
    ```
  </Step>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'zuvix dashboard --no-open'
    ```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/zuvix-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/zuvix-compile-cache
mkdir -p /var/tmp/zuvix-compile-cache
export ZUVIX_NO_RESPAWN=1
EOF
source ~/.bashrc
```

`ZUVIX_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running Zuvix, add a service drop-in:

```bash
systemctl --user edit zuvix-gateway.service
```

```ini
[Service]
Environment=ZUVIX_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/zuvix-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```

Then `systemctl --user daemon-reload && systemctl --user restart zuvix-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
Most Zuvix features work on ARM64 without changes (Node.js, Telegram, WhatsApp/Baileys, Chromium). The binaries that occasionally lack ARM builds are typically optional Go/Rust CLI tools shipped by skills. Verify a missing binary's release page for `linux-arm64` / `aarch64` artifacts before falling back to building from source.

## Persistence and backups

Zuvix state lives under:

- `~/.zuvix/` — `zuvix.json`, per-agent `auth-profiles.json`, channel/provider state, sessions.
- `~/.zuvix/workspace/` — agent workspace (SOUL.md, memory, artifacts).
...
**Service will not start** -- Check logs with `journalctl --user -u zuvix-gateway.service --no-pager -n 100` and run `zuvix doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
...
- [Updating](/install/updating) -- keep Zuvix up to date
