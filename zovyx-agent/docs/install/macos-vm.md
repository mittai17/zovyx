---
summary: "Run Zuvix in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want Zuvix isolated from your main macOS environment
...
Run Zuvix in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
1. Install Lume
2. `lume create zuvix --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run zuvix --no-display`
5. SSH in, install Zuvix, configure channels
6. Done
...
```bash
lume create zuvix --os macos --ipsw latest
```
...
```bash
lume stop zuvix
lume run zuvix --no-display
```

The VM runs in the background. Zuvix's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "zuvix status"
```
...
This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to Zuvix.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running Zuvix/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your Zuvix config:
...
```bash
lume stop zuvix
lume clone zuvix zuvix-golden
```

Reset anytime:

```bash
lume stop zuvix && lume delete zuvix
lume clone zuvix-golden zuvix
lume run zuvix --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get zuvix` again                           |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                    |
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `zuvix channels login` |
