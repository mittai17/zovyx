---
summary: "Deploy Zuvix on Render with Infrastructure-as-Code"
read_when:
  - Deploying Zuvix to Render
...
Deploy Zuvix on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/zuvix/zuvix)
...
repository configures everything needed to run Zuvix:
...
services:
  - type: web
    ...
    envVars:
      - key: ZUVIX_GATEWAY_PORT
        value: "8080"
      - key: ZUVIX_STATE_DIR
        value: /data/.zuvix
      - key: ZUVIX_WORKSPACE_DIR
        value: /data/workspace
      - key: ZUVIX_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Zuvix state
...
`ZUVIX_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Zuvix repository, Render will not auto-deploy your Zuvix. To update it, run a manual Blueprint sync from the dashboard.
...
For Zuvix, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Zuvix state plus any configured
...
- Missing `ZUVIX_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ZUVIX_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `zuvix backup create` in the Render shell.
