---
summary: "Install Zuvix - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Zuvix, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://zuvix.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Zuvix and Node kept under a local prefix such as
`~/.zuvix`, without depending on a system-wide Node install:

```bash
curl -fsSL https://zuvix.ai/install-cli.sh | bash
```
...
`zuvix update --channel dev` and `zuvix update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    for the Zuvix package install. If you install manually with npm, your own
...
Or skip the link and use `pnpm zuvix ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
zuvix --version      # confirm the CLI is available
zuvix doctor         # check for config issues
zuvix gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `zuvix onboard --install-daemon` or `zuvix gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Zuvix on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Zuvix completely.
  </Card>
</CardGroup>

## Troubleshooting: `zuvix` not found

If the install succeeded but `zuvix` is not found in your terminal:
