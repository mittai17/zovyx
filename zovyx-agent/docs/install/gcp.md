---
summary: "Run Zuvix Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Zuvix running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Zuvix Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Zuvix 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Zuvix Gateway in Docker
- Persist `~/.zuvix` + `~/.zuvix/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.zuvix` state includes `zuvix.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-zuvix-project --name="Zuvix Gateway"
    gcloud config set project my-zuvix-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create zuvix-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh zuvix-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Zuvix repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    ZUVIX_IMAGE=zuvix:latest
    ZUVIX_GATEWAY_TOKEN=
    ZUVIX_GATEWAY_BIND=lan
    ZUVIX_GATEWAY_PORT=18789

    ZUVIX_CONFIG_DIR=/home/$USER/.zuvix
    ZUVIX_WORKSPACE_DIR=/home/$USER/.zuvix/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.zuvix
    ```

    Set `ZUVIX_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, Zuvix uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `ZUVIX_GATEWAY_TOKEN`.
    ...
    services:
      zuvix-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - ZUVIX_GATEWAY_BIND=${ZUVIX_GATEWAY_BIND}
          - ZUVIX_GATEWAY_PORT=${ZUVIX_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${ZUVIX_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`ZUVIX_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm zuvix-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh zuvix-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop zuvix-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type zuvix-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start zuvix-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create zuvix-deploy \
     --display-name="Zuvix Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-zuvix-project \
     --member="serviceAccount:zuvix-deploy@my-zuvix-project.iam.gserviceaccount.com" \
