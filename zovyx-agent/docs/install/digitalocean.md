---
summary: "Host Zuvix on a DigitalOcean Droplet"
read_when:
  - Setting up Zuvix on DigitalOcean
  - Looking for a simple paid VPS for Zuvix
title: "DigitalOcean"
---

Run a persistent Zuvix Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own Zuvix state and services.
    adduser zuvix
    usermod -aG sudo zuvix
    loginctl enable-linger zuvix

    su - zuvix
    zuvix --version
    ```

    Use the root shell only for system bootstrap. Run Zuvix commands as the non-root `zuvix` user so state lives under `/home/zuvix/.zuvix/` and the Gateway installs as that user's systemd service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    zuvix config set gateway.bind tailnet
    zuvix gateway restart
    ```
...
DigitalOcean snapshots back the whole Droplet up; `zuvix backup create` is portable across hosts.
