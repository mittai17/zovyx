...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Zuvix
...
Running Zuvix via Node with `tsx` fails at startup with:

```
[zuvix] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `zuvix status` and `gateway:watch` worked.
