...
`zuvix nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`zuvix node run` or the
...
zuvix devices reject <requestId>
zuvix nodes status
...
`zuvix devices list` before approving.
...
- `node.pair.*` (CLI: `zuvix nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `zuvix nodes remove --node <id|name|ip>` deletes stale entries from that
...
- **Approvals**: enforced on the node host via `~/.zuvix/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, Zuvix also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If Zuvix cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
zuvix node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export ZUVIX_GATEWAY_TOKEN="<gateway-token>"
zuvix node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `zuvix node run` supports token or password auth.
- Env vars are preferred: `ZUVIX_GATEWAY_TOKEN` / `ZUVIX_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `ZUVIX_GATEWAY_*` env vars.

### Start a node host (service)

```bash
zuvix node install --host <gateway-host> --port 18789 --display-name "Build Node"
zuvix node start
zuvix node restart
```
...
If the node retries with changed auth details, re-run `zuvix devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `zuvix node run` / `zuvix node install` (persists in `~/.zuvix/node.json` on the node).
- `zuvix nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
zuvix approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
zuvix approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.zuvix/exec-approvals.json`.
...
```bash
zuvix config set tools.exec.host node
zuvix config set tools.exec.security allowlist
zuvix config set tools.exec.node "<id-or-name>"
```
...
```bash
zuvix nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
zuvix nodes canvas snapshot --node <idOrNameOrIp> --format png
zuvix nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
zuvix nodes canvas present --node <idOrNameOrIp> --target https://example.com
zuvix nodes canvas hide --node <idOrNameOrIp>
zuvix nodes canvas navigate https://example.com --node <idOrNameOrIp>
zuvix nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
zuvix nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
zuvix nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
zuvix nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
zuvix nodes camera list --node <idOrNameOrIp>
zuvix nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
zuvix nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
zuvix nodes camera clip --node <idOrNameOrIp> --duration 10s
zuvix nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
zuvix nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
zuvix nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
zuvix nodes location get --node <idOrNameOrIp>
zuvix nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
zuvix nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from Zuvix"}'
```
...
```bash
zuvix nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
zuvix nodes invoke --node <idOrNameOrIp> --command device.apps --params '{"limit":10}'
zuvix nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
zuvix nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
zuvix nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
zuvix nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.zuvix/exec-approvals.json`).
...
```bash
zuvix config set tools.exec.node "node-id-or-name"
```
...
```bash
zuvix config unset tools.exec.node
zuvix config unset 'agents.list[0].tools.exec.node'
```
...
Zuvix can run a **headless node host** (no UI) that connects to the Gateway
...
```bash
zuvix node run --host <gateway-host> --port 18789
```

Notes:

- Pairing is still required (the Gateway will show a device pairing prompt).
- The node host stores its node id, token, display name, and gateway connection info in `~/.zuvix/node.json`.
- Exec approvals are enforced locally via `~/.zuvix/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `ZUVIX_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `ZUVIX_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `zuvix nodes …` works against this Mac).
