...
- `consultThinkingLevel`: optional thinking level override for the full Zuvix agent run behind realtime `zuvix_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `zuvix_agent_consult` calls.
...
- `realtime.consultRouting`: `provider-direct` preserves the provider's direct reply when it skips `zuvix_agent_consult`; `force-agent-consult` makes Gateway relay route finalized user transcripts through Zuvix instead.
- `realtime.instructions`: appends provider-facing system instructions to Zuvix's built-in realtime prompt. Use it for voice style and tone; Zuvix keeps the default `zuvix_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `zuvix_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `zuvix-mlx-tts` helper when present, or an executable on `PATH`. Set `ZUVIX_MLX_TTS_BIN` to point at a custom helper binary during development.
