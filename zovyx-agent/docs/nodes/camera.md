...
Zuvix supports **camera capture** for agent workflows:
...
```bash
zuvix nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
zuvix nodes camera snap --node <id> --facing front
zuvix nodes camera clip --node <id> --duration 3000
zuvix nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`zuvix.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `zuvix` CLI to invoke camera commands on the macOS node.

Examples:

```bash
zuvix nodes camera list --node <id>            # list camera ids
zuvix nodes camera snap --node <id>            # prints saved path
zuvix nodes camera snap --node <id> --max-width 1280
zuvix nodes camera snap --node <id> --delay-ms 2000
zuvix nodes camera snap --node <id> --device-id <id>
zuvix nodes camera clip --node <id> --duration 10s          # prints saved path
zuvix nodes camera clip --node <id> --duration-ms 3000      # prints saved path (legacy flag)
zuvix nodes camera clip --node <id> --device-id <id>
zuvix nodes camera clip --node <id> --no-audio
```

Notes:

- `zuvix nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
zuvix nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```
