...
```bash
zuvix nodes describe --node <idOrNameOrIp>
zuvix nodes canvas snapshot --node <idOrNameOrIp>
zuvix logs --follow
```
...
zuvix nodes status
zuvix approvals get --node <idOrNameOrIp>
zuvix approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`zuvix approvals get --node ...`), not in the gateway pairing record.
