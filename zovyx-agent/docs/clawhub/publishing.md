...
An owner is a ClawHub publisher handle, such as `@alice` or `@zuvix`.
...
`@zuvix/dronzer`, it can only be published as `@zuvix`. If you publish as
...
```text
Package scope "@zuvix" must match selected owner "@vintageayu".
Publish as "@zuvix" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-zuvix --to opik
```
...
This protects org namespaces. A package named `@zuvix/dronzer` claims the
`@zuvix` namespace, so only publishers with access to the `@zuvix` owner
can publish it.
