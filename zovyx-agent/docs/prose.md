...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In Zuvix it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to Zuvix primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In Zuvix it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    zuvix plugins enable open-prose
    ```
...
For a local checkout: `zuvix plugins install ./path/to/local/open-prose-plugin`
...
## Zuvix runtime mapping

OpenProse programs map to Zuvix primitives:

| OpenProse concept         | Zuvix tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review them before running. Use Zuvix tool
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    Zuvix's native multi-agent coordination layer.
  </Card>
