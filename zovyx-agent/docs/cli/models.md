---
summary: "CLI reference for `zuvix models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
zuvix models status
zuvix models list
zuvix models set <model-or-alias>
zuvix models scan
```

`zuvix models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Zuvix falls back to matching OAuth/API-key
...
the command uses `ZUVIX_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For OpenAI ChatGPT/Codex OAuth troubleshooting, `zuvix models status`,
`zuvix models auth list --provider openai`, and
`zuvix config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, Zuvix resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, Zuvix
...
By default Zuvix tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `ZUVIX_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Zuvix could not resolve a probeable
...
```bash
zuvix models auth add
zuvix models auth list [--provider <id>] [--json]
zuvix models auth login --provider <id>
zuvix models auth login --provider openai --profile-id openai:work
zuvix models auth paste-api-key --provider <id>
zuvix models auth setup-token --provider <id>
zuvix models auth paste-token
```
...
`zuvix plugins list` to see which providers are installed.
Use `zuvix models auth --agent <id> <subcommand>` to write auth results to a
...
usually as a backup for Codex subscription limits. Run `zuvix doctor --fix`
to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
zuvix models auth login --provider openai --set-default
zuvix models auth login --provider openai --method api-key
zuvix models auth paste-api-key --provider openai
...
- `paste-api-key` accepts API keys generated elsewhere, prompts for the key
  ...
  `printf "%s\n" "$OPENAI_API_KEY" | zuvix models auth paste-api-key --provider openai`.
...
- Anthropic note: Anthropic staff told us Zuvix-style Claude CLI usage is allowed again, so Zuvix treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported Zuvix token path, but Zuvix now prefers Claude CLI reuse and `claude -p` when available.
