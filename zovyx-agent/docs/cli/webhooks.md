---
summary: "CLI reference for `zuvix webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into Zuvix
...
```bash
zuvix webhooks gmail setup --account <email> [...]
zuvix webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `gmail setup` | Configure Gmail watch, Pub/Sub topic/subscription, and the Zuvix webhook delivery target. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop.                                        |

## `webhooks gmail setup`

Configure Gmail watch, Pub/Sub, and Zuvix webhook delivery.

```bash
zuvix webhooks gmail setup --account you@example.com
zuvix webhooks gmail setup --account you@example.com --project my-gcp-project --json
zuvix webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```
...
### Zuvix delivery options

| Flag                   | Default | Description                                |
| ---------------------- | ------- | ------------------------------------------ |
| `--hook-url <url>`     | (none)  | Zuvix webhook URL.                      |
| `--hook-token <token>` | (none)  | Zuvix webhook token.                    |
...
```bash
zuvix webhooks gmail run --account you@example.com
```

`run` accepts the same `gog watch serve`, Zuvix delivery, Pub/Sub, and Tailscale flags as `setup`, except:
...
| Zuvix delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |
