---
summary: "CLI reference for `zuvix secrets` (reload, audit, configure, apply)"
read_when:
...
Use `zuvix secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
zuvix secrets reload
zuvix secrets reload --json
zuvix secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Zuvix state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `zuvix.json` refs)
...
```bash
zuvix secrets configure
zuvix secrets configure --plan-out /tmp/zuvix-secrets-plan.json
zuvix secrets configure --apply --yes
zuvix secrets configure --providers-only
zuvix secrets configure --skip-provider-setup
zuvix secrets configure --agent ops
zuvix secrets configure --json
```
...
- `configure` targets secret-bearing fields in `zuvix.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Zuvix fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
zuvix secrets apply --from /tmp/zuvix-secrets-plan.json --json
```
...
- `zuvix.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.zuvix/.env` known secret keys whose values were migrated
