---
summary: "CLI reference for `zuvix workboard` cards, dispatch, and worker runs"
read_when:
...
`zuvix workboard` is the terminal surface for the bundled
...
```bash
zuvix plugins enable workboard
...
```bash
zuvix workboard list [--board <id>] [--status <status>] [--json]
zuvix workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
zuvix workboard show <id> [--json]
zuvix workboard dispatch [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
zuvix workboard list
zuvix workboard list --board default --status ready
zuvix workboard list --json
```
...
```bash
zuvix workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
zuvix workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
zuvix workboard show 7f4a2c10
zuvix workboard show 7f4a2c10 --json
```
...
```bash
zuvix workboard dispatch
zuvix workboard dispatch --json
zuvix workboard dispatch --url http://127.0.0.1:18789 --token "$ZUVIX_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local Zuvix state
directory used by the current profile. Use `--dev` or `--profile <name>` on the
top-level `zuvix` command when you need a different state root.
...
```bash
zuvix plugins inspect workboard --runtime --json
```
...
Then retry `zuvix workboard dispatch`. Data-only fallback is useful for local
...
```bash
zuvix workboard list --status ready
```
