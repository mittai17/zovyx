---
summary: "CLI reference for `zuvix devices` (device pairing + token rotation/revocation)"
read_when:
...
```
zuvix devices remove <deviceId>
zuvix devices remove <deviceId> --json
```

### `zuvix devices clear --yes [--pending]`

Clear paired devices in bulk.

```
zuvix devices clear --yes
zuvix devices clear --yes --pending
zuvix devices clear --yes --pending --json
```

### `zuvix devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Zuvix only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Zuvix supersedes the previous pending entry and issues a new `requestId`. Run `zuvix devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
Zuvix keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `zuvix devices list`
or use `zuvix devices approve --latest` to preview the exact upgrade before
approving it.
...
```
zuvix devices approve
zuvix devices approve <requestId>
zuvix devices approve --latest
```

## Paperclip / `zuvix_gateway` first-run approval

When a new Paperclip agent connects through the `zuvix_gateway` adapter for the first time, the Gateway may require a one-time device pairing approval before runs can succeed. If Paperclip reports `zuvix_gateway_pairing_required`, approve the pending device and retry.
...
The preview prints the exact `zuvix devices approve <requestId>` command. Verify the request details, then rerun that command with the request ID to approve it.

For remote gateways or explicit credentials, pass the same options while previewing and approving:

```bash
zuvix devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `zuvix devices list` first to confirm a pending request exists.
...
### `zuvix devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
zuvix devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `zuvix devices revoke --device <id> --role <role>`
...
```
zuvix devices revoke --device <deviceId> --role node
```
...
```bash
zuvix devices rotate --device <deviceId> --role operator
```
