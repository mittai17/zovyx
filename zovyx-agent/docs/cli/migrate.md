---
summary: "CLI reference for `zuvix migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into Zuvix
...
```bash
zuvix migrate list
zuvix migrate claude --dry-run
zuvix migrate codex --dry-run
zuvix migrate codex --skill gog-vault77-google-workspace
zuvix migrate codex --plugin google-calendar --dry-run
zuvix migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
zuvix migrate hermes --dry-run
zuvix migrate hermes
zuvix migrate apply codex --yes --skill gog-vault77-google-workspace
zuvix migrate apply codex --yes --plugin google-calendar
zuvix migrate apply codex --yes
zuvix migrate apply claude --yes
zuvix migrate apply hermes --yes
zuvix migrate apply hermes --include-secrets --yes
zuvix onboard --flow import
zuvix onboard --import-from claude --import-source ~/.claude
zuvix onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `zuvix migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local Zuvix state exists.
</ParamField>
...
`zuvix migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `zuvix migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an Zuvix backup before applying the migration. If no local Zuvix state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the Zuvix agent workspace.
...
- Claude command Markdown files converted into Zuvix skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. Zuvix does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the Zuvix Codex harness and you want to
...
Running `zuvix migrate codex` in an interactive terminal previews the full
...
start checked unless the target Zuvix Codex plugin config already has that
...
```bash
zuvix migrate codex --dry-run --skill gog-vault77-google-workspace
zuvix migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
zuvix migrate codex --dry-run --plugin google-calendar
zuvix migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  Zuvix agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to Zuvix provider runs,
...
- `SOUL.md` and `AGENTS.md` into the Zuvix agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for Zuvix file memory, plus archive or manual-review items for external memory providers such as Honcho.
- Skills that include a `SKILL.md` file under `skills/<name>/`.
- Per-skill config values from `skills.config`.
- Zuvix OpenAI OAuth credentials from Zuvix `auth.json` when interactive credential migration is accepted, or when `--include-secrets` is set. Hermes `auth.json` OAuth entries are legacy state reported for manual OpenAI reauth or doctor repair.
- Supported API keys and tokens from Hermes `.env` and Zuvix `auth.json` when interactive credential migration is accepted, or when `--include-secrets` is set.
...
Hermes state that Zuvix cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live Zuvix config or credentials. This preserves opaque or unsafe state without pretending Zuvix can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `zuvix.plugin.json`:
...
Provider plugins can use `zuvix/plugin-sdk/migration` for item construction and summary counts, plus `zuvix/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `zuvix onboard --flow import` and `zuvix setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh Zuvix setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move Zuvix to a new machine.
