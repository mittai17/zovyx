---
summary: "Zuvix CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `zuvix` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`zuvix` is the main CLI entry point. Each core command has either a
...
- `zuvix setup` creates the baseline config and workspace without walking the full guided onboarding flow.
- `zuvix onboard` is the full guided first-run path for gateway, model auth, workspace, channels, skills, and health.
- `zuvix configure` changes targeted parts of an existing setup, such as model auth, gateway, channels, plugins, or skills.
- `zuvix channels add` configures channel accounts after the baseline exists; run it without flags for guided channel setup or with channel-specific flags for scripts.
...
| Flag                    | Purpose                                                               |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.zuvix-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.zuvix-<name>`                              |
| `--container <name>`    | Target a named container for execution                                |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                  |
| `--update`              | Shorthand for [`zuvix update`](/cli/update) (source installs only) |
...
```
zuvix [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
[`zuvix workboard`](/cli/workboard) or `zuvix voicecall`.
...
`zuvix status --usage` and the Control UI surface provider usage/quota when
