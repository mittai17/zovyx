---
summary: "CLI reference for `zuvix policy` conformance checks"
read_when:
  - You want to check Zuvix settings against an authored policy.jsonc
...
`zuvix policy` is provided by the bundled Policy plugin. Policy is an
enterprise conformance layer over existing Zuvix settings. It does not add a
second configuration system. `policy.jsonc` defines authored requirements,
Zuvix observes the active workspace as evidence, and policy health checks
...
data-handling posture, Zuvix config secret provider/auth profile posture, and governed tool
...
posture, require Zuvix config SecretRefs to use managed providers, require
...
repeatable way to prove that Zuvix still conforms to that statement. Use
...
exposure, agent workspace posture, configured sandbox runtime posture, Zuvix
...
when a concrete rule is present. Zuvix reads current `channels.*` settings
...
posture, Zuvix config agent sandbox workspace access and tool deny posture,
...
`edit`, and `apply_patch`; Zuvix config `group:fs` covers file mutation tools
...
present in `policy.jsonc`. The observed state is existing Zuvix config or
...
present in `agents.list[]`, Zuvix evaluates the scoped rule against inherited
...
Container posture policy is evaluated only against evidence Zuvix can
...
```bash
zuvix policy check
zuvix policy check --json
zuvix policy check --severity-min error
```

`policy check` runs only the policy check set and emits evidence, findings, and
attestation hashes. The same findings also appear in `zuvix doctor --lint`
when the Policy plugin is enabled.

Compare an operator policy file to an authored baseline policy file:

```bash
zuvix policy compare --baseline official.policy.jsonc
zuvix policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```

`policy compare` compares policy file syntax to policy file syntax. It does not
inspect Zuvix runtime state, evidence, credentials, or secrets. The command
...
```json
{
  ...
  "evidence": {
    "channels": [
      {
        "id": "telegram",
        "provider": "telegram",
        "source": "oc://zuvix.config/channels/telegram",
        "enabled": false
      }
    ],
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://zuvix.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://zuvix.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.5",
        "source": "oc://zuvix.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://zuvix.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://zuvix.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://zuvix.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://zuvix.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://zuvix.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed Zuvix state used by the policy checks. The
...
5. Re-run `zuvix doctor --lint` in CI or release gates.
...
`zuvix policy watch` runs the same check repeatedly and reports when the
current evidence no longer matches `expectedAttestationHash`:

```bash
zuvix policy watch --json
```
...
```json
{
  ...
  "target": "oc://zuvix.config/mcp/servers/remote",
...
```json
{
  ...
  "target": "oc://zuvix.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://zuvix.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "target": "oc://zuvix.config/gateway/bind",
...
```json
{
  ...
  "target": "oc://zuvix.config/agents/defaults/sandbox/workspaceAccess",
...
In this version, repair can disable channels that are enabled in Zuvix config
