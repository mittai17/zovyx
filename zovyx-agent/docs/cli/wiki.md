---
summary: "CLI reference for `zuvix wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `zuvix wiki`
title: "Wiki"
---
...
Use `zuvix wiki` when you want a compiled knowledge vault with:
...
zuvix wiki doctor
zuvix wiki init
zuvix wiki ingest ./notes/alpha.md
zuvix wiki okf import ./knowledge-catalog/okf/bundles/ga4
zuvix wiki compile
zuvix wiki lint
zuvix wiki search "alpha"
zuvix wiki search "who should I ask about Teams?" --mode route-question
zuvix wiki get entity.alpha --from 1 --lines 80

zuvix wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

zuvix wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

zuvix wiki bridge import
zuvix wiki unsafe-local import

zuvix wiki obsidian status
zuvix wiki obsidian search "alpha"
zuvix wiki obsidian open syntheses/alpha-summary.md
zuvix wiki obsidian command workspace:quick-switcher
zuvix wiki obsidian daily
```
...
```bash
zuvix wiki okf import ./bundles/ga4
zuvix wiki okf import ./bundles/ga4 --json
zuvix wiki search "BigQuery Table" --mode source-evidence --json
zuvix wiki get <path-from-json-result>
```
...
- `.zuvix-wiki/cache/agent-digest.json`
- `.zuvix-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `zuvix memory search` when the
...
```bash
zuvix wiki search "bgroux" --mode find-person
zuvix wiki search "who knows Teams rollout?" --mode route-question
zuvix wiki search "maintainer-whois" --mode source-evidence
zuvix wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
zuvix wiki get entity.alpha
zuvix wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`zuvix wiki` behavior is shaped by:
