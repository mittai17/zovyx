---
summary: "CLI reference for `zuvix agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
zuvix agent --to +15555550123 --message "status update" --deliver
zuvix agent --agent ops --message "Summarize logs"
zuvix agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
zuvix agent --session-key agent:ops:incident-42 --message "Summarize status"
zuvix agent --agent ops --session-key incident-42 --message "Summarize status"
zuvix agent --session-id 1234 --message "Summarize inbox" --thinking medium
zuvix agent --to +15555550123 --message "Trace logs" --verbose on --json
zuvix agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
zuvix agent --agent ops --message "Run locally" --local
```
