---
summary: "CLI reference for `zuvix configure` (interactive configuration prompts)"
read_when:
...
Use `zuvix onboard` for the full guided first-run journey, `zuvix setup` for the baseline config/workspace only, and `zuvix channels add` when you only need channel account setup.
...
<Tip>
`zuvix config` without a subcommand opens the same wizard. Use `zuvix config get|set|unset` for non-interactive edits.
</Tip>

For web search, `zuvix configure --section web` lets you choose a provider
...
zuvix configure --section model --section channels
zuvix configure --section gateway --section daemon
```
