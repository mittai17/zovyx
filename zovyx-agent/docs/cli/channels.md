---
summary: "CLI reference for `zuvix channels` (accounts, status, login/logout, logs)"
read_when:
...
Do not use `zuvix sessions`, Gateway `sessions.list`, or the agent
...
```bash
zuvix channels add --channel telegram --token <bot-token>
zuvix channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
zuvix channels remove --channel telegram --delete
```

<Tip>
`zuvix channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, Zuvix uses the channel's default install source without opening the interactive plugin install prompt.

When you run `zuvix channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `zuvix agents bindings`, `zuvix agents bind`, and `zuvix agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, Zuvix promotes account-scoped top-level values into the channel's account map before writing the new account. Most channels land those values in `channels.<channel>.accounts.default`, but bundled channels can preserve an existing matching promoted account instead. Matrix is the current example: if one named account already exists, or `defaultAccount` points at an existing named account, promotion preserves that account instead of creating a new `accounts.default`.
...
- Run `zuvix status --deep` for a broad probe.
- Use `zuvix doctor` for guided fixes.
- `zuvix channels list` no longer prints model provider usage/quota snapshots. For those, use `zuvix status` (overview) or `zuvix models list` (per-provider).
- `zuvix channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
zuvix channels resolve --channel slack "#general" "@jane"
zuvix channels resolve --channel discord "My Server/#support" "@someone"
zuvix channels resolve --channel matrix "Project Room"
```
