---
summary: "CLI reference for `zuvix dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You're setting up split DNS for a custom discovery domain (example: zuvix.internal)
title: "DNS"
---
...
```bash
zuvix dns setup
zuvix dns setup --domain zuvix.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `zuvix.internal`)
...
- zone file path
- current tailnet IPs
- recommended `zuvix.json` discovery config
...
- If `--domain` is omitted, Zuvix uses `discovery.wideArea.domain` from config.
