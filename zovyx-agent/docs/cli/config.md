---
summary: "CLI reference for `zuvix config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `zuvix.json`: get/set/patch/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `zuvix configure`).

<Note>
When `ZUVIX_NIX_MODE=1`, Zuvix treats `zuvix.json` as immutable. Read-only commands such as `config get`, `config file`, `config schema`, and `config validate` still work, but config writers refuse. Agents should edit the Nix source for the install instead; for the first-party nix-zuvix distribution, use [nix-zuvix Quick Start](https://github.com/zuvix/nix-zuvix#quick-start) and set values under `programs.zuvix.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `zuvix config` without a subcommand.
</ParamField>
...
```bash
zuvix config file
zuvix config --section model
zuvix config --section gateway --section daemon
zuvix config schema
zuvix config get browser.executablePath
...
zuvix config set agents.defaults.heartbeat.every "2h"
zuvix config set 'agents.list[0].tools.exec.node' "node-id-or-name"
zuvix config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
zuvix config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
zuvix config set secrets.providers.vaultfile --provider-source file --provider-path /etc/zuvix/secrets.json --provider-mode json
zuvix config patch --file ./zuvix.patch.json5 --dry-run
zuvix config unset plugins.entries.brave.config.webSearch.apiKey
zuvix config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
zuvix config validate
zuvix config validate --json
```

### `config schema`

Print the generated JSON schema for `zuvix.json` to stdout as JSON.
...
```bash
zuvix config schema > zuvix.schema.json
```

### Paths

Paths use dot or bracket notation. Quote bracket-notation paths in shell examples so shells such as zsh do not expand `[0]` as a glob before Zuvix receives the path:

```bash
zuvix config get agents.defaults.workspace
zuvix config get 'agents.list[0].id'
```

Use the agent list index to target a specific agent:

```bash
zuvix config get agents.list
zuvix config set 'agents.list[1].tools.exec.node' "node-id-or-name"
```
...
```bash
zuvix config set agents.defaults.heartbeat.every "0m"
zuvix config set gateway.port 19001 --strict-json
zuvix config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
zuvix config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
zuvix config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`zuvix config set` supports four assignment styles:

<Tabs>
  <Tab title="Value mode">
    ```bash
    zuvix config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    zuvix config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    zuvix config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/zuvix-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ...
    ```bash
    zuvix config set --batch-file ./config-set.batch.json --dry-run
    ```
...
```bash
zuvix config patch --file ./zuvix.patch.json5 --dry-run
zuvix config patch --file ./zuvix.patch.json5
```

You can also pipe a patch over stdin, which is useful for remote setup scripts:

```bash
ssh zuvix-host 'zuvix config patch --stdin --dry-run' < ./zuvix.patch.json5
ssh zuvix-host 'zuvix config patch --stdin' < ./zuvix.patch.json5
```
...
```bash
zuvix config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
zuvix config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/zuvix/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `zuvix.json`.
...
`zuvix config set` and other Zuvix-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `zuvix.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `zuvix.json` layouts are unsupported for writes; use `ZUVIX_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
zuvix config set gateway.reload.mode hybrid --dry-run
zuvix config set gateway.reload.mode hybrid
zuvix config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `zuvix.json`. Run `zuvix doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- `config file`: Print the active config file path (resolved from `ZUVIX_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `zuvix config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `zuvix configure` or `zuvix doctor --fix`. `zuvix chat` does not bypass the invalid-config guard.
</Note>
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `zuvix config set` or `zuvix configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `zuvix config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `zuvix doctor` or `zuvix doctor --fix` for migration and repair help.
  </Step>
</Steps>
