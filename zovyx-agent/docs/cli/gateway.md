---
summary: "Zuvix Gateway CLI (`zuvix gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Zuvix's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `zuvix gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How Zuvix advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.zuvix/zuvix.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `zuvix onboard --mode local` and `zuvix setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `ZUVIX_GATEWAY_TOKEN` for the process).
</ParamField>
...
<ParamField path="--bind custom + gateway.customBindHost" type="string">
  Expects an IPv4 address today. For IPv6-only BYOH, place an IPv4 sidecar or proxy in front of the Gateway and point Zuvix at that IPv4 endpoint.
</ParamField>
...
zuvix gateway restart --safe
zuvix gateway restart --safe --skip-deferral
zuvix gateway restart --force
```

`zuvix gateway restart --safe` asks the running Gateway to preflight active Zuvix work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`zuvix gateway restart --safe --skip-deferral` runs the same Zuvix-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `ZUVIX_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `ZUVIX_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `ZUVIX_DIAGNOSTICS=timeline` with `ZUVIX_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `ZUVIX_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
zuvix gateway health --url ws://127.0.0.1:18789
```
...
```bash
zuvix gateway usage-cost
zuvix gateway usage-cost --days 7
zuvix gateway usage-cost --json
```
...
```bash
zuvix gateway stability
zuvix gateway stability --type payload.large
zuvix gateway stability --bundle latest
zuvix gateway stability --bundle latest --export
zuvix gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, Zuvix writes the same diagnostic snapshot to `~/.zuvix/logs/stability/zuvix-stability-*.json` when the recorder has events. Inspect the newest bundle with `zuvix gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
zuvix gateway diagnostics export --output zuvix-diagnostics.zip
zuvix gateway diagnostics export --json
```
...
zuvix gateway status --require-rpc
```
...
<AccordionGroup>
  ...
  <Accordion title="Common warning codes">
    ...
    - `multiple_gateways`: distinct gateway identities were reachable, or Zuvix could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway does not trigger this warning.
...
```bash
zuvix gateway call status
zuvix gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `zuvix` or Node with those args.

```bash
cat > ~/.local/bin/zuvix-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- zuvix "$@"
EOF
chmod +x ~/.local/bin/zuvix-doppler

zuvix gateway install --wrapper ~/.local/bin/zuvix-doppler --force
zuvix gateway restart
```
...
`ZUVIX_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
ZUVIX_WRAPPER="$HOME/.local/bin/zuvix-doppler" zuvix gateway install --force
zuvix doctor
```

To remove a persisted wrapper, clear `ZUVIX_WRAPPER` while reinstalling:

```bash
ZUVIX_WRAPPER= zuvix gateway install --force
zuvix gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active Zuvix work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the Zuvix-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `ZUVIX_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `ZUVIX_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_zuvix-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `zuvix.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
zuvix gateway discover --timeout 4000
zuvix gateway discover --json | jq '.beacons[].wsUrl'
```
