---
summary: "CLI reference for `zuvix qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
zuvix qr
zuvix qr --setup-code-only
zuvix qr --json
zuvix qr --remote
zuvix qr --url wss://gateway.example/ws
```
...
- With `--remote`, Zuvix requires either `gateway.remote.url` or
