...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Zuvix Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`zuvix acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Zuvix channel
conversations instead of hosting an ACP harness session, use
[`zuvix mcp serve`](/cli/mcp) instead.
...
- Zuvix acts as an ACP server
- an IDE or ACP client connects to Zuvix
- Zuvix forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Zuvix runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Zuvix: use `zuvix acp`
- Zuvix should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Zuvix parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Zuvix gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
zuvix acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
zuvix acp --url wss://gateway-host:18789 --token-file ~/.zuvix/gateway.token

# Attach to an existing session key
zuvix acp --session agent:main:main

# Attach by label (must already exist)
zuvix acp --session-label "support inbox"

# Reset the session key before the first prompt
zuvix acp --session agent:main:main --reset-session
```
...
zuvix acp client --server-args --url wss://gateway-host:18789 --token-file ~/.zuvix/gateway.token

# Override the server command (default: zuvix)
zuvix acp client --server "node" --server-args zuvix.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Zuvix through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`zuvix acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `zuvix gateway call sessions.list` as the only ACP proof. That
...
it to drive an Zuvix Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `zuvix acp` over stdio.

Example config (persisted):

```bash
zuvix config set gateway.remote.url wss://gateway-host:18789
zuvix config set gateway.remote.token <token>
```
...
```bash
zuvix acp --session agent:main:main
zuvix acp --session agent:design:main
zuvix acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Zuvix plugin tools or selected
...
[Zuvix tools MCP bridge](/tools/acp-agents-setup#zuvix-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Zuvix bot over ACP, use `acpx` with its built-in `zuvix` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx zuvix` at `zuvix acp`.
3. Target the Zuvix session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Zuvix ACP session
acpx zuvix exec "Summarize the active Zuvix session state."

# Persistent named session for follow-up turns
acpx zuvix sessions ensure --name codex-bridge
acpx zuvix -s codex-bridge --cwd /path/to/repo \
  "Ask my Zuvix work agent for recent context relevant to this repo."
```

If you want `acpx zuvix` to target a specific Gateway and session key every
time, override the `zuvix` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "zuvix": {
      "command": "env ZUVIX_HIDE_BANNER=1 ZUVIX_SUPPRESS_NOTES=1 zuvix acp --url ws://127.0.0.1:18789 --token-file ~/.zuvix/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Zuvix checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env ZUVIX_HIDE_BANNER=1 ZUVIX_SUPPRESS_NOTES=1 node zuvix.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Zuvix agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Zuvix ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`ZUVIX_GATEWAY_TOKEN`, `ZUVIX_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`ZUVIX_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `ZUVIX_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `zuvix acp client` sets `ZUVIX_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `zuvix`).
