---
summary: "Expose Zuvix channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Zuvix-backed channels
  - Running `zuvix mcp serve`
  - Managing Zuvix-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`zuvix mcp` has two jobs:

- run Zuvix as an MCP server with `zuvix mcp serve`
- manage Zuvix-owned outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

In other words:

- `serve` is Zuvix acting as an MCP server
- the other subcommands are Zuvix acting as an MCP client-side registry for MCP servers its runtimes may consume later

Use [`zuvix acp`](/cli/acp) when Zuvix should host a coding harness session itself and route that runtime through ACP.

## Choose the right MCP path

Zuvix has several MCP surfaces. Pick the one that matches who owns the agent runtime and who owns the tools.

| Goal                                                                | Use                                                                  | Why                                                                                                             |
| ------------------------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Let an external MCP client read/send Zuvix channel conversations | `zuvix mcp serve`                                                 | Zuvix is the MCP server and exposes Gateway-backed conversations over stdio.                                 |
| Save third-party MCP servers for Zuvix-managed agent runs        | `zuvix mcp add`, `set`, `configure`, `tools`, `login`             | Zuvix is the MCP client-side registry and later projects those servers into eligible runtimes.               |
| Check a saved server without running an agent turn                  | `zuvix mcp status`, `doctor`, `probe`                             | `status` and `doctor` inspect config; `probe` opens a live MCP connection and lists capabilities.               |
...
| Run ACP-hosted harness sessions                                     | [`zuvix acp`](/cli/acp) and [ACP Agents](/tools/acp-agents-setup) | ACP bridge mode does not accept per-session MCP server injection; configure gateway/plugin bridges instead.     |

<Tip>
If you are not sure which path you need, start with `zuvix mcp status --verbose`. It shows what Zuvix has saved without starting any MCP servers.
</Tip>

## Zuvix as an MCP server

This is the `zuvix mcp serve` path.

### When to use `serve`

Use `zuvix mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to Zuvix-backed channel conversations
- you already have a local or remote Zuvix Gateway with routed sessions
- you want one MCP server that works across Zuvix's channel backends instead of running separate per-channel bridges

Use [`zuvix acp`](/cli/acp) instead when Zuvix should host the coding runtime itself and keep the agent session inside Zuvix.

### How it works

`zuvix mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote Zuvix Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `zuvix mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the Zuvix Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `zuvix agent` and `zuvix infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by Zuvix (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when Zuvix already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    zuvix mcp serve --url wss://gateway-host:18789 --token-file ~/.zuvix/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    zuvix mcp serve --url wss://gateway-host:18789 --password-file ~/.zuvix/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    zuvix mcp serve --verbose
    zuvix mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the Zuvix equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying Zuvix channel configuration
...
Zuvix ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `zuvix mcp serve`
...
## Zuvix as an MCP client registry

This is the `zuvix mcp list`, `show`, `status`, `doctor`, `probe`, `add`, `set`,
`configure`, `tools`, `login`, `logout`, `reload`, and `unset` path.

These commands do not expose Zuvix over MCP. They manage Zuvix-owned MCP server definitions under `mcp.servers` in Zuvix config.

Those saved definitions are for runtimes that Zuvix launches or configures later, such as embedded Zuvix and other runtime adapters. Zuvix stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write Zuvix config
    ...
    - embedded Zuvix exposes configured MCP tools in normal `coding` and `messaging` tool profiles; `minimal` still hides them, and `tools.deny: ["bundle-mcp"]` disables them explicitly
    - per-server `toolFilter.include` and `toolFilter.exclude` filter discovered MCP tools before they become Zuvix tools
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Zuvix consumes Zuvix `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
Zuvix projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific Zuvix
...
Zuvix strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.

### Saved MCP server definitions

Zuvix also stores a lightweight MCP server registry in config for surfaces that want Zuvix-managed MCP definitions.

Commands:

- `zuvix mcp list`
- `zuvix mcp show [name]`
- `zuvix mcp status [--verbose]`
- `zuvix mcp doctor [name] [--probe]`
- `zuvix mcp probe [name]`
- `zuvix mcp add <name> [flags]`
- `zuvix mcp set <name> <json>`
- `zuvix mcp configure <name> [flags]`
- `zuvix mcp tools <name> [--include csv] [--exclude csv] [--clear]`
- `zuvix mcp login <name> [--code code]`
- `zuvix mcp logout <name>`
- `zuvix mcp reload`
- `zuvix mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `zuvix mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
zuvix mcp list
zuvix mcp show context7 --json
zuvix mcp status --verbose
zuvix mcp doctor --probe
zuvix mcp probe context7 --json
zuvix mcp add memory --command npx --arg -y --arg @modelcontextprotocol/server-memory
zuvix mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
zuvix mcp tools context7 --include 'resolve-library-id,get-library-docs'
zuvix mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
zuvix mcp configure docs --timeout 20 --connect-timeout 5 --include 'search,read_*'
zuvix mcp configure docs --auth oauth --oauth-scope 'docs.read'
zuvix mcp login docs
zuvix mcp logout docs
zuvix mcp unset context7
```

### Common server recipes

These examples save server definitions only. Run `zuvix mcp doctor --probe` afterward to prove that the server starts and exposes tools.
...
<Tabs>
  ...
  <Tab title="Memory">
    ```bash
    zuvix mcp add memory \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-memory
    zuvix mcp probe memory --json
    ```
  ...
  <Tab title="Desktop/CUA">
    ```bash
    zuvix mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
    zuvix mcp tools cua-driver --include 'list_apps,observe,click,type'
    zuvix mcp doctor cua-driver --probe
    ```
...
<AccordionGroup>
  <Accordion title="status --json">
    ```json
    {
      "path": "/home/user/.zuvix/zuvix.json",
      "servers": [
        {
          "name": "docs",
          "configured": true,
      ...
      "servers": [
        {
          ...
          "issues": [
            {
              "level": "error",
              "message": "OAuth credentials are not authorized; run zuvix mcp login docs"
            }
          ]
        }
      ]
...
| `auth: "oauth"`                | Use MCP OAuth token storage and `zuvix mcp login`             |
...
Sensitive values in `url` (userinfo) and `headers` are redacted in logs and status output. `zuvix mcp doctor` warns when sensitive-looking `headers` or `env` entries contain literal values, so operators can move those values out of committed config.
...
<Steps>
  <Step title="Save the server">
    ...
    ```bash
    zuvix mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"scope":"docs.read"}}'
    ```
  ...
  <Step title="Start login">
    ...
    Zuvix prints the authorization URL and stores temporary OAuth verifier state under the Zuvix state directory.

  </Step>
  <Step title="Finish with the code">
    After approving in the browser, pass the returned code back to Zuvix.

    ```bash
    zuvix mcp login docs --code abc123
    ```
...
If the provider rotates tokens or the authorization state gets stuck, run `zuvix mcp logout <name>`, then repeat `login`. `logout` can clear credentials for a saved HTTP server even after `auth: "oauth"` has been removed from config, as long as the server name and URL still identify the credential store entry.
...
| ------------------------------ | -------------------------------------------------------------------------------------- |
| `url`                          | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`                    | Set to `"streamable-http"` to select this transport; when omitted, Zuvix uses `sse` |
| `headers`                      | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs`          | Per-server connection timeout in ms (optional)                                         |
| `connectTimeout`               | Per-server connection timeout in seconds (optional)                                    |
| `timeout` / `requestTimeoutMs` | Per-server MCP request timeout in seconds or ms                                        |
...
Zuvix config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `zuvix mcp set` and repaired by `zuvix doctor --fix` in existing config, but `transport` is what embedded Zuvix consumes directly.
...
Use the page for operator edits and quick inventory. Use `zuvix mcp doctor --probe` or `zuvix mcp probe` when you need live server proof.
...
7. Run `zuvix mcp doctor --probe` when you need live proof that the edited server starts and lists tools.
...
- active runtimes may need `zuvix mcp reload`, Gateway config publish, or process restart depending on which process owns the MCP clients
