---
summary: "CLI reference for `zuvix sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `zuvix channels status --probe`,
`zuvix status --deep`, or `zuvix health --verbose` when you need live
channel connectivity.

`zuvix sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
zuvix sessions
zuvix sessions --agent work
zuvix sessions --all-agents
zuvix sessions --active 120
zuvix sessions --limit 25
zuvix sessions --verbose
zuvix sessions --json
```
...
```bash
zuvix sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
zuvix sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.zuvix/trajectory-exports/` under the selected workspace.

`zuvix sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`zuvix sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.zuvix/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.zuvix/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
zuvix sessions cleanup --agent work --dry-run
zuvix sessions cleanup --all-agents --dry-run
zuvix sessions cleanup --enforce
zuvix sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
zuvix sessions cleanup --dry-run --fix-dm-scope
zuvix sessions cleanup --json
```

`zuvix sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `zuvix sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run history, which is managed by `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`zuvix sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.zuvix/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.zuvix/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
