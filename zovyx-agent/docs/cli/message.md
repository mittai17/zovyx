---
summary: "CLI reference for `zuvix message` (send + channel actions)"
read_when:
...
```
zuvix message <subcommand> [flags]
```
...
- `zuvix message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, Zuvix will attempt a live directory lookup when the provider supports it.
...
- `zuvix message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Zuvix does not force a `default` account SecretRef scope
...
```
zuvix message send --channel discord \
...
```bash
zuvix message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
zuvix message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
zuvix message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
zuvix message poll --channel msteams \
...
```
zuvix message react --channel slack \
...
```
zuvix message react --channel signal \
...
```
zuvix message send --channel telegram --target @mychat --message "Choose:" \
...
```
zuvix message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
zuvix message send --channel telegram --target @mychat \
