---
summary: "CLI reference for `zuvix node` (headless node host)"
read_when:
...
`zuvix node run` and `zuvix node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `ZUVIX_GATEWAY_TOKEN` / `ZUVIX_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `ZUVIX_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `zuvix.json` config
key.
`zuvix node install` persists it into the supervised node service when it is
...
```bash
zuvix node install --host <gateway-host> --port 18789
```
...
zuvix node stop
zuvix node restart
zuvix node uninstall
```

Use `zuvix node run` for a foreground node host (no service).
...
- `zuvix approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Zuvix prepares a canonical `systemRunPlan`
