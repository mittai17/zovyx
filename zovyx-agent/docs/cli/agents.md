---
summary: "CLI reference for `zuvix agents` (list/add/delete/bindings/bind/unbind/set identity)"
read_when:
...
zuvix agents add work --workspace ~/.zuvix/workspace-work
zuvix agents add work --workspace ~/.zuvix/workspace-work --bind telegram:*
zuvix agents add ops --workspace ~/.zuvix/workspace-ops --bind telegram:ops --non-interactive
zuvix agents bindings
zuvix agents bind --agent work --bind telegram:ops
zuvix agents unbind --agent work --bind telegram:ops
zuvix agents set-identity --workspace ~/.zuvix/workspace --from-identity
zuvix agents set-identity --agent main --avatar avatars/zuvix.png
zuvix agents delete work
```
...
If you also want different visible skills per agent, configure `agents.defaults.skills` and `agents.list[].skills` in `zuvix.json`. See [Skills config](/tools/skills-config) and [Configuration reference](/gateway/config-agents#agents-defaults-skills).

List bindings:

```bash
zuvix agents bindings
zuvix agents bindings --agent work
zuvix agents bindings --json
```

Add bindings:

```bash
zuvix agents bind --agent work --bind telegram:ops --bind discord:guild-a
```

You can also add bindings when creating an agent:

```bash
zuvix agents add work --workspace ~/.zuvix/workspace-work --bind telegram:* --bind discord:*
```

If you omit `accountId` (`--bind <channel>`), Zuvix resolves it from plugin setup hooks, forced account binding, or the channel's configured account count.

If you omit `--agent` for `bind` or `unbind`, Zuvix targets the current default agent.
...
- If the same agent already has a matching channel binding without `accountId`, and you later bind with an explicit or resolved `accountId`, Zuvix upgrades that existing binding in place instead of adding a duplicate.

Examples:

```bash
# match all accounts on the channel
zuvix agents bind --agent work --bind telegram:*
...
zuvix agents bind --agent work --bind telegram:alerts
```
...
```bash
zuvix agents unbind --agent work --bind telegram:ops
zuvix agents unbind --agent work --all
```
...
Running `zuvix agents` with no subcommand is equivalent to `zuvix agents list`.
...
- Example path: `~/.zuvix/workspace/IDENTITY.md`
...
```bash
zuvix agents set-identity --agent main --name "Zuvix" --emoji "🦞" --avatar avatars/zuvix.png
```
