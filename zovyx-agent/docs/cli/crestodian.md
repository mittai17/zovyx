...
read_when:
  - You run zuvix with no command after setup and want to understand Crestodian
  - You need a configless-safe way to inspect or repair Zuvix
...
Crestodian is Zuvix's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `zuvix` with no command starts classic onboarding first when the
active config file is missing or has no authored settings (empty or
metadata-only). After a config file has authored settings, running `zuvix`
with no command starts Crestodian in an interactive terminal. Running
`zuvix crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`zuvix tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same Zuvix reference discovery as regular agents. In a Git checkout,
...
[https://github.com/zuvix/zuvix](https://github.com/zuvix/zuvix), with explicit
...
zuvix
zuvix crestodian
zuvix crestodian --json
zuvix crestodian --message "models"
zuvix crestodian --message "validate config"
zuvix crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
zuvix crestodian --message "set default model openai/gpt-5.5" --yes
zuvix onboard --modern
```
...
config set-ref gateway.auth.token env ZUVIX_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install clawhub:zuvix-codex-app-server
plugin uninstall zuvix-codex-app-server
talk to work agent
...
- `zuvix.json` is missing
- `zuvix.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`zuvix --help` and `zuvix --version` still use the normal fast paths.
Noninteractive bare `zuvix` exits with a short message instead of printing
...
```text
~/.zuvix/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`zuvix onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `zuvix onboard` still runs classic onboarding.
...
planner turn through Zuvix's normal runtime paths. It first uses the
configured Zuvix model. If no configured model is usable yet, it can fall
...
`zuvix tui`, `zuvix chat`, and `zuvix terminal` still open the normal
...
Zuvix: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
Zuvix: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
Zuvix: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `zuvix` for agent handoff.
