---
summary: "CLI reference for `zuvix onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want Zuvix to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Zuvix onboarding fits together.
  </Card>
...
zuvix onboard --flow quickstart
zuvix onboard --flow manual
zuvix onboard --flow import
zuvix onboard --import-from hermes --import-source ~/.hermes
zuvix onboard --skip-bootstrap
zuvix onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh Zuvix setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `zuvix onboard` keeps the classic onboarding flow.

On a fresh install where the active config file is missing or has no authored
settings (empty or metadata-only), bare `zuvix` also starts the classic
onboarding flow. Once a config file has authored settings, bare `zuvix`
...
`ZUVIX_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
```bash
ZUVIX_LOCALE=zh-CN zuvix onboard
```
...
Zuvix marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
```bash
export ZUVIX_GATEWAY_TOKEN="your-token"
zuvix onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `zuvix gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `zuvix dashboard` (Control UI, no channel setup).
...
zuvix channels add
zuvix configure
zuvix agents add <name>
```

Use `zuvix setup` instead when you only need the baseline config/workspace. Use `zuvix configure` later for targeted changes and `zuvix channels add` for channel-only setup.
