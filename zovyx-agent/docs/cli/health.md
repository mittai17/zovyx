---
summary: "CLI reference for `zuvix health` (gateway health snapshot via RPC)"
read_when:
...
zuvix health
zuvix health --json
zuvix health --timeout 2500
zuvix health --verbose
zuvix health --debug
```

Notes:

- Default `zuvix health` asks the running gateway for its health snapshot. When the
