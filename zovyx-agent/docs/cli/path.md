---
summary: "CLI reference for `zuvix path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
zuvix plugins enable oc-path
```

## Why use it

Zuvix state is spread across human-edited markdown, commented JSONC config,
...
`zuvix path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `zuvix path` for ordinary whole-file edits, rich
...
```bash
zuvix path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
zuvix path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
zuvix path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
zuvix path resolve 'oc://AGENTS.md/runtime-safety/zuvix-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
zuvix path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
zuvix path resolve 'oc://gateway.jsonc/version'

# Wildcard search
zuvix path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
zuvix path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
zuvix path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
zuvix path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
zuvix path emit ./AGENTS.md
```
...
zuvix path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
zuvix path set 'oc://zuvix.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
zuvix path set 'oc://zuvix.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
zuvix path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
zuvix path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
zuvix path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
zuvix path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
zuvix path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
zuvix path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
zuvix path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
zuvix path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
zuvix path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
zuvix path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
zuvix path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ zuvix path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ zuvix path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ zuvix path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ zuvix path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ zuvix path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ zuvix path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ zuvix path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: zuvix.invoke
```

```bash
$ zuvix path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ zuvix path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
zuvix path resolve 'oc://AGENTS.md/tools/gh/risk' --human
zuvix path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
zuvix path find 'oc://AGENTS.md/tools/**/risk'
zuvix path find 'oc://session.jsonl/[event=action]/userId'
zuvix path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
zuvix path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
zuvix path set 'oc://gateway.jsonc/version' '2.0'
zuvix path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ zuvix path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
zuvix path emit ./AGENTS.md
zuvix path emit ./gateway.jsonc --json
```
...
`zuvix path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__ZUVIX_REDACTED__` (verbatim or as a substring) is refused at write
  time.
