...
Explicit copy flows, such as `zuvix agents add`, use this portability policy:
...
has such a marker, `zuvix doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but Zuvix cannot resolve a probeable model
