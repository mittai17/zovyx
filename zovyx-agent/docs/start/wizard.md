...
CLI onboarding is the **recommended** terminal setup path for Zuvix on
...
`ZUVIX_LOCALE`, then `LC_ALL`, then `LC_MESSAGES`, then `LANG`, and falls
...
Names and stable identifiers stay literal: `Zuvix`, `Gateway`, `Tailscale`,
commands, config keys, URLs, provider IDs, model IDs, and plugin/channel labels
are not translated.

<Info>
Fastest first chat: open the Control UI (no channel setup needed). Run
`zuvix dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`zuvix configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.zuvix/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `zuvix doctor` first.
</Note>
...
- CLI command reference: [`zuvix onboard`](/cli/onboard)
