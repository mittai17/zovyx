...
read_when:
  - You need detailed behavior for zuvix onboard
  - You are debugging onboarding results or integrating onboarding clients
title: "CLI setup reference"
sidebarTitle: "CLI reference"
---

This page is the full reference for `zuvix onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.zuvix/zuvix.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `zuvix doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `zuvix pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first
      - If task creation is denied, Zuvix falls back to a per-user Startup-folder login item and starts the gateway immediately.
      - Scheduled Tasks remain preferred because they provide better supervisor status.
    - Runtime selection: Node (recommended; required for WhatsApp and Telegram). Bun is not recommended.

  </Step>
  <Step title="Health check">
    - Starts gateway (if needed) and runs `zuvix health`.
    - `zuvix status --deep` adds the live gateway health probe to status output, including channel probes when supported.
...
<AccordionGroup>
  ...
  <Accordion title="xAI (Grok) OAuth">
    ...
    recommended xAI path for most users. Zuvix stores the resulting auth
  ...
  <Accordion title="Zuvix">
    ...
    Setup URL: [zuvix.ai/auth](https://zuvix.ai/auth).
  </Accordion>
...
- Auth profiles (API keys + OAuth): `~/.zuvix/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`zuvix onboard`](/cli/onboard)
