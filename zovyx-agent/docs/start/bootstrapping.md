...
On the first agent run, Zuvix bootstraps the workspace (default
`~/.zuvix/workspace`):
...
For embedded/local model runs, Zuvix keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `zuvix onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.zuvix/workspace`).
</Note>
