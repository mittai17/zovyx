...
read_when:
  - Setting up Zuvix for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

Zuvix reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.zuvix/zuvix.json`.
The active config path must be a regular file. Symlinked `zuvix.json`
layouts are unsupported for Zuvix-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `ZUVIX_CONFIG_PATH` directly at the real file.

If the file is missing, Zuvix uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `zuvix onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    zuvix onboard       # full onboarding flow
    zuvix configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.zuvix/zuvix.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Zuvix only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`zuvix config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`zuvix doctor`, `zuvix logs`, `zuvix health`, `zuvix status`)
- Run `zuvix doctor` to see exact issues
- Run `zuvix doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `zuvix.json`
...
Run `zuvix doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `zuvix config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `ZUVIX_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push uses the hosted Zuvix relay by default: `https://ios-push-relay.zuvix.ai`.
    ...
    ```bash
    zuvix config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `ZUVIX_APNS_RELAY_BASE_URL` and `ZUVIX_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - Custom gateway relay URLs must match the relay base URL baked into the official/TestFlight iOS build.
    - `ZUVIX_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `ZUVIX_GATEWAY_TOKEN` or `gateway.auth.password` / `ZUVIX_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-zuvix-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **Zuvix-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      Zuvix updates that included file and leaves `zuvix.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for Zuvix-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `zuvix.json`. To share a tree across machines or users, set
      `ZUVIX_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.zuvix/zuvix.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `zuvix.json`. Zuvix-owned config
...
config`, inspect the config, run `zuvix config validate`, then run `zuvix
...
When you edit a source file that is referenced through `$include`, Zuvix plans
...
```bash
zuvix gateway call config.get --params '{}'  # capture payload.hash
zuvix gateway call config.patch --params '{
...
Zuvix reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.zuvix/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Zuvix runs your login shell and imports only the missing keys:
...
Env var equivalent: `ZUVIX_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${ZUVIX_GATEWAY_TOKEN}" } },
