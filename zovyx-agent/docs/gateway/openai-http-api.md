...
Zuvix's Gateway can serve a small OpenAI-compatible Chat Completions endpoint.
...
Under the hood, requests are executed as a normal Gateway agent run (same codepath as `zuvix agent`), so routing/permissions/config match your Gateway.
...
- When `gateway.auth.mode="token"`, use `gateway.auth.token` (or `ZUVIX_GATEWAY_TOKEN`).
- When `gateway.auth.mode="password"`, use `gateway.auth.password` (or `ZUVIX_GATEWAY_PASSWORD`).
...
- Internal same-host callers that bypass the proxy can use
  `gateway.auth.password` / `ZUVIX_GATEWAY_PASSWORD` as a local direct
...
- There is no separate non-owner/per-user tool boundary on this endpoint; once a caller passes Gateway auth here, Zuvix treats that caller as a trusted operator for this gateway.
- For shared-secret auth modes (`token` and `password`), the endpoint restores the normal full operator defaults even if the caller sends a narrower `x-zuvix-scopes` header.
- Trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`) honor `x-zuvix-scopes` when present and otherwise fall back to the normal operator default scope set.
...
- `gateway.auth.mode="token"` or `"password"` + `Authorization: Bearer ...`
  ...
  - ignores narrower `x-zuvix-scopes`
...
- trusted identity-bearing HTTP modes (for example trusted proxy auth, or `gateway.auth.mode="none"` on private ingress)
  ...
  - honor `x-zuvix-scopes` when the header is present
  - fall back to the normal operator default scope set when the header is absent
  - only lose owner semantics when the caller explicitly narrows scopes and omits `operator.admin`
  - require `operator.admin` for owner-level request controls such as `x-zuvix-model`
...
Zuvix treats the OpenAI `model` field as an **agent target**, not a raw provider model id.

- `model: "zuvix"` routes to the configured default agent.
- `model: "zuvix/default"` also routes to the configured default agent.
- `model: "zuvix/<agentId>"` routes to a specific agent.

Optional request headers:

- `x-zuvix-model: <provider/model-or-bare-id>` overrides the backend model for the selected agent. Shared-secret bearer callers can use this header. Identity-bearing callers, such as trusted-proxy or private no-auth ingress requests with `x-zuvix-scopes`, need `operator.admin`; write-only callers get `403 missing scope: operator.admin`.
- `x-zuvix-agent-id: <agentId>` remains supported as a compatibility override.
- `x-zuvix-session-key: <sessionKey>` fully controls session routing.
- `x-zuvix-message-channel: <channel>` sets the synthetic ingress channel context for channel-aware prompts and policies.
...
For custom apps, the safest default is to reuse the same `user` value per conversation thread. Avoid account-level identifiers unless you explicitly want multiple conversations or devices to share one Zuvix session. Use `x-zuvix-session-key` when you need explicit routing control across multiple clients or threads.
...
<AccordionGroup>
  <Accordion title="What does `/v1/models` return?">
    An Zuvix agent-target list.

    The returned ids are `zuvix`, `zuvix/default`, and `zuvix/<agentId>` entries.
  ...
  <Accordion title="Why is `zuvix/default` included?">
    `zuvix/default` is the stable alias for the configured default agent.
  ...
  <Accordion title="How do I override the backend model?">
    Use `x-zuvix-model`. This is an owner-level override: it works with the Gateway shared-secret bearer token/password path, and it requires `operator.admin` on identity-bearing HTTP paths such as trusted proxy auth.

    Examples:
    `x-zuvix-model: openai/gpt-5.4`
    `x-zuvix-model: gpt-5.5`
  ...
  <Accordion title="How do embeddings fit this contract?">
    ...
    Use `model: "zuvix/default"` or `model: "zuvix/<agentId>"`.
    When you need a specific embedding model, send it in `x-zuvix-model` from a shared-secret caller or an identity-bearing caller with `operator.admin`.
...
- `GET /v1/models` should list `zuvix/default`
- Open WebUI should use `zuvix/default` as the chat model id
- If you want a specific backend provider/model for that agent, set the agent's normal default model or send `x-zuvix-model` from a shared-secret caller or an identity-bearing caller with `operator.admin`
...
If that returns `zuvix/default`, most Open WebUI setups can connect with the same base URL and token.
...
curl -N http://127.0.0.1:18789/v1/chat/completions \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-zuvix-model: openai/gpt-5.4' \
  -d '{
    "model": "zuvix/research",
    "stream": true,
    "messages": [{"role":"user","content":"hi"}]
  }'
```
...
```bash
curl -sS http://127.0.0.1:18789/v1/models/zuvix%2Fdefault \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
...
curl -sS http://127.0.0.1:18789/v1/embeddings \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-zuvix-model: openai/text-embedding-3-small' \
  -d '{
    "model": "zuvix/default",
    "input": ["alpha", "beta"]
  }'
...
- `/v1/models` returns Zuvix agent targets, not raw provider catalogs.
- `zuvix/default` is always present so one stable id works across environments.
- Backend provider/model overrides belong in `x-zuvix-model`, not the OpenAI `model` field. On identity-bearing HTTP auth paths, this header requires `operator.admin`.
