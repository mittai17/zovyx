---
summary: "Use OpenShell as a managed sandbox backend for Zuvix agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for Zuvix. Instead of running Docker
containers locally, Zuvix delegates sandbox lifecycle to the `openshell` CLI,
...
2. Restart the Gateway. On the next agent turn, Zuvix creates an OpenShell
...
- You edit files locally outside Zuvix and want those changes visible in the
...
- When the sandbox is first created, Zuvix seeds the remote workspace from
...
- Zuvix does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside Zuvix after the initial seed, the remote sandbox does **not** see those changes. Use `zuvix sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"zuvix"`  | Sandbox source for first-time create                  |
...
1. Zuvix calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Zuvix calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `zuvix sandbox` commands
