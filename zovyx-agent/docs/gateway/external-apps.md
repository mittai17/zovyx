...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to Zuvix
...
External apps should talk to Zuvix through the Gateway protocol today. Use
...
<Warning>
  There is no public npm client package yet. Do not add Zuvix client package
...
<Note>
  This page is for code outside the Zuvix process. Plugin code that runs
  inside Zuvix should use documented `zuvix/plugin-sdk/*` subpaths instead.
</Note>
...
| [`zuvix agent`](/cli/agent)          | Ready  | One-shot script integration when shelling out to the CLI is enough.                           |
| [`zuvix message`](/cli/message)      | Ready  | Sending messages or channel actions from scripts.                                             |
...
4. Pin the Zuvix version you test against.
5. Recheck the RPC reference when upgrading Zuvix.
...
Use Gateway RPC when code lives outside Zuvix:
...
Use the Plugin SDK when code runs inside Zuvix:
...
External apps should not import `zuvix/plugin-sdk/*`; those subpaths are for
plugins loaded by Zuvix.
