...
Zuvix has three related (but different) controls:
...
Use the inspector to see what Zuvix is _actually_ doing:
...
- Zuvix validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- Gateway logs include `agents/tool-policy` audit entries when a tool policy step removes tools or a sandbox tool policy blocks a call. Use `zuvix logs` to see the rule label, config key, and affected tool names.
...
- `group:zuvix`: all built-in Zuvix tools (excludes provider plugins)
...
- Check `zuvix logs` for the `agents/tool-policy` entry. It records the sandbox mode and whether the allow or deny rule blocked the tool.
