...
Zuvix can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Zuvix requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this Zuvix Serve/Funnel
mode. `off` means Zuvix is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `ZUVIX_GATEWAY_TOKEN` is set)
- `password` (shared secret via `ZUVIX_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Zuvix verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Zuvix only treats a request as Serve when it arrives from loopback with
...
```json5
{
  gateway: {
    bind: "loopback",
    tailscale: { mode: "serve", serviceName: "svc:zuvix" },
  },
}
```

With the example above, startup reports the Service URL as
`https://zuvix.<tailnet-name>.ts.net/` instead of the device hostname.
...
Prefer `ZUVIX_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
zuvix gateway --tailscale serve
zuvix gateway --tailscale funnel --auth password
```
...
- `gateway.tailscale.serviceName` applies only to Serve mode and is passed to
  ...
  `svc:<dns-label>` Service name format, for example `svc:zuvix`.
...
- Set `gateway.tailscale.resetOnExit` if you want Zuvix to undo `tailscale serve`
...
- Set `gateway.tailscale.preserveFunnel: true` to keep an externally configured
  ...
  gateway runs in `mode: "serve"`, Zuvix checks `tailscale funnel status`
  before re-applying Serve and skips it when a Funnel route already covers the
  gateway port. The Zuvix-managed Funnel password-only policy is unchanged.
