...
Zuvix can create a local diagnostics zip for bug reports. It combines
...
2. Zuvix sends the diagnostics preamble and asks for one explicit exec
   approval. The approval runs `zuvix gateway diagnostics export --json`.
   Do not approve diagnostics through an allow-all rule.
3. After approval, Zuvix replies with a pasteable report containing the local
   bundle path, manifest summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but Zuvix does not
...
that the diagnostics flow was sent privately. If Zuvix cannot find a private
owner route, the command fails closed and asks the owner to run it from a DM.

When the active Zuvix session is using the native OpenAI Codex harness,
the same exec approval also covers an OpenAI feedback upload for the Codex
runtime threads Zuvix knows about. That upload is separate from the local
...
the channels, Zuvix session ids, Codex thread ids, and local resume commands
for the threads that were sent to OpenAI servers. If you deny or ignore the
approval, Zuvix does not run the export, does not send Codex feedback, and
...
Persisted bundles live under `~/.zuvix/logs/stability/` when events exist.

## Useful options

```bash
zuvix gateway diagnostics export \
  --output zuvix-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
