...
- Default rolling log file is under `/tmp/zuvix/` (one file per day): `zuvix-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.zuvix/zuvix.json`:
  - `logging.file`
  - `logging.level`
...
Zuvix can mask sensitive tokens before log or transcript output leaves the
...
`zuvix gateway` supports a per-gateway style switch:
