...
Zuvix can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Zuvix tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
zuvix agent --message "hi" --model claude-cli/claude-sonnet-4-6
```
...
gateway host, Zuvix now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Zuvix will
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw Zuvix transcript history before compaction.
...
2. **Builds a system prompt** using the same Zuvix prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   Zuvix session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us Zuvix-style Claude CLI usage is allowed again, so Zuvix treats
...
resolver for Zuvix skills. When the current skills snapshot includes at least
one selected skill with a materialized path, Zuvix passes a temporary Claude
Code plugin with `--plugin-dir` and omits the duplicate Zuvix skills catalog
from the appended system prompt. If the snapshot has no materialized plugin
skill, Zuvix keeps the prompt catalog as a fallback. Skill env/API key
overrides are still applied by Zuvix to the child process environment for the
run.

Claude CLI also has its own noninteractive permission mode. Zuvix maps that
to the existing exec policy instead of adding Claude-specific policy config.
For Zuvix-managed Claude live sessions, the effective Zuvix exec policy is
...
Claude launches normalize that flag to match the effective Zuvix exec policy.

The bundled Anthropic `claude-cli` backend also maps Zuvix `/think` levels
...
Before Zuvix can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
zuvix models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, Zuvix resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; Zuvix clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from Zuvix's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. Zuvix still keeps raw
...
- Zuvix drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, Zuvix lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), Zuvix seeds
...
provider would start cold because Zuvix's own session transcript is empty
for `claude-cli` runs.
...
Zuvix will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Zuvix appends the
...
- For Gemini CLI JSON output, Zuvix reads reply text from `response` and
...
agent runs use the Codex app-server harness through `openai/*`; Zuvix no
...
- `stats.cached` is normalized into Zuvix `cacheRead`.
- If `stats.input` is missing, Zuvix derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before Zuvix handles
...
Some CLI backends run an agent that compacts its **own** transcript, so Zuvix must
...
`ownsNativeCompaction: true`, and Zuvix returns a no-op from the compaction path.
...
`contextTokens: 1_000_000` purely to keep Zuvix's safeguard from firing on a
...
CLI backends do **not** receive Zuvix tool calls directly, but a backend can
...
When bundle MCP is enabled, Zuvix:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`ZUVIX_MCP_TOKEN`)
...
If no MCP servers are enabled, Zuvix still injects a strict config when a
...
When a fresh CLI session is seeded from a prior Zuvix transcript (for
...
- **No direct Zuvix tool calls.** Zuvix does not inject tool calls into
