...
read_when:
  - Generating or reviewing `zuvix secrets apply` plans
  - Debugging `Invalid plan target path` errors
  - Understanding target type and path validation behavior
title: "Secrets apply plan contract"
---

This page defines the strict contract enforced by `zuvix secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`zuvix secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `providerUpserts` — an object keyed by provider alias. Each value is a
  ...
  `secrets.providers.<alias>` in `zuvix.json`, e.g. an `exec` or `file`
  provider).
...
configured in `zuvix.json` fail with `provider "<alias>" is not
configured`.
...
- `secrets apply` writes supported `zuvix.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `zuvix secrets configure` or fix the target path to a supported shape above.
