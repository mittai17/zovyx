...
Zuvix supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Zuvix also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `zuvix gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.zuvix/.env` so the daemon can read it:
...
API keys for daemon use: `zuvix onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.zuvix/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in Zuvix as a supported token
path. Anthropic staff has since told us that Zuvix-style Claude CLI usage is
allowed again, so Zuvix treats Claude CLI reuse and `claude -p` usage as
...
2. Tell Zuvix to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching Zuvix auth profile.
...
```bash
zuvix models auth paste-token --provider openrouter
```
...
- If auth exists but Zuvix cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Zuvix integration path is allowed again.
- Zuvix therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `ZUVIX_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Zuvix retries with the next key only for rate-limit errors (for example
...
When provider auth is removed through the Gateway control plane, Zuvix deletes
...
Use `zuvix models auth login --provider <id> --profile-id <profileId>` for
...
```bash
zuvix models auth login --provider anthropic --force
```
...
When you debug order issues, `zuvix models status --probe` shows omitted
...
Run `zuvix models status` to confirm which profile is expiring. If an
