...
Zuvix supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
agent can inspect, including `zuvix.json`, `auth-profiles.json`, `.env`, or
...
`zuvix secrets audit --check` reports no plaintext secret residue.
</Warning>
...
- legacy plaintext residue has been scrubbed from `zuvix.json`,
  `auth-profiles.json`, `.env`, and generated `models.json` files
- `zuvix secrets audit --check` is clean after the migration
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `ZUVIX_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Zuvix runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Zuvix validates the resolved target path.
    ...
    - Plugin-managed exec providers can use `pluginIntegration` instead of
      copied `command`/`args`. Zuvix resolves the current command details
...
<AccordionGroup>
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/Zuvix QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
  ...
  <Accordion title="Bitwarden Secrets Manager (`bws`)">
    ...
    `scripts/secrets/zuvix-bws-resolver.mjs`; install or copy it to an absolute
    ...
    ```json5
    {
      secrets: {
        providers: {
          bws: {
            source: "exec",
            command: "/usr/local/bin/zuvix-bws-resolver.mjs",
        ...
        providers: {
          openai: {
            ...
            apiKey: {
              source: "exec",
              provider: "bws",
              id: "zuvix/providers/openai/apiKey",
            },
          },
        },
      },
    ...
    SecretRef id contract, such as `zuvix/providers/openai/apiKey`; env-var
    ...
    ```bash
    zuvix secrets audit --allow-exec
    ```
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/zuvix"],
  ...
  <Accordion title="password-store (`pass`)">
    ...
    `/usr/local/bin/zuvix-pass-resolver`. The `#!/usr/bin/env node` shebang
    ...
    ```json5
    {
      secrets: {
        providers: {
          pass_store: {
            source: "exec",
            command: "/usr/local/bin/zuvix-pass-resolver",
...
- Zuvix resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__ZUVIX_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `zuvix.json` refs)
...
When reload-time activation fails after a healthy state, Zuvix enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `zuvix memory` remote-memory paths and `zuvix qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `zuvix status`, `zuvix status --all`, `zuvix channels status`, `zuvix channels resolve`, `zuvix security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `zuvix secrets reload`.
...
with `zuvix secrets apply --from <plan-path>` before the re-audit.

<AccordionGroup>
  <Accordion title="secrets audit">
    Findings include:

    - plaintext values at rest (`zuvix.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `zuvix.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `zuvix secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `zuvix.json` plus `auth-profiles.json` for one agent scope
    ...
    - `zuvix secrets configure --providers-only`
    - `zuvix secrets configure --skip-provider-setup`
    - `zuvix secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    zuvix secrets apply --from /tmp/zuvix-secrets-plan.json
    zuvix secrets apply --from /tmp/zuvix-secrets-plan.json --allow-exec
    zuvix secrets apply --from /tmp/zuvix-secrets-plan.json --dry-run
    zuvix secrets apply --from /tmp/zuvix-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
Zuvix intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
