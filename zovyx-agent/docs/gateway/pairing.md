...
zuvix nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), Zuvix treats
...
Pairing state is stored under the Gateway state directory (default `~/.zuvix`):

- `~/.zuvix/nodes/paired.json`
- `~/.zuvix/nodes/pending.json`

If you override `ZUVIX_STATE_DIR`, the `nodes/` folder moves with it.
