---
summary: "Schema-accurate configuration examples for common Zuvix setups"
read_when:
  - Learning how to configure Zuvix
...
Save to `~/.zuvix/zuvix.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.zuvix/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/zuvix" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.zuvix/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
