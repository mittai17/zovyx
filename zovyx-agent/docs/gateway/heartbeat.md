...
- During heartbeat runs, Zuvix treats `HEARTBEAT_OK` as an ack when it appears at the **start or end** of the reply. The token is stripped and the reply is dropped if the remaining content is **≤ `ackMaxChars`** (default: 300).
...
- Explicit session key (copy from `zuvix sessions --json` or the [sessions CLI](/cli/sessions)).
...
<AccordionGroup>
  ...
  <Accordion title="Visibility and skip behavior">
    ...
    - If only alert delivery is disabled, Zuvix can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
    - If the resolved heartbeat target supports typing, Zuvix shows typing while the heartbeat run is active. This uses the same target the heartbeat would send chat output to, and it is disabled by `typingMode: "never"`.
...
If **all three** are false, Zuvix skips the heartbeat run entirely (no model call).
...
<AccordionGroup>
  <Accordion title="Behavior">
    - Zuvix parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
zuvix system event --text "Check for urgent follow-ups" --mode now
```
...
If a heartbeat previously left an existing session on a smaller local model, for example an Ollama model with a 32k window, and the next main-session turn reports context overflow, reset the session runtime model back to the configured primary model. Zuvix's reset message calls this out when the last runtime model matches configured `heartbeat.model`.
