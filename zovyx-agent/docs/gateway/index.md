...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and `Capability: ...` that matches what you expect. Use `zuvix gateway status --require-rpc` when you need read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `ZUVIX_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `ZUVIX_GATEWAY_TOKEN` / `ZUVIX_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Zuvix's highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `zuvix`, `zuvix/default`, and `zuvix/<agentId>`.
- `zuvix/default` is the stable alias that always maps to the configured default agent.
- Use `x-zuvix-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `ZUVIX_GATEWAY_PORT` → `gateway.port` → `18789` |
| Bind mode    | CLI/override → `gateway.bind` → `loopback`                    |

Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `zuvix doctor --fix` or `zuvix gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
```bash
zuvix gateway status
zuvix gateway status --deep   # adds a system-level service scan
zuvix gateway status --json
zuvix gateway install
zuvix gateway restart
zuvix gateway stop
zuvix secrets reload
zuvix logs --follow
zuvix doctor
```
...
- `gateway probe` can warn about `multiple reachable gateway identities` when distinct
  gateways answer, or when Zuvix cannot prove reachable targets are the same gateway.
...
- Unique `gateway.port`
- Unique `ZUVIX_CONFIG_PATH`
- Unique `ZUVIX_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
ZUVIX_CONFIG_PATH=~/.zuvix/a.json ZUVIX_STATE_DIR=~/.zuvix-a zuvix gateway --port 19001
ZUVIX_CONFIG_PATH=~/.zuvix/b.json ZUVIX_STATE_DIR=~/.zuvix-b zuvix gateway --port 19002
```
...
Use `zuvix gateway restart` for restarts. Do not chain `zuvix gateway stop` and `zuvix gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default — this removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `zuvix gateway stop --disable`.

LaunchAgent labels are `ai.zuvix.gateway` (default) or `ai.zuvix.<profile>` (named profile). `zuvix doctor` audits and repairs service config drift.
...
```bash
zuvix gateway install
systemctl --user enable --now zuvix-gateway[-<profile>].service
zuvix gateway status
```
...
[Service]
ExecStart=/usr/local/bin/zuvix gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `Zuvix Gateway`
(or `Zuvix Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Zuvix falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now zuvix-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/zuvix-gateway[-<profile>].service` and adjust
`ExecStart=` if your `zuvix` binary lives elsewhere.

Do not also let `zuvix doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level Zuvix gateway service; use `ZUVIX_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
...
```bash
zuvix --dev setup
zuvix --dev gateway --allow-unconfigured
zuvix --dev status
```
