---
summary: "Pre-flight and rollback checklist before exposing an Zuvix Gateway beyond loopback"
...
- Backup location for `~/.zuvix/zuvix.json` and credentials.
...
```bash
zuvix gateway probe --url ws://127.0.0.1:18789 --token "$ZUVIX_GATEWAY_TOKEN"
```
...
Run `zuvix security audit --deep` after proxy changes. Trusted-proxy findings
...
1. Re-run `zuvix security audit --deep`.
...
5. Re-run `zuvix security audit --deep`.
