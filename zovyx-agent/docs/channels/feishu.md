...
<Note>
Requires Zuvix 2026.5.29 or above. Run `zuvix --version` to check. Upgrade with `zuvix update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  zuvix channels login --channel feishu
  ```
...
```bash
zuvix pairing list feishu
zuvix pairing approve feishu <CODE>
```
...
4. Check logs: `zuvix logs --follow`
...
5. Ensure the gateway is running: `zuvix gateway status`
6. Check logs: `zuvix logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `zuvix channels login --channel feishu`
...
3. Restart the gateway: `zuvix gateway restart`
...
If a named account created an unscoped dynamic agent on an older release, that legacy agent still counts toward `maxAgents`. Confirm that it is not used by the default account before removing it, or temporarily increase `maxAgents`; Zuvix cannot safely infer which account owns ambiguous legacy state.
</Note>
  ...
  channels: {
    feishu: {
      ...
      dynamicAgentCreation: {
        enabled: true,
        workspaceTemplate: "~/.zuvix/workspace-{agentId}",
        agentDirTemplate: "~/.zuvix/agents/{agentId}/agent",
      },
    },
  },
  session: {
...
| `channels.feishu.dynamicAgentCreation.workspaceTemplate` | Path template for dynamic agent workspaces | `~/.zuvix/workspace-{agentId}`    |
| `channels.feishu.dynamicAgentCreation.agentDirTemplate`  | Agent directory name template              | `~/.zuvix/agents/{agentId}/agent` |
...
workspace: /Users/you/.zuvix/workspace-feishu-ou_xxxxxx
...
of raw `file_key` JSON. When `tools.media.audio` is configured, Zuvix
...
conversion fails, Zuvix falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, Zuvix
hydrates it from Feishu before routing the turn. Normal group replies that
Zuvix turns into threads keep using the reply root message ID (`om_*`) so the
