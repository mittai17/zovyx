...
Tencent Yuanbao is Tencent's AI assistant platform. The Zuvix channel plugin
connects Yuanbao bots to Zuvix over WebSocket so they can interact with users
...
> **Requires Zuvix 2026.4.10 or above.** Run `zuvix --version` to check. Upgrade with `zuvix update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  zuvix channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
zuvix channels login --channel yuanbao
```
...
```bash
zuvix pairing list yuanbao
zuvix pairing approve yuanbao <CODE>
```
