...
Tlon is a decentralized messenger built on Urbit. Zuvix connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Zuvix releases, so normal packaged
...
```bash
zuvix plugins install @zuvix/tlon
```
...
```bash
zuvix plugins install ./path/to/local/tlon-plugin
```
...
By default, Zuvix blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `zuvix message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Zuvix replies in-thread.
