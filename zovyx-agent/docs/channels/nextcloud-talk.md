...
Nextcloud Talk ships as a bundled plugin in current Zuvix releases, so
...
```bash
zuvix plugins install @zuvix/nextcloud-talk
```
...
```bash
zuvix plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Zuvix" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Zuvix:
   ...
   ```bash
   zuvix channels add --channel nextcloud-talk \
...
- Approve via:
  - `zuvix pairing list nextcloud-talk`
  - `zuvix pairing approve nextcloud-talk <CODE>`
