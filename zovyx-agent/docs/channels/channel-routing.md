...
Zuvix routes replies **back to the channel where a message came from**. The
...
Zuvix infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Zuvix still records inbound session metadata, but it
...
when a guarded path must not create a new Zuvix session. In that mode,
Zuvix may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Zuvix would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.zuvix/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.zuvix`):

- `~/.zuvix/agents/<agentId>/sessions/sessions.json`
