...
read_when:
  - You want to connect Zuvix to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Zuvix via the LINE Messaging API. The plugin runs as a webhook
...
```bash
zuvix plugins install @zuvix/line
```

Local checkout (when running from a git repo):

```bash
zuvix plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Zuvix applies strict pre-auth body limits and timeout before verification.
- Zuvix processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
zuvix pairing list line
zuvix pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.zuvix/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Zuvix validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
