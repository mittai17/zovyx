...
read_when:
  - Setting up Zalo Personal for Zuvix
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Zuvix.
...
Zalo Personal ships as a bundled plugin in current Zuvix releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `zuvix plugins install @zuvix/zalouser`
- Pinned version: `zuvix plugins install @zuvix/zalouser@2026.5.2`
- Or from a source checkout: `zuvix plugins install ./path/to/local/zalouser-plugin`
...
1. Ensure the Zalo Personal plugin is available.
   - Current packaged Zuvix releases already bundle it.
   - Older/custom installs can add it manually with the commands above.
2. Login (QR, on the Gateway machine):
   - `zuvix channels login --channel zalouser`
...
```bash
zuvix directory self --channel zalouser
zuvix directory peers list --channel zalouser --query "name"
zuvix directory groups list --channel zalouser --query "work"
```
...
- `zuvix pairing list zalouser`
- `zuvix pairing approve zalouser <code>`
...
- On startup, Zuvix resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, Zuvix stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Zuvix state. Example:
...
Profile names select the saved Zalo login credentials in Zuvix state. Resolution order is:
...
- Zuvix sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Zuvix sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `zuvix channels logout --channel zalouser && zuvix channels login --channel zalouser`
...
- The channel now runs fully in Zuvix without external CLI binaries.
