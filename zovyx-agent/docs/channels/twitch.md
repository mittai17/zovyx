...
read_when:
  - Setting up Twitch chat integration for Zuvix
title: "Twitch"
sidebarTitle: "Twitch"
---

Twitch chat support via IRC connection. Zuvix connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

<Note>
Twitch ships as a bundled plugin in current Zuvix releases, so normal packaged builds do not need a separate install.
</Note>
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    zuvix plugins install @zuvix/twitch
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    zuvix plugins install ./path/to/local/twitch-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Configure the token">
    - Env: `ZUVIX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "zuvix", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ZUVIX_TWITCH_ACCESS_TOKEN env var)
...
<Tabs>
  <Tab title="Env var (default account only)">
    ```bash
    ZUVIX_TWITCH_ACCESS_TOKEN=oauth:abc123...
    ```
  </Tab>
  <Tab title="Config">
    ```json5
