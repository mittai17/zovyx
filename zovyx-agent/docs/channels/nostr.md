...
read_when:
  - You want Zuvix to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Zuvix to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Zuvix releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`zuvix onboard`) and `zuvix channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
zuvix plugins install @zuvix/nostr
```
...
```bash
zuvix plugins install --link <path-to-local-nostr-plugin>
```
...
zuvix channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "zuvix@example.com",
        lud16: "zuvix@example.com",
      },
    },
  },
}
