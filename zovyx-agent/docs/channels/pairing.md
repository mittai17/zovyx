...
read_when:
  ...
  - Reviewing Zuvix security posture
title: "Pairing"
---

"Pairing" is Zuvix's explicit access approval step.
...
Supported channels: `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `zuvix-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
Stored under `~/.zuvix/credentials/`:
...
3. On your phone, open the Zuvix iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, Zuvix keeps the existing approval as-is and creates a fresh pending upgrade request. Use `zuvix devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
- The legacy `node.pair.*` API (CLI: `zuvix nodes pending|approve|reject|remove|rename`) is a
