...
- bot user = the Zuvix Matrix account that sends the reply
...
If no pushers come back, fix normal Matrix push delivery for this account before continuing.
  ...
  <Step title="Install the override push rule">
    Zuvix marks finalized text-only preview edits with `content["com.zuvix.finalized_preview"] = true`. Install a rule that matches that marker plus the bot MXID as sender:

```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/zuvix-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.zuvix\\.finalized_preview",
        "value": true
      },
...
```
    ...
    - `zuvix-finalized-preview-botname`: a rule ID unique per bot per recipient (pattern: `zuvix-finalized-preview-<botname>`)
    - `@bot:example.org`: your Zuvix bot MXID, not the recipient's
...
Push rules are keyed by `ruleId`: re-running `PUT` against the same ID updates a single rule. For multiple Zuvix bots notifying the same recipient, create one rule per bot with a distinct sender match.
