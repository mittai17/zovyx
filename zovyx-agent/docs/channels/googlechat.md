...
```bash
zuvix plugins install @zuvix/googlechat
```

Local checkout (when running from a git repo):

```bash
zuvix plugins install ./path/to/local/googlechat-plugin
```
...
2. Create a **Service Account**:
   ...
   - Name it whatever you want (e.g., `zuvix-chat`).
...
4. Store the downloaded JSON file on your gateway host (e.g., `~/.zuvix/googlechat-service-account.json`).
5. Create a Google Chat app in the [Google Cloud Console Chat Configuration](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat):
   - Fill in the **Application info**:
     - **App name**: (e.g. `Zuvix`)
     - **Avatar URL**: (e.g. `https://zuvix.ai/logo.png`)
     - **Description**: (e.g. `Personal AI Assistant`)
   - Enable **Interactive features**.
   - Under **Functionality**, check **Join spaces and group conversations**.
   - Under **Connection settings**, select **HTTP endpoint URL**.
   - Under **Triggers**, select **Use a common HTTP endpoint URL for all triggers** and set it to your gateway's public URL followed by `/googlechat`.
     - _Tip: Run `zuvix status` to find your gateway's public URL._
...
6. **Enable the app status**:
   ...
   - Click **Save** again.
7. Configure Zuvix with the service account path + webhook audience:
...
Google Chat webhooks require a public HTTPS endpoint. For security, **only expose the `/googlechat` path** to the internet. Keep the Zuvix dashboard and other sensitive endpoints on your private network.
...
With this config, any request to `your-domain.com/` will be ignored or returned as 404, while `your-domain.com/googlechat` is safely routed to Zuvix.
...
1. Google Chat sends webhook POSTs to the gateway. Each request includes an `Authorization: Bearer <token>` header.
   - Zuvix verifies bearer auth before reading/parsing full webhook bodies when the header is present.
...
2. Zuvix verifies the token against the configured `audienceType` + `audience`:
...
1. **Channel not configured**: The `channels.googlechat` section is missing from your config. Verify with:
   ...
   ```bash
   zuvix config get channels.googlechat
   ```
...
2. **Plugin not enabled**: Check plugin status:
   ...
   ```bash
   zuvix plugins list | grep googlechat
   ```
...
- Check `zuvix channels status --probe` for auth errors or missing audience config.
- If no messages arrive, confirm the Chat app's webhook URL + event subscriptions.
- If mention gating blocks replies, set `botUser` to the app's user resource name and verify `requireMention`.
- Use `zuvix logs --follow` while sending a test message to see if requests reach the gateway.
