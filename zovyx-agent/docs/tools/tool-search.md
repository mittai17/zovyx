---
summary: "Tool Search: compact large Zuvix tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want Zuvix agents to use a large tool catalog without adding every tool schema to the prompt
  - You want Zuvix tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for Zuvix runs
---

Tool Search is an experimental Zuvix agent runtime feature. It gives agents one
...
This page documents Zuvix Tool Search. It is not the Codex-native tool
...
When enabled for Zuvix runs, the model receives one `tool_search_code` tool
by default. That tool runs a short JavaScript body in an isolated Node
subprocess with an `zuvix.tools` bridge:

```js
const hits = await zuvix.tools.search("create a GitHub issue");
const tool = await zuvix.tools.describe(hits[0].id);
return await zuvix.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include Zuvix tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through Zuvix.

Codex harness runs do not receive these experimental Zuvix Tool Search
controls. Zuvix passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the Zuvix embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible Zuvix and plugin tools.
...
6. Expose the Zuvix code bridge, the structured fallback tools, or the
   compact directory surface to the model.

At execution time every real tool call returns to Zuvix. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`zuvix.tools.call(...)` crosses the bridge back into the Gateway, where the
...
- `directory`: exposes `tool_search`, `tool_describe`, and `tool_call` plus a
  ...
  providers that should see tool names without every full schema. Zuvix can
  also expose a small bounded set of likely or required tool schemas directly
  for the current turn.

All modes use the same policy-filtered catalog and normal Zuvix execution
...
for the current run while Zuvix tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small Zuvix tool
...
catalog includes eligible Zuvix, MCP, and client tools after normal policy
filtering.
...
`zuvix.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await zuvix.tools.search("calendar event", { limit: 5 });
```

`zuvix.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await zuvix.tools.describe("mcp:calendar:create_event");
```

`zuvix.tools.call(id, args)`

Calls a selected tool through Zuvix.

```js
await zuvix.tools.call(calendarCreate.id, {
  summary: "Planning",
...
the model requests an exact hidden directory tool name directly, Zuvix
hydrates it from the authorized catalog before normal execution.
Directory-mode client tool names must not collide with Zuvix, plugin, or MCP
...
network grants, and no child-process or worker grants. Zuvix enforces a
...
- `zuvix.tools.search`
- `zuvix.tools.describe`
- `zuvix.tools.call`

Normal Zuvix behavior still applies to final calls:
...
Enable Tool Search for Zuvix runs with the default code bridge:

```bash
zuvix config set tools.toolSearch true
```
...
Use the structured fallback tools instead for Zuvix runs:
...
Use the compact directory surface instead for Zuvix runs:
...
- final tool calls executed through Zuvix
...
- whether the result came from Zuvix, MCP, or a client tool

## E2E validation

The gateway E2E runner proves both paths with the Zuvix runtime:
