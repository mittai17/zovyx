...
read_when:
  - Looking for an overview of Zuvix's media capabilities
...
Zuvix generates images, videos, and music, understands inbound media
...
For async tools, Zuvix submits the request to the provider, returns a task
...
Zuvix wakes the agent with the generated media paths so it can tell the
...
Zuvix sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until Zuvix has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in Zuvix until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
