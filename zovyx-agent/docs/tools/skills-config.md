...
`~/.zuvix/zuvix.json`. Agent-specific visibility lives under
...
<ParamField path="skills.install.nodeManager" type='"npm" | "pnpm" | "yarn" | "bun"' default='"npm"'>
  ...
  for WhatsApp/Telegram). Use `zuvix setup --node-manager` for npm, pnpm,
...
runs after Zuvix has staged source material and before the install or update
...
```json5
{
  security: {
    installPolicy: {
      ...
      exec: {
        source: "exec",
        command: "/usr/local/bin/zuvix-install-policy",
        args: ["--json"],
        timeoutMs: 10000,
        noOutputTimeoutMs: 10000,
        maxOutputBytes: 1048576,
        passEnv: ["ZUVIX_STATE_DIR", "PATH"],
...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. Zuvix runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the Zuvix process into the policy
...
`zuvixVersion`, `targetType`, `targetName`, `sourcePath`, `sourcePathKind`,
...
Zuvix does not execute install policy during normal Gateway startup. Installs
and updates fail closed when policy is enabled but unavailable. `zuvix doctor`
performs static validation, and `zuvix doctor --deep` executes a synthetic
...
{
  "protocolVersion": 1,
  "zuvixVersion": "2026.6.1",
  "targetType": "skill",
  "targetName": "weather",
  "sourcePath": "/var/folders/.../zuvix-skill-clawhub/root",
  ...
  "origin": {
    "type": "clawhub",
    "registry": "https://clawhub.zuvix.ai",
    "slug": "weather",
    "version": "1.0.0"
  },
  "request": {
...
`metadata.zuvix.skillKey`, use that key instead. Quote hyphenated names
...
<ParamField path="skills.entries.<key>.enabled" type="boolean">
  ...
  `codex`, `zuvix`, or another supported CLI is installed and authenticated.
</ParamField>
...
Managed `~/.zuvix/skills` and personal `~/.agents/skills` directories
