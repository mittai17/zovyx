---
summary: "Install, configure, and manage Zuvix plugins"
read_when:
...
Plugins extend Zuvix with channels, model providers, agent harnesses, tools,
...
- an Zuvix checkout or installation with the `zuvix` CLI available
...
<Steps>
  <Step title="Find the plugin">
    ...
    they match an official plugin id. Raw `@zuvix/*` package specs that match
    bundled plugins use the bundled copy from the current Zuvix build. Use an
  ...
  <Step title="Configure and enable it">
    ...
    ```bash
    zuvix plugins enable <plugin-id>
    ```
    ...
    `zuvix plugins install` adds the installed id to an existing
  ...
  <Step title="Let the Gateway reload">
    ...
    enabled, Zuvix detects the changed plugin install record and restarts the
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want Zuvix-native discovery, scans, version metadata, and install hints | `zuvix plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `zuvix plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `zuvix plugins install git:github.com/<owner>/<repo>@<ref>` |
...
a bundled plugin id, Zuvix uses that bundled source. If it matches an
official external plugin id, Zuvix uses the official package catalog. Other
ordinary bare package specs install through npm during the launch cutover. Raw
`@zuvix/*` package specs that match bundled plugins also resolve to the
bundled copy before npm fallback. Use `npm:@zuvix/<plugin>@<version>` when
...
deterministic source selection. See [`zuvix plugins`](/cli/plugins#install)
for the full command contract.

For npm installs, unpinned package specs and `@latest` choose the newest stable
package that advertises compatibility with this Zuvix build. If npm's
current latest release declares a newer `zuvix.compat.pluginApi` or
`zuvix.install.minHostVersion`, Zuvix scans older stable package versions
...
Zuvix processes where plugin hooks are loaded, so use `security.installPolicy`
...
not bypass install policy or Zuvix's built-in plugin dependency denylist.
...
Run `zuvix doctor` or `zuvix doctor --fix` when config validation reports
stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin paths.

## Understand plugin formats

Zuvix recognizes two plugin formats:

| Format                 | How it loads                                                                 | Use when                                                               |
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native Zuvix plugin | `zuvix.plugin.json` plus a runtime module loaded in process               | You are installing or building Zuvix-specific runtime capabilities  |
| Compatible bundle      | Codex, Claude, or Cursor plugin layout mapped into Zuvix plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `zuvix plugins list`, `zuvix plugins inspect`,
`zuvix plugins enable`, and `zuvix plugins disable`. See
...
Plugin-managed internal hooks show up in `zuvix hooks list` with
`plugin:<id>`. You cannot enable or disable them through `zuvix hooks`;
enable or disable the plugin instead.

## Verify the active Gateway

`zuvix plugins list` and plain `zuvix plugins inspect` read cold config,
...
```bash
zuvix gateway status --deep --require-rpc
zuvix plugins inspect <plugin-id> --runtime --json
zuvix gateway restart
```
...
sure any manual restart targets the actual `zuvix gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `zuvix plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `zuvix plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `zuvix doctor --fix` when it points to stale plugin state                                           | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `zuvix plugins registry --refresh`                    |
| `ZUVIX_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `zuvix plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |
...
other channel. Run `zuvix doctor --fix` to remove stale plugin and channel
...
plugin id. If both plugins are explicitly enabled, Zuvix keeps that request
...
Zuvix needs at runtime. Update or reinstall after the publisher ships
...
and config validation follows with `plugin present but blocked`, Zuvix found
...
Zuvix as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Zuvix config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Zuvix as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/zuvix-config/npm
```

After fixing ownership, rerun `zuvix doctor --fix` or
`zuvix plugins registry --refresh` so the persisted plugin registry matches
the repaired files.
...
```bash
zuvix config set logging.level trace
zuvix logs --follow
```
...
Zuvix caches successful plugin tool factory results for repeated resolutions
