...
Zuvix agents can generate videos from text prompts, reference images, or
...
Zuvix treats video generation as three runtime modes:
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    zuvix config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. Zuvix submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, Zuvix wakes the same session with an internal completion event.
4. The agent tells the user through the session's normal visible-reply mode:
   ...
   completion reply, Zuvix sends an idempotent direct fallback with only the
   missing video.
...
generation. Use `zuvix tasks list` or `zuvix tasks show <taskId>` to
...
Generated video files are saved under Zuvix-managed media storage when
...
larger renders. When a provider also returns a hosted output URL, Zuvix
...
```bash
zuvix tasks list
zuvix tasks show <taskId>
zuvix tasks cancel <taskId>
```
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. Zuvix normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `480P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. Zuvix normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, Zuvix uses `agents.defaults.videoGenerationModel.timeoutMs` if configured, otherwise the plugin-authored provider default when one exists.</ParamField>
...
<Note>
Not all providers support all parameters. Zuvix normalizes duration to
...
Zuvix resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Requires the [`@zuvix/byteplus-modelark`](https://www.npmjs.com/package/@zuvix/byteplus-modelark)
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. Zuvix waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. Zuvix submits the
...
```bash
ZUVIX_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- Per-provider operation cap from
  `ZUVIX_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `ZUVIX_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your Zuvix config:
...
```bash
zuvix config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
