...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `zuvix approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `zuvix exec-policy show`                                      | Local-machine merged view.                                                             |
| `zuvix exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `zuvix` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, Zuvix also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
`ZUVIX_STATE_DIR` is set, the file follows that state directory;
otherwise it uses the default Zuvix state directory:

```text
$ZUVIX_STATE_DIR/exec-approvals.json
# otherwise
~/.zuvix/exec-approvals.json
```

The default approval socket follows the same root:
`$ZUVIX_STATE_DIR/exec-approvals.sock`, or
`~/.zuvix/exec-approvals.sock` when the variable is unset.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.zuvix/exec-approvals.sock",
...
- `auto` - use allowlist policy, run deterministic matches directly, and send approval misses through Zuvix's native auto reviewer before falling back to a human approval route.
...
<ParamField path="askFallback" type='"deny" | "allowlist" | "full"'>
  ...
  field is omitted, Zuvix defaults to `deny`.
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, Zuvix treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  Zuvix may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in Zuvix config
(`tools.exec.*`) **and** host-local approvals policy in
the execution host approvals file.

Zuvix defaults omitted `askFallback` to `deny`. Set host
...
- In YOLO mode, Zuvix does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when Zuvix's effective exec
policy is YOLO. For Zuvix-managed Claude live sessions, Zuvix's
...
If you want a more conservative setup, tighten Zuvix exec policy back to
...
approvals remotely, use `zuvix approvals set --gateway` or
`zuvix approvals set --node <id|name|ip>`.
...
- `zuvix exec-policy` does not synchronize node approvals.
- `zuvix exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `zuvix approvals --node ...`.
...
specific argument shape. Zuvix evaluates the regular expression
...
entries instead of hand-editing the encoded value. If Zuvix cannot
...
CLI: `zuvix approvals` supports gateway or node editing - see
...
Zuvix posts the denial back into that session as an internal followup so the
agent can stop waiting on the async command and avoid a missing-result repair.
If there is no session or the session cannot be resumed, Zuvix can still
...
When an async exec approval is denied, Zuvix treats the host command as
...
session delivery is unavailable, Zuvix falls back to a concise operator or
