...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, Zuvix best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - Zuvix hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, Zuvix first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, Zuvix falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - For ACP harness sessions (Claude Code, Gemini CLI, Zuvix, or explicit Codex ACP/acpx), use `sessions_spawn` with `runtime: "acp"` when the tool advertises that runtime. See [ACP delivery model](/tools/acp-agents#delivery-model) when debugging completions or agent-to-agent loops. When the `codex` plugin is enabled, Codex chat/thread control should prefer `/codex ...` over ACP unless the user explicitly asks for ACP/acpx.
    - Zuvix hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.list[]` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal Zuvix config agents from `agents_list`.
...
- **Run timeout:** Zuvix uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout). `sessions_spawn` does not accept per-call timeout overrides.
...
<ParamField path="runtime" type='"subagent" | "acp"' default="subagent">
  `acp` is only for external ACP harnesses (`claude`, `droid`, `gemini`, `zuvix`, or explicitly requested Codex ACP/acpx) and for `agents.list[]` entries whose `runtime.type` is `acp`.
</ParamField>
...
When active children exist, Zuvix injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    Zuvix creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish Zuvix, Codex
...
and `agents_list` omits it. Run `zuvix doctor --fix` to clean stale
...
- If a nested requester subagent session is gone, Zuvix falls back to that session's requester when available.
...
target agent first. After that, Zuvix applies the sub-agent restriction
layer.
...
Zuvix does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, Zuvix persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`zuvix tasks maintenance --apply` to reconcile the task record, or
`zuvix doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
