...
read_when:
  ...
  - Enabling the plugin-tools or Zuvix-tools MCP bridge
...
When Zuvix uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Zuvix `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by Zuvix before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Zuvix/acpx cannot force a model selection.
...
Packaged installs use the official `@zuvix/acpx` runtime plugin for ACP.
...
```bash
zuvix plugins install ./path/to/local/acpx-plugin
```
...
`ready` signal. Set `ZUVIX_ACPX_RUNTIME_STARTUP_PROBE=0` or
`ZUVIX_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `command` accepts an absolute path, relative path (resolved from the Zuvix workspace), or command name.
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before Zuvix passes it through the current acpx command-string registry.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install Zuvix globally with `npm install -g zuvix`, the acpx
...
normally and reports the missing dependency through `zuvix acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Zuvix plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Zuvix plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
zuvix config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `zuvix-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Zuvix
  plugins.
...
### Zuvix tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Zuvix tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
zuvix config set plugins.entries.acpx.config.zuvixToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `zuvix-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Zuvix tools. The initial server exposes `cron`.
...
```bash
zuvix config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use Zuvix agent/run timeouts, including `/acp timeout`.
...
```bash
zuvix config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from Zuvix exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between Zuvix `tools.exec.mode`, Codex Guardian
...
```bash
zuvix config set plugins.entries.acpx.config.permissionMode approve-all
zuvix config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Zuvix defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
