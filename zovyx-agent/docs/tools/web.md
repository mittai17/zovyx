...
Zuvix also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Zuvix plugin SDK" });
    ```
...
If native Codex search is enabled but the current model is not Codex-capable, Zuvix keeps the normal managed `web_search` behavior.

## Network safety

Managed `web_search` provider calls use Zuvix's guarded fetch path. For
trusted provider API hosts, Zuvix allows Surge, Clash, and sing-box fake-IP
...
If no `provider` is set, Zuvix checks providers in this order and uses the
...
When no API-backed provider is configured, Zuvix defaults to **Parallel
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Zuvix resolves only the
...
Grok can also reuse an xAI OAuth auth profile from `zuvix models auth login
...
Zuvix keeps startup resilient and reports a warning so you can reinstall the
plugin or run `zuvix doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let Zuvix auto-detect the first ready web-fetch
...
When you choose **Kimi** during `zuvix onboard` or
`zuvix configure --section web`, Zuvix can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `zuvix doctor --fix`.
When you choose Grok during `zuvix onboard` or `zuvix configure --section web`,
Zuvix can also offer optional `x_search` setup with the same credential.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, Zuvix does not
...
<Tabs>
  <Tab title="Config file">
    Run `zuvix configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.zuvix/.env`.
...
optional structured filters. Zuvix only enables the built-in xAI `x_search`
