...
Run shell commands in the workspace. `exec` is a mutating shell surface: commands can create, edit, or delete files wherever the selected host or sandbox filesystem permits. Disabling Zuvix filesystem tools such as `write`, `edit`, or `apply_patch` does not make `exec` read-only.
...
- `tools.exec.mode` is the normalized policy knob. Values are `deny`, `allowlist`, `ask`, `auto`, and `full`. `auto` runs deterministic allowlist/safe-bin matches directly and routes every remaining exec approval case through Zuvix's native auto reviewer before asking a human. `ask` / `ask=always` still asks a human every time.
...
- `timeout` applies to foreground, background, `yieldMs`, gateway, sandbox, and node `system.run` execution. If omitted, Zuvix uses `tools.exec.timeoutSec`; explicit `timeout: 0` disables the exec process timeout for that call.
...
- On non-Windows gateway hosts, bash and zsh exec commands use a startup snapshot. Zuvix captures sourceable
  aliases/functions and a small safe environment set from shell startup files into
  `$ZUVIX_STATE_DIR/cache/shell-snapshots/`, then sources that snapshot before each exec command.
  Secret-looking variables are excluded; sandbox and node exec do not use this snapshot. Set
  `ZUVIX_EXEC_SHELL_SNAPSHOT=0` in the Gateway process environment to disable this snapshot path.
- Host execution (`gateway`/`node`) rejects `env.PATH` and loader overrides (`LD_*`/`DYLD_*`) to
  prevent binary hijacking or injected code.
- Zuvix sets `ZUVIX_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
- `zuvix channels login` is blocked from `exec` because it is an interactive channel-auth flow; run it in a terminal on the gateway host, or use the channel-native login tool from chat when one exists.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Zuvix prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`zuvix security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `zuvix doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`zuvix security audit` and `zuvix doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
