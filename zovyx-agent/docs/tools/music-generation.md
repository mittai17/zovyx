...
For session-backed agent runs, Zuvix starts music generation as a
...
from the completion reply, Zuvix sends an idempotent direct fallback with
...
<Note>
Not all providers support all parameters. Zuvix still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Zuvix
...
Provider request timeouts are operator configuration only. Zuvix uses
...
- **Status lookup:** `zuvix tasks list` or `zuvix tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Zuvix injects an internal completion event back
...
```bash
ZUVIX_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
