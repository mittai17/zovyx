...
frontmatter and a markdown body. Zuvix loads bundled skills plus any local
...
Zuvix loads from these sources, **highest precedence first**. When the same
...
| 4           | Managed / local skills | `~/.zuvix/skills`                    |
| 5           | Bundled skills         | shipped with the install                |
| 6 — lowest  | Extra directories      | `skills.load.extraDirs` + plugin skills |

Skill roots support grouped layouts. Zuvix discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an Zuvix
  skill root. Use `zuvix migrate plan codex` to inventory those skills, then
  `zuvix migrate codex` to copy them into your Zuvix workspace.
</Note>
...
| Shared managed | `~/.zuvix/skills`         | All agents on this machine  |
...
`zuvix.plugin.json` (paths relative to the plugin root). Plugin skills load
...
skill overrides them. Gate them via `metadata.zuvix.requires.config` on the
...
`zuvix skills` commands for install and update, or the `clawhub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `zuvix skills install <slug>`                       |
| Install from a Git repository      | `zuvix skills install git:owner/repo@ref`           |
| Install a local skill directory    | `zuvix skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `zuvix skills install <slug> --global`              |
| Update all workspace skills        | `zuvix skills update --all`                         |
| Update a shared managed skill      | `zuvix skills update <slug> --global`               |
| Update all shared managed skills   | `zuvix skills update --all --global`                |
| Verify a skill's trust envelope    | `zuvix skills verify <slug>`                        |
| Print the generated Skill Card     | `zuvix skills verify <slug> --card`                 |
| Publish / sync via ClawHub CLI     | `clawhub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `zuvix skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.zuvix/skills` directory, visible to all local agents unless agent
    ...
    `zuvix skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `zuvix skills verify <slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `zuvix.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.zuvix/skills` and personal `~/.agents/skills` may contain
...
<Note>
  Zuvix follows the [AgentSkills](https://agentskills.io) spec. The
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Zuvix keeps the skill's instructions out of the agent's normal
...
Zuvix filters skills at load time using `metadata.zuvix` (single-line
JSON in the frontmatter). A skill with no `metadata.zuvix` block is always
...
<ParamField path="requires.config" type="string[]">
  Each `zuvix.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.zuvix` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Zuvix lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `zuvix.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** Zuvix does not auto-install Homebrew or translate brew
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.zuvix/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="enabled" type="boolean">
  ...
  and ensure one of `claude`, `codex`, `zuvix`, or another supported CLI
  is installed and authenticated.
</ParamField>

<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.zuvix.primaryEnv`.
...
<Note>
  ...
  `metadata.zuvix.skillKey`, use that key under `skills.entries`. Quote
  hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, Zuvix:

<Steps>
  <Step title="Reads skill metadata">
    Zuvix resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, Zuvix also materializes the same
...
Zuvix snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, Zuvix refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, Zuvix watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, Zuvix can treat macOS-only skills as eligible when
    ...
    answering bin probes, Zuvix clears its cached bin matches.
...
When skills are eligible, Zuvix injects a compact XML block into the system
