...
Zuvix includes syntax highlighting for common source, config, and documentation languages:
...
Set plugin-wide defaults in `~/.zuvix/zuvix.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/zuvix",
        },
      },
    },
  },
...
- Artifacts are stored under the temp subfolder: `$TMPDIR/zuvix-diffs`.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in Zuvix config.
  </Step>
