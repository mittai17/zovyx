...
Zuvix supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Zuvix also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `zuvix configure --section web`. It stores the key in
`~/.zuvix/zuvix.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.zuvix/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
