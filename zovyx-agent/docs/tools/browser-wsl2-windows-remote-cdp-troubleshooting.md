...
read_when:
  - Running Zuvix Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Zuvix Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/zuvix/zuvix/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Zuvix and Chrome are on the same machine
...
- Zuvix points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Zuvix is not the problem yet.
...
Fix that before touching Zuvix config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Zuvix at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Zuvix to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Zuvix to succeed
...
```bash
zuvix browser open https://example.com --browser-profile remote
zuvix browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `zuvix browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `zuvix browser stop --browser-profile remote`
...
3. Zuvix config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Zuvix config or Control UI auth
