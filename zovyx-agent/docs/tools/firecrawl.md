...
Zuvix can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `zuvix configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `zuvix doctor --fix`.
...
when you run Firecrawl yourself. Zuvix accepts `http://` only for loopback,
...
Zuvix always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Zuvix
