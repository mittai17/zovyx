...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Zuvix
...
Today this applies to Zuvix embedded runs and Zuvix dynamic tools in the Codex
app-server harness. Tokenjuice hooks Zuvix's tool-result middleware and
...
```bash
zuvix plugins install clawhub:@zuvix/tokenjuice
```
...
```bash
zuvix config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
zuvix plugins disable tokenjuice
```
