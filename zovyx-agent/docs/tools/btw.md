...
It is modeled after Claude Code's `/btw` behavior, but adapted to Zuvix's
...
instead of falling back to a direct provider call. Zuvix seeds sanitized
conversation context into a fresh one-shot CLI invocation, disables Zuvix MCP
...
If the main run is currently active, Zuvix snapshots the current message
