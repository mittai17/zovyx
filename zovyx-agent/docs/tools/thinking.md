...
- Provider notes:
  ...
  - OpenAI GPT models map `/think` through model-specific Responses API effort support. `/think off` sends `reasoning.effort: "none"` only when the target model supports it; otherwise Zuvix omits the disabled reasoning payload instead of sending an unsupported value.
...
- **Embedded Zuvix**: the resolved level is passed to the in-process Zuvix agent runtime.
...
- Zuvix resolves fast mode in this order:
...
- For Codex-backed `openai/*` models, fast mode sends the same `service_tier=priority` flag on Codex Responses. Zuvix keeps one shared `/fast` toggle across both auth paths.
...
- Explicit Anthropic `serviceTier` / `service_tier` model params override the fast-mode default when both are set. Zuvix still skips Anthropic service-tier injection for non-Anthropic proxy base URLs.
...
- Provider plugins that proxy Claude models should reuse `resolveClaudeThinkingProfile(modelId)` from `zuvix/plugin-sdk/provider-model-shared` so direct Anthropic and proxy catalogs stay aligned.
