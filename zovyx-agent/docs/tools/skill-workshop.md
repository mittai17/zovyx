...
Skill Workshop is Zuvix's governed path for creating and updating workspace
skills.
...
```bash
zuvix skills workshop propose-update trip-planning --proposal ./PROPOSAL.md
```
...
- `autonomous.enabled`: allows Zuvix to create pending proposals from durable
...
```text
<ZUVIX_STATE_DIR>/skill-workshop/
  proposals.json
...
| Proposal does not show in list                 | Check the selected `--agent` workspace and `ZUVIX_STATE_DIR`.                                                                                                                                            |
...
- [Skills CLI](/cli/skills) for `zuvix skills` commands
