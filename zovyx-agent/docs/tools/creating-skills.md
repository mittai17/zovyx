...
summary: "Build, test, and publish custom SKILL.md workspace skills for your Zuvix agents."
read_when:
...
Zuvix loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.zuvix/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.zuvix/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    Zuvix watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ...
    ```bash
    zuvix agent --message "give me a greeting"
    ```
...
metadata: { "zuvix": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `zuvix.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `zuvix.json`:
...
zuvix skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.zuvix` gating fields
  ...
  <Step title="Install the ClawHub skill">
    ...
    ```bash
    zuvix skills install clawhub-publish
    ```
...
<Tip>
  ...
  - **Test locally** — use `zuvix agent --message "..."` before sharing.
