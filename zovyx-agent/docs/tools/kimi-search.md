...
Zuvix supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Zuvix reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.zuvix/.env`.

If you omit `baseUrl`, Zuvix defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Zuvix defaults to `kimi-k2.6`.
...
Zuvix treats Kimi `web_search` as successful only after Moonshot returns
...
Zuvix returns a structured `kimi_web_search_ungrounded` error instead of
