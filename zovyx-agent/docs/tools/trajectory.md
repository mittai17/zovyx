---
summary: "Export redacted trajectory bundles for debugging an Zuvix agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Zuvix session
...
Trajectory capture is Zuvix's per-session flight recorder. It records a
...
Zuvix writes the bundle under the workspace:

```text
.zuvix/trajectory-exports/zuvix-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.zuvix/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, Zuvix sends the
...
Trajectory capture is on by default for Zuvix agent runs.
...
| `metadata.json`       | Zuvix version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
Zuvix also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `ZUVIX_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export ZUVIX_TRAJECTORY_DIR=/var/lib/zuvix/trajectories
```

When this variable is set, Zuvix writes one JSONL file per session id in that
directory.
...
Set `ZUVIX_TRAJECTORY=0` before starting Zuvix:

```bash
export ZUVIX_TRAJECTORY=0
```
...
Zuvix flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`ZUVIX_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Zuvix:

```bash
export ZUVIX_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Zuvix logs an `zuvix-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `ZUVIX_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
Zuvix redacts sensitive values before writing export files:
...
- confirm Zuvix was started without `ZUVIX_TRAJECTORY=0`
- check whether `ZUVIX_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.zuvix/trajectory-exports/`
