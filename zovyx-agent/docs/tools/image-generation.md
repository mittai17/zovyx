...
Zuvix records a background task, returns the task id immediately, and wakes
...
generated images are still missing from the completion reply, Zuvix sends an
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, Zuvix routes image requests
...
nearby geometry option instead of the exact requested one, Zuvix remaps to
...
Zuvix tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when Zuvix can
  ...
  <Accordion title="Timeouts">
    ...
    Zuvix's 600000 ms dynamic-tool bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, Zuvix reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. Zuvix
    ...
    Zuvix forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible Zuvix maps
    ...
    transparency-capable OpenAI image model. Zuvix routes default
  ...
  <Accordion title="Microsoft Foundry MAI image models">
    ...
    onboarding/model metadata so Zuvix can verify that the deployment is
  ...
  <Accordion title="OpenRouter image models">
    ...
    Zuvix forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. Zuvix sends:
    ...
    Krea; Zuvix maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as Zuvix-managed image attachments

    Zuvix intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for Zuvix image generation" size=3840x2160 count=1
```
  </Tab>
...
`zuvix infer image edit`; `--openai-background` remains as an
