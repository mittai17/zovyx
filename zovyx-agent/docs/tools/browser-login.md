...
When a site requires login, **sign in manually** in the **host** browser profile (the zuvix browser).
...
Zuvix controls a **dedicated Chrome profile** (named `zuvix`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `zuvix` browser.
...
```bash
zuvix browser start
zuvix browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `zuvix`).
...
```bash
zuvix browser open https://x.com --browser-profile zuvix
```
