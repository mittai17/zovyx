...
read_when:
  - You want Zuvix to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current Zuvix session.
...
Zuvix waits for the next fresh session token snapshot and uses that as the
...
cost reporting, and context-window behavior still use the normal Zuvix
usage and model controls.

## Model tools

Zuvix exposes three core goal tools to agent harnesses:
...
The `/goal` command works in command-capable Zuvix sessions, including the
...
token snapshot yet. Usage refreshes as Zuvix records session usage and
