...
- repo: `zuvix/docs`
- local checkout: `~/Projects/zuvix-docs`

## Source of truth

- English docs are authored in `zuvix/zuvix`.
...
1. Edit English docs in `zuvix/zuvix`.
2. Push to `main`.
3. `zuvix/zuvix/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `zuvix/docs`.
...
5. `zuvix/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `zuvix/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `zuvix/docs` instead.
...
- Sync metadata is written to `.zuvix-sync/source.json` in the publish repo.
- Source repo secret: `ZUVIX_DOCS_SYNC_TOKEN`
- Publish repo secret: `ZUVIX_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `zuvix/docs` first.
