...
This hub links the core docs for how Zuvix connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`zuvix gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__zuvix__/canvas/`, `/__zuvix__/a2ui/`), protected by Gateway auth when bound beyond loopback.
