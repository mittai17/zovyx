---
summary: "Use Vydra image, video, and speech in Zuvix"
read_when:
  - You want Vydra media generation in Zuvix
...
Zuvix uses the same `VYDRA_API_KEY` for all three capabilities.
...
<Steps>
  <Step title="Run interactive onboarding">
    ```bash
    zuvix onboard --auth-choice vydra-api-key
    ```
...
<AccordionGroup>
  <Accordion title="Image generation">
    ...
    Current bundled support is text-to-image only. Vydra's hosted edit routes expect remote image URLs, and Zuvix does not add a Vydra-specific upload bridge in the bundled plugin yet.
  ...
  <Accordion title="Video live tests">
    ...
    ```bash
    export ZUVIX_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
    ```
