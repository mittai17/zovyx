---
summary: "Use Mistral models and Voxtral transcription with Zuvix"
read_when:
  - You want to use Mistral models in Zuvix
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

Zuvix includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    zuvix onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    zuvix onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    zuvix models list --provider mistral
    ```
  </Step>
</Steps>
...
Zuvix currently ships this bundled Mistral catalog:
...
```bash
zuvix models list --all --provider mistral --plain
```
...
<Note>
Zuvix defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    Zuvix maps the session **thinking** level to Mistral's API:

    | Zuvix thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so Zuvix sends
