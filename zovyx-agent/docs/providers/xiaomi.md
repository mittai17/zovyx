---
summary: "Use Xiaomi MiMo pay-as-you-go and Token Plan models with Zuvix"
read_when:
  - You want Xiaomi MiMo models in Zuvix
  - You need Xiaomi MiMo auth or Token Plan setup
title: "Xiaomi MiMo"
---

Xiaomi MiMo is the API platform for **MiMo** models. Zuvix includes a bundled Xiaomi plugin with two text-provider presets:
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    ```bash
    zuvix onboard --auth-choice xiaomi-api-key
    ```

    Token Plan:

    ```bash
    zuvix onboard --auth-choice xiaomi-token-plan-sgp
    ```

    Or pass the keys directly:

    ```bash
    zuvix onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    zuvix onboard --auth-choice xiaomi-token-plan-sgp --xiaomi-token-plan-api-key "$XIAOMI_TOKEN_PLAN_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    zuvix models list --provider xiaomi
    zuvix models list --provider xiaomi-token-plan
    ```
  </Step>
</Steps>
...
Zuvix sends `speakerVoice` for `mimo-v2.5-tts` and `mimo-v2-tts`.
...
`style` with the desired voice description; Zuvix sends it as the `user`
...
For voice-note targets such as Feishu and Telegram, Zuvix transcodes Xiaomi
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.zuvix/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.zuvix/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Zuvix configuration reference.
  </Card>
