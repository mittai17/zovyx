---
summary: "Use NovitaAI's OpenAI-compatible API with Zuvix"
read_when:
  - You want to run Zuvix with NovitaAI models
...
API. In Zuvix it is a bundled model provider, so the provider id is
...
This provider uses Novita's OpenAI-compatible endpoint. Zuvix handles
...
```bash
zuvix onboard --auth-choice novita-api-key
```
...
The catalog is a starting point for Zuvix model selection. Your account,
...
```bash
zuvix models list --provider novita
```

## Troubleshooting

- `401` or `403`: verify the key in Novita's key management page and re-run
  `zuvix onboard --auth-choice novita-api-key` if the stored profile is
  stale.
