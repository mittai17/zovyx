---
summary: "Use Venice AI privacy-focused models in Zuvix"
read_when:
  - You want privacy-focused inference in Zuvix
...
<Steps>
  ...
  <Step title="Configure Zuvix">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        zuvix onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    zuvix agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Zuvix shows all available Venice models. Pick based on your needs:
...
```bash
zuvix models set venice/kimi-k2-5
zuvix models set venice/claude-opus-4-6
```

List all available models:

```bash
zuvix models list --all --provider venice
```

You can also run `zuvix configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, Zuvix fills the required DeepSeek V4
...
Zuvix keeps that provider-specific replay fix separate from the native
...
Zuvix ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
zuvix agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
zuvix agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
zuvix agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
zuvix agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
zuvix agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `zuvix models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
