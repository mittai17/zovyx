---
summary: "Use Z.AI (GLM models) with Zuvix"
read_when:
  - You want Z.AI / GLM models in Zuvix
...
Zuvix uses the `zai` provider with a Z.AI API key.
...
GLM is a model family, not a separate provider. In Zuvix, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Zuvix probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        zuvix onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        zuvix models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        zuvix onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        zuvix onboard --auth-choice zai-coding-cn

        # General API
        zuvix onboard --auth-choice zai-global

        # General API CN (China region)
        zuvix onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
<Tip>
`zai-api-key` lets Zuvix detect the matching Z.AI endpoint from the key and
...
Zuvix ships the bundled `zai` provider catalog in the plugin manifest, so read-only
...
can change; run `zuvix models list --all --provider zai` to see the catalog
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows Zuvix's `/think` controls. With thinking off,
    Zuvix sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, Zuvix sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; Zuvix copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Zuvix config schema, including provider and model settings.
  </Card>
</CardGroup>
