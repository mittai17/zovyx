---
summary: "Use OpenRouter's unified API to access many models in Zuvix"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Zuvix
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        zuvix onboard --auth-choice openrouter-oauth
        ```

        Zuvix opens OpenRouter's browser sign-in flow, exchanges the PKCE
        code for an OpenRouter API key, and stores that key in the default
        OpenRouter auth profile. On remote/headless hosts, Zuvix prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        zuvix models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        zuvix onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
Zuvix submits text-to-video and image-to-video jobs to OpenRouter, polls
...
`google/lyria-3-clip-preview`. Zuvix sends `modalities: ["text",
...
Zuvix sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter STT contract), not as multipart OpenAI form uploads.

## Fusion router

Use OpenRouter Fusion when you want one Zuvix model ref to ask several
...
the upstream model slug is `openrouter/fusion`, the Zuvix model ref includes
both the Zuvix provider prefix and the upstream OpenRouter namespace:

```bash
zuvix models set openrouter/openrouter/fusion
```
...
`"required"` in normal Zuvix agent/chat turns to try to force Fusion;
Zuvix turns may include Zuvix tool definitions, and a top-level required
tool choice can require one of those tools instead of the Fusion router. When
this Fusion plugin config is present, Zuvix also adds a sanitized
...
Fusion is slower by design. OpenRouter may send the same Zuvix prompt to
...
OAuth is a PKCE login flow that issues an OpenRouter API key, so Zuvix stores
...
```bash
zuvix models auth login --provider openrouter --method oauth
```

Use `zuvix models auth login --provider openrouter --method api-key` when
you want to paste a key you created manually at OpenRouter.

On real OpenRouter requests (`https://openrouter.ai/api/v1`), Zuvix also adds
OpenRouter's documented app-attribution headers:

| Header                    | Value                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------ |
| `HTTP-Referer`            | `https://zuvix.ai`                                                                                  |
| `X-OpenRouter-Title`      | `Zuvix`                                                                                             |
| `X-OpenRouter-Categories` | `cli-agent,cloud-agent,programming-app,creative-writing,writing-assistant,general-chat,personal-agent` |

<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Zuvix
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Zuvix sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that Zuvix uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Zuvix maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. Zuvix sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Zuvix keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    Zuvix forwards that object to OpenRouter as the request `provider`
