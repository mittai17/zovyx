...
Kimi K2.7 Code always uses native thinking. Zuvix exposes only the `on`
thinking state for this model and omits outbound `thinking` and
`reasoning_effort` controls, as required by Moonshot. Zuvix also omits
    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        zuvix onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        zuvix onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        zuvix models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        ZUVIX_CONFIG_PATH=/tmp/zuvix-kimi/zuvix.json \
        ZUVIX_STATE_DIR=/tmp/zuvix-kimi \
        zuvix agent --local \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        zuvix onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        zuvix models list --provider kimi
        ```
      </Step>
    </Steps>
...
Zuvix also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    omit the `thinking` field for this model, so Zuvix exposes only `on` and
    ignores stale `off` settings. K2.7 also fixes `temperature`, `top_p`, `n`,
    `presence_penalty`, and `frequency_penalty`; Zuvix omits configured
    ...
    Zuvix maps runtime `/think` levels for those models:
    ...
    <Warning>
    When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Zuvix normalizes incompatible values to `auto`. This includes Kimi K2.7 Code, whose thinking mode cannot be disabled to preserve a pinned tool choice.
    </Warning>
    ...
    default strategy. Zuvix only forwards `thinking.keep` for
    `moonshot/kimi-k2.6` and strips it from other models. Kimi K2.7 Code
    preserves full reasoning history by default while Zuvix omits the entire
    `thinking` field.
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves native tool_call ids shaped like `functions.<name>:<index>`. For the OpenAI-completions transport, Zuvix preserves the first occurrence of each native Kimi id and rewrites later duplicates to deterministic OpenAI-style `call_*` ids. Matching tool results are remapped with the same id so replay remains unique without stripping Kimi's first native id.
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. Zuvix keys that off endpoint
