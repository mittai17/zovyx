---
summary: "Use NVIDIA's OpenAI-compatible API in Zuvix"
read_when:
  - You want to use open models in Zuvix for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). Zuvix
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    zuvix onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    zuvix models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
zuvix onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
When an NVIDIA API key is configured, Zuvix setup and model-selection paths
...
Zuvix release. When the live feed is available, the first returned model is
...
malformed, Zuvix falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in Zuvix. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    Zuvix prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. Zuvix's bundled Ultra row
