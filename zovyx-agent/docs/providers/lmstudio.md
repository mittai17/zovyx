---
summary: "Run Zuvix with LM Studio"
read_when:
  - You want to run Zuvix with open source models via LM Studio
...
If LM Studio authentication is disabled, you can leave the API key blank during interactive Zuvix setup.
...
```bash
zuvix models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). Zuvix
...
For unauthenticated LM Studio servers, omit the key; Zuvix stores a local non-secret marker.
...
`usage` object, Zuvix recovers token counts from llama.cpp-style
...
options, Zuvix exposes the matching OpenAI-compatible `reasoning_effort`
...
on `/v1/chat/completions`; Zuvix normalizes that binary discovery shape to
