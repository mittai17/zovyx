...
Deepgram is a speech-to-text API. In Zuvix it is used for inbound
...
For batch transcription, Zuvix uploads the complete audio file to Deepgram
and injects the transcript into the reply pipeline (`{{Transcript}}` +
`[Audio]` block). For Voice Call streaming, Zuvix forwards live G.711
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Zuvix transcribes it
