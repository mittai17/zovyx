---
summary: "Use Synthetic's Anthropic-compatible API in Zuvix"
read_when:
...
Zuvix registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    zuvix onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Zuvix's Anthropic client appends `/v1` to the base URL automatically, so use
...
`zuvix models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Zuvix appends `/v1` automatically.
