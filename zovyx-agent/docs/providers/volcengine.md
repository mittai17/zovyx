...
read_when:
  - You want to use Volcano Engine or Doubao models with Zuvix
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    zuvix onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    zuvix models list --provider volcengine
    zuvix models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
For voice-note targets, Zuvix asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `zuvix onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Zuvix falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.zuvix/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running Zuvix as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Zuvix setup.
  </Card>
</CardGroup>
