---
summary: "Use Ollama Cloud directly with Zuvix"
read_when:
...
Ollama Cloud is Ollama's hosted model API. It lets Zuvix call Ollama-hosted
...
`/api/chat` style, not the OpenAI-compatible `/v1` route. Zuvix registers it
...
- You want the same native Ollama chat API shape Zuvix uses for local
...
Zuvix discovers Ollama Cloud models from the live hosted catalog. Commonly
...
```bash
zuvix models list --provider ollama-cloud
zuvix models set ollama-cloud/kimi-k2.6
```
...
ZUVIX_LIVE_OLLAMA_MODEL=kimi-k2.6 \
...
- Unknown model errors: run `zuvix models list --provider ollama-cloud` and
