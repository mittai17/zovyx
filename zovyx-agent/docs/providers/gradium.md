---
summary: "Use Gradium text-to-speech in Zuvix"
read_when:
...
[Gradium](https://gradium.ai) is a bundled text-to-speech provider for Zuvix. The plugin can render normal audio replies (WAV), voice-note-compatible Opus output, and 8 kHz u-law audio for telephony surfaces.
...
Create a Gradium API key, then expose it to Zuvix with either an env var or the config key.
...
The output audio format is selected automatically by the runtime based on the target surface and is not configurable from `zuvix.json`. See [Output](#output) below.
...
Among configured TTS providers, Gradium's auto-select order is `30`. See [Text-to-Speech](/tools/tts) for how Zuvix picks the active provider when `messages.tts.provider` is not pinned.
