---
summary: "Run Zuvix through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run Zuvix against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the Zuvix config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. Zuvix connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled Zuvix provider plugin. Configure it under
...
<Warning>
Zuvix agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the Zuvix provider config">
    ...
    zuvix infer model run \
      ...
      --thinking off \
      --prompt "Reply with exactly: zuvix-ds4-ok" \
      --json
    ```
...
aligned with `--tokens` unless you intentionally want Zuvix to request less
output than the server default.

## On-demand startup

Zuvix can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and Zuvix model
metadata:
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the Zuvix turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when Zuvix starts the server on
    demand.
  </Accordion>
</AccordionGroup>
