---
summary: "Sign in to GitHub Copilot from Zuvix using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `zuvix models auth login-github-copilot` flow
  - You are choosing between the built-in Copilot provider, Copilot SDK harness, and Copilot Proxy
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. Zuvix can use Copilot as a model
provider or agent runtime in three different ways.

## Three ways to use Copilot in Zuvix

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Zuvix runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        zuvix models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        zuvix models set github-copilot/claude-opus-4.7
        ```
  ...
  <Tab title="Copilot SDK harness plugin (copilot)">
    Install the external `@zuvix/copilot` plugin when you want GitHub's
    Copilot CLI and SDK to own the low-level agent loop for selected
    `github-copilot/*` models.

    ```bash
    zuvix plugins install clawhub:@zuvix/copilot
    ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Zuvix talks to
...
```bash
# Skip confirmation
zuvix models auth login-github-copilot --yes

# Login and set the default model in one step
zuvix models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

If you already have a GitHub OAuth access token for Copilot, import it during
headless setup with `zuvix onboard --non-interactive`:
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    Zuvix refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    churn. Newly published Copilot models become visible without an Zuvix
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Zuvix
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    Zuvix sends Copilot IDE-style request headers on Copilot transports,
  ...
  <Accordion title="Environment variable resolution order">
    Zuvix resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Zuvix uses the highest-priority one.
    The device-login flow (`zuvix models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Zuvix runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Zuvix can use it for embeddings without a separate API key.

### Config

Set `memorySearch.provider` explicitly to use GitHub Copilot embeddings. If a
GitHub token is available, Zuvix discovers available embedding models from
...
1. Zuvix resolves your GitHub token (from env vars or auth profile).
...
available, Zuvix skips Copilot and tries the next provider.
