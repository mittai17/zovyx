...
```bash
zuvix tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `zuvix chat` and `zuvix terminal` are aliases for `zuvix tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- After a config file has authored settings, `zuvix` and `zuvix crestodian` also use this TUI shell, with Crestodian as the local setup and repair chat backend.
...
- To show the Gateway host for non-local URL-backed connections, opt in with:
  ...
  ```bash
  zuvix config set tui.footer.showRemoteHost true
  ```
...
- Turn delivery on:
  ...
  - or start with `zuvix tui --deliver`
...
- Local shell commands receive `ZUVIX_SHELL=tui-local` in their environment.
...
If `zuvix config validate` is already failing, start with `zuvix configure`
or `zuvix doctor --fix` first. `zuvix chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `zuvix config set` or `zuvix configure`, then rerun `!zuvix config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!zuvix doctor --fix`.

Tips:

- Prefer `zuvix config set` or `zuvix configure` over hand-editing `zuvix.json`.
- `zuvix docs "<query>"` searches the live docs index from the same machine.
- `zuvix config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `ZUVIX_THEME=light` before launching `zuvix tui`.
- To force the original dark palette instead, set `ZUVIX_THEME=dark`.
...
- Check the Gateway logs: `zuvix logs --follow`.
- Confirm the agent can run: `zuvix status` and `zuvix models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `zuvix agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `zuvix.json`
