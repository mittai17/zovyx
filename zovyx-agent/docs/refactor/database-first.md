...
read_when:
  - Moving Zuvix runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.zuvix/state/zuvix.sqlite`
...
- Configuration stays file-backed: `zuvix.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside Zuvix's database.
...
- Raw stream logging uses `ZUVIX_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of Zuvix
  runtime or tests.
...
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/zuvix-agent.sqlite`.
- Config remains file-backed. `zuvix.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/zuvix.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/zuvix-state-schema.sql`,
      `src/state/zuvix-agent-schema.sql`,
      `src/state/*generated*`.
...
- `zuvix doctor --fix` owns the legacy file-to-database migration step.
  Runtime startup and `zuvix migrate` should not carry legacy Zuvix
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the Zuvix `sessionId` as the canonical
...
former stores now write to `state/zuvix.sqlite` or the owning
`zuvix-agent.sqlite` database.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `zuvix-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/zuvix-agent-db.ts` opens
  `agents/<agentId>/agent/zuvix-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one Zuvix session to one or more external
  ...
  Zuvix session can represent multiple external DM peers without demoting
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `zuvix.json` remains the config store;
...
- Workspace bootstrap completion markers now live in typed shared
  ...
  `.zuvix/workspace-state.json`; runtime no longer reads or rewrites the
  legacy workspace marker, and helper APIs no longer pass around a fake
  `.zuvix/setup-state` path just to derive storage identity.
- Exec approvals now live in the typed shared SQLite `exec_approvals_config`
  singleton row. Doctor imports legacy `~/.zuvix/exec-approvals.json`;
  ...
  `state/zuvix.sqlite` table row; it keeps only the Unix prompt socket on disk
...
- GitHub Copilot compaction no longer writes `zuvix-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and Zuvix keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`ZuvixKit`) uses the same
  `state/zuvix.sqlite` rows for device identity and device auth. macOS app
...
- Android device identity uses the same TypeScript-compatible key material
  stored in typed `state/zuvix.sqlite#table/device_identities` rows. It never
  reads or writes `zuvix/identity/device.json`; a leftover legacy file blocks
  startup until doctor imports it into SQLite.
- Android cached device auth tokens also use typed
  `state/zuvix.sqlite#table/device_auth_tokens` rows and share the same
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `zuvix update --channel` now fails on invalid legacy config and asks the
...
- Plugin state now uses the shared `state/zuvix.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/zuvix.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `zuvix sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `ZUVIX_CACHE_TRACE_FILE`,
  `ZUVIX_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `ZUVIX_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.zuvix-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.zuvix-wiki/state.json` or the unused
  `.zuvix-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `zuvix.json`. The config
...
- Codex app-server conversation bindings now key SQLite plugin state by
  Zuvix session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `zuvix-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the Zuvix temp root, not durable Zuvix state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$ZUVIX_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$ZUVIX_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.zuvix/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `zuvix-cli-images` cache
...
- `zuvix secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/zuvix.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.zuvix-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.zuvix-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store SQLite plugin blob rows instead of
  writing `.zuvix-wiki/cache/agent-digest.json` and
  `.zuvix-wiki/cache/claims.jsonl`. The migration provider imports old cache
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  Zuvix temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `zuvix backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `zuvix backup restore` validates the archive before extraction, reuses the
...
- `zuvix reset --scope config+creds+sessions` removes per-agent
  `zuvix-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureZuvixModelCatalog`; there is no `models.json` compatibility API in
...
- Memory-core runtime imports SQLite transcript indexing helpers from
  `zuvix/plugin-sdk/memory-core-host-engine-session-transcripts`, not the
...
- QMD's own `index.sqlite` is now a temp runtime materialization backed by the
  ...
  `~/.zuvix/agents/<agentId>/qmd` sidecar.
- The optional `memory-lancedb` plugin no longer creates
  `~/.zuvix/memory/lancedb` as an implicit Zuvix-managed store. It is an
...
`zuvix doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `zuvix migrate` must not import legacy Zuvix state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `zuvix doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/zuvix.sqlite`:
...
```text
~/.zuvix/state/zuvix.sqlite
~/.zuvix/agents/<agentId>/agent/zuvix-agent.sqlite
```
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `zuvix backup create` does this archive verification by default;
...
5. Verify the archive by opening every included SQLite snapshot and running
   ...
   `zuvix backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `zuvix sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `zuvix backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  Zuvix test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `zuvix-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- Memory Wiki `.zuvix-wiki/log.jsonl`
- Memory Wiki `.zuvix-wiki/state.json`
- Memory Wiki `.zuvix-wiki/locks/`
- Memory Wiki `.zuvix-wiki/source-sync.json`
- Memory Wiki `.zuvix-wiki/import-runs/*.json`
- Memory Wiki `.zuvix-wiki/cache/agent-digest.json`
- Memory Wiki `.zuvix-wiki/cache/claims.jsonl`
- ClawHub `.clawhub/lock.json`
- ClawHub `.clawhub/origin.json`
- Browser profile decoration `.zuvix-profile-decorated`
