...
An **agent harness** is the low level executor for one prepared Zuvix agent
...
runtime and the normal Zuvix provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Zuvix
  session transcript
...
Before a harness is selected, Zuvix has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Zuvix transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an Zuvix-owned
policy bundle for runtime decisions that must stay shared across Zuvix and native
harnesses:
...
Harnesses may use the plan for decisions that need to match Zuvix behavior, but
should still treat it as host-owned attempt state. Do not mutate it or use it to
switch providers/models inside a turn.

## Register a harness

**Import:** `zuvix/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "zuvix/plugin-sdk/agent-harness";
...
Zuvix chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, Zuvix uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, Zuvix does not
...
`agents.list[].agentRuntime`, and `ZUVIX_AGENT_RUNTIME`. `/status` shows the
...
Zuvix. The harness then claims that provider in `supports(...)`.
...
- app-server request: Zuvix sends the bare model id to Codex and lets the
...
Zuvix requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Zuvix only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning Zuvix to the newer tested stable line.
...
seam is for async tool-result transforms that must run before Zuvix or Codex
...
`zuvix/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so Zuvix's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; Zuvix does not infer it from assistant prose. The helper intentionally
leaves prompt errors, in-flight turns, and intentional silent replies such as
`NO_REPLY` unclassified.

### Native Codex harness mode

The bundled `codex` harness is the native Codex mode for embedded Zuvix
...
`zuvix doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Zuvix still owns the chat channel,
...
By default, Zuvix uses `auto` provider/model runtime policy: registered
...
fail hard. This does not block an explicit provider/model `agentRuntime.id: "zuvix"`.
...
Keep that binding explicitly associated with the Zuvix session, and keep
mirroring user-visible assistant/tool output into the Zuvix transcript.

The Zuvix transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in Zuvix harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so Zuvix can
clear it when the owning Zuvix session is reset.

## Tool and media results

Core constructs the Zuvix tool list and passes it into the prepared attempt.
...
on the same delivery path as Zuvix-backed runs.
