---
summary: "Run Zuvix embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / zuvix / copilot) and want it to run through the Copilot CLI
---

The external `@zuvix/copilot` plugin lets Zuvix run embedded subscription
...
Zuvix still owns chat channels, session files, model selection, Zuvix
...
[Side questions (`/btw`)](#side-questions-btw)), and `zuvix doctor`.
...
- Zuvix with the `@zuvix/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest
  id declared by the plugin). A restrictive
  allowlist that uses the npm-style `@zuvix/copilot` package name
...
- A writable `copilotHome` directory. The harness defaults to
  `~/.zuvix/agents/<agentId>/copilot` for full per-agent isolation. The
...
The Copilot runtime is an external plugin so the core `zuvix` package does
...
Without the opt-in, zuvix uses its built-in GitHub Copilot provider
and never installs the runtime plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@zuvix/copilot`
   package.
2. The well-known fallback dir `~/.zuvix/npm-runtime/copilot/` (the
...
4. **Env-var fallback** for direct CLI / dogfood runs where no auth
   ...
   setup:
   1. `ZUVIX_GITHUB_TOKEN` -- harness-specific override; set this
      to pin a token for the Zuvix harness without disturbing
...
(isolating SDK state from Zuvix's `models.json` / `auth-profiles.json` in
the same directory), or `~/.zuvix/agents/<agentId>/copilot` otherwise.
...
- `model` — string or `{ provider, id, api? }`. When omitted, Zuvix uses
...
- `reasoningEffort` — `"low" | "medium" | "high" | "xhigh"`. Maps from
  Zuvix's `ThinkLevel` / `ReasoningLevel` resolution in
...
- `infiniteSessionConfig` — optional override for the SDK
  ...
  leave as-is.
- `hooksConfig` — optional bridge config exposing Zuvix
...
- `permissionPolicy` — optional override for the SDK's
  ...
  invokes any of those kinds because every bridged Zuvix tool is
  registered with `overridesBuiltInTool: true` and
  `skipPermission: true` so 100% of tool calls flow through Zuvix's
...
Nothing in the rest of Zuvix needs to know about these fields. Other
...
The Zuvix side transcript mirror (see below) continues to receive the
...
deliberately leaves `harness.runSideQuestion` undefined, so Zuvix's `/btw`
...
asserted in
[`extensions/copilot/harness.test.ts`](https://github.com/zuvix/zuvix/blob/main/extensions/copilot/harness.test.ts)
...
that hosts (including `zuvix doctor`) can call to verify the environment:
...
- **Interactive `ask_user` is not yet wired.** The SDK's
  ...
  `UserInputRequest`s through the Zuvix channel/TUI prompt path; the
...
Permission enforcement for bridged Zuvix tools happens **inside the
...
`createZuvixCodingTools` to every coding tool: loop detection,
...
`createZuvixCodingTools` — identity (`senderIsOwner`,
