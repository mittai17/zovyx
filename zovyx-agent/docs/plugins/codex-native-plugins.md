---
summary: "Configure migrated native Codex plugins for Codex-mode Zuvix agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Zuvix agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Zuvix agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Zuvix turn.

Zuvix does not translate Codex plugins into synthetic `codex_plugin_*`
Zuvix dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected Zuvix agent runtime must be the native Codex harness.
...
`codexPlugins` has no effect on Zuvix runs, normal OpenAI provider runs, ACP
...
```bash
zuvix migrate codex --dry-run --verify-plugin-apps
```
...
`enable` and `disable` write only to Zuvix config at
`~/.zuvix/zuvix.json`; they do not edit `~/.codex/config.toml` or install
...
- Enabled: Zuvix config is willing to make the plugin available to Codex
  harness turns.
...
Migration is the durable install/eligibility step. During planning, Zuvix
...
Zuvix reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. The cache is
in memory only; restarting the CLI or gateway drops it, and Zuvix rebuilds it
...
A plugin app is exposed only when Zuvix can map it back to the migrated
...
Zuvix injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

Zuvix sets app-level `destructive_enabled` from the effective global or
...
plugin apps are emitted with `open_world_enabled: true`; Zuvix does not
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, Zuvix returns a deterministic decline.
- When policy is `true`, Zuvix auto-accepts only safe schemas it can map to
  an approval response, such as a boolean approve field.
- When policy is `"auto"`, Zuvix exposes destructive plugin actions to
  Codex but turns ownership-proven MCP approval elicitations into Zuvix
...
empty or stale cache. Zuvix schedules an async refresh and excludes plugin
...
Codex thread bindings keep the app config they started with until Zuvix
