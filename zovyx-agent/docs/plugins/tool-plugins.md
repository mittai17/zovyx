---
summary: "Build simple typed agent tools with defineToolPlugin and zuvix plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple Zuvix plugin that only adds agent tools
...
Tool plugins add agent-callable tools to Zuvix without adding a channel,
model provider, hook, service, or setup backend. Use `defineToolPlugin` when the
plugin owns a fixed list of tools and you want Zuvix to generate the manifest
metadata that keeps those tools discoverable without loading runtime code.

The recommended flow is:

1. Scaffold a package with `zuvix plugins init`.
2. Write tools with `defineToolPlugin`.
3. Build JavaScript.
4. Generate `zuvix.plugin.json` and `package.json` metadata with
   `zuvix plugins build`.
...
- `zuvix >=2026.5.17`, the first Zuvix version that exports
  `zuvix/plugin-sdk/tool-plugin`.
- A package root that can ship `dist/`, `zuvix.plugin.json`, and
  `package.json`.
...
- `zuvix.plugin.json`: generated manifest metadata for the initial tool.
...
import { defineToolPlugin } from "zuvix/plugin-sdk/tool-plugin";
...
`zuvix plugins build` writes the matching `toolMetadata.<tool>.optional`
manifest entry, so Zuvix can discover the tool without loading plugin
runtime code.
...
`defineToolPlugin` wraps plain return values into the Zuvix tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and Zuvix to keep the original value in `details`.
...
`configSchema` is optional. If you omit it, Zuvix uses a strict empty object
...
Zuvix reads plugin config from the plugin entry in the Gateway config. Do not
...
Zuvix discovers installed plugins from cold metadata. It must be able to read
the plugin manifest before importing plugin runtime code. `defineToolPlugin`
therefore exposes static metadata, and `zuvix plugins build` writes that
...
`contracts.tools` is the important discovery contract. It tells Zuvix which
...
For the simple tool-plugin workflow, `zuvix plugins build` aligns
`package.json` to the selected single runtime entry:

```json
{
  "type": "module",
  "files": ["dist", "zuvix.plugin.json", "README.md"],
  "dependencies": {
    "typebox": "^1.1.38"
  },
  "peerDependencies": {
...
- `zuvix.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `zuvix.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate Zuvix checkout or installed CLI, install the package path:

```bash
zuvix plugins install ./stock-quotes
zuvix plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
zuvix plugins install npm-pack:./zuvix-plugin-stock-quotes-0.1.0.tgz
zuvix plugins inspect stock-quotes --runtime --json
```
...
```bash
zuvix plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs remain supported during the launch cutover, but ClawHub
is the preferred discovery and distribution surface for Zuvix plugins.
...
`zuvix plugins build --entry ./dist/index.js` or
`zuvix plugins validate --entry ./dist/index.js`.
...
### `zuvix.plugin.json generated metadata is stale`
...
Commit both `zuvix.plugin.json` and `package.json` changes.

### `package.json zuvix.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`zuvix plugins build --entry ./dist/index.js` so the generator aligns the
...
1. `zuvix plugins inspect <plugin-id> --runtime`
2. `zuvix plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `zuvix.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `zuvix.extensions: ["./dist/index.js"]`.
