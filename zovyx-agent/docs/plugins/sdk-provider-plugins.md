---
summary: "Step-by-step guide to building a model provider plugin for Zuvix"
title: "Building provider plugins"
sidebarTitle: "Provider plugins"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Zuvix
  - You need to understand provider auth, catalogs, and runtime hooks
---

This guide walks through building a provider plugin that adds a model provider
(LLM) to Zuvix. By the end you will have a provider with a model catalog,
...
<Tip>
  Provider plugins add models to Zuvix's normal inference loop. If the model
...
<Steps>
  <Step title="Package and manifest">
    ...
    The manifest declares `setup.providers[].envVars` so Zuvix can detect
    ...
    is optional and lets Zuvix auto-load your provider plugin from shorthand
    model ids like `acme-large` before runtime hooks exist. If you publish the
    provider on ClawHub, those `zuvix.compat` and `zuvix.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "zuvix/plugin-sdk/provider-auth";
    ...
    response mapping in the plugin; Zuvix owns the shared row shape, source
    labels, and help rendering.

    That is a working provider. Users can now
    `zuvix onboard --acme-ai-api-key <key>` and select
    ...
    `zuvix/plugin-sdk/provider-catalog-live-runtime` for the shared fetch
    ...
    putting provider policy in Zuvix core.
    ...
    } from "zuvix/plugin-sdk/provider-catalog-live-runtime";
    ...
    metadata and the plugin needs to project rows into Zuvix model
    definitions itself:
    ...
    If the upstream provider uses different control tokens than Zuvix, add a
    ...
    Zuvix parses its own control markers or channel delivery.
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "zuvix/plugin-sdk/provider-entry";
    ...
    `buildProvider` is the live catalog path used when Zuvix can resolve real
    ...
    Zuvix's `models list --all` display currently executes static catalogs
    ...
    `zuvix/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `zuvix/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "zuvix/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "zuvix/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "zuvix/plugin-sdk/provider-tools";
    ...
    | `passthrough-gemini` | Gemini thought-signature sanitation for Gemini models running through OpenAI-compatible proxy transports; does not enable native Gemini replay validation or bootstrap rewrites | `openrouter`, `kilocode`, `zuvix`, `zuvix-go` |
    ...
    <Accordion title="SDK seams powering the family builders">
      ...
      - `zuvix/plugin-sdk/provider-model-shared` - `ProviderReplayFamily`, `buildProviderReplayFamilyHooks(...)`, and the raw replay builders (`buildOpenAICompatibleReplayPolicy`, `buildAnthropicReplayPolicyForModel`, `buildGoogleGeminiReplayPolicy`, `buildHybridAnthropicOrOpenAIReplayPolicy`). Also exports Gemini replay helpers (`sanitizeGoogleGeminiReplayHistory`, `resolveTaggedReasoningOutputMode`) and endpoint/model helpers (`resolveProviderEndpoint`, `normalizeProviderId`, `normalizeGooglePreviewModelId`).
      - `zuvix/plugin-sdk/provider-stream` - `ProviderStreamFamily`, `buildProviderStreamFamilyHooks(...)`, `composeProviderStreamWrappers(...)`, plus the shared OpenAI/Codex wrappers (`createOpenAIAttributionHeadersWrapper`, `createOpenAIFastModeWrapper`, `createOpenAIServiceTierWrapper`, `createOpenAIResponsesContextManagementWrapper`, `createCodexNativeWebSearchWrapper`), DeepSeek V4 OpenAI-compatible wrapper (`createDeepSeekV4OpenAICompatibleThinkingWrapper`), Anthropic Messages thinking prefill cleanup (`createAnthropicThinkingPrefillPayloadWrapper`), plain-text tool-call compat (`createPlainTextToolCallCompatWrapper`), and shared proxy/provider wrappers (`createOpenRouterWrapper`, `createToolStreamWrapper`, `createMinimaxFastModeWrapper`).
      - `zuvix/plugin-sdk/provider-stream-shared` - lightweight payload and event wrappers for hot provider paths, including `createOpenAICompatibleCompletionsThinkingOffWrapper`, `createPayloadPatchStreamWrapper`, and `createPlainTextToolCallCompatWrapper`.
      - `zuvix/plugin-sdk/provider-tools` - `ProviderToolCompatFamily`, `buildProviderToolCompatFamilyHooks("deepseek" | "gemini" | "openai")`, and underlying provider schema helpers.
      ...
      reasoning output so Zuvix consumes native thought parts without adding
    ...
    <Tabs>
      ...
      <Tab title="Usage and billing">
        ...
        auth but has no usable usage token, and Zuvix must skip generic
        API-key/OAuth fallback. Return `null` or `undefined` when the provider did
        not handle the request and Zuvix should continue with generic fallback.
      </Tab>
    </Tabs>

    <Accordion title="All available provider hooks">
      Zuvix calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    web fetch, and web search alongside text inference. Zuvix classifies this as a
    ...
    <Tabs>
      <Tab title="Speech (TTS)">
        ...
        import {
          ...
          postJsonRequest,
        } from "zuvix/plugin-sdk/provider-http";
      ...
      <Tab title="Realtime transcription">
        ...
        `zuvix/plugin-sdk/provider-http`. The helper normalizes upload
      ...
      <Tab title="Media understanding">
        ...
        Zuvix still keeps the normal auth gate for providers that do not
...
├── package.json              # zuvix.providers metadata
├── zuvix.plugin.json      # Manifest with provider auth metadata
