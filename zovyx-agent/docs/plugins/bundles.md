---
summary: "Install and use Codex, Claude, and Cursor bundles as Zuvix plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Zuvix maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Zuvix can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Zuvix maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Zuvix plugins. Native plugins run
...
of requiring authors to rewrite them as native Zuvix plugins, Zuvix
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    zuvix plugins install ./my-bundle

    # Archive
    zuvix plugins install ./my-bundle.tgz
...
## What Zuvix maps from bundles

Not every bundle feature runs in Zuvix today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Zuvix skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                        | Claude, Cursor |
| Hook packs    | Zuvix-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded Zuvix settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded Zuvix LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded Zuvix defaults                                     | Claude         |

#### Skill content

- bundle skill roots load as normal Zuvix skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Zuvix skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Zuvix hook-pack
  layout. Today this is primarily the Codex-compatible case:
  - `HOOK.md`
  - `handler.ts` or `handler.js`

#### MCP for embedded Zuvix

- enabled bundles can contribute MCP server config
- Zuvix merges bundle MCP config into the effective embedded Zuvix settings as
  `mcpServers`
- Zuvix exposes supported bundle MCP tools during embedded Zuvix agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Zuvix uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Zuvix config. `zuvix mcp set` and `zuvix doctor --fix` normalize the common alias.
...
Zuvix registers bundle MCP tools with provider-safe names in the form
...
#### Embedded Zuvix settings

- Claude `settings.json` is imported as default embedded Zuvix settings when the
  bundle is enabled
- Zuvix sanitizes shell override keys before applying them
...
- Zuvix loads `.lsp.json` plus any manifest-declared `lspServers` paths
- bundle LSP config is merged into the effective embedded Zuvix LSP defaults
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `zuvix plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but Zuvix does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Zuvix best when they use skill roots and Zuvix-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded Zuvix settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded Zuvix
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded Zuvix LSP defaults
...
Zuvix checks for native plugin format first:

1. `zuvix.plugin.json` or valid `package.json` with `zuvix.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Zuvix uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `zuvix plugins install` and ship everything
  they need in the installed plugin directory.
- Zuvix-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `zuvix doctor --fix` removes legacy staged dependency directories and can
...
- Zuvix does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `zuvix plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Zuvix settings from `settings.json` are supported. Zuvix does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Zuvix hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
