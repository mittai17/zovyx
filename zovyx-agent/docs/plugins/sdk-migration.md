...
read_when:
  - You see the ZUVIX_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the ZUVIX_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before Zuvix 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Zuvix plugin
---

Zuvix has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`zuvix/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`zuvix/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`zuvix/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`zuvix/extension-api`** - a bridge that gave plugins direct access to
...
Zuvix does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`zuvix/plugin-sdk/\<subpath\>`)
...
`zuvix/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `zuvix doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `zuvix/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `ZuvixConfig` | `zuvix/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `zuvix/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `zuvix/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `zuvix/plugin-sdk/config-mutation` |
    | Session store helpers | `zuvix/plugin-sdk/session-store-runtime` |
    | Markdown table config | `zuvix/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `zuvix/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `zuvix/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `zuvix/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // Zuvix and Codex runtime dynamic tools
    api.registerAgentToolResultMiddleware(async (event) => {
      return compactToolResult(event);
    }, {
      runtimes: ["zuvix", "codex"],
    });
    ```
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `zuvix/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `zuvix/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "zuvix/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "zuvix/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "zuvix/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "zuvix/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "zuvix/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "zuvix/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `zuvix/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `zuvix/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `zuvix/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `zuvix/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `zuvix/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `zuvix/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `zuvix/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `zuvix/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `zuvix/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `zuvix/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `zuvix/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `zuvix/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `zuvix/plugin-sdk/error-runtime` |
    | Transport readiness waits | `zuvix/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `zuvix/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `zuvix/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `zuvix/plugin-sdk/number-runtime` |
    | Process-local async lock | `zuvix/plugin-sdk/async-lock-runtime` |
    | File locks | `zuvix/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `zuvix/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `ZuvixSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | Zuvix-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@zuvix/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`zuvix/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`zuvix/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "zuvix/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "zuvix/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `zuvix/plugin-sdk/channel-inbound` or
    `zuvix/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `zuvix/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `zuvix/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `zuvix/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `zuvix/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    Zuvix no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. Zuvix downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="ZuvixSchemaType alias → ZuvixConfig">
    `ZuvixSchemaType` re-exported from `zuvix/plugin-sdk` is now a
    one-line alias for `ZuvixConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { ZuvixSchemaType } from "zuvix/plugin-sdk";
    // After
    import type { ZuvixConfig } from "zuvix/plugin-sdk/config-schema";
    ```
...
```bash
ZUVIX_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 zuvix gateway run
ZUVIX_SUPPRESS_EXTENSION_API_WARNING=1 zuvix gateway run
```
