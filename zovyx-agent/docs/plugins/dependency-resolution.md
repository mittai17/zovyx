---
summary: "How Zuvix installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Zuvix installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Zuvix keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the Zuvix
package directory.
...
- SDK/core imports are peer or supplied Zuvix imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into Zuvix-owned package roots

Zuvix owns only the plugin lifecycle:
...
Zuvix uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.zuvix/npm/projects/<encoded-package>`
- git packages clone under `~/.zuvix/git`
...
```bash
cd ~/.zuvix/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`zuvix plugins install npm-pack:<path.tgz>` uses that same per-plugin npm
project root for a local npm-pack tarball. Zuvix reads the tarball's npm
...
`node_modules` beside the plugin package. Zuvix scans the managed project
...
publishable lockfile during install, and Zuvix's managed npm project root
supports it through the normal npm install path. Zuvix-owned publishable
...
Zuvix does not require it for community packages, but npm will respect it
when present.

Zuvix-owned npm plugin packages can also publish with explicit
...
with `zuvix.release.bundleRuntimeDependencies: false`; those packages still
...
`zuvix` package does not bundle its full dependency tree.

Plugins that import `zuvix/plugin-sdk/*` declare `zuvix` as a peer
dependency. Zuvix does not let npm install a separate registry copy of the
...
resolution/materialization and Zuvix reasserts plugin-local
`node_modules/zuvix` links for installed packages that declare the host peer
...
Local plugins are treated as developer-controlled directories. Zuvix does not
...
```bash
zuvix plugins update <id>
zuvix plugins install <source>
zuvix doctor --fix
```

`doctor --fix` can clean legacy Zuvix-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of Zuvix.
...
In source checkouts, Zuvix treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g zuvix`        | Built runtime tree inside the package | Zuvix package and explicit plugin install/update/doctor flows     |
...
| `zuvix plugins install ...`   | Managed npm project/git/ClawHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older Zuvix versions generated bundled-plugin dependency roots at startup or
...
`.zuvix-runtime-deps*` manifests, generated plugin `node_modules`, install
...
Older npm installs also used a shared `~/.zuvix/npm/node_modules` root.
