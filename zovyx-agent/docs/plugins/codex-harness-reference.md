...
By default, Zuvix starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Zuvix builds its inherited environment.                                                                                                                                                                                                                              |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active Zuvix sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows Zuvix sandbox egress.                                                                                                                                                        |
...
unattended Zuvix turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, Zuvix treats the implicit default as guardian
...
When an Zuvix sandbox is active, the local Codex app-server process still
runs on the Gateway host. Zuvix therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the Zuvix sandbox
backend. Shell access is exposed through Zuvix sandbox-backed dynamic tools
...
`workspace-write` without active Zuvix sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`zuvix doctor` and fix the reported host namespace policy for the Zuvix
...
The stable default is fail-closed: active Zuvix sandboxing disables native
...
try Codex's remote environment support with Zuvix's sandbox backend. This
...
When the flag is on and the current Zuvix session is sandboxed, Zuvix
...
Zuvix-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so Zuvix rejects
that combination.
...
1. An explicit Zuvix Codex auth profile for the agent.
...
Stdio app-server launches inherit Zuvix's process environment by default.
Zuvix owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's Zuvix state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the Zuvix agent
instead of leaking in from the operator's personal `~/.codex` home.

Zuvix does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `zuvix`, `gh`, `git`, cloud CLIs, and shell commands see
...
Zuvix plugins and Zuvix skill snapshots still flow through Zuvix's own
...
part of an Zuvix agent, inventory them explicitly:

```bash
zuvix migrate codex --dry-run
zuvix migrate apply codex --yes
```
...
Most remaining Zuvix integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `zuvix` namespace. This keeps the initial
...
surface, while explicit Zuvix or ACP delegation is still available through
the `zuvix` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, Zuvix aborts the
...
post-tool idle budget: after Zuvix returns an `item/tool/call` response, after
...
`turn/completed`, Zuvix best-effort interrupts the native turn and releases
...
availability is owned by Codex app-server, so the list can change when Zuvix
...
If discovery fails or times out, Zuvix uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. Zuvix
...
For Zuvix workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
Zuvix Codex developer instructions because they define the active agent,
available workspace guidance, and user profile. The compact Zuvix skills
...
`BOOTSTRAP.md` when present is forwarded as Zuvix turn input reference
context.
