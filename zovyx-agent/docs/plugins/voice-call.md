...
read_when:
  - You want to place an outbound voice call from Zuvix
...
Voice calls for Zuvix via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        zuvix plugins install @zuvix/voice-call
        ```
      </Tab>
      <Tab title="From a local folder (dev)">
        ```bash
        PLUGIN_SRC=./path/to/local/voice-call-plugin
        zuvix plugins install "$PLUGIN_SRC"
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `zuvix doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `zuvix doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `zuvix_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Zuvix tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `zuvix_agent_consult`.
...
configured Zuvix agent without paying a full agent-consult round trip on
...
`zuvix_agent_consult` still run the full Zuvix agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call zuvix_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `zuvix doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
zuvix voicecall call --to "+15555550123" --message "Hello from Zuvix"
zuvix voicecall start --to "+15555550123"   # alias for call
zuvix voicecall continue --call-id <id> --message "Any questions?"
zuvix voicecall speak --call-id <id> --message "One moment"
zuvix voicecall dtmf --call-id <id> --digits "ww123456#"
zuvix voicecall end --call-id <id>
zuvix voicecall status --call-id <id>
zuvix voicecall tail
zuvix voicecall latency                      # summarize turn latency from logs
zuvix voicecall expose --mode funnel
```
...
Provider signatures are checked against the public URL Zuvix reconstructs
...
Use `zuvix logs --follow` for the live phase trace. A healthy Twilio Meet
...
`zuvix voicecall tail` still shows persisted call records; it is useful for
...
- `zuvix logs --follow` shows realtime TwiML served, the realtime bridge
