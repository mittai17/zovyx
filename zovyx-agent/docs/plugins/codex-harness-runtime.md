...
read_when:
  ...
  - You are changing plugin behavior across Zuvix and Codex harness turns
---
...
Codex mode is not Zuvix with a different model call underneath. Codex owns more of
the native model loop, and Zuvix adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

Zuvix still owns channel routing, session files, visible message delivery,
Zuvix dynamic tools, approvals, media delivery, and a transcript mirror.
...
explicit Zuvix compatibility route keeps the normal Zuvix system prompt even
...
according to the active Codex thread config. Zuvix starts and resumes native
Codex threads with Codex's built-in personality disabled so workspace
personality files and Zuvix agent identity stay authoritative. Lightweight
Zuvix runs still preserve their existing project-doc suppression. Zuvix
developer instructions cover Zuvix runtime concerns such as source-channel
delivery, Zuvix dynamic tools, ACP delegation, adapter context, and the
active agent workspace profile files. Zuvix skill catalogs and tool-routed
...
When an Zuvix session is attached to an existing Codex thread, the next turn
...
normally, and Zuvix posts the final text to the source conversation. Set
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable Zuvix
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Zuvix plugin hooks                 | Zuvix                 | Product/plugin compatibility across Zuvix and Codex harnesses.   |
| Codex app-server extension middleware | Zuvix bundled plugins | Per-turn adapter behavior around Zuvix dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Zuvix does not use project or global Codex `hooks.json` files to route
Zuvix plugin behavior. For the supported native tool and permission bridge,
Zuvix injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
Codex's app-server reviewer and Zuvix's approval bridge handle real
...
Codex-level controls. They are not exposed as Zuvix plugin hooks in the v1
contract.

For Zuvix dynamic tools, Zuvix executes the tool after Codex asks for the
call, so Zuvix fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
Zuvix can mirror selected events, but it cannot rewrite the native Codex
...
to the matching app-server approval. If an Zuvix `before_tool_call` hook
returns `requireApproval` while the native payload sets report approval mode
(`zuvix_approval_mode` is `"report"`), the native hook relay records the
plugin approval requirement and returns no native decision. When Codex sends the
app-server approval request for the same tool use, Zuvix opens the plugin
approval prompt and maps the decision back to Codex. Codex `PermissionRequest`
events are a separate approval path and can still route through Zuvix
...
notifications and Zuvix adapter state, not native Codex hook commands.
Zuvix's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke Zuvix plugin hooks.
...
| Zuvix channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                    |
| Zuvix dynamic tools                        | Supported                                                                        | Codex asks Zuvix to execute these tools, so Zuvix stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | Zuvix projects Zuvix-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. Zuvix disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Zuvix-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through Zuvix after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | Zuvix records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Zuvix does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Zuvix owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Zuvix-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Zuvix can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Zuvix does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Zuvix can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, Zuvix only returns explicit allow or deny decisions
...
Zuvix remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through Zuvix's plugin
...
default `messages.queue.mode: "steer"`, Zuvix batches steer-mode chat
...
case, Zuvix waits for the active run to finish before starting the prompt.
...
harness, Zuvix also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, Zuvix session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, Zuvix does not print those Codex ids and
...
to Codex app-server. Zuvix does not run preflight compaction for Codex turns,
does not replace Codex compaction with context-engine compaction, and does not
fall back to Zuvix or public OpenAI summarization when native Codex
compaction cannot be started. Zuvix keeps a transcript mirror for channel
...
Zuvix returns after starting that native operation. It does not wait for
completion, impose a separate Zuvix timeout, restart the shared Codex
app-server, or record the operation as an Zuvix-completed compaction.

When a context engine requests Codex thread-bootstrap projection, Zuvix
...
reasoning or plan records when the app-server emits them. Today, Zuvix only
...
Zuvix is writing an Zuvix-owned session transcript tool result.

## Media and delivery

Zuvix continues to own media delivery and media provider selection. Image,
...
through the normal Zuvix delivery path. Media generation does not require the legacy runtime.
When Codex emits a native image-generation item with a `savedPath`, Zuvix
