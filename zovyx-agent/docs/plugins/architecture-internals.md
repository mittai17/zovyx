...
At startup, Zuvix does roughly this:
...
The manifest is the control-plane source of truth. Zuvix uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, Zuvix
...
Zuvix does not cache plugin discovery results or direct manifest registry
...
Zuvix still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Zuvix calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Zuvix no longer calls, such as
...
| --  | _(built-in model lookup)_         | Zuvix tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| 24  | `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared Zuvix refreshers                                                                                          |
...
that still runs on Zuvix's normal inference loop.

`resolveUsageAuth` decides whether Zuvix should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Zuvix's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Zuvix adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while Zuvix owns the
...
- Zuvix only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Zuvix plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-zuvix-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-zuvix-scopes` only when the header is explicitly present
  - if `x-zuvix-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-zuvix-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `zuvix/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `zuvix/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `zuvix/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `zuvix/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `zuvix/plugin-sdk/config-schema` | Root `zuvix.json` Zod schema (`ZuvixSchema`) |
...
<Info>
`zuvix/plugin-sdk/channel-runtime`, `zuvix/plugin-sdk/channel-lifecycle`,
small channel helper facades, `zuvix/plugin-sdk/outbound-runtime`,
`zuvix/plugin-sdk/outbound-send-deps`, `zuvix/plugin-sdk/config-runtime`,
and `zuvix/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `zuvix/plugin-sdk/*` subpaths. Never
...
`zuvix/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Zuvix writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Zuvix's
...
- if both `catalog` and `discovery` are registered, Zuvix uses `catalog`
...
- Read-only command paths such as `zuvix status`, `zuvix status --all`,
  `zuvix channels status`, `zuvix channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `zuvix.extensions`:
...
Security guardrail: every `zuvix.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `zuvix plugins install` installs plugin dependencies with a
...
Optional: `zuvix.setupEntry` can point at a lightweight setup-only module.
When Zuvix needs setup surfaces for a disabled channel plugin, or
...
Optional: `zuvix.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Zuvix load the
...
Channel plugins can advertise setup/discovery metadata via `zuvix.channel` and
install hints via `zuvix.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@zuvix/nextcloud-talk",
  "zuvix": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Useful `zuvix.channel` fields beyond the minimal example:
...
Zuvix can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.zuvix/mpm/plugins.json`
- `~/.zuvix/mpm/catalog.json`
- `~/.zuvix/plugins/catalog.json`

Or point `ZUVIX_PLUGIN_CATALOG_PATHS` (or `ZUVIX_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "zuvix": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `zuvix.install` block. The
...
2. add typed plugin registration/runtime surfaces
   Extend `ZuvixPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Zuvix stays opinionated without becoming hardcoded to one
