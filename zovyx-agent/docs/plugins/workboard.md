...
Zuvix Gateway; it is not a replacement for GitHub Issues, Linear, Jira, or
...
state directory and move with the rest of that Gateway's Zuvix state.
...
under the Zuvix state directory. Boards, cards, labels, lifecycle events,
...
`zuvix doctor --fix` to migrate the shipped legacy plugin-state namespaces
...
system processes; normal Zuvix subagent sessions still own execution. The
...
Zuvix records the orchestration intent and exposes it in worker context; the
...
zuvix workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
zuvix workboard show <card-id>
zuvix workboard dispatch
```

`zuvix workboard dispatch` calls the running Gateway so worker starts use the
...
```bash
zuvix plugins disable workboard
zuvix gateway restart
```
