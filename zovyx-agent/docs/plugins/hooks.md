...
Plugin hooks are in-process extension points for Zuvix plugins. Use them
...
current plugin options; Zuvix injects it per handler without mutating the
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when Zuvix has resolved the child session's native model before launch.
...
use `zuvix sessions` or the Gateway session/status surfaces. When debugging
...
- Zuvix strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, Zuvix stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when Zuvix can
...
identifier when Zuvix can derive one from the session key or delivery
metadata.
...
forever. A timeout is logged and Zuvix continues; it does not cancel
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when Zuvix can derive a
...
Codex native `Stop` hooks are relayed into this hook as Zuvix
...
reach the next model turn exactly once. Zuvix drains queued injections before
...
policy runs from Zuvix config, covers CLI install and update paths, and fails
closed when enabled but unavailable.

`before_install` is a plugin-runtime lifecycle hook. It runs after
`security.installPolicy` only in the Zuvix process where plugin hooks have
...
field remains in the event payload for compatibility, but Zuvix no longer
...
context when syncing external wake schedulers, and keep Zuvix as the
