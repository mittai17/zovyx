...
If the active memory plugin exposes shared recall artifacts, Zuvix can search
...
currently exposing public bridge inputs yet. Run `zuvix wiki doctor` first,
...
`zuvix wiki status`, `zuvix wiki doctor`, and `zuvix wiki bridge
...
artifact, then let `memory-wiki` turn it into Zuvix-native concept pages and
compiled digests.
...
zuvix wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
