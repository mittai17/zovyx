...
The Policy plugin contributes doctor health checks for policy-managed Zuvix
...
posture, ingress/channel access posture, data-handling posture, and Zuvix config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
Zuvix settings and workspace declarations as evidence, and reports drift
through `zuvix policy check` and `zuvix doctor --lint`. A clean policy
check emits policy, evidence, findings, and attestation hashes that operators
can record for audit.

`zuvix policy compare --baseline <file>` compares one policy file to another
