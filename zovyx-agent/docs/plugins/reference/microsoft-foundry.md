---
summary: "Adds Microsoft Foundry model provider support to Zuvix."
read_when:
...
Adds Microsoft Foundry model provider support to Zuvix.

## Distribution

- Package: `@zuvix/microsoft-foundry`
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. Zuvix refreshes Microsoft Foundry runtime tokens through
...
Zuvix uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so Zuvix
...
deployment through onboarding or include model metadata so Zuvix can verify
