---
summary: "Adds Volcengine, Volcengine Plan model provider support to Zuvix."
read_when:
  - You are installing, configuring, or auditing the volcengine plugin
title: "Volcengine plugin"
---

# Volcengine plugin

Adds Volcengine, Volcengine Plan model provider support to Zuvix.

## Distribution

- Package: `@zuvix/volcengine-provider`
- Install route: included in Zuvix

## Surface

providers: volcengine, volcengine-plan; contracts: speechProviders

## Related docs

- [volcengine](/providers/volcengine)
