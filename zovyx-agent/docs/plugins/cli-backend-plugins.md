...
read_when:
  ...
  - You need to map a third-party CLI into Zuvix's text fallback runner
---

CLI backend plugins let Zuvix call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Zuvix at the plugin runtime module              |
| Manifest ownership   | `zuvix.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `zuvix.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json zuvix.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Zuvix",
    ...
    `cliBackends` is the runtime ownership list. It lets Zuvix auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "zuvix/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how Zuvix should launch and parse the CLI:
...
| `modelAliases`                            | Map Zuvix model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields Zuvix reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific Zuvix auth profile                                     |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into Zuvix's loopback MCP tool bridge                                |
| `ownsNativeCompaction`             | Backend owns its own compaction - Zuvix defers                           |
...
otherwise Zuvix fails closed when BTW requires a no-tools CLI run.

### `ownsNativeCompaction`: opting out of Zuvix compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so Zuvix's safeguard summarizer never runs against its
...
stay over budget / go stale (Zuvix no longer rescues it):
...
CLI backends do not receive Zuvix tools by default. If the CLI can consume an
...
"always-on"` so Zuvix can fail closed when a caller requires no native tools.
...
```bash
zuvix plugins inspect acme-cli --runtime --json
zuvix agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `zuvix.extensions` and built runtime entries for published packages</Check>
<Check>`zuvix.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
