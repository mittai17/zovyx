...
read_when:
  ...
  - You want a reference for all registration methods on ZuvixPluginApi
...
<Note>
  This page is for plugin authors using `zuvix/plugin-sdk/*` inside
  Zuvix. For external apps, scripts, dashboards, CI jobs, and IDE extensions
...
prefer `zuvix/plugin-sdk/channel-core`; keep `zuvix/plugin-sdk/core` for
...
`zuvix.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. Zuvix's
...
<Warning>
  ...
  `zuvix/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`zuvix/plugin-sdk/discord` and `zuvix/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `ZuvixPluginApi` object with these
methods:
...
agentPromptGuidance: [
  "Global command hint.",
  { text: "Only show this in the main Zuvix prompt.", surfaces: ["zuvix_main"] },
];
```

Structured `surfaces` may include `zuvix_main`, `codex_app_server`,
`cli_backend`, `acp_backend`, or `subagent`. `pi_main` remains a deprecated alias
for `zuvix_main`. Omit `surfaces` for intentional all-surface guidance. Do
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `zuvix nodes` |
...
runtime, for example `["zuvix", "codex"]`. Installed plugins without that
contract, or without explicit enablement, cannot register this middleware; keep
normal Zuvix plugin hooks for work that does not need pre-model tool-result
timing. The old
...
Gateway on a local discovery transport such as mDNS/Bonjour. Zuvix calls the
...
`zuvix nodes canvas` explicit plugin-owned node features.
...
- User config still wins. Zuvix merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping Zuvix thinking levels to a native effort
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `zuvix/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep Zuvix as the source of truth for due checks and execution.
...
| `api.config`             | `ZuvixConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `zuvix/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Zuvix is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through Zuvix's plugin
...
- **`@zuvix/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@zuvix/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `zuvix/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `zuvix/plugin-sdk/speech`, `.../provider-model-shared`, or another
