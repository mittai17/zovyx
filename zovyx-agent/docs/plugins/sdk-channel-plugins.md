---
summary: "Step-by-step guide to building a messaging channel plugin for Zuvix"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Zuvix to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects Zuvix to a
...
<Info>
  If you have not built any Zuvix plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Zuvix keeps one
...
`defineChannelMessageAdapter` from `zuvix/plugin-sdk/channel-outbound`. The
...
receive/send lifecycle helpers on `zuvix/plugin-sdk/channel-outbound`.

Channels migrating inbound authorization can use the experimental
`zuvix/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
`zuvix/plugin-sdk/outbound-media` with plugin state stores. Keep platform
...
Use `zuvix/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `zuvix/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If custom approval auth intentionally allows only same-chat fallback, return `markImplicitSameChatApprovalAuthorization({ authorized: true })` from `zuvix/plugin-sdk/approval-auth-runtime`; otherwise core treats the result as explicit approver authorization.
...
- `zuvix/plugin-sdk/approval-auth-runtime`
- `zuvix/plugin-sdk/approval-client-runtime`
- `zuvix/plugin-sdk/approval-delivery-runtime`
- `zuvix/plugin-sdk/approval-gateway-runtime`
- `zuvix/plugin-sdk/approval-handler-adapter-runtime`
- `zuvix/plugin-sdk/approval-handler-runtime`
- `zuvix/plugin-sdk/approval-native-runtime`
- `zuvix/plugin-sdk/approval-reply-runtime`
- `zuvix/plugin-sdk/channel-runtime-context`

Likewise, prefer `zuvix/plugin-sdk/setup-runtime`,
`zuvix/plugin-sdk/setup-runtime`,
`zuvix/plugin-sdk/reply-runtime`,
`zuvix/plugin-sdk/reply-dispatch-runtime`,
`zuvix/plugin-sdk/reply-reference`, and
`zuvix/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `zuvix/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `zuvix/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `zuvix/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `zuvix.setupEntry` in
...
- use the broader `zuvix/plugin-sdk/setup` seam only when you also need the
...
- `zuvix/plugin-sdk/account-core`,
  `zuvix/plugin-sdk/account-id`,
  `zuvix/plugin-sdk/account-resolution`, and
  `zuvix/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `zuvix/plugin-sdk/inbound-envelope` and
  `zuvix/plugin-sdk/channel-inbound` for inbound route/envelope and
  record-and-dispatch wiring
- `zuvix/plugin-sdk/channel-targets` for target parsing helpers
- `zuvix/plugin-sdk/outbound-media` for media loading and
  `zuvix/plugin-sdk/channel-outbound` for outbound identity/send delegates
  and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `zuvix/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `zuvix/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `zuvix/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `zuvix/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `zuvix/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `zuvix/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`zuvix/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#zuvix-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/zuvix-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "zuvix": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Zuvix to Acme Chat."
        }
      }
    }
    ```

    ```json zuvix.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { ZuvixConfig } from "zuvix/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "zuvix/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Zuvix
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "zuvix/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Zuvix loads this instead of the full entry when the channel is disabled
    ...
    `zuvix/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Zuvix. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Zuvix.
...
├── package.json              # zuvix.channel metadata
├── zuvix.plugin.json      # Manifest with config schema
