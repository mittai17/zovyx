...
when Zuvix loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, Zuvix will use a
...
**Import:** `zuvix/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the Zuvix tool-result format, and
exposes static metadata that `zuvix plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, Zuvix uses a strict empty object
...
- Tool names are static. `zuvix plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `zuvix.plugin.json` and `package.json` `zuvix.extensions`; Zuvix does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `zuvix/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `ZuvixPluginConfigSchema \| () => ZuvixPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: ZuvixPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `zuvix.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Zuvix resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `zuvix/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: ZuvixPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: ZuvixPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. Zuvix may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Zuvix
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `zuvix nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; Zuvix resolves it to the
...
Zuvix loads this instead of the full entry when a channel is disabled,
...
- `zuvix/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `zuvix/plugin-sdk/channel-setup` for optional-install setup surfaces
- `zuvix/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`zuvix/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "zuvix/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so Zuvix can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Zuvix to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; Zuvix rejects descriptor names outside
...
Zuvix classifies loaded plugins by their registration behavior:
...
Use `zuvix plugins inspect <id>` to see a plugin's shape.
