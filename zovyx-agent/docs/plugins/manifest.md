...
read_when:
  - You are building an Zuvix plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native Zuvix plugin manifest** only.
...
Zuvix auto-detects those bundle layouts too, but they are not validated
against the `zuvix.plugin.json` schema described here.

For compatible bundles, Zuvix currently reads bundle metadata plus declared
...
Zuvix runtime expectations.

Every native Zuvix plugin **must** ship a `zuvix.plugin.json` file in the
**plugin root**. Zuvix uses this manifest to validate configuration
...
`zuvix.plugin.json` is the metadata Zuvix reads **before it loads your
...
- QA runner metadata the shared `zuvix qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; Zuvix still reads this during the deprecation window.                                                             |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                                      |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that Zuvix can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                                        |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `zuvix qa` host before plugin runtime loads.                                                                                                                                                  |
...
Zuvix reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so Zuvix can
...
If a tool has no `toolMetadata`, Zuvix preserves the existing behavior and
...
Zuvix reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                                              | User-facing label. If omitted, Zuvix falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Zuvix
...
the shared `zuvix qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `zuvix qa`, for example `matrix`.    |
...
Zuvix also includes `setup.providers[].envVars` in generic provider auth and
...
Zuvix can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. Zuvix treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `zuvix.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
Zuvix reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that Zuvix can
read without importing the plugin runtime.

```json
{
  "contracts": {
    "agentToolResultMiddleware": ["zuvix", "codex"],
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `zuvix migrate`.                                                                         |
...
Zuvix reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, Zuvix can still load the plugin, but
...
the same defaults through `package.json#zuvix.channel.commands` alongside
...
When `channels.chat` is configured, Zuvix considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, Zuvix disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, Zuvix
...
Use `modelSupport` when Zuvix should infer your provider plugin from
...
Use `modelCatalog` when Zuvix should know provider model metadata before
...
provider-filtered list uses an alias, Zuvix can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching Zuvix model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when Zuvix
...
exec provider preset. Zuvix reads this metadata before plugin runtime loads,
...
The map key is the integration id. If `providerAlias` is omitted, Zuvix uses
...
When an operator selects the preset, Zuvix writes a provider reference like:
...
At startup/reload, Zuvix resolves that provider by loading current plugin
...
Zuvix materializes it at startup/reload to the current Node executable and
...
Zuvix derives `trustedDirs` for manifest presets from the plugin root and,
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the Zuvix provider id, for example `z-ai` for a `zai` provider. |
...
### Zuvix Provider Index

The Zuvix Provider Index is Zuvix-owned preview metadata for providers
...
4. Zuvix Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `zuvix doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `zuvix.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `zuvix` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Zuvix must know it before loading plugin code, put it in `zuvix.plugin.json`
...
`zuvix` block instead of `zuvix.plugin.json`.
`zuvix.bundle` and `zuvix.bundle.json` are not Zuvix plugin contracts;
native plugins must use `zuvix.plugin.json` plus the supported
`package.json#zuvix` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `zuvix.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `zuvix.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `zuvix.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `zuvix.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `zuvix.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `zuvix.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `zuvix.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `zuvix.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `zuvix.install.clawhubSpec` / `zuvix.install.npmSpec` / `zuvix.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `zuvix.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `zuvix.install.minHostVersion`                                                          | Minimum supported Zuvix host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `zuvix.compat.pluginApi`                                                                | Minimum Zuvix plugin API range required by this package, using a semver floor like `>=2026.5.27`.                                                                                 |
| `zuvix.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `zuvix.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `zuvix.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-runtime channel surfaces load before listen, then defers the full configured channel plugin until post-listen activation.                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#zuvix.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `zuvix.plugin.json`.

`zuvix.install.minHostVersion` is enforced during install and manifest
...
`zuvix.compat.pluginApi` is enforced during package install for non-bundled
plugin sources. Use it for the Zuvix plugin SDK/runtime API floor that the
...
flows. Official Zuvix release sync bumps existing official plugin API floors
to the Zuvix release version by default, but plugin-only releases can keep a
lower floor when the package intentionally supports older hosts. Do not use the
package version alone as the compatibility contract. `peerDependencies.zuvix`
remains npm package metadata; Zuvix uses the `zuvix.compat.pluginApi`
...
`"npmSpec": "@wecom/wecom-zuvix-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `zuvix.setupEntry` when status, channel list,
...
entrypoint fields. For example, `zuvix.runtimeExtensions` cannot make an
escaping `zuvix.extensions` path loadable.

`zuvix.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
`zuvix.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`zuvix.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
Zuvix discovers plugins from several roots. For the raw filesystem scan
...
2. **Bundled** — plugins shipped with Zuvix
3. **Global install** — plugins installed into the global Zuvix plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `zuvix.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native Zuvix plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `ZuvixPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
