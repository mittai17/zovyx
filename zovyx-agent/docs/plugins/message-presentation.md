...
Message presentation is Zuvix's shared contract for rich outbound chat UI.
...
```bash
zuvix message send --channel slack \
  --target channel:C123 \
...
```bash
zuvix message send --channel telegram \
...
Use helpers from `zuvix/plugin-sdk/interactive-runtime` when bridging older
code:
