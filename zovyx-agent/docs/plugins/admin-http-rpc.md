...
The plugin is included with Zuvix, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for Zuvix clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    zuvix plugins enable admin-http-rpc
...
```bash
zuvix plugins disable admin-http-rpc
zuvix gateway restart
```
...
- For `token` and `password` auth, narrower `x-zuvix-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-zuvix-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for Zuvix clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
