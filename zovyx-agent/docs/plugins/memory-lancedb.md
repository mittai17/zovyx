...
The plugin is published to npm and is not bundled into the Zuvix runtime image.
...
Set `dimensions` for non-standard embedding models. Zuvix knows the
...
`memory_forget` query path, and `zuvix ltm search`. Auto-recall prefers the
...
```bash
zuvix ltm list
zuvix ltm search "project preferences"
zuvix ltm stats
```
...
```bash
zuvix ltm query --cols id,text,createdAt --limit 20
zuvix ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.zuvix/memory/lancedb`. Override the
path with `dbPath`:
...
Zuvix treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade Zuvix and restart the
Gateway.
...
```bash
zuvix ltm stats
zuvix ltm search "recent preference"
```
