---
summary: "Bundled `oc-path` plugin: ships the `zuvix path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`zuvix path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Zuvix repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `zuvix path … --json` instead of carrying separate markdown, JSONC,
...
zuvix path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
zuvix path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `zuvix` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/zuvix.plugin.json`:
...
`zuvix path …`, so installs that never use the verb pay no cost.
...
state. Bare `zuvix path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
zuvix plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `zuvix path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__ZUVIX_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`zuvix path` CLI reference](/cli/path)
