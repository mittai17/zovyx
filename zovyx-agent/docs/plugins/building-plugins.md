---
summary: "Create your first Zuvix plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new Zuvix plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend Zuvix without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the Zuvix repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
zuvix plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because Zuvix loads bundled
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Zuvix to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through Zuvix model fallback.
  </Card>
...
```json zuvix.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to Zuvix",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    must appear in `contracts.tools` so Zuvix can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    zuvix plugins inspect my-plugin --runtime --json
    ```
    ...
    `zuvix demo-plugin ping`.

    For a bundled plugin in this repository, Zuvix discovers source-checkout
  ...
  <Step title="Install">
    ...
    ```bash
    zuvix plugins install clawhub:your-org/your-plugin
    ```
...
operator, installed plugin code, or a modified Zuvix runtime. Sensitive local
...
aligned with `api.registerTool(..., { optional: true })` so Zuvix can avoid
...
```typescript
import { definePluginEntry } from "zuvix/plugin-sdk";
```
...
`zuvix/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `zuvix` metadata</Check>
<Check>**zuvix.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [zuvix/zuvix](https://github.com/zuvix/zuvix/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Zuvix X account [@zuvix](https://x.com/zuvix) for release announcements.
