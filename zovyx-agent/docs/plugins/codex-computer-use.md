---
summary: "Set up Codex Computer Use for Codex-mode Zuvix agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Zuvix agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Zuvix
...
Use this page when Zuvix is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## Zuvix.app and Peekaboo

Zuvix.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Zuvix.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Zuvix agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Zuvix node and exposes mobile
...
Zuvix-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Zuvix's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Zuvix command:

```bash
cua-driver mcp-config --client zuvix
```
...
available as a normal Zuvix MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. Zuvix
...
Computer Use in and lets Zuvix install or re-enable it before the turn:
...
With this config, Zuvix checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, Zuvix asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, Zuvix also tries to
...
plugin command surface is available. These are Zuvix chat/runtime commands,
not `zuvix codex ...` CLI subcommands:
...
Zuvix uses the same app-server API that Codex itself exposes. The
...
During install, Zuvix polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, Zuvix prefers
...
`computer-use` is registered, Zuvix tries to add the standard bundled
...
Zuvix reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If Zuvix says Computer Use
...
Zuvix intentionally fails closed when `computerUse.enabled` is true. A
...
tool hook could not reach an active Zuvix relay through the local bridge or
Gateway fallback. Start a fresh Zuvix session with `/new` or `/reset`. If it
works once and then fails again on a later tool call, `/new` is only clearing the
current attempt; restart the Codex app-server or Zuvix Gateway so old threads
