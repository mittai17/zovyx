...
read_when:
  - You maintain an Zuvix plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Zuvix keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core Zuvix repo as a separate
...
```sh
zuvix-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. Zuvix
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `zuvix` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against Zuvix plugin packages.
Run it from a clean Zuvix checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @zuvix/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @zuvix/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @zuvix/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
...
Zuvix should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `zuvix/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `zuvix/extension-api`,
  `zuvix/plugin-sdk/channel-runtime`, `zuvix/plugin-sdk/command-auth`
  status builders, `zuvix/plugin-sdk/test-utils` (replaced by focused
  `zuvix/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `ZuvixSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `zuvix/plugin-sdk/channel-route`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `zuvix plugins registry --refresh` and
  `zuvix doctor --fix`
