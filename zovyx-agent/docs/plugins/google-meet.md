...
read_when:
  - You want an Zuvix agent to join a Google Meet call
  - You want an Zuvix agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for Zuvix — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured Zuvix agent answers, and regular Zuvix TTS speaks into Meet.
...
provider plus regular Zuvix TTS. OpenAI is the default transcription
...
```bash
zuvix googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `zuvix googlemeet setup --json` for
...
```bash
zuvix googlemeet setup --transport twilio
```
...
```bash
zuvix googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
zuvix googlemeet create --transport chrome-node --mode agent
```
...
```bash
zuvix googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. Zuvix uses the
  ...
  the Zuvix Chrome profile on the node to already be signed in to Google.
...
Zuvix's microphone/camera permission grant and avoid the Meet **Use
...
Use `zuvix googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in Zuvix browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by Zuvix. For
...
You do **not** need a full Zuvix Gateway or model API key inside a macOS VM
...
- Gateway host: Zuvix Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: Zuvix CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update Zuvix in the VM, then enable the bundled plugin there:

```bash
zuvix plugins enable google-meet
```

Start the node host in the VM:

```bash
zuvix node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
ZUVIX_ALLOW_INSECURE_PRIVATE_WS=1 \
  zuvix node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
ZUVIX_ALLOW_INSECURE_PRIVATE_WS=1 \
  zuvix node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
zuvix node restart
```

`ZUVIX_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`zuvix.json` setting. `zuvix node install` stores it in the LaunchAgent
...
```bash
zuvix googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, Zuvix browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, Zuvix auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `zuvix node run` in the VM,
  approve pairing, and make sure `zuvix plugins enable google-meet` and
  `zuvix plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses Zuvix
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. Zuvix
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by Zuvix; use separate virtual devices or Loopback-style routing
...
Zuvix does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with Zuvix, review BlackHole's
...
Chrome transport opens the Meet URL through Zuvix browser control and joins
as the signed-in Zuvix browser profile. On macOS, the plugin checks for
...
```bash
zuvix googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
zuvix googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local Zuvix audio
...
meeting; Zuvix does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Zuvix agent. Be brief.",
...
zuvix plugins list | grep -E 'google-meet|voice-call'
zuvix googlemeet setup
```
...
`ZUVIX_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes Zuvix requests:
...
`meetings.space.readonly` lets Zuvix resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets Zuvix pass `SpaceConfig` settings such as
...
```bash
zuvix googlemeet auth login --json
```
...
```bash
ZUVIX_GOOGLE_MEET_CLIENT_ID="your-client-id" \
ZUVIX_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
zuvix googlemeet auth login --json
```
...
ZUVIX_GOOGLE_MEET_CLIENT_ID="your-client-id" \
ZUVIX_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
zuvix googlemeet auth login --json --manual
```
...
support existed, rerun `zuvix googlemeet auth login --json` so the refresh
...
```bash
zuvix googlemeet doctor --oauth --json
```
...
```bash
zuvix googlemeet doctor --oauth --create-space --json
zuvix googlemeet create --no-join --json
```
...
```bash
zuvix googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
zuvix googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `zuvix googlemeet auth login
...
- `ZUVIX_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `ZUVIX_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `ZUVIX_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `ZUVIX_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `ZUVIX_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `ZUVIX_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `ZUVIX_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
zuvix googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
zuvix googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
zuvix googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
zuvix googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
zuvix googlemeet latest --today
zuvix googlemeet calendar-events --today --json
zuvix googlemeet artifacts --event "Weekly sync"
zuvix googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
zuvix googlemeet latest --meeting https://meet.google.com/abc-defg-hij
zuvix googlemeet artifacts --conference-record conferenceRecords/abc123 --json
zuvix googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
zuvix googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
Zuvix accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops Zuvix's local/session
...
```bash
zuvix googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
zuvix googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
zuvix googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
zuvix googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
ZUVIX_LIVE_TEST=1 \
ZUVIX_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
zuvix googlemeet setup --transport chrome-node --mode transcribe
zuvix googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `ZUVIX_LIVE_TEST=1` enables guarded live tests.
- `ZUVIX_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `ZUVIX_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `ZUVIX_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `ZUVIX_GOOGLE_MEET_CLIENT_SECRET`,
  `ZUVIX_GOOGLE_MEET_ACCESS_TOKEN`, and
  `ZUVIX_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `ZUVIX_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Zuvix browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the Zuvix browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, Zuvix reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured Zuvix TTS provider.
...
- `chrome.guestName: "Zuvix Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through Zuvix browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  Zuvix agent in a per-meeting sub-agent session, and spoken back through the
  normal Zuvix TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `zuvix_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `zuvix_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional Zuvix agent id for
  `zuvix_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `zuvix_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured Zuvix
agent produces the answer, and regular Zuvix TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means Zuvix did
...
transcripts are routed through the configured Zuvix agent, and the answer is
spoken through the normal Zuvix TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Zuvix agent | Normal Zuvix TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal Zuvix tools, it can call `zuvix_agent_consult`.

The consult tool runs the regular Zuvix agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
Zuvix sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated Zuvix agent workspace, model defaults,
...
```bash
zuvix googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `zuvix logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
zuvix plugins list | grep google-meet
zuvix googlemeet setup
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; Zuvix
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, Zuvix
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `ZUVIX_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `zuvix googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the Zuvix Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. Zuvix should click **Use microphone** or, for
...
```bash
zuvix googlemeet setup
zuvix googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> Zuvix agent -> TTS talk-back path,
...
run `zuvix googlemeet status --json <session-id>` after participants speak
...
utterance but Zuvix did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  Zuvix. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
zuvix googlemeet recover-tab
zuvix googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
zuvix googlemeet setup --transport twilio
zuvix voicecall setup
zuvix voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
zuvix voicecall smoke --to "+15555550123"
```
...
```bash
zuvix voicecall smoke --to "+15555550123" --yes
```
...
- Run `zuvix googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `zuvix voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `zuvix voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `zuvix logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `zuvix googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `zuvix logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: Zuvix owns the
