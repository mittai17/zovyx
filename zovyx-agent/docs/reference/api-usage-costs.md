...
Zuvix features that can generate provider usage or paid API calls.
...
- If Zuvix has usage metadata and local pricing for the active model,
...
- Gemini CLI note: when the CLI returns JSON output, Zuvix reads usage from
  `stats`, normalizes `stats.cached` into `cacheRead`, and derives input tokens
  from `stats.input_tokens - stats.cached` when needed.

Anthropic note: Anthropic staff told us Zuvix-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that Zuvix can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `zuvix status --usage` and `zuvix channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Zuvix inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Zuvix prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Zuvix falls back to matching OAuth/API-key
...
Zuvix can pick up credentials from:
...
Zuvix's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Zuvix Claude-login path with **Extra Usage** enabled.
...
`zuvix models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
