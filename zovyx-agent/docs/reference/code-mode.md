---
summary: "Zuvix code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable Zuvix code mode for an agent run
...
Code mode is an experimental Zuvix agent-runtime feature. It is off by
default. When you enable it, Zuvix changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents Zuvix code mode. It is not Codex Code mode. The two
...
- Zuvix code mode is disabled unless `tools.codeMode.enabled: true` is
  configured. It runs in the Zuvix generic agent runtime, where the model
...
surfaces. Zuvix code mode is an Zuvix-owned experimental tool-surface
adapter for generic Zuvix runs. It uses `quickjs-wasi`, a hidden Zuvix
tool catalog, and the normal Zuvix tool executor.

## What is this?

Zuvix code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal Zuvix tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same Zuvix execution path used by normal agent turns.
...
surface. It does not replace Zuvix tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for Zuvix, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through Zuvix
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, Zuvix fails closed instead of falling back to broad direct tool
  exposure.
...
```bash
ZUVIX_DEBUG_CODE_MODE=1 \
ZUVIX_DEBUG_MODEL_TRANSPORT=1 \
ZUVIX_DEBUG_MODEL_PAYLOAD=tools \
zuvix gateway
```
...
`ZUVIX_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
- Stability: experimental Zuvix surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic Zuvix agent runs.
...
**Code mode** is the Zuvix runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, Zuvix fails closed for
...
2. Build the effective Zuvix tool list.
...
- If `language` is `"typescript"`, Zuvix transpiles before evaluation.
...
final value is JSON-compatible after Zuvix's output adapter runs.
...
`wait` exists because nested Zuvix tools can be slow, interactive, approval
...
2. On suspension, Zuvix snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. Zuvix re-registers host callbacks by stable names.
5. Zuvix delivers nested tool results into the restored VM.
6. Zuvix drains QuickJS pending jobs.
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "zuvix" | "plugin" | "mcp" | "client";
  sourceName?: string;
};
```
...
const hits = await tools.web_search({ query: "Zuvix code mode" });
```
...
state directory. For each code-mode `exec` call, Zuvix builds the run-scoped
...
inside the QuickJS program while Zuvix still shows only `exec` and `wait` to
the model.
...
7. Zuvix auto-drains ready namespace bridge calls inside the active
...
- `globalName` collisions with built-in code-mode globals such as `tools`,
  `namespaces`, `text`, `json`, `yield_control`, or `__zuvix*`
...
catalog for arbitrary enabled Zuvix, plugin, and client tools; use `MCP` for
...
- images and files travel through ordinary Zuvix tools, not through the
  code-mode bridge
...
Code mode supersedes the Zuvix Tool Search model surface for runs where it is
active.

When `tools.codeMode.enabled` is true and code mode activates:

- Zuvix does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same Zuvix executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the Zuvix compact
catalog bridge. Code mode is the generic Zuvix alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal Zuvix
shell `exec` tool is enabled, it is hidden from the model and cataloged like any
other tool.

Inside the guest runtime:

- `tools.call("zuvix:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, Zuvix omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters Zuvix.
...
Zuvix loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside Zuvix's main event loop in a worker. A guest
...
inputs beyond existing Zuvix trajectory policy.
...
ZUVIX_DEBUG_MODEL_TRANSPORT=1 \
ZUVIX_DEBUG_MODEL_PAYLOAD=tools \
ZUVIX_DEBUG_SSE=events \
zuvix gateway
```

For payload-shape debugging, use `ZUVIX_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `ZUVIX_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for Zuvix tools
...
5. Send an agent turn with Zuvix, plugin, MCP, and client test tools.
...
8. In `exec`, call Zuvix/plugin/client tools through `tools.search`,
