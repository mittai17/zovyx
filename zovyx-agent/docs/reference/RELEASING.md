...
Zuvix has three public release lanes:
...
- Every stable Zuvix release ships the npm package, macOS app, and signed
...
6. Run `Zuvix NPM Release` with `preflight_only=true`. Before a tag exists,
...
9. For a tagged beta candidate, run
   ...
   `Zuvix Release Publish` command only after the evidence bundle is green.
   `Zuvix Release Publish` dispatches the selected or all-publishable plugin
   packages to npm and the same set to ClawHub in parallel, and then promotes the
   prepared Zuvix npm preflight artifact with the matching dist-tag as soon as
   plugin npm publish succeeds.
   After the Zuvix npm publish child succeeds, it creates or updates the
   ...
   `zuvix-<version>-dependency-evidence.zip` for post-release incident
   ...
   evidence as soon as Zuvix npm publish succeeds, waits for ClawHub whenever
   Zuvix npm is being published, then runs `pnpm release:verify-beta` and
   ...
   `zuvix@YYYY.M.PATCH-beta.N` or
   `zuvix@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `Zuvix Release Publish`, reusing the successful preflight artifact via
    ...
    readiness requires the signed `ZuvixCompanion-Setup-x64.exe`,
    `ZuvixCompanion-Setup-arm64.exe`, and
    `ZuvixCompanion-SHA256SUMS.txt` assets on the Zuvix GitHub release.
    Pass the exact signed `zuvix/zuvix-windows-node` release tag as
    `windows_node_tag` and its candidate-approved installer digest map as
    `windows_node_installer_digests`; `Zuvix Release Publish` keeps the
...
- Plugin version sync updates official plugin package versions and existing
  `zuvix.compat.pluginApi` floors to the Zuvix release version by
  ...
  compatible with older Zuvix hosts, keep the floor at the oldest supported
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `Zuvix Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `zuvix@beta`, `zuvix@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=zuvix@beta -f suite_profile=product -f published_upgrade_survivor_baseline=zuvix@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
...
- `Zuvix NPM Release` preflight generates dependency release evidence before
...
- The preflight uploads dependency evidence as
  `zuvix-release-dependency-evidence-<tag>` and also embeds it under
  ...
  to the GitHub release as `zuvix-<version>-dependency-evidence.zip`.
- Run `Zuvix Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.PATCH` (or `main` when publishing a
  main-reachable tag), pass the release tag, successful Zuvix npm
  ...
  ClawHub publish, and Zuvix npm publish so the core package is not published
  before its externalized plugins.
- Stable `Zuvix Release Publish` requires an exact `windows_node_tag` after
  the matching non-prerelease `zuvix/zuvix-windows-node` release exists.
  ...
  while the Zuvix release is still a draft, carrying the pinned installer
  ...
  signatures use the expected Zuvix Foundation signer on a Windows runner,
  writes a SHA-256 manifest, and uploads the installers plus manifest onto the
  canonical Zuvix GitHub release, then re-downloads the promoted assets and
  ...
  rejects unexpected `ZuvixCompanion-*` asset names before replacing the
  ...
  approved source release. Website download links should target exact Zuvix
...
- Release checks now run in a separate manual workflow:
  `Zuvix Release Checks`
- `Zuvix Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `Zuvix Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/zuvix-cross-os-release-checks-reusable.yml` directly
...
- `Zuvix Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an Zuvix branch or release tag
- `Zuvix NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `ZUVIX_LIVE_TEST=1 ZUVIX_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Before tagging a release candidate locally, run
  ...
  UI build, and `release:zuvix:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.PATCH node --import tsx scripts/zuvix-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/zuvix-npm-postpublish-verify.ts YYYY.M.PATCH`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `ZUVIX_NPM_TELEGRAM_PACKAGE_SPEC=zuvix@YYYY.M.PATCH-beta.N ZUVIX_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex ZUVIX_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `ZUVIX_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `zuvix/releases/.github/workflows/zuvix-npm-dist-tags.yml` because
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=zuvix@YYYY.M.PATCH-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `Zuvix Release Checks`, prepares a
...
`npm_telegram_package_spec` is set. `Zuvix Release
...
`Zuvix Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=zuvix@YYYY.M.PATCH-beta.N`
...
The cross-OS OpenAI install smoke uses `ZUVIX_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=zuvix@YYYY.M.PATCH-beta.N \
  -f evidence_package_spec=zuvix@YYYY.M.PATCH-beta.N \
...
validation, including required Zuvix dynamic tool drift in the standard tier.
...
`ZUVIX_RELEASE_QA_*_LIVE_CI_ENABLED` repo variable must be enabled; otherwise
...
The Docker box lives in `Zuvix Release Checks` through
`zuvix-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `Zuvix Release Checks`. It is the agentic
...
`scripts/resolve-zuvix-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `zuvix@beta`, `zuvix@latest`, or an exact Zuvix release
  version
...
`Zuvix Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=zuvix@2026.4.26
```
...
`Zuvix Release Publish` is the normal mutating publish entrypoint. It
...
6. Dispatch `Zuvix NPM Release` with the release tag, npm dist-tag, and
...
gh workflow run zuvix-release-publish.yml \
  ...
  -f windows_node_installer_digests='{"ZuvixCompanion-Setup-x64.exe":"sha256:<approved-x64-sha256>","ZuvixCompanion-Setup-arm64.exe":"sha256:<approved-arm64-sha256>"}' \
  -f preflight_run_id=<successful-zuvix-npm-preflight-run-id> \
  -f full_release_validation_run_id=<successful-full-release-validation-run-id> \
  -f npm_dist_tag=beta
```
...
only for focused repair or republish work. `Zuvix Release Publish` rejects
`plugin_publish_scope=selected` when `publish_zuvix_npm=true` so the core
package cannot ship without every publishable official plugin, including
`@zuvix/diffs-language-pack`. For a selected plugin repair, set
`publish_zuvix_npm=false` with `plugin_publish_scope=selected` and
`plugins=@zuvix/name`, or dispatch the child workflow directly.

## NPM workflow inputs

`Zuvix NPM Release` accepts these operator-controlled inputs:
...
`Zuvix Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Zuvix NPM Release` preflight run id;
  required when `publish_zuvix_npm=true`
- `full_release_validation_run_id`: successful `Full Release Validation` run
  id; required when `publish_zuvix_npm=true`
- `windows_node_tag`: exact non-prerelease `zuvix/zuvix-windows-node`
  release tag; required for stable Zuvix publish
- `windows_node_installer_digests`: candidate-approved compact JSON map of the
  ...
  for stable Zuvix publish
- `npm_dist_tag`: npm target tag for the Zuvix package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused plugin-only repair work with `publish_zuvix_npm=false`
- `plugins`: comma-separated `@zuvix/*` package names when
  `plugin_publish_scope=selected`
- `publish_zuvix_npm`: defaults to `true`; set `false` only when using the
...
`Zuvix Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an Zuvix branch or
  release tag.
...
- For `Zuvix NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `Zuvix Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `Zuvix NPM Release` with `preflight_only=true`
...
5. Select the exact non-prerelease `zuvix/zuvix-windows-node` release tag
...
7. Run `Zuvix Release Publish` with the same `tag`, the same `npm_dist_tag`,
...
8. If the release landed on `beta`, use the
   `zuvix/releases/.github/workflows/zuvix-npm-dist-tags.yml`
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/zuvix/zuvix/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/zuvix/zuvix/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/zuvix-npm-release.yml`](https://github.com/zuvix/zuvix/blob/main/.github/workflows/zuvix-npm-release.yml)
- [`.github/workflows/zuvix-release-checks.yml`](https://github.com/zuvix/zuvix/blob/main/.github/workflows/zuvix-release-checks.yml)
- [`.github/workflows/zuvix-cross-os-release-checks-reusable.yml`](https://github.com/zuvix/zuvix/blob/main/.github/workflows/zuvix-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-zuvix-package-candidate.mjs`](https://github.com/zuvix/zuvix/blob/main/scripts/resolve-zuvix-package-candidate.mjs)
- [`scripts/zuvix-npm-release-check.ts`](https://github.com/zuvix/zuvix/blob/main/scripts/zuvix-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/zuvix/zuvix/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/zuvix/zuvix/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`zuvix/maintainers/release/README.md`](https://github.com/zuvix/maintainers/blob/main/release/README.md)
