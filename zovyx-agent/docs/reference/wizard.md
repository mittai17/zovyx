...
This is the full reference for `zuvix onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.zuvix/zuvix.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `zuvix doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though Zuvix now prefers Claude CLI reuse when available.
    ...
    - **Zuvix**: prompts for `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`, get it at https://zuvix.ai/auth) and lets you pick the Zen or Go catalog.
    ...
    - Auth profiles live in `~/.zuvix/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.zuvix/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.zuvix/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$ZUVIX_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.zuvix/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `zuvix pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `zuvix configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `zuvix health`.
    - Tip: `zuvix status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
...
- Stores it under `~/.zuvix/tools/signal-cli/<version>/`.
...
Typical fields in `~/.zuvix/zuvix.json`:
...
`zuvix agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.zuvix/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.zuvix/agents/<agentId>/sessions/`.
