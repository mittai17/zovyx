...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `zuvix.internal.`), **or**
...
Run `zuvix devices list` again before approval.
...
```bash
zuvix nodes status
zuvix gateway call node.list --params "{}"
```
...
By default, official/TestFlight builds and gateways use the hosted relay at `https://ios-push-relay.zuvix.ai`.
...
- `ZUVIX_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
- `ZUVIX_PUSH_RELAY_BASE_URL` still works as a temporary env override for official/TestFlight iOS builds.
...
- Only genuine Zuvix iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Zuvix builds.
...
```bash
export ZUVIX_APNS_TEAM_ID="TEAMID"
export ZUVIX_APNS_KEY_ID="KEYID"
export ZUVIX_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.zuvix/credentials/apns
chmod 700 ~/.zuvix/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.zuvix/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.zuvix/credentials/apns/AuthKey_KEYID.p8
export ZUVIX_APNS_PRIVATE_KEY_PATH="$HOME/.zuvix/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_zuvix-gw._tcp` on `local.` and, when configured, the same
...
`zuvix.internal.`) and Tailscale split DNS.
...
```bash
zuvix nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__zuvix__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__zuvix__/canvas/` and `/__zuvix__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through Zuvix node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Zuvix by invoking node
...
```bash
zuvix nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__zuvix; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
zuvix nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `zuvix devices list` and approve manually.
