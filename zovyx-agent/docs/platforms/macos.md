---
summary: "Zuvix macOS companion app (menu bar + gateway broker)"
read_when:
...
The macOS app is the **menu-bar companion** for Zuvix. It owns permissions,
...
- Installs the global CLI (`zuvix`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `zuvix gateway install`.
...
The app manages a per-user LaunchAgent labeled `ai.zuvix.gateway`
(or `ai.zuvix.<profile>` when using `--profile`/`ZUVIX_PROFILE`; legacy `com.zuvix.*` still unloads).
...
Replace the label with `ai.zuvix.<profile>` when running a named profile.
...
The app registers the `zuvix://` URL scheme for local actions.
...
```bash
open 'zuvix://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **Zuvix.app**.
...
Avoid putting your Zuvix state dir in iCloud or other cloud-synced folders.
...
If `zuvix doctor` detects state under:
...
```bash
cd apps/macos
swift run zuvix-mac connect --json
swift run zuvix-mac discover --timeout 3000 --json
```
...
<Tip>
Compare against `zuvix gateway discover --json` to see whether the macOS app's discovery pipeline (`local.` plus the configured wide-area domain, with wide-area and Tailscale Serve fallbacks) differs from the Node CLI's `dns-sd` based discovery.
</Tip>
