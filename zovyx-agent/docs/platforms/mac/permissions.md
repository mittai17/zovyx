...
- Same path: run the app from a fixed location (for Zuvix, `dist/Zuvix.app`).
...
Prefer granting Accessibility to Zuvix.app, Peekaboo.app, or another signed
...
```bash
sudo tccutil reset Accessibility ai.zuvix.mac
sudo tccutil reset ScreenCapture ai.zuvix.mac
...
Workaround: move files into the Zuvix workspace (`~/.zuvix/workspace`) if you want to avoid per-folder grants.
