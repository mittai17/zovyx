...
service via the external `zuvix` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.zuvix.gateway`
  (or `ai.zuvix.<profile>` when using `--profile`/`ZUVIX_PROFILE`; legacy `com.zuvix.*` is supported).
