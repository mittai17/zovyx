...
<Note>
The official Android app is available on [Google Play](https://play.google.com/store/apps/details?id=ai.zuvix.app&hl=en_IN). It is a companion node and requires a running Zuvix Gateway. The source code is also available in the [Zuvix repository](https://github.com/zuvix/zuvix) under `apps/android`; see [apps/android/README.md](https://github.com/zuvix/zuvix/blob/main/apps/android/README.md) for build instructions.
</Note>
...
- Install: [Google Play](https://play.google.com/store/apps/details?id=ai.zuvix.app&hl=en_IN) for the app, [Getting Started](/start/getting-started) for the Gateway, then [Pairing](/channels/pairing).
...
- You can run the CLI (`zuvix`) on the gateway machine (or via SSH).
...
1. Set up a DNS-SD zone (example `zuvix.internal.`) on the gateway host and publish `_zuvix-gw._tcp` records.
...
1. Create `~/.zuvix/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
zuvix nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__zuvix__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__zuvix__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The Gateway also serves `/__zuvix__/a2ui/`, but the Android app treats remote A2UI pages as render-only. Action-capable A2UI commands use the bundled app-owned A2UI page before applying messages.
...
Android supports launching Zuvix from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
Zuvix..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set Zuvix as the default assistant app.
</Note>
