...
2. `npm i -g zuvix@latest`
3. `zuvix onboard --install-daemon`
...
Zuvix installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `zuvix gateway install` and
`zuvix onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/zuvix-gateway[-<profile>].service`:

```
[Unit]
Description=Zuvix Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. Zuvix therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Zuvix starts the child through a short
...
- Zuvix-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `ZUVIX_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
