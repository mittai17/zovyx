...
x64 and ARM64 installers on Zuvix releases.

Download the latest stable installer from the [Zuvix releases page](https://github.com/zuvix/zuvix/releases):

- [ZuvixCompanion-Setup-x64.exe](https://github.com/zuvix/zuvix/releases/download/v2026.6.5/ZuvixCompanion-Setup-x64.exe)
- [ZuvixCompanion-Setup-arm64.exe](https://github.com/zuvix/zuvix/releases/download/v2026.6.5/ZuvixCompanion-Setup-arm64.exe)
- [Checksums](https://github.com/zuvix/zuvix/releases/download/v2026.6.5/ZuvixCompanion-SHA256SUMS.txt)

If a download link above returns a 404, visit the [releases page](https://github.com/zuvix/zuvix/releases) and look for the `ZuvixCompanion-Setup-*` assets on the latest release.

After install, launch **Zuvix Companion** from the Start menu or the system
...
`ZuvixGateway` WSL distro, installs the Gateway inside it, and pairs the app.
...
Windows Hub can register as a first-class Zuvix node. The agent can then use
...
```powershell
zuvix devices list
zuvix devices approve <request-id>
zuvix nodes status
```
...
Windows capabilities without a running Zuvix Gateway.
...
For terminal-first use, install Zuvix from PowerShell:
...
```powershell
zuvix onboard --non-interactive --skip-health
zuvix gateway run
```
...
Then install Zuvix inside WSL with the Linux quickstart:
...
```bash
systemctl --user is-enabled zuvix-gateway.service
systemctl --user status zuvix-gateway.service --no-pager
```
...
Check Task Manager for `Zuvix.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If it is not running, launch **Zuvix
...
```powershell
notepad "$env:LOCALAPPDATA\ZuvixTray\Logs\Setup\easy-setup-latest.txt"
```
