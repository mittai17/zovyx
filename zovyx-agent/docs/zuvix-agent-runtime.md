---
summary: "Developer workflow for Zuvix agent runtime: build, test, and live validation"
title: "Zuvix agent runtime workflow"
read_when:
  - Working on Zuvix agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

A sane workflow for working on the Zuvix agent runtime in Zuvix.
...
```bash
ZUVIX_LIVE_TEST=1 pnpm test src/agents/embedded-agent-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm zuvix agent --message "Hello" --thinking low`
...
State lives under the Zuvix state directory. Default is `~/.zuvix`. If `ZUVIX_STATE_DIR` is set, use that directory instead.

To reset everything:

- `zuvix.json` for config
...
- [Zuvix agent runtime architecture](/agent-runtime-architecture)
