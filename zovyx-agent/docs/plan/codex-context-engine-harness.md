...
summary: "Specification for making the bundled Codex app-server harness honor Zuvix context-engine plugins"
read_when:
  ...
  - You are comparing embedded Zuvix and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same Zuvix context-engine
lifecycle contract that embedded Zuvix turns already honor.
...
Zuvix-level compaction policy as far as the Codex app-server boundary allows.
...
as built-in Zuvix attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in Zuvix-controlled code. The external
boundary is the Codex app-server protocol itself: Zuvix can control what it
...
Built-in Zuvix attempts call the context-engine lifecycle directly:
...
For Codex harness turns, Zuvix should preserve this lifecycle:

1. Read the mirrored Zuvix session transcript.
...
8. Mirror the Codex result back into the Zuvix transcript.
...
11. Preserve Codex native compaction signals and Zuvix compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Zuvix should
not try to mutate the app-server's internal history except through supported
protocol calls.

Zuvix's transcript mirror remains the source for Zuvix features:
...
The context-engine interface returns Zuvix `AgentMessage[]`, not a Codex
...
- Label it clearly as Zuvix assembled context.
...
inside Zuvix and preserves context-engine semantics.
...
we need strict Zuvix parity, run context-engine assembly before hook
...
1. Zuvix context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Zuvix compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Zuvix compaction should prefer the context engine's `compact()` result for
the Zuvix transcript mirror and plugin state.
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Zuvix compaction when the context engine is primary.
...
Avoid requiring lossless-claw in Zuvix core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Zuvix/plugin state.
