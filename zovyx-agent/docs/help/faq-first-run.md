...
read_when:
  ...
  - Cannot access docs.zuvix.ai, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://zuvix.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Zuvix **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/zuvix/zuvix/issues](https://github.com/zuvix/zuvix/issues)
    [https://github.com/zuvix/zuvix/pulls](https://github.com/zuvix/zuvix/pulls)
    ...
    - `zuvix status`: quick snapshot of gateway/agent health + basic config.
    - `zuvix models status`: checks provider auth + model availability.
    - `zuvix doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `zuvix status --all`, `zuvix logs --follow`,
    `zuvix gateway status`, `zuvix health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Zuvix">
    ...
    If you don't have a global install yet, run it via `pnpm zuvix onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `ZUVIX_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `ZUVIX_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `zuvix doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `zuvix gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `zuvix gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Zuvix now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Zuvix on the new machine.
    2. Copy `$ZUVIX_STATE_DIR` (default: `~/.zuvix`) from the old machine.
    3. Copy your workspace (default: `~/.zuvix/workspace`).
    4. Run `zuvix doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.zuvix/` (for example `~/.zuvix/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/zuvix/zuvix/blob/main/CHANGELOG.md](https://github.com/zuvix/zuvix/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.zuvix.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.zuvix.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.zuvix.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/zuvix/zuvix/tree/main/docs](https://github.com/zuvix/zuvix/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://zuvix.ai/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://zuvix.ai/install.ps1](https://zuvix.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://zuvix.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://zuvix.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://zuvix.ai/install.sh | bash -s -- --install-method git --verbose
    ```
    ...
    Set-PSDebug -Trace 1
    & ([scriptblock]::Create((iwr -useb https://zuvix.ai/install.ps1))) -NoOnboard
    Set-PSDebug -Trace 0
    ```
  ...
  <Accordion title="Windows install says git not found or zuvix not recognized">
    ...
    **2) zuvix is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Zuvix, track/report it in:

    - [Issue #30640](https://github.com/zuvix/zuvix/issues/30640)
  ...
  <Accordion title="How do I install Zuvix on Linux?">
  ...
  <Accordion title="How do I install Zuvix on a VPS?">
  ...
  <Accordion title="Can I ask Zuvix to update itself?">
    ...
    zuvix update
    zuvix update status
    zuvix update --channel stable|beta|dev
    zuvix update --tag <dist-tag|version>
  ...
  <Accordion title="What does onboarding actually do?">
    `zuvix onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Zuvix with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in Zuvix, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Zuvix**: Anthropic staff
      told us this usage is allowed again, and Zuvix is treating `claude -p`
    ...
    Zuvix also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Zuvix-style Claude CLI usage is allowed again, so
    Zuvix treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Zuvix treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Zuvix token path, but Zuvix now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Zuvix, see [OpenAI](/providers/openai), [Qwen / Model
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    for long-context billing (API key billing or the Zuvix Claude-login path
    with Extra Usage enabled).

    Tip: set a **fallback model** so Zuvix can keep replying while a provider is rate-limited.
  ...
  <Accordion title="How does Codex auth work?">
    ...
    legacy config repaired by `zuvix doctor --fix`. Direct OpenAI API-key
  ...
  <Accordion title="Why does Zuvix still mention legacy OpenAI Codex prefix?">
    ...
    - legacy Codex GPT-5.5 ref = legacy model route repaired by `zuvix doctor --fix`
    - `openai/gpt-5.5` plus an ordered `openai` API-key profile = API-key auth for an OpenAI agent model
    - legacy Codex auth profile ids = legacy auth profile id migrated by `zuvix doctor --fix`
    ...
    `zuvix models auth login --provider openai`. Keep the model ref as
    `openai/gpt-5.5`; legacy Codex model refs are legacy config that
    `zuvix doctor --fix` rewrites.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Zuvix can show the currently visible provider usage/quota windows in
    `zuvix models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Zuvix fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Zuvix. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `zuvix.json`.
    ...
    2. Enable the plugin: `zuvix plugins enable google`
    3. Login: `zuvix models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Zuvix needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Zuvix runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Zuvix, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `zuvix nodes status` / `zuvix nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `zuvix doctor --fix` can try to resolve them.

    Safer (no third-party bot):

    - DM your bot, then run `zuvix logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Zuvix instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Zuvix via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes. Use `zuvix update --channel ...` when Zuvix is already installed.
    This **does not delete your data** - it only changes the Zuvix code install.
    Your state (`~/.zuvix`) and workspace (`~/.zuvix/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Zuvix-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Zuvix on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Zuvix is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Zuvix in a VM and what are the requirements?">
