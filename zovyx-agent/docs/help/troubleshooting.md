---
summary: "Symptom first troubleshooting hub for Zuvix"
read_when:
  - Zuvix is not working and you need the fastest path to a fix
...
- `zuvix status` → shows configured channels and no obvious auth errors.
- `zuvix status --all` → full report is present and shareable.
- `zuvix gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `zuvix gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `zuvix doctor` → no blocking config/service errors.
- `zuvix channels status --probe` → reachable gateway returns live per-account
...
- `zuvix logs --follow` → steady activity, no repeating fatal errors.
...
and run `zuvix status --all` again. See [Tools](/tools) for the profile
...
## Local OpenAI-compatible backend works directly but fails in Zuvix

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `zuvix infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on Zuvix agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger Zuvix prompts crash the
...
## Plugin install fails with missing zuvix extensions

If install fails with `package.json missing zuvix.extensions`, the plugin package
is using an old shape that Zuvix no longer accepts.

Fix in the plugin package:

1. Add `zuvix.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `zuvix plugins install <package>` again.

Example:

```json
{
  "name": "@zuvix/my-plugin",
  "version": "1.2.3",
  "zuvix": {
...
Install policy runs on plugin installs and updates. Zuvix-owned plugin
versions normally move with the Zuvix release, so an Zuvix update can
also need matching `@zuvix/*` plugin updates during post-update sync.

Avoid these broad policy shapes unless you also maintain the matching upgrade
rule:

- Freezing Zuvix-owned plugins to one exact old version, such as allowing
  only `@zuvix/*@2026.5.3`.
...
- Approving plugin versions without considering the policy request's
  `zuvixVersion` and the plugin candidate metadata.

Safer policy rules allow trusted Zuvix-owned plugin updates when the
candidate is compatible with the current Zuvix host, instead of pinning a
single release forever. If you block npm by default, make a narrow exception
for the trusted `@zuvix/*` plugin packages or plugin ids you use. If you
...
If the policy is intentionally strict, relax it for the trusted Zuvix upgrade
window, rerun `zuvix plugins update --all`, then restore the stricter rule.
...
If `zuvix doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run Zuvix as
...
```mermaid
flowchart TD
  A[Zuvix is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `zuvix gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    zuvix config get tools.exec.host
    zuvix config get tools.exec.security
    zuvix config get tools.exec.ask
  ...
  <Accordion title="Browser tool fails">
    ...
    - `zuvix` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `zuvix browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
