---
summary: "How Zuvix validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing Zuvix update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `zuvix doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` validates plugin install smoke, local folder installs,
  ...
  and Claude-bundle enable/inspect. Set `ZUVIX_PLUGINS_E2E_CLAWHUB=0` to
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `zuvix plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `zuvix config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `zuvix update --yes --json`, and requires the candidate update command to
...
```bash
ZUVIX_UPGRADE_SURVIVOR_BASELINE_SPEC=zuvix@2026.4.23 \
ZUVIX_UPGRADE_SURVIVOR_SCENARIO=versioned-runtime-deps \
pnpm test:docker:published-upgrade-survivor

ZUVIX_UPGRADE_SURVIVOR_BASELINE_SPEC=zuvix@latest \
ZUVIX_UPGRADE_SURVIVOR_SCENARIO=bootstrap-persona \
pnpm test:docker:published-upgrade-survivor
```
...
`ZUVIX_UPGRADE_SURVIVOR_SCENARIOS=reported-issues` expands to all reported
...
- `source=npm`: validate `zuvix@beta`, `zuvix@latest`, or an exact
  published version.
...
- `source=trusted-url`: validate an HTTPS tarball with required
  ...
  `ZUVIX_TRUSTED_PACKAGE_TOKEN` secret.
...
`package_acceptance_package_spec=zuvix@YYYY.M.PATCH` so the same upgrade matrix
...
`last-stable-4` resolves to the four latest stable npm-published Zuvix
