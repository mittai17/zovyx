...
This is handy when you need to toggle obscure settings without editing `zuvix.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[zuvix]"
...
Use `ZUVIX_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
ZUVIX_PLUGIN_LIFECYCLE_TRACE=1 zuvix plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm zuvix ...`
...
`ZUVIX_RUN_NODE_CPU_PROF_DIR`:

```bash
ZUVIX_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm zuvix status
```
...
```bash
ZUVIX_TRACE_SYNC_IO=1 pnpm zuvix gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `ZUVIX_TRACE_SYNC_IO=1` when you explicitly want Node
...
`zuvix-gateway-watch-main` (or a profile/port-specific variant such as
`zuvix-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t zuvix-gateway-watch-main
```
...
# or
ZUVIX_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
ZUVIX_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`ZUVIX_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`ZUVIX_PROFILE`, `ZUVIX_CONFIG_PATH`, `ZUVIX_STATE_DIR`,
`ZUVIX_GATEWAY_PORT`, and `ZUVIX_SKIP_CHANNELS` into the pane. Put
...
`zuvix doctor --fix --non-interactive` once and restarts the Gateway child.
Use `ZUVIX_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `zuvix.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.zuvix-dev` and
...
```bash
pnpm gateway:dev
ZUVIX_PROFILE=dev zuvix tui
```

If you don't have a global install yet, run the CLI via `pnpm zuvix ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `ZUVIX_PROFILE=dev`
   - `ZUVIX_STATE_DIR=~/.zuvix-dev`
   - `ZUVIX_CONFIG_PATH=~/.zuvix-dev/zuvix.json`
   - `ZUVIX_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`ZUVIX_SKIP_CHANNELS=1`).
...
```bash
ZUVIX_PROFILE=dev zuvix gateway --dev --reset
```
...
## Raw stream logging (Zuvix)

Zuvix can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.zuvix/logs/raw-stream.jsonl
```
...
```bash
ZUVIX_RAW_STREAM=1
ZUVIX_RAW_STREAM_PATH=~/.zuvix/logs/raw-stream.jsonl
```

Default file:

`~/.zuvix/logs/raw-stream.jsonl`
...
```bash
ZUVIX_RAW_STREAM_PATH=~/.zuvix/logs/raw-openai-completions.jsonl
```

Default file:

`~/.zuvix/logs/raw-openai-completions.jsonl`
...
- If you need to use the built Zuvix CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./zuvix.mjs` or create a shell alias like `alias zuvix-build="node $(pwd)/zuvix.mjs"`
