---
summary: "Frequently asked questions about Zuvix setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/zuvix/zuvix-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   zuvix health --json
   zuvix health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for Zuvix?">
  ...
  <Accordion title="Can Zuvix help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Zuvix draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Zuvix is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Zuvix when you
    ...
    Showcase: [https://zuvix.ai/showcase](https://zuvix.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.zuvix/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.zuvix/zuvix.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.zuvix/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.zuvix/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.zuvix/zuvix.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.zuvix/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Zuvix treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Zuvix falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Zuvix intentionally suppresses the announce instead of posting stale earlier progress.
    - Tool/toolResult output is not promoted into child result text; the result is the child's latest visible assistant reply.

    Debug:

    ```bash
    zuvix tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `ZUVIX_SKIP_CRON` is not set.
    ...
    ```bash
    zuvix cron run <jobId>
    zuvix cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `zuvix skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    zuvix skills search "calendar"
    zuvix skills search --limit 20
    zuvix skills install <skill-slug>
    zuvix skills install <skill-slug> --version <version>
    zuvix skills install <skill-slug> --force
    zuvix skills install <skill-slug> --global
    ...
    Native `zuvix skills install` writes into the active workspace `skills/`
  ...
  <Accordion title="Can Zuvix run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.zuvix.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Zuvix can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.zuvix/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "zuvix": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across all local agents, use `zuvix skills install <slug> --global` (or place them manually in `~/.zuvix/skills/<name>/SKILL.md`). If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Zuvix?">
    Use the built-in `user` browser profile, which attaches through Chrome DevTools MCP:

    ```bash
    zuvix browser --browser-profile user tabs
    zuvix browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `ZUVIX_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `ZUVIX_IMAGE_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Zuvix validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    Zuvix memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Zuvix also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Zuvix uses OpenAI embeddings. Legacy
...
<AccordionGroup>
  <Accordion title="Is all data used with Zuvix saved locally?">
    No - **Zuvix's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.zuvix` + your workspace directory).
  ...
  <Accordion title="Where does Zuvix store its data?">
    Everything lives under `$ZUVIX_STATE_DIR` (default: `~/.zuvix`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$ZUVIX_STATE_DIR/zuvix.json`                             | Main config (JSON5)                                                |
    | `$ZUVIX_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$ZUVIX_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$ZUVIX_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$ZUVIX_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$ZUVIX_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$ZUVIX_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$ZUVIX_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$ZUVIX_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.zuvix/agent/*` (migrated by `zuvix doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.zuvix/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.zuvix`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md`, `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
      Lowercase root `memory.md` is legacy repair input only; `zuvix doctor --fix`
      can merge it into `MEMORY.md` when both files exist.
    - **State dir (`~/.zuvix`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.zuvix/skills`).

    Default workspace is `~/.zuvix/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.zuvix/workspace" } },
    }
    ```
  ...
  <Accordion title="Can I make SOUL.md bigger?">
    ...
    Change the shared defaults in your Zuvix config:
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.zuvix` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Zuvix?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Zuvix repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Zuvix reads an optional **JSON5** config from `$ZUVIX_CONFIG_PATH` (default: `~/.zuvix/zuvix.json`):

    ```
    $ZUVIX_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.zuvix/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `ZUVIX_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Zuvix enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and generates a runtime-only token for that startup, so **local WS clients must authenticate**. Configure `gateway.auth.token`, `gateway.auth.password`, `ZUVIX_GATEWAY_TOKEN`, or `ZUVIX_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `zuvix doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Zuvix.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `ZUVIX_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `zuvix configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Zuvix auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.zuvix/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Zuvix protects many accidental clobbers:

    - Zuvix-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Zuvix-owned writes are rejected and saved as `zuvix.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, Gateway fails closed or skips the reload; it does not rewrite `zuvix.json`.
    - `zuvix doctor --fix` owns repair and can restore last-known-good while saving the rejected file as `zuvix.json.clobbered.*`.

    Recover:

    - Check `zuvix logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `zuvix.json.clobbered.*` or `zuvix.json.rejected.*` beside the active config.
    - Run `zuvix config validate` and `zuvix doctor --fix`.
    - Copy only the intended keys back with `zuvix config set` or `config.patch`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `zuvix doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `zuvix config set` for small changes.
    - Use `zuvix configure` for interactive edits.
  ...
  <Accordion title="Can the Zuvix browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="How can my agent access my computer if the Gateway is hosted remotely?">
    ...
    5. Approve the node on the Gateway:
       ...
       ```bash
       zuvix devices list
       zuvix devices approve <requestId>
       ```
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `zuvix gateway status`
    - Gateway health: `zuvix status`
    - Channel health: `zuvix channels status`
  ...
  <Accordion title="Can two Zuvix instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `zuvix agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    zuvix agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
...
<AccordionGroup>
  <Accordion title="How does Zuvix load environment variables?">
    Zuvix reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.zuvix/.env` (aka `$ZUVIX_STATE_DIR/.env`)
    ...
    `.env` and should live in the process environment, `~/.zuvix/.env`, or config `env`.
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.zuvix/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `ZUVIX_LOAD_SHELL_ENV=1`, `ZUVIX_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `zuvix models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Zuvix won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.zuvix/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Zuvix instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Zuvix but keep it installed?">
    ...
    ```bash
    zuvix reset --scope full --yes --non-interactive
    ```
    ...
    - If you used profiles (`--profile` / `ZUVIX_PROFILE`), reset each state dir (defaults are `~/.zuvix-<profile>`).
    - Dev reset: `zuvix gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    or empty checklist stubs), Zuvix skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Zuvix runs on **your own account**, so if you're in the group, Zuvix can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    zuvix directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Zuvix not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.zuvix/agents/<agentId>/sessions/`.
    ...
    - Use `zuvix doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > ZUVIX_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does zuvix gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `zuvix gateway status` and trust these lines:
  ...
  <Accordion title='Why does zuvix gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `ZUVIX_STATE_DIR` mismatch).
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Zuvix enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `zuvix gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Zuvix in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `zuvix gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `zuvix dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `zuvix doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `ZUVIX_GATEWAY_TOKEN` or `gateway.auth.password` / `ZUVIX_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `zuvix devices list`
      - `zuvix devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `zuvix status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `ZUVIX_CONFIG_PATH` (per-instance config)
    - `ZUVIX_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `zuvix --profile <name> ...` per instance (auto-creates `~/.zuvix-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `zuvix --profile <name> gateway install`.

    Profiles also suffix service names (`ai.zuvix.<profile>`; legacy `com.zuvix.*`, `zuvix-gateway-<profile>.service`, `Zuvix Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    zuvix tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS launchd stdout: `~/Library/Logs/zuvix/gateway.log` (profiles use `gateway-<profile>.log`; stderr is suppressed)
    - Linux: `journalctl --user -u zuvix-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Zuvix Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `zuvix gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Zuvix?">
    There are **three Windows install modes**:

    **1) Windows Hub local setup:** the native app manages a local app-owned WSL Gateway.

    Open **Zuvix Companion** from the Start menu or tray, then use
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `zuvix gateway status`
    2. Is the Gateway healthy? `zuvix status`
    3. Does the UI have the right token? `zuvix dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    zuvix channels status
    zuvix channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Zuvix already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: zuvix gateway restart vs zuvix gateway">
    - `zuvix gateway restart`: restarts the **background service** (launchd/systemd).
    - `zuvix gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `zuvix gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must use structured media fields such as `media`, `mediaUrl`, `path`, or `filePath`. See [Zuvix assistant setup](/start/zuvix) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    zuvix message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Zuvix to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `zuvix pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `zuvix pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `zuvix doctor` to surface risky DM policies.
  ...
  <Accordion title="Is Zuvix less safe because it uses TypeScript/Node instead of Rust/WASM?">
    Language and runtime matter, but they are not the main risk for a personal
    agent. The practical Zuvix risks are gateway exposure, who can message the
    ...
    - run `zuvix security audit --deep` after config changes

    Details: [Security](/gateway/security), [Sandboxing](/gateway/sandboxing).

  </Accordion>

  <Accordion title="I saw reports about exposed Zuvix instances. What should I check?">
    First check your actual deployment:

    ```bash
    zuvix security audit --deep
    zuvix gateway status
    ```
  ...
  <Accordion title="Are ClawHub skills and third-party plugins safe to install?">
    ...
    complete security boundary. Zuvix does not run built-in local
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Zuvix only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    zuvix pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    zuvix pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Zuvix blocks **cross-provider** messaging by default. If a tool call is bound
