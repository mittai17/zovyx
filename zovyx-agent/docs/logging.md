...
read_when:
  - You need a beginner-friendly overview of Zuvix logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

Zuvix has two main log surfaces:
...
Zuvix keeps up to five numbered archives beside the active file, such as
`zuvix-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.zuvix/zuvix.json`:

```json
{
  "logging": {
    "file": "/path/to/zuvix.log"
  }
}
```
...
or times out before `logs.tail` answers, `zuvix logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `zuvix logs --follow` is stricter: on Linux it uses the active
...
```bash
zuvix channels logs --channel whatsapp
```
...
Zuvix preserves the original structured log arguments alongside these fields
...
`zuvix gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
zuvix gateway
zuvix gateway --verbose --ws-log compact
zuvix gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.zuvix/zuvix.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/zuvix/zuvix-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`ZUVIX_LOG_LEVEL`** environment variable (e.g. `ZUVIX_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `zuvix.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `zuvix --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
ZUVIX_DEBUG_MODEL_TRANSPORT=1 zuvix gateway
ZUVIX_DEBUG_MODEL_PAYLOAD=tools ZUVIX_DEBUG_SSE=events zuvix gateway
```

Available flags:

- `ZUVIX_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `ZUVIX_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `ZUVIX_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `ZUVIX_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `ZUVIX_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `ZUVIX_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `ZUVIX_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal Zuvix logging, so `zuvix logs --follow`
...
Zuvix writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
Zuvix can redact sensitive tokens before they hit console output, file logs,
...
policy. Zuvix still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `ZUVIX_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `zuvix doctor` first.
