...
description: Fleet supervision plan for Codex app-server sessions controlled by Zuvix.
readWhen:
  - Designing Codex fleet supervision
  - Building Zuvix tools that read, steer, or spawn Codex sessions
...
Claw Supervisor lets one always-on Zuvix instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into Zuvix through MCP.

## Product Model

Codex remains the primary work surface. Zuvix supervises Codex rather than hiding Codex inside an opaque Zuvix subagent.

The Zuvix plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor Claw: an always-on Zuvix agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

Zuvix may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
...
- Zuvix tools for Claw-to-Codex control.
...
Every supervised Codex gets an MCP server named `zuvix-codex-supervisor`.
...
- Supervisor state lives in Zuvix Gateway storage.
...
- Transcript reads redact secrets at Zuvix boundary.
...
- Durable state owner: Zuvix Gateway DB, Cloudflare Durable Object, or VPS database.
