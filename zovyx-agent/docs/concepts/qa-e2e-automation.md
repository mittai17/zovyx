...
The private QA stack is meant to exercise Zuvix in a more realistic,
...
Every QA flow runs under `pnpm zuvix qa <subcommand>`. Many have `pnpm qa:*`
...
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm zuvix qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                  |
| `qa coverage`                                       | Print the YAML scenario-coverage inventory (`--json` for machine output).                                                                                                                                                                                               |
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files and write the agentic parity report, or use `--runtime-axis --token-efficiency` to write Codex-vs-Zuvix runtime parity and token-efficiency reports from one runtime-pair summary.                                         |
...
Stable/LTS proof lane. When a command also needs an Zuvix root profile, put
the root profile before the QA command:

```bash
pnpm zuvix --profile work qa run --qa-profile smoke-ci
```
...
```bash
pnpm zuvix qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`zuvix.run`, `zuvix.harness.run`, a latest GenAI semantic-convention
model-call span, `zuvix.context.assembled`, and `zuvix.message.delivery`
...
`zuvix.content.*` attributes must stay out of the trace. The raw OTLP
...
```bash
pnpm zuvix qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `ZUVIX_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.

For transport-real Telegram, Discord, Slack, and WhatsApp smoke lanes:

```bash
pnpm zuvix qa telegram
pnpm zuvix qa discord
pnpm zuvix qa slack
pnpm zuvix qa whatsapp
```
...
```bash
pnpm zuvix qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent Zuvix Slack gateway
...
```bash
pnpm zuvix qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `zuvix infer image describe`
...
`zuvix/plugin-sdk/qa-live-transport-scenarios`.
...
```bash
pnpm zuvix qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Zuvix
...
- `ZUVIX_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
...
The implicit default set always covers canary, mention gating, native command replies, command addressing, and bot-to-bot group replies. `mock-openai` defaults also include deterministic reply-chain and final-message streaming checks. `telegram-current-session-status-tool` remains opt-in because it is only stable when threaded directly after canary, not after arbitrary native command replies. Use `pnpm zuvix qa telegram --list-scenarios --provider-mode mock-openai` to print the current default/optional split with regression refs.
...
```bash
ZUVIX_QA_CREDENTIAL_SOURCE=convex \
pnpm test:docker:npm-telegram-live
```

When `ZUVIX_QA_CREDENTIAL_SOURCE=convex` is set, the package live wrapper
...
`ZUVIX_NPM_TELEGRAM_RTT_SAMPLES`, `ZUVIX_NPM_TELEGRAM_RTT_TIMEOUT_MS`,
or `ZUVIX_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune RTT measurement without
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child Zuvix gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `ZUVIX_QA_DISCORD_GUILD_ID`
- `ZUVIX_QA_DISCORD_CHANNEL_ID`
- `ZUVIX_QA_DISCORD_DRIVER_BOT_TOKEN`
- `ZUVIX_QA_DISCORD_SUT_BOT_TOKEN`
- `ZUVIX_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `ZUVIX_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `ZUVIX_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `ZUVIX_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child Zuvix gateway through the bundled Slack plugin.

Required env when `--credential-source env`:

- `ZUVIX_QA_SLACK_CHANNEL_ID`
- `ZUVIX_QA_SLACK_DRIVER_BOT_TOKEN`
- `ZUVIX_QA_SLACK_SUT_BOT_TOKEN`
- `ZUVIX_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `ZUVIX_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `ZUVIX_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `ZUVIX_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-qa-report.md`
- `qa-evidence.json` - evidence entries for the live transport checks.
- `slack-qa-observed-messages.json` - bodies redacted unless `ZUVIX_QA_SLACK_CAPTURE_CONTENT=1`.
- `approval-checkpoints/` - only when Mantis sets
  `ZUVIX_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "Zuvix QA Driver",
    "description": "Test driver bot for Zuvix QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "Zuvix QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
```json
{
  "display_information": {
    "name": "Zuvix QA SUT",
    "description": "Zuvix QA SUT connector for Zuvix"
  },
  "features": {
    "bot_user": {
      "display_name": "Zuvix QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#zuvix-qa`) and invite both bots from inside the channel:

```
/invite @Zuvix QA Driver
/invite @Zuvix QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `ZUVIX_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `ZUVIX_QA_CONVEX_SITE_URL` and `ZUVIX_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm zuvix qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm zuvix qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child Zuvix gateway through the
bundled WhatsApp plugin.

Required env when `--credential-source env`:

- `ZUVIX_QA_WHATSAPP_DRIVER_PHONE_E164`
- `ZUVIX_QA_WHATSAPP_SUT_PHONE_E164`
- `ZUVIX_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64`
- `ZUVIX_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64`

Optional:

- `ZUVIX_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
  `whatsapp-mention-gating` and `whatsapp-group-allowlist-block`.
- `ZUVIX_QA_WHATSAPP_CAPTURE_CONTENT=1` keeps message bodies in
...
`@zuvix/whatsapp/api.js` package surface instead of reaching into private
...
- `mock-openai` is the scenario-aware Zuvix mock. It remains the default
...
- how `zuvix qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `zuvix qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `zuvix.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm zuvix qa coverage` (add `--json` for machine-readable output).
When choosing focused proof for a touched behavior or file path, run `pnpm zuvix qa coverage --match <query>`.
