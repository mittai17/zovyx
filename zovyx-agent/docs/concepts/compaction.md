---
summary: "How Zuvix summarizes long conversations to stay within model limits"
read_when:
...
Every model has a context window: the maximum number of tokens it can process. When a conversation approaches that limit, Zuvix **compacts** older messages into a summary so the chat can continue.
...
When Zuvix splits history into compaction chunks, it keeps assistant tool calls paired with their matching `toolResult` entries. If a split point lands inside a tool block, Zuvix moves the boundary so the pair stays together and the current unsummarized tail is preserved.

The full conversation history stays on disk. Compaction only changes what the model sees on the next turn.

## Auto-compaction

Auto-compaction is on by default. It runs when the session nears the context limit, or when the model returns a context-overflow error (in which case Zuvix compacts and retries).
...
<Info>
Before compacting, Zuvix automatically reminds the agent to save important notes to [memory](/concepts/memory) files. This prevents context loss.
</Info>

<AccordionGroup>
  <Accordion title="Recognized overflow signatures">
    Zuvix detects context overflow from these provider error patterns:
...
When `agents.defaults.compaction.keepRecentTokens` is set, manual compaction honors that Zuvix cut-point and keeps the recent tail in rebuilt context. Without an explicit keep budget, manual compaction behaves as a hard checkpoint and continues from the new summary alone.

## Configuration

Configure compaction under `agents.defaults.compaction` in your `zuvix.json`. The most common knobs are listed below; for the full reference, see [Session management deep dive](/reference/session-management-compaction).
...
When `agents.defaults.compaction.truncateAfterCompaction` is enabled, Zuvix does not rewrite the existing transcript in place. It creates a new active successor transcript from the compaction summary, preserved state, and unsummarized tail, then records checkpoint metadata that points branch/restore flows at that compacted successor.
...
Zuvix no longer writes separate `.checkpoint.*.jsonl` copies for new
...
Before compaction, Zuvix can run a **silent memory flush** turn to store durable notes to disk. Set `agents.defaults.compaction.memoryFlush.model` when this housekeeping turn should use a local model instead of the active conversation model:
...
Plugins can register a custom compaction provider via `registerCompactionProvider()` on the plugin API. When a provider is registered and configured, Zuvix delegates summarization to it instead of the built-in LLM pipeline.
...
Setting a `provider` automatically forces `mode: "safeguard"`. Providers receive the same compaction instructions and identifier-preservation policy as the built-in path, and Zuvix still preserves recent-turn and split-turn suffix context after provider output.

<Note>
If the provider fails or returns an empty result, Zuvix falls back to built-in LLM summarization.
</Note>
