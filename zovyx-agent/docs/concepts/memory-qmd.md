...
alongside Zuvix. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Zuvix falls back to the
...
Zuvix creates a self-contained QMD home under
`~/.zuvix/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Zuvix creates collections from your workspace memory files and any
...
- If QMD fails entirely, Zuvix falls back to the builtin SQLite engine.
  ...
  `zuvix memory status` and one-shot CLI probes still recheck QMD directly.
...
Zuvix keeps the QMD search path compatible with both current and older QMD
installs.

On startup, Zuvix checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, Zuvix searches all
...
Older QMD builds only accept one collection filter. When Zuvix detects one
...
process, so you can tune QMD globally without adding new Zuvix config:
...
collection under `~/.zuvix/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Zuvix logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Zuvix
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but Zuvix still reports
...
with `zuvix memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Zuvix uses.

**Many QMD subprocesses during search?** Update QMD if possible. Zuvix uses
...
`memory.qmd.searchMode = "search"`. Zuvix treats that mode as lexical-only,
...
the next startup reconciliation. Zuvix recreates stale managed collections
...
Zuvix's builtin symlink rules. Keep temporary monorepo checkouts under
