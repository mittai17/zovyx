---
summary: "How Zuvix separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between Zuvix, Codex, ACP, or another native agent runtime
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `github-copilot`      | How Zuvix authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`                 | The model selected for the agent turn.                              |
| Agent runtime | `zuvix`, `codex`, `copilot`, `claude-cli` | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp           | Where messages enter and leave Zuvix.                            |
...
`zuvix doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside Zuvix's prepared agent loop. Today this
  is the built-in `zuvix` runtime plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | Zuvix name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `zuvix doctor --fix` owns legacy
...
That means Zuvix selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or Zuvix session store
becomes Codex.
...
adapter path. Claude Code, Gemini CLI, Zuvix, Cursor, and similar external
...
3. If the user explicitly chooses **Zuvix for an OpenAI model**, keep the model ref
   as `openai/<model>` and set provider/model runtime policy to
   `agentRuntime.id: "zuvix"`. A selected `openai` OAuth profile is routed
   internally through Zuvix's Codex-auth transport.
4. If legacy config still contains **legacy Codex model refs**, repair it to
   `openai/<model>` with `zuvix doctor --fix`; doctor keeps the Codex auth
   ...
   app-server route; Zuvix no longer keeps a bundled Codex CLI backend.
5. If the user explicitly says **ACP**, **acpx**, or **Codex ACP adapter**, use
   ACP with `runtime: "acp"` and `agentId: "codex"`.
6. If the request is for **Claude Code, Gemini CLI, Zuvix, Cursor, Droid, or
...
| Surface                     | Zuvix embedded                             | Codex app-server                                                            |
| --------------------------- | --------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Zuvix through the Zuvix embedded runner | Codex app-server                                                            |
| Canonical thread state      | Zuvix transcript                           | Codex thread, plus Zuvix transcript mirror                               |
| Zuvix dynamic tools      | Native Zuvix tool loop                     | Bridged through the Codex adapter                                           |
| Native shell and file tools | Zuvix path                                 | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Zuvix context assembly              | Zuvix projects assembled context into the Codex turn                     |
| Compaction                  | Zuvix or selected context engine           | Codex-native compaction, with Zuvix notifications and mirror maintenance |
| Channel delivery            | Zuvix                                      | Zuvix                                                                    |

This ownership split is the main design rule:

- If Zuvix owns the surface, Zuvix can provide normal plugin hook behavior.
- If the native runtime owns the surface, Zuvix needs runtime events or native hooks.
- If the native runtime owns canonical thread state, Zuvix should mirror and project context, not rewrite unsupported internals.

## Runtime selection

Zuvix chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, Zuvix uses `zuvix` as the
...
`ZUVIX_AGENT_RUNTIME`, session `agentHarnessId`/`agentRuntimeOverride` state,
`agents.defaults.agentRuntime`, and `agents.list[].agentRuntime`. Run
`zuvix doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where Zuvix can preserve the intent.
...
selection/runtime error; it is never silently routed back to Zuvix.
...
harness. Explicit Zuvix runtime config remains an opt-in compatibility route for
`openai/*` agent turns; when paired with a selected `openai` OAuth profile,
Zuvix routes that path internally through the Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI runtime session pins are
ignored by runtime selection and can be cleaned with `zuvix doctor --fix`.

If `zuvix doctor` warns that the `codex` plugin is enabled while
legacy Codex model refs remain in config, treat that as legacy route state. Run
`zuvix doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@zuvix/copilot` plugin registers an opt-in `copilot` runtime
...
`zuvix doctor` auto-loads. For configuration, auth, transcript mirroring,
...
When a runtime is not Zuvix, it should document what Zuvix surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether Zuvix can edit history or only mirror it.                                   |
| Do Zuvix dynamic tools work?        | Messaging, sessions, cron, and Zuvix-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Zuvix-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume Zuvix equivalence where the native runtime owns more state.            |
