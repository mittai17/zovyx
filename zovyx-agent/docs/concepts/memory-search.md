...
`@zuvix/llama-cpp-provider` and set `provider: "local"`. Source checkouts
...
Zuvix runs two retrieval paths in parallel and merges the results:
...
**No results?** Run `zuvix memory status` to check the index. If empty, run
