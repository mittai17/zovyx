...
That shape fixes local bugs, but it leaves Zuvix with too many public
...
That gives Zuvix at-least-once recovery. Exactly-once behavior exists only
...
- Structured Zuvix-origin metadata for operational/system output so visible
...
Zuvix should borrow the vocabulary, not copy the surface.

What Zuvix needs beyond that model:
...
`thread.post()` style promises are not enough for Zuvix. They hide the
...
Origin describes who produced a message and how Zuvix should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be Zuvix-originated operational output.
...
Core owns the meaning of Zuvix-originated output. Channels own how that
...
Zuvix operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that Zuvix accepted
...
- **Inbound record ack:** confirms Zuvix persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared Zuvix echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropZuvixEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
...
same visible gateway-failure text but without Zuvix origin metadata still
...
middleware chain, but Zuvix persists only the safe completed update id after
...
source if we need platform-level redelivery beyond Zuvix's restart
...
process dies there, Zuvix cannot know whether the platform message exists
...
```typescript
import { defineChannelMessageAdapter } from "zuvix/plugin-sdk/channel-outbound";
```
...
Before preflight authorization, core must run the shared Zuvix echo predicate
whenever `origin.decode` returns Zuvix-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  Zuvix can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- Zuvix gateway failure output must stay visible to humans, but tagged
...
| Slack           | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps Zuvix gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                  |
...
| Matrix          | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode Zuvix gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.              |
...
| Google Chat     | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged Zuvix gateway-failure echoes.                                                                                                                                                                                        |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve Zuvix operational provenance.
...
- Add `zuvix/plugin-sdk/channel-outbound`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropZuvixEcho` predicate in the channel-outbound SDK surface.
...
This is where the model stops being "agent replies" and becomes "Zuvix sends
messages".
...
- Gateway-failure origin factory and `shouldDropZuvixEcho` predicate.
...
- Slack tagged Zuvix gateway failure messages stay visible outbound, tagged
...
- Matrix tagged Zuvix gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only Zuvix's persisted restart watermark.
...
- Zuvix-originated gateway failure output is visible to humans but tagged
