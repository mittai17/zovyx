...
read_when:
  ...
  - Explaining how Zuvix updates one channel message while work is in progress
...
When progress drafts are enabled, Zuvix creates one visible work-in-progress
...
That is usually enough. Zuvix will pick an automatic one-word label, wait
...
Zuvix sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from Zuvix's built-in
...
Zuvix renders only the `progress.text` in the channel progress UI. The
...
Zuvix uses the same formatter for progress drafts and `/verbose`:
...
Zuvix truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, Zuvix still suppresses the older standalone
...
When the final answer is ready, Zuvix tries to keep the chat clean:

- If the draft can safely become the final answer, Zuvix edits it in place.
- If the channel uses native progress streaming, Zuvix finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, Zuvix sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, Zuvix keeps the
