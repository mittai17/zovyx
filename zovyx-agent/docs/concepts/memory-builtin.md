...
| Local             | `local`             | `@zuvix/llama-cpp-provider`      |
| Mistral           | `mistral`           |                                     |
...
Zuvix indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.zuvix/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `zuvix memory index --force`
...
**Memory search disabled?** Check `zuvix memory status`. If no provider is
...
**Stale results?** Run `zuvix memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Zuvix falls back to in-process cosine similarity
automatically. `zuvix memory status --deep` reports the local vector store
