...
Dreaming is the background memory consolidation system in `memory-core`. It helps Zuvix move strong short-term signals into durable memory while keeping the process explainable and reviewable.
...
<Tabs>
  <Tab title="Promotion preview / apply">
    ```bash
    zuvix memory promote
    zuvix memory promote --apply
    zuvix memory promote --limit 5
...
If `zuvix memory status` reports `Dreaming status: blocked`, the managed cron exists but the default agent heartbeat is not firing. Check that heartbeat is enabled for the default agent and that its target is not `none`, then run `zuvix memory status --deep` again after the next heartbeat interval.
