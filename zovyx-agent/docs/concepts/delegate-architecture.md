---
summary: "Delegate architecture: running Zuvix as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Zuvix as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Zuvix agent that:
...
Zuvix's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Zuvix's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.zuvix/workspace-delegate",
  tools: {
...
- Cron run history: Zuvix shared SQLite state database
- Session transcripts: `~/.zuvix/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Zuvix's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
zuvix agents add delegate
```

This creates:

- Workspace: `~/.zuvix/workspace-delegate`
- State: `~/.zuvix/agents/delegate/agent`
- Sessions: `~/.zuvix/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.zuvix/workspace" },
      {
        id: "delegate",
        workspace: "~/.zuvix/workspace-delegate",
        tools: {
...
~/.zuvix/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.zuvix/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.zuvix/workspace-org",
        agentDir: "~/.zuvix/agents/org-assistant/agent",
...
recall view. Zuvix redacts credential/token-like text, truncates long
