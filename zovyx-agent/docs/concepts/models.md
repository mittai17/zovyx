...
<CardGroup cols={2}>
  ...
  <Card title="Agent runtimes" href="/concepts/agent-runtimes">
    Zuvix, Codex, and other agent loop runtimes.
  </Card>
...
Zuvix selects models in this order:
...
<AccordionGroup>
  <Accordion title="Related model surfaces">
    - `agents.defaults.models` is the allowlist/catalog of models Zuvix can use (plus aliases). Use `provider/*` entries to limit visible providers while keeping provider discovery dynamic.
...
- Auto fallback selections are temporary recovery state. They are stored with `modelOverrideSource: "auto"` so later turns can keep using the fallback chain without probing a known-bad primary every time; Zuvix periodically probes the original primary again, clears the auto selection when it recovers, and announces fallback/recovery transitions once per state change.
- User session selections are exact. `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` store `modelOverrideSource: "user"`; if that selected provider/model is unreachable, Zuvix fails visibly instead of falling through to another configured model.
...
Provider configuration examples (including Zuvix) live in [Zuvix](/providers/zuvix).
</Note>
...
<AccordionGroup>
  <Accordion title="Clobber protection rules">
    `zuvix config set` protects model/provider maps from accidental clobbers. A plain object assignment to `agents.defaults.models`, `models.providers`, or `models.providers.<id>.models` is rejected when it would remove existing entries. Use `--merge` for additive changes; use `--replace` only when the provided value should become the complete target value.

    Interactive provider setup and `zuvix configure --section model` also merge provider-scoped selections into the existing allowlist, so adding Codex, Ollama, or another provider does not drop unrelated model entries. Configure preserves an existing `agents.defaults.model.primary` when provider auth is re-applied. Explicit default-setting commands such as `zuvix models auth login --provider <id> --set-default` and `zuvix models set <model>` still replace `agents.defaults.model.primary`.
...
If `agents.defaults.models` is set, it becomes the **allowlist** for `/model` and for session overrides. When a user selects a model that isn't in that allowlist, Zuvix returns:
...
exact provider/model shown by `zuvix models list --provider <provider>`.
...
<AccordionGroup>
  <Accordion title="Picker behavior">
    ...
    - On Telegram, `/models` picker selections are session-scoped; they do not change the agent's persistent default in `zuvix.json`.
  ...
  <Accordion title="Persistence and live switching">
    ...
    - If a run is already active, Zuvix marks a live switch as pending and only restarts into the new model at a clean retry point.
  ...
  <Accordion title="Ref parsing">
    ...
    - If you omit the provider, Zuvix resolves the input in this order:
      1. alias match
      2. unique configured-provider match for that exact unprefixed model id
      3. deprecated fallback to the configured default provider — if that provider no longer exposes the configured default model, Zuvix instead falls back to the first configured provider/model to avoid surfacing a stale removed-provider default.
  </Accordion>
</AccordionGroup>
...
zuvix models list
zuvix models status
zuvix models set <provider/model>
zuvix models set-image <provider/model>

zuvix models aliases list
zuvix models aliases add <alias> <provider/model>
zuvix models aliases remove <alias>

zuvix models fallbacks list
zuvix models fallbacks add <provider/model>
zuvix models fallbacks remove <provider/model>
zuvix models fallbacks clear

zuvix models image-fallbacks list
zuvix models image-fallbacks add <provider/model>
zuvix models image-fallbacks remove <provider/model>
zuvix models image-fallbacks clear
```

`zuvix models` (no subcommand) is a shortcut for `models status`.
...
`zuvix models scan` inspects OpenRouter's **free model catalog** and can optionally probe models for tool and image support.
...
When live probes run in a TTY, you can select fallbacks interactively. In non-interactive mode, pass `--yes` to accept defaults. Metadata-only results are informational; `--set-default` and `--set-image` require live probes so Zuvix does not configure an unusable keyless OpenRouter model.

## Models registry (`models.json`)

Custom providers in `models.providers` are written into `models.json` under the agent directory (default `~/.zuvix/agents/<agentId>/agent/models.json`). Provider-plugin catalogs are stored as generated plugin-owned catalog shards under the agent's plugin state and loaded automatically. This file is merged by default unless `models.mode` is set to `replace`.
...
<Note>
Marker persistence is source-authoritative: Zuvix writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values. This applies whenever Zuvix regenerates `models.json`, including command-driven paths like `zuvix agent`.
</Note>

## Related

- [Agent runtimes](/concepts/agent-runtimes) — Zuvix, Codex, and other agent loop runtimes
