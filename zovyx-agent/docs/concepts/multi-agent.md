...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.zuvix/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.zuvix/agents/<agentId>/sessions`.
...
a local profile, but Zuvix does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.zuvix/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.zuvix` (or `ZUVIX_STATE_DIR`)
- Workspace: `~/.zuvix/workspace` (or `~/.zuvix/workspace-<agentId>`)
- Agent dir: `~/.zuvix/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.zuvix/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Zuvix runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.zuvix/workspace` (or `~/.zuvix/workspace-<profile>` when `ZUVIX_PROFILE` is set).
- State defaults to `~/.zuvix/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    zuvix agents add coding
    zuvix agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.zuvix/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.zuvix/workspace-alex" },
      { id: "mia", workspace: "~/.zuvix/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, Zuvix upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, Zuvix falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.zuvix/workspace-main" },
          { id: "coding", workspace: "~/.zuvix/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.zuvix/workspace-main" },
          { id: "alerts", workspace: "~/.zuvix/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    zuvix channels login --channel whatsapp --account personal
    zuvix channels login --channel whatsapp --account biz
    ```

    `~/.zuvix/zuvix.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.zuvix/workspace-home",
            agentDir: "~/.zuvix/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.zuvix/workspace-work",
            agentDir: "~/.zuvix/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.zuvix/credentials/whatsapp/personal
              // authDir: "~/.zuvix/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.zuvix/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.zuvix/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.zuvix/workspace-opus",
