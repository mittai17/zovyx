...
Zuvix gives agents tools to work across sessions, inspect status, and
...
After the target responds, Zuvix can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `zuvix-tui` are
not session keys.
...
- `deliveryContext` is the persisted delivery route stored on the session,
  which Zuvix can reuse for later delivery even when the active surface
  differs.
...
`subagents` is the visibility helper for already spawned Zuvix
...
the completion origin only identifies a channel Zuvix can still reuse the
