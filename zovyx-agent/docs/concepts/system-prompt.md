---
summary: "What the Zuvix system prompt contains and how it is assembled"
read_when:
...
Zuvix builds a custom system prompt for every agent run. The prompt is **Zuvix-owned** and does not use a runtime default prompt.

The prompt is assembled by Zuvix and injected into each agent run.
...
the full Zuvix-owned prompt. The provider runtime can:
...
- **Zuvix Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **Zuvix Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Zuvix docs/source and when to read them.
...
Zuvix keeps large stable content, including **Project Context**, above the
...
Zuvix can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **Zuvix
...
For channel auto-reply runs, Zuvix omits the generic **Silent Replies**
...
Zuvix keeps committed prompt snapshots for the Codex runtime happy path under
...
Zuvix developer instructions, turn-scoped collaboration-mode instructions
when Zuvix provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after Zuvix sends
...
On the native Codex harness, Zuvix avoids repeating stable workspace files
...
Codex developer instructions. The compact Zuvix skills list is also forwarded
...
Zuvix prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, Zuvix can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, Zuvix injects a compact **available skills list**
...
points to the local Zuvix docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.zuvix.ai](https://docs.zuvix.ai).

The same section also includes the Zuvix source location. Git checkouts expose the local
...
consult docs first for Zuvix behavior, commands, configuration, or architecture, and to
run `zuvix status` itself when possible (asking the user only when it lacks access).
