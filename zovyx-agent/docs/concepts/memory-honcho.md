...
[Honcho](https://honcho.dev) adds AI-native memory to Zuvix. It persists
...
```bash
zuvix plugins install @honcho-ai/zuvix-honcho
zuvix honcho setup
zuvix gateway --force
```
...
Settings live under `plugins.entries["zuvix-honcho"].config`:
    ...
    entries: {
      "zuvix-honcho": {
        config: {
          ...
          workspaceId: "zuvix", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `zuvix honcho setup` detects and
...
```bash
zuvix honcho setup                        # Configure API key and migrate files
zuvix honcho status                       # Check connection status
zuvix honcho ask <question>               # Query Honcho about the user
zuvix honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/zuvix-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Zuvix integration guide](https://docs.honcho.dev/v3/guides/integrations/zuvix)
- [Memory](/concepts/memory) -- Zuvix memory overview
