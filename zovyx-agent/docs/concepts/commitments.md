...
read_when:
  - You want Zuvix to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, Zuvix can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. Zuvix may check in afterward.
- You say you are exhausted. Zuvix may ask later whether you slept.
- The agent says it will follow up after something changes. Zuvix may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: Zuvix remembers a
...
```bash
zuvix config set commitments.enabled true
zuvix config set commitments.maxPerDay 3
```

Equivalent `zuvix.json`:
...
After an agent reply, Zuvix may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, Zuvix stores a commitment with:
...
Zuvix never delivers an inferred commitment immediately after writing it.
...
zuvix commitments dismiss cm_abc123
```

See [`zuvix commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local Zuvix state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
zuvix config set commitments.enabled false
```
...
- Check `zuvix commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`zuvix commitments`](/cli/commitments)
