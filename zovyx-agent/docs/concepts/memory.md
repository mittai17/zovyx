---
summary: "How Zuvix remembers things across sessions"
...
Zuvix remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.zuvix/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, Zuvix keeps the file on
...
`zuvix doctor` to see raw vs injected sizes and truncation status.
...
Memory can preserve approval context, but it does not enforce policy. Use Zuvix approval settings, sandboxing, and scheduled tasks for hard operational controls.
...
for that case. Zuvix infers them in a hidden background pass, scopes them to
...
<Info>
Zuvix uses OpenAI embeddings by default. Set
...
Before [compaction](/concepts/compaction) summarizes your conversation, Zuvix
...
```bash
zuvix memory rem-backfill --rollback
zuvix memory rem-backfill --rollback-short-term
```

## CLI

```bash
zuvix memory status          # Check index status and provider
zuvix memory search "query"  # Search from the command line
zuvix memory index --force   # Rebuild the index
```
