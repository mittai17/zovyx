...
Zuvix handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. Zuvix keeps a
...
Zuvix keeps that boundary explicit:
...
Zuvix separates the **prompt body** from the **command body**:
...
Zuvix can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, Zuvix
strips the silent text but still delivers the media attachment.
Zuvix resolves that behavior by conversation type:

- Direct conversations never receive `NO_REPLY` prompt guidance. If a direct
  run accidentally returns a bare silent token, Zuvix suppresses it instead
...
Zuvix also uses silent replies for generic internal runner failures in
