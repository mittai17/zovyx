...
In Zuvix, a loop is a single, serialized run per session that emits lifecycle and stream events
...
2. `agentCommand` runs the agent:
   ...
   - calls `runEmbeddedAgent` (Zuvix agent runtime)
   - emits **lifecycle end/error** if the embedded loop does not emit one
3. `runEmbeddedAgent`:
   - serializes runs via per-session + global queues
   - resolves model + auth profile and builds the Zuvix session
...
4. `subscribeEmbeddedAgentSession` bridges agent runtime events to Zuvix `agent` stream:
...
- System prompt is built from Zuvix's base prompt, skills prompt, bootstrap context, and per-run overrides.
- Model-specific limits and compaction reserve tokens are enforced.
- See [System prompt](/concepts/system-prompt) for what the model sees.

## Hook points (where you can intercept)

Zuvix has two hook systems:
...
- **`before_install`**: inspect staged skill or plugin install material after operator install policy runs, when plugin hooks are loaded in the current Zuvix process.
- **`tool_result_persist`**: synchronously transform tool results before they are written to an Zuvix-owned session transcript.
...
Zuvix plugin hooks as the compatibility contract for documented mirrored
