---
summary: "Mantis is the visual end-to-end verification system for reproducing Zuvix bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for Zuvix bugs
...
Mantis is the Zuvix end-to-end verification system for bugs that need a real
...
Mantis lives in the Zuvix QA stack.

- Zuvix owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm zuvix qa mantis`.
...
- Zuvix agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in Zuvix, machine scheduling in
...
```bash
pnpm zuvix qa mantis discord-smoke \
...
thread, calls Zuvix's `message.thread-reply` action with a repo-local
...
```bash
pnpm zuvix qa mantis desktop-browser-smoke \
...
`ZUVIX_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `ZUVIX_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `ZUVIX_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `ZUVIX_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `ZUVIX_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `ZUVIX_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm zuvix qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT Zuvix gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable Zuvix
home at `$HOME/.zuvix-mantis/slack-zuvix`, patches Slack Socket Mode
configuration for the selected channel, starts `zuvix gateway run` on port
...
- `ZUVIX_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `ZUVIX_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `ZUVIX_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `ZUVIX_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent Zuvix Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `ZUVIX_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/zuvix-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`zuvix/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm zuvix qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm zuvix qa telegram --credential-source convex
...
`@zuvix-mantis telegram desktop proof`, from the Actions UI with freeform
...
Zuvix with the leased Telegram SUT bot token, starts `zuvix gateway run`
...
token never logs Telegram Desktop in; it only configures Zuvix. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring Zuvix.
...
comment or creates one. The workflows publish to `zuvix-crabbox-artifacts`
with public URLs under `https://artifacts.zuvix.ai`. They provide bucket,
...
```text
@zuvix-mantis discord status reactions
```
...
```text
@zuvix-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
6. Start a child Zuvix Gateway with an isolated state directory.
...
- It exercises a real Zuvix Gateway, Discord bot auth, message dispatch,
...
- `pnpm zuvix qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or Zuvix browser control captures screenshots.
...
- Node 22 and pnpm
- Zuvix checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one Zuvix Gateway, one browser, and one model run
...
- `ZUVIX_QA_DISCORD_MANTIS_BOT_TOKEN`
- `ZUVIX_QA_DISCORD_DRIVER_BOT_TOKEN`
- `ZUVIX_QA_DISCORD_SUT_BOT_TOKEN`
- `ZUVIX_QA_DISCORD_GUILD_ID`
- `ZUVIX_QA_DISCORD_CHANNEL_ID`
- `ZUVIX_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `ZUVIX_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `ZUVIX_QA_CONVEX_SITE_URL`
- `ZUVIX_QA_CONVEX_SECRET_CI`
- `ZUVIX_QA_MANTIS_CRABBOX_COORDINATOR`
- `ZUVIX_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`ZUVIX_QA_REDACT_PUBLIC_METADATA=1` for this reason.
