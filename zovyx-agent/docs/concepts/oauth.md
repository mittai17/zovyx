---
summary: "OAuth in Zuvix: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand Zuvix OAuth end-to-end
...
Zuvix supports "subscription auth" via OAuth for providers that offer it
(notably **OpenAI Codex (ChatGPT OAuth)**). For Anthropic, the practical split
is now:

- **Anthropic API key**: normal Anthropic API billing
- **Anthropic Claude CLI / subscription auth inside Zuvix**: Anthropic staff
  told us this usage is allowed again

OpenAI Codex OAuth is explicitly supported for use in external tools like
Zuvix.

Zuvix stores both OpenAI API-key auth and ChatGPT/Codex OAuth under the
...
`zuvix doctor --fix`; use `openai:*` profile ids and `auth.order.openai` for
new config.
...
Zuvix also supports **provider plugins** that ship their own OAuth or API-key
flows. Run them via:

```bash
zuvix models auth login --provider <id>
```
...
- you log in via Zuvix _and_ via Claude Code / Codex CLI → one of them randomly gets "logged out" later

To reduce that, Zuvix treats `auth-profiles.json` as a **token sink**:

- the runtime reads credentials from **one place**
- we can keep multiple profiles and route them deterministically
- external CLI reuse is provider-specific: Codex CLI can bootstrap an empty
  `openai:default` profile, but once Zuvix has a local OAuth profile,
  the local refresh token is canonical. If that local refresh token is rejected,
  Zuvix can use a usable same-account Codex CLI token as a runtime-only
...
- Auth profiles (OAuth + API keys + optional value-level refs): `~/.zuvix/agents/<agentId>/agent/auth-profiles.json`
- Legacy compatibility file: `~/.zuvix/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.zuvix/credentials/oauth.json` (imported into `auth-profiles.json` on first use)

All of the above also respect `$ZUVIX_STATE_DIR` (state dir override). Full reference: [/gateway/configuration](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, Zuvix uses read-through
...
Claude subscription limits, and Anthropic staff told us Zuvix-style Claude
CLI usage is allowed again. Zuvix therefore treats Claude CLI reuse and
...
If you want other subscription-style options in Zuvix, see [OpenAI
Codex](/providers/openai), [Qwen Cloud Coding
Plan](/providers/qwen), [MiniMax Coding Plan](/providers/minimax),
and [Z.AI / GLM Coding Plan](/providers/zai).
</Warning>

Zuvix also exposes Anthropic setup-token as a supported token-auth path, but it now prefers Claude CLI reuse and `claude -p` when available.

## Anthropic Claude CLI migration

Zuvix supports Anthropic Claude CLI reuse again. If you already have a local
Claude login on the host, onboarding/configure can reuse it directly.

## OAuth exchange (how login works)

Zuvix's interactive login flows are implemented in `zuvix/plugin-sdk/llm` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. start Anthropic setup-token or paste-token from Zuvix
2. Zuvix stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including Zuvix workflows.
...
`zuvix models auth list --provider openai` after repair before copying
...
Wizard path is `zuvix onboard` → auth choice `openai`.
...
- exception: some external CLI credentials stay externally managed; Zuvix
  ...
  `openai:default` profile, then Zuvix-owned refreshes keep the local
  profile canonical. If the local Codex refresh fails and Codex CLI has a
  usable token for the same account, Zuvix may use that token for the current
...
- `zuvix channels list --json` (shows `auth[]`)
