---
summary: "Move one Zuvix session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Zuvix session.
...
Alice can message Zuvix on Telegram and Discord:
...
Zuvix keeps the current session context and changes the reply route:
...
4. Keep the same Zuvix session, but receive future replies in Discord.
