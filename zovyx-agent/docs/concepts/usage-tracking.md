...
- `/usage cost` in chats: local cost summary aggregated from Zuvix session logs.
- CLI: `zuvix status --usage` prints a full per-provider breakdown.
- CLI: `zuvix channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
{
  "messages": {
    "usageTemplate": "~/.zuvix/usage-footer.json"
  }
}
```
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Zuvix treats
  ...
  quota, so Zuvix inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Zuvix prefers the
...
- **DeepSeek**: API key via env/config/auth store (`DEEPSEEK_API_KEY`).
  Zuvix calls DeepSeek's balance endpoint and shows the provider-reported
...
can supply plugin-specific usage auth logic; otherwise Zuvix falls back to
