---
summary: "How Zuvix manages conversation sessions"
read_when:
...
Zuvix organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `zuvix security audit`.
...
Zuvix automatically bounds session storage over time. By default, it runs
...
`zuvix sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `zuvix sessions cleanup --dry-run`.

## Inspecting sessions

- `zuvix status` -- session store path and recent activity.
- `zuvix sessions --json` -- all sessions (filter with `--active <minutes>`).
