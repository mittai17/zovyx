...
Paste this into `zuvix.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, Zuvix can show:
...
case-insensitive. If `allowedChatIds` is non-empty and Zuvix cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. Zuvix
...
5. Verify memory search itself works with `zuvix memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, Zuvix uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `zuvix memory status --deep` to inspect the memory-search backend
      and index health.
