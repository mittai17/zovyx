...
import type { ZuvixModalDialog } from "../ui/components/modal-dialog.ts";
...
export async function getRenderedModalDialog(container: HTMLElement) {
  const modal = container.querySelector<ZuvixModalDialog>("zuvix-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected zuvix-modal-dialog");
  }
