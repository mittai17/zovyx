...
function resolvePendingUpdateHandoffTimeoutBanner(): {
  ...
  return {
    tone: "danger",
    text: "Update handoff started, but completion was not reported after reconnect. Run `zuvix update status` for the final result.",
  };
}
      ...
      if (host.pendingAbort) {
        ...
        void host.client
          ...
          .catch((err: unknown) => {
            ...
            console.warn("[zuvix] pending abort failed:", err);
          });
      }
