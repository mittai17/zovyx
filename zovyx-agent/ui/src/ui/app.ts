...
declare global {
  interface Window {
    __ZUVIX_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class ZuvixApp extends LitElement {
  readonly i18nController = new I18nController(this);
  clientInstanceId = generateUUID();
  connectGeneration = 0;
  @state() settings: UiSettings = loadSettings();
  @state() supabaseSession: Session | null = null;
  ...
  @state() theme: ThemeName = this.settings.theme ?? "nova";
  ...
  clearCustomTheme() {
    const nextTheme = this.theme === "custom" ? "nova" : this.theme;
    this.customThemeImportExpanded = true;
    this.customThemeImportSelectOnSuccess = false;
    applySettingsInternal(this as unknown as Parameters<typeof applySettingsInternal>[0], {
      ...this.settings,
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "Zuvix"}: ${entry.text}`;
...
if (!customElements.get("zuvix-app")) {
  customElements.define("zuvix-app", ZuvixApp);
}
