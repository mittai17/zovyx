...
function renderServerRow(props: McpViewProps, server: McpServerRow) {
  ...
  const probeCommand = `zuvix mcp probe ${quotedName}`;
  const loginCommand = `zuvix mcp login ${quotedName}`;
  return html`
    ...
    <section class="mcp-page">
      ...
      <section class="card mcp-command-card">
        ...
        <div class="mcp-command-card__grid">
          <code>zuvix mcp status --verbose</code>
          <code>zuvix mcp doctor --probe</code>
          <code>zuvix mcp login &lt;name&gt;</code>
          <code>zuvix mcp reload</code>
        </div>
      </section>
