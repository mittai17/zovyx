...
const SETTINGS_KEY_PREFIX = "zuvix.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "zuvix.control.settings.v1";
const LOCAL_USER_IDENTITY_KEY = "zuvix.control.user.v1";
const LOCAL_ASSISTANT_IDENTITY_KEY = "zuvix.control.assistant.v1";
const LEGACY_TOKEN_SESSION_KEY = "zuvix.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "zuvix.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    normalizeOptionalString(window["__ZUVIX_CONTROL_UI_BASE_PATH__"]);
...
export function loadSettings(): UiSettings {
  ...
  try {
    ...
    const settings = {
      ...
      theme: theme === "custom" && !customTheme ? "nova" : theme,
      themeMode: mode,
      chatShowThinking:
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("zuvix.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
