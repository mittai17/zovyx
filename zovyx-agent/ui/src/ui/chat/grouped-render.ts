...
const SKIP_DELETE_CONFIRM_KEY = "zuvix:skipDeleteConfirm";
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__zuvix__|media|api\/chat\/media\/outgoing)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__zuvix__/assistant-media?${params.toString()}`;
}
  ...
  if (!pending) {
    pending = (async () => {
      ...
      if (requesterSessionKey) {
        headers.set("x-zuvix-requester-session-key", requesterSessionKey);
      }
...
function renderGroupedMessage(
  ...
  const transcriptMeta =
    m["__zuvix"] && typeof m["__zuvix"] === "object" && !Array.isArray(m["__zuvix"])
      ? (m["__zuvix"] as Record<string, unknown>)
      : null;
