...
const SKILL_WORKSHOP_MODE_KEY = "zuvix:control-ui:skill-workshop-mode:v1";
const SKILL_WORKSHOP_CURRENT_CHAT_REVISIONS_KEY =
  "zuvix:control-ui:skill-workshop-current-chat-revisions:v1";
...
const lazyAgentWorld = createLazyView(() => import("./views/agent-world.ts"), notifyLazyViewChanged);
...
const UPDATE_BANNER_DISMISS_KEY = "zuvix:control-ui:update-banner-dismissed:v1";
...
const BOTTOM_NAV_TABS: Tab[] = ["chat", "overview", "agentWorld", "agents", "config"];

function renderBottomNav(state: AppViewState) {
  return html`
    <nav class="shell-bottom-nav">
      ${BOTTOM_NAV_TABS.map((tab) => {
        ...
        return html`
          ...
          >
            <span class="bottom-nav-item__icon" aria-hidden="true"
              >${icons[iconForTab(tab)]}</span
            >
            <span class="bottom-nav-item__label">${titleForTab(tab)}</span>
          </a>
        `;
      })}
    </nav>
    ...
    >
                ...
                ${navCollapsed
                  ...
                  : html`
                      ...
                      <span class="sidebar-brand__copy">
                        <span class="sidebar-brand__title">Zuvix</span>
                      </span>
                    `}
              </div>
              <button
      ...
      >
        ...
        ${state.tab === "cron"
          ...
          : nothing}
        ${state.tab === "agentWorld"
          ? renderLazyView(lazyAgentWorld, (m) =>
              m.renderAgentWorld({
                basePath: state.basePath ?? "",
