...
import type { ZuvixModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: ZuvixModalDialog, id: string): HTMLElement {
...
describe("zuvix-modal-dialog", () => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("zuvix-modal-dialog-label");
    expect(descriptionId).toBe("zuvix-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "zuvix-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "zuvix-modal-dialog-description").textContent).toBe(
