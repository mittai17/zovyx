...
import type { ZuvixFilePreviewModal } from "./file-preview-modal.ts";
...
async function renderPreview(query = "") {
  ...
  const modal = container.querySelector<ZuvixFilePreviewModal>("zuvix-file-preview-modal");
...
function shadowText(modal: ZuvixFilePreviewModal): string {
  return modal.shadowRoot?.textContent ?? "";
}

describe("zuvix-file-preview-modal", () => {
  beforeEach(() => {
