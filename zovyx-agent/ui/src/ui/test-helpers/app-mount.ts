...
import "../app.ts";
import type { ZuvixApp } from "../app.ts";
...
const mountedApps = new Set<ZuvixApp>();

function collectMountedApps() {
  return new Set<ZuvixApp>([
    ...mountedApps,
    ...document.querySelectorAll<ZuvixApp>("zuvix-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<ZuvixApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<ZuvixApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("zuvix-app") as ZuvixApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__ZUVIX_CONTROL_UI_BASE_PATH__"] = undefined;
