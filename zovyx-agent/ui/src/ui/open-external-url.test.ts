...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://zuvix.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://zuvix.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://zuvix.ai/abc-123", baseHref)).toBe(
      "blob:https://zuvix.ai/abc-123",
    );
  });
