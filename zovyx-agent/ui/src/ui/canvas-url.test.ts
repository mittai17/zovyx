...
describe("resolveCanvasIframeUrl", () => {
  it("allows same-origin hosted canvas document paths", () => {
    expect(resolveCanvasIframeUrl("/__zuvix__/canvas/documents/cv_demo/index.html")).toBe(
      "/__zuvix__/canvas/documents/cv_demo/index.html",
    );
  });
  ...
  it("rewrites safe canvas paths through the scoped canvas host", () => {
    expect(
      resolveCanvasIframeUrl(
        "/__zuvix__/canvas/documents/cv_demo/index.html",
        "http://127.0.0.1:19003/__zuvix__/cap/cap_123",
      ),
    ).toBe(
      "http://127.0.0.1:19003/__zuvix__/cap/cap_123/__zuvix__/canvas/documents/cv_demo/index.html",
    );
  });
