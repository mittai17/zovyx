...
function resolveControlUiBuildId(): string {
  const explicit =
    process.env.ZUVIX_CONTROL_UI_BUILD_ID?.trim() || process.env.ZUVIX_VERSION?.trim();
  if (explicit) {
...
function controlUiServiceWorkerBuildIdPlugin(buildId: string): Plugin {
  return {
    ...
    closeBundle() {
      ...
      const placeholder = '"__ZUVIX_CONTROL_UI_BUILD_ID__"';
...
export default function controlUiViteConfig(): UserConfig {
  const envBase = process.env.ZUVIX_CONTROL_UI_BASE_PATH?.trim();
  ...
  return {
    base,
    define: {
      ZUVIX_CONTROL_UI_BUILD_ID: JSON.stringify(controlUiBuildId),
    },
