...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `zuvix: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[zuvix] Failed to respawn launcher: ${
...
const respawnWithoutCompileCacheIfNeeded = () => {
  ...
  if (process.env.ZUVIX_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.ZUVIX_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    ZUVIX_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["zuvix: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:zuvix/zuvix#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g zuvix@latest`.");
  return lines.join("\n");
};
...
const isHelpFastPathDisabled = () =>
  process.env.ZUVIX_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.ZUVIX_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.ZUVIX_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.ZUVIX_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "zuvix.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".zuvix", "zuvix.json"),
    path.join(homeDir, ".zuvix", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "zuvix.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    if (normalizeLauncherMetadataValue(process.env.ZUVIX_CONTAINER)) {
      return false;
    }
    ...
    process.stdout.write(commit ? `Zuvix ${version} (${commit})\n` : `Zuvix ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.ZUVIX_BUNDLED_VERSION) ?? "0.0.0";
}
