...
const RUN_NODE_SKIP_DTS_BUILD = process.env.ZUVIX_RUN_NODE_SKIP_DTS_BUILD === "1";
...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.ZUVIX_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
const shouldBuildPrivateQaEntries = process.env.ZUVIX_BUILD_PRIVATE_QA === "1";
...
function shouldAlwaysBundleDependency(id: string): boolean {
  return (
    id === "@zuvix/fs-safe" ||
    id.startsWith("@zuvix/fs-safe/") ||
    id === "@zuvix/normalization-core" ||
    id.startsWith("@zuvix/normalization-core/") ||
    id === "@zuvix/media-core" ||
    id.startsWith("@zuvix/media-core/") ||
    id === "@zuvix/acp-core" ||
    id.startsWith("@zuvix/acp-core/") ||
    id === "zod" ||
    id.startsWith("zod/")
  );
}
...
function shouldExternalizeGatewayClientDependency(id: string): boolean {
  return (
    id === "ws" ||
    id.startsWith("ws/") ||
    id === "@zuvix/gateway-protocol" ||
    id.startsWith("@zuvix/gateway-protocol/")
  );
}
...
function shouldExternalizeSpeechCoreDependency(id: string): boolean {
  return id === "zuvix" || id.startsWith("zuvix/");
}
...
function shouldExternalizeLlmRuntimeDependency(id: string): boolean {
  return id === "@zuvix/llm-core" || id.startsWith("@zuvix/llm-core/");
}
