...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    ...
    expect(resolved.ZUVIX_BASE_ENV).toBe("base");
    expect(resolved.ZUVIX_TEST_ENV).toBe("ok");
    expect(resolved.ZUVIX_TO_REMOVE).toBeUndefined();
    expect(resolved.ZUVIX_CLI).toBe(ZUVIX_CLI_ENV_VALUE);
  });
