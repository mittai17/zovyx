...
import {
  buildZuvixCompileCacheRespawnPlan,
  isSourceCheckoutInstallRoot,
  resolveZuvixCompileCacheDirectory,
  resolveEntryInstallRoot,
  runZuvixCompileCacheRespawnPlan,
  shouldEnableZuvixCompileCache,
...
describe("entry compile cache", () => {
  ...
  it("resolves install roots from source and dist entry paths", () => {
    expect(resolveEntryInstallRoot("/repo/zuvix/src/entry.ts")).toBe("/repo/zuvix");
    expect(resolveEntryInstallRoot("/repo/zuvix/dist/entry.js")).toBe("/repo/zuvix");
    expect(resolveEntryInstallRoot("/pkg/zuvix/entry.js")).toBe("/pkg/zuvix");
  });

  it("treats git and source entry markers as source checkouts", async () => {
    const root = makeTempDir(tempDirs, "zuvix-compile-cache-source-");
    await fs.writeFile(path.join(root, ".git"), "gitdir: .git/worktrees/zuvix\n", "utf8");

    expect(isSourceCheckoutInstallRoot(root)).toBe(true);
  });

  it("disables compile cache for source-checkout installs", async () => {
    const root = makeTempDir(tempDirs, "zuvix-compile-cache-src-entry-");
  ...
  it("keeps compile cache enabled for packaged installs unless disabled by env", () => {
    const root = makeTempDir(tempDirs, "zuvix-compile-cache-package-");

    expect(shouldEnableZuvixCompileCache({ env: {}, installRoot: root })).toBe(true);
    expect(
      shouldEnableZuvixCompileCache({
  ...
  it("scopes packaged compile cache by package install metadata", async () => {
    const root = makeTempDir(tempDirs, "zuvix-compile-cache-package-key-");
    const packageJsonPath = path.join(root, "package.json");
    await fs.writeFile(packageJsonPath, '{"version":"2026.4.29"}\n', "utf8");

    const directory = resolveZuvixCompileCacheDirectory({
      env: { NODE_COMPILE_CACHE: path.join(root, ".node-cache") },
      installRoot: root,
    });

    expect(directory).toContain(path.join(".node-cache", "zuvix"));
  ...
  it("builds a one-shot no-cache respawn plan when source checkout inherits NODE_COMPILE_CACHE", async () => {
    const root = makeTempDir(tempDirs, "zuvix-compile-cache-respawn-");
    await fs.mkdir(path.join(root, "src"), { recursive: true });
    await fs.writeFile(path.join(root, "src", "entry.ts"), "export {};\n", "utf8");

    const plan = buildZuvixCompileCacheRespawnPlan({
      currentFile: path.join(root, "dist", "entry.js"),
      env: { NODE_COMPILE_CACHE: "/tmp/zuvix-cache" },
    ...
    expect(plan).toEqual({
      ...
      env: {
        ...
        ZUVIX_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
      },
    });
  });

  it("does not respawn packaged installs when NODE_COMPILE_CACHE is configured", () => {
    const root = makeTempDir(tempDirs, "zuvix-compile-cache-package-respawn-");
  ...
  it("does not respawn source checkouts twice", async () => {
    const root = makeTempDir(tempDirs, "zuvix-compile-cache-respawn-once-");
    await fs.mkdir(path.join(root, "src"), { recursive: true });
    await fs.writeFile(path.join(root, "src", "entry.ts"), "export {};\n", "utf8");
    ...
    expect(
      buildZuvixCompileCacheRespawnPlan({
        currentFile: path.join(root, "dist", "entry.js"),
        env: {
          NODE_COMPILE_CACHE: "/tmp/zuvix-cache",
          ZUVIX_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
        },
        installRoot: root,
      }),
  ...
  it("runs compile-cache respawn plans with the child-process bridge", () => {
    ...
    runZuvixCompileCacheRespawnPlan(
      {
        command: "/usr/bin/node",
        args: ["/repo/zuvix/dist/entry.js", "status"],
        env: { NODE_DISABLE_COMPILE_CACHE: "1" },
      },
      {
    ...
    expect(spawn).toHaveBeenCalledWith(
      "/usr/bin/node",
      ["/repo/zuvix/dist/entry.js", "status"],
      {
        stdio: "inherit",
        env: { NODE_DISABLE_COMPILE_CACHE: "1" },
      },
