...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "ZUVIX_ENV_TEST_A";
    const keyB = "ZUVIX_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "ZUVIX_ENV_TEST_ADDED";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "ZUVIX_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "ZUVIX_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "ZUVIX_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "ZUVIX_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "ZUVIX_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "zuvix-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousZuvixHome = process.env.ZUVIX_HOME;
    const previousStateDir = process.env.ZUVIX_STATE_DIR;
    const previousBundledDir = process.env.ZUVIX_BUNDLED_PLUGINS_DIR;
    process.env.ZUVIX_HOME = "/srv/zuvix-home";
    process.env.ZUVIX_STATE_DIR = "/srv/zuvix-state";
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR = "/srv/zuvix-bundled";
    ...
    try {
      ...
      expect(env.ZUVIX_HOME).toBeUndefined();
      expect(env.ZUVIX_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.ZUVIX_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("ZUVIX_HOME", previousZuvixHome);
      restoreEnvKey("ZUVIX_STATE_DIR", previousStateDir);
      restoreEnvKey("ZUVIX_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processZuvixHome: process.env.ZUVIX_HOME,
          processBundledDir: process.env.ZUVIX_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.ZUVIX_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processZuvixHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.ZUVIX_HOME).toBe("/srv/zuvix-home");
    } finally {
      restoreEnvKey("ZUVIX_HOME", previousZuvixHome);
    }
  });
