// Creates isolated Zuvix state directories for integration-style tests.
...
type ZuvixTestStateLayout = "home" | "state-only" | "split";

type ZuvixTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type ZuvixTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: ZuvixTestStateLayout;
  scenario?: ZuvixTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "zuvix-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "zuvix.json"),
...
function scenarioConfig(options: ZuvixTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "zuvix-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: ZuvixTestStateOptions): Record<string, string | undefined> {
...
export async function createZuvixTestState(
  options: ZuvixTestStateOptions = {},
): Promise<ZuvixTestState> {
  ...
  const state: ZuvixTestState = {
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed Zuvix env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withZuvixTestState<T>(
  options: ZuvixTestStateOptions,
  fn: (state: ZuvixTestState) => Promise<T>,
): Promise<T> {
  const state = await createZuvixTestState(options);
  try {
