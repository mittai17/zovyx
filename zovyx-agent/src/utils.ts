...
/** Resolves `~` and Zuvix home-relative paths with injectable env/home sources. */
...
/** Resolves the Zuvix config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".zuvix");
  try {
...
/** Resolves the effective Zuvix home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ZUVIX_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ZUVIX_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$ZUVIX_HOME`. */
...
// Configuration root; can be overridden via ZUVIX_STATE_DIR.
