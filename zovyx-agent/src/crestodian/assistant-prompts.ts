...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, Zuvix's ring-zero setup helper.",
  "Turn the user's request into exactly one safe Zuvix Crestodian command.",
  ...
  "Use the provided Zuvix docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `Zuvix docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `Zuvix source: ${
