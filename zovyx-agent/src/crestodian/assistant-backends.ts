...
type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => ZuvixConfig;
};
...
function buildCliPlannerConfig(workspaceDir: string, modelRef: string): ZuvixConfig {
  return {
    agents: {
      defaults: {
...
function buildCodexAppServerPlannerConfig(workspaceDir: string): ZuvixConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,
