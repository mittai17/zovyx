...
const SETUP_RE =
  /^(?:setup(?!\s+agent\b)|set\s+me\s+up|set\s+up\s+zuvix|onboard|onboard\s+me|bootstrap|first\s+run)(?:\b|$)/i;
...
function formatSetupPlanDescription(
  ...
  return `bootstrap Zuvix setup for workspace ${workspace}${model}`;
}
