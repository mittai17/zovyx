...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/zuvix.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
    ...
    references: {
      docsUrl: "https://docs.zuvix.ai",
      sourceUrl: "https://github.com/zuvix/zuvix",
    },
  })),
}));
...
describe("parseCrestodianOperation", () => {
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @zuvix/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@zuvix/demo",
    });
    expect(parseCrestodianOperation("plugin install clawhub:zuvix-demo")).toEqual({
      kind: "plugin-install",
      spec: "clawhub:zuvix-demo",
    });
    expect(parseCrestodianOperation("plugin uninstall zuvix-demo")).toEqual({
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/zuvix.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("installs plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-install", spec: "clawhub:zuvix-demo" },
      runtime,
      { deps: { runPluginInstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: install plugin clawhub:zuvix-demo. Say yes to apply.",
    });
    ...
    expect(installCall[0]).toBe("clawhub:zuvix-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Installed plugin clawhub:zuvix-demo",
      },
      { rescue: true, spec: "clawhub:zuvix-demo" },
    );
  });
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "zuvix-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: uninstall plugin zuvix-demo. Say yes to apply.",
    });
    ...
    expect(uninstallCall[0]).toBe("zuvix-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Uninstalled plugin zuvix-demo",
      },
      { rescue: true, pluginId: "zuvix-demo" },
    );
  });
