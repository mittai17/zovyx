...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ZUVIX_HOME for tilde expansion", () => {
    vi.stubEnv("ZUVIX_HOME", "/srv/zuvix-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/zuvix-home", "cron", "jobs.json"));
  });
});
