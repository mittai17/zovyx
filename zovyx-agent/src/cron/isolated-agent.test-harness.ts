...
/** Runs a test callback with an isolated Zuvix home for cron tests. */
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "zuvix-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".zuvix", "sessions");
