// Plugin state SQLite helpers persist plugin state in the Zuvix state database.
...
type PluginStateEntriesTable = ZuvixStateKyselyDatabase["plugin_state_entries"];
type PluginStateStoreDatabase = Pick<ZuvixStateKyselyDatabase, "plugin_state_entries">;
...
function wrapPluginStateError(
  ...
  message: string,
  pathname = resolveZuvixStateSqlitePath(process.env),
): PluginStateStoreError {
...
function parseStoredJson(raw: string, operation: PluginStateStoreOperation): unknown {
  try {
    ...
    throw createPluginStateError({
      ...
      path: resolveZuvixStateSqlitePath(process.env),
      cause: error,
    });
  }
}
...
function openPluginStateDatabase(
  ...
  const pathname = resolveZuvixStateSqlitePath(env);
...
function envOptions(env?: NodeJS.ProcessEnv): ZuvixStateDatabaseOptions {
  return env ? { env } : {};
}
...
function runWriteTransaction<T>(
  ...
  return runZuvixStateWriteTransaction(() => {
...
export function probePluginStateStore(): PluginStateStoreProbeResult {
  const databasePath = resolveZuvixStateSqlitePath(process.env);
  const steps: PluginStateStoreProbeStep[] = [];
  const wasOpen = cachedDatabase !== null;
  const stateWasOpen = isZuvixStateDatabaseOpen();
  ...
  try {
    ...
    openZuvixStateDatabase().walMaintenance.checkpoint();
    pushOk("checkpoint");
  } catch (error) {
