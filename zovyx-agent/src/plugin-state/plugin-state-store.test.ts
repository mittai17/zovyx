...
import { openZuvixStateDatabase } from "../state/zuvix-state-db.js";
...
beforeAll(async () => {
  testState = await createZuvixTestState({ label: "plugin-state-store" });
  rmSync(path.dirname(resolveZuvixStateSqlitePath()), { recursive: true, force: true });
});
...
describe("plugin state keyed store", () => {
      ...
      async (stateA) => {
        await withZuvixTestState(
          ...
          async (stateB) => {
            ...
            expect(resolveZuvixStateSqlitePath(stateA.env)).not.toBe(
              resolveZuvixStateSqlitePath(stateB.env),
            );
          },
        );
      },
  ...
  it("reopens after the shared state DB cache closes its handle", async () => {
    await withPluginStateTestState(async () => {
      ...
      const secondary = await createZuvixTestState({
        label: "plugin-state-cache-secondary",
        applyEnv: false,
      });
      try {
        openZuvixStateDatabase({ env: secondary.env });
        testState?.applyEnv();
        await expect(store.lookup("k")).resolves.toEqual({ ok: true });
      } finally {
  ...
  it.runIf(process.platform !== "win32")("hardens DB directory and file permissions", async () => {
    await withPluginStateTestState(async () => {
      ...
      const databasePath = resolveZuvixStateSqlitePath();
