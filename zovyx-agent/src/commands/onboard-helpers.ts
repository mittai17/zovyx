...
export function summarizeExistingConfig(config: ZuvixConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: ZuvixConfig): string | null {
...
export function printWizardHeader(runtime: RuntimeEnv) {
  ...
  const title = supportsDecorativeEmoji() && icon ? `${icon} ZUVIX ${icon}` : "ZUVIX";
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    "Docs:",
    "https://docs.zuvix.ai/gateway/remote",
    "https://docs.zuvix.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
