...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("zuvix tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("zuvix tasks flow cancel <flow-id>")}`,
    ]
...
function notePluginVersionDrift(drift: PluginVersionDriftReport | undefined) {
  ...
  const lines = [
    ...
    } not on Zuvix ${drift.gatewayVersion}`,
    ...
    singleDrift
      ? `Fix: ${formatCliCommand(
          `zuvix plugins update ${singleDrift.pluginId}`,
        )} && ${formatCliCommand("zuvix gateway restart")}.`
      : `Fix: ${formatCliCommand(
          "zuvix plugins update <plugin-id>",
        )} for each drifted plugin, then ${formatCliCommand("zuvix gateway restart")}.`,
  ];
  note(lines.join("\n"), "Plugin version drift");
}

/** Emits workspace, skills, plugin, and TaskFlow recovery status notes for doctor. */
export function noteWorkspaceStatus(cfg: ZuvixConfig, options: NoteWorkspaceStatusOptions = {}) {
