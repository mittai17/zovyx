...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("zuvix/plugin-sdk/test-node-mocks");
...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 zuvix-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/zuvix/dist/index.js gateway --port 18789",
        " 103 zuvix channels",
        " 104 zuvix tui --local",
        " 105 /usr/bin/zuvix chat",
        " 106 helper --note 'zuvix tui'",
        " 107 zuvix-helper zuvix terminal",
        " 108 zuvix --flag tui",
      ].join("\n"),
    });

    expect(listLocalTuiProcesses()).toEqual([
      { pid: 101, command: "zuvix-tui" },
      { pid: 104, command: "zuvix tui --local" },
      { pid: 105, command: "/usr/bin/zuvix chat" },
    ]);
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "zuvix-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    const cfg = { channels: { whatsapp: { enabled: true } } } as ZuvixConfig;
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "zuvix-tui" }],
