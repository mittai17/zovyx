...
function mockConfig(storePath: string, overrides?: Partial<ZuvixConfig>) {
  const config = {
    agents: {
      defaults: {
        timeoutSeconds: 600,
...
async function withTempStore(
  ...
  overrides?: Partial<ZuvixConfig>,
) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-agent-cli-"));
