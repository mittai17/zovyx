...
import { resolvePrimaryStringValue } from "@zuvix/normalization-core/string-coerce";
import type { ZodIssue } from "zod";
import { note } from "../../packages/terminal-core/src/note.js";
import { CONFIG_PATH } from "../config/config.js";
import { resolveAgentModelFallbackValues } from "../config/model-input.js";
import type { ZuvixConfig } from "../config/types.zuvix.js";
import { ZuvixSchema } from "../config/zod-schema.js";
...
function isUpdateInProgress(): boolean {
  const value = process.env.ZUVIX_UPDATE_IN_PROGRESS;
 ...
 */
export function stripUnknownConfigKeys(config: ZuvixConfig): {
  config: ZuvixConfig;
  removed: string[];
} {
  ...
  const parsed = ZuvixSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
/** Warns when legacy Zuvix provider overrides shadow the built-in catalog. */
export function noteOpencodeProviderOverrides(cfg: ZuvixConfig): void {
  ...
  if (providers.zuvix) {
    overrides.push("zuvix");
  }
  if (providers["zuvix-zen"]) {
    overrides.push("zuvix-zen");
  }
  if (providers["zuvix-go"]) {
    overrides.push("zuvix-go");
  }
  ...
  const lines = overrides.flatMap((id) => {
    const providerLabel = id === "zuvix-go" ? "Zuvix Go" : "Zuvix Zen";
...
export function collectImplicitFallbackClobberWarnings(cfg: ZuvixConfig): string[] {
...
export function noteImplicitFallbackClobberWarnings(cfg: ZuvixConfig): void {
