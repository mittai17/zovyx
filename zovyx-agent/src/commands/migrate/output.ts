...
const REASON_CODE_MESSAGES: Record<string, string> = {
  ...
  already_active: "Plugin is already active in Zuvix",
  installed: "Plugin is already installed in Zuvix",
...
function formatItemMessage(item: MigrationItem, mode: FormatMode): string | undefined {
  if (mode === "preview") {
    ...
    if (item.kind === "skill" && item.action === "copy") {
      return "Copy Codex skill into Zuvix";
    }
    if (item.kind === "plugin" && item.action === "install") {
      return "Install Codex plugin into Zuvix";
    }
...
export function assertConflictFreePlan(plan: MigrationPlan, providerId: string): void {
  if (plan.summary.conflicts > 0) {
    throw new Error(
      `Migration has ${formatCount(plan.summary.conflicts, "conflict")}. Re-run with --overwrite after reviewing zuvix migrate plan ${providerId}.`,
    );
  }
}
...
export function writeApplyResult(
  ...
  if (result.backupPath) {
    ...
    runtime.log("Backup: skipped (no existing Zuvix state found)");
  }
  if (result.reportDir) {
