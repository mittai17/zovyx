...
function isTempBackedZuvixRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const zuvixIndex = lower.lastIndexOf("zuvix");
  if (zuvixIndex < 1) {
    return false;
  }
  return lower[zuvixIndex - 1] === "tmp" || lower[zuvixIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("zuvix@"),
    ) || isTempBackedZuvixRoot(beforeSkillRoot)
  );
}
