...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    promptRemoteGatewayConfig: vi.fn(async (cfg: ZuvixConfig) => ({
      ...cfg,
      gateway: { mode: "remote", remote: { url: "wss://gateway.example.test" } },
    })),
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: ZuvixConfig }) =>
      Boolean(config.auth?.profiles?.["openai:default"]),
    ),
    setupChannels: vi.fn(async (cfg: ZuvixConfig) => cfg),
  };
});
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.zuvix/workspace",
  applyWizardMetadata: (cfg: ZuvixConfig) => cfg,
...
function setupBaseWizardState(config: ZuvixConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: ZuvixConfig) => cfg);
    mocks.promptAuthConfig.mockReset();
    mocks.promptAuthConfig.mockImplementation(async (cfg: ZuvixConfig) => cfg);
    mocks.promptGatewayConfig.mockReset();
    mocks.promptGatewayConfig.mockImplementation(async (cfg: ZuvixConfig) => ({
      config: cfg,
      port: 18789,
    }));
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: ZuvixConfig) =>
  ...
  it("still supports keyless web search providers through the shared setup flow", async () => {
    ...
    mocks.resolveSearchProviderOptions.mockReturnValue([
      createSearchProviderOption({
        ...
        docsUrl: "https://docs.zuvix.ai/tools/web",
        credentialPath: "",
      }),
    ]);
    mocks.setupSearch.mockImplementation(async (cfg: ZuvixConfig) =>
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    ...
    expect(String(defaults.workspace)).toContain("/.zuvix/workspace");
