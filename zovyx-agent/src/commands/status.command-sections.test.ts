...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:zuvix security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:zuvix security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:zuvix status --all",
      "  Need to debug live? cmd:zuvix logs --follow",
      "  Fix reachability first: cmd:zuvix gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:zuvix devices approve req-123)",
      "muted(Fallback: cmd:zuvix devices approve --latest)",
      "muted(Inspect: cmd:zuvix devices list)",
    ]);
  });
