...
beforeAll(() => {
  envSnapshot = captureEnv(["ZUVIX_PROFILE"]);
  process.env.ZUVIX_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByZuvix: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/zuvix/pnpm-lock.yaml",
        markerPath: "/tmp/zuvix/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.ZUVIX_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.zuvix.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.zuvix.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.ZUVIX_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/zuvix-root.js", () => ({
  resolveZuvixPackageRoot: vi.fn().mockResolvedValue("/tmp/zuvix"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /zuvix (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("zuvix --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("ZUVIX_GATEWAY_TOKEN", "abcd1234", async () => {
