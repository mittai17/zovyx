...
describe("resolveDaemonZuvixBinDir", () => {
  it("uses the active zuvix command directory", () => {
    expect(
      resolveDaemonZuvixBinDir({
        argv: ["node", "/Users/testuser/.npm-global/bin/zuvix", "gateway", "install"],
        env: { PATH: "" },
        platform: "darwin",
      }),
  ...
  it("finds the PATH shim that resolves to the active package entrypoint", () => {
    const realpaths = new Map([
      ["/Users/testuser/.npm-global/bin/zuvix", "/pkg/zuvix/zuvix.mjs"],
      [
        "/Users/testuser/.npm-global/lib/node_modules/zuvix/zuvix.mjs",
        "/pkg/zuvix/zuvix.mjs",
      ],
    ]);
    ...
    expect(
      resolveDaemonZuvixBinDir({
        ...
        platform: "darwin",
        existsSync: (candidate) => candidate === "/Users/testuser/.npm-global/bin/zuvix",
        realpathSync: (candidate) => realpaths.get(candidate) ?? candidate,
      }),
    ).toEqual(["/Users/testuser/.npm-global/bin"]);
  });

  it("ignores unrelated zuvix commands elsewhere on PATH", () => {
    expect(
      resolveDaemonZuvixBinDir({
        argv: ["node", "/opt/zuvix/zuvix.mjs", "gateway", "install"],
        env: { PATH: "/Users/testuser/.npm-global/bin" },
        platform: "darwin",
        existsSync: () => true,
        realpathSync: (candidate) =>
          candidate === "/Users/testuser/.npm-global/bin/zuvix"
            ? "/other/zuvix.mjs"
            : candidate,
      }),
    ).toBeUndefined();
...
describe("resolveDaemonServicePathDirs", () => {
  it("combines node and active zuvix command directories", () => {
    expect(
      resolveDaemonServicePathDirs({
