...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.zuvix.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.zuvix.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        ZUVIX_STATE_DIR: "/tmp/zuvix-service",
        ZUVIX_CONFIG_PATH: "/tmp/zuvix-service/zuvix.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { ZUVIX_STATE_DIR?: string; ZUVIX_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.ZUVIX_STATE_DIR).toBe("/tmp/zuvix-service");
    expect(handoffEnv?.ZUVIX_CONFIG_PATH).toBe("/tmp/zuvix-service/zuvix.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-zuvix/bin/zuvix logs --follow",
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "zuvix-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "zuvix-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "zuvix-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("zuvix gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system Zuvix gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "zuvix-gateway.service",
        detail: "unit: /etc/systemd/system/zuvix-gateway.service",
        scope: "system",
        marker: "zuvix",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level Zuvix gateway service detected while the user gateway service is not installed.",
        "- zuvix-gateway.service (unit: /etc/systemd/system/zuvix-gateway.service)",
        "Zuvix will not install a second user-level gateway service automatically.",
        "Run `zuvix gateway status --deep` or `zuvix doctor --deep` to inspect duplicate services.",
