...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<ZuvixTestState> {
  const state = await createZuvixTestState({
    layout: "state-only",
    prefix: "zuvix-doctor-oauth-sidecar-",
    env: {
      ZUVIX_AGENT_DIR: undefined,
      ZUVIX_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "zuvix-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans ZUVIX_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
    const previousAgentDir = process.env.ZUVIX_AGENT_DIR;
    ...
    try {
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.ZUVIX_AGENT_DIR = previousAgentDir;
      }
    }
  });
