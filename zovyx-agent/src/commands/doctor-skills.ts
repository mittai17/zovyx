...
export function formatUnavailableSkillDoctorLines(skills: SkillStatusEntry[]): string[] {
  ...
  lines.push(`Disable unused skills: ${formatCliCommand("zuvix doctor --fix")}`);
  lines.push(
    `Inspect details: ${formatCliCommand("zuvix skills check --agent <id>")} or ${formatCliCommand("zuvix skills info <name> --agent <id>")}`,
  );
  return lines;
}
