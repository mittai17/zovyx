...
function formatFlowLookupMiss(lookup: string): string {
  return `TaskFlow not found: ${lookup}. Run ${formatCliCommand("zuvix tasks flow list")} to see recent flow ids.`;
}
...
export async function flowsListCommand(
  ...
  if (flows.length === 0) {
    runtime.log(
      `No TaskFlows found. Run ${formatCliCommand("zuvix tasks list")} to inspect standalone background tasks.`,
    );
    return;
  }
  const rich = isRich();
