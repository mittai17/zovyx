...
async function stopAndUninstallService(runtime: RuntimeEnv): Promise<boolean> {
  if (isNixMode) {
    ...
    runtime.error(
      `Nix mode detected; service uninstall is disabled. Manage the service through your Nix profile instead, then run ${formatCliCommand("zuvix status")} to verify.`,
    );
    return false;
  }
  ...
  try {
    ...
    runtime.error(
      `Gateway service check failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("zuvix gateway status --deep")} for service diagnostics.`,
    );
    return false;
  }
  if (!loaded) {
  ...
  try {
    ...
    runtime.error(
      `Gateway stop failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("zuvix gateway status --deep")} before retrying uninstall.`,
    );
  }
  try {
    ...
    runtime.error(
      `Gateway uninstall failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("zuvix gateway status --deep")} for the service state.`,
    );
    return false;
  }
}
...
async function removeMacApp(runtime: RuntimeEnv, dryRun?: boolean) {
  ...
  await removePath("/Applications/Zuvix.app", runtime, {
    dryRun,
    label: "/Applications/Zuvix.app",
  });
}

function logBackupRecommendation(runtime: RuntimeEnv) {
  runtime.log(`Recommended first: ${formatCliCommand("zuvix backup create")}`);
}
...
export async function uninstallCommand(runtime: RuntimeEnv, opts: UninstallOptions) {
  ...
  if (!interactive && !opts.yes) {
    runtime.error(
      `Non-interactive uninstall requires --yes. Preview first with ${formatCliCommand("zuvix uninstall --dry-run --all")}.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled<UninstallScope>({
      ...
      options: [
        ...
        { value: "state", label: "State + config", hint: "~/.zuvix" },
        { value: "workspace", label: "Workspace", hint: "agent files" },
        {
          value: "app",
          label: "macOS app",
          hint: "/Applications/Zuvix.app",
        },
      ],
