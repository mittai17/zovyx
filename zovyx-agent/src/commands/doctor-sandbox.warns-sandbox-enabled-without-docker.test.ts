...
describe("maybeRepairSandboxImages", () => {
  ...
  function createSandboxConfig(mode: "off" | "all" | "non-main"): ZuvixConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
  ...
  function createSandboxConfigWithDockerNetwork(network: string): ZuvixConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/zuvix/sandbox/containers.json",
        shardedDir: "/tmp/zuvix/sandbox/containers",
        source: "monolithic",
        exists: true,
        valid: true,
        entries: 2,
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- containers monolithic: /tmp/zuvix/sandbox/containers.json (2 entries)",
        "Run zuvix doctor --fix to migrate them to SQLite.",
      ].join("\n"),
      "Sandbox",
    );
  });
