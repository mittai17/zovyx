...
function hasExplicitMemorySearchConfig(cfg: ZuvixConfig, agentId: string): boolean {
...
export function resolveMemoryPluginStatus(cfg: ZuvixConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: ZuvixConfig, agentId: string) => { store: { path: string } } | null;
