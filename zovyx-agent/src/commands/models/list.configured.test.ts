...
describe("resolveConfiguredEntries", () => {
  ...
  it("canonicalizes manifest-owned provider aliases in configured rows", () => {
    vi.stubEnv("ZUVIX_BUNDLED_PLUGINS_DIR", path.resolve("extensions"));
  ...
  it("recovers bundled source aliases when stale dist metadata omits them", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-model-alias-source-"));
    try {
      ...
      fs.writeFileSync(
        path.join(sourcePluginRoot, "zuvix.plugin.json"),
        JSON.stringify({
          id: "zai",
            ...
            plugins: [
              {
                ...
                manifestPath: path.join(distPluginRoot, "zuvix.plugin.json"),
              },
            ],
          },
        },
