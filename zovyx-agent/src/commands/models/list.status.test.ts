...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/zuvix-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/zuvix-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/zuvix-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/zuvix-dev/zuvix.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/zuvix-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/zuvix-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/zuvix-dev/zuvix.json");
    expect(payload.auth.storePath).toBe("/tmp/zuvix-agent/auth-profiles.json");
  ...
  it("honors ZUVIX_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync({ ZUVIX_AGENT_DIR: "/tmp/zuvix-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/zuvix-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/zuvix-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/zuvix-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when ZUVIX_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync(
      {
        ZUVIX_AGENT_DIR: undefined,
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/zuvix-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/zuvix-legacy-agent");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/zuvix-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/zuvix-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/zuvix-agent-custom/auth-profiles.json",
        });
      },
    );
  });
      ...
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          ...
          effective: {
            kind: "profiles",
            detail: "/tmp/zuvix-agent/auth-profiles.json",
          },
        },
      ]);
      expect(localRuntime.exit).not.toHaveBeenCalledWith(1);
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/zuvix-agent/workspace"
          ? {
