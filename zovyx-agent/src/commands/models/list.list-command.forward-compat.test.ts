...
function resetMocks() {
  ...
  mocks.ensureZuvixModelsJson.mockResolvedValue({ wrote: false });
  mocks.ensureAuthProfileStore.mockReturnValue({ version: 1, profiles: {}, order: {} });
  mocks.resolveDefaultAgentDir.mockReturnValue("/tmp/zuvix-agent");
...
function installModelsListCommandForwardCompatMocks() {
  ...
  vi.doMock("../../agents/agent-scope.js", () => ({
    ...
    resolveAgentWorkspaceDir: vi.fn(() => "/tmp/zuvix-workspace"),
...
describe("modelsListCommand forward-compat", () => {
  ...
  describe("--all catalog supplementation", () => {
    it("uses the provider catalog fast path for Codex provider lists", async () => {
      ...
      expect(mocks.ensureZuvixModelsJson).not.toHaveBeenCalled();
      expect(mocks.loadModelRegistry).not.toHaveBeenCalled();
    ...
    it("includes provider-owned supplemental catalog rows with provider filters", async () => {
      ...
      mocks.loadModelRegistry.mockResolvedValueOnce({
        models: [],
        availableKeys: new Set(["zuvix-go/deepseek-v4-pro"]),
        registry: {
          getAll: () => [],
        },
      });
      ...
      mocks.resolveModelWithRegistry.mockImplementation(
        ({ provider, modelId }: { provider: string; modelId: string }) =>
          provider === "zuvix-go" && modelId === "deepseek-v4-pro"
            ? {
                provider,
                id: modelId,
                name: "DeepSeek V4 Pro",
      ...
      await modelsListCommand({ all: true, provider: "zuvix-go", json: true }, runtime as never);

      expectRowKeys(lastPrintedRows<{ key: string }>(), ["zuvix-go/deepseek-v4-pro"]);
    });
