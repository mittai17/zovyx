...
export async function loadValidConfigOrThrow(): Promise<ZuvixConfig> {
...
export async function updateConfig(
  mutator: (cfg: ZuvixConfig, context: UpdateConfigContext) => ZuvixConfig,
): Promise<ZuvixConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: ZuvixConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ZuvixConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("zuvix agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
