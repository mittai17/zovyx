...
describe("buildGatewayRuntimeHints", () => {
  ...
  it("surfaces suspicious systemd cgroup hygiene with inspection commands", () => {
    expect(
      ...
      "Run: systemctl --user show zuvix-gateway.service -p KillMode -p TasksCurrent -p MemoryCurrent -p MainPID",
      "Run: systemd-cgls --user-unit zuvix-gateway.service",
      "After reviewing service settings, run: zuvix gateway restart",
    ]);
  });
