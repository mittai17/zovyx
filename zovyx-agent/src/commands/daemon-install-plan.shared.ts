...
function isZuvixCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "zuvix") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "zuvix.cmd" || basename === "zuvix.ps1" || basename === "zuvix.exe"
    );
  }
  return false;
}
...
/** Resolve the Zuvix CLI binary directory from argv/PATH for daemon PATH. */
export function resolveDaemonZuvixBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isZuvixCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "zuvix.cmd" : "zuvix");
...
/** Merge Node and Zuvix binary directories for the daemon service PATH. */
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonZuvixBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}
