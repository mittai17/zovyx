...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is Zuvix ${VERSION}; the running Gateway is Zuvix ${gatewayVersion}.`,
      "Check `zuvix --version`, `which zuvix`, and `zuvix gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `zuvix` points to the version you want, or reinstall the Gateway service from that same Zuvix install.",
    ].join("\n"),
    "Zuvix version mismatch",
  );
}
