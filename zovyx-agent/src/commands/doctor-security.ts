...
import type { ZuvixConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: ZuvixConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: ZuvixConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Zuvix default (full)";
  const defaultRequestedAskSource = "Zuvix default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("zuvix approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: ZuvixConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: ZuvixConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: ZuvixConfig): string[] {
  ...
  return [
    "- WARNING: zuvix.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("zuvix secrets configure")} or ${formatCliCommand("zuvix secrets apply")}, then verify with ${formatCliCommand("zuvix secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("zuvix approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.zuvix.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("zuvix configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("zuvix config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("zuvix doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("zuvix config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('zuvix config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: ZuvixConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("zuvix security audit --deep")}`;
