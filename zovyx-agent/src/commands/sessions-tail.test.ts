...
describe("sessionsTailCommand", () => {
  ...
  beforeEach(() => {
    ...
    previousTrajectoryDir = process.env.ZUVIX_TRAJECTORY_DIR;
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-sessions-tail-"));
    process.env.ZUVIX_STATE_DIR = path.join(tmpDir, "state");
    delete process.env.ZUVIX_TRAJECTORY_DIR;
  ...
  afterEach(() => {
    ...
    if (previousTrajectoryDir === undefined) {
      ...
      process.env.ZUVIX_TRAJECTORY_DIR = previousTrajectoryDir;
    }
  ...
  it("uses a session trajectory pointer for relocated runtime files", async () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        traceSchema: "zuvix-trajectory-pointer",
        schemaVersion: 1,
        sessionId: "session-one",
  ...
  it("resolves the target store from a fully qualified non-default agent session key", async () => {
    ...
    const opsSessionsDir = path.join(process.env.ZUVIX_STATE_DIR!, "agents", "ops", "sessions");
