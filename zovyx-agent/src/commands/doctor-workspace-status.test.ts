...
async function runNoteWorkspaceStatusForTest(
  ...
  const cfg: ZuvixConfig = opts?.cfg ?? {};
...
describe("noteWorkspaceStatus", () => {
  ...
  it("surfaces active official managed plugin version drift", async () => {
    ...
    try {
      ...
      expect(body).toContain("1 active official plugin not on Zuvix 2026.6.1");
      expect(body).toContain("codex: 2026.5.30-beta.1 (npm) -> expected 2026.6.1");
      expect(body).toContain("zuvix plugins update codex");
      expect(body).toContain("zuvix gateway restart");
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("adds TaskFlow recovery hints for broken blocked flows", async () => {
    ...
    try {
      ...
      expect(body).toContain("zuvix tasks flow show <flow-id>");
    } finally {
      noteSpy.mockRestore();
    }
  });
