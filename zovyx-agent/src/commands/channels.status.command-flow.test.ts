...
const mocks = vi.hoisted(() => ({
  ...
  readConfigFileSnapshot: vi.fn(async () => ({ path: "/tmp/zuvix.json" })),
  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          channelId,
          label: "Feishu",
          installSpec: "@zuvix/feishu",
          installCommand: "zuvix plugins install @zuvix/feishu",
          doctorFixCommand: "zuvix doctor --fix",
          repairHint:
            "Install the official external plugin with: zuvix plugins install @zuvix/feishu, or run: zuvix doctor --fix.",
        }
      : null,
}));
...
describe("channelsStatusCommand SecretRef fallback flow", () => {
  ...
  it("shows missing official external plugin repair hints in config-only output", async () => {
    ...
    expect(joined).toContain(
      "Feishu: Install the official external plugin with: zuvix plugins install @zuvix/feishu, or run: zuvix doctor --fix.",
    );
  });
