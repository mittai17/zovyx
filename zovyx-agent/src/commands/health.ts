...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.ZUVIX_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: ZuvixConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: ZuvixConfig) => {
...
/** Runs the `zuvix health` command against the gateway and renders JSON or text. */
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.ZUVIX_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<ZuvixConfig> {
  const { readBestEffortConfig } = await loadConfigRuntime();
  return await readBestEffortConfig();
}

async function readRuntimeHealthConfig(): Promise<ZuvixConfig> {
