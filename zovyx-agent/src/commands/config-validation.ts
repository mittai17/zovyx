...
export async function requireValidConfigFileSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Zuvix config is invalid: ${snapshot.path}\n${issues}`);
    runtime.error(
    ...
    runtime.error(`Inspect: ${formatCliCommand("zuvix config validate")}`);
    runtime.exit(1);
    return null;
  }
...
/** Read and return a valid Zuvix config, or null after reporting validation errors. */
