...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".zuvix-work");
    const defaultStateDir = path.join(homeDir, ".zuvix");
    const profileConfigPath = path.join(profileStateDir, "zuvix.json");
    ...
    vi.stubEnv("ZUVIX_HOME", homeDir);
    vi.stubEnv("ZUVIX_PROFILE", "work");
    vi.stubEnv("ZUVIX_STATE_DIR", profileStateDir);
    vi.stubEnv("ZUVIX_CONFIG_PATH", profileConfigPath);
...
describe("moveToTrash", () => {
  it("uses fs-safe trash instead of resolving a PATH trash command", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-trash-helper-"));
  ...
  it("allows fs-safe trash to move a symlink whose target resolves outside the parent", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-trash-symlink-"));
    const targetPath = path.join(testRoot, "target-link");
    const outsideTarget = path.join(os.tmpdir(), "zuvix-trash-symlink-target");
  ...
  it("canonicalizes a symlinked parent before calling fs-safe trash", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-trash-parent-link-"));
    const lexicalParent = path.join(testRoot, "state-link");
    const realParent = path.join(testRoot, "state-real");
    const targetPath = path.join(lexicalParent, "zuvix.json");
    const sourcePath = path.join(realParent, "zuvix.json");
