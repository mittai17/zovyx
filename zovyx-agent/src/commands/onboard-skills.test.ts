...
describe("setupSkills", () => {
  ...
  it("hides brew-only installs in Linux containers when brew is missing", async () => {
    ...
    try {
      ...
      await setupSkills({} as ZuvixConfig, "/tmp/ws", runtime, prompter);
  ...
  it("displays a clear empty state note when all skill dependencies are ready", async () => {
    ...
    expect(emptyStateNote?.message).toContain("zuvix skills list --verbose");
    expect(emptyStateNote?.message).toContain("zuvix skills check");
  });
});
