...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: ZuvixConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => ZuvixConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: ZuvixConfig, sourceConfig: ZuvixConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      resolvedConfig: ZuvixConfig;
      effectiveConfig: ZuvixConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "zuvix-agent-" });
}
...
function mockConfig(home: string, storePath: string): ZuvixConfig {
  const cfg = {
    agents: {
      defaults: {
