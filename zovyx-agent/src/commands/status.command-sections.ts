...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("zuvix security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("zuvix security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusModelSelectionLines(params: {
  ...
  for (const sess of mismatches.slice(0, limit)) {
    ...
    lines.push(
      ...
      "  Docs: https://docs.zuvix.ai/concepts/models#selection-source-and-fallback-behavior",
    );
  }
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.zuvix.ai/faq",
    "Troubleshooting: https://docs.zuvix.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("zuvix status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("zuvix logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("zuvix node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("zuvix status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("zuvix gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`zuvix devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("zuvix devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("zuvix devices list")}`),
  ];
}
