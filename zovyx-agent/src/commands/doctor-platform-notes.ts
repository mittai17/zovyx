...
import { findStaleZuvixUpdateLaunchdJobs } from "../daemon/launchd.js";
...
export function collectMacLaunchAgentOverrideWarning(deps?: {
  ...
  const markerCandidates = [path.join(home, ".zuvix", "disable-launchagent")];
...
/** Returns a warning for stale Zuvix updater launchd jobs left after interrupted updates. */
export async function collectMacStaleZuvixUpdateLaunchdJobsWarning(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleZuvixUpdateLaunchdJobs;
  ...
  const jobs = await (deps?.findJobs ?? findStaleZuvixUpdateLaunchdJobs)(scanEnv).catch(
    () => [],
  );
  if (jobs.length === 0) {
    return null;
  ...
  return [
    "- Stale Zuvix updater launchd job(s) detected.",
    ...jobs.map((job) => {
      const exitStatus =
    ...
    `  ${formatCliCommand("zuvix gateway restart")}`,
  ].join("\n");
}

/** Emits stale updater launchd job notes using the gateway service environment when available. */
export async function noteMacStaleZuvixUpdateLaunchdJobs(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleZuvixUpdateLaunchdJobs;
  env?: NodeJS.ProcessEnv;
...
function hasConfigGatewayCreds(cfg: ZuvixConfig): boolean {
...
export async function collectMacLaunchctlGatewayEnvOverrideWarning(
  ...
  const tokenEntries = [
    ["ZUVIX_GATEWAY_TOKEN", await getenv("ZUVIX_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["ZUVIX_GATEWAY_PASSWORD", await getenv("ZUVIX_GATEWAY_PASSWORD")],
  ] as const;
  ...
  return [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "ZUVIX_GATEWAY_PASSWORD"}\` is set; it can make local clients use a different password than gateway.auth.password.`
      : undefined,
...
export async function collectMacGatewayPlatformWarnings(
  ...
  const serviceEnv =
    platform === "darwin" ? await resolveGatewayServiceEnvForPlatformNotes(deps) : deps?.env;
  const staleUpdateWarning = await collectMacStaleZuvixUpdateLaunchdJobsWarning({
    env: serviceEnv,
    findJobs: deps?.findJobs,
    platform,
  });
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.ZUVIX_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- ZUVIX_NO_RESPAWN is not set to 1; set it when you want routine gateway restarts to stay in-process instead of handing off to a managed supervisor.",
    );
  }
