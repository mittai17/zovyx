...
export async function runNonInteractiveLocalSetup(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `zuvix gateway run` or `zuvix agent --local`.",
              ]
            : [`Run \`${formatCliCommand("zuvix gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("zuvix gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("zuvix configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.zuvix.ai/tools/web`,
    );
  }
}
