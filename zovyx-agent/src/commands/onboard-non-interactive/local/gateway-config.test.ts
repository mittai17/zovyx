...
const SAMPLE_SECRET_REF = {
  source: "env" as const,
  provider: "default",
  id: "ZUVIX_GATEWAY_TOKEN_REF",
};

function createTokenConfig(token: unknown): ZuvixConfig {
  return {
...
describe("applyNonInteractiveGatewayConfig token resolution chain", () => {
  ...
  it("prefers existing plaintext token over ambient ZUVIX_GATEWAY_TOKEN on re-onboard", () => {
    // A stale shell/launchd ZUVIX_GATEWAY_TOKEN must not rotate a
    // persisted token — that would break already-paired clients.
    const nextConfig = createTokenConfig("existing-user-token");

    const result = applyGatewayConfig({
      nextConfig,
      env: { ZUVIX_GATEWAY_TOKEN: "stale-env-token" },
    });
  ...
  it("uses ZUVIX_GATEWAY_TOKEN to fill an empty config on first-run", () => {
    const result = applyGatewayConfig({ env: { ZUVIX_GATEWAY_TOKEN: "env-token" } });
  ...
  it("preserves an existing SecretRef even when ambient ZUVIX_GATEWAY_TOKEN is set", () => {
  ...
  it("overrides an existing SecretRef when --gateway-token-ref-env is provided", () => {
    const newRefId = "ZUVIX_GATEWAY_TOKEN_NEW_REF";
    const nextConfig = createTokenConfig(SAMPLE_SECRET_REF);
  ...
  it("fails when --gateway-token-ref-env points to a missing env var", () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      'Environment variable "MISSING_GATEWAY_TOKEN_ENV" is missing or empty. Export it first, then rerun zuvix onboard --non-interactive.',
    );
