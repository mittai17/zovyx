...
describe("noteSessionLockHealth", () => {
  let state: ZuvixTestState;

  beforeEach(async () => {
    note.mockClear();
    state = await createZuvixTestState({
      layout: "state-only",
      prefix: "zuvix-doctor-locks-",
    });
  });
  ...
  it("uses configured stale threshold without removing live Zuvix lock files", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("removes fresh live locks when the owner is not an Zuvix process", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(note).toHaveBeenCalledTimes(1);
    const [message] = firstNoteCall();
    expect(message).toContain("stale=yes (non-zuvix-owner)");
    expect(message).toContain("[removed]");
    expect(message).toContain("Removed 1 stale session lock file");
