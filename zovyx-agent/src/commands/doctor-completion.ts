...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(zuvix completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `zuvix doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`zuvix completion --write-state\` or \`zuvix doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
...
export async function ensureCompletionCacheExists(binName = "zuvix"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
