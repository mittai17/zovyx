...
/** CLI options accepted by `zuvix reset`. */
...
async function stopGatewayIfRunning(runtime: RuntimeEnv) {
  if (isNixMode) {
    // Nix mode owns service lifecycle outside Zuvix-managed launchd/systemd
...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Next: ${formatCliCommand("zuvix onboard --install-daemon")}`);
    return;
  }
