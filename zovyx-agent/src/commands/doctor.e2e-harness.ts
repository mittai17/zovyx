...
export const resolveZuvixPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadZuvixPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("zuvix/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("zuvix/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("zuvix/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/zuvix-root.js", () => ({
  ...
  resolveZuvixPackageRootSync: vi.fn(() => "/tmp/zuvix"),
}));
...
beforeEach(() => {
  ...
  resolveZuvixPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ZUVIX_STATE_DIR;
  originalUpdateInProgress = process.env.ZUVIX_UPDATE_IN_PROGRESS;
  process.env.ZUVIX_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-doctor-state-"));
