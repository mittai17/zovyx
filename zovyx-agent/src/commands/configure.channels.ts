...
/** Prompt for configured channel sections to remove from zuvix.json. */
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in zuvix.json.",
          `Tip: \`${formatCliCommand("zuvix channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    if (Object.keys(nextChannels).length) {
      next.channels = nextChannels as ZuvixConfig["channels"];
    } else {
