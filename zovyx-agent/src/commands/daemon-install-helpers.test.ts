...
vi.mock("../daemon/program-args.js", () => ({
  ZUVIX_WRAPPER_ENV_KEY: "ZUVIX_WRAPPER",
  resolveGatewayProgramArguments: mocks.resolveGatewayProgramArguments,
  resolveZuvixWrapperPath: mocks.resolveZuvixWrapperPath,
}));
...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/zuvix/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\zuvix\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/zuvix/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { ZUVIX_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveZuvixWrapperPath.mockImplementation(async (value: string | undefined) =>
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.zuvix/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ ZUVIX_PORT: "3000" });
  ...
  it("adds the active zuvix command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const zuvixBinPath = path.join(isolatedHome, ".npm-global", "bin", "zuvix");
    process.argv = ["node", zuvixBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(zuvixBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".zuvix"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes ZUVIX_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/zuvix-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ZUVIX_PORT: "3000",
        ZUVIX_WRAPPER: wrapperPath,
      },
    });
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.ZUVIX_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.ZUVIX_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".zuvix", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ZUVIX_PORT: "3000",
    ...
    expect(plan.environment.ZUVIX_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring ZUVIX_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.ZUVIX_PORT).toBe("3000");
    expect(plan.environment.ZUVIX_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("includes env SecretRef values from config into the service environment", async () => {
    ...
    expect(plan.environment.ZUVIX_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/zuvix-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/zuvix-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        ZUVIX_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "ZUVIX_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.ZUVIX_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.ZUVIX_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".zuvix"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ZUVIX_PORT: "3000",
    ...
    expect(plan.environment.ZUVIX_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".zuvix"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.ZUVIX_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { ZUVIX_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.ZUVIX_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        ZUVIX_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.ZUVIX_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.ZUVIX_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.ZUVIX_GATEWAY_TOKEN).toBeUndefined();
  });
  ...
  it("does not embed auth-profile env refs when the key is already durable", async () => {
    ...
    expect(plan.environment.ZUVIX_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:zuvix|zuvix)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale ZUVIX_ALLOW_ROOT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      { ZUVIX_ALLOW_ROOT: "1" },
      managedKeys,
    );
    expect(result.ZUVIX_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves ZUVIX_CLI_CONTAINER_BYPASS and ZUVIX_CONTAINER_HINT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        ZUVIX_CLI_CONTAINER_BYPASS: "1",
        ZUVIX_CONTAINER_HINT: "ci",
      },
      managedKeys,
    );
    expect(result.ZUVIX_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.ZUVIX_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary ZUVIX_FOO", () => {
    const result = collectPreservedExistingServiceEnvVars({ ZUVIX_FOO: "bar" }, managedKeys);
    expect(result.ZUVIX_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated ZUVIX_* keys", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        ZUVIX_CLI_CONTAINER_BYPASS: "1",
        ZUVIX_CONTAINER_HINT: "ci",
    ...
    expect(result.ZUVIX_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.ZUVIX_CONTAINER_HINT).toBe("ci");
    expect(result.ZUVIX_BAZ).toBeUndefined();
  });
});
