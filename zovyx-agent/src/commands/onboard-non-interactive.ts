...
async function runNonInteractiveMigrationImport(params: {
  ...
  if (!providerId) {
    ...
    params.runtime.error(
      `--import-from is required for non-interactive migration import. Run ${formatCliCommand("zuvix migrate list")} to choose a provider.`,
    );
...
export async function runNonInteractiveSetup(
  ...
  if (mode !== "local" && mode !== "remote") {
    runtime.error(
      `Invalid --mode "${String(mode)}". Use "local" or "remote", or run ${formatCliCommand("zuvix onboard")} for interactive setup.`,
    );
    runtime.exit(1);
    return;
  }
