 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "zuvix"`, `source: "implicit"`.
...
/**
 * Build a minimal `ZuvixConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "zuvix"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): ZuvixConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="zuvix", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "zuvix". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "zuvix" and reflects the actual runtime driving the session.
