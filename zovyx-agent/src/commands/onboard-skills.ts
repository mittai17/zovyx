...
export async function setupSkills(
  ...
  let next: ZuvixConfig = cfg;
  if (installable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        "No missing skill dependencies to install.",
        `To inspect available skills, run: ${formatCliCommand("zuvix skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("zuvix skills check")}`,
      ].join("\n"),
  ...
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      // visible through `zuvix skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("zuvix doctor")}\` to review skills + requirements.`,
      );
