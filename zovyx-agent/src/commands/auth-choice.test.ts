...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentDir: (configForTest: unknown, agentId: string) =>
    `${process.env.ZUVIX_STATE_DIR ?? "/tmp/zuvix-state"}/agents/${agentId}/agent`,
  resolveAgentWorkspaceDir: (configForTest: unknown, agentId: string) =>
    `/tmp/zuvix-workspaces/${agentId}`,
...
vi.mock("../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: () => "/tmp/zuvix-workspace",
}));
...
function resolveTestAuthStoreKey(agentDir?: string): string {
  return agentDir?.trim() || process.env.ZUVIX_AGENT_DIR || "__main__";
}
...
async function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<ZuvixConfig>;
    ...
    auth: [
      {
        ...
        run: async (ctx) => {
          ...
          return {
            ...
            ...(params.applyConfig ? { configPatch: params.applyConfig as ZuvixConfig } : {}),
...
async function createDefaultProviderPlugins(): Promise<ProviderPlugin[]> {
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as ZuvixConfig,
  ...
  return [
    ...
    await createApiKeyProvider({
      ...
      profileIds: ["zuvix:default", "zuvix-go:default"],
      defaultModel: "zuvix/claude-opus-4-6",
    ...
    await createApiKeyProvider({
      ...
      envVar: "OPENCODE_API_KEY",
      promptMessage: "Enter Zuvix API key",
      profileIds: ["zuvix-go:default", "zuvix:default"],
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    process.env.ZUVIX_AGENT_DIR = agentDir;
  }
  ...
  async function readAuthProfiles() {
    return readTestAuthProfileStore(resolveAgentDir({} as ZuvixConfig, "main"));
  }
  ...
  beforeAll(async () => {
    authTestRoot = (await setupAuthTestEnv("zuvix-auth-")).stateDir;
  ...
  it("fails fast when a removed provider auth choice is passed to the interactive flow", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "openai-chatgpt-import" is no longer supported. Use "openai" instead, or run zuvix onboard to choose interactively.',
      );
    } finally {
      spy.mockRestore();
    }
  ...
  it("keeps existing default model for explicit provider keys when setDefaultModel=false", async () => {
    const scenarios: Array<{
      authChoice: "synthetic-api-key" | "zuvix-zen";
      ...
      expectProviderConfigUndefined?: "zuvix";
      agentId?: string;
    }> = [
      {
      ...
      {
        authChoice: "zuvix-zen",
        token: "sk-zuvix-zen-test",
        promptMessage: "Enter Zuvix API key",
        existingPrimary: "anthropic/claude-opus-4-5",
        expectedOverride: "zuvix/claude-opus-4-6",
        profileId: "zuvix:default",
        profileProvider: "zuvix",
        extraProfileId: "zuvix-go:default",
        expectProviderConfigUndefined: "zuvix",
      },
    ];
    await setupTempState();
