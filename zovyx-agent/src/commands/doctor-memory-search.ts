...
import {
  ...
  normalizeProviderId,
} from "@zuvix/model-catalog-core/provider-id";
...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: ZuvixConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: ZuvixConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("zuvix doctor --fix")} or ${formatCliCommand("zuvix memory status --fix")}`
    : `Verify: ${formatCliCommand("zuvix memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: ZuvixConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: ZuvixConfig): boolean {
 ...
 * Runs as part of `zuvix doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("zuvix config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("zuvix config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("zuvix memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (provider === "local") {
    ...
    note(
      [
        ...
        "",
        "Fix (pick one):",
        `- Install the llama.cpp provider plugin: ${formatCliCommand("zuvix plugins install @zuvix/llama-cpp-provider")}`,
        `- Set a local GGUF model path in config`,
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`zuvix config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("zuvix config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("zuvix memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("zuvix config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("zuvix memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `zuvix doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("zuvix configure --section model")}`,
      `- To disable: ${formatCliCommand("zuvix config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("zuvix memory status --deep")}`,
    ].join("\n"),
    "Memory search",
