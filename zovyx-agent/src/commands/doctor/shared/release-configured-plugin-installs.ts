...
import { normalizeNullableString as normalizeId } from "@zuvix/normalization-core/string-coerce";
...
import { compareZuvixVersions } from "../../../config/version.js";
import {
...
function isPluginsGloballyDisabled(cfg: ZuvixConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: ZuvixConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: ZuvixConfig): string[] {
...
function isPluginEntryDisabled(cfg: ZuvixConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: ZuvixConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: ZuvixConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: ZuvixConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: ZuvixConfig): string[] {
...
function collectConfiguredChannelIds(cfg: ZuvixConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectConfiguredProviderIds(cfg: ZuvixConfig): Set<string> {
  const ids = new Set<string>();
...
function collectProviderPluginIds(cfg: ZuvixConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: ZuvixConfig): string[] {
...
function collectAcpRuntimePluginIds(cfg: ZuvixConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: ZuvixConfig): string[] {
...
function addEligiblePluginId(cfg: ZuvixConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareZuvixVersions(
  ...
  const touchedComparedToRelease = compareZuvixVersions(params.touchedVersion, releaseVersion);
