...
const ZUVIX_BETA_COMPANION_VERSION_RE = /^(\d{4}\.[1-9]\d?\.[1-9]\d?)-beta\.[1-9]\d*$/;
const ZUVIX_STABLE_OR_BETA_COMPANION_VERSION_RE =
...
function addConfiguredAgentRuntimePluginIds(ids: Set<string>, cfg: ZuvixConfig): void {
...
function collectConfiguredPluginIds(cfg: ZuvixConfig): Set<string> {
...
function collectBlockedPluginIds(cfg: ZuvixConfig): Set<string> {
  const ids = new Set<string>();
...
function collectConfiguredChannelIds(cfg: ZuvixConfig, env?: NodeJS.ProcessEnv): Set<string> {
  const ids = new Set<string>();
...
function installedRuntimePackageVersionIsStale(params: {
  ...
  const comparison = compareZuvixReleaseVersions(params.installedVersion, params.currentVersion);
...
function betaCompanionMatchesCurrentStableVersion(params: {
  ...
  const installedBase = ZUVIX_BETA_COMPANION_VERSION_RE.exec(params.installedVersion)?.[1];
  const currentBase = ZUVIX_STABLE_OR_BETA_COMPANION_VERSION_RE.exec(params.currentVersion)?.[1];
...
/** Repair missing installs inferred from the current Zuvix config. */
...
async function repairMissingPluginInstalls(params: {
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      const detail = `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "zuvix doctor --fix" after the update completes.`;
      changes.push(detail);
