...
function isRuntimeDependencyMarkerName(name: string): boolean {
  return (
    name === ".zuvix-runtime-deps.json" ||
    name === ".zuvix-runtime-deps-stamp.json" ||
    name.startsWith(".zuvix-runtime-deps-")
  );
}

function isInstallStageDebrisName(name: string): boolean {
  return /^\.zuvix-install-stage(?:-.+)?$/u.test(name);
}

function isLegacyDependencyDebrisName(name: string): boolean {
  return (
    isRuntimeDependencyMarkerName(name) ||
    name === ".zuvix-pnpm-store" ||
    name === ".zuvix-install-backups" ||
...
function collectExplicitStageTargets(env: NodeJS.ProcessEnv): CleanupTarget[] {
  return splitPathList(env.ZUVIX_PLUGIN_STAGE_DIR).map((entry) => ({
    kind: "explicit-stage",
...
async function hasZuvixRenameResidue(root: string): Promise<boolean> {
  const nodeModulesRoot = path.join(root, "node_modules");
  if (await isFile(path.join(nodeModulesRoot, ".zuvix-rename-tmp"))) {
    return true;
  }
  ...
  for (const entry of entries) {
    ...
    if (await isFile(path.join(entryPath, ".zuvix-rename-tmp"))) {
      return true;
    }
    ...
    for (const scopedEntry of scopedEntries) {
      ...
      if (await isFile(path.join(entryPath, scopedEntry.name, ".zuvix-rename-tmp"))) {
        return true;
      }
    }
  }
...
async function hasExplicitStageDebrisProof(root: string): Promise<boolean> {
  ...
  return await hasZuvixRenameResidue(root);
}
...
function filterLegacyStaleRootCandidates(
  ...
  for (const target of targets) {
    ...
    if (!cleanupRootPaths.some((rootPath) => isPathInsideRoot(targetPath, rootPath))) {
      warnings.push(
        `Skipped legacy plugin dependency state ${targetPath}: outside Zuvix cleanup roots`,
      );
      continue;
    }
    safeTargets.push({ ...target, path: targetPath });
...
async function resolveSafeRemovalTarget(
  ...
  if (!cleanupRoots.some((root) => isPathInsideRoot(realPath, root.realPath))) {
    return {
      warning: `Skipped legacy plugin dependency state ${targetPath}: resolved outside Zuvix cleanup roots`,
    };
  }
  return { target: targetPath };
}
...
/** Remove legacy plugin dependency state under trusted Zuvix cleanup roots. */
