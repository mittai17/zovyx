...
describe("cleanupLegacyPluginDependencyState", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-plugin-deps-cleanup-"));
  });
  ...
  it("collects and removes legacy plugin dependency state roots", async () => {
    ...
    const explicitStageDir = path.join(stateDir, ".zuvix-install-stage-explicit");
    ...
    const env = {
      ...
      ZUVIX_PLUGIN_STAGE_DIR: explicitStageDir,
  ...
  it("removes configured plugin stage roots outside Zuvix roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    ...
    await fs.writeFile(
      path.join(stageRoot, "node_modules", "ansi-escapes", ".zuvix-rename-tmp"),
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        ZUVIX_STATE_DIR: stateDir,
        ZUVIX_PLUGIN_STAGE_DIR: stageRoot,
      },
      packageRoot,
    });
  ...
  it("refuses arbitrary explicit plugin stage roots outside Zuvix roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
  ...
  it("refuses explicit plugin stage paths with parent segments", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const dotDotStage = `${stateDir}${path.sep}..${path.sep}.zuvix-install-stage-dotdot`;
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        ZUVIX_STATE_DIR: stateDir,
        ZUVIX_PLUGIN_STAGE_DIR: dotDotStage,
      },
      packageRoot,
    });
  ...
  it("does not follow symlinked extension roots outside Zuvix roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    ...
    await fs.writeFile(path.join(externalPlugin, ".zuvix-runtime-deps.json"), "{}");
  ...
  it("refuses legacy roots that resolve outside Zuvix roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const legacyRuntimeRoot = path.join(stateDir, "plugin-runtime-deps");
    ...
    expect(result.changes).toStrictEqual([]);
    expect(result.warnings).toContain(
      `Skipped legacy plugin dependency state ${legacyRuntimeRoot}: resolved outside Zuvix cleanup roots`,
    );
  ...
  it("does not unlink global runtime symlinks through unsafe cleanup roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "prefix", "lib", "node_modules", "zuvix");
