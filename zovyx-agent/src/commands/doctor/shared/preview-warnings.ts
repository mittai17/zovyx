...
import { isRecord as hasRecord } from "@zuvix/normalization-core/record-coerce";
...
function listAgentRecords(cfg: ZuvixConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: ZuvixConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: ZuvixConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: ZuvixConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: ZuvixConfig): boolean {
...
function hasConfiguredSafeBins(cfg: ZuvixConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: ZuvixConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: ZuvixConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: ZuvixConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Zuvix falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Zuvix falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: ZuvixConfig): string[] {
...
export function collectProfileConfiguredToolSectionWarnings(cfg: ZuvixConfig): string[] {
  const warnings: string[] = [];
...
async function resolveDoctorChannelPreviewConfig(params: {
  ...
  allowExec?: boolean;
}): Promise<{ cfg: ZuvixConfig; diagnostics: string[] }> {
