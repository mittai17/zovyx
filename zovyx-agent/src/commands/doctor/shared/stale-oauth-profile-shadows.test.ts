...
describe("stale OAuth profile shadow doctor repair", () => {
  const envSnapshot = captureEnv(["ZUVIX_STATE_DIR", "ZUVIX_HOME"]);
  let tempRoot = "";
  let stateDir = "";

  beforeEach(async () => {
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-stale-oauth-shadow-"));
    stateDir = path.join(tempRoot, "state");
    process.env.ZUVIX_STATE_DIR = stateDir;
    process.env.ZUVIX_HOME = stateDir;
  });
  ...
  it("warns about stale local OAuth shadows without modifying the child store", async () => {
    ...
    expect(warnings[0]).toContain("zuvix doctor --fix");
  ...
  it("uses the injected env for the main auth store", async () => {
    ...
    const injectedEnv = {
      ...process.env,
      ZUVIX_STATE_DIR: injectedStateDir,
      ZUVIX_HOME: injectedStateDir,
    };
    saveAuthProfileStore(
      storeWith(
        profileId,
  ...
  it("removes stale child OAuth shadows and local cooldown state", async () => {
    ...
    const result = await repairStaleOAuthProfileShadows({
      cfg: { agents: { list: [{ id: "telegram" }] } } satisfies ZuvixConfig,
      now,
    });
