...
const LEGACY_OAUTH_SECRET_KEY_ENV = "ZUVIX_AUTH_PROFILE_SECRET_KEY";
const LEGACY_OAUTH_SECRET_KEYCHAIN_SERVICE = "Zuvix Auth Profile Secrets";
...
function buildLegacyOAuthSecretKey(seed: string): Buffer {
  ...
  return hash("sha256", `zuvix:auth-profile-oauth:${seed}`, "buffer");
}
...
function resolveLegacyOAuthSecretKeyFileCandidates(env: NodeJS.ProcessEnv): string[] {
  if (process.platform === "win32") {
    ...
    return uniquePaths([
      root ? path.join(root, "Zuvix", LEGACY_OAUTH_SECRET_KEY_FILE_NAME) : undefined,
      home
        ? path.join(home, ".zuvix-auth-profile-secrets", LEGACY_OAUTH_SECRET_KEY_FILE_NAME)
        : undefined,
    ]);
  }
...
function resolveLegacyOAuthSecretKeySeeds(env: NodeJS.ProcessEnv): string[] {
  ...
  if (env.NODE_ENV === "test" && env.VITEST === "true") {
    addSeed("zuvix-test-oauth-profile-secret-key");
  }
...
function emitKeychainOnlyMigrationHintOnce(profileId: string): void {
  ...
  log.warn(
    "Legacy Codex OAuth credentials are stored only in macOS Keychain on this host. " +
      "Headless paths cannot prompt for Keychain access; run `zuvix doctor --fix` " +
