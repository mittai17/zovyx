...
async function writeLegacySidecarThatNeedsKeychain(): Promise<{
  ...
  const state = await createZuvixTestState({
    layout: "state-only",
    prefix: "zuvix-legacy-oauth-keychain-warn-",
    env: {
      ZUVIX_AGENT_DIR: undefined,
      ZUVIX_AUTH_PROFILE_SECRET_KEY: undefined,
    },
  });
  states.push(state);
...
describe("loadLegacyOAuthSidecarMaterial keychain-only headless warning", () => {
  ...
  function envWithoutVitestSignals(state: ZuvixTestState): NodeJS.ProcessEnv {
  ...
  it("emits a single doctor-pointer warning when only Keychain can decrypt and prompts are disabled", async () => {
    ...
    expect(String(firstMessage)).toContain("zuvix doctor --fix");
