...
function resolveBundledWorkspaceDir(cfg: ZuvixConfig): string | undefined {
  return resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) ?? undefined;
}

function isZuvixNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "zuvix" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by Zuvix. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed Zuvix package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isZuvixNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; Zuvix loads the packaged bundled plugin from "${hit.toPath}".`,
  );
