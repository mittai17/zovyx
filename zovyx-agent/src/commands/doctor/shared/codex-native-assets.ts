...
function isCodexRuntimeConfigured(cfg: ZuvixConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: ZuvixConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: ZuvixConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      "- Personal Codex CLI assets were found, but native Codex-mode Zuvix agents use isolated per-agent Codex homes.",
      ...
      "- If the Codex plugin is not installed, run `zuvix plugins install npm:@zuvix/codex` first. Then run `zuvix migrate plan codex` to inventory them. Applying that migration copies skills into the current Zuvix agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
