...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<ZuvixConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as ZuvixConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(cfg: ZuvixConfig, changes: string[]): ZuvixConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as ZuvixConfig["talk"]) ?? {};
