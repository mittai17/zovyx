...
const GATEWAY_BIND_RULE: LegacyConfigRule = {
  path: ["gateway", "bind"],
  message:
    'gateway.bind host aliases (for example 0.0.0.0/localhost) are legacy; use bind modes (lan/loopback/custom/tailnet/auto) instead. Run "zuvix doctor --fix".',
...
const GATEWAY_WEBCHAT_RULE: LegacyConfigRule = {
  ...
  message: 'gateway.webchat is retired. Run "zuvix doctor --fix".',
};
