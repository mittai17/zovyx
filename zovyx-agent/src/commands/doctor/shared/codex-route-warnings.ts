...
import { AGENT_MODEL_CONFIG_KEYS } from "@zuvix/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@zuvix/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@zuvix/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString as normalizeString } from "@zuvix/normalization-core/string-coerce";
...
function resolveImplicitDefaultAgentModelRef(cfg: ZuvixConfig): string {
...
function collectConfigModelRefs(cfg: ZuvixConfig): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: ZuvixConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: ZuvixConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: ZuvixConfig): boolean {
...
function collectDisabledCodexPluginRouteHits(cfg: ZuvixConfig): DisabledCodexPluginRouteHit[] {
...
}): { cfg: ZuvixConfig; changes: string[] } {
...
function clearConfigLegacyAgentRuntimePolicies(cfg: ZuvixConfig): string[] {
  const changes: string[] = [];
...
function configRepairWouldClearLegacyRuntimePins(params: { cfg: ZuvixConfig }): boolean {
...
function rewriteConfigModelRefs(params: { cfg: ZuvixConfig }): ConfigRouteRepairResult {
...
function formatUnsupportedCompactionWarning(params: {
  ...
  return [
    "- Codex runtime uses native server-side compaction and ignores Zuvix compaction summarizer overrides.",
    ...params.hits.map(
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `zuvix doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ? "- Enable plugin loading and remove `codex` from plugins.deny, or set the affected OpenAI models to an Zuvix runtime policy."
    : "- Run `zuvix doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an Zuvix runtime policy.";
  return [
...
function collectCodexAppServerCommandWarnings(cfg: ZuvixConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `zuvix doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
  ...
  if (fixableHits.length > 0) {
    warnings.push(
      formatUnsupportedCompactionWarning({
        hits: fixableHits,
        fixHint:
          "- Run `zuvix doctor --fix`: it removes unsupported Codex compaction overrides.",
      }),
    );
  }
  return warnings;
...
}): { cfg: ZuvixConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "zuvix") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "zuvix") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `zuvix doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
