...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("zuvix cron list")} and ${formatCliCommand("zuvix cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.zuvix/bin/ensure-whatsapp.sh` is not maintained by current Zuvix and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("zuvix channels status --probe")}, ${formatCliCommand("zuvix doctor")}, and ${formatCliCommand("zuvix gateway status")} for current health checks.`,
