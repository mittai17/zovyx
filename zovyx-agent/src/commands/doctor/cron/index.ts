...
export async function maybeRepairLegacyCronStore(params: {
  ...
  try {
    ...
    note(
      [
        ...
        `- ${reason}`,
        `Fix the file's permissions or contents and re-run ${formatCliCommand("zuvix doctor")}; later health checks will continue.`,
      ].join("\n"),
      "Cron",
    );
    return;
  ...
  if (rawJobs.length === 0) {
    ...
    note(
      [
        ...
        ...previewLines,
        `Repair with ${formatCliCommand("zuvix doctor --fix")} to finish the migration.`,
      ].join("\n"),
      "Cron",
    );
  ...
  note(
    [
      noteHeading,
      ...previewLines,
      `Repair with ${formatCliCommand("zuvix doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
