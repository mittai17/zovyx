...
describe("normalizeStoredCronJobs", () => {
  ...
  it("converts legacy agent command prompts into command cron payloads", () => {
    const command =
      "cd /home/zuvix/.razor/quant && ./scripts/system/run_position_control.sh --write-card --silent-token NO_REPLY";
    const { job, result } = normalizeOneJob(
      makeLegacyJob({
        ...
        payload: {
          ...
          message: [
            ...
            "Command to run:",
            `- command: ${command}`,
            "- workdir: /home/zuvix/.razor/quant",
            "- background: false",
            "- timeout: 840",
            "",
      ...
      makeLegacyJob({
        ...
        payload: {
          ...
          message: [
            "Command to run:",
            `- command: ${command}`,
            "- workdir: /home/zuvix/.razor/clawd",
          ].join("\n"),
