...
describe("buildAuthChoiceOptions", () => {
  ...
  it("groups Zuvix Zen and Go under one Zuvix entry", () => {
    resolveManifestProviderAuthChoices.mockReturnValue([
      {
        pluginId: "zuvix",
        providerId: "zuvix",
    ...
    const openCodeGroup = requireChoiceGroup(groups, "zuvix");

    const openCodeValues = openCodeGroup.options.map((option) => option.value);
    expect(openCodeValues).toContain("zuvix-zen");
    expect(openCodeValues).toContain("zuvix-go");
  });
