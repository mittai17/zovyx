...
import {
  ZUVIX_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveZuvixWrapperPath,
...
// they share the ZUVIX_ prefix that is otherwise stripped from preserved
...
const PRESERVED_ZUVIX_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "ZUVIX_CLI_CONTAINER_BYPASS",
  "ZUVIX_CONTAINER_HINT",
]);
...
export function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("ZUVIX_") && !PRESERVED_ZUVIX_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[ZUVIX_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${ZUVIX_WRAPPER_ENV_KEY} because it points to the Windows task script; using the Zuvix gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveZuvixWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [ZUVIX_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, ZUVIX_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.ZUVIX_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("zuvix gateway install")}\` after fixing the error.`;
}
