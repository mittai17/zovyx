// Builds overview table rows for `zuvix status` and `zuvix status --all`.
...
/** Builds the default `zuvix status` overview rows from scan, health, memory, and session inputs. */
  ...
  return buildStatusOverviewRowsFromSurface({
    ...
    middleRows: [
      ...(params.updateRestartValue
        ? [{ Item: "Update restart", Value: params.updateRestartValue }]
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("zuvix security audit --deep")}` },
    ],
