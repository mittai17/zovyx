// Appends the read-only diagnosis section for `zuvix status --all`.
...
export async function appendStatusAllDiagnosis(params: {
  ...
  if (params.portUsage) {
    ...
    if (!portOk) {
      ...
      if (gatewayPidCount > 1) {
        lines.push(
          `  ${muted(`${gatewayPidCount} Zuvix gateway processes appear to be listening on port ${params.port}; stop stale gateway processes before trusting channel health.`)}`,
        );
      }
      ...
      lines.push(`  ${muted("Detected Zuvix Gateway listener on the configured port.")}`);
    }
  }
  ...
  lines.push("Troubleshooting: https://docs.zuvix.ai/troubleshooting");
  lines.push("");
}
