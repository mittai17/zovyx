...
const providerEnvVarsById = vi.hoisted(
  (): Record<string, readonly string[]> => ({
    ...
    zuvix: ["OPENCODE_API_KEY"],
    "zuvix-go": ["OPENCODE_API_KEY"],
    volcengine: ["VOLCANO_ENGINE_API_KEY"],
  }),
);

vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.ZUVIX_STATE_DIR ?? "/tmp/zuvix-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  const upsert = (params: { profileId: string; credential: unknown; agentDir?: string }) => {
    const stateDir = process.env.ZUVIX_STATE_DIR ?? "/tmp/zuvix-state";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under the default agent dir", async () => {
    const env = await setupAuthTestEnv("zuvix-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-oauth-sync-"));
    ...
    process.env.ZUVIX_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-oauth-nosync-"));
    process.env.ZUVIX_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ZUVIX_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-oauth-external-"));
    process.env.ZUVIX_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ZUVIX_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("zuvix-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("zuvix-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
    ...
    process.env.OPENCODE_API_KEY = "sk-zuvix-env"; // pragma: allowlist secret
    ...
    expectFields(await readProfile(env.agentDir, "zuvix:default"), {
      keyRef: { source: "env", provider: "default", id: "OPENCODE_API_KEY" },
    });
    expectFields(await readProfile(env.agentDir, "zuvix-go:default"), {
      keyRef: { source: "env", provider: "default", id: "OPENCODE_API_KEY" },
    });
  });
});
...
describe("upsertApiKeyProfile", () => {
  const lifecycle = createAuthTestLifecycle(["ZUVIX_STATE_DIR", "ZUVIX_AGENT_DIR"]);
  ...
  it("writes to the default agent dir", async () => {
    const env = await setupAuthTestEnv("zuvix-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
    const defaultAgentDir = path.join(env.stateDir, "agents", "main", "agent");
