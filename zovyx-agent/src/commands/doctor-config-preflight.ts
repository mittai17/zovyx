...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".zuvix");
  const targetPath = path.join(targetDir, "zuvix.json");
  try {
...
export function shouldSkipPluginValidationForDoctorConfigPreflight(
  ...
  return isTruthyEnvValue(env.ZUVIX_UPDATE_IN_PROGRESS);
}
...
export async function runDoctorConfigPreflight(
  ...
  if (options.repairPrefixedConfig === true && snapshot.exists && !snapshot.valid) {
    if (await recoverConfigFromJsonRootSuffix(snapshot)) {
      note("Removed non-JSON prefix from zuvix.json; original saved as .clobbered.*.", "Config");
      snapshot = addDoctorLegacyIssues(await readConfigFileSnapshot(readOptions));
    } else if (
      await recoverConfigFromLastKnownGood({ snapshot, reason: "doctor-invalid-config" })
    ) {
      note(
        "Restored zuvix.json from last-known-good; original saved as .clobbered.*.",
        "Config",
      );
      snapshot = addDoctorLegacyIssues(await readConfigFileSnapshot(readOptions));
    }
