// Builds the data model for the standard `zuvix status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("zuvix security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("zuvix status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("zuvix tasks maintenance --apply")}`,
