// Implements guided and non-interactive `zuvix channels add` account setup.
...
async function resolveCatalogChannelEntry(raw: string, cfg: ZuvixConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as ZuvixConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run zuvix doctor --fix, then retry zuvix channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("zuvix channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
