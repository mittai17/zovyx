...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/zuvix-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = ZuvixConfig>(
  overrides: T = {} as T,
): ZuvixConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): ZuvixConfig {
