...
type EnvSnapshot = {
  HOME?: string;
  ZUVIX_HOME?: string;
  ZUVIX_STATE_DIR?: string;
  ZUVIX_OAUTH_DIR?: string;
  ZUVIX_AGENT_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ZUVIX_HOME: process.env.ZUVIX_HOME,
    ZUVIX_STATE_DIR: process.env.ZUVIX_STATE_DIR,
    ZUVIX_OAUTH_DIR: process.env.ZUVIX_OAUTH_DIR,
    ZUVIX_AGENT_DIR: process.env.ZUVIX_AGENT_DIR,
  };
}
...
function setupSessionState(cfg: ZuvixConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("ZUVIX_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: ZuvixConfig) {
...
async function runStateIntegrityText(cfg: ZuvixConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.ZUVIX_HOME = tempHome;
    process.env.ZUVIX_STATE_DIR = path.join(tempHome, ".zuvix");
    delete process.env.ZUVIX_OAUTH_DIR;
    delete process.env.ZUVIX_AGENT_DIR;
    fs.mkdirSync(process.env.ZUVIX_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when ZUVIX_OAUTH_DIR is explicitly configured", async () => {
    process.env.ZUVIX_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: ZuvixConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when ZUVIX_AGENT_DIR points at the live compatibility agent dir", async () => {
    ...
    process.env.ZUVIX_AGENT_DIR = legacyAgentDir;
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("zuvix tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.ZUVIX_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.ZUVIX_HOME = symlinkHome;
        process.env.ZUVIX_STATE_DIR = path.join(symlinkHome, ".zuvix");
  ...
  it("prints zuvix-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: ZuvixConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/zuvix sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/zuvix sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /zuvix sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-tui-pointer-clear-"));
    try {
