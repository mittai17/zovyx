...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    await withEnvAsync({ ZUVIX_STATE_DIR: missingDir }, async () => {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      });
  ...
  it("warns about zuvix provider overrides", async () => {
    mockDoctorConfigSnapshot({
      config: {
        models: {
          providers: {
    ...
    const warned = terminalNoteMock.mock.calls.some(
      ([message, title]) =>
        title === "Zuvix" &&
        String(message).includes("models.providers.zuvix") &&
        String(message).includes("models.providers.zuvix-go"),
    );
  ...
  it("skips gateway auth warning when ZUVIX_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    const prevToken = process.env.ZUVIX_GATEWAY_TOKEN;
    process.env.ZUVIX_GATEWAY_TOKEN = "env-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      ...
      if (prevToken === undefined) {
        ...
        process.env.ZUVIX_GATEWAY_TOKEN = prevToken;
      }
    }
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote[0])).toContain("zuvix config set gateway.auth.mode token");
    expect(String(gatewayAuthNote[0])).toContain("zuvix config set gateway.auth.mode password");
  });
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.ZUVIX_GATEWAY_TOKEN;
    delete process.env.ZUVIX_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousToken === undefined) {
        ...
        process.env.ZUVIX_GATEWAY_TOKEN = previousToken;
      }
    }
  ...
  it("does not let ZUVIX_GATEWAY_TOKEN hide an unresolved SecretRef-managed token", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: {
          mode: "local",
          auth: {
            ...
            token: {
              source: "env",
              provider: "default",
              id: "ZUVIX_MISSING_GATEWAY_REF_TOKEN",
            },
          },
        },
        secrets: {
    ...
    const previousFallbackToken = process.env.ZUVIX_GATEWAY_TOKEN;
    const previousRefToken = process.env.ZUVIX_MISSING_GATEWAY_REF_TOKEN;
    process.env.ZUVIX_GATEWAY_TOKEN = "fallback-token-1234567890";
    delete process.env.ZUVIX_MISSING_GATEWAY_REF_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      ...
      if (previousFallbackToken === undefined) {
        ...
        process.env.ZUVIX_GATEWAY_TOKEN = previousFallbackToken;
      }
      if (previousRefToken === undefined) {
        delete process.env.ZUVIX_MISSING_GATEWAY_REF_TOKEN;
      } else {
        process.env.ZUVIX_MISSING_GATEWAY_REF_TOKEN = previousRefToken;
      }
    }
  ...
  it("skips token-mode exec token probes even when env password is set", async () => {
    ...
    const previousPassword = process.env.ZUVIX_GATEWAY_PASSWORD;
    process.env.ZUVIX_GATEWAY_PASSWORD = "fallback-password";
    try {
      callGateway.mockClear();
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousPassword === undefined) {
        ...
        process.env.ZUVIX_GATEWAY_PASSWORD = previousPassword;
      }
    }
  ...
  it("skips password-mode exec password probes even when env token is set", async () => {
    ...
    const previousToken = process.env.ZUVIX_GATEWAY_TOKEN;
    process.env.ZUVIX_GATEWAY_TOKEN = "fallback-token";
    try {
      callGateway.mockClear();
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves", async () => {
    ...
    const previousToken = process.env.ZUVIX_GATEWAY_TOKEN;
    process.env.ZUVIX_GATEWAY_TOKEN = "resolved-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
