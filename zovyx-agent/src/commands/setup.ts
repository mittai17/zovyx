...
async function writeDefaultConfigFile(config: ZuvixConfig): Promise<void> {
...
async function readConfigFileRaw(configPath: string): Promise<{
  ...
  try {
    ...
    return { exists: true, parsed: (parsed ?? {}) as ZuvixConfig };
  } catch {
...
export async function setupCommand(
  ...
  runtime.log(`Next guided path: ${formatCliCommand("zuvix onboard")}.`);
  runtime.log(
    `Next targeted changes: ${formatCliCommand("zuvix configure")} for models, channels, Gateway, plugins, skills, and health checks.`,
  );
  runtime.log(`Add a chat channel later: ${formatCliCommand("zuvix channels add")}.`);
}
