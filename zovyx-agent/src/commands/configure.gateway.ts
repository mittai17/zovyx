...
import { validateIPv4AddressInput } from "@zuvix/net-policy/ipv4";
import {
...
export async function promptGatewayConfig(
  ...
  if (authMode === "token") {
    ...
    if (tokenInputMode === "ref") {
      ...
      note(`Validated ${envVarName}. Zuvix will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Zuvix trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.zuvix.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
