/**
 * Top-level `zuvix onboard` command entrypoint.
 *
...
export async function setupWizardCommand(
  ...
  if (
    ...
    runtime.error(
      `Invalid --secret-input-mode. Use "plaintext" or "ref", or run ${formatCliCommand("zuvix onboard")} for the interactive setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.resetScope && !VALID_RESET_SCOPES.has(normalizedOpts.resetScope)) {
    runtime.error(
      `Invalid --reset-scope. Use "config", "config+creds+sessions", or "full". Run ${formatCliCommand("zuvix onboard --reset --reset-scope config")} for a config-only reset.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    ...
    runtime.error(
      [
        ...
        "Read: https://docs.zuvix.ai/security",
        `Re-run with: ${formatCliCommand("zuvix onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected - Zuvix runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.zuvix.ai/windows",
      ].join("\n"),
    );
  }
