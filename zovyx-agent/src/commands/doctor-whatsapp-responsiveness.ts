...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "zuvix-tui") {
    return true;
  }
  return executable === "zuvix" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local Zuvix TUI processes that can contend with gateway responsiveness. */
...
function hasWhatsappEnabled(cfg: ZuvixConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("zuvix doctor --fix")}.`,
      );
    }
  }
