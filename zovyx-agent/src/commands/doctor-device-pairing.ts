...
import { normalizeUniqueSingleOrTrimmedStringList } from "@zuvix/normalization-core/string-normalization";
...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["zuvix", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["zuvix", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["zuvix", "devices", "remove", pending.deviceId]),
    };
  }
...
function formatPairingStoreReadIssue(error: JsonFileReadError): string {
  ...
  return `- Device pairing store ${error.filePath} ${problem}. Zuvix refused to treat it as empty to avoid overwriting approved pairings. Fix the JSON or file permissions, or move it aside and re-pair devices.`;
}
