...
export type ZuvixSkillSecurityVerdictItem = Omit<
...
export function projectClawHubVerdictItem(
  ...
  registry: string,
): ZuvixSkillSecurityVerdictItem {
  const projected: ZuvixSkillSecurityVerdictItem = {
    registry,
    ok: item.ok,
  ...
  if (item.error) {
    const error: ZuvixSkillSecurityVerdictItem["error"] = {};
...
export async function fetchZuvixSkillSecurityVerdicts(
  targets: Array<{ registry: string; slug: string; version: string }>,
): Promise<ZuvixSkillSecurityVerdictItem[]> {
  ...
  const items: ZuvixSkillSecurityVerdictItem[] = [];
