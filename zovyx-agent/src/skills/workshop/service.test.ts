...
beforeEach(async () => {
  testState = await createZuvixTestState({
    layout: "state-only",
    prefix: "zuvix-skill-workshop-state-",
  });
...
async function makeWorkspace(): Promise<string> {
  return await tempDirs.make("zuvix-skill-workshop-");
}
...
describe("skill workshop proposals", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("zuvix-skill-workshop-target-skills-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("zuvix-skill-workshop-readonly-skills-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("zuvix-skill-workshop-support-trusted-");
      const untrustedSkillsDir = await tempDirs.make("zuvix-skill-workshop-support-untrusted-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("zuvix-skill-workshop-untrusted-skills-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
      const proposal = await proposeCreateSkill({
        workspaceDir,
