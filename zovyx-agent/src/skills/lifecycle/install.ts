...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "zuvix");
  }
  return path.join(homedir(), ".zuvix");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "zuvix-bundled"
        ? { kind: "bundled", authority: "zuvix", mutable: false, network: false }
        : skillSource === "zuvix-managed" || skillSource === "zuvix-extra"
          ? { kind: "managed", authority: "zuvix", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["zuvix-bundled", "zuvix-managed", "zuvix-extra"]);
