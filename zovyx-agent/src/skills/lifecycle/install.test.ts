...
import { resolveZuvixMetadata, resolveSkillInvocationPolicy } from "../loading/frontmatter.js";
...
metadata: {"zuvix":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveZuvixMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("zuvix-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["ZUVIX_STATE_DIR"]);
  try {
...
describe("installSkill before_install hooks", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("ZUVIX_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Zuvix-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["ZUVIX_STATE_DIR", "ZUVIX_CONFIG_PATH"]);
    try {
      process.env.ZUVIX_STATE_DIR = "/tmp/untrusted-state";
      process.env.ZUVIX_CONFIG_PATH = "/tmp/untrusted-config/zuvix.json";
  ...
  it("surfaces plugin hook findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("zuvix-workspace");
