...
import { resolveZuvixMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".zuvix", "source-origin.json");
...
function isContainerStateHomeWherePromptTildeEscapes(home: string): boolean {
  ...
  return home === "/data" && (configDir === "/data/.zuvix" || isPathInside("/data/.zuvix", configDir));
}
...
function resolveSkillsLimits(config?: ZuvixConfig, agentId?: string): ResolvedSkillsLimits {
...
export function resolveSkillRootScanLimit(config?: ZuvixConfig): number {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "zuvix-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "zuvix-bundled") {
    return {
...
function resolveSkillEntryMetadata(params: {
  frontmatter: ParsedSkillFrontmatter;
  skillDir: string;
}): ZuvixSkillMetadata | undefined {
  const metadata = resolveZuvixMetadata(params.frontmatter);
...
function shouldEnforceConfiguredSkillRootContainment(source: string): boolean {
  return source !== "zuvix-managed" && source !== "agents-skills-personal";
}
...
function loadGeneratedPluginSkillRecords(params: {
  ...
  for (const name of childDirs) {
    ...
    // Plugin skills live as symlinks under ~/.zuvix/plugin-skills/, so
...
function buildSkillsLimitNote(params: {
  ...
  if (params.truncated) {
    return `⚠️ Skills truncated: included ${params.included} of ${params.total}${params.compact ? " (compact format, descriptions omitted)" : ""}. Run \`zuvix skills check\` to audit.`;
  }
  if (params.compact) {
    return `⚠️ Skills catalog using compact format (descriptions omitted). Run \`zuvix skills check\` to audit.`;
  }
  return "";
}
