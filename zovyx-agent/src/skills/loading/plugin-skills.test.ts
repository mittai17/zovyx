    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "zuvix.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "zuvix.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "zuvix.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("zuvix-");
  const acpxRoot = await tempDirs.make("zuvix-acpx-plugin-");
  const helperRoot = await tempDirs.make("zuvix-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("zuvix-");
  const pluginRoot = await tempDirs.make("zuvix-plugin-");
  const outsideDir = await tempDirs.make("zuvix-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("zuvix-");
    const pluginRoot = await tempDirs.make("zuvix-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("zuvix-");
    const pluginRoot = await tempDirs.make("zuvix-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
