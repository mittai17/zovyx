...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ZUVIX_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns ZUVIX_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-bundled-override-"));
    process.env.ZUVIX_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.ZUVIX_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "zuvix" }));
