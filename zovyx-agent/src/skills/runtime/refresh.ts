...
function resolveWatchTargets(workspaceDir: string, config?: ZuvixConfig): WatchTarget[] {
  const baseRoots: Array<{ path: string; source: string }> = [];
  if (workspaceDir.trim()) {
    baseRoots.push({ path: path.join(workspaceDir, "skills"), source: "zuvix-workspace" });
    baseRoots.push({
      path: path.join(workspaceDir, ".agents", "skills"),
      source: "agents-skills-project",
    });
  }
  baseRoots.push({ path: path.join(CONFIG_DIR, "skills"), source: "zuvix-managed" });
  baseRoots.push({
...
function isTrustedSymlinkSkillTarget(
  ...
  if (source === "zuvix-managed" || source === "agents-skills-personal") {
    return true;
  }
  return (
...
function resolveWatchDebounceMs(config?: ZuvixConfig): number {
...
export function ensureSkillsWatcher(params: { workspaceDir: string; config?: ZuvixConfig }) {
