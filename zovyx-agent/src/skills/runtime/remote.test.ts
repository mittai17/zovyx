...
function createRemoteSkillWorkspace(bin: string): { cfg: ZuvixConfig; workspaceDir: string } {
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-remote-skills-"));
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `metadata: { "zuvix": { "os": ["darwin"], "requires": { "bins": ["${bin}"] } } }`,
      "---",
      "# Remote Skill",
      "",
    ].join("\n"),
