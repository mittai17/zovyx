...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("zuvix-bundled-");
  const workspaceDir = await createTempDir("zuvix-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: ZuvixConfig) {
