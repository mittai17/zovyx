...
describe("resolveEmbeddedRunSkillEntries", () => {
  ...
  it("can constrain live loading to materialized workspace skills", () => {
    ...
    resolveEmbeddedRunSkillEntries({
      workspaceDir: "/tmp/workspace/.zuvix/sandbox-skills",
      config: {},
      eligibility,
      skillsSnapshot: {
        prompt: "skills prompt",
    ...
    expect(loadWorkspaceSkillEntriesSpy).toHaveBeenCalledWith(
      "/tmp/workspace/.zuvix/sandbox-skills",
      {
        config: {},
        eligibility,
        workspaceOnly: true,
  ...
  it("prefers caller config when the active runtime snapshot still contains raw skill SecretRefs", () => {
    ...
    const runtimeConfig: ZuvixConfig = structuredClone(sourceConfig);
    const callerConfig: ZuvixConfig = {
      skills: {
        entries: {
          diffs: {
            apiKey: "resolved-key",
