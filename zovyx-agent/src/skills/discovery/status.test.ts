...
describe("buildWorkspaceSkillStatus", () => {
  it("surfaces valid ClawHub linkage and local Skill Card metadata", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-skill-status-"));
    try {
  ...
  it("classifies a mixed broken skill pack without flattening visibility reasons", () => {
    const missingBin = "zuvix-test-definitely-missing-skill-bin";
    const report = buildWorkspaceSkillStatus("/tmp/ws", {
      agentId: "specialist",
      config: {
        agents: {
      ...
      entries: [
        ...
        createEntry("needs-bin", {
          metadata: {
            ...
            install: [
              {
                kind: "node",
                package: "@zuvix/missing-skill-bin",
                bins: [missingBin],
              },
            ],
          },
        ...
        createEntry("needs-env", {
          metadata: {
            primaryEnv: "ZUVIX_TEST_MISSING_SKILL_KEY",
            requires: { env: ["ZUVIX_TEST_MISSING_SKILL_KEY"] },
          },
        }),
        ...
        createEntry("bundled-blocked", { source: "zuvix-bundled" }),
      ],
    });
    ...
    expect(needsBin.install).toStrictEqual([
      {
        kind: "node",
        id: "node-0",
        label: "Install @zuvix/missing-skill-bin (pnpm)",
        bins: [missingBin],
      },
    ]);
    const needsEnv = requireSkillStatus(byName, "needs-env");
    expect(needsEnv.eligible).toBe(false);
    expect(needsEnv.primaryEnv).toBe("ZUVIX_TEST_MISSING_SKILL_KEY");
    expect(needsEnv.missing).toStrictEqual({
      anyBins: [],
      bins: [],
      config: [],
      env: ["ZUVIX_TEST_MISSING_SKILL_KEY"],
      os: [],
    });
