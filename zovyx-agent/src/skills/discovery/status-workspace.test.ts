...
function makeEntry(params: {
  ...
  return {
    skill: createCanonicalFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "zuvix-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-bundled-"));
    ...
    await withEnvAsync({ ZUVIX_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill.source).toBe("zuvix-workspace");
