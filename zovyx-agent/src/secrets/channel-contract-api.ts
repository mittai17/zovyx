...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `zuvix.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built zuvix
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.ZUVIX_DEBUG_CHANNEL_CONTRACT_API === "1") {
