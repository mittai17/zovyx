...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: ZuvixConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "zuvix.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.ZUVIX_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_ZUVIX_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.ZUVIX_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.ZUVIX_BUNDLED_PLUGINS_DIR ??= "extensions";
process.env.ZUVIX_TEST_TRUST_BUNDLED_PLUGINS_DIR ??= "1";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.ZUVIX_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function collectZuvixCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "zuvix.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_ZUVIX_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForZuvixTarget(
  config: ZuvixConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
async function expectZuvixCoverageBatchResolved(
  label: string,
  batch: readonly SecretRegistryEntry[],
): Promise<void> {
  logCoverageBatch(label, batch);
  const config = {} as ZuvixConfig;
  const env: Record<string, string> = {};
  for (const [index, entry] of batch.entries()) {
    const envId = `ZUVIX_SECRET_TARGET_${entry.id}`;
    ...
    applyConfigForZuvixTarget(config, entry, envId, wildcardToken);
  }
...
const ZUVIX_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectZuvixCoverageEntries({ includePluginEntries: false }),
);
const ZUVIX_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectZuvixCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = ZUVIX_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectZuvixCoverageBatchResolved("zuvix.json core", googleChatBatch);
    }
  });

  describe("zuvix.json core and channel registry targets", () => {
    test.each(ZUVIX_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectZuvixCoverageBatchResolved("zuvix.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("zuvix.json plugin registry targets", () => {
    test.each(ZUVIX_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectZuvixCoverageBatchResolved("zuvix.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = `ZUVIX_AUTH_SECRET_TARGET_${entry.id}`;
        ...
        const snapshot = await prepareAuthCoverageSnapshot({
          ...
          env,
          agentDirs: ["/tmp/zuvix-agent-main"],
