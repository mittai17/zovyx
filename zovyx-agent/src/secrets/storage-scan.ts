...
import { isRecord as isJsonObject } from "@zuvix/normalization-core/record-coerce";
...
export function listAuthProfileStorePaths(config: ZuvixConfig, stateDir: string): string[] {
...
export function listAuthProfileStoreAgentDirs(config: ZuvixConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.ZUVIX_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
