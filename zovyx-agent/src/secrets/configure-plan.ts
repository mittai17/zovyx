/** Builds the interactive `zuvix secrets configure` target list and apply plan. */
...
/** Credential target shown by `zuvix secrets configure` before a SecretRef is selected. */
...
function getSecretProviders(config: ZuvixConfig): Record<string, SecretProviderConfig> {
...
/** Builds configure candidates for the current Zuvix config only. */
export function buildConfigureCandidates(config: ZuvixConfig): ConfigureCandidate[] {
...
/** Builds configure candidates for Zuvix config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: ZuvixConfig;
  authoredZuvixConfig?: ZuvixConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredZuvixConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const zuvixCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `zuvix.json` as const,
  ...
  return [...zuvixCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "zuvix secrets configure",
