...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("zuvix-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".zuvix", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".zuvix", "agents", "ops", "agent");
