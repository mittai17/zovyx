...
async function writeJsonFile(filePath: string, value: unknown): Promise<void> {
  if (path.basename(filePath) === "zuvix-agent.sqlite") {
...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-secrets-audit-"));
  const stateDir = path.join(rootDir, ".zuvix");
...
describe("secrets audit", () => {
  ...
  it("scans active agent-dir override models.json even when outside state dir", async () => {
    ...
    const report = await runSecretsAudit({
      env: {
        ...fixture.env,
        ZUVIX_AGENT_DIR: externalAgentDir,
      },
    });
    expect(
      hasFinding(
  ...
  it("does not flag non-sensitive routing headers in zuvix config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
  ...
  it("keeps request headers in zuvix config covered by plaintext audit", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
