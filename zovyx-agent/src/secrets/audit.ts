...
import { resolveStateDir, type ZuvixConfig } from "../config/config.js";
...
/** Stable finding codes emitted by `zuvix secrets audit`. */
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as ZuvixConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,
