...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["ZUVIX_AGENT_DIR", "ZUVIX_STATE_DIR"]);
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-usage-format-"));
  ...
  it("prefers models.json pricing over zuvix config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to zuvix config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
