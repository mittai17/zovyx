// Generates setup codes used to pair external channels with Zuvix.
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.ZUVIX_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.ZUVIX_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.ZUVIX_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.ZUVIX_GATEWAY_PASSWORD)),
  });
