// Zuvix state database manages shared persisted state and migrations.
...
import {
  resolveZuvixStateSqliteDir,
  resolveZuvixStateSqlitePath,
} from "./zuvix-state-db.paths.js";
import { ZUVIX_STATE_SCHEMA_SQL } from "./zuvix-state-schema.generated.js";

/**
 * Shared Zuvix SQLite state database lifecycle and metadata writers.
 *
 ...
 */
const ZUVIX_STATE_SCHEMA_VERSION = 1;
/** Shared timeout used by state and agent SQLite handles before surfacing busy errors. */
export const ZUVIX_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const ZUVIX_STATE_DIR_MODE = 0o700;
const ZUVIX_STATE_FILE_MODE = 0o600;
const ZUVIX_STATE_SIDECAR_SUFFIXES = ["", "-shm", "-wal"] as const;

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type ZuvixStateDatabase = {
  db: DatabaseSync;
  path: string;
...
export type ZuvixStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};

/** Status stored for a state migration run. */
export type ZuvixMigrationRunStatus = "completed" | "warning" | "failed";
/** Status stored for a state backup run. */
export type ZuvixBackupRunStatus = "completed" | "failed";

export type ZuvixStateDatabaseSchemaMigration = {
  kind: "agent-databases-composite-primary-key";
  path: string;
};

/** Input for recording one state migration run summary. */
export type RecordZuvixStateMigrationRunOptions = ZuvixStateDatabaseOptions & {
  id?: string;
  startedAt: number;
  finishedAt?: number;
  status: ZuvixMigrationRunStatus;
  report: Record<string, unknown>;
};

/** Input for recording one migrated source file/table pair. */
export type RecordZuvixStateMigrationSourceOptions = ZuvixStateDatabaseOptions & {
  runId: string;
...
export type RecordZuvixStateBackupRunOptions = ZuvixStateDatabaseOptions & {
  id?: string;
  createdAt: number;
  archivePath: string;
  status: ZuvixBackupRunStatus;
  manifest: Record<string, unknown>;
};

const cachedDatabases = new Map<string, ZuvixStateDatabase>();

type ZuvixStateMetadataDatabase = Pick<
  ZuvixStateKyselyDatabase,
...
function assertSupportedSchemaVersion(db: DatabaseSync, pathname: string): void {
  ...
  if (userVersion > ZUVIX_STATE_SCHEMA_VERSION) {
    throw new Error(
      `Zuvix state database ${pathname} uses newer schema version ${userVersion}; this Zuvix build supports ${ZUVIX_STATE_SCHEMA_VERSION}.`,
    );
  }
}
...
function filesystemRejectsChmod(target: string): boolean {
  ...
  try {
    ...
    probePath = path.join(probeDir, `.zuvix-chmod-probe-${randomUUID()}`);
    writeFileSync(probePath, "", { flag: "wx", mode: ZUVIX_STATE_FILE_MODE });
  } catch {
    return false;
  }
  try {
    chmodSync(probePath, ZUVIX_STATE_FILE_MODE);
    return false;
  } catch (err) {
...
function ensureZuvixStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveZuvixStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveZuvixStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`Zuvix state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: ZUVIX_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, ZUVIX_STATE_DIR_MODE);
  }
  for (const suffix of ZUVIX_STATE_SIDECAR_SUFFIXES) {
    const candidate = `${pathname}${suffix}`;
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, ZUVIX_STATE_FILE_MODE);
    }
  }
}
...
function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  throw new Error(
    `Zuvix state database ${pathname} has a legacy agent database registry schema; run zuvix doctor --fix to migrate it.`,
  );
}

export function detectZuvixStateDatabaseSchemaMigrations(
  options: ZuvixStateDatabaseOptions = {},
): ZuvixStateDatabaseSchemaMigration[] {
...
export function repairZuvixStateDatabaseSchema(options: ZuvixStateDatabaseOptions = {}): {
  changes: string[];
  warnings: string[];
} {
  ...
  ensureZuvixStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
  const db = new sqlite.DatabaseSync(pathname);
  db.exec(`PRAGMA busy_timeout = ${ZUVIX_SQLITE_BUSY_TIMEOUT_MS};`);
  try {
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(ZUVIX_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${ZUVIX_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  const kysely = getNodeSqliteKysely<ZuvixStateMetadataDatabase>(db);
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: ZUVIX_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
...
function resolveDatabasePath(options: ZuvixStateDatabaseOptions = {}): string {
  return path.resolve(options.path ?? resolveZuvixStateSqlitePath(options.env ?? process.env));
}

/** Open or return a cached shared state database after schema and migration checks. */
export function openZuvixStateDatabase(
  options: ZuvixStateDatabaseOptions = {},
): ZuvixStateDatabase {
  const env = options.env ?? process.env;
  const pathname = resolveDatabasePath(options);
...
export function runZuvixStateWriteTransaction<T>(
  operation: (database: ZuvixStateDatabase) => T,
  options: ZuvixStateDatabaseOptions = {},
): T {
  const database = openZuvixStateDatabase(options);
  const result = runSqliteImmediateTransactionSync(database.db, () => operation(database));
  try {
    ensureZuvixStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function recordZuvixStateMigrationRun(
  options: RecordZuvixStateMigrationRunOptions,
): string {
  const id = options.id ?? randomUUID();
  runZuvixStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<ZuvixStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
      database.db,
...
export function recordZuvixStateMigrationSource(
  options: RecordZuvixStateMigrationSourceOptions,
): void {
  runZuvixStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<ZuvixStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
...
export function recordZuvixStateBackupRun(options: RecordZuvixStateBackupRunOptions): string {
  const id = options.id ?? randomUUID();
  runZuvixStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<ZuvixStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
...
export function closeZuvixStateDatabase(): void {
...
export function isZuvixStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

/** Test alias for closing shared state handles from teardown code. */
export const closeZuvixStateDatabaseForTest = closeZuvixStateDatabase;
