// Zuvix state database tests cover state DB migrations and persistence.
...
import { resolveZuvixStateSqlitePath } from "./zuvix-state-db.paths.js";
import {
  collectSqliteSchemaShape,
  createSqliteSchemaShapeFromSql,
} from "./sqlite-schema-shape.test-support.js";

type StateDbTestDatabase = Pick<ZuvixStateKyselyDatabase, "diagnostic_events" | "schema_meta">;

function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-state-db-"));
}
...
describe("zuvix state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveZuvixStateSqlitePath({ ZUVIX_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "zuvix.sqlite"),
    );
  });
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "zuvix-test-state", `${process.pid}-7`, "state", "zuvix.sqlite"),
    );
  });

  it("creates the shared state schema from the committed SQL shape", () => {
    const stateDir = createTempStateDir();
    const database = openZuvixStateDatabase({
      env: { ZUVIX_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./zuvix-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "zuvix.sqlite"));
  });
  ...
  it("migrates requester and executor attribution for existing cross-agent tasks", () => {
    ...
    const reopened = openZuvixStateDatabase({
      env: { ZUVIX_STATE_DIR: stateDir },
    });
    ...
    const currentReopened = openZuvixStateDatabase({
      env: { ZUVIX_STATE_DIR: stateDir },
    });
    expect(
      currentReopened.db
  ...
  it("opens databases with early cron run-log tables before creating cron indexes", () => {
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env["ZUVIX_STATE_DIR"] = previousStateDir;
      }
    }
  });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `zuvix-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openZuvixStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openZuvixStateDatabase({ path: firstPath });
    const second = openZuvixStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openZuvixStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(1);
  });

  it("keys explicit relative paths by resolved database pathname", () => {
    const moduleUrl = new URL("./zuvix-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-state-db-relative-"));
          ...
          try {
            ...
            const first = openZuvixStateDatabase({ path: "state.sqlite" });
            first.db
            ...
            const second = openZuvixStateDatabase({ path: "state.sqlite" });
            second.db
              .prepare("INSERT INTO diagnostic_events (scope, event_key, payload_json, created_at) VALUES (?, ?, ?, ?)")
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    ...
    runZuvixStateWriteTransaction((database) => {
      ...
      expect(() =>
        runZuvixStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runZuvixStateWriteTransaction(async () => {
        return "not sync";
      }, options),
