...
import { resolveZuvixStateSqliteDir } from "./zuvix-state-db.paths.js";
...
export type ZuvixAgentSqlitePathOptions = {
  agentId: string;
...
export function resolveZuvixAgentSqlitePath(options: ZuvixAgentSqlitePathOptions): string {
  const agentId = normalizeAgentId(options.agentId);
  return path.resolve(
    options.path ??
      path.join(
        path.dirname(resolveZuvixStateSqliteDir(options.env ?? process.env)),
        "agents",
        agentId,
        "agent",
        "zuvix-agent.sqlite",
...
export function resolveZuvixAgentSqliteDir(options: ZuvixAgentSqlitePathOptions): string {
  return path.dirname(resolveZuvixAgentSqlitePath(options));
}
