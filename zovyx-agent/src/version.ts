...
declare const __ZUVIX_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "zuvix";
...
function readInjectedVersion(): string | undefined {
  return typeof __ZUVIX_VERSION__ === "string" ? __ZUVIX_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["ZUVIX_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["ZUVIX_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["ZUVIX_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.ZUVIX_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Zuvix version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.ZUVIX_BUNDLED_VERSION,
});
