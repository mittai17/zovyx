...
import { clampTimerTimeoutMs } from "@zuvix/normalization-core/number-coercion";
...
type CapabilityProviderCandidate = {
  ...
  isConfigured?: (ctx: { cfg?: ZuvixConfig; agentDir?: string }) => boolean;
};
...
function resolveCurrentDefaultProviderId(cfg?: ZuvixConfig): string {
...
function resolveAutoCapabilityFallbackRefs(params: {
  cfg: ZuvixConfig;
  agentDir?: string;
  listProviders: (cfg?: ZuvixConfig) => CapabilityProviderCandidate[];
}): string[] {
