...
import { ensureZuvixModelsJson } from "../agents/models-config.js";
import { STREAM_ERROR_FALLBACK_TEXT } from "../agents/stream-message-shared.js";
import { clearRuntimeConfigSnapshot, getRuntimeConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, ZuvixConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ZUVIX_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.ZUVIX_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.ZUVIX_LIVE_GATEWAY_SMOKE);
const THINKING_LEVEL = resolveGatewayLiveThinkingLevel({
  raw: process.env.ZUVIX_LIVE_GATEWAY_THINKING,
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.ZUVIX_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.ZUVIX_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.ZUVIX_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["ZUVIX_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveSetupTimeoutMs(
  raw = process.env.ZUVIX_LIVE_GATEWAY_SETUP_TIMEOUT_MS,
): number {
  return Math.max(1_000, toInt(raw, 180_000));
}

function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.ZUVIX_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.ZUVIX_LIVE_GATEWAY_MODELS?.trim();
  ...
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.ZUVIX_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.ZUVIX_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.ZUVIX_LIVE_MODEL_TIMEOUT_MS,
...
function enterProductionEnvForLiveRun() {
  const previous = {
    ...
    testFast: process.env.ZUVIX_TEST_FAST,
  };
  delete process.env.VITEST;
  delete process.env.ZUVIX_TEST_FAST;
...
function restoreProductionEnvForLiveRun(previous: {
  ...
  if (previous.testFast === undefined) {
    ...
    process.env.ZUVIX_TEST_FAST = previous.testFast;
  }
...
function shouldSkipEmptyResponseForLiveModel(params: {
  ...
  if (params.provider === "openrouter" || params.provider === "zuvix") {
    return true;
  }
  if (params.provider === "zuvix-go") {
    return true;
  }
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.ZUVIX_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.ZUVIX_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.ZUVIX_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("ZUVIX_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("ZUVIX_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("ZUVIX_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.ZUVIX_LIVE_GATEWAY_MODELS;
    delete process.env.ZUVIX_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ZUVIX_LIVE_MAX_MODELS;
  ...
  it("defaults small gateway sweeps to the curated small-model cap", () => {
    process.env.ZUVIX_LIVE_GATEWAY_MODELS = "small";
    delete process.env.ZUVIX_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ZUVIX_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.ZUVIX_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.ZUVIX_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ZUVIX_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.ZUVIX_LIVE_GATEWAY_MAX_MODELS = "2";
...
describe("buildLiveGatewayConfig", () => {
  it("pins selected live gateway models to the Zuvix runtime", () => {
...
describe("enterProductionEnvForLiveRun", () => {
  it("clears Vitest fast-reply flags while preserving caller state", () => {
    ...
    try {
      ...
      restoreOptionalEnv("ZUVIX_TEST_FAST", previous.testFast);
    }
  });
});
...
describe("shouldSkipToolNonceProbeMissForLiveModel", () => {
  it.each([
    ...
    { modelKey: "zuvix/big-pickle", expected: true },
    { modelKey: "zuvix-go/glm-5", expected: true },
...
describe("getHighSignalLiveModelPriorityIndex", () => {
  it("prefers curated Google replacements over big-pickle", () => {
    ...
    expect(getHighSignalLiveModelPriorityIndex({ provider: "zuvix", id: "big-pickle" })).toBe(
      null,
    );
  });
});
...
describe("shouldSkipEmptyResponseForLiveModel", () => {
  it.each([
    ...
    { provider: "zuvix", allowNotFoundSkip: false, expected: true },
    { provider: "zuvix-go", allowNotFoundSkip: false, expected: true },
      ...
      defaults: {
        ...
        models: Object.fromEntries(
          params.candidates.map((m) => [
            `${m.provider}/${m.id}`,
            { agentRuntime: { id: "zuvix" as const } },
          ]),
        ),
      },
    },
...
async function sanitizeAuthConfig(params: {
  cfg: ZuvixConfig;
  agentDir: string;
}): Promise<ZuvixConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<ZuvixConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const runtimeEnv = enterProductionEnvForLiveRun();
  const previous = {
    configPath: process.env.ZUVIX_CONFIG_PATH,
    token: process.env.ZUVIX_GATEWAY_TOKEN,
    skipChannels: process.env.ZUVIX_SKIP_CHANNELS,
    skipGmail: process.env.ZUVIX_SKIP_GMAIL_WATCHER,
    skipCron: process.env.ZUVIX_SKIP_CRON,
    skipCanvas: process.env.ZUVIX_SKIP_CANVAS_HOST,
    disableBonjour: process.env.ZUVIX_DISABLE_BONJOUR,
    logLevel: process.env.ZUVIX_LOG_LEVEL,
    agentDir: process.env.ZUVIX_AGENT_DIR,
    stateDir: process.env.ZUVIX_STATE_DIR,
  };

  process.env.ZUVIX_SKIP_CHANNELS = "1";
  process.env.ZUVIX_SKIP_GMAIL_WATCHER = "1";
  process.env.ZUVIX_SKIP_CRON = "1";
  process.env.ZUVIX_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.ZUVIX_DISABLE_BONJOUR = "1";
    process.env.ZUVIX_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.ZUVIX_GATEWAY_TOKEN = token;
  const agentId = "dev";
  ...
  const tempStateDir: string | undefined = await fs.mkdtemp(
    path.join(os.tmpdir(), "zuvix-live-state-"),
  );
  ...
  process.env.ZUVIX_AGENT_DIR = tempAgentDir;

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".zuvix"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".zuvix", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.zuvix-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDefaultAgentDir(params.cfg);
  const sanitizedCfg: ZuvixConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-live-"));
  const tempConfigPath = path.join(tempDir, "zuvix.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.ZUVIX_CONFIG_PATH = tempConfigPath;
  ...
  try {
        ...
        try {
              ...
              for (
                ...
                try {
                  toolText = await requestGatewayAgentText({
                    ...
                    modelKey,
                    message: strictReply
                      ? "Zuvix live tool probe (local, safe): " +
          ...
          if (
            (model.provider === "minimax" ||
              model.provider === "zuvix" ||
              model.provider === "zuvix-go" ||
              model.provider === "zai") &&
            isLiveRateLimitDrift(message)
          ) {
            skippedCount += 1;
    ...
    restoreOptionalEnv("ZUVIX_CONFIG_PATH", previous.configPath);
    restoreOptionalEnv("ZUVIX_GATEWAY_TOKEN", previous.token);
    restoreOptionalEnv("ZUVIX_SKIP_CHANNELS", previous.skipChannels);
    restoreOptionalEnv("ZUVIX_SKIP_GMAIL_WATCHER", previous.skipGmail);
    restoreOptionalEnv("ZUVIX_SKIP_CRON", previous.skipCron);
    restoreOptionalEnv("ZUVIX_SKIP_CANVAS_HOST", previous.skipCanvas);
    restoreOptionalEnv("ZUVIX_DISABLE_BONJOUR", previous.disableBonjour);
    restoreOptionalEnv("ZUVIX_LOG_LEVEL", previous.logLevel);
    restoreOptionalEnv("ZUVIX_AGENT_DIR", previous.agentDir);
    restoreOptionalEnv("ZUVIX_STATE_DIR", previous.stateDir);
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        const modelsJsonResult = await withGatewayLiveSetupTimeout(
          ensureZuvixModelsJson(cfg, undefined, {
            workspaceDir,
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ZUVIX_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    try {
      ...
      await ensureZuvixModelsJson(cfg);
      ...
      const nonceA = randomUUID();
      const nonceB = randomUUID();
      toolProbePath = path.join(workspaceDir, `.zuvix-live-zai-fallback.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
