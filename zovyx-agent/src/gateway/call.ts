...
import { isLoopbackIpAddress } from "@zuvix/net-policy/ip";
import { redactSensitiveUrlLikeString } from "@zuvix/net-policy/redact-sensitive-url";
...
type GatewayRuntimeConfigLoader = () => ZuvixConfig | Promise<ZuvixConfig>;
const defaultGetRuntimeConfig = async (): Promise<ZuvixConfig> =>
...
async function loadGatewayConfig(): Promise<ZuvixConfig> {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: ZuvixConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallAuth(config: ZuvixConfig) {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.ZUVIX_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.ZUVIX_TEST_HANDSHAKE_TIMEOUT_MS);
...
async function resolveGatewayCallContext(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.ZUVIX_GATEWAY_URL);
  ...
  const config =
    opts.config ?? (canSkipConfigLoad ? ({} as ZuvixConfig) : await loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `zuvix doctor` for diagnostics.";
  }
  return message;
}
