...
const ZUVIX_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const ZUVIX_CLI_CHILD_TIMEOUT_MS = ZUVIX_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function shouldRunLiveImageProbe(params: { agent: string; override?: string }): boolean {
  ...
  return normalizeOptionalLowercaseString(params.agent) !== "zuvix";
}
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the Zuvix MCP tool `zuvix-tools/cron` (server `zuvix-tools`, tool `cron`). " +
      "If the harness shows Claude-style MCP names, use `mcp__zuvix-tools__cron` or `mcp__zuvix_tools__cron`. " +
      ...
      "Do the actual tool call; I will verify externally with the Zuvix cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the Zuvix MCP tool `zuvix-tools/cron` now. " +
      "If the harness shows Claude-style MCP names, use `mcp__zuvix-tools__cron` or `mcp__zuvix_tools__cron`. " +
      ...
      "I will verify externally with the Zuvix cron CLI."
    );
  }
  return (
    "Your previous Zuvix cron MCP tool call was cancelled before the job was created. " +
    "Retry the Zuvix MCP tool `zuvix-tools/cron` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__zuvix-tools__cron` or `mcp__zuvix_tools__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
...
export async function runZuvixCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(ZUVIX_CLI_GATEWAY_TIMEOUT_MS)];
  const { stdout, stderr } = await execFileAsync(process.execPath, ["zuvix.mjs", ...cliArgs], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: ZUVIX_CLI_CHILD_TIMEOUT_MS,
    maxBuffer: 1024 * 1024,
  });
  ...
  if (!trimmed) {
    throw new Error(
      [
        `zuvix ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `zuvix ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runZuvixCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
