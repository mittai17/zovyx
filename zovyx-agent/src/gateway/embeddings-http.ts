...
// Bridges /v1/embeddings requests to configured Zuvix memory providers.
...
// OpenAI-compatible `/v1/embeddings` bridge. It maps Zuvix agent/model
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== ZUVIX_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `zuvix` or `zuvix/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-zuvix-model")) ||
