...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ZUVIX_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.ZUVIX_DISABLE_BONJOUR) &&
  ...
  const sshPort = mdnsMinimal
    ? undefined
    : (parseTcpPort(process.env.ZUVIX_SSH_PORT) ?? undefined);
