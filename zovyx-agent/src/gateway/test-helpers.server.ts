...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ZUVIX_BUNDLED_PLUGINS_DIR",
  "ZUVIX_DISABLE_BUNDLED_PLUGINS",
  "ZUVIX_SKIP_CHANNELS",
  "ZUVIX_SKIP_PROVIDERS",
  "ZUVIX_SKIP_CRON",
  "ZUVIX_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.ZUVIX_CONFIG_PATH) {
    configPaths.add(process.env.ZUVIX_CONFIG_PATH);
  }
  if (process.env.ZUVIX_STATE_DIR) {
    configPaths.add(path.join(process.env.ZUVIX_STATE_DIR, "zuvix.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.ZUVIX_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "zuvix-test-no-bundled-extensions")
    : "zuvix-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".zuvix-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".zuvix-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".zuvix-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>zuvix-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__zuvixTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__zuvixTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["ZUVIX_GATEWAY_TOKEN"]);
  const prev = process.env.ZUVIX_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ZUVIX_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.ZUVIX_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
