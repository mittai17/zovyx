...
const CODEX_HARNESS_LIVE = process.env.ZUVIX_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.ZUVIX_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["ZUVIX_TRAJECTORY", "ZUVIX_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-zuvix-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.ZUVIX_TRAJECTORY = "1";
      process.env.ZUVIX_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".zuvix", "trajectory-exports", "bundle");
