...
import { detectMime } from "@zuvix/media-core/mime";
import {
  asDateTimestampMs,
  resolveTimestampMsToIsoString,
} from "@zuvix/normalization-core/number-coercion";
...
const ROOT_PREFIX = "/";
const CONTROL_UI_ASSISTANT_MEDIA_PREFIX = "/__zuvix__/assistant-media";
...
const CONTROL_UI_NAMESPACE_PREFIX = "/__zuvix__/";
...
// `/__zuvix__/`, so a browser that opens the default entry infers
// `/__zuvix__` as its base path (see `inferBasePathFromPathname`) and fetches
// `/__zuvix__/control-ui-config.json`. Accept that namespaced alias so the
...
// Single-underscore `/__zuvix` prefix used by the pre-base-path-relative
...
// `/__zuvix/control-ui-config.json`, so current main and the v2026.6.1
// release serve and document that exact path under an empty base path.
const LEGACY_CONTROL_UI_NAMESPACE_PREFIX = "/__zuvix";

// The old documented no-base-path bootstrap endpoint
// (`/__zuvix/control-ui-config.json`, single underscore). It is derived from
// the legacy `/__zuvix` namespace joined with the canonical config constant
 ...
 * single-underscore suffix `${basePath}/__zuvix/control-ui-config.json` that
 ...
 * the default-namespace alias `/__zuvix__/control-ui-config.json`, which is
 * what the default `/__zuvix__/` entry requests after inferring its base path
...
function matchesControlUiBootstrapConfigPath(pathname: string, basePath: string): boolean {
  ...
  // `/__zuvix` namespace, so joining it with the base path yields
  // `${basePath}/__zuvix/control-ui-config.json` (or the bare legacy path
  ...
  // The default `/__zuvix__/` namespace alias only applies when no base path
