...
export const ZUVIX_MODEL_ID = "zuvix";
/** Default OpenAI-compatible model alias that targets the default Zuvix agent. */
export const ZUVIX_DEFAULT_MODEL_ID = "zuvix/default";
...
function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-zuvix-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-zuvix-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === ZUVIX_MODEL_ID || lowered === ZUVIX_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^zuvix[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
/** Validates and resolves the `x-zuvix-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `zuvix` or `zuvix/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-zuvix-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-zuvix-model`." };
  }
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-zuvix-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-zuvix-message-channel")) ??
