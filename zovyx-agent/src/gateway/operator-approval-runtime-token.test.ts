...
function useTempHome(): string {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-approval-runtime-"));
  tempHomes.push(home);
...
function execApprovalsPath(home: string): string {
  return path.join(home, ".zuvix", "exec-approvals.json");
}

function writeExecApprovalsToken(home: string, token: string): void {
  fs.mkdirSync(path.join(home, ".zuvix"), { recursive: true });
  fs.writeFileSync(
...
afterEach(() => {
  ...
  if (originalEnv.ZUVIX_HOME === undefined) {
    delete process.env.ZUVIX_HOME;
  } else {
    process.env.ZUVIX_HOME = originalEnv.ZUVIX_HOME;
  }
