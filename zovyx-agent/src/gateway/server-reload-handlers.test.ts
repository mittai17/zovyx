...
const hoisted = vi.hoisted(() => ({
  ...
  runtimeConfig: { value: { session: { store: "/tmp/active-sessions.json" } } as ZuvixConfig },
  reloadEvents: [] as string[],
  resetModelCatalogCache: vi.fn(() => {}),
  refreshContextWindowCache: vi.fn(async (_cfg: ZuvixConfig) => {}),
  clearCurrentProviderAuthState: vi.fn(() => {}),
  warmCurrentProviderAuthStateOffMainThread: vi.fn(async (_cfg: ZuvixConfig) => {}),
...
vi.mock("../agents/context.js", () => ({
  refreshContextWindowCache: async (cfg: ZuvixConfig) => {
...
vi.mock("../agents/model-provider-auth.js", () => ({
  ...
  warmCurrentProviderAuthStateOffMainThread: async (cfg: ZuvixConfig) => {
...
describe("gateway hot reload model state", () => {
  it("resets prepared model runtime state for every hot reload and rewarms after plugin reload", async () => {
    ...
    const nextConfig = { plugins: { enabled: true } } as ZuvixConfig;
  ...
  it("disposes cached MCP runtimes on MCP config hot reloads", async () => {
    ...
    const nextConfig = { mcp: { servers: {} } } as ZuvixConfig;
  ...
  it.each([
    ...
    const { applyHotReload } = createReloadHandlersForTest();
    const previousConfig = testCase.previousConfig as ZuvixConfig;
    const nextConfig = testCase.nextConfig as ZuvixConfig;
...
describe("gateway Gmail hot reload handlers", () => {
  ...
  function createGmailConfig(account: string): ZuvixConfig {
    return {
  ...
  it("aborts an in-flight managed Gmail restart when the reloader stops", async () => {
    ...
    const reloader = startManagedGatewayConfigReloader({
      ...
      activateRuntimeSecrets: vi.fn(async (config: ZuvixConfig) => ({
        sourceConfig: config,
        config,
        authStores: [],
        warnings: [],
