...
const { createZuvixTools } = await import("../agents/zuvix-tools.js");
...
type SessionSendTool = ReturnType<typeof createZuvixTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createZuvixTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ZUVIX_GATEWAY_PORT", "ZUVIX_GATEWAY_TOKEN"]);
  ...
  process.env.ZUVIX_GATEWAY_PORT = String(gatewayPort);
  process.env.ZUVIX_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-sessions-send-route-"));
