// Zuvix Gateway client facade.
// Wraps the shared gateway-client package with Zuvix host dependencies.
import {
...
function createZuvixGatewayClientHostDeps(
  overrides?: GatewayClientHostDeps,
): GatewayClientHostDeps {
  return {
    // This wrapper is the only place the package reaches into Zuvix runtime
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    ...
    // Zuvix device identity, token storage, proxy routing, and logging.
    this.#client = new BaseGatewayClient({
      ...opts,
      clientVersion: opts.clientVersion ?? VERSION,
      hostDeps: createZuvixGatewayClientHostDeps(opts.hostDeps),
    });
  }
