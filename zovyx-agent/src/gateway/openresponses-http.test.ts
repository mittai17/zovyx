...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("ZUVIX_CONFIG_PATH is required for gateway config tests");
  }
...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "zuvix", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `zuvix` or `zuvix/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "zuvix", input: "hi" },
        { "x-zuvix-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "zuvix/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "zuvix/default", input: "hi" });
...

      {
        ...
        const res = await postResponses(
          port,
          { model: "zuvix", input: "hi" },
          { "x-zuvix-agent-id": "missing-agent" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
      ...
      {
        agentCommand.mockClear();
        const res = await postResponses(port, { model: "zuvix/missing-agent", input: "hi" });
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Unknown agent 'missing-agent'.");
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "zuvix", input: "hi" },
        { "x-zuvix-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-zuvix-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("accepts write-scoped and admin-scoped HTTP callers", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "zuvix", input: "hi" },
      { "x-zuvix-scopes": "operator.admin, operator.write" },
    );
