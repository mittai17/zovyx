...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.ZUVIX_LIVE_CODEX_BIND);
const describeLive = LIVE && CODEX_BIND_LIVE ? describe : describe.skip;
const CODEX_BIND_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.ZUVIX_LIVE_CODEX_BIND_TIMEOUT_MS,
  900_000,
);
const CODEX_BIND_REQUEST_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.ZUVIX_LIVE_CODEX_BIND_REQUEST_TIMEOUT_MS,
  300_000,
);
...
async function writePluginBindingApproval(params: {
  ...
  const zuvixDir = path.join(params.homeDir, ".zuvix");
  await fs.mkdir(zuvixDir, { recursive: true });
  await fs.writeFile(
    path.join(zuvixDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.ZUVIX_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.ZUVIX_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.ZUVIX_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-live-codex-bind-"));
      ...
      const bindModel =
        process.env.ZUVIX_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        try {
          ...
          restoreEnvVar("ZUVIX_CONFIG_PATH", previous.configPath);
          restoreEnvVar("ZUVIX_GATEWAY_TOKEN", previous.gatewayToken);
          restoreEnvVar("HOME", previous.home);
          restoreEnvVar("ZUVIX_SKIP_CANVAS_HOST", previous.skipCanvas);
          restoreEnvVar("ZUVIX_SKIP_CHANNELS", previous.skipChannels);
          restoreEnvVar("ZUVIX_SKIP_CRON", previous.skipCron);
          restoreEnvVar("ZUVIX_SKIP_GMAIL_WATCHER", previous.skipGmail);
          restoreEnvVar("ZUVIX_STATE_DIR", previous.stateDir);
        }
      }
    },
