...
type ConnectionAuthCase = {
  name: string;
  cfgLocal: ZuvixConfig;
  env: NodeJS.ProcessEnv;
  options?: Partial<Omit<GatewayConnectionAuthOptions, "config" | "env">>;
  expected: ResolvedAuth;
};

function cfg(input: Partial<ZuvixConfig>): ZuvixConfig {
  return input as ZuvixConfig;
}
...
const DEFAULT_ENV = {
  ...
  ZUVIX_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when ZUVIX env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when ZUVIX env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when ZUVIX env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
