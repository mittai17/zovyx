...
const getRuntimeConfig = vi.hoisted(() =>
  vi.fn<() => ZuvixConfig>(() => ({}) as ZuvixConfig),
);
...
function setRuntimeConfig(config: ZuvixConfig): void {
  getRuntimeConfig.mockReturnValue(config);
}

function pluginEntries(...ids: string[]): ZuvixConfig["plugins"] {
  return {
    entries: Object.fromEntries(ids.map((id) => [id, { enabled: true }])),
  };
}

function telegramConfig(): ZuvixConfig {
  return {
    channels: {
      telegram: {
...
function telegramSlackConfig(params: { includeMainSession?: boolean } = {}): ZuvixConfig {
  return {
...
function msteamsConfig(): ZuvixConfig {
  return {
    channels: {
      msteams: {
...
function slackSynologyConfig(): ZuvixConfig {
  return {
    channels: {
      slack: {
        botToken: "xoxb-slack-token",
...
function slackConfig(params: { includeMainSession?: boolean } = {}): ZuvixConfig {
  return {
    ...(params.includeMainSession ? { session: { mainKey: "main" } } : {}),
    channels: {
      slack: {
...
describe("cron method validation", () => {
  beforeEach(() => {
    getRuntimeConfig.mockReset().mockReturnValue({} as ZuvixConfig);
