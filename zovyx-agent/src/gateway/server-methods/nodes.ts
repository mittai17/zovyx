...
import { normalizeUniqueTrimmedStringList } from "@zuvix/normalization-core/string-normalization";
import {
  type ConnectParams,
  ErrorCodes,
  errorShape,
...
export async function maybeWakeNodeWithApns(
  nodeId: string,
  opts?: { force?: boolean; wakeReason?: string; cfg?: ZuvixConfig },
  ...
  try {
    ...
    if (registration.transport === "relay") {
      ...
      result = await sendApnsAlert({
        registration,
        nodeId,
        title: "Zuvix needs a quick reopen",
        body: "Tap to reopen Zuvix and restore the node connection.",
        relayConfig: relay.relayConfig,
      });
    } else {
      const auth = await resolveDirectNodePushConfig();
  ...
  "node.list": async ({ params, respond, client, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      const catalog = createKnownNodeCatalog({
        ...
        pendingNodes: canReadPendingNodePairing(client) ? nodePairing.pending : undefined,
