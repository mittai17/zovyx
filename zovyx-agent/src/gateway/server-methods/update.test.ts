...
const startManagedServiceUpdateHandoffMock = vi.fn(async () => ({
  ...
  pid: 12345,
  command: "zuvix update --yes --timeout 1800",
  logPath: "/tmp/zuvix-update-run-handoff/handoff.log",
}));
...
vi.mock("../../infra/zuvix-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/zuvix-root.js")>(
    "../../infra/zuvix-root.js",
  );
  return {
    ...actual,
    resolveZuvixPackageRoot: async () => "/tmp/zuvix",
  };
});
...
vi.mock("../../infra/update-managed-service-handoff.js", () => ({
  ...
  formatManagedServiceUpdateCommand: (params?: {
    ...
    params?.timeoutMs
      ? `zuvix update --yes --timeout ${Math.ceil(params.timeoutMs / 1000)}`
      : "zuvix update --yes",
  buildManagedServiceHandoffUnavailableMessage: (command: string) =>
    [
      "Zuvix updates cannot safely run inside the live gateway process without a managed-service handoff.",
...
function mockGlobalInstallSurface() {
  resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
    kind: "global",
    mode: "npm",
    root: "/tmp/zuvix-global",
    packageRoot: "/tmp/zuvix-global",
  });
}
...
describe("update.run restart scheduling", () => {
  ...
  it("hands managed package updates to the CLI path instead of running them in-process", async () => {
    ...
    const payload = await withProcessEnv({ ZUVIX_LAUNCHD_LABEL: "ai.zuvix.gateway" }, () =>
  ...
  it("keeps a startup grace before restarting after systemd handoff spawn", async () => {
    ...
    await withProcessEnv({ ZUVIX_SYSTEMD_UNIT: "zuvix-gateway.service" }, () =>
  ...
  it("starts managed package handoff when the gateway cwd is unavailable", async () => {
    ...
    try {
      await withProcessEnv({ ZUVIX_LAUNCHD_LABEL: "ai.zuvix.gateway" }, () =>
        invokeUpdateRun({}),
      );
    } finally {
      cwdSpy.mockRestore();
  ...
  it("hands supervised git/dev updates to the CLI path instead of rebuilding live dist in-process", async () => {
    detectRespawnSupervisorMock.mockReturnValueOnce("launchd");
    mockGitInstallSurface("/tmp/zuvix-git");
    const payload = await withProcessEnv({ ZUVIX_LAUNCHD_LABEL: "ai.zuvix.gateway" }, () =>
      captureUpdateRunPayload(),
    );
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      message:
        "Zuvix updates cannot safely run inside the live gateway process without a managed-service handoff.\n" +
        "Run `zuvix update --yes --timeout 1800` from a shell outside the gateway service, or restart/update from the host UI.",
    });
  });
