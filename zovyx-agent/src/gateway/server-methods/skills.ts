...
import {
  ...
  fetchZuvixSkillSecurityVerdicts,
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.securityVerdicts": async ({ params, respond, context }) => {
    ...
    try {
      ...
      if (targets.length === 0) {
        respond(true, { schema: "zuvix.skills.security-verdicts.v1", items: [] }, undefined);
        return;
      }
      const items = await fetchZuvixSkillSecurityVerdicts(targets);
      respond(true, { schema: "zuvix.skills.security-verdicts.v1", items }, undefined);
    } catch (err) {
