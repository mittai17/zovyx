...
vi.mock("../../skills/security/clawhub-verdicts.js", () => ({
  ...
  fetchZuvixSkillSecurityVerdicts: vi.fn(),
}));
...
describe("skills proposal gateway handlers", () => {
  beforeEach(async () => {
    testState = await createZuvixTestState({
      layout: "state-only",
      prefix: "zuvix-skills-proposals-gateway-state-",
    });
    ...
    mocks.workspaceDir = await tempDirs.make("zuvix-skills-proposals-gateway-");
  ...
  it("scopes list and inspect to the resolved agent workspace", async () => {
    ...
    const secondWorkspaceDir = await tempDirs.make("zuvix-skills-proposals-gateway-second-");
