...
describe("sessions.files RPC handlers", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-session-files-test-"));
    hoisted.resolveDefaultAgentId.mockReturnValue("main");
    hoisted.resolveAgentWorkspaceDir.mockReturnValue(workspaceRoot);
    writeWorkspaceFile(workspaceRoot, "package.json", '{"name":"zuvix-test"}\n');
  ...
  it("does not read absolute paths outside the configured workspace", async () => {
    const outsidePath = path.join(os.tmpdir(), `zuvix-outside-${Date.now()}.txt`);
  ...
  it("does not follow workspace symlinks for file previews", async () => {
    const outsidePath = path.join(os.tmpdir(), `zuvix-linked-${Date.now()}.txt`);
  ...
  it("does not follow symlinked parent directories for file previews", async () => {
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-linked-parent-"));
  ...
  it("does not derive a workspace root from transcript cwd", async () => {
    ...
    const transcriptCwd = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-transcript-cwd-"));
