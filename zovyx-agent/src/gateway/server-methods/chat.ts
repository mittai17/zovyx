...
import { isAudioFileName } from "@zuvix/media-core/mime";
...
export function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  const rawMetadata =
    message && typeof message === "object"
      ? (message as Record<string, unknown>)["__zuvix"]
      : undefined;
  const metadata =
...
async function findSourceReplyTranscriptMirrorByIdempotencyKey(
  ...
  if (found?.message.provider !== "zuvix" || found.message.model !== "delivery-mirror") {
    return null;
  }
  return found;
}
...
function sendGlobalAwareNodeChatPayload(params: {
  ...
  const deliveryKeys = resolveGlobalAwareNodeChatDeliveryKeys({
    cfg: params.context.getRuntimeConfig?.() ?? ({} as ZuvixConfig),
    sessionKey: params.sessionKey,
    agentId: params.agentId,
  });
...
function readChatHistoryMessageId(message: unknown): string | undefined {
  const metadata = asOptionalRecord(asOptionalRecord(message)?.["__zuvix"]);
...
async function handleChatHistoryRequest({
  ...
  const requestedAgentId = resolveRequestedChatAgentId({
    cfg: (context as { getRuntimeConfig?: () => ZuvixConfig }).getRuntimeConfig?.(),
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
      ...
      try {
        await measureDiagnosticsTimelineSpan(
          ...
          async () => {
            ...
            // attachments even when the parent Zuvix session model is text-only.
    ...
    try {
      ...
      // See: https://github.com/zuvix/zuvix/issues/3658
        ...
        .then(async () => {
          ...
          await measureDiagnosticsTimelineSpan(
            ...
            async () => {
              ...
              // through Zuvix runtime's SessionManager; this dispatcher only owns live delivery payloads.
                  ...
                  if (shouldAppendAssistantTranscript) {
                    ...
                    if (appended.ok) {
                      ...
                      message = {
                        ...
                        timestamp: nowValue,
                        ...(ttsSupplementMarker
                          ? { zuvixTtsSupplement: ttsSupplementMarker }
                          : {}),
