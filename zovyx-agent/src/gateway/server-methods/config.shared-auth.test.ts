...
const persistedConfigResultMock = vi.fn((config: ZuvixConfig) => config);
...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/zuvix.json" }),
    writeConfigFile: writeConfigFileMock,
    replaceConfigFile: async (params: { nextConfig: ZuvixConfig; writeOptions?: unknown }) => {
...
function tokenAuthConfig(token: string): ZuvixConfig {
  return {
    gateway: {
      auth: {
        mode: "token",
...
function hotReloadConfig(): ZuvixConfig {
  return {
    gateway: {
      reload: {
        mode: "hot",
...
function mockPreviousConfig(config: ZuvixConfig): void {
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: ZuvixConfig) => ({
    ok: true,
    config,
  }));
  prepareSecretsRuntimeSnapshotMock.mockImplementation(
    async ({ config }: { config: ZuvixConfig }) => ({
      config,
    }),
  );
  restartSentinelMocks.writeRestartSentinel.mockClear();
  persistedConfigResultMock.mockImplementation((config: ZuvixConfig) => config);
});

describe("config shared auth disconnects", () => {
  it("returns the persisted config from config.set write results", async () => {
    const prevConfig: ZuvixConfig = {
      gateway: {
        port: 19000,
      },
    };
    const submittedConfig: ZuvixConfig = {
      gateway: {
        port: 19001,
      },
    };
    const persistedConfig: ZuvixConfig = {
      gateway: {
        port: 19001,
      },
      meta: {
