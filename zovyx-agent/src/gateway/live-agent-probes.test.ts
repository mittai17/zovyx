...
describe("live-agent-probes", () => {
  it("only special-cases Claude-like retry prompts", () => {
    ...
    expect(isClaudeLikeLiveAgent("zuvix-ai")).toBe(false);
  ...
  it("skips the shared image probe for text-only live agents unless forced", () => {
    ...
    expect(shouldRunLiveImageProbe({ agent: "zuvix" })).toBe(false);
    expect(shouldRunLiveImageProbe({ agent: "zuvix", override: "1" })).toBe(true);
  ...
  it("builds a retryable cron prompt with provider-specific fallback wording", () => {
    ...
    ).toContain("previous Zuvix cron MCP tool call was cancelled");
