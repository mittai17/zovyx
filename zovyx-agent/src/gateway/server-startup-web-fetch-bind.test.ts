...
async function writeConfig(config: ZuvixConfig): Promise<void> {
...
describe("gateway startup web fetch config", () => {
  it("binds HTTP with credential-free tools.web.fetch config without fetch provider discovery", async () => {
    const previousMinimal = process.env.ZUVIX_TEST_MINIMAL_GATEWAY;
    process.env.ZUVIX_TEST_MINIMAL_GATEWAY = "0";
    ...
    try {
      ...
      if (previousMinimal === undefined) {
        ...
        process.env.ZUVIX_TEST_MINIMAL_GATEWAY = previousMinimal;
      }
    }
  });
});
