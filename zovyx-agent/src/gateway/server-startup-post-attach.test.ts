...
const hoisted = vi.hoisted(() => {
  ...
  const ensureZuvixModelsJson = vi.fn(async () => {});
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/zuvix-state",
    resolveConfigPath: vi.fn(() => "/tmp/zuvix-state/zuvix.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/zuvix-state"),
  };
});
...
vi.mock("../agents/models-config.js", () => ({
  ensureZuvixModelsJson: hoisted.ensureZuvixModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("ZUVIX_SKIP_CHANNELS", "0");
    vi.stubEnv("ZUVIX_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureZuvixModelsJson.mockReset();
    hoisted.ensureZuvixModelsJson.mockResolvedValue(undefined);
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-sentinel-"));
  ...
  it("expands tilde-based restart sentinel state paths", async () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-home-"));
    try {
      const zuvixHome = path.join(osHome, "zuvix-home");
      const stateDirFromHome = path.join(zuvixHome, ".zuvix");
      ...
      expect(
        await testing.hasRestartSentinelFileFast({
          HOME: osHome,
          ZUVIX_HOME: "~/zuvix-home",
        } as NodeJS.ProcessEnv),
      ).toBe(true);

      const backslashStateDir = path.resolve(`${osHome}\\zuvix-state`);
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-async-sentinel-"));
    try {
      fs.writeFileSync(path.join(stateDir, "restart-sentinel.json"), "{}\n");
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/zuvix-workspace",
        deps: {} as never,
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/zuvix-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
  ...
  it("starts channels when channel startup is enabled", async () => {
    await withEnvAsync(
      {
        ZUVIX_SKIP_CHANNELS: undefined,
        ZUVIX_SKIP_PROVIDERS: undefined,
      },
      async () => {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { ZUVIX_SKIP_CHANNELS: undefined, ZUVIX_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { ZUVIX_SKIP_CHANNELS: "1", ZUVIX_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
          cfg: { hooks: { internal: { enabled: false } } } as never,
          pluginRegistry: createPostAttachParams().pluginRegistry,
    ...
    expect(logChannels.info).toHaveBeenCalledWith(
      "skipping channel start (ZUVIX_SKIP_CHANNELS=1 or ZUVIX_SKIP_PROVIDERS=1)",
    );
  });
