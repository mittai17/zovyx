...
// Emits a one-shot notice for ignored pre-Zuvix environment prefixes.
...
export function warnLegacyZuvixEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but Zuvix only reads ZUVIX_* names now.`,
      "Rename them by replacing the legacy prefix with ZUVIX_; the old names are ignored.",
    ].join("\n"),
    { code: "ZUVIX_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

/** Resets the one-shot legacy env warning latch for tests. */
export function resetLegacyZuvixEnvWarningForTest(): void {
  warned = false;
}
