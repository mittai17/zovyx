...
export function resolveHooksConfig(cfg: ZuvixConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ZuvixConfig, defaultAgentId: string): Set<string> {
...
/** Extract the hook bearer token from Authorization or x-zuvix-token headers. */
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-zuvix-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-zuvix-idempotency-key"]) ||
