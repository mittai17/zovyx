...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.ZUVIX_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: ZuvixConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: ZuvixConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: ZuvixConfig, requestedAgentId: string) => {
