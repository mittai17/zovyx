...
const mocks = vi.hoisted(() => ({
  replaceConfigFile: vi.fn(async (_params: { nextConfig: ZuvixConfig }) => {}),
}));
...
type GatewayAuthConfig = NonNullable<NonNullable<ZuvixConfig["gateway"]>["auth"]>;
...
function gatewayAuthConfig(auth: GatewayAuthConfig): ZuvixConfig {
  return {
    gateway: { auth },
  };
}

function gatewayAuthConfigWithDefaultEnvProvider(auth: GatewayAuthConfig): ZuvixConfig {
  return {
...
describe("mergeGatewayTailscaleConfig", () => {
  ...
  it("preserves explicit serviceName overrides", () => {
    expect(
      mergeGatewayTailscaleConfig(
        { mode: "serve", serviceName: "svc:old-zuvix", resetOnExit: false },
        { serviceName: "svc:zuvix" },
      ),
    ).toEqual({ mode: "serve", serviceName: "svc:zuvix", resetOnExit: false });
  });
});
...
describe("ensureGatewayStartupAuth", () => {
  ...
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: ZuvixConfig) {
  ...
  async function expectNoTokenGeneration(cfg: ZuvixConfig, mode: string) {
    const result = await runStartupAuth({
      cfg,
      persist: true,
    });
  ...
  function createMissingGatewayTokenSecretRefConfig(): ZuvixConfig {
  ...
  it("resolves env-template gateway.auth.token before env-token short-circuiting", async () => {
    await expectResolvedToken({
      cfg: gatewayAuthConfig({ mode: "token", token: "${ZUVIX_GATEWAY_TOKEN}" }),
      env: {
        ZUVIX_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${ZUVIX_GATEWAY_TOKEN}",
    });
  });

  it("uses ZUVIX_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        ZUVIX_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
  ...
  it("uses ZUVIX_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await runStartupAuth({
      cfg: gatewayAuthConfigWithDefaultEnvProvider({
        mode: "password",
      ...
      env: {
        ZUVIX_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
  ...
  it("keeps startup non-breaking when hooks token reuses gateway token resolved from env", async () => {
    ...
    const result = await runStartupAuth({
      ...
      env: {
        ZUVIX_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
      } as NodeJS.ProcessEnv,
      warn,
    });
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("zuvix security audit"));
  });
