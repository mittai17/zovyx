...
describe("resolveGatewayRequestContext", () => {
  it("uses normalized x-zuvix-message-channel when enabled", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-zuvix-message-channel": " Custom-Channel " }),
      model: "zuvix",
  ...
  it("does not build session state for explicit unknown agent ids", () => {
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-zuvix-agent-id": "missing-agent" }),
        model: "zuvix",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-zuvix-agent-id": "!!!" }),
        model: "zuvix",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
...
describe("resolveHttpSenderIsOwner", () => {
  it("requires operator.admin on a trusted HTTP scope-bearing request", () => {
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-zuvix-scopes": "operator.admin" }), noneAuth),
    ).toBe(true);
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-zuvix-scopes": "operator.write" }), noneAuth),
    ).toBe(false);
  });
...
describe("resolveOpenAiCompatibleHttpSenderIsOwner", () => {
  ...
  it("still requires operator.admin for trusted scope-bearing requests", () => {
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-zuvix-scopes": "operator.write" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(false);
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-zuvix-scopes": "operator.admin" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(true);
  });
...
describe("authorizeOpenAiCompatibleHttpModelOverride", () => {
  it("allows shared-secret bearer callers to use x-zuvix-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({ authorization: "Bearer secret", "x-zuvix-model": "openai/gpt-5.4" }),
        { authMethod: "token", trustDeclaredOperatorScopes: false },
      ),
    ).toEqual({ allowed: true });
  });

  it("allows trusted admin callers to use x-zuvix-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-zuvix-scopes": "operator.admin, operator.write",
  ...
  it("rejects trusted write-only callers that try to use x-zuvix-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-zuvix-scopes": "operator.write",
