...
function createSingleAgentAvatarConfig(workspace: string): ZuvixConfig {
  return {
...
describe("gateway session utils", () => {
  ...
  test("session list search includes direct-session origin display labels", () => {
    const cfg = { agents: { list: [{ id: "main", default: true }] } } as ZuvixConfig;
    const store = {
      "agent:main:telegram:direct:42": {
        chatType: "direct",
        channel: "telegram",
        origin: { label: "zuvix-tui" },
        updatedAt: 2,
      } as SessionEntry,
    ...
    const listed = listSessionsFromStore({
      ...
      storePath: "",
      store,
      opts: { search: "zuvix-tui" },
    });

    expect(listed.sessions.map((session) => session.key)).toEqual([
      "agent:main:telegram:direct:42",
    ]);
    expect(listed.sessions[0]?.displayName).toBe("zuvix-tui");
  });
  ...
  test("session rows expose estimated context budget status", () => {
    const row = buildGatewaySessionRow({
      ...
      entry: {
        sessionId: "session-1",
        sessionFile: "/tmp/zuvix/agents/main/sessions/session-1.jsonl",
        updatedAt: 1,
        contextBudgetStatus: {
          schemaVersion: 1,
  ...
  test("buildGatewaySessionRow displayName falls through to origin label for direct sessions", () => {
    ...
    expect(row.displayName).toBe("zuvix-tui");
  });
  ...
  test("buildGatewaySessionRow displayName uses group display name for group sessions", () => {
    ...
    expect(row.displayName).not.toBe("zuvix-tui");
  });
  ...
  test("listAgentsForGateway keeps explicit agents.list scope over disk-only agents (scope boundary)", async () => {
    await withStateDirEnv("zuvix-agent-list-scope-", async ({ stateDir }) => {
...
describe("listSessionsFromStore selected model display", () => {
  test("async list yields during bulk transcript title and last-message hydration", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-sessions-list-yield-"));
    try {
  ...
  test("caps transcript title and last-message hydration for bulk list responses", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-sessions-list-cap-"));
    try {
      const storePath = path.join(tmpDir, "sessions.json");
      const store: Record<string, SessionEntry> = {};
      const now = Date.now();
...
describe("resolveSessionModelIdentityRef", () => {
  const resolveLegacyIdentityRef = (cfg: ZuvixConfig, modelProvider?: string) =>
  ...
  test("infers provider from configured provider catalogs when allowlist is absent", () => {
    ...
    } as unknown as ZuvixConfig["models"];
