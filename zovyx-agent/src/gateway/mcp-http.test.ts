...
describe("mcp loopback server", () => {
  it("passes session, account, message channel, and inbound event headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: jsonHeaders({
        ...
        "x-zuvix-account-id": "work",
        "x-zuvix-message-channel": "telegram",
        "x-zuvix-current-channel-id": "telegram:chat123",
        "x-zuvix-current-thread-ts": "42",
        "x-zuvix-current-message-id": "reply-message-1",
        "x-zuvix-current-inbound-audio": "true",
        "x-zuvix-inbound-event-kind": "room_event",
        "x-zuvix-source-reply-delivery-mode": "message_tool_only",
        "x-zuvix-require-explicit-message-target": "true",
      }),
      body: mcpToolsListBody(),
    });
    ...
    const sendToolsList = async (
      ...
      await sendLoopbackToolsList({
        ...
        headers: {
          "x-session-key": "agent:main:telegram:group:chat123",
          "x-zuvix-message-channel": "telegram",
          "x-zuvix-inbound-event-kind": inboundEventKind,
          ...(sourceReplyDeliveryMode
            ? { "x-zuvix-source-reply-delivery-mode": sourceReplyDeliveryMode }
            : {}),
          ...(currentInboundAudio ? { "x-zuvix-current-inbound-audio": "true" } : {}),
    ...
    const sendToolsList = async (token?: string) =>
      await sendLoopbackToolsList({
        ...
        headers: {
          ...
          "x-zuvix-message-channel": "matrix",
        },
      });
  ...
  it("times out stalled request bodies and closes uploads after flushing 408", async () => {
    const previousTimeout = process.env.ZUVIX_MCP_LOOPBACK_BODY_TIMEOUT_MS;
    process.env.ZUVIX_MCP_LOOPBACK_BODY_TIMEOUT_MS = "20";
    try {
      server = await startMcpLoopbackServer(0);
      const runtime = getActiveMcpLoopbackRuntime();
      if (!runtime) {
      ...
      if (previousTimeout === undefined) {
        delete process.env.ZUVIX_MCP_LOOPBACK_BODY_TIMEOUT_MS;
      } else {
        process.env.ZUVIX_MCP_LOOPBACK_BODY_TIMEOUT_MS = previousTimeout;
      }
    }
  });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.zuvix?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.zuvix?.headers?.Authorization).toBe(
      "Bearer ${ZUVIX_MCP_TOKEN}",
    );
    expect(config.mcpServers?.zuvix?.headers?.["x-zuvix-message-channel"]).toBe(
      "${ZUVIX_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.zuvix?.headers?.["x-zuvix-current-channel-id"]).toBe(
      "${ZUVIX_MCP_CURRENT_CHANNEL_ID}",
    );
    expect(config.mcpServers?.zuvix?.headers?.["x-zuvix-current-thread-ts"]).toBe(
      "${ZUVIX_MCP_CURRENT_THREAD_TS}",
    );
    expect(config.mcpServers?.zuvix?.headers?.["x-zuvix-current-message-id"]).toBe(
      "${ZUVIX_MCP_CURRENT_MESSAGE_ID}",
    );
    expect(config.mcpServers?.zuvix?.headers?.["x-zuvix-current-inbound-audio"]).toBe(
      "${ZUVIX_MCP_CURRENT_INBOUND_AUDIO}",
    );
    expect(config.mcpServers?.zuvix?.headers?.["x-zuvix-source-reply-delivery-mode"]).toBe(
      "${ZUVIX_MCP_SOURCE_REPLY_DELIVERY_MODE}",
    );
    expect(
      config.mcpServers?.zuvix?.headers?.["x-zuvix-require-explicit-message-target"],
    ).toBe("${ZUVIX_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET}");
    expect(config.mcpServers?.zuvix?.headers).not.toHaveProperty("x-zuvix-sender-is-owner");
  });
