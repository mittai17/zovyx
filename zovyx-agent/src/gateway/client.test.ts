...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("zuvix doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.ZUVIX_PROXY_ACTIVE;
    delete process.env.ZUVIX_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with ZUVIX_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ZUVIX_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://zuvix-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { ZUVIX_HOME: "/tmp/custom-zuvix-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      ZUVIX_STATE_DIR: "/tmp/zuvix-client-service-state",
