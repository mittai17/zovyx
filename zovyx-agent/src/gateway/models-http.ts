// OpenAI-compatible `/v1/models` HTTP route backed by configured Zuvix agents.
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([ZUVIX_MODEL_ID, ZUVIX_DEFAULT_MODEL_ID]);
  ids.add(`zuvix/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`zuvix/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== ZUVIX_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
