...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: ZuvixConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride
      ? undefined
      : normalizeOptionalString(process.env.ZUVIX_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.ZUVIX_ALLOW_INSECURE_PRIVATE_WS === "1";
