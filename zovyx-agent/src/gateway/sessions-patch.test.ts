...
const EMPTY_CFG = {} as ZuvixConfig;
...
async function applySubagentModelPatch(cfg: ZuvixConfig) {
  return expectPatchOk(
    await runPatch({
      cfg,
...
function createAllowlistedAnthropicModelCfg(): ZuvixConfig {
  return {
    agents: {
      defaults: {
    ...
    const entry = expectPatchOk(
      await runPatch({
        ...
        loadGatewayModelCatalog: async () => [
          ...
          { provider: "zuvix-go", id: "kimi-k2.6", name: "kimi-k2.6" },
        ],
      }),
    );
    expect(entry.providerOverride).toBe("zuvix-go");
