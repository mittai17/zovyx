...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.ZUVIX_GATEWAY_STARTUP_TRACE);
  let timelineConfig: ZuvixConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.ZUVIX_DIAGNOSTICS_EVENT_LOOP);
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`zuvix config set gateway.auth.mode token\` and \`zuvix config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed Zuvix config and rebuild.",
    "For the first-party Nix flow, see https://github.com/zuvix/nix-zuvix#quick-start and https://docs.zuvix.ai/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing zuvix.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.ZUVIX_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.ZUVIX_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "ZUVIX_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "ZUVIX_RAW_STREAM_PATH",
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: ZuvixConfig) =>
    ...
    const gatewayRequestContext = await startupTrace.measure(
      ...
      async () => {
        ...
        return createGatewayRequestContext({
          ...
          clients,
          enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: ZuvixConfig) => {
            enforceSharedGatewaySessionGenerationForConfigWrite({
