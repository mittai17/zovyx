...
const resolveZuvixPackageRootSync = vi.hoisted(() => vi.fn((_params: unknown) => "/package"));
...
vi.mock("../infra/zuvix-root.js", () => ({
  resolveZuvixPackageRootSync: (params: unknown) => resolveZuvixPackageRootSync(params),
}));
...
function slackConfig(): ZuvixConfig {
  return {
    channels: {
...
describe("prepareGatewayPluginBootstrap startup plugins", () => {
  beforeEach(() => {
    ...
    resolveZuvixPackageRootSync.mockClear().mockReturnValue("/package");
...
describe("warnUnregisteredConfiguredMemoryEmbeddingProviders", () => {
  ...
  function customOllamaConfig(source: "provider" | "fallback" = "provider"): ZuvixConfig {
    const memorySearch =
      source === "provider"
