...
const ORIGINAL_GATEWAY_TOKEN_ENV = process.env.ZUVIX_GATEWAY_TOKEN;
const OLD_TOKEN = "shared-token-old";
const NEW_TOKEN = "shared-token-new";
const DEFERRED_RESTART_DELAY_MS = 1_000;
const SECRET_REF_TOKEN_ID = "ZUVIX_SHARED_AUTH_ROTATION_SECRET_REF";
...
afterAll(() => {
  ...
  if (ORIGINAL_GATEWAY_TOKEN_ENV === undefined) {
    ...
    process.env.ZUVIX_GATEWAY_TOKEN = ORIGINAL_GATEWAY_TOKEN_ENV;
  }
});
...
async function openDeviceTokenWsWithDetails(
  ...
  const identityPath = path.join(os.tmpdir(), `zuvix-shared-auth-${process.pid}-${port}.json`);
