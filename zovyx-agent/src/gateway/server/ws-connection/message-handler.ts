...
/**
 * Lazily resolve the local node host's nodeId from ~/.zuvix/node.json.
 * Process-stable: only changes on `zuvix node install`, which requires restart.
 */
...
function resolveTrustedProxyControlUiScopes(params: {
  ...
  const rawHeader = firstHeaderValue(params.upgradeReq.headers["x-zuvix-scopes"]);
  ...
  const handleMessage = async (data: RawData) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // matching instanceId against ~/.zuvix/node.json nodeId). SSH-tunneled remote
