...
async function startMemoryBackendForTest(cfg: ZuvixConfig) {
  const log = createGatewayLogMock();
  await startGatewayMemoryBackend({ cfg, log });
  return log;
}

async function startQmdBackendWithManager(cfg: ZuvixConfig) {
...
function expectQmdManagerRequests(cfg: ZuvixConfig, agentIds: string[]) {
