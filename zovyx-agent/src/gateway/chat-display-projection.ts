...
import { asFiniteNumber } from "@zuvix/normalization-core/number-coercion";
import { asOptionalRecord as readRecord } from "@zuvix/normalization-core/record-coerce";
import { normalizeOptionalString } from "@zuvix/normalization-core/string-coerce";
import { ZUVIX_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../agents/internal-runtime-context.js";
...
import { isZuvixDeliveryMirrorAssistantMessage } from "../shared/transcript-only-zuvix-assistant.js";
...
function sanitizeChatHistoryContentBlock(
  ...
  if ("zuvixReasoningReplay" in entry) {
    delete entry.zuvixReasoningReplay;
    changed = true;
  }
...
function buildMessageToolVisibleReplyMirror(
  ...
  const transcriptMeta = readRecord((pending.completionAnchor ?? pending.anchor)["__zuvix"]);
  if (transcriptMeta) {
    mirror["__zuvix"] = { ...transcriptMeta };
  }
  return mirror;
}
...
function readMessageToolDeliveryMirrorText(message: Record<string, unknown>): string | undefined {
  ...
  if (!isZuvixDeliveryMirrorAssistantMessage(message)) {
    return undefined;
  }
...
function readTtsSupplementMarker(
  ...
  const marker = message.zuvixTtsSupplement;
...
function readChatHistoryRecordTimestampMs(message: unknown): number | undefined {
  const meta = readRecord(readRecord(message)?.["__zuvix"]);
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === ZUVIX_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function zuvixAssistantModel(message: Record<string, unknown>): string | undefined {
  return message.role === "assistant" &&
    message.provider === "zuvix" &&
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    zuvixAssistantModel(previousVisible) !== "acp-runtime" ||
    zuvixAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }
