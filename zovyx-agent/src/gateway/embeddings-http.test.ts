...
const WRITE_SCOPE_HEADER = { "x-zuvix-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-zuvix-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("zuvix/default");
  ...
  it("rejects explicit unknown agent ids", async () => {
    try {
      ...
      const header = await postEmbeddings(
        { model: "zuvix/default", input: "hello" },
        { "x-zuvix-agent-id": "missing-agent" },
      );
      await expectInvalidEmbeddingRequest(header, "Unknown agent 'missing-agent'.");

      const model = await postEmbeddings({ model: "zuvix/missing-agent", input: "hello" });
  ...
  it("rejects disallowed x-zuvix-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "zuvix/default",
        input: "hello",
      },
      { "x-zuvix-model": "ollama/nomic-embed-text" },
    );
    await expectInvalidEmbeddingRequest(
      res,
  ...
  it("rejects x-zuvix-model for trusted write-only callers", async () => {
