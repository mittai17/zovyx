...
describe("gateway auth", () => {
  ...
  it("resolves token/password from ZUVIX gateway env vars", () => {
    const auth = resolveGatewayAuth({
      authConfig: {},
      env: {
        ZUVIX_GATEWAY_TOKEN: "env-token",
        ZUVIX_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides ZUVIX_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        ZUVIX_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
