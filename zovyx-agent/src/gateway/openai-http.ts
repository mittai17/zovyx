...
// Translates OpenAI chat requests to Zuvix agent runs and SSE/JSON responses.
import { randomUUID } from "node:crypto";
import type { IncomingMessage, ServerResponse } from "node:http";
import { estimateBase64DecodedBytes } from "@zuvix/media-core/base64";
...
function resolveAgentResponseText(result: unknown): string {
  ...
  if (!Array.isArray(payloads) || payloads.length === 0) {
    return "No response from Zuvix.";
  }
  const content = payloads
    .map((p) => (typeof p.text === "string" ? p.text : ""))
    .filter(Boolean)
    .join("\n\n");
  return content || "No response from Zuvix.";
}
...
export async function handleOpenAiHttpRequest(
  ...
  const model = typeof payload.model === "string" ? payload.model : "zuvix";
