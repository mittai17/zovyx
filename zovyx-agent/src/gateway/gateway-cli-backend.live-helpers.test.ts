...
describe("gateway cli backend live helpers", () => {
  ...
  afterEach(() => {
    ...
    delete process.env.ZUVIX_SKIP_CHANNELS;
    delete process.env.ZUVIX_SKIP_PROVIDERS;
    delete process.env.ZUVIX_SKIP_GMAIL_WATCHER;
    delete process.env.ZUVIX_SKIP_CRON;
    delete process.env.ZUVIX_SKIP_CANVAS_HOST;
    delete process.env.ZUVIX_SKIP_BROWSER_CONTROL_SERVER;
    delete process.env.ZUVIX_BUNDLED_PLUGINS_DIR;
    delete process.env.ZUVIX_TEST_MINIMAL_GATEWAY;
    delete process.env.ZUVIX_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP;
    delete process.env.ZUVIX_LIVE_CLI_BACKEND_ADVISORY;
  ...
  it("applies and restores live env including minimal gateway mode", async () => {
    ...
    process.env.ZUVIX_SKIP_CHANNELS = "old-channels";
    process.env.ZUVIX_SKIP_PROVIDERS = "old-providers";
    process.env.ZUVIX_SKIP_GMAIL_WATCHER = "old-gmail";
    process.env.ZUVIX_SKIP_CRON = "old-cron";
    process.env.ZUVIX_SKIP_CANVAS_HOST = "old-canvas";
    process.env.ZUVIX_SKIP_BROWSER_CONTROL_SERVER = "old-browser";
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR = "old-bundled";
    process.env.ZUVIX_TEST_MINIMAL_GATEWAY = "old-minimal";
    ...
    expect(process.env.ZUVIX_SKIP_CHANNELS).toBe("1");
    expect(process.env.ZUVIX_SKIP_PROVIDERS).toBe("1");
    expect(process.env.ZUVIX_SKIP_GMAIL_WATCHER).toBe("1");
    expect(process.env.ZUVIX_SKIP_CRON).toBe("1");
    expect(process.env.ZUVIX_SKIP_CANVAS_HOST).toBe("1");
    expect(process.env.ZUVIX_SKIP_BROWSER_CONTROL_SERVER).toBe("1");
    expect(process.env.ZUVIX_BUNDLED_PLUGINS_DIR).toBe("old-bundled");
    expect(process.env.ZUVIX_TEST_MINIMAL_GATEWAY).toBe("1");
    expect(process.env.ANTHROPIC_API_KEY).toBeUndefined();
    expect(process.env.ANTHROPIC_API_KEY_OLD).toBeUndefined();

    restoreCliBackendLiveEnv(snapshot);

    expect(process.env.ZUVIX_SKIP_CHANNELS).toBe("old-channels");
    expect(process.env.ZUVIX_SKIP_PROVIDERS).toBe("old-providers");
    expect(process.env.ZUVIX_SKIP_GMAIL_WATCHER).toBe("old-gmail");
    expect(process.env.ZUVIX_SKIP_CRON).toBe("old-cron");
    expect(process.env.ZUVIX_SKIP_CANVAS_HOST).toBe("old-canvas");
    expect(process.env.ZUVIX_SKIP_BROWSER_CONTROL_SERVER).toBe("old-browser");
    expect(process.env.ZUVIX_BUNDLED_PLUGINS_DIR).toBe("old-bundled");
    expect(process.env.ZUVIX_TEST_MINIMAL_GATEWAY).toBe("old-minimal");
  ...
  it("defaults the model switch probe to Claude Sonnet -> Opus", async () => {
    ...
    delete process.env.ZUVIX_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE;
  ...
  it("lets env disable the model switch probe", async () => {
    ...
    process.env.ZUVIX_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE = "0";
  ...
  it("requires provider results by default for explicit CLI backend live probes", async () => {
    ...
    expect(
      ...
      action: "fail",
      message:
        'agent request for provider "claude-cli" was blocked by auth drift. Set ZUVIX_LIVE_CLI_BACKEND_ADVISORY=1 and ZUVIX_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP=1 only for advisory live probes.',
    });
  ...
  it("allows live env overrides for fresh and resume CLI args", async () => {
    ...
    process.env.ZUVIX_LIVE_CLI_BACKEND_ARGS = JSON.stringify([
      "exec",
      "--sandbox",
      "danger-full-access",
    ]);
    process.env.ZUVIX_LIVE_CLI_BACKEND_RESUME_ARGS = JSON.stringify([
      "exec",
      "resume",
      "{sessionId}",
      "-c",
