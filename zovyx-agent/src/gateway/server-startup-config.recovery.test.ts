...
import type { ConfigFileSnapshot, ModelDefinitionConfig, ZuvixConfig } from "../config/types.js";
import type { PluginMetadataSnapshot } from "../plugins/plugin-metadata-snapshot.js";
import { buildTestConfigSnapshot } from "./test-helpers.config-snapshots.js";

const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: ZuvixConfig }) => ({
...
vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: ZuvixConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: ZuvixConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/zuvix-startup-recovery.json";
...
function buildRuntimeSnapshot(
  ...
  runtimeConfig: ZuvixConfig = sourceConfig,
): ConfigFileSnapshot {
  return {
...
function expectPluginAutoEnableFor(config: ZuvixConfig) {
...
function mockRuntimeAutoEnable(config: ZuvixConfig) {
...
function buildInvalidConfigSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: false,
    config: params.config ?? (params.rawConfig as ZuvixConfig),
...
describe("gateway startup config validation", () => {
  ...
  it("does not suggest doctor repair for plugin packaging compiled-output failures", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as ZuvixConfig,
      issues: [
        {
    ...
    await start.catch((error: unknown) => {
      expect(String(error)).not.toContain("zuvix doctor --fix");
    });
  });
  ...
  it("keeps doctor repair guidance for mixed plugin packaging and core invalidity", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as unknown as ZuvixConfig,
      issues: [
        {
          path: "plugins.slots.memory",
          message: "plugin not found: source-only-pack",
    ...
    await expectStartupRejects('Run "zuvix doctor --fix" to repair, then retry.');
  });
      ...
      issues: [
        {
          ...
          message:
            "plugin feishu: plugin requires Zuvix >=2026.4.23, but this host is 2026.4.22; skipping load",
        },
      ],
    });
    vi.mocked(configIo.readConfigFileSnapshot).mockResolvedValueOnce(invalidSnapshot);
