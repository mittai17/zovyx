...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.ZUVIX_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.ZUVIX_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: ZuvixConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("ZUVIX_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-zuvix-message-channel")) ?? undefined,
    currentChannelId: normalizeOptionalString(getHeader(req, "x-zuvix-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-zuvix-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-zuvix-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-zuvix-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-zuvix-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-zuvix-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-zuvix-source-reply-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-zuvix-require-explicit-message-target"),
    ),
