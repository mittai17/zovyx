...
import { clearActivatedPluginRuntimeState, loadZuvixPlugins } from "../plugins/loader.js";
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "zuvix.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "zuvix.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: ZuvixConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.zuvix.ai/plugins/sdk-runtime#api-runtime-subagent and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadZuvixPlugins({
    config: resolvedConfig,
