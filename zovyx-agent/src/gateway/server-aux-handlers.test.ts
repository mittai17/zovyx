...
function createSnapshot(config: ZuvixConfig): PreparedSecretsRuntimeSnapshot {
  return {
...
function activateSnapshot(config: ZuvixConfig) {
  activateSecretsRuntimeSnapshot(createSnapshot(config));
}

function mockResolvedSecrets(config: ZuvixConfig) {
...
// set ZUVIX_SKIP_CHANNELS / ZUVIX_SKIP_PROVIDERS at module load. When a
