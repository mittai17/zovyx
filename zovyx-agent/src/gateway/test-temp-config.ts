...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "zuvix-test-owner-display-secret",
    },
  };
}

/** Writes a temp Zuvix config, installs it as runtime state, then restores globals. */
export async function withTempConfig(params: {
  cfg: unknown;
  run: () => Promise<void>;
  prefix?: string;
}): Promise<void> {
  const prevConfigPath = process.env.ZUVIX_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as ZuvixConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "zuvix-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.ZUVIX_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
