...
type CreateZuvixToolsArg = {
  inheritedToolDenylist?: string[];
  pluginToolDenylist?: string[];
};

const hoisted = vi.hoisted(() => ({
  createZuvixToolsMock: vi.fn((_args: CreateZuvixToolsArg) => [
    {
      name: "read",
...
vi.mock("../agents/zuvix-tools.js", () => ({
  createZuvixTools: (args: CreateZuvixToolsArg) => hoisted.createZuvixToolsMock(args),
}));
...
describe("resolveGatewayScopedTools excludeToolNames", () => {
  beforeEach(() => {
    hoisted.createZuvixToolsMock.mockClear();
  });
  ...
  function readCreateToolsArgs(): {
    ...
    const args = hoisted.createZuvixToolsMock.mock.calls[0]?.[0];
    if (!args || typeof args !== "object") {
      throw new Error("expected createZuvixTools args");
    }
    return args as {
      inheritedToolDenylist?: string[];
      pluginToolDenylist?: string[];
