...
function asPluginInstallConfig(records: PluginInstallRecords): ZuvixConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: ZuvixConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: ZuvixConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: ZuvixConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
