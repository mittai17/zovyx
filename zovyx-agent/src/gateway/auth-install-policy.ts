...
type GatewayInstallAuthMode = NonNullable<NonNullable<ZuvixConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: ZuvixConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.ZUVIX_GATEWAY_PASSWORD) ||
