...
type GatewayStartupStateEmitterMock = ReturnType<
  typeof vi.fn<(code: string, message: string, cfg: ZuvixConfig) => void>
>;

const RESOLVED_GATEWAY_TOKEN = "resolved-gateway-token";

function gatewayTokenConfig(config: ZuvixConfig): ZuvixConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function asConfig(value: unknown): ZuvixConfig {
  return value as ZuvixConfig;
}

function buildSnapshot(config: ZuvixConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/zuvix-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: ZuvixConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
function runtimeSecretsActivatorOptionsForTest() {
  return {
    ...
    emitStateEvent: vi.fn<(code: string, message: string, cfg: ZuvixConfig) => void>(),
  };
}
...
function installDiagnosticsTimelineEnv() {
  const root = mkdtempSync(path.join(tmpdir(), "zuvix-startup-secrets-timeline-"));
  const timelinePath = path.join(root, "timeline.jsonl");
  const previousDiagnostics = process.env.ZUVIX_DIAGNOSTICS;
  const previousTimelinePath = process.env.ZUVIX_DIAGNOSTICS_TIMELINE_PATH;
  process.env.ZUVIX_DIAGNOSTICS = "timeline";
  process.env.ZUVIX_DIAGNOSTICS_TIMELINE_PATH = timelinePath;

  return {
    timelinePath,
    cleanup: () => {
      if (previousDiagnostics === undefined) {
        delete process.env.ZUVIX_DIAGNOSTICS;
      } else {
        process.env.ZUVIX_DIAGNOSTICS = previousDiagnostics;
      }
      if (previousTimelinePath === undefined) {
        delete process.env.ZUVIX_DIAGNOSTICS_TIMELINE_PATH;
      } else {
        process.env.ZUVIX_DIAGNOSTICS_TIMELINE_PATH = previousTimelinePath;
      }
...
async function activateImportedStartupConfig(config: ZuvixConfig) {
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.ZUVIX_SKIP_CHANNELS;
  const previousSkipProviders = process.env.ZUVIX_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.ZUVIX_SKIP_CHANNELS;
    } else {
      process.env.ZUVIX_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.ZUVIX_SKIP_PROVIDERS;
    } else {
      process.env.ZUVIX_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
  ...
  it("activates no-SecretRef startup config without importing the full secrets runtime", async () => {
    vi.resetModules();
    const agentDir = mkdtempSync(path.join(tmpdir(), "zuvix-startup-fast-path-"));
    const runtimeImport = vi.fn();
    const prepareRuntimeSecretsSnapshot = vi.fn(async ({ config }) => preparedSnapshot(config));
    const activateRuntimeSecretsSnapshot = vi.fn();
  ...
  it("keeps the full secrets runtime path when startup config has a SecretRef", async () => {
    const harness = createGatewayStartupSecretsRuntimeHarness("zuvix-startup-secret-ref-");
  ...
  it("keeps the full secrets runtime path when auth profile files are present", async () => {
    const harness = createGatewayStartupSecretsRuntimeHarness("zuvix-startup-auth-store-");
    writeFileSync(
