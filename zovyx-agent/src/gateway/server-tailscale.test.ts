...
describe("startGatewayTailscaleExposure preserveFunnel", () => {
  ...
  it("passes serviceName through to Tailscale Serve setup and cleanup", async () => {
    ...
    expect(mocks.enableTailscaleServe).toHaveBeenCalledWith(18789, undefined, "svc:zuvix");
    expect(logTailscale.info).toHaveBeenCalledWith(
      "serve enabled for svc:zuvix: https://zuvix.tailnet.ts.net/ (WS via wss://zuvix.tailnet.ts.net)",
    );

    await cleanup?.();

    expect(mocks.disableTailscaleServe).toHaveBeenCalledWith(undefined, "svc:zuvix");
  });
