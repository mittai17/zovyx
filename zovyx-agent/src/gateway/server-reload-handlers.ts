...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: ZuvixConfig) => ChannelHealthMonitor | null;
...
> & {
  ...
  initialConfig: ZuvixConfig;
  initialCompareConfig?: ZuvixConfig;
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: ZuvixConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const markActiveMainSessionsForRestart = async (nextConfig: ZuvixConfig, reason: string) => {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: ZuvixConfig) => {
            ...
            await startGmailWatcherWithLogs({
              ...
              onSkipped: () =>
                params.logHooks.info(
                  "skipping gmail watcher restart (ZUVIX_SKIP_GMAIL_WATCHER=1)",
                ),
            });
          }
        }
    ...
    if (channelsToRestart.size > 0) {
      if (shouldSkipChannelRestart()) {
        params.logChannels.info(
          "skipping channel reload (ZUVIX_SKIP_CHANNELS=1 or ZUVIX_SKIP_PROVIDERS=1)",
        );
      } else {
        if (!plan.reloadPlugins) {
          await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig);
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: ZuvixConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
