...
/** Formats the Bonjour instance name while preserving user-provided Zuvix names. */
export function formatBonjourInstanceName(displayName: string) {
  const trimmed = displayName.trim();
  if (!trimmed) {
    return "Zuvix";
  }
  if (/zuvix/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (Zuvix)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const envPath = env.ZUVIX_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "zuvix");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.ZUVIX_TAILNET_DNS?.trim();
