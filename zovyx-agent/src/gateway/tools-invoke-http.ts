...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-zuvix-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-zuvix-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-zuvix-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-zuvix-thread-id"));
