...
// Converts Claude project JSONL into Zuvix transcript-compatible messages.
...
import { attachZuvixTranscriptMeta } from "./session-utils.fs.js";
...
function normalizeClaudeCliContent(
  ...
  for (const item of content) {
    ...
    if (type === "tool_use") {
      // Claude stores tool calls as `tool_use` with `input`; Zuvix history
...
function parseClaudeCliHistoryEntry(
  ...
  if (type === "user") {
    return attachZuvixTranscriptMeta(
      {
        role: "user",
        content,
