...
/** Attach Zuvix metadata to a transcript message without dropping existing metadata. */
export function attachZuvixTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record["__zuvix"] &&
    typeof record["__zuvix"] === "object" &&
    !Array.isArray(record["__zuvix"])
      ? (record["__zuvix"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    __zuvix: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __zuvix: { truncated: true, reason: "oversized" },
    },
  };
  return {
    ...(id ? { id } : {}),
...
export async function readSessionMessagesAsync(
  ...
  const result = await readSessionMessagesWithSourceAsync(sessionId, storePath, sessionFile, opts);
...
export async function readSessionMessagesWithSourceAsync(
  ...
  const filePath =
    opts.allowResetArchiveFallback === true
      ? await findExistingTranscriptHistoryPathAsync(sessionId, storePath, sessionFile)
      : findExistingTranscriptPath(sessionId, storePath, sessionFile);
  if (!filePath) {
    return { messages: [] };
  }
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachZuvixTranscriptMeta(message, { seq: firstSeq + index }),
  );
    ...
    const recordTimestampMs =
      typeof entry.timestamp === "string"
        ...
        : typeof entry.timestamp === "number"
          ? entry.timestamp
          : Number.NaN;
    return attachZuvixTranscriptMeta(entry.message, {
...
function extractTranscriptTokenEstimateFromLine(line: string): {
  ...
  try {
    ...
    const isDeliveryMirror =
      role === "assistant" && modelProvider === "zuvix" && model === "delivery-mirror";
    if (isDeliveryMirror) {
      return null;
    }
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "zuvix" && model === "delivery-mirror";
