...
let lastCreateZuvixToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/zuvix-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateZuvixToolsContext?.agentTo,
          agentThreadId: lastCreateZuvixToolsContext?.agentThreadId,
        },
        inheritedToolDenylist: lastCreateZuvixToolsContext?.inheritedToolDenylist,
      }),
    },
    {
      name: "sessions_send",
  ...
  return {
    createZuvixTools: (ctx: Record<string, unknown>) => {
      lastCreateZuvixToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateZuvixToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-zuvix-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-zuvix-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateZuvixToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateZuvixToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateZuvixToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateZuvixToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateZuvixToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateZuvixToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateZuvixToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-zuvix-message-to": "channel:24514",
        "x-zuvix-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("keeps shared-secret bearer auth as owner for explicitly allowed owner-only tools", async () => {
    ...
    expect(lastCreateZuvixToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateZuvixToolsContext?.senderIsOwner).toBe(false);
