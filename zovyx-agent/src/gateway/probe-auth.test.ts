...
function configWithDefaultEnvProvider(gateway: NonNullable<ZuvixConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: ZuvixConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: ZuvixConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("ZUVIX_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        ZUVIX_GATEWAY_TOKEN: "test-token-from-env",
