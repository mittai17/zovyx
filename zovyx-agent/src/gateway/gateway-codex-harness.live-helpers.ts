...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Zuvix session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsZuvixStatus =
    normalized.includes("zuvix is running on") ||
    /zuvix\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("zuvix status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsZuvixStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from zuvix status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live zuvix config shows") ||
