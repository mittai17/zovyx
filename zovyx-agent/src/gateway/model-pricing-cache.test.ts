...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listZuvixPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...
    listZuvixPluginManifestMetadata: (
      params?: Parameters<typeof actual.listZuvixPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listZuvixPluginManifestMetadata(params);
      return actual.listZuvixPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listZuvixPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as ZuvixConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listZuvixPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/zuvix.plugin.json",
    ...overrides,
  };
}
