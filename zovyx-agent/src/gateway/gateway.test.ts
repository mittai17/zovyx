...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "zuvix-test-empty-bundled-plugins");
  await fs.mkdir(bundledPluginsDir, { recursive: true });
  return bundledPluginsDir;
}

async function createGatewayConfigPath(tempHome: string): Promise<string> {
  const configPath = path.join(tempHome, ".zuvix", "zuvix.json");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".zuvix", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["ZUVIX_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "zuvix");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.ZUVIX_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "zuvix-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-minimal-gateway-home-"));
      const configPath = await createGatewayConfigPath(tempHome);
      const bundledPluginsDir = path.join(tempHome, "zuvix-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.ZUVIX_STATE_DIR = path.join(tempHome, ".zuvix");
      process.env.ZUVIX_CONFIG_PATH = configPath;
      process.env.ZUVIX_SKIP_CHANNELS = "1";
