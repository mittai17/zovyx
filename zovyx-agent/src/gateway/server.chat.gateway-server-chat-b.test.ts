...
async function withGatewayChatHarness(
  ...
  const createSessionDir = async () => {
    const sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-gw-"));
  ...
  try {
    ...
    if (process.env.ZUVIX_CONFIG_PATH) {
      await fs.rm(process.env.ZUVIX_CONFIG_PATH, { force: true });
    }
    clearConfigCache();
...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("ZUVIX_CONFIG_PATH missing in gateway test environment");
  }
...
describe("gateway server chat", () => {
  ...
  test("chat.send starts the next WebChat turn after the prior internal run finishes", async () => {
    ...
    try {
      ...
      expect(dispatchOptions[0]?.promptCacheKey).toEqual(
        expect.stringMatching(/^zuvix-webchat-[a-f0-9]{32}$/u),
      );
  ...
  test("chat.send diagnostics timeline carries run correlation attributes", async () => {
    const timelineDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-chat-timeline-"));
  ...
  test("chat.history hard-caps single oversized nested payloads", async () => {
    await withGatewayChatHarness(async ({ ws, createSessionDir }) => {
      ...
      expect(messages[0]).toMatchObject({
        __zuvix: { id: "msg-huge", truncated: true, reason: "oversized" },
      });
