...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: ZuvixConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: ZuvixConfig) => {
...
function hasActiveGatewayAuthSecretRef(config: ZuvixConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: ZuvixConfig): ZuvixConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: ZuvixConfig): void {
