...
type ConfiguredBinding = NonNullable<ZuvixConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: ZuvixConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/zuvix" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
