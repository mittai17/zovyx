#!/usr/bin/env node
/** ACP stdio server that bridges Agent Client Protocol clients to the Zuvix Gateway. */
...
function printHelp(): void {
  console.log(`Usage: zuvix acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or ZUVIX_GATEWAY_TOKEN.",
    );
  }
