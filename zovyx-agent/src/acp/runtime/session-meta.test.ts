...
describe("ACP session metadata SQLite store", () => {
  ...
  it("persists ACP metadata in SQLite without writing sessions.json acp blocks", async () => {
    await withTempDir({ prefix: "zuvix-acp-meta-" }, async (dir) => {
      const storePath = path.join(dir, "sessions.json");
      const databasePath = path.join(dir, "state", "zuvix.sqlite");
  ...
  it("honors ZUVIX_STATE_DIR when joining listed SQLite rows to session stores", async () => {
    await withTempDir({ prefix: "zuvix-acp-meta-" }, async (dir) => {
      const env = { ...process.env, ZUVIX_STATE_DIR: dir } as NodeJS.ProcessEnv;
