...
const createTempDir = () => tempDirs.make("zuvix-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets ZUVIX_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "zuvix",
    });

    expect(env.ZUVIX_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("zuvix");
  });

  it("overrides pre-existing ZUVIX_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      ZUVIX_SHELL: "wrong",
    });
    expect(env.ZUVIX_SHELL).toBe("acp-client");
  ...
  it("preserves ZUVIX_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        ZUVIX_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["ZUVIX_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Zuvix bridge", () => {
    ...
    expect(env.ZUVIX_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.ZUVIX_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("ZUVIX_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "zuvix", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "zuvix", "dist", "entry.js");
    const shimPath = path.join(dir, "zuvix.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\zuvix\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/zuvix-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/zuvix-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/zuvix-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
