...
import { readBool, readNonNegativeInteger, readString } from "@zuvix/acp-core/meta";
import { defaultAcpSessionStore, type AcpSessionStore } from "@zuvix/acp-core/session";
import { toAcpSessionLineageMeta } from "@zuvix/acp-core/session-lineage-meta";
...
/** ACP Agent implementation backed by the Zuvix Gateway and replay ledger. */
export class AcpGatewayAgent implements Agent {
  ...
  private assertSupportedSessionSetup(mcpServers: ReadonlyArray<unknown>): void {
    ...
    throw new Error(
      "ACP bridge mode does not support per-session MCP servers. Configure MCP on the Zuvix gateway or agent instead.",
    );
  }
