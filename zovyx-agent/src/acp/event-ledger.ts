...
import { resolveIntegerOption } from "@zuvix/acp-core/numeric-options";
...
/** Resolves the legacy file-backed ACP ledger path under the Zuvix state directory. */
...
export async function migrateFileAcpEventLedgerToSqlite(
  params: { filePath: string; archiveSource?: boolean } & ZuvixStateDatabaseOptions,
...
export function createSqliteAcpEventLedger(
  params: ZuvixStateDatabaseOptions & LedgerOptions = {},
): AcpEventLedger {
  const normalized = normalizeLedgerOptions(params);
  ...
  const mutate = (fn: (db: DatabaseSync) => void) =>
    runZuvixStateWriteTransaction((database) => fn(database.db), dbOptions);
  const read = <T>(fn: (db: DatabaseSync) => T): T => fn(openZuvixStateDatabase(dbOptions).db);
