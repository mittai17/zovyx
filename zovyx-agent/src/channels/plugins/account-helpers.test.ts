...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): ZuvixConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as ZuvixConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: ZuvixConfig) => string[];
  input: ZuvixConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: ZuvixConfig, expected: string) {
