...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: ZuvixConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: ZuvixConfig }>(
  collector: WarningCollector<{ cfg: ZuvixConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: ZuvixConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: ZuvixConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: ZuvixConfig },
>(
  projectCfg: (cfg: ZuvixConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: ZuvixConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: ZuvixConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: ZuvixConfig },
>(params: {
  providerConfigPresent: (cfg: ZuvixConfig) => boolean;
