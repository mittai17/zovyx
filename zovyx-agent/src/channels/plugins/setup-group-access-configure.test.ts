...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: ZuvixConfig, policy: ChannelAccessPolicy) => ZuvixConfig;
  resolveAllowlist: (params: { cfg: ZuvixConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: ZuvixConfig; resolved: TResolved }) => ZuvixConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: ZuvixConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: ZuvixConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: ZuvixConfig, policy: ChannelAccessPolicy): ZuvixConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: ZuvixConfig, policy: ChannelAccessPolicy): ZuvixConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: ZuvixConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: ZuvixConfig; resolved: string[] }) => {
      calls.push("apply");
