...
import type { ZuvixPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["ZUVIX_PLUGIN_CATALOG_PATHS", "ZUVIX_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = uniqueStrings(
    [
      resolveZuvixPackageRootSync({ cwd: process.cwd() }),
      resolveZuvixPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<ZuvixPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
