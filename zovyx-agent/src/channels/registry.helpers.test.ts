...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://zuvix.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("zuvix-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["zuvix-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("zuvix-weixin");
    expect(getRegisteredChannelPluginMeta("ZUVIX-WEIXIN")?.aliases).toEqual(["weixin"]);
  });
