 ...
 *
 * Stores, claims, completes, and tombstones inbound channel events in Zuvix state.
 */
...
import type {
  ...
  DB as ZuvixStateKyselyDatabase,
} from "../../state/zuvix-state-db.generated.js";
import {
...
type ChannelIngressDatabase = Pick<ZuvixStateKyselyDatabase, "channel_ingress_events">;
...
function openStateDatabase(stateDir?: string) {
  return openZuvixStateDatabase({
    env: stateDir ? { ...process.env, ZUVIX_STATE_DIR: stateDir } : process.env,
  });
}
...
/** Creates a durable channel/account-scoped ingress queue backed by the Zuvix state database. */
