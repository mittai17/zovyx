...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "Zuvix ignores provider credential environment variables from workspace `.env` files",
    );
    expect(markdown).toContain("~/.zuvix/.env");
    expect(markdown).toContain("$ZUVIX_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("ZUVIX_LOAD_SHELL_ENV=1");
