...
function requireFirstEmbeddedAgentRequest(): {
  ...
  if (!call) {
    throw new Error("expected embedded Zuvix agent extraction request");
  }
  const [request] = call;
...
describe("commitment extraction runtime", () => {
  ...
  async function createConfig(): Promise<ZuvixConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-commitment-runtime-"));
    tmpDirs.push(tmpDir);
    stateDirEnvSnapshot ??= captureEnv(["ZUVIX_STATE_DIR"]);
    setTestEnvValue("ZUVIX_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },
