...
function resolveInstallerVersionCases(params: { stdinCwd: string }): string[] {
  const output = execFileSync(
    ...
    [
      "-c",
      `${versionHelperSource}
fake_zuvix_decorated() { printf '%s\\n' 'Zuvix 2026.3.10 (abcdef0)'; }
fake_zuvix_raw() { printf '%s\\n' "Zuvix dev's build"; }
ZUVIX_BIN=fake_zuvix_decorated resolve_zuvix_version
ZUVIX_BIN=fake_zuvix_raw resolve_zuvix_version
(
  cd "$1"
  source /dev/stdin <<'ZUVIX_STDIN_INSTALLER'
${versionHelperSource}
fake_zuvix_stdin() { printf '%s\\n' 'Zuvix 2026.3.10 (abcdef0)'; }
ZUVIX_BIN=fake_zuvix_stdin
resolve_zuvix_version
ZUVIX_STDIN_INSTALLER
)`,
      "zuvix-version-test",
      params.stdinCwd,
    ],
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const hostileCwd = makeTempDir(tempRoots, "zuvix-install-stdin-");
...
`,
      ...
      ).toEqual(["2026.3.10", "Zuvix dev's build", "2026.3.10"]);
    },
  );
});
