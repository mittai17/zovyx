...
describe("exportTrajectoryBundle", () => {
  it("sanitizes session ids in default export directory names", () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".zuvix",
        "trajectory-exports",
        "zuvix-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
    ...
    fs.writeFileSync(
      ...
      [
        ...
        },
        {
          ...
          sessionId: "session-1",
          data: {
            harness: { type: "zuvix", token: rawSecrets[3] },
            metadata: {
  ...
  it("uses the recorded runtime pointer before current environment overrides", async () => {
    ...
    const previous = process.env.ZUVIX_TRAJECTORY_DIR;
    process.env.ZUVIX_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = await exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        ...
        process.env.ZUVIX_TRAJECTORY_DIR = previous;
      }
    }
  });
  ...
  it("exports merged runtime and transcript events plus convenience files", async () => {
    ...
    const runtimeEvents: TrajectoryEvent[] = [
      ...
      },
      {
        ...
        sessionId: "session-1",
        data: {
          harness: { type: "zuvix", version: "0.1.0" },
