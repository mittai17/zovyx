...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/zuvix.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.zuvix/zuvix.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          ZUVIX_STATE_DIR: "/Users/tester/.zuvix",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$ZUVIX_STATE_DIR/zuvix.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/zuvix.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.zuvix" };
