...
import { normalizeCsvOrLooseStringList } from "@zuvix/normalization-core/string-normalization";
...
/** Parses the JSON5 Zuvix manifest block embedded inside a string frontmatter field. */
export function resolveZuvixManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type ZuvixManifestRequires = {
...
/** Extracts normalized runtime requirement lists from an Zuvix manifest block. */
export function resolveZuvixManifestRequires(
  metadataObj: Record<string, unknown>,
): ZuvixManifestRequires | undefined {
  const requiresRaw =
...
export function resolveZuvixManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an Zuvix manifest block. */
export function resolveZuvixManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedZuvixManifestInstallBase = {
...
export function parseZuvixManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedZuvixManifestInstallBase | undefined {
  ...
  const spec: ParsedZuvixManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyZuvixManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedZuvixManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
