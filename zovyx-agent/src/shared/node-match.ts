...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current Zuvix nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentZuvixClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("zuvix-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentZuvixClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  ...
  // During Clawdbot -> Zuvix migration, a unique current client should win only
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentZuvixClient(node.clientId)) {
    score += 10;
