...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  Zuvix  ", 20)).toBe("Zuvix");
    expect(coerceIdentityValue("  Zuvix  ", 8)).toBe("Zuvix");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  Zuvix Assistant  ", 8)).toBe("Zuvix");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  Zuvix  ", 0)).toBe("");
    expect(coerceIdentityValue("  Zuvix  ", -1)).toBe("OpenCla");
  });
});
