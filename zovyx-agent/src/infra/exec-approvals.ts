...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.zuvix";
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.ZUVIX_STATE_DIR?.trim()
    ? `$ZUVIX_STATE_DIR/${EXEC_APPROVALS_FILE}`
...
function hasUnmigratedLegacyExecApprovals(filePath: string): boolean {
  if (!process.env.ZUVIX_STATE_DIR?.trim()) {
    return false;
  }
...
function isReadOnlySecurityAuditSuppressionInspection(argv: string[]): boolean {
  ...
  let offset = command === "pnpm" && argv[1] === "zuvix" ? 1 : 0;
  if (normalizeCommandName(argv[offset]) !== "zuvix") {
    return false;
  }
  offset += 1;
