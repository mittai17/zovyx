// Tests Zuvix home directory resolution.
...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers ZUVIX_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands ZUVIX_HOME when set to ~", () => {
    const env = {
      ZUVIX_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for ZUVIX_HOME",
      env: { ZUVIX_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when ZUVIX_HOME is tilde-only and no fallback home exists",
      env: { ZUVIX_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores ZUVIX_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          ZUVIX_HOME: "/srv/zuvix-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/zuvix-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/zuvix-home " },
      expected: "/srv/zuvix-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/zuvix-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of ZUVIX_HOME", () => {
    expect(
