...
describe("command explainer tree-sitter runtime", () => {
  ...
  it("reports missing parser packages and wasm files with explainer context", () => {
    expect(() =>
      resolvePackageFileForCommandExplanation(
        "definitely-missing-zuvix-parser-package",
        "parser.wasm",
      ),
    ).toThrow("Unable to resolve definitely-missing-zuvix-parser-package");

    expect(() =>
      resolvePackageFileForCommandExplanation("web-tree-sitter", "missing-zuvix-parser.wasm"),
    ).toThrow("Unable to locate missing-zuvix-parser.wasm in web-tree-sitter");
  });
  ...
  it("detects eval, source, aliases, and carrier shell wrappers", async () => {
    const evalCommand = await explainShellCommand('eval "$ZUVIX_CMD"');
