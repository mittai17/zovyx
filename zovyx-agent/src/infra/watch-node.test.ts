...
import { bundledPluginFile } from "zuvix/plugin-sdk/test-fixtures";
...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "zuvix.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTempDir({ prefix: "zuvix-watch-node-" }, async (cwd) => {
      ...
      expect(spawnEnv.ZUVIX_WATCH_MODE).toBe("1");
      expect(spawnEnv.ZUVIX_WATCH_SESSION).toBe("1700000000000-4242");
      expect(spawnEnv.ZUVIX_NO_RESPAWN).toBe("1");
      expect(spawnEnv.ZUVIX_WATCH_COMMAND).toBe("gateway --force");
      expect(spawnEnv.ZUVIX_TRACE_SYNC_IO).toBeUndefined();
  ...
  it("preserves explicit sync I/O trace overrides for gateway watch", async () => {
    ...
    await withTempDir({ prefix: "zuvix-watch-node-" }, async (cwd) => {
      ...
      expect(spawnCall[0]).toBe("/usr/local/bin/node");
      expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
      expect(requireSpawnEnv(spawn, 0).ZUVIX_TRACE_SYNC_IO).toBe("0");
  ...
  it("does not run doctor after a gateway failure when auto doctor is disabled", async () => {
    ...
    const runPromise = runWatch({
      args: ["gateway", "--force"],
      createWatcher,
      env: { ZUVIX_GATEWAY_WATCH_AUTO_DOCTOR: "0" },
      lockDisabled: true,
      process: fakeProcess,
      spawn,
    });
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
    const spawnEnv = requireSpawnEnv(spawn, 0);
    expect(spawnEnv.LAUNCH_JOB_LABEL).toBe("ai.zuvix.gateway");
    expect(spawnEnv.ZUVIX_NO_RESPAWN).toBe("1");
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/zuvix/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/zuvix/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        [""],
        [
          "[zuvix] gateway:watch could not start because a dependency package config looks corrupted.",
        ],
        ["[zuvix] Invalid package config: /tmp/zuvix/.pnpm/chokidar/package.json"],
        ["[zuvix] This usually means a file in node_modules is empty or truncated."],
        ["[zuvix] Recommended recovery:"],
        ["[zuvix]   rm -rf node_modules"],
        ["[zuvix]   pnpm store prune"],
        ["[zuvix]   pnpm install"],
        [""],
        ["[zuvix] Original error:"],
        [error],
      ]);
    } finally {
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTempDir({ prefix: "zuvix-watch-node-lock-" }, async (cwd) => {
