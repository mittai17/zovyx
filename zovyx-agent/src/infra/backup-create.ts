...
import { resolveDateTimestampMs } from "@zuvix/normalization-core/number-coercion";
import {
...
async function createSanitizedStateSqliteBackupAsset(params: {
  ...
  const archiveSourcePath = resolveZuvixStateSqlitePath({
    ...process.env,
  ...
  const sourcePath = path.join(params.tempDir, "zuvix-state-backup.sqlite");
  try {
...
async function listAgentSqlitePaths(stateDir: string): Promise<string[]> {
  ...
  async function visit(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && entry.name === "zuvix-agent.sqlite") {
...
async function createAgentSqliteBackupAssets(params: {
  ...
  for (const archiveSourcePath of sqlitePaths) {
    ...
    const sourcePath = path.join(
      params.tempDir,
      `zuvix-agent-backup-${snapshots.length}.sqlite`,
    );
    try {
      source.exec("PRAGMA busy_timeout = 30000;");
      source.prepare("VACUUM INTO ?").run(sourcePath);
...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Zuvix config file was found to back up."
        : "No local Zuvix state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(tempRoot, "zuvix-backup-"));
