...
vi.mock("../channels/plugins/bundled.js", () => {
  ...
  function resolveChatAppAccountId(cfg: ZuvixConfig): string {
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: ZuvixConfig; env: NodeJS.ProcessEnv }) => {
        const root = env.ZUVIX_STATE_DIR;
        if (!root) {
          return [];
        }
...
const createTempDir = () => tempDirs.make("zuvix-state-migrations-test-");
...
describe("state migrations", () => {
  ...
  it("migrates legacy delivery queue files into shared SQLite state", async () => {
    ...
    const { db } = openZuvixStateDatabase({ env });
    const rows = db
      .prepare(
