...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevZuvixHome = process.env.ZUVIX_HOME;

    try {
      process.env.ZUVIX_HOME = dir;
      const approvalsPath = path.join(dir, ".zuvix", "exec-approvals.json");
      ...
      if (prevZuvixHome === undefined) {
        delete process.env.ZUVIX_HOME;
      } else {
        process.env.ZUVIX_HOME = prevZuvixHome;
      }
    }
  });
});
