...
function buildSystemdHandoffUnitName(handoffId: string | undefined): string {
  const suffix =
    ...
    "handoff";
  return `zuvix-update-${suffix}.scope`;
}
...
async function resolveHandoffSpawn(params: {
  ...
  if (!systemdRunPath) {
    throw new Error(
      "systemd-run is required to start the managed update handoff outside zuvix-gateway.service",
    );
  }
