// Creates temporary Zuvix directories for runtime scratch work.
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

/** Preferred shared Zuvix temp root on POSIX systems when ownership and permissions are safe. */
export const POSIX_ZUVIX_TMP_DIR = "/tmp/zuvix";
...
export type ResolvePreferredZuvixTmpDirOptions = {
...
/** Resolves a safe Zuvix temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredZuvixTmpDir(
  options: ResolvePreferredZuvixTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "zuvix" : `zuvix-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[zuvix] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Zuvix temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Zuvix temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_ZUVIX_TMP_DIR;
