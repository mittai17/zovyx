...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ZUVIX_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*zuvix-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "zuvix";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "zuvix");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "zuvix-gw");
  ...
  records.push(`_zuvix-gw._tcp IN PTR ${instanceLabel}._zuvix-gw._tcp`);
  records.push(`${instanceLabel}._zuvix-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._zuvix-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; zuvix-content-hash: ${contentHash}\n${contentBody}`;
}
