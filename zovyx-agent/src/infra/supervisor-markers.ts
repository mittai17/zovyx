...
const SUPERVISOR_HINTS = {
  launchd: ["ZUVIX_LAUNCHD_LABEL"],
  systemd: ["ZUVIX_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["ZUVIX_WINDOWS_TASK_NAME"],
} as const;
...
export interface DetectRespawnSupervisorOptions {
  includeLinuxZuvixGatewayServiceMarker?: boolean;
}
...
function hasZuvixGatewayServiceMarker(env: NodeJS.ProcessEnv): boolean {
  return (
    env.ZUVIX_SERVICE_MARKER?.trim() === "zuvix" &&
    env.ZUVIX_SERVICE_KIND?.trim() === "gateway"
  );
}

function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.ZUVIX_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "linux") {
    return hasAnyHint(env, SUPERVISOR_HINTS.systemd) ||
      (options.includeLinuxZuvixGatewayServiceMarker === true &&
        hasZuvixGatewayServiceMarker(env))
      ? "systemd"
      : null;
  }
  if (platform === "win32") {
    ...
    const marker = env.ZUVIX_SERVICE_MARKER?.trim();
