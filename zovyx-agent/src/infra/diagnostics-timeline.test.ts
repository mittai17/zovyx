...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "zuvix-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      ZUVIX_DIAGNOSTICS: "timeline",
      ZUVIX_DIAGNOSTICS_RUN_ID: "run-1",
      ZUVIX_DIAGNOSTICS_ENV: "env-1",
      ZUVIX_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ZUVIX_DIAGNOSTICS: "1" } })).toBe(true);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ZUVIX_DIAGNOSTICS: "yes" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ZUVIX_DIAGNOSTICS: "on" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ZUVIX_DIAGNOSTICS: "all" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ZUVIX_DIAGNOSTICS: "*" } })).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, ZUVIX_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({ env: { ...env, ZUVIX_DIAGNOSTICS: "telegram.http" } }),
    ).toBe(false);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ZUVIX_DIAGNOSTICS: "0" } })).toBe(
      false,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, ZUVIX_DIAGNOSTICS_TIMELINE_PATH: "" },
      }),
    ).toBe(false);
  });
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.ZUVIX_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as ZuvixConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as ZuvixConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as ZuvixConfig;
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("zuvix.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.ZUVIX_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as ZuvixConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
