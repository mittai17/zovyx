...
async function runHelperWithExistingSentinel(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-handoff-helper-test-"));
  tempDirs.add(tmpDir);
...
describe("managed service update handoff", () => {
  it("strips process supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    const serviceIdentityEnv = {
      ZUVIX_LAUNCHD_LABEL: "com.example.zuvix.test",
      ZUVIX_SYSTEMD_UNIT: "zuvix-test.service",
      ZUVIX_WINDOWS_TASK_NAME: "Zuvix Test Gateway",
    ...
    expect(result.command).toBe("zuvix update --yes --timeout 1800");
    ...
    expect(options.env.ZUVIX_UPDATE_RUN_HANDOFF).toBe("1");
  ...
  it("launches systemd handoffs through a transient user scope", async () => {
    const { startManagedServiceUpdateHandoff } =
      await import("./update-managed-service-handoff.js");
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-systemd-run-bin-"));
    tempDirs.add(binDir);
    ...
    expect(args.slice(0, 4)).toEqual([
      ...
      "--scope",
      "--collect",
      "--unit=zuvix-update-handoff-123.scope",
    ]);
    ...
    expect(options.detached).toBe(true);
    expect(options.env.ZUVIX_SYSTEMD_UNIT).toBe("zuvix-gateway.service");
  ...
  it("sweeps stale handoff temp directories while keeping fresh handoff logs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-handoff-cleanup-test-"));
    tempDirs.add(tmpDir);
    ...
    const unrelatedDir = path.join(tmpDir, "zuvix-other-temp");
