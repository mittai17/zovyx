// Runs Zuvix package update checks, package steps, and restart handoff.
...
const DEFAULT_PACKAGE_NAME = "zuvix";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR_ENV =
  "ZUVIX_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR";
const UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE_ENV =
  "ZUVIX_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE";
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "zuvix-update-preflight-";
...
const DEV_PREFLIGHT_LINT_ENV: NodeJS.ProcessEnv = {
  ...
  ZUVIX_OXLINT_SHARDS_SERIAL: "1",
};
const DEV_PREFLIGHT_LINT_OPT_IN_ENV = "ZUVIX_UPDATE_PREFLIGHT_LINT";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["zuvix", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "zuvix.mjs");
      ...
      const doctorStep = await runStep(
        step("zuvix doctor", doctorArgv, gitRoot, {
