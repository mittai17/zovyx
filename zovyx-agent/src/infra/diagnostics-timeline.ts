...
const ZUVIX_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "zuvix.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.ZUVIX_DIAGNOSTICS)) &&
    typeof env.ZUVIX_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.ZUVIX_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: ZUVIX_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.ZUVIX_DIAGNOSTICS_RUN_ID ? { runId: env.ZUVIX_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.ZUVIX_DIAGNOSTICS_ENV ? { envName: env.ZUVIX_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.ZUVIX_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
