...
import { readRootJsonObjectSync } from "@zuvix/fs-safe/json";
...
/** Return true when an installed package needs an zuvix peer link repair. */
export function installedPackageNeedsZuvixPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "zuvix"));
    return false;
  } catch (error) {
