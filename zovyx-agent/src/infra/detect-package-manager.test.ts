...
async function withPackageManagerRoot<T>(
  ...
  return await withTempDir({ prefix: "zuvix-detect-pm-" }, async (root) => {
...
async function writePublishedZuvixRoot(root: string): Promise<void> {
...
describe("detectPackageManager", () => {
  ...
  it("keeps pnpm-owned direct package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "zuvix-detect-pm-pnpm-direct-" }, async (base) => {
      const nodeModulesRoot = path.join(base, "pnpm-global", "node_modules");
      const packageRoot = path.join(nodeModulesRoot, "zuvix");
      await writePublishedZuvixRoot(packageRoot);
  ...
  it("keeps pnpm-owned virtual-store package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "zuvix-detect-pm-pnpm-virtual-" }, async (base) => {
  ...
  it("keeps bun-owned global package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "zuvix-detect-pm-bun-" }, async (base) => {
      const bunInstall = path.join(base, "bun-home");
      await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
        const packageRoot = path.join(bunInstall, "install", "global", "node_modules", "zuvix");
        await writePublishedZuvixRoot(packageRoot);
