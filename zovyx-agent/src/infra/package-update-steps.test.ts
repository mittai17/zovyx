...
function createNpmTarget(globalRoot: string): ResolvedGlobalInstallTarget {
  return {
    ...
    command: "npm",
    globalRoot,
    packageRoot: path.join(globalRoot, "zuvix"),
  };
}
...
describe("runGlobalPackageUpdateSteps", () => {
  it("installs npm updates into a clean staged prefix before swapping the global package", async () => {
    await withTempDir({ prefix: "zuvix-package-update-staged-" }, async (base) => {
      ...
      const runStep = vi.fn(
        async ({ name, argv, cwd, timeoutMs }): Promise<PackageUpdateStepResult> => {
          ...
          await fs.symlink(
            "../lib/node_modules/zuvix/dist/index.js",
            path.join(stagePrefix, "bin", "zuvix"),
          );
          return {
            name,
            command: argv.join(" "),
      ...
      await expect(fs.readlink(path.join(prefix, "bin", "zuvix"))).resolves.toBe(
        "../lib/node_modules/zuvix/dist/index.js",
      );
    });
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "zuvix-package-update-hardlinks-" }, async (base) => {
        const prefix = path.join(base, "prefix");
        const globalRoot = path.join(prefix, "lib", "node_modules");
        const packageRoot = path.join(globalRoot, "zuvix");
        await writePackageRoot(packageRoot, "1.0.0");
        ...
        const result = await runGlobalPackageUpdateSteps({
          ...
          runStep: async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
            ...
            const stagedPackageRoot = path.join(stagePrefix, "lib", "node_modules", "zuvix");
  ...
  it("swaps staged npm updates into an explicitly selected direct node_modules root", async () => {
    await withTempDir({ prefix: "zuvix-package-update-direct-root-" }, async (base) => {
      const managedRoot = path.join(base, ".zuvix", "npm", "node_modules");
      const packageRoot = path.join(managedRoot, "zuvix");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        await writePackageRoot(path.join(stagePrefix, "lib", "node_modules", "zuvix"), "2.0.0");
        await fs.mkdir(path.join(stagePrefix, "bin"), { recursive: true });
        await fs.symlink(
          "../lib/node_modules/zuvix/dist/index.js",
          path.join(stagePrefix, "bin", "zuvix"),
      ...
      await expectPathMissing(path.join(managedRoot, ".bin", "zuvix"));
    });
  });

  it("accepts v-prefixed exact npm specs when verifying staged installs", async () => {
    await withTempDir({ prefix: "zuvix-package-update-v-prefix-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "zuvix");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toContain("zuvix@v2.0.0");
        const prefixIndex = argv.indexOf("--prefix");
        const stagePrefix = argv[prefixIndex + 1];
        if (!stagePrefix) {
          throw new Error("missing staged prefix");
  ...
  it("packs npm GitHub specs before installing into the staged prefix", async () => {
    await withTempDir({ prefix: "zuvix-package-update-npm-pack-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "zuvix");
      const sourceSpec = "Zuvix@github:zuvix/zuvix#release/2026.5.12";
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toEqual([
          ...
          "--prefix",
          stagePrefix,
          path.join(packDir, "zuvix-2.0.0.tgz"),
          "--no-fund",
          "--no-audit",
          "--loglevel=error",
          "--min-release-age=0",
  ...
  it.each([
    {
      name: "full git url",
      sourceSpec: "https://github.com/zuvix/zuvix.git#main",
    },
    {
      name: "hosted GitHub URL without git suffix",
      sourceSpec: "https://github.com/zuvix/zuvix#main",
    },
    {
      name: "aliased hosted GitHub URL without git suffix",
      sourceSpec: "zuvix@https://github.com/zuvix/zuvix#main",
    },
    {
      name: "GitHub shorthand",
      sourceSpec: "zuvix/zuvix#main",
    },
    {
      name: "SCP-style SSH",
      sourceSpec: "git@github.com:zuvix/zuvix.git#main",
    },
  ] as const)(
    "packs additional npm git source spec forms before install: $name",
    async ({ sourceSpec }) => {
      await withTempDir({ prefix: "zuvix-package-update-npm-pack-variant-" }, async (base) => {
        ...
        const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
          if (name === "global update pack") {
            ...
            tarball = path.join(destination, "zuvix-2.0.0.tgz");
  ...
  it("swaps staged npm package roots through the copy fallback when rename crosses devices", async () => {
    await withTempDir({ prefix: "zuvix-package-update-exdev-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "zuvix");
      ...
      const renameSpy = vi
        ...
        .mockImplementation(async (...args: Parameters<typeof fs.rename>) => {
          ...
          if (
            exdevMoves === 0 &&
            fromPath.includes(`${path.sep}.zuvix-update-stage-`) &&
            path.basename(fromPath) === "zuvix" &&
      ...
      try {
        const result = await runGlobalPackageUpdateSteps({
          ...
          runStep: async ({ name, argv, cwd }) => {
            ...
            await writePackageRoot(path.join(stageLayout.globalRoot, "zuvix"), "2.0.0");
            return {
              name,
              command: argv.join(" "),
              cwd: cwd ?? process.cwd(),
  ...
  it("stages pnpm-detected updates through npm when the global root has npm prefix layout", async () => {
    await withTempDir({ prefix: "zuvix-package-update-pnpm-staged-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "zuvix");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toContain("zuvix@2.0.0");
  ...
  it("keeps Windows pnpm global roots on the pnpm update path", async () => {
    ...
    try {
      await withTempDir({ prefix: "zuvix-package-update-win32-pnpm-" }, async (base) => {
        ...
        const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
          ...
          expect(argv).toEqual(["pnpm", "add", "-g", "--global-dir", globalDir, "zuvix@2.0.0"]);
  ...
  it("keeps a successful staged swap when old package cleanup hits a transient Windows native module error", async () => {
    await withTempDir({ prefix: "zuvix-package-update-staged-cleanup-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "zuvix");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const rmSpy = vi.spyOn(fs, "rm").mockImplementation(async (target, options) => {
        ...
        if (
          targetPath.includes(`${path.sep}.zuvix-`) &&
          !targetPath.includes(".zuvix-update-stage-") &&
          !targetPath.includes(".zuvix-shim-backup-")
        ) {
  ...
  it("does not run post-verify work when staged npm verification fails", async () => {
    await withTempDir({ prefix: "zuvix-package-update-verify-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "zuvix");
      await writePackageRoot(packageRoot, "1.0.0");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "zuvix-package-update-shim-rollback-" }, async (base) => {
        const prefix = path.join(base, "prefix");
        const globalRoot = path.join(prefix, "lib", "node_modules");
        const packageRoot = path.join(globalRoot, "zuvix");
        const targetShim = path.join(prefix, "bin", "zuvix");
        await writePackageRoot(packageRoot, "1.0.0");
        ...
        try {
          result = await runGlobalPackageUpdateSteps({
            ...
            runStep: async ({ name, argv, cwd }) => {
              ...
              const stagedShim = path.join(stagePrefix, "bin", "zuvix");
  ...
  it("cleans the staged npm prefix when the install command throws", async () => {
    await withTempDir({ prefix: "zuvix-package-update-cleanup-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "zuvix");
      await writePackageRoot(packageRoot, "1.0.0");
