...
import { normalizeOptionalString as readOptionalString } from "@zuvix/normalization-core/string-coerce";
...
import { resolveZuvixPackageRootSync } from "./zuvix-root.js";
...
type ManagedNpmRootZuvixMetadata = {
...
type ManagedNpmRootZuvixHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedZuvixMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootZuvixMetadata | undefined {
  const metadata: ManagedNpmRootZuvixMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
/** Read host Zuvix pnpm overrides for reuse inside a managed npm root. */
export async function readZuvixManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  for (const key of readManagedOverrideKeys(manifest.zuvix)) {
    delete overrides[key];
  }
  Object.assign(overrides, managedOverrides);
  const zuvixMetadata = buildManagedZuvixMetadata({
    current: manifest.zuvix,
    managedOverrideKeys,
  });
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "zuvix" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.zuvix;
  ...
  if (isRecord(value.peerDependenciesMeta) && "zuvix" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.zuvix;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])zuvix(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.zuvix);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-managed-peer-plan-"));
  try {
    delete dependencies.zuvix;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_zuvix-pack-archives"),
      path.join(tempRoot, "_zuvix-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.zuvix).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.zuvix)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const managedOverrideKeys = readManagedOverrideKeys(manifest.zuvix).toSorted();
  const zuvixMetadata = buildManagedZuvixMetadata({
    current: manifest.zuvix,
    managedOverrideKeys,
...
/** Remove stale managed-root zuvix peer installs while preserving active host links. */
export async function repairManagedNpmRootZuvixPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootZuvixHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "zuvix" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasZuvixPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "zuvix"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootZuvixPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall zuvix" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall zuvix" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootZuvixPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootZuvixHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootZuvixHostState> {
  const packageRoot =
  ...
  const managedZuvixPackageDir = path.join(params.npmRoot, "node_modules", "zuvix");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedZuvixPackageDir),
    lstatIfExists(managedZuvixPackageDir),
  ]);
...
async function managedNpmRootLockfileHasZuvixPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "zuvix" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/zuvix" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "zuvix" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootZuvixPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("zuvix" in dependencies) {
    const { zuvix: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("zuvix" in dependenciesValue) {
          delete dependenciesValue.zuvix;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/zuvix" in parsed.packages) {
        delete parsed.packages["node_modules/zuvix"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "zuvix" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.zuvix;
  ...
  const zuvixPackageDir = path.join(params.npmRoot, "node_modules", "zuvix");
  if (!params.preservePackageDir && (await pathExists(zuvixPackageDir))) {
    await fs.rm(zuvixPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["zuvix", "zuvix.cmd", "zuvix.ps1"].map((binName) =>
