...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Zuvix to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "zuvix.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ZUVIX_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ZUVIX_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ZUVIX_APNS_TEAM_ID and ZUVIX_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ZUVIX_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ZUVIX_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ZUVIX_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ZUVIX_APNS_PRIVATE_KEY_P8 or ZUVIX_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ZUVIX_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
