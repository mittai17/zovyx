// Covers Zuvix CLI PATH construction.
import path from "node:path";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { ensureZuvixCliOnPath } from "./path-env.js";
...
describe("ensureZuvixCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ZUVIX_PATH_BOOTSTRAPPED",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/zuvix-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "zuvix");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureZuvixCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.ZUVIX_PATH_BOOTSTRAPPED;
    delete process.env.ZUVIX_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling zuvix exists", () => {
  ...
  it("keeps the current runtime directory ahead of system PATH hardening", () => {
    const tmp = abs("/tmp/zuvix-path/case-runtime-dir");
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "zuvix");
      ...
      if (envValue === undefined) {
        ...
        process.env.ZUVIX_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/zuvix-path/case-linuxbrew");
