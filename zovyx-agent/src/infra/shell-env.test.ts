...
describe("shell env fallback", () => {
  ...
  it("is disabled by default", () => {
    ...
    expect(shouldEnableShellEnvFallback({ ZUVIX_LOAD_SHELL_ENV: "0" })).toBe(false);
    expect(shouldEnableShellEnvFallback({ ZUVIX_LOAD_SHELL_ENV: "1" })).toBe(true);
  });
  ...
  it("uses the same truthy env parsing for deferred fallback", () => {
    ...
    expect(shouldDeferShellEnvFallback({ ZUVIX_DEFER_SHELL_ENV_FALLBACK: "false" })).toBe(false);
    expect(shouldDeferShellEnvFallback({ ZUVIX_DEFER_SHELL_ENV_FALLBACK: "yes" })).toBe(true);
  });
  ...
  it("resolves timeout from env with default fallback", () => {
    ...
    expect(resolveShellEnvFallbackTimeoutMs({ ZUVIX_SHELL_ENV_TIMEOUT_MS: "42" })).toBe(42);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        ZUVIX_SHELL_ENV_TIMEOUT_MS: "nope",
      }),
    ).toBe(15000);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        ZUVIX_SHELL_ENV_TIMEOUT_MS: "42abc",
      }),
    ).toBe(15000);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        ZUVIX_SHELL_ENV_TIMEOUT_MS: String(Number.MAX_SAFE_INTEGER),
      }),
  ...
  it("imports missing expected keys even when another expected key already exists", () => {
    const env: NodeJS.ProcessEnv = { ZUVIX_GATEWAY_TOKEN: "set" };
    const exec = vi.fn(() =>
      Buffer.from(
        "ZUVIX_GATEWAY_TOKEN=from-shell\0TWILIO_ACCOUNT_SID=AC123\0TWILIO_AUTH_TOKEN=secret\0TWILIO_FROM_NUMBER=+15550001234\0",
      ),
    );
    ...
    expect(env.ZUVIX_GATEWAY_TOKEN).toBe("set");
