...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.zuvix.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  ...
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const zuvixPayload = requireRecord(payload.zuvix, "zuvix payload");
    expectRecordFields(zuvixPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof zuvixPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.zuvix.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open Zuvix to review this request.",
      },
      sound: "default",
      category: "zuvix.exec-approval",
      "content-available": 1,
    });
    const zuvixPayload = requireRecord(payload.zuvix, "zuvix payload");
    expectRecordFields(zuvixPayload, {
    ...
    expect(typeof zuvixPayload.ts).toBe("number");
    expectNoProperties(zuvixPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.zuvix, "zuvix payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
