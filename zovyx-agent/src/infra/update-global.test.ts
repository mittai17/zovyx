...
import { bundledDistPluginFile } from "zuvix/plugin-sdk/test-fixtures";
...
const execFileSyncMock = vi.hoisted(() => vi.fn(() => "/tmp/zuvix-test-global-npmrc\n"));
...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "zuvix", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["ZUVIX_UPDATE_PACKAGE_SPEC"]);
    process.env.ZUVIX_UPDATE_PACKAGE_SPEC = "file:/tmp/zuvix.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "zuvix", tag: "latest" })).toBe(
      "file:/tmp/zuvix.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "zuvix",
        tag: "beta",
        env: { ZUVIX_UPDATE_PACKAGE_SPEC: "zuvix@next" },
      }),
    ).toBe("zuvix@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "zuvix"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "zuvix", tag: "main" })).toBe(
      ZUVIX_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "zuvix",
        tag: "github:zuvix/zuvix#feature/my-branch",
      }),
    ).toBe("github:zuvix/zuvix#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "zuvix",
        tag: "https://example.com/zuvix-main.tgz",
      }),
    ).toBe("https://example.com/zuvix-main.tgz");
  });
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "zuvix-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:zuvix/zuvix#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/zuvix-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/zuvix-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("/tmp/zuvix-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("zuvix-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:zuvix/zuvix#main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("/tmp/zuvix-main.tgz")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "zuvix-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "zuvix");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "zuvix"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "zuvix"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "zuvix"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "zuvix"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "zuvix-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "zuvix@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "zuvix@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "zuvix@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "zuvix@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "zuvix-update-npm-missing-bin-" }, async (base) => {
  ...
  it("honors an explicitly selected direct npm node_modules package root", async () => {
    await withTempDir({ prefix: "zuvix-update-managed-service-root-" }, async (base) => {
      const managedNpmRoot = path.join(base, ".zuvix", "npm", "node_modules");
      const pkgRoot = path.join(managedNpmRoot, "zuvix");
      ...
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(otherPnpmRoot, "zuvix"), { recursive: true });
  ...
  it("preserves bun ownership for direct node_modules package roots", async () => {
    await withTempDir({ prefix: "zuvix-update-managed-bun-root-" }, async (base) => {
      envSnapshot = captureEnv(["BUN_INSTALL"]);
      process.env.BUN_INSTALL = path.join(base, ".bun");
      const bunRoot = path.join(process.env.BUN_INSTALL, "install", "global", "node_modules");
      const pkgRoot = path.join(bunRoot, "zuvix");
      const pathNpmRoot = path.join(base, "shell", "lib", "node_modules");
      await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "zuvix-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "zuvix");
  ...
  it("detects custom pnpm global layouts from the running package root", async () => {
    await withTempDir({ prefix: "zuvix-update-pnpm-custom-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "zuvix");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "zuvix-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(
        ...
        "5",
        ".pnpm",
        "zuvix@file+..+pack+zuvix-2026.5.6.tgz",
        "node_modules",
        "zuvix",
      );
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "zuvix"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "zuvix-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "zuvix");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "zuvix"),
      });
    });
  });
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "zuvix@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "zuvix@latest",
    ...
    expect(globalInstallArgs("pnpm", "zuvix@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "zuvix@latest",
    ]);
    expect(globalInstallArgs("pnpm", "github:zuvix/zuvix#release/2026.5.12")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=zuvix",
      "github:zuvix/zuvix#release/2026.5.12",
    ]);
    expect(
      globalInstallArgs("pnpm", "zuvix@git+https://github.com/zuvix/zuvix.git"),
    ).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=zuvix",
      "zuvix@git+https://github.com/zuvix/zuvix.git",
    ]);
    expect(globalInstallArgs("bun", "zuvix@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "zuvix@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "zuvix@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "zuvix@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "zuvix@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "zuvix@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "zuvix@latest"]);
    expect(globalInstallArgs("pnpm", "zuvix@latest", null, "/opt/pnpm-global")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--global-dir",
  ...
  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "zuvix@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "zuvix@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/zuvix/lib/node_modules")).toEqual({
      prefix: "/opt/zuvix",
      globalRoot: "/opt/zuvix/lib/node_modules",
      binDir: "/opt/zuvix/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "zuvix-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".zuvix-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".zuvix-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".zuvix-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "zuvix"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".zuvix-123", ".zuvix-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "zuvix"));
      const markerFileStat = await fs.stat(path.join(root, ".zuvix-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "zuvix-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "zuvix-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".zuvix-install-stage", ".zuvix-install-stage-retry"]) {
      ...
      await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
        "unexpected packaged dist file dist/extensions/brave/.zuvix-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
        "unexpected packaged dist file dist/extensions/brave/.zuvix-install-stage/node_modules/typebox/build/compile/code.mjs",
      ]);
    });
  });

  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "zuvix-update-global-source-checkout-" }, async (base) => {
      const checkoutRoot = path.join(base, "checkout");
      const globalRoot = path.join(base, "prefix", "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "zuvix");
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "zuvix-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "zuvix-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "zuvix-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "zuvix-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "telegram", "@zuvix/telegram");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "zuvix-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@zuvix/telegram");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "zuvix-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@zuvix/qa-lab");
        await writePackageDistInventory(packageRoot);
