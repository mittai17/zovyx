...
import {
  ...
  isZuvixOrgNpmSpec,
  isZuvixStableCorrectionVersion,
...
describe("npm registry spec validation", () => {
  it.each([
    "@zuvix/voice-call",
    "@zuvix/voice-call@1.2.3",
    "@zuvix/voice-call@1.2.3-beta.4",
    "@zuvix/voice-call@latest",
    "@zuvix/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    ...
    {
      spec: "git+ssh://github.com/zuvix/zuvix",
      expected: "URLs are not allowed",
    },
    {
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    {
      spec: "@zuvix/voice-call@2026.5.3-1",
      expected: {
        name: "@zuvix/voice-call",
        raw: "@zuvix/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@zuvix/voice-call@1.2.3-beta.1",
      expected: {
        name: "@zuvix/voice-call",
        raw: "@zuvix/voice-call@1.2.3-beta.1",
  ...
  it.each([
    { spec: "@zuvix/voice-call", expected: true },
    { spec: "@zuvix/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@zuvix/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects Zuvix-org npm specs for %s", ({ spec, expected }) => {
    expect(isZuvixOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("detects Zuvix stable correction versions for %s", ({ value, expected }) => {
    expect(isZuvixStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares Zuvix release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareZuvixReleaseVersions(left, right)).toBe(expected);
  });
});
  ...
  it.each([
    {
      spec: "@zuvix/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@zuvix/voice-call@beta"`,
    },
    {
      spec: "@zuvix/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
