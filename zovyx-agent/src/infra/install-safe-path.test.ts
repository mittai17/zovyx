...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@zuvix/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@zuvix/matrix", id: "matrix", expected: true },
    { packageName: "@zuvix/matrix", id: "@zuvix/matrix", expected: true },
    { packageName: "@zuvix/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@zuvix/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@zuvix__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "zuvix-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "zuvix-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "zuvix-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "zuvix-install-safe-" }, async (parentDir) => {
