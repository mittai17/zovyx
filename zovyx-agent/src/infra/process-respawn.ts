...
import { triggerZuvixRestart } from "./restart.js";
 ...
 * - ZUVIX_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthy(process.env.ZUVIX_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerZuvixRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthy(process.env.ZUVIX_NO_RESPAWN)) {
    return { mode: "disabled", detail: "ZUVIX_NO_RESPAWN" };
  }
