...
import { readResponseWithLimit } from "@zuvix/media-core/read-response-with-limit";
import {
...
// Keep aligned with @zuvix/plugin-package-contract ExternalPluginCompatibility.
...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    normalizeOptionalString(process.env.ZUVIX_CLAWHUB_URL) ||
...
function normalizeGitHubCodeloadBaseUrl(): string {
  const value =
    normalizeOptionalString(process.env.ZUVIX_CLAWHUB_GITHUB_CODELOAD_BASE_URL) ||
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.ZUVIX_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.ZUVIX_CLAWHUB_TOKEN) ||
...
function normalizePluginApiVersionForComparator(version: string, target: string): string {
  const normalizedCorrection = normalizeZuvixNumericCorrectionForPluginApi(version);
  if (normalizedCorrection) {
    return normalizedCorrection;
  }
  return shouldPreservePluginApiPrereleaseFloor(target)
    ? version
    : normalizeZuvixReleaseSuffixForPluginApi(version);
}
...
const ZUVIX_RELEASE_SUFFIX_PATTERN =
  /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)(?:-\d+|-(?:alpha|beta|rc)\.\d+)$/i;
const ZUVIX_NUMERIC_CORRECTION_PATTERN = /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)-\d+$/;

function normalizeZuvixNumericCorrectionForPluginApi(
  pluginApiVersion: string,
): string | undefined {
  return ZUVIX_NUMERIC_CORRECTION_PATTERN.exec(pluginApiVersion.trim())?.[1];
}

function normalizeZuvixReleaseSuffixForPluginApi(pluginApiVersion: string): string {
  const match = ZUVIX_RELEASE_SUFFIX_PATTERN.exec(pluginApiVersion.trim());
...
export async function downloadClawHubGitHubSkillArchive(params: {
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "zuvix-clawhub-github-skill",
