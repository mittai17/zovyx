  ...
  return withEnvAsync(
    {
      ...
      ZUVIX_SERVICE_VERSION: undefined,
...
describe("system-presence version fallback", () => {
  ...
  it("uses runtime VERSION when ZUVIX_VERSION is not set", async () => {
  ...
  it("prefers ZUVIX_VERSION over runtime VERSION", async () => {
    await expectSelfVersion(
      {
  ...
  it("still prefers runtime VERSION over ZUVIX_SERVICE_VERSION when ZUVIX_VERSION is blank", async () => {
    await expectSelfVersion(
      {
        ZUVIX_VERSION: " ",
        ZUVIX_SERVICE_VERSION: "2.4.6-service",
  ...
  it("uses runtime VERSION when ZUVIX_VERSION and ZUVIX_SERVICE_VERSION are blank", async () => {
    await expectSelfVersion(
      {
        ZUVIX_VERSION: " ",
        ZUVIX_SERVICE_VERSION: "\t",
