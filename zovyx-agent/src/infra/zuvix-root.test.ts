// Covers Zuvix package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__zuvix_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "zuvix-root");
...
function setPackageRoot(root: string, name = "zuvix") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./zuvix-root.js").resolveZuvixPackageRootSync,
  asyncResolver: typeof import("./zuvix-root.js").resolveZuvixPackageRoot,
  opts: Parameters<typeof import("./zuvix-root.js").resolveZuvixPackageRootSync>[0],
...
vi.mock("./zuvix-root.fs.runtime.js", () => ({
  zuvixRootFsSync: mockFsModule(),
  zuvixRootFs: mockFsPromisesModule(),
}));

describe("resolveZuvixPackageRoot", () => {
  let resolveZuvixPackageRoot: typeof import("./zuvix-root.js").resolveZuvixPackageRoot;
  let resolveZuvixPackageRootSync: typeof import("./zuvix-root.js").resolveZuvixPackageRootSync;
  let clearZuvixPackageRootCaches: typeof import("./zuvix-root.js").testing.clearZuvixPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveZuvixPackageRoot,
      resolveZuvixPackageRootSync,
      testing: { clearZuvixPackageRootCaches },
    } = await import("./zuvix-root.js"));
  });

  beforeEach(() => {
    clearZuvixPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "zuvix");
        const pkgRoot = path.join(project, "node_modules", "zuvix");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "zuvix");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "zuvix.mjs")));
    ...
    {
      name: "falls through from a non-zuvix moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-zuvix");
    ...
    {
      name: "returns null for non-zuvix package roots",
      setup: () => {
        const pkgRoot = fx("not-zuvix");
        setPackageRoot(pkgRoot, "not-zuvix");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "zuvix.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "zuvix");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
