...
function buildScheduledTaskRestartScript(params: {
  ...
  const lines = [
    ...
    "setlocal",
    ...setupLines,
    `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] zuvix restart attempt source=windows-task-handoff target=${quotedTaskName}`,
    ...
    "goto retry",
    ":fallback",
    `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] zuvix restart fallback source=windows-task-handoff`,
  ];
  if (taskScriptPath) {
  ...
  lines.push(
    ":cleanup",
    `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] zuvix restart finished source=windows-task-handoff`,
    'del "%~f0" >nul 2>&1',
  );
  return lines.join("\r\n");
}
...
export function relaunchGatewayScheduledTask(env: NodeJS.ProcessEnv = process.env): RestartAttempt {
  ...
  const scriptPath = path.join(
    ...
    `zuvix-schtasks-restart-${randomUUID()}.cmd`,
  );
