...
import { asNullableObjectRecord as readRecord } from "@zuvix/normalization-core/record-coerce";
import {
...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.ZUVIX_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Zuvix. You can keep running the web gateway without it: \`${formatCliCommand("zuvix gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
