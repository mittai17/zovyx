...
import { resolveZuvixPackageRoot } from "./zuvix-root.js";
...
function resolveAutoUpdatePolicy(cfg: ZuvixConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: ZuvixConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.ZUVIX_NO_AUTO_UPDATE);
  ...
  const root = await resolveZuvixPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("zuvix update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by ZUVIX_NO_AUTO_UPDATE", {
