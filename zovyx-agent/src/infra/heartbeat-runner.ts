...
import { timestampMsToIsoString } from "@zuvix/normalization-core/number-coercion";
import {
...
function resolveHeartbeatTimeoutOverrideSeconds(cfg: ZuvixConfig, heartbeat?: HeartbeatConfig) {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ZuvixConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: ZuvixConfig) {
...
function resolveHeartbeatAgents(cfg: ZuvixConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: ZuvixConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: ZuvixConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: ZuvixConfig, heartbeat?: HeartbeatConfig) {
...
function resolveHeartbeatAckMaxChars(cfg: ZuvixConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: ZuvixConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: ZuvixConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ZuvixConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
