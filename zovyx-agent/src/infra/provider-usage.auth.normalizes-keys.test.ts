...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "zuvix-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".zuvix");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".zuvix", "agents", "main", "agent");
  }
