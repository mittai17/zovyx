...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".zuvix");
...
function sharedMainOpsConfig(sharedStorePath: string): ZuvixConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: ZuvixConfig = OPS_WORK_CONFIG) {
