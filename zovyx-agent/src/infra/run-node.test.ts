...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "zuvix.plugin.json");
...
const DIST_RUNTIME_EXTENSION_MANIFEST = "dist-runtime/extensions/demo/zuvix.plugin.json";
...
const DIST_ZUVIX_ALIAS_PACKAGE = "dist/extensions/node_modules/zuvix/package.json";
const DIST_ZUVIX_ALIAS_PLUGIN_SDK_INDEX =
  "dist/extensions/node_modules/zuvix/plugin-sdk/index.js";
const DIST_ZUVIX_ALIAS_PLUGIN_SDK_STRING_COERCE =
  "dist/extensions/node_modules/zuvix/plugin-sdk/string-coerce-runtime.js";
...
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "zuvix.plugin.json");
...
async function writeRuntimePostBuildScaffold(tmp: string): Promise<void> {
  ...
  await writeProjectFiles(tmp, {
    ...
    [DIST_ZUVIX_ALIAS_PACKAGE]:
      '{"name":"zuvix","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
  });
...
function statusCommandSpawn() {
  return [process.execPath, "zuvix.mjs", "status"];
}

function gatewayCallStatusCommandSpawn() {
  return [process.execPath, "zuvix.mjs", "gateway", "call", "status", "--json"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "zuvix.mjs", "--version"],
        ]);
      });
    },
  );
  ...
  it("skips DTS generation only for launcher-triggered local builds", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      expect(spawnCalls[2]?.args).toEqual(["zuvix.mjs", "status"]);
      expect(spawnCalls[0]?.env.ZUVIX_RUN_NODE_SKIP_DTS_BUILD).toBeUndefined();
      expect(spawnCalls[1]?.env.ZUVIX_RUN_NODE_SKIP_DTS_BUILD).toBe("1");
      expect(spawnCalls[2]?.env.ZUVIX_RUN_NODE_SKIP_DTS_BUILD).toBeUndefined();
    });
  });
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "zuvix.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "zuvix.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...
          ZUVIX_FORCE_BUILD: "1",
          ZUVIX_RUNNER_LOG: "1",
          ZUVIX_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[zuvix]");
      expect(spawnCalls.at(-1)?.args).toEqual(["zuvix.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.ZUVIX_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "zuvix.mjs" ? childStderr : "",
        });
      const stderrChunks: string[] = [];
      const stderr = {
        write: (chunk: string | Buffer) => {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          ZUVIX_RUNNER_LOG: "0",
          ZUVIX_RUN_NODE_FILTER_SYNC_IO_STDERR: "1",
          ZUVIX_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr,
  ...
  it("adds Node CPU profiling flags to the launched Zuvix child when requested", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          ZUVIX_RUNNER_LOG: "0",
          ZUVIX_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
        },
        spawn,
        spawnSync,
        runRuntimePostBuild: skipRuntimePostBuild,
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=zuvix-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["zuvix.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.ZUVIX_RUN_NODE_CPU_PROF_DIR).toBe(profileDir);
  ...
  it("rotates old Node CPU profiles when a retention cap is set", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      const oldProfiles = [
        "zuvix-status-oldest.cpuprofile",
        "zuvix-status-middle.cpuprofile",
        "zuvix-status-newest.cpuprofile",
      ];
      ...
      fsSync.writeFileSync(path.join(profileDir, "zuvix-models-old.cpuprofile"), "{}");
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...
          ZUVIX_RUNNER_LOG: "0",
          ZUVIX_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
          ZUVIX_RUN_NODE_CPU_PROF_MAX_FILES: "2",
        },
        spawn,
        spawnSync,
        runRuntimePostBuild: skipRuntimePostBuild,
      ...
      expect(fsSync.existsSync(path.join(profileDir, "zuvix-models-old.cpuprofile"))).toBe(true);
    });
  });

  it("adds Node sync I/O tracing flag to the launched Zuvix child when requested", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls.at(-1)).toEqual(["--trace-sync-io", "zuvix.mjs", "gateway", "--force"]);
    });
  });
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["zuvix.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.ZUVIX_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
  ...
  it("passes the synthesized private QA env into runtime postbuild staging", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      expect(postBuildParams?.env?.ZUVIX_BUILD_PRIVATE_QA).toBe("1");
      expect(postBuildParams?.env?.ZUVIX_ENABLE_PRIVATE_QA_CLI).toBe("1");
      expect(postBuildParams?.env?.ZUVIX_DISABLE_BUNDLED_PLUGINS).toBe("0");
    });
  });
  ...
  it("preserves an explicit bundled plugin disable flag for QA runs", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      const exitCode = await runQaCommand({
        ...
        spawnSync,
        runRuntimePostBuild,
        env: { ZUVIX_DISABLE_BUNDLED_PLUGINS: "1" },
      });
      ...
      const postBuildParams = firstMockCall(runRuntimePostBuild)?.[0] as
        | { cwd?: string; env?: Record<string, string | undefined> }
        | undefined;
      expect(postBuildParams?.env?.ZUVIX_DISABLE_BUNDLED_PLUGINS).toBe("1");
    });
  });
  ...
  it("reruns runtime postbuild in watch mode when required outputs are missing with no runtime stamp", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      await fs.rm(resolvePath(tmp, DIST_ZUVIX_ALIAS_PACKAGE));
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"zuvix":{"extensions":["./index.ts"]}}\n',
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      const runs = Promise.all([
        runStatusCommand({
          ...
          spawnSync,
          env: {
            ZUVIX_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
          },
          runRuntimePostBuild,
        }),
        runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active zuvix child and returns 143", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(spawnCall?.[1]).toEqual(["zuvix.mjs", "status"]);
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_INDEX]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","zuvix":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","zuvix":{"extensions":["./stale.js"]}}\n',
        },
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"zuvix":{"extensions":["./src/index.ts","./src/extra.ts"]}}\n',
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_SRC]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"zuvix":{"extensions":["./src/index.ts"]}}\n',
        },
        buildPaths: [
          ROOT_SRC,
          EXTENSION_SRC,
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_PACKAGE]: '{"zuvix":{"extensions":["./src/index.js"]}}\n',
          [DIST_EXTENSION_RUNTIME_SRC]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"zuvix":{"extensions":["./src/index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_EXTENSION_PACKAGE]: '{"zuvix":{"extensions":["./index.js"]}}\n',
          [DIST_RUNTIME_EXTENSION_INDEX]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"zuvix":{"extensions":["./index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
  ...
  it("does not require Zuvix SDK alias outputs when dist extensions are absent", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("reports missing Zuvix SDK alias outputs when runtime stamps match HEAD", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          ...
          [DIST_ZUVIX_ALIAS_PACKAGE]:
            '{"name":"zuvix","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
          [DIST_ZUVIX_ALIAS_PLUGIN_SDK_INDEX]:
      ...
      await fs.rm(resolvePath(tmp, DIST_ZUVIX_ALIAS_PLUGIN_SDK_INDEX));
  ...
  it("does not require private Zuvix SDK dist files that package exports omit", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          ...
          [DIST_ZUVIX_ALIAS_PACKAGE]:
            '{"name":"zuvix","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js","./plugin-sdk/string-coerce-runtime":"./plugin-sdk/string-coerce-runtime.js"}}\n',
          [DIST_ZUVIX_ALIAS_PLUGIN_SDK_INDEX]:
            "export * from '../../../../plugin-sdk/index.js';\n",
          [DIST_ZUVIX_ALIAS_PLUGIN_SDK_STRING_COERCE]:
    ...
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [DIFFS_PACKAGE]:
            '{"zuvix":{"build":{"staticAssets":[{"source":"./assets/viewer-runtime.js","output":"assets/viewer-runtime.js"}]}}}\n',
  ...
  it("does not require static asset outputs when runtime static assets are disabled", async () => {
    await withTempDir({ prefix: "zuvix-run-node-" }, async (tmp) => {
      ...
      const requirement = resolveRuntimePostBuildRequirement(
        createBuildRequirementDeps(tmp, {
          env: { ZUVIX_RUNTIME_POSTBUILD_STATIC_ASSETS: "0" },
          gitHead: "abc123\n",
          gitStatus: "",
        }),
      );
