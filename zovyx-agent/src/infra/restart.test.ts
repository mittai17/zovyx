...
const { triggerZuvixRestart } = await import("./restart.js");
...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-zuvix/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "czuvix", `p${stalePidB}`, "czuvix-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerZuvixRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    setPlatform("darwin");
    withEnv(
      { VITEST: undefined, NODE_ENV: undefined, HOME: "/Users/test", ZUVIX_PROFILE: "default" },
      () => {
        ...
        const result = triggerZuvixRestart();
        ...
        expect(result).toEqual({
          ...
          tried: [
            ...
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.zuvix.gateway.plist`,
          ],
        });
      },
    );
      ...
      () => {
        ...
        expect(result).toEqual({
          ...
          tried: [
            `launchctl kickstart -k gui/${uid}/ai.zuvix.gateway`,
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.zuvix.gateway.plist`,
            `launchctl kickstart gui/${uid}/ai.zuvix.gateway`,
          ],
        });
      },
    );
