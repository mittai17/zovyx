...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "zuvix.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Zuvix)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "zuvix.internal", expected: "zuvix.internal." },
    { value: "zuvix.internal.", expected: "zuvix.internal." },
    { value: "  zuvix.internal  ", expected: "zuvix.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { ZUVIX_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { ZUVIX_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { ZUVIX_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("zuvix.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "zuvix.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("zuvix.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN zuvix.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_zuvix-gw._tcp IN PTR studio-london._zuvix-gw._tcp`,
      `studio-london._zuvix-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Zuvix)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/zuvix`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN zuvix.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._zuvix-gw._tcp IN TXT`,
        `displayName=Mac Studio (Zuvix)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("zuvix.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("zuvix.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
