// Builds PATH values for Zuvix child processes.
...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current Zuvix process is on.
  try {
  ...
  // Bundled macOS app: `zuvix` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "zuvix");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ZUVIX_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "zuvix"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `zuvix`
...
/**
 * Best-effort PATH bootstrap so skills that require the `zuvix` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureZuvixCliOnPath(opts: EnsureZuvixPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ZUVIX_PATH_BOOTSTRAPPED)) {
    return;
  }
  // Mark before filesystem probing so repeated calls from nested bootstraps do
  // not keep reshuffling PATH.
  process.env.ZUVIX_PATH_BOOTSTRAPPED = "1";
