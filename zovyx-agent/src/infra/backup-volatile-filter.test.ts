...
const stateDir = "/opt/zuvix/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/zuvix/state/sessions/../config.jsonl` resolves to
    // `/opt/zuvix/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\zuvix\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/zuvix/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
