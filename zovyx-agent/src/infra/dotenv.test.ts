...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "ZUVIX_LIVE_ANTHROPIC_KEYS",
  "ZUVIX_LIVE_GEMINI_KEY",
  "ZUVIX_LIVE_OPENAI_KEY",
  "ZUVIX_GATEWAY_TOKEN",
  "ZUVIX_GATEWAY_PASSWORD",
  "ZUVIX_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "ZUVIX_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "ZUVIX_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "ZUVIX_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "ZUVIX_BUNDLED_SKILLS_DIR=./attacker-skills",
  "ZUVIX_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  process.env.ZUVIX_STATE_DIR = stateDir;
...
describe("loadDotEnv", () => {
  it("loads ~/.zuvix/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.zuvix/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".zuvix");
        process.env.ZUVIX_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "zuvix", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ZUVIX_STATE_DIR=./evil-state",
            "ZUVIX_CONFIG_PATH=./evil-config.json",
        ...
        delete process.env.ZUVIX_CONFIG_PATH;
        ...
        expect(process.env.ZUVIX_STATE_DIR).toBe(stateDir);
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ZUVIX_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "ZUVIX_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "ZUVIX_LIVE_GEMINI_KEY=sk-gemini-live",
            "ZUVIX_LIVE_OPENAI_KEY=sk-openai-live",
            "ZUVIX_GATEWAY_TOKEN=attacker-token",
            "ZUVIX_GATEWAY_PASSWORD=attacker-password",
            "ZUVIX_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks state-directory controls from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.ZUVIX_STATE_DIR).toBeUndefined();
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ZUVIX_AGENT_DIR=./evil-agent",
            `ZUVIX_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "ZUVIX_OAUTH_DIR=./evil-oauth",
            "PI_CODING_AGENT_DIR=./evil-pi-agent",
          ].join("\n"),
        );

        delete process.env.ZUVIX_AGENT_DIR;
        delete process.env.ZUVIX_BUNDLED_PLUGINS_DIR;
        ...
        expect(process.env.ZUVIX_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.ZUVIX_OAUTH_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
      });
    });
  });

  it("blocks ZUVIX_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "ZUVIX_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.ZUVIX_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ZUVIX_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            'ZUVIX_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.ZUVIX_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.ZUVIX_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ZUVIX_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.ZUVIX_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ZUVIX_PINNED_PYTHON=./attacker-python",
            "ZUVIX_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.ZUVIX_PINNED_PYTHON;
        delete process.env.ZUVIX_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ZUVIX_PINNED_PYTHON).toBeUndefined();
        expect(process.env.ZUVIX_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ZUVIX_PINNED_PYTHON=/trusted/python",
            "ZUVIX_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.ZUVIX_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.ZUVIX_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ZUVIX_LIVE_GEMINI_KEY=sk-gemini-live",
            "ZUVIX_LIVE_OPENAI_KEY=sk-openai-live",
            "ZUVIX_GATEWAY_TOKEN=trusted-token",
            "ZUVIX_GATEWAY_PASSWORD=trusted-password",
            "ZUVIX_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.ZUVIX_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.ZUVIX_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.ZUVIX_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.ZUVIX_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.ZUVIX_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.ZUVIX_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.ZUVIX_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "ZUVIX_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks ZUVIX_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "ZUVIX_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when ZUVIX_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.ZUVIX_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "zuvix", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.ZUVIX_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        const plugin: PluginManifestRecord = {
          ...
          manifestPath: "/plugins/runtime-cloud/zuvix.plugin.json",
          providerAuthEnvVars: {
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "ZUVIX_WORKSPACE_DIR",
          "ZUVIX_MDNS_HOSTNAME",
          "ZUVIX_SESSION_CACHE_TTL_MS",
          "ZUVIX_UPDATE_PACKAGE_SPEC",
          "ZUVIX_GATEWAY_PORT",
          "ZUVIX_GATEWAY_URL",
          "ZUVIX_CLAWHUB_URL",
          "CLAWHUB_URL",
          ...
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
          "ZUVIX_DISABLE_BUNDLED_PLUGINS",
          "ZUVIX_ALLOW_INSECURE_PRIVATE_WS",
          "ZUVIX_BROWSER_EXECUTABLE_PATH",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "ZUVIX_SKIP_CHANNELS",
          "ZUVIX_SKIP_PROVIDERS",
          "ZUVIX_SKIP_CRON",
          "ZUVIX_RAW_STREAM",
          "ZUVIX_RAW_STREAM_PATH",
          "ZUVIX_CACHE_TRACE",
          "ZUVIX_CACHE_TRACE_FILE",
          "ZUVIX_CACHE_TRACE_MESSAGES",
          "ZUVIX_CACHE_TRACE_PROMPT",
          "ZUVIX_CACHE_TRACE_SYSTEM",
          "ZUVIX_SHOW_SECRETS",
          "ZUVIX_PLUGIN_CATALOG_PATHS",
          "ZUVIX_MPM_CATALOG_PATHS",
          "ZUVIX_NODE_EXEC_HOST",
          "ZUVIX_NODE_EXEC_FALLBACK",
          "ZUVIX_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("zuvix/zuvix");
