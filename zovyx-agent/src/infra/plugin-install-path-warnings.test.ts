...
const MATRIX_REPO_INSTALL_COMMAND = `zuvix plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/zuvix-matrix-missing",
        installPath: "/tmp/zuvix-matrix-missing",
      },
    });
    ...
    expect(
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/zuvix-matrix-missing",
      'Reinstall with "zuvix plugins install @zuvix/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/zuvix-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<zuvix plugins install @zuvix/matrix>" when you want to return to the standard Matrix plugin.',
