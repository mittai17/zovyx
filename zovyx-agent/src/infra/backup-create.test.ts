...
import { closeZuvixAgentDatabasesForTest } from "../state/zuvix-agent-db.js";
import {
  closeZuvixStateDatabase,
  openZuvixStateDatabase,
} from "../state/zuvix-state-db.js";
import { withZuvixTestState } from "../test-utils/zuvix-test-state.js";
import {
...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "zuvix-backup-2026-01-01",
    archivePath: "/tmp/zuvix-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/zuvix-backup.tar.gz";
  ...
  it.each([
    {
      ...
      expected: [
        ...
        "- state: ~/.zuvix",
        "Skipped 1 path:",
        "- workspace: ~/Projects/zuvix (covered by ~/.zuvix)",
        "Created /tmp/zuvix-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.zuvix/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.zuvix/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.zuvix/config.json",
        "- credentials: ~/.zuvix/oauth",
  ...
  it("surfaces the volatile skip count in the summary", () => {
    expect(
      ...
      "Backup archive: /tmp/zuvix-backup.tar.gz",
      "Included 1 path:",
      "- state: ~/.zuvix",
      "Created /tmp/zuvix-backup.tar.gz",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/zuvix.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.zuvix\\");

    expect(filter(String.raw`C:\Users\me\.zuvix\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.zuvix\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  it("falls back when injected nowMs is outside Date range", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-backup-invalid-now-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        try {
          ...
          expect(path.basename(result.archivePath)).toContain("zuvix-backup.tar.gz");
  ...
  it("falls back to epoch when injected nowMs and Date.now are outside Date range", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-backup-invalid-fallback-now-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("scrubs transient SQLite delivery queue rows from archive snapshots", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-backup-sqlite-queue-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const { db } = openZuvixStateDatabase({ env: state.env });
        db.prepare(
          `
        ...
        try {
          ...
          const archivedDbEntry = entries.find((entry) =>
            entry.endsWith("/state/state/zuvix.sqlite"),
          );
          expect(archivedDbEntry).toBeDefined();
          expect(entries.some((entry) => entry.endsWith("/state/state/zuvix.sqlite-wal"))).toBe(
            false,
          );
  ...
  it("snapshots per-agent SQLite auth stores into the archive", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-backup-agent-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const entries = await listArchiveEntries(result.archivePath);
        const archivedDbEntry = entries.find((entry) =>
          entry.endsWith("/state/agents/main/agent/zuvix-agent.sqlite"),
        );
        expect(archivedDbEntry).toBeDefined();
        expect(
          entries.some((entry) =>
            entry.endsWith("/state/agents/main/agent/zuvix-agent.sqlite-wal"),
          ),
        ).toBe(false);
  ...
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "zuvix.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(entrySuffixes).toContain("/state/extensions/demo/zuvix.plugin.json");
  ...
  it("dereferences hardlinks instead of emitting restore-hostile Link entries", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-backup-hardlink-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
        const outputDir = state.path("backups");
        const sourcePath = path.join(stateDir, "workspace-adx", "zuvix-src", "node_modules");
  ...
  it("does not duplicate the root manifest when the system tempdir lives inside the state dir", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-backup-tmp-overlap-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
  ...
  it("does not duplicate the root manifest when the system tempdir is the state dir itself", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-backup-tmp-equals-state-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
