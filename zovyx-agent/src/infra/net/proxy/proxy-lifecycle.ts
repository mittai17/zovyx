...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    ZUVIX_PROXY_ACTIVE: process.env["ZUVIX_PROXY_ACTIVE"],
    ZUVIX_PROXY_LOOPBACK_MODE: process.env["ZUVIX_PROXY_LOOPBACK_MODE"],
    ZUVIX_PROXY_CA_FILE: process.env["ZUVIX_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["ZUVIX_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["ZUVIX_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["ZUVIX_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["ZUVIX_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or ZUVIX_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["ZUVIX_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["ZUVIX_PROXY_CA_FILE"],
  });
 ...
 * loopback CDP probe to a Chromium instance Zuvix spawned itself.
 *
