// Bridges Zuvix-managed proxy TLS trust into Undici EnvHttpProxyAgent and
// explicit ProxyAgent options without changing unrelated operator proxies.
import { isRecord as isProxyTlsRecord } from "@zuvix/normalization-core/record-coerce";
...
/** Resolves managed proxy TLS trust only when the target proxy is Zuvix's active proxy. */
export function resolveActiveManagedProxyTlsOptions(
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    ...
    caFileOverride: env["ZUVIX_PROXY_CA_FILE"],
  });
  try {
