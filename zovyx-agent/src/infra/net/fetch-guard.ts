...
const ZUVIX_DEBUG_PROXY_ENABLED = "ZUVIX_DEBUG_PROXY_ENABLED";
...
function isManagedProxyActive(): boolean {
  return process.env["ZUVIX_PROXY_ACTIVE"] === "1";
}
...
async function captureGuardedFetchExchange(params: {
  ...
  if (params.capture === false || !isTruthyEnvValue(process.env[ZUVIX_DEBUG_PROXY_ENABLED])) {
    return;
  }
