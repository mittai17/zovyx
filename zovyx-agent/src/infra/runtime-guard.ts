// Validates the current runtime against Zuvix's Node engine floor.
...
/** Returns whether a detected runtime meets Zuvix's minimum runtime contract. */
...
/** Checks a Node version label against Zuvix's current minimum Node version. */
...
export function assertSupportedRuntime(
  ...
  runtime.error(
    [
      "zuvix requires Node >=22.19.0.",
      `Detected: ${runtimeLabel} (exec: ${execLabel}).`,
      `PATH searched: ${details.pathEnv}`,
      "Install Node: https://nodejs.org/en/download",
      "Upgrade Node and re-run zuvix.",
    ].join("\n"),
  );
  runtime.exit(1);
}
