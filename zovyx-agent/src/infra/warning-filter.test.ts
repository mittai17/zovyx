...
const warningFilterKey = Symbol.for("zuvix.warning-filter");
...
describe("warning filter", () => {
  ...
  it("installs once and suppresses known warnings at emit time", async () => {
    ...
    try {
      ...
      emitWarning("Visible warning", { type: "Warning", code: "ZUVIX_TEST_WARNING" });
      emitWarning(
        Object.assign(new Error("The punycode module is deprecated."), {
          name: "DeprecationWarning",
          code: "DEP0040",
        }),
        { type: "Warning", code: "ZUVIX_VISIBLE_OVERRIDE" },
      );
      await flushWarnings();
      expect(
        seenWarnings.find((warning) => warning.code === "ZUVIX_TEST_WARNING"),
      ).toStrictEqual({
        code: "ZUVIX_TEST_WARNING",
        name: "Warning",
