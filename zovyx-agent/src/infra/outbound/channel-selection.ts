...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `zuvix agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: ZuvixConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: zuvix doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run zuvix channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use zuvix channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: ZuvixConfig): Promise<boolean> {
