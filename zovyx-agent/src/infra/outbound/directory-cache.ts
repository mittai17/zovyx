...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ZuvixConfig | null = null;
   ...
   */
  get(key: string, cfg: ZuvixConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: ZuvixConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: ZuvixConfig): void {
