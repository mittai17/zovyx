...
async function withTempZuvixStateDir<T>(test: (stateDir: string) => Promise<T>): Promise<T> {
...
describe("runMessageAction media behavior", () => {
  ...
  it("materializes buffer-only send attachments into outbound media paths", async () => {
    ...
    await withTempZuvixStateDir(async () => {
  ...
  it("does not stage buffer-only send attachments before target validation passes", async () => {
    ...
    await withTempZuvixStateDir(async (stateDir) => {
      await expect(
        runMessageAction({
          cfg: workspaceConfig,
          action: "send",
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before zuvix#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Zuvix tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideZuvix = path.join(os.tmpdir(), "outside-zuvix", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
