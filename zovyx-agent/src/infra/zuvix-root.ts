// Resolves the Zuvix package root from runtime and package metadata.
import path from "node:path";
import { fileURLToPath } from "node:url";
import { zuvixRootFs, zuvixRootFsSync } from "./zuvix-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["zuvix"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await zuvixRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(zuvixRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = zuvixRootFsSync.realpathSync(normalized);
...
export async function resolveZuvixPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveZuvixPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
...
export const testing = {
  clearZuvixPackageRootCaches(): void {
