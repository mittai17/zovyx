// Covers Zuvix's default fs-safe Python helper configuration.
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.ZUVIX_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in Zuvix", async () => {
  ...
  it("honors the Zuvix-specific env mode override", async () => {
    process.env.ZUVIX_FS_SAFE_PYTHON_MODE = "auto";
