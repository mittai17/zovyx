// Applies persisted state migrations across Zuvix config files.
...
import type { DB as ZuvixStateKyselyDatabase } from "../state/zuvix-state-db.generated.js";
import {
  detectZuvixStateDatabaseSchemaMigrations,
...
type LegacyPluginStateImportDatabase = Pick<ZuvixStateKyselyDatabase, "plugin_state_entries">;
...
function resolveDefaultExecApprovalsStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env, homedir), ".zuvix");
}
...
function detectLegacyExecApprovalsMigration(params: {
  ...
  return {
    ...
    targetPath,
    hasLegacy:
      Boolean(params.env.ZUVIX_STATE_DIR?.trim()) &&
...
async function migrateLegacyTaskRunsSidecar(params: {
  ...
  try {
    ...
    runZuvixStateWriteTransaction(
      ...
      { env: { ...process.env, ZUVIX_STATE_DIR: params.stateDir } },
    );
    if (importedTasks > 0) {
      changes.push(
...
async function withPluginStateImportEnv<T>(
  ...
  const previous = process.env.ZUVIX_STATE_DIR;
  process.env.ZUVIX_STATE_DIR = plan.stateDir;
  try {
    return await run();
  } finally {
    if (previous === undefined) {
      ...
      process.env.ZUVIX_STATE_DIR = previous;
    }
  }
}
...
export async function autoMigrateLegacyStateDir(params: {
  ...
  const warnings: string[] = [];
  const changes: string[] = [];
  const hasCustomStateDir = Boolean(env.ZUVIX_STATE_DIR?.trim());
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set ZUVIX_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function detectLegacyStateMigrations(params: {
  ...
  const stateSchemaMigrations = detectZuvixStateDatabaseSchemaMigrations({
    env: { ...env, ZUVIX_STATE_DIR: stateDir },
  });
...
async function migrateLegacySessions(
  ...
  if (!targetReadable) {
    if (options.recoverCorruptTargetStore) {
      ...
      warnings.push(
        `Target sessions store unreadable; left untouched to avoid overwriting at ${detected.sessions.targetStorePath}. Run zuvix doctor --fix to archive it and retry the legacy merge.`,
      );
    }
  }
...
function migrateLegacyStateSchema(detected: LegacyStateDetection): {
  ...
  return repairZuvixStateDatabaseSchema({
    env: { ...process.env, ZUVIX_STATE_DIR: detected.stateDir },
  });
}
...
export async function autoMigrateLegacyState(params: {
  ...
  const stateSchema = repairZuvixStateDatabaseSchema({
    env: { ...env, ZUVIX_STATE_DIR: stateDir },
  });
  ...
  const hasCustomAgentDir = env.ZUVIX_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
