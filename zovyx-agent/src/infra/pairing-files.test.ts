...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/zuvix-state", "devices")).toEqual({
      dir: path.join("/tmp/zuvix-state", "devices"),
      pendingPath: path.join("/tmp/zuvix-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/zuvix-state", "devices", "paired.json"),
    });
  });
