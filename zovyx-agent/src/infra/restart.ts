...
export function emitGatewayRestart(
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerZuvixRestart() uses schtasks to restart the gateway.
...
export function triggerZuvixRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.ZUVIX_SYSTEMD_UNIT,
      process.env.ZUVIX_PROFILE,
    );
  ...
  const label =
    process.env.ZUVIX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ZUVIX_PROFILE);
