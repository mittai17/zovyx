...
function listAgents(cfg: ZuvixConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: ZuvixConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<ZuvixConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: ZuvixConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: ZuvixConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ZuvixConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<ZuvixConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  ZuvixConfig,
  {
    bindingsRef: ZuvixConfig["bindings"];
    agentsRef: ZuvixConfig["agents"];
    sessionRef: ZuvixConfig["session"];
...
function resolveRouteCacheForConfig(cfg: ZuvixConfig): Map<string, ResolvedAgentRoute> {
