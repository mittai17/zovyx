...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers ZUVIX_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.ZUVIX_GATEWAY_TOKEN;
    process.env.ZUVIX_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("keeps ZUVIX_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.ZUVIX_GATEWAY_TOKEN;
    process.env.ZUVIX_GATEWAY_TOKEN = "advanced-env-token";
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.ZUVIX_GATEWAY_PASSWORD;
    process.env.ZUVIX_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "ZUVIX_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      expect(result.nextConfig.gateway?.auth?.password).toEqual({
        source: "env",
        provider: "default",
        id: "ZUVIX_GATEWAY_PASSWORD",
      });
    } finally {
      if (previous === undefined) {
        delete process.env.ZUVIX_GATEWAY_PASSWORD;
      } else {
        process.env.ZUVIX_GATEWAY_PASSWORD = previous;
      }
    }
  });

  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.ZUVIX_GATEWAY_TOKEN;
    process.env.ZUVIX_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["18789", "ZUVIX_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.ZUVIX_GATEWAY_TOKEN = previous;
      }
    }
  });
