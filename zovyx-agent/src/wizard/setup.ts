// Setup wizard orchestrates onboarding prompts and generated Zuvix config.
...
async function writeWizardConfigFile(
  configInput: ZuvixConfig,
  opts: {
    allowConfigSizeDrop?: boolean;
    migrationBaseConfig?: ZuvixConfig;
...
export async function runSetupWizard(
  ...
  let baseConfig: ZuvixConfig = snapshot.valid
    ? snapshot.exists
  ...
  let pendingPluginInstallMigrationBaseConfig: ZuvixConfig | undefined = baseConfig;
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("zuvix doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("zuvix doctor")}`,
        `Inspect: ${formatCliCommand("zuvix plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("zuvix configure"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: zuvix onboard --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
  ...
  let localGatewayToken = process.env.ZUVIX_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.ZUVIX_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: ZuvixConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
