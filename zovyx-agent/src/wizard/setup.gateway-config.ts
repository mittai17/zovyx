...
import type { GatewayBindMode, GatewayTailscaleMode, ZuvixConfig } from "../config/config.js";
    ...
    if (tokenMode === "ref") {
      if (flow === "quickstart" && quickstartTokenRef) {
        ...
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "ZUVIX_GATEWAY_TOKEN",
          copy: {
            sourceMessage: t("wizard.gateway.authTokenStoredMessage"),
            envVarPlaceholder: "ZUVIX_GATEWAY_TOKEN",
          },
        });
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.ZUVIX_GATEWAY_TOKEN)) ||
        randomToken();
    ...
    if (!password) {
      ...
      if (selectedMode === "ref") {
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "ZUVIX_GATEWAY_PASSWORD",
          copy: {
            sourceMessage: t("wizard.gateway.authPasswordStoredMessage"),
            envVarPlaceholder: "ZUVIX_GATEWAY_PASSWORD",
          },
        });
