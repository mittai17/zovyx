...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    if (gatewayProbe.ok) {
      try {
        const healthConfig: ZuvixConfig =
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.startGatewayNow", {
            command: formatCliCommand("zuvix gateway run"),
          }),
          t("wizard.finalize.rerunInstallDaemon", {
            command: formatCliCommand("zuvix onboard --install-daemon"),
          }),
          t("wizard.finalize.skipHealthNextTime", {
            command: formatCliCommand("zuvix onboard --skip-health"),
          }),
        ].join("\n"),
        "Gateway",
      );
    ...
    if (gatewayProbe.ok) {
      const tokenNotes = [
        ...
        t("wizard.finalize.gatewayTokenView", {
          command: formatCliCommand("zuvix config get gateway.auth.token"),
        }),
        t("wizard.finalize.gatewayTokenGenerate", {
          command: formatCliCommand("zuvix doctor --generate-gateway-token"),
        }),
        suppressGatewayTokenOutput ? undefined : t("wizard.finalize.dashboardTokenMemory"),
        t("wizard.finalize.dashboardOpenAnytime", {
          command: formatCliCommand("zuvix dashboard --no-open"),
        }),
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("zuvix configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.webSearchGetKey", {
            url: entry?.signupUrl ?? "https://docs.zuvix.ai/tools/web",
          }),
          t("wizard.finalize.webDocs"),
        ].join("\n"),
        t("wizard.finalize.webSearchTitle"),
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.webSearchReenable", {
            command: formatCliCommand("zuvix configure --section web"),
          }),
          "",
          t("wizard.finalize.webDocs"),
        ].join("\n"),
