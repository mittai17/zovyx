...
function hasMeaningfulConfig(config: ZuvixConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.ZUVIX_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh Zuvix setup.",
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: ZuvixConfig) => Promise<ZuvixConfig>;
}): Promise<void> {
  const [
  ...
  // can reference a concrete Zuvix config target.
