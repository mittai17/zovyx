...
type TaskFlowRegistryTestDatabase = Pick<ZuvixStateKyselyDatabase, "flow_runs">;
...
async function withFlowRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withZuvixTestState(
    {
      layout: "state-only",
      prefix: "zuvix-task-flow-store-",
    },
    async (state) => {
      const root = state.stateDir;
      resetTaskFlowRegistryForTests();
      try {
        return await withEnvAsync({ ZUVIX_STATE_DIR: root }, async () => await run(root));
      } finally {
        resetTaskFlowRegistryForTests();
      }
    },
