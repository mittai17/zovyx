// Persists managed task-flow records through the Zuvix SQLite state database.
...
type FlowRunsTable = ZuvixStateKyselyDatabase["flow_runs"];
type FlowRegistryStoreDatabase = Pick<ZuvixStateKyselyDatabase, "flow_runs">;
...
// SQLite-backed task-flow store mirrors the in-process registry into zuvix-state.db.
...
function pruneFlowsNotInSnapshot(params: { db: DatabaseSync; ids: readonly string[] }) {
  const tempTableName = "zuvix_live_flow_ids";
