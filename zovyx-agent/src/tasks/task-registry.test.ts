...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronJobsStorePath: () => "/tmp/zuvix-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/zuvix-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "zuvix-task-registry-" }, async (root) => {
    return await withEnvAsync({ ZUVIX_STATE_DIR: root }, async () => {
...
describe("task-registry", () => {
  ...
  it("cancels childless codex-native tasks without routing through Zuvix subagent sessions", async () => {
    await withTaskRegistryTempDir(async () => {
      resetTaskRegistryForTests();
      const task = createTaskRecord({
        runtime: "subagent",
