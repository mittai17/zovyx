...
vi.mock("../../agents/acp-spawn.js", () => ({
  resolveAcpSpawnRuntimePolicyError: (params: { cfg?: ZuvixConfig }) =>
...
function createDiscordParams(commandBody: string, cfg: ZuvixConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: ZuvixConfig = baseCfg) {
...
async function runDiscordAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
...
async function runTelegramDmAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runSlackDmAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
function createMatrixThreadParams(commandBody: string, cfg: ZuvixConfig = baseCfg) {
...
async function runMatrixAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runMatrixThreadAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(createMatrixThreadParams(commandBody, cfg), true);
}

async function runFeishuDmAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runLineDmAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runIMessageDmAcpCommand(commandBody: string, cfg: ZuvixConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
describe("/acp command", () => {
  ...
  it("inherits the target agent workspace when /acp spawn omits --cwd", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-codex-"));
    try {
      const cfg = {
        ...baseCfg,
        agents: {
  ...
  it("sanitizes leaked task and runtime details in ACP status output", async () => {
    ...
    expect(result?.reply?.text).not.toContain("Zuvix runtime context (internal):");
