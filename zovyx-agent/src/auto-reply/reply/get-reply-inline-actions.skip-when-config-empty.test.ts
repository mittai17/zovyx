...
vi.mock("../../agents/zuvix-tools.runtime.js", () => ({
  createZuvixTools: (...args: unknown[]) => createZuvixToolsMock(...args),
}));
...
function mockToolDispatchedSkillCommand() {
  ...
  createZuvixToolsMock.mockReturnValue([
    {
      name: "send_status",
      execute: toolExecute,
    },
...
describe("handleInlineActions", () => {
  beforeEach(() => {
    ...
    createZuvixToolsMock.mockReset();
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(
      mockObjectArg(createZuvixToolsMock, "createZuvixTools").requesterAgentIdOverride,
    ).toBe("named-worker");
  ...
  it("passes sender identity into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    const toolsArgs = mockObjectArg(createZuvixToolsMock, "createZuvixTools");
  ...
  it("applies subagent policy to ACP envelope inline dispatch sessions", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-inline-acp-policy-"));
    try {
  ...
  it("passes sandboxed runtime state into inline tool construction", async () => {
    ...
    expect(createZuvixToolsMock).toHaveBeenCalledWith(
