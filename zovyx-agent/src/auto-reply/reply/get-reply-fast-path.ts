...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("zuvix.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("zuvix.reply.full-runtime");

type ReplyConfigWithMarker = ZuvixConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.ZUVIX_ALLOW_SLOW_REPLY_TESTS === "1" || env.ZUVIX_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends ZuvixConfig>(
  config: T,
...
export function withFastReplyConfig<T extends ZuvixConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends ZuvixConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is ZuvixConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set ZUVIX_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as ZuvixConfig;
}
