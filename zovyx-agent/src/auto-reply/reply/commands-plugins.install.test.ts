...
const workspaceHarness = createCommandWorkspaceHarness("zuvix-command-plugins-install-");
...
function expectPersistedInstall(pluginId: string, expectedInstall: Record<string, unknown>): void {
  ...
  expectObjectFields(persisted.snapshot, {
    writeOptions: expect.objectContaining({
      ...
      expectedConfigPath: expect.stringContaining("zuvix.json"),
      ownedConfigPathForWrite: expect.stringContaining("zuvix.json"),
    }),
  });
...
describe("handleCommands /plugins install", () => {
  ...
  it("installs a plugin from a local path", async () => {
    ...
    await withTempHome("zuvix-command-plugins-home-", async () => {
  ...
  it("installs from an explicit clawhub: spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.2.3",
      extensions: ["index.js"],
      packageName: "@zuvix/clawhub-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@zuvix/clawhub-demo",
    ...
    await withTempHome("zuvix-command-plugins-home-", async () => {
      ...
      const params = buildPluginsParams(
        "/plugins install clawhub:@zuvix/clawhub-demo@1.2.3",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@zuvix/clawhub-demo@1.2.3",
      );
      expectPersistedInstall("clawhub-demo", {
        source: "clawhub",
        spec: "clawhub:@zuvix/clawhub-demo@1.2.3",
  ...
  it("refuses plugin installs in Nix mode before package installer side effects", async () => {
    const previousNixMode = process.env.ZUVIX_NIX_MODE;
    ...
    try {
      await withTempHome("zuvix-command-plugins-home-", async () => {
        ...
        expect(result.reply?.text).toContain("ZUVIX_NIX_MODE=1");
        expect(result.reply?.text).toContain("nix-zuvix#quick-start");
      ...
      if (previousNixMode === undefined) {
        ...
        process.env.ZUVIX_NIX_MODE = previousNixMode;
      }
    }
  });

  it("refuses installs through a root include before package installer side effects", async () => {
    await withTempHome("zuvix-command-plugins-home-", async (home) => {
      const sharedConfigPath = path.join(home, ".zuvix", "shared.json5");
      await fs.writeFile(sharedConfigPath, `${JSON.stringify({ plugins: {} }, null, 2)}\n`);
      await fs.writeFile(
        path.join(home, ".zuvix", "zuvix.json"),
  ...
  it("treats /plugin add as an install alias", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.0.0",
      extensions: ["index.js"],
      packageName: "@zuvix/alias-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@zuvix/alias-demo",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.0.0",
    ...
    await withTempHome("zuvix-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugin add clawhub:@zuvix/alias-demo@1.0.0",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@zuvix/alias-demo@1.0.0",
      );
    });
  });
  ...
  it("trusts catalog npm package installs with alternate selectors", async () => {
    ...
    await withTempHome("zuvix-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install @wecom/wecom-zuvix-plugin@latest",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
