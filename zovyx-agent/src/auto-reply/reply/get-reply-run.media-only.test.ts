...
describe("runPreparedReply media-only handling", () => {
  ...
  it("does not persist turn-local thinking fallback over a stored session override", async () => {
    ...
    await runPreparedReply(
      baseParams({
        ...
        sessionEntry,
        sessionStore,
        storePath: "/tmp/zuvix-sessions.json",
        modelState: {
          resolveDefaultThinkingLevel: async () => "high",
          resolveThinkingCatalog: async () => [
            {
  ...
  it("hydrates current image MediaPaths by extension when MediaTypes are missing", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "zuvix-followup-image-"));
  ...
  it("runs room events as contextual events instead of direct user prompts", async () => {
    ...
    const call = requireLastRunReplyAgentCall();
    expect(call?.commandBody).toBe("[Zuvix room event]");
    expect(call?.transcriptCommandBody).toBe("");
    expect(call?.followupRun.prompt).toBe("[Zuvix room event]");
    expect(call?.followupRun.transcriptPrompt).toBe("");
    ...
    expect(call?.followupRun.currentInboundContext?.text).toContain("[Zuvix room event]");
    expect(call?.followupRun.currentInboundContext?.text).toContain(
  ...
  it("keeps heartbeat prompts out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[Zuvix heartbeat poll]");
    expect(call?.followupRun.transcriptPrompt).toBe("[Zuvix heartbeat poll]");
  });
  ...
  it.each([
    ...
    async (commandText, startupAction) => {
      ...
      expect(call?.transcriptCommandBody).toBe(`[Zuvix session ${startupAction}]`);
      expect(call?.followupRun.transcriptPrompt).toBe(`[Zuvix session ${startupAction}]`);
