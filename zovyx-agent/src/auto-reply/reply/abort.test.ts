...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop zuvix!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@zuvix_bot", { botUsername: "zuvix_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@zuvix_bot", { botUsername: "zuvix_bot" })).toBe(true);
