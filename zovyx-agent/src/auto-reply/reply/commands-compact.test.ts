...
function requireResolveSessionAgentIdCall(index = 0) {
  ...
  )?.[0] as { sessionKey?: string; config?: ZuvixConfig } | undefined;
  if (!call) {
...
function requireResolveAgentDirCall(index = 0) {
  const call = resolveAgentDirMock.mock.calls[index] as [ZuvixConfig, string] | undefined;
  if (!call) {
...
describe("handleCompactCommand", () => {
  beforeEach(() => {
    ...
    resolveAgentDirMock.mockImplementation(
      (_cfg: unknown, agentId: string) => `/tmp/workspace/.zuvix/agents/${agentId}/agent`,
    );
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const result = await handleCompactCommand(
      {
        ...buildCompactParams("/compact", {
          commands: { text: true },
          channels: { whatsapp: { allowFrom: ["*"] } },
          session: { store: "/tmp/zuvix-session-store.json" },
        } as ZuvixConfig),
        ctx: {
          Provider: "whatsapp",
          Surface: "whatsapp",
        ...
        agentDir: "/tmp/zuvix-agent-compact",
        sessionEntry: {
          sessionId: "session-1",
          updatedAt: Date.now(),
          groupId: "group-1",
    ...
    expect(call.agentDir).toBe("/tmp/zuvix-agent-compact");
