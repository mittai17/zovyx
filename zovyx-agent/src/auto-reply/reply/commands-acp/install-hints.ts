...
export function resolveAcpInstallCommandHint(cfg: ZuvixConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `zuvix plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^zuvix plugins install /u, "");
    ...
    return "zuvix plugins install acpx";
  }
