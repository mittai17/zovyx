...
import { parseStrictPositiveInteger } from "@zuvix/normalization-core/number-coercion";
import {
...
function buildRuntimeChoice(params: {
  ...
  return {
    ...
    description:
      id === "zuvix"
        ? "Use the built-in Zuvix runtime."
        : params.cli
...
export async function buildModelsProviderData(
  ...
  for (const binding of runtimeBindings) {
    ...
    addRuntimeChoice(choices, buildRuntimeChoice({ cfg, provider, runtime: "zuvix" }));
    addRuntimeChoice(
      choices,
      buildRuntimeChoice({
        cfg,
