...
const DIAGNOSTICS_DOCS_URL = "https://docs.zuvix.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "zuvix gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentZuvixCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
async function requestGatewayDiagnosticsExportApproval(
  ...
  try {
    ...
    const result = await execTool.execute("chat-diagnostics-gateway-export", {
      command,
      env: buildCurrentZuvixCliExecEnv(),
      security: "allowlist",
      ask: "always",
      background: true,
      timeout: timeoutSec,
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this Zuvix session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an Zuvix session file.",
    ) === true
  );
}
