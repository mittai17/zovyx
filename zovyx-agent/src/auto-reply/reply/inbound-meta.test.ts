...
describe("buildInboundMetaSystemPrompt", () => {
  it("includes stable routing fields and omits chat ids", () => {
    ...
    expect(payload["schema"]).toBe("zuvix.inbound_meta.v2");
...
describe("buildInboundUserContextPrefix", () => {
  ...
  it("does not treat group chats as direct based on sender id", () => {
    ...
    expect(conversationInfo["sender_id"]).toBe("zuvix-control-ui");
  ...
  it("renders chat window structured context as compact transcript text", () => {
              ...
              messages: [
                {
                  ...
                  media_path: "/home/user/.zuvix/media/inbound/sticker.webp",
    ...
    expect(text).not.toContain("/home/user/.zuvix/media/inbound/sticker.webp");
  ...
  it("includes inbound history media metadata without leaking paths or URLs", () => {
      ...
      InboundHistory: [
        {
          ...
          media: [
            {
              path: "/tmp/zuvix-secret-image.png",
    ...
    expect(text).not.toContain("/tmp/zuvix-secret-image.png");
