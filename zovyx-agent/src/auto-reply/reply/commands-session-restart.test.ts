...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(
    () =>
      "Recommended follow-up: run zuvix doctor --non-interactive in a terminal or approvals-capable Zuvix surface.",
  ),
  ...
  triggerZuvixRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerZuvixRestart: mocks.triggerZuvixRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerZuvixRestart.mockReset();
    mocks.triggerZuvixRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerZuvixRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerZuvixRestart).not.toHaveBeenCalled();
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerZuvixRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
