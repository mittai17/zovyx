...
import { asDateTimestampMs } from "@zuvix/normalization-core/number-coercion";
...
type AgentDefaults = NonNullable<ZuvixConfig["agents"]>["defaults"];
...
type RunPreparedReplyParams = {
  ...
  sessionCfg: ZuvixConfig["session"];
...
export async function runPreparedReply(
  ...
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.ZUVIX_TEST_FAST === "1",
  });
  ...
  const userTurnInput =
    userTurnTranscriptText !== undefined || userTurnMediaForPersistence.length > 0
      ? {
          ...
          // replayed as history. See: https://github.com/zuvix/zuvix/issues/3658
