...
const EXPORT_TRAJECTORY_DOCS_URL = "https://docs.zuvix.ai/tools/trajectory";
...
function buildTrajectoryExportExecRequest(
  ...
  return {
    argv: buildCurrentZuvixCliArgv(args),
    command: buildCurrentZuvixCliCommand(args),
    displayCommand: ["zuvix", ...args].join(" "),
    encodedRequest,
    request,
  };
}
