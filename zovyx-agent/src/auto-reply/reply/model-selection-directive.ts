...
function formatAddModelCommand(modelRef: string): string {
  return `zuvix config set agents.defaults.models '${JSON.stringify({ [modelRef]: {} })}' --strict-json --merge`;
}
...
function formatNotAllowedError(params: {
  ...
  if (rawRuntime && normalizeProviderId(rawRuntime) === "codex") {
    lines.push("If the Codex runtime is missing, run: zuvix plugins enable codex");
  }
  return lines.join("\n");
}
