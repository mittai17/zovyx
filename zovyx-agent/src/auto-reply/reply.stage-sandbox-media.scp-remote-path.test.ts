...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "zuvix"),
    sessionKey,
    remoteCacheDir: join(home, ".zuvix", "media", "remote-cache", slugifySessionKey(sessionKey)),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  ...
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("zuvix-triggers-", async (home) => {
