...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("Zuvix");
    ...
    expect(normalized).toContain("Runtime: Zuvix Default");
  ...
  it("shows the default Zuvix harness as the model runtime", () => {
    const text = buildStatusMessage({
      agent: {
        model: "openai/gpt-5.4",
      },
      sessionEntry: {
        sessionId: "zuvix-harness",
        updatedAt: 0,
        fastMode: true,
      },
      sessionKey: "agent:main:main",
    ...
    expect(normalized).toContain("Fast");
    expect(normalized).toContain("Runtime: Zuvix Default");
    expect(normalized).not.toContain("· zuvix");
  });
