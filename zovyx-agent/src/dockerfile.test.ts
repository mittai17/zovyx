...
import { BUNDLED_PLUGIN_ROOT_DIR } from "zuvix/plugin-sdk/test-fixtures";
...
describe("Dockerfile", () => {
  ...
  it("uses full bookworm for build stages and slim bookworm for runtime", async () => {
    ...
    expect(dockerfile).toContain(
      'ARG ZUVIX_NODE_BOOKWORM_IMAGE="docker.io/library/node:24-bookworm@sha256:8530f76a96d88820d288761f022e318970dda93d01536919fbc16076b7983e63"',
    );
    expect(dockerfile).toContain(
      'ARG ZUVIX_NODE_BOOKWORM_SLIM_IMAGE="docker.io/library/node:24-bookworm-slim@sha256:242549cd46785b480c832479a730f4f2a20865d61ea2e404fdb2a5c3d3b73ecf"',
    );
    expect(dockerfile).toContain(
      'ARG ZUVIX_BUN_IMAGE="docker.io/oven/bun:1.3.13@sha256:87416c977a612a204eb54ab9f3927023c2a3c971f4f345a01da08ea6262ae30e"',
    );
    expect(dockerfile).toContain("FROM ${ZUVIX_NODE_BOOKWORM_IMAGE} AS workspace-deps");
    expect(dockerfile).toContain("FROM ${ZUVIX_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${ZUVIX_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime");
    ...
    expect(dockerfile).not.toContain("ZUVIX_VARIANT");
  });
  ...
  it("installs CA certificates in the slim runtime stage", async () => {
    ...
    const runtimeIndex = collapsed.indexOf(
      "FROM ${ZUVIX_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime",
    );
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG ZUVIX_INSTALL_BROWSER");
  ...
  it("copies install workspace manifests before pnpm install", async () => {
    ...
    const extensionManifestIndex = dockerfile.indexOf(
      "COPY --from=workspace-deps /out/${ZUVIX_BUNDLED_PLUGIN_DIR}/ ./${ZUVIX_BUNDLED_PLUGIN_DIR}/",
    );
    ...
    expect(dockerfile).toContain(
      `if [ -f "/tmp/\${ZUVIX_BUNDLED_PLUGIN_DIR}/$ext/package.json" ]; then`,
    );
  ...
  it("does not let pnpm resync the full source workspace during Docker build scripts", async () => {
    ...
    const privateQaExportIndex = collapsed.indexOf(
      "export ZUVIX_BUILD_PRIVATE_QA=1 ZUVIX_ENABLE_PRIVATE_QA_CLI=1",
    );
  ...
  it("prunes runtime dependencies and omitted plugin packages after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG ZUVIX_EXTENSIONS");
    expect(dockerfile).toContain("ARG ZUVIX_BUNDLED_PLUGIN_DIR");
    expect(dockerfile).toContain(
      "Opt-in plugin dependencies at build time (space- or comma-separated directory names).",
    );
    expect(dockerfile).toContain(
      'Example: docker build --build-arg ZUVIX_EXTENSIONS="diagnostics-otel,matrix" .',
    );
    expect(dockerfile).toContain(
      "RUN --mount=type=cache,id=zuvix-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \\",
    );
    ...
    expect(dockerfile).toContain(
      'ZUVIX_EXTENSIONS="$ZUVIX_EXTENSIONS" ZUVIX_BUNDLED_PLUGIN_DIR="$ZUVIX_BUNDLED_PLUGIN_DIR" node scripts/prune-docker-plugin-dist.mjs',
    );
  ...
  it("keeps the Codex plugin in official Docker release images", async () => {
    ...
    const releaseKeepList = "ZUVIX_EXTENSIONS=diagnostics-otel,codex";

    expect(workflow.match(new RegExp(releaseKeepList, "g"))).toHaveLength(4);
    expect(workflow).not.toContain("ZUVIX_EXTENSIONS=diagnostics-otel\n");
  });
  ...
  it("publishes official Docker browser images with baked Chromium", async () => {
    ...
    expect(workflow).toContain("ZUVIX_INSTALL_BROWSER=1");
    ...
    expect(workflow).toContain("grep -q '^ARG ZUVIX_INSTALL_BROWSER' Dockerfile");
  ...
  it("smokes runtime workspace templates before Docker release manifests publish", async () => {
    ...
    expect(workflow).toContain('test -f "${temp_root}/home/.zuvix/workspace/HEARTBEAT.md"');
  });
  ...
  it("keeps only the runtime-assets prune proof in full release validation", async () => {
    ...
    expect(workflow).not.toContain('test -f "${temp_root}/home/.zuvix/workspace/HEARTBEAT.md"');
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    const dockerfile = collapseDockerContinuations(await readFile(dockerfilePath, "utf8"));
    expect(dockerfile).toContain(`ARG ZUVIX_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bZUVIX_BUNDLED_PLUGINS_DIR\b/m);
  });

  it("normalizes plugin and agent paths permissions in image layers", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    expect(dockerfile).toContain(
      "RUN for dir in /app/${ZUVIX_BUNDLED_PLUGIN_DIR} /app/.agent /app/.agents; do \\",
    );
    expect(dockerfile).toContain('find "$dir" -type d -exec chmod 755 {} +');
    expect(dockerfile).toContain('find "$dir" -type f -exec chmod 644 {} +');
  });

  it("Docker GPG fingerprint awk uses correct quoting for ZUVIX_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
  ...
  it("pre-creates named-volume mount points before switching to the node user", async () => {
    ...
    // before the leaf .config/zuvix dir (issue #85968).
    ...
    expect(dockerfile).not.toContain("mkdir -p /home/node/.zuvix");
    expect(dockerfile).toContain("/home/node/.zuvix/workspace");
    expect(dockerfile).toContain("/home/node/.config/zuvix");
    expect(dockerfile).toContain(
      "stat -c '%U:%G %a' /home/node/.zuvix | grep -qx 'node:node 700'",
    );
    expect(dockerfile).toContain(
      "stat -c '%U:%G %a' /home/node/.zuvix/workspace | grep -qx 'node:node 700'",
    );
    ...
    expect(dockerfile).toContain(
      "stat -c '%U:%G %a' /home/node/.config/zuvix | grep -qx 'node:node 700'",
    );
  });
});
