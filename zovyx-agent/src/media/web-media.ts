...
import { maxBytesForKind, type MediaKind } from "@zuvix/media-core/constants";
import { basenameFromAnyPath, extnameFromAnyPath } from "@zuvix/media-core/file-name";
import {
  detectMime,
  extensionForMime,
  getFileExtension,
...
async function isTrustedGeneratedHostReadHtmlPath(filePath: string | undefined): Promise<boolean> {
  ...
  const [resolvedFilePath, resolvedTmpRoot] = await Promise.all([
    ...
    realpath(resolvePreferredZuvixTmpDir()).catch(() => undefined),
  ]);
  return Boolean(
