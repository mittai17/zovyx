...
describe("local media roots", () => {
  function withStateDir<T>(stateDir: string, run: () => T): T {
    return withEnv({ ZUVIX_STATE_DIR: stateDir }, run);
  }
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "zuvix-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "zuvix-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "zuvix-flexible-media-roots-state"),
      cfg: {},
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "zuvix-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "zuvix-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "zuvix-messaging-read-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
  ...
  it("keeps the config-dir media cache root when state and config paths differ", () => {
    const stateDir = path.join("/tmp", "zuvix-legacy-state");
    const configDir = path.join("/tmp", "zuvix-current-config");
