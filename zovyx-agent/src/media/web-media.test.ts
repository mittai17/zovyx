...
const CANVAS_HOST_PATH = "/__zuvix__/canvas";
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredZuvixTmpDir(), "web-media-core-"));
...
describe("loadWebMedia", () => {
  ...
  it("resolves home-relative local media paths through allowed local roots", async () => {
    await withEnvAsync({ ZUVIX_HOME: fixtureRoot }, async () => {
  ...
  it("allows trusted generated host-read HTML reports under Zuvix temp root", async () => {
  ...
  it("rejects host-read HTML files outside the trusted Zuvix temp root", async () => {
  ...
  it("rejects trusted host-read HTML symlinks that resolve outside Zuvix temp root", async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), "web-media-host-html-"));
  ...
  it("rejects trusted host-read HTML hardlinks to files outside Zuvix temp root", async () => {
    const outsideRoot = await fs.mkdtemp(
      path.join(path.dirname(resolvePreferredZuvixTmpDir()), "web-media-host-html-"),
    );
    const outsideHtml = path.join(outsideRoot, "report.html");
