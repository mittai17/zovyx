...
import { canonicalizeBase64, estimateBase64DecodedBytes } from "@zuvix/media-core/base64";
import { parseMediaContentLength } from "@zuvix/media-core/content-length";
...
export async function fetchWithGuard(params: {
  ...
  const { response, release } = await fetchWithSsrFGuard({
    ...
    init: { headers: { "User-Agent": "Zuvix-Gateway/1.0" } },
  });
