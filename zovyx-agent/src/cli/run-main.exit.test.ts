...
vi.mock("../infra/path-env.js", () => ({
  ensureZuvixCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.ZUVIX_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.ZUVIX_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    await runCli(["node", "zuvix", "status"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "zuvix", "status"]);
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "zuvix", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "zuvix", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "zuvix", "agent", "--local"]);
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "zuvix", "config"]);

    expect(createCliProgressMock).toHaveBeenCalledWith({
      label: "Loading Zuvix CLI…",
      indeterminate: true,
      delayMs: 0,
    });
  ...
  it("suppresses startup progress for json output commands before full CLI parsing", async () => {
    ...
    await runCli(["node", "zuvix", "sessions", "--json", "--limit", "all"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "zuvix", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "zuvix", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "zuvix", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "zuvix", "gateway", "--force"],
    });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.ZUVIX_HIDE_BANNER = "1";

    await runCli(["node", "zuvix", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "zuvix", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "zuvix", "browser", "--help"]);
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "zuvix", "secrets", "--help"]);
  ...
  it("renders nodes help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "zuvix", "nodes", "--help"]);
  ...
  it("defers nodes help startup metadata when plugin config can change command metadata", async () => {
    const argv = ["node", "zuvix", "nodes", "--help"];
    const parseAsync = vi.fn().mockResolvedValueOnce(undefined);
    const program = {
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "zuvix", "doctor", "--help"]);
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "zuvix", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "zuvix", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "zuvix", "gateway", "run"]],
    ["bare gateway runtime", ["node", "zuvix", "gateway"]],
    ["node runtime", ["node", "zuvix", "node", "run"]],
    ["local agent runtime", ["node", "zuvix", "agent", "--local"]],
    ["provider inference", ["node", "zuvix", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "zuvix", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "zuvix", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "zuvix", "skills", "search", "browser"]],
    ["update command", ["node", "zuvix", "update", "check"]],
    ["channel probe", ["node", "zuvix", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "zuvix", "channels", "capabilities"]],
    ["directory plugin command", ["node", "zuvix", "directory", "peers", "list"]],
    ["message plugin command", ["node", "zuvix", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "zuvix", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "zuvix", "--help"]],
    ["root version", ["node", "zuvix", "--version"]],
    ["gateway help", ["node", "zuvix", "gateway", "--help"]],
    ["gateway run help", ["node", "zuvix", "gateway", "run", "--help"]],
    ["status", ["node", "zuvix", "status"]],
    ["health", ["node", "zuvix", "health"]],
    ["gateway status", ["node", "zuvix", "gateway", "status"]],
    ["gateway health", ["node", "zuvix", "gateway", "health"]],
    ["remote agent control-plane", ["node", "zuvix", "agent", "run"]],
    ["chat control-plane", ["node", "zuvix", "chat"]],
    ["terminal control-plane", ["node", "zuvix", "terminal"]],
    ["config", ["node", "zuvix", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "zuvix", "channels"]],
    ["completion", ["node", "zuvix", "completion", "zsh"]],
    ["debug proxy cli", ["node", "zuvix", "proxy", "start"]],
    ["agents list", ["node", "zuvix", "agents", "list"]],
    ["models list", ["node", "zuvix", "models", "list"]],
    ["models status without live probe", ["node", "zuvix", "models", "status"]],
    ["skills check", ["node", "zuvix", "skills", "check"]],
    ["skills info", ["node", "zuvix", "skills", "info", "weather"]],
    ["skills list", ["node", "zuvix", "skills", "list"]],
    ["tasks list", ["node", "zuvix", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "zuvix", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "zuvix", "tools", "effective"]],
    ["migrate", ["node", "zuvix", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "zuvix", "plugins", "marketplace", "list"]);
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "zuvix", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "zuvix", "foo"])).rejects.toThrow(
  ...
  it("rejects unowned command roots even when --help is appended (regression for #81077)", async () => {
    await expect(runCli(["node", "zuvix", "foo", "--help"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("rejects unowned command roots even when --version is appended", async () => {
    await expect(runCli(["node", "zuvix", "foo", "--version"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "zuvix", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "zuvix", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "zuvix", "lcm_recent"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "zuvix", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "zuvix", "auth", "--help"]],
    ["tool", ["node", "zuvix", "tool", "image_generate"]],
    ["tools", ["node", "zuvix", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "zuvix", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "zuvix", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "zuvix", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy after normal gateway runtime completion", async () => {
    ...
    await runCli(["node", "zuvix", "gateway", "run"]);
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "zuvix", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("starts onboarding for bare root invocations when config only has metadata", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      valid: true,
      sourceConfig: {
        $schema: "https://zuvix.ai/config.json",
  ...
  it("points noninteractive fresh bare root invocations to onboarding automation", async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Onboarding needs an interactive TTY. Use `zuvix onboard --non-interactive --accept-risk ...` for automation.",
      );
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "zuvix", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "zuvix", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "zuvix", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "zuvix", "status"]],
    ]);
    expect(process.exitCode).toBe(1);
    process.exitCode = exitCode;
  });
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "zuvix", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "zuvix", "doctor", "--help"]],
    ]);
  });
  ...
  it("restores terminal state before uncaught CLI exits", async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[zuvix] Zuvix hit an unexpected runtime error.",
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith("[zuvix] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[zuvix] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
