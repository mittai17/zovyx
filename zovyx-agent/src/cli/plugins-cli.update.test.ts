...
function primeUpdateConfigSnapshot(params: {
  ...
  const configPath = params.configPath ?? path.join(process.cwd(), "zuvix.json5");
...
function primeBlockedUpdateConfig(section: "hooks" | "plugins", config: ZuvixConfig): void {
...
describe("plugins cli update", () => {
  ...
  it("uses the mutation-start snapshot for updater input and hook selection", async () => {
        ...
        internal: {
          installs: {
            "new-hooks": {
              ...
              installPath: "~/.zuvix/hooks/new-hooks",
            },
          },
        },
      },
          ...
          internal: {
            installs: {
              "new-hooks": {
                source: "npm",
                spec: "@acme/new-hooks@1.0.0",
                installPath: "/home/test/.zuvix/hooks/new-hooks",
              },
            },
          },
        },
    ...
    updateNpmInstalledPlugins.mockImplementation(async (params: { config: ZuvixConfig }) => ({
      config: params.config,
      changed: false,
      outcomes: [],
    }));
    updateNpmInstalledHookPacks.mockImplementation(async (params: { config: ZuvixConfig }) => ({
      config: params.config,
      changed: false,
      outcomes: [],
    }));
  ...
  it("allows index-only legacy id migration when an included plugins section has no references", async () => {
    const cfg = { plugins: {} } as ZuvixConfig;
    const pluginRecords = createTrackedPluginConfig({
      pluginId: "voice-call",
      spec: "@zuvix/voice-call@1.0.0",
    }).plugins?.installs;
    const nextConfig = {
      ...cfg,
      plugins: {
      ...
      outcomes: [
        {
          ...
          status: "updated",
          message: "Updated @zuvix/voice-call.",
        },
      ],
    });
  ...
  it.each([
    {
      ...
      record: {
        source: "clawhub",
        spec: "clawhub:@zuvix/voice-call",
        clawhubPackage: "@zuvix/voice-call",
        installPath: "/tmp/voice-call",
      },
    },
    {
      ...
      record: {
        source: "git",
        spec: "https://github.com/zuvix/voice-call.git",
        installPath: "/tmp/voice-call",
      },
    },
    {
  ...
  it("blocks possible legacy id migration when an included plugins section is unresolved", async () => {
    ...
    primeUpdateConfigSnapshot({
      ...
      sourceConfig: { plugins: { $include: externalPath } } as unknown as ZuvixConfig,
      includeFileTargetsForWrite: {
        [externalPath]: externalPath,
      },
    });
  ...
  it("preserves an include-owned plugins section during legacy-record cleanup", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-update-"));
  ...
  it("passes dangerous force unsafe install to plugin updates", async () => {
    ...
    expect(updateParams.pluginIds).toEqual(["zuvix-codex-app-server"]);
