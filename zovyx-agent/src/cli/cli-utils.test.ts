...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "zuvix", "-V"] },
    { argv: ["node", "zuvix", "tui"] },
    { argv: ["node", "zuvix", "terminal"] },
    { argv: ["node", "zuvix", "chat"] },
    { argv: ["node", "zuvix", "gateway"] },
    { argv: ["node", "zuvix", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "zuvix", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "zuvix", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "zuvix", "gateway", "status"] },
    { argv: ["node", "zuvix", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "zuvix", "--help"] },
    { argv: ["node", "zuvix", "gateway"] },
    { argv: ["node", "zuvix", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "zuvix.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("zuvix.internal");
    } finally {
      log.mockRestore();
    }
  });
