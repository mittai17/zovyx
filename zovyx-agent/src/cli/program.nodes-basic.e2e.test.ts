...
describe("cli program (nodes basics)", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        ...
        "zuvix nodes approve request-approval",
      ],
    },
    {
      ...
      expectedOutput: [
        ...
        "zuvix nodes approve request-reapproval",
      ],
    },
  ...
  it("runs nodes describe and calls node.describe", async () => {
    ...
    expect(out).toContain("zuvix nodes approve request-approval");
  ...
  it("keeps explicit gateway options in node reapproval guidance without leaking auth", async () => {
    ...
    await runProgram([
      ...
      "status",
      "--url",
      "ws://gateway-user:url-secret@gateway.example:18789/zuvix?cluster=qa",
      "--timeout",
      "3000",
      "--token",
      "secret-token",
    ...
    const output = getRuntimeOutput();
    expect(output).toContain("zuvix nodes approve request-reapproval --timeout 3000");
  ...
  it("does not recommend approval from a stale pending request id alone", async () => {
    ...
    expect(output).not.toContain("zuvix nodes approve stale-request");
