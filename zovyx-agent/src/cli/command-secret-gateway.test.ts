...
describe("resolveCommandSecretRefsViaGateway", () => {
  function makeTalkProviderApiKeySecretRefConfig(envKey: string): ZuvixConfig {
  ...
  async function createExecProviderConfig(refId: string): Promise<{
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-command-secret-exec-"));
    tempRoots.add(root);
    ...
    const resolverScript = [
      ...
      "  fs.writeFileSync(process.env.ZUVIX_EXEC_MARKER, 'executed');",
        ...
        secrets: {
          providers: {
            default: {
              ...
              env: { ZUVIX_EXEC_MARKER: markerPath },
