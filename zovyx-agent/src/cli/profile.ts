...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.zuvix${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ZUVIX_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.ZUVIX_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.ZUVIX_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.ZUVIX_CONFIG_PATH)) {
    env.ZUVIX_CONFIG_PATH = path.join(stateDir, "zuvix.json");
  }

  if (profile === "dev" && !env.ZUVIX_GATEWAY_PORT?.trim()) {
    env.ZUVIX_GATEWAY_PORT = "19001";
  }
}
