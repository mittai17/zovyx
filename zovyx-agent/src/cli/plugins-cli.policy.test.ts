...
describe("plugins cli policy mutations", () => {
  ...
  function requireFirstWrittenConfig(): ZuvixConfig {
  ...
  function requirePluginEntries(
    config: ZuvixConfig,
  ): NonNullable<NonNullable<ZuvixConfig["plugins"]>["entries"]> {
  ...
  it("refreshes the persisted plugin registry after enabling a plugin", async () => {
    const sourceConfig = {} as ZuvixConfig;
    const enabledConfig = {
      plugins: {
        entries: {
          alpha: { enabled: true },
  ...
  it.each(["enable", "disable"] as const)(
    ...
    async (command) => {
      ...
      expect(runtimeErrors).toContain(
        "Plugin not found: missing-plugin. Run `zuvix plugins list` to see installed plugins, or `zuvix plugins search missing-plugin` to look for installable plugins.",
      );
      expect(enablePluginInConfig).not.toHaveBeenCalled();
      expect(writeConfigFile).not.toHaveBeenCalled();
