...
describe("command-path-policy", () => {
  ...
  it("applies exact overrides after broader channel plugin rules", () => {
    ...
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "zuvix", "channels", "status"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("bypass");
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "zuvix", "channels", "status", "--probe"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("default");
  ...
  it("defaults unknown command paths to network proxy routing", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "googlemeet", "login"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "tool", "image_generate"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "tools", "effective"])).toBe("bypass");
  });

  it("resolves static network proxy bypass policies from the catalog", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "zuvix", "config", "get", "proxy.enabled"]),
    ).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "proxy", "start"])).toBe("bypass");
  });

  it("resolves mixed network proxy policies from argv-sensitive catalog entries", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "gateway"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "gateway", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "gateway", "health"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "node", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "node", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "agent", "--local"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "agent", "run"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "channels", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "zuvix", "channels", "status", "--probe"]),
    ).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "models", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "models", "status", "--probe"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "skills", "info", "browser"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "skills", "check"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "skills", "list"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "skills", "search", "browser"])).toBe(
      "default",
    );
  });
  ...
  it("uses the longest catalog command path for deep network proxy overrides", async () => {
    ...
    expect(
      resolveCliCatalogCommandPathLocal(["node", "zuvix", "nodes", "camera", "snap"]),
    ).toEqual(["nodes", "camera", "snap"]);
    expect(resolveCliNetworkProxyPolicyLocal(["node", "zuvix", "nodes", "camera", "snap"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicyLocal(["node", "zuvix", "nodes", "camera", "list"])).toBe(
      "bypass",
    );
  });
  ...
  it("stops catalog command path resolution before positional arguments", () => {
    expect(
      resolveCliCatalogCommandPath(["node", "zuvix", "config", "get", "proxy.enabled"]),
    ).toEqual(["config", "get"]);
    expect(
      resolveCliCatalogCommandPath(["node", "zuvix", "message", "send", "--to", "demo"]),
    ).toEqual(["message"]);
  });

  it("treats bare gateway invocations with options as the gateway runtime", () => {
    const argv = ["node", "zuvix", "gateway", "--port", "1234"];
  ...
  it("does not let gateway run option values spoof bypass subcommands", () => {
    for (const argv of [
      ["node", "zuvix", "gateway", "--token", "status"],
      ["node", "zuvix", "gateway", "--token=status"],
      ["node", "zuvix", "gateway", "--password", "health"],
      ["node", "zuvix", "gateway", "--password-file", "status"],
      ["node", "zuvix", "gateway", "--ws-log", "compact"],
    ]) {
  ...
  it("still resolves real gateway bypass subcommands after their command token", () => {
    expect(resolveCliCatalogCommandPath(["node", "zuvix", "gateway", "status"])).toEqual([
      "gateway",
      "status",
    ]);
    expect(
      resolveCliCatalogCommandPath(["node", "zuvix", "gateway", "status", "--token", "secret"]),
    ).toEqual(["gateway", "status"]);
    expect(resolveCliNetworkProxyPolicy(["node", "zuvix", "gateway", "status"])).toBe("bypass");
  });
});
