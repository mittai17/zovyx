// Commander wiring for `zuvix update`, its status/finalize subcommands, and help text.
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix update repair", "Rerun post-update doctor and plugin convergence."],
          ["zuvix update repair --channel beta", "Repair against the beta update channel."],
          ["zuvix update repair --json", "JSON output for automation."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.zuvix.ai/cli/update")}`,
    )
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Zuvix and inspect update channel status")
    .option("--json", "Output result as JSON", false)
    ...
    .addHelpText("after", () => {
      const examples = [
        ["zuvix update", "Update a source checkout (git)"],
        ["zuvix update --channel beta", "Switch to beta channel (git + npm)"],
        ["zuvix update --channel dev", "Switch to dev channel (git + npm)"],
        ["zuvix update --tag beta", "One-off update to a dist-tag or version"],
        ["zuvix update --tag main", "One-off package update from GitHub main"],
        ["zuvix update --dry-run", "Preview actions without changing anything"],
        ["zuvix update --no-restart", "Update without restarting the service"],
        ["zuvix update --json", "Output result as JSON"],
        ["zuvix update --yes", "Non-interactive (accept downgrade prompts)"],
        ["zuvix update repair", "Repair stranded post-update plugin state"],
        ["zuvix update wizard", "Interactive update wizard"],
        ["zuvix --update", "Shorthand for zuvix update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run zuvix update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.zuvix.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix update status", "Show channel + version status."],
          ["zuvix update status --json", "JSON output."],
          ["zuvix update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
