...
const mocks = vi.hoisted(() => ({
  ...
  relinkZuvixPeerDependenciesInManagedNpmRoot: vi.fn(),
...
vi.mock("../../plugins/plugin-peer-link.js", () => ({
  relinkZuvixPeerDependenciesInManagedNpmRoot:
    mocks.relinkZuvixPeerDependenciesInManagedNpmRoot,
}));
...
describe("runPostCorePluginConvergence", () => {
  ...
  beforeEach(() => {
    ...
    mocks.relinkZuvixPeerDependenciesInManagedNpmRoot.mockResolvedValue({
      checked: 0,
      attempted: 0,
      repaired: 0,
      skipped: 0,
  ...
  function makeTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-post-core-convergence-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("calls repair with ZUVIX_UPDATE_POST_CORE_CONVERGENCE=1 set", async () => {
    const cfg = { plugins: { entries: {} } } as unknown as ZuvixConfig;
    await runPostCorePluginConvergence({
      cfg,
      env: { ZUVIX_UPDATE_IN_PROGRESS: "1" },
    });
    ...
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      ...
      env: {
        ...
        ZUVIX_COMPATIBILITY_HOST_VERSION: VERSION,
  ...
  it("uses the candidate runtime version over a stale inherited host version", async () => {
    ...
    await runPostCorePluginConvergence({
      cfg,
      env: { ZUVIX_COMPATIBILITY_HOST_VERSION: "2026.5.12" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      cfg,
      env: {
  ...
  it("returns the post-repair install records so callers can re-seed pluginConfig", async () => {
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { discord: { enabled: true } } } } as unknown as ZuvixConfig,
      env: {},
    });
  ...
  it("repairs managed npm zuvix peer links in every managed npm project before payload smoke checks", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
    ...
    mocks.listManagedPluginNpmRoots.mockResolvedValue([
      "/tmp/zuvix-state/npm",
      "/tmp/zuvix-state/npm/projects/codex",
    ]);
    mocks.relinkZuvixPeerDependenciesInManagedNpmRoot
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { codex: { enabled: true } } } } as unknown as ZuvixConfig,
      env: { ZUVIX_STATE_DIR: "/tmp/zuvix-state" },
    });

    expect(mocks.relinkZuvixPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(1, {
      npmRoot: "/tmp/zuvix-state/npm",
      logger: {},
    });
    expect(mocks.relinkZuvixPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(2, {
      npmRoot: "/tmp/zuvix-state/npm/projects/codex",
      logger: {},
    });
    expect(result.changes).toEqual([
      "Repaired Zuvix host peer link(s) for 1 managed npm plugin package(s).",
    ]);
    expect(
      mocks.relinkZuvixPeerDependenciesInManagedNpmRoot.mock.invocationCallOrder[0],
  ...
  it("keeps repair warnings nonblocking with actionable guidance", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "discord" from @zuvix/discord: ENETUNREACH.',
      ],
      records: {},
    });
    const result = await runPostCorePluginConvergence({
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        message:
          'Failed to install missing configured plugin "discord" from @zuvix/discord: ENETUNREACH.',
        guidance: ["Run `zuvix update repair` to retry plugin repair."],
      },
    ]);
  });
  ...
  it("keeps failed configured-plugin repair fetches nonblocking", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "matrix" from clawhub:@zuvix/matrix@beta: ClawHub ClawPack download for @zuvix/matrix@2026.6.1-beta.1 body stalled after 30000ms.',
      ],
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        guidance: [
          "Run `zuvix update repair` to retry plugin repair.",
          "Run `zuvix plugins inspect brave --runtime --json` for details.",
        ],
      },
    ]);
  });
      ...
      warnings: [
        {
          ...
          guidance: ["Run `zuvix update repair`."],
        },
        {
