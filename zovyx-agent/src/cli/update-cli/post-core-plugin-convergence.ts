...
import { relinkZuvixPeerDependenciesInManagedNpmRoot } from "../../plugins/plugin-peer-link.js";
...
const REPAIR_GUIDANCE = "Run `zuvix update repair` to retry plugin repair.";
const inspectGuidance = (pluginId: string) =>
  `Run \`zuvix plugins inspect ${pluginId} --runtime --json\` for details.`;

async function repairManagedNpmZuvixPeerLinks(params: {
  ...
  try {
    ...
    const message = `Failed to repair managed npm Zuvix host peer links: ${err instanceof Error ? err.message : String(err)}`;
    return {
      changes: [],
      warnings: [
        {
...
export async function runPostCorePluginConvergence(params: {
  ...
  const peerLinkRepair = await repairManagedNpmZuvixPeerLinks({ env });
