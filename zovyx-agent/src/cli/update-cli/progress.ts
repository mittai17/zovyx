...
const STEP_LABELS: Record<string, string> = {
  ...
  "zuvix doctor entry": "Checking doctor entrypoint",
  "zuvix doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g zuvix@latest");
    hints.push(
      "System install outline: zuvix gateway stop -> sudo <system-npm> i -g zuvix@latest -> zuvix gateway install --force -> zuvix gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g zuvix@latest --omit=optional");
  }
