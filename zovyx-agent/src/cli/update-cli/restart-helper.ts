...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.ZUVIX_SYSTEMD_UNIT);
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.ZUVIX_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.ZUVIX_LAUNCHD_LABEL);
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.ZUVIX_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.ZUVIX_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.ZUVIX_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `zuvix-restart-${timestamp}.sh`;
...
exec 3>&2
${logSetup}
printf '[%s] zuvix restart attempt source=update target=%s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
if systemctl --user is-active --quiet '${escaped}' || systemctl --user is-enabled --quiet '${escaped}'; then
  if systemctl --user restart '${escaped}'; then
    status=0
    printf '[%s] zuvix restart done source=update\\n' "$(date -u +%FT%TZ)" >&2
  else
    status=$?
    printf '[%s] zuvix restart failed source=update status=%s\\n' "$(date -u +%FT%TZ)" "$status" >&2
  fi
elif systemctl is-active --quiet '${escaped}' || systemctl is-enabled --quiet '${escaped}'; then
  status=78
  printf '[%s] system-scoped zuvix gateway unit detected; update cannot restart it without sudo. Run: sudo systemctl restart %s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
...
# Self-cleanup (log is retained under the Zuvix state logs directory).
script_dir=$(dirname "$0")
rm -f "$0"
rmdir "$script_dir" 2>/dev/null || true
exit "$status"
`;
      ...
      filename = `zuvix-restart-${timestamp}.cmd`;
...
setlocal
set "ZUVIX_RESTART_SCRIPT=%~f0"
set "ZUVIX_RESTART_SCRIPT_DIR=%~dp0."
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p=$env:ZUVIX_RESTART_SCRIPT; $s=Get-Content -Raw -LiteralPath $p; $m='# POWERSHELL'; $i=$s.IndexOf($m); if ($i -lt 0) { exit 1 }; Invoke-Expression $s.Substring($i)"
set "status=%ERRORLEVEL%"
del "%~f0" >nul 2>&1
rmdir "%ZUVIX_RESTART_SCRIPT_DIR%" >nul 2>&1
exit /b %status%
# POWERSHELL
# Wait briefly to ensure file locks are released after update.
...
try {
  ...
  Add-Content -LiteralPath $logPath -Value "[$(Get-Date -Format o)] zuvix restart log initialized"
} catch {
...
function Join-ZuvixProcessArguments {
...
function Invoke-ZuvixSchtasksWithTimeout {
  param(
    [string[]]$Arguments,
    [int]$TimeoutSeconds
  )
  ...
  try {
    ...
    $startInfo.Arguments = Join-ZuvixProcessArguments -Arguments $Arguments
    ...
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
      ...
      Write-RestartLog "zuvix restart schtasks timeout source=update args=$($Arguments -join ' ')"
      return 124
    }
    ...
    Write-RestartLog "zuvix restart schtasks failed source=update args=$($Arguments -join ' ') error=$($_.Exception.Message)"
    return 1
  }
}

function Get-ZuvixScheduledTaskState {
...
function Get-ZuvixListenerPids {
  param([int]$Port)
  $listenerPids = @()
...
function Invoke-ZuvixStartupLauncher {
  $launcherPath = Join-Path $env:USERPROFILE ".zuvix\\gateway.cmd"
  if (-not (Test-Path -LiteralPath $launcherPath)) {
    Write-RestartLog "zuvix restart startup launcher missing source=update path=$launcherPath"
    return 1
  }

  try {
    Start-Process -FilePath $launcherPath -WindowStyle Hidden | Out-Null
    Write-RestartLog "zuvix restart launched startup fallback source=update path=$launcherPath"
    return 0
  } catch {
    Write-RestartLog "zuvix restart startup fallback failed source=update error=$($_.Exception.Message)"
    return 1
  }
}
...
$port = ${port}
Write-RestartLog "zuvix restart attempt source=update target=$taskName"

$taskState = Get-ZuvixScheduledTaskState -TaskName $taskName
if ($taskState -eq "Running") {
  $endStatus = Invoke-ZuvixSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10
  if ($endStatus -ne 0) {
    Write-RestartLog "zuvix restart schtasks end did not complete cleanly source=update status=$endStatus"
  }
} else {
  Write-RestartLog "zuvix restart skipped schtasks end source=update state=$taskState"
}

for ($attempt = 1; $attempt -le 10; $attempt++) {
  $listeners = @(Get-ZuvixListenerPids -Port $port)
  ...
  if ($attempt -eq 10) {
    foreach ($listenerPid in $listeners) {
      try {
        ...
        Write-RestartLog "zuvix restart killed stale listener source=update pid=$listenerPid"
      } catch {
        Write-RestartLog "zuvix restart failed to kill stale listener source=update pid=$listenerPid error=$($_.Exception.Message)"
      }
    }
    break
  }
...
$status = Invoke-ZuvixSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30
if ($status -ne 0) {
  $status = Invoke-ZuvixStartupLauncher
}
if ($status -eq 0) {
  Write-RestartLog "zuvix restart done source=update"
} else {
  Write-RestartLog "zuvix restart failed source=update status=$status"
}
...
`;
    ...
    const scriptDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-restart-"));
