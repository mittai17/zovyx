...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/zuvix-root")).toEqual([
      path.join("/tmp/zuvix-root", "dist", "index.js"),
      path.join("/tmp/zuvix-root", "dist", "index.mjs"),
      path.join("/tmp/zuvix-root", "dist", "entry.js"),
      path.join("/tmp/zuvix-root", "dist", "entry.mjs"),
    ]);
  });
});
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { ZUVIX_GATEWAY_PORT: "19001" },
        serviceEnv: { ZUVIX_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostUpdateServiceStateReadEnv", () => {
  it("keeps package restart preparation anchored to the pre-update service env", () => {
    const processEnv = {
      ZUVIX_STATE_DIR: "/source/state",
      ZUVIX_CONFIG_PATH: "/source/zuvix.json",
    } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = {
      ZUVIX_STATE_DIR: "/managed/state",
      ZUVIX_CONFIG_PATH: "/managed/zuvix.json",
    } as NodeJS.ProcessEnv;
  ...
  it("keeps git updates tied to the caller environment", () => {
    const processEnv = { ZUVIX_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = { ZUVIX_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
  ...
  it("uses the managed service environment for git updates stopped by this updater", () => {
    const processEnv = { ZUVIX_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const preManagedServiceEnv = { ZUVIX_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/zuvix",
      baseEnv: {
        PATH: "/bin",
        ZUVIX_STATE_DIR: "/wrong/state",
        ZUVIX_CONFIG_PATH: "/wrong/zuvix.json",
        ZUVIX_PROFILE: "wrong",
      },
      serviceEnv: {
        ZUVIX_STATE_DIR: "daemon-state",
        ZUVIX_CONFIG_PATH: "daemon-state/zuvix.json",
        ZUVIX_PROFILE: "work",
      },
    });
    ...
    expect(env.ZUVIX_STATE_DIR).toBe(path.join("/srv/zuvix", "daemon-state"));
    expect(env.ZUVIX_CONFIG_PATH).toBe(
      path.join("/srv/zuvix", "daemon-state", "zuvix.json"),
    );
    expect(env.ZUVIX_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    const env = resolvePostInstallDoctorEnv({
      baseEnv: {
        PATH: "/bin",
        ZUVIX_STATE_DIR: "/caller/state",
        ZUVIX_PROFILE: "caller",
      },
    });

    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.ZUVIX_STATE_DIR).toBe("/caller/state");
    expect(env.ZUVIX_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@zuvix", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@zuvix", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@zuvix/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@zuvix/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@zuvix/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@zuvix/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@zuvix/missing-install-path@beta",
            },
            local: {
              source: "path",
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@zuvix", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("zuvix gateway restart");
    expect(line).toContain("zuvix gateway install --force");
    expect(line).toContain("zuvix gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { ZUVIX_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, ZUVIX_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { ZUVIX_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { ZUVIX_PROFILE: "stomme", ZUVIX_PORT: "18790" },
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `zuvix update` completes (#78445).
