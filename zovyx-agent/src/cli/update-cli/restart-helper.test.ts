...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-ZuvixScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-ZuvixSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "zuvix restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-ZuvixListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-ZuvixSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'zuvix-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `zuvix-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("zuvix-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^zuvix-restart-/);
    ...
    it("uses ZUVIX_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ZUVIX_PROFILE: "default",
        ZUVIX_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("zuvix-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        ZUVIX_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        ZUVIX_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped zuvix gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart zuvix-gateway.service");
      expect(calls).toContain("--user is-active --quiet zuvix-gateway.service");
      expect(calls).toContain("is-active --quiet zuvix-gateway.service");
      expect(calls).not.toContain("--user restart zuvix-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.zuvix.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.zuvix.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.zuvix/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.zuvix/logs/gateway-restart.log' 2>&1");
    ...
    it("uses ZUVIX_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/zuvix-state/logs' 2>/dev/null && : >>'/tmp/zuvix-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/zuvix-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("zuvix restart attempt source=update target=ai.zuvix.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.zuvix.gateway");
      expect(log).toContain("zuvix restart failed source=update status=42");
      expect(log).not.toContain("zuvix restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        ZUVIX_LAUNCHD_LABEL: "ai.zuvix.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        ZUVIX_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.zuvix.$(echo injected)");
      expect(log).not.toContain("target=ai.zuvix.injected");
    });

    it("uses ZUVIX_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        ZUVIX_PROFILE: "default",
        ZUVIX_LAUNCHD_LABEL: "com.custom.zuvix",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.zuvix'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'Zuvix Gateway'");
      expect(content).toContain("function Invoke-ZuvixSchtasksWithTimeout");
      expect(content).toContain("function Get-ZuvixScheduledTaskState");
      expect(content).toContain("function Invoke-ZuvixStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("zuvix restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".zuvix\\gateway.cmd"',
      );
      expect(content).toContain("zuvix restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%ZUVIX_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses ZUVIX_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        ZUVIX_PROFILE: "default",
        ZUVIX_WINDOWS_TASK_NAME: "Zuvix Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'Zuvix Gateway (custom)'");
      expect(content).toContain("Get-ZuvixScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-ZuvixSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-ZuvixStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ZUVIX_PROFILE: "production",
      });
      expect(content).toContain("zuvix-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        ZUVIX_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.zuvix.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'Zuvix Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        ZUVIX_LAUNCHD_LABEL: "ai.zuvix.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.zuvix.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
