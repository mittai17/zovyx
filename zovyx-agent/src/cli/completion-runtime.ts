...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "ZUVIX_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Zuvix Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# Zuvix Completion\n${sourceLine}`;
  const next = trimmed ? `${trimmed}\n\n${block}\n` : `${block}\n`;
  return { next, changed: next !== content, hadExisting };
}

/** Resolves the shell startup profile path that should contain the Zuvix completion block. */
...
/** Returns whether a shell profile already contains an Zuvix completion block or source line. */
 ...
 * Returns true if profile has `source <(zuvix completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "zuvix") {
