    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "zuvix node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["zuvix node status", "Check node host service status."],
          ["zuvix node install", "Install the node host service."],
          ["zuvix node start", "Start the installed node host service."],
          ["zuvix node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.zuvix.ai/cli/node")}\n`,
    );
