...
import type { ConfigFileSnapshot, ZuvixConfig } from "../config/types.zuvix.js";
import { isTruthyEnvValue, normalizeEnv } from "../infra/env.js";
import { isMainModule } from "../infra/is-main.js";
import type { ProxyHandle } from "../infra/net/proxy/proxy-lifecycle.js";
import { ensureZuvixCliOnPath } from "../infra/path-env.js";
...
function createGatewayCliMainStartupTrace(argv: string[]) {
  ...
  const enabled =
    isTruthyEnvValue(process.env.ZUVIX_GATEWAY_STARTUP_TRACE) &&
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.ZUVIX_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.ZUVIX_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
function createAllowlistAgnosticCliLookupConfig(config: ZuvixConfig): ZuvixConfig {
...
async function resolveUnownedCliPrimaryMessage(params: {
  ...
  return (
    ...
    `Unknown command: zuvix ${params.primary}. No built-in command or plugin CLI metadata owns "${params.primary}".`
  );
}
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.ZUVIX_CONTAINER) ?? null;
  ...
  let bestEffortConfigPromise: Promise<ZuvixConfig> | null = null;
  const readBestEffortCliConfig = async (): Promise<ZuvixConfig> => {
  ...
  try {
    ...
    // `zuvix <typo> --help` surfaces the same Unknown command error as
    // `zuvix <typo>` instead of silently showing generic top-level help.
    ...
    if (shouldRunBareRootCrestodian) {
      // START IN HEADLESS GATEWAY MODE INSTEAD OF INTERACTIVE WIZARD
      const headlessArgv = [normalizedArgv[0], normalizedArgv[1], 'gateway', 'run'];
      if (await tryRunGatewayRunFastPath(headlessArgv, startupTrace)) {
        return;
      }
    }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[zuvix] Non-fatal uncaught exception (continuing):",
            formatUncaughtError(error),
          );
          return;
        }
        for (const line of formatCliFailureLines({
          title: "Zuvix hit an unexpected runtime error.",
          error,
          argv: normalizedArgv,
        })) {
          console.error(line);
