...
export function rejectAgentScopedModelWrite(
  ...
  throw new Error(
    `zuvix models ${commandName} does not support --agent; it only updates global model defaults. Remove --agent, or run ${formatCliCommand("zuvix agents list")} and set the per-agent model in agent config.`,
  );
}
