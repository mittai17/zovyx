...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: ZuvixConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: ZuvixConfig }>
    >(async () => ({
      path: "/tmp/zuvix.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: ZuvixConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/zuvix.json");
    ...
    expect(output).not.toContain("/tmp/zuvix.json\nforged");
