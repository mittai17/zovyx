...
/** Parse `zuvix health` flags for the route-first status family. */
...
/** Parse `zuvix status` flags without registering the full command tree. */
...
/** Parse `zuvix gateway status` RPC-only flags accepted by the fast route. */
...
/** Parse `zuvix sessions` filters for JSON/list route execution. */
...
/** Parse `zuvix agents list` display switches for route-first execution. */
...
/** Parse `zuvix config get <path>` while preserving root option handling. */
...
/** Parse `zuvix config unset <path>` and its mutation guard flags. */
...
/** Parse `zuvix models list` filters for the lightweight model catalog route. */
...
/** Parse `zuvix models status` probe controls for the route-first status path. */
...
/** Parse `zuvix channels list` display flags for the route-first list path. */
...
/** Parse `zuvix channels status` probe flags without full CLI registration. */
...
/** Parse JSON-only `zuvix plugins list` flags for plugin inventory output. */
...
/** Parse both `zuvix tasks --json` and `zuvix tasks list --json` aliases. */
...
/** Parse JSON-only `zuvix tasks audit` filters for the route-first audit path. */
