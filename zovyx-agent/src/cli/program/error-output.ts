...
function resolveHelpCommand(argv: string[] | undefined, options?: { root?: boolean }): string {
  if (options?.root || !argv) {
    return formatCliCommand("zuvix --help");
  }
  const commandPath = getCommandPathWithRootOptions(argv, 2);
  if (commandPath.length === 0) {
    return formatCliCommand("zuvix --help");
  }
  return formatCliCommand(`zuvix ${commandPath.join(" ")} --help`);
}
...
function formatDocsHint(): string {
  return `${theme.muted("Docs:")} ${formatDocsLink("/cli", "docs.zuvix.ai/cli")}`;
}

/** Convert Commander parse errors into Zuvix-specific help and docs guidance. */
export function formatCliParseErrorOutput(
  ...
  if (unknownCommand) {
    ...
    return lines(
      theme.error(`Zuvix does not know the command ${quote(command)}.`),
      formatHelpHint(options.argv, { root: true }),
      `${theme.muted("Plugin command?")} ${theme.command(formatCliCommand("zuvix plugins list"))}`,
      formatDocsHint(),
    );
  }
  ...
  if (unknownOption) {
    ...
    return lines(
      theme.error(`Zuvix does not recognize option ${quote(option)}.`),
  ...
  return lines(
    theme.error(`Zuvix could not parse this command: ${message}`),
    formatHelpHint(options.argv),
  );
}
