...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.ZUVIX_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.ZUVIX_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.ZUVIX_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.ZUVIX_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.ZUVIX_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.ZUVIX_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.ZUVIX_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "zuvix", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "zuvix"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "zuvix", "nodes", "list"], "zuvix");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "zuvix", "infer"], "zuvix");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "zuvix", "acp", "--help"], "zuvix");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "zuvix", "gateway", "--force"];
    process.argv = argv;
    const program = new Command().name("zuvix");
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "zuvix", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("zuvix");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "zuvix", "completion", "--write-state"];
    const program = new Command().name("zuvix");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "zuvix", "plugins"];
    const program = new Command().name("zuvix");
