...
describe("ensureConfigReady", () => {
  ...
  const originalZuvixStateDir = process.env.ZUVIX_STATE_DIR;
  ...
  function useTempZuvixHome(): string {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-config-guard-"));
    tempRoots.push(root);
    process.env.ZUVIX_HOME = root;
    delete process.env.ZUVIX_STATE_DIR;
    return root;
  }

  function writeLegacyTaskSidecarMarker(root: string): void {
    const markerPath = path.join(root, ".zuvix", "tasks", "runs.sqlite");
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "");
  }

  function writeStateMarker(root: string, relativePath: string): void {
    const markerPath = path.join(root, ".zuvix", relativePath);
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "{}");
  }
  ...
  afterEach(() => {
    ...
    if (originalZuvixStateDir === undefined) {
      delete process.env.ZUVIX_STATE_DIR;
    } else {
      process.env.ZUVIX_STATE_DIR = originalZuvixStateDir;
    }
    for (const root of tempRoots.splice(0)) {
      fs.rmSync(root, { recursive: true, force: true });
    }
  ...
  it("runs doctor flow for legacy sessions without task sidecars", async () => {
    const root = useTempZuvixHome();
    fs.mkdirSync(path.join(root, ".zuvix", "sessions"), { recursive: true });
  ...
  it("uses shared tilde expansion for ZUVIX_HOME in the startup detector", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-config-guard-home-"));
    tempRoots.push(root);
    process.env.HOME = root;
    process.env.ZUVIX_HOME = "~/svc";
    delete process.env.ZUVIX_STATE_DIR;
  ...
  it("exits for invalid config on non-allowlisted commands", async () => {
    ...
    expect(plainErrorCalls(runtime)).toEqual([
      "Zuvix config is invalid",
      "File: /tmp/zuvix.json",
      "Problem:",
      "  - channels.quietchat: invalid",
      "",
      `Fix: ${formatCliCommand("zuvix doctor --fix")}`,
      `Inspect: ${formatCliCommand("zuvix config validate")}`,
  ...
  it("replaces doctor fix advice for plugin packaging-only invalid config", async () => {
    ...
    expect(calls).not.toContain(`Fix: ${formatCliCommand("zuvix doctor --fix")}`);
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("runs doctor migration flow only once per module instance", async () => {
    writeLegacyTaskSidecarMarker(useTempZuvixHome());
