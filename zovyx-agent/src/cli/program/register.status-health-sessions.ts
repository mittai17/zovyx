...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix status", "Show channel health + session summary."],
          ["zuvix status --all", "Full diagnosis (read-only)."],
          ["zuvix status --json", "Machine-readable output."],
          ["zuvix status --usage", "Show model provider usage/quota snapshots."],
          [
            "zuvix status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["zuvix status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.zuvix.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.zuvix.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix sessions", "List all sessions."],
          ["zuvix sessions --agent work", "List sessions for one agent."],
          ["zuvix sessions --all-agents", "Aggregate sessions across agents."],
          ["zuvix sessions --active 120", "Only last 2 hours."],
          ["zuvix sessions --limit 25", "Show the newest 25 sessions."],
          ["zuvix sessions --json", "Machine-readable output."],
          ["zuvix sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.zuvix.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "zuvix sessions cleanup --dry-run --fix-missing",
          ...
          ["zuvix sessions cleanup --enforce", "Apply maintenance now."],
          ["zuvix sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["zuvix sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "zuvix sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .zuvix/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix commitments", "List pending inferred follow-ups."],
          ["zuvix commitments --all", "List all inferred follow-ups."],
          ["zuvix commitments --agent work", "List one agent's inferred follow-ups."],
          ["zuvix commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
