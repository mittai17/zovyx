...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "zuvix", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    setRawArgs(program, ["node", "zuvix", "status", "--json"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      programName: "zuvix",
      rawArgs: ["node", "zuvix", "status", "--json"],
      fallbackArgv: ["status", "--json"],
    });
    expect(parseAsync).toHaveBeenCalledWith(["node", "zuvix", "status"]);
  });

  it("falls back to action args without command name when action has no name", async () => {
    const program = new Command().name("zuvix");
    setRawArgs(program, ["node", "zuvix"]);
    const parseAsync = vi.spyOn(program, "parseAsync").mockResolvedValue(program);
    const actionCommand = {
      name: () => "",
      parent: program,
  ...
  it("uses root raw args and reparses the root for nested lazy commands", async () => {
    const root = new Command().name("zuvix");
    setRawArgs(root, ["node", "zuvix", "workspaces", "audit", "export", "--since", "1"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      programName: "zuvix",
      rawArgs: ["node", "zuvix", "workspaces", "audit", "export", "--since", "1"],
