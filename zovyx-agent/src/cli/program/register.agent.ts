...
export function registerAgentsCommands(program: Command): void {
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.zuvix.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ['zuvix agents set-identity --agent main --name "Zuvix" --emoji "🦞"', "Set name + emoji."],
  ["zuvix agents set-identity --agent main --avatar avatars/zuvix.png", "Set avatar path."],
  [
    "zuvix agents set-identity --workspace ~/.zuvix/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "zuvix agents set-identity --identity-file ~/.zuvix/workspace/IDENTITY.md --agent main",
