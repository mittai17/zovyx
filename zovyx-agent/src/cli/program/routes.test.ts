...
describe("program routes", () => {
  ...
  it("passes parsed agents list flags through", async () => {
    await expect(expectRoute(["agents"]).run(["node", "zuvix", "agents"])).resolves.toBe(true);
  ...
  it("passes parsed channel read-only route flags through", async () => {
    ...
    await expect(listRoute.run(["node", "zuvix", "channels", "list", "--json"])).resolves.toBe(
      true,
    );
  ...
  it("routes plugins list JSON without importing the full plugins CLI", async () => {
    ...
    expect(route.loadPlugins).toBeUndefined();
    expect(route.canRun?.(["node", "zuvix", "plugins", "list"])).toBe(false);

    await expect(
      route.run(["node", "zuvix", "plugins", "list", "--json", "--enabled", "--verbose"]),
    ).resolves.toBe(true);
  ...
  it("returns false for plugins list JSON route with unsupported arguments", async () => {
    await expectRunFalse(
      ["plugins", "list"],
      ["node", "zuvix", "plugins", "list", "--json", "--wat"],
    );
  });
  ...
  it("returns false for gateway status route when option values are missing", async () => {
    await expectRunFalse(["gateway", "status"], ["node", "zuvix", "gateway", "status", "--url"]);
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "zuvix", "gateway", "status", "--token"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "zuvix", "gateway", "status", "--password"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "zuvix", "gateway", "status", "--timeout"],
    );
  });
  ...
  it("returns false for gateway status route when probe-only flags are present", async () => {
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "zuvix", "gateway", "status", "--ssh", "user@host"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "zuvix", "gateway", "status", "--ssh-identity", "~/.ssh/id_test"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "zuvix", "gateway", "status", "--ssh-auto"],
    );
  });
  ...
  it("passes --no-probe through to daemon status", async () => {
    const route = expectRoute(["gateway", "status"]);
    await expect(route.run(["node", "zuvix", "gateway", "status", "--no-probe"])).resolves.toBe(
      true,
    );
  ...
  it("returns false when status timeout flag value is missing", async () => {
    await expectRunFalse(["status"], ["node", "zuvix", "status", "--timeout"]);
  });

  it("routes status --json through the lean JSON command", async () => {
    const route = expectRoute(["status"]);
    await expect(
      route.run(["node", "zuvix", "status", "--json", "--deep", "--usage", "--timeout", "5000"]),
    ).resolves.toBe(true);
  ...
  it("returns false for sessions route when --store value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "zuvix", "sessions", "--store"]);
  });

  it("returns false for sessions route when --active value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "zuvix", "sessions", "--active"]);
  });

  it("returns false for sessions route when --agent value is missing", async () => {
    await expectRunFalse(["sessions"], ["node", "zuvix", "sessions", "--agent"]);
  });
  ...
  it("returns false for config get route when path argument is missing", async () => {
    await expectRunFalse(["config", "get"], ["node", "zuvix", "config", "get", "--json"]);
  });

  it("returns false for config unset route when path argument is missing", async () => {
    await expectRunFalse(["config", "unset"], ["node", "zuvix", "config", "unset"]);
  });
  ...
  it("passes config unset path correctly when root option values precede command", async () => {
    ...
    await expect(
      route.run(["node", "zuvix", "--profile", "work", "config", "unset", "update.channel"]),
    ).resolves.toBe(true);
  ...
  it("passes config unset path when root value options appear after subcommand", async () => {
    const route = expectRoute(["config", "unset"]);
    await expect(
      route.run(["node", "zuvix", "config", "unset", "--profile", "work", "update.channel"]),
    ).resolves.toBe(true);
    expect(runConfigUnsetMock).toHaveBeenCalledWith({
      path: "update.channel",
      cliOptions: {
  ...
  it("returns false for models list route when --provider value is missing", async () => {
    await expectRunFalse(["models", "list"], ["node", "zuvix", "models", "list", "--provider"]);
  });

  it("returns false for models status route when probe flags are missing values", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "zuvix", "models", "status", "--probe-provider"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "zuvix", "models", "status", "--probe-timeout"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "zuvix", "models", "status", "--probe-concurrency"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "zuvix", "models", "status", "--probe-max-tokens"],
    );
    await expectRunFalse(
      ["models", "status"],
      ["node", "zuvix", "models", "status", "--probe-provider", "openai", "--agent"],
    );
  });
  ...
  it("returns false for models status route when --probe-profile has no value", async () => {
    await expectRunFalse(
      ["models", "status"],
      ["node", "zuvix", "models", "status", "--probe-profile"],
    );
  });
  ...
  it("routes tasks list JSON through the lean task JSON command", async () => {
    ...
    expect(rootRoute.canRun?.(["node", "zuvix", "tasks"])).toBe(false);
    await expect(
      rootRoute.run([
        "node",
        "zuvix",
    ...
    await expect(
      listRoute.run(["node", "zuvix", "tasks", "list", "--json", "--runtime=cron"]),
    ).resolves.toBe(true);
  ...
  it("routes tasks audit JSON through the lean task JSON command", async () => {
    ...
    expect(route.loadPlugins).toBeUndefined();
    expect(route.canRun?.(["node", "zuvix", "tasks", "audit"])).toBe(false);
    await expect(
      route.run([
        "node",
        "zuvix",
  ...
  it("returns false for task JSON routes when option values are missing or unknown", async () => {
    await expectRunFalse(["tasks"], ["node", "zuvix", "tasks", "--json", "--runtime"]);
    await expectRunFalse(["tasks", "list"], ["node", "zuvix", "tasks", "list"]);
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "zuvix", "tasks", "audit", "--json", "--limit"],
    );
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "zuvix", "tasks", "audit", "--json", "--limit", "5abc"],
    );
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "zuvix", "tasks", "audit", "--json", "--unknown"],
    );
    expect(
      findRoutedCommand(["tasks", "cli"], ["node", "zuvix", "tasks", "--runtime", "cli"]),
    ).toBeNull();
  });
});
