  ...
  program
    ...
    .addHelpText(
      ...
      () =>
        ...
        `  ${theme.command("zuvix setup")}\n` +
        `    ${theme.muted("Create config, workspace, and session folders.")}\n` +
        `  ${theme.command("zuvix setup --wizard")}\n` +
        `    ${theme.muted("Run full onboarding for auth, models, Gateway, and channels.")}\n\n` +
        `${theme.muted("Docs:")} ${formatDocsLink("/cli/setup", "docs.zuvix.ai/cli/setup")}\n`,
    )
    .option(
      "--workspace <dir>",
      "Agent workspace directory (default: ~/.zuvix/workspace; stored as agents.defaults.workspace)",
    )
