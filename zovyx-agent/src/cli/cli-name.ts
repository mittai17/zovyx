...
const DEFAULT_CLI_NAME = "zuvix";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(zuvix)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `zuvix`. */
...
/** Replace a leading `zuvix` command prefix with the active CLI name. */
