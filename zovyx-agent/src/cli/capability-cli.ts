...
import { detectMime, extensionForMime, normalizeMimeType } from "@zuvix/media-core/mime";
import {
...
const LOCAL_MODEL_RUN_SYSTEM_PROMPT = "You are a personal assistant running inside Zuvix.";
...
function getAuthProfileIdsForProvider(cfg: ZuvixConfig, providerId: string): string[] {
...
function pinRuntimeConfigSnapshot(config: ZuvixConfig): void {
...
export function registerCapabilityCli(program: Command) {
  ...
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.zuvix.ai/cli/infer")}\n`,
    );
