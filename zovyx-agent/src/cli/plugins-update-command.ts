// `zuvix plugins update` command implementation for tracked npm plugins and hook packs.
...
function projectUpdaterResultOntoSourceConfig(params: {
  runtimeBase: ZuvixConfig;
  sourceBase: ZuvixConfig;
  updatedConfig: ZuvixConfig;
}): ZuvixConfig {
  const updatePatch = createMergePatch(params.runtimeBase, params.updatedConfig);
  return applyMergePatch(params.sourceBase, updatePatch) as ZuvixConfig;
}
