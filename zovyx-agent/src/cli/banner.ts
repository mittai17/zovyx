...
export function formatCliBannerLine(version: string, options: BannerOptions = {}): string {
  ...
  const title = decorativePrefix("🦞", "Zuvix", emojiOptions);
...
function formatCliBannerArtLines(options: BannerOptions): string[] {
  ...
  const emojiOptions = resolveEmojiOptions(options);
  const title = supportsDecorativeEmoji(emojiOptions) ? "🦞 ZUVIX 🦞" : "ZUVIX";
  return [...LOBSTER_ASCII_BODY, centerText(title, width), " "];
}

/** Format the large decorative Zuvix banner art. */
export function formatCliBannerArt(options: BannerOptions = {}): string {
  ...
  const colored = lines.map((line) => {
    if (line.includes("ZUVIX")) {
      if (!icon) {
        return theme.info(centerText("ZUVIX", visibleWidth(line)));
      }
      return (
        theme.muted("              ") +
        theme.accent(icon) +
