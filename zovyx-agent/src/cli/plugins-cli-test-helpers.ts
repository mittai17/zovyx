...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as ZuvixConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: ZuvixConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/zuvix-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: ZuvixConfig) =>
    invokeMock<
      [ZuvixConfig],
...
export function resetPluginsCliTestState() {
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: ZuvixConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
