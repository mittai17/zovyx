...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix security audit", "Run a local security audit."],
          [
            "zuvix security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["zuvix security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "zuvix security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "zuvix security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["zuvix security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["zuvix security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.zuvix.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Zuvix security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("zuvix security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("zuvix security audit --fix")}`));
        if (!fixResult) {
