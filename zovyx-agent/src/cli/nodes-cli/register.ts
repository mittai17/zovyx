    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix nodes status", "List known nodes with live status."],
          ["zuvix nodes pairing pending", "Show pending node pairing requests."],
          ["zuvix nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'zuvix nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["zuvix nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.zuvix.ai/cli/nodes")}\n`,
    );
