...
vi.mock("../terminal/links.js", () => ({
  formatDocsLink: () => "docs.zuvix.ai/cli/skills",
}));
...
describe("skills workshop cli", () => {
  ...
  beforeEach(async () => {
    testState = await createZuvixTestState({
      layout: "state-only",
      prefix: "zuvix-skills-cli-workshop-state-",
    });
    mocks.workspaceDir = await tempDirs.make("zuvix-skills-cli-workshop-");
  ...
  it("scopes list and inspect to the selected workspace", async () => {
    ...
    mocks.workspaceDir = await tempDirs.make("zuvix-skills-cli-workshop-second-");
    await runCommand(["skills", "workshop", "list"]);
