...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ZUVIX_CONTAINER_HINT", "ZUVIX_PROFILE"]);
    delete process.env.ZUVIX_CONTAINER_HINT;
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["zuvix", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("repairs stale loaded service definitions from gateway start", async () => {
    ...
    runServiceStart.mockImplementation(
      async (params: {
        ...
        await params.repairLoadedService?.({
          json: true,
          stdout: process.stdout,
          state: { command: { environment: { ZUVIX_SERVICE_VERSION: "2026.4.24" } } },
    ...
    expect(repairParams.state?.command?.environment).toEqual({
      ZUVIX_SERVICE_VERSION: "2026.4.24",
    });
  ...
  it("does not fall back to unmanaged restart when launchd repair reports headless GUI bootstrap failure", async () => {
    ...
    recoverInstalledLaunchAgent.mockRejectedValue(
      new Error("LaunchAgent zuvix gateway restart requires a logged-in macOS GUI session"),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
  ...
  it("delegates system-scope restart to systemctl without unmanaged signaling when root (zuvix#87577)", async () => {
  ...
  it("surfaces systemd sudo guidance and never signals when restarting a system-scope unit as non-root (zuvix#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "zuvix.service",
    ...
    restartSystemdService.mockRejectedValue(
      new Error(
        "zuvix.service is a system-scope unit (/etc/systemd/system/zuvix.service); run `sudo systemctl restart zuvix.service` to restart it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    mockUnmanagedRestart();
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toThrow(
      /sudo systemctl restart zuvix\.service/,
    );
  ...
  it("delegates system-scope stop to systemctl without unmanaged signaling when root (zuvix#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "zuvix-gateway.service",
  ...
  it("surfaces systemd sudo guidance and never signals when stopping a system-scope unit as non-root (zuvix#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "zuvix-gateway.service",
    ...
    stopSystemdService.mockRejectedValue(
      new Error(
        "zuvix-gateway.service is a system-scope unit (/etc/systemd/system/zuvix-gateway.service); run `sudo systemctl stop zuvix-gateway.service` to stop it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    runServiceStop.mockImplementation(async (params: { onNotLoaded?: () => Promise<unknown> }) => {
