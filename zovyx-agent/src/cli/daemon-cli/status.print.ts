...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("zuvix doctor")}" (or "${formatCliCommand("zuvix doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("zuvix gateway install --force")}\` from the same --profile / ZUVIX_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this Zuvix command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `zuvix --version`, `which zuvix`, and `zuvix gateway status --deep`; if this mismatch is unexpected, update PATH so `zuvix` points to the version you want, or reinstall the Gateway service from that same Zuvix install.",
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("zuvix gateway restart")} and re-check with ${formatCliCommand("zuvix gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (status.connections?.established.length) {
    ...
    defaultRuntime.log(
      warnText(
        "If logs show protocol mismatch after rollback, stop stale Zuvix client processes listed here and re-run gateway status.",
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.ZUVIX_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("zuvix gateway install")}`),
    );
    spacer();
  }

  if (service.staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale Zuvix updater launchd job(s) detected."));
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("zuvix gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.ZUVIX_PROFILE);
      defaultRuntime.error(
  ...
  if (drift && drift.drifts.length > 0) {
    ...
    if (opts.deep) {
      ...
      defaultRuntime.log(
        `${label("Fix:")} ${formatCliCommand("zuvix plugins update <plugin-id>")} for each drifted plugin, then ${formatCliCommand("zuvix gateway restart")}.`,
      );
    } else {
      defaultRuntime.log(
        infoText(
          `Run ${formatCliCommand("zuvix gateway status --deep")} for affected plugin ids and fix commands.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("zuvix status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.zuvix.ai/troubleshooting`);
}
