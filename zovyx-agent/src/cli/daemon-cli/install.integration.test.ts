...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("zuvix-daemon-install-int-");
    configPath = path.join(tempHome, "zuvix.json");
    process.env.HOME = tempHome;
    process.env.ZUVIX_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer Zuvix", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.ZUVIX_GATEWAY_TOKEN).toBeUndefined();
  });
});
