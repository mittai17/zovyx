...
export async function repairLoadedGatewayServiceForStart(params: {
  ...
  const wrapperPath = await resolveZuvixWrapperPath(installEnv[ZUVIX_WRAPPER_ENV_KEY]);
  ...
  return {
    result: "started",
    message:
      "Gateway service definition repaired and started. Reopen the Control UI with `zuvix dashboard` or copy a fresh auth URL with `zuvix dashboard --no-open`.",
