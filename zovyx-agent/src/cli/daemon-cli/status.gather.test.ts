...
import type { StaleZuvixUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const findStaleZuvixUpdateLaunchdJobs = vi.fn<
  (env?: NodeJS.ProcessEnv) => Promise<StaleZuvixUpdateLaunchdJob[]>
>(async () => []);
...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ...
    ZUVIX_CONFIG_PATH: "/tmp/zuvix-daemon/zuvix.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.ZUVIX_STATE_DIR ?? "/tmp/zuvix-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.ZUVIX_CONFIG_PATH ?? `${stateDir}/zuvix.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/zuvix-daemon/");
...
vi.mock("../../daemon/launchd.js", () => ({
  findStaleZuvixUpdateLaunchdJobs: (env?: NodeJS.ProcessEnv) =>
    findStaleZuvixUpdateLaunchdJobs(env),
}));
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ZUVIX_STATE_DIR = "/tmp/zuvix-cli";
    process.env.ZUVIX_CONFIG_PATH = "/tmp/zuvix-cli/zuvix.json";
    ...
    findStaleZuvixUpdateLaunchdJobs.mockReset();
    findStaleZuvixUpdateLaunchdJobs.mockResolvedValue([]);
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/zuvix-daemon/zuvix.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/zuvix-cli/zuvix.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ZUVIX_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.ZUVIX_GATEWAY_PORT ?? "missing-port",
    }));
    ...
    expect(
      serviceReadRuntime.mock.calls.some(([env]) => env?.ZUVIX_GATEWAY_PORT === "19001"),
    ).toBe(true);
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.ZUVIX_STATE_DIR).toBe("/tmp/zuvix-daemon");
    expect(handoffInput.ZUVIX_CONFIG_PATH).toBe("/tmp/zuvix-daemon/zuvix.json");
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      ...
      findStaleZuvixUpdateLaunchdJobs.mockResolvedValueOnce([
        {
      ...
      const staleScanEnv = findStaleZuvixUpdateLaunchdJobs.mock.calls[0]?.[0];
      expect(staleScanEnv?.ZUVIX_STATE_DIR).toBe("/tmp/zuvix-daemon");
      expect(staleScanEnv?.ZUVIX_CONFIG_PATH).toBe("/tmp/zuvix-daemon/zuvix.json");
      expect(staleScanEnv?.ZUVIX_LAUNCHD_LABEL).toBe("ai.zuvix.manual-update.gateway");
  ...
  it("does not read restart handoffs during normal status", async () => {
    ...
    expect(findStaleZuvixUpdateLaunchdJobs).not.toHaveBeenCalled();
      ...
      connections: [
        {
          ...
          ppid: 1,
          command: "node",
          commandLine: "node /tmp/newer-zuvix/dist/index.js logs --follow",
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-status-config-"));
    const configPath = path.join(tmp, "zuvix.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    process.env.ZUVIX_STATE_DIR = tmp;
    process.env.ZUVIX_CONFIG_PATH = configPath;
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        ZUVIX_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.ZUVIX_GATEWAY_TOKEN = "env-token";
    process.env.ZUVIX_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "zuvix-gateway" }],
        hints: [],
      },
      healthy: false,
  ...
  it("reads install records from the merged daemon service environment, not the CLI process env", async () => {
    ...
    // The mock daemon service command sets ZUVIX_STATE_DIR=/tmp/zuvix-daemon,
    // distinct from the CLI process ZUVIX_STATE_DIR=/tmp/zuvix-cli. Drift
