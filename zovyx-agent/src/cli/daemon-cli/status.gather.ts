...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): ZuvixConfig | null {
...
function coerceStatusConfig(value: unknown): ZuvixConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleZuvixUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
   ...
   * can happen after `npm install -g zuvix@<v>` updates the gateway binary
   * without a corresponding `zuvix plugins update`.
   */
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleZuvixUpdateLaunchdJobs }) =>
            findStaleZuvixUpdateLaunchdJobs(serviceEnv),
          )
          .catch(() => [])
      : [];
