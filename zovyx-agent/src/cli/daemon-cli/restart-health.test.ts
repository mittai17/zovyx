...
describe("inspectGatewayRestart", () => {
  ...
  it("treats a gateway listener child pid as healthy ownership", async () => {
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 7001, ppid: 7000, commandLine: "zuvix-gateway" }],
        hints: [],
      },
    });
  ...
  it("treats a busy port as healthy when runtime status lags but the probe succeeds", async () => {
    ...
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 9100, commandLine: "zuvix-gateway" }],
        hints: [],
      },
    });
  ...
  it("requires the expected gateway version when provided", async () => {
    ...
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 8000, commandLine: "zuvix-gateway" }],
        hints: [],
      },
    });
  ...
  it("times out when running service proof never arrives", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 5151, commandLine: "zuvix-gateway" }],
      hints: [],
    });
  ...
  it("uses configured local probe auth while waiting for a matching-version restart", async () => {
    ...
    const serviceEnv = {
      ...process.env,
      ZUVIX_STATE_DIR: "/tmp/zuvix-restart-service-state",
