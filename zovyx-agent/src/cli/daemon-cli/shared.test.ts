...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        ZUVIX_CONTAINER: "zuvix-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("zuvix-demo-container");
    expect(
      resolveDaemonContainerContext({
        ZUVIX_CONTAINER_HINT: "zuvix-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("zuvix-demo-container");
  });

  it("prepends a single container restart hint when ZUVIX_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        ZUVIX_CONTAINER: "zuvix-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when ZUVIX_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        ZUVIX_CONTAINER_HINT: "zuvix-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when ZUVIX_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
          "If you're in a container, run the gateway in the foreground instead of `zuvix gateway`.",
        ],
        { ZUVIX_CONTAINER: "zuvix-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when ZUVIX_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { ZUVIX_CONTAINER_HINT: "zuvix-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
