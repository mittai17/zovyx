...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-bundled-skills-test-"));
    process.env.ZUVIX_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
