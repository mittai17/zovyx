...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix channels list", "List configured channels."],
          ["zuvix channels list --all", "Show configured, bundled, and installable channels."],
          ["zuvix channels add", "Open guided channel setup."],
          ["zuvix channels status --probe", "Run channel status checks and probes."],
          [
            "zuvix channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["zuvix channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.zuvix.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zuvix channels add", "Open guided setup for available chat channels."],
          [
            "zuvix channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["zuvix channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
