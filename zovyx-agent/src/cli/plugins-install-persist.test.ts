...
describe("persistPluginInstall", () => {
  ...
  it("adds installed plugins to restrictive allowlists before enabling", async () => {
    ...
    enablePluginInConfig.mockImplementation((...args: unknown[]) => {
      const [cfg, pluginId] = args as [ZuvixConfig, string];
  ...
  it("removes a replaced managed install directory before refreshing the registry", async () => {
    ...
    setInstalledPluginIndexInstallRecords({
      codex: {
        source: "clawhub",
        spec: "clawhub:@zuvix/codex",
        installPath: "/tmp/zuvix/extensions/codex",
      },
    });
    planPluginUninstall.mockReturnValueOnce({
      ...
      directoryRemoval: {
        target: "/tmp/zuvix/extensions/codex",
      },
    });
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@zuvix/codex",
        installPath: "/tmp/zuvix/npm/node_modules/@zuvix/codex",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "Removed previous plugin install directory: /tmp/zuvix/extensions/codex",
    );
  });
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@zuvix/discord",
        installPath: "/tmp/zuvix/npm/node_modules/@zuvix/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/zuvix-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/zuvix/npm/node_modules/@zuvix/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("zuvix plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/zuvix/npm/node_modules/@zuvix/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@zuvix/discord",
        installPath: "/tmp/zuvix/npm/node_modules/@zuvix/discord",
      },
    });
