...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.ZUVIX_GATEWAY_STARTUP_TRACE);
...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or ZUVIX_GATEWAY_PASSWORD.",
  );
}
...
async function maybeLogPendingControlUiBuild(cfg: ZuvixConfig): Promise<void> {
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("zuvix setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("zuvix onboard --mode local")}\` or \`${formatCliCommand("zuvix setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
export async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  if (process.env.ZUVIX_SERVICE_MARKER?.trim()) {
    process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV] = String(process.pid);
  }
  const isDevProfile = normalizeOptionalLowercaseString(process.env.ZUVIX_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.ZUVIX_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.ZUVIX_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.ZUVIX_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (futureStartupBlock && process.env.ZUVIX_SERVICE_MARKER?.trim()) {
  ...
  if (opts.force) {
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("zuvix gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or ZUVIX_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set ZUVIX_GATEWAY_TOKEN or ZUVIX_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or ZUVIX_GATEWAY_TOKEN/ZUVIX_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  const deferStartupSidecars =
    isTruthyEnvValue(process.env.ZUVIX_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.ZUVIX_SKIP_PROVIDERS);
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("zuvix gateway stop")}`,
      );
      try {
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("zuvix gateway status --deep")} for diagnostics.`,
    );
    defaultRuntime.exit(1);
  }
}
