...
const SANDBOX_EXAMPLES = {
  main: [
    ["zuvix sandbox list", "List all sandbox containers."],
    ["zuvix sandbox list --browser", "List only browser containers."],
    ["zuvix sandbox recreate --all", "Recreate all containers."],
    ["zuvix sandbox recreate --session main", "Recreate a specific session."],
    ["zuvix sandbox recreate --agent mybot", "Recreate agent containers."],
    ["zuvix sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["zuvix sandbox list", "List all sandbox containers."],
    ["zuvix sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["zuvix sandbox recreate --all", "Recreate all containers."],
    ["zuvix sandbox recreate --session main", "Recreate a specific session."],
    ["zuvix sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["zuvix sandbox recreate --browser --all", "Recreate only browser containers."],
    ["zuvix sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["zuvix sandbox explain", "Show effective sandbox config."],
    ["zuvix sandbox explain --session agent:main:main", "Explain a specific session."],
    ["zuvix sandbox explain --agent work", "Explain an agent sandbox."],
    ["zuvix sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.zuvix.ai/cli/sandbox")}\n`,
    )
    .action(() => {
