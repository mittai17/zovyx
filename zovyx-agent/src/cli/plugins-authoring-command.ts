...
export function buildToolPluginPackageManifest(params: {
  ...
  const zuvix =
    params.packageManifest.zuvix &&
    typeof params.packageManifest.zuvix === "object" &&
    !Array.isArray(params.packageManifest.zuvix)
      ? { ...(params.packageManifest.zuvix as JsonObject) }
      : {};
  const existingExtensions = Array.isArray(zuvix.extensions)
    ? zuvix.extensions.filter((entry): entry is string => typeof entry === "string")
    : [];
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("zuvix.plugin.json generated metadata is stale. Run zuvix plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `zuvix.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("zuvix.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`zuvix.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `zuvix.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include zuvix.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json zuvix.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run zuvix plugins build.");
...
export async function runPluginsInitCommand(id: string, opts: PluginsInitOptions): Promise<void> {
  ...
  const packageManifest = {
    name: `zuvix-plugin-${id}`,
    version: "0.1.0",
    type: "module",
    private: true,
    scripts: {
      build: "tsc -p tsconfig.json",
      "plugin:build": "npm run build && zuvix plugins build --entry ./dist/index.js",
      "plugin:validate": "npm run build && zuvix plugins validate --entry ./dist/index.js",
      test: "vitest run",
    },
  ...
  const descriptionLiteral = jsStringLiteral(`Add ${name} tools to Zuvix.`);
...
import { getToolPluginMetadata } from "zuvix/plugin-sdk/tool-plugin";
...
`;
  ...
  writeJsonFile(path.join(rootDir, PLUGIN_MANIFEST_FILENAME), {
    id,
    name,
    description: `Add ${name} tools to Zuvix.`,
