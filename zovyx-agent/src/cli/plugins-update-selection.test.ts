  ...
  it("maps an explicit scoped npm dist-tag update to the tracked plugin id", () => {
    expect(
      resolvePluginUpdateSelection({
        ...
        rawId: "@zuvix/voice-call@beta",
      }),
    ).toEqual({
      pluginIds: ["voice-call"],
      specOverrides: {
        "voice-call": "@zuvix/voice-call@beta",
      },
    });
  });
  ...
  it("maps an explicit npm version update to the tracked plugin id", () => {
    expect(
      resolvePluginUpdateSelection({
        ...
        rawId: "zuvix-codex-app-server@0.2.0-beta.4",
      }),
    ).toEqual({
      pluginIds: ["zuvix-codex-app-server"],
      specOverrides: {
