...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "zuvix", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "zuvix", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "zuvix", "--profile", "work", "gateway", "status"],
