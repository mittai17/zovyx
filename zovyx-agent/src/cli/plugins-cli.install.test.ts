...
const CLI_STATE_ROOT = "/tmp/zuvix-state";
const ORIGINAL_ZUVIX_STATE_DIR = process.env.ZUVIX_STATE_DIR;
const ORIGINAL_ZUVIX_NIX_MODE = process.env.ZUVIX_NIX_MODE;
const PROFILE_STATE_ROOT = "/tmp/zuvix-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.ZUVIX_STATE_DIR = PROFILE_STATE_ROOT;
  return path.resolve(PROFILE_STATE_ROOT, "extensions");
}

function createEnabledPluginConfig(pluginId: string): ZuvixConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): ZuvixConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createNpmPackPluginInstallResult(
  ...
  return {
    ...
    manifestName: `@zuvix/${pluginId}`,
    npmTarballName: `zuvix-${pluginId}-1.2.3.tgz`,
    npmResolution: {
      name: `@zuvix/${pluginId}`,
      version: "1.2.3",
      resolvedSpec: `@zuvix/${pluginId}@1.2.3`,
...
function createPathHookPackInstalledConfig(tmpRoot: string): ZuvixConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): ZuvixConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  ...
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing zuvix.plugin.json",
  });
...
function primeBlockedNpmPluginInstall(params: {
  ...
  loadConfig.mockReturnValue({} as ZuvixConfig);
...
function replaceConfigCall(callIndex = 0): { baseHash?: string; nextConfig?: ZuvixConfig } {
...
function primeBlockedPluginConfigMutation(
  params: { blockHooks?: boolean; config?: ZuvixConfig } = {},
): void {
  const configPath = path.join(process.cwd(), "zuvix.json5");
  ...
  const config = params.config ?? ({} as ZuvixConfig);
  const parsed = {
...
function primeBlockedRootConfigMutation(config = {} as ZuvixConfig): void {
  const configPath = path.join(process.cwd(), "zuvix.json5");
  loadConfig.mockReturnValue(config);
  readConfigFileSnapshotForWrite.mockResolvedValue({
    snapshot: {
...
function primeBlockedHookConfigMutation(config = {} as ZuvixConfig): void {
  const configPath = path.join(process.cwd(), "zuvix.json5");
  const externalHooksPath = path.join(
    path.parse(process.cwd()).root,
    "external-zuvix",
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_ZUVIX_STATE_DIR === undefined) {
      delete process.env.ZUVIX_STATE_DIR;
    } else {
      process.env.ZUVIX_STATE_DIR = ORIGINAL_ZUVIX_STATE_DIR;
    }
    if (ORIGINAL_ZUVIX_NIX_MODE === undefined) {
      delete process.env.ZUVIX_NIX_MODE;
    } else {
      process.env.ZUVIX_NIX_MODE = ORIGINAL_ZUVIX_NIX_MODE;
    }
  });
  ...
  it.each(["@acme/demo-plugin", "npm:@acme/demo-plugin"])(
    ...
    async (spec) => {
      ...
      installHooksFromNpmSpec.mockResolvedValue({
        ok: false,
        error: "package.json missing zuvix.hooks",
      });
  ...
  it("blocks local package inspection when plugin and hook config are include-owned", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-hook-pack-"));
    primeBlockedPluginConfigMutation({ blockHooks: true });
    installHooksFromPath.mockResolvedValue(createHookPackInstallResult(localPath));
    installPluginFromPath.mockResolvedValue({
      ok: false,
      error: "package.json missing zuvix.extensions",
      code: "missing_zuvix_extensions",
    });
  ...
  it("fails closed for ambiguous local plugins when the whole config is include-owned", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-demo-plugin-"));
    primeBlockedRootConfigMutation();
  ...
  it("fails closed before installing a blocked ambiguous local plugin", async () => {
    const archivePath = path.join(os.tmpdir(), `zuvix-plugin-${process.pid}.tgz`);
  ...
  it("fails closed when a local hook probe finds a plugin-capable package", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-dual-package-"));
    primeBlockedPluginConfigMutation();
    installHooksFromPath.mockResolvedValue({
  ...
  it("fails closed for a local bundle plugin instead of installing its hooks", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-bundle-plugin-"));
    primeBlockedPluginConfigMutation();
    installHooksFromPath.mockResolvedValue({
      ...createHookPackInstallResult(localPath),
      packageKind: "plugin-capable",
  ...
  it("fails closed when a blocked-config local hook probe throws", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-local-plugin-"));
    primeBlockedPluginConfigMutation();
  ...
  it("blocks explicit plugins through nested include config before installer side effects", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-nested-"));
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `zuvix doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("resolves exact official external plugin ids through their npm package", async () => {
    ...
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
    expect(npmInstallCall().spec).toBe("@zuvix/brave-plugin");
    ...
    expect(record.spec).toBe("@zuvix/brave-plugin");
  ...
  it("passes third-party external catalog integrity with catalog install trust", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("wecom-zuvix-plugin");
    loadConfig.mockReturnValue(cfg);
    findBundledPluginSourceMock.mockReturnValue(undefined);
    installPluginFromNpmSpec.mockResolvedValue(
      createNpmPluginInstallResult("wecom-zuvix-plugin"),
    );
    enablePluginInConfig.mockReturnValue({ config: enabledCfg });
    applyExclusiveSlotSelection.mockReturnValue({
      config: enabledCfg,
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-zuvix-plugin@2026.5.7");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-zuvix-plugin");
  ...
  it("passes third-party external catalog integrity to hook-pack fallback", async () => {
    ...
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "aborted: npm package integrity drift detected for @wecom/wecom-zuvix-plugin@2026.5.7",
    });

    await expect(runPluginsCommand(["plugins", "install", "wecom"])).rejects.toThrow("__exit__:1");

    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(hookNpmInstallCall().spec).toBe("@wecom/wecom-zuvix-plugin@2026.5.7");
  ...
  it("installs npm-pack archives through npm install semantics", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    const archivePath = "/tmp/zuvix-demo-1.2.3.tgz";
    ...
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("npm");
    expect(record.spec).toBe("@zuvix/demo@1.2.3");
    ...
    expect(record.npmTarballName).toBe("zuvix-demo-1.2.3.tgz");
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
  });
  ...
  it("marks explicit official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "npm:@zuvix/discord"]);
  ...
  it("marks scoped official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@zuvix/discord"]);
  ...
  it("uses bundled Zuvix package specs instead of pinning stale managed npm overrides", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("discord");
    ...
    findBundledPluginSourceMock.mockImplementation((params: unknown) => {
      ...
      return lookup.kind === "npmSpec" && lookup.value === "@zuvix/discord"
        ? {
            pluginId: "discord",
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@zuvix/discord@2026.5.20" },
    });
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@zuvix/discord" },
    });
    const record = persistedInstallRecord("discord");
    expect(record.source).toBe("path");
    expect(record.spec).toBe("@zuvix/discord@2026.5.20");
    expect(record.sourcePath).toBe(bundledPath);
    expect(record.installPath).toBe(bundledPath);
    expect(runtimeLogsContain("ships with the current Zuvix build")).toBe(true);
    expect(runtimeLogsContain("npm:@zuvix/discord@2026.5.20")).toBe(true);
  });
  ...
  it("marks catalog npm package installs with alternate selectors as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@wecom/wecom-zuvix-plugin@latest"]);
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-zuvix-plugin@latest");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-zuvix-plugin");
    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(npmInstallCall().expectedIntegrity).toBeUndefined();
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("adds a Git PATH hint when npm plugin dependency install cannot spawn git", async () => {
    ...
    await expect(
      runPluginsCommand(["plugins", "install", "npm:@zuvix/whatsapp"]),
    ).rejects.toThrow("__exit__:1");
  ...
  it("does not resolve npm: prefixed bundled plugin ids through bundled installs", async () => {
    loadConfig.mockReturnValue({ plugins: { load: { paths: [] } } } as ZuvixConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("rejects --pin for git installs and points at git refs", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain("zuvix plugins install git:<repo>@<ref>");
  });
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/zuvix/.zuvix/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing zuvix.hooks",
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `zuvix plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-local-hook-pack-"));
    const cfg = {} as ZuvixConfig;
    const installedCfg = {
      hooks: {
        internal: {
  ...
  it("does not fall back to npm when explicit ClawHub rejects a real package", async () => {
    ...
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "zuvix skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "zuvix skills install demo" instead.');
  });
