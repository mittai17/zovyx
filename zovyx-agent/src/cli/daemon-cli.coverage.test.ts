...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      ZUVIX_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { ZUVIX_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { ZUVIX_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-daemon-cli-"));
    ...
    process.env.ZUVIX_STATE_DIR = tmpDir;
    process.env.ZUVIX_CONFIG_PATH = path.join(tmpDir, "zuvix.json");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ZUVIX_PROFILE: "dev",
        ZUVIX_STATE_DIR: "/tmp/zuvix-daemon-state",
        ZUVIX_CONFIG_PATH: "/tmp/zuvix-daemon-state/zuvix.json",
        ZUVIX_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.zuvix.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).ZUVIX_WRAPPER).toBe(
      "/usr/local/bin/zuvix-doppler",
    );
  });
