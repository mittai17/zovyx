...
describe("zuvix skills install ClawHub GitHub-backed E2E", () => {
  it("installs from the install resolver and reports install telemetry", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-clawhub-cli-e2e-"));
    try {
      const result = await spawnZuvix(["skills", "install", "aiq-deploy", "--global"], {
        cwd: process.cwd(),
        env: {
          ...process.env,
          ZUVIX_STATE_DIR: stateDir,
          ZUVIX_CONFIG_PATH: path.join(stateDir, "zuvix.json"),
          ZUVIX_CLAWHUB_URL: registry,
          ZUVIX_CLAWHUB_TOKEN: "test-token",
          ZUVIX_CLAWHUB_GITHUB_CODELOAD_BASE_URL: registry,
