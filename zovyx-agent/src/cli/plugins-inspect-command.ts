// `zuvix plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `zuvix plugins inspect`. */
...
export async function runPluginsInspectCommand(
  ...
  if (!inspect) {
    defaultRuntime.error(
      formatMissingPluginMessage({ id, listCommand: "zuvix plugins list --json" }),
    );
    return defaultRuntime.exit(1);
  }
  ...
  lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "zuvix"}`);
