...
async function installBundledPluginSource(params: {
  ...
  const configWarning = shouldEnable
    ? ""
    : `Installed bundled plugin "${params.bundledSource.pluginId}" without enabling it because it requires configuration first. Configure it, then run \`zuvix plugins enable ${params.bundledSource.pluginId}\`.`;
...
async function loadConfigFromSnapshotForInstall(
  ...
  if (!snapshot.exists || Object.keys(parsed).length === 0) {
    throw buildInvalidPluginInstallConfigError(
      "Config file could not be parsed; run `zuvix doctor` to repair it.",
    );
  }
  ...
  if (
    ...
    throw buildInvalidPluginInstallConfigError(
      `Config invalid outside the plugin recovery path for ${pluginLabel}; run \`zuvix doctor --fix\` before reinstalling it.`,
    );
  }
  if (!supportsPluginRecoveryIncludeShape(parsed)) {
    throw buildInvalidPluginInstallConfigError(
      "Config plugin recovery uses an unsupported $include shape; use a single-file top-level plugins include or run `zuvix doctor --fix` before reinstalling it.",
    );
  }
...
export async function runPluginInstallCommand(params: {
  ...
  if (opts.marketplace) {
    if (opts.link) {
      runtime.error(
        `--link is not supported with --marketplace. Remove --link, or install a local path with ${formatCliCommand("zuvix plugins install --link <path>")}.`,
      );
      return runtime.exit(1);
    }
    if (opts.pin) {
      runtime.error(
        `--pin is not supported with --marketplace. Use ${formatCliCommand("zuvix plugins install <plugin> --marketplace <name>")} without --pin.`,
      );
      return runtime.exit(1);
    }
  }
  ...
  if (gitPrefix && !gitSpec) {
    runtime.error(
      `Unsupported git plugin spec: ${raw}. Use ${formatCliCommand("zuvix plugins install git:<repo>@<ref>")}.`,
    );
    return runtime.exit(1);
  }
  if (gitSpec && opts.link) {
    runtime.error(
      `--link is not supported with git: installs. Use ${formatCliCommand("zuvix plugins install git:<repo>@<ref>")} for Git installs or ${formatCliCommand("zuvix plugins install --link <path>")} for local paths.`,
    );
    return runtime.exit(1);
  }
  if (gitSpec && opts.pin) {
    runtime.error(
      `--pin is not supported with git: installs. Pin the ref in the spec instead, for example ${formatCliCommand("zuvix plugins install git:<repo>@<ref>")}.`,
    );
    return runtime.exit(1);
  }
  if (opts.link && opts.force) {
    runtime.error(
      `--force is not supported with --link. Linked plugins point at the source path directly; remove --force and re-run ${formatCliCommand("zuvix plugins install --link <path>")}.`,
    );
    return runtime.exit(1);
  }
  ...
  if (opts.link) {
    runtime.error(
      `--link requires a local path. Run ${formatCliCommand("zuvix plugins install --link <path>")}.`,
    );
    return runtime.exit(1);
  }
  ...
  if (npmPrefixSpec !== null) {
    if (!npmPrefixSpec) {
      runtime.error(
        `Unsupported npm plugin spec: missing package. Use ${formatCliCommand("zuvix plugins install npm:<package>")}.`,
      );
      return runtime.exit(1);
    }
  ...
  if (npmPackPath !== null) {
    if (!npmPackPath) {
      runtime.error(
        `Unsupported npm-pack plugin spec: missing archive path. Use ${formatCliCommand("zuvix plugins install npm-pack:<path-to.tgz>")}.`,
      );
      return runtime.exit(1);
    }
  ...
  if (
    ...
    runtime.error(
      `Plugin path not found: ${resolved}. Check the path, or install from npm with ${formatCliCommand("zuvix plugins install npm:<package>")}.`,
    );
    return runtime.exit(1);
  }
