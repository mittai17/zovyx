...
describe("plugins cli list", () => {
  ...
  it("reports stale plugin config in doctor output without claiming full plugin health", async () => {
    ...
    expect(output).toContain(
      'Run "zuvix doctor --fix" to remove stale plugin ids and dangling channel references.',
    );
    expect(output).toContain(
  ...
  it("reports missing configured Codex runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("zuvix doctor --fix");
    expect(output).toContain("zuvix plugins install @zuvix/codex");
    expect(output).toContain(
      "No plugin install-tree issues detected; configuration warnings remain.",
    );
    expect(output).not.toContain("No plugin issues detected.");
  ...
  it("reports missing configured ACPX runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("zuvix doctor --fix");
    expect(output).toContain("zuvix plugins install @zuvix/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured ACPX runtime with ACP-specific guidance", async () => {
    ...
    expect(output).not.toContain('runtime policy to "zuvix"');
    expect(output).not.toContain("zuvix plugins install @zuvix/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports configured Codex runtime when the plugin record is disabled", async () => {
    ...
    expect(output).not.toContain("zuvix plugins install @zuvix/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured Codex runtime without install advice", async () => {
    ...
    expect(output).not.toContain('Run "zuvix doctor --fix" to install');
    expect(output).not.toContain("zuvix plugins install @zuvix/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports config-selected plugin source shadowing in doctor output", async () => {
    buildPluginDiagnosticsReport.mockReturnValue({
      plugins: [
        createPluginRecord({
          id: "discord",
          origin: "config",
          source: "/tmp/zuvix-upstream/extensions/discord/index.ts",
          status: "error",
      ...
      diagnostics: [
        {
          level: "warn",
          pluginId: "discord",
          source: "/tmp/zuvix/npm/node_modules/@zuvix/discord/index.ts",
          message:
            "duplicate plugin id resolved by explicit config-selected plugin; global plugin will be overridden by config plugin (/tmp/zuvix-upstream/extensions/discord/index.ts)",
        },
      ],
    });
    ...
    expect(output).toContain("active: /tmp/zuvix-upstream/extensions/discord/index.ts");
    expect(output).toContain("shadowed: /tmp/zuvix/npm/node_modules/@zuvix/discord/index.ts");
    expect(output).toContain("zuvix plugins registry --refresh");
  });
  ...
  it("reports persisted plugin registry state without refreshing", async () => {
    ...
    expect(runtimeLogs.join("\n")).toContain("zuvix plugins registry --refresh");
  });
  ...
  it("keeps inspect on the static snapshot by default", async () => {
    setInstalledPluginIndexInstallRecords({
      "zuvix-mem0": {
        source: "clawhub",
        spec: "clawhub:zuvix-mem0",
        installPath: "/plugins/zuvix-mem0",
        version: "2026.5.1",
        clawhubPackage: "zuvix-mem0",
        clawhubChannel: "official",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmIntegrity: "sha512-clawpack",
        npmShasum: "1".repeat(40),
        npmTarballName: "zuvix-mem0-2026.5.1.tgz",
    ...
    buildPluginSnapshotReport.mockReturnValue({
      plugins: [createPluginRecord({ id: "zuvix-mem0", name: "Mem0" })],
      diagnostics: [],
    });
    buildPluginInspectReport.mockReturnValue({
      workspaceDir: "/workspace",
      plugin: createPluginRecord({ id: "zuvix-mem0", name: "Mem0" }),
      shape: "hook-only",
    ...
    await runPluginsCommand(["plugins", "inspect", "zuvix-mem0"]);

    expect(buildPluginDiagnosticsReport).not.toHaveBeenCalled();
    expect(runtimeLogs.join("\n")).toContain("Policy");
    expect(runtimeLogs.join("\n")).toContain("allowConversationAccess: true");
    expect(runtimeLogs.join("\n")).toContain("ClawHub package: zuvix-mem0");
  ...
  it("runtime-inspects without repairing deps", async () => {
    ...
    await runPluginsCommand(["plugins", "inspect", "zuvix-mem0", "--runtime"]);

    expect(buildPluginDiagnosticsReport).toHaveBeenCalledWith({
      config: {},
      onlyPluginIds: ["zuvix-mem0"],
    });
  });
