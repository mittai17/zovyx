...
describe("json output mode", () => {
  ...
  it("detects json output flags before argv terminators", () => {
    expect(hasJsonOutputFlag(["node", "zuvix", "nodes", "list", "--json"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "zuvix", "nodes", "list", "--json=true"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "zuvix", "nodes", "--", "--json"])).toBe(false);
  });
