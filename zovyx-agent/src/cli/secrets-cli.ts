...
async function readPlanFile(pathname: string): Promise<SecretsApplyPlan> {
  ...
  if (!isSecretsApplyPlan(parsed)) {
    throw new Error(
      `Invalid secrets plan file: ${pathname}. Generate a fresh plan with ${formatCliCommand("zuvix secrets configure --plan-out <path>")}.`,
    );
  }
  return parsed;
}
...
export function registerSecretsCli(program: Command): void {
  const secrets = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/gateway/security", "docs.zuvix.ai/gateway/security")}\n`,
    );
      ...
      defaultRuntime.error(
        danger(
          formatGatewayCommandFailure({
            ...
            error: err,
            inspectCommand: "zuvix gateway status --deep",
          }),
        ),
      );
  ...
  secrets
    ...
    .action(async (opts: SecretsAuditOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets audit failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("zuvix doctor")} to inspect config and credential state.`,
          ),
        );
  ...
  secrets
    ...
    .action(async (opts: SecretsConfigureOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets configure failed: ${formatErrorMessage(err)}. Re-run ${formatCliCommand("zuvix secrets audit")} before applying changes.`,
          ),
        );
        defaultRuntime.exit(1);
      }
  ...
  secrets
    ...
    .action(async (opts: SecretsApplyOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets apply failed: ${formatErrorMessage(err)}. Re-run ${formatCliCommand("zuvix secrets apply --from <path> --dry-run")} to inspect the plan without writing.`,
          ),
        );
        defaultRuntime.exit(1);
      }
