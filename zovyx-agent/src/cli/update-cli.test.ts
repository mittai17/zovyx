...
import type { ZuvixConfig, ConfigFileSnapshot } from "../config/types.zuvix.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: ZuvixConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
const launchdUpdateCleanupMocks = vi.hoisted(() => ({
  disableCurrentZuvixUpdateLaunchdJob: vi.fn(async () => false),
}));
...
vi.mock("../infra/zuvix-root.js", () => ({
  resolveZuvixPackageRoot: vi.fn(),
  resolveZuvixPackageRootSync: vi.fn(() => process.cwd()),
}));

vi.mock("../config/config.js", () => ({
  assertConfigWriteAllowedInCurrentMode: () => {
    if (process.env.ZUVIX_NIX_MODE === "1") {
      throw new Error(
        [
          ...
          "Agent-first Nix setup: https://github.com/zuvix/nix-zuvix#quick-start",
          "Zuvix Nix overview: https://docs.zuvix.ai/install/nix",
        ].join("\n"),
      );
    }
  },
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/zuvix-config",
  };
});
...
vi.mock("../daemon/launchd.js", async (importOriginal) => ({
  ...
  disableCurrentZuvixUpdateLaunchdJob:
    launchdUpdateCleanupMocks.disableCurrentZuvixUpdateLaunchdJob,
}));
...
const { resolveZuvixPackageRoot } = await import("../infra/zuvix-root.js");
const {
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/zuvix-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as ZuvixConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/zuvix-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveZuvixPackageRoot).mockResolvedValue(root);
  ...
  const stripZuvixPackageAlias = (spec: string) => {
    const trimmed = spec.trim();
    return trimmed.toLowerCase().startsWith("zuvix@")
      ? trimmed.slice("zuvix@".length)
      : trimmed;
  };

  const isNpmGitPackageSpec = (spec: string) => {
    const target = stripZuvixPackageAlias(spec);
  ...
  const syncPluginCall = (index = 0) => {
    const calls = syncPluginsForUpdateChannel.mock.calls as unknown as Array<
      [{ channel?: string; config?: ZuvixConfig }]
    >;
  ...
  const npmPluginUpdateCall = (index = 0) => {
    const calls = updateNpmInstalledPlugins.mock.calls as unknown as Array<
      [{ config?: ZuvixConfig; timeoutMs?: number }]
    >;
    return calls[index]?.[0];
  };
  ...
  const setupConfigMutationWithRetryMock = () => {
    vi.mocked(mutateConfigFileWithRetry).mockImplementation(async (params) => {
      ...
      const nextConfig = structuredClone(snapshot.sourceConfig) as ZuvixConfig;
  ...
  const expectPackageInstallSpec = (spec: string) => {
    ...
    if (isNpmGitPackageSpec(spec)) {
      ...
      installSpec = path.join(packDir, "zuvix-9999.0.0.tgz");
    } else {
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("zuvix-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("zuvix-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveZuvixPackageRoot).mockResolvedValue(process.cwd());
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        if (destination) {
          await fs.writeFile(path.join(destination, "zuvix-9999.0.0.tgz"), "packed\n", "utf8");
        }
      }
      return {
        stdout: "",
    ...
    readPackageName.mockResolvedValue("zuvix");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["zuvix", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/zuvix-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "zuvix-gateway" }],
      hints: [],
    });
    ...
    checkShellCompletionStatus.mockResolvedValue({
      ...
      cacheExists: false,
      cachePath: "/tmp/zuvix-completion.zsh",
    ...
    launchdUpdateCleanupMocks.disableCurrentZuvixUpdateLaunchdJob.mockReset();
    launchdUpdateCleanupMocks.disableCurrentZuvixUpdateLaunchdJob.mockResolvedValue(false);
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("zuvix-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "zuvix.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.ZUVIX_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
  ...
  it("disarms legacy launchd updater jobs before refusing mutating updates in Nix mode", async () => {
    await withEnvAsync({ ZUVIX_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("ZUVIX_NIX_MODE=1");
    });

    expect(launchdUpdateCleanupMocks.disableCurrentZuvixUpdateLaunchdJob).toHaveBeenCalledOnce();
    expect(runGatewayUpdate).not.toHaveBeenCalled();
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("zuvix-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logOutput).toContain("zuvix completion --write-state");
  ...
  it("keeps update completion refresh best-effort when profile install fails", async () => {
    ...
    expect(installCompletion).toHaveBeenCalledWith("zsh", true, "zuvix");
    const logOutput = vi
      .mocked(runtimeCapture.log)
      .mock.calls.map((call) => String(call[0]))
      .join("\n");
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.ZUVIX_UPDATE_IN_PROGRESS).toBe("1");
    expect(call?.[2]?.env?.ZUVIX_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.ZUVIX_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.ZUVIX_COMPATIBILITY_HOST_VERSION).toBe("1.0.0");
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.ZUVIX_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    const root = createCaseDir("zuvix-update");
            ...
            warnings: [
              {
                ...
                guidance: ["Run zuvix update repair to retry post-update plugin repair."],
              },
            ],
            sync: {
              changed: false,
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    expect(spawnEnv?.ZUVIX_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.ZUVIX_SERVICE_KIND).toBeUndefined();
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        source: "npm",
        spec: "@zuvix/demo@1.0.0",
        installPath: "/tmp/zuvix-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.ZUVIX_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      const sourceConfigPath = env?.ZUVIX_UPDATE_POST_CORE_SOURCE_CONFIG_PATH;
      if (!recordsPath) {
  ...
  it("clears stale npm resolution metadata before post-core downgrade resume", async () => {
    ...
    const pluginInstallRecords = {
      msteams: {
        source: "npm",
        spec: "@zuvix/msteams",
        installPath: "/tmp/zuvix-msteams-plugin",
        version: "1.0.0",
        resolvedName: "@zuvix/msteams",
        resolvedVersion: "1.0.0",
        resolvedSpec: "@zuvix/msteams@1.0.0",
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.ZUVIX_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.ZUVIX_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.ZUVIX_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(syncPluginsForUpdateChannel).not.toHaveBeenCalled();
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  });
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("zuvix-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("pins the compatibility host version to the downgraded target during current-process post-core plugin convergence (#87914)", async () => {
    const downgradedRoot = createCaseDir("zuvix-downgraded-compat-root");
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
    ...
    delete process.env.ZUVIX_COMPATIBILITY_HOST_VERSION;
    let hostVersionDuringPluginUpdate: string | undefined = "unset";
    updateNpmInstalledPlugins.mockImplementation(async () => {
      hostVersionDuringPluginUpdate = process.env.ZUVIX_COMPATIBILITY_HOST_VERSION;
    ...
    try {
      ...
      expect(process.env.ZUVIX_COMPATIBILITY_HOST_VERSION).toBeUndefined();
    } finally {
      delete process.env.ZUVIX_COMPATIBILITY_HOST_VERSION;
    }
  });
    ...
    await withEnvAsync(
      {
        ...
        ZUVIX_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("zuvix-post-core-result");
    ...
    await withEnvAsync(
      {
        ZUVIX_UPDATE_POST_CORE: "1",
        ZUVIX_UPDATE_POST_CORE_CHANNEL: "stable",
        ZUVIX_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("zuvix-post-core-records");
    ...
    await withEnvAsync(
      {
        ZUVIX_UPDATE_POST_CORE: "1",
        ZUVIX_UPDATE_POST_CORE_CHANNEL: "stable",
        ZUVIX_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode prefers post-doctor disk install records over the stale parent snapshot", async () => {
    const resultDir = createCaseDir("zuvix-post-core-disk-records");
    const recordsPath = path.join(resultDir, "plugin-install-records.json");
    await fs.mkdir(resultDir, { recursive: true });
    await fs.writeFile(
      recordsPath,
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as ZuvixConfig,
      sourceConfig: { update: { channel: "stable" } } as ZuvixConfig,
      runtimeConfig: { update: { channel: "stable" } } as ZuvixConfig,
      config: { update: { channel: "stable" } } as ZuvixConfig,
      hash: "stable-hash",
    });
    ...
    await withEnvAsync(
      {
        ZUVIX_UPDATE_POST_CORE: "1",
        ZUVIX_UPDATE_POST_CORE_CHANNEL: "dev",
        ZUVIX_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @zuvix/demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      "Run zuvix update repair to retry post-update plugin repair.",
      "Run zuvix plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("zuvix-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run zuvix update repair to retry post-update plugin repair.");
    expect(logs).toContain("Run zuvix plugins inspect demo --runtime --json for details.");
  });
      ...
      outcomes: [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; Zuvix will continue without it. Failed to update demo: registry timeout',
        },
      ],
    });
    vi.mocked(defaultRuntime.writeJson).mockClear();
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(() => {
        void (async () => {
          const resultPath = env?.ZUVIX_UPDATE_POST_CORE_RESULT_PATH;
          if (resultPath) {
            await fs.writeFile(
              resultPath,
              JSON.stringify({
                ...
                warnings: [
                  {
                    ...
                    message:
                      'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run zuvix update repair to retry post-update plugin repair. Run zuvix plugins inspect demo --runtime --json for details.',
                    guidance: [
                      "Run zuvix update repair to retry post-update plugin repair.",
                      "Run zuvix plugins inspect demo --runtime --json for details.",
                    ],
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "zuvix", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as ZuvixConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("zuvix@latest");
      }
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("zuvix-update"));
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "That path replaces the active Zuvix dist tree while the live gateway may still lazy-load old chunks.",
        "Run `zuvix update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("zuvix@latest");
  });
  ...
  it("refuses package updates from inherited gateway service env when --no-restart leaves the gateway running", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["zuvix", "gateway", "run"],
      environment: {
        ZUVIX_SERVICE_MARKER: "zuvix",
        ZUVIX_SERVICE_KIND: "gateway",
      },
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    const errors = vi.mocked(defaultRuntime.error).mock.calls.map((call) => String(call[0]));
    expect(errors.join("\n")).toContain(
      "zuvix update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g zuvix` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:zuvix/zuvix#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("zuvix-update"));
        await updateCommand({ yes: true, tag: "github:zuvix/zuvix#main" });
      },
      expectedSpec: "github:zuvix/zuvix#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("zuvix-update"));
        await updateCommand({ yes: true, tag: "Zuvix@github:zuvix/zuvix#main" });
      },
      expectedSpec: "Zuvix@github:zuvix/zuvix#main",
    },
    {
      name: "full git URL package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("zuvix-update"));
        await updateCommand({ yes: true, tag: "https://github.com/zuvix/zuvix.git#main" });
      },
      expectedSpec: "https://github.com/zuvix/zuvix.git#main",
    },
    {
      name: "hosted GitHub URL package spec without git suffix",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("zuvix-update"));
        await updateCommand({ yes: true, tag: "https://github.com/zuvix/zuvix#main" });
      },
      expectedSpec: "https://github.com/zuvix/zuvix#main",
    },
    {
      ...
      run: async () => {
        ...
        await updateCommand({
          yes: true,
          tag: "zuvix@https://github.com/zuvix/zuvix#main",
        });
      },
      expectedSpec: "https://github.com/zuvix/zuvix#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("zuvix-update"));
        await updateCommand({ yes: true, tag: "zuvix/zuvix#main" });
      },
      expectedSpec: "zuvix/zuvix#main",
    },
    {
      name: "SCP-style SSH package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("zuvix-update"));
        await updateCommand({ yes: true, tag: "git@github.com:zuvix/zuvix.git#main" });
      },
      expectedSpec: "git@github.com:zuvix/zuvix.git#main",
    },
    {
      name: "ZUVIX_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("zuvix-update"));
        await withEnvAsync(
          { ZUVIX_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/zuvix-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/zuvix-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "zuvix");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "zuvix", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "zuvix", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "zuvix");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "zuvix", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "zuvix");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "zuvix", version: "2026.4.21" }),
      "utf-8",
    );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.ZUVIX_UPDATE_IN_PROGRESS,
    ).toBe("1");
  });

  it("continues package post-core work for explicit post-update doctor advisories", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-package-doctor-warning-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "zuvix");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    expect(postCoreCall?.[2]?.env?.ZUVIX_UPDATE_POST_CORE).toBe("1");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    const jsonOutput = lastWriteJsonCall() as UpdateRunResult | undefined;
    const doctorStep = jsonOutput?.steps.find((step) => step.name === "zuvix doctor");
    expect(jsonOutput?.status).toBe("ok");
  ...
  it("fails package updates when the post-update doctor is killed after verification", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-package-doctor-timeout-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "zuvix");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
  ...
  it("runs package post-update doctor from the verified package root after a staged swap", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-staged-doctor-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (argv[0] === "npm" && argv[1] === "i" && argv.includes("--prefix")) {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "zuvix", version: "2026.5.14" }),
          "utf-8",
        );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.ZUVIX_COMPATIBILITY_HOST_VERSION,
    ).toBe("2026.5.14");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });

  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "zuvix");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    const serviceStopCall = serviceStop.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(serviceStopCall?.env?.ZUVIX_SERVICE_MARKER).toBe("zuvix");
    expect(serviceStopCall?.env?.ZUVIX_SERVICE_KIND).toBe("gateway");
  ...
  it("stops a running managed git gateway when wrapper commands hide the service root", async () => {
    const wrapperPath = path.join(
      createCaseDir("zuvix-update-wrapper-service"),
      "gateway-wrapper",
    );
    serviceReadCommand.mockResolvedValue({
  ...
  it("stops a managed gateway rooted at the git checkout when switching package installs to dev", async () => {
    const packageRoot = createCaseDir("zuvix-update-package-root");
    const gitRoot = await createTrackedTempDir("zuvix-update-git-service-root-");
    ...
    await withEnvAsync({ ZUVIX_GIT_DIR: gitRoot }, async () => {
  ...
  it("stops a managed gateway rooted at the package install when switching package installs to dev", async () => {
    const packageRoot = await createTrackedTempDir("zuvix-update-package-service-root-");
  ...
  it("does not stop or restart a managed gateway owned by another git checkout", async () => {
    const otherRoot = await createTrackedTempDir("zuvix-update-other-service-root-");
  ...
  it("keeps managed service stop output off stdout during json package updates", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-json-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "zuvix");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    ...
    serviceStop.mockImplementationOnce(async (params: { stdout?: NodeJS.WritableStream }) => {
      params.stdout?.write("Stopped systemd service: zuvix-gateway.service\n");
    });
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
  ...
  it("disarms legacy launchd updater jobs before stopping the gateway", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-launchd-loop-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "zuvix");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    launchdUpdateCleanupMocks.disableCurrentZuvixUpdateLaunchdJob.mockResolvedValue(true);
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
        return true;
    ...
    const cleanupOrder =
      launchdUpdateCleanupMocks.disableCurrentZuvixUpdateLaunchdJob.mock.invocationCallOrder[0];
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "zuvix");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "zuvix", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
    ...
    expect(postCoreSpawn?.[2].env?.ZUVIX_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.ZUVIX_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("zuvix-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "zuvix");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "zuvix", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "zuvix");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("zuvix-update");
    const localAppData = createCaseDir("zuvix-localappdata");
  ...
  it("warns when a package update targets a managed service root outside the shell root", async () => {
    const shellRoot = createCaseDir("zuvix-shell-root");
    const serviceRoot = await createTrackedTempDir("zuvix-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "zuvix", version: "2026.5.18" }),
      "utf-8",
    );
    ...
    expect(logs).toContain(
      `Shell Zuvix root differs from the managed gateway service root: ${shellRoot}`,
    );
    expect(logs).toContain("make sure `zuvix` on PATH resolves to the managed service root");
  ...
  it("runs managed service package follow-up commands with the service Node", async () => {
    const shellRoot = createCaseDir("zuvix-shell-root");
    const servicePrefix = await createTrackedTempDir("zuvix-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const serviceRoot = path.join(nodeModules, "zuvix");
  ...
  it("uses the managed service Node when package roots match but node binaries differ", async () => {
    const root = createCaseDir("zuvix-same-root");
  ...
  it("uses the managed service Node for follow-up commands when roots match but nodes differ", async () => {
    const servicePrefix = await createTrackedTempDir("zuvix-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "zuvix");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const serviceNpm = path.join(servicePrefix, "bin", "npm");
    const entrypoint = path.join(root, "dist", "index.js");
    await fs.mkdir(path.dirname(entrypoint), { recursive: true });
  ...
  it("pins package install to the service root when nodes differ and no owning npm exists at the prefix", async () => {
    const servicePrefix = await createTrackedTempDir("zuvix-no-npm-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "zuvix");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const entrypoint = path.join(root, "dist", "index.js");
  ...
  it("does not repair legacy config during a dry run", async () => {
    ...
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).not.toHaveBeenCalled();
    expect(launchdUpdateCleanupMocks.disableCurrentZuvixUpdateLaunchdJob).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("refreshes post-doctor config before post-update plugin sync", async () => {
    const tempDir = createCaseDir("zuvix-update");
    mockPackageInstallStatus(tempDir);
    const preUpdateConfig = { update: { channel: "stable" } } as ZuvixConfig;
    ...
    syncPluginsForUpdateChannel.mockImplementation(async ({ config }) => ({
      ...
      config: {
        ...
        plugins: {
          ...config.plugins,
          load: { paths: ["/tmp/zuvix-updated-plugin"] },
        },
      },
      summary: {
        switchedToBundled: [],
    ...
    const syncConfig = syncPluginCall()?.config as
      | (ZuvixConfig & { meta?: { lastTouchedVersion?: string } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          baseHash?: string;
          nextConfig?: ZuvixConfig & { meta?: { lastTouchedVersion?: string } };
        }
      | undefined;
  ...
  it("restores pre-update channels when post-core resume sees post-doctor config without them", async () => {
    const tempDir = createCaseDir("zuvix-update");
    const configPath = path.join(tempDir, "zuvix.json");
    const preUpdateConfig = {
      update: { channel: "stable" },
      channels: {
        whatsapp: {
    ...
    await withEnvAsync(
      {
        ZUVIX_UPDATE_POST_CORE: "1",
        ZUVIX_UPDATE_POST_CORE_CHANNEL: "stable",
        ZUVIX_UPDATE_POST_CORE_STARTED_AT_MS: String(updateStartedAtMs),
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
  ...
  it("persists authored channel values when post-core restore input is resolved", async () => {
    ...
    await withEnvAsync(
      {
        ZUVIX_UPDATE_POST_CORE: "1",
        ZUVIX_UPDATE_POST_CORE_CHANNEL: "stable",
        ZUVIX_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath,
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
    ...
    const syncConfig = syncPluginCall()?.config as
      | (ZuvixConfig & { channels?: { whatsapp?: { token?: string } } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          nextConfig?: ZuvixConfig & {
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("zuvix-update");
    const gitRoot = path.join(tempDir, "..", "zuvix");
    ...
    const syncCall = syncPluginCall() as
      | { channel?: string; config?: ZuvixConfig; workspaceDir?: string }
      | undefined;
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `zuvix update`.",
    );
    expect(serviceStop).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  ...
  it("tries the updated install restart when package service refresh fails", async () => {
    const updatedRoot = createCaseDir("zuvix-updated-root");
  ...
  it("accepts same-version refresh failure recovery when the managed service restarts", async () => {
    ...
    await fs.writeFile(
      updatedPackageJson,
      JSON.stringify({ name: "zuvix", version: "2026.4.24" }),
      "utf8",
    );
    setupUpdatedRootRefresh({
      entrypoints: [updatedEntrypoint],
  ...
  it("rejects same-version refresh failure recovery from a stale service definition", async () => {
    const oldRoot = createCaseDir("zuvix-old-root");
    const updatedRoot = createCaseDir("zuvix-updated-root");
  ...
  it("writes the control-plane update sentinel after managed package restart health passes", async () => {
    const stateDir = await createTrackedTempDir("zuvix-update-sentinel-state-");
    const metaDir = await createTrackedTempDir("zuvix-update-sentinel-meta-");
  ...
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            ZUVIX_CONFIG_PATH: "./config/zuvix.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedEnv: () => ({
        ZUVIX_STATE_DIR: path.resolve("./state"),
        ZUVIX_CONFIG_PATH: path.resolve("./config/zuvix.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
    },
    {
      ...
      expectedEnv: (context?: { originalCwd: string }) => ({
        ZUVIX_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ZUVIX_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ZUVIX_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("marks the whole update command as update-in-progress", async () => {
    await withEnvAsync({ ZUVIX_UPDATE_IN_PROGRESS: undefined }, async () => {
      ...
      vi.mocked(runGatewayUpdate).mockImplementationOnce(async () => {
        observedUpdateEnv = process.env.ZUVIX_UPDATE_IN_PROGRESS;
  ...
  it("updateFinalizeCommand runs doctor and plugin convergence with full update env", async () => {
    await withEnvAsync(
      {
        ZUVIX_UPDATE_IN_PROGRESS: undefined,
        ZUVIX_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR: undefined,
        ZUVIX_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: undefined,
      },
      async () => {
        ...
        expect(doctorEnv?.ZUVIX_UPDATE_IN_PROGRESS).toBe("1");
        expect(doctorEnv?.ZUVIX_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBe("1");
        expect(doctorEnv?.ZUVIX_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
        expect(process.env.ZUVIX_UPDATE_IN_PROGRESS).toBeUndefined();
        expect(process.env.ZUVIX_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBeUndefined();
        expect(process.env.ZUVIX_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBeUndefined();
  ...
  it("updateFinalizeCommand repairs doctor by default and refreshes plugin state after doctor", async () => {
    ...
    syncPluginsForUpdateChannel.mockImplementationOnce(
      async (params: { config?: ZuvixConfig }) => ({
        changed: true,
  ...
  it("updateFinalizeCommand reapplies requested channel against post-doctor config", async () => {
    const preDoctorConfig = { update: { channel: "stable" } } as ZuvixConfig;
    const postDoctorConfig = { update: { channel: "beta" } } as ZuvixConfig;
    const preDoctorSnapshot: ConfigFileSnapshot = {
      ...baseSnapshot,
      sourceConfig: preDoctorConfig,
      resolved: preDoctorConfig,
  ...
  it.each([
    ...
    },
    {
      ...
      requireTty: false,
      expectedError:
        "Update wizard requires a TTY. Use `zuvix update --channel <stable|beta|dev>` instead.",
    },
  ] as const)(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("zuvix-update-wizard");
    await withEnvAsync({ ZUVIX_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/zuvix as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        {
          HOME: undefined,
          ZUVIX_GIT_DIR: undefined,
          ZUVIX_HOME: undefined,
          USERPROFILE: undefined,
        },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "zuvix"));
        },
      );
    } finally {
  ...
  it("uses ZUVIX_HOME for the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        { ZUVIX_GIT_DIR: undefined, ZUVIX_HOME: "/srv/zuvix-home" },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/srv/zuvix-home", "zuvix"));
        },
      );
    } finally {
      homedirSpy.mockRestore();
  ...
  it("creates the parent directory before cloning the default dev checkout", async () => {
    const root = await createTrackedTempDir("zuvix-update-home-");
    const home = path.join(root, "custom-zuvix-home");
    const checkoutDir = path.join(home, "zuvix");

    await withEnvAsync({ ZUVIX_GIT_DIR: undefined, ZUVIX_HOME: home }, async () => {
    ...
    expect(cloneCall?.[0]).toEqual([
      "git",
      "clone",
      "https://github.com/zuvix/zuvix.git",
      checkoutDir,
    ]);
  });
});
