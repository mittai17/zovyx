// Runtime implementations for `zuvix plugins` subcommands. Heavy plugin modules stay
...
function formatBlockedRuntimePluginGuidance(params: {
  ...
  const alternative =
    pluginId === "acpx"
      ...
      : 'change the runtime policy to "zuvix"';
...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "zuvix doctor --fix" to install ${installSpec}, or install it manually with "zuvix plugins install ${installSpec}".`,
    ];
  });
}
...
export async function runPluginsEnableCommand(idInput: string): Promise<void> {
  ...
  const cfg = (snapshot.sourceConfig ?? snapshot.config) as ZuvixConfig;
  ...
  let next: ZuvixConfig = enableResult.config;
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(`${theme.muted("Repair:")} ${theme.command("zuvix plugins registry --refresh")}`);
  }
...
export async function runPluginsDoctorCommand(): Promise<void> {
  ...
  if (shadowed.length > 0) {
    ...
    for (const diag of shadowed) {
      ...
      lines.push("  repair:");
      lines.push("    zuvix plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
      lines.push("    edit or remove the config-selected plugin source");
      lines.push("    zuvix plugins registry --refresh");
      lines.push("    zuvix gateway restart --force");
    }
  }
  ...
  const docs = formatDocsLink("/plugin", "docs.zuvix.ai/plugin");
  lines.push("");
