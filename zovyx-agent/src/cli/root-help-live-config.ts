...
export function hasPluginHelpAffectingConfig(config: ZuvixConfig | null | undefined): boolean {
...
export function hasPluginHelpAffectingEnv(env: NodeJS.ProcessEnv): boolean {
  return Boolean(
    env.ZUVIX_BUNDLED_PLUGINS_DIR?.trim() || env.ZUVIX_DISABLE_BUNDLED_PLUGINS?.trim(),
  );
}
