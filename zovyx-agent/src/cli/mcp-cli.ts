...
import { serveZuvixChannelMcp } from "../mcp/channel-server.js";
      ...
      if (server.auth === "oauth") {
        ...
        if (!authStatus.hasTokens) {
          issues.push(
            issue(
              "warning",
              `OAuth credentials are not authorized; run ${formatCliCommand(`zuvix mcp login ${name}`)}`,
            ),
          );
        }
...
async function probeMcpServerIssue(params: {
  ...
  const runtime = createSessionMcpRuntime({
    sessionId: "zuvix-cli-mcp-doctor",
...
export function registerMcpCli(program: Command) {
  const mcp = program.command("mcp").description("Manage Zuvix MCP config and channel bridge");

  mcp
    .command("serve")
    .description("Expose Zuvix channels over MCP stdio")
    ...
    .action(async (opts) => {
      try {
        ...
        defaultRuntime.error(
          `MCP server failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("zuvix mcp list")} to inspect configured servers.`,
        );
        defaultRuntime.exit(1);
      }
    });
  ...
  mcp
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      if (names.length === 0) {
        defaultRuntime.log(
          `No MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand('zuvix mcp set <name> \'{"command":"uvx","args":["context7-mcp"]}\'')}.`,
        );
        return;
      }
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && !value) {
        fail(
          `No MCP server named "${name}" in ${loaded.path}. Run ${formatCliCommand("zuvix mcp list")} to see configured servers.`,
        );
      }
      if (opts.json) {
        printJson(value ?? {});
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && loaded.mcpServers[name]?.enabled === false) {
        fail(
          `MCP server "${name}" is disabled in ${loaded.path}. Run ${formatCliCommand(`zuvix mcp configure ${name} --enable`)} before probing it.`,
        );
      }
      const runtime = createSessionMcpRuntime({
        sessionId: "zuvix-cli-mcp-probe",
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { probe?: boolean; json?: boolean }) => {
      ...
      if (servers.length === 0) {
        defaultRuntime.log(
          `No MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand("zuvix mcp add <name> --command <command>")}.`,
        );
        return;
      }
  ...
  mcp
    ...
    .action(
      async (
        ...
        if (server.auth === "oauth") {
          defaultRuntime.log(
            `Run ${formatCliCommand(`zuvix mcp login ${name}`)} to authorize this MCP server.`,
          );
        }
      },
    );
  ...
  mcp
    ...
    .action(async (name: string, opts: { include?: string; exclude?: string; clear?: boolean }) => {
      ...
      if (!result.updated) {
        fail(
          `No MCP server named "${name}" in ${result.path}. Run ${formatCliCommand("zuvix mcp list")} to see configured servers.`,
        );
      }
    ...
    .action(async (name: string, opts: { code?: string }) => {
      ...
      const result = await runMcpOAuthLogin({
        ...
        onAuthorizationUrl: (url) => {
          ...
          defaultRuntime.log(
            `After approval, run ${formatCliCommand(`zuvix mcp login ${name} --code <code>`)}.`,
          );
        },
      });
