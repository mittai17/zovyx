...
describe("normalizePluginsConfig", () => {
  ...
  it("normalizes unknown plugin ids without consulting discovery", async () => {
    const discoverPlugins = vi.spyOn(discovery, "discoverZuvixPlugins");
  ...
  it("does not consult discovery or manifests for alias lookup", async () => {
    const discoverPlugins = vi.spyOn(discovery, "discoverZuvixPlugins").mockReturnValue({
      candidates: [
        {
          idHint: "anthropic",
          source: "/tmp/zuvix-bundled-anthropic/index.js",
          rootDir: "/tmp/zuvix-bundled-anthropic",
          origin: "bundled",
          bundledManifest: {
            id: "anthropic",
            configSchema: {},
        ...
        },
        {
          ...
          source: "/tmp/zuvix-global-anthropic/index.js",
          rootDir: "/tmp/zuvix-global-anthropic",
          origin: "global",
        },
      ],
      diagnostics: [],
    ...
    const loadManifest = vi.spyOn(manifest, "loadPluginManifest").mockReturnValue({
      ok: true,
      manifestPath: "/tmp/zuvix-global-anthropic/zuvix.plugin.json",
      manifest: {
