...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".zuvix", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "ZUVIX_HOME", "ZUVIX_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("zuvix-bundle-commands-home-");
      const workspaceDir = await createTempDir("zuvix-bundle-commands-workspace-");
