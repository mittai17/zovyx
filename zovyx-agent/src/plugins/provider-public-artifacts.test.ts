...
describe("provider public artifacts", () => {
  ...
  it("resolves multi-provider policy artifacts by manifest-owned provider id", async () => {
    const bundledPluginsDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-provider-policy-"));
        ...
        plugins: [
          {
            ...
            hooks: [],
            origin: "bundled",
            manifestPath: "/tmp/openai/zuvix.plugin.json",
            providers: ["openai"],
  ...
  it("does not cache manifest-owned provider policy aliases across bundled metadata changes", async () => {
    const bundledPluginsDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "zuvix-provider-policy-refresh-"),
    );
        ...
        plugins: [
          {
            ...
            hooks: [],
            origin: "bundled",
            manifestPath: "/tmp/owner/zuvix.plugin.json",
            providers: ["alias"],
            rootDir: "/tmp/owner",
            skills: [],
