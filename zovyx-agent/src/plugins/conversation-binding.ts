...
const APPROVALS_PATH = "~/.zuvix/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "zuvix-app-server:thread:",
  "zuvix-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("zuvix.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("zuvix.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Zuvix instead. If this started after an update, run "zuvix doctor --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
