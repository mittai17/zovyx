...
import { listZuvixPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
const ORIGINAL_ENV = {
  ...
  ZUVIX_DISABLE_BUNDLED_PLUGINS: process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS,
  ZUVIX_BUNDLED_PLUGINS_DIR: process.env.ZUVIX_BUNDLED_PLUGINS_DIR,
} as const;

const tempDirs: string[] = [];

function restoreZuvixStateDirEnv(): void {
  const value = ORIGINAL_ENV.ZUVIX_STATE_DIR;
  if (value === undefined) {
    delete process.env.ZUVIX_STATE_DIR;
  } else {
    process.env.ZUVIX_STATE_DIR = value;
  }
}

function restoreZuvixHomeEnv(): void {
  const value = ORIGINAL_ENV.ZUVIX_HOME;
  if (value === undefined) {
    delete process.env.ZUVIX_HOME;
  } else {
    process.env.ZUVIX_HOME = value;
  }
}

function restoreZuvixDisableBundledPluginsEnv(): void {
  const value = ORIGINAL_ENV.ZUVIX_DISABLE_BUNDLED_PLUGINS;
  if (value === undefined) {
    delete process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS = value;
  }
}

function restoreZuvixBundledPluginsDirEnv(): void {
  const value = ORIGINAL_ENV.ZUVIX_BUNDLED_PLUGINS_DIR;
  if (value === undefined) {
    delete process.env.ZUVIX_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR = value;
  }
}

function restoreEnv(): void {
  restoreZuvixStateDirEnv();
  restoreZuvixHomeEnv();
  restoreZuvixDisableBundledPluginsEnv();
  restoreZuvixBundledPluginsDirEnv();
}

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
  ...
  writePersistedInstalledPluginIndexSync(
    {
      ...
      plugins: [
        {
          pluginId: "normalizer",
          manifestPath: path.join(params.pluginDir, "zuvix.plugin.json"),
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/zuvix.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reuses current metadata when callers omit config", () => {
    const config: ZuvixConfig = { plugins: { allow: ["normalizer"] } };
    setCurrentPluginMetadataSnapshot(
      createCurrentSnapshot({
        manifestHash: "alpha",
        prefix: "alpha",
  ...
  it("keeps process metadata stable across manifest edits and reflects state-dir changes", () => {
    ...
    process.env.ZUVIX_STATE_DIR = stateDirA;
    process.env.ZUVIX_HOME = undefined;
    process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR = undefined;
    ...
    process.env.ZUVIX_STATE_DIR = stateDirB;
    clearPluginMetadataLifecycleCaches();
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listZuvixPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listZuvixPluginManifestMetadata(process.env)).toHaveLength(1);
