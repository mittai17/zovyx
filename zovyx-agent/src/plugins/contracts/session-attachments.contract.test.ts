...
import { FILE_TYPE_SNIFF_MAX_BYTES } from "@zuvix/media-core/mime";
import {
...
async function withSessionStore(
  ...
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredZuvixTmpDir(), "zuvix-session-attachments-"),
  );
...
describe("plugin session attachments", () => {
  afterEach(() => {
    ...
    delete (globalThis as { proofAttachmentApi?: ZuvixPluginApi }).proofAttachmentApi;
  ...
  it("wires sendSessionAttachment through the plugin API with stale-registry protection", async () => {
    await withSessionStore(async ({ storePath, filePath }) => {
      ...
      let capturedApi: ZuvixPluginApi | undefined;
      registerTestPlugin({
        registry,
        config,
