...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "zuvix/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "zuvix/plugin-sdk/test-env";
...
type TtsRuntimeModule = typeof import("zuvix/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("zuvix/plugin-sdk/speech-core");
...
let completeSimple: typeof import("zuvix/plugin-sdk/llm").completeSimple;
...
function asLegacyTtsConfig(value: unknown): ZuvixConfig {
  return value as ZuvixConfig;
}

function asLegacyZuvixConfig(value: Record<string, unknown>): ZuvixConfig {
  return value as unknown as ZuvixConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): ZuvixConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("zuvix/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: ZuvixConfig): ResolvedTtsConfig {
  const rawConfig =
...
async function setupSummarizationMocks() {
  ...
  ({ completeSimple } = await import("zuvix/plugin-sdk/llm"));
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: ZuvixConfig = asLegacyZuvixConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.ZUVIX_TTS_PREFS;
      process.env.ZUVIX_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.ZUVIX_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: ZuvixConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
