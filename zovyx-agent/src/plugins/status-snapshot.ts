...
/** Control-plane plugin status shape used by `zuvix plugins status` style surfaces. */
...
function isPluginLifecycleTraceEnabled(): boolean {
  const raw = process.env.ZUVIX_PLUGIN_LIFECYCLE_TRACE?.trim().toLowerCase();
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "zuvix";
