...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as ZuvixConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as ZuvixConfig;
    const load = vi.fn(async (config?: ZuvixConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: ZuvixConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
