...
const mocks = vi.hoisted(() => ({
  loadZuvixPlugins: vi.fn<typeof import("../loader.js").loadZuvixPlugins>(),
...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadZuvixPlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
vi.mock("../loader.js", () => ({
  loadZuvixPlugins: (...args: Parameters<typeof mocks.loadZuvixPlugins>) =>
    mocks.loadZuvixPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadZuvixPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadZuvixPlugins>) => mocks.loadZuvixPlugins(...args),
    );
  ...
  it("uses the shared runtime load context for configured-channel loads", () => {
    ...
    const env = { HOME: "/tmp/zuvix-home" } as NodeJS.ProcessEnv;
  ...
  it("does not reuse non-empty all-scope registries without loader compatibility", () => {
    ...
    mocks.loadZuvixPlugins.mockClear();
