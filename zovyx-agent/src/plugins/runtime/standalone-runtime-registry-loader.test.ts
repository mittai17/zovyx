...
vi.mock("../loader.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    loadZuvixPlugins: (...args: Parameters<typeof loaderMocks.loadZuvixPlugins>) =>
      loaderMocks.loadZuvixPlugins(...args),
  };
});
...
beforeEach(() => {
  loaderMocks.loadZuvixPlugins.mockReset();
});
...
describe("ensureStandaloneRuntimePluginRegistryLoaded", () => {
  it("reuses a compatible gateway startup registry for gateway-bindable dispatch load options", () => {
    ...
    expect(loaderMocks.loadZuvixPlugins).not.toHaveBeenCalled();
  });
  ...
  it("loads a fresh registry when dispatch config is not startup-compatible", () => {
    ...
    loaderMocks.loadZuvixPlugins.mockReturnValue(loadedRegistry);
    ...
    expect(loaderMocks.loadZuvixPlugins).toHaveBeenCalledOnce();
  });
});
