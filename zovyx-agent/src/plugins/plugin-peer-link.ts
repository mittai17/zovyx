...
export type ZuvixPeerLinkAuditIssue = {
  packageName: string;
  packageDir: string;
  reason: string;
};
...
type AuditManagedNpmRootResult = {
  checked: number;
  broken: number;
  issues: ZuvixPeerLinkAuditIssue[];
};

type ZuvixPeerLinkResult = "linked" | "skipped" | "unchanged";
...
async function auditZuvixPeerDependency(params: {
  hostRoot: string;
  packageDir: string;
  npmRoot?: string;
  packageName?: string;
}): Promise<ZuvixPeerLinkAuditIssue | null> {
  const packageName =
    params.packageName ??
    (params.npmRoot
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return {
        packageName,
        packageDir: params.packageDir,
        reason: `missing ${path.join(nodeModulesDir, "zuvix")}`,
      };
    }
    throw error;
  }

  const linkPath = path.join(nodeModulesDir, "zuvix");
...
export async function auditZuvixPeerDependencyLink(params: {
  packageDir: string;
  packageName?: string;
}): Promise<ZuvixPeerLinkAuditIssue | null> {
  ...
  if (!hostRoot) {
    return {
      packageName,
      packageDir: params.packageDir,
      reason: "could not locate zuvix package root",
    };
  }
  return await auditZuvixPeerDependency({
    hostRoot,
    packageDir: params.packageDir,
    packageName,
  });
...
async function ensureRealNodeModulesDir(params: {
  ...
  try {
    ...
    if (!existing.isDirectory() || existing.isSymbolicLink()) {
      params.logger.warn?.(
        `Skipping zuvix peerDependency link because ${nodeModulesDir} is not a real directory.`,
      );
      return null;
    }
...
async function linkZuvixPeerDependency(params: {
  hostRoot: string;
  installedDir: string;
  peerName: string;
  logger: PluginPeerLinkLogger;
}): Promise<ZuvixPeerLinkResult> {
  ...
  try {
    ...
    if (existing) {
      if (!existing.isSymbolicLink()) {
        if (params.peerName === "zuvix" && existing.isDirectory()) {
          const existingPackageName = await readPackageName(linkPath);
          if (existingPackageName === "zuvix") {
        ...
        params.logger.warn?.(
          `Skipping zuvix peerDependency link because ${linkPath} already exists and is not a symlink.`,
        );
        return "skipped";
      }
...
/**
 * Symlink the host zuvix package for plugins that declare it as a peer.
 ...
 */
export async function linkZuvixPeerDependencies(params: {
  installedDir: string;
  peerDependencies: Record<string, string>;
  logger: PluginPeerLinkLogger;
}): Promise<{ repaired: number; skipped: number }> {
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "zuvix");
  if (peers.length === 0) {
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate zuvix package root to symlink peerDependencies; plugin may fail to resolve zuvix at runtime.",
    );
  ...
  for (const peerName of peers) {
    const result = await linkZuvixPeerDependency({
      hostRoot,
      installedDir: params.installedDir,
      peerName,
      logger: params.logger,
...
export async function relinkZuvixPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  logger: PluginPeerLinkLogger;
...
export async function auditZuvixPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  ...
  let checked = 0;
  const issues: ZuvixPeerLinkAuditIssue[] = [];
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    const peerDependencies = await readPackagePeerDependencies(packageDir);
    if (!Object.hasOwn(peerDependencies, "zuvix")) {
      continue;
    ...
    checked += 1;
    const issue = await auditZuvixPeerDependency({
      hostRoot,
      npmRoot: params.npmRoot,
      packageDir,
    });
