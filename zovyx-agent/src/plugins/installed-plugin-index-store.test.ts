...
function makeTempDir() {
  return makeTrackedTempDir("zuvix-installed-plugin-index-store", tempDirs);
}
...
function createIndex(overrides: Partial<InstalledPluginIndex> = {}): InstalledPluginIndex {
  return {
    ...
    plugins: [
      {
        pluginId: "demo",
        manifestPath: "/plugins/demo/zuvix.plugin.json",
  ...
  it("preserves startup config paths across persisted index roundtrips", async () => {
    ...
    const index = createIndex({
      plugins: [
        {
          pluginId: "browser",
          manifestPath: "/plugins/browser/zuvix.plugin.json",
      ...
      plugins: [
        {
          ...
          manifestPath: "/plugins/provider-owner/zuvix.plugin.json",
