...
import { normalizeOptionalString as normalizeTrimmedString } from "@zuvix/normalization-core/string-coerce";
...
const PLUGIN_MANIFEST_FILENAME = "zuvix.plugin.json";
...
function resolveUserPath(value: string, env: NodeJS.ProcessEnv): string {
  if (value === "~" || value.startsWith("~/")) {
    const home = env.ZUVIX_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  const override = normalizeTrimmedString(env.ZUVIX_STATE_DIR);
  if (override) {
...
export function listZuvixPluginManifestMetadata(
