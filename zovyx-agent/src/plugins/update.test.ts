...
import { bundledPluginRootAt } from "zuvix/plugin-sdk/test-fixtures";
...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-zuvix",
    targetDir: params?.targetDir ?? "/tmp/opik-zuvix",
...
function createSuccessfulClawHubUpdateResult(params?: {
  ...
  return {
    ...
    targetDir: params?.targetDir ?? "/tmp/zuvix-plugins/legacy-chat",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "zuvix-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/zuvix-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createInstalledPackageDir(params: {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createZuvixPeerLinkFixtures(plugins: Array<{ pluginId: string; packageName: string }>) {
  const peerTarget = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-peer-target-"));
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { zuvix: ">=2026.5.4" },
      }),
    ]),
  );
  const peerLinkPath = (pluginId: string) =>
    path.join(installPaths[pluginId], "node_modules", "zuvix");
...
function mockNpmViewMetadata(params: {
  ...
  runCommandWithTimeoutMock.mockResolvedValueOnce({
    ...
    stdout: JSON.stringify({
      ...
      ...(params.zuvix ? { zuvix: params.zuvix } : {}),
    }),
    stderr: "",
  });
}
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@zuvix/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  const install = params.result.config.plugins?.installs?.["zuvix-codex-app-server"];
  expect(install?.source).toBe("npm");
  expect(install?.spec).toBe(params.spec);
  expect(install?.installPath).toBe("/tmp/zuvix-codex-app-server");
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-zuvix",
        spec: "@opik/opik-zuvix",
        integrity: "sha512-old",
        installPath: "/tmp/opik-zuvix",
      }),
      pluginIds: ["opik-zuvix"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-zuvix",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "zuvix-codex-app-server",
        spec: "zuvix-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/zuvix-codex-app-server",
      }),
      pluginIds: ["zuvix-codex-app-server"],
      specOverrides: {
        "zuvix-codex-app-server": "zuvix-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "zuvix-codex-app-server",
        targetDir: "/tmp/zuvix-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "zuvix-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        spec: "@zuvix/acpx",
        installPath,
        resolvedName: "@zuvix/acpx",
        resolvedSpec: "@zuvix/acpx@2026.5.2-beta.1",
    ...
    expect(npmInstallCall()?.spec).toBe("@zuvix/acpx");
    expect(npmInstallCall()?.expectedPluginId).toBe("acpx");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@zuvix/acpx@2026.5.2-beta.2");
  });
  ...
  it("pins unchanged official npm records during official sync", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@zuvix/acpx",
        resolvedSpec: "@zuvix/acpx@2026.5.2",
    ...
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@zuvix/acpx@2026.5.2");
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/acpx",
          version: "2026.5.1",
          resolvedSpec: "@zuvix/acpx@2026.5.1",
        },
      }),
    );
  ...
  it("keeps integrity drift checks for exact prerelease-only official pins", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/voice-call",
          version: "0.0.2-beta.1",
          resolvedSpec: "@zuvix/voice-call@0.0.2-beta.1",
        },
      }),
    );
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "voice-call",
        spec: "@zuvix/voice-call@0.0.2-beta.1",
        installPath,
        resolvedName: "@zuvix/voice-call",
        resolvedSpec: "@zuvix/voice-call@0.0.2-beta.1",
  ...
  it("keeps integrity drift checks for exact official pins during beta fallback", async () => {
    ...
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error: "No matching version found for @zuvix/acpx@beta",
    ...
    expect(npmInstallCall(0)?.spec).toBe("@zuvix/acpx@beta");
    expect(npmInstallCall(0)?.expectedIntegrity).toBeUndefined();
    expect(npmInstallCall(1)?.spec).toBe("@zuvix/acpx");
  ...
  it("does not skip trusted official default updates when latest resolves to the installed prerelease", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "acpx",
        spec: "@zuvix/acpx@2026.5.2-beta.2",
        installPath,
        integrity: "sha512-beta",
        shasum: "beta",
        resolvedName: "@zuvix/acpx",
  ...
  it("updates trusted official npm plugins when latest resolves to a stable correction release", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/acpx",
          version: "2026.5.3-1",
          resolvedSpec: "@zuvix/acpx@2026.5.3-1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@zuvix/acpx",
        resolvedSpec: "@zuvix/acpx@2026.5.3",
  ...
  it("does not skip unchanged npm plugins when package metadata requires a newer plugin API", async () => {
    vi.stubEnv("ZUVIX_COMPATIBILITY_HOST_VERSION", "2026.5.28-beta.3");
    const installPath = createInstalledPackageDir({
      name: "@zuvix/msteams",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/msteams",
          version: "2026.5.28-beta.3",
          resolvedSpec: "@zuvix/msteams@2026.5.28-beta.3",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        resolvedSpec: "@zuvix/msteams@2026.5.28-beta.4",
        integrity: "sha512-newer",
        shasum: "newer",
      }),
      pluginIds: ["msteams"],
    ...
    expect(npmInstallCall()?.spec).toBe("@zuvix/msteams");
  ...
  it("repairs missing zuvix peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@zuvix/codex",
      version: "2026.5.3",
      peerDependencies: { zuvix: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@zuvix/codex",
      version: "2026.5.3",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/codex",
          version: "2026.5.3",
          resolvedSpec: "@zuvix/codex@2026.5.3",
        },
      }),
    );
    const config: ZuvixConfig = {
    ...
    expect(npmInstallCall()?.spec).toBe("@zuvix/codex");
    expect(npmInstallCall()?.mode).toBe("update");
  ...
  it("skips unchanged npm plugins when the zuvix peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@zuvix/codex",
      version: "2026.5.3",
      peerDependencies: { zuvix: ">=2026.5.3" },
    });
    fs.mkdirSync(path.join(installPath, "node_modules", "zuvix"), { recursive: true });
    mockNpmViewMetadata({
      name: "@zuvix/codex",
      version: "2026.5.3",
      integrity: "sha512-same",
  ...
  it("repairs zuvix peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@zuvix/brave-plugin" },
      { pluginId: "codex", packageName: "@zuvix/codex" },
      { pluginId: "discord", packageName: "@zuvix/discord" },
    ];
    const { installPaths, peerLinkPath, linkPeer } = createZuvixPeerLinkFixtures(plugins);
  ...
  it("repairs sibling zuvix peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@zuvix/brave-plugin" },
      { pluginId: "codex", packageName: "@zuvix/codex" },
      { pluginId: "discord", packageName: "@zuvix/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@zuvix/codex",
            version: "2026.5.4",
            resolvedSpec: "@zuvix/codex@2026.5.4",
          },
        }),
      );
    });
  ...
  it("continues repairing sibling zuvix peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@zuvix/brave-plugin" },
      { pluginId: "codex", packageName: "@zuvix/codex" },
    ];
      ...
      config: {
        plugins: {
          installs: {
            broken: {
              source: "npm",
              spec: "@zuvix/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@zuvix/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@zuvix/broken-plugin@2026.5.4",
            },
            ...Object.fromEntries(
              plugins.map(({ pluginId, packageName }) => [
                pluginId,
    ...
    expect(warnMessages).toEqual([
      `Could not repair zuvix peer link for "broken" at ${brokenInstallPath}: Skipping zuvix peerDependency link because ${path.join(brokenInstallPath, "node_modules")} is not a real directory.`,
    ]);
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".zuvix", "extensions", "lossless-claw");
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "lossless-claw",
        spec: "@martian-engineering/lossless-claw",
        installPath: "~/.zuvix/extensions/lossless-claw",
        resolvedName: "@martian-engineering/lossless-claw",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
        integrity: "sha512-same",
  ...
  it("preserves exact official npm pins when official install sync is not requested", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/codex",
          version: "2026.5.28",
          resolvedSpec: "@zuvix/codex@2026.5.28",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@zuvix/codex@2026.5.28");
    expect(npmInstallCall()?.expectedPluginId).toBe("codex");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.changed).toBe(false);
    expectRecordFields(result.outcomes[0], {
  ...
  it("reinstalls missing exact official npm pins without official install sync", async () => {
    const extensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-missing-plugin-"));
  ...
  it("updates disabled trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@zuvix/diagnostics-otel",
      }),
    );
    ...
    const config = createClawHubInstallConfig({
      ...
      clawhubFamily: "code-plugin",
      clawhubChannel: "official",
      spec: "clawhub:@zuvix/diagnostics-otel@2026.5.3",
    });
    const result = await updateNpmInstalledPlugins({
      config: {
        ...config,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@zuvix/diagnostics-otel");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-otel"], {
      source: "clawhub",
      spec: "clawhub:@zuvix/diagnostics-otel",
      version: "2026.5.4",
      clawhubPackage: "@zuvix/diagnostics-otel",
      clawhubChannel: "official",
    });
  ...
  it("updates bare trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@zuvix/diagnostics-prometheus",
      }),
    );
        ...
        plugins: {
          installs: {
            "diagnostics-prometheus": {
              source: "clawhub",
              spec: "clawhub:@zuvix/diagnostics-prometheus@2026.5.3",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@zuvix/diagnostics-prometheus");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-prometheus"], {
      source: "clawhub",
      spec: "clawhub:@zuvix/diagnostics-prometheus",
      version: "2026.5.4",
      clawhubPackage: "@zuvix/diagnostics-prometheus",
      clawhubChannel: "official",
    });
  ...
  it("clears stale plugin policy and slot references when disabling failed updates", async () => {
    ...
    const message =
      'Disabled "demo" after plugin update failure; Zuvix will continue without it. Failed to update demo: security scan blocked install';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.changed).toBe(true);
    expect(result.config.plugins?.entries?.demo).toEqual({
      enabled: false,
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-zuvix@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-zuvix@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-zuvix" (@opik/opik-zuvix@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-zuvix: aborted: npm package integrity drift detected for @opik/opik-zuvix@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ok: false,
        code: "npm_package_not_found",
        error: "Package not found on npm: @zuvix/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@zuvix/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @zuvix/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "zuvix-codex-app-server@beta",
        resolvedName: "zuvix-codex-app-server",
        resolvedSpec: "zuvix-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "zuvix-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "zuvix-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "zuvix-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "zuvix-codex-app-server",
      }),
      specOverrides: {
        "zuvix-codex-app-server": "zuvix-codex-app-server@beta",
      },
      expectedSpec: "zuvix-codex-app-server@beta",
      expectedRecordSpec: "zuvix-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "zuvix-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "zuvix-codex-app-server",
      });
  ...
  it("preserves explicit official npm tag overrides during manual updates", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/acpx",
          version: "2026.5.3-beta.1",
          resolvedSpec: "@zuvix/acpx@2026.5.3-beta.1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      ...
      pluginIds: ["acpx"],
      specOverrides: {
        acpx: "@zuvix/acpx@beta",
      },
    });

    expectNpmUpdateCall({
      spec: "@zuvix/acpx@beta",
      expectedPluginId: "acpx",
    });
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for zuvix-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "zuvix-codex-app-server",
          ...
          npmResolution: {
            name: "zuvix-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "zuvix-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    expect(npmInstallCall(0)?.spec).toBe("zuvix-codex-app-server@beta");
    expect(npmInstallCall(1)?.spec).toBe("zuvix-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "zuvix-codex-app-server" has no beta npm release for zuvix-codex-app-server@beta; using zuvix-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "zuvix-codex-app-server",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated zuvix-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback used zuvix-codex-app-server because zuvix-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback).toEqual({
      requestedSpec: "zuvix-codex-app-server@beta",
      usedSpec: "zuvix-codex-app-server",
      requestedLabel: "@beta",
      usedLabel: "@latest",
      reason: "unavailable",
      message:
        "plugin channel fallback: zuvix-codex-app-server used @latest because @beta was unavailable",
    });
  });
  ...
  it("reports npm beta fallback as tentative during dry-run checks", async () => {
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Would update zuvix-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback would use zuvix-codex-app-server because zuvix-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback?.message).toBe(
      "plugin channel fallback: zuvix-codex-app-server would use @latest because @beta was unavailable",
    );
  });
  ...
  it("falls back to the default npm spec when the beta package exists but is invalid", async () => {
    ...
    expect(npmInstallCall(1)?.spec).toBe("zuvix-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "zuvix-codex-app-server" failed beta npm update for zuvix-codex-app-server@beta; using zuvix-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "zuvix-codex-app-server",
    ...
    expect(result.outcomes[0]?.channelFallback).toMatchObject({
      ...
      usedLabel: "@latest",
      reason: "failed",
      message: "plugin channel fallback: zuvix-codex-app-server used @latest after @beta failed",
    });
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update zuvix-codex-app-server: npm package not found for zuvix-codex-app-server.",
        channelFallback: {
          requestedSpec: "zuvix-codex-app-server@beta",
          usedSpec: "zuvix-codex-app-server",
          requestedLabel: "@beta",
          ...
          reason: "failed",
          message:
            "plugin channel fallback: zuvix-codex-app-server used @latest after @beta failed",
        },
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to check zuvix-codex-app-server: npm package not found for zuvix-codex-app-server.",
        channelFallback: {
          requestedSpec: "zuvix-codex-app-server@beta",
          usedSpec: "zuvix-codex-app-server",
          requestedLabel: "@beta",
          ...
          reason: "failed",
          message:
            "plugin channel fallback: zuvix-codex-app-server would use @latest after @beta failed",
        },
      },
    ]);
  });
  ...
  it("preserves explicit npm tags when updating on the beta channel", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createCodexAppServerInstallConfig({
        spec: "zuvix-codex-app-server@rc",
      }),
      pluginIds: ["zuvix-codex-app-server"],
      updateChannel: "beta",
      dryRun: true,
  ...
  it("falls back to npm for trusted official ClawHub artifact blocks", async () => {
    ...
    installPluginFromClawHubMock.mockResolvedValueOnce({
      ...
      error:
        'ClawHub artifact download for "@zuvix/discord@2026.5.16-beta.5" is not available yet (ClawHub /api/v1/packages/%40zuvix%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded.). Use "npm:@zuvix/discord@2026.5.16-beta.5" for launch installs while ClawHub artifact routing is being rolled out.',
    });
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/discord",
          version: "2026.5.16-beta.5",
          resolvedSpec: "@zuvix/discord@2026.5.16-beta.5",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createClawHubInstallConfig({
        ...
        installPath,
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@zuvix/discord",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        spec: "clawhub:@zuvix/discord",
      }),
      pluginIds: ["discord"],
      updateChannel: "beta",
      disableOnFailure: true,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@zuvix/discord@beta");
    expect(npmInstallCall()?.spec).toBe("@zuvix/discord@beta");
    ...
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@zuvix/discord@2026.5.16-beta.5",
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          "Updated discord: 2026.5.12 -> 2026.5.16-beta.5. (warning: official ClawHub artifact fallback used @zuvix/discord@beta).",
      },
    ]);
    expect(warnMessages).toEqual([
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@zuvix/discord@beta; using npm @zuvix/discord@beta instead. Core update can still complete.',
    ]);
  });
  ...
  it("uses the default npm spec when beta ClawHub falls back before an artifact block", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@zuvix/discord",
          version: "2026.5.16",
          resolvedSpec: "@zuvix/discord@2026.5.16",
        },
      }),
    );
    ...
    expect(clawHubInstallCall(0)?.spec).toBe("clawhub:@zuvix/discord@beta");
    expect(clawHubInstallCall(1)?.spec).toBe("clawhub:@zuvix/discord");
    expect(npmInstallCall()?.spec).toBe("@zuvix/discord");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@zuvix/discord@2026.5.16",
      installPath: "/tmp/zuvix-plugins/discord",
      version: "2026.5.16",
    });
    expect(result.outcomes[0]?.message).toBe(
      "Updated discord: 2026.5.12 -> 2026.5.16. (warning: official ClawHub artifact fallback used @zuvix/discord).",
    );
    expect(warnMessages).toEqual([
      'Plugin "discord" has no beta ClawHub release for clawhub:@zuvix/discord@beta; using clawhub:@zuvix/discord instead. Core update can still complete.',
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@zuvix/discord; using npm @zuvix/discord instead. Core update can still complete.',
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        nextVersion: "2026.5.16-beta.5",
        message:
          "discord is up to date (2026.5.16-beta.5). (warning: official ClawHub artifact fallback would use @zuvix/discord@beta).",
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update discord: artifact unavailable (ClawHub clawhub:@zuvix/discord@beta).",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@zuvix/voice-call",
      targetDir: "/tmp/zuvix-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@zuvix/voice-call");
    expect(npmInstallCall()?.expectedPluginId).toBe("voice-call");
    expect(result.config.plugins?.allow).toEqual(["@zuvix/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@zuvix/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@zuvix/voice-call");
    expect(result.config.plugins?.entries?.["@zuvix/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["@zuvix/voice-call"], {
      source: "npm",
      spec: "@zuvix/voice-call",
      installPath: "/tmp/zuvix-voice-call",
      version: "0.0.2",
    });
  ...
  it("keeps authored plugin config shape when only the install key migrates", async () => {
    ...
    expect(result.config.plugins).toEqual({
      installs: {
        "@zuvix/voice-call": expect.objectContaining({
          source: "npm",
          spec: "@zuvix/voice-call",
          installPath: "/tmp/zuvix-voice-call",
        }),
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@zuvix/context-engine",
      targetDir: "/tmp/zuvix-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@zuvix/context-engine");
    expectRecordFields(result.config.plugins?.installs?.["@zuvix/context-engine"], {
      source: "npm",
      spec: "@zuvix/context-engine",
      installPath: "/tmp/zuvix-context-engine",
      version: "0.0.2",
    });
  ...
  it("forwards dangerous force unsafe install to plugin update installers", async () => {
    ...
    expect(npmInstallCall()?.spec).toBe("zuvix-codex-app-server@beta");
    expect(npmInstallCall()?.dangerouslyForceUnsafeInstall).toBe(true);
    expect(npmInstallCall()?.expectedPluginId).toBe("zuvix-codex-app-server");
  });

  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/zuvix/extensions/demo";
    const extensionsDir = "/var/zuvix/extensions";
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "demo",
        targetDir: installPath,
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    const env = { ZUVIX_HOME: "/srv/zuvix-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/zuvix-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/zuvix-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@zuvix/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@zuvix/legacy-chat@2.0.0",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@zuvix/legacy-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      spec: "@zuvix/legacy-chat",
      installPath: "/tmp/zuvix-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@zuvix/legacy-chat",
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/zuvix-plugins/voice-call",
        version: "0.0.2-beta.1",
      }),
    );
  ...
  it("falls back from ClawHub to npm only when the ClawHub package is absent", async () => {
    ...
    expect(result.summary.warnings).toEqual([
      "ClawHub clawhub:legacy-chat@2026.5.1-beta.2 unavailable for legacy-chat; falling back to npm @zuvix/legacy-chat.",
    ]);
    expect(result.summary.errors).toStrictEqual([]);
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      ...
      externalizedBundledPluginBridges: [
        {
          bundledPluginId: "voice-call",
          preferredSource: "clawhub",
          clawhubSpec: "clawhub:@zuvix/voice-call",
          npmSpec: "@zuvix/voice-call",
          channelIds: ["voice-call"],
        },
      ],
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/zuvix-plugins/default-chat",
        version: "2.0.0",
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@zuvix/default-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["default-chat"], {
      source: "npm",
      spec: "@zuvix/default-chat",
      installPath: "/tmp/zuvix-plugins/default-chat",
      version: "2.0.0",
    });
  });
      ...
      config: {
        ...
        plugins: {
          ...
          installs: {
            "legacy-chat": {
              source: "npm",
              spec: "@zuvix/legacy-chat@1.2.3",
              resolvedSpec: "@zuvix/legacy-chat@1.2.3",
              installPath: "/tmp/zuvix-plugins/legacy-chat",
            },
          },
        },
