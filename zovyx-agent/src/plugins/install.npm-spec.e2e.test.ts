...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `zuvix-${label}-`));
  tempDirs.push(dir);
  return dir;
}

function configWithInstalledPackageTreeBlockPolicy(): ZuvixConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
...
async function packPlugin(params: {
  ...
  await fs.writeFile(
    path.join(packageDir, "zuvix.plugin.json"),
    `${JSON.stringify(
      {
        id: params.pluginId,
...
describe("installPluginFromNpmSpec e2e", () => {
  it("installs the newest compatible stable package when npm latest requires a newer plugin API", async () => {
    ...
    const versions = [
      ...
      await packPlugin({
        ...
        version: "2026.5.27",
        rootDir,
        zuvix: incompatibleZuvix,
      }),
    ];
    const registry = await startStaticRegistry([{ packageName, latest: "2026.5.27", versions }]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    const previousHostVersion = process.env.ZUVIX_COMPATIBILITY_HOST_VERSION;
    process.env.ZUVIX_COMPATIBILITY_HOST_VERSION = "2026.5.10-beta.1";
    ...
    try {
      ...
      if (previousHostVersion === undefined) {
        ...
        process.env.ZUVIX_COMPATIBILITY_HOST_VERSION = previousHostVersion;
      }
    }
  });

  it("scrubs root zuvix materialized by required npm peers", async () => {
    ...
    const registry = await startStaticRegistry([
      ...
      { packageName: "zuvix", latest: "2026.0.0", versions: zuvixVersions },
    ]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    ...
    const rawZuvixLockEntry = rawLock.packages?.["node_modules/zuvix"] as
      | { peer?: unknown; version?: unknown }
      | undefined;
    expect(rawZuvixLockEntry?.peer).toBe(true);
    expect(rawZuvixLockEntry?.version).toBe("2026.0.0");
    ...
    expect(lock.packages?.["node_modules/zuvix"]).toBeUndefined();
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", "zuvix")),
    ).rejects.toHaveProperty("code", "ENOENT");
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "zuvix"))
  ...
  it("plans peers from installed optional dependencies", async () => {
    ...
    const rootManifest = JSON.parse(
      ...
      dependencies?: Record<string, string>;
      zuvix?: { managedPeerDependencies?: string[] };
    };
    expect(rootManifest.dependencies?.[runtimePeer]).toBe("1.0.0");
    expect(rootManifest.zuvix?.managedPeerDependencies ?? []).toContain(runtimePeer);
  });
  ...
  it("leaves legacy flat-root peer dependencies alone during isolated later installs", async () => {
    ...
    expect(rootManifest.zuvix?.managedPeerDependencies ?? []).not.toContain(runtimePeer);
  });
  ...
  it("falls back to the legacy npm peer mode inside the plugin project when npm cannot plan third-party peers", async () => {
    ...
    expect(rootManifest.zuvix?.managedPeerDependencies ?? []).not.toContain(missingPeer);
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", blockedPlugin, "package.json")),
    ).resolves.toBeTruthy();
  });
  ...
  it("does not take ownership of an existing root dependency observed as a peer", async () => {
    ...
    expect(rootManifest.dependencies?.[blockedPlugin]).toBeUndefined();
    expect(rootManifest.dependencies?.[runtimePeer]).toBeUndefined();
    expect(rootManifest.zuvix?.managedPeerDependencies ?? []).not.toContain(
  ...
  it("scrubs host peers inside each isolated npm project", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { zuvix: ">=2026.5.5-beta.2" },
            peerDependenciesMeta: { zuvix: { optional: true } },
            pluginId: codexName,
            version: "1.0.0",
            rootDir,
          }),
      ...
      },
      {
        ...
        versions: [
          await packPlugin({
            packageName: opikName,
            peerDependencies: { zuvix: ">=2026.3.2" },
            peerDependenciesMeta: {},
            pluginId: opikName,
            version: "1.0.0",
            rootDir,
    ...
    await expect(
      fs
        .lstat(path.join(first.targetDir, "node_modules", "zuvix"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(second.targetDir, "node_modules", "zuvix"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("keeps an earlier isolated zuvix peer link after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "zuvix");
    ...
    expect(manifest.dependencies?.zuvix).toBeUndefined();
    const lock = JSON.parse(
