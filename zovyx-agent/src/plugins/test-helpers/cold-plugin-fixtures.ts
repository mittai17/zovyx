...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/zuvix-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "zuvix.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): ZuvixConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    ZUVIX_HOME: path.join(homeDir, "home"),
    ZUVIX_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
    ZUVIX_DISABLE_PERSISTED_PLUGIN_REGISTRY:
