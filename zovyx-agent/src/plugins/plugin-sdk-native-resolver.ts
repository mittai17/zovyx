/** Installs native Node resolution aliases so plugins can import the Zuvix SDK in dev and tests. */
...
export type InstallZuvixPluginSdkNativeResolverOptions = {
  modulePath?: string;
...
const PLUGIN_SDK_PACKAGE_PREFIXES = ["zuvix/plugin-sdk", "@zuvix/plugin-sdk"] as const;
...
function resolveLoaderModulePath(options: InstallZuvixPluginSdkNativeResolverOptions): string {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        if (
          packageJson.name === "zuvix" ||
          (typeof packageJson.bin === "object" &&
            packageJson.bin !== null &&
            typeof (packageJson.bin as { zuvix?: unknown }).zuvix === "string")
        ) {
          return cursor;
        }
      } catch {
...
function resolveAllowedParentRoots(
  options: InstallZuvixPluginSdkNativeResolverOptions,
): string[] {
...
export function installZuvixPluginSdkNativeResolver(
  options: InstallZuvixPluginSdkNativeResolverOptions = {},
): string[] {
...
export function installZuvixInternalCorePackageNativeResolver(
  options: Pick<InstallZuvixPluginSdkNativeResolverOptions, "moduleUrl"> = {},
): string[] {
  for (const alias of listInternalCorePackageNativeAliases(options)) {
    registerNativeAlias(alias);
  }
...
export function resetZuvixPluginSdkNativeResolverForTest(): void {
