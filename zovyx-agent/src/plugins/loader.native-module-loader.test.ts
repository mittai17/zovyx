...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-plugin-loader-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("createPluginModuleLoader", () => {
  it("loads bundled JavaScript without creating a module loader", async () => {
    ...
    const { loadZuvixPlugins } = await importFreshModule<typeof import("./loader.js")>(
      import.meta.url,
      "./loader.js?scope=native-module-loader",
    );

    const pluginRoot = writeBundledPluginFixture("demo");
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR = pluginRoot;
  ...
  it("loads packaged JavaScript without creating a module loader", async () => {
    ...
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR = makeTempDir();
