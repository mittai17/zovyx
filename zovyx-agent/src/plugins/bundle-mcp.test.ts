...
function createEnabledBundleConfig(pluginIds: string[]): ZuvixConfig {
  return {
    plugins: {
...
describe("loadEnabledBundleMcpConfig", () => {
  ...
  it("uses a provided manifest registry instead of rediscovering bundle plugins", async () => {
    const homeDir = await tempHarness.createTempDir("zuvix-bundle-mcp-home-");
    const workspaceDir = await tempHarness.createTempDir("zuvix-bundle-mcp-workspace-");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    await withBundleHomeEnv(
      tempHarness,
      "zuvix-bundle-inline-placeholder",
      async ({ homeDir, workspaceDir }) => {
