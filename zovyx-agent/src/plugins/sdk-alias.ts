...
import { resolveZuvixDevSourceRoot } from "./dev-source-root.js";
...
function hasTrustedZuvixRootIndicator(params: {
  packageRoot: string;
  ...
  const hasZuvixBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("zuvix")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.zuvix === "string");
  const hasZuvixEntrypoint = fs.existsSync(path.join(params.packageRoot, "zuvix.mjs"));
  return hasCliEntryExport || hasZuvixBin || hasZuvixEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedZuvixRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedZuvixRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedZuvixRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveDevSourceRootParam(params: { devSourceRoot?: string | null }): string | null {
  return params.devSourceRoot !== undefined
    ...
    : resolveZuvixDevSourceRoot(process.env);
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveZuvixPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedZuvixRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["zuvix/plugin-sdk", "@zuvix/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@zuvix/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  {
    packageName: "@zuvix/gateway-client",
  ...
  {
    packageName: "@zuvix/gateway-protocol",
  ...
  {
    packageName: "@zuvix/markdown-core",
  ...
  {
    packageName: "@zuvix/media-generation-core",
  ...
  {
    packageName: "@zuvix/normalization-core",
  ...
  {
    packageName: "@zuvix/terminal-core",
  ...
  {
    packageName: "@zuvix/model-catalog-core",
...
export function listWorkspacePackageExportAliasEntries(params: {
  ...
  const fallbackPackageRoot = resolveZuvixPackageRootSync({ cwd: process.cwd() });
  const packageJson =
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@zuvix/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.ZUVIX_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `zuvix/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "zuvix/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
...
export function isBundledPluginExtensionPath(params: {
  modulePath: string;
  zuvixPackageRoot: string;
  bundledPluginsDir?: string;
}): boolean {
  const normalizedModulePath = path.resolve(params.modulePath);
  const roots = [
    params.bundledPluginsDir ? path.resolve(params.bundledPluginsDir) : null,
    path.join(params.zuvixPackageRoot, "extensions"),
    path.join(params.zuvixPackageRoot, "dist", "extensions"),
    path.join(params.zuvixPackageRoot, "dist-runtime", "extensions"),
