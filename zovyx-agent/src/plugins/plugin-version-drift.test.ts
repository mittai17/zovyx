...
function npmRecord(
  ...
  const resolvedName = overrides.resolvedName ?? "@zuvix/whatsapp";
  return {
    source: "npm",
...
describe("detectPluginVersionDrift", () => {
  it("returns empty drifts when all externalized plugins match the gateway", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4", { resolvedName: "@zuvix/discord" }),
      },
    });
  ...
  it("treats a build-qualifier suffix on either side as matching (2026.5.4-1 ≈ 2026.5.4)", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4-1", { resolvedName: "@zuvix/discord" }),
      },
    });
  ...
  it("ignores official catalog installs pinned to independent package versions", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        "zuvix-plugin-yuanbao": npmRecord("2.13.1", {
          resolvedName: "zuvix-plugin-yuanbao",
          spec: "zuvix-plugin-yuanbao@2.13.1",
        }),
      },
    });
  ...
  it("ignores exact catalog pins even when the pin matches the gateway version", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        "wecom-zuvix-plugin": npmRecord("2026.5.6", {
          resolvedName: "@wecom/wecom-zuvix-plugin",
          spec: "@wecom/wecom-zuvix-plugin@2026.5.6",
        }),
      },
    });
  ...
  it("skips plugins with no recorded version (cannot detect drift)", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        whatsapp: { source: "npm", spec: "@zuvix/whatsapp@latest" },
      },
    });
  ...
  it("skips plugins that are explicitly disabled in config", () => {
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@zuvix/discord" }),
      },
      config,
    });
  ...
  it("includes plugins with no entry in config (default-enabled)", () => {
    const config: ZuvixConfig = { plugins: { entries: {} } } as ZuvixConfig;
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
  ...
  it("returns drifts sorted by pluginId for deterministic output", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@zuvix/discord" }),
        matrix: npmRecord("2026.5.3", { resolvedName: "@zuvix/matrix" }),
      },
    });
