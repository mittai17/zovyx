...
/** Env var that points bundled-plugin lookup at an Zuvix source checkout. */
export const ZUVIX_DEV_SOURCE_ROOT_ENV = "ZUVIX_DEV_SOURCE_ROOT";
...
/** Resolves and validates the configured Zuvix development source root. */
export function resolveZuvixDevSourceRoot(env: NodeJS.ProcessEnv = process.env): string | null {
  const rawRoot = env[ZUVIX_DEV_SOURCE_ROOT_ENV]?.trim();
  if (!rawRoot) {
    return null;
  }
  ...
  if (readPackageName(path.join(realRoot, "package.json")) !== "zuvix") {
    return null;
  }
...
export function isBundledPluginInsideDevSourceRoot(params: {
  ...
  const devSourceRoot = resolveZuvixDevSourceRoot(params.env);
  if (!devSourceRoot) {
    return false;
  }
