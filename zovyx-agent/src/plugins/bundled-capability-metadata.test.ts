  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        zuvix?: ZuvixPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.zuvix?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "zuvix.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
