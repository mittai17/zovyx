/** Loads and normalizes Zuvix plugin manifests, including contracts and config schemas. */
import fs from "node:fs";
import path from "node:path";
import { normalizeModelCatalog } from "@zuvix/model-catalog-core/model-catalog-normalize";
...
export const PLUGIN_MANIFEST_FILENAME = "zuvix.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `zuvix qa`, for example `matrix`. */
  commandName: string;
...
// package.json "zuvix" metadata (used for setup/catalog)
...
export type ZuvixPackageStartup = {
  /**
...
export type ZuvixPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type ZuvixPackageCompat = {
  pluginApi?: string;
};

export type ZuvixPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: ZuvixPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
  channel?: PluginPackageChannel;
  compat?: ZuvixPackageCompat;
  install?: PluginPackageInstall;
  startup?: ZuvixPackageStartup;
};
...
} & Partial<Record<ManifestKey, ZuvixPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): ZuvixPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
...
export function resolvePackageExtensionEntries(
  ...
  const rawZuvix = manifest?.[MANIFEST_KEY] as unknown;
  if (rawZuvix === undefined || rawZuvix === null) {
    return { status: "missing", entries: [] };
  }
  if (!isRecord(rawZuvix)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json zuvix must be an object",
    };
  }
  const raw = rawZuvix.extensions;
  ...
  if (!Array.isArray(raw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json zuvix.extensions must be an array",
    };
  }
  ...
  for (const [index, entry] of raw.entries()) {
    ...
    if (!normalized) {
      return {
        status: "invalid",
        entries: [],
        error: `package.json zuvix.extensions[${index}] must be a non-empty string`,
      };
    }
