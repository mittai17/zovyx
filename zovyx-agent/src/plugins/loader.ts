...
import { installZuvixPluginSdkNativeResolver } from "./plugin-sdk-native-resolver.js";
...
function createGuardedPluginRegistrationApi(api: ZuvixPluginApi): {
  api: ZuvixPluginApi;
  close: () => void;
} {
  let closed = false;
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<ZuvixPluginDefinition["register"]>>[0],
): void {
...
function resolvePluginLoadCacheContext(options: PluginLoadOptions = {}) {
  ...
  const devSourceRoot = resolveZuvixDevSourceRoot(env);
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadZuvixPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadZuvixPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ZuvixPluginDefinition;
  register?: ZuvixPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as ZuvixPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as ZuvixPluginDefinition;
...
export function loadZuvixPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into Zuvix yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: ZuvixPluginModule | null = null;
      let moduleLoadMs = 0;
      ...
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => loadPluginModule(safeSource) as ZuvixPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (registrationPlan.loadSetupEntry && manifestRecord.setupSource) {
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: ZuvixPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => loadPluginModule(safeRuntimeSource) as ZuvixPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'zuvix plugins inspect <id> --runtime --json' for runtime diagnostics, 'zuvix plugins list' for registry state, and restart the Gateway after plugin code or load-path changes.`,
        );
      }
    }
...
export async function loadZuvixPluginCliRegistry(
  options: PluginLoadOptions = {},
  ...
  for (const candidate of orderedCandidates) {
    ...
    let mod: ZuvixPluginModule | null;
    try {
      mod = withProfile(
        { pluginId: record.id, source: safeSource },
        "cli-metadata",
