...
import { sortUniqueStrings } from "@zuvix/normalization-core/string-normalization";
...
type LoadZuvixPlugins = typeof import("./loader.js").loadZuvixPlugins;
...
const loadZuvixPluginsMock = vi.fn<LoadZuvixPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/zuvix.plugin.json`,
  };
}
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadZuvixPluginsMock, "Zuvix plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadZuvixPluginsMock, "Zuvix plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: ZuvixConfig = {
    plugins: {},
  };
  const autoEnabledConfig: ZuvixConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadZuvixPlugins: (...args: Parameters<LoadZuvixPlugins>) =>
        loadZuvixPluginsMock(...args),
