...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "zuvix-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): ZuvixConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: ZuvixConfig, pluginId: string): Record<string, unknown> {
