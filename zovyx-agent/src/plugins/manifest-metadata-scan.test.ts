...
function createTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-manifest-metadata-"));
  tempRoots.push(root);
  return root;
}
...
describe("listZuvixPluginManifestMetadata", () => {
  afterEach(() => {
  ...
  it("prefers the active bundled manifest over stale persisted bundled installs", () => {
    ...
    writeJson(path.join(bundledRoot, "openai", "zuvix.plugin.json"), {
      id: "openai",
      providerEndpoints: [{ endpointClass: "openai-public", hosts: ["api.openai.com"] }],
    });
    writeJson(path.join(staleBundledRoot, "openai", "zuvix.plugin.json"), {
      id: "openai",
      providers: ["openai"],
    });
    writePersistedInstalledPluginIndexSync(
      {
        ...
        plugins: [
          {
            pluginId: "openai",
            manifestPath: path.join(staleBundledRoot, "openai", "zuvix.plugin.json"),
    ...
    const records = listZuvixPluginManifestMetadata({
      ZUVIX_HOME: home,
