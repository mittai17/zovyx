...
function collectConfiguredPluginEntryIds(config: ZuvixConfig): string[] {
...
function resolveRegister(mod: ZuvixPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: ZuvixPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as ZuvixPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: ZuvixPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Zuvix ignored the plugin setup runtime entry. Remove setup-api/zuvix.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
