...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("zuvix-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("zuvix-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("zuvix-marketplace-test-", async (homeDir) => {
      const zuvixHome = path.join(homeDir, "zuvix-home");
      await fs.mkdir(path.join(homeDir, ".claude", "plugins"), { recursive: true });
      await fs.mkdir(zuvixHome, { recursive: true });
      await fs.writeFile(
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, ZUVIX_HOME: zuvixHome },
