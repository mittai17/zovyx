...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Zuvix consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Zuvix should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Zuvix's bundle
   ...
   *
   * Use this when the backend should consume a canonical Zuvix auth profile
   ...
   * `profile-only` treats the selected Zuvix auth profile as the sole auth
   ...
   * as static config, such as mapping Zuvix thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside Zuvix's tool
