...
const nixEnv: NodeJS.ProcessEnv = { ZUVIX_NIX_MODE: "1" };
...
describe("plugin hardlink policy", () => {
  ...
  it("does not treat ZUVIX_NIX_MODE as enough by itself", () => {
    expect(
      shouldRejectHardlinkedPluginFiles({
        origin: "config",
        rootDir: "/tmp/plugin",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      expect(isNixStorePluginRoot("/nix/store/abc-zuvix-plugin")).toBe(true);
      expect(isNixStorePluginRoot("/tmp/nix/store/abc-zuvix-plugin")).toBe(false);
      expect(
        shouldRejectHardlinkedPluginFiles({
          origin: "config",
          rootDir: "/nix/store/abc-zuvix-plugin",
          env: nixEnv,
        }),
      ).toBe(false);
    },
