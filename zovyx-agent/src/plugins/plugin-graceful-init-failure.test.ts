...
const fixtureRoot = makeTrackedTempDir("zuvix-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadZuvixPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "zuvix.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("zuvix plugins inspect <id> --runtime --json");
    expect(summary).toContain("zuvix plugins list");
  });
});
