...
function isPackageRootZuvixPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "zuvix") ||
    (segments.length === 3 &&
...
function isManagedNpmRootPackagePeerSymlink(segments: string[]): boolean {
  ...
  return isPackageRootZuvixPeerSymlink(segments.slice(packageEndIndex));
}

function isTrustedZuvixPeerSymlink(params: {
  allowManagedNpmRootPackagePeerSymlinks?: boolean;
  relativePath: string;
}): boolean {
  const segments = params.relativePath.split(/[\\/]+/);
  return (
    isPackageRootZuvixPeerSymlink(segments) ||
    (params.allowManagedNpmRootPackagePeerSymlinks === true &&
      isManagedNpmRootPackagePeerSymlink(segments))
  );
}

async function resolveTrustedHostZuvixRootRealPath(): Promise<string | null> {
...
function isTrustedHostZuvixPath(params: {
  resolvedTargetPath: string;
  trustedHostZuvixRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostZuvixRootRealPath !== null &&
    isPathInside(params.trustedHostZuvixRootRealPath, params.resolvedTargetPath)
  );
}
...
> {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    if (
      ...
      isTrustedHostZuvixPath({
        resolvedTargetPath,
        trustedHostZuvixRootRealPath: params.trustedHostZuvixRootRealPath,
      })
    ) {
      return {};
    }
...
function collectManifestRuntimeDependencyNames(manifest: PackageManifest): string[] {
  ...
  for (const dependencyName of Object.keys(manifest.peerDependencies ?? {})) {
    if (dependencyName !== "zuvix" && isInstallScannableDependencyName(dependencyName)) {
      dependencyNames.add(dependencyName);
    }
  }
...
async function collectPackageManifestPaths(params: {
  ...
  const trustedHostZuvixRootRealPath = await resolveTrustedHostZuvixRootRealPath();
...
function resolvePolicySource(params: {
  ...
  if (params.requestKind === "skill-install") {
    switch (params.origin?.type) {
      case "clawhub":
        return { kind: "clawhub", authority: "zuvix", mutable: false, network: true };
      case "git":
        return {
          kind: "git",
      ...
      case "zuvix-bundled":
        return { kind: "bundled", authority: "zuvix", mutable: false, network: false };
      case "zuvix-managed":
      case "zuvix-extra":
        return { kind: "managed", authority: "zuvix", mutable: false, network: false };
      default:
