...
import {
  ...
  readZuvixManagedNpmRootOverrides,
  repairManagedNpmRootZuvixPeer,
...
import { installedPackageNeedsZuvixPeerLinkRepair } from "../infra/package-update-utils.js";
import {
...
function formatUnresolvedZuvixPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares zuvix as a peer dependency, but Zuvix could not create a plugin-local node_modules/zuvix link. Run from a packaged Zuvix install or reinstall Zuvix, then retry.`;
}
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing zuvix.extensions; update the plugin package to include zuvix.extensions (for example ["./dist/index.js"]). See https://docs.zuvix.ai/help/troubleshooting#plugin-install-fails-with-missing-zuvix-extensions';
...
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_zuvix-pack-archives";
const MANAGED_NPM_PROJECT_QUARANTINE_DIR = "_zuvix-quarantined-npm-projects";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_ZUVIX_EXTENSIONS: "missing_zuvix_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_ZUVIX_EXTENSIONS: "empty_zuvix_extensions",
  INVALID_ZUVIX_EXTENSIONS: "invalid_zuvix_extensions",
...
function validateZuvixPackageCompatibility(params: {
  pluginId: string;
  currentHostVersion: string;
  packageMetadata?: ZuvixPackageManifest;
  ...
  if (!pluginApiRangeCheck.ok) {
    return {
      ok: false,
      error: `invalid package.json zuvix.compat.pluginApi: ${pluginApiRangeCheck.error}`,
  ...
  if (pluginApiRange && !satisfiesPluginApiRange(params.currentHostVersion, pluginApiRange)) {
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires plugin API ${pluginApiRange}, but this Zuvix runtime exposes ${params.currentHostVersion}. Upgrade Zuvix or install a compatible plugin version and retry.`,
...
function validateZuvixPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json zuvix.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${params.pluginId}" requires Zuvix >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set ZUVIX_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires Zuvix >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Zuvix and retry.`,
      code: PLUGIN_INSTALL_ERROR_CODE.INCOMPATIBLE_HOST_VERSION,
    };
  }

  return validateZuvixPackageCompatibility({
...
function ensureZuvixExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_ZUVIX_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json zuvix.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_ZUVIX_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_ZUVIX_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareZuvixReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@zuvix/")) {
    return null;
  }
  ...
  if (!stableVersion) {
    ...
    if (prereleaseVersion && semverVersions.every(isPrereleaseSemverVersion)) {
      if (prereleaseVersion !== params.resolvedPrereleaseVersion) {
        ...
        params.logger.warn?.(
          `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; using newest prerelease ${prereleaseSpec} because this trusted official Zuvix package has no stable npm versions yet.`,
        );
        return { kind: "prerelease-only", resolution: metadataResult.metadata };
      }
      params.logger.warn?.(
        `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; allowing it because this trusted official Zuvix package has no stable npm versions yet.`,
      );
  ...
  params.logger.warn?.(
    `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; falling back to stable ${stableSpec} for this trusted official Zuvix install.`,
  );
...
function validateNpmResolutionCompatibility(params: {
  ...
  return validateZuvixPackageInstallCompatibility({
    runtime: params.runtime,
    pluginId: params.expectedPluginId ?? params.resolution.name ?? params.parsedSpec.name,
    packageMetadata: params.resolution.packageZuvix as ZuvixPackageManifest | undefined,
  });
}
...
async function resolveLatestCompatibleNpmResolution(params: {
  ...
  for (const version of candidates) {
    ...
    if (!compatibilityError) {
      params.logger.warn?.(
        `Resolved ${params.parsedSpec.raw} to ${params.currentResolution.resolvedSpec ?? currentVersion}, but that version is incompatible with this Zuvix runtime; using newest compatible ${metadataResult.metadata.resolvedSpec ?? spec}.`,
      );
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  if (params.snapshot) {
    try {
      ...
      await relinkZuvixPeerDependenciesInManagedNpmRoot({
        npmRoot: params.npmRoot,
  ...
  if (params.packageName !== "zuvix") {
    try {
      await repairManagedNpmRootZuvixPeer({
        npmRoot: params.npmRoot,
        timeoutMs: params.timeoutMs,
        logger: params.logger,
      });
      ...
      params.logger.warn?.(
        `Failed to repair managed npm zuvix peer after rollback: ${String(error)}`,
      );
    }
  }
  try {
...
async function createManagedNpmPluginInstallRollbackSnapshot(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-npm-plugin-rollback-"));
...
async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  ...
  for (const entry of entries.toSorted((left, right) => left.name.localeCompare(right.name))) {
    if (entry.name === ".bin" || entry.name === "zuvix") {
      continue;
    }
...
async function installPluginFromManagedNpmRoot(
  ...
  const runManagedNpmInstall = async (
    ...
    if (params.packageName !== "zuvix") {
      const repairedZuvixPeer = await repairManagedNpmRootZuvixPeer({
        npmRoot,
        timeoutMs,
        logger,
      });
      if (repairedZuvixPeer) {
        logger.info?.(`Repaired stale zuvix peer dependency in ${npmRoot}`);
      }
    }
    let preInstallRootPackageNames = await listManagedNpmRootPackageNames(npmRoot);
    const managedOverrides = await readZuvixManagedNpmRootOverrides();
    ...
    if (install.code !== 0 && isManagedNpmProjectCorruptionInstallFailure(install)) {
      ...
      try {
        ...
        return await rollbackFailedManagedNpmInstall({
          ok: false,
          error: `npm install failed with a managed npm project corruption signature, but Zuvix could not quarantine ${npmRoot} for rebuild: ${String(error)}. Original npm error: ${originalError}`,
        });
      }
      logger.warn?.(
    ...
    if (params.packageName !== "zuvix") {
      ...
      if (repairedZuvixPeer) {
        logger.info?.(`Repaired stale zuvix peer dependency in ${npmRoot} after npm install`);
      }
    }
    try {
      await relinkZuvixPeerDependenciesInManagedNpmRoot({
      ...
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: `Failed to repair zuvix peer links after npm install: ${String(error)}`,
      });
    }
    if (installedPackageNeedsZuvixPeerLinkRepair(installRoot)) {
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: formatUnresolvedZuvixPeerLinkError(params.packageName),
      });
    }
...
> {
  ...
  try {
    ...
    backupTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-npm-pack-archive-"));
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "zuvix security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function installBundleFromSourceDir(
  ...
  const packageMetadata = packageManifestResult.manifest
    ...
    : undefined;
  const compatibilityError = validateZuvixPackageInstallCompatibility({
    runtime,
    pluginId,
    packageMetadata,
  });
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureZuvixExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid zuvix.plugin.json: ${ocManifestResult.error}`,
  ...
  const extensionsResult = ensureZuvixExtensions({
    manifest,
  });
...
async function scanAndLinkInstalledPackage(params: {
  ...
  const peerLinkRepair = await linkZuvixPeerDependencies({
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedZuvixPeerLinkError(params.pluginId),
    };
  }
  return null;
}
...
export async function installPluginFromNpmSpec(
  ...
  const policyTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-npm-policy-"));
  try {
...
export async function installPluginFromPath(
  ...
  return {
    ...
    error:
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains zuvix.plugin.json, or list standalone plugin files in plugins.load.paths.",
  };
}
