...
import {
  ...
  installedPackageNeedsZuvixPeerLinkRepair,
...
import { linkZuvixPeerDependencies } from "./plugin-peer-link.js";
...
function compareNpmSemverForUpdate(left: string, right: string): number {
  const releaseCmp = compareZuvixReleaseVersions(left, right);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
> {
  ...
  if (
    !parsedSpec ||
    !parsedSpec.name.startsWith("@zuvix/") ||
...
function isNpmMetadataCompatibleWithCurrentHost(metadata: NpmSpecResolution): boolean {
  ...
  const installMetadata = metadata.packageZuvix?.install;
  ...
  const pluginApiRangeCheck = resolvePackagePluginApiRange(metadata.packageZuvix);
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareZuvixReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function shouldFallbackClawHubBridgeToNpm(params: {
  ...
  if (!isZuvixOrgNpmSpec(params.npmSpec)) {
    return false;
  }
  return (
...
function migratePluginConfigId(cfg: ZuvixConfig, fromId: string, toId: string): ZuvixConfig {
...
function withoutPluginInstallRecord(cfg: ZuvixConfig, pluginId: string): ZuvixConfig {
...
function resetDisabledPluginSlots(
  slots: NonNullable<ZuvixConfig["plugins"]>["slots"] | undefined,
  pluginId: string,
): NonNullable<ZuvixConfig["plugins"]>["slots"] | undefined {
  if (!slots) {
    return slots;
  }
  let next = slots;
...
function disablePluginConfigEntry(config: ZuvixConfig, pluginId: string): ZuvixConfig {
...
async function repairZuvixPeerLinksForNpmInstalls(params: {
  config: ZuvixConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair zuvix peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsZuvixPeerLinkRepair(installPath)) {
      continue;
    }

    const peerDependencies = readInstalledPackagePeerDependencies(installPath);
    if (!Object.hasOwn(peerDependencies, "zuvix")) {
      continue;
    }
    ...
    try {
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair zuvix peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      repaired = !installedPackageNeedsZuvixPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair zuvix peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  const recordFailure = (
    ...
    if (params.disableOnFailure && !params.dryRun) {
      const disabledMessage =
        `Disabled "${pluginId}" after plugin update failure; Zuvix will continue without it. ` +
        message;
    ...
    if (
      ...
      if (metadataResult.ok) {
        ...
        if (
          ...
          !installedPackageNeedsZuvixPeerLinkRepair(installPath) &&
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairZuvixPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
