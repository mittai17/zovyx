...
import type {
  ZuvixPluginHookOptions,
  ZuvixPluginToolFactory,
  ZuvixPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type ZuvixPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<ZuvixConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Zuvix uses this to expose
 ...
 *
 * Runs after Zuvix resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Zuvix auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Zuvix wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Zuvix builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Zuvix no
...
/**
 * Provider-owned stream wrapper hook after Zuvix applies its generic
 ...
 *
 * Return `true` or `false` to override Zuvix's built-in provider cache TTL
 ...
 *
 * Runs only after Zuvix exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Zuvix should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Zuvix loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Zuvix falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Zuvix calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Zuvix performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Zuvix registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Zuvix wrappers.
   *
   ...
   *
   * Called after Zuvix resolves the raw configured credential but before the
   ...
   * hint than Zuvix's generic auth-store guidance.
   */
   ...
   * Zuvix's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Zuvix should surface a provider-specific hint for
   ...
   * merging. Zuvix deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Zuvix stores the
   ...
   * without replacing the full Zuvix-owned system prompt.
   */
   ...
   *
   * Runs after Zuvix's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Zuvix's default fallback policy.
   */
   ...
   * rewrites after Zuvix has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Zuvix handles
   ...
   *
   * Zuvix uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `zuvix doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `zuvix doctor` should offer.
   *
   ...
   *
   * Zuvix calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep Zuvix's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Zuvix falls back to that stored profile.
   */
...
export type ZuvixPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral Zuvix session id when available. */
  sessionId?: string;
  /** Transcript file for this Zuvix session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Zuvix session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current Zuvix configuration */
  config: ZuvixConfig;
...
export type ZuvixPluginCommandDefinition = {
...
export type ZuvixPluginHttpRouteAuth = "gateway" | "plugin";
export type ZuvixPluginHttpRouteMatch = "exact" | "prefix";
export type ZuvixPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type ZuvixPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type ZuvixPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type ZuvixPluginHttpRouteParams = {
  path: string;
  handler: ZuvixPluginHttpRouteHandler;
  handleUpgrade?: ZuvixPluginHttpRouteUpgradeHandler;
  auth: ZuvixPluginHttpRouteAuth;
  match?: ZuvixPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: ZuvixPluginGatewayRuntimeScopeSurface;
  nodeCapability?: {
    surface: string;
    ttlMs?: number;
  };
...
export type ZuvixPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type ZuvixPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `zuvix` program. For nested
...
export type ZuvixPluginCliRegistrar = (ctx: ZuvixPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Zuvix to keep a plugin command lazy-loaded while still
 ...
 */
export type ZuvixPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type ZuvixPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `zuvix nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `zuvix nodes`, so a descriptor named
   * `"camera"` exposes `zuvix nodes camera`.
   */
  descriptors?: ZuvixPluginCliCommandDescriptor[];
};

export type ZuvixPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type ZuvixPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type ZuvixPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type ZuvixPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type ZuvixPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: ZuvixPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type ZuvixPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: ZuvixPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<ZuvixPluginNodeInvokeTransportResult>;
};

export type ZuvixPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type ZuvixPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: ZuvixPluginNodeInvokePolicyContext,
  ) => Promise<ZuvixPluginNodeInvokePolicyResult> | ZuvixPluginNodeInvokePolicyResult;
};

export type ZuvixPluginSecurityAuditContext = {
  config: ZuvixConfig;
...
export type ZuvixPluginSecurityAuditCollector = (
  ctx: ZuvixPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type ZuvixGatewayDiscoveryAdvertiseContext = {
...
export type ZuvixGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: ZuvixGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type ZuvixPluginServiceContext = {
  config: ZuvixConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type ZuvixPluginService = {
  id: string;
  start: (ctx: ZuvixPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: ZuvixPluginServiceContext) => void | Promise<void>;
};

export type ZuvixPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type ZuvixPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  /**
   * @deprecated Declare exclusive plugin kind in `zuvix.plugin.json` via
  ...
  configSchema?: ZuvixPluginConfigSchema;
  reload?: ZuvixPluginReloadRegistration;
  nodeHostCommands?: ZuvixPluginNodeHostCommand[];
  securityAuditCollectors?: ZuvixPluginSecurityAuditCollector[];
  register?: (api: ZuvixPluginApi) => void;
  activate?: (api: ZuvixPluginApi) => void;
};

export type ZuvixPluginModule = ZuvixPluginDefinition | ((api: ZuvixPluginApi) => void);
...
export type PluginConfigMigration = (config: ZuvixConfig) =>
  | {
      config: ZuvixConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `zuvix migrate`. */
...
export type ZuvixPluginSessionStateApi = {
...
export type ZuvixPluginSessionWorkflowApi = {
...
export type ZuvixPluginSessionControlsApi = {
...
export type ZuvixPluginSessionApi = {
  state: ZuvixPluginSessionStateApi;
  workflow: ZuvixPluginSessionWorkflowApi;
  controls: ZuvixPluginSessionControlsApi;
};

export type ZuvixPluginAgentEventsApi = {
...
export type ZuvixPluginAgentApi = {
  events: ZuvixPluginAgentEventsApi;
};

export type ZuvixPluginRunContextApi = {
...
export type ZuvixPluginLifecycleApi = {
...
export type ZuvixPluginApi = {
   ...
   */
  session: ZuvixPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: ZuvixPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: ZuvixPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: ZuvixPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | ZuvixPluginToolFactory,
    opts?: ZuvixPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: ZuvixPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: ZuvixPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: ZuvixPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: ZuvixPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: ZuvixPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, Zuvix
  ...
  /**
   * Register a plugin-owned node feature command group under `zuvix nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: ZuvixPluginCliRegistrar,
    opts?: ZuvixPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: ZuvixPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: ZuvixPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: ZuvixPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: ZuvixPluginSecurityAuditCollector) => void;
  registerService: (service: ZuvixPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: ZuvixGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `zuvix migrate` (migration capability). */
   ...
   */
  registerCommand: (command: ZuvixPluginCommandDefinition) => void;
