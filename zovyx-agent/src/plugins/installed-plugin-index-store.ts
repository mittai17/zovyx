...
import type { ZuvixStateDatabaseOptions } from "../state/zuvix-state-db.js";
import {
  ...
  if (options.stateDir) {
    return {
      env: {
        ...
        ZUVIX_STATE_DIR: options.stateDir,
      },
    };
  }
...
function assertWritableInstalledPluginIndexStoreOptions(
  ...
  if (isExplicitLegacyJsonStorePath(options)) {
    throw new Error(
      "Explicit JSON installed plugin index paths are retired. Use the shared SQLite state DB or run zuvix doctor --fix to migrate legacy plugins/installs.json.",
    );
  }
}
...
function readPersistedInstalledPluginIndexFromSqlite(
  ...
  try {
    const database = openZuvixStateDatabase(resolveStateDatabaseOptions(options));
