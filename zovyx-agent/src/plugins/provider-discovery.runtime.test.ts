...
function createManifestPlugin(id: string): PluginManifestRecord {
  return {
    ...
    rootDir: `/tmp/${id}`,
    source: "bundled",
    manifestPath: `/tmp/${id}/zuvix.plugin.json`,
...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("clears bundled dist discovery chunks from the dist root", () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      manifestRegistry: {
        plugins: [
          {
            ...createManifestPlugin("deepseek"),
            rootDir: "/tmp/zuvix/dist/extensions/deepseek",
            manifestPath: "/tmp/zuvix/dist/extensions/deepseek/zuvix.plugin.json",
            providerDiscoverySource: "/tmp/zuvix/dist/extensions/deepseek/provider-discovery.js",
          },
        ],
        diagnostics: [],
      },
    ...
    expect(mocks.clearNativeRequireJavaScriptModuleCache).toHaveBeenCalledWith(
      "/tmp/zuvix/dist/extensions/deepseek/provider-discovery.js",
      { dependencyRoot: "/tmp/zuvix/dist" },
    );
  });
