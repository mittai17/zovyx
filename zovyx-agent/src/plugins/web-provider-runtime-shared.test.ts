...
vi.mock("./loader.js", () => ({
  ...
  loadZuvixPlugins: mocks.loadZuvixPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadZuvixPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadZuvixPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadZuvixPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to a scoped provider load when the active runtime registry has no web providers", () => {
    ...
    mocks.loadZuvixPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadZuvixPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadZuvixPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
