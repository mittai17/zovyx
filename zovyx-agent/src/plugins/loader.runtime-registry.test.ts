...
function makeZuvixDevSourceRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-loader-dev-source-"));
...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadZuvixPlugins(loadOptions);

      loadZuvixPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadZuvixPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadZuvixPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadZuvixPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    const registry = loadZuvixPlugins(loadOptions);

    clearPluginRegistryLoadCache();

    expect(loadZuvixPlugins(loadOptions)).not.toBe(registry);
  });
});
