...
export type PluginCliCommandGroupEntry = {
  ...
  placeholders: readonly ZuvixPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: ZuvixPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadZuvixPluginCliRegistry(
...
): Promise<ZuvixPluginCliCommandDescriptor[]> {
  try {
