...
import { ensureZuvixPluginSdkAlias } from "./plugin-sdk-dist-alias.js";
...
function updatePluginManifest(plugin: Pick<TempPlugin, "dir">, patch: Record<string, unknown>) {
  const manifestPath = path.join(plugin.dir, "zuvix.plugin.json");
...
function writeBundledPlugin(params: {
  ...
  delete process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS;
  process.env.ZUVIX_BUNDLED_PLUGINS_DIR = bundledDir;
  return { bundledDir, plugin };
}

function makeZuvixDevSourceRoot() {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "zuvix" }), "utf-8");
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".zuvix", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ ZUVIX_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadZuvixPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          zuvix: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.ZUVIX_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadZuvixPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadZuvixPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadZuvixPlugins>[0], "cache" | "config">,
) {
  return loadZuvixPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadZuvixPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadZuvixPlugins>;
  loadSecond: () => ReturnType<typeof loadZuvixPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadZuvixPlugins>;
  loadVariant: () => ReturnType<typeof loadZuvixPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const zuvixHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(zuvixHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    ZUVIX_HOME: zuvixHome,
    HOME: ignoredHome,
    ZUVIX_STATE_DIR: stateDir,
    ZUVIX_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadZuvixPlugins({
    cache: false,
    config: {
      plugins: {
...
describe("loadZuvixPlugins", () => {
  ...
  it("can load scoped plugins from a supplied manifest registry without rereading manifests", () => {
    ...
    fs.rmSync(path.join(plugin.dir, "zuvix.plugin.json"));
  ...
  it("loads installed plugin packages discovered from persisted install records", () => {
    ...
    const registry = withEnv({ ZUVIX_STATE_DIR: stateDir }, () =>
      loadZuvixPlugins({
        cache: false,
        config: {
          plugins: {
  ...
  it("refreshes bundled plugin-sdk aliases without deleting the shared alias directory", () => {
    ...
    const aliasDir = path.join(distRoot, "extensions", "node_modules", "zuvix", "plugin-sdk");
    ...
    ensureZuvixPluginSdkAlias(distRoot);
  ...
  it("keeps private local-only plugin-sdk artifacts out of package dist aliases", () => {
    ...
    const pluginSdkDir = path.join(distRoot, "plugin-sdk");
    const aliasRoot = path.join(distRoot, "extensions", "node_modules", "zuvix");
  ...
  it("keeps private local-only plugin-sdk artifacts out of legacy package dist aliases", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "zuvix", version: "2026.4.22", type: "module" }, null, 2),
      "utf8",
    );
    fs.writeFileSync(path.join(pluginSdkDir, "index.js"), "export const root = true;\n", "utf8");
    fs.writeFileSync(
  ...
  it("loads bundled plugins with plugin-sdk imports from a package dist root", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "zuvix", version: "2026.4.22", type: "module" }),
      "utf-8",
    );
    fs.writeFileSync(
    ...
    ensureZuvixPluginSdkAlias(path.join(packageRoot, "dist"));
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(pluginRoot, "zuvix.plugin.json"),
      JSON.stringify(
        {
          id: "discord",
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadZuvixPlugins>) => {
...
module.exports = { id: "manifest-surfaces-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
        ...
        fs.writeFileSync(
          path.join(plugin.dir, "zuvix.plugin.json"),
          JSON.stringify(
            {
              id: "manifest-surfaces-plugin",
              configSchema: EMPTY_PLUGIN_SCHEMA,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "zuvix.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__zuvix_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__zuvix_loader_reentry_error";
        const reenterFnMarker = "__zuvix_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadZuvixPlugins>[0]) => loadZuvixPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadZuvixPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadZuvixPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__zuvix_runtime_registry_reentry_marker";
        const resolverMarker = "__zuvix_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadZuvixPlugins(options);
        const scoped = loadZuvixPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadZuvixPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("skips discovery and manifest registry loading entirely when onlyPluginIds is an explicit empty array", async () => {
    ...
    const discoverySpy = vi.spyOn(discovery, "discoverZuvixPlugins");
  ...
  it("can scope bundled provider loads without hanging", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@zuvix/scoped-provider",
        zuvix: { extensions: ["./index.cjs"] },
      }),
      "utf-8",
    );
    const plugin = writePlugin({
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadZuvixPlugins(options);
    ...
    const second = loadZuvixPlugins(options);
  ...
  it("uses discovery registration mode for non-activating loads", () => {
    useNoBundledPlugins();
    const marker = "__zuvixDiscoveryModeTest";
    const plugin = writePlugin({
    ...
    const snapshot = loadZuvixPlugins({
      activate: false,
      cache: false,
      workspaceDir: plugin.dir,
      config,
  ...
  it("caches non-activating snapshots without restoring global side effects", () => {
    useNoBundledPlugins();
    clearPluginCommands();
    const marker = "__zuvixSnapshotCacheRegisterCount";
    const plugin = writePlugin({
      id: "snapshot-cache",
  ...
  it("does not re-register non-bundled plugins after gateway-bindable boot loads", () => {
    useNoBundledPlugins();
    const marker = "__zuvixGatewayBootRegisterCount";
    const plugin = writePlugin({
  ...
  it("reuses a gateway-bindable cache entry for later default-mode loads", () => {
    useNoBundledPlugins();
    const marker = "__zuvixGatewayBindableCacheRegisterCount";
    const plugin = writePlugin({
    ...
    const gatewayBindable = loadZuvixPlugins({
      ...options,
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
      },
    });
    const defaultMode = loadZuvixPlugins(options);
  ...
  it("preserves the gateway-bindable hook runner across later default-mode activating loads", () => {
    ...
    const gatewayRegistry = loadZuvixPlugins({
    ...
    const defaultRegistry = loadZuvixPlugins({
        ...
        return {
          ...
          loadFirst: () =>
            loadZuvixPlugins({
              ...
              env: {
                ...process.env,
                ZUVIX_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadZuvixPlugins({
              ...
              env: {
                ...process.env,
                ZUVIX_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadZuvixPlugins({
              ...
              env: {
                ...
                ZUVIX_STATE_DIR: stateDir,
                ZUVIX_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadZuvixPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(zuvixHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadZuvixPlugins(options),
          loadVariant: () =>
            loadZuvixPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadZuvixPlugins({
      env: {
        ...
        HOME: homeDir,
        ZUVIX_HOME: undefined,
        ZUVIX_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers ZUVIX_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const zuvixHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "zuvix-home-demo",
      dir: path.join(zuvixHome, "plugins", "zuvix-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "zuvix-home-demo", register() {} };`,
    });
    ...
    const registry = loadZuvixPlugins({
      ...
      config: {
        plugins: {
          allow: ["zuvix-home-demo"],
          entries: {
            "zuvix-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/zuvix-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "zuvix-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("can include plugin export shape when register is missing", () => {
    ...
    const registry = withEnv({ ZUVIX_PLUGIN_LOAD_DEBUG: "1" }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadZuvixPlugins>) =>
  ...
  api.registerGatewayDiscoveryService({ id: "shared-discovery", advertise() {} });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadZuvixPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@zuvix/nested-default-channel",
          zuvix: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "zuvix.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    const broadSetupRegistry = loadZuvixPlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadZuvixPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(workspacePluginDir, "zuvix.plugin.json"),
      JSON.stringify(
        {
          id: "workspace-shadow",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    api.registerChannel({
      ...
      fs.writeFileSync(
        path.join(globalDir, "zuvix.plugin.json"),
        JSON.stringify(
          {
            id: "untrusted-global-channel",
            configSchema: EMPTY_PLUGIN_SCHEMA,
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: "@zuvix/untrusted-global-channel",
            version: "0.0.0-test",
            main: "./index.cjs",
            zuvix: {
              extensions: ["./index.cjs"],
  ...
  register(api) {
    api.registerChannel({
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: "@zuvix/trusted-global-channel",
            version: "0.0.0-test",
            main: "./index.cjs",
            zuvix: {
              extensions: ["./index.cjs"],
...
module.exports = {
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-only-bundled-contract-test",
        fullBlurb: "full entry should not run in setup-only mode",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-runtime-bundled-contract-runtime-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-runtime-bundled-route-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-runtime-bundled-runtime-merge-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-runtime-external-deferred-test",
        fullBlurb: "full entry should defer while configured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@zuvix/setup-runtime-not-preferred-test",
  ...
  it("applies the bundled runtime setter before loading the merged setup-runtime plugin", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@zuvix/setup-runtime-order-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("records setup runtime setter failures without aborting the full load pass", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@zuvix/setup-runtime-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledSetupEntryContract: true,
  ...
  it("rolls back setup-runtime registrations when setup side effects fail", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@zuvix/setup-runtime-route-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("closes setup-runtime registration APIs after synchronous registration", async () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@zuvix/setup-runtime-late-route-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("rejects mismatched bundled runtime entry ids before applying setup-runtime setters", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@zuvix/setup-runtime-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("rejects mismatched bundled setup export ids before loading setup-runtime entry code", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@zuvix/setup-export-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@zuvix/setup-entry-throws-test",
          zuvix: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@zuvix/setup-entry-throws-sibling-test",
          zuvix: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "zuvix.plugin.json"),
      JSON.stringify(
        {
  ...
  it("records a diagnostic when registerChannel throws in the setup-entry path", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@zuvix/register-channel-throws-test",
          zuvix: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
...
module.exports = {
  ...
  it("prefers built bundled plugin artifacts over source TS when requested", () => {
    ...
    fs.writeFileSync(
      path.join(sourceDir, "zuvix.plugin.json"),
      JSON.stringify(
        {
    ...
    const registry = withEnv(
      {
        ZUVIX_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
        ZUVIX_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
        ZUVIX_DISABLE_BUNDLED_PLUGINS: undefined,
      },
      () =>
        loadZuvixPlugins({
          cache: false,
    ...
    const scenarios = [
      {
        ...
        loadRegistry: () => {
          ...
          return withEnv(
            {
              ZUVIX_DISABLE_BUNDLED_PLUGINS: "1",
              ZUVIX_BUNDLED_PLUGINS_DIR: undefined,
            },
            () =>
              loadZuvixPlugins({
                cache: false,
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "zuvix.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "zuvix.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryCoreDir, "zuvix.plugin.json"),
            JSON.stringify(
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "zuvix.plugin.json"),
            JSON.stringify(
  ...
  it("resolves duplicate plugin ids by source precedence", () => {
    const scenarios = [
      ...
      },
      {
        ...
        bundledFilename: "index.cjs",
        loadRegistry: () => {
          const devSourceRoot = makeZuvixDevSourceRoot();
          ...
          process.env.ZUVIX_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
          return withEnv({ ZUVIX_DEV_SOURCE_ROOT: devSourceRoot }, () =>
            withStateDir((stateDir) => {
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          return withStateDir((stateDir) => {
            const globalDir = path.join(stateDir, "node_modules", "@zuvix", "memory-lancedb");
            mkdirSafe(globalDir);
              ...
              config: {
                plugins: {
                  ...
                  installs: {
                    "memory-lancedb": {
                      source: "npm",
                      spec: "@zuvix/memory-lancedb",
                      resolvedName: "@zuvix/memory-lancedb",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ ZUVIX_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("uses the source runtime snapshot allowlist for plugin trust checks", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ ZUVIX_STATE_DIR: stateDir }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("zuvix/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ ZUVIX_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadZuvixPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__zuvixLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("zuvix/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("zuvix/plugin-sdk");
