...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@zuvix/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@zuvix/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@zuvix/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts",
  "@zuvix/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts",
  "@zuvix/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@zuvix/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@zuvix/discord:dangerous-exec:src/voice/audio.ts",
  "@zuvix/google-meet:dangerous-exec:src/node-host.ts",
  "@zuvix/google-meet:dangerous-exec:src/realtime.ts",
  "@zuvix/matrix:dangerous-exec:src/matrix/deps.ts",
  "@zuvix/voice-call:dangerous-exec:src/tunnel.ts",
  "@zuvix/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@zuvix/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@zuvix/acpx:dangerous-exec:dist/service-<hash>.js",
  "@zuvix/codex:dangerous-exec:dist/client-<hash>.js",
  "@zuvix/google-meet:dangerous-exec:dist/index.js",
  "@zuvix/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@zuvix/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "zuvix-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        zuvix?: { release?: { publishToNpm?: unknown } };
      };
      try {
      ...
      if (packageJson.zuvix?.release?.publishToNpm !== true) {
        return [];
      }
