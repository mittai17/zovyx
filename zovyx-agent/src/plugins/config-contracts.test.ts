...
const mocks = vi.hoisted(() => {
  ...
  return {
    discoverZuvixPlugins: vi.fn(() => ({ candidates: [], diagnostics: [] })),
...
vi.mock("./discovery.js", () => ({
  discoverZuvixPlugins: mocks.discoverZuvixPlugins,
}));
...
function createPluginRecord(
  ...
  return {
    ...
    hooks: [],
    source: `/tmp/${overrides.id}/zuvix.plugin.json`,
...
describe("resolvePluginConfigContractsById", () => {
  beforeEach(() => {
    mocks.discoverZuvixPlugins.mockReset();
    mocks.discoverZuvixPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
