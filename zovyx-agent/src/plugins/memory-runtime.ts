...
function resolveMemoryRuntimePluginIds(config: ZuvixConfig): string[] {
...
function resolveMemoryRuntimeWorkspaceDir(cfg: ZuvixConfig): string | undefined {
...
function ensureMemoryRuntime(cfg?: ZuvixConfig) {
...
export function resolveActiveMemoryBackendConfig(params: { cfg: ZuvixConfig; agentId: string }) {
  return ensureMemoryRuntime(params.cfg)?.resolveMemoryBackendConfig(params) ?? null;
}

/** Closes all active plugin-backed memory search managers. */
export async function closeActiveMemorySearchManagers(cfg?: ZuvixConfig): Promise<void> {
  void cfg;
