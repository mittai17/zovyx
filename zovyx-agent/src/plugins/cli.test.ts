...
const mocks = vi.hoisted(() => ({
  ...
  loadZuvixPluginCliRegistry: vi.fn(),
  loadZuvixPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadZuvixPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadZuvixPluginCliRegistry(...args),
  loadZuvixPlugins: (...args: unknown[]) => mocks.loadZuvixPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: ZuvixConfig;
  autoEnabledConfig: ZuvixConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadZuvixPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadZuvixPluginCliRegistry.mockReset();
    mocks.loadZuvixPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadZuvixPlugins.mockReset();
    mocks.loadZuvixPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as ZuvixConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as ZuvixConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as ZuvixConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as ZuvixConfig);

    const loadOptions = getMockCallObject(mocks.loadZuvixPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadZuvixPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as ZuvixConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as ZuvixConfig);

    expect(mocks.loadZuvixPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as ZuvixConfig;
    const configB = { plugins: {} } as ZuvixConfig;
    const envA = { ZUVIX_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { ZUVIX_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadZuvixPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadZuvixPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadZuvixPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadZuvixPluginCliRegistry.mockImplementationOnce((options: { logger?: unknown }) => {
    ...
    await expect(
      getPluginCliCommandDescriptors({ plugins: { entries: { stale: {} } } } as ZuvixConfig),
    ).resolves.toEqual([]);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadZuvixPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as ZuvixConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadZuvixPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadZuvixPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadZuvixPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadZuvixPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("skips full plugin runtime loading when no metadata owns the requested primary", async () => {
    ...
    expect(mocks.loadZuvixPluginCliRegistry).toHaveBeenCalled();
    expect(mocks.loadZuvixPlugins).not.toHaveBeenCalled();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as ZuvixConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
