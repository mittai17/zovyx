...
import { normalizeOptionalString as normalizeString } from "@zuvix/normalization-core/string-coerce";
...
function getConfigPath(config: ZuvixConfig | undefined, path: string): unknown {
...
function setConfigPath(target: ZuvixConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: ZuvixConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: ZuvixConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: ZuvixConfig) =>
