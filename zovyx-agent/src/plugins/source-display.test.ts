...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "zuvix", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".zuvix", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".zuvix", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "zuvix-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".zuvix", "extensions"),
      },
    });
  });
});
