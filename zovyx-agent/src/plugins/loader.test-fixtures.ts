...
import { clearPluginLoaderCache, loadZuvixPlugins } from "./loader.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadZuvixPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadZuvixPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "zuvix-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.ZUVIX_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".zuvix", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ ZUVIX_STATE_DIR: stateDir, ...params.env }, () =>
    loadZuvixPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ZUVIX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.ZUVIX_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
