...
import { uniqueValues } from "@zuvix/normalization-core/string-normalization";
import {
...
import type {
  ...
  ZuvixPluginApi,
  ZuvixPluginChannelRegistration,
  ZuvixPluginCliCommandDescriptor,
  ZuvixPluginCliRegistrar,
  ZuvixPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  ZuvixPluginGatewayRuntimeScopeSurface,
  ZuvixGatewayDiscoveryService,
  ZuvixPluginHostedMediaResolver,
  ZuvixPluginHttpRouteParams,
  ZuvixPluginHookOptions,
  ZuvixPluginNodeHostCommand,
  ZuvixPluginNodeInvokePolicy,
  ZuvixPluginReloadRegistration,
  ZuvixPluginSecurityAuditCollector,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@zuvix/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@zuvix/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("zuvix.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<ZuvixPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<ZuvixPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<ZuvixPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
  ...
  const registerTool = (
    ...
    const factory: ZuvixPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: ZuvixPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: ZuvixPluginHookOptions | undefined,
    config: ZuvixPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: ZuvixPluginHttpRouteParams) => {
  ...
  const registerHostedMediaResolver = (
    record: PluginRecord,
    resolver: ZuvixPluginHostedMediaResolver,
  ) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: ZuvixPluginChannelRegistration | ChannelPlugin,
    ...
    const normalized =
      typeof (registration as ZuvixPluginChannelRegistration).plugin === "object"
        ? (registration as ZuvixPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is ZuvixPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: ZuvixPluginReloadRegistration) => {
    const normalized: ZuvixPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: ZuvixPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: ZuvixPluginNodeInvokePolicy,
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: ZuvixPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: ZuvixPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    record: PluginRecord,
    service: ZuvixGatewayDiscoveryService,
  ) => {
    const id = service.id.trim();
    if (!id) {
      return;
  ...
  const registerCommand = (record: PluginRecord, command: ZuvixPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<ZuvixPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<ZuvixPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as ZuvixConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
              ...
              sendSessionAttachment: async (attachment) => {
                ...
                try {
                  ...
                  const runtimeConfig =
                    (registryParams.runtime.config?.current?.() as ZuvixConfig | undefined) ??
                    params.config;
