...
const resolveZuvixPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/zuvix-root.js", () => ({
  resolveZuvixPackageRootSync: (...args: unknown[]) =>
    resolveZuvixPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("zuvix-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      zuvix: params.zuvix ?? { extensions: ["./dist/index.js"] },
      ...(params.dependency
  ...
  fs.writeFileSync(
    path.join(pluginDir, "zuvix.plugin.json"),
    JSON.stringify({
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootZuvix) {
      lockPackages["node_modules/zuvix"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalZuvixPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "zuvix"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.zuvix ? { zuvix: viewPackage.zuvix } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalZuvixPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "zuvix") {
            const zuvixRoot = path.join(npmRoot, "node_modules", "zuvix");
            fs.mkdirSync(zuvixRoot, { recursive: true });
            fs.writeFileSync(
              path.join(zuvixRoot, "package.json"),
              JSON.stringify({ name: "zuvix", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootZuvix) {
            ...
            fs.writeFileSync(
              path.join(zuvixRoot, "package.json"),
              JSON.stringify({ name: "zuvix", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "zuvix") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "zuvix"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveZuvixPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "zuvix", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveZuvixPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "zuvix-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@zuvix/pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
    ...
    {
      spec: "@zuvix/voice-call@0.0.1",
      packageName: "@zuvix/voice-call",
      version: "0.0.1",
      pluginId: "voice-call",
      npmRoot,
    },
  ...
  const dependencySpec = managedManifest.dependencies?.["@zuvix/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@zuvix/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@zuvix/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("zuvix-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@zuvix/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_zuvix-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_zuvix-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@zuvix/pack-demo";
    const archiveV1Path = path.join(stateDir, "zuvix-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "zuvix-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@zuvix/pack-demo";
    const archivePath = path.join(stateDir, "zuvix-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .zuvix/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@zuvix/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@zuvix/voice-call@0.0.1");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@zuvix/voice-call";
    ...
    const quarantineParent = path.join(npmProjectRoot, "_zuvix-quarantined-npm-projects");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed zuvix peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstallMany([
        {
          ...
          npmRoot,
          peerDependencies: { zuvix: "^2026.0.0" },
        },
        {
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "zuvix")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/zuvix");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "zuvix")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their zuvix peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveZuvixPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@zuvix/codex@2026.5.7",
      packageName: "@zuvix/codex",
      version: "2026.5.7",
      pluginId: "@zuvix/codex",
      npmRoot,
      peerDependencies: { zuvix: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@zuvix/codex");
    expect(result.error).toContain("plugin-local node_modules/zuvix link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate zuvix package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@zuvix/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@zuvix/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("ZUVIX_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@zuvix/whatsapp@2026.5.27",
      packageName: "@zuvix/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { zuvix: ">=2026.5.27" },
      zuvix: {
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@zuvix", "whatsapp"))).toBe(false);
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@zuvix/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @zuvix/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@zuvix/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@zuvix/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      {
        spec: "@zuvix/msteams@beta",
        packageName: "@zuvix/msteams",
      ...
      {
        spec: "@zuvix/msteams@2026.5.28-beta.3",
        packageName: "@zuvix/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@zuvix/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @zuvix/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@zuvix/msteams",
    ...
    expect(managedManifest.dependencies?.["@zuvix/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root zuvix materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "zuvix"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/zuvix"]).toBeUndefined();
      expect(
        fs.lstatSync(path.join(requiredPeerPluginDir, "node_modules", "zuvix")).isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed zuvix root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@zuvix/discord",
    });
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "zuvix"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/zuvix": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/zuvix/-/zuvix-2026.5.4.tgz",
            },
          },
          dependencies: {
            zuvix: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "zuvix", "package.json"),
      JSON.stringify({
        name: "zuvix",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@zuvix/discord@beta",
      packageName: "@zuvix/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { zuvix: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("zuvix");
    expect(manifest.dependencies?.["@zuvix/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/zuvix"]).toBeUndefined();
    expect(lockfile.dependencies?.zuvix).toBeUndefined();
  });

  it("preserves the active host zuvix runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "zuvix");
    ...
    resolveZuvixPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/zuvix-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/zuvix-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.zuvix).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/zuvix-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/zuvix-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/zuvix-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/zuvix-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@zuvix/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "zuvix"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "zuvix")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local zuvix peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@zuvix/codex",
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@zuvix/codex": "0.0.1" },
      packages: [
        {
          packageName: "@zuvix/codex",
          version: "0.0.1",
    ...
    const installedDir = writeInstalledNpmPlugin({
      ...
      packageName: "@zuvix/codex",
      version: "0.0.1",
      peerDependencies: { zuvix: "*" },
    });
    const peerLink = path.join(installedDir, "node_modules", "zuvix");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@zuvix/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
              name: "@zuvix/codex",
              version: "0.0.2",
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmProjectRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; zuvix?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.zuvix?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.zuvix?.managedOverrides).toEqual(["axios", "nested"]);
        }
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official Zuvix package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@zuvix/voice-call");
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@zuvix/voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@zuvix/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@zuvix/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@zuvix/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @zuvix/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@zuvix/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@zuvix/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @zuvix/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@zuvix/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@zuvix/voice-call",
