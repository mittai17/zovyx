...
export type ConfigScopedRuntimeCache<T> = WeakMap<ZuvixConfig, Map<string, T>>;
...
export type ConfigScopedPromiseLoader<T> = {
  load(config?: ZuvixConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: ZuvixConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<ZuvixConfig, Promise<T>>();

  const createPromise = (config?: ZuvixConfig): Promise<T> => {
  ...
  return {
    async load(config?: ZuvixConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<ZuvixConfig, Promise<T>>();
    },
  };
}
