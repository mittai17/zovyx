...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: ZuvixConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when ZUVIX_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: ZuvixConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from ZUVIX_GATEWAY_TOKEN", () => {
    const cfg: ZuvixConfig = {
