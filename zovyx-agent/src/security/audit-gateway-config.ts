...
type CollectDangerousConfigFlags = (cfg: ZuvixConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.ZUVIX_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.ZUVIX_GATEWAY_PASSWORD);
