...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: ZuvixConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: ZuvixConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: ZuvixConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): ZuvixConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: ZuvixConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
