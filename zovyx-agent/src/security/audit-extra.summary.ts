...
function summarizeGroupPolicy(cfg: ZuvixConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: ZuvixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ZuvixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ZuvixConfig): boolean {
...
function isBrowserEnabled(cfg: ZuvixConfig): boolean {
  return cfg.browser?.enabled !== false;
}

/** Produce a concise inventory of major security-relevant surfaces. */
export function collectAttackSurfaceSummaryFindings(cfg: ZuvixConfig): SecurityAuditFinding[] {
