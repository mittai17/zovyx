...
import type { ZuvixConfig, SecurityConfig } from "../config/types.zuvix.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "zuvix" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  if (!config) {
    try {
      ...
      return failClosed(`could not load Zuvix config (${formatErrorMessage(err)})`);
    }
  }
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    zuvixVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });
