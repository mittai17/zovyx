...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-security-audit-async-"));
  ...
  it("ignores install backup and debris dirs when scanning installed plugin roots", async () => {
    ...
    try {
      ...
      expect(findings.map((f) => f.title).join("\n")).not.toContain(".zuvix-install-backups");
    } finally {
      scanSpy.mockRestore();
    }
  });
  ...
  it("audits canonical auth profile SQLite store permissions", async () => {
    ...
    const databasePath = path.join(agentDir, "zuvix-agent.sqlite");
    ...
    expect(readableAuthTargets).toEqual(
      expect.arrayContaining([
        expect.stringContaining("zuvix-agent.sqlite"),
        expect.stringContaining("zuvix-agent.sqlite-wal"),
        expect.stringContaining("zuvix-agent.sqlite-shm"),
      ]),
    );
  });
});
