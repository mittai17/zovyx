...
describe("security audit exec surface findings", () => {
  // Redirect the Zuvix home (ZUVIX_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.zuvix/exec-approvals.json` on the host running
  // the suite.
  ...
  it("warns when YOLO exec overrides restrictive Claude permission mode", () => {
    ...
    expect(finding?.detail).toContain("Zuvix exec is YOLO");
  });
  ...
  it("does not warn for restrictive Claude permission mode when Zuvix exec is restrictive", () => {
    const findings = collectExecRuntimeFindings({
