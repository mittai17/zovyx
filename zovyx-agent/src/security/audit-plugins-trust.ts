...
function readChannelCommandSetting(
  ...
  const channelCfg = cfg.channels?.[channelId as keyof NonNullable<ZuvixConfig["channels"]>];
  ...
  if (pluginDirs.length > 0) {
    ...
    if (enabledExtensionPluginIds.length > 0) {
      ...
      for (const context of contexts) {
        ...
        const broadPolicy = deps.isToolAllowedByPolicies("__zuvix_plugin_probe__", policies);
  ...
  if (npmPluginInstalls.length > 0) {
    ...
    if (pluginVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `zuvix plugins update --all` (or reinstall affected plugins) to refresh install metadata.",
      });
    }
  }
  ...
  if (npmHookInstalls.length > 0) {
    ...
    if (hookVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `zuvix hooks update --all` (or reinstall affected hooks) to refresh install metadata.",
      });
    }
  }
