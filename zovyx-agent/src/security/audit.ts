...
import { asNullableRecord } from "@zuvix/normalization-core/record-coerce";
import {
...
import type { ConfigFileSnapshot, ZuvixConfig } from "../config/config.js";
import { resolveConfigPath, resolveStateDir } from "../config/paths.js";
import type { CliBackendConfig } from "../config/types.agent-defaults.js";
import type { GatewayAuthConfig } from "../config/types.gateway.js";
import type { SecurityAuditSuppression } from "../config/types.zuvix.js";
import {
...
async function materializeAuditGatewayAuthRefs(params: {
  ...
  const materializeParams = {
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(params.env.ZUVIX_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(params.env.ZUVIX_GATEWAY_PASSWORD)),
  };
...
function shouldMaterializeHooksGatewayAuthRefs(cfg: ZuvixConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Zuvix state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Zuvix state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
export function collectLoggingFindings(cfg: ZuvixConfig): SecurityAuditFinding[] {
...
export function collectElevatedFindings(cfg: ZuvixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectYoloExecScopeIds(cfg: ZuvixConfig, approvals: ExecApprovalsFile): string[] {
...
export function collectExecRuntimeFindings(cfg: ZuvixConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (claudePermissionModeHits.length > 0 && yoloExecScopeIds.length > 0) {
    findings.push({
      ...
      detail: `claude-cli sets ${claudePermissionModeHits.map((hit) => `${hit.argSet}=${hit.mode}`).join(", ")}, but Zuvix exec is YOLO for: ${yoloExecScopeIds.join(", ")}. Managed Claude live sessions use --permission-mode bypassPermissions.`,
      remediation:
        "Restrict Zuvix tools.exec.security/tools.exec.ask, or remove the Claude --permission-mode override.",
    });
  }
...
function collectOpenExecSurfacePaths(cfg: ZuvixConfig): string[] {
