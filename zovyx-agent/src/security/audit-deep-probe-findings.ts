...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("zuvix status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("zuvix security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set ZUVIX_GATEWAY_TOKEN/ZUVIX_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("zuvix security audit --deep")}".`,
    });
  }
  return findings;
}
