 ...
 */
/** Opening delimiter for protected Zuvix runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_ZUVIX_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected Zuvix runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_ZUVIX_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[ZUVIX_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[ZUVIX_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const ZUVIX_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
/** Header for context attached to the immediately preceding user message. */
export const ZUVIX_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "Zuvix runtime context for the immediately preceding user message.";
/** Header for runtime events passed as prompt context. */
export const ZUVIX_RUNTIME_EVENT_HEADER = "Zuvix runtime event.";
/** Custom message type used for structured runtime-context messages. */
export const ZUVIX_RUNTIME_CONTEXT_CUSTOM_TYPE = "zuvix.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["Zuvix runtime context (internal):", ZUVIX_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === ZUVIX_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === ZUVIX_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === ZUVIX_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${ZUVIX_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${ZUVIX_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${ZUVIX_RUNTIME_EVENT_HEADER}\n${ZUVIX_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isZuvixRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === ZUVIX_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

/** Remove all structured runtime-context custom messages. */
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isZuvixRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isZuvixRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!isZuvixRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!isZuvixRuntimeContextCustomMessage(message)) {
      return true;
    }
