...
describe("isModernModelRef", () => {
  ...
  it("includes plugin-advertised modern models", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(({ provider, context }) =>
      ...
      ["gpt-5.5", "gpt-5.5-pro", "gpt-5.4", "gpt-5.4-pro", "gpt-5.4-mini", "gpt-5.4-nano"].includes(
        ...
        : provider === "openai" &&
          ...
          ? true
          : provider === "zuvix" && ["claude-opus-4-6", "gemini-3-pro"].includes(context.modelId)
            ? true
            : provider === "zuvix-go"
              ? true
              : undefined,
    ...
    expect(isModernModelRef({ provider: "openai", id: "gpt-5.4-pro" })).toBe(true);
    expect(isModernModelRef({ provider: "openai", id: "gpt-5.4-mini" })).toBe(true);
    expect(isModernModelRef({ provider: "zuvix", id: "claude-opus-4-6" })).toBe(true);
    expect(isModernModelRef({ provider: "zuvix", id: "gemini-3-pro" })).toBe(true);
    expect(isModernModelRef({ provider: "zuvix-go", id: "kimi-k2.5" })).toBe(true);
    expect(isModernModelRef({ provider: "zuvix-go", id: "glm-5" })).toBe(true);
    expect(isModernModelRef({ provider: "zuvix-go", id: "minimax-m2.7" })).toBe(true);
  });
  ...
  it("excludes provider-declined modern models", () => {
    providerRuntimeMocks.resolveProviderModernModelRef.mockImplementation(({ provider, context }) =>
      provider === "zuvix" && context.modelId === "minimax-m2.7" ? false : undefined,
    );

    expect(isModernModelRef({ provider: "zuvix", id: "minimax-m2.7" })).toBe(false);
  });
});
...
describe("isHighSignalLiveModelRef", () => {
  ...
  it("drops low-signal or old Claude variants even when provider marks them modern", () => {
    ...
    expect(
      isHighSignalLiveModelRef({ provider: "zuvix", id: "claude-3-5-haiku-20241022" }),
    ).toBe(false);
  });
  ...
  it("keeps only the current direct OpenAI-family model in the default live matrix", () => {
    ...
    expect(isHighSignalLiveModelRef({ provider: "zuvix", id: "gpt-5.1-codex-mini" })).toBe(
      false,
    );
