...
function buildCoreToolGroupMap() {
  ...
  const zuvixTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInZuvixGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:zuvix": zuvixTools,
