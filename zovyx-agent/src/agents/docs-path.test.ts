// Covers locating Zuvix docs and source paths from package roots.
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { describe, expect, it } from "vitest";
import {
  resolveZuvixDocsPath,
  resolveZuvixReferencePaths,
  resolveZuvixSourcePath,
} from "./docs-path.js";
...
async function makePackageRoot(prefix: string): Promise<string> {
  ...
  await fs.writeFile(path.join(root, "package.json"), '{"name":"zuvix"}\n');
  return root;
}
...
describe("resolveZuvixDocsPath", () => {
  it("uses the workspace docs directory when it has canonical docs metadata", async () => {
    const root = await makePackageRoot("zuvix-docs-workspace-");
    await writeDocsJson(root);

    await expect(resolveZuvixDocsPath({ workspaceDir: root })).resolves.toBe(
      path.join(root, "docs"),
    );
  });

  it("finds bundled package docs from a nested package path", async () => {
    const root = await makePackageRoot("zuvix-docs-package-");
    await writeDocsJson(root);
    const nested = path.join(root, "dist", "agents");
    await fs.mkdir(nested, { recursive: true });

    await expect(resolveZuvixDocsPath({ cwd: nested })).resolves.toBe(path.join(root, "docs"));
  });
  ...
  it("does not accept incomplete template-only docs directories", async () => {
    ...
    const root = await makePackageRoot("zuvix-docs-incomplete-");
    await fs.mkdir(path.join(root, "docs", "reference", "templates"), { recursive: true });

    await expect(resolveZuvixDocsPath({ cwd: root })).resolves.toBeNull();
  });
});

describe("resolveZuvixSourcePath", () => {
  it("returns the package root only for git checkouts", async () => {
    const root = await makePackageRoot("zuvix-source-git-");
    await fs.mkdir(path.join(root, ".git"));

    await expect(resolveZuvixSourcePath({ cwd: root })).resolves.toBe(root);
  });
  ...
  it("omits source path for npm-style package installs", async () => {
    ...
    const root = await makePackageRoot("zuvix-source-npm-");

    await expect(resolveZuvixSourcePath({ cwd: root })).resolves.toBeNull();
  });
});

describe("resolveZuvixReferencePaths", () => {
  it("returns docs and local source together for git checkouts", async () => {
    const root = await makePackageRoot("zuvix-reference-git-");
    await writeDocsJson(root);
    await fs.mkdir(path.join(root, ".git"));

    await expect(resolveZuvixReferencePaths({ cwd: root })).resolves.toEqual({
