...
import { ZUVIX_AGENT_RUNTIME_ID, isDefaultAgentRuntimeId } from "./agent-runtime-id.js";
import { normalizeOptionalAgentRuntimeId } from "./agent-runtime-id.js";
import { resolveAgentHarnessPolicy } from "./harness/policy.js";

// Harness runtime discovery feeds plugin preloading/setup. Only plugin runtimes
// are selectable here; built-in Zuvix/default runtime ids are excluded.
...
function isSelectablePluginRuntime(runtime: string | undefined): runtime is string {
  return (
    ...
    normalizeOptionalAgentRuntimeId(runtime) !== ZUVIX_AGENT_RUNTIME_ID
  );
}
...
function pushConfiguredModelRuntimeIds(config: ZuvixConfig, runtimes: Set<string>): void {
