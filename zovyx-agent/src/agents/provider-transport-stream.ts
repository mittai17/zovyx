 ...
 *
 * Routes models that need Zuvix-managed proxy/TLS/local-service semantics onto built-in transport implementations.
 */
...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "zuvix-openai-responses-transport",
  "openai-chatgpt-responses": "zuvix-openai-responses-transport",
  "openai-completions": "zuvix-openai-completions-transport",
  "azure-openai-responses": "zuvix-azure-openai-responses-transport",
  "anthropic-messages": "zuvix-anthropic-messages-transport",
  "google-generative-ai": "zuvix-google-generative-ai-transport",
};
...
function hasZuvixTransportRequirement(model: Model): boolean {
  const request = getModelProviderRequestTransport(model);
  return Boolean(request?.proxy || request?.tls || getModelProviderLocalService(model));
}

/** Returns whether Zuvix has a managed transport implementation for this API. */
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasZuvixTransportRequirement(model)) {
    return undefined;
  }
...
/** Creates a managed Zuvix transport stream for explicit fallback/runtime callers. */
export function createZuvixTransportStreamFnForModel(
  model: Model,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need Zuvix's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // HTTP streams preserve the same Zuvix request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
