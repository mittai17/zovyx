...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-managed-media-"));
  try {
    return await withEnvAsync({ ZUVIX_STATE_DIR: stateDir }, async () => {
...
async function withOutsideHardlinkInZuvixTmp<T>(
  params: {
    zuvixTmpDir: string;
  ...
  const hardlinkPath = path.join(params.zuvixTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.zuvixTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.zuvixTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.zuvixTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const zuvixTmpDir = resolvePreferredZuvixTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Zuvix tmp root",
      media: path.join(zuvixTmpDir, "image.png"),
      expected: path.join(zuvixTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Zuvix tmp root",
      media: pathToFileURL(path.join(zuvixTmpDir, "photo.png")).href,
      expected: path.join(zuvixTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Zuvix tmp root",
      media: path.join(zuvixTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(zuvixTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(zuvixTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside zuvix tmp root",
      media: path.join(os.tmpdir(), "outside-zuvix", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Zuvix tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(zuvixTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(zuvixTmpDir, { recursive: true });
      const symlinkPath = path.join(zuvixTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Zuvix tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInZuvixTmp(
      {
        zuvixTmpDir,
  ...
  it("rejects symlinked Zuvix tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInZuvixTmp(
