...
import { normalizeTrimmedStringList } from "@zuvix/normalization-core/string-normalization";
import { listZuvixPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
...
const ZUVIX_ATTRIBUTION_PRODUCT = "Zuvix";
const ZUVIX_ATTRIBUTION_ORIGINATOR = "zuvix";
...
function formatZuvixUserAgent(version: string): string {
  return `${ZUVIX_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listZuvixPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: ZUVIX_ATTRIBUTION_PRODUCT,
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": ZUVIX_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: ZUVIX_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatZuvixUserAgent(identity.version),
    },
  };
}
