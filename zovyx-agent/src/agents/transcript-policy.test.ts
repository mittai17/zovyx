      ...
      return {
        buildReplayPolicy: (context?: {
          ...
          switch (provider) {
            case "env-sensitive":
              return {
                sanitizeToolCallIds: context?.env?.ZUVIX_TEST_TRANSCRIPT_POLICY === "strict",
                ...(context?.env?.ZUVIX_TEST_TRANSCRIPT_POLICY === "strict"
...
describe("resolveTranscriptPolicy", () => {
  ...
  it("does not reuse cached replay policies across custom env objects", () => {
    ...
    const strictEnv = {
      ...process.env,
      ZUVIX_TEST_TRANSCRIPT_POLICY: "strict",
    };
    const looseEnv = {
      ...process.env,
      ZUVIX_TEST_TRANSCRIPT_POLICY: "loose",
    };
  ...
  it.each([
    ...
    { provider: "zuvix", modelId: "google/gemini-2.5-flash" },
