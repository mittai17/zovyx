...
function hasConfiguredProvider(params: { cfg?: ZuvixConfig; provider: string }): boolean {
  const normalizedProvider = normalizeProviderId(params.provider);
...
function isCliAgentRuntime(runtime: string | undefined, cfg: ZuvixConfig | undefined): boolean {
...
async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  // Explicit non-Codex plugin harnesses own transport/auth; stale Zuvix
...
export function resolveImageFallbackDefaultProvider(cfg: ZuvixConfig | undefined): string {
...
function resolveFallbackCandidateModelProviderCacheParts(cfg: ZuvixConfig | undefined): unknown {
