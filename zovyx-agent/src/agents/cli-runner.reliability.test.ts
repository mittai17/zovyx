...
const hookRunnerGlobalStateKey = Symbol.for("zuvix.plugins.hook-runner-global-state");
...
function createSessionFile(params?: { history?: Array<{ role: "user"; content: string }> }) {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-hooks-"));
...
function buildPreparedContext(params?: {
  ...
  return {
    ...
    ...(params?.zuvixHistoryPrompt
      ? { zuvixHistoryPrompt: params.zuvixHistoryPrompt }
      : {}),
    authEpochVersion: 2,
  };
}
...
const CLI_RESEED_PROMPT =
  "Continue this conversation using the Zuvix transcript below as prior session history.\n\n<conversation_history>\nUser: earlier context\n</conversation_history>\n\n<next_user_message>\nhi\n</next_user_message>";
...
describe("runCliAgent reliability", () => {
  ...
  it("preserves fresh retry for direct CLI callers without a pre-clear hook", async () => {
    ...
    const context = buildPreparedContext({
      ...
      provider: "claude-cli",
      model: "opus",
      zuvixHistoryPrompt: CLI_RESEED_PROMPT,
    });
  ...
  it("seeds fresh CLI sessions from the Zuvix transcript", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        zuvixHistoryPrompt:
          "Continue this conversation using the Zuvix transcript below.\n\nUser: earlier ask\n\nAssistant: earlier answer\n\n<next_user_message>\nhi\n</next_user_message>",
      }),
    );
  ...
  it("keeps resumed CLI sessions on native resume history", async () => {
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        ...
        zuvixHistoryPrompt: "User: earlier ask",
      }),
    );
  ...
  it("passes cwd to approved CLI user-turn persistence", async () => {
    ...
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-persist-cwd-"));
  ...
  it("does not execute the CLI when approved user turn persistence fails", async () => {
    supervisorSpawnMock.mockClear();
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-persist-fail-"));
  ...
  it("blocks CLI runs before llm_input and model execution when before_agent_run blocks", async () => {
    ...
    try {
      ...
      expect(blockedLine.message["__zuvix"].beforeAgentRunBlocked.blockedBy).toBe(
        "policy-plugin",
      );
      expect(blockedLine.message["__zuvix"].beforeAgentRunBlocked).not.toHaveProperty("reason");
      expect(Object.hasOwn(blockedLine.message["__zuvix"], "beforeAgentRunBlocked")).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("builds fresh-session history reseed prompts from hook-mutated prompts", async () => {
    ...
    try {
      ...
      expect(context.zuvixHistoryPrompt).toContain("Compaction summary: compacted earlier ask");
      expect(context.zuvixHistoryPrompt).toContain("hook context");
      expect(context.zuvixHistoryPrompt).toContain("current ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
