...
export function resolveModelRuntimePolicy(params: {
  ...
  if (process.env.ZUVIX_BUILD_PRIVATE_QA === "1") {
    const forcedRuntime = process.env.ZUVIX_QA_FORCE_RUNTIME?.trim().toLowerCase();
    if (forcedRuntime === "zuvix" || forcedRuntime === "codex") {
