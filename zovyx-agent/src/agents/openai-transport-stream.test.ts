...
import type { Api, Model } from "zuvix/plugin-sdk/llm";
...
import {
  ...
  createZuvixTransportStreamFnForModel,
...
describe("openai transport stream", () => {
  ...
  it("tags Responses encrypted reasoning with replay provenance while streaming", async () => {
    ...
    expect(replayItem).not.toHaveProperty("__zuvix_replay");
    expect(thinkingBlock.zuvixReasoningReplay).toEqual(expectedReplayMetadata);
  });
  ...
  it("summarizes model payload tools with full names when requested", () => {
    const previous = process.env.ZUVIX_DEBUG_MODEL_PAYLOAD;
    process.env.ZUVIX_DEBUG_MODEL_PAYLOAD = "tools";
    try {
      expect(
      ...
      if (previous === undefined) {
        delete process.env.ZUVIX_DEBUG_MODEL_PAYLOAD;
      } else {
        process.env.ZUVIX_DEBUG_MODEL_PAYLOAD = previous;
      }
    }
  });
  ...
  it("redacts full model payload debug summaries", () => {
    const previous = process.env.ZUVIX_DEBUG_MODEL_PAYLOAD;
    process.env.ZUVIX_DEBUG_MODEL_PAYLOAD = "full-redacted";
    try {
  ...
  it("enforces the code mode responses tool surface before requests leave Zuvix", () => {
    const payload = {
      tools: [
        { type: "function", name: "exec" },
  ...
  it("adds Zuvix attribution to native OpenAI transport headers and protects it from provider overrides", () => {
  ...
  it("adds Zuvix attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("ZUVIX_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      {
        id: "gpt-5.4-codex",
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("zuvix-openai-responses-transport");
    expectRecordFields(prepared, {
      api: "zuvix-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("zuvix-anthropic-messages-transport");
    expectRecordFields(prepared, {
      api: "zuvix-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("reports the Google simple-completion api alias without loading provider runtime", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "zuvix-google-generative-ai-transport",
    );
  });
  ...
  it("uses top-level instructions for Codex responses and preserves prompt cache identity", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        zuvix_session_id: "session-123",
        zuvix_turn_id: "turn-123",
      },
    ) as Record<string, unknown> & {
  ...
  it("keeps Codex response shaping when simple completions use the Zuvix transport alias", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "gpt-5.5",
        name: "GPT-5.5",
        api: "zuvix-openai-responses-transport" as Api,
        provider: "openai",
        baseUrl: "https://chatgpt.com/backend-api/codex",
        reasoning: true,
        input: ["text"],
  ...
  it("sanitizes Codex responses params after payload hooks mutate them without stripping cache identity", () => {
    const payload = {
      ...
      metadata: { zuvix_session_id: "session-123" },
  ...
  it("omits prior Responses replay item ids when store is disabled for custom Codex-compatible responses", () => {
            ...
            content: [
              {
                ...
                zuvixReasoningReplay: testing.buildOpenAIResponsesReasoningReplayMetadata(
                  model,
                  {
                    authProfileId: "openai:oauth",
                    sessionId: "session-123",
    ...
    expect(reasoningItem?.id).toBeUndefined();
    expect(reasoningItem).not.toHaveProperty("__zuvix_replay");
    const assistantMessage = params.input?.find(
      (item) => item.type === "message" && item.role === "assistant",
    );
    expectRecordFields(assistantMessage, {
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      { sessionId: "session-123" } as never,
      {
        zuvix_session_id: "session-123",
        zuvix_turn_id: "turn-123",
        zuvix_turn_attempt: "1",
        zuvix_transport: "stream",
      },
...
describe("buildOpenAICompletionsParams sanitizes reasoning replay fields", () => {
  ...
  // Regression for #87575: Zuvix Zen exposes DeepSeek V4 with a `-free`
  ...
  it.each([
    [
      "Zuvix Zen DeepSeek V4 Flash Free",
      {
