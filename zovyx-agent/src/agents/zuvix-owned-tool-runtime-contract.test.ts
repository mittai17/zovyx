// Verifies Zuvix-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "zuvix/plugin-sdk/agent-core";
import {
  installZuvixOwnedToolHooks,
  resetZuvixOwnedToolHooks,
  textToolResult,
} from "zuvix/plugin-sdk/agent-runtime-test-contracts";
import type { ExtensionContext } from "zuvix/plugin-sdk/agent-sessions";
...
describe("Zuvix-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetZuvixOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installZuvixOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installZuvixOwnedToolHooks();
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/zuvix-reply.png"]);
    expect(
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installZuvixOwnedToolHooks({ blockReason: "blocked by policy" });
