...
} from "@zuvix/normalization-core/string-coerce";
import {
  normalizeStringEntries,
  normalizeStringEntriesLower,
  normalizeUniqueStringEntries,
} from "@zuvix/normalization-core/string-normalization";
...
import {
  buildZuvixToolFallbackText,
  shouldRenderZuvixToolWorkflowHints,
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__zuvix__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__zuvix__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__zuvix__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Zuvix handles all routing internally.",
      ...
      ? [
          ...
          messageToolOnly
            ...
            : suppressSilentTokenGuidance
              ? "- Do not use `message(action=send)` to deliver the current source-channel reply; reply normally so Zuvix can route it once."
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.zuvix.ai",
    docsPath ? "Mirror: https://docs.zuvix.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/zuvix/zuvix",
    docsPath
      ? "Zuvix behavior/config/architecture: read local docs first."
      : "Zuvix behavior/config/architecture: read docs mirror first.",
    ...
    "Diagnosing issues: run `zuvix status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to Zuvix main. */
  ...
  const promptSurface = params.promptSurface ?? "zuvix_main";
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Zuvix process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Zuvix agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Zuvix agent ids allowed for sessions_spawn",
  ...
  const renderZuvixToolWorkflowHints = shouldRenderZuvixToolWorkflowHints({
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? "tools.fs.workspaceOnly is enabled: scratch/temp/meta files that file tools must later read/write/edit must stay inside the workspace, preferably as relative paths under `.zuvix/tmp/`. Do not write files to `/tmp/...` with exec if a later read/write/edit/apply_patch tool needs them; use `.zuvix/tmp/...` instead."
      : "";
  const safetySection = [
    "## Safety",
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside Zuvix.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      "You are a personal assistant running inside Zuvix.",
      "",
      "## Tooling",
      ...
      ...(acpHarnessSpawnAllowed
        ? [
            'For requests like "do this in claude code/cursor/gemini/zuvix" or similar ACP harnesses, treat it as ACP harness intent and call `sessions_spawn` with `runtime: "acp"`.',
      ...
      "CLI lifecycle only on explicit user request: `zuvix gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...skillWorkshopSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## Zuvix Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, Zuvix pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by Zuvix and included below in Project Context.",
      "",
