...
let prepareZuvixModelsJsonSourceMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    prepareZuvixModelsJsonSourceMock = vi.fn().mockResolvedValue({
      agentDir: "/tmp/zuvix",
    ...
    vi.doMock("./models-config.js", () => ({
      ...
      prepareZuvixModelsJsonSource: prepareZuvixModelsJsonSourceMock,
    }));
    ...
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: ZuvixConfig, agentId: string) => {
        const entry = cfg.agents?.list?.find((entryEntry) => entryEntry.id === agentId);
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/zuvix-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/zuvix",
      resolveDefaultAgentId: (cfg: ZuvixConfig) =>
  ...
  beforeEach(() => {
    ...
    prepareZuvixModelsJsonSourceMock.mockReset();
    prepareZuvixModelsJsonSourceMock.mockResolvedValue({
      agentDir: "/tmp/zuvix",
      fingerprint: "source-fingerprint",
      workspaceDir: "/tmp/zuvix-workspace",
      wrote: false,
  ...
  it("uses the state cached catalog before runtime discovery", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as ZuvixConfig });
    ...
    expect(prepareZuvixModelsJsonSourceMock).not.toHaveBeenCalled();
  ...
  it("bypasses the state cached catalog when a refresh is requested", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as ZuvixConfig, useCache: false });
  ...
  it("misses the state cached catalog when source freshness changes", async () => {
    ...
    await expect(loadModelCatalog({ config: {} as ZuvixConfig })).resolves.toEqual([
      { id: "cached-stale", name: "Cached Stale", provider: "openai" },
    ]);
    resetModelCatalogCacheForTest();
    mockAgentDiscoveryModels([{ id: "fresh-fast", name: "Fresh Fast", provider: "openai" }]);
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as ZuvixConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as ZuvixConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as ZuvixConfig, readOnly: true });
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/zuvix/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/zuvix/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/zuvix/plugins/read-only-shard", { recursive: true, force: true });
    }
  });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as ZuvixConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/zuvix-model-catalog-env" } as NodeJS.ProcessEnv;
