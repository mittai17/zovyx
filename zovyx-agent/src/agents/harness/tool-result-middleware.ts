...
type MiddlewareContentBlock = ZuvixAgentToolResult["content"][number];
...
function isValidMiddlewareToolResult(value: unknown): value is ZuvixAgentToolResult {
...
function coerceMiddlewareToolResult(
...
): ZuvixAgentToolResult | undefined {
  ...
  const content: ZuvixAgentToolResult["content"] = [];
 ...
 */
function sanitizeToolResultForMiddleware(result: ZuvixAgentToolResult): ZuvixAgentToolResult {
...
function buildMiddlewareFailureResult(): ZuvixAgentToolResult {
  return {
    content: [
      {
        type: "text",
