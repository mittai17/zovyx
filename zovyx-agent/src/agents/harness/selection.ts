...
import { createZuvixAgentHarness } from "./builtin-zuvix.js";
...
type AgentHarnessSelectionDecision = {
  ...
  selectedReason:
    | "forced_zuvix"
    | "forced_plugin"
    // Implicit Codex preference found no registered Codex harness, so Zuvix handled the run.
    | "implicit_plugin_unavailable_zuvix"
    // Provider-owned CLI runtime aliases have no agent harness plugin counterpart.
    | "cli_runtime_passthrough_zuvix"
    // Auto mode chose a registered plugin harness that supports the provider/model.
    | "auto_plugin"
    // Auto mode found no supporting plugin harness, so Zuvix handled the run.
    | "auto_zuvix";
...
function selectAgentHarnessDecision(params: {
  ...
  const policy =
    runtimeOverride && !isDefaultAgentRuntimeId(runtimeOverride)
      ...
      : resolvedPolicy;
  // Zuvix's built-in harness is intentionally not part of the plugin candidate list. Explicit plugin
  // runtimes fail closed; only `auto` may route an unmatched turn to Zuvix.
  const pluginHarnesses = listPluginAgentHarnesses();
  const zuvixHarness = createZuvixAgentHarness();
  const runtime = policy.runtime;
  if (runtime === "zuvix") {
    return buildSelectionDecision({
      harness: zuvixHarness,
      policy,
      selectedReason: "forced_zuvix",
  ...
  if (runtime !== "auto") {
    ...
    if (forced) {
      ...
      if (isCliRuntimeAliasForProvider({ runtime, provider: params.provider })) {
        return buildSelectionDecision({
          ...
          selectedReason: "cli_runtime_passthrough_zuvix",
          candidates: listHarnessCandidates(pluginHarnesses),
        });
      }
      throw new Error(
    ...
    if (runtime === "codex" && policy.runtimeSource === "implicit") {
      return buildSelectionDecision({
        ...
        selectedReason: "implicit_plugin_unavailable_zuvix",
        candidates: listHarnessCandidates(pluginHarnesses),
      });
    }
    if (
...
export async function runAgentHarnessAttempt(
  ...
  const attemptParams =
    harness.id === "zuvix" ? params : applyPluginHarnessDenyAllToolPolicy(params);
  ...
  try {
    ...
    log.warn(`${harness.label} failed; not falling back to embedded Zuvix backend`, {
