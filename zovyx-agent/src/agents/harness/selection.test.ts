...
vi.mock("./builtin-zuvix.js", () => ({
  createZuvixAgentHarness: (): AgentHarness => ({
    id: "zuvix",
...
const originalRuntime = process.env.ZUVIX_AGENT_RUNTIME;
...
afterEach(() => {
  ...
  if (originalRuntime == null) {
    delete process.env.ZUVIX_AGENT_RUNTIME;
  } else {
    process.env.ZUVIX_AGENT_RUNTIME = originalRuntime;
  }
});

function createAttemptParams(config?: ZuvixConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
function groupSenderDenyAllConfig(): ZuvixConfig {
...
function groupDenyAllConfig(): ZuvixConfig {
  return {
    channels: {
      telegram: {
        groups: {
...
function providerRuntimeConfig(provider: string, runtime: string): ZuvixConfig {
  return {
    models: {
      providers: {
        [provider]: {
...
describe("runAgentHarnessAttempt", () => {
  it("fails when a forced plugin harness is unavailable and fallback is omitted", async () => {
    process.env.ZUVIX_AGENT_RUNTIME = "codex";
  ...
  it("falls back to the Zuvix harness in auto mode when no plugin harness matches", async () => {
    const result = await runAgentHarnessAttempt(createAttemptParams());

    expect(result.sessionIdUsed).toBe("zuvix");
    expect(agentRunAttempt).toHaveBeenCalledTimes(1);
  });

  it("allows the selected Zuvix harness to satisfy context-engine pre-prompt assembly", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("codex", "zuvix")),
  ...
  it("surfaces an auto-selected plugin harness failure instead of replaying through Zuvix", async () => {
  ...
  it("surfaces a forced plugin harness failure instead of replaying through Zuvix", async () => {
    registerFailingCodexHarness();
  ...
  it("falls back to Zuvix when the implicit OpenAI Codex harness is unavailable", async () => {
    expect(resolveAgentHarnessPolicy({ provider: "openai", modelId: "gpt-5.4" })).toEqual({
      runtime: "codex",
      runtimeSource: "implicit",
    });
  ...
  it("honors explicit Zuvix runtime for OpenAI agent model runs", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("openai", "zuvix")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("zuvix");
  ...
  it("honors provider wildcard Zuvix runtime policy for OpenAI agent model runs", async () => {
    registerSuccessfulCodexHarness();

    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(agentModelRuntimeConfig("openai/*", "zuvix")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("zuvix");
  ...
  it("leaves Zuvix harness params unchanged for channel group sender deny-all policy", async () => {
    await runAgentHarnessAttempt({
      ...createAttemptParams(groupSenderDenyAllConfig()),
      sessionKey: "agent:main:telegram:group:test-deny-room",
      messageProvider: "telegram",
  ...
  it("does not let a strict agent model plugin runtime fall back to Zuvix", async () => {
    await expect(
      runAgentHarnessAttempt({
...
describe("selectAgentHarness", () => {
  ...
  it("ignores session-level Zuvix pins when selecting a harness", () => {
    const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
    registerAgentHarness({
      id: "codex",
      label: "Codex",
  ...
  it("honors explicit Zuvix runtime overrides when selecting a harness", async () => {
    registerSuccessfulCodexHarness();

    const harness = selectAgentHarness({
      provider: "openai",
      modelId: "gpt-5.4",
      agentHarnessRuntimeOverride: "zuvix",
    });

    expect(harness.id).toBe("zuvix");
  ...
  it("treats legacy PI runtime overrides as the built-in Zuvix harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("selects Zuvix when the implicit OpenAI Codex harness is unavailable", () => {
    expect(selectAgentHarness({ provider: "openai", modelId: "gpt-5.4" }).id).toBe("zuvix");
  });
  ...
  it("ignores existing session Zuvix pins when provider policy forces a plugin harness", () => {
    registerFailingCodexHarness();
  ...
  it("ignores env-forced Zuvix for OpenAI default runtime selection", () => {
    process.env.ZUVIX_AGENT_RUNTIME = "zuvix";
    registerFailingCodexHarness();
      ...
      maybeCompactAgentHarnessSession({
        ...
        config: {
          agents: {
            ...
            defaults: {
              models: {
                "openai/gpt-5.5": { agentRuntime: { id: "zuvix" } },
              },
            },
          },
        } as ZuvixConfig,
  ...
  it("keeps compaction recoverable when auth profile lookup fails", async () => {
    ...
    await expect(
      maybeCompactAgentHarnessSession({
        ...
        agentHarnessId: "codex",
        config: agentModelRuntimeConfig("openai/gpt-5.5", "zuvix"),
      }),
    ).resolves.toEqual({ ok: true, compacted: false });
    expect(compact).toHaveBeenCalledTimes(1);
  ...
  it("does not compact a selected plugin harness through Zuvix when the plugin has no compactor", async () => {
    registerFailingCodexHarness();
  ...
  it.each([
    ...
    "returns Zuvix for explicit CLI runtime alias $alias on $provider instead of throwing MissingAgentHarnessError",
