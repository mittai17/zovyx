...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("zuvix.nativeHookRelay.state");
        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by Zuvix",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveZuvixCliExecutable();
  const argv =
    executable === "zuvix"
      ? ["zuvix"]
...
function registerNativeHookRelayBridge(registration: ActiveNativeHookRelayRegistration): void {
  ...
  // directory is scoped by OS user (uid) and is shared across all Zuvix
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `zuvix-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("zuvix:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.zuvix_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `zuvix-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveZuvixCliExecutable(): string {
  const envPath = process.env.ZUVIX_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "zuvix.mjs"),
  ...
  throw new Error("Cannot resolve Zuvix CLI executable path for native hook relay");
}
