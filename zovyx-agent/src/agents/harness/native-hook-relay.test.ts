...
async function writeForeignNativeHookRelayBridgeRecordForTests(
  ...
  // without starting a second Zuvix process in the unit test.
...
function getNativeHookRelaySharedStateForTests(): NativeHookRelaySharedStateForTests {
  ...
  )[Symbol.for("zuvix.nativeHookRelay.state")];
  if (!state) {
...
describe("native hook relay registry", () => {
  it("registers a short-lived relay and builds hidden CLI commands", () => {
    const relay = registerNativeHookRelay({
      ...
      ttlMs: 10_000,
      command: {
        executable: "/opt/Open Claw/zuvix.mjs",
    ...
    expect(relay.commandForEvent("pre_tool_use")).toBe(
      "/usr/local/bin/node '/opt/Open Claw/zuvix.mjs' hooks relay --provider codex --relay-id " +
  ...
  it("uses the Codex no-op output when no Zuvix hook decides", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      runId: "run-1",
  ...
  it("maps Codex PreToolUse to Zuvix before_tool_call and blocks before execution", async () => {
  ...
  it("normalizes Codex exec_command cmd input before running Zuvix policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("normalizes Codex exec_command argv cmd input before running Zuvix policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("passes config to trusted policies for native pre-tool session extension reads", async () => {
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "zuvix-native-relay-policy-"));
  ...
  it("uses the Codex cwd when deriving apply_patch paths for PreToolUse", async () => {
    ...
    const cwd = path.join("/tmp", "zuvix-native-hook-cwd");
  ...
  it("maps Codex PostToolUse to Zuvix after_tool_call observation", async () => {
  ...
  it("maps Codex MCP PreToolUse to Zuvix before_tool_call and can block", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          entities: [{ name: "Zuvix", entityType: "project", observations: ["test"] }],
        },
      },
    });
  ...
  it("lets security-style plugins block native MCP calls by scanning tool params", async () => {
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          command: "rm -rf /tmp/zuvix-important-state",
        },
      },
    });
  ...
  it("maps Codex MCP PostToolUse to Zuvix after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_response: {
          content: [{ type: "text", text: '{ "name": "zuvix" }' }],
  ...
  it("routes Codex MCP PermissionRequest payloads through Zuvix approval policy", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      agentId: "agent-1",
      sessionId: "session-1",
  ...
  it("defers PermissionRequest when Zuvix approval does not decide", async () => {
  ...
  it("does not reuse pending PermissionRequest approvals when a tool call id is reused with different input", async () => {
    ...
    const second = invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_name: "Bash",
        tool_use_id: "reused-call-id",
        tool_input: { command: "rm -rf /tmp/zuvix-important-state" },
      },
    });
...
describe("native hook relay command builder", () => {
  it("uses the Codex hook relay command shape", () => {
    expect(
      ...
      "zuvix hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event permission_request --timeout 5000",
    );
  });
  ...
  it("includes explicit unavailable noop mode only for PreToolUse", () => {
    expect(
      ...
      "zuvix hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event pre_tool_use --pre-tool-use-unavailable noop --timeout 5000",
    );
  });
});
