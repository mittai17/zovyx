...
function cfg(mode: "tools" | "off", patterns?: string[]): ZuvixConfig {
  return {
    logging: {
...
describe("redactTranscriptMessage", () => {
  ...
  it("preserves OpenAI encrypted reasoning inside thinkingSignature", () => {
    ...
    const replayItem = JSON.parse(block.thinkingSignature) as {
      ...
      summary: unknown[];
      content?: unknown[];
      __zuvix_replay: Record<string, unknown>;
    };
    const blockMetadata = (block as unknown as { zuvixReasoningReplay: Record<string, unknown> })
      .zuvixReasoningReplay;
    ...
    expect(replayItem["__zuvix_replay"]).toEqual(OPENAI_REASONING_REPLAY_METADATA);
  ...
  it.each([
    ...
    {
      api: "zuvix-openai-completions-transport",
      provider: "google",
      block: {
        type: "toolCall",
        id: "call_1",
  ...
  it.each(["openai-responses", "zuvix-openai-responses-transport"])(
  ...
  it("redacts nested strings in assistant tool-call arguments", () => {
      ...
      content: [
        {
          ...
          name: "shell",
          arguments: {
            command: "OPENAI_API_KEY=sk-abcdef1234567890xyz zuvix health",
    ...
    expect(argumentsValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz zuvix health");
    expect(argumentsValue.env.nested[0]).toBe("token sk-abc…0xyz");
    expect(argumentsValue.count).toBe(1);
    expect(serializedArguments).toContain("zuvix health");
  ...
  it("redacts structured tool-use input payloads", () => {
    ...
    expect(inputValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz zuvix health");
