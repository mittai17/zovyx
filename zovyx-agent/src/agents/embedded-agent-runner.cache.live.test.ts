...
import type { AssistantMessage, Message, Tool } from "zuvix/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.ZUVIX_CACHE_TRACE,
      file: process.env.ZUVIX_CACHE_TRACE_FILE,
      messages: process.env.ZUVIX_CACHE_TRACE_MESSAGES,
      prompt: process.env.ZUVIX_CACHE_TRACE_PROMPT,
      system: process.env.ZUVIX_CACHE_TRACE_SYSTEM,
    };
    process.env.ZUVIX_CACHE_TRACE = "1";
    process.env.ZUVIX_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.ZUVIX_CACHE_TRACE_MESSAGES = "0";
    process.env.ZUVIX_CACHE_TRACE_PROMPT = "0";
    process.env.ZUVIX_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("ZUVIX_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("ZUVIX_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("ZUVIX_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("ZUVIX_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("ZUVIX_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "ZUVIX_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "ZUVIX_LIVE_ANTHROPIC_CACHE_MODEL",
