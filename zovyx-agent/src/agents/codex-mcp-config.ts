...
function isZuvixLoopbackMcpServer(name: string, server: BundleMcpServerConfig): boolean {
  return (
    name === "zuvix" &&
...
export function normalizeCodexMcpServerConfig(
  ...
  } else if (isZuvixLoopbackMcpServer(name, server)) {
    // Zuvix's loopback MCP exposes local tools; Codex should ask for approval
