...
const createZuvixTransportStreamFnForModel = vi.fn();
...
describe("prepareModelForSimpleCompletion", () => {
  ...
  beforeEach(() => {
    ...
    createZuvixTransportStreamFnForModel.mockReset();
    ...
    createZuvixTransportStreamFnForModel.mockReturnValue(undefined);
  ...
  it("routes provider-owned simple-completion wrappers through an internal API alias", () => {
    ...
    expect(result.api).toBe(
      "zuvix-provider-simple:moonshot:kimi-k2.7-code:moonshot-simple-source:https%3A%2F%2Fapi.moonshot.ai%2Fv1",
    );
  ...
  it("uses a custom api alias for Anthropic Vertex simple completions", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "zuvix-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
      "vertex-stream",
    );
    expect(result).toEqual({
      ...model,
      api: "zuvix-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
    });
  });
  ...
  it("uses a transport-aware custom api alias when llm request transport overrides are present", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "zuvix-openai-responses-transport",
      "transport-stream",
    );
    expect(result).toEqual({
      ...model,
  ...
  it("uses the Google simple-completion sanitizer alias after transport checks pass through", () => {
    ...
    prepareGoogleSimpleCompletionModel.mockImplementationOnce((m: unknown) => ({
      ...
      api: "zuvix-google-generative-ai-simple",
    }));
    resolveProviderStreamFn.mockReturnValueOnce(undefined);
  ...
  it.each([
    ...
    "uses Zuvix transport for OpenAI Codex-response simple completions with baseUrl %s",
    (baseUrl, expectedBaseUrl) => {
      ...
      resolveProviderStreamFn.mockReturnValueOnce(undefined);
      createZuvixTransportStreamFnForModel.mockReturnValueOnce("codex-transport-stream");
      resolveTransportAwareSimpleApi.mockReturnValueOnce("zuvix-openai-responses-transport");
      ...
      expect(createZuvixTransportStreamFnForModel).toHaveBeenCalledWith(
        {
          ...model,
          baseUrl: expectedBaseUrl,
        },
