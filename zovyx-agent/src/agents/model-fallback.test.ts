    ...
    plugins: [
      {
        ...
        manifestPath: `/tmp/fallback-normalizer-${params.manifestHash}/zuvix.plugin.json`,
...
function makeFallbacksOnlyCfg(): ZuvixConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): ZuvixConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "zuvix-auth-suite-mock");
...
// https://github.com/zuvix/zuvix/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';

describe("runWithModelFallback", () => {
  it("uses the opt-in auth skip cache on the second turn for the same session", async () => {
    const previous = process.env.ZUVIX_FALLBACK_SKIP_TTL_MS;
    process.env.ZUVIX_FALLBACK_SKIP_TTL_MS = "60000";
    try {
      const cfg = makeCfg({
        agents: {
          defaults: {
      ...
      if (previous === undefined) {
        delete process.env.ZUVIX_FALLBACK_SKIP_TTL_MS;
      } else {
        process.env.ZUVIX_FALLBACK_SKIP_TTL_MS = previous;
      }
    }
  });
  ...
  it("resolves primary model aliases before running", () => {
    const cases = [
      ...
      },
      {
          ...
          agents: {
            defaults: {
              ...
              models: {
                ...
                "zuvix-go/deepseek-v4-pro": { alias: "Zuvix Go DeepSeek V4 Pro" },
              },
            },
          },
        }),
        ...
        expected: ["zuvix-go", "deepseek-v4-pro"],
      },
    ] satisfies Array<{
      name: string;
      cfg: ZuvixConfig;
  ...
  it("does not prepare agent harness plugins for forced Zuvix candidates", async () => {
    const cfg = makeCfg({
      models: {
        providers: {
          openai: {
    ...
    const prepareAgentHarnessRuntime = vi.fn(() => {
      throw new Error("Zuvix candidates should not prepare plugin harnesses");
    });
    const run = vi.fn().mockResolvedValueOnce("ok");
  ...
  it("does not prepare agent harness plugins for forced Zuvix runtime candidates", async () => {
    const cfg = makeCfg({
      models: {
        providers: {
          openai: {
  ...
  it("aborts the fallback chain on session write-lock timeout instead of trying every model (#83510)", async () => {
    ...
    const lockError = new SessionWriteLockTimeoutError({
      timeoutMs: 10_000,
      owner: "pid=37121",
      lockPath: "/tmp/zuvix/session.jsonl.lock",
    });
  ...
  it("warns when falling back due to model_not_found", async () => {
    const warnLogs = createWarnLogCapture("zuvix-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
