...
const state = vi.hoisted(() => ({
  cfg: undefined as ZuvixConfig | undefined,
...
vi.mock("./agent-scope.js", async () => {
  ...
  return {
    ...
    resolveAgentDir: () => state.agentDir ?? "/tmp/zuvix-agent",
    resolveDefaultAgentId: () => "main",
    resolveEffectiveModelFallbacks: () => undefined,
    resolveSessionAgentId: () => "main",
    resolveAgentWorkspaceDir: () => state.workspaceDir ?? "/tmp/zuvix-workspace",
  };
});
...
beforeEach(async () => {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-rotation-e2e-"));
