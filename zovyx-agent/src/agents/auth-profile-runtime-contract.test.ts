...
function providerRuntimeConfig(provider: string, runtime: string): ZuvixConfig {
  return {
    models: {
      providers: {
        [provider]: {
          baseUrl: "https://api.zuvix.test/v1",
...
async function runAuthContractAttempt(params: {
  ...
  const cfg = params.cfg ?? ({} as ZuvixConfig);
...
describe("Auth profile runtime contract - embedded Zuvix and CLI adapter", () => {
  let tmpDir: string;
  let storePath: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-auth-contract-"));
  ...
  it("forwards a legacy OpenAI Codex auth profile through the embedded Zuvix path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
  ...
  it("forwards an OpenAI auth profile through the explicit embedded OpenAI Zuvix path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      ...
      authProfileProvider: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
      authProfileOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProfileId,
      cfg: providerRuntimeConfig(AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider, "zuvix"),
    });
  ...
  it("routes explicit OpenAI Zuvix runs with legacy Codex OAuth through OpenAI transport", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      providerOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
