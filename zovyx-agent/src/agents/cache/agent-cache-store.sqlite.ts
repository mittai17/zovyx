...
import {
  ...
  type ZuvixAgentDatabaseOptions,
...
export type SqliteAgentCacheStoreOptions = ZuvixAgentDatabaseOptions & {
  scope: string;
  now?: () => number;
};
...
type CacheEntriesTable = ZuvixAgentKyselyDatabase["cache_entries"];
type AgentCacheDatabase = Pick<ZuvixAgentKyselyDatabase, "cache_entries">;
...
function toDatabaseOptions(options: SqliteAgentCacheStoreOptions): ZuvixAgentDatabaseOptions {
  return {
...
export function writeSqliteAgentCacheEntry(
  ...
  runZuvixAgentWriteTransaction((database) => {
...
export function readSqliteAgentCacheEntry(
  ...
  const scope = normalizeScope(options);
  const key = normalizeKey(options.key);
  const database = openZuvixAgentDatabase(toDatabaseOptions(options));
  const db = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
  const row =
    executeSqliteQueryTakeFirstSync(
      database.db,
...
export function deleteSqliteAgentCacheEntry(
  ...
  const scope = normalizeScope(options);
  const key = normalizeKey(options.key);
  return runZuvixAgentWriteTransaction((database) => {
    const db = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
...
/** Agent runtime cache store implementation backed by Zuvix's SQLite DB. */
