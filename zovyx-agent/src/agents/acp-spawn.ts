...
} from "@zuvix/acp-core/runtime/session-identifiers";
import type { AcpRuntimeSessionMode } from "@zuvix/acp-core/runtime/types";
import {
...
function resolveHeartbeatConfigForAgent(params: {
  cfg: ZuvixConfig;
  agentId: string;
}): NonNullable<NonNullable<ZuvixConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an Zuvix config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for Zuvix config agents. ' +
          'Use runtime="acp" only with external ACP harness ids such as codex, claude, droid, gemini, or zuvix, or configure agents.list[].runtime.type="acp" with runtime.acp.agent.',
      };
    }
    return {
      ok: true,
...
function isExplicitlyAllowedAcpAgent(cfg: ZuvixConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: ZuvixConfig): string[] {
