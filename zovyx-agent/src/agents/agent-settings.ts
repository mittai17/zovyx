...
export function resolveCompactionReserveTokensFloor(cfg?: ZuvixConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: ZuvixConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers Zuvix runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (zuvix#75799).
 *
 ...
 * `ollama/glm-*`, `zuvix-go/glm-*`) are NOT included because their hosts
...
/**
 * Disable Zuvix runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when Zuvix or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (zuvix#75799).
 * Default-mode runs against ordinary providers keep Zuvix runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores Zuvix runtime's
