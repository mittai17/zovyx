 ...
 */
import { resolveIntegerOption } from "@zuvix/normalization-core/number-coercion";
...
import { isTranscriptOnlyZuvixAssistantModel } from "../shared/transcript-only-zuvix-assistant.js";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000ZUVIX_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[Zuvix persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[Zuvix persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[Zuvix persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[Zuvix persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[Zuvix persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyZuvixAssistantMessage(message: AgentMessage): boolean {
  ...
  return isTranscriptOnlyZuvixAssistantModel(provider, model);
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyZuvixAssistantMessage(nextMessage);
    if (
