...
describe("tool mutation helpers", () => {
  ...
  it.each([
    ...
    ["exec", "gh search prs --repo zuvix/zuvix tool-mutation --json number,title,state"],
    ["bash", "gh pr view 123 --repo zuvix/zuvix --json title,state"],
  ...
  it.each([
    ...
    ["exec", "gh api --method POST repos/zuvix/zuvix/issues"],
