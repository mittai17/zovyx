...
import { isPassThroughRemoteMediaSource } from "@zuvix/media-core/media-source-url";
...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const zuvixTmpDir = path.resolve(resolvePreferredZuvixTmpDir());
  if (!isPathInside(zuvixTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: zuvixTmpDir });
  return resolved;
}
