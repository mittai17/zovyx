...
describe("prepareSimpleCompletionModel", () => {
  ...
  it("applies provider runtime auth before storing simple-completion credentials", async () => {
    ...
    expect(runtimeAuthInput.workspaceDir).toBe("/tmp/zuvix-agent");
...
describe("completeWithPreparedSimpleCompletionModel", () => {
  ...
  it("normalizes Zuvix-only thinking levels before using shared model runtime simple completion", async () => {
    const model = {
      provider: "openai",
      id: "gpt-5.4",
      name: "gpt-5.4",
