...
describe("external CLI auth scope", () => {
  ...
  it("scopes zuvix-only config without adding unrelated CLI providers", () => {
    const scope = resolveExternalCliAuthScopeFromConfig({
      auth: {
        profiles: {
          "zuvix-go:default": { provider: "zuvix-go", mode: "api_key" },
        },
      },
      agents: {
        defaults: {
          model: { primary: "zuvix-go/kimi-k2.6" },
        },
      },
      models: {
        providers: {
    ...
    expect(scope?.providerIds).toContain("zuvix-go");
    expect(scope?.profileIds).toEqual(["zuvix-go:default"]);
    ...
    const cfg = {
      ...
      agents: {
        ...
        list: [
          {
            id: "worker",
            model: "zuvix-go/kimi-k2.6",
            models: {
              "zuvix-go/kimi-k2.6": { agentRuntime: { id: "codex-app-server" } },
            },
