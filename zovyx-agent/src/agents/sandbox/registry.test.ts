...
const {
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "zuvix-sandbox-registry-"));
  const previousStateDir = process.env.ZUVIX_STATE_DIR;
  process.env.ZUVIX_STATE_DIR = baseDir;

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_ZUVIX_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_ZUVIX_STATE_DIR === undefined) {
    delete process.env.ZUVIX_STATE_DIR;
  } else {
    process.env.ZUVIX_STATE_DIR = PREVIOUS_ZUVIX_STATE_DIR;
  }
});
