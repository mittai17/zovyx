...
const CDP_SOURCE_RANGE_ENV_KEY = "ZUVIX_BROWSER_CDP_SOURCE_RANGE";
const CDP_AUTH_TOKEN_ENV_KEY = "ZUVIX_BROWSER_CDP_AUTH_TOKEN";
const SANDBOX_BROWSER_IMAGE_CONTRACT_LABEL = "org.zuvix.sandbox-browser.contract";

function buildSandboxCdpAuthHeader(token: string): string {
  return `Basic ${Buffer.from(`zuvix:${token}`).toString("base64")}`;
}
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_ZUVIX_BROWSER_PROFILE_NAME,
    extraArgs: [],
    tabCleanup: {
      enabled: true,
      idleMinutes: 120,
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "zuvix.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `zuvix sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `zuvix sandbox recreate --browser --agent ${agentId}`;
          }
          return "zuvix sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        "zuvix.sandboxBrowser": "1",
        "zuvix.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `ZUVIX_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `ZUVIX_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `ZUVIX_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `${CDP_AUTH_TOKEN_ENV_KEY}=${cdpAuthToken}`);
    args.push(
      "-e",
      `ZUVIX_BROWSER_AUTO_START_TIMEOUT_MS=${params.cfg.browser.autoStartTimeoutMs}`,
    );
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `ZUVIX_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `ZUVIX_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    args.push("-e", "ZUVIX_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_ZUVIX_BROWSER_PROFILE_NAME)
    : null;
