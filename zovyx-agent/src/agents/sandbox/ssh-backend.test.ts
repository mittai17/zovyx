...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "zuvix-test-ssh-config"),
    host: "zuvix-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "zuvix-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "zuvix-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/zuvix");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/zuvix/zuvix-ssh-agent-worker");
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const skillsWorkspaceDir = await createTempDir("zuvix-ssh-skills-");
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/zuvix/zuvix-ssh-agent-worker");
    ...
    expect(skillsUploadParams.remoteDir).toContain("/workspace/.zuvix/sandbox-skills");
  ...
  it("clears stale remote materialized skills when the local copy is missing", async () => {
    const tmpDir = await createTempDir("zuvix-ssh-skills-");
    ...
    expect(commandParams.remoteCommand).toContain("zuvix-sandbox-clear");
    expect(commandParams.remoteCommand).toContain("/workspace/.zuvix/sandbox-skills");
    await backend.finalizeExec?.({
      status: "completed",
      exitCode: 0,
      timedOut: false,
