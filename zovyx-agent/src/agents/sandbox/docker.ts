...
import { markZuvixExecEnv } from "../../infra/zuvix-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; Zuvix will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "zuvix.sandbox=1");
  args.push("--label", `zuvix.sessionKey=${params.scopeKey}`);
  args.push("--label", `zuvix.createdAtMs=${createdAtMs}`);
  args.push("--label", `zuvix.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `zuvix.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markZuvixExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "zuvix.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`zuvix sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`zuvix sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("zuvix sandbox recreate --all");
}
