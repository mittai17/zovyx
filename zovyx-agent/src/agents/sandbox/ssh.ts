...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "zuvix-sandbox-ssh-"));
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredZuvixTmpDir() ?? os.tmpdir());
}
