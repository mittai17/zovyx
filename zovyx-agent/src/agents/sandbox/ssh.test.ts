...
describe("sandbox ssh helpers", () => {
  it("materializes inline ssh auth data into a temp config", async () => {
    ...
    expect(config).toContain("Host zuvix-sandbox");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-ssh-real-"));
      ...
      const target = path.join(root, "workspace", ".zuvix", "sandbox-skills");
      ...
      await expect(
        fs.stat(path.join(realParent, "runtime", "workspace", ".zuvix", "sandbox-skills")),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.symlink(realParent, path.join(root, "workspace", ".zuvix"));
      ...
      await expect(
        execFileAsync("/bin/sh", [
          ...
          ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
          "zuvix-remote-dir",
          path.join(root, "workspace", ".zuvix", "sandbox-skills"),
          root,
        ]),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-ssh-upload-"));
      ...
      await expect(
        uploadDirectoryToSshTarget({
          session: {
            command: "ssh",
            configPath: "/tmp/zuvix-test-ssh-config",
            host: "zuvix-sandbox",
          },
          localDir,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-ssh-upload-safe-"));
      tempDirs.push(localDir);
