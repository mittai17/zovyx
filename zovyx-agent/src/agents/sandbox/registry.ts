...
type SandboxRegistryTable = ZuvixStateKyselyDatabase["sandbox_registry_entries"];
type SandboxRegistryDatabase = Pick<ZuvixStateKyselyDatabase, "sandbox_registry_entries">;
...
function readRegistryRows(kind: SandboxRegistryKind): SandboxRegistryRow[] {
  const { db } = openZuvixStateDatabase();
