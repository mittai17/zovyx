...
describe("getBlockedBindReason", () => {
  ...
  it("still blocks OS-home credential paths when ZUVIX_HOME points elsewhere", () => {
    withEnv({ HOME: "/home/tester", ZUVIX_HOME: "/srv/zuvix-home" }, () => {
  ...
  it("blocks canonical OS-home aliases for credential paths", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "zuvix-home-"));
...
describe("validateBindMounts", () => {
  it("allows legitimate project directory mounts", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "zuvix-sbx-safe-"));
    expect(
      validateBindMounts([
  ...
  it("allows drive-absolute Windows bind sources", () => {
    expect(validateBindMounts(["D:/data/zuvix/src:/src:ro"])).toBeUndefined();
    expect(validateBindMounts(["D:\\data\\zuvix\\output:/output:rw"])).toBeUndefined();
  });

  it("compares Windows allowed roots case-insensitively", () => {
    expect(
      validateBindMounts(["d:/DATA/Zuvix/src:/src:ro"], {
        allowedSourceRoots: ["D:/data/zuvix"],
      }),
    ).toBeUndefined();
  ...
  it("blocks symlink escapes into blocked directories", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "zuvix-sbx-"));
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "zuvix-not-created");
    expect(() =>
  ...
  it("blocks bind sources outside allowed roots when allowlist is configured", () => {
    const allowedRoot = mkdtempSync(join(tmpdir(), "zuvix-sbx-allowed-root-"));
    const externalRoot = mkdtempSync(join(tmpdir(), "zuvix-sbx-external-"));
    expect(() =>
  ...
  it("allows bind sources in allowed roots when allowlist is configured", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "zuvix-sbx-allowed-"));
    expect(
  ...
  it("blocks reserved container target paths by default", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "zuvix-sbx-reserved-default-"));
    expect(() =>
  ...
  it("allows reserved container target paths with explicit dangerous override", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "zuvix-sbx-reserved-"));
    expect(
...
describe("validateApparmorProfile", () => {
  it("allows named profile/undefined", () => {
    expect(validateApparmorProfile("zuvix-sandbox")).toBeUndefined();
...
describe("validateSandboxSecurity", () => {
  it("passes with safe config", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "zuvix-sbx-safe-config-"));
    expect(
      validateSandboxSecurity({
