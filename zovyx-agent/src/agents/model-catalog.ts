...
import { resolveClaudeFable5ModelIdentity } from "@zuvix/llm-core";
...
let manifestModelCatalogCache = new WeakMap<ZuvixConfig, ManifestModelCatalogCacheEntry>();
...
function shouldLogModelCatalogTiming(): boolean {
  return process.env.ZUVIX_DEBUG_INGRESS_TIMING === "1";
}
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg: ZuvixConfig): boolean {
...
export async function loadModelCatalog(params?: {
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      if (!readOnly) {
        const preparedSource = await prepareZuvixModelsJsonSource(cfg, agentDir, {
          pluginMetadataSnapshot: params?.metadataSnapshot,
          workspaceDir,
        });
