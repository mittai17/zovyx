...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "ZUVIX_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { ZUVIX_LIVE_MODEL_IMAGE_PROBE: "false" },
        "ZUVIX_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { ZUVIX_LIVE_MODEL_IMAGE_PROBE: "1" },
        "ZUVIX_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
  ...
  it("skips known stale file probe routes", () => {
    ...
    expect(shouldSkipLiveModelFileProbe({ provider: "zuvix-go", id: "glm-5" })).toBe(true);
    expect(shouldSkipLiveModelFileProbe({ provider: "google", id: "gemini-3.1-pro-preview" })).toBe(
      true,
    );
    expect(shouldSkipLiveModelFileProbe({ provider: "zuvix-go", id: "mimo-v2-omni" })).toBe(
      true,
    );
    expect(shouldSkipLiveModelFileProbe({ provider: "zuvix-go", id: "mimo-v2-pro" })).toBe(true);
    expect(shouldSkipLiveModelFileProbe({ provider: "zuvix-go", id: "minimax-m2.5" })).toBe(
      true,
    );
    expect(
    ...
    expect(shouldSkipLiveModelFileProbe({ provider: "zuvix-go", id: "kimi-k2.5" })).toBe(true);
  ...
  it("skips known stale image probe routes", () => {
    ...
    expect(shouldSkipLiveModelImageProbe({ provider: "zuvix-go", id: "mimo-v2-omni" })).toBe(
      true,
    );
    expect(shouldSkipLiveModelImageProbe({ provider: "zuvix-go", id: "kimi-k2.5" })).toBe(true);
    expect(
      shouldSkipLiveModelImageProbe({
        provider: "google",
    ...
    expect(shouldSkipLiveModelImageProbe({ provider: "zuvix", id: "kimi-k2.6" })).toBe(true);
    expect(
