...
import type { UserMessage } from "zuvix/plugin-sdk/llm";
...
vi.mock("zuvix/plugin-sdk/agent-sessions", async () => {
  const actual = await vi.importActual<typeof import("zuvix/plugin-sdk/agent-sessions")>(
    "zuvix/plugin-sdk/agent-sessions",
  );
  return {
    ...actual,
