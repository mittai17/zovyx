...
function setCliBackendMetadataSnapshot(cliBackends: string[]) {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "anthropic",
        manifestPath: "/tmp/anthropic/zuvix.plugin.json",
...
describe("isCliProvider", () => {
  ...
  it("returns true for setup-registered cli backends", () => {
    expect(isCliProvider("claude-cli", {} as ZuvixConfig)).toBe(true);
  });

  it("returns false for provider ids", () => {
    expect(isCliProvider("example-cli", {} as ZuvixConfig)).toBe(false);
  });
  ...
  it("does not execute setup runtime when descriptor metadata has no matching backend", () => {
    ...
    expect(isCliProvider("openai", {} as ZuvixConfig)).toBe(false);
  });
});
