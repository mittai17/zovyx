...
import { __setFsSafeTestHooksForTest } from "@zuvix/fs-safe/test-hooks";
...
const parseZuvixChannelsLoginShellCommand = testing.parseZuvixChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("zuvix-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive Zuvix channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseZuvixChannelsLoginShellCommand("zuvix channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseZuvixChannelsLoginShellCommand(
        "pnpm exec zuvix channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseZuvixChannelsLoginShellCommand("zuvix channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-zuvix-channel-login", {
        command: "zuvix channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive Zuvix channel login commands/);
    await expect(
      tool.execute("call-wrapped-zuvix-channel-login", {
        command: "sudo -u zuvix bash -lc 'zuvix channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Zuvix channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'zuvix channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Zuvix channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env zuvix channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Zuvix channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'zuvix channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Zuvix channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("zuvix-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("zuvix-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("zuvix-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("zuvix-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__zuvix_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
