/**
 * Zuvix built-in and plugin tool assembly.
 *
...
import { resolveZuvixPluginToolsForOptions } from "./zuvix-plugin-tools.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./zuvix-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./zuvix-tools.nodes-workspace-guard.js";
import {
  collectPresentZuvixTools,
  shouldIncludeUpdatePlanToolForZuvixTools,
} from "./zuvix-tools.registration.js";
...
const defaultZuvixToolsDeps: ZuvixToolsDeps = {
  callGateway,
};

let zuvixToolsDeps: ZuvixToolsDeps = defaultZuvixToolsDeps;

export function createZuvixTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? zuvixToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("zuvix-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("zuvix-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("zuvix-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("zuvix-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("zuvix-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("zuvix-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("zuvix-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("zuvix-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("zuvix-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("zuvix-tools:nodes-tool");
  ...
  const effectiveCallGateway = embedded
    ...
    : zuvixToolsDeps.callGateway;
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForZuvixTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentZuvixTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentZuvixTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: zuvixToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentZuvixTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("zuvix-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveZuvixPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("zuvix-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("zuvix-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<ZuvixToolsDeps>) {
    zuvixToolsDeps = overrides
      ? {
          ...defaultZuvixToolsDeps,
          ...overrides,
        }
      : defaultZuvixToolsDeps;
  },
