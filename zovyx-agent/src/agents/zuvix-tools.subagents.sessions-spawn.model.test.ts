...
function createConfig(overrides?: Record<string, unknown>): ZuvixConfig {
  return {
...
describe("subagent spawn model + thinking plan", () => {
  ...
  it("prefers per-agent subagent model over defaults", () => {
    const cfg = createConfig({
      agents: {
        ...
        list: [{ id: "research", subagents: { model: "zuvix/claude" } }],
      },
    });
    const targetAgentConfig = {
      id: "research",
      subagents: { model: "zuvix/claude" },
    };
    const plan = expectOkPlan(
      resolveSubagentModelAndThinkingPlan({
        cfg,
    ...
    expect(plan.resolvedModel).toBe("zuvix/claude");
    expect(plan.initialSessionPatch.model).toBe("zuvix/claude");
    expect(plan.initialSessionPatch.modelOverrideSource).toBe("auto");
    expect(plan.initialSessionPatch.modelOverrideFallbackOriginProvider).toBe("zuvix");
  ...
  it("prefers default subagent model over target agent primary model", () => {
    const cfg = createConfig({
      agents: {
        defaults: { subagents: { model: "minimax/MiniMax-M2.7" } },
        list: [{ id: "research", model: { primary: "zuvix/claude" } }],
      },
    });
    const targetAgentConfig = {
      id: "research",
      model: { primary: "zuvix/claude" },
    };
    const plan = expectOkPlan(
      resolveSubagentModelAndThinkingPlan({
        cfg,
