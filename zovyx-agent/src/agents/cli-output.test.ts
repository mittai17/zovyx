...
describe("createCliJsonlStreamingParser", () => {
  ...
  it.each(["server_tool_use", "mcp_tool_use"])("recognizes %s blocks", (type) => {
      ...
      [
        ...
        JSON.stringify({
          ...
          event: {
            type: "content_block_delta",
            index: 0,
            delta: { type: "input_json_delta", partial_json: '{"query":"zuvix"}' },
          },
        }),
        JSON.stringify({
          type: "stream_event",
    ...
    expect(starts).toEqual([
      { toolCallId: "toolu_hosted", name: "web_search", args: { query: "zuvix" } },
    ]);
  });
  ...
  it.each([
    {
      ...
      name: "web_search",
      input: { query: "zuvix" },
      result: [{ type: "web_search_result", title: "Zuvix", url: "https://example.com" }],
      isError: false,
    },
    {
