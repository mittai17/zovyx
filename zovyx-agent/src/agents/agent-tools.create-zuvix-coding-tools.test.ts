/**
 * Broad coverage for createZuvixCodingTools.
...
import "./test-helpers/fast-zuvix-tools.js";
import { wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call.js";
import { createZuvixCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as zuvixPluginTools from "./zuvix-plugin-tools.js";
import { createZuvixTools } from "./zuvix-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createZuvixCodingTools>) {
...
type ZuvixCodingTool = ReturnType<typeof createZuvixCodingTools>[number];
type ZuvixToolsOptions = NonNullable<Parameters<typeof createZuvixTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: ZuvixCodingTool[], name: string): ZuvixCodingTool {
...
function requireToolExecute(tool: ZuvixCodingTool): NonNullable<ZuvixCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateZuvixToolsOptions(): ZuvixToolsOptions {
  const calls = vi.mocked(createZuvixTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createZuvixTools call");
  }
  return options;
}
...
describe("createZuvixCodingTools", () => {
  const testConfig: ZuvixConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createZuvixCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createZuvixTools).mockReturnValueOnce([wrapped as never]);

    const tools = createZuvixCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core Zuvix tools are not materialized", () => {
    const createZuvixToolsMock = vi.mocked(createZuvixTools);
    createZuvixToolsMock.mockClear();
    ...
    expect(createZuvixToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createZuvixToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateZuvixToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:zuvix"], ["*"]]) {
      const tools = createZuvixCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to Zuvix tool construction", () => {
    const createZuvixToolsMock = vi.mocked(createZuvixTools);
    createZuvixToolsMock.mockClear();
    ...
    expect(createZuvixToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateZuvixToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into Zuvix tool construction plans", () => {
    const createZuvixToolsMock = vi.mocked(createZuvixTools);
    createZuvixToolsMock.mockClear();
    ...
    expect(createZuvixToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateZuvixToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the Zuvix core factory", () => {
    const createZuvixToolsMock = vi.mocked(createZuvixTools);
    createZuvixToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(zuvixPluginTools, "resolveZuvixPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createZuvixToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateZuvixToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to Zuvix tool factory planning", () => {
    const createZuvixToolsMock = vi.mocked(createZuvixTools);
    createZuvixToolsMock.mockClear();
    ...
    expect(createZuvixToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateZuvixToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to Zuvix plugin discovery", async () => {
    const createZuvixToolsMock = vi.mocked(createZuvixTools);
    createZuvixToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `zuvix-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createZuvixToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateZuvixToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createZuvixToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateZuvixToolsOptions().inheritedToolAllowlist;
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("zuvix-tools:test-helper")).toBeLessThan(
      stages.indexOf("zuvix-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createZuvixCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createZuvixCodingTools({
      config: testConfig,
    ...
    const codexTools = createZuvixCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: ZuvixConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createZuvixCodingTools({
    ...
    const anthropicTools = createZuvixCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: ZuvixConfig = {
      tools: {
        exec: {
    ...
    const allowed = createZuvixCodingTools({
    ...
    const denied = createZuvixCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createZuvixCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createZuvixCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createZuvixTools).mockClear();
    ...
    expect(latestCreateZuvixToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createZuvixCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createZuvixCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createZuvixCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createZuvixCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createZuvixCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createZuvixCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createZuvixCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createZuvixCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createZuvixCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-read-"));
    try {
      ...
      const contents = "Hello from zuvix read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "zuvix-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "zuvix-workspace"),
    ...
    const tools = createZuvixCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-canonical-"));
    try {
      const tools = createZuvixCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-legacy-alias-"));
    try {
      const tools = createZuvixCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-structured-write-"));
    try {
      const tools = createZuvixCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-structured-edit-"));
    try {
