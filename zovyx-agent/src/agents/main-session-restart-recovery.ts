...
export async function markRestartAbortedMainSessions(params: {
  cfg?: ZuvixConfig;
  additionalCfgs?: Iterable<ZuvixConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, ZUVIX_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is ZuvixConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "Zuvix was waiting on tool/model work. Continue from the existing " +
