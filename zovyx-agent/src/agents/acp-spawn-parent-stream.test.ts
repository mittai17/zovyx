...
function progressModeConfig(acp?: ZuvixConfig["acp"]): ZuvixConfig {
  return {
    ...(acp ? { acp } : {}),
    channels: {
      forum: {
...
describe("startAcpSpawnParentStreamRelay", () => {
  ...
  it("resolves ACP spawn stream log path from session metadata", () => {
    readAcpSessionEntryMock.mockReturnValue({
      storePath: "/tmp/zuvix/agents/codex/sessions/sessions.json",
      entry: {
        sessionId: "sess-123",
        sessionFile: "/tmp/zuvix/agents/codex/sessions/sess-123.jsonl",
      },
    });
    resolveSessionFilePathMock.mockReturnValue(
      "/tmp/zuvix/agents/codex/sessions/sess-123.jsonl",
    );
    ...
    expect(resolved).toBe("/tmp/zuvix/agents/codex/sessions/sess-123.acp-stream.jsonl");
    ...
    expect(options.storePath).toBe("/tmp/zuvix/agents/codex/sessions/sessions.json");
  });
});
