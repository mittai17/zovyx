...
const planZuvixModelsJsonMock = vi.fn();
...
function planParamsAt(callIndex: number): {
  ...
  // Planner call shape is the contract between ensureZuvixModelsJson and planning.
  const call = planZuvixModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planZuvixModelsJson: (...args: unknown[]) => planZuvixModelsJsonMock(...args),
  }));
  ...
  ({ ensureZuvixModelsJson } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureZuvixModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureZuvixModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureZuvixModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".zuvix", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".zuvix", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("writes plugin-owned model catalogs beside the agent plugin state", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planZuvixModelsJsonMock.mockImplementation(async () => ({
        action: "write",
  ...
  it("removes stale plugin-owned model catalogs", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureZuvixModelsJson({}, agentDir);
  ...
  it("keeps generated plugin catalogs on non-authoritative skip plans", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planZuvixModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      ...
      expect(planZuvixModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureZuvixModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureZuvixModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planZuvixModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planZuvixModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureZuvixModelsJson(first);
      await ensureZuvixModelsJson(second);
      await ensureZuvixModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureZuvixModelsJson(first),
        ensureZuvixModelsJson(second),
      ]);
