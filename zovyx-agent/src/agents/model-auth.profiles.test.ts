...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "zuvix-adc-"));
...
vi.mock("./model-auth-env-vars.js", () => {
  ...
  const aliasMap = {
    ...
    "z.ai": "zai",
    "z-ai": "zai",
    "zuvix-go-auth": "zuvix-go",
    bedrock: "amazon-bedrock",
    "aws-bedrock": "amazon-bedrock",
  };
  const resolveProviderEnvAuthEvidence = (params?: { config?: ZuvixConfig }) => {
    const evidence = {
      "google-vertex": [
        {
  ...
  return {
    ...
    resolveProviderEnvAuthLookupMaps: (params?: { config?: ZuvixConfig }) => ({
      aliasMap,
...
function buildDemoLocalProviderCfg(apiKey: string): ZuvixConfig {
  return {
    models: {
      providers: {
        "demo-local": {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    await withZuvixTestState(
      ...
      async (state) => {
        ...
        const store = ensureAuthProfileStore(process.env.ZUVIX_AGENT_DIR, {
  ...
  it("reports the config default agent dir when provider auth is missing", async () => {
    await withZuvixTestState(
      {
        layout: "state-only",
        prefix: "zuvix-auth-missing-agent-dir-",
        agentEnv: "clear",
        env: {
          XAI_API_KEY: undefined,
        },
  ...
  it("uses trusted workspace manifest auth evidence in runtime auth checks", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-workspace-cloud-auth-"));
  ...
  it("resolveEnvApiKey('zuvix-go') falls back to OPENCODE_ZEN_API_KEY", async () => {
    await withEnvAsync(
      {
        OPENCODE_API_KEY: undefined,
        OPENCODE_ZEN_API_KEY: "sk-zuvix-zen-fallback", // pragma: allowlist secret
      },
      async () => {
        const resolved = resolveEnvApiKey("zuvix-go");
        expect(resolved?.apiKey).toBe("sk-zuvix-zen-fallback");
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode explicit ADC credential paths", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-google-adc-unicode-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode ADC fallback home paths", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-google-adc-home-"));
  ...
  it("resolveEnvApiKey('google-vertex') rejects GOOGLE_CLOUD_PROJECT_ID-only ADC auth evidence", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-google-adc-project-id-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
  ...
  it("resolveEnvApiKey('google-vertex') accepts Windows APPDATA ADC fallback evidence", async () => {
    const appDataDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-google-adc-appdata-"));
  ...
  it("resolveEnvApiKey('google-vertex') does not synthesize APPDATA from USERPROFILE", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-google-adc-home-"));
    const userProfileDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "zuvix-google-adc-userprofile-"),
    );
  ...
  it("resolveEnvApiKey('google-vertex') keeps ADC fallback when manifest env candidates are empty", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-google-adc-candidates-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
