...
} from "@zuvix/media-core/inline-image-data-url";
...
function redactTranscriptOptions(cfg?: ZuvixConfig) {
...
function isTranscriptRedactionDisabled(cfg?: ZuvixConfig): boolean {
  return (cfg?.logging?.redactSensitive ?? readLoggingConfig()?.redactSensitive) === "off";
}

function redactTranscriptText(value: string, cfg?: ZuvixConfig): string {
...
const OPENAI_RESPONSES_APIS = new Set([
  ...
  "zuvix-azure-openai-responses-transport",
]);
...
const ANTHROPIC_REASONING_APIS = new Set([
  ...
  "zuvix-anthropic-messages-transport",
]);
const OPENAI_COMPLETIONS_APIS = new Set([
  "openai-completions",
  "zuvix-openai-completions-transport",
]);
...
function isGitHubCopilotResponsesRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return (
    (route?.api === "openai-responses" || route?.api === "zuvix-openai-responses-transport") &&
...
const OPENAI_REASONING_REPLAY_METADATA_KEY = "__zuvix_replay";
...
export function redactTranscriptMessage(message: AgentMessage, cfg?: ZuvixConfig): AgentMessage {
