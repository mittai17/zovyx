...
const CAPTURE_MARKER = "__ZUVIX_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__ZUVIX_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // ZUVIX_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.ZUVIX_SHELL === "exec") {
    env.ZUVIX_SHELL = "exec";
  }
  return env;
}
