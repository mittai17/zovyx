...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("zuvix-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("zuvix-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("zuvix-sbx-");
