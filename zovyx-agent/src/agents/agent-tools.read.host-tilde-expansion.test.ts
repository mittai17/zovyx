...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("zuvix-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("zuvix-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("zuvix-tilde-test-mkdir-");
  ...
  it("ignores ZUVIX_HOME for write operations", async () => {
    const zuvixHome = await createTempDir("zuvix-home-override-", os.tmpdir());
    const dir = await createTempDir("zuvix-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ ZUVIX_HOME: zuvixHome }, async () => {
      createHostWorkspaceWriteTool(zuvixHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(zuvixHome, path.basename(testFile))));
    });
  });

  it("ignores ZUVIX_HOME for mkdir operations", async () => {
    const zuvixHome = await createTempDir("zuvix-home-override-", os.tmpdir());
    const dir = await createTempDir("zuvix-tilde-test-mkdir-");
    ...
    await withEnvAsync({ ZUVIX_HOME: zuvixHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(zuvixHome, path.basename(newDir))));
    });
  });

  it("ignores ZUVIX_HOME for readFile operations", async () => {
    const zuvixHome = await createTempDir("zuvix-home-override-", os.tmpdir());
    const dir = await createTempDir("zuvix-tilde-test-edit-");
    ...
    await withEnvAsync({ ZUVIX_HOME: zuvixHome }, async () => {
      createHostWorkspaceEditTool(zuvixHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores ZUVIX_HOME for access operations", async () => {
    const zuvixHome = await createTempDir("zuvix-home-override-", os.tmpdir());
    const dir = await createTempDir("zuvix-tilde-test-edit-");
