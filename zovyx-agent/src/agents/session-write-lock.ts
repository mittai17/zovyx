...
const CLEANUP_STATE_KEY = Symbol.for("zuvix.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("zuvix.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("zuvix.session-write-lock");
...
const SESSION_WRITE_LOCK_ENV: Record<SessionWriteLockMsKey, string> = {
  acquireTimeoutMs: "ZUVIX_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS",
  staleMs: "ZUVIX_SESSION_WRITE_LOCK_STALE_MS",
  maxHoldMs: "ZUVIX_SESSION_WRITE_LOCK_MAX_HOLD_MS",
};
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.ZUVIX_TEST_SESSION_LOCK_WATCHDOG === "1";
}
...
function isZuvixSessionOwnerArgv(args: string[]): boolean {
  const normalized = args.map(normalizeOwnerProcessArg).filter(Boolean);
  if (normalized.length === 0) {
    return false;
  }
  const exe = (normalized[0] ?? "").replace(/\.(bat|cmd|exe)$/i, "");
  if (exe === "zuvix" || exe.endsWith("/zuvix") || exe.endsWith("/zuvix-gateway")) {
    return true;
  }
  if (
    normalized.some(
  ...
  const hasZuvixCommandToken = normalized.some((arg) => arg === "gateway" || arg === "agent");
  return normalized.some(
    (arg) => entryCandidates.some((entry) => arg.endsWith(entry)) && hasZuvixCommandToken,
  );
}
...
function shouldTreatAsNonZuvixOwner(params: {
  payload: LockFilePayload | null;
  ...
  return !isZuvixSessionOwnerArgv(args);
}
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-zuvix-owner"],
    };
  }
