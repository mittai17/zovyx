...
export function listAgentEntries(cfg: ZuvixConfig): AgentEntry[] {
...
export function listAgentIds(cfg: ZuvixConfig): string[] {
...
export function resolveDefaultAgentId(cfg: ZuvixConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: ZuvixConfig, agentId: string): AgentEntry | undefined {
