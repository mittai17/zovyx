 ...
 *
 * Presents large Zuvix/MCP/client tool inventories through search, describe, call, and optional code-mode tools.
 */
...
type CatalogSource = "zuvix" | "mcp" | "client";
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    zuvix: { value: controller.zuvix, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("zuvix.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: ZuvixConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: ZuvixConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "zuvix", sourceName: pluginId };
  }
  return { source: "zuvix", sourceName: "core" };
}
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with zuvix.tools.search, zuvix.tools.describe, and zuvix.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The zuvix.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call a selected Tool Search catalog entry through Zuvix.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
