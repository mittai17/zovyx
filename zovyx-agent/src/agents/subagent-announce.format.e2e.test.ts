...
let configOverride: ZuvixConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: ZuvixConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/zuvix/zuvix/issues/31298
    previousFastTestEnv = process.env.ZUVIX_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.ZUVIX_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // ZUVIX_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Zuvix runtime context (internal):");
