...
let createZuvixCodingTools: typeof import("./agent-tools.js").createZuvixCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      ZUVIX_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "zuvix-test-no-bundled-extensions"),
    },
    async () => {
      ({ createZuvixCodingTools } = await import("./agent-tools.js"));
    },
  );
});
...
vi.mock("zuvix/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
describe("createZuvixCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "zuvix-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "zuvix-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "zuvix-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "zuvix-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "zuvix-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "zuvix-safe-bins-grep-",
        safeBins: ["grep"],
