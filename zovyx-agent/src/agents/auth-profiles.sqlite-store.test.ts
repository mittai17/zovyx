...
describe("auth profile sqlite store", () => {
  ...
  it("persists auth profiles and runtime scheduling state in the agent sqlite database", async () => {
    await withAgentDirEnv("zuvix-auth-sqlite-", (agentDir) => {
      saveAuthProfileStore(
        {
          ...apiKeyStore("sk-test"),
      ...
      expect(fs.existsSync(path.join(agentDir, "zuvix-agent.sqlite"))).toBe(true);
    });
  });

  it("does not read legacy auth-profiles.json at runtime", async () => {
    await withAgentDirEnv("zuvix-auth-no-json-fallback-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("does not create sqlite files for missing-store reads", async () => {
    await withAgentDirEnv("zuvix-auth-sqlite-no-create-", (agentDir) => {
      expect(loadPersistedAuthProfileStore(agentDir)).toBeNull();
      expect(fs.existsSync(path.join(agentDir, "zuvix-agent.sqlite"))).toBe(false);
    });
  });

  it("reads existing sqlite auth stores without registering shared state", async () => {
    await withAgentDirEnv("zuvix-auth-sqlite-readonly-", (agentDir) => {
      saveAuthProfileStore(apiKeyStore("sk-test"), agentDir);
      closeZuvixAgentDatabasesForTest();
      closeZuvixStateDatabaseForTest();
      const stateDbPath = resolveZuvixStateSqlitePath();
  ...
  it("uses the configured agent id for custom agentDir databases", async () => {
    await withAgentDirEnv("zuvix-auth-sqlite-custom-agent-", (envAgentDir) => {
  ...
  it("keeps SecretRef-backed credentials from persisting duplicate plaintext", async () => {
    await withAgentDirEnv("zuvix-auth-sqlite-secret-ref-", (agentDir) => {
      saveAuthProfileStore(
        {
          version: 1,
          profiles: {
  ...
  it("recomputes runtime-only external auth overlays from the sqlite base store", async () => {
    await withAgentDirEnv("zuvix-auth-sqlite-overlay-", (agentDir) => {
      saveAuthProfileStore(apiKeyStore("sk-test"), agentDir);
