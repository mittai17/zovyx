...
describe("isSilentOverflowProneModel", () => {
  ...
  // zuvix#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable Zuvix runtime's secondary compaction path;
  // Zuvix's preemptive compaction continues to handle real overflow.
  ...
  it("does not flag namespaced GLM ids routed through non-z.ai hosts", () => {
    ...
    expect(
      isSilentOverflowProneModel({ provider: "zuvix-go", modelId: "zuvix-go/glm-5.1" }),
    ).toBe(false);
  });
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of zuvix#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering Zuvix runtime's
  ...
  // state.messages intact; Zuvix's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep Zuvix runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove Zuvix runtime's
