 ...
 *
 * Registers provider-specific stream functions and rewrites models that need Zuvix-managed transport semantics.
 */
...
const PROVIDER_SIMPLE_COMPLETION_API_PREFIX = "zuvix-provider-simple:";
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `zuvix-anthropic-vertex-simple:${suffix}`;
}
...
function applyProviderSimpleCompletionWrapper(model: Model, cfg?: ZuvixConfig): Model {
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use Zuvix's transport so resolved request auth is applied.
  const transportModel = {
    ...model,
    baseUrl: normalizeCodexResponsesBaseUrlForOpenAISdk(model.baseUrl),
  } as Model;
  const api = resolveTransportAwareSimpleApi(model.api);
  const streamFn = createZuvixTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }
