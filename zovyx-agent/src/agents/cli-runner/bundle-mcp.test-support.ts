...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["ZUVIX_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("zuvix-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("zuvix-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("zuvix-cli-bundle-mcp-bundled-");
    setTestEnvValue("ZUVIX_BUNDLED_PLUGINS_DIR", emptyBundledDir);
...
function createEnabledBundleProbeConfig(): ZuvixConfig {
  return {
    plugins: {
      entries: {
