/** Tests merging user Zuvix MCP server config into Claude bundle-MCP overlays. */
...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  it("merges user-configured mcp.servers from Zuvix config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zuvix-cli-bundle-mcp-user-servers-",
    );
  ...
  it("translates Zuvix transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zuvix-cli-bundle-mcp-user-servers-transport-",
    );
  ...
  it("preserves explicit type and still strips transport on user mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zuvix-cli-bundle-mcp-user-servers-transport-explicit-",
    );
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    // The Zuvix loopback server is generated runtime state and must win over
    // user config with the same server name.
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zuvix-cli-bundle-mcp-user-servers-loopback-",
    );
      ...
      additionalConfig: {
        mcpServers: {
          zuvix: {
            ...
            headers: { Authorization: "Bearer ${ZUVIX_MCP_TOKEN}" },
          },
        },
      },
    });
    ...
    expect(raw.mcpServers?.zuvix?.url).toBe("http://127.0.0.1:23119/mcp");
  ...
  it("replaces overlapping bundle server entries with user-configured mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zuvix-cli-bundle-mcp-user-servers-replace-",
    );
