...
const prepareDeps = {
  ...
  resolveZuvixReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveZuvixReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveZuvixReferencePaths(params),
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          ZUVIX_MCP_TOKEN: prepareDeps.resolveMcpLoopbackBearerToken(
            mcpLoopbackRuntime,
            params.senderIsOwner === true,
          ),
          ZUVIX_MCP_AGENT_ID: sessionAgentId ?? "",
          ZUVIX_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          ZUVIX_MCP_SESSION_KEY: params.sessionKey ?? "",
          ZUVIX_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
          ZUVIX_MCP_CURRENT_CHANNEL_ID: params.currentChannelId ?? "",
          ZUVIX_MCP_CURRENT_THREAD_TS: params.currentThreadTs ?? "",
          ZUVIX_MCP_CURRENT_MESSAGE_ID:
            params.currentMessageId != null ? String(params.currentMessageId) : "",
          ZUVIX_MCP_CURRENT_INBOUND_AUDIO: params.currentInboundAudio === true ? "true" : "",
          ZUVIX_MCP_INBOUND_EVENT_KIND: params.currentInboundEventKind ?? "",
          ZUVIX_MCP_SOURCE_REPLY_DELIVERY_MODE: params.sourceReplyDeliveryMode ?? "",
  ...
  let zuvixHistoryMessages: unknown[] | undefined;
  const loadZuvixHistoryMessages = async () => {
    zuvixHistoryMessages ??= await loadCliSessionHistoryMessages({
      sessionId: params.sessionId,
  ...
  const zuvixReferences = isSideQuestion
    ? { docsPath: null, sourcePath: null }
    : await prepareDeps.resolveZuvixReferencePaths({
        workspaceDir,
  ...
  const builtSystemPrompt = isSideQuestion
    ...
    : buildCliAgentSystemPrompt({
        ...
        heartbeatPrompt,
        docsPath: zuvixReferences.docsPath ?? undefined,
        sourcePath: zuvixReferences.sourcePath ?? undefined,
  ...
  if (!isSideQuestion) {
    ...
    try {
      const hookResult = await resolvePromptBuildHookResult({
        ...
        messages: await loadZuvixHistoryMessages(),
        hookCtx: {
          runId: params.runId,
          agentId: sessionAgentId,
          sessionKey: params.sessionKey,
  ...
  const shouldPrepareZuvixHistoryPrompt =
    !isSideQuestion && (!reusableCliSession.sessionId || allowRawTranscriptReseed);
  const zuvixHistoryPrompt = shouldPrepareZuvixHistoryPrompt
    ? buildCliSessionHistoryPrompt({
  ...
  try {
    ...
    return {
      ...
      claudeSkillsPluginArgs: claudeSkillsPlugin.args,
      bootstrapPromptWarningLines: bootstrapPromptWarning.lines,
      ...(zuvixHistoryPrompt ? { zuvixHistoryPrompt } : {}),
      heartbeatPrompt,
      authEpoch,
      authEpochVersion: CLI_AUTH_EPOCH_VERSION,
      extraSystemPromptHash,
