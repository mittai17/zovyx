...
import { extensionForMime } from "@zuvix/media-core/mime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
} from "@zuvix/normalization-core/string-coerce";
import { KeyedAsyncQueue } from "zuvix/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredZuvixTmpDir(), "zuvix-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".zuvix-cli-images");
  }
  return path.join(resolvePreferredZuvixTmpDir(), "zuvix-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const workspace = await tempWorkspace({
    ...
    prefix: "zuvix-cli-system-prompt-",
  });
