...
describe("prepareCliBundleMcpConfig", () => {
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "zuvix-cli-bundle-mcp-workspace-root-",
    );
    const pluginRoot = path.join(workspaceDir, ".zuvix", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "zuvix"]);
    expect(raw.mcpServers?.zuvix?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.zuvix?.headers?.Authorization).toBe("Bearer ${ZUVIX_MCP_TOKEN}");
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      env: {
        ZUVIX_MCP_TOKEN: "loopback-token-123",
        ZUVIX_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/zuvix-bundle-mcp-disabled",
    });
