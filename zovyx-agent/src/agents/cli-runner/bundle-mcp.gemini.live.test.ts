...
const LIVE = isLiveTestEnabled(["ZUVIX_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "zuvix-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("zuvix_live_probe", "Zuvix Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Zuvix-configured streamable-http server", async () => {
    const geminiCommand = process.env.ZUVIX_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("zuvixLiveProbe");
