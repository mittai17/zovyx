...
const CLI_BACKEND_PRESERVE_ENV = "ZUVIX_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.ZUVIX_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.ZUVIX_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.ZUVIX_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.ZUVIX_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.ZUVIX_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.zuvixHistoryPrompt ?? params.prompt);
      ...
      try {
        cliBackendLog.info(
          buildCliExecLogLine({
            ...
            hasHistoryPrompt: Boolean(context.zuvixHistoryPrompt),
          }),
        );
        const logOutputText =
        ...
        if (logOutputText) {
          ...
          if (env.ZUVIX_MCP_TOKEN || env.ZUVIX_MCP_SESSION_KEY || env.ZUVIX_MCP_AGENT_ID) {
        ...
        if (result.exitCode !== 0 || result.reason !== "exit") {
          if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
            ...
            const deferWatchdogNoticeForFreshRetry =
              ...
              Boolean(context.zuvixHistoryPrompt) &&
