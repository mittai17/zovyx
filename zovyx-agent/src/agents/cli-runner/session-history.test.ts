...
function createSessionTranscript(params: {
  ...
  // same JSONL record order used by persisted Zuvix sessions.
  const sessionFile =
    params.filePath ??
    path.join(
      params.rootDir,
...
async function withCliSessionState<T>(stateDir: string, run: () => Promise<T>): Promise<T> {
  return await withEnvAsync({ ZUVIX_STATE_DIR: stateDir }, run);
}
...
describe("loadCliSessionHistoryMessages", () => {
  it("reads the canonical session transcript instead of an arbitrary external path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-state-"));
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-outside-"));
  ...
  it("honors custom session store roots when resolving hook history transcripts", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-state-"));
    const customStoreDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-store-"));
...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders Zuvix transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[Zuvix reseed history truncated; older turns dropped]");
