...
function createSessionFile() {
  // Prepare tests use canonical Zuvix session paths because several cases
  // assert that external or stale transcript paths are ignored.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-prepare-"));
  sessionFileEnvSnapshot ??= captureEnv(["ZUVIX_STATE_DIR"]);
  setTestEnvValue("ZUVIX_STATE_DIR", dir);
...
describe("shouldSkipLocalCliCredentialEpoch", () => {
  beforeEach(() => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolveZuvixReferencePaths: vi.fn(async () => ({ docsPath: null, sourcePath: null })),
  ...
  it("prepares side questions without agent-turn context, tools, hooks, or reusable sessions", async () => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolveZuvixReferencePaths: vi.fn(async () => ({ docsPath: "docs", sourcePath: "src" })),
    });
    ...
    expect(context.zuvixHistoryPrompt).toBeUndefined();
  ...
  it("uses compact current-turn context when a room event resumes a CLI session", async () => {
    ...
    try {
      ...
      // richer room context in Zuvix history for reseed and audits.
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionKey: "agent:main:test",
        agentId: "main",
      ...
      expect(context.zuvixHistoryPrompt).toContain("Room context:\nAlice: lunch?");
      expect(context.zuvixHistoryPrompt).toContain("Current event:\nBob: yes");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("does not allocate a non-legacy context engine before fallible CLI preparation finishes", async () => {
    ...
    setCliRunnerPrepareTestDeps({
      resolveZuvixReferencePaths: vi.fn(async () => {
        ...
        info: {
          ...
          hostRequirements: {
            "agent-run": {
              ...
              unsupportedMessage: "Use the native Codex or Zuvix embedded runtime.",
            },
          },
        },
        ingest: vi.fn(async () => ({ ingested: true })),
  ...
  it("uses cwd for CLI system prompt workspace guidance", async () => {
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-cli-task-"));
    try {
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionFile,
  ...
  it("prepares raw-tail history for safe invalidations only when the backend opts in", async () => {
    ...
    try {
      ...
      expect(context.zuvixHistoryPrompt).toContain("prior no-compaction ask");
      expect(context.zuvixHistoryPrompt).toContain("latest ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("prepares opted-in raw-tail history for session-expired retry without disabling native resume", async () => {
    ...
    try {
      ...
      expect(context.reusableCliSession).toEqual({ sessionId: "cli-session" });
      expect(context.zuvixHistoryPrompt).toContain("prior resumable ask");
      expect(context.zuvixHistoryPrompt).toContain("latest ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("passes current turn kind into bundle MCP loopback env", async () => {
    ...
    try {
      ...
      expect(context.preparedBackend.env).toMatchObject({
        ZUVIX_MCP_MESSAGE_CHANNEL: "telegram",
        ZUVIX_MCP_CURRENT_CHANNEL_ID: "telegram:-100123:topic:42",
        ZUVIX_MCP_CURRENT_THREAD_TS: "42",
        ZUVIX_MCP_CURRENT_MESSAGE_ID: "reply-message-1",
        ZUVIX_MCP_CURRENT_INBOUND_AUDIO: "true",
        ZUVIX_MCP_INBOUND_EVENT_KIND: "room_event",
        ZUVIX_MCP_SOURCE_REPLY_DELIVERY_MODE: "message_tool_only",
        ZUVIX_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET: "true",
      });
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("renders CLI skills from sandbox-readable paths instead of persisted host snapshots", async () => {
    const { dir, sessionFile } = createSessionFile();
    const hostSkillDir = "/home/tzdai/.npm-global/lib/node_modules/zuvix/skills/gog";
    ...
    try {
      ...
      expect(context.systemPrompt).toContain(
        "/workspace/.zuvix/sandbox-skills/skills/gog/SKILL.md",
      );
  ...
  it("omits Claude CLI prompt skills when the native skills plugin can carry them", async () => {
    ...
    try {
      ...
      setCliRunnerPrepareTestDeps({
        prepareClaudeCliSkillsPlugin: vi.fn(async () => ({
          args: ["--plugin-dir", path.join(dir, "zuvix-skills")],
          cleanup: vi.fn(async () => undefined),
          pluginDir: path.join(dir, "zuvix-skills"),
        })),
      });
  ...
  it("uses a larger automatic reseed history cap for Claude CLI", async () => {
    ...
    try {
      ...
      expect(context.zuvixHistoryPrompt).toBeDefined();
      expect(context.zuvixHistoryPrompt).toContain(summaryMarker);
      expect(context.zuvixHistoryPrompt).not.toContain("Zuvix reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("keeps the default reseed history cap for non-Claude CLI backends", async () => {
    ...
    try {
      ...
      expect(context.zuvixHistoryPrompt).toBeDefined();
      expect(context.zuvixHistoryPrompt).toContain("Zuvix reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("uses the automatic Claude CLI cap through the raw-tail reseed path", async () => {
    ...
    try {
      ...
      expect(context.reusableCliSession).toEqual({ sessionId: "cli-session" });
      expect(context.zuvixHistoryPrompt).toBeDefined();
      expect(context.zuvixHistoryPrompt).toContain(recentMarker);
      expect(context.zuvixHistoryPrompt).toContain("EARLIEST_USER");
      expect(context.zuvixHistoryPrompt).not.toContain("Zuvix reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
