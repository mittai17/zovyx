/** Tests projecting Zuvix user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as ZuvixConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as ZuvixConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as ZuvixConfig),
    ).toBeUndefined();
  });
  ...
  it("filters Codex-scoped user MCP servers by Zuvix agent id", () => {
    // Agent-scoped MCP servers should follow the active Zuvix agent, while
  ...
  it("omits scoped Codex MCP servers when no Zuvix agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
