/**
 * Materializes selected Zuvix skills as a temporary Claude CLI plugin.
 */
...
const ZUVIX_CLAUDE_PLUGIN_NAME = "zuvix-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredZuvixTmpDir(), "zuvix-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, ZUVIX_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: ZUVIX_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Zuvix skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
