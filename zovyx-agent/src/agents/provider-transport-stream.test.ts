...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to Zuvix transport aliases when needed.
    const cases = [
      {
        ...
        alias: "zuvix-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "zuvix-openai-completions-transport",
      },
      {
        ...
        alias: "zuvix-azure-openai-responses-transport",
      },
      {
        ...
        alias: "zuvix-anthropic-messages-transport",
      },
      {
        ...
        alias: "zuvix-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Zuvix transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createZuvixTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the Zuvix simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("zuvix-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the Zuvix transport until Zuvix preserves attribution", () => {
