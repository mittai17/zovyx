...
import { parseStrictInteger } from "@zuvix/normalization-core/number-coercion";
...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via ZUVIX_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `ZUVIX_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${ZUVIX_PREPEND_PATH}:$PATH"; unset ZUVIX_PREPEND_PATH; '
    : "";
