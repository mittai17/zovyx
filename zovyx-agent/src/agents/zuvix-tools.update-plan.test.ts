// Verifies update_plan registration gates and base Zuvix tool inclusion policy.
...
type UpdatePlanGatingParams = Parameters<typeof isUpdatePlanToolEnabledForZuvixTools>[0];
type CreateZuvixToolsOptions = NonNullable<Parameters<typeof createZuvixTools>[0]>;

function expectUpdatePlanEnabled(params: UpdatePlanGatingParams, expected: boolean): void {
  expect(isUpdatePlanToolEnabledForZuvixTools(params)).toBe(expected);
}

function toolNames(tools: ReturnType<typeof createZuvixTools>): string[] {
  return tools.map((tool) => tool.name);
}

function createFastToolNames(options: CreateZuvixToolsOptions): string[] {
...
function expectToolNamed(
  tools: ReturnType<typeof createZuvixTools>,
  name: string,
): ReturnType<typeof createZuvixTools>[number] {
...
describe("zuvix-tools update_plan gating", () => {
  afterEach(() => {
    setEmbeddedMode(false);
  });

  it("keeps update_plan disabled by default", () => {
    expectUpdatePlanEnabled({ config: {} as ZuvixConfig }, false);
  });
  ...
  it("does not expose update_plan from default tool construction", () => {
    ...
    expect(shouldIncludeUpdatePlanToolForZuvixTools(emptyAllowlistParams)).toBe(false);
  });

  it("wraps constructed tools with before-tool-call hooks by default", () => {
    const tools = createZuvixTools({
      config: {} as ZuvixConfig,
      disablePluginTools: true,
    });
    const unwrappedTools = createZuvixTools({
      config: {} as ZuvixConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
  ...
  it("requires explicit transcripts enablement before registering the transcripts tool", () => {
    ...
    const enabledTools = createFastToolNames({
      config: { transcripts: { enabled: true } } as ZuvixConfig,
    });
  ...
  it("keeps explicitly allowed message tool in embedded completions", () => {
    setEmbeddedMode(true);
    const fromRuntimeAllowlist = createZuvixTools({
      config: {} as ZuvixConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
      wrapBeforeToolCallHook: false,
    });
    const fromGlobalAlsoAllow = createZuvixTools({
      config: { tools: { profile: "minimal", alsoAllow: ["message"] } } as ZuvixConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
    const denied = createZuvixTools({
      config: {} as ZuvixConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
  ...
  it("includes update_plan when a config allowlist group includes it", () => {
    const includeUpdatePlan = shouldIncludeUpdatePlanToolForZuvixTools({
      config: { tools: { allow: ["group:agents"] } } as ZuvixConfig,
      modelProvider: "anthropic",
      modelId: "claude-sonnet-4-6",
    });
