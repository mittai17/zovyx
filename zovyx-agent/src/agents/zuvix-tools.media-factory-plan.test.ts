...
import { resolveOptionalMediaToolFactoryPlan } from "./zuvix-tools.media-factory-plan.js";
...
type CreateZuvixToolsOptions = Parameters<
  typeof import("./zuvix-tools.js").createZuvixTools
>[0];
let createZuvixToolsForTestModule: typeof import("./zuvix-tools.js").createZuvixTools;
let legacyComfyToolNames: string[];

async function createZuvixToolsForTest(options?: CreateZuvixToolsOptions) {
  return createZuvixToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/zuvix.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): ZuvixConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createZuvixTools: createZuvixToolsForTestModule } =
      await import("./zuvix-tools.js"));
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: ZuvixConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createZuvixToolsForTest({
      config,
      agentDir: "/tmp/zuvix-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: ZuvixConfig = {};
    const workspaceDir = "/tmp/zuvix-workspace";
