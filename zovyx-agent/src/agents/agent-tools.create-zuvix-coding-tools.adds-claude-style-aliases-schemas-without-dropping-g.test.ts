...
import type { AgentTool, AgentToolResult } from "zuvix/plugin-sdk/agent-core";
import { Type } from "typebox";
import { describe, expect, it, vi } from "vitest";
import { createZuvixReadTool, createSandboxedReadTool } from "./agent-tools.read.js";
...
describe("createZuvixCodingTools read behavior", () => {
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "zuvix-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-read-autopage-"));
  ...
  it("maps inbound media refs to sandbox-staged media for reads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-read-media-sbx-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-read-cap-"));
  ...
  it("returns empty content for explicit offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-read-offset-eof-"));
  ...
  it("returns empty content for adaptive offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-read-offset-adaptive-"));
    await fs.writeFile(path.join(tmpDir, "notes.txt"), "one\ntwo\nthree\n", "utf8");
    try {
      const readTool = createSandboxedReadTool({
        root: tmpDir,
  ...
  it("returns already-read adaptive content when pagination reaches EOF", async () => {
    ...
    const readTool = createZuvixReadTool({
      name: "read",
      label: "read",
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createZuvixReadTool(
      baseRead as unknown as Parameters<typeof createZuvixReadTool>[0],
    );
