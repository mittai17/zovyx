...
describe("MCP OAuth provider", () => {
  it("stores token state under the Zuvix state directory with restricted permissions", async () => {
    await withTempHome(
      async (home) => {
        ...
        const tokenDir = `${home}/.zuvix/mcp-oauth`;
  ...
  it("does not persist localhost when the fallback attempt fails", async () => {
    await withTempHome(
      async (home) => {
        ...
        await expect(fs.readdir(`${home}/.zuvix/mcp-oauth`)).rejects.toThrow();
      },
      {
        prefix: "zuvix-mcp-oauth-localhost-failure-",
        skipSessionCleanup: true,
        env: {
          ZUVIX_CONFIG_PATH: undefined,
          ZUVIX_STATE_DIR: undefined,
  ...
  it("persists localhost redirect for a later code exchange login", async () => {
    await withTempHome(
      ...
      {
        prefix: "zuvix-mcp-oauth-localhost-persist-",
        skipSessionCleanup: true,
        env: {
          ZUVIX_CONFIG_PATH: undefined,
          ZUVIX_STATE_DIR: undefined,
  ...
  it("does not start hidden authorization flows without an authorization callback", async () => {
    ...
    await withTempHome(
      async () => {
        ...
        await expect(provider.state?.()).rejects.toThrow("Run zuvix mcp login Remote Docs.");
        await expect(provider.saveCodeVerifier?.("verifier")).rejects.toThrow(
          "Run zuvix mcp login Remote Docs.",
        );
        await expect(
          provider.redirectToAuthorization?.(new URL("https://auth.example.com/authorize")),
        ).rejects.toThrow("Run zuvix mcp login Remote Docs.");
      },
      {
        prefix: "zuvix-mcp-oauth-noninteractive-",
        skipSessionCleanup: true,
        env: {
          ZUVIX_CONFIG_PATH: undefined,
          ZUVIX_STATE_DIR: undefined,
