...
function resolveAnthropicOpusThinking(cfg: ZuvixConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: ZuvixConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicOpus48Thinking(cfg: ZuvixConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicVertexOpus48Thinking(cfg: ZuvixConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic-vertex",
    model: "claude-opus-4-8",
...
function resolveClaudeCliOpus48Thinking(cfg: ZuvixConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "claude-cli",
    model: "claude-opus-4-8",
...
function resolveConfiguredRefForTest(cfg: Partial<ZuvixConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as ZuvixConfig,
...
describe("model-selection", () => {
  describe("normalizeProviderId", () => {
    it("should normalize provider names", () => {
      ...
      expect(normalizeProviderId("z-ai")).toBe("z-ai");
      expect(normalizeProviderId("Zuvix-Zen")).toBe("zuvix-zen");
  ...
  describe("resolveAllowedModelRef", () => {
    ...
    it("infers provider from allowlist for bare model ids to prevent prefix drift (#48369)", () => {
      ...
      // should resolve to zuvix-go/kimi-k2.6, not openai/kimi-k2.6
      const result = resolveAllowedModelRef({
        cfg,
        catalog: [],
        raw: "kimi-k2.6",
      ...
      expect(result).toEqual({
        key: "zuvix-go/kimi-k2.6",
        ref: { provider: "zuvix-go", model: "kimi-k2.6" },
      });
    });
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("should fall back to the configured default provider and warn if provider is missing for non-alias", async () => {
      const warnLogs = createWarnLogCapture("zuvix-model-selection-test");
      try {
        const cfg: Partial<ZuvixConfig> = {
          agents: {
            defaults: {
