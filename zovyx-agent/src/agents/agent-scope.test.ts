...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: ZuvixConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: ZuvixConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: ZuvixConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: ZuvixConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: ZuvixConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["custom-zuvix-go-extras/deepseek-v4-flash"],
          },
        },
        list: [
          {
    ...
    const cfgInheritDefaultsWithoutAgentModel: ZuvixConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: ZuvixConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: ZuvixConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: ZuvixConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/zuvix");
  });

  it("uses ZUVIX_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "zuvix-home");
    withEnv({ ZUVIX_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as ZuvixConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".zuvix", "workspace"));
    });
  });

  it("uses ZUVIX_WORKSPACE_DIR for default agent workspace", () => {
    const workspaceDir = path.join(path.sep, "srv", "zuvix-workspace");
    withEnv(
      {
        ZUVIX_WORKSPACE_DIR: workspaceDir,
        ZUVIX_HOME: path.join(path.sep, "srv", "zuvix-home"),
      },
      () => {
        const workspace = resolveAgentWorkspaceDir({} as ZuvixConfig, "main");
  ...
  it("uses ZUVIX_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "zuvix-home");
    withEnv({ ZUVIX_HOME: home, ZUVIX_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as ZuvixConfig, "main");
      expect(agentDir).toBe(path.join(path.resolve(home), ".zuvix", "agents", "main", "agent"));
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ ZUVIX_STATE_DIR: stateDir }, () => resolveDefaultAgentDir(cfg));
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ ZUVIX_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/zuvix-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/zuvix-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-agent-scope-"));
