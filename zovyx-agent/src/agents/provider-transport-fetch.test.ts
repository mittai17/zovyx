...
describe("buildGuardedModelFetch", () => {
  beforeEach(() => {
    ...
    delete process.env.ZUVIX_DEBUG_PROXY_URL;
    delete process.env.ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS;
  });
  ...
  it("does not force explicit debug proxy overrides onto plain HTTP model transports", async () => {
    process.env.ZUVIX_DEBUG_PROXY_ENABLED = "1";
    process.env.ZUVIX_DEBUG_PROXY_URL = "http://127.0.0.1:7799";
    const model = {
      id: "kimi-k2.5:cloud",
      provider: "ollama",
      api: "ollama-chat",
  ...
  describe("long retry-after handling", () => {
    ...
    it("respects ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS", async () => {
      process.env.ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS = "10";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
    ...
    it("ignores partial ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS values", async () => {
      process.env.ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS = "10s";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
          headers: { "retry-after": "30" },
    ...
    it.each(["0x10", "1e3"])(
      "ignores non-decimal ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS values: %s",
      async (value) => {
        process.env.ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS = value;
        fetchWithSsrFGuardMock.mockResolvedValue({
          response: new Response(null, {
            status: 429,
            headers: { "retry-after": "30" },
    ...
    it("ignores unsafe ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS values", async () => {
      process.env.ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS = "9007199254740993";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
          headers: { "retry-after": "30" },
    ...
    it("can be disabled with ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS=0", async () => {
      process.env.ZUVIX_SDK_RETRY_MAX_WAIT_SECONDS = "0";
      fetchWithSsrFGuardMock.mockResolvedValue({
        response: new Response(null, {
          status: 429,
          headers: { "retry-after": "239" },
