...
let testState: ZuvixTestState | undefined;

beforeEach(async () => {
  testState = await createZuvixTestState({
    layout: "state-only",
    prefix: "zuvix-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ZUVIX_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ZUVIX_HOME: "/srv/zuvix-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/zuvix-home"), ".zuvix", "workspace"));
  });

  it("prefers ZUVIX_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ZUVIX_WORKSPACE_DIR: "/srv/zuvix-workspace",
      ZUVIX_HOME: "/srv/zuvix-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/zuvix-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".zuvix", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("zuvix-workspace-");
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".zuvix"), { recursive: true });
  ...
  it("does not overwrite a sibling file that is not an Zuvix attestation marker", async () => {
    const tempDir = await makeTempWorkspace("zuvix-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-workspace-hardlink-"));
    try {
