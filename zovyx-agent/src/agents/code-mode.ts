...
function readCodeModeRawConfig(config?: ZuvixConfig, agentId?: string): Record<string, unknown> {
...
export function resolveCodeModeConfig(config?: ZuvixConfig, agentId?: string): CodeModeConfig {
...
export function createCodeModeTools(ctx: CodeModeToolContext): AnyAgentTool[] {
  ...
  const waitTool = markCodeModeControlTool({
    ...
    label: "wait",
    description: "Resume a suspended Zuvix code mode run returned by exec.",
    parameters: Type.Object({
