/**
 * Zuvix stdio transport wrapper for MCP server subprocesses.
 */
...
export type ZuvixStdioServerParameters = {
  command: string;
  args?: string[];
...
export class ZuvixStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: ZuvixStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "ZuvixStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
