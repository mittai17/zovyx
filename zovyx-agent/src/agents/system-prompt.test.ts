...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("zuvix_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("zuvix_main");
  });
  ...
  it("includes an Zuvix control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zuvix",
    });

    expect(prompt).toContain("## Zuvix Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("zuvix gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__zuvix__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("keeps the Zuvix empty-tool fallback on the main prompt surface", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zuvix",
      toolNames: [],
    });

    expect(prompt).toContain("Zuvix lists the standard tools above");
  ...
  it("guides harness requests to ACP thread-bound spawns", () => {
    ...
    expect(prompt).toContain(
      'For requests like "do this in claude code/cursor/gemini/zuvix" or similar ACP harnesses, treat it as ACP harness intent',
    );
    expect(prompt).toContain(
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- sessions_spawn: Spawn an isolated sub-agent session");
    expect(prompt).toContain("- agents_list: List Zuvix agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/zuvix/docs");
    expect(prompt).toContain("Zuvix behavior/config/architecture: read local docs first.");
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zuvix",
      docsPath: "/tmp/zuvix/docs",
      sourcePath: "/tmp/zuvix",
    });

    expect(prompt).toContain("## Documentation");
    expect(prompt).toContain("Docs: /tmp/zuvix/docs");
    expect(prompt).toContain("Source: /tmp/zuvix");
    expect(prompt).toContain("Zuvix behavior/config/architecture: read local docs first.");
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.zuvix.ai");
    expect(prompt).toContain("Source: https://github.com/zuvix/zuvix");
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/zuvix",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/zuvix/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/zuvix/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Zuvix Self-Update");
  ...
  it("describes Telegram rich text only for rich Telegram runtimes", () => {
    ...
    expect(telegramPrompt).toContain("Zuvix renders Telegram-safe rich messages");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/zuvix. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSections", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/zuvix/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("sessions_spawn");
    expect(prompt).toContain('runtime: "acp"');
    expect(prompt).toContain("For ACP harness sessions (claudecode/gemini/zuvix");
    ...
    expect(prompt).toContain("Do not use `exec` (`zuvix ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Zuvix subagents");
  ...
  it("omits ACP spawning guidance when ACP is disabled", () => {
    ...
    expect(prompt).not.toContain("For ACP harness sessions (claudecode/gemini/zuvix");
