...
// https://github.com/zuvix/zuvix/issues/33785
...
describe("failover-error", () => {
  ...
  it("403 OpenRouter 'Key limit exceeded' returns billing (model fallback trigger)", () => {
    // GitHub: zuvix/zuvix#53849 — OpenRouter returns 403 with "Key limit exceeded"
  ...
  describe("isNonProviderRuntimeCoordinationError", () => {
    ...
    const makeEmbeddedTakeoverError = () => {
      const err = new Error(
        "session file changed while embedded prompt lock was released: /tmp/zuvix/session.jsonl",
      );
...
describe("buildFailoverRemediationHint", () => {
  it("returns a copy-pasteable login command for auth failures", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: zuvix models auth login --provider 'anthropic' --force",
    );
  });
  ...
  it("returns a hint for auth_permanent as well", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: zuvix models auth login --provider 'google-gemini-cli' --force",
    );
  });

  it("quotes provider ids that contain shell metacharacters", () => {
    expect(buildProviderReauthCommand("custom;touch /tmp/pwned")).toBe(
      "zuvix models auth login --provider 'custom;touch /tmp/pwned' --force",
    );
    expect(buildProviderReauthCommand("custom'provider")).toBe(
      "zuvix models auth login --provider 'custom'\\''provider' --force",
    );
  });
  ...
  it("wraps rendered provider commands in the standard CLI formatter", () => {
    expect(buildProviderReauthCommand("anthropic", { ZUVIX_PROFILE: "work" })).toBe(
      "zuvix --profile work models auth login --provider 'anthropic' --force",
    );
    expect(buildProviderReauthCommand("anthropic", { ZUVIX_CONTAINER_HINT: "dev" })).toBe(
      "zuvix --container dev models auth login --provider 'anthropic' --force",
    );
  });
