 ...
 * provider-specific payload policy before converting SDK streams into Zuvix assistant events.
 */
...
const OPENAI_RESPONSES_REASONING_REPLAY_META_KEY = "__zuvix_replay";
const OPENAI_RESPONSES_REASONING_REPLAY_BLOCK_META_KEY = "zuvixReasoningReplay";
...
type BaseStreamOptions = {
  ...
  zuvixCodeModeToolSurface?: boolean;
...
function createResponsesFirstEventTimeoutError(model: Model, timeoutMs: number): Error {
  return new Error(
    `Azure OpenAI Responses stream did not deliver a first event within ${timeoutMs}ms after HTTP streaming headers. ` +
      ...
      "The provider may be stalled while parsing the tool payload; retry with a smaller tool surface or enable ZUVIX_DEBUG_MODEL_PAYLOAD=tools to inspect exposed tools.",
  );
}
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { zuvixCodeModeToolSurface?: unknown } | undefined)
            ?.zuvixCodeModeToolSurface === true
        ) {
...
function resolveCacheRetention(cacheRetention: string | undefined): "short" | "long" | "none" {
  ...
  if (typeof process !== "undefined" && process.env.ZUVIX_CACHE_RETENTION === "long") {
    return "long";
  }
  return "short";
}
...
function isOpenAICodexResponsesModel(model: Model): boolean {
  return (
    ...
    (model.api === "openai-chatgpt-responses" ||
      model.api === "zuvix-openai-responses-transport")
  );
}
...
export function buildOpenAIResponsesParams(
  ...
  const messages = convertResponsesMessages(
    model,
    context,
    new Set(["openai", "zuvix", "azure-openai-responses", "github-copilot"]),
    {
...
function getCompletionsContentDeltas(content: unknown): CompletionsReasoningDelta[] {
  ...
  // Preserve provider reasoning as Zuvix thinking blocks so channel/UI
...
// ids (Zuvix Zen exposes `deepseek-v4-flash-free`, OpenRouter exposes
