...
function resolveAttachmentLimits(config: ZuvixConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".zuvix", "attachments");
  const relDir = path.posix.join(".zuvix", "attachments", attachmentId);
