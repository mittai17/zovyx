...
type SkillDiscoveryMode = "zuvix" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { zuvix?: ResourceManifest };
    return pkg.zuvix ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "zuvix");
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "zuvix-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "zuvix"),
      projectMetadata,
    ...
    // User extensions from ~/.zuvix/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.zuvix/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "zuvix"),
      userMetadata,
