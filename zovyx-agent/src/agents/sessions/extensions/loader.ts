...
import * as bundledLlm from "zuvix/plugin-sdk/llm";
...
import { installZuvixInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "zuvix/plugin-sdk/agent-core": bundledAgentCore,
  "@zuvix/plugin-sdk/agent-core": bundledAgentCore,
  "zuvix/plugin-sdk/llm": bundledLlm,
  "@zuvix/plugin-sdk/llm": bundledLlm,
  "zuvix/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@zuvix/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@zuvix\/plugin-sdk|zuvix\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "zuvix/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@zuvix/plugin-sdk/agent-sessions": agentSessionsEntry,
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installZuvixInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.zuvix && typeof pkg.zuvix === "object") {
      return pkg.zuvix as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "zuvix.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "zuvix" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "zuvix" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with Zuvix manifest or index.ts
