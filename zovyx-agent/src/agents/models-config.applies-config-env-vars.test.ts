...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/zuvix-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): ZuvixConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: ZuvixConfig) {
...
let unauthenticatedProviderWritePlan: Awaited<ReturnType<typeof planZuvixModelsJsonWithDeps>>;
...
beforeAll(async () => {
  ...
  unauthenticatedProviderWritePlan = await planZuvixModelsJsonWithDeps(
    {
...
describe("models-config", () => {
  it("keeps the implicit provider catalog when explicit baseUrl is blank", async () => {
    let observedConfig: ZuvixConfig | undefined;
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/zuvix-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planZuvixModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/zuvix-models-config-env-vars-test",
        env: {},
  ...
  it("keeps google-vertex static catalog rows when an auth profile supplies the API key", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-google-vertex-models-"));
    try {
  ...
  it("keeps google-vertex static catalog rows when ADC auth evidence supplies the marker", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-google-vertex-adc-models-"));
    ...
    try {
      const plan = await withEnvAsync(
        {
          ZUVIX_BUNDLED_PLUGINS_DIR: BUNDLED_PLUGINS_DIR,
