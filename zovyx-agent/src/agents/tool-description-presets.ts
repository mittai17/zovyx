...
export function describeSessionsSpawnTool(options?: {
  ...
  return [
    ...
    '`runtime="acp"` for ACP harness ids: codex, claude, gemini, zuvix, or agent ACP runtime config.',
...
export function describeSessionStatusTool(): string {
  return [
    ...
    'Use `sessionKey="current"` for current session; UI labels like `zuvix-tui` are not keys.',
