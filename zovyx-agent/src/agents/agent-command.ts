...
type AcpSessionIdentifiersRuntime = typeof import("@zuvix/acp-core/runtime/session-identifiers");
...
async function prepareAgentCommandExecution(opts: AgentCommandOpts, runtime: RuntimeEnv) {
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("zuvix agents list")}" to see configured agents.`,
      );
    }
  }
