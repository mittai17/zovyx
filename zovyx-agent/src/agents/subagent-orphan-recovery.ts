 ...
 *
 * @see https://github.com/zuvix/zuvix/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /zuvix\.json|zuvix gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify zuvix.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
