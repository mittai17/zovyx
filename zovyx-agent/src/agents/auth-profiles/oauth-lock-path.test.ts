...
describe("resolveOAuthRefreshLockPath", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-auth-lock-path-"));
    setTestEnvValue("ZUVIX_STATE_DIR", stateDir);
  });
...
describe("resolveOAuthRefreshLockPath fuzz", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-auth-lock-path-fuzz-"));
    setTestEnvValue("ZUVIX_STATE_DIR", stateDir);
  });
