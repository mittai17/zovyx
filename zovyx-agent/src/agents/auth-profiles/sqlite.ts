...
import type { DB as ZuvixAgentKyselyDatabase } from "../../state/zuvix-agent-db.generated.js";
import {
  openZuvixAgentDatabase,
  runZuvixAgentWriteTransaction,
  type ZuvixAgentDatabase,
} from "../../state/zuvix-agent-db.js";
import { resolveUserPath } from "../../utils.js";
import { resolveRegisteredAgentIdForDir } from "../agent-dir-registry.js";
import { resolveDefaultAgentDir } from "../agent-scope-config.js";

type AuthProfileDatabase = Pick<
  ZuvixAgentKyselyDatabase,
...
// The auth database lives in the agent dir and shares the zuvix-agent schema
...
function resolveAuthProfileDatabaseOptions(agentDir?: string) {
  ...
  return {
    ...
    path: path.join(dir, "zuvix-agent.sqlite"),
  };
}
...
export function openAuthProfileDatabase(agentDir?: string): ZuvixAgentDatabase {
  return openZuvixAgentDatabase(resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function writePersistedAuthProfileStoreRaw(
  ...
  const write = (target: ZuvixAgentDatabase) => {
  ...
  runZuvixAgentWriteTransaction(write, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function deletePersistedAuthProfileStoreRaw(
  ...
  const remove = (target: ZuvixAgentDatabase) => {
    const db = getAuthProfileKysely(target.db);
    executeSqliteQuerySync(
      target.db,
  ...
  runZuvixAgentWriteTransaction(remove, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function runAuthProfileWriteTransaction<T>(
  ...
  return runZuvixAgentWriteTransaction(operation, resolveAuthProfileDatabaseOptions(agentDir));
}
