// Verifies OpenAI model selections route between Zuvix and Codex runtimes.
...
describe("OpenAI runtime routing policy", () => {
  ...
  it("keeps explicit Zuvix plus Codex auth profile under the unified OpenAI provider", () => {
    // OpenAI auth now stays canonical even when the runtime is not Codex.
    expect(
      listOpenAIAuthProfileProvidersForAgentRuntime({
        provider: "openai",
        harnessRuntime: "zuvix",
      }),
    ).toEqual(["openai"]);
    expect(
      resolveOpenAIRuntimeProvider({
  ...
  it("keeps explicit OpenAI Zuvix API-key auth order ahead of Codex backups", () => {
    const config = {
      auth: {
        order: {
  ...
  it("does not route custom OpenAI-compatible Zuvix configs through Codex auth order", () => {
    const config = {
      models: {
        providers: {
          openai: {
