...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    previousConfigPath = process.env.ZUVIX_CONFIG_PATH;
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-sessions-history-redact-"));
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.ZUVIX_CONFIG_PATH = previousConfigPath;
    }
    if (tempDir) {
