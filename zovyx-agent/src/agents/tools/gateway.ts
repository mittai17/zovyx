...
function resolveLocalGatewayUrlKeys(cfg: ZuvixConfig): Set<string> {
...
function resolveConfiguredRemoteGatewayKey(cfg: ZuvixConfig): string | undefined {
...
export function resolveGatewayOptions(opts?: GatewayCallOptions) {
  ...
  const timeoutMs =
    typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs)
      ...
      : 30_000;
  const envGatewayUrl = trimToUndefined(process.env.ZUVIX_GATEWAY_URL);
  const target =
  ...
  try {
    ...
    throw new Error(
      [
        ...
        "Fix the Zuvix state directory permissions or use the local approval-runtime gateway.",
      ].join(" "),
      { cause: error },
    );
  }
