...
async function createStoreConfig(): Promise<{ config: ZuvixConfig; template: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-goal-tools-"));
  const template = path.join(dir, "{agentId}", "sessions.json");
  return {
    config: { session: { store: template } } as ZuvixConfig,
    template,
  };
}
