...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    zuvix: process.env.ZUVIX_GATEWAY_TOKEN,
    gatewayUrl: process.env.ZUVIX_GATEWAY_URL,
  };
  ...
  afterAll(() => {
    if (envSnapshot.zuvix === undefined) {
      delete process.env.ZUVIX_GATEWAY_TOKEN;
    } else {
      process.env.ZUVIX_GATEWAY_TOKEN = envSnapshot.zuvix;
    }
    if (envSnapshot.gatewayUrl === undefined) {
      delete process.env.ZUVIX_GATEWAY_URL;
    } else {
      process.env.ZUVIX_GATEWAY_URL = envSnapshot.gatewayUrl;
    }
  });
  ...
  it("uses ZUVIX_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    ...
    process.env.ZUVIX_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
  ...
  it("does not send the local approval runtime token to env-selected gateways", async () => {
    process.env.ZUVIX_GATEWAY_URL = "wss://gateway.example";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
  ...
  it("does not send the local approval runtime token to loopback env-selected gateways", async () => {
    process.env.ZUVIX_GATEWAY_URL = "ws://127.0.0.1:18789";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
  ...
  it("does not send the local approval runtime token to loopback env-selected gateway paths", async () => {
    process.env.ZUVIX_GATEWAY_URL = "ws://127.0.0.1:18789/ws";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
