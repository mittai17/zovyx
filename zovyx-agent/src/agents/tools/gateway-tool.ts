...
import { isRecord as isPlainObject } from "@zuvix/normalization-core/record-coerce";
import {
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `zuvix security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as ZuvixConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as ZuvixConfig);
