...
import { isInboundPathAllowed } from "@zuvix/media-core/inbound-path-policy";
...
type CreateZuvixCodingToolsArgs = Parameters<typeof createZuvixCodingTools>[0];
type MockZuvixToolsOptions = {
  config?: ZuvixConfig;
  agentDir?: string;
  workspaceDir?: string;
  sandboxRoot?: string;
...
vi.mock("../auth-profiles.js", () => ({
  ...
  ensureAuthProfileStore: (agentDir?: string) => {
    ...
    if (process.env.ZUVIX_TEST_CODEX_CLI_OAUTH === "1") {
...
vi.mock("../auth-profiles/external-cli-sync.js", () => ({
  resolveExternalCliAuthProfiles: (
    ...
    if (
      process.env.ZUVIX_TEST_CODEX_CLI_OAUTH !== "1" ||
...
vi.mock("../model-auth.js", () => ({
  ...
  hasRuntimeAvailableProviderAuth: (params: {
    ...
    if (params.provider === "codex") {
      return process.env.ZUVIX_TEST_CODEX_ROUTE === "1";
    }
  ...
  hasUsableCustomProviderApiKey: (cfg?: ZuvixConfig, provider?: string) => {
  ...
  resolveEnvApiKey: (provider: string) => {
    const envVarByProvider: Record<string, string[]> = {
      ...
      zuvix: ["OPENCODE_API_KEY", "OPENCODE_ZEN_API_KEY"],
      "zuvix-go": ["OPENCODE_API_KEY", "OPENCODE_ZEN_API_KEY"],
...
vi.mock("../zuvix-tools.js", async () => {
  const { createImageTool: createImageToolLocal } = await import("./image-tool.js");
  return {
    createZuvixTools: vi.fn((options?: MockZuvixToolsOptions) => {
...
async function createZuvixCodingToolsWithFreshModules(options?: CreateZuvixCodingToolsArgs) {
  ...
  return createZuvixCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "zuvix-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): ZuvixConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  registerImageToolEnvReset(priorFetch, [
    ...
    "ZAI_API_KEY",
    "Z_AI_API_KEY",
    "ZUVIX_TEST_CODEX_CLI_OAUTH",
    "ZUVIX_TEST_CODEX_ROUTE",
  ...
  it.each(implicitImageRoutingCases)(
    ...
    async ({ cfg, profiles, codexRoute, openAiApiKey, expected }) => {
      if (codexRoute) {
        vi.stubEnv("ZUVIX_TEST_CODEX_ROUTE", "1");
      }
      if (openAiApiKey) {
  ...
  it("lets external CLI Codex OAuth survive the candidate auth filter", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("ZUVIX_TEST_CODEX_CLI_OAUTH", "1");
      vi.stubEnv("ZUVIX_TEST_CODEX_ROUTE", "1");
  ...
  it("honors a per-call model override when no imageModel is configured", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const result = await tool.execute("t1", {
        ...
        model: "zuvix-go/mimo-v2-omni",
      });

      const request = firstImageRequest(describeImage);
      expect(request.provider).toBe("zuvix-go");
      expect(request.model).toBe("mimo-v2-omni");
      expectToolText(result, "ok zuvix-go/mimo-v2-omni");
    });
  });
  ...
  it("pairs zuvix primary with the plugin-owned image model when auth exists", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("OPENCODE_API_KEY", "zuvix-test");
      const cfg: ZuvixConfig = {
        agents: { defaults: { model: { primary: "zuvix/minimax-m2.7" } } },
      };
      expect(resolveImageModelConfigForTool({ cfg, agentDir })).toEqual({
        primary: "zuvix/gpt-5-nano",
      });
      expect(typeof createImageTool({ config: cfg, agentDir })?.execute).toBe("function");
    });
  });

  it("pairs zuvix-go primary with the Go plugin-owned image model when auth exists", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("OPENCODE_API_KEY", "zuvix-test");
      const cfg: ZuvixConfig = {
        agents: { defaults: { model: { primary: "zuvix-go/minimax-m2.7" } } },
      };
      expect(resolveImageModelConfigForTool({ cfg, agentDir })).toEqual({
        primary: "zuvix-go/kimi-k2.6",
      });
      expect(typeof createImageTool({ config: cfg, agentDir })?.execute).toBe("function");
    });
  });
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-outside-"));
  ...
  it("still rejects non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows image paths from the current iMessage account attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-imessage-root-"));
  ...
  it("allows image paths from current iMessage wildcard attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRootParent = await fs.mkdtemp(
        path.join(os.tmpdir(), "zuvix-imessage-wildcard-root-"),
      );
  ...
  it("allows workspace images via createZuvixCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createZuvixCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.zuvix/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
    const tool = createRequiredImageTool({ config: cfg, agentDir });
    return { fetch, tool };
...
describe("image tool managed inbound media", () => {
  ...
  async function withManagedInboundPng(
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-image-managed-inbound-"));
