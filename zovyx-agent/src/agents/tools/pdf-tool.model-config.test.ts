...
const TEST_AGENT_DIR = "/tmp/zuvix-pdf-model-config";
...
vi.mock("./model-config.helpers.js", () => ({
  ...
  hasProviderAuthForTool: ({ provider, cfg }: { provider: string; cfg?: ZuvixConfig }) => {
  ...
  resolveDefaultModelRef: (cfg?: ZuvixConfig) => {
