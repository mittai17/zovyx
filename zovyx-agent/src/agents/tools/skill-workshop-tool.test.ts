...
describe("skill_workshop tool", () => {
  it("is exposed in the Zuvix tool set", async () => {
    const workspaceDir = await tempDirs.make("zuvix-skill-workshop-tool-");
  ...
  it("is not exposed from sandboxed Zuvix tool sets", async () => {
    const workspaceDir = await tempDirs.make("zuvix-skill-workshop-tool-");
    const tools = createZuvixTools({
      workspaceDir,
      config: {},
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("zuvix-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("zuvix-skill-workshop-tool-second-");
