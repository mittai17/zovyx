...
const LIVE_IMAGE_TOOL_ENABLED = isLiveTestEnabled(["ZUVIX_LIVE_IMAGE_TOOL_TEST"]);
const LIVE_OPENAI_MODEL =
  process.env.ZUVIX_LIVE_IMAGE_TOOL_OPENAI_MODEL?.trim() ||
  process.env.ZUVIX_LIVE_IMAGE_TOOL_MODEL?.trim() ||
  "gpt-4.1-mini";
const LIVE_ANTHROPIC_MODEL =
  process.env.ZUVIX_LIVE_IMAGE_TOOL_ANTHROPIC_MODEL?.trim() || "claude-sonnet-4-6";
...
function createLiveConfig(testCase: LiveProviderCase): ZuvixConfig {
  return {
    agents: {
      defaults: {
...
async function withLiveWorkspace<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-image-tool-live-"));
  try {
