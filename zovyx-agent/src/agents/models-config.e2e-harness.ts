...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousZuvixAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousZuvixAgentDir = process.env.ZUVIX_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousZuvixAgentDir === undefined) {
      delete process.env.ZUVIX_AGENT_DIR;
    } else {
      process.env.ZUVIX_AGENT_DIR = previousZuvixAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "ZUVIX_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: ZuvixConfig = {
  models: {
    providers: {
      "custom-proxy": {
