...
const ensureZuvixModelsJsonMock = vi.fn(async () => ({ wrote: false }));
const loggerWarnMock = vi.fn();
let refreshRuntimeAuthOnFirstPromptError = false;

vi.mock("zuvix/plugin-sdk/llm", async () => {
  const actual =
    await vi.importActual<typeof import("zuvix/plugin-sdk/llm")>("zuvix/plugin-sdk/llm");
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureZuvixModelsJson: (...args: Parameters<typeof ensureZuvixModelsJsonMock>) =>
        ensureZuvixModelsJsonMock(...args),
    };
  });
};

let runEmbeddedAgent: typeof import("./embedded-agent-runner/run.js").runEmbeddedAgent;
let SessionManager: typeof import("zuvix/plugin-sdk/agent-sessions").SessionManager;
...
beforeAll(async () => {
  ...
  ({ SessionManager } = await import("zuvix/plugin-sdk/agent-sessions"));
  e2eWorkspace = await createEmbeddedAgentRunnerTestWorkspace("zuvix-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureZuvixModelsJsonMock.mockReset();
  ensureZuvixModelsJsonMock.mockResolvedValue({ wrote: false });
...
describe("runEmbeddedAgent", () => {
  ...
  it("resolves explicit OpenAI Zuvix runs through Codex when auth order starts with Codex OAuth", async () => {
    const sessionFile = nextSessionFile();
    ...
    await runEmbeddedAgent({
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("codex-first-zuvix"),
      enqueue: immediateEnqueue,
    });
