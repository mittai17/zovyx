...
describe("sanitizeUserFacingText", () => {
  ...
  it.each(["disk full", "ENOSPC: no space left on device"])(
    ...
    (input) => {
      expect(sanitizeUserFacingText(input, { errorContext: true })).toBe(
        "Zuvix could not write local session data because the disk is full. Free some disk space and try again.",
      );
    },
  );
  ...
  it("strips internal tool trace warning lines from error-context delivery text", () => {
    const input = [
      "Visible intro.",
      "⚠️ 🛠️ `run zuvix definitely-not-a-real-subcommand (agent)` failed",
      "⚠️ 🛠️ gh search issues --repo zuvix/zuvix --state open --no-search-pages.jsonl /tmp/zuvix_open_unlabeled_current.json (agent) failed",
      "⚠️ 🛠️ gh search issues --repo zuvix/zuvix --state open (agent) failed: command timed out",
      "🛠️ run git status",
  ...
  it("strips copied inbound metadata blocks from user-facing assistant text", () => {
    const input = [
      ...
      "```json",
      '{"chat_id":"channel:123","sender":"Zuvix"}',
      "```",
      "",
  ...
  it("does not strip ordinary text that merely mentions internal marker strings", () => {
    const input = [
      "The literal header `Zuvix runtime context (internal):` appears in this note.",
  ...
  it("does not strip text that starts with the legacy header phrase but is not the canonical block", () => {
    const input =
      "Zuvix runtime context (internal): is the label used by the old runtime block formatter.";
