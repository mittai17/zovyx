...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__zuvixCatalog) ? globalThis.__zuvixCatalog : [];
  const apiFiles = Array.isArray(globalThis.__zuvixApiFiles) ? globalThis.__zuvixApiFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__zuvixNamespaces) ? globalThis.__zuvixNamespaces : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__zuvixHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  Object.defineProperties(globalThis, {
    ...
    __zuvixSettleBridge: { value: settle },
    __zuvixTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__zuvixResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__zuvixCatalog", catalogHandle);
  } finally {
    catalogHandle.dispose();
  }
  const namespacesHandle = vm.hostToHandle(params.namespaces);
  try {
    vm.setProp(vm.global, "__zuvixNamespaces", namespacesHandle);
  } finally {
    namespacesHandle.dispose();
  }
  const apiFilesHandle = vm.hostToHandle(params.apiFiles);
  try {
    vm.setProp(vm.global, "__zuvixApiFiles", apiFilesHandle);
  } finally {
  ...
  try {
    vm.setProp(vm.global, "__zuvixHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "zuvix-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__zuvixTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__zuvixResult");
}
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  let output: unknown[] = [];
  try {
    const settle = vm.global.getProp("__zuvixSettleBridge");
    try {
