...
import type { Context, Model } from "zuvix/plugin-sdk/llm";
...
const TARGET_MODEL_REFS = parseTargetModelRefs(
  process.env.ZUVIX_LIVE_TOOL_REPLAY_REPAIR_MODELS ?? DEFAULT_TARGET_MODEL_REFS,
);
...
function parseTargetModelRefs(raw: string | undefined): TargetModelRef[] {
  ...
  for (const item of (raw ?? "").split(",")) {
    ...
    if (!provider?.trim() || !modelId) {
      throw new Error(
        `Invalid ZUVIX_LIVE_TOOL_REPLAY_REPAIR_MODELS entry: ${JSON.stringify(ref)}`,
      );
    }
