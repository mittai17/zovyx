...
const providerEndpointPlugins = vi.hoisted(() => [
  {
    ...
    providerEndpoints: [
      ...
      { endpointClass: "zuvix-native", hostSuffixes: ["zuvix.ai"] },
...
vi.mock("../plugins/manifest-metadata-scan.js", () => ({
  listZuvixPluginManifestMetadata: () =>
...
describe("provider attribution", () => {
  it("resolves the canonical Zuvix product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      ZUVIX_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Zuvix runtime wrapper.",
      product: "Zuvix",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a documented NVIDIA attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      version: "2026.3.22",
      headers: {
        "X-BILLING-INVOKE-ORIGIN": "Zuvix",
      },
    });
    expect(resolveProviderAttributionHeaders("NVIDIA", { ZUVIX_VERSION: "2026.3.22" })).toEqual({
      "X-BILLING-INVOKE-ORIGIN": "Zuvix",
    });
  });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { ZUVIX_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { ZUVIX_VERSION: "2026.3.22" })).toEqual({
      originator: "zuvix",
      version: "2026.3.22",
      "User-Agent": "zuvix/2026.3.22",
    });
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { ZUVIX_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
      reviewNote:
        "xAI api.x.ai accepts a standard zuvix User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
      product: "Zuvix",
      version: "2026.3.22",
      headers: {
        originator: "zuvix",
    ...
    expect(resolveProviderAttributionHeaders("xai", { ZUVIX_VERSION: "2026.3.22" })).toEqual({
      originator: "zuvix",
      version: "2026.3.22",
      "User-Agent": "zuvix/2026.3.22",
    });
  ...
  it("lists the current attribution support matrix", () => {
    ...
    expect(
      listProviderAttributionPolicies({ ZUVIX_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
  ...
  it("classifies native OpenAI-compatible vendor hosts centrally", () => {
    ...
    expectRecordFields(resolveProviderEndpoint("https://zuvix.ai/api"), {
      endpointClass: "zuvix-native",
      hostname: "zuvix.ai",
    });
