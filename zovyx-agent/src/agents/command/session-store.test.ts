...
async function withTempSessionStore<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-session-store-"));
  try {
...
describe("updateSessionStoreAfterAgentRun", () => {
  ...
  it("persists claude-cli session bindings when the backend is configured", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const sessionId = "test-zuvix-session";
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          sessionId,
          updatedAt: 1,
  ...
  it("preserves runtime model and contextTokens when preserveRuntimeModel is true (heartbeat bleed fix)", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      expect(sessionStore[sessionKey]?.agentHarnessId).toBe("zuvix");
      ...
      expect(persisted[sessionKey]?.agentHarnessId).toBe("zuvix");
