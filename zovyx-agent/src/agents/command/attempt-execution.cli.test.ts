...
function firstEmbeddedAgentArg(callIndex = 0) {
  return requireMockArg(runEmbeddedAgentMock, callIndex, "embedded Zuvix agent argument");
}
...
describe("CLI attempt execution", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-cli-attempt-"));
  ...
  function makeClaudeCliSessionEntry(
    zuvixSessionId: string,
    cliSessionId: string,
  ): SessionEntry {
    return {
      sessionId: zuvixSessionId,
      updatedAt: Date.now(),
      cliSessionBindings: {
        "claude-cli": {
  ...
  it("checks Claude CLI transcript content under the process cwd", async () => {
    ...
    const sessionEntry = makeClaudeCliSessionEntry("zuvix-session-cwd", cliSessionId);
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runCliAgentMock.mockResolvedValueOnce(makeCliResult("resumed cli response"));
  ...
  it("embedded assistant gap-fill skips trailing zuvix.cache-ttl custom entries (regression for #83427)", async () => {
  ...
  it("embedded assistant gap-fill appends repeated replies after a user tail", async () => {
    ...
    expect(
      sessionFile.endsWith(
        path.join(".zuvix", "agents", "main", "sessions", `${sessionEntry.sessionId}.jsonl`),
      ),
    ).toBe(true);
  ...
  it("forwards separate user trigger, channel, and provider context to CLI runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-channel",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards runtime toolsAllow into CLI attempts so the CLI harness can fail closed", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-cli-tools-allow",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("stamps CLI prompts with current timestamp context", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-cli-timestamp",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    await runAgentAttempt({
      ...
      originalProvider: "claude-cli",
      modelOverride: "opus",
      cfg: { agents: { defaults: { userTimezone: "UTC" } } } as ZuvixConfig,
      sessionEntry,
      sessionId: sessionEntry.sessionId,
      sessionKey,
      sessionAgentId: "main",
  ...
  it("routes canonical Anthropic models through the configured Claude CLI runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-canonical-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("routes provider-qualified Anthropic shorthand through the configured Claude CLI runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-shorthand-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("routes canonical OpenAI models through the configured embedded Codex runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-canonical-codex-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards user-pinned OpenAI API-key backup profiles to Codex harness runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-openai-chatgpt-api-key",
      updatedAt: Date.now(),
  ...
  it("keeps one-shot model runs on the raw embedded provider path", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-model-run-raw",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards trusted elevated defaults to embedded agent runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-elevated-followup",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    const bashElevated = {
  ...
  it("forwards one-shot CLI cleanup to CLI providers", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "zuvix-session-cleanup-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
...
describe("embedded attempt harness pinning", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-embedded-attempt-"));
    runCliAgentMock.mockReset();
    runEmbeddedAgentMock.mockReset();
  });
  ...
  it("ignores stale OpenAI sessions pinned to Zuvix and relies on default Codex routing", async () => {
    const sessionEntry: SessionEntry = {
  ...
  it("routes explicit OpenAI native runs with legacy Codex OAuth through Zuvix", async () => {
    const sessionEntry: SessionEntry = {
