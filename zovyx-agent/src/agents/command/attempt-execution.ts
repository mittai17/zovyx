 ...
 */
import type { AcpRuntimeEvent } from "@zuvix/acp-core/runtime/types";
...
export function runAgentAttempt(params: {
  ...
  const requestedAgentHarnessId = isRawModelRun ? "zuvix" : undefined;
  ...
  const agentHarnessPolicy = isRawModelRun
    ? ({ runtime: "zuvix", runtimeSource: "model" } as const)
  ...
  const embeddedAgentHarnessOverride =
    ...
    (agentHarnessPolicy.runtime === "zuvix" && agentHarnessPolicy.runtimeSource !== "implicit"
      ? "zuvix"
      : undefined);
