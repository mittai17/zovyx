...
import type { Context, Model, SimpleStreamOptions } from "zuvix/plugin-sdk/llm";
...
describe("applyExtraParamsToAgent", () => {
  ...
  it("passes agentDir and workspaceDir to provider stream wrappers", () => {
    ...
    expect(capturedContext?.agentDir).toBe("/tmp/zuvix-agent");
    expect(capturedContext?.workspaceDir).toBe("/tmp/zuvix-workspace");
  ...
  it("promotes reasoning-only MiMo V2 proxy finals to visible text", async () => {
    ...
    const agent = { streamFn: baseStreamFn };
    applyExtraParamsToAgent(agent, undefined, "zuvix", "xiaomi/mimo-v2-pro", undefined, "high");
