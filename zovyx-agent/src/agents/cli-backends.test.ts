...
function createClaudeCliOverrideConfig(config: CliBackendConfig): ZuvixConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from Zuvix's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__zuvix__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__zuvix__*");
  ...
  it("keeps Claude permission mode unset when Zuvix exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
