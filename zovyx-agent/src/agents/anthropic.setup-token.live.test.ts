...
import { completeSimple, type Model } from "zuvix/plugin-sdk/llm";
...
const SETUP_TOKEN_MODEL = process.env.ZUVIX_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "ZUVIX_LIVE_SETUP_TOKEN did not look like a setup-token. Use ZUVIX_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set ZUVIX_LIVE_SETUP_TOKEN_VALUE or ZUVIX_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureZuvixModelsJson(cfg, tokenSource.agentDir);
