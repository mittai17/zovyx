...
function resolveAdaptiveReadMaxBytes(options?: ZuvixReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".zuvix/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.zuvix/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with Zuvix result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  const base = createReadTool(params.root, {
    operations: createSandboxReadOperations(params),
  }) as unknown as AnyAgentTool;
  return createZuvixReadTool(base, {
...
/** Wrap the base read tool with Zuvix paging, MIME, and image handling. */
export function createZuvixReadTool(
  base: AnyAgentTool,
  options?: ZuvixReadToolOptions,
): AnyAgentTool {
  return {
    ...base,
