...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: ZuvixConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<ZuvixConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<ZuvixConfig["agents"]>["list"]>[number]["tools"];
  }): ZuvixConfig {
    return {
      tools: params.tools,
      agents: {
