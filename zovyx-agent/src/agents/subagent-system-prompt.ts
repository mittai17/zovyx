...
export function buildSubagentSystemPrompt(params: {
  ...
  if (canSpawn) {
    lines.push(
      ...
      ...(acpEnabled
        ? [
            'For ACP harness sessions (claudecode/gemini/zuvix, or Codex only when explicit ACP/acpx), use `sessions_spawn` with `runtime: "acp"` (set `agentId` unless `acp.defaultAgent` is configured).',
            '`agents_list` and `subagents` apply to Zuvix sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`zuvix ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for Zuvix subagents (`runtime: "subagent"`).',
