/** Tests merging bundled MCP defaults with Zuvix user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets Zuvix mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps Zuvix transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled Zuvix MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled Zuvix MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
