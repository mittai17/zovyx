 ...
 * ZUVIX_FALLBACK_SKIP_TTL_MS.
 */
export const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "ZUVIX_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    zuvixFallbackSkipCache?: SkipBySession;
    zuvixFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.zuvixFallbackSkipCacheState) {
    ...
    const buckets = globalStore.zuvixFallbackSkipCache ?? new Map();
    globalStore.zuvixFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.zuvixFallbackSkipCache = buckets;
  }
  return globalStore.zuvixFallbackSkipCacheState;
}
