...
import { MAX_TIMER_TIMEOUT_MS } from "@zuvix/normalization-core/number-coercion";
...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("reports live Zuvix-owned stale locks without removing them", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      const owner = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)", "zuvix"], {
        stdio: "ignore",
      });
      if (!owner.pid) {
  ...
  it("lets session write-lock env override config for emergency tuning", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            ZUVIX_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "120000",
            ZUVIX_SESSION_WRITE_LOCK_STALE_MS: "60000",
            ZUVIX_SESSION_WRITE_LOCK_MAX_HOLD_MS: "50000",
          },
        },
      ),
    ).toEqual({
  ...
  it("ignores non-decimal and unsafe session write-lock env values", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            ZUVIX_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "1e3",
            ZUVIX_SESSION_WRITE_LOCK_STALE_MS: "0x1000",
            ZUVIX_SESSION_WRITE_LOCK_MAX_HOLD_MS: "9007199254740993",
          },
        },
      ),
    ).toEqual({
  ...
  it("allows explicit infinite acquire timeout but not infinite stale policy", () => {
    expect(
      resolveSessionWriteLockOptions(undefined, {
        env: {
          ZUVIX_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "Infinity",
          ZUVIX_SESSION_WRITE_LOCK_STALE_MS: "Infinity",
        },
      }),
    ).toMatchObject({
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-lock-policy-"));
    ...
    try {
      ...
      const configOnly = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/zuvix/zuvix.mjs", "doctor"],
      });
      ...
      const envOverride = await cleanStaleLockFiles({
        sessionsDir,
        config: { session: { writeLock: { staleMs: 30_000 } } },
        env: { ZUVIX_SESSION_WRITE_LOCK_STALE_MS: "60000" },
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/zuvix/zuvix.mjs", "doctor"],
      });
  ...
  it("does not clean live Zuvix locks just because holder max hold expired", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-lock-policy-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/zuvix/zuvix.mjs", "agent"],
      });
  ...
  it("cleans old live .jsonl lock files owned by non-Zuvix processes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    const lockPath = path.join(sessionsDir, "old-non-zuvix.jsonl.lock");
    ...
    try {
      ...
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
          name: "old-non-zuvix.jsonl.lock",
          removed: true,
          stale: true,
          staleReasons: ["too-old", "non-zuvix-owner"],
        },
      ]);
      await expectPathMissing(lockPath);
    } finally {
  ...
  it("cleans fresh live .jsonl lock files owned by a non-Zuvix process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-Zuvix entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps fresh live .jsonl lock files with Zuvix or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const zuvixLock = path.join(sessionsDir, "zuvix-live.jsonl.lock");
    ...
    try {
      ...
      const zuvixResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      expect(zuvixResult.cleaned).toEqual([]);
      await expect(fs.access(zuvixLock)).resolves.toBeUndefined();

      await fs.rm(zuvixLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          pid: process.pid,
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
