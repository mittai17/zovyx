/**
 * Builds the effective Zuvix agent tool surface.
 * Assembles core, shell, channel, Zuvix, plugin, and Tool Search tools, then
...
function resolveExecConfig(params: { cfg?: ZuvixConfig; agentId?: string }) {
...
export type ZuvixCodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includeZuvixTools: boolean;
  includePluginTools: boolean;
};

/** Build the runtime tool list for one agent run. */
export function createZuvixCodingTools(options?: {
  agentId?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep Zuvix web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
  ...
  toolConstructionPlan?: ZuvixCodingToolConstructionPlan;
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeZuvixTools: includeCoreTools,
  ...
  const includeZuvixTools = includeCoreTools && toolConstructionPlan.includeZuvixTools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(codingRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createZuvixReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
        base.push(
  ...
  const pluginToolsOnly =
    includeZuvixTools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("zuvix-tools");
  ...
  // Always normalize tool JSON Schemas before handing them to Zuvix model runtime.
