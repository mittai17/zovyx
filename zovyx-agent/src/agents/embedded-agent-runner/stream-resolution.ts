...
let zuvixNativeCodexResponsesStreamFnForTest: StreamFn | undefined;
...
function isDefaultZuvixStreamFnForModel(
...
function resolveZuvixNativeCodexResponsesStreamFn(params: {
  model: EmbeddedRunAttemptParams["model"];
  ...
  if (!isDefaultZuvixStreamFnForModel(params.model, params.currentStreamFn)) {
    return undefined;
  }
  return zuvixNativeCodexResponsesStreamFnForTest ?? params.currentStreamFn ?? streamSimple;
}
...
export function describeEmbeddedAgentStreamStrategy(params: {
  ...
  if (
    resolveZuvixNativeCodexResponsesStreamFn({
      model: params.model,
      currentStreamFn: params.currentStreamFn,
    })
  ) {
    return "zuvix-native-codex-responses";
  }
  if (isDefaultZuvixStreamFnForModel(params.model, params.currentStreamFn)) {
...
export function resolveEmbeddedAgentStreamFn(params: {
  ...
  const zuvixNativeCodexResponsesStreamFn = resolveZuvixNativeCodexResponsesStreamFn({
    model: params.model,
    currentStreamFn: params.currentStreamFn,
  });
  if (zuvixNativeCodexResponsesStreamFn) {
    return wrapEmbeddedAgentStreamFn(zuvixNativeCodexResponsesStreamFn, {
      runSignal: params.signal,
      resolvedApiKey: params.resolvedApiKey,
      authProfileId: params.authProfileId,
      authStorage: params.authStorage,
  ...
  if (
    isDefaultZuvixStreamFnForModel(params.model, params.currentStreamFn) ||
    hasResolvedRuntimeApiKey(params.resolvedApiKey)
  ) {
    const boundaryAwareStreamFn = createBoundaryAwareStreamFnForModel(params.model);
    if (boundaryAwareStreamFn) {
      // Some Zuvix session factories return a provider-specific stream wrapper
      // once runtime auth is resolved. Keep transport-supported APIs on
      // Zuvix's HTTP transport so provider-specific auth/header semantics
...
export const testing = {
  setZuvixNativeCodexResponsesStreamFnForTest(streamFn: StreamFn | undefined): void {
    zuvixNativeCodexResponsesStreamFnForTest = streamFn;
  },
  resetZuvixNativeCodexResponsesStreamFnForTest(): void {
    zuvixNativeCodexResponsesStreamFnForTest = undefined;
  },
};
