...
import type { StreamFn } from "zuvix/plugin-sdk/agent-core";
...
afterEach(() => {
  testing.resetZuvixNativeCodexResponsesStreamFnForTest();
});
...
describe("describeEmbeddedAgentStreamStrategy", () => {
  ...
  it("describes default Codex fallback as Zuvix native", () => {
    expect(
      describeEmbeddedAgentStreamStrategy({
        currentStreamFn: undefined,
        model: {
    ...
    ).toBe("zuvix-native-codex-responses");
  });
...
describe("resolveEmbeddedAgentStreamFn", () => {
  ...
  it("routes Codex responses fallbacks through Zuvix native transport", async () => {
    // Codex OAuth models use the Zuvix native transport, with prompt-cache
    ...
    testing.setZuvixNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
      currentStreamFn: undefined,
      sessionId: "session-1",
      model: {
  ...
  it("routes Zuvix native OpenAI-compatible provider streams through boundary-aware transports", async () => {
  ...
  it("injects the resolved run api key into the Zuvix native Codex Responses fallback", async () => {
  ...
  it("falls back to authStorage when no resolved api key is available for Zuvix native fallback", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const authStorage = {
  ...
  it("forwards the run abort signal into the Zuvix native fallback when callers omit one", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setZuvixNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("does not overwrite an explicit signal on the Zuvix native fallback path", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    const explicitSignal = new AbortController().signal;
    testing.setZuvixNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
  ...
  it("forwards the run signal on the sync Zuvix native fallback path without auth credentials", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setZuvixNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("strips cache boundary markers on the Zuvix native fallback path", async () => {
