...
describe("readTranscriptFileState", () => {
  it("skips malformed session entries without moving the active leaf", async () => {
    ...
    const root = await makeRoot("zuvix-transcript-state-malformed-");
  ...
  it("keeps assistant rows with legacy string content", async () => {
    const root = await makeRoot("zuvix-transcript-state-assistant-string-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("preserves repair-supported assistant tool call payload shapes", async () => {
    const root = await makeRoot("zuvix-transcript-state-tool-input-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("preserves Zuvix-authored non-model content blocks", async () => {
    const root = await makeRoot("zuvix-transcript-state-zuvix-blocks-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("preserves empty compaction summary entries as the active leaf", async () => {
    const root = await makeRoot("zuvix-transcript-state-empty-compaction-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("skips JSON-valid non-object rows", async () => {
    const root = await makeRoot("zuvix-transcript-state-null-row-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("skips JSON-valid non-object rows before legacy migration", async () => {
    const root = await makeRoot("zuvix-transcript-state-v1-null-row-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("preserves legacy compaction keep indexes across JSON-valid non-object rows", async () => {
    const root = await makeRoot("zuvix-transcript-state-v1-compaction-null-row-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("relinks valid current rows past malformed parents", async () => {
    const root = await makeRoot("zuvix-transcript-state-current-suffix-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("remaps compaction keep markers past malformed rows", async () => {
    const root = await makeRoot("zuvix-transcript-state-compaction-marker-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("keeps valid suffixes when a compaction marker points at a malformed root", async () => {
    const root = await makeRoot("zuvix-transcript-state-compaction-root-marker-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("remaps compaction keep markers through consecutive malformed rows", async () => {
    const root = await makeRoot("zuvix-transcript-state-compaction-chain-marker-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("remaps malformed compaction markers to descendants on the active branch", async () => {
    const root = await makeRoot("zuvix-transcript-state-compaction-branch-marker-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("does not hang on rejected parent cycles", async () => {
    const root = await makeRoot("zuvix-transcript-state-rejected-cycle-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("drops missing parents reached through rejected rows before rewrite replay", async () => {
    const root = await makeRoot("zuvix-transcript-state-rejected-missing-parent-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("drops labels targeting rejected entries before transcript rewrite replay", async () => {
    const root = await makeRoot("zuvix-transcript-state-rejected-label-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("keeps legacy roots that are missing tree metadata", async () => {
    const root = await makeRoot("zuvix-transcript-state-legacy-root-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
  ...
  it("relinks migrated legacy suffixes past malformed rows", async () => {
    const root = await makeRoot("zuvix-transcript-state-legacy-suffix-");
    const sessionFile = path.join(root, "session.jsonl");
    await fs.writeFile(
      sessionFile,
      [
