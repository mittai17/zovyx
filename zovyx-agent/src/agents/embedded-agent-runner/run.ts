 ...
 * See: https://github.com/zuvix/zuvix/issues/60552
 */
...
async function runEmbeddedAgentInternal(
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(async () => {
      ...
      const pluginHarnessOwnsTransport = agentHarness.id !== "zuvix";
      ...
      for (const candidateProvider of modelResolutionProviders) {
        const candidateResolution = await resolveModelAsync(
          ...
          params.config,
          {
            ...
            // first generating Zuvix models.json. This keeps one-shot model runs from
      ...
      if (!modelResolution) {
        await ensureZuvixModelsJson(params.config, agentDir, {
          workspaceDir: resolvedWorkspace,
        });
        for (const candidateProvider of modelResolutionProviders) {
          const candidateResolution = await resolveModelAsync(
      ...
      const pluginHarnessNeedsZuvixAuthBootstrap =
        pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      const zuvixNativeCodexResponsesNeedsAuthBootstrap =
        !pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      let piExternalCliAuthScope = pluginHarnessOwnsTransport
        ? { ignoreAutoPreferredProfile: false }
        : zuvixNativeCodexResponsesNeedsAuthBootstrap
          ? {
      ...
      if (
        !pluginHarnessOwnsTransport &&
        !pluginHarnessNeedsZuvixAuthBootstrap &&
      ...
      const authStore =
        pluginHarnessOwnsTransport && !pluginHarnessNeedsZuvixAuthBootstrap
          ? createEmptyAuthProfileStore()
          : pluginHarnessNeedsZuvixAuthBootstrap
      ...
      // Plugin harnesses own their model transport/auth. Running Zuvix's generic
      ...
      if (!pluginHarnessOwnsTransport || pluginHarnessNeedsZuvixAuthBootstrap) {
      ...
      const maybeMarkAuthProfileFailure = async (failure: {
        ...
        if (pluginHarnessOwnsTransport && reason === "timeout") {
          ...
          // credential evidence. Do not poison Zuvix auth cooldowns.
          return;
        }
      ...
      try {
        ...
        while (true) {
          ...
          const rawAttempt = await runEmbeddedAttemptWithBackend({
            ...
            // attempt too. Otherwise plugin-owned transports can skip Zuvix auth
            // bootstrap but drift back to Zuvix when the attempt is created.
            agentHarnessId: agentHarness.id,
            ...(params.sessionKey
              ? {
            ...
            // These harnesses build Zuvix tools internally. Keep transport auth
            // scoped while letting tool construction see plugin/provider creds.
            toolAuthProfileStore: harnessBuildsZuvixTools ? attemptAuthProfileStore : undefined,
            modelRegistry,
            agentId: workspaceResolution.agentId,
