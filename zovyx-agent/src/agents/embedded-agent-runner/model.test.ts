...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/zuvix.plugin.json and src/plugins/manifest-model-suppression.ts.
  ...
  return {
    ...
    buildSuppressedBuiltInModelError: ({
      ...
      if (
        ...
        return `Unknown model: ${provider}/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run \`zuvix models auth login --provider openai\` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.`;
      }
      return undefined;
    },
  };
...
import type { ZuvixConfig, ZuvixConfigInput } from "../../config/config.js";
...
describe("resolveModel", () => {
  ...
  it("invalidates agent discovery stores when generated plugin catalogs change", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-model-cache-plugin-"));
  ...
  it("invalidates agent discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-model-cache-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("uses the resolved default agent workspace for cached model discovery", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-model-workspace-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("invalidates agent discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-model-cache-state-"));
    const agentDir = path.join(rootDir, "agents", "worker", "agent");
    const mainAgentDir = path.join(rootDir, "agents", "main", "agent");
    fs.mkdirSync(agentDir, { recursive: true });
    fs.mkdirSync(mainAgentDir, { recursive: true });
    try {
      await withEnvAsync({ ZUVIX_STATE_DIR: rootDir }, async () => {
        mockDiscoveredModel(discoverModels, {
          provider: "openai",
          modelId: "gpt-5.5",
          templateModel: {
  ...
  it("skips Zuvix auth and model discovery during dynamic model resolution", async () => {
  ...
  it("prefers user zuvix.json config over the Fireworks manifest for the same id", () => {
  ...
  it("defaults missing model cost before handing models to Zuvix", () => {
  ...
  it("does not create fallback models from provider overlays alone", () => {
    ...
    } satisfies ZuvixConfigInput;

    const result = resolveModelForTest(
      "typoProvider",
      "typoed-model",
      "/tmp/agent",
      cfg as unknown as ZuvixConfig,
    );
  ...
  it("does not build an openai fallback for removed gpt-5.3-codex-spark", () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: openai/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run `zuvix models auth login --provider openai` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.",
    );
  });
