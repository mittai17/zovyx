...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const zuvix = (message as unknown as Record<string, unknown>)["__zuvix"];
    if (!zuvix || typeof zuvix !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (zuvix as { beforeAgentRunBlocked?: unknown })
    ...
    const nextZuvix = {
      ...(zuvix as Record<string, unknown>),
