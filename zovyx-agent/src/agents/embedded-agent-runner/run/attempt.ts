...
import {
  ...
  ZUVIX_EMBEDDED_CONTEXT_ENGINE_HOST,
...
import { resolveZuvixReferencePaths } from "../../docs-path.js";
import {
  ...
  try {
    ...
    if (activeContextEngine && activeContextEngine.info.id !== "legacy") {
      assertContextEngineHostSupport({
        ...
        operation: "agent-run",
        host: ZUVIX_EMBEDDED_CONTEXT_ENGINE_HOST,
      });
    }
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-zuvix-coding-tools");
    ...
    const zuvixReferences = await resolveZuvixReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Zuvix
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores Zuvix runtime's auto-compaction (zuvix#75799), so re-apply
      // both guards.
      applyAgentCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact Zuvix-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal Zuvix agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        if (isRawModelRun) {
          ...
          if (params.sessionKey && params.config && !isRawModelRun) {
            // Capability guidance must include deferred Zuvix tools without
      ...
      toolSearchCatalogExecutor = async (toolParams) => {
        try {
          if (toolParams.source === "zuvix" && toolParams.sourceName === "core") {
      ...
      try {
        ...
        await sessionLockController.withSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/zuvix/zuvix/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("zuvix:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
      ...
      // See: https://github.com/zuvix/zuvix/issues/8643
