...
type PromptReleaseStreamFn = ((...args: unknown[]) => unknown) & {
  __zuvixSessionLockPromptReleaseInstalled?: boolean;
};
...
const TRANSCRIPT_ONLY_ZUVIX_ASSISTANT_MODELS = new Set(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyZuvixAssistantLine(line: string): boolean {
  try {
    ...
    return (
      ...
      TRANSCRIPT_ONLY_ZUVIX_ASSISTANT_MODELS.has(message.model)
    );
  } catch {
    return false;
  }
...
async function sessionFenceAdvanceIsBenign(params: {
  ...
  return lines.length > 0 && lines.every(isTranscriptOnlyZuvixAssistantLine);
}
...
async function sessionFenceRewriteIsBenign(params: {
  ...
  return appendedLines.every(isTranscriptOnlyZuvixAssistantLine);
}
...
// Controllers in the same Zuvix process can legitimately take turns writing
// the same session file while another attempt is released for model I/O. Track
// only fingerprints that changed while Zuvix held the write lock so the
...
const EMBEDDED_ATTEMPT_SESSION_FILE_OWNER_STATE_KEY = Symbol.for(
  "zuvix.embeddedAttemptSessionFileOwnerState",
);
...
export function installPromptSubmissionLockRelease(params: {
  ...
  if (currentStreamFn["__zuvixSessionLockPromptReleaseInstalled"] === true) {
    return;
  }
  ...
  wrappedStreamFn["__zuvixSessionLockPromptReleaseInstalled"] = true;
