...
describe("detectImageReferences", () => {
  ...
  it("ignores Zuvix CLI image cache paths from prior prompt transcripts", () => {
    ...
    const refs = detectImageReferences(
      [
        '<system-reminder>Called the Read tool with {"file_path":"/Users/ada/.zuvix/workspace/.zuvix-cli-images/stale.png"}</system-reminder>',
  ...
  it("ignores temporary Zuvix CLI image cache paths", () => {
    expectNoImageReferences(
      `Prior turn wrote ${path.join(resolvePreferredZuvixTmpDir(), "zuvix-cli-images", "stale.jpg")}`,
    );
    expectNoImageReferences(
      `[media attached: ${path.join(resolvePreferredZuvixTmpDir(), "zuvix-cli-images", "stale.jpg")} (image/jpeg)]`,
    );
    expectNoImageReferences(
      `Prior turn wrote ${path.join(os.tmpdir(), "zuvix", "zuvix-cli-images", "stale.jpg")}`,
    );
    expectNoImageReferences(
      `Prior turn wrote ${path.join(os.tmpdir(), "zuvix-501", "zuvix-cli-images", "stale.jpg")}`,
    );
  });

  it("ignores file URLs into the Zuvix CLI image cache", () => {
    const stalePath = path.join(os.tmpdir(), "zuvix", "zuvix-cli-images", "stale.png");
  ...
  it("detects normal user image paths in similarly named directories", () => {
    expect(detectImageReferences("/workspace/zuvix-cli-images/current.png")).toStrictEqual([
      {
        raw: "/workspace/zuvix-cli-images/current.png",
        type: "path",
        resolved: "/workspace/zuvix-cli-images/current.png",
      },
    ]);
  });
...
[media attached 1/2: /Users/tyleryust/.zuvix/media/IMG_6430.jpeg (image/jpeg)]
[media attached 2/2: /Users/tyleryust/.zuvix/media/IMG_6431.jpeg (image/jpeg)]
what about these images?`,
    ...
    expect(refs).toStrictEqual([
      {
        raw: "/Users/tyleryust/.zuvix/media/IMG_6430.jpeg",
        type: "path",
        resolved: "/Users/tyleryust/.zuvix/media/IMG_6430.jpeg",
      },
      {
        raw: "/Users/tyleryust/.zuvix/media/IMG_6431.jpeg",
        type: "path",
        resolved: "/Users/tyleryust/.zuvix/media/IMG_6431.jpeg",
      },
    ]);
  });
...
what is this?`);
  ...
  it("handles paths with spaces in filename", () => {
    ...
    const ref =
      expectSingleImageReference(`[media attached: /Users/test/.zuvix/media/ChatGPT Image Apr 21, 2025.png (image/png) | https://example.com/same.png]
what is this?`);

    expect(ref).toStrictEqual({
      raw: "/Users/test/.zuvix/media/ChatGPT Image Apr 21, 2025.png",
      type: "path",
      resolved: "/Users/test/.zuvix/media/ChatGPT Image Apr 21, 2025.png",
    });
  });
...
describe("loadImageFromRef", () => {
  it("hydrates managed inbound media URIs before workspace path resolution", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-native-image-uri-"));
  ...
  it("hydrates sandbox-staged inbound media URIs", async () => {
    const sandboxRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-native-image-sbx-uri-"));
  ...
  it("allows sandbox-validated host paths outside default media roots", async () => {
    ...
    const sandboxParent = await fs.mkdtemp(path.join(homeDir, "zuvix-sandbox-image-"));
    try {
...
describe("detectAndLoadPromptImages", () => {
  ...
  it("skips generated media-note refs already supplied inline", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-native-image-dedupe-"));
  ...
  it("blocks prompt image refs outside workspace when sandbox workspaceOnly is enabled", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-native-image-sandbox-"));
  ...
  it("loads managed inbound absolute paths when workspaceOnly is enabled", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-native-image-managed-"));
    const workspaceDir = path.join(stateDir, "workspace-agent");
    const inboundDir = path.join(stateDir, "media", "inbound");
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.mkdir(inboundDir, { recursive: true });
