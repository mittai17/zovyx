...
import type { AgentMessage } from "zuvix/plugin-sdk/agent-core";
...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      toolSearchControlsCase = mockParams(
        hoisted.createZuvixCodingToolsMock,
        0,
        "createZuvixCodingTools options",
      );
    } finally {
  ...
  it("enables Tool Search controls for embedded Zuvix runs when configured", async () => {
  ...
  it("defaults local-model lean embedded runs to Tool Search controls", async () => {
    ...
    expect(hoisted.createZuvixCodingToolsMock).toHaveBeenCalledTimes(1);
    ...
    const optionsConfig = requireRecord(options.config, "createZuvixCodingTools config");
    const toolsConfig = requireRecord(
      optionsConfig.tools,
      "createZuvixCodingTools tools config",
    );
  ...
  it("keeps Tool Search controls off for lean message-tool-only delivery", async () => {
    hoisted.createZuvixCodingToolsMock.mockReturnValueOnce([
      {
        name: "message",
        label: "Message",
  ...
  it("quarantines unsupported tool schemas before creating the model session", async () => {
    hoisted.createZuvixCodingToolsMock.mockReturnValue([
      {
        name: "healthy_lookup",
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.zuvixCodeModeToolSurface).toBe(true);
  ...
  it("sends transcriptPrompt visibly and keeps runtime context out of transcript messages", async () => {
    ...
    const result = await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        prompt: [
          "visible ask",
          "",
          "<<<BEGIN_ZUVIX_INTERNAL_CONTEXT>>>",
          "secret runtime context",
          "<<<END_ZUVIX_INTERNAL_CONTEXT>>>",
        ].join("\n"),
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "zuvix.runtime-context",
        "runtime context message",
      ),
      {
        role: "custom",
        customType: "zuvix.runtime-context",
        display: false,
      },
    );
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("ZUVIX_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.zuvix/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.zuvix/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.zuvix/skills/smaug",
              source: "zuvix-workspace",
              sourceInfo: {
                path: "/Users/alice/.zuvix/skills/smaug/SKILL.md",
                source: "zuvix-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.zuvix/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/zuvix-warning-workspace/AGENTS.md",
          content: "A".repeat(200),
          missing: false,
        },
      ],
      contextFiles: [
        { path: "/tmp/zuvix-warning-workspace/AGENTS.md", content: "A".repeat(20) },
      ],
    });
  ...
  it("preserves bootstrap system context in the assembled system prompt", async () => {
    ...
    hoisted.createZuvixCodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/zuvix-bootstrap-workspace/BOOTSTRAP.md",
  ...
  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/zuvix-hook-workspace";
  ...
  it("adds current-turn context to the current model input without exposing internal runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("ZUVIX_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the Zuvix runtime event.");
    expect(result.finalPromptText).toBe("Continue the Zuvix runtime event.");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the Zuvix runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
  ...
  it("submits suppressed room event context as the model prompt", async () => {
    ...
    expect(seenPrompt).toContain("[Zuvix room event]");
    ...
    expect(seenPrompt?.trim().endsWith("[Zuvix room event]")).toBe(true);
    expect(seenPrompt).not.toBe("Continue the Zuvix runtime event.");
    ...
    expect(contextCompiled?.data?.prompt).toContain("[Zuvix room event]");
  });
