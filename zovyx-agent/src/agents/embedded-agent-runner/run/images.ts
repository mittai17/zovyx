...
function isZuvixCliImageCachePath(filePath: string): boolean {
  const parts = filePath.replaceAll("\\", "/").split("/");
  return parts.some((part, index) => {
    if (part === ".zuvix-cli-images") {
      return true;
    }
    const parent = parts[index - 1] ?? "";
    return part === "zuvix-cli-images" && /^zuvix(?:-\d+)?$/.test(parent);
  });
}
...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    if (isZuvixCliImageCachePath(resolved)) {
      return;
    }
    seen.add(dedupeKey);
