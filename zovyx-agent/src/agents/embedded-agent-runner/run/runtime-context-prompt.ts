...
import {
  ...
  ZUVIX_RUNTIME_CONTEXT_CUSTOM_TYPE,
  ZUVIX_RUNTIME_CONTEXT_NOTICE,
  ZUVIX_RUNTIME_EVENT_HEADER,
} from "../../internal-runtime-context.js";
import type { CurrentInboundPromptContext } from "./params.js";
export { ZUVIX_RUNTIME_CONTEXT_CUSTOM_TYPE };

const ZUVIX_RUNTIME_EVENT_USER_PROMPT = "Continue the Zuvix runtime event.";
...
export function resolveRuntimeContextPromptParts(params: {
  ...
  if (!prompt.trim()) {
    return runtimeContext
      ? {
          prompt: ZUVIX_RUNTIME_EVENT_USER_PROMPT,
          ...(modelPromptText.trim() && modelPromptText !== ZUVIX_RUNTIME_EVENT_USER_PROMPT
            ? { modelPrompt: modelPromptText }
            : {}),
          runtimeContext,
          runtimeOnly: true,
...
export function buildRuntimeContextCustomMessage(
  ...
  return {
    role: "custom",
    customType: ZUVIX_RUNTIME_CONTEXT_CUSTOM_TYPE,
