...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createZuvixCodingToolsMock: UnknownMock;
...
vi.mock("../../docs-path.js", () => ({
  resolveZuvixReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createZuvixCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createZuvixCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("zuvix.cache-ttl", data),
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createZuvixCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.ZUVIX_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.ZUVIX_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.ZUVIX_TRAJECTORY;
    } else {
      process.env.ZUVIX_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
