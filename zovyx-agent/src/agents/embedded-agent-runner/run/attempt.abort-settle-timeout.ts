...
type AbortSettleTimeoutEnv = Partial<
  Pick<NodeJS.ProcessEnv, "ZUVIX_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS" | "ZUVIX_TEST_FAST">
>;
...
export function resolveEmbeddedAbortSettleTimeoutMs(
  ...
  const override = parseStrictPositiveInteger(env.ZUVIX_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS);
  if (override !== undefined) {
    return override;
  }
  return env.ZUVIX_TEST_FAST === "1" ? 250 : 2_000;
}
