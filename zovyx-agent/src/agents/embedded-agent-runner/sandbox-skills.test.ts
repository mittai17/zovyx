...
const hostSkillPath = "/usr/lib/node_modules/zuvix/skills/demo/SKILL.md";
const hostSkillBaseDir = "/usr/lib/node_modules/zuvix/skills/demo";
const snapshot: SkillSnapshot = {
  prompt:
    "<available_skills><skill><location>/usr/lib/node_modules/zuvix/skills/demo/SKILL.md</location></skill></available_skills>",
  skills: [{ name: "demo" }],
  resolvedSkills: [
    {
      name: "demo",
...
describe("resolveSandboxSkillRuntimeInputs", () => {
  ...
  it("uses the materialized skills workspace and drops host-path snapshots for sandboxes", () => {
    ...
    expect(
      ...
      skillsPromptWorkspaceDir: "/workspace/.zuvix/sandbox-skills",
      skillsWorkspaceDir: "/state/sandbox-skills",
      workspaceOnly: true,
    });
  });
  ...
  it("rebuilds sandbox prompts from materialized skill paths", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-sandbox-skills-"));
    try {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "name: demo",
          "description: Demo skill",
          'zuvix: {"requires":{"anyBins":["sandboxbin"]}}',
          "---",
          "# Demo",
          "",
        ].join("\n"),
      ...
      expect(prompt).toContain("/workspace/.zuvix/sandbox-skills/skills/demo/SKILL.md");
