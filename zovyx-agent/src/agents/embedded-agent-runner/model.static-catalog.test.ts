...
vi.mock("../../plugins/manifest-metadata-scan.js", () => ({
  listZuvixPluginManifestMetadata: manifestMocks.listZuvixPluginManifestMetadata,
}));
...
function setManifestPlugins(plugins: unknown[]) {
  ...
  manifestMocks.listZuvixPluginManifestMetadata.mockReturnValue(
  ...
  manifestMocks.loadPluginManifest.mockImplementation((pluginDir: string) => {
    ...
    return plugin
      ...
      : { ok: false, error: "missing manifest", manifestPath: `${pluginDir}/zuvix.plugin.json` };
  });
}
...
beforeEach(() => {
  manifestMocks.listZuvixPluginManifestMetadata.mockReset();
...
describe("resolveBundledStaticCatalogModel", () => {
  it("reuses one manifest scan across prepared lookups", () => {
    ...
    expect(manifestMocks.listZuvixPluginManifestMetadata).toHaveBeenCalledTimes(1);
  });
