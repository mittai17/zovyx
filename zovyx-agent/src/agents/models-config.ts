...
import { planZuvixModelsJson } from "./models-config.plan.js";
import {
...
export type PreparedZuvixModelsJsonSource = ModelsJsonReadyResult & {
  fingerprint: string;
  workspaceDir?: string;
};

type EnsureZuvixModelsJsonOptions = {
...
function resolveModelsConfigInput(config?: ZuvixConfig): {
  config: ZuvixConfig;
  sourceConfigForSecrets: ZuvixConfig;
} {
...
export async function prepareZuvixModelsJsonSource(
  config?: ZuvixConfig,
  agentDirOverride?: string,
  options: EnsureZuvixModelsJsonOptions = {},
): Promise<PreparedZuvixModelsJsonSource> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  ...
  const pending: Promise<ModelsJsonReadyState> = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planZuvixModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
...
/** Ensures models.json and plugin catalog sidecars are current for an agent. */
export async function ensureZuvixModelsJson(
  config?: ZuvixConfig,
  agentDirOverride?: string,
  options: EnsureZuvixModelsJsonOptions = {},
): Promise<ModelsJsonReadyResult> {
  const prepared = await prepareZuvixModelsJsonSource(config, agentDirOverride, options);
