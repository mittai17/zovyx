...
import { findNormalizedProviderKey } from "@zuvix/model-catalog-core/provider-id";
...
export type { ZuvixConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
const LEGACY_OPENCODE_ZEN_DEFAULT_MODELS = new Set([
  "zuvix/claude-opus-4-5",
  "zuvix-zen/claude-opus-4-5",
]);

/** Current Zuvix Zen default model ref used by onboarding and repair flows. */
export const OPENCODE_ZEN_DEFAULT_MODEL = "zuvix/claude-opus-4-6";

/** Pair of preset appliers exposed by provider setup modules. */
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: ZuvixConfig, ...args: TArgs) => ZuvixConfig;
  applyConfig: (cfg: ZuvixConfig, ...args: TArgs) => ZuvixConfig;
};
...
function hasAgentDefaultModelPrimary(cfg: ZuvixConfig): boolean {
...
>(params: {
  ...
  applyPreset: (cfg: ZuvixConfig, preset: TParams) => ZuvixConfig;
  primaryModelRef: string;
...
/** Move configs without a primary default onto the current Zuvix Zen model. */
export function applyOpencodeZenModelDefault(cfg: ZuvixConfig): {
  next: ZuvixConfig;
  changed: boolean;
} {
