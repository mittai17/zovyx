...
const ORIGINAL_ENV = {
  ...
  ZUVIX_CONFIG_PATH: process.env.ZUVIX_CONFIG_PATH,
  ZUVIX_STATE_DIR: process.env.ZUVIX_STATE_DIR,
  ZUVIX_TEST_FAST: process.env.ZUVIX_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("zuvix-qa-runner-state-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@zuvix/qa-linked",
        type: "module",
        zuvix: {
          extensions: ["./index.js"],
          install: {
            npmSpec: "@zuvix/qa-linked",
          },
        },
      }),
      "utf8",
