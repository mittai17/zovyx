...
const pluginSdkPackageNames = ["zuvix/plugin-sdk", "@zuvix/plugin-sdk"];
...
const DIAGNOSTIC_EVENTS_STATE_KEY = Symbol.for("zuvix.diagnosticEvents.state.v1");
...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.ZUVIX_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function listPluginSdkExportedSubpaths() {
  ...
  const cacheKey = `${packageRoot}::privateQa=${process.env.ZUVIX_ENABLE_PRIVATE_QA_CLI === "1" ? "1" : "0"}`;
...
function listPrivateLocalOnlyPluginSdkSubpaths() {
  if (process.env.ZUVIX_ENABLE_PRIVATE_QA_CLI !== "1") {
    return [];
  }
  try {
...
function buildPluginSdkAliasMap(useDist) {
  ...
  // Agent-core intentionally imports @zuvix/llm-core by package name so built
...
function getModuleLoader(tryNative) {
  ...
  const moduleLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled Zuvix core graph.
    tryNative,
