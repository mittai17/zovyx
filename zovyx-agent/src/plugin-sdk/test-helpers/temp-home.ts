// Temp home test helpers create isolated Zuvix home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ZUVIX_HOME.
  delete process.env.ZUVIX_HOME;
  process.env.ZUVIX_STATE_DIR = path.join(base, ".zuvix");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "zuvix-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".zuvix", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
