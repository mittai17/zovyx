...
export const ZUVIX_LIVE_TRANSCRIPT_MARKER_RE = /open(?:claw|cl|flaw|clar|core)/;

export function expectZuvixLiveTranscriptMarker(value: string): void {
  expect(normalizeTranscriptForMatch(value)).toMatch(ZUVIX_LIVE_TRANSCRIPT_MARKER_RE);
}
...
export async function runRealtimeSttLiveTest(params: {
  ...
  const errors: Error[] = [];
  const expected = params.expectedNormalizedText ?? ZUVIX_LIVE_TRANSCRIPT_MARKER_RE;
