...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "zuvix/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "zuvix/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "zuvix/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "zuvix-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "zuvix-sdk-file-access-compat-" }, async (root) => {
