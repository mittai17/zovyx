...
export { VERSION as ZUVIX_VERSION } from "../version.js";
...
export { resolveZuvixAgentDir } from "./agent-dir-compat.js";
export {
  resolveAgentDir,
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  config?: import("../config/types.zuvix.js").ZuvixConfig;
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@zuvix/media-core/constants"),
    ]);
...
/**
 * Derive the same compact user-facing tool detail that embedded Zuvix uses for progress logs.
 */
 ...
 * preserve the same Zuvix-owned fallback signals as the built-in Zuvix path
