...
import type { ZuvixPluginApi, PluginRegistrationMode } from "../plugins/types.js";
...
const pluginModuleLoaderJitiFactoryOverrideKey = Symbol.for(
  "zuvix.pluginModuleLoaderJitiFactoryOverride",
);
...
function createApi(registrationMode: PluginRegistrationMode): ZuvixPluginApi {
  return {
    registrationMode,
    runtime: { registrationMode } as unknown as PluginRuntime,
    registerChannel: vi.fn(),
    registerTool: vi.fn(),
  } as unknown as ZuvixPluginApi;
}
...
function createBundledChannelEntry(params: {
  importerPath: string;
  pluginId: string;
  registerCliMetadata?: (api: ZuvixPluginApi) => void;
  registerFull?: (api: ZuvixPluginApi) => void;
}) {
...
describe("defineBundledChannelEntry", () => {
  it("runs tool registrations without channel sidecar hydration during tool discovery", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-bundled-entry-tools-"));
    ...
    const registerCliMetadata = vi.fn<(api: ZuvixPluginApi) => void>();
    const registerFull = vi.fn<(api: ZuvixPluginApi) => void>((api) => {
      api.registerTool(
        {
          name: "channel_tool",
          label: "Channel Tool",
  ...
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-bundled-entry-runtime-"));
    tempDirs.push(tempRoot);
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    ...
    const registerCliMetadata = vi.fn<(api: ZuvixPluginApi) => void>();
    const registerFull = vi.fn<(api: ZuvixPluginApi) => void>();
    const entry = createBundledChannelEntry({
      importerPath,
      pluginId,
      registerCliMetadata,
...
describe("defineBundledChannelSetupEntry", () => {
  it("exposes setup-runtime registrations without loading the full channel entry", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-bundled-setup-entry-"));
    tempDirs.push(tempRoot);
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    const setupRuntimeRegister = vi.fn<(api: ZuvixPluginApi) => void>();
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("ZUVIX_PLUGIN_LOAD_PROFILE", "1");
  ...
  try {
    ...
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-channel-entry-contract-"));
    tempDirs.push(tempRoot);
...
function runCompiledEsmSidecarFastPathProbe(): SpawnSyncReturns<string> {
  ...
  fs.writeFileSync(path.join(tempRoot, "zuvix.mjs"), "#!/usr/bin/env node\n", "utf8");
  ...
  return spawnSync(process.execPath, ["--import", "tsx", probePath], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: { ...process.env, ZUVIX_PLUGIN_LOAD_PROFILE: "1" },
  });
}
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before module loads them", async () => {
    ...
    vi.doMock("../infra/boundary-file-read.js", () => ({
      openRootFileSync: () => ({
        ok: true,
        path: "C:\\Users\\alice\\zuvix\\dist\\extensions\\feishu\\helper.ts",
      ...
      try {
        ...
        expect(
          channelEntryContract.loadBundledEntryExportSync<number>(
            "file:///C:/Users/alice/zuvix/dist/extensions/feishu/index.js",
            {
              specifier: "./helper.ts",
              exportName: "load",
            },
        ...
        expect(jitiLoad).toHaveBeenCalledWith(
          "file:///C:/Users/alice/zuvix/dist/extensions/feishu/helper.ts",
        );
      } finally {
  ...
  it("can disable source-tree fallback for dist bundled entry checks", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"zuvix"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("ZUVIX_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "1");
