...
/**
 * @deprecated Compatibility facade for the `zuvix/plugin-sdk/discord` subpath.
...
export type { ZuvixPluginApi, PluginRuntime } from "./channel-plugin-common.js";
...
/** Discord channel config shape for one account in Zuvix config. */
export type DiscordAccountConfig = NonNullable<NonNullable<ZuvixConfig["channels"]>["discord"]>;
...
type DiscordApiFacadeModule = {
  ...
  inspectDiscordAccount: (params: { cfg: ZuvixConfig; accountId?: string | null }) => unknown;
  listDiscordAccountIds: (cfg: ZuvixConfig) => string[];
  ...
  resolveDefaultDiscordAccountId: (cfg: ZuvixConfig) => string;
...
function resolveCompatRuntimeConfig(params: { cfg?: ZuvixConfig }): ZuvixConfig {
...
/** List configured Discord account ids from Zuvix config. */
export function listDiscordAccountIds(cfg: ZuvixConfig): string[] {
...
export function resolveDefaultDiscordAccountId(cfg: ZuvixConfig): string {
