...
export const toolPluginMetadataSymbol = Symbol.for("zuvix.plugin-sdk.tool-plugin.metadata");

/** Runtime context supplied to a concrete tool plugin execution handler. */
export type ToolPluginExecutionContext = {
  /** Plugin runtime API for tool implementations that need Zuvix services. */
  api: ZuvixPluginApi;
...
export type ToolPluginFactoryContext<TConfig> = {
  ...
  toolContext: ZuvixPluginToolContext;
};
