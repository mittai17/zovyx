...
import { withTempHome as withTempHomeBase } from "zuvix/plugin-sdk/test-env";
import { resetPluginLoaderTestStateForTest } from "../plugins/loader.test-fixtures.js";
import { clearPluginSetupRegistryCache } from "../plugins/setup-registry.js";
import { resetConfigRuntimeState, type ZuvixConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        ZUVIX_MPM_CATALOG_PATHS: undefined,
...
export async function writeZuvixConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.ZUVIX_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected ZUVIX_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeZuvixConfig(home, config);
