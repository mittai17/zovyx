...
import type { ConfigFileSnapshot, ZuvixConfig } from "./types.js";

type MockValidationIssue = { path: string; message: string };
type MockValidationResult =
  | { ok: true; config: ZuvixConfig; warnings: MockValidationIssue[] }
...
const validationMocks = vi.hoisted(() => ({
  validateConfigObjectWithPlugins: vi.fn(
    (config: ZuvixConfig): MockValidationResult => ({
      ok: true,
      config,
      warnings: [],
    }),
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/zuvix.json",
    exists: true,
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "zuvix-config-mutate-" });
  const originalNixMode = process.env.ZUVIX_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.ZUVIX_NIX_MODE = originalNixMode;
    }
  ...
  it("preserves config path ownership across transform retries", async () => {
    const initial = createSnapshot({
      hash: "hash-1",
      path: "/tmp/first-zuvix.json",
      sourceConfig: { agents: { list: [] } },
    });
    const fresh = createSnapshot({
      hash: "hash-2",
      path: "/tmp/second-zuvix.json",
      sourceConfig: { agents: { list: [] } },
    });
    ioMocks.readConfigFileSnapshotForWrite
      .mockResolvedValueOnce({
    ...
    const transform = vi.fn((config: ZuvixConfig) => ({ nextConfig: config }));
  ...
  it("does not retry transform mutations after config path ownership changes", async () => {
    ...
    await expect(
      transformConfigFileWithRetry({
        transform(config) {
          activeConfigPath = "/tmp/second-zuvix.json";
  ...
  it("rejects replace attempts when the active config path changed", async () => {
    ...
    await expect(
      replaceConfigFile({
        baseHash: snapshot.hash,
        nextConfig: { gateway: { port: 19002 } },
        writeOptions: { expectedConfigPath: "/tmp/first-zuvix.json" },
      }),
    ).rejects.toThrow("config path changed since last load");
    expect(ioMocks.writeConfigFile).not.toHaveBeenCalled();
  });
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("Zuvix Nix overview: https://docs.zuvix.ai/install/nix");
  ...
  it("repairs invalid config through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".zuvix", "config", "plugins.json5");
      ...
      `${JSON.stringify(
        {
          entries: {
            old: {
              enabled: true,
              config: { token: "${ZUVIX_TEST_PLUGIN_TOKEN}" },
            },
          },
        },
        null,
    ...
    ioMocks.readConfigFileSnapshotForWrite
      .mockResolvedValueOnce({
        ...
        writeOptions: {
          ...
          envSnapshotForRestore: { ZUVIX_TEST_PLUGIN_TOKEN: "plugin-token-runtime" },
    ...
    try {
      await replaceConfigFile({
        ...
        io: {
          env: { ZUVIX_TEST_PLUGIN_TOKEN: "plugin-token-after-read" },
    ...
    expect(persistedPlugins.entries?.old?.config?.token).toBe("${ZUVIX_TEST_PLUGIN_TOKEN}");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const linkPath = path.join(home, ".zuvix", "link");
  ...
  it("rejects direct mutations to external include roots", async () => {
    ...
    await expect(
      replaceConfigFile({
        ...
        nextConfig,
        io: {
          env: { ZUVIX_INCLUDE_ROOTS: "~/shared" },
          readConfigFileSnapshotForWrite: ioMocks.readConfigFileSnapshotForWrite,
          writeConfigFile: ioMocks.writeConfigFile,
        },
      }),
  ...
  it("does not write an include after the active config path changes during preflight", async () => {
    ...
    const firstConfigPath = path.join(home, "first", "zuvix.json");
    const secondConfigPath = path.join(home, "second", "zuvix.json");
  ...
  it("rolls back an include write when config path ownership changes during commit", async () => {
    ...
    const assertConfigPathForWrite = () => {
      if (fsNode.readFileSync(pluginsPath, "utf-8") !== initialPluginsRaw) {
        activeConfigPath = "/tmp/other-zuvix.json";
      }
  ...
  it.each(["active path", "refreshed snapshot path"] as const)(
    ...
    async (changeKind) => {
      ...
      const configPath = path.join(home, "first", "zuvix.json");
      const otherConfigPath = path.join(home, "second", "zuvix.json");
      const pluginsPath = path.join(home, "first", "plugins.json5");
      const rootConfig = { plugins: { $include: "./plugins.json5" } };
      const initialPluginsRaw = `${JSON.stringify({ entries: {} }, null, 2)}\n`;
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const includeDir = path.join(home, ".zuvix", "config");
      const movedIncludeDir = path.join(home, ".zuvix", "config-original");
  ...
  it("preflights the restored include payload with the current environment", async () => {
    ...
    const initialPluginsRaw = `${JSON.stringify(
      {
        entries: {
          old: { enabled: true, config: { token: "${ZUVIX_TEST_INCLUDE_TOKEN}" } },
        },
      },
      null,
      2,
    ...
    const observedSources: ZuvixConfig[] = [];
    ...
    try {
      ...
      await expect(
        replaceConfigFile({
          ...
          writeOptions: {
            expectedConfigPath: snapshot.path,
            envSnapshotForRestore: { ZUVIX_TEST_INCLUDE_TOKEN: "old-token" },
            assertConfigPathForWrite: allowConfigPathWrite,
            includeFileTargetsForWrite: { [pluginsPath]: await resolveIncludeTarget(pluginsPath) },
          },
          nextConfig: {
          ...
          io: {
            env: { ZUVIX_TEST_INCLUDE_TOKEN: "new-token" },
            readConfigFileSnapshotForWrite: ioMocks.readConfigFileSnapshotForWrite,
            writeConfigFile: ioMocks.writeConfigFile,
          },
        }),
  ...
  it("rolls back single-file top-level include writes when runtime refresh fails", async () => {
    ...
    const envKey = "ZUVIX_TEST_INCLUDE_ROLLBACK_ENV";
    await fs.mkdir(path.dirname(pluginsPath), { recursive: true });
    await fs.writeFile(
      configPath,
      `${JSON.stringify({ plugins: { $include: "./config/plugins.json5" } }, null, 2)}\n`,
  ...
  it("preflights injected root writers before persisting", async () => {
    ...
    const configPath = path.join(home, ".zuvix", "zuvix.json");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
    const initialConfig = { gateway: { mode: "local" } } satisfies ZuvixConfig;
    ...
    const injectedWrite = vi.fn(async (config: ZuvixConfig, options?: ConfigWriteOptions) => {
