// Defines the top-level Zuvix configuration type.
...
/** Top-level Zuvix config as read from user/project config files. */
export type ZuvixConfig = {
  /** JSON schema URL used by editors and generated config files. */
  $schema?: string;
  meta?: {
    /** Last Zuvix version that wrote this config. */
  ...
  wizard?: {
    ...
    lastRunAt?: string;
    /** Zuvix version used by the last completed wizard run. */
  ...
  ui?: {
    /** Accent color for Zuvix UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type ZuvixConfigInput = Omit<ZuvixConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const zuvixConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = ZuvixConfig & {
  readonly [zuvixConfigStateBrand]?: TState;
};
