...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "zuvix-web-search": {
                source: "npm",
                spec: "@ollama/zuvix-web-search",
                installPath: "/tmp/zuvix-web-search",
                resolvedName: "@ollama/zuvix-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "zuvix-web-search": {
                source: "npm",
                spec: "@ollama/zuvix-web-search@0.2.2",
                installPath: "/tmp/zuvix-web-search",
                resolvedName: "@ollama/zuvix-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("zuvix config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('zuvix config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/zuvix.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: ZuvixConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://zuvix.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
