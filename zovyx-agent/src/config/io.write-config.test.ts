...
import type { ConfigFileSnapshot, ZuvixConfig } from "./types.zuvix.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "zuvix-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    return withEnvAsync(
      {
        ZUVIX_DEFER_SHELL_ENV_FALLBACK: undefined,
        ZUVIX_LOAD_SHELL_ENV: undefined,
        ZUVIX_SHELL_ENV_TIMEOUT_MS: undefined,
      },
      () => fn(home),
    );
  }
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { ZUVIX_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("logs health-state write failures through public config reads", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const healthPath = path.join(home, ".zuvix", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const pluginDir = path.join(home, ".zuvix", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "zuvix.plugin.json");
  ...
  it("retains included shipped plugin install records in write snapshots", async () => {
    await withSuiteHome(async (home) => {
      const configDir = path.join(home, ".zuvix");
      const configPath = path.join(configDir, "zuvix.json");
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const pluginDir = path.join(home, ".zuvix", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const unwritableStatePath = path.join(home, ".zuvix");
  ...
  it("keeps writes inside an ZUVIX_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".zuvix", "zuvix.json");
      ...
      const env = { ZUVIX_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const io = createConfigIO({
        env,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "zuvix.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "zuvix.json"), "utf-8"),
      ) as {
  ...
  it("prints missing-meta write anomalies when anomaly logging is requested", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          ZUVIX_CONFIG_WRITE_ANOMALY_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          ZUVIX_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(persisted.$schema).toBe("https://zuvix.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/zuvix-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as ZuvixConfig;
  ...
  it("returns the read-time environment snapshot for invalid config repairs", async () => {
    await withSuiteHome(async (home) => {
        ...
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${ZUVIX_GATEWAY_TOKEN}" },
            },
            channels: { "test-plugin-channel": { enabled: true } },
          },
          null,
      ...
      const io = createConfigIO({
        env: {
          ZUVIX_GATEWAY_TOKEN: "gateway-token-at-read",
          ZUVIX_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(result.writeOptions.envSnapshotForRestore?.ZUVIX_GATEWAY_TOKEN).toBe(
  ...
  it("returns the snapshot-time hash when an included file is malformed", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const includePath = path.join(home, ".zuvix", "plugins.json5");
  ...
  it("returns a write guard that rejects a changed active config path", async () => {
    await withSuiteHome(async (home) => {
      const firstConfigPath = path.join(home, ".zuvix", "first.json");
      const secondConfigPath = path.join(home, ".zuvix", "second.json");
      ...
      const env = {
        ZUVIX_CONFIG_PATH: firstConfigPath,
        ZUVIX_TEST_FAST: "1",
      } as NodeJS.ProcessEnv;
      const io = createConfigIO({ env, homedir: () => home, logger: silentLogger });

      const result = await io.readConfigFileSnapshotForWrite();
      env.ZUVIX_CONFIG_PATH = secondConfigPath;
  ...
  it("rejects local write ownership when config env changes path selection during the read", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const configuredNextPath = path.join(home, ".zuvix", "next.json");
      const sourceConfig = {
        env: { ZUVIX_CONFIG_PATH: configuredNextPath },
        gateway: { mode: "local" },
  ...
  it("follows config env path selection before returning global write ownership", async () => {
    await withSuiteHome(async (home) => {
      ...
      const initialConfig = JSON.parse(await fs.readFile(configPath, "utf-8")) as ZuvixConfig;
  ...
  it("does not use expectedConfigPath as the write destination", async () => {
    await withSuiteHome(async (home) => {
      const expectedConfigPath = path.join(home, ".zuvix", "expected.json");
      const activeConfigPath = path.join(home, ".zuvix", "active.json");
      ...
      await withEnvAsync(
        {
          ZUVIX_CONFIG_PATH: activeConfigPath,
          ZUVIX_TEST_FAST: "1",
        },
        async () => {
          await writeConfigFile(
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const includePath = path.join(home, ".zuvix", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://zuvix.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
  ...
  it("rejects invalid include-backed repairs instead of persisting substituted secrets", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const includePath = path.join(home, ".zuvix", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify(
      ...
      const io = createConfigIO({
        env: {
          ZUVIX_GATEWAY_TOKEN: "gateway-token-runtime",
          ZUVIX_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("repairs invalid root-authored siblings without flattening included config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const includePath = path.join(home, ".zuvix", "agent-defaults.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
  ...
  it("rejects repairs that would flatten a valid outer include with a broken nested include", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const pluginsPath = path.join(home, ".zuvix", "plugins.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        pluginsPath,
  ...
  it("preserves escaped root literals before validating unrelated includes", async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/zuvix-test-literal-plugin",
          source: "/tmp/zuvix-test-literal-plugin/index.ts",
          manifestPath: "/tmp/zuvix-test-literal-plugin/zuvix.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const agentsPath = path.join(home, ".zuvix", "agents.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
  ...
  it("repairs invalid config without flattening array-nested includes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const includePath = path.join(home, ".zuvix", "main-agent.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ id: "main", workspace: "${ZUVIX_AGENT_WORKSPACE}" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      const io = createConfigIO({
        env: {
          ZUVIX_AGENT_WORKSPACE: "/resolved/agent-workspace",
          ZUVIX_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      await expect(fs.readFile(includePath, "utf-8")).resolves.toContain(
        '"workspace": "${ZUVIX_AGENT_WORKSPACE}"',
      );
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/zuvix-test-required-plugin",
          source: "/tmp/zuvix-test-required-plugin/index.ts",
          manifestPath: "/tmp/zuvix-test-required-plugin/zuvix.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      ...
      await withEnvAsync({ ZUVIX_CONFIG_PATH: configPath }, async () => {
        setRuntimeConfigSnapshot(
          {
            gateway: { mode: "local" },
            models: {
      ...
      try {
        await withEnvAsync(
          ...
          async () => {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${ZUVIX_GATEWAY_TOKEN}");
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/zuvix-test-demo",
          source: "/tmp/zuvix-test-demo/index.ts",
          manifestPath: "/tmp/zuvix-test-demo/zuvix.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ ZUVIX_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createConfigIO({
            env: { ZUVIX_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
            homedir: () => home,
            logger: silentLogger,
  ...
  it("rolls back the root config when post-write runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies ZuvixConfig;
  ...
  it("blocks runtime preflight failures before committing root writes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const initialRaw = `${JSON.stringify({ gateway: { mode: "local" } }, null, 2)}\n`;
      let observedSource: ZuvixConfig | undefined;
  ...
  it("restores config env vars when post-write runtime refresh rollback succeeds", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const envKey = "ZUVIX_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies ZuvixConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
  ...
  it("rolls back root writes when canonical reread changes config path ownership", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const nextConfigPath = path.join(home, ".zuvix", "next.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies ZuvixConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        async () => {
          ...
          await expect(
            writeConfigFile(
              {
                gateway: { mode: "local", port: 19001 },
                env: { ZUVIX_CONFIG_PATH: nextConfigPath },
              },
              {
                baseSnapshot: prepared.snapshot,
                ...prepared.writeOptions,
          ...
          await expect(fs.readFile(configPath, "utf-8")).resolves.toBe(initialRaw);
          expect(process.env.ZUVIX_CONFIG_PATH).toBeUndefined();
  ...
  it("uses injected filesystem operations when rolling back ownership loss", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".zuvix", "zuvix.json");
      const otherConfigPath = path.join(home, ".zuvix", "other.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies ZuvixConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        promises: {
          ...
          rename: async (from, to) => {
            ...
            if (!committed && to === configPath) {
              committed = true;
              env.ZUVIX_CONFIG_PATH = otherConfigPath;
            }
          },
        },
      } as typeof fsNode;
    ...
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/zuvix-test-strict-plugin",
            source: "/tmp/zuvix-test-strict-plugin/index.ts",
            manifestPath: "/tmp/zuvix-test-strict-plugin/zuvix.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await withEnvAsync({ ZUVIX_CONFIG_PATH: configPath }, async () => {
          ...
          await expect(
            writeConfigFile({ agents: { list: "not-array" } } as unknown as ZuvixConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/zuvix-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "zuvix-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
        { baseSnapshot: snapshot },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as ZuvixConfig;
      expect(persisted.logging?.file).toBe("~/zuvix-upgrade-survivor/gateway.jsonl");
