...
function parseAssistantTranscriptText(
  line: string,
  options?: { excludeTranscriptOnlyZuvixAssistant?: boolean },
  ...
  if (
    options?.excludeTranscriptOnlyZuvixAssistant &&
    isTranscriptOnlyZuvixAssistantMessage(message)
  ) {
    return undefined;
  }
...
function isTranscriptOnlyZuvixAssistantMessage(message: {
  provider?: unknown;
  model?: unknown;
}): boolean {
  return isTranscriptOnlyZuvixAssistantModel(message.provider, message.model);
}
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      const assistantText = parseAssistantTranscriptText(line, {
        excludeTranscriptOnlyZuvixAssistant: true,
      });
      if (assistantText) {
...
export async function readTailAssistantTextFromSessionTranscript(
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      ...
      // Skip non-message entries (e.g. `zuvix.cache-ttl` custom events) so
...
export async function appendAssistantMessageToSessionTranscript(params: {
  ...
  return appendExactAssistantMessageToSessionTranscript({
    ...
    message: {
      ...
      timestamp: Date.now(),
      ...(params.deliveryMirror ? { zuvixDeliveryMirror: params.deliveryMirror } : {}),
...
function isRedundantDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  return message.provider === "zuvix" && message.model === "delivery-mirror";
}

function isChannelFinalDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  const marker = (message as { zuvixDeliveryMirror?: SessionTranscriptDeliveryMirror })
    .zuvixDeliveryMirror;
