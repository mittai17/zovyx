...
// Keep integration tests deterministic: never read a real zuvix.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "zuvix-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.ZUVIX_SESSION_CACHE_TTL_MS;
    process.env.ZUVIX_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  });
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.ZUVIX_SESSION_CACHE_TTL_MS;
    } else {
      process.env.ZUVIX_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("removes trajectory sidecars for stale sessions pruned on write", async () => {
    ...
    await fs.writeFile(staleTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(freshTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(staleRuntime, '{"traceSchema":"zuvix-trajectory"}\n', "utf-8");
    await fs.writeFile(freshRuntime, '{"traceSchema":"zuvix-trajectory"}\n', "utf-8");
    await fs.writeFile(
      stalePointer,
      JSON.stringify({
