...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): ZuvixConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: ZuvixConfig, env: NodeJS.ProcessEnv) =>
    ...
    await withTempHome(async () => {
      const cfg: ZuvixConfig = {
        ...
        agents: {
          list: [
            { id: "ops", default: true },
            { id: "review", runtime: { type: "acp", acp: { agent: "zuvix" } } },
          ],
        },
        acp: {
          defaultAgent: "claude",
      ...
      expect(targets).toEqual([
        ...
        {
          agentId: "zuvix",
          storePath: resolveStorePath(cfg.session?.store, { agentId: "zuvix", env }),
        },
      ]);
    });
  });
