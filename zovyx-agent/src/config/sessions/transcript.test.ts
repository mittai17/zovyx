...
describe("appendAssistantMessageToSessionTranscript", () => {
  ...
  it("emits transcript update events for delivery mirrors", async () => {
    ...
    expect(message?.provider).toBe("zuvix");
  ...
  it("idempotently appends identified channel finals while preserving repeated replies", async () => {
    ...
    if (first.ok && replay.ok && nextTurn.ok) {
      ...
      expect(JSON.parse(lines[1]).message.zuvixDeliveryMirror).toEqual({
        kind: "channel-final",
  ...
  it("skips transcript-only Zuvix assistant entries when reading latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("does not report transcript-only Zuvix assistant entries as latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("keeps transcript-only Zuvix assistant entries available to the tail reader", async () => {
    writeTranscriptStore();
  ...
  it("scans past trailing non-assistant entries (e.g. zuvix.cache-ttl) to find the latest assistant text", async () => {
    // Regression for zuvix/zuvix#83427: the cache-ttl custom entry was
  ...
  it("does not reuse an older matching assistant message across turns", async () => {
    ...
    if (olderResult.ok && latestResult.ok && mirrorResult.ok) {
      ...
      expect(messageLine.message.provider).toBe("zuvix");
