...
import { SENSITIVE_URL_HINT_TAG } from "@zuvix/net-policy/redact-sensitive-url";
...
describe("config schema", () => {
  ...
  it("validates MCP OAuth client metadata URLs against the SDK contract", () => {
        ...
        mcp: {
          servers: {
            docs: {
              ...
              auth: "oauth",
              oauth: {
                clientMetadataUrl: "https://client.example.com/zuvix-mcp.json",
              },
            },
          },
        },
    ...
    for (const clientMetadataUrl of [
      "http://client.example.com/zuvix-mcp.json",
  ...
  it("accepts exec command highlighting config in global and agent scopes", () => {
    ...
    const config = ZuvixSchema.parse({
      agents: {
        list: [
          {
            id: "main",
  ...
  it("accepts install policy exec config in the runtime zod schema", () => {
    const parsed = ZuvixSchema.parse({
      security: {
        installPolicy: {
          enabled: true,
    ...
    expect(parsed.security?.installPolicy?.exec?.command).toBe(
      "/usr/local/bin/zuvix-install-policy",
    );
  });
