// Loads, validates, migrates, snapshots, and writes Zuvix config files.
import crypto from "node:crypto";
import fs from "node:fs";
import os from "node:os";
import path from "node:path";
import { collectManifestModelIdNormalizationPolicies } from "@zuvix/model-catalog-core/provider-model-id-normalization";
...
import type { ZuvixConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: ZuvixConfig };
...
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: ZuvixConfig;
  /**
   ...
   */
  preCommitRuntimePreflight?: (sourceConfig: ZuvixConfig) => Promise<unknown>;
...
function coerceConfig(value: unknown): ZuvixConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ZuvixConfig;
}
...
function stampConfigVersion(cfg: ZuvixConfig, version?: string): ZuvixConfig {
  return stampConfigWriteMetadata(cfg, new Date().toISOString(), version);
}

function warnIfConfigFromFuture(cfg: ZuvixConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your Zuvix config was written by version ${touched}, but this command is running ${VERSION}.`,
        "Check: `zuvix --version`, `which zuvix`, and `zuvix gateway status --deep`.",
        "If unexpected, update PATH so `zuvix` points to the version you want, or reinstall the Gateway service from that same Zuvix install.",
      ].join("\n"),
    );
  }
}
...
function shouldSuppressFutureVersionWarningForEnv(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.ZUVIX_UPDATE_IN_PROGRESS) ||
    isTruthyEnvValue(env.ZUVIX_UPDATE_POST_CORE)
  );
}
...
export function parseConfigJson5(
  ...
  try {
    ...
    // on the JSON files Zuvix writes itself.
  }
  try {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ZuvixConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: ZuvixConfig): ZuvixConfig {
  ...
  function resolveRuntimePreflightSourceConfig(candidate: ZuvixConfig): ZuvixConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run zuvix plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function validateSuspiciousRecoveryBackup(parsed: unknown): boolean {
    try {
      ...
      const loadValidationPluginMetadataSnapshot = (config: ZuvixConfig) => {
  ...
  function loadConfigLocal(options: { skipSuspiciousRecovery?: boolean } = {}): ZuvixConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as ZuvixConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
  ...
  async function readConfigFileSnapshotInternal(
    ...
    let fallbackSourceConfig: ZuvixConfig = {};
  ...
  async function readBestEffortConfigLocal(): Promise<ZuvixConfig> {
    return (await readBestEffortConfigSnapshotLocal()).config;
  }

  async function readSourceConfigBestEffortLocal(): Promise<ZuvixConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFileLocal(
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as ZuvixConfig, unsetPaths);
    ...
    const validationCandidate = containsConfigIncludeDirective(validationSourceCandidate)
      ? resolveRuntimePreflightSourceConfig(validationSourceCandidate as ZuvixConfig)
    ...
    let cfgToWrite = persistCandidate as ZuvixConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ZUVIX_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.ZUVIX_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ZUVIX_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      const shouldLogBenignMissingMeta =
        isVerbose() || deps.env.ZUVIX_CONFIG_WRITE_ANOMALY_LOG === "1" || shouldLogInVitest;
    ...
    const preCommitRuntimePreflight =
      ...
      (async (sourceConfig: ZuvixConfig) => {
...
// module scope. `ZUVIX_CONFIG_PATH` (and friends) are expected to work even
...
export function loadConfig(options?: {
  ...
  // reparsing zuvix.json on hot code paths.
...
export async function readSourceConfigBestEffort(): Promise<ZuvixConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: ZuvixConfig = nextCfg;
