...
import { isSensitiveUrlConfigPath } from "@zuvix/net-policy/redact-sensitive-url";
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = ZuvixSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ZuvixConfig";
    const hints = mapSensitivePaths(ZuvixSchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(ZuvixSchema, "", isSensitiveUrlConfigPath);
