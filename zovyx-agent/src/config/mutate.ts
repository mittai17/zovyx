...
async function tryWriteSingleTopLevelIncludeMutation(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
}): Promise<{ persistedHash: string | null; persistedConfig: ZuvixConfig } | null> {
...
function resolveConfigWriteResult(
  ...
  fallbackConfig: ZuvixConfig,
): { persistedHash: string | null; persistedConfig: ZuvixConfig } {
  if (result) {
    return {
...
export async function mutateConfigFile<T = void>(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
  mutate: (draft: ZuvixConfig, context: ConfigMutationContext) => Promise<T | void> | T | void;
}): Promise<ConfigMutationResult<T>> {
