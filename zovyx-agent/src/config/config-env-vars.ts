...
function collectConfigEnvVarsByTarget(cfg?: ZuvixConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: ZuvixConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: ZuvixConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.ZUVIX_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
