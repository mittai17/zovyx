...
import type { ConfigFileSnapshot } from "./types.zuvix.js";
...
describe("redactConfigSnapshot", () => {
          ...
          plugins: [
            {
              ...
              manifestPath: "/private/plugin/root/zuvix.plugin.json",
            },
          ],
          diagnostics: [],
        },
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/zuvix/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/zuvix/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/zuvix/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/zuvix/nickserv-password.txt");
