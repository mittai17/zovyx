...
/** Parses stable, prerelease, and legacy dot-beta Zuvix versions. */
export function parseZuvixVersion(raw: string | null | undefined): ZuvixVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeZuvixVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseZuvixVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameZuvixStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseZuvixVersion(a);
  const parsedB = parseZuvixVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareZuvixVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseZuvixVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseZuvixVersion(current);
  const parsedTouched = parseZuvixVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameZuvixStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareZuvixVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: ZuvixVersion): number {
