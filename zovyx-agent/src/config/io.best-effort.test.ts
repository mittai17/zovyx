...
import { withTempHome, writeZuvixConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      ...
      const healthPath = `${home}/.zuvix/logs/config-health.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeZuvixConfig(home, {
      ...
      const entries = await fs.readdir(`${home}/.zuvix`);
      expect(entries.some((entry) => entry.startsWith("zuvix.json.clobbered."))).toBe(false);
    });
  });
