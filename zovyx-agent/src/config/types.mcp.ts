...
export type McpServerCodexConfig = {
  /** Zuvix agent ids that should receive this server in Codex app-server threads. */
  agents?: string[];
...
export type McpServerConfig = {
  ...
  /** HTTP OAuth mode. Tokens are stored in Zuvix state, not in config. */
  auth?: "oauth";
  ...
  /** Alias for clientKey. */
  client_key?: string;
  /** Optional per-server Zuvix MCP tool selection. */
...
export type McpConfig = {
  /** Named MCP server definitions managed by Zuvix. */
