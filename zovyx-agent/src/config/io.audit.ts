...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.ZUVIX_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.ZUVIX_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.ZUVIX_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `zuvix doctor --fix` once the
