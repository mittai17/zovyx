...
import { normalizeConfiguredProviderCatalogModelId } from "@zuvix/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  zuvix config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  zuvix config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: ZuvixConfig,
  pathSegments: string[],
): { changed: boolean; next: ZuvixConfig } {
