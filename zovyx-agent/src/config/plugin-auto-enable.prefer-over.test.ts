...
function makeTempDir(): string {
  ...
  const dir = fs.mkdtempSync(path.join(trustedRoot, ".zuvix-plugin-prefer-over-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin auto-enable preferOver", () => {
  it("tolerates bundled channel id metadata drift during auto-enable", async () => {
    ...
    vi.stubEnv("ZUVIX_BUNDLED_PLUGINS_DIR", rootDir);
    ...
    vi.stubEnv("ZUVIX_BUNDLED_PLUGINS_DIR", path.join(rootDir, "missing"));
    await setBundledPluginsDirFixture(undefined);
    ...
    const result = materializePluginAutoEnableCandidates({
      ...
      env: {
        ...
        ZUVIX_BUNDLED_PLUGINS_DIR: path.join(rootDir, "missing"),
      },
