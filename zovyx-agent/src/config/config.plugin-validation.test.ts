...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "zuvix.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      ZUVIX_STATE_DIR: path.join(suiteHome, ".zuvix"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-config-plugin-validation-"));
  ...
  describe("missing Codex plugin diagnostics", () => {
    ...
    it("still warns when only one provider model route is pinned to Zuvix", () => {
  ...
  it("deduplicates catalog install hints for missing configured official external plugins", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: brave — install the official external plugin with: zuvix plugins install @zuvix/brave-plugin";
  ...
  it("warns instead of failing when an official external memory slot plugin is not installed", () => {
    ...
    expect(res.ok).toBe(true);
    const slotMessage =
      "plugin not installed: memory-lancedb — gateway will run without persistent memory until installed; install the official external plugin with: zuvix plugins install @zuvix/memory-lancedb";
    const entryMessage =
      "plugin not installed: memory-lancedb — install the official external plugin with: zuvix plugins install @zuvix/memory-lancedb";
  ...
  it("deduplicates yuanbao missing-plugin warnings across entries and allow", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: yuanbao — install the official external plugin with: zuvix plugins install zuvix-plugin-yuanbao@2.13.1";
            ...
            diagnostics: [
              {
                ...
                source: path.join(suiteHome, "extensions", "broken-local", "zuvix.plugin.json"),
  ...
  it("warns instead of failing for stale channel config backed by missing plugin refs", () => {
    ...
    expect(res.warnings).toContainEqual({
      ...
      message:
        "unknown channel id: missing-chat (stale channel plugin config ignored; run zuvix doctor --fix to remove stale config, or install the plugin)",
    });
    expect(res.warnings).toContainEqual({
      path: "plugins.allow",
      message:
    ...
    expect(res.warnings ?? []).toEqual([
      {
        path: "plugins.allow",
        message:
          "plugin not installed: discord — install the official external plugin with: zuvix plugins install @zuvix/discord",
      },
    ]);
  });

  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    const stateDir = path.join(suiteHome, ".zuvix");
    ...
    try {
      ...
      expect(res.warnings).toContainEqual({
        ...
        message:
          "unknown channel id: missing-sms (stale channel plugin config ignored; run zuvix doctor --fix to remove stale config, or install the plugin)",
      });
    } finally {
      await writePersistedInstalledPluginIndex(
        {
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          ZUVIX_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
    expect(res.ok).toBe(true);
  ...
  it("warns for removed skill-workshop plugin id instead of failing validation", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin removed: skill-workshop (stale plugin config ignored; Skill Workshop is built into Zuvix skills now. Use skills.workshop settings and zuvix skills workshop commands, then remove this plugins config entry)";
  ...
  it("ignores standalone helper scripts in auto-discovered global extensions", async () => {
    const helperPath = path.join(suiteHome, ".zuvix", "extensions", "my-helper.mjs");
  ...
  it("surfaces invalid Codex native plugin marketplaces as config diagnostics", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          ZUVIX_BUNDLED_PLUGINS_DIR: path.join(process.cwd(), "extensions"),
        },
      },
    );
  ...
  it("accepts known plugin ids and valid channel/heartbeat enums", () => {
    const res = validateInSuite({
      agents: {
        ...
        list: [{ id: "zuvix", heartbeat: { directPolicy: "allow" } }],
      },
      channels: {
        modelByChannel: {
          openai: {
  ...
  it("accepts plugin heartbeat targets", () => {
    const res = validateInSuite({
      agents: { defaults: { heartbeat: { target: "chat" } }, list: [{ id: "zuvix" }] },
