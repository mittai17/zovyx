...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: ZuvixConfig): boolean {
...
function isProviderConfigured(cfg: ZuvixConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: ZuvixConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: ZuvixConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: ZuvixConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: ZuvixConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: ZuvixConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: ZuvixConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: ZuvixConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: ZuvixConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: ZuvixConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: ZuvixConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: ZuvixConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: ZuvixConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: ZuvixConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: ZuvixConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: ZuvixConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: ZuvixConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: ZuvixConfig): boolean {
  return (
...
function hasPluginEntries(cfg: ZuvixConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: ZuvixConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: ZuvixConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: ZuvixConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: ZuvixConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: ZuvixConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: ZuvixConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: ZuvixConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: ZuvixConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
