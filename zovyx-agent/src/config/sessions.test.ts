...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ZUVIX_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-zuvix",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-zuvix#general");
  });
  ...
  it("derives session transcripts dir from ZUVIX_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ZUVIX_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ZUVIX_STATE_DIR differs from stored paths", () => {
