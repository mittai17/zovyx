...
/**
 * Nix mode detection: When ZUVIX_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
const NEW_STATE_DIRNAME = ".zuvix";
const CONFIG_FILENAME = "zuvix.json";
...
/** Build a homedir thunk that respects ZUVIX_HOME for the given env. */
 ...
 * Can be overridden via ZUVIX_STATE_DIR.
 * Default: ~/.zuvix
 */
export function resolveStateDir(
  ...
  const override = env.ZUVIX_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.ZUVIX_TEST_FAST === "1") {
    return newDir;
  }
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const zuvixOverride = env.ZUVIX_STATE_DIR?.trim();
  if (zuvixOverride) {
    env.ZUVIX_STATE_DIR = resolveUserPath(zuvixOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `ZUVIX_INCLUDE_ROOTS` as a
 ...
 * directory containing `zuvix.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.ZUVIX_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via ZUVIX_CONFIG_PATH.
 * Default: ~/.zuvix/zuvix.json (or $ZUVIX_STATE_DIR/zuvix.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.ZUVIX_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.ZUVIX_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.ZUVIX_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const zuvixStateDir = env.ZUVIX_STATE_DIR?.trim();
  if (zuvixStateDir) {
    const resolved = resolveUserPath(zuvixStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/zuvix-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `zuvix-${uid}` : "zuvix";
 ...
 *
 * Precedence:
 * - `ZUVIX_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.ZUVIX_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.ZUVIX_GATEWAY_PORT?.trim();
