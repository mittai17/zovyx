...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, Zuvix env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still Zuvix-managed keys:
 ...
 * literal references for keys Zuvix previously managed.
 */
...
/**
 * Read and parse `~/.zuvix/.env` (or `$ZUVIX_STATE_DIR/.env`), returning
