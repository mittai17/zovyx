...
function isOpenAiCodexDefaultRuntimeSelection(params: {
  ...
  // Custom OpenAI-compatible base URLs stay on the Zuvix runtime path.
...
function codexPluginEntryEnabled(cfg: ZuvixConfig): boolean | undefined {
...
function openAiProviderRuntimePolicy(cfg: ZuvixConfig): AgentRuntimePolicyConfig | undefined {
...
function listConfiguredAgentIds(cfg: ZuvixConfig): Array<string | undefined> {
...
function openAiHasCodexDefaultRuntimePolicy(cfg: ZuvixConfig): boolean {
  for (const [providerId, providerConfig] of Object.entries(cfg.models?.providers ?? {})) {
    if (normalizeProviderId(providerId) !== OPENAI_PROVIDER_ID) {
      continue;
    }
    ...
    // A model-scoped explicit "auto"/"default" overrides provider-wide PI/Zuvix
...
function openAiDefaultRouteKeepsCodexUnavailable(cfg: ZuvixConfig): boolean {
  const policy = openAiDefaultRouteRuntimePolicy(cfg);
  if (!policy?.id?.trim()) {
    // With no explicit runtime policy, the OpenAI route only needs Codex on the
    // official OpenAI endpoint. OpenAI-compatible proxies stay on Zuvix.
    return !openAIProviderUsesCodexRuntimeByDefault({
      provider: OPENAI_PROVIDER_ID,
      config: cfg,
    });
 ...
 */
export function configExplicitlyKeepsCodexUnavailableForOpenAi(cfg: ZuvixConfig): boolean {
 ...
 * all default routes would otherwise stay on the Zuvix runtime.
 */
export function shouldSuppressMissingCodexPluginDiagnostics(cfg: ZuvixConfig): boolean {
