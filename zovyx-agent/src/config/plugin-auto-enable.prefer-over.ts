...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.zuvix) || !isRecord(entry.zuvix.channel)) {
      continue;
    }
    const channel = entry.zuvix.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: ZuvixConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: ZuvixConfig, pluginId: string) => boolean;
