...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".zuvix", "zuvix.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
function readAuditLog(home: string): unknown[] {
  const auditPath = path.join(home, ".zuvix", "logs", "config-audit.jsonl");
  return fs
...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "zuvix-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".zuvix", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      formatConfigOverwriteLogMessage({
        configPath: "/tmp/zuvix.json",
        previousHash: "prev-hash",
        nextHash: "next-hash",
        changedPathCount: 3,
      }),
    ).toBe(
      "Config overwrite: /tmp/zuvix.json (sha256 prev-hash -> next-hash, backup=/tmp/zuvix.json.bak, changedPaths=3)",
    );
  });
  ...
  it("captures watch markers and next stat metadata for successful writes", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      env: {
        ZUVIX_WATCH_MODE: "1",
        ZUVIX_WATCH_SESSION: "watch-session-1",
        ZUVIX_WATCH_COMMAND: "gateway --force",
      } as NodeJS.ProcessEnv,
      existsBefore: true,
      previousHash: "prev-hash",
      nextHash: "next-hash",
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/zuvix.json");
  ...
  it("redacts structured audit records before persistence", async () => {
    ...
    const record = finalizeConfigWriteAuditRecord({
      base: {
        ...createAuditRecordBase(path.join(home, ".zuvix", "zuvix.json")),
        suspicious: [
    ...
    const raw = fs.readFileSync(
      path.join(home, ".zuvix", "logs", "config-audit.jsonl"),
      "utf-8",
    );
  ...
  it("redacts the value half of `--flag=value` for secret flags", () => {
    const argv = ["zuvix", "--token=ghp_realgithubtoken1234567890ABCD", "--port=8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["zuvix", "--token=***", "--port=8080"]);
  });
  ...
  it("leaves non-secret arguments untouched", () => {
    const argv = ["node", "zuvix", "gateway", "--port", "8080", "--bind", "lan"];
  ...
  it("masks the next arg after a secret flag even when it looks like another option", () => {
    const argv = ["zuvix", "--token", "--port", "8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["zuvix", "--token", "***", "8080"]);
  });

  it("redacts dash-leading secret values after bare secret flags", () => {
    const argv = ["zuvix", "--password", "-secret-value"];
    expect(redactConfigAuditArgv(argv)).toEqual(["zuvix", "--password", "***"]);
  });

  it("does not mask when a secret flag is the final arg with no value", () => {
    const argv = ["zuvix", "--token"];
    expect(redactConfigAuditArgv(argv)).toEqual(["zuvix", "--token"]);
  });
  ...
  it("redacts processInfo.argv when explicitly supplied to createConfigWriteAuditRecordBase", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      processInfo: {
        ...
        ppid: 1,
        cwd: "/work",
        argv: ["node", "zuvix", "--token", "leaked-but-not-anymore-12345"],
        execArgv: [],
      },
    });
    expect(base.argv).toEqual(["node", "zuvix", "--token", "***"]);
  });
  ...
  it("rewrites historical config-audit entries through redactConfigAuditArgv and preserves 0600 mode", async () => {
    ...
    const unredactedRecord = {
      ...
      source: "config-io",
      event: "config.write",
      configPath: path.join(home, ".zuvix", "zuvix.json"),
      pid: 1590563,
      ppid: 1590548,
      cwd: home,
      argv: [
        "/usr/bin/node",
        "/usr/local/bin/zuvix.mjs",
        "config",
        "set",
    ...
    const alreadyRedactedRecord = {
      ...
      ppid: 1,
      cwd: home,
      argv: ["/usr/bin/node", "/usr/local/bin/zuvix.mjs", "config", "set", "ui.theme", "dark"],
      execArgv: ["--disable-warning=ExperimentalWarning"],
      suspicious: [],
      result: "rename",
    };
  ...
  it("preserves malformed lines verbatim and counts them as skipped", async () => {
    ...
    const validUnredacted = {
      ts: "2026-05-02T00:03:48.471Z",
      argv: ["node", "zuvix.mjs", "config", "set", "x", "xoxb-bad-token-1234567890abcdef"],
    };
  ...
  it("aborts without overwriting when the audit log is appended to after temp write", async () => {
    ...
    const appended = {
      ...
      argv: ["node", "zuvix.mjs", "config", "set", "theme", "dark"],
      execArgv: [],
    };
    const original = `${JSON.stringify(unredacted)}\n`;
