...
type ZuvixMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_ZUVIX_TRANSPORT: Record<string, ZuvixMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to Zuvix HTTP transport names. */
export function resolveZuvixMcpTransportAlias(
  value: unknown,
): ZuvixMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_ZUVIX_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias Zuvix can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_ZUVIX_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveZuvixMcpTransportAlias(next.type);
  // `transport` is Zuvix's canonical field; legacy `type` only fills a gap.
