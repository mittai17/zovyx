...
describe("resolveGatewayService", () => {
  ...
  it("guards mutating service adapters when config was written by a newer Zuvix", async () => {
    const tempHome = await makeTempWorkspace("zuvix-service-future-config-");
    const stateDir = path.join(tempHome, ".zuvix");
    const configPath = path.join(stateDir, "zuvix.json");
    const envSnapshot = captureEnv(["HOME", "ZUVIX_STATE_DIR", "ZUVIX_CONFIG_PATH"]);
    try {
...
describe("readGatewayServiceState", () => {
  it("tracks installed, loaded, and running separately", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { ZUVIX_GATEWAY_PORT: "1" },
    });

    expect(state.installed).toBe(true);
    expect(state.loaded).toBe(true);
    expect(state.running).toBe(true);
    expect(state.env.ZUVIX_GATEWAY_PORT).toBe("18789");
  });
  ...
  it("keeps the caller-selected service identity when merging persisted env", async () => {
    ...
    const service = createService({
      ...
      readCommand: vi.fn(async () => ({
        ...
        environment: {
          ...
          ZUVIX_SYSTEMD_UNIT: "zuvix-gateway.service",
        },
      })),
      readRuntime,
    });
    ...
    const state = await readGatewayServiceState(service, {
      env: { ZUVIX_SYSTEMD_UNIT: "zuvix-gateway-maintenance.service" },
    });

    expect(state.env.ZUVIX_SYSTEMD_UNIT).toBe("zuvix-gateway-maintenance.service");
...
describe("startGatewayService", () => {
  ...
  it("requests repair before start when the loaded service version is stale", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: ["zuvix", "gateway", "run"],
        environment: { ZUVIX_SERVICE_VERSION: "2026.4.24" },
      })),
      isLoaded: vi.fn(async () => true),
    ...
    if (result.outcome === "repair-required") {
      expect(formatGatewayServiceStartRepairIssues(result.issues)).toContain(
        "service was installed by Zuvix 2026.4.24",
      );
    }
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("requests repair before start when the loaded service points at temporary install paths", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: [
          "/private/tmp/zuvix-ai-install-cli-pr118/tools/node/bin/node",
          "/tmp/zuvix-ai-install-cli-pr118/lib/node_modules/zuvix/dist/index.js",
          "gateway",
        ],
        environment: {},
      })),
