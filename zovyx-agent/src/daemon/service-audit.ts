...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ZUVIX_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `zuvix gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ZUVIX_GATEWAY_TOKEN does not match gateway.auth.token in zuvix.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.ZUVIX_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.ZUVIX_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  const serviceVersion = command?.environment?.ZUVIX_SERVICE_VERSION?.trim();
  ...
  issues.push({
    ...
    message: `Gateway service was installed by Zuvix ${serviceVersion}; current CLI is ${VERSION}.`,
