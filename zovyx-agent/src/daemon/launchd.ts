...
const ZUVIX_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.zuvix.update.";
const ZUVIX_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.zuvix\.manual-update\.\d+$/;

export type StaleZuvixUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeZuvixUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  if (trimmed.startsWith(ZUVIX_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  // Manual update jobs include a timestamp-like suffix and should be cleaned up
  // without matching arbitrary ai.zuvix labels.
  return ZUVIX_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.ZUVIX_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.ZUVIX_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.ZUVIX_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.ZUVIX_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

export function isZuvixUpdateLaunchdLabel(label: unknown): label is string {
  return normalizeZuvixUpdateLaunchdLabel(label) !== null;
}

function resolveCurrentZuvixUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.ZUVIX_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeZuvixUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.ZUVIX_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.ZUVIX_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by Zuvix. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListZuvixUpdateJobs(
  output: string,
): StaleZuvixUpdateLaunchdJob[] {
  const jobs: StaleZuvixUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    const label = normalizeZuvixUpdateLaunchdLabel(labelParts.join(" "));
    if (!label) {
      continue;
    }
...
export async function findStaleZuvixUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleZuvixUpdateLaunchdJob[]> {
  ...
  return parseLaunchctlListZuvixUpdateJobs(result.stdout).filter(
    (job) => !isCurrentGatewayLaunchdLabel(job.label, env),
  );
}

export async function removeZuvixUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeZuvixUpdateLaunchdLabel(label);
...
export async function disableZuvixUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeZuvixUpdateLaunchdLabel(label);
  if (process.platform !== "darwin" || !normalizedLabel) {
    return false;
  }
...
export async function disableCurrentZuvixUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentZuvixUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableZuvixUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.ZUVIX_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.ZUVIX_GATEWAY_PORT ?? "");
}
...
export function formatLaunchAgentGuiSessionError(params: {
  ...
  return [
    ...
    "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.zuvix.ai/gateway",
  ].join("\n");
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `zuvix gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.ZUVIX_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "zuvix gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "zuvix gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `zuvix gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "zuvix gateway restart",
    });
