...
export const ZUVIX_WRAPPER_ENV_KEY = "ZUVIX_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/zuvix -> .pnpm/zuvix@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // zuvix from node_modules/.bin points at the package root sibling.
...
export async function resolveZuvixWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${ZUVIX_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveZuvixWrapperPath(params.wrapperPath);
  if (wrapperPath) {
