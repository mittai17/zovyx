...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `zuvix-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ZUVIX_LAUNCHD_LABEL: `ai.zuvix.launchd-int-${testId}`,
      ZUVIX_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
