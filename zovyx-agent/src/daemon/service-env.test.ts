...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ZUVIX_GATEWAY_PORT).toBe("18789");
    expect(env.ZUVIX_GATEWAY_TOKEN).toBeUndefined();
    expect(env.ZUVIX_SERVICE_MARKER).toBe("zuvix");
    expect(env.ZUVIX_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ZUVIX_SERVICE_VERSION).toBe("string");
    expect(env.ZUVIX_SYSTEMD_UNIT).toBe("zuvix-gateway.service");
    expect(env.ZUVIX_WINDOWS_TASK_NAME).toBe("Zuvix Gateway");
    if (process.platform === "darwin") {
      expect(env.ZUVIX_LAUNCHD_LABEL).toBe("ai.zuvix.gateway");
    }
  });

  it("sets the Zuvix-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through ZUVIX_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ZUVIX_WRAPPER: " /usr/local/bin/zuvix-doppler ",
      },
      port: 18789,
    });

    expect(env.ZUVIX_WRAPPER).toBe("/usr/local/bin/zuvix-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".zuvix", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ZUVIX_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ZUVIX_SYSTEMD_UNIT).toBe("zuvix-gateway-work.service");
    expect(env.ZUVIX_WINDOWS_TASK_NAME).toBe("Zuvix Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.ZUVIX_LAUNCHD_LABEL).toBe("ai.zuvix.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ZUVIX_PROFILE: "work",
        ZUVIX_SYSTEMD_UNIT: "zuvix-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.ZUVIX_SYSTEMD_UNIT).toBe("zuvix-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ZUVIX_SYSTEMD_UNIT: "zuvix-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", ZUVIX_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ZUVIX_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.ZUVIX_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the Zuvix-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.ZUVIX_LAUNCHD_LABEL).toBe("ai.zuvix.node");
  });

  it("passes through ZUVIX_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ZUVIX_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.ZUVIX_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through ZUVIX_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ZUVIX_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.ZUVIX_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits ZUVIX_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        ZUVIX_GATEWAY_TOKEN: "   ",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.ZUVIX_WINDOWS_TASK_NAME).toBe("Zuvix Node");
    expect(env.ZUVIX_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.ZUVIX_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".zuvix"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ZUVIX_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".zuvix-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ZUVIX_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".zuvix"));
  });

  it("uses ZUVIX_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ZUVIX_STATE_DIR: "/var/lib/zuvix" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/zuvix"));
  });

  it("expands ~ in ZUVIX_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ZUVIX_STATE_DIR: "~/zuvix-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/zuvix-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ZUVIX_STATE_DIR: "C:\\State\\zuvix" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\zuvix");
  });
});
