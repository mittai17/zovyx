/** Inspects installed platform services for extra Zuvix or legacy gateway jobs. */
...
const EXTRA_MARKERS = ["zuvix", "clawdbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  ...
  const markerKeys = ["zuvix_service_marker"];
  const kindKeys = ["zuvix_service_kind"];
...
function isZuvixGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "zuvix") {
    return false;
  }
  return label.startsWith("ai.zuvix.");
}

function isZuvixGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("zuvix-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isZuvixGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  // Windows schtasks /Query returns task names prefixed with \ (e.g.
  // \Zuvix Gateway for root-folder tasks). Strip the leading
  ...
  return stripped === defaultName || /^zuvix gateway \(.+\)$/.test(stripped);
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "zuvix"
        ? "zuvix"
    ...
    if (marker === "zuvix" && isZuvixGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "zuvix" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  dir: string;
  scope: "user" | "system";
  includeManagedZuvix?: boolean;
}): Promise<ExtraGatewayService[]> {
  const results: ExtraGatewayService[] = [];
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedZuvix ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedZuvix &&
      marker === "zuvix" &&
      isZuvixGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isZuvixGatewayTaskName(name)) {
        continue;
      }
