...
function readServiceProxyEnvironment(
  ...
  // Service env intentionally preserves only the canonical Zuvix proxy knob;
  // generic shell proxy vars are audited but not frozen into services.
  const proxyUrl = normalizeOptionalString(env.ZUVIX_PROXY_URL);
  return proxyUrl ? { ZUVIX_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.ZUVIX_PROFILE;
  const wrapperPath = normalizeOptionalString(env.ZUVIX_WRAPPER);
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = resolveGatewaySystemdUnitEnv(env);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ZUVIX_PROFILE: profile,
    ZUVIX_WRAPPER: wrapperPath,
    ZUVIX_GATEWAY_PORT: String(port),
    ZUVIX_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ZUVIX_SYSTEMD_UNIT: systemdUnit,
    ZUVIX_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    ZUVIX_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ZUVIX_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    ZUVIX_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.ZUVIX_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.ZUVIX_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ZUVIX_GATEWAY_TOKEN: gatewayToken,
    ZUVIX_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    ZUVIX_STATE_DIR: sharedEnv.stateDir,
    ZUVIX_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.ZUVIX_STATE_DIR;
