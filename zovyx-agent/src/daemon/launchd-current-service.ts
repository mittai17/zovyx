...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.ZUVIX_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.ZUVIX_SERVICE_MARKER) === "zuvix" &&
    Boolean(normalizeOptionalString(env.ZUVIX_SERVICE_KIND))
  ) {
