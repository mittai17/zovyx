...
import {
  ...
  disableZuvixUpdateLaunchdJob,
  findStaleZuvixUpdateLaunchdJobs,
  isLaunchAgentListed,
  isZuvixUpdateLaunchdLabel,
  parseLaunchctlPrint,
  parseLaunchctlListZuvixUpdateJobs,
  readLaunchAgentProgramArguments,
  readLaunchAgentRuntime,
  repairLaunchAgentBootstrap,
  removeZuvixUpdateLaunchdJob,
  restartLaunchAgent,
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.zuvix.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", ZUVIX_PROFILE: "default" },
    });
  ...
  it("parses stale Zuvix updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListZuvixUpdateJobs(
      [
        "123 0 ai.zuvix.gateway",
        "- 127 ai.zuvix.update.2026.5.12",
        "- 0 ai.zuvix.manual-update.1717168800",
        "8142 0 ai.zuvix.update.2026.5.13-beta.1",
        "- 0 ai.zuvix.manual-updater.1717168800",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      ...
      {
        label: "ai.zuvix.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    "finds stale Zuvix updater jobs via launchctl list",
    async () => {
      state.listOutput = "- 127 ai.zuvix.update.2026.5.12\n";

      const jobs = await findStaleZuvixUpdateLaunchdJobs();
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      state.listOutput = [
        "- 0 ai.zuvix.manual-update.1717168800",
        "812 0 ai.zuvix.manual-update.profile",
        "913 0 ai.zuvix.manual-update.custom-label",
      ].join("\n");

      const jobs = await findStaleZuvixUpdateLaunchdJobs({
        ZUVIX_PROFILE: "manual-update.profile",
        ZUVIX_LAUNCHD_LABEL: "ai.zuvix.manual-update.custom-label",
        ZUVIX_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
        ZUVIX_SERVICE_KIND: GATEWAY_SERVICE_KIND,
  ...
  it("recognizes only Zuvix updater launchd labels", () => {
    expect(isZuvixUpdateLaunchdLabel("ai.zuvix.update.2026.5.12")).toBe(true);
    expect(isZuvixUpdateLaunchdLabel("ai.zuvix.manual-update.1717168800")).toBe(true);
    expect(isZuvixUpdateLaunchdLabel("ai.zuvix.gateway")).toBe(false);
    expect(isZuvixUpdateLaunchdLabel("ai.zuvix.manual-update.gateway")).toBe(false);
    expect(isZuvixUpdateLaunchdLabel("ai.zuvix.manual-update.profile")).toBe(false);
    expect(isZuvixUpdateLaunchdLabel("ai.zuvix.manual-updater.1717168800")).toBe(false);
    expect(isZuvixUpdateLaunchdLabel("com.example.update")).toBe(false);
  });

  it.runIf(process.platform === "darwin")("removes legacy updater launchd jobs", async () => {
    await expect(removeZuvixUpdateLaunchdJob(" ai.zuvix.update.2026.5.12 ")).resolves.toBe(
      true,
    );

    expect(state.launchctlCalls).toContainEqual(["remove", "ai.zuvix.update.2026.5.12"]);
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentZuvixUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.zuvix.update.2026.5.12",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.zuvix.update.2026.5.12`,
      ]);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentZuvixUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.zuvix.manual-update.1717168800",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.zuvix.manual-update.1717168800`,
      ]);
      expect(launchctlCommandNames()).not.toContain("remove");
    },
  );
  ...
  it.runIf(process.platform === "darwin")(
    "disables the current legacy updater launchd job from Zuvix label env",
    async () => {
      await expect(
        disableCurrentZuvixUpdateLaunchdJob({
          ZUVIX_LAUNCHD_LABEL: "ai.zuvix.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    "does not let non-update launchd markers mask the Zuvix update label",
    async () => {
      await expect(
        disableCurrentZuvixUpdateLaunchdJob({
          XPC_SERVICE_NAME: "0",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentZuvixUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.zuvix.update.2026.5.12",
          ZUVIX_PROFILE: "update.2026.5.12",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentZuvixUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.zuvix.manual-update.1717168800",
          ZUVIX_PROFILE: "manual-update.1717168800",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentZuvixUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.zuvix.manual-update.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableZuvixUpdateLaunchdJob("ai.zuvix.update.2026.5.12")).resolves.toBe(
      true,
    );
  ...
  it.runIf(process.platform === "darwin")("disables explicit manual updater jobs", async () => {
    await expect(
      disableZuvixUpdateLaunchdJob("ai.zuvix.manual-update.1717168800"),
    ).resolves.toBe(true);
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.zuvix/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.zuvix/service-env/ai.zuvix.gateway.env";
    const wrapperPath = "/Users/test/.zuvix/service-env/ai.zuvix.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.zuvix/service-env")).toBe(0o700);
  ...
  it("repairs a mangled label-derived service-env wrapper path on restart", async () => {
    ...
    const serviceEnv = {
      ...callerEnv,
      ZUVIX_STATE_DIR: "/Users/test/service-env/custom-state",
    };
    await installLaunchAgent({
      env: serviceEnv,
      stdout: new PassThrough(),
      ...
      environment: {
        ...
        ZUVIX_STATE_DIR: serviceEnv.ZUVIX_STATE_DIR,
      },
    });

    const plistPath = resolveLaunchAgentPlistPath(callerEnv);
    const envFilePath = "/Users/test/service-env/custom-state/service-env/ai.zuvix.gateway.env";
    const wrapperPath =
      "/Users/test/service-env/custom-state/service-env/ai.zuvix.gateway-env-wrapper.sh";
    const callerEnvFilePath = "/Users/test/.zuvix/service-env/ai.zuvix.gateway.env";
    const callerWrapperPath =
      "/Users/test/.zuvix/service-env/ai.zuvix.gateway-env-wrapper.sh";
    const mangledEnvFilePath =
      "/Users/test/service-env/custom-state/service-env/[ai.zuvix.gateway.env](http:/ai.zuvix.gateway.env)";
    const mangledWrapperPath =
      "/Users/test/service-env/custom-state/service-env/[ai.zuvix.gateway-env-wrapper.sh](http:/ai.zuvix.gateway-env-wrapper.sh)";
    state.files.set(
      plistPath,
    ...
    expect(command?.programArguments).toEqual(defaultProgramArguments);
    expect(command?.environment?.ZUVIX_GATEWAY_PORT).toBe("18789");
    expect(command?.environment?.ZUVIX_STATE_DIR).toBe(serviceEnv.ZUVIX_STATE_DIR);
    expect(command?.environmentValueSources?.ZUVIX_GATEWAY_PORT).toBe("file");
    ...
    expect(rewrittenEnv).toContain("export ZUVIX_GATEWAY_PORT='18789'");
    expect(rewrittenEnv).toContain(
      "export ZUVIX_STATE_DIR='/Users/test/service-env/custom-state'",
    );
  });
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/zuvix/gateway.log</string>");
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.zuvix.gateway</string>",
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.zuvix.gateway`;
    ...
    await withProcessEnv(
      ...
      async () => {
        await expect(stopLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
          "Refusing to stop LaunchAgent ai.zuvix.gateway from inside the same launchd service",
        );
      },
    );
  ...
  it("allows external LaunchAgent label overrides to stop the selected target", async () => {
    const env = {
      ...
      ZUVIX_LAUNCHD_LABEL: "com.example.zuvix.gateway",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/com.example.zuvix.gateway`;
    expect(state.launchctlCalls).toEqual([["bootout", serviceId]]);
    expect(output).toContain("Stopped LaunchAgent");
  });
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ZUVIX_GATEWAY_PORT: "19006" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "    <string>/Users/test/.zuvix-default/logs/gateway.log</string>",
        "  </dict>",
        "</plist>",
      ].join("\n"),
    );
  ...
  it("uses the stored LaunchAgent environment port for restart stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ZUVIX_GATEWAY_PORT: "19007" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("ignores invalid stored LaunchAgent environment ports for stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ZUVIX_GATEWAY_PORT: "65536" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("hands restart off to a detached helper when invoked from the current LaunchAgent", async () => {
    ...
    const result = await withProcessEnv({ LAUNCH_JOB_LABEL: "ai.zuvix.gateway" }, async () =>
      restartLaunchAgent({
        env,
        stdout: new PassThrough(),
      }),
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/zuvix/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("surfaces detached handoff failures", async () => {
    ...
    await expect(
      withProcessEnv({ LAUNCH_JOB_LABEL: "ai.zuvix.gateway" }, async () =>
        restartLaunchAgent({
          env,
          stdout: new PassThrough(),
        }),
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.zuvix.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when ZUVIX_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.zuvix.gateway.plist",
    },
    {
      name: "uses profile-specific label when ZUVIX_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", ZUVIX_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.zuvix.jbphoenix.plist",
    },
    {
      name: "prefers ZUVIX_LAUNCHD_LABEL over ZUVIX_PROFILE",
      env: {
        HOME: "/Users/test",
        ZUVIX_PROFILE: "jbphoenix",
        ZUVIX_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from ZUVIX_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        ZUVIX_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty ZUVIX_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        ZUVIX_PROFILE: "myprofile",
        ZUVIX_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.zuvix.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        ZUVIX_LAUNCHD_LABEL: "../evil/label",
      }),
