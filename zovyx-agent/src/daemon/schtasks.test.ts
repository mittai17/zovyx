...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when ZUVIX_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".zuvix", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when ZUVIX_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", ZUVIX_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".zuvix-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers ZUVIX_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        ZUVIX_PROFILE: "rescue",
        ZUVIX_STATE_DIR: "C:\\State\\zuvix",
      },
      expected: path.join("C:\\State\\zuvix", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", ZUVIX_PROFILE: "default" },
      expected: path.join("/home/test", ".zuvix", "gateway.cmd"),
    },
    {
      name: "uses a custom task script file name inside the state directory",
      env: {
        USERPROFILE: "C:\\Users\\test",
        ZUVIX_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".zuvix", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        ZUVIX_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("ZUVIX_TASK_SCRIPT_NAME must be a file name only");
  });
});
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Zuvix Gateway",
          "cd /d C:\\Projects\\zuvix",
          "set NODE_ENV=production",
          "set ZUVIX_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\zuvix",
          environment: {
            NODE_ENV: "production",
            ZUVIX_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\zuvix\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\zuvix\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Zuvix Share\\node.exe" "\\\\fileserver\\Zuvix Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Zuvix Share\\node.exe",
            "\\\\fileserver\\Zuvix Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from ZUVIX_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ ZUVIX_STATE_DIR: path.join(tmpDir, "custom-state") }),
