/**
 * Shared realtime voice controls for active Zuvix agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "zuvix_agent_control";
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Control an active Zuvix tool-backed voice run. Use this when the caller asks in any language for status/progress, cancellation, a redirect/change to the active work, or a follow-up after the current work. Do not use this for ordinary greetings or chatter unless the caller is asking about the active work.",
  parameters: {
    type: "object",
    properties: {
      text: {
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal Zuvix voice control result.",
    "Do not call zuvix_agent_consult or any other tool for this message.",
    "Speak this exact Zuvix status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "Zuvix is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "Zuvix has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "Zuvix could not queue that follow-up."
    : "Zuvix could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "Zuvix finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `Zuvix is starting ${name}.` : "Zuvix is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `Zuvix finished ${name} and is continuing.`
        : "Zuvix finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `Zuvix is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "Zuvix is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `Zuvix is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "Zuvix is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "Zuvix is working on the current voice request.";
  }
