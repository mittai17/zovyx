#!/usr/bin/env node
// Boots the Zuvix CLI entry point under Node.
// CLI process entrypoint for Zuvix command execution.
...
import {
  enableZuvixCompileCache,
  resolveEntryInstallRoot,
  respawnWithoutZuvixCompileCacheIfNeeded,
} from "./entry.compile-cache.js";
import { buildCliRespawnPlan, runCliRespawnPlan } from "./entry.respawn.js";
import { tryHandleRootVersionFastPath } from "./entry.version-fast-path.js";
import { isTruthyEnvValue, normalizeEnv } from "./infra/env.js";
import { isMainModule } from "./infra/is-main.js";
import { ensureZuvixExecMarkerOnProcess } from "./infra/zuvix-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "zuvix.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "zuvix.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutZuvixCompileCacheIfNeeded({
  ...
  if (!waitingForCompileCacheRespawn) {
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.ZUVIX_AUTH_STORE_READONLY = "1";
    }
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[zuvix] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[zuvix] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[zuvix] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
...
export async function tryHandlePrecomputedCommandHelpFastPath(
  ...
  if (env.ZUVIX_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
