...
export function hasConfiguredInternalHooks(config: ZuvixConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: ZuvixConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
