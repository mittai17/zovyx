...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/zuvix-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/zuvix-agent/.zuvix-agent"),
  resolveAgentEffectiveModelPrimary: vi.fn((cfg: ZuvixConfig) => {
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded Zuvix agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded Zuvix agent run options");
  }
