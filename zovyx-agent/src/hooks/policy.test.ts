...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "zuvix-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "zuvix-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "zuvix-bundled");
      const plugin = makeHookEntry("shared", "zuvix-plugin");
      const managed = makeHookEntry("shared", "zuvix-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("zuvix-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "zuvix-managed");
      const workspace = makeHookEntry("shared", "zuvix-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "zuvix-workspace");
      const second = makeHookEntry("shared", "zuvix-workspace");
      second.hook.handlerPath = "/tmp/zuvix-workspace/shared/handler-2.js";
