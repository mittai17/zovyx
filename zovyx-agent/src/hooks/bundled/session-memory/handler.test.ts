...
// Avoid calling the embedded Zuvix agent (global command lane); keep this unit test deterministic.
...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => ZuvixConfig;
  action?: "new" | "reset";
