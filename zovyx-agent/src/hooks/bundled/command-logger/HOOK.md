...
homepage: https://docs.zuvix.ai/automation/hooks#command-logger
metadata:
  {
    "zuvix":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.zuvix/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.zuvix/logs/commands.log ~/.zuvix/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/zuvix`:
   ```
   /home/username/.zuvix/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
