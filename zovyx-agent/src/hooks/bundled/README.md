...
This directory contains hooks that ship with Zuvix. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.zuvix/workspace`)
...
homepage: https://docs.zuvix.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.zuvix/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.zuvix/zuvix.json`:
...
2. Restart gateway: `pkill -9 -f 'zuvix.*gateway' && pnpm zuvix gateway`
3. Enable the hook: `zuvix hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.zuvix.ai/automation/hooks
