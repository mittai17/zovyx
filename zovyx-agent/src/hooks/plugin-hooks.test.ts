...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "zuvix-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.ZUVIX_BUNDLED_HOOKS_DIR;
    process.env.ZUVIX_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.ZUVIX_BUNDLED_HOOKS_DIR;
    } else {
      process.env.ZUVIX_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".zuvix", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): ZuvixConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("zuvix-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Zuvix hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".zuvix", "extensions", "claude-bundle");
