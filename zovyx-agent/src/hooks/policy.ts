...
import type { ZuvixConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "zuvix-bundled": {
    ...
    canOverride: ["zuvix-bundled"],
    canBeOverriddenBy: ["zuvix-managed", "zuvix-plugin"],
  },
  "zuvix-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["zuvix-bundled", "zuvix-plugin"],
    canBeOverriddenBy: ["zuvix-managed"],
  },
  "zuvix-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["zuvix-bundled", "zuvix-managed", "zuvix-plugin"],
    canBeOverriddenBy: ["zuvix-managed"],
  },
  "zuvix-workspace": {
    precedence: 40,
    ...
    canOverride: ["zuvix-workspace"],
    canBeOverriddenBy: ["zuvix-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "zuvix-plugin") {
    return { enabled: true };
  }
