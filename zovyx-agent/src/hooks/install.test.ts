...
const fixtureRoot = path.join(process.cwd(), ".tmp", `zuvix-hook-install-${randomUUID()}`);
...
function writeHookPackFiles(params: {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      'metadata: {"zuvix":{"events":["command:new"]}}',
      "---",
      "",
      `# ${params.heading}`,
    ].join("\n"),
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    zuvix: { hooks: [`./hooks/${params.hookName}`] },
  });
...
describe("installHooksFromPath", () => {
  ...
  it("blocks a staged single hook before publishing the target", async () => {
    ...
    expect(scanCall.packageDir).toContain(".zuvix-install-stage-");
  ...
  it.each([".codex-plugin/plugin.json", "hooks/hooks.json", "zuvix.plugin.json"])(
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "zuvix.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "zuvix.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
          ...
          expect(params.archiveInstallParams).toEqual(
            expect.objectContaining({
              installPolicyRequest: {
                kind: "plugin-npm",
                requestedSpecifier: "@zuvix/test-hooks@0.0.1",
                source: {
                  kind: "npm",
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.packageKind).toBe("hook-only");
    expect(result.npmResolution?.resolvedSpec).toBe("@zuvix/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@zuvix/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@zuvix/test-hooks@0.0.2-beta.1",
      name: "@zuvix/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@zuvix/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@zuvix/test-hooks@beta"');
    }
  });
});
