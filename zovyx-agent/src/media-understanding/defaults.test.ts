...
const mediaMetadataPlugins = vi.hoisted(() => [
  {
    ...
    mediaUnderstandingProviderMetadata: {
      ...
      zuvix: { capabilities: ["image"], defaultModels: { image: "gpt-5-nano" } },
      "zuvix-go": { capabilities: ["image"], defaultModels: { image: "kimi-k2.6" } },
      openrouter: {
        capabilities: ["image", "audio"],
...
describe("resolveDefaultMediaModel", () => {
  ...
  it("resolves bundled image defaults beyond the historical core set", () => {
    ...
    expect(resolveDefaultMediaModel({ providerId: "zuvix", capability: "image" })).toBe(
      "gpt-5-nano",
    );
    expect(resolveDefaultMediaModel({ providerId: "zuvix-go", capability: "image" })).toBe(
      "kimi-k2.6",
    );
  });
