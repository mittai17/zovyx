...
import { mergeInboundPathRoots } from "@zuvix/media-core/inbound-path-policy";
import { findNormalizedProviderValue } from "@zuvix/model-catalog-core/provider-id";
import {
...
async function probeAntigravityCliCandidate(command: string): Promise<string | null> {
  ...
  const probeDir = await fs.mkdtemp(
    path.join(resolvePreferredZuvixTmpDir(), "zuvix-antigravity-probe-"),
  );
  try {
...
async function resolveAntigravityCliBinary(): Promise<string | null> {
  ...
  const resolved = (async () => {
    const configured = process.env.ZUVIX_ANTIGRAVITY_CLI?.trim();
