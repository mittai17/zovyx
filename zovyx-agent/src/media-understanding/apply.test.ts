...
const TEMP_MEDIA_PREFIX = "zuvix-media-";
...
function createGroqAudioConfig(): ZuvixConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): ZuvixConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): ZuvixConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      ZUVIX_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: ZuvixConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredZuvixTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: ZuvixConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("zuvix-antigravity-probe-"),
    });
