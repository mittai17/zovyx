...
const hoisted = vi.hoisted(() => ({
  completeMock: vi.fn(),
  ensureZuvixModelsJsonMock: vi.fn(async () => {}),
...
vi.mock("../agents/models-config.js", async () => ({
  ...
  ensureZuvixModelsJson: ensureZuvixModelsJsonMock,
}));
...
describe("describeImageWithModel", () => {
  ...
  it("resolves configured image models when discovery has not registered the provider", async () => {
    ...
    expect(resolveRequest.agentDir).toBe("/tmp/zuvix-agent");
    expect(
      requireRecord(
        requireRecord(
