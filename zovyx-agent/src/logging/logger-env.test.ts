...
const logPathTracker = createSuiteLogPathTracker("zuvix-test-env-log-level-");

describe("ZUVIX_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ZUVIX_LOG_LEVEL"]);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.ZUVIX_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.ZUVIX_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("ZUVIX_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid ZUVIX_LOG_LEVEL="nope"');
  });
});
