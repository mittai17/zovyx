...
const logPathTracker = createSuiteLogPathTracker("zuvix-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", () => {
    ...
    withEnv({ ZUVIX_CONFIG_PATH: configPath, ZUVIX_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-zuvix.log")).toBe(
        path.join(home, "custom-zuvix.log"),
      );
    });
  });
