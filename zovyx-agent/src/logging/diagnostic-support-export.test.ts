...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "zuvix.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["zuvix", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              ZUVIX_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).toContain("$ZUVIX_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/zuvix-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.ZUVIX_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\zuvix`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$ZUVIX_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\zuvix\\dist\\index.js`,
              "--config",
              `${stateDir}\\zuvix.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\zuvix\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\zuvix\\\\dist\\\\index.js");
    expect(serialized).toContain("$ZUVIX_STATE_DIR\\\\zuvix.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\zuvix\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/zuvix.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/zuvix-sanitized.jsonl");
