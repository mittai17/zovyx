...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredZuvixTmpDir() : POSIX_ZUVIX_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "zuvix.log")
    : `${POSIX_ZUVIX_TMP_DIR}/zuvix.log`;
}
...
type LoggerConfigLoader = () => ZuvixConfig["logging"] | undefined;
...
function resolveDefaultActiveLogFile(): string {
  if (process.env.VITEST === "true" && process.env.ZUVIX_TEST_FILE_LOG === "1") {
    return path.join(
      process.cwd(),
      ".artifacts",
      "test-logs",
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: ZuvixConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ZUVIX_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[zuvix] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
