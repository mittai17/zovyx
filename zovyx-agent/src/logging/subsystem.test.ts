...
const logPathTracker = createSuiteLogPathTracker("zuvix-subsystem-log-");
...
describe("createSubsystemLogger().isEnabled", () => {
  ...
  it("keeps long-lived subsystem loggers on the current-day rolling file", () => {
    ...
    const firstDay = path.join(logDir, "zuvix-2026-01-01.log");
    const secondDay = path.join(logDir, "zuvix-2026-01-02.log");
    vi.useFakeTimers();
