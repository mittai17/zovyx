...
const SUPPORT_EXPORT_PREFIX = "zuvix-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.ZUVIX_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$ZUVIX_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/zuvix-sanitized.jsonl`: sanitized log summaries and metadata",
