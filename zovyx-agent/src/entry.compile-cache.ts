...
export function shouldEnableZuvixCompileCache(params: {
...
export function resolveZuvixCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
export type ZuvixCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
};

type ZuvixCompileCacheRespawnRuntime = RespawnChildRuntime & {
  writeError: (message: string) => void;
};

export function buildZuvixCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
...
}): ZuvixCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
  if (!isSourceCheckoutInstallRoot(params.installRoot)) {
    return undefined;
  }
  if (env.ZUVIX_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return undefined;
  }
...
export function respawnWithoutZuvixCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
  ...
  runZuvixCompileCacheRespawnPlan(plan);
  return true;
}

export function runZuvixCompileCacheRespawnPlan(
  plan: ZuvixCompileCacheRespawnPlan,
  runtime: ZuvixCompileCacheRespawnRuntime = {
    spawn,
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        `[zuvix] Failed to respawn CLI without compile cache: ${
...
export function enableZuvixCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableZuvixCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveZuvixCompileCacheDirectory(params));
  } catch {
