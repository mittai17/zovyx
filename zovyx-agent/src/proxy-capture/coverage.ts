...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[zuvix proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[zuvix proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`zuvix proxy coverage\` for details.`,
  );
}
