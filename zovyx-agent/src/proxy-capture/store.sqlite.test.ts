...
function makeStore() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-proxy-capture-"));
...
describe("DebugProxyCaptureStore", () => {
  it("keeps the cached store open until the last lease releases", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-proxy-capture-lease-"));
    cleanupDirs.push(root);
  ...
  it("uses rollback journaling for captures on NFS-backed volumes", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "zuvix-proxy-capture-nfs-"));
    cleanupDirs.push(root);
