  ...
  return {
    ...
    agents: {
      defaults: {
        ...
        models: {
          "tui-pty-mock/gpt-5.5": { agentRuntime: { id: "zuvix" } },
        },
        skills: [],
        skipBootstrap: true,
      },
...
async function startLocalModeTui() {
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "zuvix-tui-pty-local-"));
  ...
  const script = [
    ...
    `program.parseAsync([process.execPath, "zuvix", "tui", "--local"], { from: "node" }).catch((error) => {`,
    `  console.error(error);`,
    `  process.exit(1);`,
    `});`,
  ].join("\n");
