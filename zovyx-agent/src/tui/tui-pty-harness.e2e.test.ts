...
async function writeTuiPtyFixtureScript(dir: string) {
  ...
  await writeFile(
    ...
    `
      ...
      const actionLogPath = process.env.ZUVIX_TUI_PTY_LOG_PATH;
      const gatewayStatus = process.env.ZUVIX_TUI_PTY_GATEWAY_STATUS ?? "fixture gateway ok";
      ...
      let fastMode = process.env.ZUVIX_TUI_PTY_FAST_MODE === "true";
...
async function startTuiFixture(opts: { env?: NodeJS.ProcessEnv } = {}) {
  const tempDir = await mkdtemp(path.join(tmpdir(), "zuvix-tui-pty-"));
  ...
  const run = startPty(process.execPath, ["--import", "tsx", scriptPath], {
    ...
    env: {
      ZUVIX_THEME: "dark",
      ZUVIX_TUI_PTY_LOG_PATH: logPath,
      NO_COLOR: undefined,
      ...opts.env,
    },
