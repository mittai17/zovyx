...
function hasProviderWildcardModelAllowlist(cfg: ZuvixConfig) {
  const modelMaps = [
...
function resolveConfiguredReplaceModeCatalog(cfg: ZuvixConfig) {
...
function shouldLoadFullGatewayCatalogForReplaceMode(cfg: ZuvixConfig) {
...
async function loadEmbeddedTuiModelCatalog(cfg: ZuvixConfig) {
      ...
      const result = await agentCommandFromIngress(
        {
          ...
          // bytes on the wire. See: https://github.com/zuvix/zuvix/issues/3658
