...
describe("resolveTuiFooterHostLabel", () => {
  ...
  it("renders only remote hosts when explicitly enabled", () => {
    const config = { tui: { footer: { showRemoteHost: true } } } satisfies ZuvixConfig;
...
describe("resolveInitialTuiAgentId", () => {
  const cfg: ZuvixConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/zuvix" },
        { id: "ops", workspace: "/tmp/zuvix/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/zuvix/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run zuvix devices list");
    expect(state.pairingHint).toContain("zuvix devices list");
  });
...
describe("TUI shutdown safety", () => {
  ...
  it("schedules a process-exit guard after standalone TUI return", () => {
    ...
    callback?.();
    expect(writeStderr).toHaveBeenCalledWith("zuvix tui forcing process exit after return\n");
...
describe("resolveLocalAuthCliInvocation", () => {
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/zuvix.mjs", "models", "auth", "login"],
    });
  });
});
