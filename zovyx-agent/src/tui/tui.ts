...
import { getRuntimeConfig, type ZuvixConfig } from "../config/config.js";
...
const ZUVIX_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../zuvix.mjs", import.meta.url));
const ZUVIX_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const ZUVIX_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const ZUVIX_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(ZUVIX_DIST_ENTRY_JS_PATH) || existsSync(ZUVIX_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(ZUVIX_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? ZUVIX_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? ZUVIX_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run zuvix devices list",
      pairingHint:
        "Pairing required. Run `zuvix devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("zuvix tui forcing process exit after return\n");
    } catch {
...
function resolveEmptySessionInfoDefaults(config: ZuvixConfig): SessionInfo {
  return {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__zuvix_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "zuvix tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("zuvix tui forcing exit\n");
    } catch {
    ...
    void Promise.resolve()
      ...
      .catch((err: unknown) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`zuvix tui shutdown failed: ${String(err)}\n`);
          } catch {
