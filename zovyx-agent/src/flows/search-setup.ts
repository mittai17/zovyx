...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<ZuvixConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<ZuvixConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.zuvix.ai/tools/web";
...
function rawKeyValue(config: ZuvixConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: ZuvixConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: ZuvixConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: ZuvixConfig, result: ZuvixConfig): ZuvixConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as ZuvixConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Zuvix will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.zuvix.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Zuvix will store a reference instead of the API key.",
