...
/**
 * Runtime bridge between MCP tools and the Zuvix Gateway channel APIs.
 *
...
export class ZuvixChannelBridge {
  ...
  constructor(
    private readonly cfg: ZuvixConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`zuvix mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(notification: ServerNotification): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`zuvix mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `zuvix mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
    }
  }
