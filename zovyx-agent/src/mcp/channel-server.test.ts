...
import { createZuvixChannelMcpServer, ZuvixChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createZuvixChannelMcpServer({
...
describe("zuvix channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new ZuvixChannelBridge({} as never, {
        ...
        expect((messages[1]?.["__zuvix"] as { id?: string } | undefined)?.id).toBe(
          "msg-attachment",
        );
        expect(
