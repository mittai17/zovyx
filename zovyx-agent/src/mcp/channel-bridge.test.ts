...
function makeBridge(verbose = false): BridgeInternals {
  return new ZuvixChannelBridge({} as never, {
    claudeChannelMode: "off",
    verbose,
  }) as unknown as BridgeInternals;
}

describe("ZuvixChannelBridge — pendingClaudePermissions / pendingApprovals memory bounds", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(0);
  });
  ...
  test("a failed notification still emits exactly one diagnostic record with verbose off", async () => {
    ...
    try {
      ...
      expect(writes[0]).toBe("zuvix mcp: notification channel/event failed\n");
