/**
 * Standalone MCP server that exposes Zuvix plugin-registered tools
...
function resolvePluginToolPolicy(config: ZuvixConfig): {
...
function resolveTools(config: ZuvixConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "zuvix-plugin-tools", tools });
}
