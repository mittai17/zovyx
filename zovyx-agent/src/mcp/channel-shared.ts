...
/** Resolve the visible message id, including Zuvix metadata attached to raw entries. */
export function resolveMessageId(entry: Record<string, unknown>): string | undefined {
  return (
    toText(entry.id) ??
    (entry["__zuvix"] && typeof entry["__zuvix"] === "object"
      ? toText((entry["__zuvix"] as { id?: unknown }).id)
      : undefined)
  );
}
