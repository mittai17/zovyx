...
vi.mock("./channel-bridge.js", () => ({
  ZuvixChannelBridge: class MockZuvixChannelBridge {
...
describe("serveZuvixChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveZuvixChannelMcp } = await import("./channel-server.js");

    const servePromise = serveZuvixChannelMcp({ verbose: false });
