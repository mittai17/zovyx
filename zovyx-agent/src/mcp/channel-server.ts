...
import { ZuvixChannelBridge } from "./channel-bridge.js";
import { ClaudePermissionRequestSchema, type ClaudeChannelMode } from "./channel-shared.js";
import { getChannelMcpCapabilities, registerChannelMcpTools } from "./channel-tools.js";

/**
 * MCP stdio server assembly for Zuvix channel conversations.
 *
 ...
 */
export { ZuvixChannelBridge } from "./channel-bridge.js";

/** Options accepted by the channel MCP server factory and stdio entry point. */
export type ZuvixMcpServeOptions = {
  gatewayUrl?: string;
  gatewayToken?: string;
...
async function resolveMcpConfig(config: ZuvixConfig | undefined): Promise<ZuvixConfig> {
  if (config) {
    return config;
  }
...
export async function createZuvixChannelMcpServer(opts: ZuvixMcpServeOptions = {}): Promise<{
  server: McpServer;
  bridge: ZuvixChannelBridge;
  start: () => Promise<void>;
  close: () => Promise<void>;
}> {
  ...
  const server = new McpServer(
    { name: "zuvix", version: VERSION },
    capabilities ? { capabilities } : undefined,
  );
  const bridge = new ZuvixChannelBridge(cfg, {
...
export async function serveZuvixChannelMcp(opts: ZuvixMcpServeOptions = {}): Promise<void> {
  const { server, start, close } = await createZuvixChannelMcpServer(opts);
