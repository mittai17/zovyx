// Zuvix SDK helper module supports normalize behavior.
import type { GatewayEvent, JsonObject, ZuvixEvent, ZuvixEventType } from "./types.js";
...
function normalizeLifecycleEndEventType(data: JsonObject): ZuvixEventType {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
...
function normalizeAgentEventType(payload: JsonObject): ZuvixEventType {
...
function normalizeNamedEventType(event: GatewayEvent): ZuvixEventType {
...
export function normalizeGatewayEvent(event: GatewayEvent): ZuvixEvent {
  const payload = asRecord(event.payload);
