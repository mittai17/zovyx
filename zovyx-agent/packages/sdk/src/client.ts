// Zuvix SDK module implements client behavior.
...
// High-level Zuvix SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the Zuvix SDK client. */
export type ZuvixOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: ZuvixTransport;
};

function resolveGatewayUrl(options: ZuvixOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Zuvix Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Zuvix Gateway yet`);
}
...
function readChatProjection(event: ZuvixEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: ZuvixEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: ZuvixEvent): boolean {
  return (
...
export class Zuvix {
  ...
  private readonly transport: ZuvixTransport;
  private readonly normalizedEvents = new EventHub<ZuvixEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, ZuvixEvent[]>();
  private connected = false;
  private eventPumpPromise: Promise<void> | null = null;
  private eventPumpReady: Promise<void> | null = null;

  constructor(options: ZuvixOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: ZuvixEvent) => boolean): AsyncIterable<ZuvixEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: ZuvixEvent) => boolean,
  ): AsyncIterable<ZuvixEvent> {
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: ZuvixEvent): ZuvixEvent | undefined => {
    ...
    const matches = (event: ZuvixEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: ZuvixEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): ZuvixEvent[] {
...
export class AgentsNamespace {
  constructor(private readonly client: Zuvix) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<ZuvixEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Zuvix,
