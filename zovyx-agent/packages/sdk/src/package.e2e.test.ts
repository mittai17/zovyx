// Zuvix SDK tests cover package behavior.
...
const WORKSPACE_PACKAGE_NAMES = [
  "@zuvix/gateway-protocol",
  "@zuvix/gateway-client",
  "@zuvix/sdk",
] as const;
...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@zuvix/") && spec === "workspace:*" ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
async function startZuvixRegistry(packages: PackedPackage[]): Promise<{
  registryUrl: string;
  close: () => Promise<void>;
}> {
...
describe("Zuvix SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zuvix-sdk-consumer-"));
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@zuvix/sdk")?.tarball ?? "";
    expect(sdkTarball).not.toBe("");
    const registry = await startZuvixRegistry(packedPackages);

    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ private: true, type: "module" }),
    );
    await fs.writeFile(path.join(tempDir, ".npmrc"), `@zuvix:registry=${registry.registryUrl}`);
    try {
      await runCommand(
        "npm",
    ...
    const importScript = `
      import { GatewayClientTransport, Zuvix, normalizeGatewayEvent } from "@zuvix/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof Zuvix !== "function") throw new Error("missing client export");
