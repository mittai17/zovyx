...
import { resolveChannelTtsVoiceDelivery } from "zuvix/plugin-sdk/channel-targets";
import type {
  ZuvixConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { redactSensitiveText } from "zuvix/plugin-sdk/logging-core";
import { transcodeAudioBuffer } from "zuvix/plugin-sdk/media-runtime";
import { clampTimerTimeoutMs } from "zuvix/plugin-sdk/number-runtime";
import {
...
import { isVerbose, logVerbose } from "zuvix/plugin-sdk/runtime-env";
import { tempWorkspaceSync, resolvePreferredZuvixTmpDir } from "zuvix/plugin-sdk/sandbox";
import { privateFileStoreSync } from "zuvix/plugin-sdk/security-runtime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
  normalizeOptionalString,
} from "zuvix/plugin-sdk/string-coerce-runtime";
import { stripMarkdown } from "zuvix/plugin-sdk/text-chunking";
import { resolveConfigDir, resolveUserPath } from "zuvix/plugin-sdk/text-utility-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.ZUVIX_TTS_PREFS?.trim();
  if (envPath) {
...
function resolveConfiguredSpeechVoiceModelRefs(cfg: ZuvixConfig | undefined): VoiceModelRef[] {
...
function sortSpeechProvidersForAutoSelection(cfg?: ZuvixConfig) {
...
function resolveTtsRuntimeConfig(cfg: ZuvixConfig): ZuvixConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: ZuvixConfig): TtsProvider[] {
  const effectiveCfg = cfg ? resolveTtsRuntimeConfig(cfg) : undefined;
