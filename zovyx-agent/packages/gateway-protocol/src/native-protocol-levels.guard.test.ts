...
describe("native Gateway protocol levels", () => {
  it("match the TypeScript source of truth", async () => {
    ...
    const androidPath = "apps/android/app/src/main/java/ai/zuvix/app/gateway/GatewayProtocol.kt";
  ...
  it("uses the min constant for native connect compatibility ranges", async () => {
    const swiftConnectFiles = [
      "apps/shared/ZuvixKit/Sources/ZuvixKit/GatewayChannel.swift",
      "apps/macos/Sources/ZuvixMacCLI/WizardCommand.swift",
    ];
    ...
    const androidPath = "apps/android/app/src/main/java/ai/zuvix/app/gateway/GatewaySession.kt";
    const android = await readRepoFile(androidPath);
    assertPattern(
      android,
      androidPath,
