...
function captureStateDirEnv() {
  const value = process.env.ZUVIX_STATE_DIR;
  return {
    restore() {
      if (value === undefined) {
        Reflect.deleteProperty(process.env, "ZUVIX_STATE_DIR");
      } else {
        Reflect.set(process.env, "ZUVIX_STATE_DIR", value);
      }
    },
  };
}
...
beforeEach(() => {
  ...
  Reflect.set(process.env, "ZUVIX_STATE_DIR", tmpDir);
});
...
describe("buildSessionEntry", () => {
  it("returns lineMap tracking original JSONL line numbers", async () => {
    ...
    const jsonlLines = [
      ...
      JSON.stringify({ type: "custom", customType: "zuvix.cache-ttl", data: {} }),
