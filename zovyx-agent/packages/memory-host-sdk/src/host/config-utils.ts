...
export { splitShellArgs } from "./zuvix-runtime-io.js";

// Shared Zuvix config helpers used by memory host, QMD, and agent context code.
...
/** Narrow Zuvix config shape consumed by memory host utilities. */
...
/** Resolve the underlying OS home before applying Zuvix-specific overrides. */
...
/** Resolve ZUVIX_HOME or the OS home, falling back to cwd for hermetic tests. */
function resolveRequiredHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const explicitHome = normalizeHomeValue(env.ZUVIX_HOME);
...
/** Resolve absolute user paths, including "~" against the effective Zuvix home. */
...
export function resolveStateDir(
  ...
  if (env.ZUVIX_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
  // Existing legacy state remains authoritative until an explicit migration creates .zuvix.
...
/** Resolve the default agent workspace, partitioned by ZUVIX_PROFILE when set. */
...
function listAgentEntries(cfg: ZuvixConfig): AgentConfig[] {
...
function resolveDefaultAgentId(cfg: ZuvixConfig): string {
...
function resolveAgentConfig(cfg: ZuvixConfig, agentId: string): AgentConfig | undefined {
