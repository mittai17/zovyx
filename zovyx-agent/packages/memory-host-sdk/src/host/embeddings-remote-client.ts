...
import { requireApiKey, resolveApiKeyForProvider } from "./zuvix-runtime-auth.js";
...
function resolveZuvixAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.ZUVIX_VERSION?.trim() : undefined;
  return {
    originator: "zuvix",
    ...(version ? { version } : {}),
    "User-Agent": version ? `zuvix/${version}` : "zuvix",
  };
}
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    Object.assign(headers, resolveZuvixAttributionHeaders());
  }
