...
import type {
  ...
  RequestFrame,
  ResponseFrame,
} from "@zuvix/gateway-protocol";
import {
  GATEWAY_CLIENT_MODES,
  GATEWAY_CLIENT_NAMES,
  type GatewayClientMode,
  type GatewayClientName,
} from "@zuvix/gateway-protocol/client-info";
import {
...
} from "@zuvix/gateway-protocol/connect-error-details";
import { resolveGatewayStartupRetryAfterMs } from "@zuvix/gateway-protocol/startup-unavailable";
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@zuvix/gateway-protocol/version";
...
// The package stays reusable by depending on host callbacks for Zuvix-owned
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    this.deps = {
      // Defaults keep the package inert outside Zuvix; device signing throws
  ...
  start() {
    ...
    const allowPrivateWs =
      (this.opts.env ?? process.env).ZUVIX_ALLOW_INSECURE_PRIVATE_WS === "1";
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          (allowPrivateWs
            ? ""
            : "Break-glass (trusted private networks only): set ZUVIX_ALLOW_INSECURE_PRIVATE_WS=1. ") +
          "Run `zuvix doctor --fix` for guidance.",
      );
