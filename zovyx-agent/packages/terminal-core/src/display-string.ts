...
/** Resolve the underlying OS home before applying Zuvix overrides. */
...
/** Resolve raw home with ZUVIX_HOME tilde expansion. */
function resolveRawHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string | undefined {
  const explicitHome = normalize(env.ZUVIX_HOME);
  if (explicitHome) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$ZUVIX_HOME" } : { home, prefix: "~" };
}

/** Replace the effective home path with "~" or "$ZUVIX_HOME" for terminal display. */
