...
Zuvix is the AI that actually does things.
...
Zuvix started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> Zuvix.
...
Zuvix runtime code reads the current configuration schema only.
...
`zuvix doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in Zuvix is a deliberate tradeoff: strong defaults without killing capability.
...
Zuvix has an extensive plugin API.
...
- Code plugins run Zuvix plugin code and are appropriate for deeper runtime extension.
...
Zuvix docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
Zuvix supports MCP as both a server and a runtime integration surface.
...
Zuvix is currently terminal-first by design.
...
Zuvix is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep Zuvix hackable by default.
