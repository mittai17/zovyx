  ...
  case "$requested" in
    *x)
      ...
      if [[ "${requested%%.*}" == "22" ]]; then
        zuvix_node_version_at_least "$actual" "22.19.0"
      fi
      ;;
    *.*.*)
      [[ "$actual" == "$requested" ]]
...
zuvix_active_node_version() {
  node -p 'process.versions.node' 2>/dev/null || true
}

zuvix_prepend_node_bin() {
...
zuvix_find_toolcache_node() {
  ...
  for root in \
    ...
    "${ZUVIX_CONTAINER_TOOL_CACHE:-/__t}" \
    "/opt/hostedtoolcache" \
  ...
  for node_root in ${roots[@]+"${roots[@]}"}; do
    while IFS= read -r candidate; do
      ...
      if zuvix_node_version_matches "$candidate_version" "$requested_node"; then
        printf '%s\n' "$candidate"
        return 0
      fi
...
zuvix_resolve_node_download_version() {
  local requested_node="$1"
  ...
  curl -fsSL https://nodejs.org/dist/index.json |
    ZUVIX_NODE_PREFIX="$prefix" python3 -c 'import json, os, sys
prefix = os.environ["ZUVIX_NODE_PREFIX"]
...
zuvix_download_node() {
  ...
  version="$(zuvix_resolve_node_download_version "$requested_node")"
  platform="$(zuvix_node_download_platform)" || return 1
  temp_root="${RUNNER_TEMP:-/tmp}"
  if command -v cygpath >/dev/null 2>&1; then
    temp_root="$(cygpath -u "$temp_root" 2>/dev/null || printf '%s\n' "$temp_root")"
  fi
  install_root="${temp_root}/zuvix-node-${version}-${platform}"
  if [[ "$platform" == win-* ]]; then
    ...
    if command -v pwsh >/dev/null 2>&1; then
      ...
      zuvix_prepend_node_bin "$node_bin_dir" "$ps_bin_dir"
      ...
      zuvix_prepend_node_bin "$node_bin_dir"
    fi
  else
    ...
    zuvix_prepend_node_bin "$install_root/bin"
  fi
}
...
zuvix_ensure_node() {
  ...
  active_node_version="$(zuvix_active_node_version)"
  if zuvix_node_version_matches "$active_node_version" "$requested_node"; then
    echo "Using active Node ${active_node_version} at $(command -v node)"
    return 0
  fi

  node_bin="$(zuvix_find_toolcache_node "$requested_node" || true)"
  if [[ -n "$node_bin" ]]; then
    echo "Using Node $("$node_bin" -p 'process.versions.node') from $node_bin"
    zuvix_prepend_node_bin "$(dirname "$node_bin")"
  else
    zuvix_download_node "$requested_node" || true
  fi
