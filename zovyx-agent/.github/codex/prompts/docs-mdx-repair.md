# Zuvix Docs MDX Repair Agent

You are repairing generated Zuvix documentation after a fast MDX validation failure.
...
1. Read `.zuvix-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".zuvix-sync/mdx/${LOCALE}.json"`.
