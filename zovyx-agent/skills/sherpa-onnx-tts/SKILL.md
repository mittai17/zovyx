...
1. Download the runtime for your OS (extracts into `$ZUVIX_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.zuvix/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$ZUVIX_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.zuvix/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Zuvix config file (`$ZUVIX_CONFIG_PATH`, default `~/.zuvix/zuvix.json`):
