---
name: coding-agent
description: "Delegate coding work to Codex, Claude Code, or Zuvix as background workers; not simple edits or read-only code lookup."
metadata:
  {
    "zuvix":
      {
...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.zuvix`, `$ZUVIX_STATE_DIR`, or active Zuvix state dirs.

## Hard rules

- Always launch with `background:true`.
- Codex and Zuvix: use `pty:true`.
- Claude Code: no PTY; use `claude --permission-mode bypassPermissions --print`.
- Capture a real notification route before spawning.
- Worker must send completion/failure via `zuvix message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/zuvix`; use an isolated checkout.
...
zuvix message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use zuvix system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t zuvix-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
...
```bash
bash pty:true background:true workdir:/path/repo command:"zuvix run < \"$PROMPT\""
```
