---
name: canvas
description: "Present HTML on connected Zuvix node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "zuvix": { "emoji": "🖼️" } }
---
...
Active config: `$ZUVIX_CONFIG_PATH` or `~/.zuvix/zuvix.json`.
...
4. Present the hosted URL: `/__zuvix__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__zuvix__/canvas/index.html
http://<gateway-host>:<gateway.port>/__zuvix__/canvas/games/snake.html
```

Path mapping:

- `/__zuvix__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__zuvix__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
