...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${ZUVIX_CONFIG_PATH:-${ZUVIX_STATE_DIR:-$HOME/.zuvix}/zuvix.json}"
...
```text
${ZUVIX_STATE_DIR:-$HOME/.zuvix}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with zuvix message send if route provided.
```
