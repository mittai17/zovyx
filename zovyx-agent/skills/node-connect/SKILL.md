---
name: node-connect
description: "Diagnose Zuvix Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Zuvix is advertising that route, then fix pairing/auth.
...
- whether `zuvix devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `zuvix qr --json`. It uses the same setup-code payload Android scans.

```bash
zuvix config get gateway.mode
zuvix config get gateway.bind
zuvix config get gateway.tailscale.mode
zuvix config get gateway.remote.url
zuvix config get gateway.auth.mode
zuvix config get gateway.auth.allowTailscale
zuvix config get plugins.entries.device-pair.config.publicUrl
zuvix qr --json
zuvix devices list
zuvix nodes status
```

If this Zuvix instance is pointed at a remote gateway, also run:

```bash
zuvix qr --remote --json
```
...
If `zuvix qr --json` says `Gateway is only bound to loopback`:
...
```bash
zuvix devices list
zuvix devices approve --latest   # preview only; copy the requestId from output
...
- `zuvix devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run zuvix qr again, rescan, then approve the pending device pairing.`
