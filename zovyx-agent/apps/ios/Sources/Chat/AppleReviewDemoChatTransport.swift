...
struct AppleReviewDemoChatTransport: ZuvixChatTransport {
...
private actor AppleReviewDemoChatStore {
    ...
    private var messages: [ZuvixChatMessage]

    init() {
        self.messages = AppleReviewDemoChatStore.seedMessages()
    }

    func createSession(key: String) throws -> ZuvixChatCreateSessionResponse {
        try Self.decode(
            CreateSessionPayload(ok: true, key: key, sessionId: "apple-review-demo-\(key)"),
            as: ZuvixChatCreateSessionResponse.self)
    }

    func history(sessionKey: String) throws -> ZuvixChatHistoryPayload {
        ...
        return try Self.decode(
            HistoryPayload(
                ...
                thinkingLevel: "auto"),
            as: ZuvixChatHistoryPayload.self)
    }

    func sendMessage(sessionKey _: String, message: String, runId: String) throws -> ZuvixChatSendResponse {
        ...
        return try Self.decode(
            ...
            as: ZuvixChatSendResponse.self)
    }

    func sessions() throws -> ZuvixChatSessionsListResponse {
        let entry = ZuvixChatSessionEntry(
            key: self.sessionKey,
            kind: "chat",
            ...
            thinkingLevels: [
                ZuvixChatThinkingLevelOption(id: "auto", label: "Auto"),
    ...
    private static func seedMessages() -> [ZuvixChatMessage] {
        let now = Date().timeIntervalSince1970 * 1000
        return [
            self.message(
                role: "assistant",
    ...
    private static func message(role: String, text: String, timestamp: Double) -> ZuvixChatMessage {
        ZuvixChatMessage(
            role: role,
            content: [
