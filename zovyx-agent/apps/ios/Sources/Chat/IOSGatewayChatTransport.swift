...
struct IOSGatewayChatTransport: ZuvixChatTransport {
    static let logger = Logger(subsystem: "ai.zuvix", category: "ios.chat.transport")
    ...
    private struct ChatSendParams: Codable {
        ...
        var message: String
        var thinking: String
        var attachments: [ZuvixChatAttachmentPayload]?
        var timeoutMs: Int
    ...
    static func makeChatSendParamsJSON(
        ...
        attachments: [ZuvixChatAttachmentPayload]) throws -> String
    {
        ...
        parentSessionKey: String?) async throws -> ZuvixChatCreateSessionResponse
    {
        ...
        return try JSONDecoder().decode(ZuvixChatCreateSessionResponse.self, from: res)
    }
    ...
    func listSessions(limit: Int?) async throws -> ZuvixChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ZuvixChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ZuvixChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ZuvixChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ZuvixChatAttachmentPayload]) async throws -> ZuvixChatSendResponse
    {
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(ZuvixChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(ZuvixGatewayHealthOK.self, from: res))?.ok ?? true
    }
    ...
    static func mapEventFrame(_ evt: EventFrame) -> ZuvixChatTransportEvent? {
        switch evt.event {
        case "tick":
            return .tick
        case "seqGap":
        ...
        case "health":
            ...
            let ok = (try? GatewayPayloadDecoding.decode(
                payload,
                as: ZuvixGatewayHealthOK.self))?.ok ?? true
            return .health(ok: ok)
        case "chat":
            guard let payload = evt.payload else { return nil }
        ...
        case "session.message":
            ...
            guard let message = try? GatewayPayloadDecoding.decode(
                payload,
                as: ZuvixSessionMessageEventPayload.self)
            else {
                return nil
            }
        ...
        case "agent":
            ...
            guard let agentPayload = try? GatewayPayloadDecoding.decode(
                payload,
                as: ZuvixAgentEventPayload.self)
            else {
                return nil
            }
