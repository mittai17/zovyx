...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "zuvix.exec-approval"
    static let reviewActionIdentifier = "zuvix.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.zuvixPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.zuvixPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func zuvixPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
