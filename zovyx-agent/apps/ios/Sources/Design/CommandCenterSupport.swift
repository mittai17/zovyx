...
struct CommandPanel<Content: View>: View {
    ...
    var body: some View {
        ProCard(
            ...
            radius: ZuvixProMetric.cardRadius)
        {
            self.content
        }
    }
...
struct CommandSessionRow: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 8)
        .background {
            RoundedRectangle(cornerRadius: ZuvixProMetric.controlRadius, style: .continuous)
                .fill(self.rowFill)
                .overlay {
                    RoundedRectangle(cornerRadius: ZuvixProMetric.controlRadius, style: .continuous)
    ...
    var body: some View {
        HStack(spacing: 10) {
            Image(systemName: self.icon)
                ...
                .foregroundStyle(ZuvixBrand.ok)
                .frame(width: 30, height: 30)
                .background {
                    RoundedRectangle(cornerRadius: 8, style: .continuous)
                        .fill(ZuvixBrand.ok.opacity(0.10))
                }
