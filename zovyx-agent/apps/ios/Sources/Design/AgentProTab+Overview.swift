...
extension AgentProTab {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                self.metricTile(
                    ...
                    color: self.gatewayConnected ? ZuvixBrand.accent : .secondary,
                    route: .skills)
                self.metricTile(
    ...
    func agentRow(_ agent: AgentSummary) -> some View {
        ...
        return HStack(alignment: .top, spacing: 12) {
            ...
            .buttonStyle(.plain)
            .foregroundStyle(isActive ? ZuvixBrand.accent : .primary)
    ...
    func cronJobRow(_ job: CronJob) -> some View {
        HStack(spacing: 12) {
            ProIconBadge(
                ...
                color: job.enabled ? ZuvixBrand.accent : .secondary)
            VStack(alignment: .leading, spacing: 3) {
                Text(job.name)
                    .font(.subheadline.weight(.semibold))
                    .lineLimit(1)
            ...
            Text(self.cronJobState(job))
                .font(.caption2.weight(.semibold))
                .foregroundStyle(job.enabled ? ZuvixBrand.accent : .secondary)
                .lineLimit(1)
        }
        .padding(.vertical, 10)
    ...
    var instancesColor: Color {
        ...
        return (self.overview?.presence.isEmpty == false) ? ZuvixBrand.accent : .secondary
    }
    ...
    var cronColor: Color {
        guard self.gatewayConnected else { return .secondary }
        return self.overview?.cronStatus?.enabled == true ? ZuvixBrand.accent : .secondary
    }
    ...
    var dreamingColor: Color {
        guard self.gatewayConnected else { return .secondary }
        return self.overview?.dreaming?.enabled == true ? ZuvixBrand.accent : .secondary
    }
