...
extension SettingsProTab {
        ...
        ProCard(padding: 0, radius: SettingsLayout.cardRadius) {
            VStack(spacing: 0) {
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.gatewayController.gateways.isEmpty ? .secondary : ZuvixBrand.accent)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "waveform",
                    title: "Talk Config",
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.appModel.screenRecordActive ? ZuvixBrand.ok : .secondary)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "mic",
                    title: "Voice Wake",
                    ...
                    color: self.voiceWakeEnabled ? ZuvixBrand.ok : .secondary)
            }
        }
        .padding(.horizontal, ZuvixProMetric.pagePadding)
    }
    ...
    func handleLocationModeChange(_ newValue: String) {
        ...
        guard let mode = ZuvixLocationMode(rawValue: newValue) else { return }
    ...
    func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Run /pair approve in your Zuvix chat, then connect again."
        }
    ...
    var gatewayStatusColor: Color {
        if self.appModel.isAppleReviewDemoModeEnabled { return ZuvixBrand.accent }
        return self.gatewayConnected ? ZuvixBrand.ok : .secondary
    }
    ...
    var gatewayTalkConfigColor: Color {
        ...
        return self.appModel.talkMode.gatewayTalkConfigLoaded ? ZuvixBrand.ok : .secondary
    }
    ...
    var gatewayServer: String {
        self.appModel.gatewayServerName ?? "Zuvix Gateway"
    }

    var permissionsDetail: String {
        var enabled = 0
        if self.cameraEnabled { enabled += 1 }
        if self.locationModeRaw != ZuvixLocationMode.off.rawValue { enabled += 1 }
    ...
    var diagnosticsRunColor: Color {
        ...
        return diagnosticsIssueCount == 0 ? ZuvixBrand.ok : ZuvixBrand.warn
    }

    var privacyDetail: String {
        let location = ZuvixLocationMode(rawValue: self.locationModeRaw) ?? .off
        return location == .off ? "Location off" : "Location \(self.locationLabel)"
    }

    var locationLabel: String {
        switch ZuvixLocationMode(rawValue: self.locationModeRaw) ?? .off {
        case .off: "Off"
