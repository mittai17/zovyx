...
struct TalkProTab: View {
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 11) {
            ...
            ZuvixProMark(size: 31, shadowRadius: 9)
...
struct TalkProState: Equatable {
    ...
    var title: String {
        ...
        if self.normalizedStatus.contains("thinking") { return "Asking Zuvix" }
    ...
    var color: Color {
        ...
        case .requestFailed, .loadFailed:
            return ZuvixBrand.danger
        case .missingScope, .requestingUpgrade, .upgradeRequested, .apiKeyMissing:
            return ZuvixBrand.warn
        default:
            if !self.isConfigLoaded { return ZuvixBrand.warn }
            return self.isEnabled ? ZuvixBrand.ok : ZuvixBrand.accentHot
        }
    }
    ...
    var primaryButtonFill: AnyShapeStyle {
        switch self.primaryAction {
        case .stop:
            AnyShapeStyle(ZuvixBrand.danger)
        case .waiting:
            AnyShapeStyle(ZuvixBrand.warn.opacity(0.72))
        default:
            AnyShapeStyle(LinearGradient(
                colors: [self.color.opacity(0.95), ZuvixBrand.accent],
