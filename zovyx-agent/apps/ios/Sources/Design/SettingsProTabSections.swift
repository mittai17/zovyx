...
extension SettingsProTab {
    ...
    var settingsListSection: some View {
        VStack(spacing: 10) {
            self.settingsListRow(
                ...
                route: .approvals,
                color: self.pendingApproval == nil ? .secondary : ZuvixBrand.warn,
            ...
            self.settingsListRow(
                icon: "info.circle",
                title: "About",
                detail: DeviceInfoHelper.zuvixVersionString(),
                route: .about)
        }
        .padding(.horizontal, ZuvixProMetric.pagePadding)
    }
    ...
    func destination(for route: SettingsRoute) -> some View {
        ZStack {
            ...
            ScrollView {
                ...
                .padding(.top, 18)
                .padding(.bottom, ZuvixProMetric.bottomScrollInset)
            }
        }
    ...
    var approvalsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.pendingApproval == nil ? ZuvixBrand.ok : ZuvixBrand.warn)
                ...
                if let pendingApproval {
                    ...
                    if let errorText = self.appModel.pendingExecApprovalPromptErrorText {
                        Text(errorText)
                            ...
                            .foregroundStyle(ZuvixBrand.danger)
                    }
                    ...
                    HStack(spacing: 12) {
                        ProIconBadge(systemName: "checkmark.shield.fill", color: ZuvixBrand.ok)
                        VStack(alignment: .leading, spacing: 3) {
    ...
    var permissionsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.toggleCard(
                icon: "camera",
                title: "Camera",
                detail: "Allow the gateway to request photos or video while Zuvix is foregrounded.",
            ...
            self.toggleCard(
                icon: "lock.display",
                title: "Keep Awake",
                detail: "Keep the screen awake while Zuvix is open.",
    ...
    var voiceDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? ZuvixBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.gatewayDiagnosticConnected ? ZuvixBrand.ok : ZuvixBrand.warn)
            ...
            self.detailListCard {
                ...
                Divider()
                self.detailRow("App", value: DeviceInfoHelper.zuvixVersionString())
                Divider()
    ...
    var privacyDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "hand.raised",
                title: "Privacy",
                detail: "Control what device context Zuvix can expose to the gateway.",
    ...
    var notificationsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "bell",
                title: "Notifications",
                detail: "Approvals and event alerts from Zuvix.",
                value: self.notificationStatusText,
                color: self.notificationStatusText == "Allowed" ? ZuvixBrand.ok : .secondary)
            ...
            ProCard(radius: SettingsLayout.cardRadius) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("Zuvix uses notifications for approval prompts and mirrored event alerts.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    var aboutDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                value: DeviceInfoHelper.zuvixVersionString(),
                color: ZuvixBrand.accent)

            self.detailListCard {
                self.detailRow("Version", value: DeviceInfoHelper.zuvixVersionString())
                Divider()
                self.detailRow("Device", value: DeviceInfoHelper.deviceFamily())
                Divider()
                self.detailRow("Platform", value: DeviceInfoHelper.platformStringForDisplay())
        ...
        isOn: Binding<Bool>) -> some View
    {
        ProCard(radius: SettingsLayout.cardRadius) {
            Toggle(isOn: isOn) {
                HStack(spacing: 12) {
                    ProIconBadge(systemName: icon, color: isOn.wrappedValue ? ZuvixBrand.accent : .secondary)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text(detail)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 12) {
                    ProIconBadge(
                        systemName: "location",
                        color: self.locationModeRaw == ZuvixLocationMode.off.rawValue ? .secondary : ZuvixBrand
                            .accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Location")
                            .font(.subheadline.weight(.semibold))
                ...
                Picker("Location", selection: self.$locationModeRaw) {
                    Text("Off").tag(ZuvixLocationMode.off.rawValue)
                    Text("While Using").tag(ZuvixLocationMode.whileUsing.rawValue)
                    Text("Always").tag(ZuvixLocationMode.always.rawValue)
                }
                .pickerStyle(.segmented)
                ...
                if let locationStatusText {
                    Text(locationStatusText)
                        .font(.caption2)
                        .foregroundStyle(ZuvixBrand.warn)
                }
            }
        }
        .padding(.horizontal, ZuvixProMetric.pagePadding)
