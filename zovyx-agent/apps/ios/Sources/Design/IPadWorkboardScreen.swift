            ...
            Text(IPadWorkboardDefaults.label(for: status))
                ...
                .overlay {
                    Capsule()
                        .strokeBorder(
                            self.selectedStatus == status
                                ? ZuvixBrand.accent.opacity(0.42)
                                : Color.primary.opacity(0.08),
                            lineWidth: 1)
                }
        }
        .buttonStyle(.plain)
        .foregroundStyle(self.selectedStatus == status ? ZuvixBrand.accent : .primary)
...
private struct IPadWorkboardKanbanCard: View {
    ...
    private var color: Color {
        switch self.card.status {
        case "running": ZuvixBrand.ok
        case "review": ZuvixBrand.accent
        case "blocked": ZuvixBrand.warn
        case "done": .secondary
        default: ZuvixBrand.accentHot
        }
    }
