...
struct CommandCenterTab: View {
    ...
    @State private var defaultChatSessionEntry: ZuvixChatSessionEntry?
    @State private var recentChatSessions: [ZuvixChatSessionEntry] = []
    var headerTitle: String = "Zuvix"
    var headerLeadingAction: ZuvixSidebarHeaderAction?
        ...
        NavigationStack {
            GeometryReader { geometry in
                ZStack {
                    ...
                    .safeAreaPadding(.bottom, ZuvixProMetric.bottomScrollInset)
                }
            }
    ...
    private var header: some View {
            ...
            subtitleLineLimit: 1)
        {
            ...
            {
                ZuvixProMark(size: 28, shadowRadius: 5)
            }
        } accessory: {
        ...
        HStack(spacing: 8) {
            ...
            if let badgeValue {
                Text(badgeValue)
                    ...
                    .padding(.vertical, 3)
                    .background(ZuvixBrand.accentHot, in: Capsule())
            }
            Spacer(minLength: 8)
            if let value {
                if let action {
    ...
    private var gatewayStatusColor: Color {
        self.gatewayConnected ? ZuvixBrand.ok : .secondary
    }
    ...
    private var defaultChatWorkItem: WorkItem {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isOpen ? ZuvixBrand.accent : ZuvixBrand.ok,
            progress: nil,
            route: .chat(nil))
    }
    ...
    private func refreshRecentSessionsIfNeeded() async {
        ...
        do {
            let transport: any ZuvixChatTransport = self.appModel.isAppleReviewDemoModeEnabled
    ...
    private static func sessionChoices(
        _ sessions: [ZuvixChatSessionEntry],
        currentSessionKey: String,
        defaultSessionKey: String) -> [ZuvixChatSessionEntry]
    {
    ...
    static func sessionWorkItem(
        for session: ZuvixChatSessionEntry,
        currentSessionKey: String) -> WorkItem
    {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isCurrent ? ZuvixBrand.accent : ZuvixBrand.ok,
            progress: nil,
            route: .chat(session.key))
    }

    fileprivate static func sessionTitle(_ session: ZuvixChatSessionEntry) -> String {
    ...
    fileprivate static func sessionDetail(_ session: ZuvixChatSessionEntry) -> String {
...
struct CommandSessionsScreen: View {
    ...
    @State private var sessions: [ZuvixChatSessionEntry] = []
    @State private var isLoading = false
    @State private var loadErrorText: String?
    let headerLeadingAction: ZuvixSidebarHeaderAction?
    let openChat: () -> Void

    init(headerLeadingAction: ZuvixSidebarHeaderAction? = nil, openChat: @escaping () -> Void) {
