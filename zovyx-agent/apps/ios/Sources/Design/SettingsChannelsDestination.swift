...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return ZuvixBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? ZuvixBrand.ok : ZuvixBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return ZuvixBrand.ok }
        if self.lastError != nil { return ZuvixBrand.warn }
        return self.configured ? ZuvixBrand.accent : .secondary
    }
