...
final class RemindersService: RemindersServicing {
    func list(params: ZuvixRemindersListParams) async throws -> ZuvixRemindersListPayload {
        ...
        let payload: [ZuvixReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return ZuvixRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ZuvixRemindersAddParams) async throws -> ZuvixRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ZuvixReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ZuvixRemindersAddPayload(reminder: payload)
    }
