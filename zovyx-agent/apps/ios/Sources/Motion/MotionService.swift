...
final class MotionService: MotionServicing {
    func activities(params: ZuvixMotionActivityParams) async throws -> ZuvixMotionActivityPayload {
        ...
        let mapped: [ZuvixMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return ZuvixMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = ZuvixPedometerPayload(
