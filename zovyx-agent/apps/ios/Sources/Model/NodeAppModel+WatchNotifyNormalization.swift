...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: ZuvixWatchNotifyParams) -> ZuvixWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [ZuvixWatchAction]?,
        kind: String?,
        promptId: String?) -> [ZuvixWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> ZuvixWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ZuvixWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                ZuvixWatchAction(id: "approve", label: "Approve"),
                ZuvixWatchAction(id: "decline", label: "Decline", style: "destructive"),
                ZuvixWatchAction(id: "open_phone", label: "Open iPhone"),
                ZuvixWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            ZuvixWatchAction(id: "done", label: "Done"),
            ZuvixWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            ZuvixWatchAction(id: "open_phone", label: "Open iPhone"),
            ZuvixWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: ZuvixWatchRisk?,
        priority: ZuvixNotificationPriority?) -> ZuvixWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: ZuvixNotificationPriority?,
        risk: ZuvixWatchRisk?) -> ZuvixNotificationPriority?
    {
