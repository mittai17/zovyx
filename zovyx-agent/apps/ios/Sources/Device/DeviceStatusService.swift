...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> ZuvixDeviceStatusPayload {
        ...
        return ZuvixDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> ZuvixBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: ZuvixBatteryState = switch device.batteryState {
        ...
        return ZuvixBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> ZuvixThermalStatusPayload {
        let state: ZuvixThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return ZuvixThermalStatusPayload(state: state)
    }

    private func storageStatus() -> ZuvixStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return ZuvixStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
