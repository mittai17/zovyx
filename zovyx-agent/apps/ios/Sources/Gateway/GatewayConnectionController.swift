...
extension GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [
            ...
            ZuvixCapability.screen.rawValue,
        ]
        ...
        if cameraEnabled { caps.append(ZuvixCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(ZuvixCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = ZuvixLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(ZuvixCapability.location.rawValue) }

        caps.append(ZuvixCapability.device.rawValue)
        caps.append(ZuvixCapability.talk.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(ZuvixCapability.watch.rawValue)
        }
        caps.append(ZuvixCapability.photos.rawValue)
        caps.append(ZuvixCapability.contacts.rawValue)
        caps.append(ZuvixCapability.calendar.rawValue)
        caps.append(ZuvixCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(ZuvixCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            ZuvixScreenCommand.record.rawValue,
            ZuvixSystemCommand.notify.rawValue,
            ZuvixChatCommand.push.rawValue,
        ...
        if caps.contains(ZuvixCapability.camera.rawValue) {
            commands.append(ZuvixCameraCommand.list.rawValue)
            commands.append(ZuvixCameraCommand.snap.rawValue)
            commands.append(ZuvixCameraCommand.clip.rawValue)
        }
        if caps.contains(ZuvixCapability.location.rawValue) {
            commands.append(ZuvixLocationCommand.get.rawValue)
        }
        if caps.contains(ZuvixCapability.device.rawValue) {
            commands.append(ZuvixDeviceCommand.status.rawValue)
            commands.append(ZuvixDeviceCommand.info.rawValue)
        }
        if caps.contains(ZuvixCapability.watch.rawValue) {
            commands.append(ZuvixWatchCommand.status.rawValue)
            commands.append(ZuvixWatchCommand.notify.rawValue)
        }
        if caps.contains(ZuvixCapability.photos.rawValue) {
            commands.append(ZuvixPhotosCommand.latest.rawValue)
        }
        if caps.contains(ZuvixCapability.contacts.rawValue) {
            commands.append(ZuvixContactsCommand.search.rawValue)
            commands.append(ZuvixContactsCommand.add.rawValue)
        }
        if caps.contains(ZuvixCapability.calendar.rawValue) {
            commands.append(ZuvixCalendarCommand.events.rawValue)
            commands.append(ZuvixCalendarCommand.add.rawValue)
        }
        if caps.contains(ZuvixCapability.reminders.rawValue) {
            commands.append(ZuvixRemindersCommand.list.rawValue)
            commands.append(ZuvixRemindersCommand.add.rawValue)
        }
        if caps.contains(ZuvixCapability.motion.rawValue) {
            commands.append(ZuvixMotionCommand.activity.rawValue)
            commands.append(ZuvixMotionCommand.pedometer.rawValue)
        }
