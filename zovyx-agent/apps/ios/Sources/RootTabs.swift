...
struct RootTabs: View {
    ...
    private static var initialTab: AppTab {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--zuvix-initial-tab") else {
            return .control
        }
    ...
    private static var requestedInitialSidebarDestination: SidebarDestination? {
        let arguments = ProcessInfo.processInfo.arguments
        guard let flagIndex = arguments.firstIndex(of: "--zuvix-initial-destination") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
    ...
    private static var initialChatSessionKey: String? {
        let arguments = ProcessInfo.processInfo.arguments
        guard let flagIndex = arguments.firstIndex(of: "--zuvix-chat-session") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
        guard arguments.indices.contains(valueIndex) else { return nil }
    ...
    private var sidebarIdentityHeader: some View {
        HStack(spacing: 10) {
            ZuvixProMark(size: 30, shadowRadius: 3)
        ...
        .accessibilityLabel("Zuvix \(self.sidebarGatewayStatusTitle)")
    }
        ...
        title: String? = nil) -> some View
    {
        ...
        .buttonStyle(.plain)
        .foregroundStyle(destination == self.selectedSidebarDestination ? ZuvixBrand.accent : .primary)
        .listRowBackground(
            destination == self.selectedSidebarDestination
                ? ZuvixBrand.accent.opacity(0.12)
                : Color.clear)
    ...
    private var sidebarHeaderLeadingAction: ZuvixSidebarHeaderAction? {
        ...
        if self.isSidebarVisible {
            return ZuvixSidebarHeaderAction(
    ...
    private var sidebarHideButton: some View {
        ...
        .contentShape(Rectangle())
        .buttonStyle(.plain)
        .foregroundStyle(ZuvixBrand.accent)
        .accessibilityLabel("Hide Sidebar")
        .accessibilityIdentifier(Self.sidebarHideButtonAccessibilityIdentifier)
    }
    ...
    private func makeHomeCanvasPayload() -> RootTabsHomeCanvasPayload {
        ...
        case .connected:
            return RootTabsHomeCanvasPayload(
                ...
                footer: "Zuvix only runs phone-side capabilities while the app is connected and permitted.")
        case .connecting:
            return RootTabsHomeCanvasPayload(
        ...
        case .error, .disconnected:
            return RootTabsHomeCanvasPayload(
                ...
                eyebrow: self.gatewayStatus == .error ? "Gateway needs attention" : "Zuvix iOS",
                ...
                agents: Array(agents.prefix(4)),
                footer:
                "Use Settings to scan a pairing QR code or paste a setup code from your Zuvix gateway.")
        }
    }
