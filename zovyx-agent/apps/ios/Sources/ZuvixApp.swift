...
@MainActor
enum ZuvixAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class ZuvixAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.zuvix.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.zuvix.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.zuvix.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? ZuvixAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "zuvix.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "zuvix.watch.promptId"
    static let sessionKeyKey = "zuvix.watch.sessionKey"
    static let actionPrimaryIDKey = "zuvix.watch.action.primary.id"
    static let actionPrimaryLabelKey = "zuvix.watch.action.primary.label"
    static let actionSecondaryIDKey = "zuvix.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "zuvix.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "zuvix.watch.action.primary"
    static let actionSecondaryIdentifier = "zuvix.watch.action.secondary"
    static let actionIdentifierPrefix = "zuvix.watch.action."
    static let actionIDKeyPrefix = "zuvix.watch.action.id."
    static let actionLabelKeyPrefix = "zuvix.watch.action.label."
    static let categoryPrefix = "zuvix.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: ZuvixWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> ZuvixWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return ZuvixWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Zuvix" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [ZuvixWatchAction]) -> [UNNotificationAction] {
...
struct ZuvixApp: App {
    ...
    @UIApplicationDelegateAdaptor(ZuvixAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        ZuvixAppModelRegistry.appModel = appModel
...
extension ZuvixApp {
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("Zuvix: installing uncaught exception handler")
