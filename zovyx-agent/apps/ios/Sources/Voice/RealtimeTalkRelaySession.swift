...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "zuvix_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.zuvix", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "Zuvix tool call failed"]
                : ["text": completion.text ?? "Zuvix finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: ZuvixChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
