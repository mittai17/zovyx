...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ZUVIX_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<ZuvixTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.zuvix", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = ZuvixTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> ZuvixTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return ZuvixTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> ZuvixTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> ZuvixTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return ZuvixTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> ZuvixTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = ZuvixTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: ZuvixTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = ZuvixChatEventText.assistantText(from: chatEvent) {
