...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.zuvix.ios", category: "LiveActivity")
    private let connectingStaleSeconds: TimeInterval = 120
    private let hydrationStaleSeconds: TimeInterval = 300
    private var currentActivity: Activity<ZuvixActivityAttributes>?
    ...
    func showConnecting(statusText: String = "Connecting...", agentName: String, sessionKey: String) {
        ...
        let attributes = ZuvixActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<ZuvixActivityAttributes>.activities
    ...
    private func updateCurrent(state: ZuvixActivityAttributes.ContentState, staleDate: Date? = nil) {
    ...
    private func end(activity: Activity<ZuvixActivityAttributes>) {
        Task {
            await activity.end(
                ActivityContent(state: self.disconnectedState(), staleDate: nil),
                dismissalPolicy: .immediate)
    ...
    private func connectingState(statusText: String = "Connecting...") -> ZuvixActivityAttributes.ContentState {
        ZuvixActivityAttributes.ContentState(
    ...
    private func attentionState(statusText: String) -> ZuvixActivityAttributes.ContentState {
        ZuvixActivityAttributes.ContentState(
            statusText: statusText,
            isIdle: false,
            isDisconnected: false,
    ...
    private func disconnectedState() -> ZuvixActivityAttributes.ContentState {
        ZuvixActivityAttributes.ContentState(
