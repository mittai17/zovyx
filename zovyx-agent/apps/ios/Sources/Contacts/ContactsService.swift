...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ZuvixContactsSearchParams) async throws -> ZuvixContactsSearchPayload {
        ...
        return ZuvixContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ZuvixContactsAddParams) async throws -> ZuvixContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ZuvixContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ZuvixContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ZuvixContactPayload {
        ZuvixContactPayload(
