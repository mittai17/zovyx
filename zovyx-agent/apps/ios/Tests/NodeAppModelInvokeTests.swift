...
private final class MockWatchMessagingService: @preconcurrency WatchMessagingServicing, @unchecked Sendable {
    ...
    var sendError: Error?
    var lastSent: (id: String, params: ZuvixWatchNotifyParams)?
    var lastSentExecApprovalPrompt: ZuvixWatchExecApprovalPromptMessage?
    var lastSentExecApprovalResolved: ZuvixWatchExecApprovalResolvedMessage?
    var lastSentExecApprovalExpired: ZuvixWatchExecApprovalExpiredMessage?
    var lastSentExecApprovalSnapshot: ZuvixWatchExecApprovalSnapshotMessage?
    ...
    func sendNotification(id: String, params: ZuvixWatchNotifyParams) async throws -> WatchNotificationSendResult {
...
@Suite(.serialized) struct NodeAppModelInvokeTests {
    @Test @MainActor func decodeParamsFailsWithoutJSON() {
        #expect(throws: Error.self) {
            _ = try NodeAppModel._test_decodeParams(ZuvixCanvasNavigateParams.self, from: nil)
        }
    }
    ...
    @Test @MainActor func handleInvokeRejectsBackgroundCommands() async {
        ...
        let req = BridgeInvokeRequest(id: "bg", command: ZuvixCanvasCommand.present.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsCameraWhenDisabled() async {
        let appModel = NodeAppModel()
        let req = BridgeInvokeRequest(id: "cam", command: ZuvixCameraCommand.snap.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsInvalidScreenFormat() async {
        let appModel = NodeAppModel()
        let params = ZuvixScreenRecordParams(format: "gif")
        let data = try? JSONEncoder().encode(params)
        let json = data.flatMap { String(data: $0, encoding: .utf8) }

        let req = BridgeInvokeRequest(
            id: "screen",
            command: ZuvixScreenCommand.record.rawValue,
            paramsJSON: json)
    ...
    @Test @MainActor func handleInvokeCanvasCommandsUpdateScreen() async throws {
        ...
        let present = BridgeInvokeRequest(id: "present", command: ZuvixCanvasCommand.present.rawValue)
        ...
        let navigateParams = ZuvixCanvasNavigateParams(url: "http://example.com/")
        let navData = try JSONEncoder().encode(navigateParams)
        let navJSON = String(decoding: navData, as: UTF8.self)
        let navigate = BridgeInvokeRequest(
            id: "nav",
            command: ZuvixCanvasCommand.navigate.rawValue,
            paramsJSON: navJSON)
        let navRes = await appModel._test_handleInvoke(navigate)
        #expect(navRes.ok == true)
        #expect(appModel.screen.urlString == "http://example.com/")

        let evalParams = ZuvixCanvasEvalParams(javaScript: "1+1")
        ...
        let eval = BridgeInvokeRequest(
            id: "eval",
            command: ZuvixCanvasCommand.evalJS.rawValue,
            paramsJSON: evalJSON)
    ...
    @Test @MainActor func handleInvokeA2UICommandsFailWhenLocalHostUnavailable() async throws {
        ...
        let reset = BridgeInvokeRequest(id: "reset", command: ZuvixCanvasA2UICommand.reset.rawValue)
        ...
        let pushParams = ZuvixCanvasA2UIPushJSONLParams(jsonl: jsonl)
        ...
        let push = BridgeInvokeRequest(
            id: "push",
            command: ZuvixCanvasA2UICommand.pushJSONL.rawValue,
            paramsJSON: pushJSON)
    ...
    @Test @MainActor func handleInvokeWatchStatusReturnsServiceSnapshot() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let req = BridgeInvokeRequest(id: "watch-status", command: ZuvixWatchCommand.status.rawValue)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)

        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(ZuvixWatchStatusPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRoutesToWatchService() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = ZuvixWatchNotifyParams(
            title: "Zuvix",
        ...
        let req = BridgeInvokeRequest(
            id: "watch-notify",
            command: ZuvixWatchCommand.notify.rawValue,
            paramsJSON: paramsJSON)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)
        #expect(watchService.lastSent?.params.title == "Zuvix")
        ...
        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(ZuvixWatchNotifyPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRejectsEmptyMessage() async throws {
        let watchService = MockWatchMessagingService()
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = ZuvixWatchNotifyParams(title: "   ", body: "\n")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyDerivesPriorityFromRiskAndCapsActions() async throws {
        ...
        let params = ZuvixWatchNotifyParams(
            ...
            risk: .high,
            actions: [
                ZuvixWatchAction(id: "a1", label: "A1"),
                ZuvixWatchAction(id: "a2", label: "A2"),
                ZuvixWatchAction(id: "a3", label: "A3"),
                ZuvixWatchAction(id: "a4", label: "A4"),
                ZuvixWatchAction(id: "a5", label: "A5"),
            ])
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyReturnsUnavailableOnDeliveryFailure() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = ZuvixWatchNotifyParams(title: "Zuvix", body: "Delivery check")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleDeepLinkSetsErrorWhenNotConnected() async throws {
        let appModel = NodeAppModel()
        let url = try #require(URL(string: "zuvix://agent?message=hello"))
    ...
    @Test @MainActor func handleDeepLinkRejectsOversizedMessage() async throws {
        ...
        let url = try #require(URL(string: "zuvix://agent?message=\(msg)"))
        await appModel.handleDeepLink(url: url)
    ...
    @Test @MainActor func operatorAdminScopeCacheRefreshesFromStoredToken() throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["ZUVIX_STATE_DIR"]
        setenv("ZUVIX_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("ZUVIX_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("ZUVIX_STATE_DIR")
            }
