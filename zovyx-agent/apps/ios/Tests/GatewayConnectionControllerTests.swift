...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": ZuvixLocationMode.always.rawValue,
            ...
            #expect(caps.contains(ZuvixCapability.canvas.rawValue))
            #expect(caps.contains(ZuvixCapability.screen.rawValue))
            #expect(caps.contains(ZuvixCapability.camera.rawValue))
            #expect(caps.contains(ZuvixCapability.location.rawValue))
            #expect(caps.contains(ZuvixCapability.voiceWake.rawValue))
            #expect(caps.contains(ZuvixCapability.talk.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": ZuvixLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(ZuvixLocationCommand.get.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(commands.contains(ZuvixSystemCommand.notify.rawValue))
            #expect(!commands.contains(ZuvixSystemCommand.run.rawValue))
            #expect(!commands.contains(ZuvixSystemCommand.which.rawValue))
            #expect(!commands.contains(ZuvixSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(ZuvixSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.zuvix.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(prior, service: "ai.zuvix.gateway", account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.zuvix.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.zuvix.gateway", account: "lastConnection")
