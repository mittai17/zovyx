...
enum CommandResolver {
    private static let projectRootDefaultsKey = "zuvix.gatewayProjectRootPath"
    private static let helperName = "zuvix"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let zuvixEntry = root.appendingPathComponent("zuvix.mjs").path
        if FileManager().isReadableFile(atPath: zuvixEntry) { return zuvixEntry }
        let binEntry = root.appendingPathComponent("bin/zuvix.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/zuvix")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let zuvixPaths = self.zuvixManagedPaths(home: home)
        if !zuvixPaths.isEmpty {
            extras.insert(contentsOf: zuvixPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + zuvixPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func zuvixManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".zuvix"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func zuvixExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectZuvixExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("zuvix.mjs").path,
            root.appendingPathComponent("bin/zuvix.js").path,
        ]
    ...
    static func hasAnyZuvixInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.zuvixExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let zuvixPath = self.projectZuvixExecutable(projectRoot: root) {
            return [zuvixPath, subcommand] + extraArgs
        }
        if let zuvixPath = self.zuvixExecutable(searchPaths: searchPaths) {
            return [zuvixPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "zuvix", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            zuvix entrypoint missing (looked for dist/index.js or zuvix.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real zuvix CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/zuvix"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/zuvix.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/zuvix.mjs"
            node "$PRJ/zuvix.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/zuvix.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/zuvix.js"
            node "$PRJ/bin/zuvix.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent zuvix"
          pnpm --silent zuvix \(quotedArgs);
        else
          echo "zuvix CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ZuvixConfigFile.loadDict()
