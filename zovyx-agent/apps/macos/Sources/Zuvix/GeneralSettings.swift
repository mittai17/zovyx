...
struct GeneralSettings: View {
    ...
    private var generalPage: some View {
        VStack(alignment: .leading, spacing: 20) {
            SettingsPageHeader(
                title: "General",
                subtitle: "Everyday Zuvix app behavior.")

            self.zuvixStatusPanel

            SettingsCardGroup("App") {
                SettingsCardToggleRow(
                    title: "Launch at login",
                    subtitle: "Automatically start Zuvix after you sign in.",
                    binding: self.$state.launchAtLogin)

                SettingsCardToggleRow(
                    title: "Show Dock icon",
                    subtitle: "Keep Zuvix visible in the Dock. When off, windows still show the Dock icon while open.",
    ...
    private var zuvixStatusPanel: some View {
        HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(self.state.isPaused ? "Zuvix paused" : "Zuvix active")
                    .font(.headline)
            ...
            Toggle("Zuvix active", isOn: self.activeBinding)
                .labelsHidden()
                .toggleStyle(.switch)
        }
    ...
    private var connectionStatusSubtitle: String {
        switch self.state.connectionMode {
        case .local:
            return "Zuvix starts and monitors the Gateway on this Mac."
        case .remote:
        ...
        SettingsCardGroup("Advanced") {
            DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    self.advancedTextField(
                        "Project root",
                        placeholder: "/home/you/Projects/zuvix",
                        text: self.$state.remoteProjectRoot)
                    self.advancedTextField(
                        "CLI path",
                        placeholder: "/Applications/Zuvix.app/.../zuvix",
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for zuvix logs in /tmp/zuvix/.
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/zuvix"
        state.remoteCliPath = "/tmp/zuvix"
