...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case ZuvixBrowserCommand.proxy.rawValue:
            ...
            case ZuvixLocationCommand.get.rawValue:
            ...
            case ZuvixSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case ZuvixSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case ZuvixSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case ZuvixSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case ZuvixSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ZuvixCanvasA2UICommand.push.rawValue,
             ZuvixCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleBrowserProxyInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        guard ZuvixConfigFile.browserControlEnabled() else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ZuvixNodeError(
    ...
    private func handleScreenSnapshotInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let res: (data: Data, format: ZuvixScreenSnapshotFormat, width: Int, height: Int)
        do {
    ...
    private static func resolveA2UIHostUrl(from raw: String?) -> String? {
        ...
        return baseUrl.appendingPathComponent("__zuvix__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ZuvixSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(ZuvixSystemWhichParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> ZuvixLocationMode {
    ...
    private static func errorResponse(
        ...
        code: ZuvixNodeErrorCode,
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: ZuvixNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: ZuvixCanvasSnapshotFormat,
        maxWidth: Int?,
