...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.zuvix", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = ZuvixConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: ZuvixNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: ZuvixLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        ...
        if browserControlEnabled, connectionMode == .local {
            caps.append(ZuvixCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(ZuvixCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(ZuvixCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: ZuvixConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: ZuvixLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            ZuvixSystemCommand.which.rawValue,
            ZuvixSystemCommand.run.rawValue,
            ZuvixSystemCommand.execApprovalsGet.rawValue,
            ZuvixSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(ZuvixCapability.browser.rawValue) {
            commands.append(ZuvixBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(ZuvixCapability.camera.rawValue) {
        ...
        if capsSet.contains(ZuvixCapability.location.rawValue) {
