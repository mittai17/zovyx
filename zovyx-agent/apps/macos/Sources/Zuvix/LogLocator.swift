...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ZUVIX_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/zuvix")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("zuvix-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("zuvix-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/zuvix/ (rolling or stdout), or nil if none exist.
