...
enum ExecApprovalsStore {
    private static let logger = Logger(subsystem: "ai.zuvix", category: "exec-approvals")
    ...
    static func fileURL() -> URL {
        ZuvixPaths.stateDirURL.appendingPathComponent("exec-approvals.json")
    }

    static func socketPath() -> String {
        ZuvixPaths.stateDirURL.appendingPathComponent("exec-approvals.sock").path
    }

    private static func legacyStateDirURLs() -> [URL] {
        if let home = ZuvixEnv.path("ZUVIX_HOME") {
            var urls = [
                URL(fileURLWithPath: home, isDirectory: true)
                    .appendingPathComponent(".zuvix", isDirectory: true),
            ]
    ...
    private static func legacyFileURLIfPending() -> URL? {
        guard ZuvixEnv.path("ZUVIX_STATE_DIR") != nil else { return nil }
    ...
    private static func ensureSecureStateDirectory() {
        let url = ZuvixPaths.stateDirURL
        do {
