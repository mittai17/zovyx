...
private let webChatSwiftLogger = Logger(subsystem: "ai.zuvix", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "zuvix.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: ZuvixChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> ZuvixChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(ZuvixChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            ZuvixChatSessionsDefaults(
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return ZuvixChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ZuvixChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ZuvixGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    ZuvixSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: ZuvixProtocol.ModelChoice) -> ZuvixChatModelChoice {
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ZuvixChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ZuvixChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ZuvixChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: ZuvixChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ZuvixChatView>) -> NSViewController
    {
