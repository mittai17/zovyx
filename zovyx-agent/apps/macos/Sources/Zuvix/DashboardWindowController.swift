...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    private static func installNativeChromeScript(into userContentController: WKUserContentController) {
        let css = """
        html.zuvix-native-macos {
          --zuvix-native-titlebar-height: 50px;
        }
        @media (min-width: 700px) {
          html.zuvix-native-macos .sidebar-shell {
            padding-top: max(14px, var(--zuvix-native-titlebar-height)) !important;
          }
          html.zuvix-native-macos .sidebar-shell__header {
        ...
        (() => {
          try {
            if (document.getElementById("zuvix-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "zuvix-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("zuvix-native-macos");
        ...
        auth: DashboardWindowAuth)
    {
        ...
        (() => {
          try {
            ...
            Object.defineProperty(window, "__ZUVIX_NATIVE_CONTROL_AUTH__", {
              value: \(json),
              configurable: true,
            });
          } catch {}
