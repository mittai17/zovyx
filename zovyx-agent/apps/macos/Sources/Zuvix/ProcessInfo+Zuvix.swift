...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.zuvix.mac`) even if the shipped
    ...
    {
        if environment["ZUVIX_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "zuvix.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "zuvix.nixMode") { return true }
