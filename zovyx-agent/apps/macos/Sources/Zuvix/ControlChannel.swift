...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ZuvixProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.zuvix", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ZuvixKit.AnyCodable]()) {
                $0[$1.key] = ZuvixKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking Zuvix Local Network access.
                    Allow Zuvix in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ZuvixProtocol.AnyCodable?) -> [String: ZuvixProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ZuvixProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ZuvixKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ZuvixProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("zuvix.control.heartbeat")
    static let controlAgentEvent = Notification.Name("zuvix.control.agent")
}
