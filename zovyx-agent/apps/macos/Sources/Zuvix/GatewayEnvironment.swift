...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.zuvix", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["ZUVIX_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = ZuvixConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.zuvixExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "zuvix CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["ZUVIX_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "zuvix@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "zuvix@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "zuvix@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "zuvix@\(target)"])
            }

        statusHandler("Installing zuvix@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed zuvix@\(target)")
        } else {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("zuvix ") {
            normalized = String(normalized.dropFirst("zuvix ".count))
        }
