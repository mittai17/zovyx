...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            ZuvixConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                ZuvixConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                ZuvixConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            ZuvixConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            ZuvixConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
