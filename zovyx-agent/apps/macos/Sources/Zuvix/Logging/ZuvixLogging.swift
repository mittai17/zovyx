...
enum ZuvixLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = ZuvixOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ZuvixFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ZuvixLogging.bootstrapIfNeeded()
        let label = ZuvixLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ZuvixOSLogHandler: AppLogLevelBackedHandler {
...
struct ZuvixFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ZuvixLogging.parseLabel(self.label)
