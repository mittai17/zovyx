...
final class AppState {
    private static let logger = Logger(subsystem: "ai.zuvix", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = ZuvixConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            ZuvixConfigFile.canonicalHostForComparison(host) ==
                ZuvixConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = ZuvixConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: ZuvixConfigFile.loadDict(),
            draft: .init(
        ...
        guard ZuvixConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/zuvix"
        state.remoteCliPath = ""
        return state
    }
}
