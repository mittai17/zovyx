...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.zuvix", category: "gateway-endpoint")
    ...
    {
        let raw = env["ZUVIX_GATEWAY_PASSWORD"] ?? ""
    ...
    private static func resolveGatewayToken(
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        let raw = env["ZUVIX_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
    ...
    private let logger = Logger(subsystem: "ai.zuvix", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "zuvix.onboardingSeen")
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            ...
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: ZuvixConfigFile.loadDict())
    ...
    {
        if let envBind = env["ZUVIX_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["ZUVIX_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
