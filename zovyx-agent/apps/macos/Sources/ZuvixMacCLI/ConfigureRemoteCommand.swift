...
private let appDefaultsSuites = ["ai.zuvix.mac", "ai.zuvix.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              zuvix-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
                                          [--remote-port <port>] [--token <token>] [--password <password>]
                                          [--identity <path>] [--project-root <path>] [--cli-path <path>] [--json]
              zuvix-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote Zuvix checkout for CLI commands.
              --cli-path <path>   Remote zuvix executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
...
private func zuvixConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["ZUVIX_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".zuvix/zuvix.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String) {
    for suite in appDefaultsSuites {
        ...
        defaults.set("remote", forKey: "zuvix.connectionMode")
        setDefaultString(defaults, key: "zuvix.remoteTarget", value: target)
        defaults.set(true, forKey: "zuvix.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "zuvix.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "zuvix.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "zuvix.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "zuvix.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("Zuvix macOS Remote Config")
