...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: ZuvixChatTransport {
        func requestHistory(sessionKey: String) async throws -> ZuvixChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(ZuvixChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [ZuvixChatAttachmentPayload]) async throws -> ZuvixChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(ZuvixChatSendResponse.self, from: Data(json.utf8))
        }
