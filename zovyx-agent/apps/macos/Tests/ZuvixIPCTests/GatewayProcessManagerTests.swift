...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["ZUVIX_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "zuvix-gateway",
                executablePath: "/tmp/zuvix-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 zuvix-gateway @ /tmp/zuvix-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
