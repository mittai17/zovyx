...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["zuvix"]
        let text = "uh zuvix"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "zuvix")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["zuvix", "hey zuvix"]
        let text = "hey zuvix"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey zuvix")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey zuvix do thing"
        let config = WakeWordGateConfig(triggers: ["zuvix"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["zuvix"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "zuvix")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "zuvix tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["zuvix", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["zuvix", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "zuvix")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["zuvix"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "zuvix", start: 0.2, duration: 0.1, range: transcript.range(of: "zuvix")),
