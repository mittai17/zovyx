...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: ZuvixProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ZuvixProtocol.AnyCodable] = [
            "presence": .init([ZuvixProtocol.AnyCodable(entry)]),
        ]
        let payload = ZuvixProtocol.AnyCodable(payloadMap)
