...
import Foundation
import Testing
@testable import ZuvixDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("ZUVIX_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("ZUVIX_WIDE_AREA_DOMAIN", "zuvix.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("ZUVIX_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("ZUVIX_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._zuvix-gw._tcp.zuvix.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.zuvix.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Zuvix)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/zuvix/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Zuvix)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/zuvix/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._zuvix-gw._tcp.zuvix.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
