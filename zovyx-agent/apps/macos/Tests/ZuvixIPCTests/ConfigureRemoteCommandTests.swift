...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("zuvix-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["ai.zuvix.mac", "ai.zuvix.mac.debug"]
        let keys = [
            "zuvix.connectionMode",
            "zuvix.remoteTarget",
            "zuvix.onboardingSeen",
            "zuvix.onboardingVersion",
            "zuvix.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["ZUVIX_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "zuvix.connectionMode") == "remote")
                #expect(defaults.string(forKey: "zuvix.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "zuvix.onboardingSeen") == true)
                #expect(defaults.string(forKey: "zuvix.remoteCliPath") == "/opt/homebrew/bin/zuvix")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("zuvix-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("zuvix-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("zuvix-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["ZUVIX_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
