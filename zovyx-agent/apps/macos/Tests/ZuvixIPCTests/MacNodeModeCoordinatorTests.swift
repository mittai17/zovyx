...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(ZuvixCapability.browser.rawValue))
        #expect(!commands.contains(ZuvixBrowserCommand.proxy.rawValue))
        #expect(commands.contains(ZuvixCanvasCommand.present.rawValue))
