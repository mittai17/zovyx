...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> ZuvixChatHistoryPayload
{
    ZuvixChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> ZuvixChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> ZuvixChatThinkingLevelOption {
    ZuvixChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> ZuvixChatSessionEntry
{
    ZuvixChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> ZuvixChatModelChoice {
    ZuvixChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [ZuvixChatHistoryPayload],
    sessionsResponses: [ZuvixChatSessionsListResponse] = [],
    modelResponses: [[ZuvixChatModelChoice]] = [],
    ...
    -> (TestChatTransport, ZuvixChatViewModel)
{
...
private func sendUserMessage(_ vm: ZuvixChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, ZuvixChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ZuvixChatHistoryPayload]
    private let sessionsResponses: [ZuvixChatSessionsListResponse]
    private let modelResponses: [[ZuvixChatModelChoice]]
    ...
    private let stream: AsyncStream<ZuvixChatTransportEvent>
    private let continuation: AsyncStream<ZuvixChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<ZuvixChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> ZuvixChatCreateSessionResponse
    {
        ...
        return ZuvixChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ZuvixChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ZuvixChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [ZuvixChatAttachmentPayload]) async throws -> ZuvixChatSendResponse
    {
        ...
        return ZuvixChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ZuvixChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ZuvixChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ZuvixChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> ZuvixChatMessage {
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(ZuvixChatTransportEvent.seqGap)
    ...
    @Test func `appends external session user message for active session`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                ZuvixSessionMessageEventPayload(
                    sessionKey: "agent:aiden:main",
                    message: ZuvixChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let sessions = ZuvixChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = ZuvixChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: ZuvixChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(ZuvixChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == ZuvixChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = ZuvixChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: ZuvixChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = ZuvixChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = ZuvixChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = ZuvixChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            ZuvixChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { ZuvixChatViewModel(sessionKey: "main", transport: transport) }
