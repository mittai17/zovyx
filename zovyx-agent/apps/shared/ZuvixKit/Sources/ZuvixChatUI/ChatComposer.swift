...
public struct ZuvixChatTalkControl {
...
@MainActor
struct ZuvixChatComposer: View {
    @Bindable var viewModel: ZuvixChatViewModel
    let style: ZuvixChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: ZuvixChatView.ComposerChrome
    let isComposerEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: ZuvixChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(ZuvixChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(ZuvixChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(ZuvixChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ZuvixPendingAttachment.id)
                { (att: ZuvixPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ZuvixPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var fullEditor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(ZuvixChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(ZuvixChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: ZuvixChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: ZuvixChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: ZuvixChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(ZuvixChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return ZuvixChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: ZuvixChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return ZuvixChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: ZuvixChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: ZuvixChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(ZuvixChatTheme.subtleCard)
            }
        }
    }
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? ZuvixChatTheme.userBubble
    }
