...
private let chatUILogger = Logger(subsystem: "ai.zuvix", category: "ZuvixChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class ZuvixChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [ZuvixChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [ZuvixChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [ZuvixChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [ZuvixPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ZuvixChatPendingToolCall] = []
    public private(set) var sessions: [ZuvixChatSessionEntry] = []
    private let transport: any ZuvixChatTransport
    private var sessionDefaults: ZuvixChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: ZuvixChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any ZuvixChatTransport,
    ...
    public var sessionChoices: [ZuvixChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [ZuvixChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [ZuvixChatThinkingLevelOption] = [
        ZuvixChatThinkingLevelOption(id: "off", label: "off"),
        ZuvixChatThinkingLevelOption(id: "minimal", label: "minimal"),
        ZuvixChatThinkingLevelOption(id: "low", label: "low"),
    ...
    public func removeAttachment(_ id: ZuvixPendingAttachment.ID) {
    ...
    private func applyHistoryPayload(
        _ payload: ZuvixChatHistoryPayload,
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ZuvixChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ZuvixChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: ZuvixChatMessage) -> ZuvixChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> ZuvixChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return ZuvixChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: ZuvixChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: ZuvixChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: ZuvixChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func adoptPendingLocalUserEcho(incoming: ZuvixChatMessage) -> Bool {
        ...
        updated[matchIndex] = ZuvixChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func reconcileMessageIDs(
        previous: [ZuvixChatMessage],
        incoming: [ZuvixChatMessage]) -> [ZuvixChatMessage]
    {
    ...
    private static func reconcileRunRefreshMessages(
        previous: [ZuvixChatMessage],
        incoming: [ZuvixChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [ZuvixChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [ZuvixChatMessage]) -> [ZuvixChatMessage] {
        var result: [ZuvixChatMessage] = []
    ...
    private static func dedupeKey(for message: ZuvixChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ZuvixChatMessageContent] = [
            ZuvixChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> ZuvixChatAttachmentPayload in
            ZuvixChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "ZuvixChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: ZuvixChatSessionEntry?) -> [ZuvixChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: ZuvixChatSessionEntry,
        defaults: ZuvixChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [ZuvixChatThinkingLevelOption]?) -> [ZuvixChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return ZuvixChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [ZuvixChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return ZuvixChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [ZuvixChatThinkingLevelOption],
        current: String) -> [ZuvixChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [ZuvixChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [ZuvixChatThinkingLevelOption]) -> [ZuvixChatThinkingLevelOption]
    {
        var result: [ZuvixChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> ZuvixChatSessionEntry {
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = ZuvixChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: ZuvixChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: ZuvixSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: ZuvixChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: ZuvixChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = ZuvixChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: ZuvixChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: ZuvixChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: ZuvixChatMessage) -> ZuvixChatMessage {
    ...
    private func handleAgentEvent(_ evt: ZuvixAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ZuvixChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: ZuvixAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: ZuvixAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    private static func hasUnansweredLatestUser(in messages: [ZuvixChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    private static func latestUserTurn(in messages: [ZuvixChatMessage]) -> LatestUserTurn? {
    ...
    private static func hasAnsweredUser(
        ...
        in messages: [ZuvixChatMessage])
        -> Bool
    {
        guard let refreshKey = user.refreshKey else { return false }
        var occurrence = 0
        var latestMatchingUserIndex: [ZuvixChatMessage].Index?
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [ZuvixChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
