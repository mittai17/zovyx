...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ZuvixChatTheme.userText : ZuvixChatTheme.assistantText
    ...
    private var inlineAttachments: [ZuvixChatMessageContent] {
    ...
    private var toolCalls: [ZuvixChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ZuvixChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ZuvixChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ZuvixChatTheme.onboardingAssistantBubble
        }
        return ZuvixChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ZuvixChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ZuvixChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ZuvixChatTheme.userText : ZuvixChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ZuvixChatMessageContent
    let isUser: Bool
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ZuvixChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ZuvixChatTheme.assistantText)
            }
        }
    }
}
