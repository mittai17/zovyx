...
@MainActor
public struct ZuvixChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @State private var viewModel: ZuvixChatViewModel
    ...
    private let talkControl: ZuvixChatTalkControl?
    ...
    public init(
        ...
        talkControl: ZuvixChatTalkControl? = nil)
    {
    ...
    private var visibleMessages: [ZuvixChatMessage] {
        let base: [ZuvixChatMessage]
    ...
    private func mergeToolResults(in messages: [ZuvixChatMessage]) -> [ZuvixChatMessage] {
    ...
    private func isToolResultMessage(_ message: ZuvixChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: ZuvixChatMessage) -> Bool {
    ...
    private func primaryText(in message: ZuvixChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return ZuvixChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: ZuvixChatMessage) -> Bool {
    ...
    private func toolCalls(in message: ZuvixChatMessage) -> [ZuvixChatMessageContent] {
    ...
    private func inlineToolResults(in message: ZuvixChatMessage) -> [ZuvixChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: ZuvixChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: ZuvixChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(ZuvixChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(ZuvixChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
