...
val androidStoreFile = providers.gradleProperty("ZUVIX_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("ZUVIX_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("ZUVIX_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("ZUVIX_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set ZUVIX_ANDROID_STORE_FILE, " +
      "ZUVIX_ANDROID_STORE_PASSWORD, ZUVIX_ANDROID_KEY_ALIAS, and " +
      "ZUVIX_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/ZuvixKit/Sources/ZuvixKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.zuvix.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "zuvix-$versionName-$buildType.apk"
          } else {
            "zuvix-$versionName-$flavorName-$buildType.apk"
          }
