...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    // Command strings come from ZuvixProtocolConstants; the registry above owns advertised availability.
    return when (command) {
      // Canvas commands
      ...
      ZuvixCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      ZuvixCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      ZuvixCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      ZuvixCanvasA2UICommand.Push.rawValue, ZuvixCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      ZuvixCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      ZuvixCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      ZuvixCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      ZuvixLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      ZuvixDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      ZuvixDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      ZuvixDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      ZuvixDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)
      ZuvixDeviceCommand.Apps.rawValue -> deviceHandler.handleDeviceApps(paramsJson)

      // Notifications command
      ZuvixNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      ZuvixNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      ZuvixSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      ZuvixTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      ZuvixTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      ZuvixTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      ZuvixTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      ai.zuvix.app.protocol.ZuvixPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      ZuvixContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      ZuvixContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      ZuvixCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      ZuvixCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      ZuvixMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      ZuvixMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      ZuvixSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      ZuvixSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      ZuvixCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
