...
import ai.zuvix.app.GatewayConnectionProblem
import ai.zuvix.app.LocationMode
import ai.zuvix.app.MainViewModel
import ai.zuvix.app.R
import ai.zuvix.app.SensitiveFeatureConfig
import ai.zuvix.app.node.DeviceNotificationListenerService
import ai.zuvix.app.ui.design.ClawDesignTheme
import ai.zuvix.app.ui.design.ClawErrorState
import ai.zuvix.app.ui.design.ClawListItem
...
fun OnboardingFlow(
  ...
  ClawDesignTheme(dark = onboardingDark) {
    ...
    ZuvixSystemBarAppearance(lightAppearance = !onboardingDark && step != OnboardingStep.Welcome)
...
private fun WelcomeLogo() {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxSize().padding(12.dp), contentAlignment = Alignment.Center) {
      Image(painter = painterResource(id = R.drawable.zuvix_logo), contentDescription = "Zuvix logo", modifier = Modifier.fillMaxSize())
    }
  }
}
      ...
      LazyColumn(
        ...
        item {
          Column(modifier = Modifier.padding(top = 10.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Text(
              text = "These permissions keep Zuvix secure\nand useful.",
              style = ClawTheme.type.body,
              color = ClawTheme.colors.textMuted,
            )
          }
...
private fun PermissionTopBar(onBack: () -> Unit) {
  ...
  if (showHelp) {
    AlertDialog(
      ...
      text = {
        Text(
          "Choose what this phone can share with Zuvix. You can change these later in Settings.",
          style = ClawTheme.type.body,
          color = ClawTheme.colors.textMuted,
        )
      },
...
internal enum class GatewayRecoveryUiState(
  ...
  Pairing(
    title = "Pairing Gateway",
    message = "Approval is in progress.\nZuvix will reconnect automatically.",
  ),
  Finishing(
    title = "Connecting Gateway",
    message = "Zuvix is checking gateway and node access.",
  ),
  Failed(
  ...
  remoteAddress
    ...
    ?: if (ready) {
      ...
      recoveryGatewayApprovalCommand(gatewayConnectionProblem)
        ...
        ?: "Gateway approval is pending. Run zuvix devices list on the gateway host, approve this phone, then retry."
      ...
      "Gateway approval is in progress. Zuvix will retry automatically."
    } else {
      "Gateway unreachable"
    }
...
private fun copyApprovalCommand(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Zuvix pairing approval command", command))
...
private fun copyGatewayDiagnostic(
  ...
  val diagnostic =
    listOfNotNull(
      "Zuvix Android gateway diagnostic",
      "Status: $statusText",
  ...
  val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
  clipboard.setPrimaryClip(ClipData.newPlainText("Zuvix gateway diagnostic", diagnostic))
