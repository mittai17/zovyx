...
private const val NOTIFICATION_CHANNEL_BASE_ID = "zuvix.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Zuvix Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Zuvix Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Zuvix Active")
      }
