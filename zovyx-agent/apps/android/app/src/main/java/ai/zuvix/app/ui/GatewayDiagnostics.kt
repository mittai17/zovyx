...
internal fun zuvixAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Zuvix Android gateway connection failure.
    ...
    - tell me whether `zuvix devices list` should show a pending pairing request
    - if more signal is needed, ask for `zuvix qr --json`, `zuvix devices list`, and `zuvix nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${zuvixAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Zuvix gateway diagnostics", report))
