...
PACKAGE_NAME="ai.zuvix.app"
RECEIVER="$PACKAGE_NAME/.VoiceE2eReceiver"
RUN_ACTION="ai.zuvix.app.debug.RUN_VOICE_E2E"
OPEN_ACTION="ai.zuvix.app.debug.OPEN_VOICE_E2E"
PORT=18789
HOST="127.0.0.1"
MODE="both"
...
ARTIFACT_DIR="/tmp/zuvix-android-voice-e2e-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$START_GATEWAY" -eq 1 ]]; then
  if command -v bws_get_secret >/dev/null 2>&1; then
    ZUVIX_OPENAI_API_KEY="$(bws_get_secret ZUVIX_OPENAI_API_KEY)"
  else
    ZUVIX_OPENAI_API_KEY="$(zsh -ic 'bws_get_secret ZUVIX_OPENAI_API_KEY')"
  fi
  (
    cd "$ROOT_DIR"
    OPENAI_API_KEY="$ZUVIX_OPENAI_API_KEY" \
      pnpm zuvix gateway run \
        --port "$PORT" \
        --auth none \
        --bind loopback \
        --force \
...
adb logcat -d -v time |
  rg -i 'Zuvix|TalkMode|MicCapture|AudioRecord|SpeechRecognizer|realtime|talk.session|appendAudio|transcript|Talk failed|Transcription failed|Speech network|VoiceE2E' |
